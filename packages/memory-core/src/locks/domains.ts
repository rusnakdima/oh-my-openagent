...
export function runFinalizationLockPath(
  ...
  const slug =
    trimmed.replace(/[^a-zA-Z0-9._-]+/g, "-").replace(/^[.-]+|[.-]+$/g, "")
      .slice(0, 48) || "run";
  const digest = createHash("sha256").update(trimmed).digest("hex").slice(
    0,
    16,
  );
...
export function transcriptStateLockPath(
  locksDirectory: string,
  transcriptId: string,
): string {
  if (transcriptId.length === 0) {
    throw new Error("transcript id must not be empty");
  }
  const digest = createHash("sha256").update(transcriptId).digest("hex").slice(
    0,
    16,
  );
