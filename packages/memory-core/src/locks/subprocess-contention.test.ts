...
const workerPath = fileURLToPath(
  new URL("./subprocess-worker.ts", import.meta.url),
);
  ...
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(
      () =>
        reject(
          new Error(
            `timed out waiting for ${expected}; stderr=${
              child.stderr.read() ?? ""
            }`,
          ),
        ),
    ...
    child.once("exit", (code, signal) => {
      clearTimeout(timeout);
      reject(
        new Error(`child exited before output: code=${code} signal=${signal}`),
      );
    });
  });
}
...
async function createFixture(): Promise<
  { directory: string; lockPath: string; counterPath: string; logPath: string }
> {
  ...
  return {
    directory,
    lockPath: path.join(directory, "writer.lock"),
    counterPath,
    logPath,
  };
}
...
describe("real subprocess lock contention", () => {
  test(
    "#given two Bun subprocess writers #when they contend on one counter #then critical-section log entries never interleave",
    async () => {
      // #given
  ...
  test(
    "#given a SIGKILLed Bun lock holder #when another process acquires #then proven-dead ownership is recovered",
    async () => {
      // #given
      const fixture = await createFixture();
      ...
      await acquireLock(fixture.lockPath, successor, {
        waitTimeoutMs: 5_000,
        retryDelayMs: 10,
      });
  ...
  test(
    "#given a live Bun lock holder #when a contender tries immediately #then the live owner is never stolen",
    async () => {
      // #given
      const fixture = await createFixture();
      const holder = spawnWorker([
      ...
      // #when
      const error = await acquireLock(fixture.lockPath, contender).then(
        () => null,
        (cause: unknown) => cause,
      );
      ...
      expect(JSON.parse(await readFile(fixture.lockPath, "utf8")))
        .toMatchObject({ run_id: "live" });
      const exitPromise = waitForExit(holder);
      holder.kill("SIGKILL");
      await exitPromise;
    },
