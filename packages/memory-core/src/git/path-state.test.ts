...
import { createNodeGitExec, GitMemoryRepo, GitPathStateError } from "./index";

const AUTHOR = { agentId: "path-state-agent", authorName: "Path State Agent" };
const tempDirs: string[] = [];

async function fixture(
  seedFiles: Array<{ relativePath: string; content: string }> = [],
) {
...
describe("GitPathStateStore", () => {
  it("captures stage-zero index and raw worktree identities independently", async () => {
    ...
    expect(clean.index).toEqual({
      mode: "100644",
      oid: clean.worktree.kind === "file" ? clean.worktree.oid : "",
    });
  ...
  it.skipIf(process.platform === "win32")(
    "captures and restores executable modes",
    async () => {
      const { dir, repo } = await fixture([{
      ...
      expect(state.worktree.kind === "file" ? state.worktree.mode : 0).toBe(
        0o755,
      );
  ...
  it("hashes and writes blobs both raw and through path filters", async () => {
    ...
    expect(
      (await createNodeGitExec().run(["cat-file", "-e", rawUnwritten], {
        cwd: dir,
        timeoutMs: 30_000,
      })).code,
    ).not.toBe(0);
    ...
    const filtered = await repo.pathState.hashIndexBlob(
      "filtered.txt",
      content,
      true,
    );
    ...
    expect(captured.worktree.kind === "file" ? captured.worktree.oid : "").toBe(
      raw,
    );
    expect(await repo.pathState.readBlob(raw)).toBe(content);
    expect(await repo.pathState.readBlob(filtered)).toBe(
      "unique-filtered-content\n",
    );
  });
  ...
  it("sets and removes index entries without changing the worktree", async () => {
    ...
    const replacement = await repo.pathState.hashIndexBlob(
      "entry.md",
      "index-only\n",
      true,
    );

    await repo.pathState.setIndex("entry.md", {
      mode: "100644",
      oid: replacement,
    });
  ...
  it("atomically materializes and removes worktree files idempotently", async () => {
    ...
    await repo.pathState.writeWorktree("nested/a b.md", identity);
    expect(await readFile(join(dir, "nested/a b.md"), "utf8")).toBe(
      "materialized\n",
    );
    ...
    if (process.platform !== "win32") {
      expect((await lstat(join(dir, "nested/a b.md"))).mode & 0o777).toBe(
        0o640,
      );
    }
    ...
    await repo.pathState.removeWorktree("nested/a b.md");
    expect((await repo.pathState.capture("nested/a b.md")).worktree).toEqual({
      kind: "missing",
    });
  });
  ...
  it("rejects traversal, worktree symlinks, and index symlinks", async () => {
    ...
    expect(await rejectedError(repo.pathState.capture("../outside.md")))
      .toBeInstanceOf(GitPathStateError);
    expect(await rejectedError(repo.pathState.capture("C:\\outside.md")))
      .toBeInstanceOf(GitPathStateError);
    expect(await rejectedError(repo.pathState.capture("nested/.git/config")))
      .toBeInstanceOf(GitPathStateError);
    expect(await rejectedError(repo.pathState.capture("link.md")))
      .toBeInstanceOf(GitPathStateError);
  ...
  it("rejects unmerged entries", async () => {
    ...
    const merge = await createNodeGitExec().run(["merge", "other"], {
      cwd: dir,
      timeoutMs: 30_000,
    });
