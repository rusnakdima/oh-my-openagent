...
export async function moveWorktreeFile(
  root: string,
  path: string,
...
export async function restoreMovedWorktreeFile(
  root: string,
  path: string,
  moved: string,
): Promise<boolean> {
...
export async function reserveMovedWorktreeDeletion(
  ...
  return {
    verify: async () => {
      ...
      if (
        !current.isDirectory() || current.dev !== reserved.dev ||
      ...
      return {
        finish: async () => {
          ...
          if (
            !owned.isDirectory() || owned.dev !== reserved.dev ||
            owned.ino !== reserved.ino || ownedMarker !== marker
          ) {
...
export async function removeWorktreeFile(
  root: string,
  path: string,
): Promise<void> {
  const target = join(root, path);
...
export function unsupportedWorktree(
  path: string,
  kind: string,
): GitPathStateError {
