...
export type GitWorktreeIdentity = GitWorktreeFileIdentity | {
...
export class GitPathStateStore {
  ...
  ): Promise<ReadonlyMap<string, GitPathState>> {
    const normalized = [...new Set(paths.map(normalizeGitPath))];
    const states = await Promise.all(
      normalized.map(async (path) => [path, await this.capture(path)] as const),
    );
    return new Map(states);
  }
  async hashWorktreeBlob(
  ...
  async reserveWorktreeDeletion(
    ...
    return reserveMovedWorktreeDeletion(
      this.dir,
      path,
      moved,
      (claimed) => this.restoreWorktreeClaim(path, claimed),
    );
  }
  async restoreWorktreeClaim(
    path: string,
    claimed: string,
  ): Promise<true | string> {
  ...
  private async captureIndex(path: string): Promise<GitIndexIdentity | null> {
    ...
    if (!INDEX_MODES.has(record.mode as GitIndexMode)) {
      throw new GitPathStateError(
        `Unsupported Git index mode ${record.mode} for path: ${path}`,
      );
    }
  ...
  private async captureMovedWorktree(
    path: string,
  ): Promise<GitWorktreeFileIdentity> {
    ...
    return {
      ...
      oid: await this.hashWorktreeBlob(await readFile(path), true),
    };
  }
  private async hashBlob(
...
function assertWorktreeIdentity(identity: GitWorktreeFileIdentity): void {
  if (
    identity.kind !== "file" || !Number.isInteger(identity.mode) ||
