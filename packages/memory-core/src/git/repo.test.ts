...
describe("GitMemoryRepo", () => {
  it("#given a committed HEAD with a fixed committer date #when timestamp is read #then epoch seconds are returned", async () => {
    ...
    await repo.init({
      seedFiles: [{ relativePath: "system/persona.md", content: "initial\n" }],
    });
  ...
  it("#given no effective path changes #when commitWrite runs #then it throws a typed no-change error", async () => {
    ...
    await repo.init({
      seedFiles: [{ relativePath: "memory.md", content: "same\n" }],
    });
    ...
    // then
    expect(await rejectedError(operation)).toBeInstanceOf(
      NoEffectiveChangesError,
    );
  });
  ...
  it("#given a UTF-16 markdown file is dirty #when cleanCheck runs #then its encoding problem is readable", async () => {
    ...
    // then
    expect((await rejectedError(operation)).message).toContain(
      "utf16.md has UTF-16 LE BOM",
    );
  });
  ...
  it("#given local identity overrides #when init reconciles config #then email is preserved and agent identity is updated", async () => {
    ...
    await exec.run(
      ["config", "--local", "user.email", "operator@example.com"],
      { cwd: dir, timeoutMs: 30_000 },
    );
    await exec.run(["config", "--local", "user.name", "Operator"], {
      cwd: dir,
      timeoutMs: 30_000,
    });
    await exec.run(["config", "--local", "omo.agentId", "old-agent"], {
      cwd: dir,
      timeoutMs: 30_000,
    });
  ...
  it("#given commit signing is explicitly enabled locally #when init reconciles config #then the override is preserved", async () => {
    ...
    await exec.run(["config", "--local", "commit.gpgsign", "true"], {
      cwd: dir,
      timeoutMs: 30_000,
    });
  ...
  it(
    "#given conflicting local identity #when a tool write commits #then explicit author flags control attribution",
    async () => {
      // given
      const { dir, repo } = await createRepo();
      await repo.init();
      ...
      // then
      const exec = createNodeGitExec();
      const log = await exec.run(["log", "-1", "--format=%an <%ae>"], {
        cwd: dir,
        timeoutMs: 30_000,
      });
  ...
  it(
    "#given commits with bodies and changed paths #when log reads history #then metadata trailers ranges filters and optional paths are parsed newest-first",
    async () => {
      // given
      const { dir, repo } = await createRepo();
      const initial = await repo.init({
      ...
      const filtered = await repo.log({
        paths: ["system/persona.md"],
        limit: 1,
      });

      // then
      expect(history.map((commit) => commit.sha)).toEqual([
        second.sha,
        first.sha,
      ]);
  ...
  it(
    "#given a reflection worktree commit #when it is merged no-ff #then the parent exposes the content and removes the worktree",
    async () => {
      // given
      const { repo } = await createRepo();
      await repo.init();
      const worktreeParent = realpathSync.native(
        await mkdtemp(join(tmpdir(), "memory-worktree-")),
      );
      ...
      const childRepo = new GitMemoryRepo({
        dir: worktreeDir,
        agentId: "agent-one",
      });
  ...
  it(
    "#given concurrent commits across worktrees #when index and ref locks contend #then every commit lands",
    async () => {
      ...
      const parent = realpathSync.native(
        await mkdtemp(join(tmpdir(), "memory-worktrees-")),
      );
      ...
      const results = await Promise.allSettled(
        checkouts.map(async (checkout, index) => {
          ...
          const child = new GitMemoryRepo({
            dir: checkout,
            agentId: "agent-one",
          });
          return child.commitWrite(
            ["learned.md"],
  ...
  it(
    "#given a transient ref lock on the first attempt #when a repo is initialized #then the retry lets HEAD settle",
    async () => {
      // given - git loses the HEAD.lock race once, exactly as it does under Windows contention
      const dir = realpathSync.native(
        await mkdtemp(join(tmpdir(), "memory-git-lock-")),
      );
      tempDirs.push(dir);
      ...
      class FlakyLockExec implements GitExec {
        async run(
          ...
          const shouldFail = (verb === "symbolic-ref" || verb === "commit") &&
            !failedOnce.has(verb);
          if (shouldFail) {
            failedOnce.add(verb);
            throw new Error(
      ...
      const repo = new GitMemoryRepo({
        dir,
        agentId: "agent-one",
        exec: new FlakyLockExec(),
      });

      // when
      const head = await repo.init({
        seedFiles: [{ relativePath: "system/persona.md", content: "seed\n" }],
      });
