...
async function createJournal(): Promise<
  { dir: string; journal: TranscriptJournal }
> {
  const dir = realpathSync.native(
    await mkdtemp(join(tmpdir(), "memory-journal-")),
  );
  tempDirs.push(dir);
  return {
    dir,
...
describe("cancellable journal flush", () => {
  it("#given a journal with rows #when flush runs #then it settles and leaves no lock behind", async () => {
    ...
    expect(
      (await readFile(join(dir, "transcript.jsonl"), "utf8")).trim().split(
        "\n",
      ),
    ).toHaveLength(1);
  });
  ...
  it("#given a signal aborted before acquisition #when flush runs #then it throws AbortError and creates no lock file", async () => {
    ...
    // when
    const failure = await journal.flush(controller.signal).catch((
      error: unknown,
    ) => error);
  ...
  it("#given an abort during the acquisition retry loop #when the lock is contended #then it throws without acquiring", async () => {
    // given
    const dir = realpathSync.native(
      await mkdtemp(join(tmpdir(), "memory-journal-lock-")),
    );
    tempDirs.push(dir);
  ...
  describe("stale journal lock recovery", () => {
    ...
    it(
      "#given a lock file left by a dead process #when the lock is acquired #then the stale lock is reclaimed and the task runs",
      async () => {
        // given
        const dir = realpathSync.native(
          await mkdtemp(join(tmpdir(), "memory-journal-lock-")),
    ...
    it(
      "#given a payload-free lock file abandoned mid-acquisition #when the lock is acquired #then the artifact is reclaimed by age",
      async () => {
        // given
        const dir = realpathSync.native(
          await mkdtemp(join(tmpdir(), "memory-journal-lock-")),
    ...
    it(
      "#given a lock file owned by a live process #when the wait deadline passes #then it throws and the live lock is never stolen",
      async () => {
        // given
        const dir = realpathSync.native(
          await mkdtemp(join(tmpdir(), "memory-journal-lock-")),
  ...
  it("#given a signal aborted mid-flush #when the remaining fsyncs are reached #then flush returns early without throwing", async () => {
    ...
    const aborting = new TranscriptJournal({
      journalDir: dir,
      lock: async (lockPath, task, signal) =>
