...
export { type JournalLock, JournalLockTimeoutError, withLocalJournalLock };
...
function isTranscriptEntry(value: unknown): value is TranscriptEntry {
  ...
  return (
    (row.kind === "user" || row.kind === "assistant" ||
      row.kind === "reasoning" || row.kind === "error") &&
...
function parseState(raw: string | null): ReflectionTranscriptState {
  ...
  try {
    ...
    return {
      ...
      last_reflection_started_at: optionalString(
        state.last_reflection_started_at,
      ),
      last_reflection_succeeded_at: optionalString(
        state.last_reflection_succeeded_at,
      ),
      pending_compaction: typeof state.pending_compaction === "boolean"
...
export class TranscriptJournal {
  ...
  async reconcile(
    messages: readonly TranscriptProjection[],
  ): Promise<AppendResult> {
    return this.locked(async () => {
      const capturedAt = this.now().toISOString();
      return this.appendUnlocked(
        messages.flatMap((message) =>
          projectTranscriptEntries(message, capturedAt)
        ),
      );
    });
  }
    ...
    return raw
      ...
      .map((line) => {
        ...
        if (!isTranscriptEntry(parsed)) {
          throw new Error("Invalid transcript journal row");
        }
        return parsed;
      });
  }
