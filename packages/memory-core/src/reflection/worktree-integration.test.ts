...
async function fixture(runId = "run-123") {
  const root = realpathSync.native(
    await mkdtemp(join(tmpdir(), "reflection-integration-")),
  );
  roots.push(root);
  const parentDir = join(root, "memory");
  const repo = new GitMemoryRepo({ dir: parentDir, agentId: "agent-one" });
  await repo.init({
    seedFiles: [{ relativePath: "memory.md", content: "base\n" }],
  });
...
async function commit(
  ...
  return child.commitWrite([path], "reflect", {
    agentId: "agent-one",
...
describe("reflection worktree integration", () => {
  it("#given a validated tip #when auto integration lands #then it creates a no-ff receipt without cleaning resources", async () => {
    ...
    if (validation.status !== "valid") {
      throw new Error("expected valid reflection");
    }
    ...
    expect(result.integrationSha).toBe(
      (await git(parentDir, ["rev-parse", "HEAD"])).stdout.trim(),
    );
    expect(
      (await git(parentDir, ["rev-list", "--parents", "-n", "1", "HEAD"]))
        .stdout.trim().split(" "),
    ).toHaveLength(3);
    expect((await git(parentDir, ["show", "-s", "--format=%B", "HEAD"])).stdout)
      .toContain("Omo-Run: run-123");
  ...
  it("#given legacy auto receipts #when probed without a validated tip #then only the exact trailer is accepted", async () => {
    ...
    expect(
      (await git(parentDir, [
        ...
        "non-exact",
        "-m",
        "Omo-Run: legacy-extra",
      ])).code,
    ).toBe(0);
    await expect(probeLegacyAutoRunReceipt(worktree, "legacy")).resolves
      .toEqual({ status: "not_integrated" });
    ...
    const integrationSha = (await git(parentDir, ["rev-parse", "HEAD"])).stdout
      .trim();
    await expect(probeLegacyAutoRunReceipt(worktree, "legacy")).resolves
      .toEqual({
        status: "integrated",
  ...
  it("#given a landed auto merge #when probed #then exact receipt and validated-tip ancestry prove integration", async () => {
    ...
    if (merged.outcome !== "merged" || merged.integrationSha === undefined) {
      throw new Error("expected merge receipt");
    }
    await expect(probeReflectionIntegration(worktree, {
      ...
      status: "integrated",
      evidence: "receipt",
      integrationSha: merged.integrationSha,
    });
  });
  ...
  it("#given external integration without a receipt #when either mode probes #then ancestry proves integration", async () => {
    ...
    const integrationSha = await repo.merge(validation.tipSha, {
      noFF: true,
      message: "manual integration",
    });
  ...
  it("#given an owned interrupted merge #when auto integration retries #then it aborts that merge and lands one clean merge", async () => {
    ...
    expect(
      (await git(parentDir, ["rev-parse", "-q", "--verify", "MERGE_HEAD"]))
        .stdout.trim(),
    ).toBe(validation.tipSha);
  ...
  it(
    "#given foreign merge residue #when auto integration runs #then parent index and merge state remain untouched",
    async () => {
      ...
      const foreign = await createReflectionWorktree(
        repo,
        "foreign",
        join(root, "worktrees"),
      );
      await commit(foreign, "foreign.md", "foreign\n");
      const foreignTip = (await git(foreign.dir, ["rev-parse", "HEAD"])).stdout
        .trim();
      expect(
        (await git(parentDir, ["merge", "--no-ff", "--no-commit", foreignTip]))
          .code,
      ).toBe(0);
      const beforeStatus =
        (await git(parentDir, ["status", "--porcelain=v2"])).stdout;
      ...
      expect((await git(parentDir, ["status", "--porcelain=v2"])).stdout).toBe(
        beforeStatus,
      );
      expect((await git(parentDir, ["write-tree"])).stdout.trim()).toBe(
        beforeTree,
      );
    },
  ...
  it("#given present then absent resources #when cleanup repeats #then both receipts confirm absence", async () => {
    ...
    expect(await cleanupReflectionWorktree(worktree)).toEqual({
