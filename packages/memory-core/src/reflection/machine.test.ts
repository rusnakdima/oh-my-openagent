...
describe("reflection trigger machine", () => {
  it("#given a successful settle at the configured step threshold #when evaluated #then it requests a step-count reservation", () => {
    const result = evaluateTransitions(machine(5), {
      kind: "settled",
      success: true,
    });
  ...
  it("#given zero or undefined thresholds #when a successful run settles #then step count never launches", () => {
    expect(
      evaluateTransitions(machine(99, 0), { kind: "settled", success: true })
        .action,
    ).toEqual({ kind: "none" });
    const noThreshold = { ...machine(99), config: { onCompaction: true } };
    expect(
      evaluateTransitions(noThreshold, { kind: "settled", success: true })
        .action,
    ).toEqual({ kind: "none" });
  });
  ...
  it("#given an aborted or failed run #when it settles #then no automatic trigger launches", () => {
    expect(
      evaluateTransitions(machine(99), { kind: "settled", success: false })
        .action,
    ).toEqual({ kind: "none" });
  });
  ...
  it("#given compaction was accepted #when several successful boundaries settle #then its reservation is emitted exactly once", () => {
    const accepted =
      evaluateTransitions(machine(0), { kind: "compaction_accepted" }).state;
    const first = evaluateTransitions(accepted, {
      kind: "settled",
      success: true,
    });
    ...
    const reserved = reserveTransition(
      first.state.reservation,
      request("compaction"),
      "run-active",
    );
    const second = evaluateTransitions({
      ...first.state,
      reservation: reserved.state,
    }, { kind: "settled", success: true });
  ...
  it("#given an active threshold run #when compaction and manual requests arrive #then one pending slot unions conversations and preserves manual priority", () => {
    ...
    const compacted = reserveTransition(
      active.state,
      request("compaction", ["conversation-b"]),
      "pending-1",
    );
  ...
  it("#given two dream requests during an active reflection #when they merge #then one pending dream unions snapshots and the newest equal-priority request wins", () => {
    ...
    const idle = reserveTransition(
      active.state,
      request("dream", ["conversation-a"], "idle"),
      "dream-idle",
    );
    ...
    expect(
      newerIdle.state.pending?.request.snapshots.map((item) =>
        item.conversationId
      ),
    ).toEqual([
      "conversation-a",
    ...
    for (const first of candidates) {
      for (const second of candidates) {
        for (const third of candidates) {
          ...
          expect([state.active, state.pending].filter(Boolean).length)
  ...
  it("#given every completion outcome #when the active run completes #then only merged and no_changes advance its captured cursor", () => {
    ...
    for (const outcome of outcomes) {
      ...
      const result = completeTransition(
        ...
        "run",
        outcome,
        new Map([["conversation-a", journal(6)]]),
        { stepCount: 5, onCompaction: true },
      );
      expect(result.finalize).toHaveLength(
        outcome === "merged" || outcome === "no_changes" ? 1 : 0,
      );
  ...
  it("#given a failed active run and a queued request #when completed #then the cursor stays unmoved and pending is promoted", () => {
    const active = reserveTransition({}, request("manual"), "active");
    const queued = reserveTransition(
      active.state,
      request("step-count"),
      "pending",
    );
  ...
  it("#given pending automatic work became stale #when dequeued #then it is dropped, while manual work survives", () => {
    const active = reserveTransition({}, request("manual"), "active");
    const stale = reserveTransition(
      active.state,
      request("step-count"),
      "stale",
    );
    const staleResult = completeTransition(
      stale.state,
      "active",
      "failed",
      new Map([["conversation-a", journal(1)]]),
      { stepCount: 5 },
    );
    ...
    const manualResult = completeTransition(
      manual.state,
      "active",
      "failed",
      new Map([["conversation-a", journal(0)]]),
      { stepCount: 5 },
    );
