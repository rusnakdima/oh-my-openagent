...
export function evaluateTransitions(
  ...
  if (event.kind === "compaction_accepted") {
    return {
      state: {
        ...
        journal: {
          ...state.journal,
          state: { ...state.journal.state, pending_compaction: true },
        },
      },
      action: { kind: "none" },
    };
  ...
  if (event.kind === "manual") {
    const ids = event.conversationIds?.length
      ? event.conversationIds
      : [state.journal.conversationId];
    return {
      state,
      action: {
        kind: "reserve",
        request: makeRequest(state.journal, "manual", ids, event),
      },
    };
  }
  if (!event.success) return { state, action: { kind: "none" } };

  const compactionReady = state.config.onCompaction === true &&
  ...
  if (compactionReady) {
    return {
      ...
      action: {
        kind: "reserve",
        request: makeRequest(state.journal, "compaction"),
      },
    };
  }

  const threshold = state.config.stepCount;
  const thresholdReady = threshold !== undefined &&
    threshold > 0 &&
  ...
  return thresholdReady
    ? {
      ...
      action: {
        kind: "reserve",
        request: makeRequest(state.journal, "step-count"),
      },
    }
...
export function reserveTransition(
  ...
  if (!state.active) {
    return { state: { active: { runId, request } }, result: "active" };
  }
  const pending = state.pending
    ? {
      runId: state.pending.runId,
      request: mergeRequests(state.pending.request, request),
    }
    : { runId, request };
...
export function completeTransition(
  ...
  if (state.active?.runId !== runId) {
    throw new Error(`Reflection run is not active: ${runId}`);
  }
  ...
  const clearPendingCompaction =
    succeeded && state.active.request.trigger === "compaction"
      ? state.active.request.conversationIds.filter((id) =>
        journals.get(id)?.state.pending_compaction
      )
      : [];
  ...
  for (const captured of finalize) {
    ...
    const reflected = Math.min(
      ...
      current.state.reflected_completed_steps +
        countCompletedSteps(captured.snapshot.entries),
    );
    effectiveJournals.set(captured.conversationId, {
      ...
      state: {
        ...
        steps_since_last_successful_reflection:
          current.state.total_completed_steps - reflected,
      },
    });
  }
  ...
  const pending = state.pending;
  if (
    !pending || !isStillTriggered(pending.request, effectiveJournals, config)
  ) {
...
function makeRequest(
  ...
  return {
    ...
    snapshots: journal.snapshot
      ? [{ conversationId: journal.conversationId, snapshot: journal.snapshot }]
      : [],
...
function containsTrigger(
  ...
  return state.active?.request.trigger === trigger ||
    state.pending?.request.trigger === trigger;
}

function mergeRequests(
  left: ReflectionRequest,
  right: ReflectionRequest,
): ReflectionRequest {
  const strongest = requestPriority(right) >= requestPriority(left)
    ? right
    : left;
  const merged = {
    conversationIds: unique([
      ...left.conversationIds,
      ...right.conversationIds,
    ]),
...
function requestPriority(request: ReflectionRequest): number {
  ...
  if (request.origin === undefined) {
    throw new TypeError("dream requests require an origin");
  }
  return DREAM_PRIORITY[request.origin];
}

function mergeSnapshots(
  left: readonly CapturedConversation[],
  right: readonly CapturedConversation[],
) {
...
function isStillTriggered(
  ...
  if (request.trigger === "compaction") {
    return config.onCompaction === true &&
      request.conversationIds.some((id) =>
        journals.get(id)?.state.pending_compaction === true
      );
  }
  ...
  return threshold !== undefined && threshold > 0 &&
    request.conversationIds.some(
      (id) =>
        (journals.get(id)?.state.steps_since_last_successful_reflection ?? 0) >=
          threshold,
    );
}
