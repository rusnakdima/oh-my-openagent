...
export class ReflectionReservationStore {
  ...
  constructor(private readonly options: ReflectionReservationStoreOptions) {
    ...
    this.schedulerLockPath = reflectionSchedulerLockPath(
      options.identity.paths.locks,
    );
  ...
  async evaluate(
    ...
    for (const id of evaluated.action.request.conversationIds) {
      const captured = await (await this.options.getJournal(id))
  ...
  async tryReserve(
    ...
    return this.locked(runId, async () => {
      ...
      const state = transition.result === "active"
        ? {
          ...transition.state,
          active: await this.withLaunchOwner(transition.state.active),
        }
        : transition.state;
      signal?.throwIfAborted();
  ...
  async complete(
    ...
    return this.locked(runId, async () => {
      ...
      if (
        (outcome === "merged" || outcome === "no_changes") &&
        current.active?.request.trigger === "dream"
      ) {
        await writeJsonAtomic(
          join(this.options.identity.paths.runtime, "dream", "state.json"),
          {
      ...
      const promoted = transition.launch === undefined
        ? undefined
        : await this.withLaunchOwner(transition.launch);
      const nextState = promoted === undefined
        ? transition.state
        : { ...transition.state, active: promoted };
  ...
  private async withLaunchOwner(
    run: ReservedRun | undefined,
  ): Promise<ReservedRun | undefined> {
  ...
  private async readStateUnlocked(): Promise<ReservationState> {
    ...
    return {
      ...(active === null ? {} : { active }),
      ...(pending === null || pending.runId === active?.runId
        ? {}
        : { pending }),
    };
  }
  ...
  private async writeStateUnlocked(state: ReservationState): Promise<void> {
    await mkdir(this.options.identity.paths.reflection, {
      recursive: true,
      mode: 0o700,
    });
...
async function readRun(path: string): Promise<ReservedRun | null> {
  ...
  if (!isReservedRun(parsed)) {
    throw new Error(`Invalid reflection reservation: ${path}`);
  }
  return parsed;
}
...
async function writeOptionalRun(
  path: string,
  run: ReservedRun | undefined,
): Promise<void> {
  if (!run) {
...
function isReservedRun(value: unknown): value is ReservedRun {
  ...
  return typeof run.runId === "string" && run.runId.length > 0 &&
    isReflectionRequest(run.request);
}
...
function isReflectionRequest(value: unknown): value is ReflectionRequest {
  ...
  return (
    (request.trigger === "manual" || request.trigger === "compaction" ||
      request.trigger === "step-count" || request.trigger === "dream") &&
    (request.trigger === "dream"
      ? request.origin === "manual" || request.origin === "idle" ||
        request.origin === "shutdown" || request.origin === "pressure"
      : request.origin === undefined) &&
    Array.isArray(request.conversationIds) &&
    request.conversationIds.every((id) => typeof id === "string") &&
    Array.isArray(request.snapshots) &&
    (request.targetDoc === undefined ||
      (request.trigger === "dream" && typeof request.targetDoc === "string"))
  );
}
