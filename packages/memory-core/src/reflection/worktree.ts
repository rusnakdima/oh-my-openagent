...
export async function createReflectionWorktree(
  ...
  if (!isAbsolute(worktreesDir)) {
    throw new TypeError("worktreesDir must be absolute");
  }
  const baseSha = await repo.head();
  if (!baseSha) {
    throw new Error(
      "Cannot create a reflection worktree without a parent HEAD",
    );
  }
  ...
  try {
    ...
    const commonDirResult = await git(exec, repo.dir, [
      "rev-parse",
      "--git-common-dir",
    ]);
...
export async function finalizeReflectionWorktree(
  ...
  try {
    ...
    if (validation.status !== "valid") {
      ...
      options.mode === "auto" && options.allowedPaths !== undefined &&
      validation.changedPaths.some((path) =>
        !options.allowedPaths?.includes(path)
      )
    ) {
      detail = `Dream document maintenance changed paths outside its target: ${
        validation.changedPaths.join(", ")
      }`;
    } else {
      const integrated = await integrateValidatedReflection(worktree, {
        mode: options.mode === "auto" ? "auto" : "integration",
        runId: options.mode === "auto"
          ? options.runId ?? worktree.branch
          : worktree.branch,
        summary: options.mode === "auto"
          ? options.summary
  ...
  if (!cleanup.worktreeRemoved || !cleanup.branchRemoved) {
    status = "failed";
    detail = [detail, "Reflection cleanup did not fully complete"].filter(
      Boolean,
    ).join("; ");
  }
...
async function removeWorktreeAndBranch(
  ...
  const branchRef = await run(exec, repo.dir, [
    "show-ref",
    "--verify",
    `refs/heads/${branch}`,
  ]);
  return {
    worktreeRemoved: !existsSync(dir) &&
...
function sanitizeRunId(runId: string): string {
  const id = basename(runId.trim()).replace(/[^a-zA-Z0-9._-]+/g, "-").replace(
    /^-+|-+$/g,
    "",
  );
  if (!id || id === "." || id === "..") {
