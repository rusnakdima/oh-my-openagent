...
export async function probeReflectionIntegration(
  ...
  if (input.mode === "auto") {
    const receipt = await findExactRunReceipt(
      worktree,
      input.runId,
      input.validatedTipSha,
    );
    if (receipt !== null) {
      return {
        status: "integrated",
        evidence: "receipt",
        integrationSha: receipt,
      };
    }
  }
  ...
  if (
    ...
    const head = await git(worktree.exec, worktree.parent.dir, [
      "rev-parse",
      "--verify",
      "HEAD",
    ]);
    return {
      status: "integrated",
      evidence: "ancestry",
      integrationSha: head.stdout.trim(),
    };
  }
...
export async function integrateValidatedReflection(
  ...
  return input.withWriterLock(async () => {
    ...
    if (input.mode === "integration") {
      return {
        outcome: "failed",
        detail: "Reflection branch tip is not reachable from parent HEAD",
      };
    }

    const mergeHead = await optionalRevision(
      worktree.exec,
      worktree.parent.dir,
      "MERGE_HEAD",
    );
    if (mergeHead === input.validated.tipSha) {
      const aborted = await run(worktree.exec, worktree.parent.dir, [
        "merge",
        "--abort",
      ]);
      if (aborted.code !== 0) {
        return {
          outcome: "failed",
          detail: aborted.stderr.trim() ||
            "Owned reflection merge could not be aborted",
        };
      }
    } else if (mergeHead !== null) {
      return {
        outcome: "parent_dirty",
        detail: "Parent repository contains another interrupted merge",
      };
    }
    ...
    const merge = await run(worktree.exec, worktree.parent.dir, [
      "merge",
      "--no-ff",
      input.validated.tipSha,
      "-m",
      `merge(reflection): ${input.summary}`,
      "-m",
      `Omo-Run: ${input.runId}`,
    ]);
    if (merge.code === 0) {
    ...
    const landedMergeHead = await optionalRevision(
      worktree.exec,
      worktree.parent.dir,
      "MERGE_HEAD",
    );
    const unmerged = await run(worktree.exec, worktree.parent.dir, [
      "diff",
      "--name-only",
      "--diff-filter=U",
    ]);
    if (landedMergeHead === input.validated.tipSha) {
      ...
      if (aborted.code !== 0) {
        return {
          outcome: "failed",
          detail: aborted.stderr.trim() ||
            "Reflection merge conflict could not be aborted",
        };
      }
    }
    ...
    return {
      outcome: "failed",
      detail: merge.stderr.trim() || "Reflection merge failed",
    };
  });
}
...
export async function cleanupReflectionWorktree(
  ...
  await worktree.parent.worktreeRemove(worktree.dir, true).catch(() =>
    undefined
  );
  await rm(worktree.dir, { recursive: true, force: true }).catch(() =>
    undefined
  );
  await run(worktree.exec, worktree.parent.dir, ["worktree", "prune"]);
  await run(worktree.exec, worktree.parent.dir, [
    "branch",
    "-D",
    worktree.branch,
  ]);
  const listed = await run(worktree.exec, worktree.parent.dir, [
    "worktree",
    "list",
    "--porcelain",
  ]);
  const branchRef = await run(worktree.exec, worktree.parent.dir, [
    "show-ref",
    "--verify",
...
async function findExactRunReceipt(
  ...
  const commits = await run(worktree.exec, worktree.parent.dir, [
    "rev-list",
...
async function hasExactRunReceipt(
  exec: GitExec,
  cwd: string,
  commit: string,
  runId: string,
): Promise<boolean> {
  const trailers = await run(exec, cwd, [
    "show",
    "-s",
    "--format=%(trailers:key=Omo-Run,valueonly)",
    commit,
  ]);
  return trailers.code === 0 &&
    trailers.stdout.split(/\r?\n/).some((value) => value === runId);
}
...
async function optionalRevision(
  ...
  const result = await run(exec, cwd, [
    "rev-parse",
    "-q",
    "--verify",
    revision,
...
async function git(
  ...
  if (result.code !== 0) {
    throw new Error(result.stderr.trim() || `git ${argv.join(" ")} failed`);
  }
  return result;
}
