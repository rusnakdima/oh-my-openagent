...
async function fixture() {
  const root = realpathSync.native(
    await mkdtemp(join(tmpdir(), "reflection-worktree-")),
  );
  roots.push(root);
...
async function commit(
  ...
  await child.commitWrite([path], message, {
    agentId: "agent-one",
...
describe("reflection worktree finalization", () => {
  ...
  it("#given conflicting child and parent commits #when auto-finalized #then merge aborts cleanly and cleanup always runs", async () => {
    ...
    await repo.commitWrite(["memory.md"], "parent advance", {
      agentId: "agent-one",
      authorName: "Parent",
    });
    ...
    expect(
      (await exec.run(["rev-parse", "-q", "--verify", "MERGE_HEAD"], {
        cwd: parentDir,
        timeoutMs: 30_000,
      })).code,
    ).toBe(1);
  ...
  it(
    "#given a non-conflicting parent advance after launch #when auto-finalized #then recorded-base drift merges successfully",
    async () => {
      // #given
      const { repo, parentDir, worktreesDir } = await fixture();
      const worktree = await createReflectionWorktree(
      ...
      await repo.commitWrite(["parent.md"], "parent advance", {
        agentId: "agent-one",
        authorName: "Parent",
      });
  ...
  it(
    "#given an externally merged reflection branch #when explicit finalization verifies reachability #then it reports merged and cleans up",
    async () => {
      // #given
      const { repo, parentDir, worktreesDir } = await fixture();
      const worktree = await createReflectionWorktree(
      ...
      await repo.merge(worktree.branch, {
        noFF: true,
