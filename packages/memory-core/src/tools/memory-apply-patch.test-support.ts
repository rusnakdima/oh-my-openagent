...
export const author = {
  agentId: "patch-agent",
  authorName: "Patch Agent",
  authorEmail: "patch@example.test",
};
...
export async function memoryApplyPatch(
  repo: GitMemoryRepo,
  patchParams: MemoryApplyPatchParams,
) {
  return runMemoryApplyPatch({
    repo,
    params: patchParams,
    lock: async (_domain, operation) => operation(),
  });
}
