...
export async function runMemoryApplyPatch(
  ...
  try {
    ...
    return await lock("memory-write", async () => {
      ...
      const paths = await applyOperations(
        repo.dir,
...
async function applyOperations(
  ...
  const loadCurrent = async (
    ...
    if (
      pendingDeletes.has(path.absolute) && !pendingWrites.has(path.absolute)
    ) {
      throw new Error(
        `memory_apply_patch: file not found for update: ${path.relative}`,
      );
    }
    ...
    try {
      ...
      throw new Error(
        `memory_apply_patch: failed to read ${path.relative}: ${
          errorMessage(error)
        }`,
      );
    }
  ...
  for (const operation of operations) {
    if (operation.kind === "add") {
      ...
      if (pendingWrites.has(target.absolute)) {
        throw new Error(
          `memory_apply_patch: duplicate add/update target in patch: ${target.relative}`,
        );
      }
      if (await exists(target.absolute)) {
        throw new Error(
          `memory_apply_patch: cannot add existing memory file: ${target.relative}`,
        );
      }
      const rawContent = operation.contentLines.join("\n");
      pendingWrites.set(
        target.absolute,
        normalizeAddedContent(target.relative.slice(0, -3), rawContent),
      );
...
function parseForTool(content: string): ReturnType<typeof parseMemoryFile> {
  try {
    ...
    throw new Error(
      `memory_apply_patch: ${
        errorMessage(error).replace(/^frontmatter: /, "")
      }`,
    );
  }
}
...
function assertEditable(readOnly: string | undefined, path: string): void {
  if (readOnly === "true") {
    throw new Error(
      `memory_apply_patch: ${path} is read_only and cannot be modified`,
    );
  }
}
...
async function readUtf8TextStrict(path: string): Promise<string> {
  ...
  if (bom !== null) {
    throw new Error(
      `File is not valid UTF-8 text: ${path}. Detected ${bom} BOM; convert the file to UTF-8 and retry.`,
    );
  }
  try {
    ...
    throw new Error(
      `File is not valid UTF-8 text: ${path}. The file contains bytes that cannot be decoded as UTF-8.`,
    );
  }
}
