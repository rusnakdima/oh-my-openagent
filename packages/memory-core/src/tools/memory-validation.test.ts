...
async function fixture(): Promise<{
  ...
  const root = realpathSync.native(
    await mkdtemp(join(tmpdir(), "omo-memory-tool-")),
  );
  roots.push(root);
...
describe("runMemoryTool", () => {
  it("#given read_only frontmatter #when update_description runs #then it rejects without changing HEAD", async () => {
    ...
    await expect(run(setup, {
      ...
      reason: "change locked",
      file_path: "system/locked",
      description: "New",
    })).rejects.toThrow(
      "memory: system/locked is read_only and cannot be modified",
    );
  ...
  it("#given editable frontmatter with read_only false #when update_description runs #then body and flag are preserved", async () => {
    ...
    await run(setup, {
      command: "update_description",
      reason: "describe note",
      file_path: "system/note",
      description: "New description",
    });

    expect(
      parseMemoryFile(
        await readFile(join(setup.repo.dir, "system/note.md"), "utf8"),
      ),
    ).toEqual({
      frontmatter: { description: "New description", read_only: "false" },
      body: "keep body",
    });
  });
  ...
  it("#given read_only descendants #when recursive delete runs #then tool-level enforcement rejects the directory", async () => {
    ...
    await expect(run(setup, {
      command: "delete",
      reason: "delete locked directory",
  ...
  it("#given each editable mutation command targets read_only content #when run #then every command rejects", async () => {
    ...
    for (const params of cases) {
      await expect(run(setup, params)).rejects.toThrow(
        /memory: locked is read_only/,
      );
    }
  });
  ...
  it("#given invalid and missing command inputs #when run #then caller-neutral typed tool errors retain the memory prefix", async () => {
    const rows: Array<
      { params: Omit<MemoryToolParams, "author">; message: RegExp }
    > = [
      {
        params: {
          command: "create",
        ...
        message: /^memory: 'reason' must be a non-empty string/,
      },
      {
        params: {
          command: "create",
        ...
        message: /^memory: create: 'description'/,
      },
      {
        params: {
          command: "insert",
          reason: "x",
          file_path: "x",
          insert_line: Number.NaN,
          insert_text: "x",
        },
        message: /^memory: insert: 'insert_line' must be a number/,
      },
      {
        params: {
          command: "insert",
        ...
        message: /^memory: insert: 'insert_text'/,
      },
      {
        params: { command: "rename", reason: "x", old_path: "", new_path: "x" },
        message: /^memory: rename: 'old_path'/,
      },
    ];

    const setup = await fixture();
    for (const row of rows) {
      const error = await run(setup, row.params).then(
        () => null,
        (cause: unknown) => cause,
      );
  ...
  it("#given byte-identical replacements or descriptions #when commit detects no diff #then it reports no effective changes", async () => {
    ...
    for (const params of rows) {
      ...
      const head = await setup.repo.head();
      await expect(run(setup, params)).rejects.toThrow(
        `memory: ${params.command} made no effective changes`,
      );
      expect(await setup.repo.head()).toBe(head);
  ...
  it("#given unrelated dirty files #when a command starts #then cleanCheck fails first with porcelain listing", async () => {
    ...
    await expect(run(setup, {
      ...
      /memory: Memory repo has uncommitted changes[\s\S]*\?\? dirty\.md/,
    );
    expect(await readFile(join(setup.repo.dir, "dirty.md"), "utf8")).toBe(
      "dirty",
    );
  });
});
