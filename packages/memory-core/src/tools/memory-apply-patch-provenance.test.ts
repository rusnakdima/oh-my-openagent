...
describe("memoryApplyPatch provenance and locks", () => {
  it(
    "#given accepted-turn provenance #when a patch commits #then the in-band writer session and turn trailers are durable",
    async () => {
      // given
  ...
  it(
    "#given a delete and configured remote #when applied #then writer lock, deletion, and remote result shape are pinned",
    async () => {
      // #given
      ...
      expect(result.message).toMatch(
        /^memory_apply_patch committed \([a-f0-9]{7}\); harness will sync after the turn\.$/,
      );
    },
    { timeout: MEMORY_GIT_TEST_TIMEOUT_MS },
  );
