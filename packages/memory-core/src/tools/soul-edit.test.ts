...
import { type MemoryToolLock, runMemoryTool } from "./memory";
...
async function fixture(): Promise<
  { repo: GitMemoryRepo; lock: MemoryToolLock }
> {
  const root = realpathSync.native(
    await mkdtemp(join(tmpdir(), "omo-memory-soul-edit-")),
  );
  roots.push(root);
...
async function seed(
  ...
  await writeFile(
    fullPath,
    renderMemoryFile({ description: "Seed" }, text),
    "utf8",
  );
...
describe("runMemoryTool soul-edit result", () => {
  it(
    "#given a create committed under system/persona.md #when the tool runs #then the result carries the commit metadata and the soul-edit directive",
    async () => {
      // given
  ...
  it(
    "#given a str_replace on system/identity.md #when the tool runs #then the soul-edit directive appears",
    async () => {
      // given
      const setup = await fixture();
  ...
  it(
    "#given a rename that moves system/persona.md #when the tool runs #then the soul-edit directive appears",
    async () => {
      // given
      const setup = await fixture();
      ...
      // then
      expect(result.message).toContain(MEMORY_SOUL_EDIT_RESULT_TOKEN);
      expect(result.commit?.affectedPaths).toEqual([
        "system/persona.md",
        "reference/old-persona.md",
      ]);
    },
    WINDOWS_INTEGRATION_TEST_TIMEOUT,
  );
  ...
  it(
    "#given a commit that touches only non-soul files #when the tool runs #then no soul-edit directive appears but commit metadata is present",
    async () => {
      // given
      const setup = await fixture();
...
describe("runMemoryApplyPatch soul-edit result", () => {
  it(
    "#given a patch updating system/persona.md #when applied #then the result carries the soul-edit directive and commit metadata",
    async () => {
      // given
      const setup = await fixture();
  ...
  it(
    "#given a patch adding a non-soul file #when applied #then no soul-edit directive appears",
    async () => {
      // given
      const setup = await fixture();
