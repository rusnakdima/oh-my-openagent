...
const MEMORY_GIT_TEST_TIMEOUT_MS = process.platform === "win32"
  ? 60_000
  : 20_000;

describe("memoryApplyPatch validation failures", () => {
  it(
    "#given read_only source or resulting target #when updated #then both are rejected before disk writes",
    async () => {
      // #given
      const locked = "---\ndescription: Locked\nread_only: true\n---\nkeep";
      const open = "---\ndescription: Open\n---\nkeep";
  ...
  it(
    "#given near-matching context #when updated #then no fuzzy match is attempted",
    async () => {
      // #given
      const original =
        "---\ndescription: Similar\n---\nRemember Apollo details.\nKeep nuance.";
      const { dir, repo, locksDirectory } = await fixture({
        "system/similar.md": original,
      });
      ...
      expect(await readFile(join(dir, "system/similar.md"), "utf8")).toBe(
        original,
      );
    },
    { timeout: MEMORY_GIT_TEST_TIMEOUT_MS },
  ...
  it(
    "#given backticks in mismatched previews #when diagnosed #then the exact hardened format sizes fences safely",
    async () => {
      // #given
      const original =
        "---\ndescription: Fenced\n---\nBefore\n```ts\ncurrent\n```\nAfter";
      const { repo, locksDirectory } = await fixture({
        "system/fenced.md": original,
      });
  ...
  it(
    "#given oversized failed and current chunks #when diagnosed #then previews truncate at 2000 and 4000 characters",
    async () => {
      // #given
  ...
  it(
    "#given UTF-16LE source #when read #then it is rejected with conversion guidance and remains byte-identical",
    async () => {
      // #given
      const text = "---\ndescription: UTF16\n---\nold";
      ...
      expect(
        Buffer.compare(await readFile(join(dir, "system/utf16.md")), bytes),
      ).toBe(0);
    },
  ...
  it(
    "#given duplicate add, existing add, and empty patch #when applied #then each fails with the tool prefix",
    async () => {
      // #given
      const rows = [
        [
