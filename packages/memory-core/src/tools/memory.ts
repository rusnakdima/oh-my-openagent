...
export async function runMemoryTool(
  ...
  try {
    ...
    return await lock("memory-write", async () => {
      ...
      const summary = local
        ? `Memory ${params.command} committed locally (${
          result.sha.slice(0, 7)
        }).`
        : `Memory ${params.command} committed (${
          result.sha.slice(0, 7)
        }); harness will sync after the turn.`;
      return {
...
async function strReplace(
  ...
  const body = file.body.slice(0, index) + newString +
...
async function insert(
  ...
  if (
    typeof params.insert_line !== "number" || Number.isNaN(params.insert_line)
  ) {
  ...
  await writeFile(
    path,
    renderMemoryFile(file.frontmatter, lines.join("\n")),
    "utf8",
  );
  return affected(root, path);
}
...
async function remove(
  ...
  if (
    existsSync(repositoryPath) && (await stat(repositoryPath)).isDirectory()
  ) {
...
async function assertDirectoryEditable(
  directory: string,
  root: string,
): Promise<void> {
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) await assertDirectoryEditable(path, root);
      ...
      await loadEditable(path, relativePath(root, path));
    }
  }
}
...
async function hasConfiguredRemote(repo: GitMemoryRepo): Promise<boolean> {
  const gitConfig = await readFile(join(repo.dir, ".git", "config"), "utf8")
    .catch(() => "");
...
function memoryCommitMessage(
  reason: string,
  provenance: MemoryToolProvenance | undefined,
): string {
