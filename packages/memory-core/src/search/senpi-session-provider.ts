...
function toDocument(
  ...
  const toolCalls = toolName
    ? [...mapped.toolCalls, { name: toolName }]
    : mapped.toolCalls;
...
function readSessionFile(
  filePath: string,
): { header?: SenpiSessionHeader; lines: Record<string, unknown>[] } {
  let raw: string;
  try {
...
function listSessionFiles(
  sessionsDir: string,
): { filePath: string; directoryName: string }[] {
      ...
      if (!stat.isDirectory()) {
        ...
        if (directoryName.endsWith(SESSION_SUFFIX)) {
          files.push({
            ...
            directoryName: basename(sessionsDir),
          });
        }
        continue;
      }
...
export class SenpiSessionProvider implements TranscriptProvider {
  ...
  listConversations(): TranscriptConversation[] {
    ...
    for (
      const { filePath, directoryName } of listSessionFiles(
  ...
  private isHidden(candidate: SenpiHiddenCandidate): boolean {
    ...
    if (this.options.excludedDirs?.includes(candidate.directoryName)) {
      return true;
    }
    const marker = this.options.hiddenMarkerFile;
    if (marker && existsSync(join(candidate.filePath, "..", marker))) {
      return true;
    }
