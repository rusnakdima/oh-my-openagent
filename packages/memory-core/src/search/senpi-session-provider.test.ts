...
import { searchTranscripts, SenpiSessionProvider } from "./index";
...
describe("SenpiSessionProvider", () => {
  it("#given a session file #when conversations are listed #then the file is one conversation keyed by the header id", () => {
    ...
    // when
    const conversations = new SenpiSessionProvider({ sessionsDir: root })
      .listConversations();

    // then
    expect(conversations).toHaveLength(1);
    expect(conversations[0]?.id).toBe("sess-a");
    expect(conversations[0]?.messages.map((document) => document.id)).toEqual([
      "e1",
    ]);
  });
  ...
  it("#given user, assistant and tool-result entries #when mapped #then text, reasoning, tool names, arguments and returns are searchable", () => {
    ...
    expect(byId.get("a1")?.toolCalls).toEqual([{
      name: "bash",
      arguments: '{"cmd":"psql migrate"}',
    }]);
    ...
    expect(
      searchTranscripts(provider, "rollback").map((result) => result.messageId),
    ).toEqual(["a1"]);
    expect(
      searchTranscripts(provider, '"psql migrate"').map((result) =>
        result.messageId
      ),
    ).toEqual(["a1"]);
    expect(
      searchTranscripts(provider, "applied").map((result) => result.messageId),
    ).toEqual(["t1"]);
  });
  ...
  it("#given branches that repeat a shared ancestor entry #when mapped #then each entry id appears once", () => {
    ...
    // when
    const documents =
      new SenpiSessionProvider({ sessionsDir: root }).listConversations()[0]
        ?.messages ?? [];
  ...
  it("#given a malformed line and a missing header #when read #then bad lines are skipped and the rest of the file survives", () => {
    ...
    writeSession(join(root, "--tmp-project--"), "headerless.jsonl", [
      userEntry("orphan", null, "no header here"),
    ]);
    ...
    // then
    expect(withHeader?.messages.map((document) => document.id)).toEqual([
      "ok-1",
      "ok-2",
    ]);
    expect(headerless?.messages.map((document) => document.id)).toEqual([
      "orphan",
    ]);
  });
  ...
  it("#given an archived sidecar, an excluded directory and a hidden-marker hook #when listed #then those conversations are hidden but still readable with includeHidden", () => {
    ...
    writeSession(projectDir, "plain.jsonl", [
      header("plain"),
      userEntry("p1", null, "beta visible"),
    ]);
    writeSession(projectDir, "archived.jsonl", [
      header("archived"),
      userEntry("a1", null, "beta archived"),
    ]);
    writeFileSync(join(projectDir, "archived.jsonl.archived"), "");
    writeSession(join(root, "--omo-reflection--"), "reflect.jsonl", [
      header("reflect"),
      userEntry("r1", null, "beta reflection"),
    ]);
    writeSession(projectDir, "task-child.jsonl", [
      header("task-child"),
      userEntry("k1", null, "beta task child"),
    ]);
    ...
    // then
    expect(
      conversations.filter((item) => item.hidden !== true).map((item) =>
        item.id
      ),
    ).toEqual(["plain"]);
    expect(visible.map((result) => result.messageId)).toEqual(["p1"]);
    expect(all.map((result) => result.messageId).sort()).toEqual([
      "a1",
      "k1",
      "p1",
      "r1",
  ...
  it("#given a hidden marker file inside a session directory #when listed #then every session in that directory is hidden", () => {
    ...
    writeSession(hiddenDir, "one.jsonl", [
      header("one"),
      userEntry("h1", null, "beta hidden dir"),
    ]);
    writeFileSync(join(hiddenDir, ".omo-hidden"), "");
    writeSession(join(root, "--tmp-project--"), "two.jsonl", [
      header("two"),
      userEntry("v1", null, "beta plain dir"),
    ]);
    const provider = new SenpiSessionProvider({
      sessionsDir: root,
      hiddenMarkerFile: ".omo-hidden",
    });
  ...
  it("#given a missing sessions directory or non-jsonl files #when listed #then reading is empty and never throws", () => {
    ...
    const missing = new SenpiSessionProvider({
      sessionsDir: join(root, "does-not-exist"),
    });

    // when
    const empty = missing.listConversations();
    const ignored = new SenpiSessionProvider({ sessionsDir: root })
      .listConversations();
