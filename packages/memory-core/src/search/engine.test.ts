...
describe("searchTranscripts", () => {
    ...
    const messages = Array.from(
      ...
      (_, index) =>
        message({
          id: `m${index}`,
          content: "beta",
          date: `2026-01-01T00:00:${String(index % 60).padStart(2, "0")}.000Z`,
        }),
    );
  ...
  it("#given hidden conversations #when searched #then they are excluded unless includeHidden is set", () => {
    ...
    const provider = fakeProvider([
      ...
      conversation("c2", [
        message({ id: "concealed", conversationId: "c2", content: "beta" }),
      ], true),
    ]);

    // when
    const defaulted = searchTranscripts(provider, "beta");
    const withHidden = searchTranscripts(provider, "beta", {
      includeHidden: true,
    });

    // then
    expect(defaulted.map((result) => result.messageId)).toEqual(["visible"]);
    expect(withHidden.map((result) => result.messageId).sort()).toEqual([
      "concealed",
      "visible",
    ]);
  });
  ...
  it("#given a conversation filter #when searched #then only that conversation contributes results", () => {
    ...
    const provider = fakeProvider([
      conversation("c1", [message({ id: "m1", content: "beta" })]),
      conversation("c2", [
        message({ id: "m2", conversationId: "c2", content: "beta" }),
      ]),
    ]);
