...
export function searchTranscripts(
  ...
  for (const conversation of provider.listConversations()) {
    if (conversation.hidden === true && options.includeHidden !== true) {
      continue;
    }
    ...
    for (const document of conversation.messages) {
      ...
      if (!dateInRange(document.date, options.startDate, options.endDate)) {
        continue;
      }
  ...
  return scored
    .sort((left, right) => {
      ...
      return (right.document.date ?? "").localeCompare(
        left.document.date ?? "",
      );
    })
    .slice(0, limit)
