...
describe("observation entry format (IC-16)", () => {
  ...
  describe("#given an explicit observation with full comment fields", () => {
    ...
    it("#then parsePeopleCard extracts the section and entry", () => {
      ...
      expect((result.card.observations ?? [])[0].entries[0].date).toBe(
        "2026-01-15",
      );
      expect((result.card.observations ?? [])[0].entries[0].content).toBe(
        "Prefers dark mode",
      );
      expect((result.card.observations ?? [])[0].entries[0].src).toBe(
        "msg-001",
      );
      expect((result.card.observations ?? [])[0].entries[0].n).toBe(2);
      expect((result.card.observations ?? [])[0].entries[0].pattern).toBe(
        "repeated",
      );
      expect((result.card.observations ?? [])[0].entries[0].confidence).toBe(
        "high",
      );
      expect((result.card.observations ?? [])[0].entries[0].status).toBe(
        "open",
      );
    });
  });
  ...
  describe("#given an observation with minimal fields (just src)", () => {
    ...
    it("#then parsePeopleCard extracts with optional fields undefined", () => {
      ...
      expect((result.card.observations ?? [])[0].entries[0].pattern)
        .toBeUndefined();
      expect((result.card.observations ?? [])[0].entries[0].confidence)
        .toBeUndefined();
      expect((result.card.observations ?? [])[0].entries[0].status)
        .toBeUndefined();
    });
  });
  ...
  describe("#given an observation with no comment at all", () => {
    ...
    it("#then parsePeopleCard extracts with all comment fields undefined", () => {
      ...
      expect((result.card.observations ?? [])[0].entries[0].src)
        .toBeUndefined();
    });
  });
