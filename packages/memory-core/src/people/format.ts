...
const OBSERVATION_ENTRY_RE = /^-\s+\[(\d{4}-\d{2}-\d{2})\]\s+(.*)$/;
...
export function parsePeopleCard(
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (
      ...
      !OBSERVATION_SECTION_RE.test(line.trim())
    ) {
      continue;
    }
    ...
    if (sectionMatch) {
      ...
      if (!VALID_SECTIONS.has(sectionName as ObservationSection)) {
        diagnostics.push(
          `line ${
            i + 1
          }: unknown observation section '${sectionName}' (expected: ${
            [...VALID_SECTIONS].join(", ")
          })`,
        );
    ...
    if (entryMatch) {
      ...
      currentEntries.push({
        date,
        content,
        ...(commentFields.src !== undefined
          ? { src: String(commentFields.src) }
          : {}),
        ...(commentFields.n !== undefined
          ? { n: Number(commentFields.n) }
          : {}),
        ...(commentFields.pattern !== undefined
          ? { pattern: String(commentFields.pattern) }
          : {}),
        ...(commentFields.confidence !== undefined
          ? { confidence: String(commentFields.confidence) }
          : {}),
        ...(commentFields.status !== undefined
          ? { status: String(commentFields.status) }
          : {}),
      });
      continue;
    }
    ...
    if (colonIdx > 0) {
      ...
      if (VALID_PREFIXES.has(prefix as CardPrefix)) {
        ...
        if (content.length > limits.maxEntryChars) {
          diagnostics.push(
            `line ${
              i + 1
            }: entry exceeds ${limits.maxEntryChars} chars (got ${content.length})`,
          );
        }
      ...
      diagnostics.push(
        `line ${i + 1}: invalid prefix '${prefix}' (expected: ${
          [...VALID_PREFIXES].join(", ")
        })`,
      );
      continue;
    ...
    for (const group of card.observations) {
      ...
      for (const entry of group.entries) {
        ...
        if (entry.pattern !== undefined) {
          commentParts.push(`pattern: ${entry.pattern}`);
        }
        if (entry.confidence !== undefined) {
          commentParts.push(`confidence: ${entry.confidence}`);
        }
        if (entry.status !== undefined) {
          commentParts.push(`status: ${entry.status}`);
        }
