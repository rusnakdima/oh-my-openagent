...
async function consumeUnderLock(
  ...
  try {
    commits = await repo.log({
      range: `${watermark.lastNotifiedHead}..HEAD`,
      paths: SOUL_PATHS,
    });
  } catch (error) {
  ...
  const newest = commits.find((commit) =>
...
async function readWatermark(
  noticesDir: string,
): Promise<SoulHeadWatermark | undefined> {
  let parsed: unknown;
  try {
    parsed = JSON.parse(
      await readFile(join(noticesDir, SOUL_NOTICE_WATERMARK_FILENAME), "utf8"),
    );
  } catch {
    return undefined;
  }
