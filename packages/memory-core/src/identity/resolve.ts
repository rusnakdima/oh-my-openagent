...
export function shortHash(input: string): string {
  return createHash("sha256").update(input, "utf8").digest("hex").slice(
    0,
    SHORT_HASH_LENGTH,
  );
}
...
export function isAutoAgentValue(
...
function deriveExplicitId(
  trimmedValue: string,
): { id: string; safeSlug: string } {
...
export function resolveMemoryIdentity(
  ...
  const trimmed = typeof configAgentValue === "string"
    ? configAgentValue.trim()
    : "";
  const derived = trimmed === "" || trimmed === AUTO_AGENT_VALUE
    ? deriveAutoId(cwd)
