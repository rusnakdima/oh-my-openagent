...
describe("compileMemoryBlock", () => {
  it(
    "#given nested committed memory #when compiled #then block sections, projection order, and metadata values form the structural contract",
    async () => {
      // given
      const { repo } = await repoWith([
        {
          relativePath: "system/persona.md",
          content: memory("PERSONA_DESCRIPTION", "PERSONA_BODY\n"),
        },
        {
          relativePath: "system/facts.md",
          content: memory("FACTS_DESCRIPTION", "FACTS_BODY\n"),
        },
        {
          relativePath: "system/human/prefs/coding.md",
          content: memory("PREFS_DESCRIPTION", "PREFS_BODY\n"),
        },
        {
          relativePath: "reference/details.md",
          content: memory("REFERENCE_DESCRIPTION", "EXTERNAL_BODY_SENTINEL\n"),
        },
        {
          relativePath: "archive/diagram.png",
          content: "BINARY_BODY_SENTINEL",
        },
        { relativePath: "README.MD", content: "UPPERCASE_BODY_SENTINEL" },
        {
          relativePath: "skills/deploy/SKILL.md",
          content: memory("SKILL_DESCRIPTION", "SKILL_BODY_SENTINEL\n"),
        },
      ]);
  ...
  it(
    "#given a committed persona and identity #when compiled #then both projection paths share the self section and metadata remains structured",
    async () => {
      // given
      const { repo } = await repoWith([
        {
        ...
        {
          relativePath: "system/identity.md",
          content: memory("IDENTITY_DESCRIPTION", "IDENTITY_BODY\n"),
        },
        {
          relativePath: "system/facts.md",
          content: memory("FACTS_DESCRIPTION", "FACTS_BODY\n"),
  ...
  it(
    "#given only a committed identity #when compiled #then it renders under self without a persona projection",
    async () => {
      // given
      const { repo } = await repoWith([
        {
  ...
  it(
    "#given an empty committed repository #when compiled #then only structured metadata is emitted",
    async () => {
      // given
      const { repo } = await repoWith([]);
  ...
  it(
    "#given only a committed persona #when compiled #then its body is projected without its description",
    async () => {
      // given
      const { repo } = await repoWith([
        {
          relativePath: "system/persona.md",
          content: memory("DESCRIPTION_SENTINEL", "PERSONA_BODY_SENTINEL\n"),
        },
      ]);
