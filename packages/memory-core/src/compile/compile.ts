...
async function readSystemFiles(
  ...
  const files = await Promise.all(
    paths.map((path) => readSystemFile(repo, revision, path)),
  );
...
function renderProjection(
  ...
  if (
    !persona && !identity && systemFiles.length === 0 &&
...
function isOtherSystemMarkdown(path: string): boolean {
  return path.startsWith("system/") && path !== PERSONA_PATH &&
    path !== IDENTITY_PATH && path.endsWith(".md");
}
