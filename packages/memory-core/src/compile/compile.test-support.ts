...
export function parseCompiledBlock(block: string): CompiledBlockStructure {
  const sections = [...block.matchAll(/^<(self|memory|memory_metadata)>$/gm)]
    .map((match) => match[1] ?? "");
  ...
  const projectionPaths = [
    ...block.matchAll(/<projection>\$MEMORY_DIR\/([^<]+)<\/projection>/g),
  ]
    .map((match) => match[1] ?? "");
  const memory = region(block, "memory");
  const memoryOpenTags = memory === undefined
    ? []
    : [...memory.matchAll(/^\s*<([a-z][a-z0-9_-]*)>$/gm)].map((match) =>
      match[1] ?? ""
    );
...
function requiredMatch(input: string, pattern: RegExp): string {
  ...
  if (value === undefined) {
    throw new Error(`missing compiled field: ${pattern.source}`);
  }
  return value;
}

export async function repoWith(
