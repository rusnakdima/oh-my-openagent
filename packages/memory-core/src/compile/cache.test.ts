...
import { hashMemoryTemplate, MemoryBlockCache } from "./cache";
...
async function createRepo() {
  const dir = realpathSync.native(
    await mkdtemp(join(tmpdir(), "memory-cache-")),
  );
  tempDirs.push(dir);
...
describe("MemoryBlockCache", () => {
  it(
    "#given the same template identity and HEAD #when compiled twice #then the stable projection is reused",
    async () => {
      // given
      ...
      const first = await cache.compile(
        repo,
        "raw prompt {CORE_MEMORY}",
        options,
      );
      const second = await cache.compile(
        repo,
        "raw prompt {CORE_MEMORY}",
        options,
      );
  ...
  it(
    "#given either template content or HEAD changes #when compiled #then each stable key retains only its latest revision",
    async () => {
      // given
  ...
  it(
    "#given repeated calls for one stable projection #when compile runs many times #then the cache stays bounded to one entry",
    async () => {
      // given
      const { repo } = await createRepo();
      const cache = new MemoryBlockCache();
      ...
      for (const iteration of [2, 3, 10, 100]) {
        try {
          ...
          const state = {
            ...
            headProbe: await repo.head().catch((probeError: unknown) =>
              String(probeError)
            ),
          };
          throw new Error(
            `compile failed at iteration ${iteration}: ${
  ...
  it(
    "#given two identities at the same HEAD #when compiled through one cache #then identity-stable projections remain isolated",
    async () => {
      // given
      const { repo } = await createRepo();
      const cache = new MemoryBlockCache();
      ...
      // when
      const first = await cache.compile(repo, "template", {
        agentId: "cache-agent",
      });
      const second = await cache.compile(repo, "template", {
        agentId: "other-agent",
      });
      const firstAgain = await cache.compile(repo, "template", {
        agentId: "cache-agent",
      });
  ...
  it("#given template content #when hashed #then the structure version participates in sha256", () => {
    ...
    expect(hashMemoryTemplate("template")).not.toBe(
      hashMemoryTemplate("template changed"),
    );
  });
});
