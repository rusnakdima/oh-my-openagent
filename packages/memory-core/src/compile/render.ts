...
export function renderSystemTree(files: readonly CompiledSystemFile[]): string {
  ...
  for (const file of files) {
    const parts = file.relativePath.replace(/^system\//, "").replace(
      /\.md$/,
      "",
...
function renderSystemNode(
  ...
  for (const [label, child] of entries) {
    ...
    if (child.file) {
      lines.push(
        `${pad}  <projection>${MEMORY_DIR}/system/${
          childParts.join("/")
        }.md</projection>`,
      );
...
function sentinelIdentity(block: string): string {
  ...
  if (!match?.[1]) {
    throw new Error("Memory block is missing a valid begin sentinel");
  }
  return match[1];
}
...
function sentinelPattern(identity: string): RegExp {
  return new RegExp(
    `<!-- senpi-memory:${identity}:begin -->[\\s\\S]*?<!-- senpi-memory:${identity}:end -->`,
    "g",
  );
}
