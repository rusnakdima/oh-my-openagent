...
const [modeValue, repoPath, locksPath, writerId, countValue, contentionPath] =
  process.argv.slice(2);
if (
  (modeValue !== "normal" && modeValue !== "kill-holder" &&
    modeValue !== "recovery") ||
  repoPath === undefined ||
  locksPath === undefined ||
  writerId === undefined ||
...
if (!Number.isInteger(count) || count < 1) {
  throw new Error(`invalid writer count: ${countValue}`);
}
...
function tasksForWriter(): WriterTask[] {
  ...
  return [
    ...Array.from(
      { length: count - 1 },
      (_, offset) => ({ writerId: "holder", index: offset + 1 }),
    ),
...
async function commit(task: WriterTask): Promise<void> {
  ...
  await runMemoryTool({
    ...
    params: {
      ...
      author: {
        agentId: `concurrency-${task.writerId}`,
        authorName: `Writer ${task.writerId}`,
      },
