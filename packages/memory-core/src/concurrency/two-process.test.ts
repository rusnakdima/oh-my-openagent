...
} from "../reflection/reservation";
import { realpathSync } from "node:fs";

const writerChildPath = fileURLToPath(
  new URL("./writer-child.ts", import.meta.url),
);
const reflectionChildPath = fileURLToPath(
  new URL("./reflection-child.ts", import.meta.url),
);
...
class ChildHarness {
  ...
  failureContext(): string {
    return `stdout=${JSON.stringify(this.lines)} stderr=${
      JSON.stringify(this.stderr)
    }`;
  }
...
async function fixture(): Promise<{
  ...
  const root = realpathSync.native(
    await mkdtemp(join(tmpdir(), "memory-two-process-")),
  );
...
async function assertTwentyLinearCommits(
  identity: MemoryIdentity,
): Promise<void> {
  expect(
    Number.parseInt(
      (await git(identity.paths.repo, ["rev-list", "--count", "HEAD"])).trim(),
      10,
    ),
  ).toBe(20);
  expect(
    Number.parseInt(
      (await git(identity.paths.repo, [
        "rev-list",
        "--count",
        "--merges",
        "HEAD",
...
describe("one memory identity across real Bun processes", () => {
  test(
    "#given two synchronized tool writers #when each makes ten writes #then twenty linear commits land with observed lock contention",
    async () => {
      // #given
      ...
      await Promise.all([
        first.waitForLine("ready"),
        second.waitForLine("ready"),
      ]);
      const holding = Promise.race([
        first.waitForLine("holding"),
        second.waitForLine("holding"),
      ]);
      const contended = Promise.race([
        first.waitForLine("contended"),
        second.waitForLine("contended"),
      ]);
      ...
      await Promise.all([
        first.waitForPrefix("done:"),
        second.waitForPrefix("done:"),
      ]);
      await Promise.all([expectSuccessful(first), expectSuccessful(second)]);

      // #then
      const contentionAttempts = (await readFile(shared.contentionPath, "utf8"))
        .split("\n").filter(Boolean);
  ...
  test(
    "#given two synchronized reflection reservers #when they race one identity #then one is active, one is pending, and completion promotes pending",
    async () => {
      // #given
      const shared = await fixture();
      const first = new ChildHarness(reflectionChildPath, [
        shared.root,
        shared.identity.id,
        "reflection-first",
      ]);
      const second = new ChildHarness(reflectionChildPath, [
        shared.root,
        shared.identity.id,
        "reflection-second",
      ]);
      ...
      const results = resultLines.map((line) =>
        JSON.parse(line.slice("result:".length)) as ReservationResult
      );
      ...
      const activeRecord = JSON.parse(
        await readFile(
          join(shared.identity.paths.reflection, "active.lock"),
          "utf8",
        ),
      ) as ReservationResult["run"];
      const pendingRecord = JSON.parse(
        await readFile(
          join(shared.identity.paths.reflection, "pending.json"),
          "utf8",
        ),
      ) as ReservationResult["run"];
      ...
      expect(
        ...
        ) as ReservationResult["run"]).runId,
      ).toBe(pendingRecord.runId);
    },
    30_000,
  );

  test(
    "#given a writer SIGKILLed while holding the shared lock #when its peer continues #then liveness takeover completes twenty clean commits",
    async () => {
      // #given
      const shared = await fixture();
      ...
      await Promise.all([
        holder.waitForLine("ready"),
        survivor.waitForLine("ready"),
      ]);
