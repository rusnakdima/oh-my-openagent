...
describe("facts extraction JSONL validation", () => {
  test(
    "#given valid project and person arms #when parsed #then scope remains a discriminated union",
    () => {
      // given
  ...
  test(
    "#given a project record carrying person #when parsed #then the whole extraction is rejected",
    () => {
      // given
      const raw = JSON.stringify({
        ...project(),
        person: { name: "Mina", aliases: [] },
      });
  ...
  test(
    "#given a person record missing person #when parsed #then the whole extraction is rejected",
    () => {
      // given
      const raw = JSON.stringify({
        scope: "person",
        text: "Mina prefers Bun.",
        date: "2026-08-10",
      });
...
describe("atomic facts batch application", () => {
  test(
    "#given project and resolved person records #when applied without a people policy #then one notes commit preserves both texts and trailers",
    async () => {
      // given
      ...
      if (result.outcome !== "committed") {
        throw new Error("expected a committed facts batch");
      }
      const memory = parseMemoryFile(
        await readFile(join(dir, "notes/facts/2026-08.md"), "utf8"),
      );
      expect(memory.body).toContain("- [2026-08-10] The project uses Bun.");
      expect(memory.body).toContain(
        "- [2026-08-10] Mina prefers concise reviews.",
      );
  ...
  test(
    "#given zero applicable records #when applied #then no commit is attempted",
    async () => {
      // given
  ...
  test(
    "#given git commit fails after staging #when the batch aborts #then index and working tree are restored",
    async () => {
      // given
      ...
      expect(
        await readFile(join(dir, "notes/facts/2026-08.md"), "utf8").catch(() =>
          undefined
        ),
      ).toBeUndefined();
    },
