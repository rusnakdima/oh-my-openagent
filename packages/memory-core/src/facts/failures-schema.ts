...
function requireNonNegativeInteger(value: unknown, field: string): number {
  if (typeof value !== "number" || !Number.isInteger(value) || value < 0) {
    throw new FactsFailuresCorruptError(
...
function requireReason(value: unknown): FactsFailureReason {
  ...
  if (typeof value !== "string" || !candidates.includes(value)) {
    throw new FactsFailuresCorruptError(
      `lastReason "${String(value)}" is not a known reason`,
    );
  }
...
function requireState(value: unknown): FactsFailureState {
  if (value !== "backoff" && value !== "parked") {
    throw new FactsFailuresCorruptError(
      `state "${String(value)}" is not backoff or parked`,
    );
  }
  return value;
}
...
function parseRecord(value: unknown): FactsFailureRecord {
  if (!isRecord(value)) {
    throw new FactsFailuresCorruptError("entries must contain objects");
  }
  const state = requireState(value.state);
  const nextEligibleAt = value.nextEligibleAt === null
    ? null
    : requireInstant(value.nextEligibleAt, "nextEligibleAt");
  const parkedAt = value.parkedAt === undefined
    ? undefined
    : requireInstant(value.parkedAt, "parkedAt");
  ...
  if (
    state === "parked" && (nextEligibleAt !== null || parkedAt === undefined)
  ) {
    throw new FactsFailuresCorruptError(
  ...
  if (
    state === "backoff" && (nextEligibleAt === null || parkedAt !== undefined)
  ) {
    throw new FactsFailuresCorruptError(
  ...
  if (streak < 1) {
    throw new FactsFailuresCorruptError("streak must be at least 1");
  }
  const record: FactsFailureRecord = {
    ...
    end_message_id: requireNonEmptyString(
      value.end_message_id,
      "end_message_id",
    ),
    end_snapshot_line: requireNonNegativeInteger(
      value.end_snapshot_line,
      "end_snapshot_line",
    ),
    state,
    streak,
...
export function parseFailuresFile(raw: string): FactsFailuresFile {
  ...
  try {
    ...
    throw new FactsFailuresCorruptError(
      error instanceof Error ? error.message : "unreadable JSON",
    );
  }
  if (!isRecord(parsed)) {
    throw new FactsFailuresCorruptError("root must be an object");
  }
  if (parsed.version !== FACTS_FAILURES_VERSION) {
    throw new FactsFailuresCorruptError(
      `unsupported version ${String(parsed.version)}`,
    );
  }
  if (!Array.isArray(parsed.entries)) {
    throw new FactsFailuresCorruptError("entries must be an array");
  }
...
export function sortFailureRecords(
...
export function renderFailuresFile(
  entries: readonly FactsFailureRecord[],
  updatedAt: string,
): string {
