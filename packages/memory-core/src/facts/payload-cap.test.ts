...
function endpoints(selected: readonly FactsQueueEntry[]): readonly string[] {
  return selected.map((candidate) =>
    `${candidate.conversationId}/${candidate.range.end_message_id}`
  );
}
...
function entrySizedTo(
  ...
  for (let attempt = 0; attempt < 64; attempt += 1) {
    const candidate = entry(conversationId, endMessageId, endSnapshotLine, {
      text: "x".repeat(padding),
    });
    const bytes = measureFactsPayloadBytes({
      ...ENVELOPE,
      entries: [candidate],
    });
    ...
    if (bytes > target) {
      throw new Error(
        `cannot size entry to ${target} bytes (overshot at ${bytes})`,
      );
    }
...
describe("facts payload measurement", () => {
  test("#given a payload #when it is measured #then the bytes equal the shared serializer's utf8 length", () => {
    ...
    const payload: FactsPayload = {
      ...ENVELOPE,
      entries: [entry("alpha", "m1", 1, { text: "café ☕" })],
    };
    ...
    expect(serializeFactsPayload(payload)).toBe(
      `${JSON.stringify(payload, null, 2)}\n`,
    );
    expect(measured).toBe(
      Buffer.byteLength(serializeFactsPayload(payload), "utf8"),
    );
...
describe("facts capped batch selection", () => {
  test("#given a payload exactly at the cap #when selecting #then the entry is accepted, and one byte more is rejected", () => {
    ...
    // when
    const accepted = selectCappedFactsBatch({
      entries: [exact],
      envelope: ENVELOPE,
      now: T0,
    });
    const rejected = selectCappedFactsBatch({
      entries: [over],
      envelope: ENVELOPE,
      now: T0,
    });
    ...
    // then
    expect(measureFactsPayloadBytes({ ...ENVELOPE, entries: [exact] })).toBe(
      MAX_FACTS_PAYLOAD_BYTES,
    );
    expect(endpoints(accepted.selected)).toEqual(["alpha/m1"]);
    expect(measureFactsPayloadBytes({ ...ENVELOPE, entries: [over] })).toBe(
      MAX_FACTS_PAYLOAD_BYTES + 1,
    );
    expect(rejected.selected).toEqual([]);
    expect(
      rejected.oversized.map((candidate) => candidate.range.end_message_id),
    ).toEqual(["m1"]);
  });
  ...
  test("#given a conversation whose newest entry fits but whose older one does not #when selecting #then prefix closure ships neither", () => {
    ...
    expect(
      measureFactsPayloadBytes({ ...ENVELOPE, entries: selection.selected }),
    )
  ...
  test("#given a middle entry that does not fit #when a tiny entry follows it #then the follower is held back too", () => {
    ...
    const budget =
      measureFactsPayloadBytes({ ...ENVELOPE, entries: [first, last] }) + 100;
    ...
    expect(measureFactsPayloadBytes({ ...ENVELOPE, entries: [middle] }))
  ...
  test("#given an entry waiting past 24 hours #when newer conversations compete for the cap #then the starved one is selected first", () => {
    ...
    const fresh = entry("conv-b", "mb", 1, {
      text: "rival",
      enqueuedAt: new Date(T0.getTime() - 60_000),
    });
    const budget = measureFactsPayloadBytes({
      ...ENVELOPE,
      entries: [starved],
    });
    ...
    // then
    expect(measureFactsPayloadBytes({ ...ENVELOPE, entries: [fresh] })).toBe(
      budget,
    );
  ...
  test("#given an entry waiting just under 24 hours #when newer entries compete #then the newest wins the budget", () => {
    ...
    // then
    expect(measureFactsPayloadBytes({ ...ENVELOPE, entries: [almostStarved] }))
      .toBe(budget);
  ...
  test("#given a multi-conversation batch #when everything fits #then entries ship ascending per conversation", () => {
    ...
    const alpha = selection.selected.filter((candidate) =>
      candidate.conversationId === "alpha"
    );
    const beta = selection.selected.filter((candidate) =>
      candidate.conversationId === "beta"
    );
    expect(alpha.map((candidate) => candidate.range.end_snapshot_line)).toEqual(
      [3, 4],
    );
    expect(beta.map((candidate) => candidate.range.end_snapshot_line)).toEqual([
      1,
      2,
    ]);
  ...
  test("#given an envelope that already exceeds the cap #when selecting #then nothing ships", () => {
    ...
    const envelope = {
      ...ENVELOPE,
      identity: "x".repeat(MAX_FACTS_PAYLOAD_BYTES),
    };

    // when
    const selection = selectCappedFactsBatch({
      entries: [entry("alpha", "m1", 1)],
      envelope,
      now: T0,
    });
