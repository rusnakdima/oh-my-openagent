...
export async function planFactsMutation(
  ...
  const routing = await planFactsRouting(
    repo.dir,
    batch.records,
    people,
    onAliasTie,
  ...
  if ((await repo.status()).trim().length > 0) {
    throw new FactsPlanParentDirtyError(
      "facts repository changed during planning",
    );
  }
  ...
  for (const path of paths) {
    ...
    if (before === undefined || value === undefined) {
      throw new Error(`Incomplete facts mutation plan: ${path}`);
    }
...
export function factsRecordsHash(
...
async function renderFactsNote(
  ...
  const existing = await readFile(join(root, relativePath), "utf8").catch(() =>
    undefined
  );
  const parsed = existing === undefined
    ? {
      frontmatter: {
        description: `Explicit facts for ${relativePath.slice(-10, -3)}`,
      },
      body: "",
    }
  ...
  const prefix = parsed.body.length === 0 || parsed.body.endsWith("\n")
    ? ""
    : "\n";
  const bullets = records.map((record) => `- [${record.date}] ${record.text}`)
    .join("\n");
  return renderMemoryFile(
    parsed.frontmatter,
    `${parsed.body}${prefix}${bullets}\n`,
  );
}
