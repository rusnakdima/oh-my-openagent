...
import { type ObservationEntry, parsePeopleCard } from "../people";
...
async function fixture(
  exec?: GitExec,
): Promise<{ readonly dir: string; readonly repo: GitMemoryRepo }> {
  const dir = realpathSync.native(
    await mkdtemp(join(tmpdir(), "memory-facts-extraction-")),
  );
  tempDirs.push(dir);
...
async function commitObservations(
  ...
  await repo.commitWrite(
    ...
    `test: seed ${slug} observations`,
    AUTHOR,
  );
}

async function readExplicitEntries(
  dir: string,
  slug: string,
): Promise<readonly ObservationEntry[]> {
  const memory = parseMemoryFile(
    await readFile(join(dir, "people", slug, "observations.md"), "utf8"),
  );
  const { card, diagnostics } = parsePeopleCard(memory.body, PEOPLE_LIMITS);
  expect(diagnostics).toEqual([]);
  return card.observations?.find((group) => group.section === "Explicit")
    ?.entries ?? [];
}

describe("person routing reinforcement", () => {
  test(
    "#given an existing explicit observation #when a textually equal fact arrives #then n increments and the date refreshes without a duplicate line",
    async () => {
      // given
      const { dir, repo } = await fixture();
      await commitPersonCard(repo, "mina", "Mina", ["Min"]);
      await commitObservations(
        repo,
        "mina",
        "Mina",
      ...
      // when
      await applyFactsBatch(
        repo,
        {
          batchId: "88888888-8888-4888-8888-888888888888",
          records: [
            personRecord("Mina", [], "mina   prefers concise  reviews"),
          ],
        },
        AUTHOR,
        { people: PEOPLE },
      ...
      expect(entries[0]).toMatchObject({
        date: "2026-08-10",
        content: "Mina prefers concise reviews.",
        n: 2,
      });
  ...
  test(
    "#given an existing explicit observation #when a different fact arrives #then it appends a separate line",
    async () => {
      // given
      const { dir, repo } = await fixture();
      await commitPersonCard(repo, "mina", "Mina", ["Min"]);
      ...
      expect(entries[1]).toMatchObject({
        date: "2026-08-10",
        content: "Mina prefers dark themes.",
      });
    },
    30_000,
  );
...
describe("person routing new people", () => {
  test(
    "#given no matching card #when a confidently named person fact arrives #then a card skeleton and ledger are created",
    async () => {
      // given
      const { dir, repo } = await fixture();

      // when
      const result = await applyFactsBatch(
        repo,
        {
      ...
      if (result.outcome === "committed") {
        expect(result.affectedPaths).toEqual([
          "people/yeongyu/card.md",
          "people/yeongyu/observations.md",
        ]);
      }
      const card = parseMemoryFile(
        await readFile(join(dir, "people", "yeongyu", "card.md"), "utf8"),
      );
      ...
      expect(
        (await readExplicitEntries(dir, "yeongyu")).map((entry) =>
          entry.content
        ),
      )
  ...
  test(
    "#given a slug already taken by a different person #when a new person sanitizes to it #then a numeric suffix avoids the collision",
    async () => {
      // given
      const { dir, repo } = await fixture();
      ...
      await applyFactsBatch(
        repo,
        {
          ...
          records: [
            personRecord("Yeongyu", [], "Yeongyu joined the platform guild."),
          ],
        },
        AUTHOR,
        { people: PEOPLE },
      ...
      // then
      const card = parseMemoryFile(
        await readFile(join(dir, "people", "yeongyu-2", "card.md"), "utf8"),
      );
      expect(card.frontmatter).toMatchObject({
        description: "Person - Yeongyu",
        kind: "person",
      ...
      expect(
        (await readExplicitEntries(dir, "yeongyu-2")).map((entry) =>
          entry.content
        ),
      )
      ...
      expect(
        await readFile(
          join(dir, "people", "yeongyu", "observations.md"),
          "utf8",
        ).catch(() => undefined),
      )
        .toBeUndefined();
  ...
  test(
    "#given two facts about the same new person in one batch #when applied #then one card receives both entries",
    async () => {
      // given
      const { dir, repo } = await fixture();
