...
import { type FactsApplyRecovery, planFactsMutation } from "./mutation-plan";
import { applyFactsRecovery } from "./recovery";

const AUTHOR = {
  agentId: "facts-reservation-race",
  authorName: "Facts Reservation Race",
};
...
async function deletionRecovery(
  repo: GitMemoryRepo,
  ...
  if (current.index === null || current.worktree.kind !== "file") {
    throw new Error("expected August");
  }
  return {
    ...planned,
    paths: planned.paths.map((entry) =>
      entry.path === august
        ? {
          ...
          post: {
            index: current.index!,
            worktree: current.worktree as Extract<
              typeof current.worktree,
              { kind: "file" }
            >,
          },
        }
...
function expectNoReceipt(repo: GitMemoryRepo, batchId: string): Promise<void> {
  return repo.log().then((commits) => {
    expect(
      commits.some((commit) => commit.trailers["Omo-Facts-Batch"] === batchId),
    ).toBe(false);
  });
}
...
describe("facts deletion reservation finalization", () => {
  test(
    "preserves a foreign directory replacing the verified reservation",
    async () => {
      ...
      const originalWrite = repo.pathState.writeWorktreeIfIdentity.bind(
        repo.pathState,
      );
      ...
      const originalReserve = repo.pathState.reserveWorktreeDeletion.bind(
        repo.pathState,
      );
      ...
      if (
        result.outcome !== "parent_dirty" || result.detail === undefined
      ) throw new Error("missing recovery detail");
      const retained = await lstat(
        result.detail.replace("Foreign entry retained at ", ""),
      );
      expect(retained.isDirectory()).toBe(true);
      if (foreignIdentity === undefined) {
        throw new Error(
          "foreign directory was not installed",
        );
      }
  ...
  test(
    "does not overwrite a newer foreign file while restoring a mismatched claim",
    async () => {
      const { dir, repo } = await fixture();
      const recovery = await deletionRecovery(repo);
      const august = "notes/facts/2026-08.md";
      ...
      const originalRestore = repo.pathState.restoreWorktreeClaim.bind(
        repo.pathState,
      );
      ...
      if (result.outcome !== "parent_dirty" || result.detail === undefined) {
        throw new Error("missing recovery detail");
      }
      const retained = result.detail.replace("Foreign entry retained at ", "");
      expect(await readFile(retained, "utf8")).toBe("foreign A\n");
      expect((await repo.pathState.capture(august)).index).toEqual(
        indexBefore.get(august)?.index ?? null,
      );
      expect((await repo.pathState.capture(september)).index).toEqual(
        indexBefore.get(september)?.index ?? null,
      );
      expect(await repo.pathState.capture(september)).toEqual(
        recovery.paths.find((entry) => entry.path === september)!.pre,
      );
      await expectNoReceipt(repo, recovery.batchId);
    },
    WINDOWS_INTEGRATION_TEST_TIMEOUT,
  ...
  test(
    "does not overwrite a newer foreign directory while restoring a mismatched claim",
    async () => {
      const { dir, repo } = await fixture();
      const recovery = await deletionRecovery(repo);
      const august = "notes/facts/2026-08.md";
      ...
      if (foreignB === undefined) {
        throw new Error("foreign directory B was not installed");
      }
  ...
  test(
    "preserves a foreign file replacing the verified reservation and returns parent_dirty",
    async () => {
      const { dir, repo } = await fixture();
      const recovery = await deletionRecovery(repo);
      const august = "notes/facts/2026-08.md";
