...
export function factsQueuePaths(
  ...
  return {
    ...
    cursorPath: (conversationId) =>
      join(cursorDir, `${hash(conversationId, 12)}.json`),
    entryPath: (conversationId, endMessageId, at) =>
      join(
        queueDir,
        `${queueTimestamp(at)}-${hash(conversationId, 12)}-${
          hash(endMessageId, 8)
        }.json`,
      ),
  };
...
function parseRange(value: unknown): FactsQueueRange | undefined {
  ...
  if (
    start === undefined || end === undefined || startLine === undefined ||
    endLine === undefined
  ) {
    return undefined;
  }
...
export function parseQueueEntry(raw: string): FactsQueueEntry | undefined {
  ...
  if (!isRecord(parsed) || parsed.version !== FACTS_QUEUE_VERSION) {
    return undefined;
  }
  ...
  const entries = parsed.entries.filter((row): row is TranscriptEntry =>
    isTranscriptEntry(row)
  );
...
export function parseCursor(raw: string): FactsCursor {
  ...
  return {
    version: FACTS_QUEUE_VERSION,
    enqueued_through_message_id: nullableString(
      parsed.enqueued_through_message_id,
    ),
    enqueued_through_snapshot_line:
      nonNegativeInteger(parsed.enqueued_through_snapshot_line) ?? -1,
    consumed_through_message_id: nullableString(
      parsed.consumed_through_message_id,
    ),
    consumed_through_snapshot_line:
      nonNegativeInteger(parsed.consumed_through_snapshot_line) ?? -1,
  };
}

export function parseConsumed(raw: string): FactsConsumedWatermark {
  const empty: FactsConsumedWatermark = {
    version: FACTS_QUEUE_VERSION,
    consumed: {},
  };
  let parsed: unknown;
  ...
  if (
    !isRecord(parsed) || parsed.version !== FACTS_QUEUE_VERSION ||
    !isRecord(parsed.consumed)
  ) {
    return empty;
  }
...
export function canonicalPosition(
  ...
  for (let index = entries.length - 1; index >= 0; index -= 1) {
    ...
    if (
      entry && isCanonicalEntry(entry) && entry.source_message_id === messageId
    ) return index;
  }
  return -1;
}
