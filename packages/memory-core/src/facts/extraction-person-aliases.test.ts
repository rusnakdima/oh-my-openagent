...
describe("person routing alias resolution", () => {
  test(
    "#given an existing card with a known alias #when a person fact names that alias #then it lands in the card ledger and no new directory appears",
    async () => {
      // given
      ...
      const result = await applyFactsBatch(
        repo,
        {
          ...
          records: [
            personRecord("Min", [], "Min is reviewing the memory plan."),
          ],
        },
        AUTHOR,
        { people: PEOPLE },
  ...
  test(
    "#given two cards whose aliases both match #when resolved #then the longest matching alias wins",
    async () => {
      // given
      const { dir, repo } = await fixture();
      ...
      // then
      expect(
        (await readExplicitEntries(dir, "mina-lee")).map((entry) =>
          entry.content
        ),
      )
      ...
      expect(
        await readFile(
          join(dir, "people", "mina-kim", "observations.md"),
          "utf8",
        ).catch(() => undefined),
      )
        .toBeUndefined();
  ...
  test(
    "#given two cards with equal-length matching aliases #when resolved #then the smallest slug wins and the tie is logged",
    async () => {
      // given
      const { dir, repo } = await fixture();
      ...
      await applyFactsBatch(
        ...
        AUTHOR,
        { people: PEOPLE, onAliasTie: (tie) => ties.push(tie) },
      );

      // then
      expect(
        (await readExplicitEntries(dir, "aaa-min")).map((entry) =>
          entry.content
        ),
      )
  ...
  test(
    "#given the primary human card carries an alias #when a fact names it #then it routes to the human ledger without a new card",
    async () => {
      // given
      const { dir, repo } = await fixture();
      await writeFile(
      ...
      expect(
        (await readExplicitEntries(dir, "human")).map((entry) => entry.content),
      )
