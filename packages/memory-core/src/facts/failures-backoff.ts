...
function nextRecord(
  ...
  const parks = previous?.state === "parked" ||
    input.reason === "payload_entry_oversize" || streak >= PARK_AT_STREAK;
  const base: FactsFailureRecord = {
    ...
    nextEligibleAt: parks
      ? null
      : new Date(input.now.getTime() + delayFor(streak)).toISOString(),
  };
  return {
    ...base,
...
export function applyFailure(
  ...
  const byKey = new Map(
    input.entries.map((record) => [recordKey(record), record]),
  );
...
export function clearForRetry(
  ...
  return entries.filter((record) => {
    ...
    if (
      filter.endMessageId !== undefined &&
      record.end_message_id !== filter.endMessageId
    ) return true;
    return false;
  });
}
