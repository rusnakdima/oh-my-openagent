...
type MutationRecord =
  | {
    ...
    readonly before: GitIndexIdentity | null;
    readonly after: GitIndexIdentity | null;
  }
  | {
    readonly path: string;
    readonly surface: "worktree";
    readonly before: GitWorktreeIdentity;
    readonly after: GitWorktreeIdentity;
  };
...
export class FactsMutationTransaction {
  ...
  async restorePreState(): Promise<void> {
    for (const entry of this.recovery.paths) {
      await this.compareAndSetIndex(
        entry.path,
        this.initialState(entry.path).index,
        entry.pre.index,
      );
    }
    for (const entry of this.recovery.paths) {
      await this.compareAndSetWorktree(
        entry.path,
        this.initialState(entry.path).worktree,
        entry.pre.worktree,
      );
    }
  }
  ...
  async rollback(): Promise<void> {
    for (const mutation of [...this.mutations].reverse()) {
      if (mutation.surface === "index") {
        await this.repo.pathState.writeIndexIfIdentity(
          mutation.path,
          mutation.after,
          mutation.before,
        );
      } else {
        await this.repo.pathState.writeWorktreeIfIdentity(
          mutation.path,
          mutation.after,
          mutation.before,
        );
  ...
  private initialState(path: string): GitPathState {
    ...
    if (state === undefined) {
      throw new FactsOwnershipLostError(`Missing initial facts state: ${path}`);
    }
    return state;
  }
  ...
  private async compareAndSetIndex(
    ...
    if (!await this.repo.pathState.writeIndexIfIdentity(path, expected, next)) {
      throw new FactsOwnershipLostError(
        `Facts ownership changed: ${path}:index`,
      );
    }
    this.mutations.push({
      path,
  ...
  private async compareAndSetWorktree(
    ...
    if (sameIdentity(expected, next)) return;
    const written = await this.repo.pathState.writeWorktreeIfIdentity(
      path,
      expected,
      next,
    );
