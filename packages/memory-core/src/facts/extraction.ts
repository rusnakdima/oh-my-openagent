...
export function parseFactsExtractionJsonl(
  raw: string,
...
export async function applyFactsBatch(
  ...
  if (batch.records.length === 0) {
    return { outcome: "no_facts", affectedPaths: [] };
  }
  try {
    ...
    if (error instanceof FactsPlanParentDirtyError) {
...
export function validateFactsRecovery(
  ...
  if (
    ...
    recovery.paths.some((entry, index) =>
      index > 0 && recovery.paths[index - 1]!.path >= entry.path
    )
  ) {
...
function parseRecord(value: unknown, index: number): FactsExtractionRecord {
  ...
  if (value.scope === "project") {
    if ("person" in value) {
      throw invalid(index, "project record must not carry person");
    }
  ...
  if (!("person" in value)) {
    throw invalid(index, "person record requires person");
  }
  ...
  return {
    scope: "person",
    person: { name, aliases: aliases as string[] },
    text,
    date,
  };
}
...
function invalid(
  ...
  return new FactsExtractionValidationError(
    `facts extraction line ${index + 1}: ${message}`,
  );
}
