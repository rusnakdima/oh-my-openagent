...
import {
  type FactsPeopleIndexEntry,
  readFactsPeopleIndex,
} from "./person-index";
...
export async function planFactsRouting(
  ...
  for (const record of records) {
    ...
    if (slug === undefined) {
      slug = resolveSlugCollision(
        sanitizePersonSlug(record.person.name),
        taken,
      );
      taken.add(slug);
      index.push({
    ...
    const bucket = observations.get(slug) ?? {
      target: {
        slug,
        displayName: index.find((entry) => entry.slug === slug)?.displayName ??
          slug,
        isNew: false,
      },
      records: [],
...
export async function renderPersonTargets(
  ...
  for (const [slug, bucket] of plan.observations) {
    if (bucket.target.isNew) {
      content.set(`people/${slug}/card.md`, renderCardSkeleton(bucket.target));
    }
    content.set(
      `people/${slug}/observations.md`,
      await renderExplicitObservations(
        repoDir,
        slug,
        bucket.target.displayName,
        bucket.records,
        limits,
      ),
    );
...
function renderCardSkeleton(target: FactsPersonTarget): string {
  if (target.person === undefined) {
    throw new Error(`New person target lacks identity: ${target.slug}`);
  }
...
async function renderExplicitObservations(
  ...
  const parsed = existing === undefined
    ? {
      frontmatter: { description: `Observations - ${displayName}` },
      body: "",
    }
  ...
  const body = serializePeopleCard({
...
function resolvePersonSlug(
  ...
  const longest = Math.max(
    ...[...matches.values()].map((match) => match.length),
  );
  const winners = [...matches.entries()].filter(([, match]) =>
