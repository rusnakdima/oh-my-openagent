...
import { createNodeGitExec, type GitExec, GitMemoryRepo } from "../git";
...
const AUTHOR = {
  agentId: "facts-setter-race",
  authorName: "Facts Setter Race",
};
...
async function fixture() {
  ...
  const exec: GitExec = {
    run: async (args, options) => {
      ...
      if (
        injection !== undefined && args[0] === "hash-object" &&
  ...
  return {
    dir,
    repo,
    injectAfterNextWorktreeHash: (operation: () => Promise<void>) => {
      injection = operation;
    },
  };
}
...
describe("facts conditional mutation primitives", () => {
  test(
    "preserves a foreign replacement injected after hashing an existing tracked path",
    async () => {
      ...
      repo.pathState.writeWorktreeIfIdentity = async (path, expected, next) => {
        ...
        injectAfterNextWorktreeHash(() =>
          writeFile(join(dir, august), "foreign existing replacement\n")
        );
      ...
      expect(await readFile(join(dir, august), "utf8")).toBe(
  ...
  test(
    "preserves a foreign write injected after deletion reservation resolves",
    async () => {
      ...
      const planned = await planFactsMutation(
        repo,
        batch("22222222-2222-4222-8222-222222222222"),
        undefined,
      );
      ...
      if (
        currentAugust.index === null || currentAugust.worktree.kind !== "file"
        ...
        paths: planned.paths.map((entry) =>
          entry.path === august
            ? {
              ...
              post: {
                index: currentAugust.index!,
                worktree: currentAugust.worktree as Extract<
                  typeof currentAugust.worktree,
                  { kind: "file" }
                >,
              },
            }
        ...
        return reservation === undefined ? undefined : {
          verify: async () => {
            try {
              await writeFile(
                join(dir, august),
                "foreign after deletion reservation\n",
              );
            } catch (error) {
              if (
                !(error instanceof Error) || !("code" in error) ||
                !["EISDIR", "EEXIST"].includes(String(error.code))
              ) throw error;
  ...
  test("preserves foreign bytes injected after hashing an existing path for deletion", async () => {
    ...
    if (expected.kind !== "file") {
      throw new Error("expected tracked facts file");
    }
    ...
    injectAfterNextWorktreeHash(() =>
      writeFile(join(dir, path), "foreign existing deletion\n")
    );

    const removed = await repo.pathState.writeWorktreeIfIdentity(
      path,
      expected,
      { kind: "missing" },
    );
