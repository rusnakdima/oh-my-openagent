...
export async function readFactsPeopleIndex(
  repoDir: string,
): Promise<FactsPeopleIndexEntry[]> {
  ...
  const entries = await readdir(join(repoDir, "people"), {
    withFileTypes: true,
  }).catch(() => []);
  for (const entry of entries) {
    if (!entry.isDirectory() || entry.name === "human") continue;
    const card = await readCardFrontmatter(
      join(repoDir, "people", entry.name, "card.md"),
    );
...
async function readCardFrontmatter(
  ...
  | { readonly description: string; readonly aliases: readonly string[] }
  | undefined
> {
