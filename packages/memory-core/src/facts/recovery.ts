...
import {
  FactsMutationTransaction,
  FactsOwnershipLostError,
} from "./recovery-mutation";
import {
  captureOwnedFactsState,
  sameFactsOwnedState,
...
export async function findFactsBatchReceipt(
  ...
  return (await repo.log()).find((commit) =>
    commit.trailers["Omo-Facts-Batch"] === batchId
  );
}
...
export async function applyFactsRecovery(
  ...
  if (boundary === undefined || !sameFactsOwnedState(observed, boundary)) {
  ...
  try {
    ...
    return {
      outcome: "committed",
      sha: result.sha,
      affectedPaths: recovery.paths.map((entry) => entry.path),
    };
  } catch (error) {
    await transaction.rollback();
    if (error instanceof FactsOwnershipLostError) {
      return {
        outcome: "parent_dirty",
        ...(error.recoveryPath === undefined
          ? {}
          : { detail: `Foreign entry retained at ${error.recoveryPath}` }),
      };
    }
    throw error;
  }
