...
import { type FactsQueueLayout, factsQueuePaths } from "./schema";

const LOCK_WAIT_MS = 2000;

/** Windows cannot fsync a directory handle; absent files are already durable. */
const SKIPPABLE_SYNC_CODES = new Set([
  "ENOENT",
  "EISDIR",
  "EPERM",
  "EACCES",
...
export class FactsFailureStore {
  ...
  private async readUnlocked(): Promise<FactsFailuresFile> {
    const raw = await readFile(this.layout.failuresPath, "utf8").catch(
      (error: unknown) => {
  ...
  private async writeAtomically(content: string): Promise<void> {
    ...
    const temporary = join(
      directory,
      `.failures-${process.pid}-${randomUUID()}.tmp`,
    );
