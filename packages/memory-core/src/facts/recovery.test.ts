...
import { createNodeGitExec, GitMemoryRepo } from "../git";
...
async function blobExists(dir: string, oid: string): Promise<boolean> {
  return (await createNodeGitExec().run(["cat-file", "-e", oid], {
    cwd: dir,
    timeoutMs: 30_000,
  })).code === 0;
}
...
describe("facts path-state recovery ownership", () => {
  test(
    "rejects a foreign edit injected immediately before initial pre-state capture",
    async () => {
      ...
      let injectedState:
        | Awaited<ReturnType<typeof originalCaptureAll>>
        | undefined;
      ...
      if (injectedState === undefined) {
        throw new Error("capture injection did not run");
      }
      const capturedInjection = injectedState.get(path);
      if (capturedInjection === undefined) {
        throw new Error("injected path was not captured");
      }
      ...
      expect(
        (await repo.log()).filter((commit) =>
          commit.trailers["Omo-Facts-Batch"] === next.batchId
        ),
      ).toHaveLength(0);
    },
    30_000,
  ...
  test(
    "rejects tracked worktree drift without changing index or worktree bytes",
    async () => {
      const { dir, repo } = await fixture();
      await applyFactsBatch(repo, batch(), AUTHOR);
      const next = batch([{
      ...
      expect((await applyFactsRecovery(repo, recovery, 1, AUTHOR)).outcome)
        .toBe("parent_dirty");
  ...
  test(
    "rejects tracked index drift without rewriting either surface",
    async () => {
      ...
      const recovery = await planFactsMutation(repo, next, undefined);
      const path = recovery.paths[0]!.path;
      const foreignOid = await repo.pathState.hashIndexBlob(
        path,
  ...
  test(
    "rejects foreign creation at a planned new path without deleting it",
    async () => {
      const { dir, repo } = await fixture();
  ...
  test(
    "rejects a foreign creation injected after bulk validation and before mutation",
    async () => {
      const { dir, repo } = await fixture();
      const recovery = await planFactsMutation(
        repo,
        batch([
      ...
      const originalCaptureAll = repo.pathState.captureAll.bind(repo.pathState);
      const indexBefore = new Map(
        paths.map((path) => [
          path,
          recovery.paths.find((entry) => entry.path === path)!.pre.index,
        ]),
      );
      ...
      expect(result.outcome).toBe("parent_dirty");
      expect(await readFile(join(dir, foreignPath), "utf8")).toBe(
        "foreign after validation\n",
      );
      for (const path of paths) {
        expect((await repo.pathState.capture(path)).index).toEqual(
          indexBefore.get(path) ?? null,
        );
      }
      for (const path of paths.slice(1)) {
        expect(await repo.pathState.capture(path)).toEqual(
          recovery.paths.find((entry) => entry.path === path)!.pre,
        );
      }
      expect(
        (await repo.log()).filter((commit) =>
          commit.trailers["Omo-Facts-Batch"] === recovery.batchId
        ),
      ).toHaveLength(0);
    },
    30_000,
  ...
  test(
    "preserves every path when one member of a multi-path batch is foreign",
    async () => {
      const { dir, repo } = await fixture();
      const recovery = await planFactsMutation(
        repo,
      ...
      const before = await repo.pathState.captureAll(
        recovery.paths.map((entry) => entry.path),
      );

      expect((await applyFactsRecovery(repo, recovery, 2, AUTHOR)).outcome)
        .toBe("parent_dirty");
      expect(await repo.pathState.captureAll([...before.keys()])).toEqual(
        before,
      );
    },
    30_000,
  ...
  test(
    "restores owned partial pre/post residue and commits the exact recorded post state",
    async () => {
      const { repo } = await fixture();
      const recovery = await planFactsMutation(
        repo,
      ...
      expect((await repo.log({ limit: 1 }))[0]?.trailers["Omo-Facts-Batch"])
        .toBe(recovery.batchId);
    },
    30_000,
  );

  test(
    "durably inserts every pre and post identity before envelope publication",
    async () => {
      const { dir, repo } = await fixture();
      ...
      await repo.commitWrite(
        [".gitattributes", "notes/facts/2026-08.md"],
        "seed filtered facts",
        AUTHOR,
      );
      const result = await applyFactsBatch(repo, batch(), AUTHOR, {
        publishRecovery: async (recovery) => {
          const oids = recovery.paths.flatMap((entry) => [
            ...
            ...(entry.pre.worktree.kind === "missing"
              ? []
              : [entry.pre.worktree.oid]),
            entry.post.index.oid,
            entry.post.worktree.oid,
          ]);
          expect(recovery.paths[0]?.pre.index?.oid).not.toBe(
            recovery.paths[0]?.pre.worktree.kind === "file"
              ? recovery.paths[0].pre.worktree.oid
              : undefined,
          );
  ...
  test(
    "publishes a complete blob-backed envelope before the first affected-path mutation",
    async () => {
      const { dir, repo } = await fixture();
      let published = false;
      const result = await applyFactsBatch(repo, batch(), AUTHOR, {
        publishRecovery: async (recovery) => {
          published = true;
          expect(recovery.paths.map((entry) => entry.path)).toEqual(
            [...recovery.paths].map((entry) => entry.path).sort(),
          );
          for (const entry of recovery.paths) {
