...
export function factsSelectionKey(
...
function dropReason(
  record: FactsFailureRecord | undefined,
  now: Date,
): FactsSkipReason | undefined {
  ...
  return now.getTime() < Date.parse(record.nextEligibleAt)
    ? "backoff"
    : undefined;
}
...
export function selectLaunchable(
  ...
  const records = new Map(
    (failures?.entries ?? []).map((record) => [recordKey(record), record]),
  );
  ...
  for (const bucket of byConversation.values()) {
    const ordered = [...bucket].sort(
      (left, right) =>
        left.range.end_snapshot_line - right.range.end_snapshot_line,
    );
    let blocked = false;
    for (const entry of ordered) {
      const reason = blocked
        ? "blocked-by-predecessor"
        : dropReason(records.get(entryKey(entry)), now);
  ...
  return {
    selected: entries.filter((entry) => skipped[entryKey(entry)] === undefined),
    skipped,
  };
}
