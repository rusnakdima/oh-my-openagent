...
describe("person routing gate and unresolved mentions", () => {
  test(
    "#given people routing is disabled #when a person fact arrives #then it falls back to the monthly notes file",
    async () => {
      // given
      ...
      await applyFactsBatch(
        ...
        AUTHOR,
        { people: { enabled: false, maxEntries: 40, maxEntryChars: 200 } },
      );
  ...
  test(
    "#given an unresolved person mention #when applied #then the prefixed bullet is stored verbatim in monthly notes",
    async () => {
      // given
      const { dir, repo } = await fixture();
      ...
      expect(memory.body).toContain(
        "- [2026-08-10] person-unresolved: A teammate said the launch slips.",
      );
