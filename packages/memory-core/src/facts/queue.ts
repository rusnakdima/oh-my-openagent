...
export class FactsQueue {
  ...
  async enqueue(request: FactsEnqueueRequest): Promise<FactsEnqueueResult> {
    ...
    return this.locked(async () => {
      const anchor = await this.effectiveAnchor(
      ...
      let endIndex = -1;
      for (
        let index = request.entries.length - 1;
        index >= startIndex;
        index -= 1
      ) {
      ...
      if (start === undefined || end === undefined) {
        return { enqueued: false, reason: "no-new-entries" };
      }
  ...
  private async locked<T>(task: () => Promise<T>): Promise<T> {
    ...
    return withLock(this.lockPath, record, task, {
  ...
  private async publish(
    ...
    const target = this.layout.entryPath(
      entry.conversationId,
      entry.range.end_message_id,
      at,
    );
  ...
  private async listPendingUnlocked(): Promise<
    (FactsQueueEntry & { readonly filePath: string })[]
  > {
    let names: string[];
    try {
  ...
  private async readCursorUnlocked(
    conversationId: string,
  ): Promise<FactsCursor> {
    const raw = await readFile(this.layout.cursorPath(conversationId), "utf8")
      .catch(() => undefined);
    return raw === undefined ? initialCursor() : parseCursor(raw);
  }

  private async readConsumedUnlocked(): Promise<FactsConsumedWatermark> {
    const raw = await readFile(this.layout.consumedPath, "utf8").catch(() =>
      undefined
    );
    return raw === undefined
      ? { version: FACTS_QUEUE_VERSION, consumed: {} }
      : parseConsumed(raw);
  }

  /** MONOTONIC: only a strictly later snapshot boundary may move the enqueue watermark. */
  private async advanceEnqueued(
    conversationId: string,
    range: FactsQueueRange,
    signal?: AbortSignal,
  ): Promise<void> {
    const cursor = await this.readCursorUnlocked(conversationId);
    if (range.end_snapshot_line <= cursor.enqueued_through_snapshot_line) {
      return;
    }
  ...
  private async advanceConsumed(
    ...
    for (const entry of entries) {
      ...
      if (
        entry.range.end_snapshot_line <= cursor.consumed_through_snapshot_line
      ) continue;
    ...
    const payload: FactsConsumedWatermark = {
      version: FACTS_QUEUE_VERSION,
      consumed: next,
    };
