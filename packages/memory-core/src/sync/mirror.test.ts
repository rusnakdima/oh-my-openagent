...
function run(
  ...
  return new Promise((resolvePromise, reject) => {
    ...
    child.on(
      "close",
      (code) => resolvePromise({ code: code ?? 1, stdout, stderr }),
    );
  });
}
...
async function createRepo(): Promise<
  { dir: string; repo: GitMemoryRepo; mirror: MirrorSync }
> {
...
async function commitFile(
  ...
  return run(["git", "commit", "-m", `write ${relativePath}`], dir, {
    [SYNC_PUSH_ENV]: "1",
  });
}
...
describe("MirrorSync lifecycle", () => {
  describe("#given a bare file:// mirror", () => {
    it(
      "#when set then a commit lands #then the mirror receives the commit",
      async () => {
        // given
        ...
        expect(
          (await run(["git", "show", "main:system/persona.md"], bare)).stdout,
        ).toBe("hooked\n");
    ...
    it(
      "#when set is called with trailing slashes #then the stored url is normalized",
      async () => {
        // given
    ...
    it("#when set is called twice #then the second url wins", async () => {
      ...
      expect(
        (await run(["git", "show", "main:reference/notes.md"], second)).stdout,
      ).toBe("second\n");
      expect(
        (await run(["git", "show", "main:reference/notes.md"], first)).code,
      ).not.toBe(0);
    }, 30_000);
  ...
  describe("#given an unreachable mirror", () => {
    it(
      "#when set #then the config survives the failed initial push and the reason is reported",
      async () => {
        // given
    ...
    it(
      "#when a commit lands #then the commit succeeds and the failure is logged in status",
      async () => {
        // given
        const { dir, mirror } = await createRepo();
        const missing = join(await tempDir("memory-mirror-gone-"), "gone.git");
        ...
        // then
        expect(commit.code).toBe(0);
        expect((await run(["git", "log", "--oneline"], dir)).stdout).toContain(
          "write system/persona.md",
        );
  ...
  describe("#given pushNow", () => {
    it(
      "#when no mirror is configured #then it reports not-configured without throwing",
      async () => {
        // given
        const { mirror } = await createRepo();
    ...
    it(
      "#when a mirror is configured #then a one-shot foreground push lands and clears ahead",
      async () => {
        // given
        const { repo, mirror } = await createRepo();
        const bare = await createBareMirror();
        ...
        // then
        expect(result.pushed).toBe(true);
        expect((await run(["git", "log", "--oneline", "main"], bare)).stdout)
  ...
  describe("#given a credentialed url", () => {
    it(
      "#when status is read #then the url is exposed redacted alongside the raw value",
      async () => {
        // given
        const { repo, mirror } = await createRepo();
        await repo.configSet(
          CONFIG_KEY,
          "https://alice:s3cr3t@example.com/acme/memory.git",
        );
        ...
        // then
        expect(status.redactedUrl).toBe(
     ...
     */
    it(
      "#when the log contains a credentialed url #then the status tail is redacted",
      async () => {
        // given
        const { dir, repo, mirror } = await createRepo();
        await repo.configSet(
          CONFIG_KEY,
          "https://x-token:s3cr3t@example.com/acme/memory.git",
        );
        await writeFile(
          mirrorLogPath(dir),
          [
    ...
    it(
      "#when a push to a credentialed remote fails #then the detail leaks no credential",
      async () => {
        // given
        const { mirror } = await createRepo();
