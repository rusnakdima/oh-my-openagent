...
function parseOwnerContent(
  content: string,
): { ownerPid: number; acquiredAtEpochMs: number } | null {
  ...
  if (!Number.isInteger(acquiredAtEpochMs) || acquiredAtEpochMs <= 0) {
    return null;
  }
...
export async function assertRetryableLockOpenError(
  ...
  if (code === "EPERM") {
    ...
    if (
      ...
      (await pathMayExist(dirname(lockPath), deps))
    ) return;
  }
  throw error;
}
...
export async function reapStaleLock(
  ...
  for (let attempt = 1; attempt <= LOCK_RELEASE_RETRY_ATTEMPTS; attempt += 1) {
    try {
      ...
      if (
        !isRetryableLockReleaseError(error) ||
