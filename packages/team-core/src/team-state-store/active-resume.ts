...
import {
  markDeadWorkersErrored,
  summarizeWorkerLiveness,
} from "./worker-resume-status";
  ...
  await Promise.all(runtimeState.members.map(async (member) => {
    try {
      ...
      log("team mailbox reservation reclaim failed", {
        ...
        member: member.name,
        error: reclaimError instanceof Error
...
async function markActiveTeamOrphaned(
  runtimeState: RuntimeState,
  config: TeamModeConfig,
): Promise<void> {
...
export async function resumeActiveTeam(
  ...
  await reconcileMemberReservations(
    ctx,
    runtimeState,
    config,
    staleReservationTtlMs,
  ...
  if (workerResumeStatus.hasAnyWorker && !workerResumeStatus.hasAliveWorker) {
    await transitionRuntimeState(
      ...
      (currentRuntimeState) => ({
        ...markDeadWorkersErrored(
          currentRuntimeState,
          workerResumeStatus.deadWorkerNames,
        ),
        status: "orphaned",
      }),
      config,
