...
async function seedRuntimeState(
  ...
  await mkdir(
    path.join(config.base_dir ?? "", "runtime", runtimeState.teamRunId),
    { recursive: true },
  );
...
describe("runtime state store", () => {
  ...
  test("createRuntimeState persists creating state with computed bounds", async () => {
    ...
    const persistedState = JSON.parse(
      await readFile(
        path.join(baseDir, "runtime", runtimeState.teamRunId, "state.json"),
        "utf8",
      ),
    );

    // then
    expect(runtimeState.teamRunId).toMatch(
  ...
  test("loadRuntimeState throws RuntimeStateError for malformed state", async () => {
    ...
    await writeFile(
      path.join(baseDir, "runtime", teamRunId, "state.json"),
      "{not-json",
    );
  ...
  test("transitionRuntimeState allows active to shutdown_requested", async () => {
    ...
    temporaryDirectories.push(baseDir);
    const config = createConfig(baseDir);
    const createdState = await createRuntimeState(
      createSpec(),
      "lead-session",
      "project",
      config,
    ...
    expect((await loadRuntimeState(createdState.teamRunId, config)).status)
      .toBe("shutdown_requested");
  });
  ...
  test("loadRuntimeState ignores crash-left tmp files and keeps valid persisted state", async () => {
    ...
    await writeFile(
      `${statePath}.tmp.mock-crash`,
      JSON.stringify({ ...runtimeState, status: "active" }),
    );
  ...
  test("loadRuntimeState accepts legacy member delegate counters without preserving them", async () => {
    ...
    expect(Object.keys(persistedState.members[0] ?? {})).not.toContain(
      "delegateTaskCallsUsed",
    );
  });
  ...
  test("listActiveTeams skips malformed runtime states and logs them", async () => {
    ...
    temporaryDirectories.push(baseDir);
    const config = createConfig(baseDir);
    const firstState = await createRuntimeState(
      createSpec("alpha-team"),
      undefined,
      "user",
      config,
    );
    const secondState = await createRuntimeState(
      createSpec("beta-team"),
      undefined,
      "project",
      config,
    );
    const malformedTeamRunId = randomUUID();
    await mkdir(path.join(baseDir, "runtime", malformedTeamRunId), {
      recursive: true,
    });
    await writeFile(
      path.join(baseDir, "runtime", malformedTeamRunId, "state.json"),
      "{oops",
    );
    ...
    expect(activeTeams).toEqual([
      {
        teamRunId: firstState.teamRunId,
        teamName: "alpha-team",
        status: "creating",
        memberCount: 2,
        scope: "user",
      ...
      {
        teamRunId: secondState.teamRunId,
        teamName: "beta-team",
        status: "creating",
        memberCount: 2,
        scope: "project",
  ...
  test("listActiveTeams removes deleted runtime directories left by interrupted cleanup", async () => {
    ...
    // then
    expect(activeTeams).toEqual([]);
    expect(await runtimeDirectoryExists(baseDir, runtimeState.teamRunId)).toBe(
      false,
    );
  });
