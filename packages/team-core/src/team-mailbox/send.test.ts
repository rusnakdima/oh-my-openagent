...
describe("sendMessage", () => {
  test("writes distinct files for concurrent writers targeting the same recipient", async () => {
    ...
    expect(new Set(parsedMessages.map((message) => message.messageId)).size)
      .toBe(4);
  });
  ...
  test("rejects sends when recipient unread bytes exceed the backpressure limit", async () => {
    ...
    await writeFile(
      path.join(inboxDir, "full.json"),
      "x".repeat(config.recipient_unread_max_bytes + 1),
      { flag: "w" },
    );

    // when
    const result = sendMessage(createMessage(), teamRunId, config, {
      isLead: false,
      activeMembers: ["m1"],
    });
  ...
  test("rejects duplicate message ids for the same recipient", async () => {
    ...
    // when
    const result = sendMessage(message, teamRunId, config, {
      isLead: false,
      activeMembers: ["m1"],
    });
  ...
  test("#given direct recipient is not an active or reserved member #when sendMessage runs #then no inbox directory is created", async () => {
    ...
    await expect(
      readdir(path.join(resolveBaseDir(config), "runtime", "escape")),
    ).rejects.toThrow();
  });
  ...
  test("#given direct recipient is reserved but not currently active #when sendMessage runs #then it can write to that reserved member", async () => {
    ...
    const inboxFiles = await readdir(
      getInboxDir(resolveBaseDir(config), teamRunId, "m2"),
    );
    expect(inboxFiles.filter((entry) => entry.endsWith(".json"))).toHaveLength(
      1,
    );
  });
  ...
  test("#given a configured lead recipient w2tc #when a member sends to the lead #then the message remains unread and visible", async () => {
    ...
    expect(unreadMessages.map((unreadMessage) => unreadMessage.messageId))
  ...
  test("gates broadcasts to leads and fans out to each active member", async () => {
    ...
    // then
    await expect(rejectedSend).rejects.toBeInstanceOf(
      BroadcastNotPermittedError,
    );
    ...
    const memberOneFiles = await readdir(
      getInboxDir(resolveBaseDir(config), teamRunId, "m1"),
    );
    const memberTwoFiles = await readdir(
      getInboxDir(resolveBaseDir(config), teamRunId, "m2"),
    );
    expect(memberOneFiles.filter((entry) => entry.endsWith(".json")))
      .toHaveLength(1);
    expect(memberTwoFiles.filter((entry) => entry.endsWith(".json")))
      .toHaveLength(1);
  });
});
