...
export async function pollAndBuildInjection(
  ...
  await transitionRuntimeState(teamRunId, (currentRuntimeState) => {
    const runtimeMember = currentRuntimeState.members.find((member) =>
      member.name === memberName
    );
    if (runtimeMember === undefined) {
      throw new Error(
        `runtime member not found for session ${sessionID}: ${memberName}`,
      );
    }
    ...
    const injectableMessages = unreadMessages.filter((message) =>
        ...
        member.name === memberName
          ? {
            ...
            pendingInjectedMessageIds: Array.from(
              new Set([...member.pendingInjectedMessageIds, ...messageIds]),
            ),
          }
          : member
      )),
  ...
  if (result === undefined) {
    throw new Error(
      `mailbox injection claim failed for session ${sessionID}: ${memberName}`,
    );
  }
