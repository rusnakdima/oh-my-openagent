...
function buildReservation(
  ...
  const reservedPath = path.join(
    inboxDir,
    `${RESERVED_PREFIX}${messageId}${RESERVED_SUFFIX}`,
  );
...
export async function commitDeliveryReservation(
...
export async function releaseDeliveryReservation(
  reservation: DeliveryReservation,
): Promise<void> {
...
export async function reclaimStaleReservations(
  ...
  for (const entry of entries) {
    ...
    if (
      ...
      !entry.name.endsWith(RESERVED_SUFFIX)
    ) continue;
    ...
    const messageId = entry.name.slice(
      RESERVED_PREFIX.length,
      -RESERVED_SUFFIX.length,
    );
