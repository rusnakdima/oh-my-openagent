...
export const TEAM_SESSION_PATTERN = new RegExp(
  `^omo-team-(${UUID_V4ISH_PATTERN})$`,
);
...
async function listTeamSessionsViaTmux(tmuxPath: string): Promise<string[]> {
  ...
  const result = await runTmuxCommand(tmuxPath, [
    "list-sessions",
    "-F",
    "#{session_name}",
  ]);
...
async function killTeamSessionViaTmux(
  tmuxPath: string,
  sessionName: string,
): Promise<void> {
  const { runTmuxCommand } = await import("@oh-my-opencode/tmux-core");
  ...
  return sweepTmuxSessionsWith(
    ...
    },
    {
      predicate: (sessionName) => {
        ...
        return teamRunId !== undefined && teamRunId.length > 0 &&
          !activeTeamRunIds.has(teamRunId);
      },
    },
  );
}

export async function sweepStaleTeamSessions(
  activeTeamRunIds: ReadonlySet<string>,
): Promise<string[]> {
