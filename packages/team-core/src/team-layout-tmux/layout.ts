...
function buildAttachCommand(
  ...
  return `opencode attach ${shellSingleQuote(serverUrl)} --session ${
    shellSingleQuote(member.sessionId)
  } --dir ${shellSingleQuote(getPaneWorkingDirectory(member))}`;
}
...
function selectExistingTeammatePane(
  teammatePanes: Array<string>,
  callerPaneId: string,
): string {
  return teammatePanes[Math.floor(teammatePanes.length / 2)] ??
    teammatePanes[teammatePanes.length - 1] ?? callerPaneId;
}
...
async function createTeamLayoutInCallerWindow(
  ...
  { focusWindowId: string; focusPanesByMember: Record<string, string> } | null
> {
  ...
  for (const member of members) {
    const split = await deps.runTmuxCommand(
      tmuxPath,
      buildSplitArgs(callerPaneId, teammatePanes, member),
    );
    ...
    await deps.runTmuxCommand(tmuxPath, [
      "select-pane",
      "-t",
      paneId,
      "-T",
      `${TEAM_PANE_TITLE_PREFIX}${member.name}`,
    ]);
    await deps.runTmuxCommand(tmuxPath, [
      "set-option",
      "-p",
  ...
  const layoutResult = await deps.runTmuxCommand(tmuxPath, [
    "select-layout",
    "-t",
    windowTarget,
    "main-vertical",
  ...
  const resizeResult = await deps.runTmuxCommand(tmuxPath, [
    "resize-pane",
    "-t",
    callerPaneId,
    "-x",
    ...
    if (!(await deps.isServerRunning(serverUrl))) {
      ...
      deps.log(
        "opencode server not reachable, skipping team layout (see issue #3963)",
        {
          ...
          teamRunId,
          serverUrl,
          ctxServerUrl: ctxServerUrl && ctxServerUrl !== serverUrl
            ? ctxServerUrl
            : undefined,
          hint: ctxServerUrl && ctxServerUrl !== serverUrl
...
export async function removeTeamLayout(
  ...
  try {
    ...
    if (cleanupTarget?.ownedSession !== false) {
      await resolvedDeps.runTmuxCommand(tmuxPath, [
        "kill-session",
        "-t",
    ...
    for (
      ...
      try {
        ...
        const errorMessage = windowError instanceof Error
          ? String(windowError)
          : String(windowError);
...
function isTeamLayoutDeps(
  ...
  return value !== undefined && "runTmuxCommand" in value &&
...
function isTeamLayoutCleanupTarget(
  ...
  return value !== undefined && "ownedSession" in value &&
    "targetSessionId" in value;
}
