...
type TeamMemberPaneIds = Pick<
  RuntimeStateMember,
  "tmuxPaneId" | "tmuxGridPaneId"
>;
...
export async function closeTeamMemberPane(
  ...
  const paneIds = [member.tmuxPaneId, member.tmuxGridPaneId].filter((
    paneId,
  ): paneId is string => paneId !== undefined && paneId.length > 0);
  if (paneIds.length === 0) {
    return false;
  }
