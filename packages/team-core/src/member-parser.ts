...
function translateMemberError(
  ...
  agentEligibilityRegistry: Readonly<
    Record<
      string,
      {
  ...
  if (input.kind === "subagent_type" || (!hasKind && hasSubagentType)) {
    ...
    if (
      ...
      !agentEligibilityRegistry[input.subagent_type]
    ) {
      return new MemberValidationError(
...
export function createParseMember<TMember>(
  memberSchema: {
    safeParse(
      input: unknown,
    ): { success: true; data: TMember } | { success: false };
  },
  agentEligibilityRegistry: Readonly<
    Record<
      string,
  ...
  return function parseMember(input: unknown) {
    ...
    const result = memberSchema.safeParse(
      raw.kind === undefined &&
        (raw.category !== undefined || raw.subagent_type !== undefined)
        ? {
          ...raw,
          kind: raw.category !== undefined ? "category" : "subagent_type",
        }
        : raw,
    );
