import type { CallerTeamLead } from "./caller-team-lead";
...
function deriveMemberNameStem(member: JsonRecord): string {
  ...
  if (
    member.kind === "subagent_type" && typeof member.subagent_type === "string"
  ) {
...
function assignGeneratedMemberNames(rawMembers: unknown[]): unknown[] {
  ...
  return rawMembers.map((member) => {
    ...
    const stem = rawName === undefined
      ? deriveMemberNameStem(member)
      : normalizeNameStem(rawName);
...
function hasMemberLeadFlag(rawMembers: unknown[]): boolean {
  return rawMembers.some((member) =>
    isJsonRecord(member) && member.isLead === true
  );
}
...
function normalizeInlineMember(
  member: JsonRecord,
  ...
  if (normalizedMember.kind === undefined) {
    ...
    if (typeof normalizedMember.category === "string") {
      ...
      typeof rawKind === "string" && rawKind !== "agent" &&
      rawKind !== "member" && rawKind !== "worker" && rawKind !== "analyst"
    ) {
      normalizedMember.kind = "category";
  ...
  if (
    normalizedMember.kind === "category" &&
    normalizedMember.prompt === undefined
  ) {
...
export function normalizeTeamSpecInput(
  ...
  const rawLead = isJsonRecord(raw.lead) &&
      Object.keys(omitEmptyStringFields(raw.lead)).length > 0
    ? raw.lead
    : undefined;
  let leadAgentId = typeof normalizedSpec.leadAgentId === "string"
    ? normalizedSpec.leadAgentId
    : undefined;
  ...
  if (Array.isArray(rawMembers)) {
    let normalizedMembers = rawMembers.map((member) =>
      isJsonRecord(member) ? normalizeInlineMember(member, options) : member
    );
    ...
    if (isJsonRecord(rawLead)) {
      ...
      const alreadyPresent = leadName !== undefined &&
        normalizedMembers.some((member) => getMemberName(member) === leadName);
      if (!alreadyPresent) {
    ...
    if (shouldUseFirstMemberAsLead) {
      ...
      if (
        callerTeamLead?.isEligibleForTeamLead &&
        callerTeamLead.agentTypeId !== undefined
      ) {
        normalizedMembers = [
          createCallerLeadMember(callerTeamLead.agentTypeId),
          ...normalizedMembers,
        ];
        leadAgentId = "lead";
      } else if (callerTeamLead?.displayName !== undefined) {
        throw new Error(
          `Caller agent ${callerTeamLead.displayName} is not eligible as team lead; specify leadAgentId explicitly`,
        );
      }
    }
    ...
    if (
      ...
      !normalizedMembers.some((member) => getMemberName(member) === leadAgentId)
    ) {
      const normalizedLeadAgentId = normalizeNameStem(leadAgentId);
      if (
        normalizedMembers.some((member) =>
          getMemberName(member) === normalizedLeadAgentId
        )
      ) {
