...
export function getTeamSpecPath(
  ...
  return path.join(
    getTeamDirectory(baseDir, teamName, scope, projectRoot),
    "config.json",
  );
}
...
export function getInboxDir(
  ...
  return resolveContainedPath(baseDir, [
    "runtime",
    teamRunId,
    "inboxes",
    memberName,
...
export function getTaskClaimLockPath(
  baseDir: string,
  teamRunId: string,
  taskId: string,
): string {
...
async function readTeamSpecDirectories(
  directoryPath: string,
  scope: "project" | "user",
): Promise<TeamSpecEntry[]> {
  try {
...
export async function discoverTeamSpecs(
  ...
  for (const userTeamSpec of userTeamSpecs) {
    if (projectTeamNames.has(userTeamSpec.name)) {
      const projectTeamSpec = projectTeamSpecs.find((entry) =>
        entry.name === userTeamSpec.name
      );
      if (projectTeamSpec) {
...
async function safeChmod(
  directoryPath: string,
  mode: number,
  deps: Pick<PathDeps, "chmod" | "log">,
): Promise<void> {
  try {
    await deps.chmod(directoryPath, mode);
  } catch (error) {
    const code = isErrnoLike(error) && typeof error.code === "string"
      ? error.code
      : undefined;
    if (code === "EPERM" || code === "ENOTSUP" || code === "EINVAL") {
      deps.log(
        "team-mode: chmod refused on base directory; continuing with existing permissions",
        {
          path: directoryPath,
          code,
          syscall: isErrnoLike(error) && typeof error.syscall === "string"
            ? error.syscall
            : undefined,
        },
      );
