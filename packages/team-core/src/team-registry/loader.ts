...
function createSpecialCaseValidationError(
  rawSpec: unknown,
): TeamSpecValidationError | undefined {
  ...
  if (rawMembers.length > 8) {
    const teamName = typeof rawSpec.name === "string"
      ? rawSpec.name
      : "<unknown>";
  ...
  for (const rawMember of rawMembers) {
    ...
    const memberName = typeof rawMember.name === "string"
      ? rawMember.name
      : "<unknown>";
    ...
    if (rawMember.kind === "category" && !Object.hasOwn(rawMember, "prompt")) {
      const category = typeof rawMember.category === "string"
        ? rawMember.category
        : "<unknown>";
      return new TeamSpecValidationError(
...
export async function loadTeamSpec(
  ...
  const matchedTeamSpec = discoveredTeamSpecs.find((entry) =>
    entry.name === teamName
  );

  if (!matchedTeamSpec) {
    const baseDir = resolveBaseDir(config);
    const projectSpecPath = getTeamSpecPath(
      baseDir,
      teamName,
      "project",
      projectRoot,
...
export async function loadAllTeamSpecs(
  ...
  Array<
    { name: string; scope: "project" | "user"; spec?: TeamSpec; error?: Error }
  >
> {
  const discoveredTeamSpecs = await discoverTeamSpecs(config, projectRoot);
