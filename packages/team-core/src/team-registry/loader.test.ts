...
const { TeamSpecValidationError, loadAllTeamSpecs, loadTeamSpec } =
  await import("./loader");
    ...
    members: [
      {
        ...
        name: "lead",
        category: "deep",
        prompt: "implement the leader task",
      },
      {
        kind: "category",
        name: "reviewer",
        category: "quick",
        prompt: "review the current output",
      },
      {
        kind: "category",
        name: "tester",
        category: "deep",
        prompt: "verify the resulting behavior",
      },
    ],
  };
}
...
describe("team-registry loader", () => {
  ...
  test("loads and validates a valid 3-member team spec", async () => {
    ...
    const teamSpec = await loadTeamSpec(
      "alpha",
      createConfig(fixturePaths.userBaseDir),
  ...
  test("defaults version when omitted from stored specs", async () => {
    ...
    const { version: _version, ...teamSpecWithoutVersion } = createBaseSpec(
      "default-version",
    );
  ...
  test("defaults createdAt from Date.now when omitted from stored specs", async () => {
    ...
    const { createdAt: _createdAt, ...teamSpecWithoutCreatedAt } =
      createBaseSpec("default-created-at");
  ...
  test("derives leadAgentId and prepends lead shorthand to members", async () => {
      ...
      members: [
        {
          ...
          name: "scout-1",
          category: "deep",
          prompt: "Scout the src directory for auth patterns.",
        },
        {
          kind: "category",
          name: "scout-2",
    ...
    expect(teamSpec.leadAgentId).toBe("lead");
    expect(teamSpec.members).toHaveLength(3);
    expect(teamSpec.members[0]).toMatchObject({
      kind: "subagent_type",
      name: "lead",
  ...
  test("derives leadAgentId from the only member when no lead hint exists", async () => {
    ...
    await writeJsonFile(fixturePaths.userConfigPath, {
      ...
      members: [{
        ...
        name: "solo-lead",
        category: "deep",
        prompt: "Implement the assigned work for the solo team.",
      }],
    });
      ...
      members: [
        {
          ...
          name: "member-1",
          category: "deep",
          prompt: "Implement the assigned work for member one.",
        },
        {
          kind: "category",
          name: "member-2",
          category: "quick",
          prompt: "Review the assigned work for member one.",
        },
      ],
    });
  ...
  test("returns malformed team specs as data during load-all startup", async () => {
    ...
    await writeJsonFile(
      goodFixturePaths.userConfigPath,
      createBaseSpec("good"),
    );
    await mkdir(path.dirname(badFixturePaths.userConfigPath), {
      recursive: true,
    });
    await writeFile(badFixturePaths.userConfigPath, "{\n  invalid json\n");

    // when
    const results = await loadAllTeamSpecs(
      createConfig(goodFixturePaths.userBaseDir),
      goodFixturePaths.projectRoot,
    );
    ...
    expect(results).toEqual(expect.arrayContaining([
      expect.objectContaining({
        name: "good",
        scope: "user",
        spec: expect.objectContaining({ name: "good" }),
      }),
      expect.objectContaining({
        name: "broken",
        scope: "user",
