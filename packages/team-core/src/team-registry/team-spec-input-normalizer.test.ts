...
describe("normalizeTeamSpecInput", () => {
  ...
  test("throws a clear error when the caller is not eligible and no lead is specified", () => {
    ...
    // when
    const result = () =>
      normalizeTeamSpecInput(rawSpec, {
      ...
      members: [
        {
          ...
          capabilities: ["structure", "modules"],
        },
      ],
    };
  ...
  test("strips empty-string optional fields injected by the tool host", () => {
    ...
    // when
    const normalizedSpec = normalizeTeamSpecInput(rawSpec) as Record<
      string,
      unknown
    >;
    ...
    expect(JSON.stringify(normalizedSpec.members)).not.toContain(
      "subagent_type",
    );
