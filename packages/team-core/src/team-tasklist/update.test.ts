...
test(
  "updateTaskStatus supports the one-way claim to complete flow",
  async () => {
    // given
    ...
    try {
      ...
      const loadedTask = await getTask(
        fixture.teamRunId,
        task.id,
        fixture.config,
      );
...
test(
  "updateTaskStatus auto-claims when a member starts a pending task directly",
  async () => {
    // given
    const fixture = await createTasklistFixture();
...
test("updateTaskStatus rejects reverse transitions", async () => {
  ...
  try {
    ...
    // when
    const reversedTransition = updateTaskStatus(
      fixture.teamRunId,
      task.id,
      "claimed",
      "member-a",
    ...
    // then
    await expect(reversedTransition).rejects.toBeInstanceOf(
      InvalidTaskTransitionError,
    );
...
test("updateTaskStatus rejects non-owner updates except deletion", async () => {
  ...
  try {
    ...
    // when
    const crossOwnerUpdate = updateTaskStatus(
      fixture.teamRunId,
      task.id,
      "in_progress",
      "member-b",
    ...
    // then
    await expect(crossOwnerUpdate).rejects.toBeInstanceOf(
      CrossOwnerUpdateError,
    );
...
test("#given traversal task id #when updating a task #then it rejects before writing outside the task directory", async () => {
  ...
  try {
    ...
    // then
    await expect(updatedTask).rejects.toThrow(
      "team path escapes base directory",
    );
  } finally {
    await fixture.cleanup();
  }
