...
test("createTask assigns distinct ids during concurrent creation", async () => {
  ...
  try {
    ...
    const [firstTask, secondTask] = await Promise.all([
      createTask(
        fixture.teamRunId,
        createTaskInput({ subject: "first task" }),
        fixture.config,
      ),
      createTask(
        fixture.teamRunId,
        createTaskInput({ subject: "second task" }),
        fixture.config,
      ),
    ]);

    const tasksDirectory = getTasksDir(
      resolveBaseDir(fixture.config),
      fixture.teamRunId,
    );
    const watermarkContent = await readFile(
      path.join(tasksDirectory, ".highwatermark"),
      "utf8",
    );
    const sortedIds = [firstTask.id, secondTask.id].sort((leftId, rightId) =>
...
test("#given traversal team run id #when creating a task #then no task directory escapes the team base", async () => {
  ...
  try {
    ...
    // then
    await expect(createdTask).rejects.toThrow(
