...
import { AlreadyClaimedError, BlockedByError, claimTask } from "./claim";
...
test("claimTask allows exactly one concurrent claimant", async () => {
  ...
  try {
    ...
    const failedClaims = claimResults.filter((result) =>
...
test("claimTask rejects blocked tasks until blockers complete", async () => {
  ...
  try {
    const blockerTask = await createTask(
      fixture.teamRunId,
      createTaskInput({ subject: "blocker" }),
      fixture.config,
    );
...
test("#given traversal task id #when claiming a task #then it rejects before creating an escaped claim lock", async () => {
  ...
  try {
    ...
    // then
    await expect(claimedTask).rejects.toThrow(
