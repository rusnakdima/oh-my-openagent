...
function normalizeGitPathText(value: string): string {
  return value.replaceAll("\\", "/").replaceAll("/private/var/", "/var/")
    .toLowerCase();
}

async function initGitRepo(): Promise<string> {
  const repositoryRoot = await fs.mkdtemp(
    path.join(tmpdir(), "team-worktree-"),
  );
...
describe("team-worktree manager", () => {
  test("given tmp git repo when createWorktree then registers detached worktree", async () => {
    ...
    // when
    const resultPath = await createWorktree(
      repositoryRoot,
      "t1",
      "m1",
      worktreePath,
    ...
    const listResult = Bun.spawnSync(["git", "worktree", "list"], {
      cwd: repositoryRoot,
      stdout: "pipe",
      stderr: "pipe",
    });
    const canonicalWorktreeDirectory = await fs.realpath(worktreeDirectory);
    expect(normalizeGitPathText(new TextDecoder().decode(listResult.stdout)))
      .toContain(
    ...
    const repoHeadResult = Bun.spawnSync([
      "git",
      "-C",
      repositoryRoot,
      "rev-parse",
    ...
    expect(new TextDecoder().decode(headResult.stdout).trim()).toBe(
      new TextDecoder().decode(repoHeadResult.stdout).trim(),
    );
  });
