...
export function stripLinePrefixes(lines: string[]): string[] {
  ...
  const stripPlus = !stripHash && diffPlusCount > 0 &&
...
export function stripInsertBoundaryEcho(
  ...
  if (
    out.length > 0 && equalsIgnoringWhitespace(out[out.length - 1], beforeLine)
  ) {
    out = out.slice(0, -1);
  }
  return out;
...
export function stripRangeBoundaryEcho(
  ...
  if (
    afterIdx < lines.length && out.length > 0 &&
    out[out.length - 1] === lines[afterIdx]
  ) {
    out = out.slice(0, -1);
  }
