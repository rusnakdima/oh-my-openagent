...
export function createEventTelemetryClient(
  ...
  const allowlists = new Map(
    Object.entries(input.propertyAllowlist).map((
      [name, keys],
    ) => [name, new Set(keys)]),
  );
    ...
    captureEvent(name, properties) {
      ...
      if (name.length === 0 || allowlist === undefined) {
        report(
          ...
          new Error("event name is empty or not allowlisted"),
        );
        return;
      }
      ...
      for (const [key, value] of Object.entries(properties)) {
        if (isForbiddenKey(key, value)) {
          report(
            ...
            new Error(`forbidden event property: ${key}`),
          );
          continue;
        }
        if (!allowlist.has(key)) {
          report(
            ...
            new Error(`unknown event property: ${key}`),
          );
          continue;
        }
        if (!isEventPropertyValue(value)) {
          report(
            "telemetry_event_property_rejected",
            new Error(`invalid event property value: ${key}`),
          );
          continue;
        }
...
function createEventTransport(
  ...
  try {
    return factory(
      getTelemetryApiKey(env, input.product.defaultApiKey),
      options,
    );
  } catch (error) {
    input.diagnostics?.({
