  ...
  test.each(
    [
      ...
      [
        ...
        `${UNCONFIGURED_POSTHOG_API_KEY}_configured`,
        true,
      ],
    ] as const,
  )(
    "#given %s input #when key availability is checked #then the expected result is returned",
    (_name, apiKey, expected) => {
  ...
  test.each(OPT_OUT_CASES)(
    ...
    (_name, env, expected) => {
      ...
      const result = shouldDisableTelemetry({
        env,
        productEnvPrefix: productPrefix,
      });
...
describe("DO_NOT_TRACK global opt-out", () => {
  describe("#given DO_NOT_TRACK is 1", () => {
    test.each(
      ...
      "#when evaluated for %s #then telemetry is disabled",
      (_product, productEnvPrefix) => {
        // when
        const result = shouldDisableTelemetry({
          env: { DO_NOT_TRACK: "1" },
          productEnvPrefix,
  ...
  describe("#given DO_NOT_TRACK is not an opt-out value", () => {
    ...
    )("#when the value is %s #then telemetry remains enabled", (_name, env) => {
      // when
      const result = shouldDisableTelemetry({
        env,
        productEnvPrefix: "OMO_SENPI",
  ...
  describe("#given the same env object changes between evaluations", () => {
    test("#when DO_NOT_TRACK changes #then each call reads the current value", () => {
      ...
      // when
      const beforeOptOut = shouldDisableTelemetry({
        env,
        productEnvPrefix: "OMO_SENPI",
      });
      env.DO_NOT_TRACK = "yes";
      const duringOptOut = shouldDisableTelemetry({
        env,
        productEnvPrefix: "OMO_SENPI",
      });
      env.DO_NOT_TRACK = "";
      const afterOptOut = shouldDisableTelemetry({
        env,
        productEnvPrefix: "OMO_SENPI",
      });

      // then
      expect([beforeOptOut, duringOptOut, afterOptOut]).toEqual([
        false,
        true,
        false,
      ]);
