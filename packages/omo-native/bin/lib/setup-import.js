...
function readProviderMap() {
  return JSON.parse(
    readFileSync(new URL("./provider-map.json", import.meta.url), "utf8"),
  );
}
...
function candidate(provider, key, source, providerMap) {
  ...
  return target
    ? { provider: target, key, source }
    : { provider, source, unmapped: true };
}
    ...
    for (const [provider, entry] of Object.entries(parsed)) {
      ...
      if (entry.type === "oauth") {
        ...
        plan.candidates.push(
          candidate(provider, entry.key, "opencode", providerMap),
        );
      }
    }
  } catch (error) {
    plan.notices.push(
      `WARN opencode: could not parse auth.json: ${error.message}`,
    );
  }
}
...
function readSqliteStore(
  ...
  try {
    ...
    const version = database.prepare("SELECT version FROM auth_schema_version")
      .get()?.version;
    if (version !== expectedVersion) {
      plan.notices.push(
        `NOTICE ${id}: auth schema version ${
          String(version)
        } is unknown; credentials not imported`,
      );
      return;
    }
    ...
    for (const row of rows) {
      ...
      try {
        ...
        if (typeof data?.key === "string") {
          plan.candidates.push(
            candidate(row.provider, data.key, id, providerMap),
          );
        }
      } catch {
        plan.notices.push(
          `WARN ${id}: ignored malformed ${row.credential_type} row for ${row.provider}`,
        );
      }
    }
  } catch (error) {
    plan.notices.push(
      `WARN ${id}: could not inspect agent.db: ${error.message}`,
    );
  } finally {
    database?.close();
  }
...
async function buildPlan(options) {
  ...
  try {
    const { DatabaseSync } = await (options.loadSqlite ?? (() =>
      import("node:sqlite")))();
    for (const [id, directory, version] of SQLITE_STORES) {
      readSqliteStore(
        id,
        join(home, directory, "agent", "agent.db"),
        version,
        DatabaseSync,
        providerMap,
        plan,
    ...
    plan.notices.push(
      "NOTICE setup: node:sqlite unavailable; database credentials not imported",
    );
  }
  return plan;
}
...
function writeTarget(path, current, additions) {
  ...
  for (const item of additions) {
    next[item.provider] = { type: "api_key", key: item.key };
  }
...
async function consent(result, target, options) {
  ...
  if (options.stdin?.isTTY !== true || options.stdout?.isTTY !== true) {
    process.stdout.write(
      "Non-interactive setup did not import credentials. Re-run with `omo setup --yes`.\n",
    );
    return false;
  }
  process.stdout.write(
    `Import API credentials for ${
      result.additions.map((item) => item.provider).join(", ")
    } into ${target}? [y/N] `,
  );
...
export async function runSetup(args = process.argv.slice(2), options = {}) {
  ...
  if (
    !await consent(result, target, { ...runtime, yes: args.includes("--yes") })
  ) {
