...
function spawnSenpi(args, withExtension) {
  ...
  const finalArgs = withExtension
    ...
    : args;
  spawnNode(senpi.cliPath, finalArgs, {
    env: senpiEnvironment(senpi.packageRoot),
  });
}

function isInteractiveDefault(args) {
  return process.stderr.isTTY === true && !args.includes("-p") &&
    !args.includes("--print");
}
...
function reportLegacyFlatAdoption() {
  ...
  console.error(
    `omo: carried forward settings from the legacy ~/.omo layout (${moved})`,
  );
}
...
export async function runLauncher(args = process.argv.slice(2)) {
  ...
  if ((command === "--version" || command === "-v") && args.length === 1) {
    console.log(
      `omo ${packageManifest().version} (engine: senpi ${engineVersion()})`,
    );
    process.exitCode = 0;
    return;
  }
  ...
  if (isInteractiveDefault(args)) {
    ...
    if (
      ...
      needsSetupSuggestion(await detectHarnesses())
    ) {
      console.error(
        "omo: sibling credentials detected; run `omo setup` to review them",
      );
    }
  }
