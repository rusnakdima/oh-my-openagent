...
function readSenpiModels(path, result) {
  ...
  try {
    ...
    if (
      providers !== null && typeof providers === "object" &&
      !Array.isArray(providers)
    ) {
      const ids = sorted(Object.keys(providers));
      result.modelHint = ids.length > 0
        ? `providers: ${ids.join(", ")}`
        : "providers: none";
    }
  } catch (error) {
...
function readGajaeModels(path) {
  ...
  try {
    ...
    if (enabledModels.length > 0) {
      hints.push(`enabledModels: ${sorted(enabledModels).join(", ")}`);
    }
    if (modelRoles.length > 0) {
      hints.push(`modelRoles: ${sorted(modelRoles).join(", ")}`);
    }
...
function inspectSqlite(id, path, DatabaseSync, modelHint) {
  ...
  try {
    ...
    const schema = database.prepare("SELECT version FROM auth_schema_version")
      .get();
    ...
    if (!KNOWN_AUTH_SCHEMA_VERSIONS.has(version)) {
      result.notices.push(
        `auth schema version ${
          String(version)
        } is unknown; credentials not inspected`,
      );
      return result;
    }
...
export async function detectHarnesses(options = {}) {
  ...
  try {
    ...
    for (const item of [omp, gjc]) {
      if (item.installed) {
        item.notices.push("node:sqlite unavailable; file presence only");
      }
    }
...
export function needsSetupSuggestion(inventory) {
  ...
  const siblingInstalled = inventory.harnesses.some((item) =>
    item.id !== "senpi" && item.installed
  );
