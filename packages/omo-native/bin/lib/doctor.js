...
function warningsForSettings() {
  ...
  const duplicate = packages.some((entry) => {
    ...
    return entry !== null && typeof entry === "object" &&
      entry.source === "@code-yeongyu/omo-senpi";
  });
  return duplicate
    ? [
...
export function runDoctor(inventory) {
  ...
  if (senpi) {
    try {
      ...
      const installed =
        readJson(join(senpi.packageRoot, "package.json")).version;
  ...
  lines.push(
    `INFO omo ${packageManifest().version} (engine: senpi ${
      engineVersionOrUnresolved(senpi)
    })`,
  );
