...
import {
  detectHarnesses,
  needsSetupSuggestion,
} from "../bin/lib/setup-detect.js";
...
const TTY_DRIVER = resolve(
  fileURLToPath(new URL("tty-driver.py", import.meta.url)),
);
const roots: string[] = [];
...
async function createDatabase(
  ...
  withDatabase(new DatabaseSync(path), (db) => {
    ...
    for (const [provider, type, disabled] of rows) {
      insert.run(provider, type, "SQLITE-SECRET", disabled);
    }
  });
}
...
async function populateAll(fixture: Fixture, skip?: StoreKey): Promise<void> {
  ...
  if (skip !== "opencode") {
    write(
      join(fixture.xdg, "opencode", "auth.json"),
      JSON.stringify({
  ...
  write(
    join(fixture.home, ".gjc", "agent", "config.yml"),
    [
      "modelRoles:",
      "  default: anthropic/fixture",
      "  fast: openai/fixture",
      "enabledModels:",
      "  - fixture-one",
      "  - fixture-two",
...
function snapshotTree(
  root: string,
): { files: string[]; hashes: Record<string, string> } {
  const files: string[] = [];
  function visit(path: string, relative = ""): void {
    for (
      const entry of readdirSync(path, { withFileTypes: true }).sort((a, b) =>
...
function createLauncherFixture(fixture: Fixture): string {
  ...
  cpSync(join(SOURCE_ROOT, "bin"), join(packageRoot, "bin"), {
    recursive: true,
  });
  write(
...
describe("omo setup sibling detection", () => {
  describe("#given all four harness stores", () => {
    test.skipIf(!SQLITE_AVAILABLE)(
      "#when detected #then the exact report contains ids and types but no values",
      async () => {
    ...
    test.skipIf(!SQLITE_AVAILABLE)(
      "#when detection runs twice #then every source hash and directory listing stays identical",
      async () => {
        const fixture = createFixture();
        await populateAll(fixture);
  ...
  describe("#given each store is absent in turn", () => {
    test.skipIf(!SQLITE_AVAILABLE)(
      "#when detected #then that harness reports installed no without crashing",
      async () => {
        const cases = [
          ["senpi", (fixture: Fixture) => join(fixture.agentDir, "auth.json")],
          [
            "opencode",
            (fixture: Fixture) => join(fixture.xdg, "opencode", "auth.json"),
          ],
          [
            "oh-my-pi",
            (fixture: Fixture) =>
              join(fixture.home, ".omp", "agent", "agent.db"),
          ],
          [
            "gajae-code",
            (fixture: Fixture) =>
              join(fixture.home, ".gjc", "agent", "agent.db"),
          ],
        ] as const;
  ...
  describe("#given an unknown sqlite auth schema", () => {
    test.skipIf(!SQLITE_AVAILABLE)(
      "#when detected #then it reports detection-only and parses no credentials",
      async () => {
        const fixture = createFixture();
        await createDatabase(
          join(fixture.home, ".omp", "agent", "agent.db"),
          99,
          [["must-not-parse", "api_key", null]],
        );
        ...
        expect(omp.notices.join("\n")).toBe(
          "auth schema version 99 is unknown; credentials not inspected",
        );
      },
    );
  });
  ...
  describe("#given node:sqlite is unavailable", () => {
    test.skipIf(!SQLITE_AVAILABLE)(
      "#when detected #then sqlite stores degrade to file presence and remain successful",
      async () => {
        const fixture = createFixture();
        await populateAll(fixture);
        const inventory = await detect(fixture, {
        ...
        for (const id of ["oh-my-pi", "gajae-code"]) {
          ...
          expect(harness.notices.join("\n")).toBe(
            "node:sqlite unavailable; file presence only",
          );
        }
      },
    );
  ...
  describe("#given no credentials found and a sibling is installed", () => {
    test("#when suggestion state is checked #then only that exact condition is true", async () => {
      ...
      write(
        join(fixture.agentDir, "auth.json"),
        JSON.stringify({ senpi: { type: "api_key", key: "SENPI-SECRET" } }),
      );
    ...
    test.skipIf(process.platform === "win32")(
      "#when default launch is on a TTY #then one hint appears and non-TTY emits none",
      () => {
        const fixture = createFixture();
        const launcher = createLauncherFixture(fixture);
        write(join(fixture.xdg, "opencode", "auth.json"), "{}");
        const nonTty = runLauncher(launcher, fixture, []);
        expect(`${nonTty.stdout}${nonTty.stderr}`).not.toContain(
          "run `omo setup`",
        );
        ...
        expect(
          output.match(/sibling credentials detected; run `omo setup`/g)
            ?.length,
        ).toBe(1);
        write(
          join(fixture.agentDir, "auth.json"),
        ...
        expect(`${configured.stdout}${configured.stderr}`).not.toContain(
          "sibling credentials detected",
        );
      },
    );
  });
  ...
  describe("#given malformed auth JSON", () => {
    test("#when setup runs #then it warns, exits zero, and never prints the partial value", () => {
      ...
      write(
        join(fixture.agentDir, "auth.json"),
        '{"provider":{"type":"api_key","key":"SENPI-SECRET"',
      );
    ...
    describe("#when no agent directory is configured", () => {
      test("#then the canonical directory is the one detected", async () => {
        ...
        write(
          ...
          JSON.stringify({
            "canonical-provider": { type: "api_key", key: "SENPI-SECRET" },
          }),
        );
        ...
        expect(
          inventory.harnesses.find((harness) => harness.id === "senpi")
            ?.providers,
        ).toContain("canonical-provider");
      });
    });
  });
});
