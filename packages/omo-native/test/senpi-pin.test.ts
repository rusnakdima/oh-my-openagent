  ...
  describe("#given the three manifests that pin @code-yeongyu/senpi", () => {
    describe("#when their pins are read", () => {
      test("#then omo-ai pins the exact senpi version", () => {
        expect(omoNativeManifest.dependencies["@code-yeongyu/senpi"]).toBe(
          SENPI_PIN,
        );
        expect(omoNativeManifest.dependencies["@code-yeongyu/senpi"]).toMatch(
          EXACT_PIN,
        );
      });

      test("#then omo-senpi peer and dev pins match", () => {
        expect(omoSenpiManifest.peerDependencies["@code-yeongyu/senpi"]).toBe(
          SENPI_PIN,
        );
        expect(omoSenpiManifest.devDependencies["@code-yeongyu/senpi"]).toBe(
          SENPI_PIN,
        );
      });

      test("#then the root devDependency pin matches", () => {
        expect(rootManifest.devDependencies["@code-yeongyu/senpi"]).toBe(
          SENPI_PIN,
        );
      });
