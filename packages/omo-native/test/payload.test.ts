  ...
  return readdirSync(join(root, prefix), { withFileTypes: true }).flatMap(
    (entry) => {
      ...
      return entry.isDirectory()
        ? listRelativeFiles(root, relative)
        : [relative];
    },
  );
}
    ...
    describe("#when the completeness check runs", () => {
      test("#then it exits 1 naming the first missing artifact", () => {
        ...
        expect(result.output).toContain(
          `missing required artifact: ${join("extensions", "omo.js")}`,
        );
      });
    });
  });
      ...
      test(
        ...
        () => {
          ...
          const skillCount = readdirSync(join(outputDir, "skills"), {
            ...
            (entry) =>
              entry.isDirectory() &&
              existsSync(join(outputDir, "skills", entry.name, "SKILL.md")),
          ).length;
          ...
          expect(
            files.filter((file) => file.split("/").includes("node_modules")),
          ).toEqual([]);
          ...
          expect(
            readFileSync(
              join(repoRoot, "packages", "omo-native", ".gitignore"),
              "utf8",
            ),
          ).toBe(
            "/plugin/\n",
...
function scannableFiles(root: string): string[] {
  return listRelativeFiles(root)
    ...
    .filter((relative) =>
      statSync(join(root, relative)).size <= MAX_SCANNED_BYTES
    );
}
  ...
  describe("#given the staged omo-ai payload", () => {
    describe("#when every shipped text file is scanned", () => {
      test("#then no file claims to be the senpi harness", () => {
        ...
        if (root === undefined) {
          expect(existsSync(join(repoRoot, "packages", "omo-native"))).toBe(
            true,
          );
          return;
        }
      ...
      test("#then the guard actually detects an identity claim", () => {
        expect(IDENTITY_CLAIM.test("You are senpi, a coding agent.")).toBe(
          true,
        );
        expect(IDENTITY_CLAIM.test("i am senpi")).toBe(true);
        expect(IDENTITY_CLAIM.test("senpi is the engine this skill documents"))
          .toBe(false);
      });
    });
  });
