...
const secrets = [
  "SK-SENTINEL-DO-NOT-LOG-1",
  "SK-SENTINEL-DO-NOT-LOG-2",
  "SK-SENTINEL-DO-NOT-LOG-3",
  "SK-SENTINEL-DO-NOT-LOG-4",
];
...
function fixture(): Fixture {
  ...
  write(
    ...
    JSON.stringify({ name: "omo-ai", version: "test", type: "module" }),
  );
...
function run(item: Fixture, args: string[], ttyInput?: string) {
  ...
  const result = ttyInput === undefined
    ? spawnSync(process.execPath, [item.launcher, ...args], {
      encoding: "utf8",
      env,
    })
    : spawnSync("python3", [
...
function expectSourcesUntouched(
  before: ReturnType<typeof sourceSnapshot>,
): void {
  expect(before.paths.map(hash)).toEqual(before.hashes);
  for (
    const path of before.paths.filter((value) => value.endsWith("agent.db"))
  ) {
...
describe("omo setup credential inheritance", () => {
  ...
  test.skipIf(!SQLITE_AVAILABLE)(
    "#given pinned omp and gjc databases #when accepted #then allow-listed rows import and unknown schema is noticed",
    async () => {
      const item = fixture();
      await database(join(item.home, ".omp", "agent", "agent.db"), 7, [[
        "google",
        "api_key",
        secrets[0],
        null,
      ...
      await database(join(unknown.home, ".omp", "agent", "agent.db"), 99, [[
        "google",
        "api_key",
        secrets[0],
        null,
      ...
      expect(unknownResult.stdout).toContain(
        "auth schema version 99 is unknown",
      );
  ...
  test("#given an existing senpi provider #when other keys import #then its entry stays structurally byte-identical", () => {
    ...
    const existing = {
      type: "api_key",
      key: "EXISTING-NOT-A-SENTINEL",
      metadata: { order: [3, 2, 1] },
    };
    write(
      join(item.agentDir, "auth.json"),
      JSON.stringify({ google: existing }),
    );
    write(
      join(item.xdg, "opencode", "auth.json"),
      JSON.stringify({
        google: { type: "api", key: secrets[0] },
        openai: { type: "api", key: secrets[1] },
      }),
    );
  ...
  test("#given a pre-existing auth file #when import writes #then mode backup and idempotency are exact", () => {
    ...
    if (process.platform !== "win32") {
      expect(statSync(join(item.agentDir, "auth.json")).mode & 0o777).toBe(
        0o600,
      );
    }
    ...
    expect(readFileSync(join(item.agentDir, "auth.json"), "utf8")).toBe(
      afterFirst,
    );
  ...
  test.skipIf(process.platform === "win32")(
    "#given declined consent #when setup runs #then auth remains absent",
    () => {
      const item = fixture();
      write(
        join(item.xdg, "opencode", "auth.json"),
  ...
  test("#given malformed senpi auth #when accepted #then it warns and skips all writes", () => {
    ...
    expect(result.stdout).toContain(
      "WARN senpi: malformed auth.json; credentials were not imported",
    );
  ...
  describe("#given no agent directory is configured", () => {
    describe("#when credentials are imported", () => {
      test("#then they land in the canonical branded directory", () => {
        ...
        expect(result.status).toBe(0);
        expect(existsSync(join(item.home, ".omo", "agent", "auth.json"))).toBe(
          true,
        );
      });
    });
