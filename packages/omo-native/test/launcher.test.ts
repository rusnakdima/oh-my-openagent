...
function createFixture(
  ...
  const root = expandShortPath(
    realpathSync(mkdtempSync(join(tmpdir(), "omo-launcher-"))),
  );
  roots.push(root);
  ...
  if (options.installLayout === "npm") {
    writeFile(
      join(root, "prefix", "lib", "node_modules", ".package-lock.json"),
      "{}\n",
    );
  }

  const modulesRoot = options.hoisted
  ...
  writeFile(
    join(senpiRoot, "dist", "index.js"),
    "export const fixture = true\n",
  );
  writeFile(
    join(senpiRoot, "dist", "cli.js"),
    `
...
`,
  ...
  writeFile(
    join(toolkitRuntime, "ulw-loop", "cli.js"),
    `
import { writeFileSync } from "node:fs"
...
function capture(
  fixture: Fixture,
): { argv: string[]; env: NodeJS.ProcessEnv; target?: string } {
  ...
  describe("#given a fake senpi package", () => {
    describe("#when the default command is launched", () => {
      ...
      test("#then launcher environment points to existing hoisted shims", () => {
        ...
        const path = Object.entries(environment).find(([key]) =>
          key.toLowerCase() === "path"
        )?.[1];
        const binDir = path?.split(process.platform === "win32" ? ";" : ":")[0];
        expect(binDir).toBeDefined();
        expect(realpathSync.native(binDir ?? "")).toBe(
          realpathSync.native(dirname(fixture.shimPath ?? "")),
        );
        expect(existsSync(binDir ?? "")).toBe(true);
        expect(environment.OMO_AGENT_TOOLKIT_BIN).toBe(
          join(fixture.packageRoot, "bin", "omo-agent-toolkit.js"),
        );
        expect(environment.OMO_CODING_AGENT_DIR).toBe(
          join(home, ".omo", "agent"),
        );
        expect(environment.SENPI_CODING_AGENT_DIR).toBe(
          join(home, ".omo", "agent"),
        );
        ...
        expect((environment.OMO_BIN ?? "").replace(/\\/g, "/")).toMatch(
          /\/bin\/omo\.js$/,
        );
    ...
    describe("#when the version is requested on its own", () => {
      test("#then the product version is reported with its engine version, without spawning senpi", () => {
        ...
        expect(result.stdout.trim()).toBe(
          "omo 1.2.3-test.0 (engine: senpi 2026.8.9)",
        );
    ...
    describe("#when a self-update is requested", () => {
      for (
        const args of [["update", "--self"], ["update", "self"], [
          "update",
          "senpi",
        ]]
      ) {
    ...
    describe("#when an early senpi command is launched", () => {
      for (
        ...
        test(`#then ${label} passes through without an extension argument`, () => {
          ...
          expect(captured.env.OMO_AGENT_TOOLKIT_BIN).toBe(
            join(fixture.packageRoot, "bin", "omo-agent-toolkit.js"),
          );
          expect((captured.env.OMO_BIN ?? "").replace(/\\/g, "/")).toMatch(
            /\/bin\/omo\.js$/,
          );
        });
      }
    ...
    describe("#when bare update is requested", () => {
      test("#then npm beta guidance is printed without spawning senpi", () => {
        ...
        expect(result.stdout).toContain(
          "omo is updated via npm: npm i -g omo-ai@beta",
        );
        expect(existsSync(fixture.captureFile)).toBe(false);
      });
      ...
      test("#then a Windows-style Bun path uses shell-compatible forward slashes", () => {
        const root = String
          .raw`C:\Users\omo user\.bun\install\global\node_modules\omo-ai`;

        expect(updateTarget(root, "win32")).toEqual({
          manager: "bun",
          command:
            'bun add --cwd "C:/Users/omo user/.bun/install/global/node_modules/omo-ai" -g omo-ai@beta',
        });
      });
    ...
    describe("#given a legacy Bun global manifest contains the empty dot dependency", () => {
      test("#then launching omo removes only the invalid dependency entry", () => {
        ...
        expect(result.status).toBe(0);
        expect(JSON.parse(readFileSync(manifestPath, "utf8")).dependencies)
          .toEqual({
            "left-pad": "1.3.0",
          });
      });
      ...
      test("#then a different empty dependency value is preserved", () => {
        ...
        const original = `${
          JSON.stringify(
            { dependencies: { "": "file:.", "left-pad": "1.3.0" } },
            null,
            2,
          )
        }\n`;
    ...
    describe("#when the child terminates", () => {
      ...
      test.skipIf(process.platform === "win32")(
        "#then SIGINT is re-raised by the launcher",
        () => {
          const fixture = createFixture();
    ...
    describe("#given the senpi CLI is missing", () => {
      ...
      test("#then a missing dependency fails with the same reinstall guidance", () => {
        const fixture = createFixture();
        rmSync(
          join(fixture.packageRoot, "node_modules", "@code-yeongyu", "senpi"),
          { recursive: true },
        );
        const result = run(fixture, ["say", "hi"]);
        expect(result.status).toBe(1);
        expect(result.stderr).toContain(
          "could not resolve @code-yeongyu/senpi",
        );
        expect(result.stderr).toContain("reinstall with: npm i -g omo-ai@beta");
      });
    });
