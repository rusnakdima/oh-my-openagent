...
function installedSenpi():
  | { readonly root: string; readonly version: string }
  | undefined {
  const candidates = [
    resolve(
      import.meta.dir,
  ...
  describe("#given the engine build the launcher spawns", () => {
    describe("#when the pin is inspected", () => {
      ...
      test("#then a brand-aware pin must ship the brand resolver", () => {
        ...
        if (
          FIRST_BRAND_AWARE_SENPI === null || installed === undefined ||
          installed.version !== PINNED_VERSION
        ) {
        ...
        expect({
          ...
          brandAware: existsSync(join(installed.root, BRAND_CONTRACT_MODULE)),
        })
