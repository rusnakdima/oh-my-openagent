...
function redactCleanupErrorMessage(message: string): string {
  const sensitiveHeaderKey =
    "(?:authorization|x-api-key|api-key|x-auth-token|auth-token|x-access-token|access-token)";
  const messageWithRedactedAuthorization = message
    .replace(
      new RegExp(`("${sensitiveHeaderKey}"\\s*:\\s*")([^"]*)(")`, "gi"),
      "$1[REDACTED]$3",
    )
    .replace(
      new RegExp(`('${sensitiveHeaderKey}'\\s*:\\s*')([^']*)(')`, "gi"),
  ...
  const messageWithRedactedSecrets = redactSensitiveData(
    messageWithRedactedAuthorization,
  );
  return messageWithRedactedSecrets.replace(
    /https?:\/\/[^\s"'<>)}\]]+/g,
    (url) => redactUrl(url),
  );
}
...
async function closeHttpResourceIgnoringFailure(
  ...
  context: {
    ...
    serverName: string;
    phase: "connect-failure" | "post-shutdown";
  },
): Promise<void> {
  try {
    await close();
    ...
    httpClientDependencies.log(
      "[skill-mcp-http-client] ignored cleanup failure",
      {
        ...context,
...
export async function createHttpClient(
  ...
  try {
    ...
    throw new Error(
      `MCP server "${info.serverName}" has invalid URL: ${
        redactUrl(config.url)
      }\n\n` +
  ...
  const transport: McpTransport = httpClientDependencies.createTransport(
    url,
    transportOptions,
  );
  ...
  try {
    ...
    const errorMessage = redactCleanupErrorMessage(
