...
export function getMcpOauthServerHash(
  serverHost: string,
  resource: string,
): string {
  return createHash("sha256").update(buildKey(serverHost, resource)).digest(
    "hex",
  ).slice(0, 32);
}
...
export function getMcpOauthStoragePath(
  ...
  return join(
    ...
    `${getMcpOauthServerHash(serverHost, resource)}.json`,
  );
}
...
export function loadToken(
  ...
  return readTokenFile(getMcpOauthStoragePath(serverHost, resource)) ??
    readLegacyStore()?.[buildKey(serverHost, resource)] ?? null;
}
...
export function listTokensByHost(serverHost: string): TokenStore {
  ...
  for (const [hash, indexedKey] of Object.entries(index)) {
    ...
    const indexedToken = readTokenFile(
      join(getMcpOauthStorageDir(), `${hash}.json`),
    );
