...
function parseMetadataFields(
  ...
  const registrationEndpoint = typeof registrationEndpointValue === "string" &&
      registrationEndpointValue.length > 0
    ? parseHttpsUrl(registrationEndpointValue, "registration_endpoint")
      .toString()
    : undefined;
...
function parseAuthorizationServers(
  ...
  return servers.filter((server): server is string =>
...
export async function discoverOAuthServerMetadata(
  ...
  const discoveryPromise = (async () => {
    const prmUrl = new URL("/.well-known/oauth-protected-resource", resourceUrl)
      .toString();
