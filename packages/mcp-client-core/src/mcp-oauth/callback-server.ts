...
async function waitForServerAcceptingHttpRequests(port: number): Promise<void> {
  ...
  throw new Error(
    `OAuth callback server did not accept HTTP requests on port ${port}`,
  );
}
...
export async function startCallbackServer(
  ...
  const requestedPort = startPort === 0
    ? 0
    : await findAvailablePort(startPort).catch(() => 0);
