...
export function writeTokenIndex(
  storageDir: string,
  index: TokenIndex,
): boolean {
  try {
...
export function saveTokenIndexEntry(
  storageDir: string,
  hash: string,
  key: string,
): boolean {
  return writeTokenIndex(storageDir, {
    ...readTokenIndex(storageDir),
    [hash]: key,
  });
}

export function deleteTokenIndexEntry(
  storageDir: string,
  hash: string,
): boolean {
