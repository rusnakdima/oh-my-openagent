...
async function createTemporaryDirectory(): Promise<string> {
  const directoryPath = await fs.mkdtemp(
    path.join(os.tmpdir(), "tmux-runner-"),
  );
...
async function createFakeCmux(
  directoryPath: string,
  argsFilePath: string,
): Promise<string> {
  const cmuxPath = path.join(
    directoryPath,
    process.platform === "win32" ? "cmux.cmd" : "cmux",
  );
...
describe("runTmuxCommand", () => {
  test("#given cmux socket and real tmux session #when run #then uses requested executable instead of cmux compat", async () => {
    ...
    try {
      // when
      const result = await runTmuxCommand("sh", [
        "-c",
        "printf '%s\\n' real-tmux",
      ]);
  ...
  test("#given retry=2 and first exit nonzero #when run #then calls spawn twice before returning failure", async () => {
    ...
    const commandScript =
      `counter_file="$1"; count=0; if [ -f "$counter_file" ]; then count=$(cat "$counter_file"); fi; count=$((count + 1)); printf '%s' "$count" > "$counter_file"; printf '%s\\n' 'temporary error' >&2; exit 1`;
  ...
  test("#given cmux environment #when run #then delegates through cmux tmux compatibility command", async () => {
    ...
    process.env.PATH = `${temporaryDirectory}${path.delimiter}${
      originalPath ?? ""
    }`;

    // when
    const result = await runTmuxCommand(cmuxPath, [
      "display-message",
      "-p",
      "#{pane_id}",
    ]);
    ...
    expect(normalizeLineEndings(argsFileContent)).toBe(
      "__tmux-compat\ndisplay-message\n-p\n#{pane_id}\n",
    );
  });
});
