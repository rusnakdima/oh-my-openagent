...
async function loadReplaceTmuxPane(): Promise<
  typeof import("./pane-replace").replaceTmuxPane
> {
  const module = await import(
    `${paneReplaceSpecifier}?test=${crypto.randomUUID()}`
  );
  return module.replaceTmuxPane;
}

function createDeps(): NonNullable<
  Parameters<typeof import("./pane-replace").replaceTmuxPane>[6]
> {
  return {
    log: logMock,
...
describe("replaceTmuxPane runner integration", () => {
  ...
  it("#given existing pane #when replaceTmuxPane called #then delegates send-keys, respawn-pane, and select-pane to shared runner", async () => {
    ...
    expect(respawnCall[1].slice(targetIndex, targetIndex + 2)).toEqual([
      "-t",
      "%42",
    ]);
    expect(selectPaneCall[1]).toEqual([
      "select-pane",
      "-t",
      "%42",
      "-T",
