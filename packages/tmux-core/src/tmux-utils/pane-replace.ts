...
async function resolveReplaceTmuxPaneDeps(
  deps?: Partial<ReplaceTmuxPaneDeps>,
): Promise<ReplaceTmuxPaneDeps> {
...
export async function replaceTmuxPane(
  ...
  if (isCmuxCompatEnvironment() && authEnvArgs.length > 0) {
    log("[replaceTmuxPane] SKIP: authenticated cmux panes are unsupported", {
      paneId,
      sessionId,
    });
    return { success: false };
