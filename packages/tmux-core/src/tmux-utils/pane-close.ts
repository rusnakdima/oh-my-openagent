...
export async function closeTmuxPaneWithDependencies(
  ...
  dependencies.log("[closeTmuxPane] sending Ctrl+C for graceful shutdown", {
    paneId,
  });
  ...
  const result = await dependencies.runTmuxCommand(tmux, [
    "kill-pane",
    "-t",
    paneId,
  ]);
  const trimmedStderr = result.stderr.trim();
  const paneAlreadyGone = result.exitCode !== 0 &&
    /can't find pane/i.test(trimmedStderr);
  ...
  if (result.exitCode !== 0) {
    dependencies.log("[closeTmuxPane] FAILED", {
      paneId,
