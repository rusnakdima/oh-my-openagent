...
function getRunTmuxCommandCall(index: number): [string, string[]] {
  ...
  if (
    !Array.isArray(call) || typeof command !== "string" || !Array.isArray(args)
  ) {
...
function createDeps(): NonNullable<
  Parameters<typeof import("./pane-spawn").spawnTmuxPane>[7]
> {
  return {
    log: logMock,
...
async function loadSpawnTmuxPane(): Promise<
  typeof import("./pane-spawn").spawnTmuxPane
> {
  const module = await import(
    `${paneSpawnSpecifier}?test=${crypto.randomUUID()}`
  );
...
describe("spawnTmuxPane runner integration", () => {
  beforeEach(() => {
    ...
    runTmuxCommandMock.mockImplementation(
      async (): Promise<TmuxCommandResult> => {
  ...
  it("#given description with spaces #when spawnTmuxPane called #then includes it in the placeholder", async () => {
    ...
    expect(getSplitWindowCommand()).toContain(
      "OMO subagent pane ready: worker with spaces",
    );
  });
  ...
  describe("eligibility gate (headless cmux: CMUX_SOCKET_PATH without TMUX)", () => {
    ...
    afterEach(() => {
      ...
      } else process.env.CMUX_SOCKET_PATH = savedCmuxSocketPath;
    });

    function createHeadlessDeps(): NonNullable<
      Parameters<typeof import("./pane-spawn").spawnTmuxPane>[7]
    > {
      return {
        log: logMock,
    ...
    it("#given CMUX_SOCKET_PATH set and TMUX unset #when spawnTmuxPane called #then split-window runs opencode attach eagerly", async () => {
      // given
    ...
    it("#given neither TMUX nor CMUX_SOCKET_PATH #when spawnTmuxPane called #then skips without issuing tmux commands (regression guard)", async () => {
      // given
      delete process.env.TMUX;
      delete process.env.TMUX_PANE;
      delete process.env.CMUX_SOCKET_PATH;
