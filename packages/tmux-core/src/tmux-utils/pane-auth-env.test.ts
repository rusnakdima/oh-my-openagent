...
function expectNoAuthEnvArgs(args: readonly string[]): void {
  ...
  expect(args.some((arg) => arg.startsWith("OPENCODE_SERVER_PASSWORD="))).toBe(
    false,
  );
  expect(args.some((arg) => arg.startsWith("OPENCODE_SERVER_USERNAME="))).toBe(
    false,
  );
}
...
describe("tmux pane auth environment propagation", () => {
  ...
  it("#given auth env vars with spaces #when activateTmuxPane respawns attach #then tmux receives env args and quoted attach values", async () => {
    ...
    // when
    await activateTmuxPane(
      "%42",
      "session with spaces",
      "http://127.0.0.1:4321/path with spaces",
  ...
  it("#given auth env vars #when pane lifecycle commands spawn or respawn #then every tmux call site receives env args", async () => {
    // given
    setAuthEnv("pa ss word", "user name");
    const paneRecorder = createTmuxCommandRecorder([
      successResult("%pane"),
      successResult(),
    ]);
    const windowRecorder = createTmuxCommandRecorder([
      successResult("%window"),
      successResult(),
    ]);
    const sessionRecorder = createTmuxCommandRecorder([
      successResult("120,40"),
      failedResult(),
      successResult("%session"),
      successResult(),
    ]);
    const existingSessionRecorder = createTmuxCommandRecorder([
      failedResult(),
      successResult("%existing"),
      successResult(),
    ]);
    const replaceRecorder = createTmuxCommandRecorder([
      successResult(),
      successResult(),
      successResult(),
    ]);
