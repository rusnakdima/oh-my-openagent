...
async function resolveSpawnTmuxSessionDeps(
  deps?: Partial<SpawnTmuxSessionDeps>,
): Promise<SpawnTmuxSessionDeps> {
...
export async function spawnTmuxSession(
  ...
  const sessionAlreadyExists = await sessionExists(
    tmux,
    isolatedSessionName,
    runTmuxCommand,
  );
