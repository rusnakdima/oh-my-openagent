...
export async function killTmuxSessionIfExists(
  ...
  if (!deps.isInsideTmux()) {
    deps.log("[killTmuxSessionIfExists] SKIP: not inside tmux", {
      sessionName,
    });
    return false;
  }
  ...
  const hasSessionResult = await deps.runTmuxCommand(tmux, [
    "has-session",
    "-t",
    sessionName,
  ]);
  ...
  if (hasSessionResult.exitCode !== 0) {
    deps.log("[killTmuxSessionIfExists] SKIP: session not found", {
      sessionName,
    });
    return false;
  }

  const killSessionResult = await deps.runTmuxCommand(tmux, [
    "kill-session",
    "-t",
    sessionName,
  ]);
