...
export function isTmuxPaneCompatibleEnvironment(
  environment: Record<string, string | undefined>,
): boolean {
  return isInsideTmuxEnvironment(environment) ||
    isCmuxCompatEnvironment(environment);
}
...
export function isNativeTmuxEnvironment(
  environment: Record<string, string | undefined>,
): boolean {
  return isInsideTmuxEnvironment(environment) &&
    !isCmuxCompatEnvironment(environment);
