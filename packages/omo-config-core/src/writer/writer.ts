...
function writeBackup(
  path: string,
  content: string,
  fileSystem: typeof DEFAULT_WRITE_FILE_SYSTEM,
): string {
...
function assertConfigPathIsSafe(
  path: string,
  fileSystem: typeof DEFAULT_WRITE_FILE_SYSTEM,
): void {
  try {
    if (fileSystem.lstatSync(path).isSymbolicLink()) {
      throw new OmoConfigWriteError(
        path,
        "read",
        new Error("Refusing to edit symlinked omo config"),
      );
    }
  } catch (error) {
...
function assertProjectConfigDirectoryIsSafe(
  directory: string,
  fileSystem: typeof DEFAULT_WRITE_FILE_SYSTEM,
): void {
  try {
...
export function updateOmoConfig(
  options: UpdateOmoConfigOptions,
): UpdateOmoConfigResult {
  const fileSystem = options.fileSystem ?? DEFAULT_WRITE_FILE_SYSTEM;
  ...
  for (const edit of options.edits) {
    nextContent = applyEdits(
      nextContent,
      modify(nextContent, [...edit.path], edit.value, {
