...
`,
    ...
    const backupFiles = readdirSync(join(configPath, "..")).filter((
      entry: string,
    ) => entry.includes(".bak."));
...
`,
    ...
    try {
      ...
      expect(first.backupPath).toBe(
        `${configPath}.bak.2026-07-06T00-00-00-000Z`,
      );
      expect(second.backupPath).toStartWith(
        `${configPath}.bak.2026-07-06T00-00-00-000Z`,
      );
  ...
  test("#given legacy dirs and env vars present #when editing user scope #then the write target never leaves ~/.omo", () => {
    ...
    expect(readFileSync(join(legacyDir, "omo.jsonc"), "utf-8")).toContain(
