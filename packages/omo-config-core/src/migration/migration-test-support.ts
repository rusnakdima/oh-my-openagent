...
export class MemoryMigrationFileSystem implements MigrationFileSystem {
  ...
  mkdirSync(
    path: string,
    _options: { readonly recursive: true },
  ): string | undefined {
  ...
  readFileSync(path: string, _encoding: "utf-8"): string {
    ...
    if (content === undefined) {
      throw fileError("ENOENT", `Missing ${this.key(path)}`);
    }
    return content;
  }
  ...
  writeFileExclusiveSync(path: string, content: string): void {
    if (this.files.has(this.key(path))) {
      throw fileError("EEXIST", `Already exists ${this.key(path)}`);
    }
    this.files.set(this.key(path), content);
