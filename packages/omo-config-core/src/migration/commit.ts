...
function parseDocument(path: string, content: string): Record<string, unknown> {
  ...
  if (parsed.errors.length > 0 || !isPlainObject(parsed.data)) {
    ...
    throw new MigrationTransactionError(
      `Migration document at ${path} is not a JSONC object${
        detail === "" ? "" : `: ${detail}`
      }`,
    );
  }
...
function markerValue(
  ...
  return hasMigrationMarker(target, migrationId)
    ? value
    : [...value, migrationId];
}
...
): { readonly projectDir?: string; readonly scope: "project" | "user" } {
  const homeDir = resolveHomeDir(env);
  const userDirectory = toPosixPath(join(homeDir, ".omo"));
  const fileName = basename(targetPath);
  if (
    toPosixPath(dirname(targetPath)) === userDirectory &&
    (fileName === "omo.json" || fileName === "omo.jsonc")
  ) {
    return { scope: "user" };
  }
  if (
    basename(dirname(targetPath)) === ".omo" &&
    (fileName === "omo.json" || fileName === "omo.jsonc")
  ) {
    return { projectDir: dirname(dirname(targetPath)), scope: "project" };
  }
  throw new MigrationTransactionError(
    `Migration target is not an omo config path: ${targetPath}`,
  );
}
...
export const writeOmoMigrationTarget: MigrationTargetWriter = (input): void => {
  ...
  if (!sameResolvedPath(result.path, input.targetPath)) {
    throw new MigrationTransactionError(
      `Migration writer resolved ${result.path} instead of ${input.targetPath}`,
    );
  }
};
...
export function prepareTargetReplacement(input: {
  ...
  for (const key of Object.keys(input.target)) {
    if (
      key !== "_migrations" &&
      !Object.prototype.hasOwnProperty.call(input.document, key)
    ) {
