...
function parseLockRecord(content: string): LockRecord | null {
  try {
    ...
    if (
      typeof leaseExpiresAt !== "number" || !Number.isFinite(leaseExpiresAt)
    ) return null;
...
function leaseContent(
  ...
  return `${
    JSON.stringify({ leaseExpiresAt: clock.now() + duration, pid: process.pid })
  }\n`;
}
...
function isReclaimable(
  ...
  return record.leaseExpiresAt +
      leaseDurationMs * LIVE_OWNER_STALE_LEASE_MULTIPLIER <= now;
}
...
function acquireMutationGuard(input: {
  ...
  for (
    let attempt = 0;
    attempt <= MUTATION_GUARD_RETRY_DELAYS_MS.length;
    attempt += 1
  ) {
    const content = leaseContent(
      input.process,
      input.clock,
      GUARD_LEASE_DURATION_MS,
    );
    try {
...
function releaseMutationGuard(input: {
  ...
  input.fileSystem.removeIfContentsMatchSync(
    mutationGuardPath(input.env),
    input.content,
  );
}
...
export function acquireMigrationLock(input: {
  ...
  input.fileSystem.mkdirSync(
    toPosixPath(join(resolveHomeDir(input.env), ".omo")),
    { recursive: true },
  );

  for (let attempt = 0; attempt < 3; attempt += 1) {
    const currentContent = leaseContent(
      input.process,
      input.clock,
      leaseDurationMs,
    );
    try {
      ...
      const mutate = (mutation: (nextContent: string) => boolean): void => {
        ...
        try {
          const renewedContent = leaseContent(
            input.process,
            input.clock,
            leaseDurationMs,
          );
      ...
      return {
        ...
        renew: (): void => {
          mutate((renewedContent) =>
            input.fileSystem.replaceIfContentsMatchSync(
              path,
              ownedContent,
              renewedContent,
            )
    ...
    try {
      ...
      if (
        observed !== null &&
        !isReclaimable(observed, input.clock, input.process, leaseDurationMs)
      ) return null;
      input.fileSystem.removeIfContentsMatchSync(path, observedContent);
    } finally {
      releaseMutationGuard({
