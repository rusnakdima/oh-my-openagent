...
describe("runMigration", () => {
  test("#given a legacy value collides with a target value #when migrating #then the target value wins with a skipped diagnostic", () => {
    ...
    expect(result.diagnostics).toEqual([
      "skipped: task.default_concurrency legacy=3 kept=5",
    ]);
    expect(parseFile(fileSystem, migrationFixture.targetPath).task).toEqual({
  ...
  test("#given a user target marker #when a project target is discovered later #then the project target migrates independently", () => {
    ...
    // then
    expect(result.status).toBe("migrated");
    expect(parseFile(fileSystem, projectTarget)._migrations).toEqual([
      "legacy-task",
    ]);
  ...
  test("#given a transformed document rejected by OmoConfigSchema #when migrating #then target and sources are untouched", () => {
    ...
    const targetBefore = fileSystem.readFileSync(
      migrationFixture.targetPath,
      "utf-8",
    );
    ...
    expect(fileSystem.readFileSync(migrationFixture.targetPath, "utf-8")).toBe(
      targetBefore,
    );
    expect(fileSystem.existsSync(migrationFixture.sourcePath)).toBe(true);
    expect(fileSystem.existsSync("/home/alice/.omo/.migration-journal.json"))
      .toBe(false);
  });
});
