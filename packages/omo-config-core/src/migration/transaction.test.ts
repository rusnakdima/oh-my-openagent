...
describe("runMigration transaction ownership", () => {
  test("#given two callers overlap #when one holds the exclusive lock #then one migration commits and the other observes completed state on retry", () => {
    ...
    expect(parseFile(fileSystem, migrationFixture.targetPath)._migrations)
      .toEqual(["concurrent-legacy"]);
  });
  ...
  test("#given a live owner whose lease is fresh #when another caller attempts takeover #then the lock is not stolen", () => {
    ...
    const lockContent = `${
      JSON.stringify({ leaseExpiresAt: 2_000, pid: 99 })
    }\n`;
  ...
  test("#given a false predicate and no journal #when entering the transaction #then no target or source is written", () => {
    ...
    markedFileSystem.files.set(
      migrationFixture.targetPath,
      `{"_migrations":["already-done"]}`,
    );
    ...
    expect(noSourceFileSystem.existsSync(migrationFixture.targetPath)).toBe(
      false,
    );
    expect(markedFileSystem.existsSync(migrationFixture.sourcePath)).toBe(true);
    expect(markedFileSystem.files.get(migrationFixture.targetPath)).toBe(
      `{"_migrations":["already-done"]}`,
    );
    expect(
      noSourceFileSystem.existsSync("/home/alice/.omo/.migration-journal.json"),
    ).toBe(false);
    expect(
      markedFileSystem.existsSync("/home/alice/.omo/.migration-journal.json"),
    ).toBe(false);
  });
});
