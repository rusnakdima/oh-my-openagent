...
import {
  ...
  prepareTargetWrite,
  targetDocument,
  writeOmoMigrationTarget,
  writePreparedTarget,
} from "./commit";
import {
  type MigrationBackupMove,
...
function loadSources(
  ...
  return sources
    ...
    .map((source) => ({
      ...
      value: parseSource(
        source.path,
        fileSystem.readFileSync(source.path, "utf-8"),
      ),
    }));
}
...
function backupBasePath(
  ...
  return source.backupPath ??
    `${source.path}.bak.${encodeURIComponent(migrationId)}`;
}
...
function backupMoves(
  ...
  for (const source of sources) {
    ...
    let attempt = 1;
    while (
      fileSystem.existsSync(destination) || destinations.has(destination)
    ) {
...
function assertSafeSourcePaths(
  sources: readonly MigrationSourceDescriptor[],
  protectedPaths: ReadonlySet<string>,
): void {
  ...
  for (const source of sources) {
    if (seen.has(source.path)) {
      throw new MigrationTransactionError(
        `Duplicate migration source: ${source.path}`,
      );
    }
    if (protectedPaths.has(source.path)) {
      throw new MigrationTransactionError(
        `Migration source is protected: ${source.path}`,
      );
    }
...
function ensureBackupDirectories(
  moves: readonly MigrationBackupMove[],
  fileSystem: MigrationFileSystem,
): void {
  for (const move of moves) {
    fileSystem.mkdirSync(directoryPath(move.to), { recursive: true });
  }
}
...
function transformResult(value: ReturnType<MigrationPlan["transform"]>): {
  ...
  if (
    isPlainObject(value) && isPlainObject(value.document) &&
...
function executePlan(input: {
  ...
  const existingSources = plan.sources.filter((source) =>
    fileSystem.existsSync(source.path)
  );
  const target = targetDocument(plan.targetPath, fileSystem);
  const replaceTarget = plan.mode === "replace-target";
  if (
    !shouldRunMigration({
      legacySourcesExist: replaceTarget
        ? fileSystem.existsSync(plan.targetPath)
  ...
  for (const move of targetRecorded.backupMoves) {
    ...
    if (fileSystem.existsSync(move.to)) {
      throw new MigrationTransactionError(
        `Migration backup path already exists: ${move.to}`,
      );
    }
...
export function runMigrations(
  ...
  try {
    ...
    lock.renew();
    const results = options.discover().map((plan) =>
      executePlan({
        clock,
