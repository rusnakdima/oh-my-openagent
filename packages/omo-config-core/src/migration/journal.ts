...
export function migrationJournalPath(env: MigrationEnvironment): string {
  return toPosixPath(
    join(resolveHomeDir(env), ".omo", ".migration-journal.json"),
  );
}
...
function parseJournal(value: unknown): MigrationJournal {
  ...
  if (
    !isPlainObject(value["targetWrite"]) ||
    !isPlainObject(value["targetWrite"]["additions"])
  ) {
  ...
  if (
    typeof value["targetWritten"] !== "boolean" ||
  ...
  if (
    ...
    (!Array.isArray(diagnostics) ||
      !diagnostics.every((entry) => typeof entry === "string"))
  ) {
  ...
  for (const move of value["backupMoves"]) {
    if (
      !isPlainObject(move) || typeof move["from"] !== "string" ||
...
export function removeMigrationJournal(
  fileSystem: MigrationFileSystem,
  env: MigrationEnvironment,
): void {
  const path = migrationJournalPath(env);
