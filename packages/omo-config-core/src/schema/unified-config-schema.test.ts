...
describe("unified omo config schema", () => {
  test("#given a VSCode-style config with every unified key #when parsed #then all supported sections are preserved", () => {
    ...
    expect(result.data.profiles.focused?.["[codex]"]?.codegraph?.enabled).toBe(
      false,
    );
    expect(result.data._migrations).toEqual([
      "2026-07-opencode-config-unification",
    ]);
    expect(result.data.legacy_migrations?.["legacy-config"]).toEqual({
      migrated: true,
    });
  });
  ...
  test("#given partial model catalog entries in harness and profile overlays #when parsed #then they remain default-free deep-partials", () => {
    ...
    expect(result.data.profiles.focused?.models?.sol).toEqual({
      reasoning: "low",
    });
    expect(result.data.profiles.focused?.["[codex]"]?.models?.sol).toEqual({
      reasoning: "minimal",
    });
  });
  ...
  test("#given an array opencode block #when parsed #then rejection identifies the block path", () => {
    ...
    if (result.success) {
      throw new Error("Expected the opaque opencode block to reject arrays");
    }
    expect(
      result.error.issues.map((issue) => issue.path.join(".")).some((path) =>
  ...
  test("#given base telemetry and an empty codex block #when parsed #then the block stays default-free", () => {
    ...
    expect(Object.hasOwn(result.data["[codex]"]?.codegraph ?? {}, "telemetry"))
      .toBe(false);
  });
});
