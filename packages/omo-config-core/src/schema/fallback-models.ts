...
import {
  type OmoModelRefObject,
  OmoModelRefObjectSchema,
  OmoReasoningSchema,
} from "./model-ref";
import {
  normalizeReasoning,
  splitReasoningSuffix,
} from "./reasoning-vocabulary";
...
function canonicalModelString(model: string): string {
  ...
  const spaced = parenthesized === null
    ? trimmed.match(/^(.*\S)\s+([a-z][a-z0-9_-]*)$/i)
    : null;
  ...
  return normalized.level === undefined
    ? trimmed
    : `${base}:${normalized.level}`;
}

export function normalizeLegacyModelFields(
...
export function normalizeLegacyModelEntry(
  entry: Readonly<Record<string, unknown>>,
): OmoModelRefObject {
...
export type OmoFallbackModelObject = z.infer<
  typeof OmoFallbackModelObjectSchema
>;
