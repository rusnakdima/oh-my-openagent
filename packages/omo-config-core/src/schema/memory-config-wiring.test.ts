...
describe("memory config wiring", () => {
  test("#given a root memory block #when the config parses #then defaults materialize once", () => {
    ...
    expect(result.data.memory?.reflection.trigger).toEqual({
      step_count: 25,
      on_compaction: true,
    });
    expect(result.data.memory?.nudge).toEqual({
      enabled: true,
      every_user_turns: 10,
    });
    expect(result.data.memory?.facts).toEqual({
      enabled: true,
      debounce_settles: 4,
    });
    ...
    expect(result.data.memory?.people).toEqual({
      enabled: true,
      max_entries: 40,
      max_entry_chars: 200,
    });
  ...
  test("#given memory blocks in harness and profile overlays #when parsed #then they stay default-free", () => {
    ...
    expect(result.success).toBe(true);
    if (!result.success) throw new Error(result.error.message);
    expect(result.data["[senpi]"]?.memory).toEqual({
      reflection: { category: "deep" },
    });
    expect(result.data.profiles.focused?.memory).toEqual({
...
describe("memory profile and harness deep-merge", () => {
  test("#given base and senpi memory blocks #when folding the harness view #then memory deep-merges", () => {
    ...
    const config = {
      memory: {
        enabled: true,
        reflection: { timeout_minutes: 20, category: "deep" },
      },
  ...
  test("#given base harness and profile memory blocks #when folding the profile view #then later layers win per leaf", () => {
    ...
    // when
    const result = resolveOmoConfigView({
      config,
      harness: "senpi",
      profile: "focused",
    });
