...
describe("loadOmoConfig", () => {
  test("#given user and walked project omo configs #when loading #then nearest project wins and keyed sections deep-merge", () => {
    ...
    writeJsonc(
      join(fixture.projectDir, ".omo", "oh-my-openagent.jsonc"),
      `{"task":{"default_concurrency":99}}`,
    );
    writeJsonc(
      join(fixture.projectDir, ".omo", "oh-my-opencode.jsonc"),
      `{"task":{"default_concurrency":98}}`,
    );
    ...
    expect(result.config.categories?.quick?.tools).toEqual({
      read: true,
      bash: true,
    });
  ...
  test("#given same-key partial team layers #when loading #then final team merges members and description", () => {
    ...
    // then
    expect(result.diagnostics).toEqual([]);
    expect(result.config.teams?.alpha?.description).toBe(
      "near layer description",
    );
    expect(result.config.teams?.alpha?.members[0]?.name).toBe("one");
  });
  ...
  test("#given symlinked project omo directory #when loading #then target config is ignored", () => {
    ...
    writeJsonc(
      join(outsideConfigDir, "omo.jsonc"),
    ...
    expect(
      result.sources.some((source) =>
        source.scope === "project" && source.loaded
      ),
    ).toBe(false);
  });
  ...
  test("#given malformed and unreadable configs #when loading #then defaults survive and typed diagnostics identify paths", () => {
    ...
    expect(result.config.task?.default_concurrency).toBe(5);
    expect(
      result.diagnostics.map((diagnostic: { readonly kind: string }) =>
        diagnostic.kind
      ),
    ...
    expect(
      result.diagnostics.some((diagnostic: { readonly path: string }) =>
        diagnostic.path === unreadablePath
      ),
    ).toBe(true);
  });
  ...
  test("#given only user omo json #when loading #then user json is read and jsonc takes precedence when both exist", () => {
    ...
    expect(jsonOnly.sources[0]).toEqual({
      exists: true,
      loaded: true,
      path: jsonOnlyPath,
      scope: "user",
...
test("#given cwd outside home #when loading #then ancestor project configs are read", () => {
  ...
  writeJsonc(
    join(outsideProject, ".omo", "omo.jsonc"),
    `{"task":{"default_concurrency":7}}`,
  );
  writeJsonc(
    join(root, "elsewhere", ".omo", "omo.jsonc"),
    `{"task":{"default_concurrency":9}}`,
  );
  ...
  expect(
    result.sources.some((source) =>
      source.path.endsWith(join("elsewhere", ".omo", "omo.jsonc"))
    ),
  ).toBe(true);
});
