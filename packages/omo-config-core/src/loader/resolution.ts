...
const HARNESS_KEYS = [...new Set([...HARNESS_IDS, ...OMO_CONFIG_HARNESS_IDS])]
  .map((harness) => `[${harness}]`);
...
function profileNameFromOpenCodeConfigDir(
...
export function resolveOmoProfileName(
  options: ResolveOmoProfileNameOptions = {},
): string | undefined {
...
export function resolveOmoConfigView(
  options: ResolveOmoConfigViewOptions,
): ResolveOmoConfigViewResult {
  const profiles = toRecord(options.config["profiles"]);
  const profile = options.profile === undefined
    ? undefined
    : toRecord(profiles?.[options.profile]);
  const diagnostics: OmoConfigDiagnostic[] =
    profile === undefined && options.profile !== undefined
      ? [{
        kind: "profile",
        message:
          `Activated omo profile \"${options.profile}\" does not exist; using the base configuration`,
  ...
  const resolvedProfile = options.profile !== undefined && profile !== undefined
    ? options.profile
    : undefined;
  return {
