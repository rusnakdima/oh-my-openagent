...
function loadSenpi(
  fixture: { readonly cwd: string; readonly homeDir: string },
  profile?: string,
) {
...
describe("loadOmoConfig telemetry resolution", () => {
  ...
  test("#given base telemetry is disabled and the senpi block enables it #when loading the senpi view #then the senpi override wins", () => {
    ...
    writeUserConfig(
      fixture.homeDir,
      `{"telemetry":{"enabled":false},"[senpi]":{"telemetry":{"enabled":true}}}`,
    );
  ...
  test("#given an unknown sibling inside telemetry #when loading the senpi view #then the strict block is rejected", () => {
    ...
    writeUserConfig(
      fixture.homeDir,
      `{"telemetry":{"enabled":false,"unexpected":true}}`,
    );
  ...
  test.each([
    ...
    [
      ...
      `{"telemetry":{"enabled":{"nested":{"garbage":true}}}}`,
    ],
  ])(
    "#given telemetry enabled is %s #when loading the senpi view #then malformed input is rejected",
    (_name, content) => {
      // given
      const fixture = makeFixture();
      ...
      try {
        ...
        expect(result.diagnostics).toHaveLength(1);
        expect(result.diagnostics[0]).toMatchObject({ kind: "validation" });
        expect(result.diagnostics[0]?.issuePaths).toContain(
          "telemetry.enabled",
        );
        expect(isOmoTelemetryEnabled(result.config)).toBe(true);
      } finally {
