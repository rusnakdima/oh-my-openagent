...
function readConfigSource(
  ...
  if (parsed.errors.length > 0) {
    return {
      diagnostic: {
        kind: "parse",
        message: `JSONC parse error in ${path}: ${
...
export function loadOmoConfig(
  options: LoadOmoConfigOptions = {},
): LoadOmoConfigResult {
  ...
  for (
    const candidate of resolveOmoConfigPaths({
      cwd,
  ...
  const finalConfig = OmoConfigSchema.safeParse(
    mergeOmoConfigRecords(DEFAULT_RAW_CONFIG, resolved.config),
  );
  ...
  return {
    config: stripResolutionControlKeys(
      OmoConfigSchema.parse(DEFAULT_RAW_CONFIG),
    ) satisfies OmoConfig,
    diagnostics: [
      ...diagnostics,
