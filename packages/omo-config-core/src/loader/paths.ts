...
export function resolveHomeDir(env: OmoConfigEnv = process.env): string {
  ...
  return homeDir.startsWith("/")
    ? posix.resolve(homeDir)
    : toPosixPath(resolve(homeDir));
}
...
function isSymlinkedProjectPath(
  path: string,
  fileSystem: OmoConfigReadFileSystem,
): boolean {
...
function isLoadableProjectConfigFile(
  path: string,
  fileSystem: OmoConfigReadFileSystem,
): boolean {
  return fileSystem.existsSync(path) &&
    !isSymlinkedProjectPath(path, fileSystem);
}

function detectOmoJsonPath(
  dir: string,
  fileSystem: OmoConfigReadFileSystem,
): string | null {
...
export function findProjectConfigPathsFarthestFirst(
  ...
  const boundaryDirs = [
    ...new Set([resolve(homeDir), resolve(accountHomeDir)]),
  ];
  const realBoundaryDirs = new Set(
    boundaryDirs.map((path) => realpathOrSelf(path, fileSystem)),
  );
  const nearestFirst: string[] = [];
  let currentDir = startDir;

  for (let depth = 0; depth < MAX_PROJECT_CONFIG_DIRECTORY_DEPTH; depth += 1) {
    const isHomeDir = boundaryDirs.includes(currentDir) ||
      realBoundaryDirs.has(realpathOrSelf(currentDir, fileSystem));
    // A home `.omo` is a user layer, so the walk must not also claim it as a project layer.
    const configPath = isHomeDir
      ? null
      : detectOmoJsonPath(currentDir, fileSystem);
...
export function resolveOmoConfigPaths(
  options: ResolveOmoConfigPathsOptions,
): readonly OmoConfigPathCandidate[] {
  const fileSystem = options.fileSystem ?? DEFAULT_READ_FILE_SYSTEM;
  const env = options.env ?? process.env;
  const userPath = detectUserOmoJsonPath(env, fileSystem);
  const projectPaths = findProjectConfigPathsFarthestFirst(
    options.cwd,
    resolveHomeDir(env),
    fileSystem,
    ACCOUNT_HOME_DIR,
  ...
  return [
    ...
    ...projectPaths.map((path): OmoConfigPathCandidate => ({
      path,
      scope: "project",
    })),
  ];
