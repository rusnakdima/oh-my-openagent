...
function resolveModelEntry(
  ...
  const resolved = catalogReference(
    entry.model,
    entry.reasoning,
    catalog,
    cycleNames,
  ...
  return {
    ...
    ...(entry.reasoning === undefined && resolved.reasoning !== undefined
...
function resolveFallbackModels(
  ...
  if (typeof fallbackModels !== "string") {
    return fallbackModels.map((entry) =>
      resolveModelEntry(entry, catalog, cycleNames)
    );
  }
...
function resolveCategoryDefinition(
  ...
  return {
    ...
    ...(definition.fallback_models === undefined ? {} : {
      fallback_models: resolveFallbackModels(
...
export function resolveModelReferences(
  ...
  const agents = view.agents === undefined ? undefined : Object.fromEntries(
    Object.entries(view.agents).map(([name, definition]) => [
      name,
  ...
  const categories = view.categories === undefined
    ? undefined
    : Object.fromEntries(
      Object.entries(view.categories).map(([name, definition]) => [
        name,
