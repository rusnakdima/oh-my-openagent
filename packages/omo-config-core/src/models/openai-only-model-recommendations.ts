...
export function isOpenAiOnlyRuntimeInventory(
  inventory: readonly OpenAiRuntimeModelIdentity[],
): boolean {
  return inventory.length > 0 &&
    inventory.every((model) =>
      canonicalOpenAiRuntimeModelId(model) !== undefined
    );
}
...
export function compileOpenAiOnlyModelRecommendations(
  ...
  return {
    agents: compileRecommendations(
      OPENAI_ONLY_AGENT_MODEL_RECOMMENDATIONS,
      inventory,
    ),
    categories: compileRecommendations(
...
export function canonicalOpenAiRuntimeModelId(
  model: OpenAiRuntimeModelIdentity,
): string | undefined {
  if (model.upstreamIdentityInvalid === true) return undefined;
  if (model.upstreamModelId !== undefined) {
    return isOpenAiUpstreamModelId(model.upstreamModelId)
