...
describe("goal continuation policy", () => {
  ...
  it("only auto-continues active goals after an agent turn", () => {
    expect(shouldQueueGoalContinuationAfterAgentEnd(null, false, cleanTurn))
      .toBe(false);
    expect(
      shouldQueueGoalContinuationAfterAgentEnd(
        testGoal({ status: "paused" }),
        false,
        cleanTurn,
      ),
    ).toBe(false);
    ...
    const abortedTurn = [
      {
        role: "assistant",
        stopReason: "toolUse",
        content: [{ type: "toolCall", id: "1", name: "bash" }],
      },
      {
        role: "toolResult",
        isError: true,
