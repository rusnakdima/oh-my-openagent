...
function setClock(milliseconds: number): void {
  ...
  setSystemTime(new Date(CLOCK_EPOCH_MILLISECONDS + milliseconds));
}
...
describe("pi-goal extension accounting", () => {
  ...
  it("starts elapsed-time accounting when a goal is created during an active agent turn", async () => {
    ...
    await harness
      ...
      .execute(
        "create-goal",
        { objective: "created after the turn started" },
        undefined,
        undefined,
        ctx,
      );
  ...
  it("accounts resumed active goal time from session start without counting offline time", async () => {
    ...
    // given
    await harness.emit("agent_start", { type: "agent_start" }, ctx);
    await harness.tool("create_goal").execute(
      "create-goal",
  ...
  it("finalizes elapsed time and usage when update_goal completes an active turn", async () => {
    ...
    expect(
      completedGoal === null ? "" : goalFooterIndicator(completedGoal).text,
    ).toBe("Goal achieved (1m)");
  ...
  it("does not check pending messages after a goal completes", async () => {
    ...
    await harness
      ...
      .execute(
        "create-goal",
        { objective: "finish without continuation checks" },
        undefined,
        undefined,
        ctx,
      );
    await harness.emit("agent_start", { type: "agent_start" }, ctx);
    await harness.tool("update_goal").execute(
      "complete-goal",
      { status: "complete" },
      undefined,
      undefined,
    ...
    await expect(
      harness.emit("agent_end", { type: "agent_end", messages: [] }, ctx),
  ...
  it("does not fail completed accounting when the UI ctx is stale", async () => {
    ...
    await harness
      ...
      .execute(
        "create-goal",
        { objective: "finish with stale ui" },
        undefined,
        undefined,
        ctx,
      );
  ...
  it("does not reread goal state during shutdown when no accounting is active", async () => {
    ...
    await expect(
      harness.emit("session_shutdown", { type: "session_shutdown" }, ctx),
    ).resolves.toBeUndefined();
  });
...
describe("pi-goal extension command UI parity", () => {
  ...
  it("shows Codex-style clear feedback when no goal exists", async () => {
    const harness = createHarness();
    const ui = createMockUi();
    const ctx = await createContext("thread-clear-no-goal", {
      hasUI: true,
      ui,
    });
  ...
  it("asks with Codex-style choices before replacing an existing goal", async () => {
    ...
    const ctx = await createContext("thread-replace-cancel", {
      hasUI: true,
      ui,
    });
    await harness.tool("create_goal").execute(
  ...
  it("replaces an existing goal only after the replace choice is selected", async () => {
    ...
    const ctx = await createContext("thread-replace-confirm", {
      hasUI: true,
      ui,
    });
    await harness.tool("create_goal").execute(
  ...
  it("prompts to resume a paused goal when a session is resumed", async () => {
    ...
    const ctx = await createContext("thread-resume-paused", {
      hasUI: true,
      ui,
    });
    await harness.tool("create_goal").execute(
    ...
    expect(harness.sentMessages[0]?.message.customType).toBe(
  ...
  it("does not prompt to resume a paused goal on non-resume session starts", async () => {
    const harness = createHarness();
    const ui = createMockUi({ selectResponses: ["Resume goal"] });
    const ctx = await createContext("thread-startup-paused", {
      hasUI: true,
      ui,
    });
    await harness.tool("create_goal").execute(
...
function createExtensionApi(
  ...
  return {
    on(event, handler) {
      ...
      eventHandlers.push((payload, ctx) =>
...
function toolContract(
  tool: RegisteredTool,
): Pick<RegisteredTool, "name" | "description" | "parameters"> {
  return {
    name: tool.name,
    description: tool.description,
    parameters: tool.parameters,
