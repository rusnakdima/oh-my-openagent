...
describe("goal prompts", () => {
  it("escapes the continuation objective at its data boundary", () => {
    const prompt = buildContinuationPrompt(
      testGoal("A & B < C > D", { tokenBudget: 100 }),
    );
  ...
  it("reflects token accounting inputs without pinning their presentation", () => {
    const baseline = buildContinuationPrompt(
      testGoal("Objective", { tokensUsed: 7 }),
    );
    const changedUsage = buildContinuationPrompt(
      testGoal("Objective", { tokensUsed: 8 }),
    );
    const bounded = buildContinuationPrompt(
      testGoal("Objective", { tokensUsed: 7, tokenBudget: 100 }),
    );
