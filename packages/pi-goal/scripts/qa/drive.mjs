...
export function findGoalStoreFiles(root) {
  ...
  return files.filter((file) =>
    file.includes(join("extensions", "pi-goal")) && file.endsWith(".json")
  );
}
...
function readGoalFromSandbox(root) {
  ...
  return {
    file: goalFiles[0],
    goal: JSON.parse(readFileSync(goalFiles[0], "utf8")).goal,
  };
}
...
function runRpcScenario(piBin, sandbox, scenario, scenarioReport) {
  writeFileSync(
    ...
    `${JSON.stringify({ steps: scenario.steps }, null, 2)}\n`,
  );
  ...
  return new Promise((resolveScenario) => {
    ...
    function handleLine(line) {
      ...
      if (event.type === "agent_end") {
        ...
        if (goal?.status === scenario.terminalStatus) {
          finish(`goal-${scenario.terminalStatus}`);
    ...
    child.stdin.write(
      `${
        JSON.stringify({ type: "prompt", message: "please create the QA goal" })
      }\n`,
    );
  });
}
...
async function runScenario(piBin, name) {
  ...
  try {
    ...
    scenarioReport.continuationObserved = sandboxText.includes(
      CONTINUATION_MARKER,
    );
    scenarioReport.codexContinuationMarkerObserved = sandboxText.includes(
      CODEX_CONTINUATION_MARKER,
    );
    scenarioReport.terminalStatusObserved =
      goal?.status === scenario.terminalStatus;

    const shapeCorrect = goal?.objective === scenario.objective &&
      goal?.tokenBudget === scenario.tokenBudget;
    if (
      shapeCorrect &&
...
function runSelfTest() {
  const sandbox = createSandbox();
  try {
    if (!existsSync(mockProviderEntry)) {
      throw new Error("mock provider entry missing");
    }
    if (!existsSync(goalExtensionEntry)) {
      throw new Error("goal extension entry missing");
    }
    if (sandbox.agentDir === realPiAgentDir) {
      throw new Error("sandbox reused the real pi agent dir");
    }
    if (digestDirectory(join(sandbox.root, "missing")) !== "absent") {
      throw new Error("missing dir digest should be absent");
    }
    writeFileSync(join(sandbox.cwd, "probe.json"), "{}");
    if (findGoalStoreFiles(sandbox.root).length !== 0) {
      throw new Error("goal store matcher over-matches");
    }
    if (resolvePiBin() === null) {
      throw new Error("pi binary could not be resolved from the workspace");
    }
    if (SCENARIOS.blocked.terminalStatus !== "blocked") {
      throw new Error("blocked scenario misconfigured");
    }
...
async function main() {
  ...
  try {
    ...
    report.result = report.scenarios.every((scenarioReport) =>
        scenarioReport.result === "PASS"
      )
      ? "PASS"
      : "FAIL";
  } finally {
    report.realAgentDirUntouched =
      digestDirectory(realPiAgentDir) === beforeDigest;
    if (report.result === "PASS" && !report.realAgentDirUntouched) {
      report.result = "FAIL";
      report.reason = "real-pi-agent-dir-mutated";
    }
    console.log(JSON.stringify(report, null, 2));
    process.exitCode = report.result === "PASS" || report.result === "SKIP"
      ? 0
      : 1;
  }
}
