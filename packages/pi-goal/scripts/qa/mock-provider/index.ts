...
const { existsSync, readFileSync, rmSync, writeFileSync } = process
  .getBuiltinModule<FsModule>("fs");
const { dirname, join } = process.getBuiltinModule<PathModule>("path");
const { fileURLToPath, pathToFileURL } = process.getBuiltinModule<UrlModule>(
  "url",
);
...
interface LocalAssistantMessageEventStream
  extends AsyncIterable<LocalStreamEvent> {
...
export function loadMockScript(cwd: string): MockScript {
  ...
  if (!existsSync(scriptPath)) {
    return {
      steps: [{ type: "text", text: "omo mock provider default response" }],
    };
  }
  ...
  if (!isMockScript(parsed)) {
    throw new Error(
      `${scriptPath} must contain {"steps":[...]} with text or tool_call steps`,
    );
  }
  return parsed;
}
...
function streamMockResponse(
  ...
  // counter so out-of-band model calls (session title generation, summaries)
  // cannot consume scripted steps meant for the agent turns.
  const assistantMessageCount =
    (context.messages ?? []).filter((message) => message.role === "assistant")
      .length;
  const step =
    script.steps[Math.min(assistantMessageCount, script.steps.length - 1)];
  callCount += 1;
    ...
    [Symbol.asyncIterator]() {
      return {
        next() {
          ...
          return new Promise<IteratorResult<LocalStreamEvent>>((resolve) =>
...
export async function selfTest(): Promise<void> {
  ...
  const previous = existsSync(scriptPath)
  ...
  try {
    ...
    if (
      stepToAssistantMessage(script.steps[0], 1).content[0]?.type !== "text"
    ) throw new Error("text step failed");
    const toolMessage = stepToAssistantMessage(script.steps[1], 2);
    if (toolMessage.content[0]?.type !== "toolCall") {
      throw new Error("tool step failed");
    }
    if (toolMessage.stopReason !== "toolUse") {
      throw new Error("tool step must stop with toolUse");
    }
    ...
    if (result.stopReason !== "toolUse") {
      throw new Error("stream result must stop with toolUse");
    }
    if (result.content[0]?.type !== "toolCall") {
      throw new Error("stream result must contain toolCall content");
    }
    if (!events.some((event) => isDoneToolUseEvent(event))) {
      throw new Error("stream must emit done/toolUse");
    }
  } finally {
