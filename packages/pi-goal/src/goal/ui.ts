...
export function updateGoalUi(ctx: ExtensionContext, goal: Goal | null): void {
  ...
  ctx.ui.setFooter((_tui, theme, footerData) =>
    new GoalFooterComponent(ctx, footerData, theme, goal)
  );
}
  ...
  switch (goal.status) {
    ...
    case "complete":
      return {
        ...
        text: usageText === null
          ? "Goal achieved"
          : `Goal achieved (${usageText})`,
      };
  }
}

export function composeFooterStatusLine(
  leftText: string,
  rightText: string,
  width: number,
): string {
  ...
  if (leftTextWidth + 2 + rightTextWidth <= width) {
    return `${sanitizedLeftText}${
      " ".repeat(width - leftTextWidth - rightTextWidth)
    }${rightText}`;
  }
...
class GoalFooterComponent implements Component {
  ...
  render(width: number): string[] {
    ...
    return [
      this.workingDirectoryLine(width),
      this.statsLine(width),
      this.goalStatusLine(goal, width),
    ];
  }
  ...
  private renderedGoal(): Goal {
    ...
    const elapsedSeconds = Math.max(
      0,
      Math.round((Date.now() - this.observedAtMilliseconds) / 1000),
    );
    return {
      ...this.goal,
      timeUsedSeconds: this.goal.timeUsedSeconds + elapsedSeconds,
    };
  }
  ...
  private workingDirectoryLine(width: number): string {
    ...
    if (
      homeDirectory !== undefined && workingDirectory.startsWith(homeDirectory)
    ) {
    ...
    return truncateToWidth(
      this.theme.fg("dim", workingDirectory),
      width,
      this.theme.fg("dim", "..."),
    );
  }
  ...
  private statsLine(width: number): string {
    ...
    const statsLine = totalNeededWidth <= width
      ? `${statsLeft}${
  ...
  private goalStatusLine(goal: Goal, width: number): string {
    ...
    return truncateToWidth(
      composeFooterStatusLine(leftText, rightText, width),
      width,
      this.theme.fg("dim", "..."),
    );
  }
...
function collectFooterTokenStats(ctx: ExtensionContext): FooterTokenStats {
  ...
  for (const entry of ctx.sessionManager.getEntries()) {
    if (entry.type !== "message" || !isFooterAssistantMessage(entry.message)) {
      continue;
    }
    ...
    stats.costTotal += nestedNumericUsageField(
      entry.message.usage,
      "cost",
      "total",
    );
...
function isFooterAssistantMessage(
  message: unknown,
): message is FooterAssistantMessage {
  return isRecord(message) && message["role"] === "assistant" &&
    isRecord(message["usage"]);
}

function footerStatsParts(
  ctx: ExtensionContext,
  tokenStats: FooterTokenStats,
  theme: Theme,
): string[] {
  const parts: string[] = [];
  if (tokenStats.input !== 0) {
    parts.push(`↑${formatFooterTokens(tokenStats.input)}`);
  }
  if (tokenStats.output !== 0) {
    parts.push(`↓${formatFooterTokens(tokenStats.output)}`);
  }
  if (tokenStats.cacheRead !== 0) {
    parts.push(`R${formatFooterTokens(tokenStats.cacheRead)}`);
  }
  if (tokenStats.cacheWrite !== 0) {
    parts.push(`W${formatFooterTokens(tokenStats.cacheWrite)}`);
  }

  const usingSubscription = ctx.model === undefined
    ? false
    : ctx.modelRegistry.isUsingOAuth(ctx.model);
  if (tokenStats.costTotal !== 0 || usingSubscription) {
    parts.push(
      `$${tokenStats.costTotal.toFixed(3)}${usingSubscription ? " (sub)" : ""}`,
    );
  }
...
function contextUsageText(ctx: ExtensionContext, theme: Theme): string {
  ...
  if (contextPercentValue > 70) {
    return theme.fg("warning", contextPercentDisplay);
  }
...
function footerRightSide(
  ...
  const rightSideWithoutProvider = model?.reasoning !== true
    ? modelName
    : thinkingLevel === "off"
    ? `${modelName} • thinking off`
    : `${modelName} • ${thinkingLevel}`;

  if (model === undefined || footerData.getAvailableProviderCount() <= 1) {
    return rightSideWithoutProvider;
  }

  const rightSideWithProvider =
    `(${model.provider}) ${rightSideWithoutProvider}`;
...
function compactStatsLine(
  ...
  const truncatedRightSide = truncateToWidth(
    rightSide,
    availableForRightSide,
    "",
  );
  const padding = " ".repeat(
    Math.max(0, width - statsLeftWidth - visibleWidth(truncatedRightSide)),
  );
...
function nestedNumericUsageField(
  usage: Record<string, unknown>,
  outerKey: string,
  innerKey: string,
): number {
