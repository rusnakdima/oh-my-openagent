...
export async function updateGoal(
  ...
  const objective = update.objective === undefined
    ? current.objective
    : validateObjective(update.objective);
  const now = nowSeconds();
  const hasObjectiveUpdate = update.objective !== undefined;
  const replacesGoal = hasObjectiveUpdate &&
    (objective !== current.objective || current.status === "complete");
  const requestedStatus = update.status ??
    (hasObjectiveUpdate ? "active" : undefined);

  if (replacesGoal) {
    const replacementBudget = tokenBudget === null ? undefined : tokenBudget;
    const status = statusAfterBudgetLimit(
  ...
  const nextTokenBudget = tokenBudget === null
    ? undefined
    : (tokenBudget ?? current.tokenBudget);
  const status = requestedStatus === undefined
    ? statusAfterBudgetUpdate(
      current.status,
      current.tokensUsed,
      nextTokenBudget,
    )
    : statusAfterExplicitStatusUpdate(
      current.status,
      requestedStatus,
      current.tokensUsed,
      nextTokenBudget,
...
function statusAfterBudgetLimit(
  ...
  return status === "active" && tokenBudget !== undefined &&
...
function parseGoalFile(raw: string): GoalFile {
  ...
  if (parsed["version"] !== STORE_VERSION) {
    throw new UnsupportedGoalStoreVersionError(
      "unsupported goal store version",
    );
  }
...
function isGoal(value: unknown): value is Goal {
  ...
  return (
    ...
    (value["lastStartedAt"] === undefined ||
      isNonNegativeSafeInteger(value["lastStartedAt"])) &&
