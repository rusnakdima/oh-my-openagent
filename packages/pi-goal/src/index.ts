...
const EMPTY_USAGE: TokenUsageSnapshot = {
  input: 0,
  output: 0,
  cacheRead: 0,
  cacheWrite: 0,
...
export default function (pi: ExtensionAPI): void {
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      const goal = await updateGoal(goalStoreRef(ctx), {
      ...
      updateGoalUi(ctx, goal);
      return toolText(
        formatGoalToolResponse(goal, params.status === "complete"),
      );
    },
  });
  ...
  pi.on("agent_end", async (event, ctx) => {
    const mode: GoalAccountingMode = completedThisTurnGoalId === null
      ? "active"
      : "activeOrComplete";
    const goal = await accountCurrentAgentTurn(
      ctx,
      collectAssistantUsage(event.messages),
      mode,
    );
    ...
    if (goal?.status === "budgetLimited") {
      if (!ctx.hasPendingMessages() && budgetLimitReportedGoalId !== goal.id) {
        ...
        queueHiddenGoalPrompt(
          pi,
          GOAL_BUDGET_LIMIT_MESSAGE_TYPE,
          buildBudgetLimitedPrompt(goal),
        );
      }
      return;
    }
  ...
  async function setGoalObjective(
    ...
    const goal = current === null
      ? await createGoal(ref, objective)
      : await updateGoal(ref, { objective });
  ...
  async function confirmReplaceGoal(
    ctx: ExtensionContext,
    objective: string,
  ): Promise<boolean> {
  ...
  async function maybePromptResumePausedGoal(
    ...
    ctx.ui.notify(
      `Goal ${goalStatusLabel(resumed.status)}\n${formatGoalForTool(resumed)}`,
      "info",
    );
  ...
  function beginAgentGoalAccounting(goal: Goal): void {
    ...
    agentGoalAccounting = {
      goalId: goal.id,
      measuredFromMilliseconds: Date.now(),
    };
  }
  ...
  async function accountCurrentAgentTurn(
    ...
    const elapsedSeconds = Math.max(
      0,
      Math.round((now - accounting.measuredFromMilliseconds) / 1000),
    );
    ...
    if (goal?.id === accounting.goalId) {
      agentGoalAccounting = {
        goalId: accounting.goalId,
        measuredFromMilliseconds: now,
      };
    } else {
      clearAgentGoalAccounting();
    }
...
function updateGoalUiBestEffort(
  ctx: ExtensionContext,
  goal: Goal | null,
): void {
  try {
...
function queueGoalContinuation(
  pi: ExtensionAPI,
  ctx: ExtensionContext,
  goal: Goal,
): void {
...
function queueHiddenGoalPrompt(
  pi: ExtensionAPI,
  customType: string,
  content: string,
): void {
...
function isAssistantUsageMessage(
  message: unknown,
): message is AssistantUsageMessage {
