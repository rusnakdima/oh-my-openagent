...
async function assertPrimaryButtonHitTargets(page: Page, minSize: number) {
  const undersized = await page.evaluate((min) => {
    ...
    const violations: Array<
      { tag: string; w: number; h: number; text: string }
    > = [];
  ...
  expect(
    undersized,
    `Buttons smaller than ${minSize}px: ${JSON.stringify(undersized)}`,
  ).toEqual([]);
}
...
test.describe("Responsive QA matrix — landing", () => {
  for (const viewport of VIEWPORTS) {
    for (const locale of LOCALES) {
      ...
      test(`${viewport.name} ${locale} landing no overflow + buttons hit target`, async ({ page }) => {
      ...
      test(`${viewport.name} ${locale} manifesto no overflow`, async ({ page }) => {
        ...
        await page.goto(`${localePrefix}/manifesto`, {
