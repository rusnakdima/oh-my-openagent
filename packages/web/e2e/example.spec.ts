...
test.describe("Landing Page", () => {
  ...
  test("renders install command", async ({ page }) => {
    ...
    // when
    const installCommand = page.getByText("bunx oh-my-openagent install")
      .first();
  ...
  test("mobile nav toggles menu", async ({ page }) => {
    ...
    await expect(mobileNav.getByRole("link", { name: "Docs", exact: true }))
      .toBeVisible();
    await expect(
      mobileNav.getByRole("link", { name: "Manifesto", exact: true }),
    ).toBeVisible();
  });
  ...
  test("navigates to docs page", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: "Configuration Reference" }))
      .toBeVisible();
  });
  ...
  test("navigates to manifesto page", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: "Ultrawork Manifesto" }))
      .toBeVisible();
  });
});
...
test.describe("Docs Page", () => {
  ...
  test("has working search input", async ({ page }) => {
    ...
    // then
    await expect(page.getByRole("button", { name: "Agent / Model Matching" }))
      .toBeVisible();
  });
  ...
  test("sidebar navigation scrolls instantly and highlights active section", async ({ page }) => {
    // given
    await page.goto("/docs");
    const installationButton = page.getByRole("button", {
      name: "Installation",
    });
  ...
  test("hash navigation opens the installation section", async ({ page }) => {
    ...
    // then
    await expect(page.locator("#installation")).toBeInViewport();
    await expect(page.getByRole("button", { name: "Installation" }))
  ...
  test("internal docs links point to section hashes", async ({ page }) => {
    // given
    await page.goto("/docs");
    const installationGuideLink = page.getByRole("link", {
    ...
    // then
    await expect(installationGuideLink).toHaveAttribute(
      "href",
      "#installation",
    );
    await expect(page).toHaveURL(/\/docs#installation$/);
...
test.describe("Manifesto Page", () => {
  test("renders hero and core philosophy", async ({ page }) => {
    ...
    const bottleneckText = page.getByText("HUMAN IN THE LOOP = BOTTLENECK")
      .first();
