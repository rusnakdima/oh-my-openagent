...
async function fetchNpmDownloadsForPackage(
  period: string,
  pkg: string,
): Promise<number> {
  try {
...
async function fetchAllNpmDownloadsForPackage(pkg: string): Promise<number> {
  ...
  while (year <= now.getFullYear()) {
    ...
    const end = endDate > now
      ? (now.toISOString().split("T")[0] ?? `${year}-12-31`)
      : `${year}-12-31`;
...
async function fetchAllNpmDownloads(): Promise<number> {
  const results = await Promise.all(
    NPM_PACKAGES.map((pkg) => fetchAllNpmDownloadsForPackage(pkg)),
  );
  return results.reduce((sum, n) => sum + n, 0);
}
...
export async function getStats(): Promise<StatsData> {
  ...
  const [stars, monthlyDownloads, weeklyDownloads, totalDownloads] =
    await Promise.all([
      fetchGitHubStars(),
...
export const FALLBACK_FORMATTED_STATS: FormattedStatsData = formatStats(
  FALLBACK_STATS_DATA,
);
