...
const sectionIdByFile = new Map(
  SECTIONS.map((section) => [section.file, section.id]),
);
...
function rewriteDocsLink(sourceFile, href) {
  ...
  const targetFile = path.posix.normalize(
    path.posix.join(sourceDirectory, hrefPath),
  );
...
if (await outputIsCurrent(out)) {
  process.stdout.write(
    "Docs content already current with " + SECTIONS.length +
      " HTML-compiled docs\n",
  );
} else {
  await writeFile(OUTPUT, out);
  process.stdout.write(
    "Generated " + OUTPUT + " with " + SECTIONS.length +
      " HTML-compiled docs\n",
  );
}
