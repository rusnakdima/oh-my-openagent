...
function readOmoJson(homeDir: string): Record<string, unknown> {
  const value: unknown = parse(
    readFileSync(join(homeDir, ".omo", "omo.jsonc"), "utf-8"),
  );
...
function writeOpenCodeConfig(homeDir: string): void {
  ...
  writeFileSync(
    ...
    '{"agents":{"finder":{"model":"provider/finder"}}}',
  );
}

function runOpenCodeMigration(
  homeDir: string,
  cwd: string,
  onBoundary?: (boundary: MigrationBoundary) => void,
): void {
  runMigrations({
    discover: () =>
...
function migrationIds(homeDir: string): readonly string[] {
  ...
  return Array.isArray(migrations)
    ? migrations.filter((entry): entry is string => typeof entry === "string")
    : [];
}
...
describe("getCodexOmoConfig", () => {
  ...
  it("#given project SOT sets install_dir #when loading config #then only the effective value changes while trusted install root stays global", () => {
    ...
    writeOmoConfig(
      homeDir,
      JSON.stringify({
        codegraph: { enabled: true, install_dir: "/global-codegraph" },
      }),
    );
    writeOmoConfig(
      cwd,
      JSON.stringify({ codegraph: { install_dir: "/project-codegraph" } }),
    );
  ...
  it("#given codex SOT sets CodeGraph excluded roots #when loading config #then roots are returned to hooks", () => {
    ...
    writeOmoConfig(
      ...
      JSON.stringify({
        "[codex]": {
          codegraph: {
            excluded_roots: ["/tmp/omo-research", "/private/tmp/omo-research"],
          },
        },
      }),
    );
    ...
    // then
    expect(result.codegraph?.excluded_roots).toEqual([
      "/tmp/omo-research",
      "/private/tmp/omo-research",
    ]);
  });
  ...
  it("#given codex SOT sets the SessionStart cooldown #when loading config #then the configured base interval is returned", () => {
    ...
    writeOmoConfig(
      homeDir,
      JSON.stringify({
        "[codex]": { codegraph: { session_start_cooldown_ms: 900_000 } },
      }),
    );
  ...
  it("#given no codegraph.daemon key #when loading config #then daemon defaults to on", () => {
    ...
    const homeDir = createTemporaryDirectory(
      "omo-codex-shared-daemon-default-home-",
    );
    const cwd = createTemporaryDirectory(
      "omo-codex-shared-daemon-default-project-",
    );
  ...
  it("#given codex SOT sets codegraph.daemon=false #when loading config #then daemon opt-out is returned", () => {
    // given
    const homeDir = createTemporaryDirectory(
      "omo-codex-shared-daemon-off-home-",
    );
    const cwd = createTemporaryDirectory(
      "omo-codex-shared-daemon-off-project-",
    );
    writeOmoConfig(
      homeDir,
      JSON.stringify({ "[codex]": { codegraph: { daemon: false } } }),
    );
  ...
  it("#given codex SOT sets codegraph.daemon to a non-boolean #when loading config #then the value is rejected with a warning", () => {
    // given
    const homeDir = createTemporaryDirectory(
      "omo-codex-shared-daemon-invalid-home-",
    );
    const cwd = createTemporaryDirectory(
      "omo-codex-shared-daemon-invalid-project-",
    );
    writeOmoConfig(
      homeDir,
      JSON.stringify({ "[codex]": { codegraph: { daemon: "yes" } } }),
    );
    ...
    expect(result.warnings).toContain(
      `Invalid omo config at ${
        join(homeDir, ".omo", "omo.jsonc")
      }: [codex].codegraph.daemon`,
    );
  });
  ...
  it("#given codex-unsupported codegraph setting #when loading config #then returns loader warnings", () => {
    ...
    writeOmoConfig(
      homeDir,
      JSON.stringify({ "[codex]": { codegraph: { watch_debounce_ms: 42 } } }),
    );
  ...
  it("#given only legacy config.jsonc #when codex starts #then migrates once with a backup before loading the unified codex view", () => {
    ...
    const backupDirectories = readdirSync(join(homeDir, ".omo")).filter((
      entry,
    ) => entry.startsWith("migration-backup-"));
    ...
    expect(
      first.warnings.some((warning) =>
        warning.startsWith("omo-codex: migrated legacy configuration from ") &&
        warning.endsWith("/.omo/config.jsonc")
      ),
    ).toBe(true);
    expect(readOmoJson(homeDir)["_migrations"]).toEqual([
    ...
    expect(
      readdirSync(join(homeDir, ".omo")).filter((entry) =>
        entry.startsWith("migration-backup-")
      ),
    ).toEqual(backupDirectories);
  });
  ...
  it("#given a legacy migration conflict #when codex starts #then exposes the migration summary and conflict through loader warnings", () => {
    ...
    writeLegacyOmoConfig(
      homeDir,
      JSON.stringify({ codegraph: { enabled: false } }),
    );
    ...
    expect(result.warnings).toContain(
      "omo-codex: configuration migration: skipped: codegraph.enabled legacy=false kept=true",
    );
  });
  ...
  it("#given overlapping [omo] and [senpi] blocks in legacy config #when Codex migrates #then the transform conflict is reported", () => {
    // given
    const homeDir = createTemporaryDirectory(
      "omo-codex-legacy-senpi-conflict-",
    );
    writeLegacyOmoConfig(
      homeDir,
      JSON.stringify({
    ...
    // when
    const result = runCodexStartupMigration({
      cwd: homeDir,
    ...
    // then
    expect(result.results[0]?.diagnostics).toContain(
  ...
  it("#given a malformed migration journal #when codex starts #then exposes the recovery failure through loader warnings", () => {
    ...
    expect(
      result.warnings.some((warning) =>
        warning.startsWith("omo-codex: configuration migration: ")
      ),
    ).toBe(true);
  });
  ...
  it("#given concurrent codex and opencode starts #when opencode owns the shared lock #then both migration ids are applied exactly once", () => {
    ...
    if (codexResult === undefined) {
      throw new Error("Expected concurrent Codex migration attempt");
    }
    expect(codexResult.error).toBe(
      "Configuration migration is already running",
    );
    expect(migrationIds(homeDir)).toEqual([
      "2026-07-opencode-config-unification",
      "2026-07-codex-config-jsonc",
