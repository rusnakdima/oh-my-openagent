...
function transformConfigJsonc(
  configPath: string,
  sources: readonly LoadedMigrationSource[],
): MigrationTransformResult {
  const config = sources.find((source) => source.path === configPath);
  const legacy = config === undefined || !isRecord(config.value)
    ? {}
    : config.value;
  ...
  return {
    ...
    document: {
      ...
      ...(recordAt(legacy, "codegraph") === undefined
        ? {}
        : { codegraph: recordAt(legacy, "codegraph") }),
      ...(recordAt(legacy, "[opencode]") === undefined
        ? {}
        : { "[opencode]": recordAt(legacy, "[opencode]") }),
      ...(recordAt(legacy, "[codex]") === undefined
        ? {}
        : { "[codex]": recordAt(legacy, "[codex]") }),
      ...(senpi === undefined && omo === undefined
        ? {}
        : { "[senpi]": senpi ?? omo }),
...
function sourceExists(
  options: CodexConfigMigrationOptions,
  path: string,
): boolean {
  return (options.discoveryFileSystem ?? options.fileSystem ?? { existsSync })
    .existsSync(path);
}
...
function migrationPlan(
  ...
  const paths = options.pathOperations ??
  ...
  const sourcePaths = [configPath, sidecarPath].filter((path) =>
    sourceExists(options, path)
  );
  if (sourcePaths.length === 0) return undefined;
  const backupRoot = paths.join(
    homeDir,
    ".omo",
    `migration-backup-${timestamp(options.backupTimestamp)}-opencode-config`,
    ".omo",
  );
  return {
    id: MIGRATION_ID,
    sources: sourcePaths.map((path) => ({
      backupPath: paths.join(
        backupRoot,
        path === configPath ? "config.jsonc" : "config.jsonc.migrations.json",
      ),
      path,
    })),
...
export function runCodexConfigMigration(
  options: CodexConfigMigrationOptions,
): CodexConfigMigrationResult {
  const environment = options.environment ?? process.env;
  const homeDir = options.homeDir ?? environment["HOME"] ??
