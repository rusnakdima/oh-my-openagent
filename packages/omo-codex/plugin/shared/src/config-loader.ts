...
export type CodexOmoConfigOptions =
  & Omit<LoadOmoConfigOptions, "fileSystem" | "harness">
  & {
...
export function runCodexStartupMigration(
  options: CodexStartupMigrationOptions,
): CodexStartupMigrationResult {
  ...
  for (const prefix of ["OMO", "CODEX"]) {
    ...
    if (cooldown !== undefined) {
      ...
      if (!Number.isFinite(value) || value < 60_000) {
        warnings.push(
          `${prefix}_CODEGRAPH_SESSION_START_COOLDOWN_MS has invalid number value "${cooldown}"`,
        );
    ...
    if (debounce !== undefined) {
      ...
      if (!Number.isFinite(value) || value < 0) {
        warnings.push(
          `${prefix}_CODEGRAPH_WATCH_DEBOUNCE_MS has invalid number value "${debounce}"`,
        );
...
function migrationWarnings(
  result: CodexStartupMigrationResult,
): readonly string[] {
  ...
  if (result.journalResumed) {
    warnings.push(
      "omo-codex: recovered an interrupted configuration migration",
    );
  }
  if (result.migratedFrom.length > 0) {
    warnings.push(
      `omo-codex: migrated legacy configuration from ${
        result.migratedFrom.join(", ")
      }`,
    );
  }
...
function codexCodegraphConfig(
  value: OmoConfig["codegraph"],
): CodexCodegraphConfig | undefined {
  ...
  return {
    ...
    ...(value.excluded_roots === undefined
      ? {}
      : { excluded_roots: value.excluded_roots }),
    ...(value.install_dir === undefined
      ? {}
      : { install_dir: value.install_dir }),
    ...(value.session_start_cooldown_ms === undefined
      ? {}
      : { session_start_cooldown_ms: value.session_start_cooldown_ms }),
    ...(value.watch_debounce_ms === undefined
      ? {}
      : { watch_debounce_ms: value.watch_debounce_ms }),
  };
}

export function getCodexOmoConfig(
  options: CodexOmoConfigOptions = {},
): CodexOmoConfig {
  const env = options.env ?? process.env;
  ...
  const config = OmoConfigSchema.parse(
    mergeOmoConfigRecords(result.config, envOverrides(env, envWarnings)),
  );
  const trustedCodegraphInstallDir = trustedConfig.config.codegraph
    ?.install_dir;
  ...
  return {
    ...
    ...(trustedCodegraphInstallDir === undefined
      ? {}
      : { trustedCodegraphInstallDir }),
    warnings: [
