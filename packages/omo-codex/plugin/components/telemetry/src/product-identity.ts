...
function isComponentPackageManifest(
  value: unknown,
): value is ComponentPackageManifest {
...
function readComponentVersionFromManifest(): string {
  try {
    ...
    if (
      isComponentPackageManifest(parsed) && typeof parsed.version === "string"
    ) {
...
export function writeComponentTelemetryDiagnostic(
