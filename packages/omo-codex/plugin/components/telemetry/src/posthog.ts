...
function resolveOsProvider(
  options: CreatePluginPostHogOptions,
): TelemetryOsProvider {
...
function createPluginPostHogClient(
  options: CreatePluginPostHogOptions = {},
): PostHogClient {
  const clientInput = {
    diagnostics: writeComponentTelemetryDiagnostic,
  ...
  const client = createTelemetryClient(
    options.transportFactory === undefined ? clientInput : {
      ...clientInput,
...
export function __createPluginPostHogForTesting(
  options: CreatePluginPostHogOptions,
): PostHogClient {
  return createPluginPostHogClient(options);
}

export function getPostHogDistinctIdForTesting(
