...
export function readContinuationState(
  ...
  const work = getWorkForSession(
    boulderState,
    normalizeSessionId(sessionId, "codex"),
  );
...
function resolveBoulderPlanPathForWork(cwd: string, work: BoulderWork): string {
  ...
  if (
    relativePlanPath.length === 0 || relativePlanPath.startsWith("..") ||
    isAbsolute(relativePlanPath)
  ) {
    return absolutePlanPath;
  }

  const worktreePlanPath = resolve(
    resolveTrackedPath(cwd, worktreePath),
    relativePlanPath,
  );
...
function normalizeSessionId(
  sessionId: string,
  platform: "codex" | "opencode" = "opencode",
): string {
