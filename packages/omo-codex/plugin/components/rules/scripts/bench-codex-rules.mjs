...
async function runHookFastPathScenario() {
  ...
  for (
    let iteration = 0;
    iteration < ITERATIONS + WARMUP_ITERATIONS;
    iteration += 1
  ) {
...
async function measureHookFastPathRun() {
  ...
  try {
    ...
    for (let index = 0; index < RULE_COUNT; index += 1) {
      writeFileSync(
        join(projectRoot, ".omo", "rules", `rule-${index}.md`),
        ruleContent(`rule-${index}`),
      );
    }
...
function distinctTargets(projectRoot) {
  return Array.from(
    { length: DISTINCT_TARGET_COUNT },
    (_, index) => join(projectRoot, "src", `file-${index}.ts`),
  );
}
...
function compareResults(baseline, current) {
  ...
  for (const scenario of current.scenarios) {
    const baselineScenario = baseline.scenarios.find((candidate) =>
    ...
    for (
      const counterName of ["findProjectRoot", "findCandidates", "readFile"]
    ) {
      if (
        scenario.counters[counterName] > baselineScenario.counters[counterName]
      ) {
        failures.push(
          `${scenario.name}.${counterName} regressed: ${
            scenario.counters[counterName]
          } > ${baselineScenario.counters[counterName]}`,
        );
      }
    }
  }
  if (baseline.hookFastPath === undefined) {
    ...
    if (
      current.hookFastPath.repeatOutputBytes >
        baseline.hookFastPath.repeatOutputBytes
    ) {
      failures.push(
