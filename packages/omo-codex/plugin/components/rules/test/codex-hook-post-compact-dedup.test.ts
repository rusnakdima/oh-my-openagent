...
function makeTempProject(): { root: string; pluginData: string } {
  const root = mkdtempSync(
    path.join(tmpdir(), "codex-rules-compact-dedup-project-"),
  );
  const pluginData = mkdtempSync(
    path.join(tmpdir(), "codex-rules-compact-dedup-data-"),
  );
...
function writeTranscriptWithRepeatedCompactions(
  root: string,
  retainedText: string,
): string {
      ...
      JSON.stringify({
        ...
        payload: {
          ...
          replacement_history: [{
            type: "message",
            role: "user",
            content: "old summary without rules",
          }],
        },
      }),
      JSON.stringify({
