...
function makeProject(): {
  ...
  const projectRoot = mkdtempSync(
    join(tmpdir(), "codex-rules-finder-project-"),
  );
  ...
  writeFileSync(
    join(projectRoot, "src", ".omo", "rules", "local.md"),
    "Local rule\n",
  );
  writeFileSync(join(projectRoot, ".omo", "rules", "root.md"), "Root rule\n");
  writeFileSync(
    join(homeRoot, ".opencode", "rules", "global.md"),
    "Global rule\n",
  );
  writeFileSync(
    join(homeRoot, ".config", "opencode", "AGENTS.md"),
    "Home rule\n",
  );
