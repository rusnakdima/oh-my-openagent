...
function globalCandidate(
  ...
  return {
    ...
    isGlobal: true,
    isSingleFile: false,
    relativePath: source === "plugin-bundled"
      ? "bundled-rules/hephaestus.md"
      : ".opencode/rules/hephaestus.md",
  };
}
...
describe("plugin bundled rule priority", () => {
  ...
  it("#given aggregate plugin root #when resolving rules root #then components rules directory is selected", () => {
    // given
    const aggregateRoot = mkdtempSync(
      join(tmpdir(), "codex-rules-aggregate-plugin-"),
    );
    ...
    writeFileSync(
      join(aggregateRoot, ".codex-plugin", "plugin.json"),
      JSON.stringify({ name: "omo" }),
    );
