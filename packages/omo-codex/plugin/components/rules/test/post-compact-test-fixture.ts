...
export function makeOversizedProject(
  prefix = "budget",
): { root: string; pluginData: string } {
  const root = mkdtempSync(
    path.join(tmpdir(), `codex-rules-post-compact-${prefix}-project-`),
  );
  const pluginData = mkdtempSync(
    path.join(tmpdir(), `codex-rules-post-compact-${prefix}-data-`),
  );
  ...
  writeFileSync(
    ...
    "Project AGENTS.md should stay Codex-native.",
  );
  writeFileSync(
...
export function writeCompactedTranscript(
  root: string,
  retainedText: string,
): string {
...
export function writeCompactedWarningTranscript(
  root: string,
  retainedText: string,
): string {
...
export function writeMalformedContextTooLargeTranscript(
  root: string,
  retainedText = "",
): string {
