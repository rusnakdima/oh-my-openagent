...
function makeFixture(options: FixtureOptions = {}): Fixture {
  ...
  const bundledBody = options.writeProjectDuplicate === true
    ? SHARED_BODY
    : BUNDLED_BODY;
...
function ruleMarkdown(body: string): string {
  return ["---", "description: Fixture", "alwaysApply: true", "---", "", body]
    .join("\n");
}
...
describe("plugin bundled rules", () => {
  it("#given PLUGIN_ROOT with bundled markdown #when finding candidates #then plugin-bundled source is cached", () => {
    ...
    expect(
      candidates.map((candidate) =>
        `${candidate.source}:${candidate.relativePath}`
      ),
    ).toEqual([
      "plugin-bundled:bundled-rules/hephaestus.md",
    ]);
    expect(cache.scannedRuleFiles.has(join(pluginRoot, "bundled-rules"))).toBe(
      true,
    );
  });
  ...
  it("#given bundled Hephaestus rule body exceeds per-rule cap #when SessionStart runs #then static context expands the body within result budget", async () => {
    ...
    const root = mkdtempSync(
      join(tmpdir(), "codex-rules-bundled-large-project-"),
    );
    const pluginRoot = mkdtempSync(
      join(tmpdir(), "codex-rules-bundled-large-plugin-"),
    );
    const pluginData = mkdtempSync(
      join(tmpdir(), "codex-rules-bundled-large-data-"),
    );
    tempDirectories.push(root, pluginRoot, pluginData);
    writeFileSync(
      join(root, "package.json"),
  ...
  it("#given project rule body exceeds per-rule cap #when SessionStart runs #then static context injects a truncated body", async () => {
    // given
    const root = mkdtempSync(
      join(tmpdir(), "codex-rules-project-large-project-"),
    );
    const pluginRoot = mkdtempSync(
      join(tmpdir(), "codex-rules-project-large-plugin-"),
    );
    const pluginData = mkdtempSync(
      join(tmpdir(), "codex-rules-project-large-data-"),
    );
    tempDirectories.push(root, pluginRoot, pluginData);
    writeFileSync(
      join(root, "package.json"),
