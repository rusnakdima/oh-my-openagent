...
const REMOVED_AGENT_DOC_SOURCE_LISTS = [
  "AGENTS.md",
  "CLAUDE.md",
  "AGENTS.md,CLAUDE.md",
] as const;
...
function makeAgentDocProject(): AgentDocProject {
  ...
  const pluginData = mkdtempSync(
    path.join(tmpdir(), "codex-rules-agent-docs-data-"),
  );
  ...
  writeFileSync(
    ...
    "Root AGENTS.md must remain Codex-native.",
  );
  writeFileSync(
  ...
  writeFileSync(
    ...
    "Child AGENTS.md must remain Codex-native.",
  );
...
describe("agent doc sources", () => {
  ...
  it("#given nested AGENTS.md and removed-only source allowlist #when PostToolUse targets a child file #then it emits no dynamic OMO rules context", async () => {
    ...
    const output = await runPostToolUseHook(
      postToolUseInput(root, nestedSourceFile),
      {
        pluginDataRoot: pluginData,
