...
describe("codex rules post-compaction read directive", () => {
  ...
  it("#given bundled hephaestus rule dropped by compaction #when compact SessionStart recovers #then hephaestus body is re-injected in full alongside the directive", async () => {
    ...
    const output = await runSessionStartHook(
      compactSessionStartInput(root, transcriptPath),
      {
        pluginDataRoot: pluginData,
        env,
      },
    ...
    // then
    const context = readAdditionalContext(output);
    const bundledRulePath = fileURLToPath(
      new URL("../bundled-rules/hephaestus/gpt-5.5.md", import.meta.url),
    );
    expect(context).toContain(`${INSTRUCTIONS_FROM}${bundledRulePath}`);
    expect(context).toContain(
      parseRule(readFileSync(bundledRulePath, "utf8")).body.trim(),
    );
    expect(context).toContain("CONTEXT.md");
  });
});
...
function makeProject(): { root: string; pluginData: string } {
  const root = mkdtempSync(
    path.join(tmpdir(), "codex-rules-directive-project-"),
  );
  const pluginData = mkdtempSync(
    path.join(tmpdir(), "codex-rules-directive-data-"),
  );
...
function compactSessionStartInput(
  root: string,
