...
function makeTempProject(
  ruleCount: number,
): { root: string; pluginData: string; targetPath: string } {
  const root = fs.mkdtempSync(
    path.join(tmpdir(), "codex-rules-hook-perf-project-"),
  );
  const pluginData = fs.mkdtempSync(
    path.join(tmpdir(), "codex-rules-hook-perf-data-"),
  );
...
function isProjectRuleRead(filePath: unknown): boolean {
  return String(filePath).includes(
    `${path.sep}.omo${path.sep}rules${path.sep}`,
  );
}
...
describe("codex rules hook performance", () => {
  it("#given unchanged dynamic target #when PostToolUse repeats #then rule files are not reread for fingerprinting", async () => {
    ...
    try {
      // when
      const firstOutput = await runPostToolUseHook(
        postToolUseInput(root, targetPath),
        {
