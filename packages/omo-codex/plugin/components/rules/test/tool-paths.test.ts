...
import {
  type CodexPostToolUseLike,
  extractCodexToolPaths,
} from "../src/tool-paths.js";
...
function postToolUse(
  input: { toolName: string; toolInput?: unknown; toolResponse?: unknown },
): CodexPostToolUseLike {
  return {
...
describe("extractCodexToolPaths", () => {
  for (
    const [name, toolName, toolInput] of [
      [
  ...
  it("#given shell command payload #when extracting #then returns only existing file tokens", () => {
    ...
    const paths = extractCodexToolPaths(
      postToolUse({
        ...
        toolInput: {
          cmd: "sed -n '1,80p' src/app.ts src/missing.ts",
          workdir: root,
        },
      }),
      "/tmp",
