...
describe("rules-engine package consumption", () => {
  it("#given Codex bundled rule sources #when imported from rules-engine #then plugin-bundled ordering is preserved", () => {
    ...
    const pluginRoot = mkdtempSync(
      join(tmpdir(), "codex-rules-engine-consumption-"),
    );
    ...
    // when
    const nonWindowsCandidates = findPluginBundledCandidates({
      pluginRoot,
      platform: "darwin",
    });
    const windowsCandidates = findPluginBundledCandidates({
      pluginRoot,
      platform: "win32",
    });
    ...
    expect(nonWindowsCandidates.map((candidate) => candidate.relativePath))
      .toEqual([
        "bundled-rules/hephaestus.md",
      ]);
    expect(windowsCandidates.map((candidate) => candidate.relativePath))
      .toEqual([
        "bundled-rules/hephaestus.md",
  ...
  it("#given Codex rule parser and matcher edge cases #when imported from rules-engine #then current behavior is preserved", () => {
    ...
    expect(dotMatch).toEqual({
      matched: true,
      reason: { kind: "glob", pattern: "**/.env" },
    });
  ...
  it("#given Codex scanner options #when imported from rules-engine #then scan depth and max files remain bounded", () => {
    ...
    // when
    const depthOne = scanRuleFiles({ rootDir: root, maxDepth: 1 }).map((
      entry,
    ) => entry.path);
