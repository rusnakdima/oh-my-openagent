...
function matchFrontmatter(
  ...
  const pathBase = {
    ...
    basename: pathBases.basename ??
      pathBases.projectRelative.split("/").at(-1) ?? pathBases.projectRelative,
...
function matchGlobs(
  ...
  return matchFrontmatter({ globs } satisfies RuleFrontmatter, {
    projectRelative,
  }).matched;
}
...
describe("matchRule", () => {
  ...
  it("#given paths alias #when target matches #then glob match is returned", () => {
    ...
    // when
    const result = matchFrontmatter(frontmatter, {
      projectRelative: "src/features/app.ts",
    });

    // then
    expect(result).toEqual({
      matched: true,
      reason: { kind: "glob", pattern: "src/**/*.ts" },
    });
  });
  ...
  it("#given applyTo alias #when basename matches #then glob match is returned", () => {
    ...
    // when
    const result = matchFrontmatter(frontmatter, {
      projectRelative: "docs/README.md",
    });

    // then
    expect(result).toEqual({
      matched: true,
      reason: { kind: "glob", pattern: "*.md" },
    });
  });
