...
describe("codex rules context-pressure recovery", () => {
  ...
  for (
    const [userPromptName, postToolName, writeTranscript]
      of TRANSCRIPT_CONTEXT_PRESSURE_CASES
  ) {
    ...
    it(postToolName, async () => {
      ...
      const output = await runPostToolUseHook(
        postToolUseInput(root, transcriptPath, filePath),
        {
...
function makeTempProject(): {
  ...
  const root = mkdtempSync(
    path.join(tmpdir(), "codex-rules-context-pressure-project-"),
  );
  const pluginData = mkdtempSync(
    path.join(tmpdir(), "codex-rules-context-pressure-data-"),
  );
...
function writeCodexContextWindowTranscript(root: string): string {
  const transcriptPath = path.join(
    root,
    "transcript-codex-context-window.jsonl",
  );
  writeFileSync(
    transcriptPath,
    [
