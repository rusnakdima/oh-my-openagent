...
if (command === "hook" && subcommand === "session-start") {
  ...
  process.stderr.write(
    "Usage: omo-rules hook [session-start|user-prompt-submit|post-tool-use|post-compact]\n",
  );
  process.exitCode = 1;
}
...
async function runHookCli(eventName: HookCliEventName): Promise<void> {
  ...
  const options: CodexRulesHookOptions = pluginDataRoot === undefined
    ? {}
    : { pluginDataRoot };
...
async function runHook(
  ...
  switch (eventName) {
    case "SessionStart":
      return isCodexSessionStartInput(parsed)
    ...
    case "UserPromptSubmit":
      return isCodexUserPromptSubmitInput(parsed)
        ...
        : "";
    case "PostToolUse":
      return isCodexPostToolUseInput(parsed)
        ? await runPostToolUseHook(parsed, options)
        : "";
    case "PostCompact":
      return isCodexPostCompactInput(parsed)
        ? await runPostCompactHook(parsed, options)
        : "";
  }
}
...
function isCodexSessionStartInput(
  value: unknown,
): value is CodexSessionStartInput {
  return (
    isRecord(value) &&
...
function isCodexPostToolUseInput(
  value: unknown,
): value is CodexPostToolUseInput {
  return (
    isRecord(value) &&
...
function isCodexPostCompactInput(
  value: unknown,
): value is CodexPostCompactInput {
  return (
    isRecord(value) &&
