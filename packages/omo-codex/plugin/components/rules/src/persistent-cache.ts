...
export function hydrateEngineState(engine: Engine, cachePath: string): void {
  ...
  for (
    const [targetKey, fingerprint] of Object.entries(
      state.dynamicTargetFingerprints ?? {},
    )
  ) {
...
export function persistEngineState(
  ...
  const postCompactPending = nextPostCompactPending(
    currentState,
    completedPostCompactKind,
  );
  const postCompactRecovering = nextPostCompactRecovering(
    currentState,
    completedPostCompactKind,
  );
  writeSessionState(cachePath, {
    ...
    dynamicDedup,
    dynamicTargetFingerprints: Object.fromEntries(
      engine.state.dynamicTargetFingerprints.entries(),
    ),
...
export function hasPostCompactPending(cachePath: string): boolean {
  const state = readSessionState(cachePath);
  return postCompactPendingKinds(state).size > 0 ||
    postCompactRecoveringKinds(state).size > 0;
}

export function isPostCompactPending(
  cachePath: string,
...
export function claimPostCompactPending(
  ...
  const result = withSessionStateLock(cachePath, () => {
    ...
    writeSessionState(
      cachePath,
      stateWithPostCompactKinds(state, pendingKinds, recoveringKinds),
    );
    return "claimed";
  });
...
export function completePostCompactRecovery(
  cachePath: string,
  kind: PostCompactPendingKind,
): void {
...
export function sessionCachePath(
  ...
  const root = pluginDataRoot ?? process.env["PLUGIN_DATA"] ??
    join(homedir(), ".codex", "codex-rules");
...
function isSerializedSessionState(
  value: unknown,
): value is SerializedSessionState {
  if (
    !isRecord(value) || !Array.isArray(value["staticDedup"]) ||
  ...
  return (
    ...
    (postCompactPending === undefined ||
      isPostCompactPendingState(postCompactPending)) &&
    (postCompactRecovering === undefined ||
      isPostCompactPendingState(postCompactRecovering)) &&
    (compacted === undefined || typeof compacted === "boolean")
  );
}

function isPostCompactPendingState(
  value: unknown,
): value is PostCompactPendingState {
  return (
    isRecord(value) &&
