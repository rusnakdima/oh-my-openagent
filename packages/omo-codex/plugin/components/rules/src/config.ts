...
export function configFromEnvironment(
  ...
  config.disabled = isTruthy(
    firstEnv(env, "CODEX_RULES_DISABLED", "PI_RULES_DISABLED"),
  );
  config.mode = parseMode(firstEnv(env, "CODEX_RULES_MODE", "PI_RULES_MODE")) ??
    config.mode;
  config.maxRuleChars = parsePositiveInteger(
    firstEnv(env, "CODEX_RULES_MAX_RULE_CHARS", "PI_RULES_MAX_RULE_CHARS"),
  ) ??
    config.maxRuleChars;
  config.maxResultChars = parsePositiveInteger(
    firstEnv(
      env,
  ...
  config.postCompactMaxRuleChars = parsePositiveInteger(
    firstEnv(
      env,
  ...
  config.postCompactMaxResultChars = parsePositiveInteger(
    firstEnv(
      env,
  ...
  config.dynamicMaxRuleChars = parsePositiveInteger(
    firstEnv(
      env,
      "CODEX_RULES_DYNAMIC_MAX_RULE_CHARS",
      "PI_RULES_DYNAMIC_MAX_RULE_CHARS",
    ),
  ) ??
    config.dynamicMaxRuleChars;
  config.dynamicMaxResultChars = parsePositiveInteger(
    firstEnv(
      env,
      "CODEX_RULES_DYNAMIC_MAX_RESULT_CHARS",
      "PI_RULES_DYNAMIC_MAX_RESULT_CHARS",
    ),
  ) ?? config.dynamicMaxResultChars;
  config.promptMaxRuleChars = parsePositiveInteger(
    firstEnv(
      env,
      "CODEX_RULES_PROMPT_MAX_RULE_CHARS",
      "PI_RULES_PROMPT_MAX_RULE_CHARS",
    ),
  ) ??
    config.promptMaxRuleChars;
  config.promptMaxResultChars = parsePositiveInteger(
    firstEnv(
      env,
      "CODEX_RULES_PROMPT_MAX_RESULT_CHARS",
...
function sourcesWithoutBundledRules(): RuleSource[] {
  return [...SOURCE_PRIORITY.keys()].filter((source) =>
