...
export function extractCodexToolPaths(
  input: CodexPostToolUseLike,
  cwd: string,
): string[] {
  const toolName = input.tool_name.toLowerCase();
  if (
    !TRACKED_TOOL_NAMES.has(toolName) ||
    isFailedToolResponse(input.tool_response)
  ) {
    return [];
  }
  ...
  if (COMMAND_TOOL_NAMES.has(toolName)) {
    const command = stringProperty(toolInput, "command") ??
      stringProperty(toolInput, "cmd");
    const workdir = stringProperty(toolInput, "workdir") ??
      stringProperty(toolInput, "cwd");
    addCommandPaths(
      paths,
      command,
      workdir === undefined ? cwd : resolvePath(cwd, workdir),
    );
  }
...
function addPatchHeaderPaths(
  ...
  for (const line of patch.split("\n")) {
    for (
      const prefix of ["*** Add File: ", "*** Update File: ", "*** Move to: "]
    ) {
...
function isFailedToolResponse(value: unknown): boolean {
  ...
  return (
    ...
    value["error"] === true || value["status"] === "error"
  );
}
