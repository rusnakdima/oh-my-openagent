...
export function runStaticInjection(
  ...
  const effectiveConfig = eventName === "UserPromptSubmit"
    ? withPromptBudget(config)
    : config;
...
function runPostCompactRecovery(input: PostCompactRecoveryInput): string {
  ...
  const dynamicRulePaths = recoverDynamicRulePaths(
    engine,
    transcriptText,
    loaded.rules,
  );
  ...
  const fullBodyRules = missingRules.filter((rule) =>
    isNeverTruncatedRule(ruleDisplayPath(rule))
  );
  const listedRules = missingRules.filter((rule) =>
    !isNeverTruncatedRule(ruleDisplayPath(rule))
  );
  const bodyBlock = fullBodyRules.length === 0
    ? ""
    : engine.formatStatic(fullBodyRules);
...
function readRecoveryTranscriptText(
  transcriptPath: string | null,
): string | null {
