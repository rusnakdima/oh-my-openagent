export type ContextInjectionHookEventName =
  | "SessionStart"
  | "UserPromptSubmit"
  | "PostToolUse";
...
export function formatAdditionalContextOutput(
  ...
  const normalizedContext = limitAdditionalContext(
...
function limitAdditionalContext(additionalContext: string): string {
  ...
  const head = additionalContext.slice(
    0,
    MAX_ADDITIONAL_CONTEXT_CHARS - marker.length,
  ).replace(/[ \t\r\n]+$/, "");
  return `${head}${marker}`;
}
