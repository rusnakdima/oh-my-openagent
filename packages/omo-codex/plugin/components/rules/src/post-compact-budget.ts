...
export function withPostCompactBudget(
  ...
  const postCompactMaxResultChars = dynamicPostCompactMaxResultChars(context) ??
...
function dynamicPostCompactMaxResultChars(
  ...
  const modelBudget = modelContextBudgetFor(context.model) ??
    fallbackModelContextBudget();
  ...
  const reservedTokens = Math.max(
    ...
    Math.floor(
      (effectiveContextWindow * POST_COMPACT_RESERVED_CONTEXT_PERCENT) / 100,
    ),
  );
  const injectableTokens = Math.max(
    0,
    effectiveContextWindow - reservedTokens - transcript.tokens,
  );
  return Math.max(
...
function estimateTranscript(
  ...
  return {
    text: transcriptText,
    tokens: Math.ceil(
      Buffer.byteLength(transcriptText, "utf8") /
        ESTIMATED_TRANSCRIPT_CHARS_PER_TOKEN,
    ),
  };
}
