...
import {
  claimedPostCompactKind,
  shouldSkipPostCompactClaim,
} from "./post-compact-claim.js";
...
export async function runSessionStartHook(
  ...
  const postCompactClaim = input.source === "clear"
    ? "not-pending"
    : claimPostCompactPending(cachePath, "static");
  const completedPostCompactKind =
    claimedPostCompactKind(postCompactClaim, "static") ??
      (input.source === "compact" && postCompactClaim === "not-pending"
        ? "static"
        : undefined);
  if (
    shouldSkipPostCompactClaim(
  ...
  const transcriptPath = input.source === "clear"
    ? null
...
export async function runPostCompactHook(
  ...
  markSessionCompacted(
    sessionCachePath(input.session_id, options.pluginDataRoot),
  );
  return "";
}
...
export async function runPostToolUseHook(
  ...
  if (
    ...
    debugTimer.done({
      outputBytes: 0,
      reason: "post-compact-recovery-in-progress",
    });
    return "";
  }
  ...
  const dynamicTargetFingerprints = fingerprintDynamicTargets(
    input.cwd,
    targetPaths,
    config,
    input.model,
  );
  debugTimer.lap("fingerprint", {
    fingerprints: dynamicTargetFingerprints.length,
  });
  const pendingTargetFingerprints = dynamicTargetFingerprints.filter(
    (target) =>
      engine.state.dynamicTargetFingerprints.get(target.cacheKey) !==
        target.fingerprint,
  );
  ...
  const rules = filterRulesAlreadyInTranscript(
    loaded.rules.filter((rule) =>
      !engine.isStaticInjected(rule) && !engine.isDynamicInjected(rule)
    ),
    input.transcript_path,
    (rule) => {
  ...
  for (const target of pendingTargetFingerprints) {
    engine.state.dynamicTargetFingerprints.set(
      target.cacheKey,
      target.fingerprint,
    );
  }
  ...
  const firstPendingTargetPath = pendingTargetFingerprints[0]?.targetPath ??
    firstTargetPath;
  const block = engine.formatDynamic(
    rules,
    displayPath(input.cwd, firstPendingTargetPath),
  );
