...
import {
  spawnDetachedSessionStartWorker,
  writeSessionStartNotice,
} from "./session-start-hook-runtime.js";
import {
...
import {
  ...
  SESSION_START_LOCK_TOKEN_ENV,
} from "./session-start-worker.js";
...
export const CODEGRAPH_SESSION_START_NOTICE =
  "LazyCodex CodeGraph bootstrap scheduled in background";
...
export async function executeCodegraphSessionStartHook(
  ...
  const context: HookContext = {
    ...
    logOutcome: options.logOutcome ??
      ((outcome) => appendSessionStartOutcome(homeDir, outcome, nowMs)),
    nowMs,
    options,
    projectRoot,
  };
  const projectDecision = handleProjectState(
    resolveHookProjectState(context),
    context,
  );
  ...
  const lockedProjectDecision = handleProjectState(
    resolveHookProjectState(context),
    context,
  );
  ...
  writeSessionStartNotice(
    options.stdout ?? processStdout,
    CODEGRAPH_SESSION_START_NOTICE,
  );
...
function resolveHookProjectState(context: HookContext): CodegraphProjectState {
  ...
  return (context.options.ancestorProbe ?? probeCodegraphAncestors)(
    context.projectRoot,
  );
}
...
function handleRecoveredStaleLock(
...
function safeLogOutcome(
  ...
  try {
    ...
    if (error instanceof Error) {
      writeDebugLog(`failed to write SessionStart outcome: ${error.message}`);
    } else throw error;
  }
}
