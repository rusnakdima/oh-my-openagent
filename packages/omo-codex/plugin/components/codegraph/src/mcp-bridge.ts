...
export async function runBridgedCodegraphProcess(
  ...
  const childAndResponsesDone = Promise.all([childExit, responseForwardingDone])
...
async function forwardCodegraphToClient(
  ...
  try {
    for await (const message of readStdioJsonRpcMessages(childOutput)) {
      ...
      const key = responseModeKey(message.payload);
      const pendingResponse = key === null
        ? undefined
        : pendingResponses.get(key);
      const responseMode = pendingResponse?.responseMode ??
        defaultResponseMode();
      if (key !== null) pendingResponses.delete(key);
      await writeStdioJsonRpcResponse(
        output,
        clarifyCodegraphResponse(message.payload, pendingResponse),
        responseMode,
      );
    }
  } catch (error) {
...
function clarifyCodegraphResponse(
  payload: unknown,
  pendingResponse: PendingClientResponse | undefined,
): unknown {
  if (pendingResponse?.method === "tools/list") {
    return clarifyCodegraphToolsList(payload);
  }
  if (
    pendingResponse?.method === "tools/call" &&
    pendingResponse.toolName === "codegraph_node"
  ) {
...
function clarifyCodegraphNodeTool(
...
function hasCodegraphNodeContractMetadata(
  tool: Record<string, unknown>,
): boolean {
...
function clarifyCodegraphNodeInputSchema(
...
function clarifyCodegraphNodeCallResult(payload: unknown): unknown {
  ...
  const content = result["content"].map((item) => {
    if (
      !isPlainRecord(item) || item["type"] !== "text" ||
...
function clarifyContainerOutlineGuidance(text: string): string {
  ...
  return text.replace(
    ...
    CODEGRAPH_CONTAINER_OUTLINE_GUIDANCE,
  );
}
