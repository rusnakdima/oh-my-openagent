...
import {
  resolveSessionStartStatePaths,
  stateParent,
} from "./session-start-paths.js";
...
export function readSessionStartCooldown(
  options: SessionStartCooldownOptions,
): SessionStartCooldownDecision {
  const paths = resolveSessionStartStatePaths(
    options.homeDir,
    options.projectRoot,
  );
...
export function recordSessionStartFailure(
  ...
  const existing = readCooldownState(
    paths.cooldownPath,
    paths.canonicalProjectRoot,
  );
  const failureCount = existing.kind === "present"
    ? existing.value.failureCount + 1
    : 1;
...
export function clearSessionStartCooldown(
  options: { readonly homeDir: string; readonly projectRoot: string },
): void {
  const path =
    resolveSessionStartStatePaths(options.homeDir, options.projectRoot)
      .cooldownPath;
...
function cooldownDuration(
  ...
  for (
    let attempt = 1;
    attempt < failureCount && duration < maxCooldownMs;
    attempt += 1
  ) {
...
function parseCooldownState(
  value: unknown,
  projectRoot: string,
): SessionStartCooldownState | null {
  ...
  if (
    typeof failureCount !== "number" || !Number.isInteger(failureCount) ||
    failureCount < 1
  ) return null;
