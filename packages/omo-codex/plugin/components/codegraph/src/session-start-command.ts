...
export async function resolveSessionStartCommand(options: {
  ...
  if (
    resolved.source !== "env" && options.config.auto_provision !== false &&
    options.deps.managedInstallExists(installDir)
  ) {
...
export function sessionStartCodegraphEnv(
  config: CodegraphBootstrapConfig,
  homeDir: string,
): Record<string, string> {
  const env = buildCodegraphEnv({ daemon: false, homeDir });
  return config.trustedCodegraphInstallDir === undefined
    ? env
    : { ...env, CODEGRAPH_INSTALL_DIR: config.trustedCodegraphInstallDir };
}
...
export async function runSessionStartCodegraphCommand(
  ...
  return runProcessWithTreeTimeout({
    ...
    maxBuffer: 1024 * 1024,
    onTerminationReport: (report) =>
      processStderr.write(
        `[codegraph-session-start] process tree termination ${
...
function canUseResolvedCommand(
  resolved: CodegraphCommandResolution,
  nodeSupport: CodegraphNodeSupport,
): boolean {
  return !codegraphCommandRequiresSupportedLocalNode(resolved) ||
