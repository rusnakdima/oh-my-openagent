...
import {
  type CodexOmoConfig,
  getCodexOmoConfig,
} from "../../../shared/src/config-loader.ts";
...
export interface RunCodegraphServeOptions {
  readonly buildEnv?: (
    options: { readonly daemon: boolean; readonly homeDir: string },
...
const CODEGRAPH_EXCLUDED_HINT =
  "CodeGraph MCP skipped: project excluded by OMO CodeGraph policy.\n";
...
export async function runCodegraphServe(
  ...
  const config = options.config ??
    getCodexOmoConfig({ cwd: projectCwd, env, homeDir });
  ...
  let resolution = options.resolve?.(resolutionOptions) ??
    resolveCodegraphCommand(resolutionOptions);
  const resolveManagedBin = options.resolveManagedBin ??
    (options.resolve === undefined ? provisionedBinFromInstallDir : () => null);
  const managedInstallExists = options.managedInstallExists ??
    (options.resolve === undefined ? hasCodegraphManagedInstall : () => false);
  ...
  if (resolution.source !== "env" && managedBin !== null) {
    ...
    resolution.source !== "env" && codegraphConfig.auto_provision !== false &&
    managedInstallExists(installDir)
  ) {
    const upgraded = await provisionMissingCodegraph({
      config: codegraphConfig,
      ensureProvisioned: options.ensureProvisioned ??
        ensureCodegraphProvisioned,
      homeDir,
      resolution,
  ...
  if (
    ...
    if (resolution.source === "path" && !nodeSupport.supported) {
      return runUnavailableMcp(
        buildCodegraphNodeSkipHint(nodeSupport),
        options,
      );
    }
  ...
  if (
    codegraphCommandRequiresSupportedLocalNode(resolution) &&
  ...
  const codegraphEnv = codegraphEnvForConfig(
    trustedInstallDir,
    homeDir,
    codegraphConfig.daemon !== false,
    options.buildEnv,
  );
...
async function provisionMissingCodegraph(options: {
  ...
  const installDir = options.trustedInstallDir ??
    join(options.homeDir, ".omo", "codegraph");
...
function codegraphEnvForConfig(
  ...
  const env = buildEnv?.({ daemon, homeDir }) ??
    buildCodegraphEnv({ daemon, homeDir });
