...
import {
  hasCodegraphManagedInstall,
  resolvePinnedCodegraphBin,
} from "../../../../../utils/src/codegraph/managed-runtime.ts";
import {
...
import {
  ...
  sessionStartLockIsOwned,
} from "./session-start-lock.js";
...
export const SESSION_START_LOCK_TOKEN_ENV =
  "OMO_CODEGRAPH_SESSION_START_LOCK_TOKEN";
...
export async function runCodegraphSessionStartWorker(
  options: SessionStartWorkerOptions = {},
): Promise<{ readonly action: WorkerAction }> {
  const env = options.env ?? processEnv;
  const homeDir = resolveHomeDir(env);
  const projectRoot = options.cwd ?? env[SESSION_START_CWD_ENV] ?? processCwd();
  const config = options.config ??
    getCodexOmoConfig({ cwd: projectRoot, env, homeDir });
  const lock = resolveWorkerLock(
    homeDir,
    projectRoot,
    env[SESSION_START_LOCK_TOKEN_ENV],
  );
  if (lock !== null && !sessionStartLockIsOwned(lock)) {
    const outcome = {
      action: "skipped-locked",
      projectRoot,
    } satisfies CodegraphSessionStartOutcome;
    safeLogWorkerOutcome(
      options.logOutcome ??
        ((value) => appendSessionStartOutcome(homeDir, value)),
      outcome,
    );
  ...
  const context: WorkerContext = {
    baseCooldownMs: config.codegraph?.session_start_cooldown_ms ??
      DEFAULT_SESSION_START_COOLDOWN_MS,
    config: {
      ...(config.codegraph ?? {}),
      ...(config.trustedCodegraphInstallDir === undefined
        ? {}
        : { trustedCodegraphInstallDir: config.trustedCodegraphInstallDir }),
    },
    deps: { ...defaultDeps, ...customResolutionDeps, ...options.deps },
    env,
    homeDir,
    logOutcome: options.logOutcome ??
      ((outcome) => appendSessionStartOutcome(homeDir, outcome)),
...
async function runBootstrap(
  ...
  try {
    ...
    return finishWorkerInitialization(
      context.probeProject(context.projectRoot),
      {
        exitCode: action.exitCode,
        source: command.resolution.source,
      },
...
function resolveWorkerLock(
  homeDir: string,
  projectRoot: string,
  token: string | undefined,
): SessionStartLock | null {
