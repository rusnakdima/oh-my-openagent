...
describe("CodeGraph SessionStart worker flow", () => {
  it("#given install_dir has the pinned platform binary #when worker resolves provisioned CodeGraph #then it uses that launcher", async () => {
    ...
    const workspace = mkdtempSync(
      join(tmpdir(), "omo-codegraph-worker-platform-"),
    );
    const homeDir = mkdtempSync(
      join(tmpdir(), "omo-codegraph-worker-platform-home-"),
    );
    const installDir = mkdtempSync(
      join(tmpdir(), "omo-codegraph-worker-platform-install-"),
    );
    const binPath = join(
      installDir,
      "bin",
    ...
    try {
      ...
      const result = await runCodegraphSessionStartWorker({
        ...
        deps: {
          ...
          ensureProvisioned: () => {
            throw new Error(
              "provisioning should not run when install_dir binary exists",
            );
          },
          prepareWorkspace: () => ({
  ...
  it("#given an uninitialized exact project #when worker runs #then it invokes init directly without a status command", async () => {
    ...
    const homeDir = mkdtempSync(
      join(tmpdir(), "omo-codegraph-init-direct-home-"),
    );
    const calls: {
      readonly args: readonly string[];
      readonly env: Record<string, string>;
    ...
    try {
      ...
      const result = await runCodegraphSessionStartWorker({
        config: {
          codegraph: { enabled: true, install_dir: "/tmp/codegraph-install" },
          sources: [],
          trustedCodegraphInstallDir: "/tmp/codegraph-install",
          warnings: [],
        },
        nodeVersion: "22.14.0",
        cwd: workspace,
        ...
        deps: {
          ensureGitignored: () => true,
          ensureProvisioned: () =>
            Promise.resolve({ binPath: "/tmp/codegraph", provisioned: true }),
          prepareWorkspace: () => ({
            dataDir: join(homeDir, ".omo/codegraph/projects/test"),
            dataRoot: join(homeDir, ".omo/codegraph"),
            linked: true,
      ...
      expect(calls[0]?.env["CODEGRAPH_INSTALL_DIR"]).toBe(
        "/tmp/codegraph-install",
      );
      expect(outcomes).toEqual([{
        action: "initialized",
        exitCode: 0,
  ...
  it("#given codegraph.daemon=true #when the indexing worker runs #then its env stays daemon-less with CODEGRAPH_NO_DAEMON=1", async () => {
    // given
    const workspace = mkdtempSync(
      join(tmpdir(), "omo-codegraph-worker-daemon-"),
    );
    const homeDir = mkdtempSync(
      join(tmpdir(), "omo-codegraph-worker-daemon-home-"),
    );
    const calls: {
      readonly args: readonly string[];
      readonly command: string;
      ...
      const result = await runCodegraphSessionStartWorker({
        config: {
          codegraph: {
            daemon: true,
            enabled: true,
            install_dir: "/tmp/codegraph-install",
          },
          sources: [],
          trustedCodegraphInstallDir: "/tmp/codegraph-install",
          warnings: [],
  ...
  it("#given ambient provider tokens #when default worker runner spawns CodeGraph #then child env only gets safe and controlled variables", async () => {
    ...
    const homeDir = mkdtempSync(
      join(tmpdir(), "omo-codegraph-worker-env-home-"),
    );
    ...
    try {
      const fakeCodegraphScript = [
        ...
        }, JSON.stringify({install:process.env.CODEGRAPH_INSTALL_DIR,openai:process.env.OPENAI_API_KEY}) + '\\n');`,
      ...
      const result = await runCodegraphSessionStartWorker({
        ...
        deps: {
          ...
          ensureProvisioned: () => {
            throw new Error(
              "provisioning should not run when command is resolved",
            );
          },
          prepareWorkspace: () => ({
            dataDir: join(homeDir, ".omo/codegraph/projects/test"),
          ...
          resolveCommand: () => ({
            argsPrefix: ["-e", fakeCodegraphScript],
            command: process.execPath,
            exists: true,
            source: "bundled",
          }),
