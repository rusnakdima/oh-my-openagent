...
describe("CodeGraph SessionStart spawn guards", () => {
  ...
  it("#given one hook already owns the project lock #when a second SessionStart runs #then it records skipped-locked without spawning", async () => {
    ...
    const options = {
      ...
      cwd: workspace,
      env: { HOME: homeDir },
      logOutcome: (outcome: CodegraphSessionStartOutcome) =>
    ...
    try {
      ...
      // when
      const second = await executeCodegraphSessionStartHook({
        ...options,
        stdin: Readable.from(["{}"]),
      });
  ...
  it("#given a failed worker cooldown #when SessionStart retries before and after expiry #then only the expired retry spawns", async () => {
    ...
    const homeDir = mkdtempSync(
      join(tmpdir(), "omo-codegraph-cooldown-hook-home-"),
    );
    const workspace = createAllowedWorkspace(
      "codegraph-cooldown-hook-workspace",
    );
    const outcomes: CodegraphSessionStartOutcome[] = [];
    const spawned: WorkerSpawnInvocation[] = [];
    try {
      ...
      // when
      const suppressed = await executeCodegraphSessionStartHook({
        ...baseOptions,
        nowMs: 60_999,
        stdin: Readable.from(["{}"]),
      });
      const expired = await executeCodegraphSessionStartHook({
        ...baseOptions,
        nowMs: 61_000,
        stdin: Readable.from(["{}"]),
      });
  ...
  it("#given an ancestor has a CodeGraph database #when SessionStart runs in a nested project #then it records skipped-nested-root", async () => {
    ...
    try {
      ...
      writeFileSync(
        join(ancestorRoot, ".codegraph", "codegraph.db"),
        "fixture",
      );
