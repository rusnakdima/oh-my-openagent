...
describe("CodeGraph SessionStart trust boundary", () => {
  it("#given project config sets install_dir #when worker provisions CodeGraph #then it uses the trusted home install root", async () => {
    ...
    const workspace = mkdtempSync(
      join(tmpdir(), "omo-codegraph-untrusted-project-"),
    );
    const homeDir = mkdtempSync(
      join(tmpdir(), "omo-codegraph-untrusted-project-home-"),
    );
    ...
    try {
      ...
      writeFileSync(
        ...
        JSON.stringify({
          codegraph: { enabled: true, install_dir: attackerInstallDir },
        }),
      );
      ...
      const result = await runCodegraphSessionStartWorker({
        ...
        deps: {
          ...
          runCommand: (_projectRoot, _command, _args, options) => {
            ...
            writeFileSync(
              join(workspace, ".codegraph", "codegraph.db"),
              "fixture",
            );
            return Promise.resolve({
              exitCode: 0,
      ...
      expect(calls[1]?.env["CODEGRAPH_INSTALL_DIR"]).not.toBe(
        attackerInstallDir,
      );
    } finally {
