...
describe("CodeGraph SessionStart worker cooldown", () => {
  it("#given codegraph init times out #when the worker finishes #then it records a failed outcome and cooldown", async () => {
    ...
    const workspace = mkdtempSync(
      join(tmpdir(), "omo-codegraph-worker-timeout-"),
    );
    const homeDir = mkdtempSync(
      join(tmpdir(), "omo-codegraph-worker-timeout-home-"),
    );
    ...
    try {
      ...
      const result = await runCodegraphSessionStartWorker({
        config: {
          codegraph: { enabled: true, session_start_cooldown_ms: 60_000 },
          sources: [],
          warnings: [],
        },
        cwd: workspace,
        ...
        deps: {
          ...
          runCommand: () =>
            Promise.resolve({ exitCode: 1, stdout: "", timedOut: true }),
        },
      });
  ...
  it("#given codegraph init exits zero without an exact database #when the worker verifies the result #then it records a failure cooldown", async () => {
    // given
    const workspace = mkdtempSync(
      join(tmpdir(), "omo-codegraph-worker-no-db-"),
    );
    const homeDir = mkdtempSync(
      join(tmpdir(), "omo-codegraph-worker-no-db-home-"),
    );
    const outcomes: CodegraphSessionStartOutcome[] = [];
    try {
      // when
      const result = await runCodegraphSessionStartWorker({
        ...
        deps: {
          ...
          runCommand: () =>
            Promise.resolve({ exitCode: 0, stdout: "", timedOut: false }),
        },
      });
      const cooldown = readSessionStartCooldown({
        baseCooldownMs: 60_000,
      ...
      expect(outcomes).toEqual([{
        action: "failed",
        error:
          "codegraph init completed without creating an exact project database",
        exitCode: 0,
        projectRoot: workspace,
        source: "path",
        timedOut: false,
