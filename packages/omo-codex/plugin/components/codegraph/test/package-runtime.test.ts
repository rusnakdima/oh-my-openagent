...
function readCodegraphPackageJson(): CodegraphPackageJson {
  const parsed: unknown = JSON.parse(
    readFileSync(resolve(componentRoot, "package.json"), "utf8"),
  );
  if (!isCodegraphPackageJson(parsed)) {
    throw new TypeError("Invalid CodeGraph package metadata");
  }
  return parsed;
}

function isCodegraphPackageJson(value: unknown): value is CodegraphPackageJson {
  return isRecord(value) && maybeStringArray(value["files"]) &&
    maybeStringRecord(value["optionalDependencies"]);
}

function maybeStringArray(
  value: unknown,
...
describe("CodeGraph component runtime package metadata", () => {
  it("#given the CodeGraph component package #when validating runtime distribution metadata #then upstream CodeGraph installs optionally with shipped legal files", () => {
    ...
    expect(existsSync(resolve(componentRoot, "NODE-RUNTIME-LICENSES.md"))).toBe(
      true,
    );
    expect(existsSync(resolve(componentRoot, "NOTICE"))).toBe(true);
    expect(existsSync(resolve(componentRoot, "dist", "cli.d.ts"))).toBe(false);
    expect(existsSync(resolve(componentRoot, "dist", "serve.d.ts"))).toBe(
      false,
    );
  });
  ...
  it("#given CodeGraph platform bundles include Node #when reading runtime licenses #then Node license text ships with the component", () => {
    ...
    const licenseText = readFileSync(
      resolve(componentRoot, "NODE-RUNTIME-LICENSES.md"),
      "utf8",
    );
  ...
  it("#given third-party notices #when validating CodeGraph component notices #then aggregate Codex notices list CodeGraph as shipped", () => {
    // given
    const codexNotice = readFileSync(
      resolve(repoRoot, "packages/omo-codex/THIRD-PARTY-NOTICES.md"),
      "utf8",
    );

    // when
    const listsCodegraphComponent = codexNotice.includes(
      "@sisyphuslabs/codex-codegraph",
    );
    const listsUpstreamRuntime = codexNotice.includes(
    ...
    expect(codexNotice).not.toContain(
      "not currently an omo-codex plugin component",
    );
  });
});
