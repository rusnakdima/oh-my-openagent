...
describe("CodeGraph SessionStart worker Node support", () => {
  it("#given an unsupported local Node and a PATH CodeGraph command with auto provisioning disabled #when worker runs #then it skips without touching the workspace", async () => {
    ...
    const homeDir = mkdtempSync(
      join(tmpdir(), "omo-codegraph-worker-node-home-"),
    );
    ...
    try {
      ...
      const result = await runCodegraphSessionStartWorker({
        config: {
          codegraph: { auto_provision: false, enabled: true },
          sources: [],
          warnings: [],
        },
        cwd: workspace,
        ...
        deps: {
          ...
          ensureProvisioned: () => {
            throw new Error(
              "ensureProvisioned should not run on unsupported Node",
            );
          },
          prepareWorkspace: () => {
            throw new Error(
              "prepareWorkspace should not run on unsupported Node",
            );
          },
          runCommand: () => {
      ...
      expect(outcomes).toEqual([{
        action: "skipped-unsupported-node",
  ...
  it("#given an unsupported local Node and a PATH CodeGraph command #when auto provisioning succeeds #then it bootstraps with the provisioned binary", async () => {
    ...
    const workspace = mkdtempSync(
      join(tmpdir(), "omo-codegraph-worker-node-provision-"),
    );
    const homeDir = mkdtempSync(
      join(tmpdir(), "omo-codegraph-worker-node-provision-home-"),
    );
    ...
    try {
      ...
      expect(provisionCalls).toEqual([
        {
          installDir: join(homeDir, ".omo", "codegraph"),
          lockDir: join(homeDir, ".omo", "codegraph", ".locks"),
          version: "1.5.0",
        },
      ]);
      expect(outcomes).toEqual([{
  ...
  it("#given bundled CodeGraph and a stale managed install #when the worker runs #then it upgrades and uses the managed runtime", async () => {
    // given
    const workspace = mkdtempSync(
      join(tmpdir(), "omo-codegraph-worker-managed-upgrade-"),
    );
    const homeDir = mkdtempSync(
      join(tmpdir(), "omo-codegraph-worker-managed-upgrade-home-"),
    );
  ...
  it("#given an unsupported local Node but bundled CodeGraph resolves through CODEGRAPH_NODE_BIN #when worker runs #then it bootstraps with the compatible runtime", async () => {
    // given
    const workspace = mkdtempSync(
      join(tmpdir(), "omo-codegraph-worker-compatible-node-"),
    );
    const homeDir = mkdtempSync(
      join(tmpdir(), "omo-codegraph-worker-compatible-node-home-"),
    );
    ...
    try {
      ...
      const result = await runCodegraphSessionStartWorker({
        ...
        deps: {
          ...
          ensureProvisioned: () => {
            throw new Error(
              "provisioning should not run when bundled CodeGraph resolved",
            );
          },
          prepareWorkspace: () => ({
            dataDir: join(homeDir, ".omo/codegraph/projects/test"),
