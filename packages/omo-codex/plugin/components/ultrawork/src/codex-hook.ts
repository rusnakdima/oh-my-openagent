...
import {
  ...
  type UltraworkAdditionalContextOptions,
} from "./skill-pointer.js";

const ULTRAWORK_CURRENT_PROMPT_PATTERN =
  /(?:ultrawork|ulw(?!-(?:plan|research)))/i;
...
function readTranscriptTail(transcriptPath: string): string {
  ...
  return rawTranscript.subarray(
    Math.max(0, rawTranscript.byteLength - TRANSCRIPT_SEARCH_BYTES),
  ).toString("utf8");
}
