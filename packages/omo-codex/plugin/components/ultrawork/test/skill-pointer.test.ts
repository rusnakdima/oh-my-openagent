...
import {
  ...
  buildUltraworkSkillPointer,
} from "../src/skill-pointer.js";
...
describe("ultrawork skill pointer", () => {
  ...
  it("#given a missing skill file #when context is built #then falls back to the full directive", () => {
    ...
    // when
    const context = buildUltraworkAdditionalContext({
      skillFilePath: missingSkillFilePath,
    });
  ...
  it("#given a null skill path #when context is built #then falls back to the full directive", () => {
    expect(buildUltraworkAdditionalContext({ skillFilePath: null })).toBe(
      ULTRAWORK_DIRECTIVE,
    );
  });
