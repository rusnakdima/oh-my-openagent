...
const CREATE_THREAD_TOOL_NAMES = new Set([
  "create_thread",
...
function extractThreadCreationReference(
  toolResponse: unknown,
): ThreadCreationReference | null {
  ...
  if (
    typeof pendingWorktreeId === "string" && pendingWorktreeId.trim().length > 0
  ) {
...
function parseToolResponseRecord(
  toolResponse: unknown,
