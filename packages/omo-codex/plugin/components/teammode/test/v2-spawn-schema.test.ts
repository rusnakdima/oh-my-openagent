...
const teamScriptPath = fileURLToPath(
  new URL("../skills/teammode/scripts/team.mjs", import.meta.url),
);
  ...
  it("#given a team member #when preparing its native spawn #then emits only exposed V2 fields", async () => {
    ...
    try {
      ...
      expect(delivery).toContain(
        'using only task_name "schema_member", message <bootstrap>, and fork_turns "none"',
      );
