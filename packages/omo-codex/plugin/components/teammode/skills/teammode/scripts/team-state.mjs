...
const LOCK_TIMEOUT_MS = Number.parseInt(
  process.env.OMO_TEAMMODE_LOCK_TIMEOUT_MS ?? "10000",
  10,
);
const LOCK_RETRY_MS = Number.parseInt(
  process.env.OMO_TEAMMODE_LOCK_RETRY_MS ?? "25",
  10,
);
...
export function buildTeam(
  ...
  return {
    ...
    leader: {
      kind: "main-session",
      sessionId: v2 ? null : sessionId,
      agentPath: v2 ? LEADER_AGENT_PATH : null,
    },
    communication: {
      memberLanguage: "english",
      replyToUserInUserLanguage: true,
    },
    worktree: {
      enabled: Boolean(worktreeEnabled),
      baseBranch,
      root: dir ? join(dir, "worktrees") : null,
    },
    paths: dir
      ? {
        dir,
        team: join(dir, "team.json"),
...
function assertUniqueMemberFocus(team) {
  ...
  for (const member of team.members) {
    ...
    if (previous) {
      throw new Error(
        `member focus "${member.focus}" duplicates "${previous.focus}" (no two members may own the same thing)`,
      );
    }
    seen.set(key, member);
  }
...
function assertUniqueMemberName(team) {
  ...
  for (const member of team.members) {
    ...
    if (previous) {
      throw new Error(
        `member name "${memberName}" duplicates "${
          previous.name ?? previous.focus
        }" (no two members may produce the same thread title)`,
      );
    }
    seen.set(key, member);
  }
...
function assertUniqueMemberThreadTitle(team) {
  ...
  for (const member of team.members) {
    ...
    if (typeof threadTitle !== "string" || !threadTitle.trim()) {
      throw new Error(
        `member "${
          member.id ?? member.name ?? member.focus ?? "(unknown)"
        }" has invalid threadTitle (non-empty string required)`,
      );
    }
    ...
    if (previous) {
      throw new Error(
        `member threadTitle "${threadTitle}" duplicates "${previous.threadTitle}" (no two members may produce the same thread title)`,
      );
    }
    seen.set(key, member);
  }
...
function assertUniqueMemberTaskName(team) {
  ...
  for (const member of team.members) {
    ...
    if (previous) {
      throw new Error(
        `member taskName "${taskName}" duplicates "${previous.taskName}" (no two members may share an agent path)`,
      );
    }
    if (member.agentPath !== agentPathForTaskName(taskName)) {
      throw new Error(
        `member "${member.id}" agentPath "${member.agentPath}" does not match task name "${taskName}"`,
      );
    }
...
export function addMember(
  ...
  if (!focus?.trim()) {
    throw new Error(
      "member focus is required - a concrete part, ownership area, or perspective",
    );
  }
  if (!LENSES.includes(lens)) {
    throw new Error(
      `invalid lens "${lens}" - use one of: ${LENSES.join(", ")}`,
    );
  }
  const v2 = isMultiAgentV2(team);
  if (!v2 && taskName !== null) {
    throw new Error("--task-name is only valid on multi_agent_v2 teams");
  }
  ...
  if (team.members.some((m) => m.id === memberId)) {
    throw new Error(`member id "${memberId}" already exists (duplicate)`);
  }
  const duplicate = team.members.find((m) =>
    normalizedFocus(m.focus) === normalizedFocus(memberFocus)
  );
  if (duplicate) {
    throw new Error(
      `member focus "${memberFocus}" duplicates "${duplicate.focus}" (no two members may own the same thing)`,
    );
  }
  const duplicateName = team.members.find((m) =>
    normalizedMemberName(m.name ?? m.focus ?? "") ===
      normalizedMemberName(memberName)
  );
  if (duplicateName) {
    throw new Error(
      `member name "${memberName}" duplicates "${
        duplicateName.name ?? duplicateName.focus
      }" (no two members may produce the same thread title)`,
    );
  }
  if (v2) {
    const duplicateTask = team.members.find((m) =>
      m.taskName === memberTaskName
    );
    if (duplicateTask) {
      throw new Error(
        `member taskName "${memberTaskName}" duplicates "${duplicateTask.taskName}" (no two members may share an agent path)`,
      );
    }
  }
  team.members.push({
  ...
  return touch(
    team,
    "add-member",
    `member ${memberId} (${lens}): ${memberFocus}`,
  );
}

export function bindThread(
  team,
  { id, threadId, cwd = null, worktreePath = null },
) {
  ...
  if (team.worktree.enabled) {
    m.worktree.path = worktreePath ?? cwd ?? m.worktree.path;
  }
  return touch(
    team,
    "bind-thread",
    `member ${id} -> thread ${threadId.trim()}`,
  );
}

export function bindAgent(
  team,
  { id, agentPath, cwd = null, worktreePath = null },
) {
  ...
  if (boundPath !== m.agentPath) {
    throw new Error(
      `agent path mismatch for member ${m.id}: expected "${m.agentPath}" (task name "${m.taskName}"), got "${boundPath}"`,
    );
  }
  m.status = "active";
  if (cwd) m.cwd = cwd;
...
export function setMemberStatus(team, { id, status, note = "" }) {
  if (!MEMBER_STATUSES.includes(status)) {
    throw new Error(
      `invalid status "${status}" - use one of: ${MEMBER_STATUSES.join(", ")}`,
    );
  }
...
export function archive(team, { id = null, note = "" } = {}) {
  if (id) {
    ...
    const memberQualifier = isMultiAgentV2(team)
      ? " (team state only; V2 exposes no runtime archive operation)"
      : "";
    return touch(
      team,
      "archive-member",
      `member ${id}${memberQualifier}${note ? `: ${note}` : ""}`,
    );
  }
...
export function validateTeam(team) {
  ...
  if (team?.schemaVersion !== TEAM_SCHEMA_VERSION) {
    throw new Error(
      `invalid team: schemaVersion must be ${TEAM_SCHEMA_VERSION}`,
    );
  }
  parseTeamTransport(team.transport);
  if (!team.teamId || !team.teamName) {
    throw new Error("invalid team: teamId and teamName are required");
  }
  if (team.leader?.kind !== "main-session") {
    throw new Error("invalid team: leader.kind must be main-session");
  }
  if (!Array.isArray(team.members)) {
    throw new Error("invalid team: members must be an array");
  }
  assertUniqueMemberFocus(team);
  assertUniqueMemberName(team);
  assertUniqueTransportIdentity(team);
...
export function resolveTeamDir(cwd, sessionId) {
  if (!SESSION_ID_PATTERN.test(sessionId ?? "")) {
    throw new Error(`invalid or unsafe session id "${sessionId}"`);
  }
...
export async function withTeamLock(dir, command, fn, options = {}) {
  ...
  const retryMs = positiveIntegerOrFallback(
    options.retryMs ?? LOCK_RETRY_MS,
    25,
  );
  ...
  for (;;) {
    try {
      ...
      if (st && !st.isDirectory()) {
        throw new Error(
          `refused: team lock path is not a directory: ${lockDir}`,
        );
      }
      if (Date.now() >= deadline) {
        const owner = await describeLockOwner(lockDir);
        throw new Error(
          `team state is locked by ${owner}; retry after that command completes`,
        );
      }
...
async function mkdirNoSymlink(dir, stopAt) {
  ...
  if (rel.startsWith("..") || isAbsolute(rel)) {
    throw new Error(`refused: path escapes ${stopAt}: ${dir}`);
  }
...
export async function readTeam(dir) {
  return validateTeam(
    JSON.parse(await readFile(join(dir, "team.json"), "utf8")),
  );
}
...
async function persistedFileTarget(team, pathKey, fileName, expectedDir) {
  validateTeam(team);
  if (!team.paths?.dir || !team.paths?.[pathKey]) {
    throw new Error(`invalid team: paths.${pathKey} is required`);
  }
  const dir = resolve(expectedDir);
  if (resolve(team.paths.dir) !== dir) {
    throw new Error(
      `refused: persisted team dir does not match trusted team dir: ${team.paths.dir}`,
    );
  }
  const target = resolve(team.paths[pathKey]);
  if (target !== resolve(dir, fileName)) {
    throw new Error(
      `refused: ${fileName} persist target escapes team dir: ${
        team.paths[pathKey]
      }`,
    );
  }
  ...
  if (dirStat?.isSymbolicLink()) {
    throw new Error(`refused: team dir is a symlink: ${dir}`);
  }
  const st = await lstatOrNull(target);
  if (st?.isSymbolicLink()) {
    throw new Error(`refused: ${fileName} is a symlink: ${target}`);
  }
  if (st && !st.isFile()) {
    throw new Error(`refused: ${fileName} is not a file: ${target}`);
  }
  return target;
}

async function writePersistedFileAtomic(
  team,
  pathKey,
  fileName,
  content,
  ...
  const target = await persistedFileTarget(
    team,
    pathKey,
    fileName,
    expectedDir,
...
export async function writeTeamAtomic(team, expectedDir) {
  return writePersistedFileAtomic(
    team,
    "team",
    "team.json",
