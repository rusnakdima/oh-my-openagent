...
function git(cwd, args) {
  ...
  if (result.error) {
    throw new Error(
      `git ${args.join(" ")} could not run: ${result.error.message}`,
    );
  }
  return result;
}
...
export function memberWorktreePath(team, member) {
  if (!MEMBER_ID_SEGMENT.test(member.id ?? "")) {
    throw new Error(`unsafe member id for a worktree path: "${member.id}"`);
  }
  ...
  if (rel !== member.id || rel.startsWith("..") || isAbsolute(rel)) {
    throw new Error(`refused: worktree path escapes ${root}`);
  }
  return path;
}
...
export function addMemberWorktree(cwd, team, member, { baseBranch } = {}) {
  ...
  if (
    listWorktrees(cwd).some((w) =>
      w.branch === `refs/heads/${branch}` ||
      samePath(resolve(cwd, w.path), resolve(cwd, path))
    )
  ) {
...
export function removeMemberWorktree(
  cwd,
  team,
  member,
  { force = false } = {},
...
export function integrateMemberBranch(cwd, branch) {
  ...
  if (result.status === 0) {
    return { branch, merged: true, into: currentBranch(cwd) };
  }
  const conflicts = gitOrThrow(cwd, ["diff", "--name-only", "--diff-filter=U"])
    .trim();
  return {
    branch,
    merged: false,
