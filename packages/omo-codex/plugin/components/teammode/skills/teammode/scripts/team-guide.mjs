...
function memberEndpoint(member, v2) {
  if (v2) {
    return member.agentPath
      ? ` | task \`${member.taskName}\` | agent \`${member.agentPath}\``
      : " | agent not spawned yet";
  }
  const thread = member.threadId
    ? ` | thread ${member.threadId} (${codexThreadLink(member.threadId)})`
    : " | thread not created yet";
  const title = member.threadTitle
    ? ` | thread title \`${member.threadTitle}\``
    : "";
  return `${title}${thread}`;
}
...
function memberLine(member, v2) {
  ...
  return `- **${member.id}**${name} (${member.lens}) - ${member.focus}${
    member.deliverable ? ` -> ${member.deliverable}` : ""
  }${memberEndpoint(member, v2)}${wt} [${member.status}]`;
}
...
that is a coordination signal: mark yourself \`blocked\` and tell the leader at once.`;
  ...
  const lines = team.members
    ...
    .map((m) => {
      ...
      return `- **${m.id}**: worktree \`${
        m.worktree.path ?? bindHint
      }\` on branch \`${m.worktree.branch ?? team.worktree.baseBranch}\`${
        worktreeMemberEndpoint(m, v2)
      }`;
    });
...
export function buildGuide(team) {
  const v2 = isMultiAgentV2(team);
  const artifacts = team.paths?.artifacts ??
    ".omo/teams/<session_id>/artifacts";
  const teamJson = team.paths?.team ?? ".omo/teams/<session_id>/team.json";
  const roster = team.members.length
    ? team.members.map((member) => memberLine(member, v2)).join("\n")
    : "- (no members yet)";
...
function memberEndpointNote(team, member) {
  ...
  return member.threadId
    ? `\nYour Codex thread link is ${
      codexThreadLink(member.threadId)
    }. Include it when reporting handoffs or worktree status.`
    : "";
}
...
export function buildMemberPrompt(team, id) {
  ...
  const readiness = team.worktree?.enabled
    ...
    : "";
  return `You are member ${member.id}${
    member.name ? ` (${member.name})` : ""
  } of team ${team.teamName} - owner of: ${member.focus}. ${
    memberIdentityLine(team, member)
  }
...
Start now. ${
    pushInstruction(team)
  } - findings, \`WORKING:\` heartbeats, and \`BLOCKED:\` the instant you stall - never just one report at the end.`;
}
