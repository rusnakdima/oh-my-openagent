...
const handlers = {
  async init(cwd, flags) {
    ...
    await withTeamLock(dir, "init", async () => {
      ...
      const team = buildTeam({
        ...
        baseBranch: typeof flags["base-branch"] === "string"
          ? flags["base-branch"]
          : "dev",
      });
  ...
  async "add-member"(cwd, flags) {
    ...
    const memberInput = {
      ...
      deliverable: typeof flags.deliverable === "string"
        ? flags.deliverable
        : "",
      branch: typeof flags.branch === "string" ? flags.branch : null,
      taskName: typeof flags["task-name"] === "string"
        ? flags["task-name"]
        : null,
    };
  ...
  async "bind-thread"(cwd, flags) {
    ...
    const input = {
      ...
      worktreePath: typeof flags["worktree-path"] === "string"
        ? flags["worktree-path"]
        : null,
    };
  ...
  async "worktree-add"(cwd, flags) {
    ...
    const baseBranch = typeof flags["base-branch"] === "string"
      ? flags["base-branch"]
      : null;
  ...
  async integrate(cwd, flags) {
    const sessionId = requireFlag(flags, "team");
    const { team } = await loadTeam(cwd, sessionId);
    const targets = typeof flags.id === "string"
      ? [memberOrThrow(team, flags.id)]
      : team.members.filter((m) => m.worktree?.branch);
    if (targets.length === 0) {
      throw new Error(
    ...
    for (const member of targets) {
      ...
      if (!result.merged) {
        if (result.conflicts.length > 0) {
          ...
          throw new Error(
            `merge conflict integrating member ${member.id} (branch ${member.worktree.branch}). Resolve the conflict, commit the merge, then re-run integrate. Conflicting files: ${
        ...
        throw new Error(
          `could not integrate member ${member.id} (branch ${member.worktree.branch}): ${
            result.message || "git merge failed; see git status"
          }`,
        );
      }
      process.stdout.write(
  ...
  async status(cwd, flags) {
    ...
    for (const m of team.members) {
      const endpoint = isMultiAgentV2(team)
        ? m.agentPath ? ` agent=${m.agentPath}` : ""
        : m.threadId
        ? ` thread=${m.threadId} link=${codexThreadLink(m.threadId)}`
        : "";
      process.stdout.write(
        `  ${m.id} (${m.lens}) ${m.focus} -> ${
          m.deliverable || "(no deliverable)"
        } [${m.status}]${endpoint}${m.cwd ? ` cwd=${m.cwd}` : ""}\n`,
      );
    }
