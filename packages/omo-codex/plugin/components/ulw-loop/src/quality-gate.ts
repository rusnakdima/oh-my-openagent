...
import {
  adversarialVerdict,
  codeQualityStatusField,
  passedVerdict,
} from "./quality-gate-verdicts.js";
import type {
...
function reviewerRoleField<T extends string>(
  value: unknown,
  expected: T,
  field: string,
): T {
...
function artifactMap(
  ...
  for (const ref of refs) {
    if (byId.has(ref.id)) {
      invalid(
        `manualQa.artifactRefs contains duplicate ${ref.id}.`,
...
function parseArtifactRefs(
  ...
  if (!Array.isArray(value) || value.length === 0) {
    invalid(
      "manualQa.artifactRefs must not be empty.",
      "manualQa.artifactRefs",
    );
  }
  return value.map((item, index) => {
    ...
    return {
      ...
      description: textField(
        ref["description"],
        `manualQa.artifactRefs[${index}].description`,
      ),
      path,
    };
  });
...
export function validateQualityGate(
  ...
  const totalCriteria = numberField(
    coverage["totalCriteria"],
    "criteriaCoverage.totalCriteria",
  );
  const passCount = numberField(
    coverage["passCount"],
    "criteriaCoverage.passCount",
  );
  if (passCount < totalCriteria) {
    invalid(
      "criteriaCoverage.passCount must cover totalCriteria.",
      "criteriaCoverage.passCount",
    );
  }
  const artifactRefs = parseArtifactRefs(manualQa["artifactRefs"], opts);
  const byId = artifactMap(artifactRefs);
  const surfaceEvidence = parseSurfaceEvidence(
    manualQa["surfaceEvidence"],
    byId,
  );
  const adversarialCases = parseAdversarialCases(
    manualQa["adversarialCases"],
    byId,
  );
  const codeReportPath = textField(
    codeReview["reportPath"],
    "codeReview.reportPath",
  );
  const gateReportPath = textField(
    gateReview["reportPath"],
    "gateReview.reportPath",
  );
  ...
  return {
    codeReview: {
      by: reviewerRoleField(
        codeReview["by"],
        REVIEWER_ROLES.codeReview,
        "codeReview.by",
      ),
      ...
      codeQualityStatus: codeQualityStatusField(
    ...
    criteriaCoverage: {
      totalCriteria,
      passCount,
      originalIntent: textField(
        coverage["originalIntent"],
        "criteriaCoverage.originalIntent",
      ),
      desiredOutcome: textField(
        coverage["desiredOutcome"],
        "criteriaCoverage.desiredOutcome",
      ),
...
function parseSurfaceEvidence(
  ...
  if (!Array.isArray(value) || value.length === 0) {
    invalid(
      "manualQa.surfaceEvidence must not be empty.",
      "manualQa.surfaceEvidence",
    );
  }
  return value.map((item, index) => {
    ...
    const surface = surfaceField(
      row["surface"],
      `manualQa.surfaceEvidence[${index}].surface`,
    );
    ...
    return {
      ...
      criterionRef: textField(
        row["criterionRef"],
        `manualQa.surfaceEvidence[${index}].criterionRef`,
      ),
      surface,
      invocation: textField(
        row["invocation"],
        `manualQa.surfaceEvidence[${index}].invocation`,
      ),
      verdict: passedVerdict(
        row["verdict"],
        `manualQa.surfaceEvidence[${index}].verdict`,
      ),
...
function parseAdversarialCases(
  ...
  if (!Array.isArray(value) || value.length === 0) {
    invalid(
      "manualQa.adversarialCases must not be empty.",
      "manualQa.adversarialCases",
    );
  }
  return value.map((item, index) => {
    ...
    const verdictInfo = adversarialVerdict(
      row,
      `manualQa.adversarialCases[${index}]`,
    );
    return {
      id: textField(row["id"], `manualQa.adversarialCases[${index}].id`),
      criterionRef: textField(
        row["criterionRef"],
        `manualQa.adversarialCases[${index}].criterionRef`,
      ),
      scenario: textField(
        row["scenario"],
        `manualQa.adversarialCases[${index}].scenario`,
      ),
