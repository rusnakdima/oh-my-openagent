...
function validateBatches(
  batches: readonly UlwLoopValidationBatch[],
  goals: readonly UlwLoopItem[],
): void {
  ...
  for (const batch of batches) {
    ...
    if (!batch.memberIds.includes(batch.finalGoalId)) {
      fail(
        `validation batch ${batch.batchId} finalGoalId must be a member.`,
        "ULW_LOOP_VALIDATION_BATCH_FINAL_NOT_MEMBER",
      );
    }
    for (const memberId of batch.memberIds) {
      if (!goalIds.has(memberId)) {
        fail(
          `validation batch ${batch.batchId} references unknown goal: ${memberId}.`,
          "ULW_LOOP_VALIDATION_BATCH_MEMBER_UNKNOWN",
        );
      }
      if (members.has(memberId)) {
        fail(
          `goal appears in multiple validation batches: ${memberId}.`,
...
export function updateBatchesAfterSupersede(
  plan: UlwLoopPlan,
  targetId: string,
  ...
  plan.validationBatches = plan.validationBatches.map((batch) => {
    ...
    const memberIds = batch.memberIds.flatMap((
      id,
    ) => (id === targetId ? [...replacementIds] : [id]));
    const replacementFinalGoalId = replacementIds[replacementIds.length - 1] ??
      batch.finalGoalId;
    const finalGoalId = batch.finalGoalId === targetId
      ? replacementFinalGoalId
      : batch.finalGoalId;
    return { batchId: batch.batchId, memberIds, finalGoalId };
  });
}

export function batchUpdateLedgerEntry(
  before: UlwLoopPlan,
  after: UlwLoopPlan,
  at: string,
): UlwLoopLedgerEntry | null {
  if (
    JSON.stringify(before.validationBatches ?? []) ===
      JSON.stringify(after.validationBatches ?? [])
  ) return null;
  return {
    at,
    kind: "batch_updated",
    before: before.validationBatches ?? [],
    after: after.validationBatches ?? [],
    message: "Validation batch membership updated after steering.",
  };
}
...
export function batchOf(
  plan: UlwLoopPlan,
  goalId: string,
): UlwLoopValidationBatch | undefined {
  return plan.validationBatches?.find((batch) =>
    batch.memberIds.includes(goalId)
  );
}

export function batchClosedBy(
  plan: UlwLoopPlan,
  goalId: string,
): UlwLoopValidationBatch | undefined {
...
export function requireBatchFinalReady(
  plan: UlwLoopPlan,
  goal: UlwLoopItem,
): void {
  const batch = batchClosedBy(plan, goal.id);
  if (batch === undefined) return;
  const open = batch.memberIds.filter((id) =>
    id !== goal.id && !memberResolved(plan, id)
  );
  if (open.length > 0) {
    throw new UlwLoopError(
      "Validation batch has unresolved members.",
      "ULW_LOOP_VALIDATION_BATCH_OPEN",
      { details: { batchId: batch.batchId, open } },
    );
  }
}

export function requireAllValidationBatchesClosed(
  plan: UlwLoopPlan,
  closingGoalId?: string,
): void {
  const open = (plan.validationBatches ?? []).filter((batch) =>
    batch.memberIds.some((id) =>
      id !== closingGoalId && !memberResolved(plan, id)
    )
  );
  if (open.length > 0) {
    throw new UlwLoopError(
      "Validation batches remain open.",
      "ULW_LOOP_VALIDATION_BATCH_OPEN",
      { details: { batchIds: open.map((batch) => batch.batchId) } },
    );
  }
}

export function requireBatchGate(
  plan: UlwLoopPlan,
  goal: UlwLoopItem,
  gate: UlwLoopQualityGate,
): void {
  const batch = batchClosedBy(plan, goal.id);
  if (batch === undefined) return;
  const members = batch.memberIds.map((id) =>
    plan.goals.find((item) => item.id === id)
  ).filter((item): item is UlwLoopItem => item !== undefined);
  const pending = members.flatMap((member) =>
    member.successCriteria.filter((criterion) => criterion.status !== "pass")
      .map((criterion) => `${member.id}:${criterion.id}`)
  );
  if (pending.length > 0) {
    throw new UlwLoopError(
      "Validation batch criteria remain pending.",
      "ULW_LOOP_VALIDATION_BATCH_CRITERIA_PENDING",
      { details: { batchId: batch.batchId, pending } },
    );
  }
  const totalCriteria = members.reduce(
    (sum, member) => sum + member.successCriteria.length,
    0,
  );
  const passCount = members.reduce(
    (sum, member) =>
      sum +
      member.successCriteria.filter((criterion) => criterion.status === "pass")
        .length,
  ...
  if (
    gate.criteriaCoverage.totalCriteria !== totalCriteria ||
    gate.criteriaCoverage.passCount !== passCount
  ) {
    throw new UlwLoopError(
      "Validation batch gate coverage does not match member criteria.",
      "ULW_LOOP_VALIDATION_BATCH_GATE_MISMATCH",
      {
        details: {
          batchId: batch.batchId,
          expected: { totalCriteria, passCount },
          actual: gate.criteriaCoverage,
        },
      },
    );
  }
...
function fail(
  message: string,
  code = "ULW_LOOP_VALIDATION_BATCH_INVALID",
): never {
