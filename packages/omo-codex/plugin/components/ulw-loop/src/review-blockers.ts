...
const BLOCKER_FIELDS =
  "blockedReason blockerSignature blockerOccurrenceCount requiredExternalDecision nonRetriable failedAt failureReason completedAt blocker blockerEvidence blockerOccurrences blockedAt"
    .split(" ");
...
export async function recordFinalReviewBlockers(
  ...
  return withUlwLoopMutationLock(repoRoot, scope, async () => {
    ...
    if (goal === undefined) {
      ulwLoopError(
        `Unknown ulw-loop id: ${args.goalId}`,
        "ulw_loop_goal_not_found",
      );
    }
    if (goal.status !== "in_progress") {
      ulwLoopError(
        `${goal.id} is ${goal.status}.`,
    ...
    const blockedEntry: UlwLoopLedgerEntry = {
      at: now,
      kind: "goal_review_blocked",
      goalId: goal.id,
      status: goal.status,
      evidence: args.evidence,
      codexGoal,
    };
    const addedEntry: UlwLoopLedgerEntry = {
      at: now,
      kind: "goal_added",
      goalId: newGoal.id,
    ...
    const summaryEntry: UlwLoopLedgerEntry = {
      at: now,
      kind: "goal_review_blocked",
      goalId: goal.id,
      status: goal.status,
      evidence: args.evidence,
      codexGoal,
      message: `Review blockers recorded; appended ${newGoal.id}.`,
    };
