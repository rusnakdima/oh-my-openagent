...
export function seedDefaultSuccessCriteria(
  ...
  const subject = truncateObjective(
    normalizeObjective(objective) || `Goal ${goalIndex + 1}`,
  );
  const rows = [
    [
      "C001",
      ...
      `Replace via revise_criterion with observable happy-path proof for goal ${
        goalIndex + 1
      }.`,
      true,
    ],
...
export function deriveGoalCandidates(
  ...
  const bulletGoals = brief
    ...
    .map((line) => ({
      original: line,
      cleaned: normalizeObjective(cleanLine(line)),
    }))
  ...
  const selected =
    (bulletGoals.length > 0 ? bulletGoals : paragraphs).length > 0
      ? bulletGoals.length > 0 ? bulletGoals : paragraphs
