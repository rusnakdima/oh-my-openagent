...
const targets = (proposal: UlwLoopSteeringProposal): string[] =>
  proposal.targetGoalIds ??
    [proposal.targetGoalId ?? text(proposal, "goalId") ?? ""].filter(Boolean);
const child = (value: unknown): UlwLoopSteeringChildGoal | null => {
  ...
  return title === undefined || objective === undefined
    ? null
    : { title, objective };
};
const children = (
  ...
  return Array.isArray(fromAfter)
    ? fromAfter.map(child).filter((item): item is UlwLoopSteeringChildGoal =>
      item !== null
    )
    : [];
};
const goal = (
  ...
  id === undefined ? undefined : plan.goals.find((item) => item.id === id);
...
export function makeGoal(
  ...
  const goalIndex = digits === undefined
    ? plan.goals.length + offset - 1
    : Number(digits) - 1;
  return {
    id,
...
export function reviseWording(
  ...
  target.objective = revised(proposal, "revisedObjective", "objective") ??
    target.objective;
...
export function splitOrBlock(
  ...
  if (replacements.length === 0) {
    ...
    updateBatchesAfterSupersede(
      plan,
      target.id,
      replacements.map((item) => item.id),
    );
  }
  if (plan.activeGoalId === target.id) delete plan.activeGoalId;
}

export function reviseCriterion(
  plan: UlwLoopPlan,
  proposal: UlwLoopSteeringProposal,
  now: string,
): void {
  const target = goal(plan, targets(proposal)[0]);
  const index =
    target?.successCriteria.findIndex((item) =>
      item.id === proposal.criterionId
    ) ?? -1;
  ...
  target.successCriteria[index] = {
    ...
    expectedEvidence: text(proposal, "expectedEvidence") ??
