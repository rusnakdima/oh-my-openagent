...
export async function runStopResumeHookCli(
  ...
  try {
    ...
    const output = runStopResumeHook(
...
function consumeResumeBudget(stateDir: string, goalId: string): boolean {
  ...
  if (
    !isInsideDir(stateDir, counterPath) || !isInsideDir(stateDir, stuckPath)
  ) return false;
  const previous = readCounter(counterPath);
  const count =
    previous !== null && previous.ledgerLineCount === ledgerLineCount
      ? previous.count
      : 0;
  if (count >= RESUME_CAP) {
  ...
  writeFileSync(
    counterPath,
    JSON.stringify({ count: count + 1, ledgerLineCount }),
  );
  return true;
}
...
function renderResumeDirective(
  plan: UlwLoopPlan,
  goal: UlwLoopItem,
  sessionId: string,
): string {
  ...
  const option =
    normalized !== null && plan.goalsPath.includes(`/${normalized}/`)
...
function readCounter(
  counterPath: string,
): { count: number; ledgerLineCount: number } | null {
  try {
    if (!existsSync(counterPath)) return null;
    const parsed = JSON.parse(readFileSync(counterPath, "utf8")) as Record<
      string,
      unknown
    >;
    if (
      typeof parsed["count"] !== "number" ||
      typeof parsed["ledgerLineCount"] !== "number"
    ) return null;
    return {
      count: parsed["count"],
      ledgerLineCount: parsed["ledgerLineCount"],
    };
  } catch (error) {
    if (error instanceof Error) return null;
    throw error;
  ...
  try {
    ...
    return entries.some((work) => {
      ...
      const sessionIds = Array.isArray(entry["session_ids"])
        ? entry["session_ids"]
        : [];
      const continuable = entry["status"] === "active" ||
        entry["status"] === "paused";
      return continuable && sessionIds.includes(`codex:${sessionId}`) &&
        boulderPlanHasChecklist(cwd, entry);
    });
  } catch (error) {
...
function boulderPlanHasChecklist(
  cwd: string,
  ...
  const candidates =
    typeof worktree === "string" && worktree.trim().length > 0 &&
      !isAbsolute(activePlan)
      ? [
        join(isAbsolute(worktree) ? worktree : join(cwd, worktree), activePlan),
        planPath,
      ]
      : [planPath];
