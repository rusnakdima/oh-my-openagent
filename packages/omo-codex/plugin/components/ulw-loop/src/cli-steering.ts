...
const SOURCES = [
  ...
  "finding",
  "cli",
] as const satisfies readonly UlwLoopSteeringSource[];
...
): value is UlwLoopSteeringMutationKind {
  return value !== undefined &&
    ULW_LOOP_STEERING_MUTATION_KINDS.some((kind) => kind === value);
}
function isSource(value: string | undefined): value is UlwLoopSteeringSource {
  return value !== undefined && SOURCES.some((source) => source === value);
}
function isModel(value: string): value is UlwLoopSuccessCriterionUserModel {
  return ULW_LOOP_SUCCESS_CRITERION_USER_MODELS.some((model) =>
    model === value
  );
}
function fail(
...
function kindMessage(prefix: string): string {
  return `${prefix}\n\n${STEERING_KIND_HELP}`;
}
...
function required(argv: readonly string[], flag: string): string {
  ...
  return value ??
    fail(`Missing ${flag}.`, "ULW_LOOP_STEERING_FIELD_REQUIRED", { flag });
}
function requiredGoal(argv: readonly string[]): string {
  const value = text(parseGoalArg(argv), "--goal-id");
  return value ??
    fail("Missing --goal-id.", "ULW_LOOP_GOAL_ID_REQUIRED", {
      flag: "--goal-id",
    });
}
function readObject(value: object, key: string): unknown {
  return Object.entries(value).find(([name]) => name === key)?.[1];
}
function isPlain(value: unknown): value is object {
  return typeof value === "object" && value !== null && !Array.isArray(value);
}
function objectText(value: object, key: string): string | undefined {
  const candidate = readObject(value, key);
  return typeof candidate === "string" ? candidate : undefined;
}
function objectStrings(value: object, key: string): string[] | undefined {
  const candidate = readObject(value, key);
...
function objectChildren(
  value: object,
  key: string,
): UlwLoopSteeringChildGoal[] | undefined {
  const candidate = readObject(value, key);
  ...
  for (const item of candidate) {
    ...
    if (next === null) {
      return fail(
        `${key} entries require title/objective.`,
        "ULW_LOOP_STEERING_CHILD_INVALID",
        { key },
      );
    }
    parsed.push(next);
...
function isProposalKind(value: unknown): value is UlwLoopSteeringMutationKind {
  return typeof value === "string" &&
    ULW_LOOP_STEERING_MUTATION_KINDS.some((kind) => kind === value);
}
function isProposalSource(value: unknown): value is UlwLoopSteeringSource {
  return typeof value === "string" &&
    SOURCES.some((source) => source === value);
}

export function parseSteeringKind(
  argv: readonly string[],
): UlwLoopSteeringMutationKind {
  const value = readValue(argv, "--kind");
  if (isKind(value)) return value;
  return value === undefined
    ? fail(kindMessage("Missing --kind."), "ULW_LOOP_STEERING_KIND_REQUIRED", {
      flag: "--kind",
      expected: ULW_LOOP_STEERING_MUTATION_KINDS,
      usage: STEERING_KIND_HELP,
    })
    : fail(
      kindMessage(`Invalid --kind: ${value}.`),
      "ULW_LOOP_STEERING_KIND_INVALID",
      {
        value,
        expected: ULW_LOOP_STEERING_MUTATION_KINDS,
        usage: STEERING_KIND_HELP,
...
export function parseSteeringSource(
  argv: readonly string[],
): UlwLoopSteeringSource {
  ...
  return isSource(value)
    ? value
    : fail(`Invalid --source: ${value}.`, "ULW_LOOP_STEERING_SOURCE_INVALID", {
      value,
      expected: SOURCES,
    });
}
...
async function children(
  ...
  for (const item of raw) {
    ...
    if (next === null) {
      return fail(
        `${flag} entries require title/objective.`,
        "ULW_LOOP_STEERING_CHILD_INVALID",
        { flag },
      );
    }
...
async function stringArray(
  ...
  for (const item of raw) {
    if (typeof item !== "string") {
      return fail(
        `${flag} entries must be strings.`,
        "ULW_LOOP_STEERING_STRING_ARRAY_REQUIRED",
        { flag },
      );
    }
...
function model(
  value: string | undefined,
): UlwLoopSuccessCriterionUserModel | undefined {
  const trimmed = text(value, "--user-model");
  if (trimmed === undefined) return undefined;
  return isModel(trimmed) ? trimmed : fail(
    `Invalid --user-model: ${trimmed}.`,
    "ULW_LOOP_STEERING_USER_MODEL_INVALID",
    { value: trimmed, expected: ULW_LOOP_SUCCESS_CRITERION_USER_MODELS },
  );
}
function neverKind(kind: never): never {
  return fail(
    `Unsupported steering kind: ${String(kind)}.`,
    "ULW_LOOP_STEERING_KIND_UNSUPPORTED",
    { kind },
  );
}
...
export async function parseSteeringProposal(
  ...
  const idempotencyKey = text(
    readValue(argv, "--idempotency-key"),
    "--idempotency-key",
  );
  const base = {
    kind,
  ...
  switch (kind) {
    ...
    case "split_subgoal": {
      ...
      return normalizeSteeringProposal({
        ...
        goalId,
        targetGoalId: goalId,
        childGoals: await children(argv, "--children", true),
      });
    }
    case "reorder_pending":
      return normalizeSteeringProposal({
        ...base,
        pendingOrder: await stringArray(argv, "--order"),
      });
    case "revise_pending_wording": {
      ...
      if (revisedTitle === undefined && revisedObjective === undefined) {
        return fail(
          "revise_pending_wording requires --title or --objective.",
          "ULW_LOOP_STEERING_UPDATE_REQUIRED",
          { kind },
        );
      }
      return normalizeSteeringProposal({
        ...
        goalId,
        targetGoalId: goalId,
        ...(revisedTitle === undefined ? {} : { revisedTitle }),
        ...(revisedObjective === undefined ? {} : { revisedObjective }),
      });
    }
    case "revise_criterion": {
      ...
      if (
        scenario === undefined && expectedEvidence === undefined &&
        userModel === undefined
      ) {
        return fail(
          "revise_criterion requires scenario, expected-evidence, or user-model.",
          "ULW_LOOP_STEERING_UPDATE_REQUIRED",
          { kind },
        );
      }
    ...
    case "mark_blocked_superseded": {
      ...
      return normalizeSteeringProposal({
        ...
        goalId,
        targetGoalId: goalId,
        ...(childGoals.length === 0 ? {} : { childGoals }),
      });
    }
    default:
...
function normalizedChildren(
  values: readonly UlwLoopSteeringChildGoal[] | undefined,
): UlwLoopSteeringChildGoal[] | undefined {
  if (values === undefined) return undefined;
  return values.map((item) => ({
    title: text(item.title, "child.title") ?? "",
    objective: text(item.objective, "child.objective") ?? "",
  }));
}
...
export function normalizeSteeringProposal(
  proposal: CliSteeringProposal,
): CliSteeringProposal {
  ...
  const targetGoalIds = normalizedStrings(
    proposal.targetGoalIds,
    "targetGoalIds",
  );
  ...
  return {
    ...
    ...(targetGoalId === undefined ? {} : { targetGoalId }),
    ...(targetGoalIds === undefined ? {} : { targetGoalIds }),
    ...(criterionId === undefined ? {} : { criterionId }),
    ...(title === undefined ? {} : { title }),
    ...(objective === undefined ? {} : { objective }),
    ...(childGoals === undefined ? {} : { childGoals }),
    ...(revisedTitle === undefined ? {} : { revisedTitle }),
    ...(revisedObjective === undefined ? {} : { revisedObjective }),
    ...
    ...(promptSignature === undefined ? {} : { promptSignature }),
    ...(idempotencyKey === undefined ? {} : { idempotencyKey }),
    ...(proposal.now === undefined ? {} : { now: proposal.now }),
    ...(scenario === undefined ? {} : { scenario }),
    ...(expectedEvidence === undefined ? {} : { expectedEvidence }),
    ...(proposal.userModel === undefined
      ? {}
      : { userModel: proposal.userModel }),
  };
}
...
export async function parseSteeringProposals(
  ...
  if (readValue(argv, "--kind") !== undefined) {
    return fail(
      "--kind and --proposals-json are mutually exclusive.",
      "ULW_LOOP_STEERING_BATCH_CONFLICT",
      { flags: ["--kind", "--proposals-json"] },
    );
  }
  const raw = await readJsonInput(input);
  if (!Array.isArray(raw) || raw.length === 0) {
    return fail(
      "--proposals-json must be a non-empty JSON array.",
      "ULW_LOOP_STEERING_BATCH_ARRAY_REQUIRED",
      { flag: "--proposals-json" },
    );
  }
...
function proposalFromObject(value: unknown): CliSteeringProposal {
  if (!isPlain(value)) {
    return fail(
      "--proposals-json entries must be objects.",
      "ULW_LOOP_STEERING_BATCH_ITEM_INVALID",
      { flag: "--proposals-json" },
    );
  }
  ...
  if (!isProposalKind(kind)) {
    return fail(
      `Invalid batch steering kind: ${String(kind)}.`,
      "ULW_LOOP_STEERING_KIND_INVALID",
      { value: kind },
    );
  }
  if (!isProposalSource(source)) {
    return fail(
      `Invalid batch steering source: ${String(source)}.`,
      "ULW_LOOP_STEERING_SOURCE_INVALID",
      { value: source },
    );
  }
  let proposal: CliSteeringProposal = {
    kind,
    source,
    evidence: objectText(value, "evidence") ?? "",
    rationale: objectText(value, "rationale") ?? "",
  };
...
export function printSteerResult(
  result: SteerUlwLoopResult,
  json: boolean,
): void {
  if (json) {
    printJson({
...
export function printSteerBatchResult(
  result: SteerUlwLoopBatchResult,
  json: boolean,
): void {
  if (json) {
    printJson({
  ...
  process.stdout.write(
    `ulw-loop steer batch: ${
      result.accepted ? "accepted" : "rejected"
    } ${result.results.length} proposal(s)\n`,
  );
  if (result.rejectedReasons.length > 0) {
    process.stdout.write(`rejected: ${result.rejectedReasons.join("; ")}\n`);
  }
