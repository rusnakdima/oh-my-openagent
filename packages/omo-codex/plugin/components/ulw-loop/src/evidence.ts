  ...
  switch (status) {
    ...
    default:
      return ulwLoopFail(
        ...
        "ULW_LOOP_CRITERION_STATUS_INVALID",
        { status },
      );
  }
}
...
function findCriterion(
  ...
  const criterion = goal.successCriteria.find((candidate) =>
    candidate.id === criterionId
  );
  return (
    criterion ??
      ulwLoopFail(
        `Success criterion not found: ${criterionId}.`,
        "ULW_LOOP_CRITERION_NOT_FOUND",
        {
          goalId: goal.id,
          criterionId,
        },
...
export function unresolvedCriteriaOf(
  goal: UlwLoopItem,
): UlwLoopSuccessCriterion[] {
  return goal.successCriteria.filter((criterion) =>
...
export function unresolvedEssentialCriteriaOf(
  ...
  const essentialCriteria = new Set(
    essentialCriteriaOf(goal).map((criterion) => criterion.id),
  );
  return goal.successCriteria.filter(
    (criterion) =>
      essentialCriteria.has(criterion.id) && criterion.status !== "pass",
  );
}
