...
export function printJsonError(error: unknown): void {
  ...
  if (error instanceof Error) {
    printJson({
      ok: false,
      error: { code: "ULW_LOOP_UNEXPECTED", message: error.message },
    });
    return;
  }
  printJson({
    ok: false,
    error: { code: "ULW_LOOP_UNKNOWN", message: "unknown error" },
  });
}
...
function criteriaCounts(goal: UlwLoopItem): CriteriaCounts {
  ...
  for (const criterion of goal.successCriteria) {
    if (criterion.status === "pass") pass += 1;
  }
...
export function blockedDecisionHandoff(plan: UlwLoopPlan): string {
  const blocked = plan.goals.find((goal) =>
    goal.status === "needs_user_decision" && goal.nonRetriable
  );
  ...
  return [
    ...
    `Required external decision: ${
      blocked.requiredExternalDecision ??
        "provide the missing authorization or choose a different unblock path"
    }.`,
    "Do not run complete-goals --retry-failed again until external state changes or the user authorizes an unblock path.",
  ].join("\n");
}

export function normalizeCodexGoalMode(
