...
export function applySpawnGuards(payload: PreToolUsePayload): string {
  if (
    ...
    !SPAWN_TOOL_TOKENS.has(payload.tool_name)
  ) return "";
  ...
  if (missingArtifact !== null) {
    return deny(
      `spawn code-review + QA first; gate audits their artifacts: missing ${missingArtifact}`,
    );
  }
  return "";
}

export async function runSpawnGuardCli(
...
function missingGateArtifact(
  payload: PreToolUsePayload,
  plan: UlwLoopPlan,
): string | null {
  if (!isGateReviewerSpawn(payload.tool_input)) return null;
  const goal = plan.goals.find((candidate) =>
    isFinalRunCompletionCandidate(plan, candidate)
  );
  ...
  if (plan.evidenceLayoutVersion === 2) {
    ...
    for (
      const name of [`${goal.id}-code-review.md`, `${goal.id}-manual-qa.md`]
    ) {
  ...
  if (
    !hasOtherEvidenceFile(
      join(payload.cwd, ".omo", "evidence"),
      `${goal.id}-code-review.md`,
    )
  ) {
...
function hasOtherEvidenceFile(
  evidenceDir: string,
  excludedName: string,
): boolean {
  try {
    return readdirSync(evidenceDir).some((name) =>
      name !== excludedName && isNonEmptyFile(join(evidenceDir, name))
    );
  } catch (error) {
    if (error instanceof Error) return false;
    throw error;
