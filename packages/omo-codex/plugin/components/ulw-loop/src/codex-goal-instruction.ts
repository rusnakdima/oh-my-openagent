...
export function buildCodexGoalInstruction(args: {
  ...
  const isFinal = args.isFinal ??
    isFinalRunCompletionCandidate(args.plan, args.goal);
  return {
    text: buildText(mode, args.plan, args.goal, createGoal, isFinal),
    json: createGoal,
  };
}

function buildCreateGoalPayload(
  plan: UlwLoopPlan,
  goal: UlwLoopItem,
): CodexCreateGoalPayload {
...
function buildText(
  ...
  return joinLines([
    mode === "aggregate"
      ? "UlwLoop aggregate-goal handoff"
      : "UlwLoop active-goal handoff",
    `Mode: ${mode}`,
    `Plan: ${plan.goalsPath}`,
...
function checkpointLines(
  ...
  const failureLine =
    `- If blocked or failed, checkpoint with --status failed and the failure evidence; rerun complete-goals${
      sessionOption(plan)
...
function successCriteriaLines(
  criteria: readonly UlwLoopSuccessCriterion[],
): readonly string[] {
  if (criteria.length === 0) {
    return [
      "Success criteria:",
      "- No success criteria recorded for this goal.",
    ];
  }
...
function formatCriterionLine(criterion: UlwLoopSuccessCriterion): string {
  const remainingWork = criterion.status === "pending"
    ? " remaining work:"
    : "";
  const marker = isEssentialCriterion(criterion)
    ? "essential"
    : "non-essential";
...
function finalSection(
  ...
  const blockerCommand =
    `omo-agent-toolkit ulw-loop record-review-blockers${option} --goal-id ${goal.id} --title "Resolve final code-review blockers" --objective "<blocker-resolution objective>" --evidence "<review findings>" --codex-goal-json "<active get_goal JSON or path>"`;
  const checkpointCommand =
    `omo-agent-toolkit ulw-loop checkpoint${option} --goal-id ${goal.id} --status complete --evidence "<targeted verification/manualQa/gateReview evidence>" --codex-goal-json "<fresh complete get_goal JSON or path>" --quality-gate-json "<quality gate JSON or path>"`;
  return joinLines([
