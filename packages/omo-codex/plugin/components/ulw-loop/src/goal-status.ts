...
export function aggregateCodexObjectiveForScope(scope?: UlwLoopScope): string {
  return `Complete the durable ulw-loop plan in ${
    ulwLoopGoalsRelativePath(scope)
  }, including later accepted/appended stories, under the original brief constraints; use ${
    ulwLoopLedgerRelativePath(scope)
  } as the audit trail.`;
}
...
export function isMemberResolved(
  goal: UlwLoopItem,
  plan: UlwLoopPlan,
): boolean {
...
export function isFinalRunCompletionCandidate(
  ...
  return (
    ...
    plan.goals.every((candidate) =>
      !isCompletionBlockingForFinalCandidate(candidate, goal, plan)
    )
  );
}
...
export function expectedCodexObjective(
  plan: UlwLoopPlan,
  goal: UlwLoopItem,
): string {
  return codexGoalMode(plan) === "aggregate"
    ? aggregateCodexObjective(plan)
    : goal.objective;
}

export function compatibleCodexObjectives(
  plan: UlwLoopPlan,
): readonly string[] {
...
export function firstUnresolvedCriterion(
  goal: UlwLoopItem,
): UlwLoopSuccessCriterion | undefined {
