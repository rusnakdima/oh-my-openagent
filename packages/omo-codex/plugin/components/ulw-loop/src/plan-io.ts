...
const LEGACY_OBJECTIVE_PREFIX =
  `Complete all ulw-loop stories in ${ULW_LOOP_DIR}/${ULW_LOOP_GOALS}: `;
const LEGACY_OBJECTIVE =
  `Complete all ulw-loop stories listed in ${ULW_LOOP_DIR}/${ULW_LOOP_GOALS}. Use ${ULW_LOOP_DIR}/${ULW_LOOP_LEDGER} as the durable audit trail.`;
...
function isLegacyEnumeratedAggregateObjective(
  ...
  return objective === LEGACY_OBJECTIVE ||
    Boolean(objective?.startsWith(LEGACY_OBJECTIVE_PREFIX));
}
...
export async function withUlwLoopMutationLock<T>(
  ...
  if (fn === undefined) {
    throw new UlwLoopError(
      "Missing ulw-loop mutation body.",
      "ULW_LOOP_LOCK_BODY_MISSING",
    );
  }
...
export async function readUlwLoopPlan(
  ...
  try {
    ...
    throw new UlwLoopError(
      `No ulw-loop plan found at ${
        repoRelative(path, repoRoot)
      }. Run \`omo-agent-toolkit ulw-loop create-goals ...\` first.`,
  ...
  if (
    ...
    parsed.codexObjectiveAliases = [
      ...new Set([...(parsed.codexObjectiveAliases ?? []), previousObjective]),
    ];
...
async function* ledgerLines(
  ...
  const stream = createReadStream(ulwLoopLedgerPath(repoRoot, scope), {
    encoding: "utf8",
  });
