...
import {
  batchClosedBy,
  requireAllValidationBatchesClosed,
  requireBatchFinalReady,
  requireBatchGate,
...
function findGoal(plan: UlwLoopPlan, goalId: string): UlwLoopItem {
  ...
  return goal ??
    ulwLoopFail(`Unknown ulw-loop id: ${goalId}.`, "ulw_loop_goal_not_found");
}
...
async function readJsonInput(
  ...
  try {
    ...
    return ulwLoopFail(
      `Quality gate path does not contain valid JSON${
...
function applyBlockedOrFailed(
  ...
  const occurrences = signature === null
    ? 0
    : sameBlockerOccurrences(plan, signature) + 1;
  ...
  if (signature !== null) {
    ...
    goal.requiredExternalDecision =
      `Resolve external authorization: ${signature}`;
  }
  ...
  return withUlwLoopMutationLock(repoRoot, scope, async () => {
    ...
    if (args.status === "complete") {
      ...
      if (!reconciliation.ok) {
        ...
        const mismatchedTaskObjective = snapshot?.available === true &&
          ...
          normalizeObjective(objective) !==
            normalizeObjective(expectedCodexObjective(plan, goal));
        ...
        if (!taskScoped) {
          throw new UlwLoopError(
            `${formatCodexGoalReconciliation(reconciliation)}${
              aggregate && snapshot?.status === "complete" &&
                objective !== undefined
      ...
      if (closesBatch && args.qualityGateJson === undefined) {
        throw new UlwLoopError(
          "Validation batch final checkpoint requires --quality-gate-json.",
      ...
      if (final || aggregateCompletion !== undefined || closesBatch) {
        qualityGate = validateQualityGate(
          await readJsonInput(args.qualityGateJson, repoRoot),
          {
            repoRoot,
            fs: QUALITY_GATE_FS,
            ...(plan.evidenceLayoutVersion === 2
              ? {
                currentAttemptDir: ulwLoopAttemptEvidenceDir(
                  goal.id,
                  goal.attempt,
                  scope,
                ),
    ...
    const closedBatch = args.status === "complete"
      ? batchClosedBy(plan, goal.id)
      : undefined;
