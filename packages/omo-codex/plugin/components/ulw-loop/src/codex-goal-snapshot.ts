  ...
  try {
    ...
    if (!existsSync(path)) {
      throw new CodexGoalSnapshotError(
        `Codex goal snapshot is neither valid JSON nor a readable path: ${trimmed}`,
      );
    }
    try {
      ...
      throw new CodexGoalSnapshotError(
        `Codex goal snapshot path does not contain valid JSON: ${trimmed}${
...
export function reconcileCodexGoalSnapshot(
  ...
  const accepted = new Set(
    [
      expected,
      ...(options.acceptedObjectives ?? []).map((objective) =>
  ...
  if (!actual) {
    ...
    errors.push(
      `Codex goal objective mismatch: expected "${expected}", got "${actual}".`,
    );
  }
  ...
  if (!allowed.includes(actualStatus)) {
    errors.push(
      `Codex goal status mismatch: expected ${
        allowed.join(" or ")
      }, got ${actualStatus}.`,
    );
  }
...
export function formatCodexGoalReconciliation(
