import {
  checkpointUlwLoop,
  type CheckpointUlwLoopArgs,
  type CheckpointUlwLoopResult,
} from "./checkpoint.js";
...
import {
  ...
  type UlwLoopGoalInstruction,
} from "./codex-goal-instruction.js";
...
export async function checkpointAndContinue(
  ...
  if (
    args.status !== "complete" || result.aggregateCompletion !== undefined ||
    !args.advance
  ) return result;
  ...
  return {
    ...result,
    plan: next.plan,
    next: { resumed: next.resumed, goal: next.goal, instruction },
  };
}
...
export async function checkpoint(
  ...
  const codexGoalJson = await parseCodexGoalJson(
    statusValue === "complete"
      ...
      : readValue(argv, "--codex-goal-json"),
  );
