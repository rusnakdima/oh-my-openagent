...
import {
  ...
  reviseWording,
  splitOrBlock,
} from "./steering-mutations.js";
import {
  buildSteeringPlanSnapshot,
  changedGoalIdsBetween,
} from "./steering-snapshot.js";
import type {
  SteerUlwLoopResult,
  UlwLoopItem,
  UlwLoopLedgerEntry,
...
const isKind = (value: unknown): value is UlwLoopSteeringMutationKind =>
  typeof value === "string" &&
  ULW_LOOP_STEERING_MUTATION_KINDS.some((kind) => kind === value);
const isSource = (value: unknown): value is UlwLoopSteeringSource =>
  typeof value === "string" && SOURCES.some((source) => source === value);
const isModel = (value: unknown): value is UlwLoopSuccessCriterionUserModel =>
  typeof value === "string" &&
  ULW_LOOP_SUCCESS_CRITERION_USER_MODELS.some((model) => model === value);
...
const pendingOrder = (proposal: object): string[] => {
  ...
  return direct.length > 0
    ? direct
    : texts(after(proposal) ?? proposal, "pendingGoalIds");
};
...
function hasProtected(value: unknown): boolean {
  ...
  for (const [key, childValue] of Object.entries(value)) {
    if (
      PROTECTED.has(key) || key.toLowerCase().includes("complete") ||
      hasProtected(childValue)
    ) return true;
  }
  return false;
}
...
function allText(value: unknown): string {
  ...
  return isObject(value)
    ? Object.values(value).map(allText).filter(Boolean).join("\n")
    : "";
}

function weakens(value: unknown): boolean {
  const valueText = allText(value).toLowerCase();
  return /\b(skip|bypass|weaken|remove|omit|auto[-\s]?complete|mark complete|complete faster)\b/
    .test(valueText) &&
    /\b(test|tests|verification|review|quality gate|complete|completion)\b/
      .test(valueText);
}
...
export function validateUlwLoopSteeringProposal(
  plan: UlwLoopPlan,
  proposal: unknown,
): UlwLoopSteeringAudit {
...
function validateKind(
  ...
  if (
    kind === "add_subgoal" &&
    (text(proposal, "title") === undefined ||
      text(proposal, "objective") === undefined)
  ) reasons.push("add_subgoal requires title/objective");
  if (
    (kind === "split_subgoal" || kind === "revise_pending_wording" ||
      kind === "mark_blocked_superseded") && target === undefined
  ) reasons.push(`${kind} requires target`);
  if (
    (kind === "split_subgoal" || kind === "revise_pending_wording") &&
    target !== undefined && target.status !== "pending"
...
function validateOrder(
  ...
  const pending = plan.goals.filter((item) =>
    item.status === "pending" && item.steeringStatus === undefined
  ).map((item) => item.id);
...
export function applySteeringMutation(
  ...
  if (proposal.kind === "reorder_pending") {
    ...
    next.goals = [
      ...order.map((id) => goal(next, id)).filter((item): item is UlwLoopItem =>
        item !== undefined
      ),
  ...
  if (
    proposal.kind === "split_subgoal" ||
    proposal.kind === "mark_blocked_superseded"
...
function isProposal(value: unknown): value is UlwLoopSteeringProposal {
  return isPlain(value) && isKind(read(value, "kind")) &&
    isSource(read(value, "source")) && isText(read(value, "evidence")) &&
    isText(read(value, "rationale"));
}

export function parseUlwLoopSteeringDirective(
  text: string,
): UlwLoopSteeringProposal | null {
  const match =
    /(?:^|\s)(?:OMO_ULW_LOOP_STEER|omo\.ulw-loop\.steer|omo ulw-loop steer|omo-agent-toolkit ulw-loop steer):\s*([\s\S]+)$/u
      .exec(text);
...
export async function steerUlwLoop(
  ...
  return withUlwLoopMutationLock(repoRoot, scope, async () => {
    ...
    const prior = key === undefined
      ? undefined
      : await findAcceptedSteeringLedgerEntry(repoRoot, key, scope);
    if (prior?.steering !== undefined) {
      ...
      const { before: _before, after: _after, ...compactPrior } =
        prior.steering;
      return {
        plan,
        accepted: true,
        audit: { ...compactPrior, deduped: true },
        rejectedReasons: [],
        deduped: true,
      };
    }
