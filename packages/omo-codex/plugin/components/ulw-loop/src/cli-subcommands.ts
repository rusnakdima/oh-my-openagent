...
import {
  blockedDecisionHandoff,
  normalizeCodexGoalMode,
  printJson,
  printStatus,
} from "./cli-output.js";
import {
  parseSteeringProposals,
  printSteerBatchResult,
  printSteerResult,
} from "./cli-steering.js";
  ...
  const plan = await createUlwLoopPlan(
    repoRoot,
    {
      ...
      codexGoalMode: normalizeCodexGoalMode(
        readValue(argv, "--codex-goal-mode"),
      ),
...
export async function completeGoals(
  ...
  const result = await startNextUlwLoop(repoRoot, {
    retryFailed: hasFlag(argv, "--retry-failed"),
  }, scope);
  if ("done" in result) {
    ...
    if (json) {
      printJson({
        ...
        `${handoff || "ulw-loop: all goals complete"}\n`,
      );}
    return 0;
  }
  const instruction = buildCodexGoalInstruction({
    plan: result.plan,
    goal: result.goal,
  });
  if (json) {
...
export async function steer(
  ...
  if (
    single !== undefined && proposals.length === 1 &&
    readValue(argv, "--proposals-json") === undefined
  ) {
...
export async function reviewBlockers(
  ...
  const codexGoalJson = await parseCodexGoalJson(
    required(argv, "--codex-goal-json"),
  );
  if (codexGoalJson === undefined) {
    throw new UlwLoopError(
      "Missing --codex-goal-json.",
      "ULW_LOOP_CODEX_GOAL_JSON_REQUIRED",
    );
  }
...
function formatCriterionForCli(
  criterion: UlwLoopItem["successCriteria"][number],
): string {
  const marker = isEssentialCriterion(criterion)
    ? "essential"
    : "non-essential";
  return `- ${criterion.id} [${criterion.status}] [${marker}] (${criterion.userModel}) ${criterion.scenario} evidence: ${
    criterion.capturedEvidence ?? "pending"
  }`;
}
...
function findGoal(
  plan: { readonly goals: readonly UlwLoopItem[] },
  goalId: string,
): UlwLoopItem {
  const goal = plan.goals.find((candidate) => candidate.id === goalId);
  if (goal !== undefined) return goal;
  throw new UlwLoopError(
    `Unknown ulw-loop id: ${goalId}.`,
