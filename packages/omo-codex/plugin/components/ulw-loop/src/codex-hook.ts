...
export function parseUserPromptSubmitPayload(
  raw: string,
): UserPromptSubmitPayload | null {
  ...
  try {
    ...
    if (proposal === null) {
      ...
      return options.ultraworkSkillFilePath === undefined
        ...
        : buildUltraworkDirectiveOutput(payload, {
          skillFilePath: options.ultraworkSkillFilePath,
        });
    }
...
function hasSteeringDirectiveMarker(prompt: string): boolean {
  return /(?:^|\s)(?:OMO_ULW_LOOP_STEER|omo\.ulw-loop\.steer|omo ulw-loop steer|omo-agent-toolkit ulw-loop steer):/u
    .test(prompt);
}
...
function isUserPromptSubmitPayload(
  value: unknown,
): value is UserPromptSubmitPayload {
  ...
  return (
    ...
    ["model", "permission_mode", "turn_id"].every((key) =>
...
function hasInvalidCreateGoalInput(value: unknown): boolean {
  return isRecord(value) &&
    Object.keys(value).some((key) => key !== "objective");
}
