...
export async function steerUlwLoopBatch(
  ...
  return withUlwLoopMutationLock(repoRoot, scope, async () => {
    ...
    for (const item of prepared.items) {
      if (item.kind === "fresh") next = item.prepared.next;
    }
    const fresh = prepared.items.filter((
      item,
    ): item is Extract<PreparedItem, { readonly kind: "fresh" }> =>
      item.kind === "fresh"
    );
    if (fresh.length > 0) {
      ...
      const entries = fresh.map((item) =>
        ledgerEntry(
          ...
          item.prepared.audit,
          item.prepared.proposal.now?.toISOString() ?? iso(),
        )
      );
...
> {
  ...
  for (const proposal of proposals) {
    ...
    const finalAudit = {
      ...audit,
      before: buildSteeringPlanSnapshot(current, changed),
      after: buildSteeringPlanSnapshot(next, changed),
    };
    const result = {
      accepted: true,
      deduped: false,
    ...
    items.push({
      kind: "fresh",
      prepared: { proposal, audit: finalAudit, before: current, next },
    });
    results.push(result);
    current = next;
  }
...
function rejectedLedgerEntry(
  results: readonly SteerUlwLoopBatchItemResult[],
): UlwLoopLedgerEntry {
  const rejectedItems = results.map((result, index) => ({ result, index }))
    .filter((item) => !item.result.accepted);
  return {
    at: iso(),
    kind: "steering_rejected",
    message: rejectedItems.map((item) =>
      `index ${item.index}: ${item.result.rejectedReasons.join(", ")}`
    ).join("; "),
  };
}
...
function ledgerEntry(
  ...
  const entry: UlwLoopLedgerEntry = {
    at,
    kind: proposal.kind === "revise_criterion"
      ? "criteria_revised"
      : "steering_accepted",
