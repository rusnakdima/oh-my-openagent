...
export async function createUlwLoopPlan(
  ...
  return withUlwLoopMutationLock(repoRoot, scope, async () => {
    ...
    const validationBatches = await parseValidationBatches(
      args.validationBatchesJson,
      goals,
    );
    ...
    await appendLedger(
      repoRoot,
      {
        at: now,
        kind: "plan_created",
        message: `${goals.length} goal(s) created`,
      },
      scope,
    );
    return plan;
...
function completedPlanExistsError(scope?: UlwLoopScope): UlwLoopError {
  return new UlwLoopError(
    [
      `Existing ulw-loop aggregate is already complete at ${
        ulwLoopGoalsRelativePath(scope)
      }.`,
...
export async function startNextUlwLoop(
  ...
  { plan: UlwLoopPlan; goal: UlwLoopItem; resumed: boolean } | {
    done: true;
    plan: UlwLoopPlan;
  }
> {
  return withUlwLoopMutationLock(repoRoot, scope, async () => {
    ...
    const existing = plan.goals.find((goal) =>
      goal.status === "in_progress" && isScheduleEligible(goal)
    );
    if (existing) return { plan, goal: existing, resumed: true };
    let next = plan.goals.find((goal) =>
      goal.status === "pending" && isScheduleEligible(goal)
    );
    if (!next && args.retryFailed) {
      next = plan.goals.find((goal) =>
        goal.status === "failed" && !goal.nonRetriable &&
        isScheduleEligible(goal)
      );
      if (next) {
        await appendLedger(
          repoRoot,
...
export function summarizeUlwLoopPlan(plan: UlwLoopPlan): UlwLoopPlanSummary {
  ...
  const countCriteria = (status: UlwLoopSuccessCriterion["status"]): number =>
    plan.goals.reduce(
      (sum, goal) =>
        sum +
        goal.successCriteria.filter((criterion) => criterion.status === status)
          .length,
      0,
    );
  return {
    ...
    superseded:
      plan.goals.filter((goal) => goal.steeringStatus === "superseded").length,
    criteria: {
      total: plan.goals.reduce(
        (sum, goal) => sum + goal.successCriteria.length,
        0,
      ),
