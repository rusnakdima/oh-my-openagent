...
const VALUE_FLAGS = new Set(
  "--brief --brief-file --session-id --codex-goal-mode --validation-batch-json --goal --goal-id --criterion-id --status --evidence --notes --codex-goal-json --quality-gate-json --kind --rationale --title --objective --target-goal-id --source --after-json --directive-json --directive-file --idempotency-key --proposals-json"
    .split(" "),
);
const SUBCOMMANDS = new Set(
  "create-goals status complete-goals criteria record-evidence checkpoint steer add-goal record-review-blockers"
    .split(" "),
);
...
export function parseGoalArg(argv: readonly string[]): string | undefined {
...
export function positionalText(argv: readonly string[]): string {
  const words: string[] = [];
  for (
    let index = SUBCOMMANDS.has(argv[0] ?? "") ? 1 : 0;
    index < argv.length;
    index += 1
  ) {
    const arg = argv[index];
...
export async function readJsonInput(
  ...
  try {
    return JSON.parse(
      looksLikeJson(value) ? value : await readFile(value, "utf8"),
    );
  } catch (error) {
    const message = error instanceof Error ? error.message : "unknown error";
    throw new UlwLoopError(
      `Invalid JSON input: ${message}`,
...
export async function parseCodexGoalJson(
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "unknown error";
    throw new UlwLoopError(
      `Invalid --codex-goal-json: ${message}`,
...
function evidenceStatus(value: string): RecordEvidenceCliArgs["status"] {
  switch (value) {
    ...
    default:
      throw new UlwLoopError(
        "Invalid --status; expected pass, fail, or blocked.",
        "ULW_LOOP_EVIDENCE_STATUS_INVALID",
        { details: { status: value } },
      );
  }
}

export function parseRecordEvidenceArgs(
  argv: readonly string[],
): RecordEvidenceCliArgs {
  const result = {
    goalId: required(argv, "--goal-id", "ULW_LOOP_GOAL_ID_REQUIRED"),
    criterionId: required(
      argv,
      "--criterion-id",
      "ULW_LOOP_CRITERION_ID_REQUIRED",
    ),
    status: evidenceStatus(
      required(argv, "--status", "ULW_LOOP_EVIDENCE_STATUS_REQUIRED"),
    ),
    evidence: required(argv, "--evidence", "ULW_LOOP_EVIDENCE_REQUIRED"),
  };
