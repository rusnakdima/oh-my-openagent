...
describe("applySpawnGuards fan-out cap", () => {
  it("#given spawns under the limit #when guarded #then allows and counts", () => {
    ...
    const counter = JSON.parse(
      readFileSync(join(sessionDir(), "spawn-count.json"), "utf8"),
    );
  ...
  it("#given the observed dotted v2 token #when guarded #then it counts too", () => {
    ...
    expect(
      applySpawnGuards(
        payload("collaboration.spawn_agent", { message: "scan" }),
      ),
    ).toBe("");
    const second = applySpawnGuards(
      payload("collaborationspawn_agent", { message: "scan" }),
    );
...
describe("applySpawnGuards gate-artifact guard", () => {
  ...
  it("#given a gate spawn identified by message only #when guarded #then still denies", () => {
    ...
    const output = applySpawnGuards(
      payload("spawn_agent", { message: "run the FINAL GATE REVIEW now" }),
    );
  ...
  it("#given v1 artifacts on disk #when the gate spawns #then allows", () => {
    ...
    writeFileSync(
      join(workDir, ".omo", "evidence", "g1-code-review.md"),
      "report\n",
    );
    writeFileSync(
      join(workDir, ".omo", "evidence", "g1-manual-qa.md"),
      "matrix\n",
    );
  ...
  it("#given a v2 plan #when the gate spawns without attempt-dir artifacts #then denies naming the attempt path", () => {
    ...
    expect(deny(output).permissionDecisionReason).toContain(
      ".omo/evidence/ulw/s1/g1/a1/g1-code-review.md",
    );
  });
      ...
      goals: [
        {
          ...
          objective: "Final goal",
          status: "in_progress",
          successCriteria: [criterion("C001"), {
            ...criterion("C002"),
            status: "pending",
          }],
          attempt: 1,
