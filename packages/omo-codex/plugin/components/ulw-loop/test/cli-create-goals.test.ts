...
function codexSnapshot(status: "active" | "complete" = "active"): string {
  return JSON.stringify({
    goal: { objective: ULW_LOOP_AGGREGATE_CODEX_OBJECTIVE, status },
  });
}
...
async function createPlan(
  ...
  resetOutput();
  expect(await ulwLoopCommand(["create-goals", "--brief", brief, "--json"]))
    .toBe(0);
...
describe("ulwLoopCommand create-goals", () => {
  it("creates plan + writes 3 artifacts + seeds criteria per goal", async () => {
    ...
    expect(await readFile(join(testDir, ".omo/ulw-loop/brief.md"), "utf8"))
      .toContain("Goal A");
    expect(await readFile(join(testDir, ".omo/ulw-loop/goals.json"), "utf8"))
      .toContain("successCriteria");
    expect(await readFile(join(testDir, ".omo/ulw-loop/ledger.jsonl"), "utf8"))
      .toContain("plan_created");
  });
  ...
  it("#given completed default aggregate #when creating another default plan #then guides to a fresh session", async () => {
    ...
    expect(await ulwLoopCommand(["create-goals", "--brief", "- New task"]))
      .toBe(1);

    expect(err.join("")).toContain(
      "Existing ulw-loop aggregate is already complete",
    );
  ...
  it("#given an active goal on a v2 plan #when reading status --json #then exposes the current attempt dir", async () => {
    ...
    expect(stdoutJson()).toMatchObject({
      currentAttemptDir: ".omo/evidence/ulw/session/G001-goal-a/a1",
    });
  });
  ...
  it("#given two session ids #when creating goals #then writes isolated session-scoped plans", async () => {
    ...
    expect(
      await readFile(
        join(testDir, ".omo/ulw-loop/session-A/goals.json"),
        "utf8",
      ),
    ).toContain("Alpha");
    expect(
      await readFile(
        join(testDir, ".omo/ulw-loop/session-B/goals.json"),
        "utf8",
      ),
    ).toContain("Beta");

    expect(
      await ulwLoopCommand(["status", "--session-id", "session-A", "--json"]),
    ).toBe(0);
    expect(stdoutJson()).toMatchObject({
      plan: {
  ...
  it("#given Codex thread env #when creating goals #then uses the thread as the session scope", async () => {
    ...
    expect(
      await readFile(
        join(testDir, ".omo/ulw-loop/thread-123/goals.json"),
        "utf8",
      ),
    ).toContain("Thread scoped");
    expect(await ulwLoopCommand(["status", "--json"])).toBe(0);
    expect(stdoutJson()).toHaveProperty(
      "plan.goalsPath",
      ".omo/ulw-loop/thread-123/goals.json",
    );
  });
  ...
  it("#given Codex thread env and explicit session id #when creating goals #then the explicit session wins", async () => {
    ...
    expect(
      await readFile(
        join(testDir, ".omo/ulw-loop/manual-456/goals.json"),
        "utf8",
      ),
  ...
  it("#given a valueless --session-id #when creating goals #then it fails and writes no plan", async () => {
    ...
    await expect(readFile(join(testDir, ".omo/ulw-loop/goals.json"), "utf8"))
      .rejects.toThrow();
  });
