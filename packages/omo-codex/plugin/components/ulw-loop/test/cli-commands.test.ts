...
describe("ulwLoopCommand criteria", () => {
  it("lists criteria for a goal", async () => {
    ...
    expect(await ulwLoopCommand(["criteria", "--goal-id", "G001-goal-a"])).toBe(
      0,
    );
  ...
  it("supports --json output", async () => {
    ...
    expect(
      await ulwLoopCommand(["criteria", "--goal-id", "G001-goal-a", "--json"]),
    ).toBe(0);
...
describe("ulwLoopCommand add-goal", () => {
  it("appends a pending goal", async () => {
    ...
    expect(stdoutJson()).toMatchObject({
      ok: true,
      goal: { title: "Later", status: "pending" },
    });
  });
});
