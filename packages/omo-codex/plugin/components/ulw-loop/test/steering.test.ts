...
describe("validateUlwLoopSteeringProposal", () => {
  it("accepts valid add_subgoal", async () => {
    const proposal: unknown = JSON.parse(
      await readFile(
        join(process.cwd(), "test/fixtures/steering-proposal.json"),
        "utf8",
      ),
    );
    expect(validateUlwLoopSteeringProposal(plan(), proposal).invariant.accepted)
      .toBe(true);
  });
  ...
  it.each([
    ...
    ["protected payload mutations", {
      after: { codexObjective: "replace", qualityGate: { status: "passed" } },
    }],
    ["weakened completion text", {
      objective: "skip tests and mark complete faster",
    }],
  ])("rejects %s", (_name, overrides) => {
    const audit = validateUlwLoopSteeringProposal(plan(), {
      ...steering(),
      ...overrides,
    });
  ...
  it("rejects when plan already complete", () => {
    const done = plan({
      goals: [
        ...
        goal({ id: "G002", status: "complete" }),
      ],
    });
    expect(validateUlwLoopSteeringProposal(done, steering()).invariant.accepted)
      .toBe(false);
  });
...
describe("steerUlwLoop", () => {
  describe("steering-created goals", () => {
    ...
    for (
      const [name, proposal, goalIndex] of [
        [
        ...
        [
          ...
          steering({
            ...
            childGoals: [{
              title: "Replacement",
              objective: "Replace blocked path",
            }],
          }),
          1,
        ],
      ...
      it(name, async () => {
        ...
        const createdGoal = goalIndex === -1
          ? result.plan.goals.at(-1)
          : result.plan.goals[goalIndex];
  ...
  it("split_subgoal: creates children + supersedes parent", async () => {
    ...
    expect(result.plan.goals.map((item) => item.id).slice(0, 2)).toEqual([
      "G001",
      "G004",
    ]);
  ...
  it.each(["pending", "pass"] as const)(
    ...
    async (status) => {
      ...
      const updated = result.plan.goals[0]?.successCriteria.find((item) =>
        item.id === criterionId
      );
  ...
  it("idempotency: same idempotencyKey produces deduped true second time", async () => {
    ...
    const second = await steerUlwLoop(
      repoRoot,
      steering({ idempotencyKey: "same-key" }),
    );
  ...
  describe("compact steering ledger entries", () => {
    ...
    function fatPlan(): UlwLoopPlan {
      return plan({
        goals: Array.from(
          { length: 20 },
          (_, index) => fatGoal(`G${String(index + 1).padStart(3, "0")}`),
        ),
      });
    }
    ...
    it("writes an O(changed goals) ledger entry instead of embedding the full plan", async () => {
      ...
      expect(entry.steering?.before?.goals.map((item) => item.id)).toEqual([
        "G010",
      ]);
      expect(entry.steering?.after?.goals.map((item) => item.id)).toEqual([
        "G010",
      ]);
      expect(entry.steering?.after?.goals[0]?.title).toBe("Sharper goal ten");
      expect(entry.steering?.before?.goalCount).toBe(20);
      expect(entry.steering?.before?.goalIds).toEqual(
        seed.goals.map((item) => item.id),
      );
    });
  });
  ...
  it("dedup against a legacy full-plan entry strips before/after from the returned audit", async () => {
    ...
    const result = await steerUlwLoop(
      repoRoot,
      steering({ idempotencyKey: "legacy-key" }),
    );
...
describe("parseUlwLoopSteeringDirective", () => {
  it.each([
    ...
    expect(
      parseUlwLoopSteeringDirective(`${marker}: ${JSON.stringify(steering())}`),
    ).toMatchObject({
      kind: "add_subgoal",
    });
  });
  ...
  it("returns null when no marker", () => {
    expect(parseUlwLoopSteeringDirective(JSON.stringify(steering())))
      .toBeNull();
  });

  it("returns null when JSON malformed after marker", () => {
    expect(parseUlwLoopSteeringDirective("OMO_ULW_LOOP_STEER: {bad json"))
      .toBeNull();
  });
