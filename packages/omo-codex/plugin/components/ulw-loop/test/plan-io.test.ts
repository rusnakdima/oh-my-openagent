...
function steeringAudit(
  overrides: Partial<UlwLoopSteeringAudit> = {},
): UlwLoopSteeringAudit {
  return {
...
function rejectedSteeringAudit(
  overrides: Partial<UlwLoopSteeringAudit> = {},
): UlwLoopSteeringAudit {
...
describe("readUlwLoopPlan", () => {
  ...
  it("throws UlwLoopError when goals.json is missing", async () => {
    ...
    await expect(readUlwLoopPlan(repoRoot)).rejects.toThrow(
      "omo-agent-toolkit ulw-loop create-goals",
    );
  });
  ...
  it("migrates legacy aggregate objective on read + writes aggregate_objective_migrated ledger entry + retains alias", async () => {
    ...
    const legacyObjective =
      "Complete all ulw-loop stories in .omo/ulw-loop/goals.json: G001 Build auth service";
    ...
    const persisted = JSON.parse(
      await readFile(ulwLoopGoalsPath(repoRoot), "utf8"),
    );
    expect(persisted).toMatchObject({
      codexObjective: STABLE_OBJECTIVE,
      codexObjectiveAliases: [legacyObjective],
    });
...
describe("writePlan", () => {
  it("writes goals.json atomically with no temp file left behind", async () => {
    ...
    expect(
      (await readdir(ulwLoopDir(repoRoot))).filter((name) =>
        name.endsWith(".tmp")
      ),
    ).toEqual([]);
  });
  ...
  it("overwrites existing file", async () => {
    ...
    // then
    expect(JSON.parse(await readFile(ulwLoopGoalsPath(repoRoot), "utf8")))
      .toMatchObject({
...
describe("appendLedger", () => {
  ...
  it("creates ledger.jsonl if missing", async () => {
    ...
    // then
    expect(await readFile(ulwLoopLedgerPath(repoRoot), "utf8")).toContain(
      "goal_completed",
    );
  });
...
describe("findAcceptedSteeringLedgerEntry", () => {
  it.each(
    [
      [
        ...
        steeringEntry("steering_accepted", steeringAudit(), {
          idempotencyKey: "needle-key",
        }),
      ],
      [
        "steering.idempotencyKey",
        steeringEntry(
          "steering_accepted",
          steeringAudit({ idempotencyKey: "needle-key" }),
        ),
      ],
      [
        ...
        steeringEntry(
          "steering_accepted",
          steeringAudit({ promptSignature: "needle-key" }),
        ),
      ],
    ] as const,
  )("finds an accepted entry by %s", async (_name, target) => {
    // given
    const repoRoot = await makeRepo();
    await appendLedger(
      repoRoot,
      steeringEntry(
        "steering_accepted",
        steeringAudit({ idempotencyKey: "other-key" }),
      ),
    );
  ...
  it("skips a rejected entry carrying the same key", async () => {
    ...
    await appendLedger(
      ...
      steeringEntry(
        "steering_rejected",
        rejectedSteeringAudit({ idempotencyKey: "needle-key" }),
      ),
    );

    // when/then
    await expect(findAcceptedSteeringLedgerEntry(repoRoot, "needle-key"))
  ...
  it("ignores an accepted entry whose key only appears in evidence text", async () => {
    ...
    await appendLedger(
      ...
      steeringEntry(
        "steering_accepted",
        steeringAudit({ evidence: "needle-key" }),
      ),
    );
  ...
  it("finds the key inside a legacy entry embedding full before/after plans", async () => {
    // given
    const repoRoot = await makeRepo();
    const bulkyGoals = Array.from(
      { length: 20 },
      (_, index) =>
        makeGoal({
          id: `G${String(index + 1).padStart(3, "0")}`,
          objective: `evidence detail ${index} `.repeat(200),
        }),
    );
...
describe("withUlwLoopMutationLock", () => {
  ...
  it("propagates the body rejection and keeps later calls working", async () => {
    ...
    await expect(withUlwLoopMutationLock(repoRoot, async () => "recovered"))
  ...
  it("still serializes a second batch after the first batch's gate settles", async () => {
    ...
    // when
    await Promise.all(
      [1, 2, 3].map(() => withUlwLoopMutationLock(repoRoot, body)),
    );
    await tick();
    await Promise.all(
      [1, 2].map(() => withUlwLoopMutationLock(repoRoot, body)),
    );
