...
describe("readCodexGoalSnapshotInput", () => {
  ...
  it("reads from file path", async () => {
    ...
    await writeFile(
      filePath,
      '{"goal":{"objective":"X","status":"complete"}}',
      "utf8",
    );
  ...
  it("throws CodexGoalSnapshotError when input is neither JSON nor a path", async () => {
    // when/then
    await expect(readCodexGoalSnapshotInput("not json and not a path")).rejects
      .toThrow(CodexGoalSnapshotError);
  });
});
