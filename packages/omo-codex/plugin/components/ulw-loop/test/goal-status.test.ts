...
describe("isUlwLoopDone", () => {
  it("returns true when all goals complete", () => {
    ...
    const plan = makePlan({
      goals: [
        makeGoal({ status: "complete" }),
        makeGoal({ id: "G002", status: "complete" }),
      ],
    });
...
describe("isFinalRunCompletionCandidate", () => {
  it("returns true when only one unresolved goal remains", () => {
    ...
    const plan = makePlan({
      goals: [makeGoal({ status: "complete" }), finalGoal],
    });
  ...
  it("returns false when multiple unresolved", () => {
    ...
    const plan = makePlan({
      goals: [goal, makeGoal({ id: "G002", status: "pending" })],
    });
...
describe("aggregateCodexObjective", () => {
  it("returns plan.codexObjective when set", () => {
    // when
    const objective = aggregateCodexObjective(
      makePlan({ codexObjective: "aggregate objective" }),
    );
...
describe("firstUnresolvedCriterion", () => {
  it("returns first non-pass criterion", () => {
    ...
    const goal = makeGoal({
      successCriteria: [makeCriterion({ status: "pass" }), unresolved],
    });
