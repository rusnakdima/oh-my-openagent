...
describe("ulwLoopDir(repo)", () => {
  ...
  it("#given a session id #when resolving the loop dir #then scopes artifacts under that session", () => {
    // when/then
    expect(ulwLoopDir("/repo", { sessionId: "sess_abc" })).toBe(
      join("/repo", ".omo", "ulw-loop", "sess_abc"),
    );
  });
});
...
describe("resolveUlwLoopSessionIdFromEnv", () => {
  it("#given a Senpi session env #when resolving scope #then uses the current Pi session id", () => {
    expect(
      resolveUlwLoopSessionIdFromEnv({
        PI_SESSION_ID: "01a00baf-feed-7f4d-a4a9-b8345c67cf72",
      }),
    ).toBe(
...
describe("ulw-loop*Path helpers", () => {
  it("compose artifact filenames under ulwLoopDir", () => {
    // when/then
    expect(ulwLoopBriefPath("/r")).toBe(
      join("/r", ".omo", "ulw-loop", "brief.md"),
    );
    expect(ulwLoopGoalsPath("/r")).toBe(
      join("/r", ".omo", "ulw-loop", "goals.json"),
    );
    expect(ulwLoopLedgerPath("/r")).toBe(
      join("/r", ".omo", "ulw-loop", "ledger.jsonl"),
    );
  });
  ...
  it("#given a session id #when composing artifact filenames #then returns session-scoped paths", () => {
    ...
    expect(ulwLoopBriefPath("/r", { sessionId: "session-A" })).toBe(
      join("/r", ".omo", "ulw-loop", "session-A", "brief.md"),
    );
    expect(ulwLoopGoalsPath("/r", { sessionId: "session-A" })).toBe(
      join("/r", ".omo", "ulw-loop", "session-A", "goals.json"),
    );
    expect(ulwLoopLedgerPath("/r", { sessionId: "session-A" })).toBe(
      join("/r", ".omo", "ulw-loop", "session-A", "ledger.jsonl"),
    );
  });
});
...
describe("repoRelative", () => {
  it("strips repo prefix when path is inside repo", () => {
    // when/then
    expect(repoRelative("/repo/.omo/ulw-loop/goals.json", "/repo")).toBe(
...
describe("isWithinAttemptDir", () => {
  ...
  it("#given a child artifact #when checked on posix #then it is contained", () => {
    expect(isWithinAttemptDir(`${posixRoot}/cli-pass.txt`, posixRoot, posix))
      .toBe(true);
  });

  it("#given a child artifact #when checked with win32 separators #then it is contained", () => {
    expect(isWithinAttemptDir(`${win32Root}\\cli-pass.txt`, win32Root, win32))
      .toBe(true);
  });
  ...
  it("#given a sibling dir sharing the prefix #when checked #then it is outside", () => {
    expect(
      isWithinAttemptDir(
        "/repo/.omo/evidence/ulw/s1/g1/a1x/f.txt",
        posixRoot,
        posix,
      ),
    ).toBe(false);
    expect(isWithinAttemptDir(`${win32Root}x\\f.txt`, win32Root, win32)).toBe(
      false,
    );
  });
  ...
  it("#given a prior-attempt artifact #when checked #then it is outside", () => {
    expect(
      isWithinAttemptDir(
        "/repo/.omo/evidence/ulw/s1/g1/a0/f.txt",
        posixRoot,
        posix,
      ),
    ).toBe(false);
    expect(
      isWithinAttemptDir(
        "C:\\repo\\.omo\\evidence\\ulw\\s1\\g1\\a0\\f.txt",
        win32Root,
        win32,
      ),
    ).toBe(false);
  ...
  it("#given a different-drive path #when checked on win32 #then it is outside", () => {
    expect(isWithinAttemptDir("D:\\elsewhere\\f.txt", win32Root, win32)).toBe(
      false,
    );
  });
});
