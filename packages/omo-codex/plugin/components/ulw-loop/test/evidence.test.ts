...
async function readLastLedgerEntry(repo: string): Promise<UlwLoopLedgerEntry> {
  const lines =
    (await readFile(join(repo, ".omo/ulw-loop/ledger.jsonl"), "utf8")).trim()
      .split("\n");
...
function makePlan(overrides: Partial<UlwLoopPlan> = {}): UlwLoopPlan {
  return {
    ...
    codexObjective:
      "Complete the durable ulw-loop plan in .omo/ulw-loop/goals.json",
...
describe("recordEvidence (status=pass)", () => {
  it("sets criterion.status=pass + capturedEvidence + capturedAt", async () => {
    ...
    expect(result.criterion.capturedEvidence).toContain(
      "curl /login returns 200",
    );
  ...
  it("persists the change so a fresh read sees status=pass", async () => {
    ...
    const criterion = firstGoal(await readUlwLoopPlan(repo)).successCriteria
      .find((c) => c.id === "C001");
    ...
    const goal = makeGoal({
      successCriteria: [
        ...
        makeCriterion({
          id: "C002",
          status: "fail",
          capturedEvidence: "older",
        }),
        makeCriterion({
          id: "C003",
          status: "blocked",
          capturedEvidence: "oldest",
        }),
      ],
    });
...
describe("criteriaSummary (pure)", () => {
  ...
  it("returns empty when no criteria exist", () => {
    const summary = criteriaSummary(
      makePlan({ goals: [makeGoal({ successCriteria: [] })] }),
    );
