...
export async function writeQualityGateArtifacts(
  repoRoot: string,
  qaDir = QA_DIR,
): Promise<void> {
  await mkdir(join(repoRoot, qaDir), { recursive: true });
  await writeFile(
    join(repoRoot, `${qaDir}/code-review.md`),
    "code review approved\n",
    "utf8",
  );
  await writeFile(
    join(repoRoot, `${qaDir}/gate-review.md`),
    "gate review approved\n",
    "utf8",
  );
  await writeFile(
    join(repoRoot, `${qaDir}/cli-pass.txt`),
    "cli scenario passed\n",
    "utf8",
  );
  await writeFile(
    join(repoRoot, `${qaDir}/rejection.txt`),
    "invalid gate rejected\n",
    "utf8",
  );
}
