...
export function plan(
  ...
  const activeGoalId = goals.find((candidate) =>
...
export async function lastLedger(repo: string): Promise<UlwLoopLedgerEntry> {
  const last = (await readFile(ulwLoopLedgerPath(repo), "utf8")).trim().split(
    /\r?\n/,
  ).at(-1);
...
export function passGoal(
  id: string,
  overrides: Partial<UlwLoopItem> = {},
): UlwLoopItem {
  return goal({
    id,
    successCriteria: [
