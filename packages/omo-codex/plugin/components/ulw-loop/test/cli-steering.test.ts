...
function steerResult(
  overrides: Partial<SteerUlwLoopResult> = {},
): SteerUlwLoopResult {
  return {
    plan: plan(),
    accepted: true,
...
describe("parseSteeringSource", () => {
  ...
  it("returns explicit value", () => {
    expect(parseSteeringSource(["--source", "user_prompt_submit"])).toBe(
      "user_prompt_submit",
    );
  });
});
...
describe("parseSteeringProposal revise_criterion", () => {
  ...
  for (
    const testCase of [
      {
        name:
          "throws when none of scenario/expected-evidence/user-model provided",
    ...
    it(testCase.name, async () => {
      await expect(parseSteeringProposal(testCase.args)).rejects.toThrow(
        UlwLoopError,
      );
    });
  }
