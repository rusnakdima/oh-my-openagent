...
function proposal(
  overrides: Partial<UlwLoopSteeringProposal> = {},
): UlwLoopSteeringProposal {
  return {
    kind: "add_subgoal",
    source: "cli",
...
describe("steerUlwLoopBatch", () => {
  it("#given two valid proposals #when applied as a batch #then both mutations commit", async () => {
    ...
    expect((await readUlwLoopPlan(repo)).goals.map((item) => item.title))
      .toContain("Revised");
    expect(
      (await readSteeringLedgerEntries(repo)).filter((entry) =>
        entry.kind === "steering_accepted"
      ),
    ).toHaveLength(2);
  });
  ...
  it("#given rejected proposals #when applied as a batch #then one rejection audit lists every rejected index", async () => {
    ...
    const result = await steerUlwLoopBatch(repo, [
      proposal({ idempotencyKey: "ok" }),
      proposal({ kind: "reorder_pending", pendingOrder: ["missing"] }),
      proposal({ evidence: "" }),
    ]);
    ...
    expect(entries.filter((entry) => entry.kind === "steering_accepted"))
      .toHaveLength(0);
    expect(entries.filter((entry) => entry.kind === "steering_rejected"))
      .toHaveLength(1);
    expect(entries.find((entry) => entry.kind === "steering_rejected")?.message)
      .toContain("index 1:");
    expect(entries.find((entry) => entry.kind === "steering_rejected")?.message)
      .toContain("index 2:");
  });
  ...
  it("#given a batch member split in proposals-json #when applied #then updates validation batch membership", async () => {
    ...
    expect(result.plan.validationBatches).toEqual([{
      batchId: "VB001",
      memberIds: ["G003", "G002"],
      finalGoalId: "G002",
    }]);
    expect((await readSteeringLedgerEntries(repo)).at(-1)).toMatchObject({
      kind: "batch_updated",
    });
  });
  ...
  it("#given a prior idempotency key #when batched with a fresh proposal #then dedupes one and applies the other", async () => {
    ...
    const result = await steerUlwLoopBatch(repo, [
      proposal({ idempotencyKey: "same" }),
      proposal({ idempotencyKey: "fresh" }),
    ]);
  ...
  it("#given mixed fresh and deduped proposals #when accepted and replayed #then one append records all fresh audits and replay records none", async () => {
    ...
    seeded.goals[0]?.successCriteria.push({
      id: "C001",
      scenario: "old",
    ...
    const revise = {
      ...proposal({
        ...
        targetGoalId: "G001",
        criterionId: "C001",
        idempotencyKey: "revise-c1",
      }),
      scenario: "new",
    } satisfies CliSteeringProposal;
    const batch = [
      ...
      proposal({
        ...
        targetGoalId: "G002",
        childGoals: [{ title: "Child", objective: "Do child" }],
        idempotencyKey: "split-g2",
      }),
    ];
    const result = await steerUlwLoopBatch(repo, batch);

    expect(result.accepted).toBe(true);
    expect(result.results.map((item) => item.deduped)).toEqual([
      true,
      false,
      false,
    ]);
    ...
    expect(
      (await readSteeringLedgerEntries(repo)).map((entry) => entry.kind).slice(
        -3,
      ),
    ).toEqual(["criteria_revised", "steering_accepted", "batch_updated"]);
    const after = JSON.stringify(await readUlwLoopPlan(repo));
    appendFileCalls.mockClear();

    const replay = await steerUlwLoopBatch(repo, batch);

    expect(replay.results.map((item) => item.deduped)).toEqual([
      true,
      true,
      true,
    ]);
