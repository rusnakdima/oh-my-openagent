...
async function resolveLatestNodeLtsVersion() {
  const releases = await (await fetchOk("https://nodejs.org/dist/index.json"))
    .json();
  const latestLts = releases.find((release) => release.lts !== false);
  if (latestLts === undefined) {
    throw new Error("nodejs.org/dist/index.json contains no LTS release");
  }
...
async function generateNodeManifest(version) {
  ...
  const shasums =
    await (await fetchOk(`https://nodejs.org/dist/v${version}/SHASUMS256.txt`))
      .text();
  const line = shasums.split("\n").find((candidate) =>
    candidate.trim().endsWith(`  ${zipName}`) ||
    candidate.trim().endsWith(` ${zipName}`)
  );
  if (line === undefined) {
    throw new Error(
      `SHASUMS256.txt for v${version} has no entry for ${zipName}`,
    );
  }
  const sha256 = line.trim().split(/\s+/)[0];
  if (!/^[a-f0-9]{64}$/.test(sha256)) {
    throw new Error(`invalid sha256 parsed for ${zipName}: ${sha256}`);
  }
