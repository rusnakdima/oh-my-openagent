...
import {
  ...
  resolveLockPath,
  resolveStatePath,
} from "../../../scripts/auto-update-state.mjs";
...
export async function detectInstallFlowDetailed(
  ...
  const snapshotPresent = await isFile(
    join(options.pluginRoot, INSTALL_SNAPSHOT_FILENAME),
  );
  const snapshotSignal: InstallFlow = snapshotPresent
    ? "npx-local"
    : "marketplace";
  ...
  const scan = options.configToml === undefined
  ...
  if (scan.kind === "absent") {
    return {
      ...
      flow: snapshotSignal,
      reason:
        `${snapshotReason}; no [marketplaces.${marketplaceName}] source to cross-check`,
      snapshotPresent,
    };
  }
  if (scan.kind === "unparsable") {
    return {
      ...
      flow: "unknown",
      reason:
        `${snapshotReason}; [marketplaces.${marketplaceName}] source value is unparsable`,
      snapshotPresent,
    };
  }
  ...
  if (configSignal === "unparsable") {
    return {
      ...
      flow: "unknown",
      reason: `${snapshotReason}; marketplace source ${
        JSON.stringify(scan.source)
      } is neither a local absolute path nor a git URL`,
      snapshotPresent,
    };
  }
  if (configSignal !== snapshotSignal) {
    return {
      ...
      configSource: scan.source,
      flow: "unknown",
      reason: `${snapshotReason}, but marketplace source ${
        JSON.stringify(scan.source)
      } indicates ${configSignal}; signals disagree`,
      snapshotPresent,
    };
  }
  return {
...
export async function detectInstallFlow(
  options: DetectInstallFlowOptions,
): Promise<InstallFlow> {
...
export async function detectInstallFlowForTest(
  pluginRoot: string,
): Promise<InstallFlow> {
  const home = await resolveCodexHome({ env: {}, pluginRoot });
  const configToml = home.source === "walk-up"
    ? await readOptionalFile(join(home.path, "config.toml"))
    : undefined;
  return detectInstallFlow({
    pluginRoot,
    ...(configToml === undefined ? {} : { configToml }),
  });
}
...
export async function bootstrapLocks(
  ...
  const autoUpdateLockPath = resolveLockPath(
    options.env,
    resolveStatePath(options.env),
  );
...
type MarketplaceSourceScan = { readonly kind: "absent" } | {
  readonly kind: "unparsable";
} | { readonly kind: "source"; readonly source: string };

function scanMarketplaceSource(
  configToml: string,
  marketplaceName: string,
): MarketplaceSourceScan {
