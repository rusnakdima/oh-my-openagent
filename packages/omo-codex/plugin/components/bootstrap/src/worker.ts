...
export function parseWorkerFlags(
...
export function parseBootstrapState(
  ...
  const completedForVersion = typeof raw["completedForVersion"] === "string"
    ? raw["completedForVersion"]
    : undefined;
  const lastAttemptAt = typeof raw["lastAttemptAt"] === "number"
    ? raw["lastAttemptAt"]
    : undefined;
  const lastStatus =
...
export async function runBootstrapWorker(
  ...
  if (
    flags.only !== undefined && !steps.some((step) => step.name === flags.only)
  ) {
  ...
  try {
    ...
    if (
      !flags.once && pluginVersion !== undefined &&
    ...
    const codexHome = flags.codexHome ??
      (await resolveCodexHome({ env, pluginRoot })).path;
    ...
    const status: BootstrapRunStatus = degraded.length === 0
      ? "success"
      : "degraded";
    const state: BootstrapState = {
      ...(pluginVersion === undefined
        ? {}
        : { completedForVersion: pluginVersion }),
      degraded,
      lastAttemptAt: now,
      lastStatus: status,
    };
...
function parseDegradedEntries(
  raw: unknown,
): readonly BootstrapDegradedEntry[] | undefined {
