...
import {
  ...
  linkRootRuntimeBin,
} from "../../../../src/install/codex-cache-bins.ts";
...
export const GIT_BASH_INSTALL_HINT =
...
export async function runWorkerSetup(
  ...
  await updateConfigStep(options, {
    agentConfigs: agents.agentConfigs,
    gitBashEnabled,
  }, degraded);
...
async function resolveGitBashStep(
  ...
  try {
    const resolution = await prepareGitBashForInstall({
      ...
      ...(options.resolveGitBash === undefined
        ? {}
        : { resolveGitBash: options.resolveGitBash }),
    });
    ...
    degraded.push({
      ...
      reason:
        "Git Bash was not found on this Windows machine; the omo git_bash MCP server stays disabled",
    });
  } catch (error) {
    degraded.push({
      component: "git-bash",
...
async function linkBundledAgentsStep(
  options: WorkerSetupOptions,
  ...
  try {
    ...
    const agentConfigs = linked
      .map((link) => ({
        ...
        name: agentNameFromToml(link.name),
      }))
      ...
      degraded: [
        {
          ...
          reason: `failed to link bundled agents into ${agentsTarget}: ${
            errorMessage(error)
          }`,
        },
      ],
...
async function stageBundledAgents(
  ...
  for (const componentName of await directoryNames(componentsRoot)) {
    ...
    const agentFiles = (await fileNames(agentsDir)).filter((name) =>
      name.endsWith(".toml")
    );
    if (agentFiles.length === 0) continue;
    const stagedAgentsDir = join(
      stageRoot,
      "components",
      componentName,
      "agents",
    ...
    for (const agentFile of agentFiles) {
      await copyFile(
        ...
        join(stagedAgentsDir, agentFile),
      );
    }
  }
}
...
async function updateConfigStep(
  ...
  try {
    ...
    const agentConfigs = inputs.agentConfigs.filter(
      (agentConfig) =>
        !hasForeignAgentRegistration(existingConfig, agentConfig),
    );
...
async function assertWritableConfigIfPresent(
  configPath: string,
): Promise<void> {
  try {
...
async function linkComponentBinsStep(
  ...
  try {
    ...
    degraded.push({
      component: "bin-links",
      hint: BOOTSTRAP_DOCTOR_HINT,
      reason: `failed to link component bins into ${binDir}: ${
        errorMessage(error)
      }`,
    });
  }
...
async function linkRuntimeWrapperStep(
  ...
  try {
    ...
    await appendBootstrapLog(
      ...
      "omo-agent-toolkit-degraded",
      {
        warning:
          `Warning: skipped the omo-agent-toolkit runtime wrapper because ${cliPath} is missing; omo-agent-toolkit ulw-loop commands will be unavailable until a package shipping dist/cli is installed`,
      },
    );
  } catch (error) {
    degraded.push({
      ...
      hint: BOOTSTRAP_DOCTOR_HINT,
      reason:
        `failed to link the omo-agent-toolkit runtime wrapper into ${binDir}: ${
          errorMessage(error)
        }`,
    });
  }
...
async function stampGitBashEnvStep(
  ...
  try {
    ...
    degraded.push({
      ...
      hint: BOOTSTRAP_DOCTOR_HINT,
      reason: `failed to stamp ${join(options.pluginRoot, ".mcp.json")}: ${
        errorMessage(error)
      }`,
    });
  }
