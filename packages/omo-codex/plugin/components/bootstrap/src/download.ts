...
export class UnsupportedPlatformError extends DownloadError {
  ...
  constructor(options: {
    ...
    super(
      "unsupported-platform",
      `Manifest "${options.manifestName}" has no asset for unsupported platform "${options.platformKey}" (available: ${
...
function proxyLimitationNote(env: NodeJS.ProcessEnv): string {
  const configuredKey = PROXY_ENV_KEYS.find((key) =>
...
export async function downloadChecksummedAsset(
  ...
  try {
    ...
    throw new DownloadError(
      "download-failed",
      `Download failed for ${options.url}: ${describeFailure(error)}.${
        proxyLimitationNote(env)
      }`,
    );
  }
  if (!response.ok) {
    throw new DownloadError(
      "download-failed",
      `Download failed for ${options.url}: HTTP ${response.status}.${
        proxyLimitationNote(env)
      }`,
    );
  }
  ...
  try {
    ...
    throw new DownloadError(
      "download-failed",
      `Download failed for ${options.url} while writing the response body: ${
...
export function parseAssetManifest(
  ...
  if (
    ...
    typeof data["version"] !== "string" || !isRecord(data["platforms"])
  ) {
    throw new Error(
  ...
  for (const [platformKey, asset] of Object.entries(data["platforms"])) {
    platforms[platformKey] = parseManifestAsset(
      asset,
      manifestName,
      platformKey,
    );
...
export async function downloadFromManifest(
  options: DownloadFromManifestOptions,
): Promise<string> {
  ...
  const destination = join(
    ...
    basename(new URL(asset.url).pathname),
  );
