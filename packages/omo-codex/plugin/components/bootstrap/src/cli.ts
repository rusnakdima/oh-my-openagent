...
async function runDownloadCommand(args: readonly string[]): Promise<number> {
  ...
  if (
    manifestName === undefined || platformKey === undefined ||
    destinationDir === undefined
  ) {
    process.stderr.write(
      `[omo-bootstrap] download requires <manifest> <platform> <destination-dir>\n${TOP_LEVEL_HELP}`,
    );
    return 1;
  }
  try {
...
async function runWorkerCommand(args: readonly string[]): Promise<number> {
  ...
  process.stdout.write(
    result.ran
      ? `[omo-bootstrap] worker finished: ${result.status}\n`
...
async function main(): Promise<number> {
  ...
  process.stderr.write(
    `[omo-bootstrap] unknown command: ${argv.join(" ")}\n${TOP_LEVEL_HELP}`,
  );
  return 1;
}
