...
export interface SgProvisionSeams {
  ...
  readonly resolvePreexistingSg?: (
    options: ResolvePreexistingSgOptions,
  ) => string | null;
  readonly runVersionProbe?: (binaryPath: string) => Promise<string>;
}

export function sgProvisionDestination(
  context: BootstrapWorkerContext,
  arch: string,
): string {
  return join(
    sgRuntimeDir(context.codexHome, context.platform, arch),
    sgBinaryName(context.platform),
  );
}

function sgRuntimeDir(
  codexHome: string,
...
export async function runSgProvision(
  ...
  if (context.env[SG_FORCE_PROVISION_ENV_KEY] !== "1") {
    const preexisting =
      (seams.resolvePreexistingSg ?? defaultResolvePreexistingSg)({
        arch,
  ...
  try {
    const version = await provisionFromSharedManifest(context, seams, {
      arch,
      destination,
    });
    ...
    return {
      degraded: [{
        component: SG_PROVISION_COMPONENT,
...
async function provisionFromSharedManifest(
  ...
  const provisionedPath = await provisionSgBinary({
    ...
    ...(seams.releaseAssets === undefined
      ? {}
      : { releaseAssets: seams.releaseAssets }),
  });
...
async function verifyProvisionedVersion(
  ...
  try {
    reported =
      (await (seams.runVersionProbe ?? defaultVersionProbe)(destination))
        .trim();
  } catch (error) {
    await rm(destination, { force: true });
    throw new Error(
      `provisioned sg at ${destination} failed its --version probe: ${
...
function defaultResolvePreexistingSg(
  options: ResolvePreexistingSgOptions,
): string | null {
