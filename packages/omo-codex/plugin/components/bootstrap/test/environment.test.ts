...
async function writePluginRoot(
  ...
  if (options.withSnapshot) {
    await writeFile(
      ...
      '{"packageName":"lazycodex-ai","version":"4.9.2"}\n',
    );
  }
  return pluginRoot;
}
...
describe("detectInstallFlow", () => {
  it("#given an install snapshot and no config #when detecting #then reports npx-local", async () => {
    await withTempDir("omo-flow-", async (directory) => {
      const pluginRoot = await writePluginRoot(directory, {
        withSnapshot: true,
      });
  ...
  it("#given no snapshot and a git marketplace source #when detecting #then both signals agree on marketplace", async () => {
    await withTempDir("omo-flow-", async (directory) => {
      ...
      expect(
        await detectInstallFlow({ configToml: GIT_SOURCE_CONFIG, pluginRoot }),
      ).toBe("marketplace");
    });
  });
  ...
  it("#given a snapshot and a local absolute marketplace source #when detecting #then both signals agree on npx-local", async () => {
    await withTempDir("omo-flow-", async (directory) => {
      ...
      expect(
        await detectInstallFlow({
          configToml: LOCAL_SOURCE_CONFIG,
          pluginRoot,
        }),
      ).toBe("npx-local");
    });
  ...
  it("#given an unclassifiable marketplace source #when detecting #then reports unknown", async () => {
    await withTempDir("omo-flow-", async (directory) => {
      ...
      const config = [
        "[marketplaces.sisyphuslabs]",
        'source = "./relative/checkout"',
        "",
      ].join("\n");
      expect(await detectInstallFlow({ configToml: config, pluginRoot })).toBe(
        "unknown",
      );
    });
  });
  ...
  it("#given a quoted marketplace header #when detecting #then the section is still recognized", async () => {
    await withTempDir("omo-flow-", async (directory) => {
      ...
      const config = [
        '[marketplaces."sisyphuslabs"]',
        'source = "https://github.com/lazycodex-ai/lazycodex"',
        "",
      ].join("\n");
      expect(await detectInstallFlow({ configToml: config, pluginRoot })).toBe(
        "marketplace",
...
describe("detectInstallFlowFromEnvironment", () => {
  it("#given a codex store layout with a git marketplace config #when detecting from environment #then reports marketplace", async () => {
    await withTempDir("omo-flow-env-", async (directory) => {
      ...
      const detection = await detectInstallFlowFromEnvironment({
        env: {},
        pluginRoot,
      });
...
describe("sync-lazycodex-marketplace output", () => {
  it(
    "#given a synced marketplace tree #when looking for the install snapshot #then it is absent and detection reports marketplace",
    async () => {
      await withTempDir("omo-flow-sync-", async (directory) => {
        ...
        const sync = spawnSync(
          ...
          [
            "run",
            join(repoRoot, "script", "sync-lazycodex-marketplace.ts"),
            sourceRoot,
            lazycodexRoot,
          ],
          { encoding: "utf8" },
        ...
        expect(existsSync(join(syncedPluginRoot, "lazycodex-install.json")))
          .toBe(false);
        expect(await detectInstallFlowForTest(syncedPluginRoot)).toBe(
          "marketplace",
        );
      });
    },
  ...
  it("#given the repo plugin source tree the sync copies #when looking for the install snapshot #then it never exists there", () => {
    ...
    expect(existsSync(join(pluginSourceRoot, ".codex-plugin", "plugin.json")))
      .toBe(true);
    expect(existsSync(join(pluginSourceRoot, "lazycodex-install.json"))).toBe(
      false,
    );
  });
});
...
describe("bootstrapLocks", () => {
  it("#given free locks #when acquiring twice #then the second acquirer gets null and release leaves no lock files", async () => {
    await withTempDir("omo-locks-", async (directory) => {
      ...
      expect(first.bootstrapLockPath).toBe(
        join(pluginData, "bootstrap", "state.json.lock"),
      );
      expect(first.statePath).toBe(join(pluginData, "bootstrap", "state.json"));
      expect(first.autoUpdateLockPath).toBe(
        join(pluginData, "auto-update.json.lock"),
      );
  ...
  it("#given the auto-update lock already held #when acquiring #then returns null without leaking the bootstrap lock", async () => {
    await withTempDir("omo-locks-", async (directory) => {
      ...
      expect(existsSync(join(pluginData, "bootstrap", "state.json.lock"))).toBe(
        false,
      );
  ...
  it("#given an explicit auto-update lock override #when acquiring #then the override path is honored", async () => {
    await withTempDir("omo-locks-", async (directory) => {
      ...
      const env = {
        LAZYCODEX_AUTO_UPDATE_LOCK_PATH: overridePath,
  ...
  it("#given the path helpers #when resolving state and lock paths #then they follow the PLUGIN_DATA bootstrap layout", () => {
    expect(resolveBootstrapStatePath("/data/omo-sisyphuslabs")).toBe(
      join("/data/omo-sisyphuslabs", "bootstrap", "state.json"),
    );
...
async function writeSyncSourceFixture(sourceRoot: string): Promise<void> {
  ...
  await writeJson(join(pluginSource, ".codex-plugin", "plugin.json"), {
    name: "omo",
    version: "1.2.3",
  });
  ...
  await writeJson(
    join(pluginSource, "hooks", "hooks.json"),
  ...
  await writeExecutableStub(
    join(pluginSource, "components", "bootstrap", "dist", "cli.js"),
  );
  await writeExecutableStub(
    join(pluginSource, "components", "ast-grep-skill", "dist", "cli.js"),
  );
  await mkdir(join(pluginSource, "components", "bootstrap", "scripts"), {
    recursive: true,
  });
  await writeFile(
