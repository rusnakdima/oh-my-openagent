...
const STANDALONE_BYTES = new Uint8Array(
  Buffer.alloc(16_384, "standalone-ast-grep-binary "),
);
...
function fetchReturning(
  bytes: Uint8Array,
): { fetchImpl: SgFetch; calls: string[] } {
  const calls: string[] = [];
...
async function readBootstrapLog(pluginData: string): Promise<string> {
  try {
    return await readFile(
      join(pluginData, "bootstrap", "bootstrap.log"),
      "utf8",
    );
  } catch {
    return "";
      ...
      releaseAssets: fixtureReleaseAssets({
        platforms: {
          "win32-x64": {
            ...
            url:
              `https://example.invalid/releases/${FIXTURE_VERSION}/app-x86_64-pc-windows-msvc.zip`,
          },
        },
      }),
      resolvePreexistingSg: () => null,
