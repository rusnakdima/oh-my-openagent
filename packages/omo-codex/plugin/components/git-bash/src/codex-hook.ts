...
export function parsePostCompactPayload(
  raw: string,
): PostCompactPayload | null {
  if (raw.trim().length === 0) return null;
  try {
    const parsed: unknown = JSON.parse(raw);
...
export function applyGitBashPreToolUseReminder(
  ...
  const markerPath = reminderMarkerPath(
    payload.session_id,
...
export function applyGitBashPostCompactReset(
  ...
  rmSync(reminderMarkerPath(payload.session_id, options.pluginDataRoot), {
    force: true,
  });
  return "";
}
...
export async function runGitBashHookCli(
  ...
  try {
    ...
    const output = eventName === "post-compact"
...
function isWindowsHost(options: GitBashHookOptions): boolean {
  ...
  return env["OS"] === "Windows_NT" || env["ComSpec"] !== undefined ||
