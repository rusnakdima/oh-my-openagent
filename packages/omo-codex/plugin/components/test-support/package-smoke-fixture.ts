...
export function readPackageJson(path: string): ComponentPackageJson {
  ...
  if (!isComponentPackageJson(parsed)) {
    throw new TypeError(`Invalid package metadata: ${path}`);
  }
  return parsed;
}
...
export function readHooksJson(path: string): HooksJson {
  ...
  if (!isHooksJson(parsed)) {
    throw new TypeError(`Invalid hooks metadata: ${path}`);
  }
  return parsed;
}
...
export function readPluginJson(path: string): PluginJson {
  ...
  if (!isPluginJson(parsed)) {
    throw new TypeError(`Invalid plugin metadata: ${path}`);
  }
  return parsed;
}
...
export function collectHookCommandsFromValue(
  value: unknown,
): readonly string[] {
  if (typeof value === "string") return [];
  if (Array.isArray(value)) return value.flatMap(collectHookCommandsFromValue);
  if (!isRecord(value)) return [];
  const ownCommand = typeof value["command"] === "string"
    ? [value["command"]]
    : [];
  return [
    ...ownCommand,
    ...Object.values(value).flatMap(collectHookCommandsFromValue),
  ];
}
...
function isComponentPackageJson(value: unknown): value is ComponentPackageJson {
  ...
  return (
    ...
    (optionalDependencies === undefined ||
      isStringRecord(optionalDependencies)) &&
