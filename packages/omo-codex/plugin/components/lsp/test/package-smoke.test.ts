...
describe("plugin package metadata", () => {
  it("#given packaged component files #when validating entrypoints #then hook command stays local and MCP command references the package", () => {
    ...
    // when
    const postToolUseCommand = hooksJson.hooks["PostToolUse"]?.[0]?.hooks[0]
      ?.command;
    const postCompactCommand = hooksJson.hooks["PostCompact"]?.[0]?.hooks[0]
      ?.command;
    ...
    expect(cliSource).toContain(
      "Usage: omo-lsp [mcp | hook post-tool-use | hook post-compact]",
    );
    expect(postToolUseCommand).toBe(
      `node "${pluginRoot}/dist/cli.js" hook post-tool-use`,
    );
    expect(postCompactCommand).toBe(
      `node "${pluginRoot}/dist/cli.js" hook post-compact`,
    );
    expect(lspServer?.command).toBe("node");
    expect(lspServer?.args).toEqual([
      "../../../../lsp-daemon/dist/cli.js",
      "mcp",
    ]);
