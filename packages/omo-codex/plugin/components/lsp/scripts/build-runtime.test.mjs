...
test("#given a built component runtime #when the manifest is inspected #then it pins sorted output hashes", async () => {
  const manifest = JSON.parse(
    await readFile(
      new URL("../dist/.omo-runtime-manifest.json", import.meta.url),
      "utf8",
    ),
  );
  ...
  assert(
    manifest.outputs.every((output) => /^[0-9a-f]{64}$/u.test(output.sha256)),
  );
});
...
test("#given a dist-only installed component #when output bytes are tampered #then validation fails actionably", async () => {
  ...
  try {
    ...
    await copyFile(
      SCRIPT_URL,
      join(componentRoot, "scripts", "build-runtime.mjs"),
    );
    await writeFile(
      join(componentRoot, "package.json"),
      JSON.stringify({ version: "4.17.0" }),
    );
    ...
    assert.match(
      result.stderr,
      /Invalid installed codex-lsp runtime manifest/u,
    );
