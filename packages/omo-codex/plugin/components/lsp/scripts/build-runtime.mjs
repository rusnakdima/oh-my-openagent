...
function validateManifest({ requireFreshInputs }) {
  ...
  if (manifest.version !== packageVersion()) {
    throw new Error("manifest version is stale");
  }
  if (requireFreshInputs && manifest.inputDigest !== inputDigest()) {
    throw new Error("manifest inputDigest is stale");
  }
  const outputs = [...manifest.outputs].sort((left, right) =>
    left.path.localeCompare(right.path)
  );
  if (JSON.stringify(outputs) !== JSON.stringify(manifest.outputs)) {
    throw new Error("manifest outputs are not sorted");
  }
  for (const output of outputs) {
    const outputPath = join(distDir, output.path);
    if (!existsSync(outputPath)) {
      throw new Error(`manifest output is missing: ${output.path}`);
    }
    if (sha256File(outputPath) !== output.sha256) {
      throw new Error(`manifest output hash mismatch: ${output.path}`);
    }
  }
}
...
function readManifest() {
  ...
  if (
    ...
    !/^sha256:[0-9a-f]{64}$/.test(parsed.inputDigest)
  ) {
    throw new Error("manifest inputDigest must be sha256:<64hex>");
  }
  if (
    !Array.isArray(parsed.outputs) || !parsed.outputs.every(isManifestOutput)
  ) {
...
function writeManifest() {
  const manifest = {
    ...
    outputs: outputFiles().map((path) => ({
      path,
      sha256: sha256File(join(distDir, path)),
    })),
  };
...
function inputFiles() {
  return [
    ...
    ...walk(join(componentRoot, "src")).map((path) =>
      relative(componentRoot, path)
    ),
  ].sort();
}
...
function assertBuiltinOnlyCli() {
  ...
  if (invalid.length > 0) {
    throw new Error(
      `dist/cli.js has non-builtin runtime imports: ${invalid.join(", ")}`,
    );
  }
}
...
function packageVersion() {
  const parsed = JSON.parse(
    readFileSync(join(componentRoot, "package.json"), "utf8"),
  );
