...
async function makeFixture() {
  ...
  await mkdir(join(root, "packages", "lsp-tools-mcp", "dist"), {
    recursive: true,
  });
  await copyFile(
  ...
  await writeFile(
    join(root, "packages", "lsp-tools-mcp", "dist", "cli.js"),
    "cli\n",
  );
...
test("#given package metadata is newer than required outputs #when bootstrapping #then it rebuilds", async () => {
  // given
  const fixture = await makeFixture();
  await mkdir(join(fixture.root, "packages", "lsp-tools-mcp", "dist", "lsp"), {
    recursive: true,
  });
  await writeFile(
    join(
  ...
  await utimes(
    join(fixture.root, "packages", "lsp-tools-mcp", "package.json"),
    newer,
    newer,
  );
