...
export function sessionIdFrom(
  input: { readonly session_id?: unknown },
): string | undefined {
...
export function readLspPostEditCache(
  sessionId: string | undefined,
): PostEditNotConfiguredCache {
...
export function writeLspPostEditCache(
  sessionId: string | undefined,
...
export function isLspDaemonUnreachableDiagnostics(
  diagnostics: string,
): boolean {
...
function isLspSessionState(value: Record<string, unknown>): boolean {
  const notConfiguredExtensions = value["notConfiguredExtensions"] ??
    value["unavailableExtensions"];
  return Array.isArray(notConfiguredExtensions) &&
    notConfiguredExtensions.every((item) => typeof item === "string");
}
...
function normalizeSessionState(
  ...
  return {
    notConfiguredExtensions: Array.isArray(notConfiguredExtensions)
      ? notConfiguredExtensions.filter((item) => typeof item === "string")
        .sort()
      : [],
  };
}
