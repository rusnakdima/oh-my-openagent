...
export function ensureLspDaemonCliEnv(
...
function resolveLspDaemonCli(): LspDaemonCliResolution {
  ...
  const bundledCli = fileURLToPath(
    new URL("../../lsp-daemon/dist/cli.js", import.meta.url),
  );
...
function resolveConfiguredLspDaemonCli(
  cliPath: string,
): LspDaemonCliResolution {
  const version = readDaemonPackageVersion(cliPath);
  if (version === null) {
    throw new Error(
      `Unable to determine packaged LSP daemon version beside ${cliPath}`,
    );
  }
  return {
    cliPath,
...
function readDaemonPackageVersion(cliPath: string): string | null {
  try {
    const parsed: unknown = JSON.parse(
      readFileSync(join(dirname(cliPath), "package.json"), "utf8"),
    );
    if (
