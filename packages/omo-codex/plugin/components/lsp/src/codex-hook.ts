...
export async function runLspDiagnosticsText(
  filePath: string,
): Promise<PostEditDiagnosticsOutcome> {
  ensureLspDaemonCliEnv();
  const result = await callDiagnosticsViaDaemon(filePath, {
    context: codexLspRequestContext(),
  });
...
export async function runLspPostToolUseHook(
  ...
  const blocks = result.blocks.filter(({ diagnostics }) =>
    !isLspDaemonUnreachableDiagnostics(diagnostics)
  );

  if (blocks.length === 0) return "";

  const rawReason = blocks.map(formatDiagnosticBlock).join("\n\n");
  const reason = limitHookText(
    rawReason,
    hookFeedbackLimit(input.transcript_path),
  );
...
export async function runLspPostCompactHook(
...
function formatDiagnosticBlock(
  { filePath, diagnostics }: DiagnosticBlock,
): string {
