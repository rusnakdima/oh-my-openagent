...
export function extractCodexCommentCheckRequests(
...
export function parseCodexPostToolUseInput(
  input: string,
): CodexPostToolUseInput | undefined {
  let parsed: unknown;
  try {
...
function formatWarnings(
  ...
  return warnings
    .map((warning) =>
      `comment-checker found issues in ${warning.filePath}:\n${warning.message}`
    )
    .join("\n\n");
}
