...
export async function runCommentChecker(
  ...
  if (!binaryPath) {
    return {
      status: "missing",
      message:
        "comment-checker binary not found. Run npm install for the codex-comment-checker plugin.",
    };
  }
...
function resolvePackageBinary(binaryName: string): string | undefined {
  try {
    ...
    const packagePath = require.resolve(
      `${commentCheckerPackageName()}/package.json`,
    );
...
function isCommentCheckerPackage(
  value: unknown,
): value is { getBinaryPath: () => string } {
...
function appendOutput(
  ...
  if (remainingBytes > 0) {
    output.text += Buffer.from(chunk, "utf8").subarray(0, remainingBytes)
      .toString("utf8");
