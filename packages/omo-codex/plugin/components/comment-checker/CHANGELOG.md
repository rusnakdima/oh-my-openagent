...
- Restore `write`, `edit`, `multi_edit`, and `multiedit` PostToolUse coverage
...
- Treat the native checker binary as an optional dependency for unsupported
  platforms.
...
- Update plugin metadata, docs, and contributor guidance to describe hook-only
  behavior.
...
- `PostToolUse` hook for `apply_patch`, `write`, `edit`, and `multiedit` style
  tool calls.
...
- Codex plugin manifest, local MCP config, bundled skill, and GitHub repository
  metadata.
