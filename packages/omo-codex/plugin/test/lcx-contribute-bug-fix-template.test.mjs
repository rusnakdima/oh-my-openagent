...
test("#given complete bug-fix evidence #when creating a PR body #then evidence and machine label are emitted", async () => {
  ...
  const input = {
    ...
    reproductionLogs:
      "npm test -- --test-name-pattern sync-skills failed before the fix.",
    approach:
      "Add the missing shared skill source and sync the aggregate plugin.",
    confidence:
      "The failing sync test now passes and the aggregate skill matches the source.",
    risks: "Low risk; this changes only packaged skill instructions.",
    userVisibleBehaviorChanges:
      "Users can ask LazyCodex to contribute a bug-fix PR directly.",
    verification: [
      "npm test -- --test-name-pattern lcx-contribute-bug-fix",
      "npm run sync:skills",
    ],
  };
...
test("#given a JSON payload path #when running the PR body script #then markdown is written to the requested file", async () => {
  ...
  assert.equal(
    ...
    createLazyCodexBugFixPrBody(input),
  );
});
