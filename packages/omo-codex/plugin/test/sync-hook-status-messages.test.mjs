...
test("#given source package versions and component without hooks #when hook status messages sync #then source versions are preserved", async () => {
  ...
  await writeJson(join(root, "components", "comment-checker", "package.json"), {
    version: "0.1.1",
  });
  await writeJson(join(root, "components", "lsp", "package.json"), {
    version: "0.2.0",
  });
  await writeJson(join(root, "components", "git-bash", "package.json"), {
    version: "0.3.0",
  });
      ...
      PostToolUse: [
        {
          hooks: [
            ...
            },
            {
              type: "command",
              command:
                'node "${PLUGIN_ROOT}/components/lsp/dist/cli.js" hook post-tool-use',
  ...
  await writeJson(
    join(root, "components", "comment-checker", "hooks", "hooks.json"),
    {
      hooks: {
        PostToolUse: [
          {
  ...
  const lspHooks = await readJson(
    join(root, "components", "lsp", "hooks", "hooks.json"),
  );
  assert.equal(
    aggregateHooks.hooks.PostToolUse[0].hooks[0].statusMessage,
    "(OmO 0.1.0) Checking Comments",
  );
  assert.equal(
    aggregateHooks.hooks.PostToolUse[0].hooks[1].statusMessage,
    "(OmO 0.1.0) Checking LSP Diagnostics",
  );
  assert.equal(
    componentHooks.hooks.PostToolUse[0].hooks[0].statusMessage,
    "(OmO 0.1.1) Checking Comments",
  );
  assert.equal(
    lspHooks.hooks.PostToolUse[0].hooks[0].statusMessage,
    "(OmO 0.2.0) Checking LSP Diagnostics",
  );
});
