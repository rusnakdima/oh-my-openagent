...
test("#given a worktree-backed team has bound member threads #when guide and status render #then they include codex thread links", () => {
  ...
  try {
    ...
    const guide = readFileSync(
      `${teamDir(tempRoot, sessionId)}/guide.md`,
      "utf8",
    );
    const status = runTeam(tempRoot, "status", "--team", sessionId).stdout;
    const prompt =
      runTeam(tempRoot, "member-prompt", "--team", sessionId, "--id", "A")
        .stdout;

    // then - guide, status, and bootstrap prompt all surface the member's bound thread link and cwd
    assert.match(
      guide,
      /codex:\/\/threads\/019ef350-ee78-72a3-bd5e-e40cebc3d814/,
    );
    assert.match(
      guide,
      /codex:\/\/threads\/019ef2dd-5b99-7ae2-8461-ffa6ca309d23/,
    );
    assert.match(guide, /worktree `\/tmp\/review-worktree`/);
    assert.match(
      status,
      /link=codex:\/\/threads\/019ef350-ee78-72a3-bd5e-e40cebc3d814/,
    );
    assert.match(
      prompt,
      /codex:\/\/threads\/019ef350-ee78-72a3-bd5e-e40cebc3d814/,
