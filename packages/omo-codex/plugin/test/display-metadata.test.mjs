...
async function readOpenAiDisplayName(skillDir) {
  ...
  assert(
    ...
    `${basename(skillDir)} must include agents/openai.yaml`,
  );
...
test("#given Codex plugin hooks #when plugin metadata is inspected #then hook keys contain human-readable hook slugs", async () => {
  ...
  // then
  assert(
    Array.isArray(hookPaths),
    "plugin manifest hooks must list per-hook metadata files",
  );
  assert(
    hookPaths.length > 0,
    "plugin manifest must expose at least one hook file",
  );
  for (const hookPath of hookPaths) {
    ...
    assert.equal(
      commandHookCount,
      1,
      `${hookPath} should describe exactly one displayed hook`,
    );
  }
});
...
test("#given Codex plugin skills #when skill metadata is inspected #then every skill has an OmO display name", async () => {
  ...
  const skillDirs = skillEntries.filter((entry) => entry.isDirectory()).map((
    entry,
  ...
  for (const skillDir of skillDirs) {
    ...
    if (displayName !== expected) {
      invalidDisplayNames.push(
        `${basename(skillDir)}: expected ${expected}, got ${displayName}`,
      );
    }
  }
