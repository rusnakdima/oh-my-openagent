...
test("#given aggregate build scripts #when inspected #then component CLIs are bundled with Bun", async () => {
  // given
  const buildComponentsScript = await readFile(
    join(root, "scripts", "build-components.mjs"),
    "utf8",
  );
  ...
  // then
  assert.match(
    componentBuildScript,
    /"bun", \["build", entry, "--target", "node", "--format", "esm", "--outfile", output\]/,
  );
...
test("#given aggregate build scripts #when inspected #then npm subprocesses resolve on Windows", async () => {
  ...
  const buildBundledMcpRuntimesScript = await readFile(
    join(root, "scripts", "build-bundled-mcp-runtimes.mjs"),
    "utf8",
  );

  // when
  const installTimeBuildScripts = [
    buildComponentsScript,
    buildBundledMcpRuntimesScript,
  ].join("\n");
...
test("#given bundled MCP runtime builds #when a locked runtime has no node_modules #then npm ci bootstraps it before build", async () => {
  ...
  assert.match(
    buildBundledMcpRuntimesScript,
    /!existsSync\(join\(runtime\.packageRoot, "node_modules"\)\)/,
  );
});
