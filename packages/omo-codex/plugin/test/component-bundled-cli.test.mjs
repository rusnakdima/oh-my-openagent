...
test("#given required component CLI contracts #when workspaces are inspected #then every contract component is covered", async () => {
  ...
  // when
  const missingWorkspaceComponents = Object.keys(HOOK_EVENTS_BY_COMPONENT)
    .filter(
...
test("#given built workspace component CLIs #when import specifiers are inspected #then each CLI is self-contained except node builtins", async () => {
  ...
  for (const component of components) {
    ...
    const invalidForComponent = imports.filter((specifier) =>
...
test("#given built workspace component CLIs #when dynamically imported with hook argv and empty stdin #then each CLI loads without external module resolution", async () => {
  ...
  const failures = [];
  for (const component of components) {
    const result = smokeImportComponent(
      component,
      HOOK_EVENTS_BY_COMPONENT[component],
    );
    if (result.status !== 0) {
      failures.push(
        `${component}: exit=${result.status} stderr=${result.stderr.trim()}`,
      );
    }
  }
...
test("#given built MCP-only CodeGraph entries #when executed with a fake binary #then each entry is self-contained", () => {
  ...
  try {
    ...
    const serveResult = runCodegraphMcpEntry(
      "serve.js",
      tempRoot,
      fakeBinaryPath,
      invocationLogPath,
    );
    const cliResult = runCodegraphMcpEntry(
      "cli.js",
      tempRoot,
      fakeBinaryPath,
      invocationLogPath,
    ...
    assert.equal(
      serveResult.status,
      0,
      `serve.js stderr: ${serveResult.stderr}`,
    );
  ...
  try {
    ...
    for (const hookCase of cases) {
      ...
      assert.equal(
        result.status,
        0,
        `${hookCase.name} exited ${result.status} signal=${result.signal} error=${
...
test("#given bundled LSP hook CLI in installed layout #when diagnostics run #then it spawns sibling daemon target", async () => {
  ...
  try {
    ...
    writeFileSync(
      join(lspDist, "cli.js"),
      readFileSync(componentCliPath("lsp"), "utf8"),
    );
    cpSync(join(root, "..", "..", "lsp-daemon", "dist"), daemonDist, {
      recursive: true,
    });
    ...
    assert.equal(
      existsSync(
        join(
          daemonDir,
          `v${
            JSON.parse(readFileSync(join(daemonDist, "package.json"), "utf8"))
              .version
          }`,
          "daemon.log",
        ),
...
test("#given aggregate hook manifest #when command hooks are inspected #then component CLI invocation contract is unchanged", async () => {
  ...
  const commands = manifests.flatMap(({ hooks }) =>
    collectHookCommands(hooks.hooks)
  );
  const components = await workspaceComponents();
  ...
  const missingContracts = components.filter(
    (component) =>
      ...
      !commands.some((command) =>
        command.startsWith(
          `node "\${PLUGIN_ROOT}/components/${component}/dist/cli.js" hook `,
        )
      ),
  );
...
function runCodegraphMcpEntry(
  ...
  return spawnSync(process.execPath, [
    join(root, "components", "codegraph", "dist", entryName),
  ], {
    cwd: root,
    encoding: "utf8",
    env: {
...
function runHookCliAsync(component, event, input, tempRoot, extraEnv = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      ...
      reject(
        new Error(
          `${component} ${event} timed out after ${HOOK_CLI_TEST_TIMEOUT_MS}ms`,
        ),
      );
...
function smokeImportComponent(component, event) {
  ...
  const script = `
		...
		process.argv = [process.execPath, ${JSON.stringify(cliPath)}, "hook", ${
...
async function waitForProcessExit(pid) {
  ...
  while (processIsRunning(pid)) {
    if (Date.now() >= deadline) {
      throw new Error(`Timed out waiting for test daemon ${pid} to exit`);
    }
