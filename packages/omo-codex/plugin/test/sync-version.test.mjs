...
async function scaffoldPlugin(repoRoot, rootVersion) {
  ...
  await writeJson(join(repoRoot, "packages", "omo-codex", "package.json"), {
  ...
  await writeJson(join(pluginRoot, "components", "rules", "package.json"), {
    name: "@code-yeongyu/codex-rules",
    version: "0.1.0",
  });
  await writeJson(
    join(pluginRoot, "components", "rules", ".codex-plugin", "plugin.json"),
...
test("#given LAZYCODEX_RELEASE_VERSION env #when resolving authoritative version #then the env value wins", async () => {
  const version = await resolveAuthoritativeVersion({
    env: { LAZYCODEX_RELEASE_VERSION: "9.9.9" },
...
test("#given drifted Codex manifests #when syncing version from root #then all versioned manifests match and version-less files are left alone", async () => {
  ...
  for (
    const target of manifestTargets(pluginRoot, [
      join(pluginRoot, "components", "rules", "package.json"),
    ])
  ) {
    assert.equal(
      (await readJson(target)).version,
      "4.8.1",
      `expected ${target} to be stamped`,
    );
  }
  // the version-less component plugin.json must not gain a version field
  const versionlessManifest = await readJson(
    join(pluginRoot, "components", "rules", ".codex-plugin", "plugin.json"),
  );
