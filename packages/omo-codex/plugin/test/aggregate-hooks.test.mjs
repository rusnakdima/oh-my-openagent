...
async function readAggregateCommandHooks() {
  ...
  return manifests.flatMap(({ source, hooks }) =>
    collectCommandHooks(hooks, source)
  );
}
...
test("#given isolated components #when hooks are inspected #then commands stay inside component roots", async () => {
  ...
  assert.doesNotMatch(
    text,
    /codex-(comment-checker|lsp|rules|telemetry|ulw-loop|ultrawork)@/,
  );
...
test("#given aggregate Stop hooks #when inspected #then start-work continuation and ulw-loop resume are separate groups", async () => {
  ...
  assert.ok(
    stopCommands.some((command) =>
      command.includes("start-work-continuation/dist/cli.js")
    ),
  );
  assert.ok(
    stopCommands.some((command) =>
      command.includes('ulw-loop/dist/cli.js" hook stop')
    ),
  );
});
...
test("#given aggregate SubagentStop hooks #when inspected #then start-work and LazyCodex executor verifier are separate groups", async () => {
  ...
  // when
  const subagentStopGroups = manifests.filter(({ hooks }) =>
    hooks.hooks.SubagentStop
  ).flatMap(({ hooks }) => hooks.hooks.SubagentStop);
  const verifierGroups = manifests.flatMap(({ source, hooks }) =>
    collectCommandHooks(hooks, source)
  ).filter(
    (hook) =>
...
test("#given aggregate PostCompact hooks #when hooks are inspected #then LSP diagnostics cache reset is registered", async () => {
  ...
  const lspPostCompactHooks = commandHooks.filter(
    (hook) =>
      ...
      hook.handler.command ===
        'node "${PLUGIN_ROOT}/components/lsp/dist/cli.js" hook post-compact',
  );
...
test("#given aggregate hook commands #when inspected #then commands stay Node-based and platform-neutral", async () => {
  // given
  // when
  const commands = (await readAggregateCommandHooks()).map(({ handler }) =>
    handler.command
  );
  ...
  assert(
    commands.includes(
      'node "${PLUGIN_ROOT}/components/ultrawork/dist/cli.js" hook user-prompt-submit',
    ),
  );
  assert(
    !commands.some((command) =>
      command.includes("components/workflow-selector/")
    ),
  );
...
test("#given aggregate OMO plugin is enabled #when hooks are inspected #then shell guidance and ulw-loop guard are registered", async () => {
  ...
  // when
  const preToolUseGroups = manifests.filter(({ hooks }) =>
    hooks.hooks.PreToolUse
  ).flatMap(({ hooks }) => hooks.hooks.PreToolUse);
...
test("#given aggregate OMO plugin has a dedicated ultrawork trigger #when hooks are inspected #then ulw-loop does not duplicate ultrawork injection", async () => {
  ...
  const ulwLoopUserPromptHooks = commandHooks.filter(
    (hook) =>
      ...
      hook.handler.command ===
        'node "${PLUGIN_ROOT}/components/ulw-loop/dist/cli.js" hook user-prompt-submit',
  );
  ...
  assert(
    ulwLoopUserPromptHooks.every((hook) =>
      !hook.handler.command.includes("--with-ultrawork")
    ),
  );
});
...
test("#given aggregate SessionStart hooks #when inspected #then LazyCodex auto-update is registered", async () => {
  ...
  assert(
    sessionStartCommands.some((command) =>
...
test("#given aggregate SessionStart hooks #when inspected #then cold-start-prone hooks carry 15s timeout headroom", async () => {
  ...
  for (const hook of coldStartHooks) {
    assert.equal(
      hook.timeout,
      15,
      `${hook.source} must carry timeout 15 for cold-start headroom`,
    );
  }
});
...
test("#given aggregate PostToolUse hooks #when inspected #then CodeGraph init guidance is registered for CodeGraph tools", async () => {
  ...
  const codegraphPostToolUseHooks = commandHooks.filter(
    (hook) =>
      ...
      hook.handler.command ===
        'node "${PLUGIN_ROOT}/components/codegraph/dist/cli.js" hook post-tool-use',
  );

  // then
  assert.equal(codegraphPostToolUseHooks.length, 1);
  assert.equal(
    codegraphPostToolUseHooks[0]?.matcher,
    "^(codegraph[._].*|mcp__codegraph__.*)$",
  );
...
test("#given aggregate PostToolUse hooks #when inspected #then thread title hygiene is registered for created Codex threads", async () => {
  ...
  const threadTitleHooks = commandHooks.filter(
    (hook) =>
      hook.eventName === "PostToolUse" &&
      hook.handler.command ===
        'node "${PLUGIN_ROOT}/components/teammode/dist/cli.js" hook post-tool-use',
  );

  // then
  assert.equal(threadTitleHooks.length, 1);
  assert.equal(
    threadTitleHooks[0]?.matcher,
...
test("#given aggregate plugin packaging #when inspected #then hooks and compatibility sentinels stay Python-free", async () => {
  // given
  const hooksText = (await Promise.all(
    (await readAggregateHookManifests()).map(({ source }) =>
      readFile(join(root, source), "utf8")
    ),
  )).join("\n");
  const aggregateTestText = await readFile(
    join(root, "test/aggregate.test.mjs"),
    "utf8",
  );
