  ...
  for (const [eventName, groups] of Object.entries(config)) {
    ...
    groups.forEach((group, groupIndex) => {
      if (
        ...
        throw new TypeError(
          `Invalid hook group in ${source}:${eventName}:${groupIndex}`,
        );
      }
...
export function hookLocation(
  { source, eventName, groupIndex, handlerIndex, handler },
) {
  return `${source}:${eventName}:${groupIndex}:${handlerIndex}:${handler.command}`;
}

const SPAWN_AGENT_START =
  /(?:(?<receiver>\b[A-Za-z_]\w*)\s*\.\s*)?(?<callee>\bspawn_agent)\s*\(/g;
...
export async function listShippedSpawnPromptFiles() {
  const skillFiles = [
    ...
    ...(await collectFiles(
      ...
      (path) =>
        path.includes(`${sep}skills${sep}`) && basename(path) === "SKILL.md",
    )),
  ];
...
export function findSpawnAgentCalls(content) {
  ...
  for (const match of content.matchAll(SPAWN_AGENT_START)) {
    ...
    for (
      ...
      calls.push({
        call: content.slice(match.index, index + 1),
        index: match.index,
        receiver,
        callee,
      });
...
function spawnParameters(call) {
  ...
  for (let index = 0; index < tokens.length - 2; index += 1) {
    ...
    const isArgumentScope = key.parenthesisDepth === 0 &&
      key.bracketDepth === 0 && key.braceDepth <= 1;
    const isSameScope = separator.braceDepth === key.braceDepth &&
      separator.bracketDepth === key.bracketDepth &&
      separator.parenthesisDepth === key.parenthesisDepth &&
      value.braceDepth === key.braceDepth &&
    ...
    if (
      !isArgumentScope || !isSameScope || !isAdjacent ||
...
export function hasSpawnIsolationArgument(call) {
  return spawnParameters(call).some(
    ({ name, value }) =>
      (name === "fork_context" && value.type === "identifier" &&
        value.value === "false") ||
      (name === "fork_turns" && value.type === "string" &&
        value.value === "none"),
  );
}

export function findSpawnAgentCallsWithoutIsolation(content) {
  return findSpawnAgentCalls(content).filter(({ call }) =>
    !hasSpawnIsolationArgument(call)
  );
}
...
export function findSpawnAgentCallsWithUnsupportedParameters(content) {
  return findSpawnAgentCalls(content).flatMap((entry) => {
    ...
    const parameters = spawnParameters(entry.call)
      .filter(({ name, direct }) =>
        name === "model" || name === "reasoning_effort" ||
...
export const findShippedSpawnIsolationViolations = () =>
  findShippedSpawnViolations(findSpawnAgentCallsWithoutIsolation);
