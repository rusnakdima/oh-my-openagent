...
const REPO_MODEL_CATALOG_PATH = fileURLToPath(
...
const BUNDLED_EXPLORER_TOML =
  'description = "Explorer agent"\nmodel_reasoning_effort = "medium"\n';
const BUNDLED_METIS_TOML =
...
test("#given a config.toml that already declares [agents.explorer] at a different path (Orca mirror) #when the worker setup runs #then no second colliding registration is added for that role", async () => {
  await withSetupFixture(async (fixture) => {
    ...
    const config = await readConfig(fixture);
    assert.equal(
      config.match(/\[agents\.explorer\]/g)?.length,
      1,
      "exactly one [agents.explorer] block",
    );
    assert.ok(
...
test("#given an unwritable config.toml #when the worker setup runs #then it degrades naming config.toml and leaves the file untouched", async () => {
  await withSetupFixture(async (fixture) => {
    ...
    try {
      ...
      const configEntries = outcome.degraded.filter((entry) =>
...
test("#given win32 without Git Bash #when the worker setup runs #then it degrades instead of throwing and disables the git_bash MCP", async () => {
  await withSetupFixture(async (fixture) => {
    ...
    const gitBashEntries = outcome.degraded.filter((entry) =>
      entry.component === "git-bash"
    );
...
test("#given the shipped plugin model catalog #when the worker setup stamps reasoning config #then the bundled fallback has not drifted from it", async () => {
  await withSetupFixture(async (fixture) => {
    ...
    const config = await readConfig(fixture);
    assert.ok(
      config.includes(`model = ${JSON.stringify(catalog.current.model)}`),
    );
    assert.ok(
      config.includes(
        `model_context_window = ${catalog.current.model_context_window}`,
      ),
    );
    assert.ok(
      config.includes(
        `model_reasoning_effort = ${
          JSON.stringify(catalog.current.model_reasoning_effort)
        }`,
      ),
    );
    assert.ok(
      config.includes(
        `plan_mode_reasoning_effort = ${
          JSON.stringify(catalog.current.plan_mode_reasoning_effort)
        }`,
      ),
    );
  });
