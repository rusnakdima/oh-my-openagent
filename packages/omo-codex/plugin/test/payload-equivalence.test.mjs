...
async function readTelemetryCoreDefaultPostHogConstants() {
  const constantsSource = await readFile(
    join(telemetryCoreRoot, "src", "constants.ts"),
    "utf8",
  );
...
test("payload-equivalence: #given telemetry-core and component shim #when session_start is captured #then payload schema matches telemetry-core", async () => {
  const stateDir = mkdtempSync(
    join(tmpdir(), "omo-codex-payload-equivalence-"),
  );
  try {
    await readTelemetryCoreDefaultPostHogConstants();
    const componentPostHog = await import(
      "../components/telemetry/dist/posthog.js"
    );
    assert.equal(
      typeof componentPostHog.__createPluginPostHogForTesting,
      "function",
    );
    ...
    const expectedDistinctId = componentPostHog.getPostHogDistinctIdForTesting(
      createOsProvider(),
    );
...
test("payload-equivalence: #given built telemetry CLI #when inspected #then telemetry-core is bundled without runtime package imports", async () => {
  const bundle = await readFile(
    join(root, "components", "telemetry", "dist", "cli.js"),
    "utf8",
  );
  ...
  assert.doesNotMatch(
    bundle,
    new RegExp(`import\\(["']${TELEMETRY_CORE_PACKAGE}`),
  );
