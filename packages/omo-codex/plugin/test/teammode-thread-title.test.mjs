...
function addMember(
  tempRoot,
  sessionId,
  { id, name, focus, lens, deliverable },
) {
  const args = [
    "add-member",
    "--team",
...
test("#given two members with distinct names #when added #then each thread title is per-member and the two never collide", () => {
  ...
  try {
    ...
    addMember(tempRoot, "title-distinct", {
      id: "A",
      name: "app-server-lifecycle",
      focus: "app-server thread lifecycle",
      lens: "area",
    ...
    addMember(tempRoot, "title-distinct", {
      ...
      focus: "mailbox live-delivery path",
      lens: "ownership",
...
test("#given a member added without an explicit name #when state is read #then the title falls back to the focus, never a shared session name", () => {
  ...
  try {
    ...
    addMember(tempRoot, "title-fallback", {
      id: "A",
      focus: "installer config",
      lens: "area",
      deliverable: "x",
    });
    addMember(tempRoot, "title-fallback", {
...
test("#given a member name already exists #when add-member receives the same name with different spacing and case #then state is not partially mutated", () => {
  ...
  try {
    ...
    assert.match(
      result.stderr,
      /member name "app   server" duplicates "App Server"/,
    );
...
test("#given persisted team.json has duplicate member names #when a command loads the team #then stale state is rejected", () => {
  const tempRoot = createTeamRoot(
    "omo-codex-teammode-title-stale-duplicate-name-",
  );
  try {
    runTeam(
      tempRoot,
...
test("#given persisted team.json has duplicate thread titles with distinct member names #when bind-thread runs #then state is rejected before activation", () => {
  const tempRoot = createTeamRoot(
    "omo-codex-teammode-title-stale-duplicate-title-",
  );
  try {
    runTeam(
      tempRoot,
    ...
    assert.match(
      result.stderr,
      /member threadTitle "\[Legacy\] Alpha" duplicates "\[Legacy\] Alpha"/,
    );
    assert.deepEqual(
    ...
    assert.equal(
      persisted.log.some((entry) => entry.event === "bind-thread"),
      false,
    );
  } finally {
    cleanupTeamRoot(tempRoot);
