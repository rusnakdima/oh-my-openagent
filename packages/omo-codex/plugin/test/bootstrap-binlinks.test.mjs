...
async function writeVersionedRoot(
  root,
  version,
  { withRuntimeCli = false, withUlwLoopAliases = false } = {},
) {
  ...
  await writeFile(
    ...
    `${
      JSON.stringify({
        bin: { [COMPONENT_BIN_NAME]: "./dist/cli.js" },
        name: "@sisyphuslabs/toolbox",
      })
    }\n`,
  );
  await writeFile(
  ...
  if (withRuntimeCli) {
    ...
    await writeFile(
      join(pluginRoot, "dist", "cli", "index.js"),
      "console.log('omo');\n",
    );
    await mkdir(join(pluginRoot, "dist", "cli-node"), { recursive: true });
    await writeFile(
      join(pluginRoot, "dist", "cli-node", "index.js"),
      "console.log('omo node runtime');\n",
    );
  }
  return pluginRoot;
}
...
async function assertComponentBinPointsTo(binDir, target, message) {
  if (process.platform !== "win32") {
    assert.equal(
      await readlink(join(binDir, COMPONENT_BIN_NAME)),
      target,
      message,
    );
    return;
  ...
  const shim = await readFile(
    join(binDir, `${COMPONENT_BIN_NAME}.cmd`),
    "utf8",
  );
  assert.ok(shim.includes(target), message);
  await assert.rejects(
    () => lstat(join(binDir, COMPONENT_BIN_NAME)),
    "win32 must not leave a posix component link behind",
  );
}
...
test("#given two versioned plugin roots #when the worker setup runs against each in turn #then bin links re-point to the new root and none resolve under the old", async () => {
  await withBinLinkFixture(async (fixture) => {
    ...
    assert.equal(
      targets.get(COMPONENT_BIN_NAME),
      join(rootV2, "components", "toolbox", "dist", "cli.js"),
    );
    for (const [name, target] of targets) {
      assert.ok(
        !target.startsWith(`${rootV1}${sep}`),
        `${name} still resolves under the old versioned root: ${target}`,
      );
    }
...
test("#given a completed v1 marker #when the worker runs against a v2 root #then the version change re-runs setup and re-points stale links", async () => {
  await withBinLinkFixture(async (fixture) => {
    ...
    const firstRun = await runBootstrapWorker({
      argv,
      env: workerEnv(rootV1),
      platform: process.platform,
    });
    assert.equal(firstRun.ran, true);

    const repeatRun = await runBootstrapWorker({
      argv,
      env: workerEnv(rootV1),
      platform: process.platform,
    });
    ...
    const upgradeRun = await runBootstrapWorker({
      argv,
      env: workerEnv(rootV2),
      platform: process.platform,
    });
    assert.equal(
      upgradeRun.ran,
      true,
      "a changed completedForVersion marker must re-run the worker",
    );
    await assertComponentBinPointsTo(
      fixture.binDir,
      join(rootV2, "components", "toolbox", "dist", "cli.js"),
  ...
  await withBinLinkFixture(async (fixture) => {
    ...
    const outcome = await runWorkerSetup(
      setupOptions(fixture, pluginRoot, {
        env: {
          CODEX_LOCAL_BIN_DIR: fixture.binDir,
          OMO_CODEX_GIT_BASH_PATH: bashPath,
        },
        platform: "win32",
        resolveGitBash: () => ({
    ...
    const shim = await readFile(
      join(fixture.binDir, `${COMPONENT_BIN_NAME}.cmd`),
      "utf8",
    );
    assert.match(shim, /@echo off/);
    const componentEntrypoint = join(
      pluginRoot,
      "components",
      "toolbox",
      "dist",
    ...
    assert.match(
      shim,
      new RegExp(`"${escapeRegExp(componentEntrypoint)}" %\\*`),
    );
    assert.doesNotMatch(
      shim,
      new RegExp(`node "${escapeRegExp(componentEntrypoint)}" %\\*`),
    );
    const wrapper = await readFile(
      join(fixture.binDir, "omo-agent-toolkit.cmd"),
      "utf8",
    );
    assert.ok(wrapper.includes(join(pluginRoot, "dist", "cli", "index.js")));
    await assert.rejects(
      () => lstat(join(fixture.binDir, COMPONENT_BIN_NAME)),
      "win32 must not leave posix symlinks behind",
    );
  });
});
...
test("#given a legacy payload without root CLI dist #when the worker setup runs #then it records degraded omo-agent-toolkit and leaves no broken link", async () => {
  await withBinLinkFixture(async (fixture) => {
    ...
    await assert.rejects(
      () => lstat(join(fixture.binDir, "omo")),
      "no omo wrapper may be written without dist/cli",
    );
    await assert.rejects(
      () => lstat(join(fixture.binDir, "omo.cmd")),
      "no Windows omo wrapper may be written without dist/cli",
    );
    await assertNoDanglingEntries(fixture.binDir);
    const log = await readFile(
      join(fixture.pluginData, "bootstrap", "bootstrap.log"),
      "utf8",
    );
    ...
    assert.equal(
      typeof warning,
      "string",
      `bootstrap.log warning must be a string, got: ${log}`,
    );
    assert.ok(
      warning.includes(
        "skipped the omo-agent-toolkit runtime wrapper because ",
      ) &&
        warning.includes(`${join("dist", "cli", "index.js")} is missing; `) &&
        warning.includes(
          "omo-agent-toolkit ulw-loop commands will be unavailable until a package shipping dist/cli is installed",
        ),
...
test("#given a payload shipping dist/cli #when the worker setup runs with no bin-dir override #then the omo-agent-toolkit wrapper lands in <codexHome>/bin without a degraded entry", async () => {
  await withBinLinkFixture(async (fixture) => {
    ...
    if (process.platform !== "win32") {
      assert.ok(
        (await stat(wrapperPath)).mode & 0o111,
        "the posix omo-agent-toolkit wrapper must be executable",
      );
      assert.equal(
      ...
      const shim = await readFile(
        join(defaultBinDir, `${COMPONENT_BIN_NAME}.cmd`),
        "utf8",
      );
      assert.ok(
        shim.includes(
...
test("#given ulw-loop aliases in the plugin payload #when worker setup links bins #then public ULW commands point at the current component without adding SessionStart hooks", async () => {
  await withBinLinkFixture(async (fixture) => {
    ...
    const sessionStartCommands = hooks.hooks.SessionStart.flatMap((group) =>
      ...
      .filter((command) =>
        command.includes("components/bootstrap/dist/cli.js")
      );
