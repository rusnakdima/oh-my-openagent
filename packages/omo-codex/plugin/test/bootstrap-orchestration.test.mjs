...
async function readStateFile(pluginData) {
  return JSON.parse(
    await readFile(join(pluginData, "bootstrap", "state.json"), "utf8"),
  );
}
...
async function writeStateFile(pluginData, state) {
  ...
  await writeFile(
    join(pluginData, "bootstrap", "state.json"),
...
test("#given a completed marker for the current version #when the hook runs #then it exits 0 silently without spawning", async () => {
  await withFixture(async (fixture) => {
    await writeStateFile(fixture.pluginData, {
...
test("#given a fresh bootstrap lock held by another process #when the hook runs #then it exits 0 without spawning", async () => {
  await withFixture(async (fixture) => {
    ...
    await writeFile(
      join(fixture.pluginData, "bootstrap", "state.json.lock"),
      `${Date.now()}\n`,
    );
    const recorder = hookRecorder();
...
test("#given PLUGIN_DATA or PLUGIN_ROOT missing #when the hook runs #then it exits 0 silently", async () => {
  await withFixture(async (fixture) => {
    ...
    const noData = await executeSessionStartHook({
      env: { PLUGIN_ROOT: fixture.pluginRoot },
      ...recorder,
    });
    const noRoot = await executeSessionStartHook({
...
test("#given the public hook runner #when every branch resolves #then it always returns exit code 0", async () => {
  await withFixture(async (fixture) => {
    ...
    assert.equal(
      await runSessionStartHook({ env: fixture.env, ...recorder }),
      0,
    );
...
test("#given both locks already held #when the worker starts #then it is refused without writing state", async () => {
  await withFixture(async (fixture) => {
    const locks = await bootstrapLocks({
      env: fixture.env,
    ...
    await assert.rejects(() =>
      stat(join(fixture.pluginData, "bootstrap", "state.json"))
    );
    await locks.release();
...
test("#given an unknown --only value #when the worker starts #then it rejects before steps or bootstrap artifacts", async () => {
  await withFixture(async (fixture) => {
    ...
    await Promise.all([
      ...
      assert.rejects(() =>
        stat(join(fixture.pluginData, "bootstrap", "bootstrap.log"))
      ),
      assert.rejects(() =>
        stat(join(fixture.pluginData, "bootstrap", "state.json.lock"))
      ),
      assert.rejects(() =>
        stat(join(fixture.pluginData, "auto-update.json.lock"))
      ),
    ]);
  });
});
