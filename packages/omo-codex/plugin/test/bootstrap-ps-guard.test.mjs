...
const BOOTSTRAP_AGGREGATE_HOOK_SOURCE =
  "hooks/session-start-checking-bootstrap-provisioning.json";
const HOOK_MANIFEST_SOURCES = [
  BOOTSTRAP_AGGREGATE_HOOK_SOURCE,
  "components/bootstrap/hooks/hooks.json",
];
const BOOTSTRAP_PS1_COMMAND_WINDOWS_TARGET =
  "\\components\\bootstrap\\scripts\\bootstrap.ps1";
...
const PWSH7_ONLY_TOKENS = [
  {
    token: "??",
    pattern: /\?\?/,
    reason: "null-coalescing operator requires pwsh 7",
  },
  {
    token: "?.",
    pattern: /\?\./,
    reason: "null-conditional member access requires pwsh 7",
  },
  {
    token: "-Parallel",
    pattern: /-Parallel\b/i,
    reason: "ForEach-Object -Parallel requires pwsh 7",
  },
];
...
test("#given bootstrap.ps1 #when scanned for pwsh-7-only syntax #then Windows PowerShell 5.1 can parse every line", async () => {
  ...
  for (const { token, pattern, reason } of PWSH7_ONLY_TOKENS) {
    for (const { line, text } of numberedLines(content)) {
      if (pattern.test(text)) {
        offenders.push(
          `pwsh-7-only token \`${token}\` (${reason}) at bootstrap.ps1:${line}: ${text.trim()}`,
        );
      }
    }
  }
  ...
  assert.deepEqual(
    offenders,
    [],
    "bootstrap.ps1 runs under Windows PowerShell 5.1 (System32); pwsh-7-only syntax breaks it",
  );
});
...
test("#given bootstrap.ps1 #when environment writes are scanned #then no Machine-scope target appears", async () => {
  ...
  const offenders = numberedLines(content)
    ...
    .map(({ line, text }) =>
      `Machine-scope token at bootstrap.ps1:${line}: ${text.trim()}`
    );
  ...
  assert.deepEqual(
    offenders,
    [],
    "bootstrap.ps1 must only touch User-scope environment values (Machine scope needs elevation)",
  );
});
...
test("#given bootstrap.ps1 #when dependency discovery misses #then it never invokes winget or mutates user PATH", async () => {
  ...
  assert.doesNotMatch(
    content,
    /&\s*winget\s+install/i,
    "bootstrap.ps1 must not install system dependencies with winget",
  );
  assert.doesNotMatch(
    content,
...
test("#given bootstrap.ps1 #when resolving Node #then Codex NODE_REPL_NODE_PATH is checked before PATH", async () => {
  ...
  assert.match(
    content,
    /NODE_REPL_NODE_PATH/,
    "bootstrap.ps1 must know how to read the Codex bundled Node setting",
  );
  assert(
    content.indexOf("NODE_REPL_NODE_PATH") <
      content.indexOf("Get-Command node"),
...
test("#given bootstrap.ps1 #when execution policy mutations are scanned #then Set-ExecutionPolicy is never invoked", async () => {
  ...
  const offenders = numberedLines(content)
    ...
    .map(({ line, text }) =>
      `Set-ExecutionPolicy at bootstrap.ps1:${line}: ${text.trim()}`
    );
  ...
  assert.deepEqual(
    offenders,
    [],
    "the hook line already passes -ExecutionPolicy Bypass; the script must not change policy",
  );
});
...
test("#given bootstrap.ps1 #when the final executable statement is located #then it is `exit 0` so sessions are never blocked", async () => {
  ...
  assert.equal(
    statements.at(-1),
    "exit 0",
    "the terminal statement of bootstrap.ps1 must be `exit 0`",
  );
});
...
test("#given bootstrap.ps1 #when every character is inspected #then the script is ASCII-only for cp949-safe consoles", async () => {
    ...
    for (const character of text) {
      ...
      if (codePoint > 0x7f) {
        offenders.push(
          ...
          } at bootstrap.ps1:${line}: ${text.trim()}`,
        );
      }
    }
  }
  ...
  assert.deepEqual(
    offenders,
    [],
    "bootstrap.ps1 output and source must be ASCII-only (cp949 consoles mangle anything else)",
  );
});
...
test("#given the aggregate and bootstrap component hook manifests #when commandWindows entries are read #then the bootstrap SessionStart hook launches bootstrap.ps1", async () => {
  ...
  for (const source of HOOK_MANIFEST_SOURCES) {
    // given
    const hooks = aggregateHooks.find((manifest) =>
    ...
    const launchers = collectCommandHooks(hooks, source)
      ...
      .filter((command) =>
        ...
        command.includes(BOOTSTRAP_PS1_COMMAND_WINDOWS_TARGET)
      );
    ...
    assert.equal(
      launchers.length,
      1,
      `${source} must register exactly one commandWindows pointing at bootstrap.ps1`,
    );
    assert.match(
      launchers[0],
