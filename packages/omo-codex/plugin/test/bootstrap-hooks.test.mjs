...
function collectAsyncOptIns(value, manifestPath, offenders) {
  if (Array.isArray(value)) {
    for (const entry of value) {
      collectAsyncOptIns(entry, manifestPath, offenders);
    }
    return offenders;
  }
...
async function readHookManifestsWithRoots() {
  const aggregate = (await readAggregateHookManifests()).map((
    { source, hooks },
  ) => ({ source, hooks, roots: [root] }));
  const components = (await readComponentHookManifests()).map((
    { source, hooks },
  ) => ({
    source,
    hooks,
...
function findBootstrapSessionStartHandlers(hooks) {
  const groups = Array.isArray(hooks.hooks?.SessionStart)
  ...
  for (const group of groups) {
    for (const handler of group.hooks ?? []) {
      ...
      if (!handler.command.includes("components/bootstrap/dist/cli.js")) {
        continue;
      }
...
test("#given every hooks.json under the plugin #when handlers are inspected #then no entry opts into unsupported async execution", async () => {
  ...
  // then
  assert(
    manifestPaths.length >= 2,
    "expected the aggregate and component hooks manifests to be discovered",
  );
  assert.deepEqual(
    offenders,
    [],
    `Codex skips async hooks silently; remove "async": true from: ${
      offenders.join(", ")
    }`,
  );
});
...
test("#given aggregate and component hook manifests #when command targets are resolved #then every command and commandWindows target exists", async () => {
  ...
  for (const manifest of manifests) {
    for (
      const { handler } of collectCommandHooks(manifest.hooks, manifest.source)
    ) {
      for (const targetParts of collectPluginRootTargets(handler)) {
        const candidates = manifest.roots.map((rootPath) =>
          join(rootPath, ...targetParts)
        );
        let found = false;
        ...
        if (!found) {
          missing.push(`${manifest.source}: ${targetParts.join("/")}`);
        }
      }
    }
  }
...
test("#given aggregate hook manifests #when command handlers are inspected #then each has a Windows dispatcher", async () => {
  ...
  assert.deepEqual(
    missing,
    [],
    `Aggregate hook commands need commandWindows dispatchers:\n${
      missing.join("\n")
    }`,
  );
});
...
test("#given the bootstrap component #when its SessionStart registration is inspected #then aggregate and component entries declare both platform commands", async () => {
  // given
  const aggregateHooks = (await readAggregateHookManifests()).find((
    { source },
  ) => source.endsWith("session-start-checking-bootstrap-provisioning.json"))
    ?.hooks;
  const componentHooks = await readJson(
    "components/bootstrap/hooks/hooks.json",
  );

  // when
  const aggregateEntries = findBootstrapSessionStartHandlers(
    aggregateHooks ?? {},
  );
  ...
  for (
    ...
    assert.equal(
      entries.length,
      1,
      `${label} hooks.json must register exactly one bootstrap SessionStart handler`,
    );
    const { group, handler } = entries[0];
    assert.equal(
      group.matcher,
      undefined,
      `${label} bootstrap SessionStart entry must be matcher-less`,
    );
    ...
    assert(
      handler.timeout <= 60,
      `${label} bootstrap timeout must stay <= 60 seconds`,
    );
...
test("#given non-bootstrap aggregate hook manifests #when commandWindows entries are read #then they launch the managed Node dispatcher", async () => {
  ...
  for (const manifest of aggregateHooks) {
    for (
      ...
      if (
        typeof handler.commandWindows !== "string" ||
        !handler.commandWindows.startsWith(EXPECTED_NODE_DISPATCH_PREFIX)
      ) {
        offenders.push(
          `${manifest.source}: ${handler.commandWindows ?? "<missing>"}`,
        );
      }
    }
  }
  ...
  assert.deepEqual(
    offenders,
    [],
    `Non-bootstrap aggregate hooks must use node-dispatch.ps1 on Windows:\n${
      offenders.join("\n")
    }`,
  );
});
...
test("#given the built bootstrap bundle #when its module references are inspected #then it depends on Node built-ins only", async () => {
  ...
  assert.equal(
    ...
    true,
    "components/bootstrap/dist/cli.js must exist after the plugin build",
  );
  const bundle = await readFile(bundlePath, "utf8");

  // when
  const externalSpecifiers = [
    ...[...bundle.matchAll(/\brequire\(["']([^"']+)["']\)/g)].map((match) =>
      match[1]
    ),
    ...[...bundle.matchAll(/\bfrom\s*["']([^"']+)["']/g)].map((match) =>
      match[1]
    ),
  ...
  assert.deepEqual(
    externalSpecifiers,
    [],
    "bootstrap dist must bundle everything except node: built-ins",
  );
});
