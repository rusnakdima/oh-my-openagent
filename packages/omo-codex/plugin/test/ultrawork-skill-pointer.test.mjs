...
function runUserPromptSubmitCli(component, extraArgs = []) {
  ...
  return spawnSync(
    process.execPath,
    [
      join(root, "components", component, "dist", "cli.js"),
      "hook",
      "user-prompt-submit",
      ...extraArgs,
    ],
...
test("#given the built ultrawork and ulw-loop CLIs #when both emit the pointer #then the payloads match byte for byte", () => {
  ...
  const ulwLoop = JSON.parse(
    runUserPromptSubmitCli("ulw-loop", ["--with-ultrawork"]).stdout,
  );
  assert.equal(
    ulwLoop.hookSpecificOutput.additionalContext,
    ultrawork.hookSpecificOutput.additionalContext,
  );
});
