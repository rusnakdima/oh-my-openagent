...
test("#given a bound team #when the member field manual renders #then it names the cross-thread tool and the team.json address book", () => {
  ...
  try {
    ...
    assert.match(
      ...
      "guide must name the cross-thread messaging tool",
    );
    // then - members are pointed at the concrete address book in team.json
    assert.match(
      guide,
      /leader\.sessionId/,
      "guide must say the leader thread id is team.json leader.sessionId",
    );
    assert.match(
      guide,
      /members\[\]\.threadId/,
      "guide must say peer thread ids are team.json members[].threadId",
    );
  } finally {
...
test("#given a member bootstrap trigger #when it is generated #then it names the cross-thread push tool", () => {
  ...
  try {
    ...
    assert.match(
      prompt,
      /codex_app\.send_message_to_thread/,
      "bootstrap must name the push tool",
    );
  } finally {
    cleanupTeamRoot(tempRoot);
  }
