...
test("#given suffixed spawn-like identifiers #when parsed #then they are not spawn calls", () => {
  const content =
    'auto_respawn_agent({agent_type: "explorer"})\nrespawn_agent(message="x")';
...
test("#given spawn parameter mutations and role-like prose #when parsed #then only unsupported machine arguments are rejected", () => {
  ...
  assert.deepEqual(
    findSpawnAgentCallsWithUnsupportedParameters(content).map((
      { parameters },
    ) => parameters),
    [["agent_type"], ["agent_type"], ["agent_type"], ["model"], [
      "reasoning_effort",
    ]],
  );
});
...
test("#given shipped generated skills and Hephaestus rules #when spawn calls are scanned #then every call disables parent context", async () => {
  ...
  const relativePaths = promptFiles.map((path) =>
  ...
  assert(
    relativePaths.some((path) =>
      path.startsWith("components/") && path.includes("/skills/") &&
      path.endsWith("/SKILL.md")
    ),
  );
  assert(
    relativePaths.some((path) =>
      path.startsWith("components/rules/bundled-rules/hephaestus/") &&
      path.endsWith(".md")
    ),
  );

  const violations = (await findShippedSpawnIsolationViolations()).map((
    { path, line, call },
  ) => ({
...
test("#given shipped generated skills and Hephaestus rules #when spawn parameters are scanned #then unsupported role and model arguments are absent", async () => {
  const violations = (await findShippedUnsupportedSpawnParameterViolations())
    .map(({ path, line, call, parameters }) => ({
      path: relative(root, path).split(sep).join("/"),
      line,
      call,
      parameters,
