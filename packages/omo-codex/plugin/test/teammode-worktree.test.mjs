...
test("#given a worktree team #when worktree-add A #then a real git worktree on a derived branch is created and recorded", () => {
  ...
  try {
    ...
    assert.equal(
      branchExists(tempRoot, expectedBranch),
      true,
      "member branch must exist",
    );
    assert.ok(
      worktreeBranches(tempRoot).includes(expectedBranch),
      "git worktree list must include the member branch",
    );
    ...
    const member = readTeamJson(tempRoot, "wt-add").members.find((m) =>
      m.id === "A"
    );
    assert.ok(
      existsSync(member.worktree.path) &&
        samePath(member.worktree.path, expectedPath),
      "worktree.path must point at the member worktree dir",
    );
    assert.equal(member.worktree.branch, expectedBranch);
    assert.ok(
      samePath(member.cwd, expectedPath),
      "cwd must be the member worktree dir",
    );
...
test("#given worktree-add ran #when the field manual is regenerated #then it shows the member's derived worktree branch", () => {
  ...
  try {
    ...
    const guide = readFileSync(
      join(teamDir(tempRoot, "wt-guide"), "guide.md"),
      "utf8",
    );
    assert.match(
      guide,
      /team\/wt-guide\/A/,
      "the member's row must show its derived worktree branch",
    );
  } finally {
    cleanupTeamRoot(tempRoot);
  }
...
test("#given a team initialized WITHOUT --worktree #when worktree-add is called mid-run #then it auto-enables isolation (conflict-triggered)", () => {
  ...
  try {
    ...
    assert.equal(
      readTeamJson(tempRoot, "wt-auto").worktree.enabled,
      false,
      "precondition: worktree starts disabled",
    );
    ...
    assert.equal(
      ...
      true,
      "worktree-add must flip the team into isolation mode",
    );
    assert.ok(
      worktreeBranches(tempRoot).includes("team/wt-auto/A"),
      "git must have created the member worktree",
    );
  } finally {
    cleanupTeamRoot(tempRoot);
  }
...
test("#given an existing member worktree #when worktree-add runs again #then it is a safe no-op that reports the existing worktree", () => {
  ...
  try {
    ...
    assert.match(
      second.stdout,
      /exist/i,
      "re-running must report the worktree already exists, not crash",
    );
    assert.equal(
      worktreeBranches(tempRoot).filter((b) => b === "team/wt-idem/A").length,
      1,
...
test("#given a member worktree #when worktree-remove A #then git drops it and team.json clears the path", () => {
  ...
  try {
    ...
    assert.ok(
      !worktreeBranches(tempRoot).includes("team/wt-rm/A"),
      "git worktree list must no longer include the removed worktree",
    );
    const member = readTeamJson(tempRoot, "wt-rm").members.find((m) =>
      m.id === "A"
    );
...
test("#given a member committed work in its worktree #when integrate --id A #then the base branch gets a NO-FF merge commit", () => {
  ...
  try {
    ...
    assert.notEqual(
      mergeCommits.trim(),
      "",
      "integrate must produce a merge commit on the base branch",
    );
    const tracked = git(tempRoot, "ls-files");
    assert.match(
      tracked,
      /alpha\.txt/,
      "the member's committed file must be present on the base branch",
    );
  } finally {
    cleanupTeamRoot(tempRoot);
  }
...
test("#given two members edited the same line #when integrate hits the conflict #then it stops, reports the conflicting member, and exits non-zero", () => {
  ...
  try {
    ...
    assert.notEqual(
      conflict.status,
      0,
      "a merge conflict must surface as a non-zero exit",
    );
...
test("#given a member branch that cannot be merged #when integrate runs #then it surfaces git's real reason, not a fake conflict", () => {
  ...
  try {
    ...
    writeFileSync(
      teamJsonPath(tempRoot, "wt-fatal"),
    ...
    assert.match(
      out,
      /could not integrate|not something we can merge|ghost-branch-404/i,
      "must surface git's actual reason",
    );
  } finally {
    cleanupTeamRoot(tempRoot);
  }
