...
function readJsonContract(workflow, contractName) {
  ...
  const pattern = new RegExp(
    `<!-- ${contractName} -->\\s*${fence}json\\s*([\\s\\S]*?)\\s*${fence}`,
  );
...
function matchesCas(state, expected, clauses) {
  return clauses.every((clause) => {
    ...
    return Object.hasOwn(expected, clause) &&
      state[clause] === expected[clause];
  });
}
...
for (const surface of surfaces) {
  test(`#given ${surface.name} #when deeper review becomes required before plan completion #then durable request state covers explicit and automatic review without inventing a digest`, async () => {
    ...
    const contract = readJsonContract(
      workflow,
    ...
    assert.equal(
      ...
      contract.pending_action_policy.review_required,
    );
    assert.deepEqual(Object.keys(contract.review).sort(), [
      "independent",
      "momus",
    ]);
  ...
  test(`#given a ${surface.name} review round #when launch, interruption, completion, or compaction occurs #then the durable transition table fails closed`, async () => {
    ...
    assert.deepEqual(contract.rejected_completions, [
      "duplicate",
      "late",
      "stale",
      "mismatched",
  ...
  test(`#given ${surface.name} round R2 replaced R1 #when a delayed R1 launch or receipt arrives #then identity-bound CAS leaves R2 unchanged`, async () => {
    ...
    assert.equal(
      matchesCas(currentRound, currentRound, contract.transitions.launch.cas),
      true,
    );
    assert.equal(
      matchesCas(currentRound, staleRound, contract.transitions.launch.cas),
      false,
    );

    const launchedCurrentRound = {
      ...currentRound,
      status: "launching",
      launch_id: "launch-r2",
    };
    assert.equal(
      matchesCas(
        launchedCurrentRound,
        launchedCurrentRound,
        contract.transitions.receipt.cas,
      ),
      true,
    );
    assert.equal(
    ...
    assert.equal(
      matchesCas(
        launchedCurrentRound,
        staleRound,
        contract.transitions.launch_interrupted.cas,
      ),
      false,
    );
  });
  ...
  test(`#given ${surface.name} independent reviewers #when exact-path retrieval drifts #then intake fails closed without alternate artifact recovery`, async () => {
    ...
    assert.equal(
      ...
      "<literal-canonical-review-workspace-root>",
    );
    ...
    assert.equal(
      contract.read_mechanism,
      "open_workspace_root_then_openat_no_follow_each_segment_fstat_read_hash",
    );
    ...
    assert.deepEqual(contract.required_receipt, [
