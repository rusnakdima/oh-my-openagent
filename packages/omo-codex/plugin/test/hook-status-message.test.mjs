...
const AGGREGATE_EXPECTED_LABELS = new Map([
  ...
  [
    "hooks/hooks.json:SubagentStop:1:0",
...
const COMPONENT_EXPECTED_LABELS = new Map([
  [
    "components/comment-checker/hooks/hooks.json:PostToolUse:0:0",
    "Checking Comments",
  ],
  [
    "components/lsp/hooks/hooks.json:PostToolUse:0:0",
    "Checking LSP Diagnostics",
  ],
  [
    "components/lsp/hooks/hooks.json:PostCompact:0:0",
    "Resetting LSP Diagnostics Cache",
  ],
  [
    "components/rules/hooks/hooks.json:SessionStart:0:0",
    "Loading Project Rules",
  ],
  [
    "components/rules/hooks/hooks.json:UserPromptSubmit:0:0",
    "Loading Project Rules",
  ],
  [
    "components/rules/hooks/hooks.json:PostToolUse:0:0",
    "Matching Project Rules",
  ],
  [
    "components/rules/hooks/hooks.json:PostCompact:0:0",
    "Resetting Project Rule Cache",
  ],
  [
    "components/telemetry/hooks/hooks.json:SessionStart:0:0",
    "Recording Session Telemetry",
  ],
  [
    "components/ultrawork/hooks/hooks.json:UserPromptSubmit:0:0",
    "Checking Ultrawork Trigger",
  ],
  [
    "components/ulw-loop/hooks/hooks.json:UserPromptSubmit:0:0",
    "Checking Ulw-Loop Steering",
  ],
  [
    "components/ulw-loop/hooks/hooks.json:PreToolUse:0:0",
    "Enforcing Unlimited Ulw-Loop Budget",
  ],
  [
    "components/start-work-continuation/hooks/hooks.json:Stop:0:0",
    "Checking Start-Work Continuation",
  ],
  [
    "components/start-work-continuation/hooks/hooks.json:SubagentStop:0:0",
    "Checking Start-Work Continuation",
  ],
  [
  ...
  [
    "components/git-bash/hooks/hooks.json:PreToolUse:0:0",
    "Recommending Git Bash MCP",
  ],
  [
    "components/git-bash/hooks/hooks.json:PostCompact:0:0",
    "Resetting Git Bash MCP Reminder",
  ],
]);
...
async function readComponentVersion(componentName) {
  return (await readJson(join("components", componentName, "package.json")))
    .version;
}
...
async function readComponentHookManifests() {
  const components = await readdir(join(root, "components"), {
    withFileTypes: true,
  });
  const manifests = [];
...
test("#given LazyCodex appears inside hook label #when normalizing #then product casing is preserved", async () => {
  ...
  assert.equal(
    message,
    `(OmO ${version}) Verifying LazyCodex Executor Evidence`,
  );
});
...
test("#given aggregate comment-checker hook #when status is inspected #then it uses OmO comments label", async () => {
  ...
  // when
  const hooks = aggregateManifests.flatMap((manifest) =>
    collectCommandHooks(manifest.hooks, manifest.source, manifest.version)
  );
  const commentCheckerHook = hooks.find((hook) =>
    hook.command.includes("components/comment-checker/dist/cli.js")
  );

  // then
  assert.equal(
    commentCheckerHook?.statusMessage,
    formatLazyCodexHookStatusMessage(
      commentCheckerHook?.version ?? "",
      "Checking Comments",
    ),
  );
  assert.doesNotMatch(
    JSON.stringify(aggregateManifests),
    /checking\s+OMO\s+comments/i,
  );
});
...
test("#given aggregate and component hooks #when status messages are inspected #then all use the OmO formatter", async () => {
  ...
  // when
  const commandHooks = [
    ...aggregateManifests.flatMap((manifest) =>
      collectCommandHooks(manifest.hooks, manifest.source, manifest.version)
    ),
    ...componentManifests.flatMap((manifest) =>
      collectCommandHooks(manifest.hooks, manifest.source, manifest.version)
    ),
  ];
  const expectedLabels = new Map([
    ...AGGREGATE_EXPECTED_LABELS,
    ...COMPONENT_EXPECTED_LABELS,
  ]);
  const mismatches = commandHooks
    .map((hook) => {
      const parsed = parseLazyCodexHookStatusMessage(hook.statusMessage);
      const label = parsed?.label;
      const expected = label === undefined
        ? undefined
        : formatLazyCodexHookStatusMessage(hook.version, label);
      return { ...hook, expected, parsed };
    })
    .filter((hook) =>
      hook.expected === undefined || hook.statusMessage !== hook.expected ||
      hook.parsed === null
    )
    .map((hook) =>
      `${hook.id}: expected ${
        hook.expected ?? "<missing expectation>"
      } but got ${hook.statusMessage}`
    );

  // then
  assert.deepEqual(mismatches, []);
  const actualLabels = new Set(
    commandHooks.map((hook) =>
      parseLazyCodexHookStatusMessage(hook.statusMessage)?.label
    ),
  );
  assert.deepEqual(
    [...expectedLabels.values()].filter((label) => !actualLabels.has(label)),
    [],
  );
