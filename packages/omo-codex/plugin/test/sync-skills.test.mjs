...
function excludeGeneratedSkillMetadata(files) {
  return files.filter((file) =>
    !generatedSkillMetadataFiles.has(file.replaceAll("\\", "/"))
  );
}
...
test("#given aggregate Codex skills #when source wiring is inspected #then shared skills are imported from the shared-skills package", async () => {
  ...
  // when
  const sharedSkillDependency = pluginPackageJson.dependencies
...
test("#given shared skill source tests #when aggregate Codex skills are synced #then source tests are not packaged", async () => {
  ...
  for (const skillName of expectedSkills) {
    for (
      const file of await listSkillFiles(join(aggregateSkillsRoot, skillName))
    ) {
      const normalized = file.replaceAll("\\", "/");
      const segments = normalized.split("/");
      const scriptsIndex = segments.lastIndexOf("scripts");
      const hasPythonTestDir = scriptsIndex !== -1 &&
        segments[scriptsIndex + 1] === "tests";
      const isSourceMetadata = normalized === ".gitignore" ||
        normalized === ".npmignore" || normalized === "pyrightconfig.json";
      if (
        normalized.endsWith(".test.ts") || hasPythonTestDir ||
        segments.includes("__pycache__") || normalized.endsWith(".pyc") ||
        isSourceMetadata
      ) {
...
test("#given packaged Codex ulw-plan surfaces #when inspected #then dangerous sandbox bypass guidance is not shipped", async () => {
  ...
  const dangerousBypassPattern = new RegExp(
    `${dangerousBypassToken}(?:-approvals-and-sandbox)?`,
  );
  ...
  const componentWorkflowPath = join(
    root,
    "components",
    "ultrawork",
    "skills",
  ...
  assert.doesNotMatch(
    ...
    dangerousBypassPattern,
    `${packagedWorkflow.path} ships unsafe Codex bypass guidance`,
  );
  assert.doesNotMatch(
    componentWorkflow.content,
    dangerousBypassPattern,
    `${componentWorkflow.path} ships unsafe Codex bypass guidance`,
  );
});
