...
test("#given app-thread archival is blocked by an ambiguous id #when archive runs with a note #then team state is archived and keeps the blocker in the log", () => {
  ...
  try {
    ...
    const blocker =
      "app-thread archive blocker: Ambiguous Codex thread id duplicate-thread-id; matching hosts: local, remote-ssh-discovered:m5";
    runTeam(
      tempRoot,
      "archive",
      "--team",
    ...
    assert.equal(
      team.log.some((entry) =>
        entry.event === "archive" && entry.detail === blocker
      ),
      true,
