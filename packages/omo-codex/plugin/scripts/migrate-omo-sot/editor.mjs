...
export function addCodexCodegraphValues(content, config, additions) {
  ...
  const codegraphRange = findObjectPropertyRange(
    content,
    codexRange.valueStart,
    "codegraph",
  );
  if (!isRecord(codegraph) || codegraphRange === null) {
    return insertPropertiesIntoObject(content, codexRange.valueStart, {
      codegraph: additions,
    });
  }
  return insertPropertiesIntoObject(
    content,
    codegraphRange.valueStart,
    additions,
  );
}
...
function insertPropertiesIntoObject(content, objectStart, properties) {
  ...
  const body = entries.map(([key, value]) =>
    `${childIndent}${JSON.stringify(key)}: ${
      formatJsoncValue(value, childIndent)
    }`
  ).join(",\n");
  const hasProperty = /"[^"]+"\s*:/.test(
    stripJsonComments(content.slice(open + 1, close)),
  );
  const beforeClose = content.slice(0, close).replace(/\s*$/, "");
  const suffix = content.slice(close);
  const comma =
    hasProperty && !beforeClose.endsWith(",") && !beforeClose.endsWith("{")
      ? ","
      : "";
...
function formatJsoncValue(value, indent) {
  ...
  const entries = Object.entries(value).map(([key, entry]) =>
    `${nextIndent}${JSON.stringify(key)}: ${
      formatJsoncValue(entry, nextIndent)
    }`
  );
  return `{\n${entries.join(",\n")}\n${indent}}`;
}
...
function valueRangeEnd(content, start) {
  ...
  let index = start;
  while (
    index < content.length && content[index] !== "," && content[index] !== "}"
  ) index += 1;
  return index;
}
