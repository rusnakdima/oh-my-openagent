...
export async function resolveAuthoritativeVersion(options = {}) {
  ...
  const explicit = normalizeVersion(
    options.version ?? env.LAZYCODEX_RELEASE_VERSION,
  );
  if (explicit.length > 0) return explicit;

  const repoRoot = options.repoRoot ?? defaultRepoRoot;
  const rootVersion = normalizeVersion(
    (await readJson(join(repoRoot, "package.json"))).version,
  );
  if (rootVersion.length === 0) {
    throw new Error(
      `Cannot resolve authoritative version: ${
...
export async function syncVersion(options = {}) {
  ...
  const version = options.version ??
    (await resolveAuthoritativeVersion({ ...options, version: undefined }));
...
if (isCliEntry(import.meta.url)) {
  ...
  console.log(
    `Synced OMO Codex manifests to version ${result.version} (${result.changed.length} updated)`,
  );
}
