...
const sharedSkillsScripts = join(
  repoRoot,
  "packages",
  "shared-skills",
  "scripts",
);
const materializeScript = join(
  sharedSkillsScripts,
  "materialize-frontend-refs.mjs",
);
...
function initSubmodules({ strict }) {
  try {
    ...
    const message =
      `[materialize-shared-upstreams] git submodule init failed: ${
...
export async function materializeSharedUpstreams({ strict }) {
  ...
  const { materializeFrontendRefs } = await import(
    pathToFileURL(materializeScript).href
  );
...
if (isCliEntry(import.meta.url)) {
  ...
  if (process.env.OMO_SKIP_MATERIALIZE === "1") {
    ...
    const strict = process.env.OMO_MATERIALIZE_STRICT === "1" ||
