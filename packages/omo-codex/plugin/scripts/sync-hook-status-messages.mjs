  ...
  for (const groups of Object.values(hooksJson.hooks)) {
    for (const group of groups) {
      for (const hook of group.hooks) {
        ...
        hook.statusMessage = formatLazyCodexHookStatusMessage(
          versionForCommand(hook.command),
          label,
        );
      }
    }
...
function readReleaseVersion(options) {
  const releaseVersion = normalizeReleaseVersion(
    options.releaseVersion ?? process.env.LAZYCODEX_RELEASE_VERSION,
  );
...
export async function syncHookStatusMessages(root = defaultRoot, options = {}) {
  ...
  const aggregateVersion = releaseVersion ??
    (await readPackageVersion(join(root, ".codex-plugin", "plugin.json")));
  ...
  for (const componentName of componentNames) {
    const componentVersion = releaseVersion ??
      (await readPackageVersion(
        join(root, "components", componentName, "package.json"),
      ));
