...
export function resolveLazyCodexUpdatePlan(
  {
    currentVersion,
    latestVersion,
    command = DEFAULT_UPDATE_COMMAND,
    args = DEFAULT_UPDATE_ARGS,
  } = {},
) {
...
export function detectAutoUpdateInstallFlow(env) {
  return detectInstallFlow({
    pluginRoot: resolveAutoUpdatePluginRoot(env),
    env,
  });
}
...
export function resolveLatestVersion(env) {
  ...
  const timeout = parsePositiveInteger(
    env.LAZYCODEX_LATEST_VERSION_TIMEOUT_MS,
    DEFAULT_LATEST_VERSION_TIMEOUT_MS,
  );
  const invocation = resolveSpawnInvocation("npm", [
    "view",
    "lazycodex-ai",
    "version",
    "--silent",
...
function resolveInstallerBinDir(env, codexHome) {
  if (env.CODEX_LOCAL_BIN_DIR?.trim()) {
    return resolve(env.CODEX_LOCAL_BIN_DIR.trim());
  }
  const home = resolveHome(env);
  const defaultCodexHome = resolve(home, ".codex");
  const resolvedCodexHome = resolve(codexHome);
  return resolvedCodexHome === defaultCodexHome
    ? resolve(home, ".local", "bin")
    : join(resolvedCodexHome, "bin");
}
...
function missingManagedBinTargets({ env, marketplaceRoot }) {
  ...
  for (const name of names) {
    ...
    if (
      !isManagedCachedComponentTarget(target, marketplaceRoot) ||
      existsSync(target)
    ) continue;
