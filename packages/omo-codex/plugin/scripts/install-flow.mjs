...
export function resolveInstallSnapshotPath(env, pluginRoot) {
  if (env.LAZYCODEX_INSTALLED_VERSION_PATH?.trim()) {
    return env.LAZYCODEX_INSTALLED_VERSION_PATH;
  }
  return join(pluginRoot, INSTALL_SNAPSHOT_FILE);
}

export function detectInstallFlow({ pluginRoot, env = {} } = {}) {
  const snapshot = classifySnapshotPath(
    resolveInstallSnapshotPath(env, pluginRoot),
  );
  if (snapshot.kind === "file") {
    return { flow: "npx-local", reason: "install-snapshot-present" };
  }
  if (snapshot.kind !== "absent") {
    return {
      flow: "unknown",
      reason: `install-snapshot-${snapshot.kind}: ${snapshot.detail}`,
    };
  }
  if (
    isVersionedWorkspaceManifest(
...
function classifySnapshotPath(path) {
  try {
    ...
    return {
      kind: "unreadable",
      detail: "code" in error ? String(error.code) : "unknown-error",
    };
  }
}
