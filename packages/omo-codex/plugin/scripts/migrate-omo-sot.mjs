...
export async function migrateOmoSotConfig(
  { env = process.env, seed = false, configPath } = {},
) {
  const targetPath = configPath ??
    join(resolveHomeDir(env), ".omo", "config.jsonc");
  if (!(await pathExists(targetPath))) {
    ...
    const content = addCodexCodegraphValues(
      SCAFFOLD,
      parsedScaffold.value,
      additions,
    );
  ...
  if (!parsed.ok) {
    return {
      ...
      seeded: false,
      warnings: [
        `Could not parse ${targetPath}; left existing OMO SOT untouched: ${parsed.error.message}`,
      ],
    };
  }
...
function collectCodexAdditions(config, env) {
  ...
  const additions = {};
  addSupportedCodegraphValues(
    additions,
    recordAt(config, "codegraph"),
    codexCodegraph,
  );
