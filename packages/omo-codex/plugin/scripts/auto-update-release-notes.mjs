...
const LAZYCODEX_RELEASE_NOTE_PATTERN =
  /\b(lazycodex|omo-codex|codex|codex light|codex cli|codex marketplace)\b/i;
...
export function formatUpdateStartedNotice({ pendingNotice, releaseNotes }) {
  return [
    ...
    formatReleaseNotesForNotice({
      version: pendingNotice.toVersion,
      releaseNotes,
    }),
  ].join(" ");
}
...
export function formatMarketplaceFlowNotice({ updateContext, releaseNotes }) {
  const versionText = updateContext.shouldUpdate
    ? `A newer LazyCodex version is available: v${
      updateContext.currentVersion ?? "unknown"
    } -> v${updateContext.latestVersion}.`
  ...
  return [
    ...
    formatReleaseNotesForNotice({
      version: updateContext.latestVersion,
      releaseNotes,
    }),
  ].join(" ");
}

export function formatMarketplaceRepairStartedNotice(
  { pendingNotice, releaseNotes, repairReasons = [] },
) {
...
function formatRepairReason(reason) {
  ...
  if (reason.kind === "missing-marketplace-payload") {
    return "missing marketplace payload";
  }
  if (reason.kind === "invalid-marketplace-payload") {
    return "invalid marketplace payload";
  }
  if (
    reason.kind === "dangling-managed-bin" && typeof reason.binName === "string"
  ) {
...
function sanitizeBinName(binName) {
  return /^[a-z0-9][a-z0-9._-]{0,80}$/i.test(binName)
    ? binName
    : "managed command";
}
...
export async function resolveReleaseNotes({ env, latestVersion }) {
  ...
  if (
    env.LAZYCODEX_RELEASE_NOTES_DISABLED === "1" || latestVersion === undefined
  ) return undefined;
  const repos =
    env.LAZYCODEX_RELEASE_NOTES_REPOS?.split(",").map((repo) => repo.trim())
      .filter(Boolean) ?? RELEASE_NOTES_REPOS;
  const timeoutMs = parsePositiveInteger(
    env.LAZYCODEX_RELEASE_NOTES_TIMEOUT_MS,
    DEFAULT_RELEASE_NOTES_TIMEOUT_MS,
  );
...
function extractLazyCodexReleaseHighlights(releaseNotes) {
  ...
  for (const rawLine of releaseNotes.split("\n")) {
    ...
    if (inLazyCodexSection || LAZYCODEX_RELEASE_NOTE_PATTERN.test(line)) {
        ...
        try {
          ...
          resolve(
            typeof parsed.body === "string" && parsed.body.trim()
              ? truncateReleaseNotes(parsed.body)
              : undefined,
          );
        } catch (error) {
