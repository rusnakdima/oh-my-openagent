...
export async function migrateConfigFile(
  ...
  const afterContext7PlaceholderGuard = removeStaleContext7PlaceholderMcpServer(
    config,
  );
  ...
  const changed = reasoningApplied || multiAgentChanged ||
    multiAgentModeChanged || context7PlaceholderChanged || subagentLimitChanged;
  if (changed) {
...
function shouldApplyCatalog(config, catalog, previousState) {
  ...
  if (Object.keys(current).length === 0) {
    return { apply: true, reason: "empty" };
  }
