...
export function resolveAutoUpdatePlan(
  {
    env = process.env,
    now = Date.now(),
    lastCheckedAt,
  ...
  if (
    ...
    env.OMO_CODEX_AUTO_UPDATE_DISABLED === "1"
  ) {
    return { shouldRun: false, reason: "disabled" };
  }

  const flow = installFlow ?? detectAutoUpdateInstallFlow(env).flow;
  const marketplaceRepair = flow === "marketplace"
    ? detectMarketplaceLocalRepair(env)
    : undefined;
  const intervalMs = parsePositiveInteger(
    env.LAZYCODEX_AUTO_UPDATE_INTERVAL_MS,
    DEFAULT_INTERVAL_MS,
  );
  const successStatus = lastStatus === undefined || lastStatus === "success";
  if (
    marketplaceRepair?.needsRepair !== true && successStatus &&
    typeof lastCheckedAt === "number" && intervalMs > 0 &&
    now - lastCheckedAt < intervalMs
  ) {
    return { shouldRun: false, reason: "throttled" };
  }
  const retryIntervalMs = parsePositiveInteger(
    env.LAZYCODEX_AUTO_UPDATE_RETRY_INTERVAL_MS,
    DEFAULT_RETRY_INTERVAL_MS,
  );
  if (
    !successStatus && typeof lastAttemptedAt === "number" &&
    retryIntervalMs > 0 && now - lastAttemptedAt < retryIntervalMs
  ) {
  ...
  if (flow === "marketplace") {
    ...
    if (!repair.needsRepair) {
      return { shouldRun: false, reason: "marketplace-flow" };
    }
    const currentVersion = resolveCurrentVersion(env) ?? "unknown";
    const latestVersion = resolveLatestVersion(env) ?? currentVersion ??
      "latest";
    return {
      shouldRun: true,
  ...
  if (!updatePlan.shouldUpdate) {
    return { shouldRun: false, reason: updatePlan.reason };
  }
...
export async function runLazyCodexManualUpdate(
  { env = process.env, dryRun = false, log = console.log, runCommand } = {},
) {
...
export async function runAutoUpdateCheck({
  ...
  const migrationNotices = await runConfigMigration({
    env,
    sessionModel,
    requireSessionModel,
  });
  ...
  const state = await settlePendingNotice({
    env,
    now,
    statePath,
  ...
  if (installFlow.flow === "unknown") {
    await appendUpdateLog(env, now, "install-flow-unknown", {
      reason: installFlow.reason,
    });
  }
    ...
    if (plan.reason === "marketplace-flow") {
      ...
      const releaseNotes = updateContext.shouldUpdate
        ? await resolveReleaseNotes({
          env,
          latestVersion: updateContext.latestVersion,
        })
        : undefined;
      notices.push(
        formatMarketplaceFlowNotice({ updateContext, releaseNotes }),
      );
  ...
  const lockStaleMs = parsePositiveInteger(
    env.LAZYCODEX_AUTO_UPDATE_LOCK_STALE_MS,
    DEFAULT_LOCK_STALE_MS,
  );
  const lock = await acquireLock(
    resolveLockPath(env, statePath),
    now,
    lockStaleMs,
  );
  if (lock === null) {
  ...
  try {
    await appendUpdateLog(
      ...
      now,
      "started",
      plan.kind === "marketplace-local-repair"
    ...
    if (env.LAZYCODEX_AUTO_UPDATE_WAIT === "1") {
      ...
      if (status === 0) {
        ...
        await recordUpdateStartedNotice({
          env,
          now,
          notices,
          pendingNotice,
...
async function settlePendingNotice({ env, now, statePath, state, notices }) {
  ...
  if (
    current !== null && target !== null && compareVersions(current, target) < 0
  ) return state;
  ...
  if (current !== null && target !== null) {
    notices.push(
      `[LazyCodex] Auto-update completed: v${pendingNotice.fromVersion} -> v${pendingNotice.toVersion}. This session is already running the new version. Tell the user the auto-update was applied.`,
    );
...
async function recordUpdateStartedNotice(
  { env, now, notices, pendingNotice, releaseNotes, plan },
) {
  notices.push(
    plan.kind === "marketplace-local-repair"
      ? formatMarketplaceRepairStartedNotice({
  ...
  await appendUpdateLog(env, now, "notified", {
    kind: plan.kind === "marketplace-local-repair"
      ? "marketplace-local-repair-started"
      : "update-started",
    fromVersion: pendingNotice.fromVersion,
    toVersion: pendingNotice.toVersion,
  });
...
async function runConfigMigration(
  { env, sessionModel = null, requireSessionModel = false },
) {
  if (
    env.LAZYCODEX_CONFIG_MIGRATION_DISABLED === "1" ||
    env.OMO_CODEX_CONFIG_MIGRATION_DISABLED === "1"
  ) return [];
