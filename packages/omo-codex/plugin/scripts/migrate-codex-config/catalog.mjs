...
export const FALLBACK_CATALOG = {
  ...
  managedProfiles: [
    ...
    },
    {
      ...
      match: { model: "gpt-5.5", model_context_window: 272_000 },
    },
  ],
};

export async function readModelCatalog(env = process.env) {
  const catalogPath = env.LAZYCODEX_MODEL_CATALOG_PATH?.trim() ||
    join(dirname(fileURLToPath(import.meta.url)), "..", "model-catalog.json");
  try {
    return parseCatalog(JSON.parse(await readFile(catalogPath, "utf8"))) ??
      FALLBACK_CATALOG;
  } catch (error) {
...
function parseCatalog(value) {
  if (
    !isRecord(value) || !isRecord(value.current) ||
    !Array.isArray(value.managedProfiles)
  ) return null;
  if (typeof value.version !== "string" || !isReasoningProfile(value.current)) {
    return null;
  }
  const managedProfiles = [];
  for (const profile of value.managedProfiles) {
    if (
      !isRecord(profile) || typeof profile.version !== "string" ||
      !isRecord(profile.match)
    ) return null;
  ...
  return {
    ...
    managedProfiles,
    roles: isRecord(value.roles) ? value.roles : {},
  };
}
