...
const MULTI_AGENT_V2_THREAD_LIMIT_KEY =
...
export function forceDisableMultiAgentV2(config, options = {}) {
  ...
  const multiAgentVersion = options.multiAgentVersion !== undefined
  ...
  if (
    multiAgentVersion == null && !sessionModel && !readRootModel(normalized)
  ) {
    return normalized;
  }
...
export function resolveMultiAgentVersionFromConfig(config, options = {}) {
  ...
  const version = resolveMultiAgentVersionForModel(model, {
    ...options,
    modelsCachePath: options.modelsCachePath?.trim() ||
...
export function resolveMultiAgentVersionForModel(model, options = {}) {
  const cachePath = options.modelsCachePath?.trim() ||
    join(
...
function clearMultiAgentV2DisableForReservedSchema(config) {
  ...
  result = removeDottedMultiAgentV2Setting(
    result,
  ...
  const cleared = section.text
    ...
    .replace(
      /^\s*hide_spawn_agent_metadata\s*=\s*false[ \t]*(?:#[^\n]*)?\n?/gm,
      "",
    );
...
function setDottedMultiAgentV2Disable(config) {
  ...
  if (!featuresSection) {
    return replaceOrInsertRootTomlSetting(
      config,
      "features.multi_agent_v2.enabled",
      "false",
    );
...
function removeDottedMultiAgentV2Setting(config, key, expectedValue) {
  ...
  return removeTomlSectionSetting(
    config,
    featuresSection,
    `multi_agent_v2.${key}`,
    expectedValue,
...
function ensureManagedComment(config) {
  ...
  return config.slice(0, section.start) + MANAGED_DISABLE_COMMENT +
    config.slice(section.start);
}

function removeManagedDisableComments(config) {
  if (
    !config.includes(MANAGED_COMMENT_MARKER) &&
    !config.includes("Managed by LazyCodex: multi_agent_v2")
  ) {
    return config;
  }
  ...
  for (const line of lines) {
    ...
    if (
      ...
      (trimmed.includes(MANAGED_COMMENT_MARKER) ||
        ...
        trimmed.includes(
          "because enabling it fails every turn with HTTP 400",
        ) ||
...
function removeFeaturesShorthand(config) {
  ...
  const shorthandPattern =
    /^\s*multi_agent_v2\s*=\s*(?:true|false)[ \t]*(?:#[^\n]*)?[ \t]*\n?/m;
