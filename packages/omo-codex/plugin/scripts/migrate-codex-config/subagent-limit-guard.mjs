...
const CODEX_MULTI_AGENT_V2_THREAD_LIMIT_KEY =
...
export function ensureSubagentConcurrencyLimit(config, options = {}) {
  ...
  const v2Preferred =
    prefersMultiAgentV2(multiAgentVersion, options.sessionModel) ||
    isMultiAgentV2Enabled(config);
...
function hasModelEvidence(config, options) {
  const sessionModel = typeof options.sessionModel === "string"
...
function removeAgentsMaxThreads(config) {
  ...
  if (bodyLines.length === 0) {
    return config.slice(0, section.start) +
      config.slice(section.end).replace(/^\n+/, "");
  }
  return config.slice(0, section.start) + patched + config.slice(section.end);
}

function ensureMultiAgentV2ThreadLimit(config) {
  if (hasTomlSetting(config, CODEX_MULTI_AGENT_V2_THREAD_LIMIT_KEY)) {
    return config;
  }
  const section = findSection(config, CODEX_MULTI_AGENT_V2_HEADER);
  if (!section) {
