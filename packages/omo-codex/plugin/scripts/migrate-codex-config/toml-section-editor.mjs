...
export function hasTomlSetting(config, keyPath) {
  ...
  for (const line of config.split("\n")) {
    ...
    if (fullPath.every((part, index) => part === targetPath[index])) {
      return true;
    }
  }
  return false;
...
export function replaceOrInsertRootTomlSetting(config, keyPath, value) {
  ...
  if (match) {
    const commentIndex = findUnquotedComment(
      match.lineBody,
      match.assignmentIndex + 1,
    );
    const comment = commentIndex === -1
      ? ""
      : ` ${match.lineBody.slice(commentIndex).trimStart()}`;
    const replacement = `${
      match.lineBody.slice(0, match.assignmentIndex + 1)
    } ${value}${comment}${match.newline}`;
    return config.slice(0, match.offset) + replacement +
      config.slice(match.offset + match.line.length);
  }
...
export function replaceOrInsertTomlSectionSetting(
  config,
  section,
  keyPath,
  value,
  ...
  for (const line of lines) {
    ...
    if (assignmentIndex >= 0) {
      ...
      if (settingPath && tomlPathMatches(settingPath, targetPath)) {
        const replacement = replaceTomlAssignmentValue(
          line,
          assignmentIndex,
          value,
        );
        const patched = section.text.slice(0, offset) + replacement +
          section.text.slice(offset + line.length);
        return config.slice(0, section.start) + patched +
  ...
  for (const line of lines) {
    ...
    if (assignmentIndex >= 0) {
      ...
      if (settingPath && tomlPathMatches(settingPath, targetPath)) {
        ...
        if (
          lineBody.slice(assignmentIndex + 1, valueEnd).trim() !== expectedValue
        ) return config;
        const patched = section.text.slice(0, offset) +
          section.text.slice(offset + line.length);
        return config.slice(0, section.start) + patched +
          config.slice(section.end);
      }
...
export function removeRootTomlSetting(config, keyPath, expectedValue) {
  ...
  if (
    match.lineBody.slice(match.assignmentIndex + 1, valueEnd).trim() !==
      expectedValue
  ) return config;
  return config.slice(0, match.offset) +
    config.slice(match.offset + match.line.length);
}
...
function findTomlMultilineDelimiter(line, delimiter, startIndex) {
  ...
  while (index !== -1) {
    if (
      delimiter === "'''" || countPrecedingBackslashes(line, index) % 2 === 0
    ) return index;
