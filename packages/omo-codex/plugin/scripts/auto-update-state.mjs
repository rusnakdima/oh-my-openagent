...
export function resolveStatePath(env) {
  ...
  const dataRoot = env.PLUGIN_DATA?.trim() ||
    join(homedir(), ".local", "share", "lazycodex");
  return join(dataRoot, "auto-update.json");
}

export function resolveLogPath(env) {
  if (env.LAZYCODEX_AUTO_UPDATE_LOG_PATH?.trim()) {
    return env.LAZYCODEX_AUTO_UPDATE_LOG_PATH;
  }
  const dataRoot = env.PLUGIN_DATA?.trim() ||
    join(homedir(), ".local", "share", "lazycodex");
