...
const builtinModuleNames = new Set(
  builtinModules.filter((moduleName) => !moduleName.startsWith("_")),
);

const workspaceComponents = [];
for (const workspace of workspaces) {
  if (typeof workspace !== "string" || !workspace.startsWith("components/")) {
    continue;
  }
...
async function readStandaloneComponentNames() {
  const entries = await readdir(join(root, "components"), {
    withFileTypes: true,
  });
  return entries
    .filter((entry) =>
      entry.isDirectory() && !workspaceSet.has(`components/${entry.name}`)
    )
...
async function hasBuildScript(relativePath) {
  ...
  try {
    manifest = JSON.parse(
      await readFile(join(root, relativePath, "package.json"), "utf8"),
    );
  } catch (error) {
...
async function bundleCli(workspace) {
  ...
  await runCaptured(
    ...
    root,
    `Bundling ${workspace}/dist/cli.js`,
  );
...
async function hasComponentOwnedBundle(workspace) {
  try {
    const manifest = JSON.parse(
      await readFile(
        join(root, workspace, "dist", ".omo-runtime-manifest.json"),
        "utf8",
      ),
    );
