...
test("#given default CODEX_HOME #when resolving local installer bin dir without override #then preserves user local bin precedence", () => {
  ...
  assert.equal(
    resolveCodexInstallerBinDir({ codexHome, env: {}, homeDir }),
...
test("#given custom CODEX_HOME and PATH without omo #when installing locally without bin override #then bootstraps via local CLI when omo is absent", async () => {
  ...
  const wrapper = await readFile(
    join(codexHome, "bin", "omo-agent-toolkit"),
    "utf8",
  );
    ...
    new RegExp(
      escapeRegExp(
        escapePosixDoubleQuoted(
          toPosixPath(join(repoRoot, "dist", "cli", "index.js")),
        ),
      ),
    ),
  );
...
test("#given repoRoot without root CLI dist #when installing locally #then warns about the skipped omo runtime wrapper", async () => {
  ...
  await assert.rejects(
    readFile(join(codexHome, "bin", "omo-agent-toolkit"), "utf8"),
  );
...
test("#given omo plugin source #when inspecting identity #then uses sisyphuslabs omo metadata", async () => {
  ...
  assert.equal(
    ...
    "https://github.com/sisyphuslabs/omo",
  );
  assert.equal(
    manifest.interface.privacyPolicyURL,
    "https://github.com/sisyphuslabs/omo#privacy",
  );
  assert.equal(
    manifest.interface.termsOfServiceURL,
    "https://github.com/sisyphuslabs/omo#license",
  );
});
...
test("#given plugin hooks #when installing #then records trusted hook hashes", async () => {
  ...
  await writeFile(
    ...
    "console.log('plugin cli')\n",
  );
  await writeFile(
    join(pluginRoot, "dist", "cli.ps1"),
    "Write-Output 'plugin cli'\n",
  );
          ...
          hooks: [
            {
              ...
              commandWindows:
                'powershell -File "${PLUGIN_ROOT}\\dist\\cli.ps1" hook user-prompt-submit',
              timeout: 10,
  ...
  assert.match(
    config,
    /\[hooks\.state\."alpha@debug-marketplace:hooks\/hooks\.json:user_prompt_submit:0:0"\]/,
  );
  assert.match(
    config,
    /trusted_hash = "sha256:605b27c7b1f93c02aa2f8052fd9df870a221c3dc432795c48b223fe48afcebc0"/,
  );
});
