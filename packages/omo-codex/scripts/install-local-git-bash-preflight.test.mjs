...
test("#given Windows without Git Bash #when installing local marketplace #then rejects before marketplace or config mutation", async () => {
  const repoRoot = await mkdtemp(
    join(tmpdir(), "omo-codex-script-git-bash-missing-repo-"),
  );
  const codexHome = await mkdtemp(
    join(tmpdir(), "omo-codex-script-git-bash-missing-home-"),
  );
  const commands = [];
...
test("#given Windows without Git Bash #when installing local marketplace #then winget is not run and install stops with guidance", async () => {
  ...
  const codexHome = await mkdtemp(
    join(tmpdir(), "omo-codex-script-git-bash-no-auto-home-"),
  );

  await assert.rejects(
    withBundledLspRuntimeForTest(async () =>
      installMarketplaceLocally({
        repoRoot: process.cwd(),
        codexHome,
        binDir: await mkdtemp(
          join(tmpdir(), "omo-codex-script-git-bash-no-auto-bin-"),
        ),
        platform: "win32",
        gitBashResolver: () => {
          resolveCallCount += 1;
...
test("#given non-Windows install #when running installer #then winget is never called", async () => {
  ...
  const result = await withBundledLspRuntimeForTest(async () =>
    installMarketplaceLocally({
      repoRoot: process.cwd(),
      codexHome: await mkdtemp(
        join(tmpdir(), "omo-codex-script-git-bash-no-winget-home-"),
      ),
      binDir: await mkdtemp(
        join(tmpdir(), "omo-codex-script-git-bash-no-winget-bin-"),
      ),
      platform: "linux",
      runCommand: async (command, args, options) => {
        runCalls.push([command, ...args, options.cwd].join(" "));
  ...
  assert.equal(
    runCalls.some((command) => command.startsWith("winget ")),
    false,
  );
});
...
test("#given Windows env override resolves Git Bash #when installing local marketplace #then install continues", async () => {
  const result = await withBundledLspRuntimeForTest(async () =>
    installMarketplaceLocally({
      repoRoot: process.cwd(),
      codexHome: await mkdtemp(
        join(tmpdir(), "omo-codex-script-git-bash-home-"),
      ),
      binDir: await mkdtemp(join(tmpdir(), "omo-codex-script-git-bash-bin-")),
      platform: "win32",
      gitBashResolver: () => ({
        found: true,
        path: windowsGitBashPath,
        source: "env",
      }),
      runCommand: async () => {},
      log: () => {},
...
test("#given Windows env override in installer options #when no custom resolver is provided #then default resolver uses it", async () => {
  const codexHome = await mkdtemp(
    join(tmpdir(), "omo-codex-script-git-bash-env-home-"),
  );
  const gitBashPath = join(
    await mkdtemp(join(tmpdir(), "omo-codex-script-git-bash-env-")),
    "bash.exe",
  );
  ...
  const result = await withBundledLspRuntimeForTest(async () =>
    installMarketplaceLocally({
      ...
      codexHome,
      binDir: await mkdtemp(
        join(tmpdir(), "omo-codex-script-git-bash-env-bin-"),
      ),
      platform: "win32",
...
test("#given non-Windows local install #when resolver would fail #then installer keeps existing behavior", async () => {
  const codexHome = await mkdtemp(
    join(tmpdir(), "omo-codex-script-git-bash-linux-home-"),
  );
  const result = await withBundledLspRuntimeForTest(async () =>
    installMarketplaceLocally({
      repoRoot: process.cwd(),
      codexHome,
      binDir: await mkdtemp(
        join(tmpdir(), "omo-codex-script-git-bash-linux-bin-"),
      ),
      platform: "linux",
      gitBashResolver: () => ({
        found: false,
