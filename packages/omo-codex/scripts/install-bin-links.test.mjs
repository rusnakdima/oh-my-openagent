...
async function writeRuntimeWrapperFixture({ withNodeCli = false } = {}) {
  ...
  const link = await linkRootRuntimeBin({
    binDir,
    codexHome,
    repoRoot,
    platform: "linux",
...
test("#given a marker-bearing legacy omo #when linking the root runtime #then deletes it", async () => {
  ...
  await assert.rejects(lstat(join(binDir, "omo")));
  assert.match(
    await readFile(join(binDir, "omo-agent-toolkit"), "utf8"),
    /OMO_GENERATED_RUNTIME_WRAPPER/,
  );
});
...
test("#given Windows platform #when writing the omo runtime wrapper #then embeds the node fallback chain", async () => {
  ...
  assert.ok(
    wrapper.indexOf("OMO_RUNTIME") < wrapper.indexOf("where bun"),
    "node override must precede bun discovery",
  );
});
...
test("#given Windows platform #when linking cached plugin bins #then writes command shims", async () => {
  ...
  assert.match(
    ...
    new RegExp(
      `"${
        join(pluginRoot, "dist", "cli.js").replace(
          /[.*+?^${}()|[\]\\]/g,
          "\\$&",
        )
      }" %\\*`,
...
test("#given existing custom Windows command shim #when linking bins #then rejects without overwriting", async () => {
  ...
  assert.match(
    await readFile(join(binDir, "alpha.cmd"), "utf8"),
    /echo custom/,
  );
});
...
test("#given managed legacy Codex component symlink #when linking bins #then removes stale symlink and writes OMO bin", async () => {
  ...
  assert.equal(
    await readlink(join(binDir, "omo-rules")),
...
test("#given managed legacy Codex LSP symlink #when linking bins #then removes stale lsp symlink", async () => {
  ...
  assert.equal(
    await readlink(join(binDir, "omo")),
    join(pluginRoot, "dist", "cli.js"),
  );
});
...
test("#given nested component declares reserved omo bin #when linking bins #then skips the nested top-level command", async () => {
  ...
  assert.equal(
    await readlink(join(binDir, "omo-ulw-loop")),
    join(componentRoot, "dist", "cli.js"),
  );
  assert.equal(
    await readlink(join(binDir, "ulw")),
    join(componentRoot, "dist", "cli.js"),
  );
  assert.equal(
    await readlink(join(binDir, "ulw-loop")),
    join(componentRoot, "dist", "cli.js"),
  );
});
