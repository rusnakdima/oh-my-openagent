...
test("#given packaged lazycodex adapter #when installing locally #then uses bundled artifacts without source builds", async () => {
  ...
  const cachedMcp = JSON.parse(
    await readFile(join(pluginCacheRoot, ".mcp.json"), "utf8"),
  );
  ...
  assert.equal(
    pluginCacheRoot,
    join(codexHome, "plugins", "cache", "sisyphuslabs", "omo", "0.1.2"),
  );
  ...
  assert.equal(
    cwd.startsWith(
      join(codexHome, "plugins", "cache", "sisyphuslabs", "omo", ".tmp-0.1.2-"),
    ),
    true,
  );
  const sotCommand = commands.find((entry) =>
    entry[1].includes("migrate-omo-sot.mjs")
  );
