...
test("#given external MCP package runtime #when installing cached plugin #then runtime is copied into the plugin cache", async () => {
  ...
  await writeJson(join(gitBashPackageRoot, "dist", "cli.js"), {
    executable: true,
  });
  await writeJson(join(lspPackageRoot, "dist", "cli.js"), { executable: true });
  await writeJson(join(lspPackageRoot, "dist", "lsp", "manager.js"), {
    copied: true,
  });
  ...
  const cachedMcp = JSON.parse(
    await readFile(join(result.path, ".mcp.json"), "utf8"),
  );
  const copiedGitBashCli = join(
    result.path,
    "components",
    "git-bash-mcp",
    "dist",
  ...
  assert.deepEqual(cachedMcp.mcpServers.git_bash.args, [
    copiedGitBashCli,
    "mcp",
  ]);
...
test("#given plugin-local MCP runtime #when installing cached plugin #then manifest args point at the cached plugin dist", async () => {
  ...
  assert.deepEqual(cachedMcp.mcpServers.omo.args, [
    join(result.path, "dist", "cli.js"),
  ]);
});
...
test("#given CodeGraph MCP runtime in plugin cache #when installing cached plugin #then manifest points at cached plugin before bootstrap", async () => {
  ...
  await writeJson(
    join(sourceRoot, "components", "codegraph", "dist", "serve.js"),
    { executable: true },
  );
  ...
  assert.deepEqual(cachedMcp.mcpServers.codegraph.args, [
    join(result.path, "components", "codegraph", "dist", "serve.js"),
  ]);
...
test("#given external MCP package not in the generated bundled runtime set #when installing cached plugin #then manifest args point at source", async () => {
  ...
  await writeJson(join(runtimePackageRoot, "dist", "cli.js"), {
    executable: true,
  });
  await writeJson(join(sourceRoot, "package.json"), {
    name: "@example/omo",
  ...
  assert.deepEqual(cachedMcp.mcpServers.language_tools.args, [
    join(runtimePackageRoot, "dist", "cli.js"),
    "mcp",
    join(sourceRoot, "..", "local-config.json"),
  ]);
...
test("#given packaged bundled MCP runtime has only dist files #when installing cached plugin #then runtime is copied into the plugin cache", async () => {
  ...
  assert.notEqual(
    ...
    join(lspPackageRoot, "dist", "cli.js"),
  );
});
