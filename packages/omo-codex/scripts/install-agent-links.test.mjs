...
test(
  "#given bundled agent roles and stale legacy links #when installing locally #then installs durable Codex agent files",
  {
    skip: process.platform === "win32"
      ? "test sets up a Unix legacy symlink fixture"
      : false,
  },
  async () => {
    ...
    const installedAgents = JSON.parse(
      await readFile(
        join(snapshotPluginPath, ".installed-agents.json"),
        "utf8",
      ),
    );
...
test(
  ...
  async () => {
    ...
    assert.equal(
      await readFile(join(snapshotRoot, ".git", "config"), "utf8"),
      '[remote "origin"]\n',
    );
    assert.equal(
      await readFile(
...
test("#given local sisyphuslabs install #when temporary marketplace snapshot is removed #then agent files still resolve from Codex home", async () => {
  ...
  assert.equal(
    await readFile(join(codexHome, "agents", "explorer.toml"), "utf8"),
    'name = "explorer"\n',
  );
  assert.equal(
...
test(
  ...
  async () => {
    ...
    const installedPlan = await readFile(
      join(codexHome, "agents", "plan.toml"),
      "utf8",
    );
    assert.ok(installedPlan.includes('model_reasoning_effort = "high"'));
    assert.equal(
      installedPlan.includes('model_reasoning_effort = "xhigh"'),
      false,
    );
...
test(
  ...
  async () => {
    ...
    const installedExplorer = await readFile(
      join(codexHome, "agents", "explorer.toml"),
      "utf8",
    );
...
test(
  ...
  async () => {
    ...
    assert.equal(
      snapshotPlan.includes('model_reasoning_effort = "high"'),
      false,
    );
  },
);
