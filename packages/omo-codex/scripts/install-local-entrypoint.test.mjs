...
test("#given lazycodex version flag #when running the Node installer entrypoint #then prints the package version", () => {
  ...
  const scriptPath = fileURLToPath(
    new URL("./install-local.mjs", import.meta.url),
  );
...
test(
  "#given lazycodex runs through an npm bin symlink #when running the Node installer entrypoint #then it still executes main",
...
test("#given dry-run cleanup path needs quoting #when running the Node installer entrypoint #then prints shell-safe command", () => {
  ...
  assert.equal(
    output,
    "npx --yes --package oh-my-openagent omo-agent-toolkit cleanup --platform=codex --project '/tmp/lazy codex'\\''s qa'",
  );
});
...
test("#given dry-run cleanup #when running the Node installer entrypoint #then prints delegated codex cleanup command", () => {
  ...
  assert.equal(
    output,
    "npx --yes --package oh-my-openagent omo-agent-toolkit cleanup --platform=codex --project /tmp/lazycodex-qa",
  );
});
...
test(
  "#given bun global lazycodex wrapper #when running update dry-run #then prints bun global update command",
  { skip: process.platform === "win32" },
  () => {
    // given
    const scriptPath = fileURLToPath(
    ...
    const tempHome = mkdtempSync(
      join(tmpdir(), "lazycodex-bun-global-dry-run-"),
    );
    ...
    try {
      ...
      assert.equal(
        output,
        "bun update -g lazycodex-ai@latest\nnpx --yes lazycodex-ai@latest install --no-tui --codex-autonomous",
      );
    } finally {
...
test(
  "#given bun global lazycodex wrapper and untrusted known scripts #when update runs noninteractively #then prints manual scoped trust command",
  { skip: process.platform === "win32" },
  () => {
    // given
    const scriptPath = fileURLToPath(
    ...
    try {
      ...
      assert.match(
        commandLog,
        /^npx --yes lazycodex-ai@latest install --no-tui --codex-autonomous$/m,
      );
      assert.doesNotMatch(commandLog, /^bun pm -g trust/m);
      assert.match(
        output,
        /bun pm -g trust oh-my-openagent @code-yeongyu\/comment-checker/,
      );
...
test("#given dry-run ulw-loop #when running the Node installer entrypoint #then prints delegated ulw-loop command", () => {
  ...
  assert.equal(
    output,
    "npx --yes --package oh-my-openagent omo-agent-toolkit ulw-loop help",
  );
});
