...
test("#given sisyphuslabs lazycodex install #when installing locally #then stamps the distribution version", async () => {
  ...
  await writeFile(
    ...
    "console.log('comment checker cli')\n",
  );
  ...
  const componentPackageJson = JSON.parse(
    await readFile(
      join(cacheRoot, "components", "ulw-loop", "package.json"),
      "utf8",
    ),
  );
  const hooks = JSON.parse(
    await readFile(join(cacheRoot, "hooks", "hooks.json"), "utf8"),
  );
  const componentHooks = JSON.parse(
    await readFile(
      join(cacheRoot, "components", "ulw-loop", "hooks", "hooks.json"),
      "utf8",
    ),
  );
  const snapshot = JSON.parse(
    await readFile(join(cacheRoot, "lazycodex-install.json"), "utf8"),
  );
  assert.equal(manifest.version, "4.7.6");
  assert.equal(packageJson.version, "4.7.6");
  assert.equal(componentPackageJson.version, "4.7.6");
  for (
    const statusMessage of [
