...
const legacyCodexPluginMarketplace = ["code", "yeongyu", "codex", "plugins"]
  .join("-");
...
test("#given local marketplace #when installing #then copies versioned plugins and enables config", async () => {
  ...
  await mkdir(join(codexPackageRoot, "shared-lsp", "dist"), {
    recursive: true,
  });
  ...
  await writeFile(
    join(codexPackageRoot, "shared-lsp", "dist", "cli.js"),
    "#!/usr/bin/env node\n",
  );
  await writeJson(join(pluginRoot, "package.json"), {
    ...
    scripts: {
      build:
        "node -e \"require('fs').writeFileSync('dist/cli.js', 'console.log(1)')\"",
    },
  ...
  await mkdir(
    join(codexHome, "plugins", "cache", "debug-marketplace", "stale", "0.1.0"),
    { recursive: true },
  );
  await writeFile(
  ...
  assert.equal(
    (await stat(join(alphaCacheRoot, "dist", "cli.js"))).isFile(),
    true,
  );
  assert.equal(
    await readlink(join(binDir, "alpha")),
    join(alphaCacheRoot, "dist", "cli.js"),
  );
  const alphaMcp = JSON.parse(
    await readFile(join(alphaCacheRoot, ".mcp.json"), "utf8"),
  );
  const sharedMcpCli = join(codexPackageRoot, "shared-lsp", "dist", "cli.js");
  assert.deepEqual(alphaMcp.mcpServers.alpha.args, [
    join(alphaCacheRoot, "dist", "cli.js"),
    "mcp",
  ]);
  ...
  const alphaPackageJson = JSON.parse(
    await readFile(join(alphaCacheRoot, "package.json"), "utf8"),
  );
  assert.equal(
    alphaPackageJson.dependencies["@example/shared-lsp"],
    `file:${join(codexPackageRoot, "shared-lsp")}`,
  );
  await assert.rejects(
    stat(join(alphaCacheRoot, "node_modules")),
    /code: 'ENOENT'|ENOENT/,
  );
  await assert.rejects(
  ...
  assert.match(
    config,
    /\[plugins\."alpha@debug-marketplace"\]\nenabled = true/,
  );
  assert.match(
    config,
...
test("#given sisyphuslabs marketplace #when installing #then registers the local built marketplace cache", async () => {
  ...
  await mkdir(join(repoRoot, "packages", "lsp-tools-mcp", "dist"), {
    recursive: true,
  });
  ...
  await writeFile(
    join(repoRoot, "packages", "lsp-tools-mcp", "dist", "cli.js"),
    "#!/usr/bin/env node\n",
  );
  ...
  assert.deepEqual(marketplace.plugins, [{
    name: "omo",
    source: { source: "local", path: "./omo/0.1.0" },
  }]);
  ...
  const snapshotMcp = JSON.parse(
    await readFile(join(snapshotPluginRoot, ".mcp.json"), "utf8"),
  );
  assert.equal(
  ...
  await assert.rejects(
    stat(
      join(codexHome, "plugins", "cache", legacyCodexPluginMarketplace, "omo"),
    ),
    /code: 'ENOENT'|ENOENT/,
  );
});
