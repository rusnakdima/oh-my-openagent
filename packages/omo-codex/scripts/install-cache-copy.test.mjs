...
test("#given cached file dependency is rewritten #when lockfile exists #then package lock stays in sync", async () => {
  ...
  assert.equal(
    cachedPackageJson.dependencies["@scope/lsp-tools"],
    `file:${sourceDependencyPath}`,
  );
  assert.equal(
    cachedPackageLock.packages[""].dependencies["@scope/lsp-tools"],
    `file:${sourceDependencyPath}`,
  );
  assert.deepEqual(cachedPackageLock.packages[packageLockDependencyPath], {
    name: "@scope/lsp-tools",
    version: "0.1.0",
  });
  assert.equal(
    cachedPackageLock.packages["node_modules/@scope/lsp-tools"].resolved,
    packageLockDependencyPath,
  );
});
...
test("#given existing cache #when npm install fails #then previous active cache is preserved", async () => {
  ...
  // then
  assert.equal(
    await readFile(join(cacheRoot, "package.json"), "utf8"),
    JSON.stringify({ name: "@scope/omo-old", version: "0.0.9" }),
  );
  const cacheParentEntries = await readdir(
    join(codexHome, "plugins", "cache", "debug", "omo"),
  );
