        ...
        hooks: [
          {
            type: "command",
            command:
              'node "${PLUGIN_ROOT}/components/git-bash/dist/cli.js" hook session-start',
          },
        ],
      },
      {
        ...
        hooks: [
          {
            type: "command",
            command:
              'node "${PLUGIN_ROOT}/scripts/auto-update.mjs" hook session-start',
          },
        ],
      },
    ],
        ...
        hooks: [
          {
            type: "command",
            command:
              'node "${PLUGIN_ROOT}/components/ulw-loop/dist/cli.js" hook stop',
          },
        ],
      },
    ],
...
async function createPluginRoot({ withGitBashDist }) {
  ...
  await writeFile(
    ...
    JSON.stringify(HOOKS_MANIFEST, null, "\t"),
  );
  const targets = [
...
async function createPluginRootWithHookArray({ withGitBashDist }) {
  ...
  await writeFile(
    ...
    JSON.stringify(
      {
        name: "omo",
        hooks: ["./hooks/session-start.json", "./hooks/stop.json"],
      },
      null,
      "\t",
    ),
  ...
  await writeFile(
    ...
    JSON.stringify(
      { hooks: { SessionStart: HOOKS_MANIFEST.hooks.SessionStart } },
      null,
      "\t",
    ),
  );
...
test("#given a hook command target missing from the payload #when scanning #then exactly that path is reported", async (t) => {
  ...
  assert.deepEqual(missing, [
    join(pluginRoot, "components/git-bash/dist/cli.js"),
  ]);
});
...
test("#given a Windows hook command target missing from the payload #when scanning #then the backslash path is reported", async (t) => {
  const pluginRoot = await createPluginRootWithWindowsHook({
    withBootstrapScript: false,
  });
  t.after(() => rm(pluginRoot, { recursive: true, force: true }));
...
test("#given hook manifests declared as an array #when scanning #then missing targets are reported", async (t) => {
  const pluginRoot = await createPluginRootWithHookArray({
    withGitBashDist: false,
  });
  t.after(() => rm(pluginRoot, { recursive: true, force: true }));
