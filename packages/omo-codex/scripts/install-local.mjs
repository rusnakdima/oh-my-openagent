...
const generatedEntrypointPath = join(
  scriptsDir,
  "install-dist",
  "install-local.mjs",
);
...
function ensureGeneratedInstaller() {
  ...
  if (result.status !== 0) {
    throw new Error(
      `bun run build:codex-install failed with exit code ${
...
async function loadGeneratedInstaller() {
  ...
  return import(
    new URL("./install-dist/install-local.mjs", import.meta.url).href
  );
}
...
function isEntrypointInvocation(invokedPath) {
  ...
  return resolveEntrypointPath(invokedPath) ===
    resolveEntrypointPath(entrypointPath);
}

const generatedInstaller = await loadGeneratedInstaller();

export const installMarketplaceLocally =
  generatedInstaller.installMarketplaceLocally;
export const resolveCodexInstallerBinDir =
  generatedInstaller.resolveCodexInstallerBinDir;
export const PASSTHROUGH_COMMANDS = generatedInstaller.PASSTHROUGH_COMMANDS;
export const buildDelegatedOmoInvocation =
  generatedInstaller.buildDelegatedOmoInvocation;
export const formatLazyCodexInstallHelp =
  generatedInstaller.formatLazyCodexInstallHelp;
export const parseLazyCodexInstallCliArgs =
  generatedInstaller.parseLazyCodexInstallCliArgs;
