...
test(
  "#given direct bun global lazycodex bin #when running update dry-run #then prints bun global update command",
...
function writeDirectBunGlobalLazyCodexBin(binPath, scriptPath) {
  const generatedInstallerUrl = pathToFileURL(
    fileURLToPath(
      new URL("./install-dist/install-local.mjs", import.meta.url),
    ),
  ).href;
  ...
  writeFileSync(
    binPath,
    `#!/usr/bin/env node
import { runLazyCodexInstallLocalCli } from ${
      JSON.stringify(generatedInstallerUrl)
    };
