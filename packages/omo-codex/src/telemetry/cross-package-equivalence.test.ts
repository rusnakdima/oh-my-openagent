...
function collectFiles(root: string, relativePrefix = ""): string[] {
  const files: string[] = [];
  for (
    const entry of readdirSync(join(root, relativePrefix), {
      withFileTypes: true,
    })
  ) {
...
describe("omo-codex telemetry single-source guard", () => {
  ...
  describe("#given the Codex telemetry component sources and package manifest", () => {
    it("#when inspected #then the component consumes telemetry-core instead of committed synced sources", () => {
      const componentPackageJson = readJson(
        join(COMPONENT_ROOT, "package.json"),
      );
      ...
      expect(
        srcFiles.filter((file) => SYNCED_COMPONENT_SOURCE_PATTERN.test(file)),
      ).toEqual([]);
      expect(readText(join(COMPONENT_ROOT, "src", "posthog.ts"))).toContain(
        TELEMETRY_CORE_PACKAGE,
      );
    });
  });
