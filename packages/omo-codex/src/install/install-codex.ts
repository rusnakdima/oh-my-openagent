...
import {
  type MarketplaceSnapshotPluginSource,
  writeInstalledMarketplaceSnapshot,
} from "./codex-marketplace-snapshot";
import {
...
const SISYPHUS_LEGACY_CACHE_MARKETPLACES = [
  "lazycodex",
  "code-yeongyu-codex-plugins",
] as const;

export async function runCodexInstaller(
  options: CodexInstallOptions = {},
): Promise<CodexInstallResult> {
  const env = options.env ?? process.env;
  const platform = options.platform ?? process.platform;
  const repoRoot = resolve(
    options.repoRoot ?? findRepoRoot({ importerDir: import.meta.dir, env }),
  );
  const codexHome = resolve(
    options.codexHome ?? env.CODEX_HOME ?? join(homedir(), ".codex"),
  );
  const projectDirectory = resolve(
    options.projectDirectory ?? env.OMO_CODEX_PROJECT ?? process.cwd(),
  );
  ...
  const gitBashResolution = await prepareGitBashForInstall({
    ...
    resolveGitBash: platform === "win32"
      ? (options.gitBashResolver ??
        (() => resolveGitBashForCurrentProcess({ platform, env })))
      : undefined,
  });
  ...
  const agentConfigs = new Map<
    string,
    { readonly name: string; readonly configFile: string }
  >();
  ...
  const legacyDaemonCleanup = await reapLspDaemons(codexHome).catch(
    (error: unknown) => {
  ...
  await updateCodexConfig({
    ...
    codegraphMcpEnabled: options.codegraphMcpEnabled ??
      resolveCodegraphNodeSupport({ env }).supported,
  ...
  for (let depth = 0; depth <= 7; depth += 1) {
    ...
    for (
      const wrapperPackageRoot of [
        join(current, "node_modules", "oh-my-openagent"),
        join(current, "oh-my-openagent"),
      ]
    ) {
...
function codexMarketplaceSource(
