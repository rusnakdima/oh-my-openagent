...
const INSTALL_CODEX_LEGACY_AGENT_PURGE_TIMEOUT_MS = process.platform === "win32"
  ? 60_000
  : 20_000;
...
const LEGACY_MANAGED_ULTRAWORK_REVIEWER_TOML =
...
describe("install-codex legacy agent purge", () => {
  test(
    "#given retired managed reviewer artifacts and user agents #when installing omo #then purges only managed legacy reviewer artifacts",
    async () => {
      // given
      const codexHome = await mkdtemp(
        join(tmpdir(), "omo-codex-home-retired-reviewer-"),
      );
      const binDir = await mkdtemp(
        join(tmpdir(), "omo-codex-bin-retired-reviewer-"),
      );
      ...
      await writeFile(
        ...
        LEGACY_MANAGED_ULTRAWORK_REVIEWER_TOML,
      );
      await writeFile(
        join(agentsDir, "user-custom.toml"),
        'name = "user-custom"\nmodel = "gpt-5.5"\n',
      );
      ...
      expect(await pathExists(join(agentsDir, "codex-ultrawork-reviewer.toml")))
        .toBe(false);
      expect(await readFile(join(agentsDir, "user-custom.toml"), "utf8")).toBe(
        'name = "user-custom"\nmodel = "gpt-5.5"\n',
      );
    },
  ...
  test(
    "#given custom file using retired reviewer filename #when installing omo #then preserves the unproven user TOML",
    async () => {
      // given
      const codexHome = await mkdtemp(
        join(tmpdir(), "omo-codex-home-custom-reviewer-"),
      );
      const binDir = await mkdtemp(
        join(tmpdir(), "omo-codex-bin-custom-reviewer-"),
      );
      const agentsDir = join(codexHome, "agents");
