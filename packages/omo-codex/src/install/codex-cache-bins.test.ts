...
async function createRepoFixture(): Promise<
  { repoRoot: string; binDir: string; codexHome: string }
> {
...
describe("linkRootRuntimeBin runtime wrapper parity", () => {
  ...
  it("#given a marker-bearing legacy omo wrapper #when writing the canonical wrapper #then deletes the legacy wrapper", async () => {
    ...
    // then
    await expect(lstat(join(fixture.binDir, "omo"))).rejects.toThrow();
    expect(await readFile(join(fixture.binDir, "omo-agent-toolkit"), "utf8"))
      .toContain("OMO_GENERATED_RUNTIME_WRAPPER");
  });
  ...
  it("#given posix platform #when writing the omo-agent-toolkit runtime wrapper #then embeds the node fallback chain", async () => {
    ...
    expect(wrapper.indexOf("OMO_RUNTIME")).toBeLessThan(
      wrapper.indexOf("command -v bun"),
    );
  });
  ...
  it("#given posix platform #when bun is absent everywhere #then the wrapper falls back to node before exiting 127", async () => {
    ...
    // then
    const wrapper = await readFile(link?.path ?? "", "utf8");
    const bunMissingBranch = wrapper.slice(
      wrapper.lastIndexOf('if [ -z "$BUN_BINARY" ]'),
    );
  ...
  it("#given win32 platform #when writing the omo runtime wrapper #then discovers Codex bundled Node from config before bare node", async () => {
    ...
    expect(wrapper).toContain(
      'if "%OMO_RUNTIME%"=="node" if defined OMO_NODE_BINARY if exist "',
    );
    expect(wrapper.indexOf("NODE_REPL_NODE_PATH")).toBeLessThan(
      wrapper.indexOf('if "%OMO_RUNTIME%"=="node"'),
    );
  ...
  posixOnly(
    "#given posix wrapper target was removed #when running omo #then exits with reinstall guidance",
    async () => {
      // given
      const fixture = await createRepoFixture();
      const link = await linkRootRuntimeBin({ ...fixture, platform: "linux" });
      ...
      // when
      const process = Bun.spawn([link.path], {
        env: { ...Bun.env, BUN_BINARY: "bun" },
        stderr: "pipe",
        stdout: "pipe",
      });
      ...
      expect(stderr).toContain(
        `omo-agent-toolkit: runtime target missing at ${
          join(fixture.repoRoot, "dist", "cli", "index.js")
        }`,
      );
      expect(stderr).toContain(
        "reinstall with: npx --yes lazycodex-ai@latest install --no-tui",
      );
    },
  );

  posixOnly(
    "#given ulw-loop command #when running omo wrapper #then preserves the ulw-loop token",
    async () => {
      // given
      const fixture = await createRepoFixture();
      const link = await linkRootRuntimeBin({ ...fixture, platform: "linux" });
      ...
      // when
      const process = Bun.spawn([link.path, "ulw-loop", "help"], {
        env: { ...Bun.env },
        stderr: "pipe",
        stdout: "pipe",
      });
...
describe("legacy omo reclamation across link kinds", () => {
  ...
  it("#given bin/omo is a dangling symlink #when writing the canonical wrapper #then completes without throwing", async () => {
    ...
    await mkdir(fixture.binDir, { recursive: true });
    await symlink(
      join(fixture.repoRoot, "missing-target.sh"),
      join(fixture.binDir, "omo"),
    );
