...
function uniqueRemovedKeys(
  configs: readonly ProjectLocalCodexConfigCleanup[],
): readonly LegacyAgentConflictKey[] {
...
> {
  ...
  const codexHomeConfigPath = codexHome === undefined
    ? null
    : join(resolve(codexHome), "config.toml");
  ...
  while (true) {
    ...
    if (await isRegularProjectLocalConfig(current, configPath)) {
      if (
        ...
        resolve(configPath) !== codexHomeConfigPath
      ) {
...
async function isRegularProjectLocalConfig(
  directory: string,
  configPath: string,
): Promise<boolean> {
...
function artifactRootsForConfigPaths(
...
async function collectProjectLocalArtifacts(
  projectRoots: readonly string[],
): Promise<readonly ProjectLocalCodexArtifact[]> {
