...
const WINDOWS_HOOK_KEY = String
  .raw`C:\Users\lenovo\hooks.json:session_start:0:0`;
...
describe("ensureHookTrusted", () => {
  test("#given single-quoted literal hook state table #when ensuring same Windows hook key #then updates in place", () => {
    // given
    const config = String
      .raw`[hooks.state.'C:\Users\lenovo\hooks.json:session_start:0:0']
...
`;
    ...
    expect(updated).toContain(
      String.raw`[hooks.state.'C:\Users\lenovo\hooks.json:session_start:0:0']`,
    );
