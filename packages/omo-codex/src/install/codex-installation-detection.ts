...
export type CodexInstallationSource =
  | "cli"
  | "mac-app"
  | "windows-standard-cli"
  | "windows-start-app";
type CodexInstallationPathSource = Exclude<
  CodexInstallationSource,
  "windows-start-app"
>;
...
export async function detectCodexInstallation(
  ...
  return missingCodexInstallation(
    checkedPaths,
    "Install OpenAI Codex CLI and make sure `codex` is on PATH.",
  );
}

export function formatCodexInstallationWarning(
  detection: Extract<CodexInstallationDetection, { readonly found: false }>,
): string {
  return [
...
function macCodexDmgPaths(homeDir: string): readonly string[] {
  ...
  return [
    posix.join(downloads, "codex.dmg"),
    posix.join(downloads, "Codex.dmg"),
  ];
}

function windowsCodexCliPaths(
  env: { readonly [key: string]: string | undefined },
): readonly string[] {
  const candidates: string[] = [];
  const explicitInstallDir = nonEmptyValue(env.CODEX_INSTALL_DIR);
  if (explicitInstallDir !== undefined) {
    candidates.push(win32.join(explicitInstallDir, "codex.exe"));
  }

  const localAppData = nonEmptyValue(env.LOCALAPPDATA) ??
    nonEmptyValue(env.LocalAppData);
