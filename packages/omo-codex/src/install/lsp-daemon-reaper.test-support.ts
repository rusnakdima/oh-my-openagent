...
function nodeBinary(): string {
  ...
  if (platform() === "win32") {
    const candidate = firstCommandLine("where.exe", ["node"]) ??
      firstCommandLine("which", ["node"]);
...
export function normalizeWindowsNodeCandidate(candidate: string): string {
  ...
  const nativePath = `${msysPath[1].toUpperCase()}:\\${
    msysPath[2].replaceAll("/", "\\")
  }`;
...
export function legacyEndpointFor(input: {
  ...
  if (input.kind === "windowsPipe") {
    return legacyWindowsPipeEndpoint(versionDir, input.version);
  }
  const digest = createHash("sha256").update(versionDir).digest("hex").slice(
    0,
    16,
  );
  return endpointPathJoin(
    ...
    `omo-lsp-${input.version}-${digest}.sock`,
  );
}
...
function legacyWindowsPipeEndpoint(
  versionDir: string,
  version: string,
): string {
  const digest = createHash("sha256").update(versionDir.replaceAll("/", "\\"))
...
export function startIdleNodeProcess(): SpawnedChild {
  ...
  return spawn(nodeBinary(), [scriptPath], {
    stdio: ["ignore", "pipe", "pipe"],
  });
}
...
export async function waitForChildReady(child: SpawnedChild): Promise<void> {
  await new Promise<void>((resolve, reject) => {
    const timeout = setTimeout(
      () => reject(new Error("legacy daemon fixture did not report ready")),
      5_000,
    );
    ...
    child.once("exit", (code, signal) => {
      ...
      reject(
        new Error(
          `legacy daemon fixture exited before ready (code=${
            code ?? "null"
          } signal=${signal ?? "null"})`,
        ),
      );
