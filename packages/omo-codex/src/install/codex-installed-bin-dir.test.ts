...
import {
  readInstalledCodexBinDir,
  writeInstalledCodexBinDir,
} from "./codex-installed-bin-dir";
...
describe("installed codex bin dir record", () => {
  ...
  test("#given a malformed or blank record #when it is read #then null is returned rather than a bad directory", async () => {
    ...
    await writeFile(
      join(malformedRoot, ".installed-bin-dir.json"),
      "{ not json",
    );

    const blankHome = await makeCodexHome("blank");
    const blankRoot = join(
      blankHome,
      "plugins",
      "cache",
      "sisyphuslabs",
    ...
    await writeFile(
      join(blankRoot, ".installed-bin-dir.json"),
