...
import {
  createRepoWithBuiltComponentBins,
  EXPECTED_OMO_COMPONENT_BINS,
  expectedBinName,
} from "./install-codex-test-fixtures";
...
describe("lazycodex install surface", () => {
  test(
    "#given codex installer #when installing omo #then exposes durable LazyCodex agent roles",
    async () => {
      // given
      const codexHome = await mkdtemp(
        join(tmpdir(), "omo-codex-home-lazycodex-agents-"),
      );
      const binDir = await mkdtemp(
        join(tmpdir(), "omo-codex-bin-lazycodex-agents-"),
      );
      const repoRoot = await createRepoWithBuiltComponentBins({
        agentNames: LAZYCODEX_AGENT_ROLE_NAMES,
      });
      ...
      for (const agentName of LAZYCODEX_AGENT_ROLE_NAMES) {
        ...
        expect(await readFile(agentFile, "utf8")).toContain(
          `name = "${agentName}"`,
        );
      }
    },
  ...
  test(
    "#given codex installer #when installing omo #then links documented component binaries to cached runtimes",
    async () => {
      // given
      ...
      expect(linkedNames).toEqual(
        EXPECTED_OMO_COMPONENT_BINS.map((entry) => expectedBinName(entry.name))
          .sort(),
      );
  ...
  test("#given installation guide #when component binaries and LazyCodex roles are documented #then docs match install surface", async () => {
    ...
    const installationGuide = await readFile(
      join(process.cwd(), "docs", "guide", "installation.md"),
      "utf8",
    );
    ...
    for (const staleName of STALE_CODEX_COMPONENT_BINS) {
      ...
      expect(installationGuide).not.toContain(
        `command not found: ${staleName}`,
      );
    }
  ...
  test(
    "#given Codex prunes an old plugin cache version #when LazyCodex roles were installed #then roles still resolve from Codex home",
    async () => {
      // given
      const codexHome = await mkdtemp(
        join(tmpdir(), "omo-codex-home-autoupdate-"),
      );
      ...
      await writeFile(
        join(marketplaceRoot, ".git", "config"),
        '[remote "origin"]\n',
      );
      await writeFile(
      ...
      if (pluginPath === undefined) {
        throw new Error(
          "Codex installer did not report an installed plugin path",
        );
      }
      ...
      expect(await readFile(agentPath, "utf8")).toBe(
        await readFile(snapshotAgentPath, "utf8"),
      );
      expect(await readFile(agentPath, "utf8")).toContain(
        `name = "${agentName}"`,
      );
      expect(await readFile(join(marketplaceRoot, ".git", "config"), "utf8"))
        .toBe('[remote "origin"]\n');
      expect(
        await readFile(
          join(marketplaceRoot, ".codex-marketplace-install.json"),
  ...
  test(
    "#given Codex temporary marketplace snapshot is removed #when LazyCodex roles were installed #then roles still resolve from Codex home",
    async () => {
      // given
      const codexHome = await mkdtemp(
        join(tmpdir(), "omo-codex-home-clean-snapshot-"),
      );
      const binDir = await mkdtemp(
        join(tmpdir(), "omo-codex-bin-clean-snapshot-"),
      );
      const repoRoot = await createRepoWithBuiltComponentBins({
        agentNames: LAZYCODEX_AGENT_ROLE_NAMES,
      });
      ...
      await rm(join(codexHome, ".tmp", "marketplaces", "sisyphuslabs"), {
        recursive: true,
        force: true,
      });
      await rm(join(codexHome, "plugins", "cache", "sisyphuslabs"), {
        recursive: true,
        force: true,
      });
      ...
      expect(
        await readFile(join(codexHome, "agents", `${agentName}.toml`), "utf8"),
      ).toContain(`name = "${agentName}"`);
    },
    { timeout: INSTALL_CODEX_INTEGRATION_TEST_TIMEOUT_MS },
  );
});
