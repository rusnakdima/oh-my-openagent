...
export interface SkippedCleanupPath {
  ...
  readonly reason:
    | "outside managed Codex cleanup scope"
    | "Codex home resolves to a filesystem root";
}
...
function isManagedBootstrapDriftPath(
  codexHome: string,
  target: string,
): boolean {
  ...
  return ownerName !== undefined && ownerName.startsWith("omo") &&
    ownerName.slice("omo".length).includes("sisyphuslabs");
}

function skipped(
  path: string,
  reason: SkippedCleanupPath["reason"],
): SkippedCleanupPath {
  return { path, reason };
}
