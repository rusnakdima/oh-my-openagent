...
describe("codex cache path helpers", () => {
  test("resolves the canonical OMO plugin cache root", () => {
    ...
    expect(cacheRoot).toBe(
      join(codexHome, "plugins", "cache", "sisyphuslabs", "omo"),
    );
  });
  ...
  test("resolves cached component CLI paths under a plugin version root", () => {
    ...
    const pluginRoot = join(
      tmpdir(),
      `omo-codex-plugin-root-${randomUUID()}`,
      "4.9.2",
    );
    ...
    // then
    expect(cliPath).toBe(
      join(pluginRoot, "components", "ulw-loop", "dist", "cli.js"),
    );
  });
  ...
  test("selects the newest cached component CLI from an OMO cache tree", () => {
    ...
    // when
    const cliPath = findNewestCachedCodexComponentCli({
      codexHome: root,
