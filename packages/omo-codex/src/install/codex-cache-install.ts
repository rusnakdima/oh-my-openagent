...
import {
  rewriteCachedManifestRoot,
  rewriteCachedMcpManifest,
} from "./codex-cache-mcp-manifest";
...
async function maybeRunNpmBuild(
  cwd: string,
  runCommand: RunCommand,
  env: NodeJS.ProcessEnv,
): Promise<void> {
...
async function maybeRunNpmSyncSkills(
  cwd: string,
  runCommand: RunCommand,
  env: NodeJS.ProcessEnv,
): Promise<void> {
...
function createTempSiblingPath(targetPath: string): string {
  return join(
    dirname(targetPath),
    `.tmp-${basename(targetPath)}-${process.pid}-${Date.now()}`,
  );
}
...
function createBackupSiblingPath(targetPath: string): string {
  return join(
    dirname(targetPath),
    `.backup-${basename(targetPath)}-${process.pid}-${Date.now()}`,
  );
}
...
const removedSparkshellReferencePattern =
  /\b(?:sparkshell|spark[-_\s]+shell)\b/i;
const removedSparkshellPromptSurfaceDirs = new Set([
  ".codex-plugin",
  "agents",
  "bundled-rules",
  "hooks",
...
const removedSparkshellPromptSurfaceFiles = new Set([
  "directive.md",
  "plugin.json",
]);
const removedSparkshellTextFilePattern = /\.(?:json|md|toml|ya?ml)$/i;

async function assertNoRemovedSparkshellPromptReferences(
  pluginRoot: string,
): Promise<void> {
  for (
...
async function listRemovedSparkshellPromptSurfaceFiles(
  ...
  const directory = relativeDirectory === ""
    ? pluginRoot
    : join(pluginRoot, relativeDirectory);
...
function shouldDescendIntoRemovedSparkshellPromptSurface(
  relativePath: string,
): boolean {
...
async function copyRootRuntimeDists(
  input: { readonly pluginRoot: string; readonly sourcePath: string },
): Promise<void> {
  ...
  for (const runtimePath of ["dist/cli", "dist/cli-node"] as const) {
    ...
    await mkdir(dirname(join(input.pluginRoot, runtimePath)), {
      recursive: true,
    });
    await cp(sourcePath, join(input.pluginRoot, runtimePath), {
      recursive: true,
    });
  }
}
