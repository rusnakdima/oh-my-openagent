...
export function removeStaleManagedAgentBlocks(
  config: string,
  keepAgentNames: Set<string>,
): string {
...
export function hasForeignAgentRegistration(
  config: string,
  agentConfig: CodexAgentConfig,
): boolean {
  const section = findTomlSection(
    config,
    `agents.${tomlKeySegment(agentConfig.name)}`,
  );
  if (!section) return false;
  return !section.text.includes(
    `config_file = ${JSON.stringify(agentConfig.configFile)}`,
  );
}

export function ensureAgentConfig(
  config: string,
  agentConfig: CodexAgentConfig,
): string {
