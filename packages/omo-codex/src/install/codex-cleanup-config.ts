...
import {
  parseAgentHeaderName,
  parseHookStateHeaderKey,
  parsePluginHeaderKey,
} from "./codex-config-toml-sections";

const MANAGED_MARKETPLACES = [
  "sisyphuslabs",
  "lazycodex",
  "code-yeongyu-codex-plugins",
] as const;

const LEGACY_MANAGED_CODEX_AGENT_NAMES_TO_PURGE = [
...
export function cleanupCodexLightConfigText(config: string): string {
  ...
  for (const marketplace of MANAGED_MARKETPLACES) {
    nextConfig = removeTomlSections(
      nextConfig,
      (header) => header === `marketplaces.${marketplace}`,
    );
    nextConfig = removeTomlSections(
      nextConfig,
      (header) => isManagedPluginHeader(header, marketplace),
    );
    nextConfig = removeTomlSections(
      nextConfig,
      (header) => isManagedHookStateHeader(header, marketplace),
    );
  }
...
export async function cleanupCodexConfig(
  configPath: string,
  now: (() => Date) | undefined,
): Promise<{
  ...
  const backupPath = `${configPath}.backup-${
    formatBackupTimestamp(now?.() ?? new Date())
  }`;
...
function isManagedHookStateHeader(
  header: string,
  marketplace: string,
): boolean {
...
function removeTomlSections(
  config: string,
  shouldRemove: (header: string) => boolean,
): string {
  return splitTomlSections(config)
    .filter((section) =>
      section.header === null || !shouldRemove(section.header)
    )
    .map((section) => section.text)
    .join("");
}

function splitTomlSections(
  config: string,
): Array<{ header: string | null; text: string }> {
  const lines = config.match(/[^\n]*\n?|$/g) ?? [];
  const sections: Array<{ header: string | null; text: string }> = [];
  let current: { header: string | null; text: string } = {
    header: null,
    text: "",
  };
