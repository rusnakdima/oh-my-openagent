...
describe("codex MultiAgentV2 config", () => {
  ...
  test("#given an inline-commented V2 thread cap #when updating config twice #then preserves the line and ordering byte-for-byte", async () => {
    ...
    expect(parsed.features.multi_agent_v2.max_concurrent_threads_per_session)
      .toBe(7);
    expect(sectionText(secondPass, "[features.multi_agent_v2]")).toBe(
      firstV2Section,
    );
  });
  ...
  for (
    const fixture of [
      ...
      },
      {
        ...
        lines: [
          "features.multi_agent_v2.max_concurrent_threads_per_session = 7 # user cap",
        ],
        preservedLine:
          "features.multi_agent_v2.max_concurrent_threads_per_session = 7 # user cap",
      },
    ...
    test(`#given installer config has a ${fixture.name} #when updating twice #then preserves the semantic cap without duplication`, async () => {
      ...
      const root = await mkdtemp(
        join(tmpdir(), "omo-codex-mav2-semantic-key-"),
      );
      const configPath = join(root, "config.toml");
      await writeFile(
        configPath,
        ['model = "gpt-5.4"', "", ...fixture.lines, ""].join("\n"),
      );
      const updateInput = {
        configPath,
        repoRoot: "/repo/packages/omo-codex",
      ...
      expect(parsed.features.multi_agent_v2.max_concurrent_threads_per_session)
        .toBe(7);
      expect(
        secondParsed.features.multi_agent_v2.max_concurrent_threads_per_session,
      ).toBe(7);
      expect(secondPass.indexOf(fixture.preservedLine)).toBe(
        firstPass.indexOf(fixture.preservedLine),
      );
      expect(secondPass.split(fixture.preservedLine)).toHaveLength(2);
      expect(secondPass).toContain(fixture.preservedLine);
      expect(secondPass).not.toMatch(
        /^max_concurrent_threads_per_session = 16$/m,
      );
      if (fixture.name === "root-qualified dotted cap key") {
        expect(secondPass).not.toContain("[features]");
      }
    });
  }
  ...
  test("#given multiline string contains V2 cap lookalikes #when updating config #then writes the absent semantic default", async () => {
    // given
    const root = await mkdtemp(
      join(tmpdir(), "omo-codex-mav2-multiline-lookalike-"),
    );
    const configPath = join(root, "config.toml");
    await writeFile(
      configPath,
  ...
  test("#given V2 section multiline value contains a cap lookalike #when updating config #then writes the absent default", async () => {
    // given
    const root = await mkdtemp(
      join(tmpdir(), "omo-codex-mav2-section-multiline-lookalike-"),
    );
    const configPath = join(root, "config.toml");
    await writeFile(
      configPath,
  ...
  test("#given root-dotted features and string lookalikes #when updating config #then replaces the semantic flag only", async () => {
    // given
    const root = await mkdtemp(
      join(tmpdir(), "omo-codex-mav2-root-feature-integrity-"),
    );
    const configPath = join(root, "config.toml");
    await writeFile(
      configPath,
  ...
  test("#given root-dotted plugin flag has a multiline value #when updating config #then replaces the whole semantic assignment", async () => {
    // given
    const root = await mkdtemp(
      join(tmpdir(), "omo-codex-mav2-root-feature-multiline-"),
    );
    const configPath = join(root, "config.toml");
    await writeFile(
      configPath,
    ...
    expect(
      parseToml(content).features.multi_agent_v2
  ...
  test("#given multiline string closes after an escaped quote #when updating config #then preserves the following explicit cap", async () => {
    // given
    const root = await mkdtemp(
      join(tmpdir(), "omo-codex-mav2-overlapping-closer-"),
    );
    const configPath = join(root, "config.toml");
    await writeFile(
      configPath,
