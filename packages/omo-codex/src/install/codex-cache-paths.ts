...
export function resolveCodexPluginCacheRoot(
  codexHome: string,
  input: CodexCacheIdentity = {},
): string {
  return join(
    codexHome,
    "plugins",
    "cache",
    input.marketplaceName ?? DEFAULT_CODEX_MARKETPLACE_NAME,
    input.pluginName ?? DEFAULT_CODEX_PLUGIN_NAME,
  );
}

export function resolveCachedCodexComponentCliPath(
  pluginRoot: string,
...
export function resolveCodexComponentBinCandidates(input: {
  ...
  return [
    input.env.CODEX_LOCAL_BIN_DIR
      ? join(input.env.CODEX_LOCAL_BIN_DIR, input.executableName)
      : undefined,
...
export function resolveCachedRuntimePath(
  pluginRoot: string,
  sourceRoot: string,
  runtimePath: string,
): string {
...
function compareVersionNames(left: string, right: string): number {
  ...
  for (let index = 0; index < length; index += 1) {
    const leftValue = Number.isFinite(leftParts[index] ?? Number.NaN)
      ? leftParts[index] ?? 0
      : 0;
    const rightValue = Number.isFinite(rightParts[index] ?? Number.NaN)
      ? rightParts[index] ?? 0
      : 0;
