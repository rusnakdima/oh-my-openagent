...
import {
  cleanupCodexConfig,
  MANAGED_CODEX_AGENT_NAMES,
} from "./codex-cleanup-config";
import {
  codexHomeResolvesToFilesystemRoot,
  validateManagedCleanupTarget,
} from "./codex-cleanup-safety";
...
export async function cleanupCodexLight(
  input: CodexCleanupOptions = {},
): Promise<CodexCleanupResult> {
  const env = input.env ?? process.env;
  const codexHome = resolve(
    input.codexHome ?? env.CODEX_HOME ?? join(homedir(), ".codex"),
  );
  ...
  const agentCleanup = await removeManifestListedAgentLinks(
    codexHome,
    agentPaths,
  );
  ...
  for (const path of managedStatePaths) {
    if (
      await removeManagedPathBestEffort(path, {
        codexHome,
        onSkip: (skip) => skippedPaths.push(skip),
      })
    ) {
  ...
  const projectDirectory = input.projectDirectory ?? env.OMO_CODEX_PROJECT ??
    process.cwd();
...
async function collectBootstrapDataDirsByGlob(
  codexHome: string,
...
async function walkForManagedBootstrapDirs(
  directory: string,
  depth: number,
  results: string[],
): Promise<void> {
...
function isManagedBootstrapOwnerName(name: string): boolean {
  return name.startsWith("omo") &&
    name.slice("omo".length).includes("sisyphuslabs");
}
...
export async function removeManagedPathBestEffort(
  ...
  const skip = validateManagedCleanupTarget({
    codexHome: seams.codexHome,
    path,
  });
  if (skip !== null) {
    seams.onSkip?.(skip);
...
async function pruneEmptyRuntimeDirBestEffort(
  codexHome: string,
): Promise<void> {
  try {
...
function isExpectedRuntimePruneFailure(error: unknown): boolean {
  return ["ENOENT", "ENOTEMPTY", "EEXIST", "EPERM", "EBUSY", "ENOTDIR"]
    .includes(nodeErrorCode(error) ?? "");
}

async function collectInstalledAgentPaths(
  codexHome: string,
  configPath: string,
): Promise<readonly string[]> {
  ...
  if (await exists(versionRoot)) {
    ...
    for (const entry of entries) {
      if (entry.isDirectory()) {
        manifestPaths.push(
          join(versionRoot, entry.name, INSTALLED_AGENTS_MANIFEST),
        );
      }
    }
  }
  ...
  for (
    const path of await readManagedAgentPathsFromConfig(codexHome, configPath)
  ) {
    paths.add(path);
  }
...
async function readManagedAgentPathsFromConfig(
  codexHome: string,
  configPath: string,
): Promise<readonly string[]> {
...
async function readInstalledAgentManifest(
  ...
  return parsed.agents.filter((path): path is string =>
    typeof path === "string"
  );
}
...
function isSafeManagedAgentPath(agentsDir: string, path: string): boolean {
  ...
  return MANAGED_CODEX_AGENT_NAMES.some((agentName) =>
