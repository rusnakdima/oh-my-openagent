...
export const EXPECTED_OMO_COMPONENT_BINS = [
  {
    ...
    target: join("dist", "cli", "index.js"),
    kind: "runtime-wrapper",
  },
  {
    name: "omo-comment-checker",
    target: join("components", "comment-checker", "dist", "cli.js"),
  },
  {
    name: "omo-git-bash-hook",
    target: join("components", "git-bash", "dist", "cli.js"),
  },
  {
    name: "lazycodex-executor-verify",
    target: join("components", "lazycodex-executor-verify", "dist", "cli.js"),
  },
  { name: "omo-lsp", target: join("components", "lsp", "dist", "cli.js") },
  { name: "omo-rules", target: join("components", "rules", "dist", "cli.js") },
  {
    name: "omo-start-work-continuation",
    target: join("components", "start-work-continuation", "dist", "cli.js"),
  },
  {
    name: "omo-telemetry",
    target: join("components", "telemetry", "dist", "cli.js"),
  },
  {
    name: "omo-ulw-loop",
    target: join("components", "ulw-loop", "dist", "cli.js"),
  },
  ...
  {
    name: "omo-ultrawork",
    target: join("components", "ultrawork", "dist", "cli.js"),
  },
] as const;
...
export async function createRepoWithBuiltComponentBins(
  ...
  await writeFile(
    ...
    JSON.stringify({ name: "oh-my-openagent", version: "4.7.5" }),
  );
  await writeFile(
  ...
  const pluginManifest = input.includeBundledGitBashMcp === true
  ...
  if (input.agentNames !== undefined) {
    ...
    await Promise.all(
      input.agentNames.map((agentName) =>
        writeFile(
          join(agentsRoot, `${agentName}.toml`),
          `name = "${agentName}"\n`,
          "utf8",
        )
      ),
  ...
  for (const entry of EXPECTED_OMO_COMPONENT_BINS) {
    ...
    if (componentName === undefined) {
      throw new Error(`missing component name for ${entry.name}`);
    }
...
export async function createRepoWithGitBashHooksFixture(
  ...
  const sourceManifest: unknown = JSON.parse(
    await readFile(
      join(sourcePluginRoot, ".codex-plugin", "plugin.json"),
      "utf8",
    ),
  );
  ...
  const sourceHooks = new Set(
    sourceManifest.hooks.filter((hook): hook is string =>
      typeof hook === "string"
    ),
  );
  if (!EXPECTED_GIT_BASH_HOOKS.every((hook) => sourceHooks.has(hook))) {
    throw new Error(
      "shipped Codex plugin manifest is missing Git Bash hook declarations",
    );
  }
  ...
  for (const hook of gitBashHooks) {
    ...
    await copyFile(
      join(sourcePluginRoot, relativeHookPath),
      join(fixturePluginRoot, relativeHookPath),
    );
  }
  ...
  await mkdir(join(fixturePluginRoot, "components", "bootstrap", "scripts"), {
    recursive: true,
  });
  await copyFile(
    join(sourcePluginRoot, bootstrapScriptPath),
    join(fixturePluginRoot, bootstrapScriptPath),
  );
  return repoRoot;
}

async function createBundledGitBashMcpFixture(
  input: { readonly pluginRoot: string; readonly repoRoot: string },
): Promise<void> {
        ...
        PreToolUse: [
          {
            hooks: [{
              type: "command",
              command:
                "node ./components/git-bash/dist/cli.js hook pre-tool-use",
            }],
          },
        ],
        PostCompact: [
          {
            hooks: [{
              type: "command",
              command:
                "node ./components/git-bash/dist/cli.js hook post-compact",
            }],
          },
        ],
      },
  ...
  await mkdir(join(input.repoRoot, "packages", "git-bash-mcp", "dist"), {
    recursive: true,
  });
  await writeFile(
    join(input.repoRoot, "packages", "git-bash-mcp", "dist", "cli.js"),
    "#!/usr/bin/env node\n",
  );
}
