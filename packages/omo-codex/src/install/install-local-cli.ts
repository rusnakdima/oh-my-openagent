...
} from "./lazycodex-delegated-command";

export async function installMarketplaceLocally(
  options: CodexInstallOptions = {},
): Promise<CodexInstallResult> {
  return runCodexInstaller(options);
}

export function resolveDefaultRepoRootForEntrypoint(
...
export async function runLazyCodexInstallLocalCli(input: {
  ...
  if (parsed.kind === "version") {
    const packageJson = JSON.parse(
      await readFile(join(input.defaultRepoRoot, "package.json"), "utf8"),
