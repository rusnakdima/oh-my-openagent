...
const INSTALL_CODEX_CODEGRAPH_TEST_TIMEOUT_MS = process.platform === "win32"
  ? 60_000
  : 20_000;

describe("install-codex CodeGraph MCP policy", () => {
  test(
    "#given unavailable CodeGraph Node runtime #when installing omo #then disables only the codegraph MCP policy",
    async () => {
      // given
      const codexHome = await mkdtemp(
        join(tmpdir(), "omo-codex-home-codegraph-node-"),
      );
      const binDir = await mkdtemp(
        join(tmpdir(), "omo-codex-bin-codegraph-node-"),
      );
      const repoRoot = await createRepoWithBuiltComponentBins();
      const missingNode = join(
        await mkdtemp(join(tmpdir(), "omo-codex-codegraph-node-bin-")),
        "missing-node",
      );
      ...
      expect(configContent).toMatch(
        /\[plugins\."omo@sisyphuslabs"\][\s\S]*?enabled = true/,
      );
      expect(configContent).toMatch(
        /\[plugins\."omo@sisyphuslabs"\.mcp_servers\.codegraph\][\s\S]*?enabled = false/,
      );
      expect(configContent).toMatch(
