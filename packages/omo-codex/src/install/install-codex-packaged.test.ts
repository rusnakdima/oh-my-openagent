...
test(
  "#given packaged lazycodex tarball layout #when installing Codex plugin #then uses bundled artifacts without source builds",
  async () => {
    // given
    ...
    await writeFile(
      ...
      JSON.stringify({ name: "oh-my-opencode", version: "4.5.12" }),
    );
    await mkdir(join(pluginRoot, ".codex-plugin"), { recursive: true });
    await mkdir(join(pluginRoot, "dist"), { recursive: true });
    await mkdir(join(pluginRoot, "components", "comment-checker", "dist"), {
      recursive: true,
    });
    await mkdir(join(pluginRoot, "components", "ulw-loop", "hooks"), {
      recursive: true,
    });
    ...
    await writeFile(
      join(pluginRoot, "components", "comment-checker", "dist", "cli.js"),
      "#!/usr/bin/env node\n",
    );
    await writeFile(
      join(lspRuntimeRoot, "dist", "cli.js"),
      "#!/usr/bin/env node\n",
    );
    ...
    const cachedComponentPackage = JSON.parse(
      await readFile(
        join(pluginPath, "components", "ulw-loop", "package.json"),
        "utf8",
      ),
    ) as { readonly version: string };
    const cachedHooks = JSON.parse(
      ...
      readonly hooks: {
        readonly PostToolUse: readonly [
          { readonly hooks: readonly [{ readonly statusMessage: string }] },
        ];
      };
    };
    const cachedComponentHooks = JSON.parse(
      await readFile(
        join(pluginPath, "components", "ulw-loop", "hooks", "hooks.json"),
        "utf8",
      ),
    ) as {
      readonly hooks: {
    ...
    expect(result.installed.map((plugin) => `${plugin.name}@${plugin.version}`))
      .toEqual(["omo@4.5.12"]);
    expect(pluginPath).toBe(
      join(codexHome, "plugins", "cache", "sisyphuslabs", "omo", "4.5.12"),
    );
    ...
    expect(cachedHooks.hooks.PostToolUse[0].hooks[0].statusMessage).toBe(
      "(OmO 4.5.12) Checking Comments",
    );
    expect(
      cachedComponentHooks.hooks.UserPromptSubmit[0].hooks[0].statusMessage,
    ).toBe("(OmO 4.5.12) Checking Ulw-Loop Steering");
    expect(commands).toHaveLength(2);
    const installCommand = commands.find((command) => command[0] === "npm");
    if (installCommand === undefined) {
      throw new Error("missing cached plugin npm install command");
    }
    expect(installCommand[0]).toBe("npm");
    expect(installCommand[1]).toBe("ci --omit=dev");
    expect(
      installCommand[2].startsWith(
        join(
          codexHome,
          "plugins",
          "cache",
    ...
    const sotCommand = commands.find((command) =>
      command[1].includes("migrate-omo-sot.mjs")
    );
    if (sotCommand === undefined) {
      throw new Error("missing OMO SOT migration command");
    }
    ...
    expect(cachedMcp.mcpServers.lsp.args[0]).not.toBe(
      join(lspRuntimeRoot, "dist", "cli.js"),
    );
    expect((await stat(cachedLspCli)).isFile()).toBe(true);
    expect(await readlink(join(binDir, "omo"))).toBe(
      join(pluginPath, "dist", "cli.js"),
    );
  },
  { timeout: 30_000 },
);
...
test("#given packaged lazycodex tarball layout #when simulating Windows install #then links bin shims for that platform", async () => {
  ...
  const repoRoot = await mkdtemp(
    join(tmpdir(), "omo-codex-packaged-root-win-"),
  );
  const codexHome = await mkdtemp(
    join(tmpdir(), "omo-codex-packaged-home-win-"),
  );
