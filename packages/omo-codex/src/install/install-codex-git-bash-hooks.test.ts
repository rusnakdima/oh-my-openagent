...
import {
  createRepoWithGitBashHooksFixture,
  EXPECTED_GIT_BASH_HOOKS,
} from "./install-codex-test-fixtures";

const [GIT_BASH_PRE_TOOL_USE_HOOK, GIT_BASH_POST_COMPACT_HOOK] =
  EXPECTED_GIT_BASH_HOOKS;
...
describe("install-codex Git Bash hooks", () => {
  ...
  test("#given simulated Linux Codex install #when installing omo #then keeps git_bash MCP disabled without registering shell reminder hooks", async () => {
    ...
    try {
      ...
      expect(await readGitBashMcpPath(pluginPath)).toBe(
        join(pluginPath, "components", "git-bash-mcp", "dist", "cli.js"),
      );
    } finally {
      await Promise.all([
...
async function readInstalledPluginHooks(
  pluginPath: string,
  ...
  return parsed.hooks.filter((hook): hook is string =>
    typeof hook === "string"
  );
}
