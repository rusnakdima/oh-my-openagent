...
describe("codex-cache", () => {
  test("rewrites cached mcp manifest relative args and cwd", async () => {
    ...
    expect(rewritten.mcpServers.lsp.args[0]).toBe(
      join(root, "./components/lsp/dist/cli.js"),
    );
  });
  ...
  test("#given cached CodeGraph manifest uses bare package-relative entrypoint #when rewriting before bootstrap #then entrypoint becomes absolute under plugin root", async () => {
    ...
    expect(rewritten.mcpServers.codegraph.args).toEqual([
      join(root, "components", "codegraph", "dist", "serve.js"),
    ]);
  });
  ...
  test("rewrites bundled mcp manifest args that point outside the plugin cache into bundled cache paths", async () => {
    ...
    // then
    const rewritten = JSON.parse(
      await readFile(join(cacheRoot, ".mcp.json"), "utf8"),
    ) as {
      mcpServers: {
        custom: { args: string[] };
    ...
    expect(Object.keys(rewritten.mcpServers).sort()).toEqual([
      "custom",
      "git_bash",
      "lsp",
    ]);
    expect(rewritten.mcpServers.custom.args).toEqual([
      "/usr/local/bin/custom-mcp",
      "--stdio",
    ]);
    expect(rewritten.mcpServers.git_bash.cwd).toBeUndefined();
    expect(rewritten.mcpServers.git_bash.args[0]).toBe(
      join(cacheRoot, "components", "git-bash-mcp", "dist", "cli.js"),
    );
    expect(rewritten.mcpServers.lsp.cwd).toBeUndefined();
    expect(rewritten.mcpServers.lsp.args[0]).toBe(
      join(cacheRoot, "components", "lsp-daemon", "dist", "cli.js"),
    );
  });
  ...
  test("rewrites cached package file dependencies that point outside the plugin cache back to the source package", async () => {
    ...
    // then
    const cachedPackageJson = JSON.parse(
      await readFile(join(installed.path, "package.json"), "utf8"),
    ) as {
    ...
    const packageLockDependencyPath = relative(
      realpathSync(installed.path),
      sourceDependencyPath,
    ).split(sep).join("/");
    expect(cachedPackageJson.dependencies["@scope/lsp-tools"]).toBe(
      `file:${sourceDependencyPath}`,
    );
    const cachedPackageLock = JSON.parse(
      await readFile(join(installed.path, "package-lock.json"), "utf8"),
    ) as {
      packages: Record<
        string,
    ...
    expect(cachedPackageLock.packages[""]?.dependencies?.["@scope/lsp-tools"])
      .toBe(`file:${sourceDependencyPath}`);
    expect(cachedPackageLock.packages[packageLockDependencyPath]).toEqual({
      name: "@scope/lsp-tools",
      version: "0.1.0",
    });
    expect(
      cachedPackageLock.packages["node_modules/@scope/lsp-tools"]?.resolved,
    ).toBe(packageLockDependencyPath);
  });
  ...
  test("#given source plugin has an npm lockfile #when caching plugin #then lockfile is preserved for deterministic install", async () => {
    ...
    // then
    expect(await readFile(join(installed.path, "package-lock.json"), "utf8"))
      .toBe(lockfile);
  });
  ...
  test("#given stale sparkshell guidance in prompt surfaces #when caching plugin #then cache promotion is rejected", async () => {
    ...
    const codexHome = join(root, "codex-home");
    const sourceRoot = join(root, "plugin");
    await mkdir(join(sourceRoot, "skills", "ulw-loop", "references"), {
      recursive: true,
    });
    await writeFile(
      join(sourceRoot, "package.json"),
    ...
    await expect(
      stat(join(codexHome, "plugins", "cache", "debug", "omo", "0.1.0")),
    ).rejects.toThrow();
  });
  ...
  test("#given stale sparkshell guidance in component prompt surfaces #when caching plugin #then cache promotion is rejected", async () => {
    ...
    for (const promptCase of promptCases) {
      const root = await mkdtemp(
        join(tmpdir(), "omo-codex-cache-component-sparkshell-"),
      );
      const codexHome = join(root, "codex-home");
      const sourceRoot = join(root, "plugin");
  ...
  test("#given existing cache #when npm install fails #then previous active cache is preserved", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({ name: "@scope/omo-old", version: "0.0.9" }),
    );
    ...
    expect(await readdir(join(codexHome, "plugins", "cache", "debug", "omo")))
      .toEqual(["0.1.0"]);
  });
    ...
    await expect(
      installCachedPlugin({
        ...
        renameDirectory: async (fromPath, toPath) => {
          if (toPath === cacheRoot && basename(fromPath).startsWith(".tmp-")) {
            throw new Error("rename final failed");
          }
  ...
  test("#given source plugin has built component runtimes #when caching plugin #then component dist files are preserved for hooks", async () => {
    // given
    const root = await mkdtemp(
      join(tmpdir(), "omo-codex-cache-component-dist-"),
    );
    const codexHome = join(root, "codex-home");
    const sourceRoot = join(root, "plugin");
    ...
    expect(
      (await stat(
        join(installed.path, "components", "rules", "dist", "cli.js"),
      )).isFile(),
    ).toBe(true);
  });
  ...
  test("#given packaged root CLI runtimes #when caching omo plugin #then root dist is materialized into the plugin cache", async () => {
    ...
    // then
    expect(
      await readFile(join(installed.path, "dist", "cli", "index.js"), "utf8"),
    ).toBe("console.log('omo')\n");
    expect(
      await readFile(
        join(installed.path, "dist", "cli-node", "index.js"),
        "utf8",
      ),
    ).toBe("console.log('omo node')\n");
  });
  ...
  test("links cached plugin bins and stays idempotent", async () => {
    ...
    // when
    const first = await linkCachedPluginBins({
      binDir,
      pluginRoot,
      platform: "linux",
    });
    const second = await linkCachedPluginBins({
      binDir,
      pluginRoot,
      platform: "linux",
    });
  ...
  test("#given nested component declares reserved omo bin #when linking cached plugin bins #then skips the nested top-level command", async () => {
    ...
    // when
    const linked = await linkCachedPluginBins({
      binDir,
      pluginRoot,
      platform: "linux",
    });
    ...
    expect(linked).toEqual([
      {
        name: "omo-ulw-loop",
        path: join(binDir, "omo-ulw-loop"),
        target: join(componentRoot, "dist", "cli.js"),
      },
      {
        name: "ulw",
        path: join(binDir, "ulw"),
        target: join(componentRoot, "dist", "cli.js"),
      },
      {
        name: "ulw-loop",
        path: join(binDir, "ulw-loop"),
        target: join(componentRoot, "dist", "cli.js"),
      },
    ]);
    await expect(readlink(join(binDir, "omo"))).rejects.toThrow();
    expect(await readlink(join(binDir, "omo-ulw-loop"))).toBe(
      join(componentRoot, "dist", "cli.js"),
    );
    expect(await readlink(join(binDir, "ulw"))).toBe(
      join(componentRoot, "dist", "cli.js"),
    );
    expect(await readlink(join(binDir, "ulw-loop"))).toBe(
      join(componentRoot, "dist", "cli.js"),
    );
  });
  ...
  test("#given stale managed ulw-loop omo symlink #when linking cached plugin bins #then removes it", async () => {
    // given
    const root = await mkdtemp(
      join(tmpdir(), "omo-codex-cache-stale-omo-bin-"),
    );
    const pluginRoot = join(root, "plugin");
    const componentRoot = join(pluginRoot, "components", "rules");
    const binDir = join(root, "bin");
    const oldTarget = join(
      root,
      "codex-home",
      "plugins",
      "cache",
    ...
    // then
    await expect(readlink(join(binDir, "omo"))).rejects.toThrow();
    expect(await readlink(join(binDir, "omo-rules"))).toBe(
      join(componentRoot, "dist", "cli.js"),
    );
  });
  ...
  test("#given stale local-source ulw-loop omo symlink #when linking cached plugin bins #then removes it", async () => {
    // given
    const root = await mkdtemp(
      join(tmpdir(), "omo-codex-cache-source-omo-bin-"),
    );
    const pluginRoot = join(root, "plugin");
    const componentRoot = join(pluginRoot, "components", "rules");
    const binDir = join(root, "bin");
  ...
  test("#given stale public ulw symlink points at deleted cache payload #when linking cached plugin bins #then removes the dangling managed link", async () => {
    // given
    const root = await mkdtemp(
      join(tmpdir(), "omo-codex-cache-stale-ulw-bin-"),
    );
    const pluginRoot = join(root, "plugin");
    const componentRoot = join(pluginRoot, "components", "rules");
    ...
    await writeFile(
      ...
      JSON.stringify({ name: "@scope/ulw-loop", bin: { ulw: "dist/cli.js" } }),
    );
    await writeFile(
      join(componentRoot, "dist", "cli.js"),
      "#!/usr/bin/env node\n",
  ...
  test("#given dangling symlink points at another cached plugin #when linking cached plugin bins #then preserves it", async () => {
    // given
    const root = await mkdtemp(
      join(tmpdir(), "omo-codex-cache-other-plugin-bin-"),
    );
    const pluginRoot = join(root, "plugin");
    const componentRoot = join(pluginRoot, "components", "rules");
    const binDir = join(root, "bin");
    const otherTarget = join(
      root,
      "codex-home",
      "plugins",
      "cache",
  ...
  test("#given non-managed bin points at deleted omo cache payload #when linking cached plugin bins #then preserves the user symlink", async () => {
    // given
    const root = await mkdtemp(
      join(tmpdir(), "omo-codex-cache-user-omo-shaped-bin-"),
    );
    const pluginRoot = join(root, "plugin");
    const componentRoot = join(pluginRoot, "components", "rules");
    const binDir = join(root, "bin");
    const userTarget = join(
      root,
      "codex-home",
      "plugins",
      "cache",
  ...
  test("#given legacy codex-prefixed component symlinks #when linking cached plugin bins #then removes stale managed symlinks without touching user files", async () => {
    ...
    await writeFile(
      join(binDir, "codex-comment-checker"),
      "user managed file\n",
    );
    ...
    expect(await readFile(join(binDir, "codex-comment-checker"), "utf8")).toBe(
      "user managed file\n",
    );
    expect(await readlink(join(binDir, "omo-rules"))).toBe(
      join(pluginRoot, "dist", "cli.js"),
  ...
  test("#given user-owned codex symlink with component-like target #when linking cached plugin bins #then preserves the user symlink", async () => {
    // given
    const root = await mkdtemp(
      join(tmpdir(), "omo-codex-cache-user-component-symlink-"),
    );
    const pluginRoot = join(root, "plugin");
    const binDir = join(root, "bin");
    const userTarget = join(
    ...
    await mkdir(join(pluginRoot, "dist"), { recursive: true });
    await mkdir(join(root, "workspace", "components", "rules", "dist"), {
      recursive: true,
    });
    await mkdir(binDir, { recursive: true });
    await writeFile(
  ...
  test("writes Windows command shims for cached plugin bins", async () => {
    ...
    expect(linked).toEqual([{
      name: "omo-hook",
      path: join(binDir, "omo-hook.cmd"),
      target: join(pluginRoot, "dist", "cli.js"),
    }]);
    ...
    expect(commandShim).not.toContain(
  ...
  test("rejects existing non-generated Windows command shims", async () => {
    ...
    expect(await readFile(join(binDir, "omo-hook.cmd"), "utf8")).toContain(
      "echo custom",
    );
  });
});
