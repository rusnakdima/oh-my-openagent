...
const resolveTestGitBash = () =>
  ({ found: true, path: "C:\\Git\\bin\\bash.exe", source: "env" }) as const;
...
async function createPackagedCodexRepoRoot(): Promise<string> {
  const repoRoot = await mkdtemp(
    join(tmpdir(), "omo-codex-project-cleanup-repo-"),
  );
  const codexPackageRoot = join(repoRoot, "packages", "omo-codex");
  const pluginRoot = join(codexPackageRoot, "plugin");
  await writeFile(
    join(repoRoot, "package.json"),
    JSON.stringify({ name: "oh-my-openagent", version: "4.5.12" }),
  );
...
describe("install-codex project-local cleanup", () => {
  test(
    "#given stale project-local Codex config #when installing Codex Light #then repairs the local conflict before returning success",
    async () => {
      // given
      const codexHome = await mkdtemp(
        join(tmpdir(), "omo-codex-home-project-cleanup-"),
      );
      const binDir = await mkdtemp(
        join(tmpdir(), "omo-codex-bin-project-cleanup-"),
      );
      const projectRoot = await mkdtemp(
        join(tmpdir(), "omo-codex-project-cleanup-install-"),
      );
      ...
      expect(await readFile(result.projectCleanup.backupPath ?? "", "utf8"))
  ...
  test(
    "#given only global CODEX_HOME config under a parent directory #when installing Codex Light #then project cleanup leaves it to the global config updater",
    async () => {
      // given
      const homeRoot = await mkdtemp(
        join(tmpdir(), "omo-codex-home-parent-cleanup-"),
      );
      const codexHome = join(homeRoot, ".codex");
      const binDir = await mkdtemp(
        join(tmpdir(), "omo-codex-bin-parent-cleanup-"),
      );
  ...
  test(
    "#given project cleanup hits a filesystem edge #when installing Codex Light #then install succeeds and reports skipped cleanup",
    async () => {
      // given
      const codexHome = await mkdtemp(
        join(tmpdir(), "omo-codex-home-project-edge-"),
      );
      const binDir = await mkdtemp(
        join(tmpdir(), "omo-codex-bin-project-edge-"),
      );
      const projectRoot = await mkdtemp(
        join(tmpdir(), "omo-codex-project-edge-"),
      );
      ...
      expect(
        logs.some((message) =>
          message.includes("Skipped project-local Codex cleanup")
        ),
      ).toBe(true);
      expect(logs.some((message) => message.includes("not a directory"))).toBe(
        true,
      );
