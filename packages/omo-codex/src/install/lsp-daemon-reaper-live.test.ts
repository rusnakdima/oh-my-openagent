...
describe("reapLspDaemons live ownership", () => {
  ...
  posixOnly(
    ...
    async () => {
      const codexHome = trackRoot(
        createLegacyCodexHome("omo-reap-reused-pid-"),
      );
      const endpoint = legacyEndpointFor({
        codexHome,
        version: "0.1.0",
  ...
  posixOnly(
    ...
    async () => {
      ...
      const daemon = trackChild(
        startLegacyDaemonProcess({ endpoint, ignoreSigterm: true }),
      );
      await waitForChildReady(daemon);
      const version = await writeLegacyVersionState({
        codexHome,
      ...
      expect(reaped).toEqual([
        {
          ...
          status: "deferred",
          reason:
            "timed out waiting 5000ms for the proven legacy daemon to exit",
        },
      ]);
  ...
  posixOnly(
    ...
    async () => {
      const codexHome = trackRoot(
        createLegacyCodexHome("omo-reap-idempotent-"),
      );
      const endpoint = legacyEndpointFor({
        codexHome,
        version: "0.1.0",
  ...
  windowsOnly(
    ...
    async () => {
      const codexHome = trackRoot(
        createLegacyCodexHome("omo-reap-win32-live-"),
      );
