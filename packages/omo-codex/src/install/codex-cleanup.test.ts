...
import {
  cleanupCodexLight,
  cleanupCodexLightConfigText,
  removeManagedPathBestEffort,
} from "./codex-cleanup";
...
describe("codex cleanup", () => {
  test("#given managed Codex Light state and project-local Codex leftovers #when cleanup runs #then removes only managed global state and repairs local config", async () => {
    ...
    const projectRoot = await mkdtemp(
      join(tmpdir(), "omo-codex-cleanup-project-"),
    );
    ...
    await writeFile(
      join(versionPluginRoot, ".installed-agents.json"),
      JSON.stringify({ agents: [managedAgentPath] }),
    );
    await writeFile(
    ...
    expect(result.configBackupPath).toBe(
      `${configPath}.backup-2026-06-01T00-00-00-000Z`,
    );
    ...
    expect(await readFile(result.configBackupPath ?? "", "utf8")).toContain(
      "[marketplaces.sisyphuslabs]",
    );
    ...
    expect(
      result.projectCleanup.artifacts.map((artifact) => artifact.relativePath)
        .sort(),
    ).toEqual([".codex/hooks.json"]);
    expect(projectConfig).not.toMatch(/^max_threads\s*=/m);
    expect(projectConfig).toContain("max_depth = 3");
    expect(await pathExists(join(projectRoot, ".codex", "hooks.json"))).toBe(
      true,
    );
  });
  ...
  test("#given malformed project directory #when cleanup runs #then global cleanup still succeeds and project cleanup is skipped", async () => {
    ...
    const codexHome = await mkdtemp(
      join(tmpdir(), "omo-codex-cleanup-malformed-"),
    );
    const configPath = join(codexHome, "config.toml");
  ...
  test("#given managed config and missing install manifests #when cleanup runs #then removes orphaned managed agent links", async () => {
    // given
    const codexHome = await mkdtemp(
      join(tmpdir(), "omo-codex-cleanup-orphan-agent-"),
    );
    const configPath = join(codexHome, "config.toml");
    const managedAgentPath = join(codexHome, "agents", "explorer.toml");
    await mkdir(join(codexHome, "agents"), { recursive: true });
    await symlink(
      join(codexHome, ".tmp", "marketplaces", "missing", "explorer.toml"),
      managedAgentPath,
    );
    await writeFile(
      configPath,
  ...
  test("#given project directory is a regular file #when cleanup runs #then global cleanup still succeeds and project cleanup is skipped", async () => {
    // given
    const codexHome = await mkdtemp(
      join(tmpdir(), "omo-codex-cleanup-file-project-home-"),
    );
    const projectDirectory = join(
      await mkdtemp(join(tmpdir(), "omo-codex-cleanup-file-project-")),
      "project-file",
    );
    const configPath = join(codexHome, "config.toml");
    await mkdir(codexHome, { recursive: true });
  ...
  test("#given provisioned runtime binaries and bootstrap plugin data #when cleanup runs #then removes only the managed subtrees", async () => {
    ...
    const autoUpdateStatePath = join(
      codexHome,
      "plugins",
      "data",
      "omo-sisyphuslabs",
    ...
    const foreignBootstrapStatePath = join(
      codexHome,
      "plugins",
      "data",
      "widget-sisyphuslabs",
    ...
    const driftedBootstrapDataDir = join(
      codexHome,
      "plugins",
      "legacy",
      "omo-next-sisyphuslabs",
      "bootstrap",
    );
    const cacheRoot = join(codexHome, "plugins", "cache", "sisyphuslabs");
    ...
    await writeFixtureFile(
      join(astGrepRuntimeDir, "darwin-arm64", "sg"),
      "sg binary\n",
    );
    await writeFixtureFile(
      join(nodeRuntimeDir, "node-v22.14.0-win-x64", "node.exe"),
      "node binary\n",
    );
    await writeFixtureFile(foreignRuntimeFile, "other owner\n");
    await writeFixtureFile(
      join(bootstrapDataDir, "state.json"),
      JSON.stringify({ lastStatus: "success" }),
    );
    ...
    await writeFixtureFile(
      join(bootstrapDataDir, "ps-bootstrap.log"),
      "ps log\n",
    );
    await writeFixtureFile(
      join(bootstrapDataDir, "agents-stage", ".installed-agents.json"),
    ...
    await writeFixtureFile(
      join(cacheRoot, "omo", "0.1.0", "package.json"),
      "{}\n",
    );
    await writeFixtureFile(
      join(snapshotRoot, "plugins", "omo", "marketplace.json"),
      "{}\n",
    );
  ...
  test("#given runtime holding only the managed subtrees #when cleanup runs #then prunes the empty runtime directory", async () => {
    // given
    const codexHome = await mkdtemp(
      join(tmpdir(), "omo-codex-cleanup-runtime-prune-"),
    );
    await writeFixtureFile(
      join(codexHome, "runtime", "ast-grep", "darwin-arm64", "sg"),
      "sg binary\n",
    );
    await writeFixtureFile(
      join(codexHome, "runtime", "node", "node-v22.14.0-win-x64", "node.exe"),
      "node binary\n",
    );

    // when
    await cleanupCodexLight({
      codexHome,
      projectDirectory: codexHome,
      now: () => new Date("2026-06-01T00:00:00Z"),
    });
  ...
  test("#given codex home without bootstrap or runtime artifacts #when cleanup runs #then succeeds with no removed paths", async () => {
    // given
    const codexHome = await mkdtemp(
      join(tmpdir(), "omo-codex-cleanup-absent-"),
    );
    await writeFile(
  ...
  test("#given a mid-flight worker recreates bootstrap state between uninstall runs #when cleanup runs twice #then the second pass clears it without error", async () => {
    // given
    const codexHome = await mkdtemp(
      join(tmpdir(), "omo-codex-cleanup-two-pass-"),
    );
    const bootstrapDataDir = join(
      codexHome,
      "plugins",
    ...
    // when
    const firstRun = await cleanupCodexLight({
      codexHome,
      projectDirectory: codexHome,
      now: () => new Date("2026-06-01T00:00:00Z"),
    });
    await writeFixtureFile(statePath, "{}\n");
    const secondRun = await cleanupCodexLight({
      codexHome,
      projectDirectory: codexHome,
      now: () => new Date("2026-06-01T00:00:00Z"),
    });
...
`;
  ...
  test("#given installer-created bin links in the bin dir #when cleanup runs #then removes the managed omo bins and keeps unmanaged files", async () => {
    ...
    const codexHome = await mkdtemp(
      join(tmpdir(), "omo-codex-cleanup-bins-home-"),
    );
  ...
  test.skipIf(process.platform === "win32")(
    ...
    async () => {
      // given
      const codexHome = await mkdtemp(
        join(tmpdir(), "omo-codex-cleanup-bins-posix-home-"),
      );
      const binDir = await mkdtemp(
        join(tmpdir(), "omo-codex-cleanup-bins-posix-dir-"),
      );
      await writeFile(
        join(codexHome, "config.toml"),
        "[features]\nplugins = true\n",
  ...
  test("#given no bin directory on disk #when cleanup runs #then reports no removed bin links and does not throw", async () => {
    // given
    const codexHome = await mkdtemp(
      join(tmpdir(), "omo-codex-cleanup-bins-absent-"),
    );
    await writeFile(
      join(codexHome, "config.toml"),
      "[features]\nplugins = true\n",
  ...
  test("#given a codex home resolving to a filesystem root #when cleanup runs #then no bin link is scanned or removed", async () => {
    ...
    const binDir = await mkdtemp(
      join(tmpdir(), "omo-codex-cleanup-rootguard-bin-"),
    );
    const projectDirectory = await mkdtemp(
      join(tmpdir(), "omo-codex-cleanup-rootguard-proj-"),
    );
    const rootBin = join(binDir, "omo.cmd");
  ...
  test("#given a user copy of a generated wrapper #when cleanup runs #then only installer bin names are removed", async () => {
    ...
    const codexHome = await mkdtemp(
      join(tmpdir(), "omo-codex-cleanup-copy-home-"),
    );
  ...
  test("#given a user-owned bin using an installer name without the marker #when cleanup runs #then it is kept", async () => {
    ...
    const codexHome = await mkdtemp(
      join(tmpdir(), "omo-codex-cleanup-usermain-home-"),
    );
    const binDir = await mkdtemp(
      join(tmpdir(), "omo-codex-cleanup-usermain-bin-"),
    );
    await writeFile(
      join(codexHome, "config.toml"),
      "[features]\nplugins = true\n",
  ...
  test("#given a managed windows wrapper left on disk with different casing #when cleanup runs #then it is still removed", async () => {
    ...
    const codexHome = await mkdtemp(
      join(tmpdir(), "omo-codex-cleanup-case-home-"),
    );
  ...
  test("#given a user-owned windows bin using an installer name in different casing #when cleanup runs #then it is kept", async () => {
    ...
    const codexHome = await mkdtemp(
      join(tmpdir(), "omo-codex-cleanup-caseuser-home-"),
    );
    const binDir = await mkdtemp(
      join(tmpdir(), "omo-codex-cleanup-caseuser-bin-"),
    );
    await writeFile(
      join(codexHome, "config.toml"),
      "[features]\nplugins = true\n",
  ...
  test("#given an install that used a one-shot CODEX_LOCAL_BIN_DIR #when uninstall runs without that variable #then the wrapper in the recorded dir is still removed", async () => {
    ...
    const codexHome = await mkdtemp(
      join(tmpdir(), "omo-codex-cleanup-binmanifest-home-"),
    );
    const customBinDir = await mkdtemp(
      join(tmpdir(), "omo-codex-cleanup-binmanifest-bin-"),
    );
    await writeFile(
      join(codexHome, "config.toml"),
      "[features]\nplugins = true\n",
    ...
    await writeFile(
      join(pluginRoot, ".installed-bin-dir.json"),
      JSON.stringify({ binDir: customBinDir }),
    );
  ...
  test("#given a recorded install bin dir and an explicit CODEX_LOCAL_BIN_DIR at uninstall #when cleanup runs #then the environment override wins", async () => {
    ...
    const codexHome = await mkdtemp(
      join(tmpdir(), "omo-codex-cleanup-binprec-home-"),
    );
    const recordedBinDir = await mkdtemp(
      join(tmpdir(), "omo-codex-cleanup-binprec-recorded-"),
    );
    const overrideBinDir = await mkdtemp(
      join(tmpdir(), "omo-codex-cleanup-binprec-override-"),
    );
    await writeFile(
      join(codexHome, "config.toml"),
      "[features]\nplugins = true\n",
    ...
    await writeFile(
      join(pluginRoot, ".installed-bin-dir.json"),
      JSON.stringify({ binDir: recordedBinDir }),
    );
  ...
  test.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      const codexHome = await mkdtemp(
        join(tmpdir(), "omo-codex-cleanup-legacy-home-"),
      );
      const binDir = await mkdtemp(
        join(tmpdir(), "omo-codex-cleanup-legacy-bin-"),
      );
      await writeFile(
        join(codexHome, "config.toml"),
        "[features]\nplugins = true\n",
  ...
  test.skipIf(process.platform === "win32")(
    ...
    async () => {
      // given
      const codexHome = await mkdtemp(
        join(tmpdir(), "omo-codex-cleanup-usersym-home-"),
      );
      const binDir = await mkdtemp(
        join(tmpdir(), "omo-codex-cleanup-usersym-bin-"),
      );
      await writeFile(
        join(codexHome, "config.toml"),
        "[features]\nplugins = true\n",
