...
} from "./codex-cache-runtime-wrapper";
...
export async function removeCachedManagedNpmBinShims(
  pluginRoot: string,
): Promise<void> {
...
export async function linkRootRuntimeBin(input: {
  ...
  | { readonly name: string; readonly path: string; readonly target: string }
  | null
> {
...
async function discoverPackageBins(
  root: string,
): Promise<readonly { name: string; target: string }[]> {
  const links: Array<{ name: string; target: string }> = [];
  await collectPackageBins(root, root, links);
  return links;
}

async function collectPackageBins(
  directory: string,
  root: string,
...
function isReservedNestedBinName(
  name: string,
  packageRoot: string,
  root: string,
): boolean {
...
async function replaceCommandShim(
  linkPath: string,
  targetPath: string,
): Promise<void> {
...
async function replaceRuntimeWrapper(
  linkPath: string,
  content: string,
): Promise<void> {
