...
export async function readMarketplace(
  ...
  const marketplacePath = options?.marketplacePath ??
    join(repoRoot, DEFAULT_MARKETPLACE_PATH);
...
function normalizePluginHooksManifestValue(
  value: unknown,
...
function normalizeMarketplacePlugin(
  ...
  if (
    ...
    const local: MarketplacePluginSourceLocal = {
      source: "local",
...
function localSourcePath(
  source: string | MarketplacePluginSourceLocal | undefined,
): string {
