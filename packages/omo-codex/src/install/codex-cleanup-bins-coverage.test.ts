...
describe("managed Codex bin coverage", () => {
  test("#given every plugin component package bin #when uninstall coverage is checked #then each name is a managed bin name", async () => {
    ...
    const uncovered = declaredBinNames.filter((name) =>
      !MANAGED_CODEX_BIN_NAMES.has(name)
    );
    expect(uncovered).toEqual([]);
  });
});

async function collectDeclaredBinNames(
  root: string,
...
async function walk(directory: string, names: Set<string>): Promise<void> {
  ...
  if (
    ...
    for (const name of await readBinNames(join(directory, "package.json"))) {
      names.add(name);
    }
  }
...
async function readBinNames(
  ...
  if (typeof record.bin === "string") {
    return typeof record.name === "string" ? [basenameOf(record.name)] : [];
  }
