...
describe("codex-cache install", () => {
  test(
    ...
    async () => {
      ...
      await mkdir(join(sourceRoot, "node_modules", "left-pad"), {
        recursive: true,
      });
      await mkdir(
        join(sourceRoot, "components", "rules", "node_modules", "debug"),
        { recursive: true },
      );
      await writeFile(
      ...
      await writeFile(
        join(sourceRoot, ".git", "HEAD"),
        "ref: refs/heads/dev\n",
      );
      await writeFile(
        join(sourceRoot, "node_modules", "left-pad", "package.json"),
        "{}",
      );
      await writeFile(
        join(
      ...
      await writeFile(
        join(sourceRoot, "components", "rules", "payload.txt"),
        "payload\n",
      );
      ...
      expect(installed.path).toBe(
        join(codexHome, "plugins", "cache", "debug", "omo", "0.1.0"),
      );
      expect(
        await readFile(
          join(installed.path, "components", "rules", "payload.txt"),
          "utf8",
        ),
      ).toBe("payload\n");
      await expect(stat(join(installed.path, ".git"))).rejects.toThrow();
      await expect(stat(join(installed.path, "node_modules"))).rejects
        .toThrow();
      await expect(
        stat(join(installed.path, "components", "rules", "node_modules")),
      ).rejects.toThrow();
    },
    15000,
  );
    ...
    async () => {
      ...
      await installCachedPlugin({
        ...
        runCommand: async (command, args, options) => {
          ...
          expect(args).toEqual(
            npmInstallEnvs.length === 0 ? ["install"] : ["ci", "--omit=dev"],
          );
  ...
  test(
    ...
    async () => {
      ...
      await writeFile(
        ...
        JSON.stringify({ name: "shared-lib", version: "0.1.0" }),
      );
      ...
      // then
      expect(npmArgs).toEqual([["install"], [
        "install",
        "--omit=dev",
        "--no-audit",
        "--no-fund",
    ...
    async () => {
      ...
      await writeFile(
        ...
        JSON.stringify({
          name: "@scope/omo",
          version: "0.1.0",
          dependencies: { "local-tool": "file:./components/local-tool" },
        }),
      );
      await writeFile(
        join(componentRoot, "package.json"),
        JSON.stringify({ name: "local-tool", version: "0.1.0" }),
      );
  ...
  test(
    ...
    async () => {
      ...
      // then
      expect(await readFile(join(installed.path, ".mcp.json"), "utf8"))
        .toContain("grep_app");
      await expect(stat(join(installed.path, "components", "lsp", ".mcp.json")))
        .rejects.toThrow();
    },
    15000,
  );
  ...
  test("#given source plugin references missing hook command target #when caching plugin #then previous active cache is preserved", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({ name: "omo", hooks: "hooks/hooks.json" }),
    );
    await writeFile(
      ...
      JSON.stringify({
        hooks: {
          Stop: [{
            hooks: [{
              type: "command",
              command: 'node "${PLUGIN_ROOT}/dist/missing.js" hook stop',
            }],
          }],
        },
      }),
  ...
  test("#given npm creates workspace bin shims in the cache #when caching plugin #then plugin-owned shims are removed", async () => {
    ...
    // then
    await expect(
      stat(join(installed.path, "node_modules", ".bin", "omo-ulw-loop")),
    ).rejects.toThrow();
    await expect(
      stat(join(installed.path, "node_modules", ".bin", "omo-ulw-loop.cmd")),
    ).rejects.toThrow();
    expect(
      await readFile(
        join(installed.path, "node_modules", ".bin", "other-tool.cmd"),
        "utf8",
      ),
    ).toBe("@echo off\r\necho preserved\r\n");
  });
  ...
  test(
    ...
    async () => {
      ...
      await writeFile(
        join(sourceRoot, "skills", "ulw-plan", "SKILL.md"),
        "---\nname: ulw-plan\n---\n",
      );
      await writeFile(
        join(sourceRoot, "package.json"),
      ...
      const installed = await installCachedPlugin({
        ...
        runCommand: async (command, args, options) => {
          ...
          if (command === "npm" && args.join(" ") === "run sync:skills") {
            await mkdir(join(options.cwd, "skills", "ulw-research"), {
              recursive: true,
            });
            await writeFile(
              join(options.cwd, "skills", "ulw-research", "SKILL.md"),
              "---\nname: ulw-research\n---\n",
            );
          }
        },
      ...
      expect(
        await readFile(
          join(installed.path, "skills", "ulw-research", "SKILL.md"),
          "utf8",
        ),
      ).toContain("name: ulw-research");
    },
    15000,
  );
});
