...
import {
  attestLegacyDaemonOwnership,
  type LegacyDaemonAttestationInput,
  probeLegacyJsonRpcEndpoint,
} from "./lsp-daemon-reaper-attestation";
...
export interface ReapLspDaemonsDeps {
  ...
  readonly attestLegacyDaemonOwnership?: (
...
export async function reapLspDaemons(
  ...
  for (
    ...
    if (
      !(await waitForProcessExit(metadata.pid, LEGACY_EXIT_WAIT_TIMEOUT_MS))
    ) {
      results.push(
        deferred(
          parsedVersion,
...
> {
  const pidText = await readRegularTrimmedFile(
    join(input.versionPath, "daemon.pid"),
  );
  ...
  const endpointText = await readRegularTrimmedFile(
    join(input.versionPath, "daemon.endpoint"),
  );
...
async function readRegularTrimmedFile(
  path: string,
): Promise<string | "non_regular" | null> {
