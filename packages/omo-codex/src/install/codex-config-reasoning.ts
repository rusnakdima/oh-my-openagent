...
function matchesProfile(
  current: CodexReasoningProfileMatch,
  profile: CodexReasoningProfileMatch,
): boolean {
  for (const [key, value] of Object.entries(profile)) {
    if (current[key as keyof CodexReasoningProfileMatch] !== value) {
      return false;
    }
  }
  return true;
