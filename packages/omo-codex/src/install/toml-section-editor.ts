...
export function replaceOrInsertSetting(
  ...
  for (const line of lines) {
    ...
    const assignmentEnd = multilineScan.nextQuote
      ...
      : offset + line.length;
    const sectionReplacement = section.text.slice(0, offset) + replacement +
...
export function replaceOrInsertRootDottedSetting(
  config: string,
  keyPath: string,
  value: string,
): string {
...
function findTomlMultilineDelimiter(
  line: string,
  delimiter: TomlMultilineQuote,
  startIndex: number,
): number {
...
function findTomlMultilineValueEnd(
  text: string,
  startOffset: number,
  quote: TomlMultilineQuote,
): number {
