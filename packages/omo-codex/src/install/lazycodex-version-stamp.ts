...
export async function readDistributionManifest(
  repoRoot: string,
): Promise<DistributionManifest | undefined> {
  try {
...
export async function stampLazyCodexPluginVersion(
  input: { readonly pluginRoot: string; readonly version: string },
): Promise<void> {
  ...
  for (const hookPath of hookPaths) {
    await stampHookStatusMessages(
      join(input.pluginRoot, hookPath),
      input.version,
    );
  }
...
async function readPluginHookPaths(
  manifestPath: string,
  ...
  try {
    ...
    if (typeof parsed.hooks === "string" && parsed.hooks.trim().length > 0) {
      return [stripDotSlash(parsed.hooks)];
    }
...
async function stampComponentVersions(
  input: { readonly pluginRoot: string; readonly version: string },
): Promise<void> {
  ...
  for (const entry of entries) {
    ...
    await stampHookStatusMessages(
      join(componentRoot, "hooks", "hooks.json"),
      input.version,
    );
  }
}
