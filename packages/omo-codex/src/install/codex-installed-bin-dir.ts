...
export async function readInstalledCodexBinDir(
  codexHome: string,
): Promise<string | null> {
...
async function installedBinDirManifestPaths(
  codexHome: string,
  ...
  if (entries !== null) {
    for (const entry of entries) {
      if (entry.isDirectory()) {
        paths.push(join(versionRoot, entry.name, INSTALLED_BIN_DIR_MANIFEST));
      }
    }
  }
  return paths;
...
async function readBinDirFromManifest(
  manifestPath: string,
): Promise<string | null> {
