...
export function isBunGlobalEntrypointPath(
  ...
  return resolveBunGlobalRoots(env).some((root) =>
...
function resolveBunGlobalRoots(env: NodeJS.ProcessEnv): readonly string[] {
  ...
  return [
    ...(bunInstallRoot
      ? [
        join(bunInstallRoot, "bin"),
        join(bunInstallRoot, "install", "global", "node_modules"),
      ]
      : []),
    ...(homeRoot
      ? [
        join(homeRoot, ".bun", "bin"),
        join(homeRoot, ".bun", "install", "global", "node_modules"),
      ]
      : []),
