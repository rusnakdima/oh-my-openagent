...
describe("managed bundled agent effort migration", () => {
  test("#given old bundled support-agent efforts #when agents are re-linked #then upgrades to current bundled defaults", async () => {
    ...
    await writeFile(
      ...
      agentToml("explorer", "gpt-5.6-luna-fast", "low"),
    );
    await writeFile(
      join(codexHome, "agents", "librarian.toml"),
      agentToml("librarian", "gpt-5.6-luna-fast", "low"),
    );
  ...
  test("#given old effort on a non-default model #when agents are re-linked #then preserves customization", async () => {
    ...
    await writeFile(
      join(codexHome, "agents", "explorer.toml"),
      agentToml("explorer", "gpt-5.5", "low"),
    );
    const preservedReasoning = await capturePreservedAgentReasoning({
      codexHome,
    });
...
async function makeAgentFixture(): Promise<
  { readonly codexHome: string; readonly pluginRoot: string }
> {
  ...
  await writeFile(
    ...
    agentToml("momus", "gpt-5.6-terra", "high"),
  );
  await writeFile(
...
async function readAgentEffort(
  codexHome: string,
  agentName: string,
): Promise<string> {
  ...
  if (match?.[1] === undefined) {
    throw new Error(`missing model_reasoning_effort for ${agentName}`);
  }
  return match[1];
}
