...
export async function rewriteCachedPackageLocalFileDependencies(
  pluginRoot: string,
  sourceRoot: string,
): Promise<boolean> {
...
function rewritePackageLockFileDependency(input: {
  ...
  const packageKey = toPackageLockPath(
    relative(input.pluginRoot, input.packageDir),
  );
  const oldTargetKey = toPackageLockPath(
    relative(input.pluginRoot, input.targetPath),
  );
  const newTargetKey = toPackageLockPath(
    relative(lockRoot, input.sourceTargetPath),
  );
...
function getPackageLockPackages(
