...
export function toProjectRuleSource(
  ...
  switch (source) {
    ...
    case ".github/instructions":
      return source;
    default:
      throw new UnsupportedRuleSourceError(
        `Unsupported project rule source: ${source}`,
      );
  }
}
...
export function toProjectSingleFileSource(ruleFile: string): RuleSource {
  switch (ruleFile) {
    ...
    default:
      throw new UnsupportedRuleSourceError(
        `Unsupported project single-file source: ${ruleFile}`,
      );
  }
}
...
export function toUserHomeRuleSource(ruleSubdir: string): RuleSource {
  ...
  switch (source) {
    ...
    default:
      throw new UnsupportedRuleSourceError(
        `Unsupported user-home rule source: ${source}`,
      );
  }
}
...
export function toUserHomeSingleFileSource(ruleFile: string): RuleSource {
  ...
  switch (source) {
    default:
      throw new UnsupportedRuleSourceError(
        `Unsupported user-home single-file source: ${source}`,
      );
  }
}
