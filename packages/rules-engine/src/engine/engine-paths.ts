...
export function isRootSingleFile(candidate: RuleCandidate): boolean {
  return candidate.distance === 0 && candidate.isSingleFile &&
...
function scopeDirectoryForCandidate(
  projectRoot: string,
  candidate: RuleCandidate,
): string | null {
  if (candidate.isGlobal) {
  ...
  const scopeRelativeDirectory = candidate.relativePath.slice(0, sourceIndex)
    .replace(/\/$/, "");
