...
export function staticDedupKey(
  cwd: string,
  rulePath: string,
  contentHash: string,
): string {
...
export function markStaticInjected(
  state: SessionState,
  rule: LoadedRule,
): boolean {
...
export function markDynamicInjected(
  state: SessionState,
  rule: LoadedRule,
): boolean {
...
export function isStaticInjected(
  state: SessionState,
  rule: LoadedRule,
): boolean {
  return state.staticDedup.has(
    staticDedupKey(state.cwd ?? "", rule.realPath, rule.contentHash),
  );
}
...
export function isDynamicInjected(
  ...
  return state.dynamicDedup.get(DYNAMIC_SESSION_KEY)?.has(
    dynamicDedupKey(rule.realPath, rule.contentHash),
  ) === true;
}
