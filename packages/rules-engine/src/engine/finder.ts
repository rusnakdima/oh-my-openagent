...
const HEPHAESTUS_MODEL_VARIANT_FILES: ReadonlyArray<
  readonly [family: string, file: string]
> = [
  ["gpt-5.6", "gpt-5.6.md"],
];
...
export function findRuleCandidates(options: FinderOptions): RuleCandidate[] {
  ...
  if (!skipUserHome) {
    candidates.push(
      ...findUserHomeCandidates(homeDirectory, disabledSources, options.cache),
    );
  }
...
function isPluginBundledCandidateEnabled(
  ...
  if (candidate.relativePath.startsWith(HEPHAESTUS_BUNDLED_RULE_PREFIX)) {
    return candidate.relativePath ===
      `${HEPHAESTUS_BUNDLED_RULE_PREFIX}${
        hephaestusVariantFileForModel(model)
      }`;
  }
  return true;
}
...
function hephaestusVariantFileForModel(model: string | undefined): string {
  ...
  const matched = HEPHAESTUS_MODEL_VARIANT_FILES.find(([family]) =>
