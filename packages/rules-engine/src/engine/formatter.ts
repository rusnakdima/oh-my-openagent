...
function truncateRules(
  ...
  const budgetedRules = truncateBudget({
    rules: perRuleBudgeted.map((rule) => ({
      body: rule.body,
      relativePath: rule.relativePath,
    })),
...
export function formatStaticBlock(
  ...
  return [
    ...
    "",
    truncateRules(orderedRules, options).map(formatRule).join("\n\n"),
  ].join("\n");
}
...
function isHephaestusRule(rule: LoadedRule): boolean {
  return isNeverTruncatedRule(
    rule.relativePath.length > 0 ? rule.relativePath : rule.path,
  );
}
...
function uniqueRulesByBody(rules: ReadonlyArray<LoadedRule>): LoadedRule[] {
  ...
  for (const rule of rules) {
    ...
    if (
      rule.source === "plugin-bundled" && descriptionKey !== undefined &&
