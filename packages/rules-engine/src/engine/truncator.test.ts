...
describe("engine isNeverTruncatedRule", () => {
  describe("#given hephaestus rule paths", () => {
    ...
    it("#when a model variant lives under the hephaestus directory #then it is never truncated", () => {
      expect(isNeverTruncatedRule("bundled-rules/hephaestus/gpt-5.5.md")).toBe(
        true,
      );
      expect(isNeverTruncatedRule("bundled-rules/hephaestus/gpt-5.6.md")).toBe(
        true,
      );
    });

    it("#when a Windows-separated variant path is used #then it is never truncated", () => {
      expect(isNeverTruncatedRule("bundled-rules\\hephaestus\\gpt-5.6.md"))
        .toBe(true);
    });
  });
  ...
  describe("#given non-hephaestus rule paths", () => {
    it("#when checked #then they remain truncatable", () => {
      expect(isNeverTruncatedRule("bundled-rules/windows-git-bash.md")).toBe(
        false,
      );
      expect(isNeverTruncatedRule(".omo/rules/typescript.md")).toBe(false);
      expect(isNeverTruncatedRule(".omo/rules/hephaestus/large.md")).toBe(
        false,
      );
      expect(isNeverTruncatedRule("bundled-rules/hephaestus/future.md")).toBe(
        false,
      );
    });
  });
