...
function makePluginRoot(): string {
  ...
  writeFileSync(
    join(hephaestusDir, "gpt-5.5.md"),
    "---\nalwaysApply: true\n---\nGPT-5.5 variant\n",
  );
  writeFileSync(
    join(hephaestusDir, "gpt-5.6.md"),
    "---\nalwaysApply: true\n---\nGPT-5.6 variant\n",
  );
  writeFileSync(
    join(pluginRoot, "bundled-rules", "other.md"),
    "---\nalwaysApply: true\n---\nOther\n",
  );
  return pluginRoot;
}
...
describe("engine findPluginBundledCandidates hephaestus model variants", () => {
  describe("#given hephaestus variant files for gpt-5.5 and gpt-5.6", () => {
    ...
    it("#when variants are gated #then other bundled rules stay unaffected", () => {
      ...
      expect(relativePaths(pluginRoot, "gpt-5.6")).toContain(
        "bundled-rules/other.md",
      );
