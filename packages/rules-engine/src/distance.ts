...
export function calculateDistance(
  ...
  const pathApi = usesWindowsPaths(projectRoot, rulePath, currentFile)
    ? win32
    : { dirname, isAbsolute, relative };
  const ruleRelative = pathApi.relative(projectRoot, pathApi.dirname(rulePath));
  const currentRelative = pathApi.relative(
    projectRoot,
    pathApi.dirname(currentFile),
  );
  if (
    isOutsideProject(ruleRelative, pathApi) ||
    isOutsideProject(currentRelative, pathApi)
  ...
  for (
    let index = 0;
    index < Math.min(ruleParts.length, currentParts.length);
    index += 1
  ) {
...
function isOutsideProject(
  relativePath: string,
  pathApi: Pick<typeof win32, "isAbsolute">,
): boolean {
