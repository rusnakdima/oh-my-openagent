  ...
  for (const name of skillNames) {
    ...
    if (match) {
      if (match.name === "git-master") {
        resolved.set(
          name,
          injectGitMasterConfig(match.template, options?.gitMasterConfig),
        );
      } else {
...
async function loadExactSkillFromRoot(
  ...
  const candidates = [
    ...
    {
      path: join(skillDirectory, `${skillName}.md`),
      resolvedPath: skillDirectory,
    },
...
async function loadConfiguredGitMasterSkill(
  options?: SkillResolutionOptions,
): Promise<LoadedSkill | null> {
  const directory = options?.directory ?? process.cwd();
  const rootGroups: Array<
    { readonly roots: readonly string[]; readonly scope: SkillScope }
  > = [
    {
      roots: findProjectOpencodeSkillDirs(directory),
  ...
  for (const group of rootGroups) {
    for (const root of group.roots) {
      const skill = await loadExactSkillFromRoot(
        root,
        group.scope,
        "git-master",
      );
...
async function resolveBuiltinSkillTemplate(
  skillName: string,
  options?: SkillResolutionOptions,
): Promise<string | null> {
  ...
  if (configuredSkill) {
    return injectGitMasterConfig(
      await extractSkillTemplate(configuredSkill),
...
export async function resolveMultipleSkillsAsync(
  ...
  if (unresolvedSkillNames.length === 0) {
    return {
      resolved: createOrderedResolvedMap(skillNames, resolvedTemplates),
      notFound: [],
    };
  }
...
function createOrderedResolvedMap(
  skillNames: string[],
