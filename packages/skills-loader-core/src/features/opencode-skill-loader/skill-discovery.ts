...
export function isDisabledSkillAlias(
  skill: LoadedSkill,
  disabledSkills: ReadonlySet<string>,
): boolean {
...
export async function getAllSkills(
  ...
  const cacheKey = `${directory}:${browserProvider}:${
    teamModeEnabled ? "team-on" : "team-off"
  }`;
  const hasDisabledSkills = options?.disabledSkills &&
    options.disabledSkills.size > 0;
  ...
  const browserSkillName = browserProvider === "openchrome-aside"
    ? "playwright"
    : browserProvider;
  ...
  const builtinSkillsAsLoaded: LoadedSkill[] = builtinSkillDefinitions.map((
    skill,
  ) => ({
    name: skill.name,
    definition: {
  ...
  const discoveredNames = new Set(
    filteredDiscoveredSkills.map((skill) => skill.name),
  );
  const uniqueBuiltins = builtinSkillsAsLoaded.filter((skill) =>
    !discoveredNames.has(skill.name)
  );
  ...
  if (hasDisabledSkills) {
    ...
    if (disabledSkills) {
      allSkills = allSkills.filter((skill) =>
        !isDisabledSkillAlias(skill, disabledSkills)
      );
    }
  } else {
