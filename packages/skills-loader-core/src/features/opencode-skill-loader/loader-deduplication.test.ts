...
`,
    ...
    it("prioritizes project Claude ulw-plan over shared ulw-plan", async () => {
      ...
      const opencodeConfigDir = join(TEST_DIR, "opencode-global");
      const projectClaudeUlwPlanDir = join(
        TEST_DIR,
        ".claude",
        "skills",
        "ulw-plan",
...
`,
    ...
    it("prioritizes project Agents ulw-plan over shared ulw-plan", async () => {
      ...
      const opencodeConfigDir = join(TEST_DIR, "opencode-global");
      const projectAgentsUlwPlanDir = join(
        TEST_DIR,
        ".agents",
        "skills",
        "ulw-plan",
...
`,
      ...
      try {
        const skills = await discoverSkills();
        const sharedPrefixed = skills.filter((skill) =>
          skill.name.startsWith("shared/")
        );
        const ulwPlanMatches = skills.filter((skill) =>
          skill.name === "ulw-plan"
        );
        ...
        expect(ulwPlanMatches[0]?.definition.description).toContain(
          "Local shadow",
        );
    ...
    it("treats a project skill literally named shared/ulw-plan as a plain name, not a canonical alias", async () => {
      ...
      const opencodeConfigDir = join(TEST_DIR, "opencode-global");
      const literalSharedNameDir = join(
        TEST_DIR,
        ".opencode",
        "skills",
        "shared-ulw-plan",
...
`,
      ...
      try {
        ...
        expect(literalMatches[0]?.definition.description).toContain(
          "Literal project skill",
        );
        expect(ulwPlanMatches).toHaveLength(1);
