...
`;
    ...
    //#when
    const { discoverGlobalAgentsSkills } = await import(
      `./loader?test=${crypto.randomUUID()}`
    );
    const skills = await discoverGlobalAgentsSkills(tempHome);
    const skill = skills.find((candidate: { readonly name: string }) =>
      candidate.name === "agent-global-skill"
    );
