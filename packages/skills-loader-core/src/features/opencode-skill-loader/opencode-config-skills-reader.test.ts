...
describe("readOpencodeConfigSkills", () => {
  ...
  beforeEach(() => {
    ...
    getOpenCodeConfigDirsSpy = spyOn(opencodeConfigDir, "getOpenCodeConfigDirs")
      .mockReturnValue([
        globalConfigDir,
      ]);
  });
  ...
  it("merges skills from multiple user config dirs", () => {
    ...
    const customDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "ohmo-custom-opencode-"),
    );
    const defaultDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "ohmo-default-opencode-"),
    );
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        skills: {
          paths: ["/custom/skill"],
          urls: ["https://custom.example.com/s.md"],
        },
      }),
    );
    fs.writeFileSync(
      ...
      JSON.stringify({
        skills: {
          paths: ["/default/skill"],
          urls: ["https://default.example.com/s.md"],
        },
      }),
    );
    ...
    try {
      ...
      expect(result?.urls).toEqual([
        "https://custom.example.com/s.md",
        "https://default.example.com/s.md",
      ]);
    } finally {
