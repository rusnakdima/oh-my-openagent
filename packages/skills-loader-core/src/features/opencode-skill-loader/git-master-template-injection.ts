...
const LEADING_GIT_COMMAND_PATTERN =
  /^([ \t]*(?:[A-Za-z_][A-Za-z0-9_]*=[^ \t]+\s+)*)git(?=[ \t]|$)/gm;
 ...
 */
export function buildShellAwareGitPrefix(
  bashPrefix: string,
  shellType?: ShellType,
): string {
...
export function injectGitMasterConfig(
  ...
  const gitEnvPrefix = assertValidGitEnvPrefix(
    config?.git_env_prefix ?? "GIT_MASTER=1",
  );

  const shellType = detectShellType();
  const shellPrefix = gitEnvPrefix
    ? buildShellAwareGitPrefix(gitEnvPrefix, shellType)
    : "";
  const codeBlockLang = shellType === "powershell"
    ? "pwsh"
    : shellType === "csh"
    ? "csh"
    : "bash";
  const skipBashBlockPrefixing = shellType === "powershell" ||
    shellType === "cmd" || shellType === "csh";

  let result = gitEnvPrefix
    ? injectGitEnvPrefix(template, shellPrefix, codeBlockLang)
    : template;
...
function prefixGitCommandsInBashCodeBlocks(
  template: string,
  prefix: string,
): string {
  return template.replace(
    BASH_CODE_BLOCK_PATTERN,
    (block, codeBlock: string) => {
      return block.replace(
        codeBlock,
        prefixGitCommandsInCodeBlock(codeBlock, prefix),
      );
    },
  );
}

function prefixGitCommandsInCodeBlock(
  codeBlock: string,
  prefix: string,
): string {
  return codeBlock
