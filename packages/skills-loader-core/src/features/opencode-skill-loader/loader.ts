...
export async function loadProjectSkills(
  ...
  const projectSkillDirs = findProjectClaudeSkillDirs(
    directory ?? process.cwd(),
  );
  const allSkills = await Promise.all(
    projectSkillDirs.map((skillsDir) =>
      loadSkillsFromDir({ skillsDir, scope: "project" })
    ),
  );
  return skillsToCommandDefinitionRecord(
    deduplicateSkillsByName(allSkills.flat()),
  );
}
...
> {
  ...
  const allSkills = await Promise.all(
    skillDirs.map((skillsDir) =>
      loadSkillsFromDir({ skillsDir, scope: "opencode" })
    ),
  );
  return skillsToCommandDefinitionRecord(
    deduplicateSkillsByName(allSkills.flat()),
...
export async function loadProjectAgentsSkills(
  ...
  const agentsProjectSkillDirs = findProjectAgentsSkillDirs(
    directory ?? process.cwd(),
  );
  const allSkills = await Promise.all(
    agentsProjectSkillDirs.map((skillsDir) =>
      loadSkillsFromDir({ skillsDir, scope: "project" })
    ),
  );
  return skillsToCommandDefinitionRecord(
...
export async function discoverSkills(
  ...
  const [opencodeProjectSkills, opencodeGlobalSkills, sharedSkills] =
    await Promise.all([
      discoverOpencodeProjectSkills(directory),
      discoverOpencodeGlobalSkills(),
      discoverSharedSkills(),
  ...
  const [projectSkills, userSkills, agentsProjectSkills, agentsGlobalSkills] =
    await Promise.all([
      discoverProjectClaudeSkills(directory),
      discoverUserClaudeSkills(),
      discoverProjectAgentsSkills(directory),
...
export async function discoverProjectClaudeSkills(
  directory?: string,
): Promise<LoadedSkill[]> {
  const projectSkillDirs = findProjectClaudeSkillDirs(
    directory ?? process.cwd(),
...
export async function discoverSharedSkills(): Promise<LoadedSkill[]> {
  return loadSkillsFromDir({
    skillsDir: sharedSkillsRootPath(),
    scope: "shared",
  });
}

export async function discoverProjectAgentsSkills(
  directory?: string,
): Promise<LoadedSkill[]> {
  const agentsProjectSkillDirs = findProjectAgentsSkillDirs(
    directory ?? process.cwd(),
