...
import {
  createOpenCodeSkillMarkdown,
  type OpenCodeSkillMarkdown,
} from "./skill-markdown";
...
export function selectRuntimeSecuritySkills(
  ...
  return [securityResearchSkill].map((skill) =>
    createOpenCodeSkillMarkdown(skill)
  );
}
...
export function applyRuntimeSkillSourceConfig(params: {
  ...
  const existingSkills = isPlainRecord(params.config.skills)
