...
import {
  ...
  playwrightSkill as directPlaywrightSkill,
} from "./playwright-mcp-skill";
...
describe("playwright browser skill facade", () => {
  ...
  test("#given absent or empty custom args #when creating builtin skills #then the legacy singleton is reused", () => {
    ...
    // when
    const defaultSkill = createBuiltinSkills().find((skill) =>
  ...
  test("#given agent-browser source markdown #when exposed through the split skill #then frontmatter is stripped and tool markers stay stable", async () => {
    // given
    const agentBrowserSkillFile = await Bun.file(
      "packages/omo-opencode/src/features/builtin-skills/agent-browser/SKILL.md",
    ).text();
    const { data, body, hadFrontmatter } = parseFrontmatter<
      { readonly name: string; readonly description: string }
    >(agentBrowserSkillFile);
