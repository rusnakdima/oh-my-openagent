...
async function readSharedSkill(
  name: TargetSkill,
): Promise<{ readonly frontmatter: string; readonly body: string }> {
  const content = await Bun.file(
    `packages/shared-skills/skills/${name}/SKILL.md`,
  ).text();
  const match = getRequiredMatch(
    content,
    /^---\n([\s\S]*?)\n---\n?([\s\S]*)$/,
    `${name} frontmatter`,
  );
...
describe("shared builtin skill extraction", () => {
  test("#given extracted builtin skill markdown #when compared to TS sources #then bodies and metadata stay byte-equivalent", async () => {
    ...
    for (const [index, source] of sources.entries()) {
      ...
      expect(sharedSkill.frontmatter).toContain(
  ...
  test("#given checked-in frontend builtin skill artifact #when compared to shared source #then it stays byte-equivalent", async () => {
    const sharedSkill = await Bun.file(
      "packages/shared-skills/skills/frontend/SKILL.md",
    ).text();
    const checkedInArtifact = await Bun.file(
      "packages/skills-loader-core/src/features/builtin-skills/frontend/SKILL.md",
    ).text();
  ...
  test("#given shared skill frontmatter #when parsed for Codex #then metadata is valid and budgeted", async () => {
    ...
    for (const path of findSkillFiles()) {
      ...
      if (parsed.data.description.length > CODEX_SKILL_DESCRIPTION_MAX_LENGTH) {
        failures.push(
          `${path}: description length ${parsed.data.description.length}`,
        );
      }
    }
