...
export function getOpenCodeCommandDirs(
    ...
    new Set([
      ...configDirs.flatMap((configDir) => {
        ...
        return [
          ...
          ...(parentConfigDir
            ? [
              join(parentConfigDir, "commands"),
              join(parentConfigDir, "command"),
            ]
            : []),
        ];
      }),
...
export function getOpenCodeSkillDirs(
  options: OpenCodeConfigDirOptions,
): string[] {
  const configDirs = getOpenCodeConfigDirs(options);
  return Array.from(
    new Set([
      ...configDirs.flatMap((configDir) => {
        ...
        return [
          ...
          ...(parentConfigDir
            ? [join(parentConfigDir, "skills"), join(parentConfigDir, "skill")]
            : []),
        ];
      }),
    ]),
