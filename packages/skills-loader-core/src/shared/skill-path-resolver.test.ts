...
describe("resolveSkillPathReferences", () => {
  it("resolves @path references containing a slash to absolute paths", () => {
    ...
    const basePath = "/home/user/.config/opencode/skills/frontend";
    ...
    //#then
    expect(result).toBe(
      "Run `python3 /home/user/.config/opencode/skills/frontend/scripts/search.py` to search",
    );
  });
  ...
  it("does not resolve npm scoped packages in commands", () => {
    //#given
    const content =
      "npx --package=@mycom/my_mcp_tools@beta cli my_cmd_tool XXX";
  ...
  it("normalizes Windows native paths for prompt display", () => {
    ...
    //#then
    expect(result).toBe(
      "C:/Users/Admin/.config/opencode/skills/frontend/scripts/search.py",
    );
  });
