...
const { mkdtempSync, readFileSync, rmSync, writeFileSync } = await loadModule<
  FsModule
>("node:fs");
const { tmpdir } = await loadModule<OsModule>("node:os");
const { join } = await loadModule<PathModule>("node:path");
const { spawnSync } = await loadModule<ChildProcessModule>(
  "node:child_process",
);
...
interface CapturedProvider {
  streamSimple(
    ...
    result?: () => Promise<
      { stopReason?: string; content?: Array<{ type?: string }> }
    >;
  };
}
