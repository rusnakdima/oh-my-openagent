...
export function memberTaskId(cwd, teamRunId, memberName) {
  const map = readJsonIfPresent(
    join(runtimeDir(cwd, teamRunId), "senpi-task-members.json"),
  );
...
export function reservedMessagePath(cwd, teamRunId, recipient, messageId) {
  return join(
    memberInboxDir(cwd, teamRunId, recipient),
    `${RESERVED_PREFIX}${messageId}.json`,
  );
}
...
export function sessionEnvelopeCount(cwd, taskId, messageId) {
  ...
  return sessionStringValues(cwd, taskId)
    .filter((value) =>
      value.includes("<peer_message ") && value.includes(marker)
    )
    .length;
}

export function sessionContainsText(cwd, taskId, needle) {
  return sessionStringValues(cwd, taskId).some((value) =>
...
export function deliveredEventCount(cwd, taskId, messageId) {
  ...
  for (const line of taskEventText(cwd, taskId).split(/\r?\n/)) {
    ...
    if (
      event?.type === "team_message_delivered" &&
...
export function inboxCounts(inboxDir) {
  ...
  if (existsSync(processedDir)) {
    processed = readdirSync(processedDir).filter((name) =>
...
export function seedCrashReservation(inboxDir, ageMs, memberName) {
  ...
  const message = buildMessage(
    "teammate",
    memberName,
    "RECLAIM-CRASH-RESERVATION redeliver me",
  );
  const reservedPath = join(
    inboxDir,
    `${RESERVED_PREFIX}${message.messageId}.json`,
  );
  ...
  return {
    ...
    reservedPath,
    restoredPath: join(inboxDir, `${message.messageId}.json`),
  };
}
...
function sessionStringValues(cwd, taskId) {
  ...
  for (const entry of readdirSync(sessionDir)) {
    ...
    for (
      const line of readFileSync(join(sessionDir, entry), "utf8").split(/\r?\n/)
    ) {
      if (line.trim().length === 0) continue;
      try {
