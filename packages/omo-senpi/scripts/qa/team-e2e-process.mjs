...
export async function terminateProcessTree(pid, operations = {}) {
  ...
  if (platform === "win32") {
    ...
    if (result.status === 0 && !aliveAfter) {
      return { kind: "terminated", pid, platform };
    }
    return {
      kind: "failed",
      pid,
  ...
  try {
    ...
    if (isMissingProcess(error)) {
      return { kind: "already-exited", pid, platform };
    }
    return {
      kind: "failed",
      pid,
  ...
  return {
    ...
    platform,
    status: null,
    error: "process group remained alive after SIGKILL",
  };
}
...
export async function cleanupProcessGroups(groupIds, operations = {}) {
  ...
  for (const groupId of groupIds) {
    ...
    leaked += listGroupPids(groupId).filter((pid) =>
      pid !== process.pid
    ).length;
  }
  return leaked;
