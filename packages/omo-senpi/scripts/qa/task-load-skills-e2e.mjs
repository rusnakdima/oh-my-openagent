      ...
      JSON.stringify(
        {
          categories: {
            mockcat: {
              description: "Local mock category.",
...
function selfTest() {
  const details = skillResult([{
    ...
    result: {
      details: {
        skills: { requested: [skillName], resolved: [skillName], missing: [] },
      },
    },
  }]);
  if (details?.resolved?.[0] !== skillName) {
    throw new Error("skill result parser failed");
  }
  if (count(`x${skillMarker}y${skillMarker}`, skillMarker) !== 2) {
    throw new Error("marker counter failed");
  }
...
function main() {
  ...
  const snapshots = realAgentDirs.map((dir) => ({
    dir,
    before: credentialDigest(dir),
  }));
  ...
  try {
    ...
    const taskPath = typeof taskId === "string"
      ...
      : "";
    const taskRecord = taskPath && existsSync(taskPath)
    ...
    const checks = {
      ...
      skill_summary: JSON.stringify(details) === JSON.stringify({
        ...
        ? "PASS"
        : "FAIL",
      result_warning: stdout.includes(`Missing skills: ${missingSkillName}`)
        ? "PASS"
        : "FAIL",
      spawn_prompt: spawnPrompt.includes(skillMarker) &&
          count(spawnPrompt, skillMarker) === 1
        ? "PASS"
        : "FAIL",
      child_session:
        transcript.includes(skillMarker) && count(transcript, skillMarker) === 1
          ? "PASS"
          : "FAIL",
