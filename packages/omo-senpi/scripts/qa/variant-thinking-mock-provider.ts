...
const { appendFileSync, existsSync, readFileSync } = process.getBuiltinModule<
  FsModule
>("fs");
...
function loadChildSteps(): ReadonlyArray<Record<string, unknown>> {
  ...
  if (!existsSync(path)) {
    return [{ type: "text", text: "variant qa child done" }];
  }
  const script: unknown = JSON.parse(readFileSync(path, "utf8"));
  const steps = typeof script === "object" && script !== null
    ? Reflect.get(script, "childSteps")
    : undefined;
  return Array.isArray(steps)
    ? steps
    : [{ type: "text", text: "variant qa child done" }];
}

function appendCapture(body: unknown): void {
  if (typeof body !== "object" || body === null) return;
  const messages = Reflect.get(body, "messages");
  const child = messagesContainChild({
