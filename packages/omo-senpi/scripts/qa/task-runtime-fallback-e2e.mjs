...
const scenarios = [
  {
    ...
    checks: (artifacts, stdoutText) => ({
      ...
      fallback_event: artifacts.log.includes("retry_fallback_applied")
        ? "PASS"
        : "FAIL",
      final_model:
        artifacts.task?.model === "omo-fallback-mock/healthy-fallback"
          ? "PASS"
          : "FAIL",
      fallback_attempts: JSON.stringify(
          artifacts.task?.fallback_attempts?.map((model) =>
  ...
  },
  {
    ...
    checks: (artifacts, stdoutText) => ({
      ...
      fallback_event: artifacts.log.includes("retry_fallback_applied")
        ? "PASS"
        : "FAIL",
      final_model: artifacts.task?.model === "quotio-openai/gpt-5.6-luna-fast"
        ? "PASS"
        : "FAIL",
      requested_model: artifacts.task?.requested_model?.display ===
  ...
  },
  {
    ...
    omoConfig: {},
    checks: (artifacts) => ({
      exhausted_event: artifacts.log.includes("retry_fallback_exhausted")
        ? "PASS"
        : "FAIL",
...
function readTaskArtifacts(stateDir) {
  ...
  const names = existsSync(tasksDir)
    ...
    : [];
  const taskFile = names[0] === undefined
    ? undefined
    : join(tasksDir, names[0]);
  const task = taskFile === undefined
    ? undefined
    : JSON.parse(readFileSync(taskFile, "utf8"));
  const logFile = task === undefined
    ? undefined
    : join(stateDir, "logs", `${task.task_id}.jsonl`);
  const log = logFile !== undefined && existsSync(logFile)
...
function runScenario(scenario, outDir) {
  ...
  try {
    ...
    writeFileSync(
      ...
      `${JSON.stringify(scenario.omoConfig, null, 2)}\n`,
    );
    ...
    writeFileSync(
      join(scenarioOutDir, "stdout.json.log"),
      runResult?.stdout ?? "",
    );
    writeFileSync(join(scenarioOutDir, "stderr.log"), runResult?.stderr ?? "");
    writeFileSync(
      join(scenarioOutDir, "task.json"),
      `${JSON.stringify(artifacts.task ?? {}, null, 2)}\n`,
    );
  ...
  const checks = {
    ...
    real_credentials_untouched: afterCredentials === beforeCredentials
      ? "PASS"
      : "FAIL",
    cleanup,
  };
  ...
  writeFileSync(
    join(scenarioOutDir, "verdict.json"),
...
function run() {
  const outDir = resolve(
    process.env.TASK_RUNTIME_FALLBACK_OUT_DIR ??
      join(process.cwd(), ".omo", "evidence", "task-runtime-fallback"),
  );
  mkdirSync(outDir, { recursive: true });
  const verdicts = scenarios.map((scenario) => runScenario(scenario, outDir));
  const result = verdicts.every((verdict) => verdict.result === "PASS")
    ? "PASS"
    : "FAIL";
...
function selfTest() {
  const parsed = parseJsonEvents(
    `${JSON.stringify({ type: "text", text: finalText })}\n`,
  );
  if (!JSON.stringify(parsed).includes(finalText)) {
    throw new Error("event parser did not preserve final text");
  }
