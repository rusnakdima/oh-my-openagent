...
const excludedSnapshotRoots = new Set([
  "omo-local-update",
  "omo-runtime-worktree",
]);
...
function prepareScenario() {
  ...
  copyFileSync(
    join(packagedPluginRoot, "extensions", "omo.js"),
  ...
  writeFileSync(
    ...
    `${
      JSON.stringify(
        { defaultProjectTrust: "ask", packages: [pluginRoot] },
        null,
        2,
      )
    }\n`,
          ...
          steps: [
            {
              ...
              arguments: {
                objective:
                  "Keep the real TUI open while the active ulw-loop footer animates.",
              },
            },
            {
              type: "tool_call",
            ...
            },
            {
              type: "text",
              text:
                "The goal and ulw-loop are active. Observe the footer animation.",
            },
          ],
        },
        null,
...
async function runScenario() {
  ...
  const beforeRealLog = existsSync(realLogPath)
  ...
  const cleanup = (reason) => {
    ...
    const changedReal = changedRealPaths(
      beforeSnapshot,
      snapshotWriteSurface(realSenpiAgentDir),
    );
    const afterRealLog = existsSync(realLogPath)
      ? readFileSync(realLogPath, "utf8")
      : "";
    const qaLogWrite = afterRealLog.slice(beforeRealLog.length).includes(
      basename(prepared.sandbox.root),
    );
    const concurrentLogPaths =
      changedReal.includes("logs/session.log") && !qaLogWrite
        ? ["logs/session.log"]
        : [];
    const classified = classifyRealSenpiChanges(
      changedReal.filter((path) => path !== "logs/session.log" || qaLogWrite),
      [basename(prepared.sandbox.root)],
    );
    const payload = {
      result: senpiBin !== null && classified.qaAttributedPaths.length === 0
        ? "PASS"
        : "FAIL",
      reason,
      ...
      concurrentSessionPaths: [
        ...classified.concurrentSessionPaths,
        ...concurrentLogPaths,
      ],
...
function runSelfTest() {
  if (!parseArgs(["--self-test"]).selfTest) {
    throw new Error("self-test: argument parser failed");
  }
  const prepared = prepareScenario();
  ...
  if (command.args.includes("-p") || command.args.includes("--mode")) {
    throw new Error("self-test: command must stay TUI-native");
  }
  if (
    !command.args.includes("create_goal") &&
    !existsSync(join(prepared.sandbox.cwd, "mock-script.json"))
  ) {
  ...
  if (env.OMO_BIN !== prepared.omoBin || !existsSync(prepared.omoBin)) {
    throw new Error("self-test: fake omo missing");
  }
  if (
    prepared.pluginRoot === undefined ||
    !existsSync(join(prepared.pluginRoot, "extensions", "omo.js"))
  ) {
    throw new Error("self-test: minimal plugin missing");
  }
  const settings = JSON.parse(
    readFileSync(join(prepared.sandbox.agentDir, "settings.json"), "utf8"),
  );
  if (settings.packages?.[0] !== prepared.pluginRoot) {
    throw new Error("self-test: minimal plugin not configured");
  }
  const manifest = JSON.parse(
    readFileSync(join(prepared.pluginRoot, "package.json"), "utf8"),
  );
  if (manifest.pi?.skills !== undefined) {
    throw new Error("self-test: minimal plugin must not scan skills");
  }
  ...
  const snapshotChanges = changedRealPaths(
    beforeSnapshot,
    snapshotWriteSurface(prepared.sandbox.agentDir),
  );
  if (
    !snapshotChanges.includes("settings.json") ||
    snapshotChanges.some((path) => path.startsWith("omo-runtime-worktree/"))
  ) {
...
async function main() {
  ...
  if (args.help) {
    console.log(
      "Usage: node packages/omo-senpi/scripts/qa/ulw-goal-footer-tui.mjs [--self-test]",
    );
    return;
  }
