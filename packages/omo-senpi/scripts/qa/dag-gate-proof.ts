...
const deps = {
  manager,
  parentSessionId: () => "ses-proof",
  rootSessionId: () => "ses-proof",
};
...
console.log(
  violating.content[0]?.type === "text"
    ? violating.content[0].text
    : "(non-text)",
);
console.log("=== violating definition: warnings payload ===");
console.log(
  JSON.stringify(
    violating.details.kind === "started"
      ? violating.details.warnings
      : "(error)",
    null,
    2,
  ),
  ...
  definition: {
    ...
    nodes: [
      ...
      },
      {
        id: "verify",
        prompt:
          "TASK: verify the plan. DELIVERABLE: transcript. SCOPE: read-only. VERIFY: bun test. STOP WHEN: tests pass.",
        category: "quick",
        dependsOn: ["plan"],
      },
    ],
...
console.log(
  compliant.content[0]?.type === "text"
    ? compliant.content[0].text
    : "(non-text)",
);
console.log("=== compliant definition: warnings payload ===");
console.log(
  JSON.stringify(
    compliant.details.kind === "started"
      ? compliant.details.warnings
      : "(error)",
  ),
);
