...
async function runCancelLane(ctx) {
  ...
  const seeded = await pollUntil(
    () => ({
      cancelled:
        findTaskByName(sandbox.cwd, "cancelchild")?.status === "cancelled",
      witnessDone:
        findTaskByName(sandbox.cwd, "cancelwitness")?.status === "completed",
    }),
  ...
  ctx.checks.resume_cancel_setup = verdict(
    result1.status === 0 && seeded.cancelled && seeded.witnessDone,
  );
  ...
  const run2 = startRun(
    ctx,
    sandbox,
    resumeProbeScript(sentinel2),
    sessionIdFromEvents(result1.events),
  );
  const witnessUp = await pollUntil(
    () => witnessRevived(sandbox.cwd, "cancelwitness"),
    (v) => v === true,
    POLL_MS,
  );
...
async function runKillLane(ctx) {
  ...
  const seeded = await pollUntil(
    () => ({
      targetDone:
        findTaskByName(sandbox.cwd, "killchild")?.status === "completed",
      witnessDone:
        findTaskByName(sandbox.cwd, "killwitness")?.status === "completed",
    }),
  ...
  ctx.checks.resume_kill_setup = verdict(
    result1.status === 0 && seeded.targetDone && seeded.witnessDone &&
      killTask !== undefined,
  );
  ...
  const witnessUp = await pollUntil(
    () => witnessRevived(sandbox.cwd, "killwitness"),
    (v) => v === true,
    POLL_MS,
  );
  writeFileSync(sentinel2, "go\n");
...
async function runLruLane(ctx) {
  ...
  ctx.checks.resume_lru_setup = verdict(
    result1.status === 0 && seeded.evicted && seeded.newestDone &&
      lruOne !== undefined,
  );
  ...
  const newestUp = await pollUntil(
    () => witnessRevived(sandbox.cwd, "lrutwo"),
    (v) => v === true,
    POLL_MS,
  );
  writeFileSync(sentinel2, "go\n");
...
async function runTtlLane(ctx) {
  ...
  const seeded = await pollUntil(
    () => ({
      targetDone:
        findTaskByName(sandbox.cwd, "ttlchild")?.status === "completed",
      witnessAssistant: childSessionHasAssistant(
        sandbox.cwd,
        findTaskByName(sandbox.cwd, "ttlwitness")?.task_id ?? "",
      ),
    }),
  ...
  ctx.checks.resume_ttl_setup = verdict(
    result1.status === 0 && seeded.targetDone && seeded.witnessAssistant &&
      ttlTask !== undefined,
  );
  const run2 = startRun(ctx, sandbox, UNRELATED_SESSION_SCRIPT);
  const result2 = await run2.completion;
  writeLaneLog(ctx, "resume-ttl-run2", result2);
  const expunged = ttlTask !== undefined &&
    !recordFileExists(sandbox.cwd, ttlTask.task_id) &&
    taskEventText(sandbox.cwd, ttlTask.task_id) === "";
  ctx.checks.resume_ttl_expunged = verdict(
    result2.status === 0 && expunged &&
      findTaskByName(sandbox.cwd, "ttlwitness") !== undefined,
  );
  const sentinel3 = join(sandbox.root, "ttl-release-3");
  const run3 = startRun(
    ctx,
    sandbox,
    resumeProbeScript(sentinel3),
    sessionIdFromEvents(result1.events),
  );
  const continued = await pollUntil(
    () =>
      childSessionText(
        sandbox.cwd,
        findTaskByName(sandbox.cwd, "ttlwitness")?.task_id ?? "",
      ).includes(MIDTURN_CONTINUED_TOKEN),
    (v) => v === true,
    POLL_MS,
  );
