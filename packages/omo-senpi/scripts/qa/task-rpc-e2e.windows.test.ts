...
const driverPath = fileURLToPath(
  new URL("./task-rpc-e2e.mjs", import.meta.url),
);
  ...
  async () => {
    ...
    const driver = spawn(process.execPath, [driverPath], {
      ...
      env: {
        ...
        SENPI_CODING_AGENT_DIR: join(
          tmpdir(),
          `omo-rpc-driver-caller-${process.pid}`,
        ),
      },
    ...
    try {
      ...
      if (!closeObserved) {
        ...
        const cleanupDeadline = createDeadline(
          CLEANUP_TIMEOUT_MS,
          "Windows production RPC driver cleanup",
        );
        try {
    ...
    if (
      exit.code !== 0 || exit.signal !== null || stderr !== "" ||
    ...
    if (
      ...
      throw new Error(
        `WINDOWS_TASK_RPC_E2E invalid payload ${
          JSON.stringify({ parsed, diagnostic })
        }`,
      );
    }
