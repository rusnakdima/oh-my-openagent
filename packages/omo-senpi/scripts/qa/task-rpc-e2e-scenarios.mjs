...
const { createSandbox, seedSandbox } = await import(
  pathToFileURL(join(scriptDir, "drive.mjs")).href
);
const { readRecords, readTaskEventTypes, pidAlive, pollRecord, sleep } =
  await import(pathToFileURL(join(scriptDir, "task-rpc-e2e-helpers.mjs")).href);
const { killProcessGroup } = await import(
  pathToFileURL(join(scriptDir, "team-e2e-process.mjs")).href
);
...
const PROJECT_OMO_CONFIG = {
  ...
  categories: {
    proc: {
      description: "Process-mode mock category.",
...
export const SCENARIO_A_STEPS = [
  {
    ...
    arguments: {
      ...
      name: "p1",
      prompt: "Do the rpc child work and stop.",
    },
  },
  {
    type: "tool_call",
    name: "task_send",
    arguments: { to: "p1", message: "steer: keep going" },
  },
  {
    type: "tool_call",
    name: "task_output",
    arguments: { name: "p1", mode: "status" },
  },
  {
    type: "tool_call",
    name: "task_output",
...
const hangingChildSteps = (name) => [
  {
    ...
    arguments: {
      ...
      run_in_background: true,
      name,
      prompt: "hang until signalled",
    },
  },
  {
    type: "tool_call",
    name: "task_output",
    arguments: { name, mode: "status" },
  },
  { type: "hang" },
];

const runningRpcChild = (r) =>
  r.execution_mode === "process" && r.status === "running" &&
  typeof r.pid === "number";
...
function writeScript(sandbox, parentSteps, childSteps) {
  writeFileSync(
    ...
    `${JSON.stringify({ parentSteps, childSteps }, null, 2)}\n`,
  );
}
...
export function driveSenpi(
  senpiBin,
  sandbox,
  sessionDir,
  parentSteps,
  childSteps = CHILD_STEPS_COMPLETE,
  prompt = "run the rpc-process task e2e",
) {
...
function waitForChildClose(child, timeoutMs) {
  ...
  return new Promise((resolve, reject) => {
    const timeout = setTimeout(() => {
      ...
      reject(
        new Error(
          `timed out waiting ${timeoutMs}ms for pid=${
            child.pid ?? "unknown"
          } to close`,
        ),
      );
...
async function cleanupSenpiHost(child) {
  ...
  if (!terminated) {
    throw new Error(
      `could not terminate Senpi host pid=${child.pid ?? "unknown"}`,
    );
  }
...
export async function runKillCheck(senpiBin) {
  ...
  const parent = driveSenpiAsync(
    senpiBin,
    sandbox,
    sessionDir,
    hangingChildSteps("pk"),
    CHILD_STEPS_HANG,
    "drive the kill scenario",
  );
  try {
    const running = await pollRecord(
      stateDir,
      (r) => r.name === "pk" && runningRpcChild(r),
      40_000,
    );
    ...
    const errored = await pollRecord(
      stateDir,
      (r) =>
        r.task_id === running.task_id && r.status === "error" &&
        r.killed === true,
      15_000,
    );
    return {
      ...
      ...(errored
        ? {}
        : { reason: "kill did not yield status=error killed:true" }),
      facts: {
        pid: running.pid,
        killed: errored?.killed ?? false,
        error_excerpt: (errored?.error_message ?? "").slice(0, 120),
      },
    };
  } finally {
...
export async function runReconcileCheck(senpiBin) {
  const { sandbox, sessionDir, stateDir } = prepareScenarioSandbox(
    RECONCILE_PROJECT_OMO_CONFIG,
  );
  const parent = driveSenpiAsync(
    senpiBin,
    sandbox,
    sessionDir,
    hangingChildSteps("pr"),
    CHILD_STEPS_HANG,
    "drive the reconcile scenario",
  );
  let orphanPid;
  try {
    const running = await pollRecord(
      stateDir,
      (r) => r.name === "pr" && runningRpcChild(r),
      40_000,
    );
    if (running === undefined) {
      return {
        ...
        verdict: "FAIL",
        reason: "no running rpc child appeared to reconcile",
      };
    }
    ...
    const relaunch = driveSenpi(
      ...
      sandbox,
      sessionDir,
      RECONCILE_RELAUNCH_STEPS,
      CHILD_STEPS_COMPLETE,
      "relaunch for reconcile",
    );
    const lost = readRecords(stateDir).find((r) =>
      r.task_id === running.task_id && r.status === "lost"
    );
    ...
    const pass = relaunch.status === 0 && lost !== undefined && lostEvent &&
      orphanDead;
    return {
      check: "reconcile_lost_terminates_orphan",
      verdict: pass ? "PASS" : "FAIL",
      ...(pass ? {} : {
        reason: `relaunchOk=${relaunch.status === 0} lostRecord=${
          lost !== undefined
        } lostEvent=${lostEvent} orphanDead=${orphanDead}`,
      }),
      facts: {
        orphanPid,
        orphanDead,
