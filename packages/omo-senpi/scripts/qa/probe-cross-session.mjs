...
const toolkitExecutable = process.platform === "win32"
  ? "omo-agent-toolkit.cmd"
  : "omo-agent-toolkit";
const toolkitBin = join(
  repoRoot,
  "packages/omo-senpi/plugin/runtime/agent-toolkit",
  toolkitExecutable,
);
...
if (process.argv[2] === "--extension-child") {
  ...
  const result = await runExtensionChild(
    sessionId === NO_SESSION ? null : sessionId,
  );
...
try {
  runToolkit(
    [
      ...
      "create-goals",
      "--brief",
      "- Session A owns this active run",
      "--json",
    ],
    sharedCwd,
    "session-A",
  ...
  const paths = {
    ownerPlan: existsSync(
      join(sharedCwd, ".omo/ulw-loop/session-A/goals.json"),
    ),
    unrelatedPlan: existsSync(
      join(sharedCwd, ".omo/ulw-loop/session-B/goals.json"),
    ),
  ...
  assert(
    ...
    `session A continuation count was ${sessionA.messageCount}, expected 1`,
  );
  assert(
    sessionB.messageCount === 0,
    `session B continuation count was ${sessionB.messageCount}, expected 0`,
  );
  ...
  try {
    runToolkit(
      [
        ...
        "create-goals",
        "--brief",
        "- An unscoped legacy run nobody owns",
        "--json",
      ],
      cwd,
      null,
...
async function runExtensionChild(sessionId) {
  const [{ FakeExtensionAPI }, { createUlwLoopComponent }] = await Promise.all([
    import(
      pathToFileURL(
        join(repoRoot, "packages/omo-senpi/test-support/fake-extension-api.ts"),
      ).href
    ),
    import(
      pathToFileURL(
        join(repoRoot, "packages/omo-senpi/src/components/ulw-loop/index.ts"),
      ).href
    ),
  ]);
...
function runChild(cwd, sessionId) {
  ...
  if (child.status !== 0) {
    throw new Error(
      `extension child ${sessionId} failed: ${child.stderr || child.stdout}`,
    );
  }
...
function runToolkit(args, cwd, sessionId, expectedStatus) {
  ...
  if (child.status !== expectedStatus) {
    throw new Error(
      `toolkit exited ${child.status}: ${child.stderr || child.stdout}`,
    );
  }
  return child.stdout;
}
