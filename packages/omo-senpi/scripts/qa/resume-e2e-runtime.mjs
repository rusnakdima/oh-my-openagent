...
export function startResumeRun(input) {
  writeFileSync(
    join(input.sandbox.cwd, "mock-script.json"),
  ...
  const child = spawn(input.senpiBin, args, {
    ...
    env: {
      ...
      ...(input.sandbox.homeDir === undefined
        ? {}
        : { HOME: input.sandbox.homeDir, USERPROFILE: input.sandbox.homeDir }),
...
export function waitForFileCommand(path) {
  const script =
    `const fs=require('fs');const p=process.argv[1];const until=Date.now()+60000;(function poll(){if(fs.existsSync(p))process.exit(0);if(Date.now()>until){console.error('timed out waiting for '+p);process.exit(1)}setTimeout(poll,50)})()`;
...
export function waitForRecordStatusCommand(cwd, name, status) {
  ...
  const script =
    `const fs=require('fs'),path=require('path');const dir=process.argv[1],name=process.argv[2],want=process.argv[3];const until=Date.now()+60000;function hit(){if(!fs.existsSync(dir))return false;for(const f of fs.readdirSync(dir)){if(!f.endsWith('.json'))continue;try{const r=JSON.parse(fs.readFileSync(path.join(dir,f),'utf8'));if(r.name===name&&r.status===want)return true}catch(e){/* mid-write record file: skip and retry on next poll */}}return false}(function poll(){if(hit())process.exit(0);if(Date.now()>until){console.error('timed out waiting for record '+name+' status '+want);process.exit(1)}setTimeout(poll,50)})()`;
  return `node -e "${script}" ${JSON.stringify(dir)} ${JSON.stringify(name)} ${
...
export function revivedAfterSuspend(logText) {
  ...
  const suspendIndex = lines.findIndex((line) =>
    line.includes('"type":"suspended"')
  );
  if (suspendIndex < 0) return false;
  return lines
    .slice(suspendIndex + 1)
    .some((line) =>
      line.includes('"type":"reconcile_reattached"') ||
      line.includes('"resident"')
    );
}
...
export function sessionIdFromEvents(events) {
  const header = events.find((event) =>
    event?.type === "session" && typeof event?.id === "string"
  );
  return header?.id;
}
