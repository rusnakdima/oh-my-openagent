...
async function main() {
  ...
  try {
    ...
    if (!existsSync(pluginEntry)) {
      throw new Error(`plugin entry missing: ${pluginEntry}`);
    }
...
function regeneratePlugin() {
  ...
  const result = spawnSync(process.execPath, [
    join(pluginRoot, "scripts", "build-extension.mjs"),
  ], {
    cwd: resolve(packageRoot, "..", ".."),
    encoding: "utf8",
    timeout: 120_000,
  });
  if (result.status !== 0 && !existsSync(pluginEntry)) {
...
async function runAttempt(input) {
  ...
  const parents = ["parent-a", "parent-b"].map((name) =>
    createParent(root, name, projectDir, input.timeoutMs)
  );
  const runs = [];
  let cleanup = {
    pids: [],
  ...
  try {
    ...
    for (const parent of parents) {
      runs.push(
        startSenpi(input.senpiBin, parent, projectDir, input.timeoutMs),
      );
    }
    await Promise.all(
      parents.map((parent) => waitForFile(parent.readyFile, input.timeoutMs)),
    );
    buckets.before = Math.floor(Date.now() / 65_536);
    for (const parent of parents) {
      writeFileSync(parent.goFile, "go\n", { flag: "wx" });
    }
    const completions = await Promise.all(
      runs.map((run) =>
        waitForCompletion(run, input.timeoutMs, attemptDir, "post-go")
      ),
    );
    ...
    for (let index = 0; index < completions.length; index += 1) {
      writeFileSync(
        join(attemptDir, `${parents[index].name}.stdout.jsonl`),
        completions[index].stdout,
      );
      writeFileSync(
        join(attemptDir, `${parents[index].name}.stderr.log`),
        completions[index].stderr,
      );
      writeJson(join(attemptDir, `${parents[index].name}.exit.json`), {
        status: completions[index].status,
        signal: completions[index].signal,
      });
      copySessionTranscript(
        parents[index].sessionDir,
        join(attemptDir, `${parents[index].name}.sessions`),
      );
      if (completions[index].status !== 0) {
        reasons.push(
          `${parents[index].name} senpi exited ${
            completions[index].status ?? completions[index].signal ?? "unknown"
          }`,
        );
      }
    }
    const transcript = completions.map((completion) =>
      `${completion.stdout}\n${completion.stderr}`
    ).join("\n");
    if (transcript.includes(COLLISION_TEXT)) {
      reasons.push(`transcript contains ${JSON.stringify(COLLISION_TEXT)}`);
    }
    if (taskIds.length !== 2) {
      reasons.push(
        `expected exactly 2 task records, observed ${taskIds.length}`,
      );
    }
    if (new Set(taskIds).size !== taskIds.length) {
      reasons.push("task ids are not unique");
    }
    if (buckets.before !== buckets.after) {
      reasons.push(`bucket changed from ${buckets.before} to ${buckets.after}`);
    }
    writeFileSync(join(attemptDir, "tasks-before.ls.txt"), before);
    writeFileSync(join(attemptDir, "tasks-after.ls.txt"), after);
    writeJson(join(attemptDir, "run.json"), {
      buckets,
      taskIds,
      reasons,
      ...input.metadata,
    ...
    writeFileSync(join(attemptDir, "tasks-before.ls.txt"), before);
    writeFileSync(
      join(attemptDir, "tasks-after.ls.txt"),
      after || lsTasks(projectDir),
    );
  } finally {
  ...
  return {
    ...
    bucketStable: buckets.before !== null && buckets.before === buckets.after,
    buckets,
    taskIds,
    reasons,
  };
...
function seedProject(projectDir) {
  ...
  writeJson(join(projectDir, ".omo", "omo.json"), {
    categories: {
      mockcat: {
        description: "local QA-only mock category",
...
function startSenpi(senpiBin, parent, projectDir, timeoutMs) {
  const child = spawn(senpiBin, [
    ...
    "--session-dir",
    parent.sessionDir,
    `launch the ${parent.name} background race task`,
  ], {
    cwd: projectDir,
    env: {
      ...process.env,
  ...
  return Promise.race([
    ...
    new Promise((_, reject) => {
      timer = setTimeout(() => {
        ...
        writeJson(
          join(artifactDir, `${run.name}.${phase}.timeout.json`),
          timeout,
        );
        reject(
          new Error(
            `timed out after ${timeoutMs}ms waiting for ${run.name} during ${phase}`,
          ),
        );
      }, timeoutMs);
    }),
...
function waitForFile(path, timeoutMs) {
  ...
  return new Promise((resolvePromise, reject) => {
    ...
    const timer = setTimeout(
      () =>
        finish(new Error(`timed out after ${timeoutMs}ms waiting for ${path}`)),
      timeoutMs,
    );
    if (existsSync(path)) finish();
  });
...
async function cleanupRuns(runs, root, timeoutMs, artifactDir) {
  const pids = runs.map((run) => run.pid).filter((pid) =>
  ...
  await Promise.all(
    runs.map((run) =>
      waitForCompletion(run, timeoutMs, artifactDir, "teardown")
    ),
  );
  const verified_dead = pids.filter((pid) => !isAlive(pid));
  if (verified_dead.length !== pids.length) {
    throw new Error(
      `cleanup could not terminate process groups: ${
        pids.filter((pid) => isAlive(pid)).join(", ")
      }`,
    );
  }
  ...
  return {
    ...
    terminated,
    verified_dead,
    sandbox_removed: !existsSync(root),
  };
}
...
function lsTasks(projectDir) {
  ...
  const result = spawnSync("ls", ["-la", tasksDir], {
    encoding: "utf8",
    timeout: 30_000,
  });
...
function taskIdsFromListing(listing) {
  ...
  ).filter((entry) => /^st_[0-9a-f]{8}\.json$/.test(entry)).map((entry) =>
    entry.slice(0, -5)
  );
}
...
function parseArgs(args) {
  ...
  if (typeof evidenceDir !== "string" || evidenceDir.length === 0) {
    throw new Error(
      "usage: node task-id-race-qa.mjs --evidence-dir <dir> [--timeout-ms 120000]",
    );
  }
