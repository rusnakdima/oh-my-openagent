...
import {
  ...
  hasCrashLivenessEvent,
  runCrashRestartScenario,
} from "./team-e2e-crash.mjs";
import {
  createOutDir,
  resolveSenpi,
  TEAM_E2E_OMO_CONFIG,
...
describe("team e2e crash recovery evidence", () => {
  it("#given both processes were live and ordered termination completed #when crash recovery is evaluated #then the kill and exact-mailbox contract pass", () => {
    const checks = evaluateCrashRecovery({
      ...
      afterRestartRecord: {
        notification: { run_epoch: 0, liveness_notified_epoch: 0 },
      },
    });
    ...
    try {
      const checks = await runCrashRestartScenario({
        ...
        createSandbox: () => ({
          root: sandboxRoot,
          cwd,
          agentDir: join(sandboxRoot, "agent"),
          xdgConfigHome: join(sandboxRoot, "xdg"),
        }),
          ...
          if (startCount === 2) {
            const stdout = JSON.stringify({
              ...
              message: {
                ...
                details: { memberName: "crash", lastKnownState: "lost" },
              },
            });
            return {
              pid: 301,
        ...
        operations: {
          ...
          isProcessAlive: () => true,
          killProcess: () => {
            order.push("member-killed");
            return true;
          },
          readMemberTerminal: () => {
            order.push("member-terminal");
            return { kind: "exit" };
          },
          terminateProcessTree: () => {
            order.push("parent-terminated");
            return { kind: "terminated", pid: 101, platform: "linux" };
          },
  ...
  it("#given a structured crash liveness wake with error state #when detected #then the valid abnormal terminal event passes", () => {
    const stdout = JSON.stringify({
      ...
      message: {
        ...
        content:
          "Team member liveness: crash exited abnormally; last known state: error.",
        details: [{
          customType: "senpi-task.team-member-liveness",
  ...
  it("#given only matching prose without structured liveness details #when detected #then it cannot satisfy the gate", () => {
    const stdout = JSON.stringify({
      ...
      message: {
        customType: "note",
        content:
          "Team member liveness: crash exited abnormally; last known state: lost.",
      },
    });
...
describe("team e2e process cleanup", () => {
  ...
  it("#given a Windows shim with no package CLI #when resolved #then the harness fails loudly instead of direct-spawning the shim", () => {
    expect(() =>
      runtime.resolveSenpiInvocation("C:\\npm\\senpi.cmd", {
        platform: "win32",
        existsSync: () => false,
      })
  ...
  it("#given invalid root pids #when process-tree termination is requested #then every platform fails closed without side effects", async () => {
    ...
    for (const pid of invalidPids) {
      expect(
        await runtime.terminateProcessTree(pid, {
          platform: "win32",
