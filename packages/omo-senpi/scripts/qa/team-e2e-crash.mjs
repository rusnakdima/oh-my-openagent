...
import {
  CRASH_RESTART_SCRIPT,
  CRASH_SEED_SCRIPT,
  crashReplacementScript,
...
export function hasCrashLivenessEvent(stdout) {
  for (const event of parseEvents(stdout)) {
    ...
    if (
      message.customType === TEAM_LIVENESS_TYPE &&
      isCrashLivenessDetails(message.details)
    ) return true;
    if (
      message.customType !== "omo-senpi:wake" || !Array.isArray(message.details)
    ) continue;
    for (const entry of message.details) {
      if (
        entry?.customType === TEAM_LIVENESS_TYPE &&
        isCrashLivenessDetails(entry.details)
      ) return true;
    }
  }
  return false;
...
export async function runCrashRestartScenario(input) {
  ...
  try {
    ...
    const parentAliveAtHold = initial.pid !== undefined &&
      ops.isProcessAlive(initial.pid);
    const memberAliveAtHold = target.pid !== undefined &&
      ops.isProcessAlive(target.pid);
    const memberKilled =
      parentAliveAtHold && memberAliveAtHold && target.pid !== undefined
    ...
    const memberTerminal = memberKilled
      ...
      : { kind: undefined };
    const parentAliveBeforeTermination = initial.pid !== undefined &&
      ops.isProcessAlive(initial.pid);
    const parentTermination =
      parentAliveBeforeTermination && memberTerminal.kind !== undefined &&
        initial.pid !== undefined
        ? await ops.terminateProcessTree(initial.pid)
        : failedParentTermination(
          initial.pid,
          "parent was not live after the member terminal transition",
        );
    const reservationAged = target.ready
      ? ops.ageCrashReservation(sandbox.cwd, target)
      : false;
    const initialResult = await initial.completion;
    const afterKillRecord = target.taskId === undefined
      ? undefined
      : ops.taskRecord(sandbox.cwd, target.taskId);
    ...
    if (target.ready && parentTermination.kind === "terminated") {
      ...
      afterRestartRecord = target.taskId === undefined
        ? undefined
        : ops.taskRecord(sandbox.cwd, target.taskId);
      afterReclaim = ops.readPostCrashMailbox(sandbox.cwd, target);
      afterReplacement = await runReplacementMember(
        input,
        sandbox,
        target,
        ops,
    ...
    writeFileSync(
      join(input.outDir, "crash-recovery.json"),
...
export function evaluateCrashRecovery(evidence) {
  ...
  const livenessNotifiedEpoch = evidence.afterRestartRecord?.notification
    ?.liveness_notified_epoch;
  return {
    crashHoldReached: evidence.target.ready &&
      evidence.parentAliveAtHold === true &&
      evidence.memberAliveAtHold === true,
    crashKilledMemberAtHold: evidence.memberKilled === true &&
    ...
    crashReservationRestoredUnread: evidence.reservationAged === true &&
      evidence.afterReclaim.reservedExists === false &&
      evidence.afterReclaim.unreadExists === true,
    crashReservationNoResidue:
      evidence.afterReplacement.reservedExists === false,
    ...
    crashLivenessAcknowledged: typeof runEpoch === "number" &&
...
async function runReplacementMember(input, sandbox, target, ops) {
  if (input.memberExtensionEntry === undefined) {
    return emptyMailboxState(target);
  }
  ...
  try {
    return await ops.pollUntil(
      ...
      (value) =>
        value.processedExists && value.eventCount === 1 &&
        value.envelopeCount === 1,
      HOLD_TIMEOUT_MS,
    );
  } finally {
