...
export function analyzeRpcRouting(records) {
  const record = records.find((r) => r.execution_mode === "process") ??
    records[0];
  if (record === undefined) {
    return { routed: false, reason: "no task record persisted", facts: {} };
  }
  const hasPid = typeof record.pid === "number";
  const message = typeof record.error_message === "string"
    ? record.error_message
    : "";
  const spawnPathFailure = /rpc-entry|--mode rpc|@code-yeongyu\/senpi/i.test(
    message,
  );
  const routed = record.execution_mode === "process" &&
    (hasPid || spawnPathFailure);
  const reason = routed
    ? undefined
  ...
  return {
    ...
    facts: {
      ...
      error_excerpt: message.slice(0, 160),
    },
  };
}
...
export function analyzeSpawn(records, stateDir) {
  const record = records.find((r) => r.execution_mode === "process") ??
    records[0];
  if (record === undefined) {
    return { pass: false, reason: "no task record persisted", facts: {} };
  }
  ...
  const pass = record.execution_mode === "process" && pid !== undefined &&
    sessionJsonl;
  const reason = pass
    ? undefined
    : `execution_mode=${record.execution_mode} pid=${
      pid ?? "absent"
    } sessionJsonl=${sessionJsonl} residency=${record.residency_state}`;
  return {
    pass,
    reason,
    facts: {
...
export function recordRpcChildPids(records) {
  return records
    .filter((record) =>
      record.execution_mode === "process" && typeof record.pid === "number"
    )
