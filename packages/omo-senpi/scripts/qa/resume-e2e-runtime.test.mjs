...
describe("routeChildStep", () => {
  ...
  test("#given the continuation nudge as the last user message #when routing #then the mid-turn child finishes", () => {
    const context = childContext([
      ...
      {
        role: "user",
        content: `Your previous turn was ${CONTINUATION_MARKER}. Resume.`,
      },
    ]);
    expect(routeChildStep(context)).toEqual({
      type: "text",
      text: MIDTURN_CONTINUED_TOKEN,
    });
  });
  ...
  test("#given a mid-turn child prompt #when no assistant message exists #then turn one is an instant read", () => {
    const step = routeChildStep(
      childContext([{ role: "user", content: "midturn-child first unit" }]),
    );
    expect(step).toEqual({
      type: "tool_call",
      name: "read",
  ...
  test("#given a finished child prompt #when routing #then a plain text completes it", () => {
    const step = routeChildStep(
      childContext([{ role: "user", content: "finished-child second unit" }]),
    );
    expect(step.type).toBe("text");
  });
});
...
describe("sessionIdFromEvents", () => {
  test("#given a json event stream #when the session header is present #then its id is returned", () => {
    expect(
      sessionIdFromEvents([{ type: "session", id: "abc-123" }, {
        type: "message",
      }]),
    ).toBe("abc-123");
  });
...
describe("revivedAfterSuspend", () => {
  test("#given a suspend then a resident transition #when scanning the log #then revival is proven", () => {
    const log = [
      JSON.stringify({
        ...
        payload: { status: "running", residency_state: "resident" },
      }),
  ...
  test("#given only non-resident transitions after the suspend #when scanning #then no revival", () => {
    const log = [
      ...
      JSON.stringify({
        type: "transition_applied",
        payload: { status: "completed", residency_state: "persisted_only" },
      }),
    ].join("\n");
    expect(revivedAfterSuspend(log)).toBe(false);
  });
    ...
    const events = [
      {
        ...
        toolName: "task_send",
        result: {
          details: { kind: "revived", task_id: "st_fin", run_epoch: 1 },
        },
      },
    ];
    ...
    const events = [
      {
        ...
        toolName: "task_send",
        result: {
          details: { kind: "revived", task_id: "st_other", run_epoch: 1 },
        },
      },
    ];
...
describe("agent-side wait commands", () => {
  ...
  test("#given a record wait #when building the command #then name and status are argv", () => {
    const command = waitForRecordStatusCommand(
      "/tmp/project",
      "lruone",
      "completed",
    );
    ...
    expect(command).toContain(
      JSON.stringify(join("/tmp/project", ".omo", "senpi-task", "tasks")),
    );
  });
});
