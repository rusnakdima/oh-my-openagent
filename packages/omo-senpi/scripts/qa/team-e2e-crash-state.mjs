...
export function createCrashOperations(overrides = {}) {
  ...
  return {
    ...
    readMemberTerminal: (cwd, target) =>
      readMemberTerminal(cwd, target, processAlive),
...
function readCrashReservationState(cwd, target) {
  if (!target.ready) {
    return { reservedExists: false, processedExists: false, eventCount: 0 };
  }
  return {
    reservedExists: existsSync(
      reservedMessagePath(cwd, target.runId, "crash", target.messageId),
    ),
    processedExists: existsSync(
      processedMessagePath(cwd, target.runId, "crash", target.messageId),
    ),
...
function readPostCrashMailbox(cwd, target) {
  ...
  return {
    ...
    unreadExists: existsSync(
      unreadMessagePath(cwd, target.runId, "crash", target.messageId),
    ),
    processedExists: existsSync(
      processedMessagePath(cwd, target.runId, "crash", target.messageId),
    ),
...
function readMemberTerminal(cwd, target, processAlive) {
  const record = target.taskId === undefined
    ? undefined
    : taskRecord(cwd, target.taskId);
  if (record !== undefined && ABNORMAL_MEMBER_STATES.has(record.status)) {
    return { kind: "record", status: record.status };
  }
  if (target.pid !== undefined && !processAlive(target.pid)) {
    return { kind: "exit" };
  }
...
function ageCrashReservation(cwd, target) {
  const path = reservedMessagePath(
    cwd,
    target.runId,
    "crash",
    target.messageId,
...
function readCrashTarget(cwd, markerPath) {
  ...
  const messageId = typeof marker?.messageId === "string"
    ? marker.messageId
    : undefined;
  const runId = discoverRunIds(cwd)[0];
  const taskId = runId === undefined
    ? undefined
    : memberTaskId(cwd, runId, "crash");
  const record = taskId === undefined ? undefined : taskRecord(cwd, taskId);
  const pid = typeof record?.pid === "number" ? record.pid : undefined;
  const leadSessionId = typeof record?.parent_session_id === "string"
    ? record.parent_session_id
    : undefined;
  return {
    ready: messageId !== undefined && runId !== undefined &&
      taskId !== undefined && pid !== undefined && leadSessionId !== undefined,
    markerPath,
    messageId,
    runId,
    taskId,
