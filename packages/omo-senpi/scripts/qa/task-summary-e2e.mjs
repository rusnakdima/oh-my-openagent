...
const OVER_LIMIT_MARKER =
  "audit auth session flow and the surrounding token refresh pipeline for regressions across the relay";
  ...
  parentSteps: [
    {
      ...
      arguments: {
        category: "mockcat",
        prompt:
          "TASK: Inspect the auth session flow and report the findings in detail.",
    ...
    },
    {
      ...
      arguments: {
        category: "mockcat",
        prompt:
          "TASK: Inspect the refresh pipeline and report the findings in detail.",
...
function runChild(command, args, options) {
  return new Promise((resolveRun) => {
    ...
    child.on(
      "error",
      (error) =>
        resolveRun({ status: 1, error: String(error.message ?? error) }),
    );
    child.on(
      "close",
      (status) => resolveRun({ status: status ?? 1, error: null }),
    );
  });
}
...
async function run() {
  ...
  try {
    run = await runChild(
      ...
      [
        ...
        "--approve",
        "--no-context-files",
        "Run the task_summary QA scenario.",
      ],
      {
        cwd: sandbox.cwd,
        env: childEnv(process.env, sandbox, sessionDir),
        stdio: "inherit",
      },
    );
    check(
      "senpi-run",
      run.status === 0,
      `exit ${run.status}${run.error ? ` (${run.error})` : ""}`,
    );
    ...
    check(
      ...
      `tool result leads with the summary: ${
        transcript.includes(`Started task ${SUMMARY} (`) ? "yes" : "no"
      }`,
    );
    ...
    check(
      "record-summary",
      normal?.task_summary === SUMMARY,
      `sum-child record task_summary=${JSON.stringify(normal?.task_summary)}`,
    );
    const long = records["sum-long"];
    const clampOk = typeof long?.task_summary === "string" &&
      long.task_summary.length === 80 &&
      long.task_summary.endsWith("...");
    check(
      "record-clamp",
      clampOk,
      `sum-long record task_summary length=${long?.task_summary?.length} tail=${
        JSON.stringify(long?.task_summary?.slice(-8))
      }`,
    );
    ...
    const rowChecks = Object.values(records)
      ...
      .map((record) =>
        `${record.task_summary} beats ${record.name ?? record.task_id}`
      );
    check(
      "widget-identity-source",
      rowChecks.length === 2,
      `records with displayable summaries: ${rowChecks.length}`,
    );
    ...
    if (outDir !== undefined) {
      ...
      writeFileSync(
        join(outDir, "task-summary-records.json"),
        `${JSON.stringify(records, null, 2)}\n`,
      );
      writeFileSync(
        join(outDir, "transcript-tail.txt"),
  ...
  const result = checks.every((entry) => entry.ok) && run.status === 0
    ? "PASS"
    : "FAIL";
  const payload = {
    result,
    checks,
    sandboxCleaned: !existsSync(sandbox.root),
    cleanup: `removed ${sandbox.root}`,
  };
  const outDir = process.env.TASK_SUMMARY_E2E_OUT_DIR;
  if (outDir !== undefined) {
    mkdirSync(outDir, { recursive: true });
    writeFileSync(
      join(outDir, "cleanup-receipt.json"),
...
if (process.argv.includes("--self-test")) {
  if (!SUMMARY.includes("auth")) {
    throw new Error("self-test: summary constant drifted");
  }
