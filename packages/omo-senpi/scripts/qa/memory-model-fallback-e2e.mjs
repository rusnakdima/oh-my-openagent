...
async function runParent(
  ...
  if (code !== 0) {
    throw new Error(stderr.slice(-1000) || `parent exited ${code}`);
  }
}
  ...
  return new Promise((resolve, reject) => {
    ...
    const subscription = watch(dir, () => {
      const file = readdirSync(dir).find((name) =>
        name.endsWith(".json") && !existing.has(name)
      );
...
async function main() {
  if (senpiBin === null || !existsSync(senpiBin)) {
    throw new Error(`senpi binary missing: ${senpiBin}`);
  }
  seedSandbox(sandbox);
  ...
  writeFileSync(
    join(state.repo, "mock-script.json"),
    `${
      JSON.stringify(
        {
          steps: [
            {
              ...
              arguments: {
                ...
                edits: [{
                  oldText: "seed before fallback",
                  newText: "seed before fallback\nfallback reflection merged",
                }],
              },
            },
            {
              ...
              arguments: {
                command:
                  "git add system/facts.md && git commit -m 'qa: fallback reflection'",
              },
            },
  ...
  const commit = Bun.spawnSync(
    ["git", "commit", "-m", "qa: reflection script"],
    { cwd: state.repo },
  );
  ...
  const existing = new Set(
    existsSync(state.completions) ? readdirSync(state.completions) : [],
  );
  ...
  const attempts = readFileSync(join(runDir, "attempts.log"), "utf8").trim()
    .split("\n")
  ...
  if (completion.model !== "omo-mock/mock-1") {
    throw new Error(`wrong completion model: ${completion.model}`);
  }
  if (attempts.join(",") !== "extension-only/primary,omo-mock/mock-1") {
    throw new Error(`wrong model attempts: ${attempts.join(",")}`);
  }
  if (
    !readFileSync(join(state.repo, "system", "facts.md"), "utf8").includes(
      "fallback reflection merged",
    )
  ) {
