...
async function main() {
  ...
  if (result.code !== 0) {
    throw new Error(
      result.stderr.slice(-1000) || `senpi exited ${result.code}`,
    );
  }
  if (output.includes("skill path does not exist")) {
    throw new Error("missing skill path warning was emitted");
  }
  if (output.includes('"frontend" collision')) {
    throw new Error("frontend collision warning was emitted");
  }
  if (existsSync(skillsPath)) {
    throw new Error(`QA requires an absent skills path: ${skillsPath}`);
  }
