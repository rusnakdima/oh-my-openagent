...
describe("team e2e injection evidence", () => {
  ...
  it("#given a missing observation directory #when main evidence is analyzed #then injection evidence is written", () => {
    ...
    // then
    expect(existsSync(join(fixture.obsDir, "team-injection-evidence.json")))
      .toBe(true);
  });
    ...
    writeSessionLine(fixture, {
      ...
      message: {
        ...
        content: [{
          type: "text",
          text:
            `<peer_message from="lead" messageId="${fixture.leadMessageId}">\nCRASH-ONCE\n</peer_message>`,
        }],
      },
    });

    // when
    const count = sessionEnvelopeCount(
      fixture.project,
      fixture.taskId,
      fixture.leadMessageId,
    );
...
function createFixture() {
  ...
  return {
    ...
    leadMessageId,
    events: [
      toolEvent("team_create", {
        kind: "created",
        team_run_id: runId,
        members: [{ name: "quick" }, { name: "fixture" }],
      }),
      toolEvent("task_send", {
        kind: "team_message",
        team: {
...
function seedInjectionEvidence(fixture: Fixture): void {
  ...
  writeFileSync(
    ...
    `${JSON.stringify({ quick: fixture.taskId })}\n`,
  );
  ...
  writeFileSync(
    join(fixture.obsDir, "lead-received.txt"),
