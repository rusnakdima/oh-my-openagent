...
const CASES = [
  {
    agent: "momus",
    prompt: "review the variant qa fixture plan",
    expected: "xhigh",
  },
  {
    agent: "metis",
    prompt: "gap-analyze the variant qa fixture plan",
    expected: "medium",
  },
];
...
function readAgentVariant(stateDir, agent) {
  ...
  const statuses = [];
  for (
    const entry of readdirSync(tasksDir).filter((name) =>
      name.endsWith(".json")
    )
  ) {
    ...
    ...Object.fromEntries(
      cases.flatMap((entry) =>
        Object.entries(entry.checks).map((
          [name, verdict],
        ) => [`${entry.agent}_${name}`, verdict])
      ),
    ),
    real_senpi_untouched: beforeCredential === afterCredential
      ? "PASS"
      : "FAIL",
  };
  const payload = {
