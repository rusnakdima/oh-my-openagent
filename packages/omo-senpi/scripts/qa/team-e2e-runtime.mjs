...
export function resolveSenpiInvocation(senpiBin, operations = {}) {
  ...
  if (platform === "win32") {
    ...
    if (
      launcherName.endsWith(".exe") ||
      (launcherName !== "senpi" && launcherName !== "senpi.cmd")
    ) {
    ...
    if (cliPath === undefined) {
      throw new Error(
        `Windows Senpi shim cannot be mapped to the package CLI: ${senpiBin}`,
      );
    }
    return {
...
export function startSenpiRun(input) {
  ...
  return {
    ...
    completion,
    kill: async () =>
      ownsLiveChild(child, childClosed) && killProcessGroup(child.pid),
  };
}
