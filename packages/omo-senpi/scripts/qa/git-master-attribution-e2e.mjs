...
function seedScenario(sandbox, gitMasterConfig) {
  ...
  const omoConfig = gitMasterConfig === undefined
    ? {}
...
function selfTest() {
  ...
  if (!payload.includes(CO_AUTHOR_TRAILER)) {
...
function main() {
  ...
  const disabled = runScenario(senpiBin, {
    commit_footer: false,
  ...
  const checks = {
    ...
    enabled_read_event: enabled.readEventCount >= 1 && enabled.bodyPresent
      ? "PASS"
      : "FAIL",
    ...
    disabled_read_event: disabled.readEventCount >= 1 && disabled.bodyPresent
      ? "PASS"
      : "FAIL",
  ...
  const payload = {
    ...
    changedRealDirs,
    sandboxAgentDirs: {
      enabled: enabled.sandboxAgentDir,
      disabled: disabled.sandboxAgentDir,
    },
  };
  writeEvidence(outDir ? resolve(outDir) : undefined, payload, {
    enabled,
    disabled,
  });
