...
const SHARED_SENPI_LOGS = new Set([
  SHARED_SENPI_LOG,
  "senpi-crash.log",
  "cache/mcp-cache.json",
]);
...
export function changedRealPaths(before, after) {
  ...
  for (const [rel, sha] of after) {
    if (!isSharedSenpiDiagnostic(rel) && before.get(rel) !== sha) {
      changed.push(rel);
    }
  }
  for (const rel of before.keys()) {
    if (!isSharedSenpiDiagnostic(rel) && !after.has(rel)) changed.push(rel);
  }
  return changed;
}
...
export function classifyRealSenpiChanges(changedPaths, sandboxTokens) {
  const tokens = sandboxTokens.filter((token) =>
    typeof token === "string" && token.length > 0
  );
  const qaAttributedPaths = [];
  const concurrentSessionPaths = [];
  for (const path of changedPaths) {
    const unrelatedSession = tokens.length > 0 &&
      path.startsWith("sessions/") && !tokens.some((token) =>
        path.includes(token)
      );
...
export function findWakeNotification(events, taskId) {
  ...
  const hasNotification = hay.includes("task completion") &&
    hay.includes("status:completed");
  const namesTask = typeof taskId === "string" && taskId.length > 0 &&
    hay.includes(taskId);
