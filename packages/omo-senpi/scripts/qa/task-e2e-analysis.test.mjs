      ...
      readdir(dir, options) {
        if (dir === lockDir) {
          ...
          const error = new Error(
            `ENOENT: no such file or directory, scandir '${lockDir}'`,
          );
          error.code = "ENOENT";
          throw error;
        }
...
describe("changedRealPaths", () => {
  ...
  test("#given settings tips timestamps change #when snapshots compare #then only substantive settings changes count", () => {
    ...
    try {
      ...
      writeFileSync(
        settings,
        JSON.stringify({ theme: "dark", tipsHistory: { one: 1 } }),
      );
      const before = snapshotDir(root);
      writeFileSync(
        settings,
        JSON.stringify({ theme: "dark", tipsHistory: { one: 2 } }),
      );
      const tipsOnly = snapshotDir(root);
      writeFileSync(
        settings,
        JSON.stringify({ theme: "light", tipsHistory: { one: 3 } }),
      );
