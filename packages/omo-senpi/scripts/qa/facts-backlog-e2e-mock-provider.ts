...
export function extractionFor(payloadPath: string, today: string): string {
    ...
    readonly entries: ReadonlyArray<
      {
        ...
        readonly range?: { readonly end_message_id?: string };
      }
    >;
  };
  ...
  return `${
    JSON.stringify({
      scope: "project",
      text: `QA backlog batch covered ${shipped}`,
      date: today,
    })
  }\n`;
}
  ...
  pi.registerProvider("omo-mock", {
    ...
    streamSimple(
      ...
      if (
        ...
        readFileSync(payloadPath, "utf8").includes(FAIL_SENTINEL)
      ) {
        (globalThis as { process?: { exit?: (code: number) => never } }).process
          ?.exit?.(7);
      }
      ...
      const step: AssistantContent[] = turn === 0
        ? [{
          ...
          arguments: {
            path: extractionPath,
            content: extractionFor(payloadPath, today),
          },
        }]
