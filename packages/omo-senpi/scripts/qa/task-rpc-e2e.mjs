...
const { digestDirectory } = await import(
  pathToFileURL(join(scriptDir, "drive.mjs")).href
);
const {
  CREDENTIAL_FILES,
  digestCredentialFiles,
  parseEvents,
  readRecords,
  analyzeSpawn,
  analyzeRpcRouting,
  eventsMentionSteerAck,
  statusSnapshots,
  liveRecordRpcChildPids,
  recordRpcChildPids,
  sleep,
} = await import(
  pathToFileURL(join(scriptDir, "task-rpc-e2e-helpers.mjs")).href
);
const {
  SCENARIO_A_STEPS,
  ...
  pathToFileURL(join(scriptDir, "task-rpc-e2e-scenarios.mjs")).href
);
...
async function runChecks(senpiBin, sandbox, sessionDir, stateDir) {
  ...
  if (aRecords.length === 0) {
    return {
      checks: [{
        ...
        verdict: "FAIL",
        reason: `scenario A parent exited status=${a.status} signal=${
          a.signal ?? "none"
        } without persisting any task record`,
        facts: {
          status: a.status,
  ...
  checks.push({
    check: "process_mode_routes_to_rpc_runner",
    verdict: routing.routed ? "PASS" : "FAIL",
    ...(routing.reason && { reason: routing.reason }),
    facts: routing.facts,
  });
  const spawn = analyzeSpawn(aRecords, stateDir);
  checks.push({
    check: "spawn_process_pid_and_session_jsonl",
    verdict: spawn.pass ? "PASS" : "FAIL",
    ...(spawn.reason && { reason: spawn.reason }),
    facts: spawn.facts,
  });
  ...
  checks.push({
    ...
    reason: spawn.pass
      ? (steerFact ? undefined : "no steer ack observed")
      : "blocked: no rpc child spawned (see spawn_process)",
  });

  const completed = readRecords(stateDir).some((r) =>
    r.status === "completed" && r.execution_mode === "process"
  );
  ...
  checks.push({
    ...
    reason: spawn.pass
      ? (completed ? undefined : "no completion recorded")
      : "blocked: no rpc child spawned (see spawn_process)",
  ...
  checks.push({
    check: "no_leaked_rpc_child_pids",
    verdict: leakedPids.length === 0 ? "PASS" : "FAIL",
    ...(leakedPids.length > 0 &&
      { reason: `leaked pids ${leakedPids.join(",")}` }),
    facts: { leakedPids },
  });
...
async function main() {
  ...
  let payload;
  try {
    const { checks, leakedPids, spawnPass, routed } = await runChecks(
      senpiBin,
      sandbox,
      sessionDir,
      stateDir,
    ...
    const wholeDirDigestStable =
      beforeWholeDir === digestDirectory(realSenpiAgentDir);
    const realCredentialsUntouched = beforeCreds === afterCreds;
    checks.unshift({
      check: "real_credentials_untouched_and_caller_env_ignored",
      verdict: realCredentialsUntouched && providedAgentDir !== "USED"
        ? "PASS"
        : "FAIL",
      ...(realCredentialsUntouched ? {} : {
        reason:
          "a real ~/.senpi/agent credential/config file changed across the run",
      }),
      facts: {
        realCredentialsUntouched,
        providedAgentDir,
...
function runSelfTest() {
  ...
  const helperSource = readFileSync(
    join(scriptDir, "task-rpc-e2e-helpers.mjs"),
    "utf8",
  );
  const staleCleanupCall = ["killProcessTree", "(pidsBefore)"].join("");
  const globalRpcPgrep = [
    "p",
    'grep", ["-f", "',
    ["senpi", "--mode", "rpc"].join(" "),
    '"]',
  ...
  if (
    driverSource.includes(staleCleanupCall) ||
    helperSource.includes(globalRpcPgrep)
  ) {
    throw new Error(
      "self-test: RPC cleanup must use sandbox-owned task record pids, not global process scans",
    );
  }
  const windowsNames = executableNames("senpi", "win32", ".EXE;.CMD");
  if (
    !windowsNames.includes("senpi.EXE") || !windowsNames.includes("senpi.CMD")
  ) {
    throw new Error(
      "self-test: Windows Senpi resolution must honor PATHEXT shims",
    );
  }
  const scenarioSource = readFileSync(
    join(scriptDir, "task-rpc-e2e-scenarios.mjs"),
    "utf8",
  );
  if (droppedToolPattern().test(scenarioSource)) {
    throw new Error(
      "self-test: RPC scenario scripts still name a dropped tool",
    );
  }
  const stateDir = join(process.cwd(), "__self_test_missing__");
  const noJsonl = analyzeSpawn([{
    task_id: "st_fix",
    execution_mode: "process",
    pid: 4242,
    residency_state: "rpc_detached",
  }], stateDir);
  if (noJsonl.pass !== false) {
    throw new Error(
      "self-test: sessions-jsonl absence must fail the spawn proof",
    );
  }
  if (noJsonl.facts.pid !== 4242) {
    throw new Error("self-test: analyzeSpawn must surface the pid fact");
  }
  ...
  try {
    ...
    if (ok.pass !== true) {
      throw new Error(
        "self-test: pid + real child JSONL must pass regardless of disposed residency",
      );
    }
  } finally {
    rmSync(jsonlRoot, { recursive: true, force: true });
  }
  const broken = analyzeSpawn([{
    task_id: "st_brk",
    execution_mode: "process",
    residency_state: "disposed",
  }], stateDir);
  if (broken.pass !== false) {
    throw new Error(
      "self-test: in-process fallback must not read as a spawned rpc child",
    );
  }
  if (
    broken.reason === undefined ||
    broken.reason.includes("pid=absent") === false
  ) throw new Error("self-test: broken shape must localize the missing pid");
  if (
    analyzeRpcRouting([{
      task_id: "st_p",
      execution_mode: "process",
  ...
  ) throw new Error("self-test: a pid must prove rpc routing");
  const spawnErr = analyzeRpcRouting([{
    task_id: "st_e",
    execution_mode: "process",
    status: "error",
    error_message: "Package subpath './rpc-entry' is not defined by exports",
  }]);
  if (spawnErr.routed !== true) {
    throw new Error(
      "self-test: an rpc spawn-path failure must prove rpc routing",
    );
  }
  if (
    analyzeRpcRouting([{
    ...
    throw new Error(
      "self-test: an in-process fallback completion must not read as rpc routing",
    );
  }
  if (
    eventsMentionSteerAck([{
      type: "toolResult",
      name: "task_send",
      details: { delivered: "steer" },
    }]) !== true
  ) throw new Error("self-test: steer ack detection failed");
  if (
    eventsMentionSteerAck([{ type: "text", text: "nothing here" }]) !== false
  ) throw new Error("self-test: steer ack false positive");
  const snaps = statusSnapshots([{
    kind: "status",
    snapshot: { task_id: "st_x", pid: 99 },
  }]);
  if (snaps.length !== 1 || snaps[0].pid !== 99) {
    throw new Error("self-test: status snapshot extraction failed");
  }
  const pids = recordRpcChildPids([{ execution_mode: "process", pid: 12 }, {
    execution_mode: "in-process",
    pid: 13,
  }, { execution_mode: "process" }]);
  if (pids.length !== 1 || pids[0] !== 12) {
    throw new Error(
      "self-test: rpc child pid scan must use process-mode task records only",
    );
  }
  ...
  try {
    ...
    if (d1 !== digestCredentialFiles(probeRoot)) {
      throw new Error("self-test: credential digest must be deterministic");
    }
    writeFileSync(join(probeRoot, "auth.json"), "BBB");
    if (digestCredentialFiles(probeRoot) === d1) {
      throw new Error(
        "self-test: credential digest must move when auth.json changes",
      );
    }
  } finally {
