...
const stagedRuntimeEntry = join(
  pluginRoot,
  "runtime",
  "ast-grep-mcp",
  "cli.js",
);
const stagedRuntimeManifest = join(
  pluginRoot,
  "runtime",
  "ast-grep-mcp",
  "manifest.json",
...
const stageScript = join(
  pluginRoot,
  "scripts",
  "stage-ast-grep-mcp-runtime.mjs",
);
const defaultEvidenceDir = join(
  repoRoot,
  ".omo",
  "evidence",
  "20260803-senpi-ast-grep-mcp",
);
const expectedRawTools = ["search", "rewrite", "scan"];
const expectedSenpiTools = expectedRawTools.map((name) =>
  `mcp__ast_grep_${name}`
);
...
function credentialDigest(agentDir) {
  ...
  for (
    const name of ["auth.json", "models.json", "settings.json", "trust.json"]
  ) {
...
function renderRun(run) {
  return [
    ...
    `status=${String(run.status)} signal=${String(run.signal)} error=${
      run.error ?? "none"
    }`,
    "--- stdout ---",
    run.stdout.trimEnd(),
...
function runHandshake(transcript) {
  ...
  const run = runCommand(process.execPath, [stagedRuntimeEntry], {
    input,
    timeout: 10_000,
  });
  ...
  assert(
    run.status === 0,
    `handshake server exited ${run.status}: ${run.stderr}`,
  );
  const responses = parseJsonLines(run.stdout);
  assert(
    responses.length === 2,
    `handshake expected 2 responses, got ${responses.length}`,
  );
  const initialize = responses.find((response) => response.id === 1)?.result;
  const tools = responses.find((response) => response.id === 2)?.result?.tools;
  assert(
    initialize?.serverInfo?.name === "ast_grep",
    `unexpected server name: ${JSON.stringify(initialize?.serverInfo)}`,
  );
  assert(
    initialize?.serverInfo?.version === "0.1.0",
    `unexpected server version: ${JSON.stringify(initialize?.serverInfo)}`,
  );
  assert(Array.isArray(tools), "tools/list did not return a tools array");
  const names = tools.map((tool) => tool.name);
  assert(
    JSON.stringify(names) === JSON.stringify(expectedRawTools),
    `unexpected raw tool order: ${JSON.stringify(names)}`,
  );
  transcript.push(
    `HANDSHAKE PASS serverInfo=ast_grep/0.1.0 rawTools=${
      JSON.stringify(names)
    }`,
  );
  return [
    `# 2026-08-03 ast-grep MCP handshake`,
    `timestamp=${timestamp()}`,
    renderRun(run),
    `PASS serverInfo=ast_grep/0.1.0 rawTools=${JSON.stringify(names)}`,
    "",
  ].join("\n");
}
...
function verifyCurrentStagedRuntime(transcript) {
  ...
  const check = runCommand(process.execPath, [stageScript, "--check"], {
    timeout: 30_000,
  });
  transcript.push(renderRun(check));
  assert(
    check.status === 0,
    `current staged runtime check failed: ${check.stderr || check.stdout}`,
  );
  ...
  assert(
    builtSha === stagedSha,
    `staged sha ${stagedSha} != built sha ${builtSha}`,
  );
  assert(
    manifest.sha256 === stagedSha,
    `manifest sha ${manifest.sha256} != staged sha ${stagedSha}`,
  );
  transcript.push(
    `STALE-STATE GUARD PASS builtSha256=${builtSha} stagedSha256=${stagedSha} manifestSha256=${manifest.sha256}`,
  );
...
function packAndExtract(workRoot, transcript) {
  ...
  assert(
    ...
    `npm pack did not report a tarball: ${pack.stdout}`,
  );
  const tarball = join(packDir, filename);
  const extract = runCommand("tar", ["-xzf", tarball, "-C", extractDir], {
    timeout: 30_000,
  });
  transcript.push(renderRun(extract));
  assert(
    extract.status === 0,
    `tar extraction failed: ${extract.stderr || extract.stdout}`,
  );
  const extractedPlugin = join(extractDir, "package");
  assert(
    existsSync(join(extractedPlugin, "package.json")),
    `extracted plugin is missing package.json: ${extractedPlugin}`,
  );
...
function runPackedBoot(workRoot, extractedPlugin, senpiBin, transcript) {
  ...
  const env = {
    ...isolatedEnv(workRoot, homeDir, agentDir),
    OMO_AST_GREP_QA_CAPTURE: capturePath,
  };
  const install = runInstaller(extractedPlugin, projectDir, env);
  transcript.push(renderRun(install));
  assert(
    install.status === 0,
    `packed install failed: ${install.stderr || install.stdout}`,
  );
  const installResult = JSON.parse(install.stdout.trim());
  assert(
    installResult.agentDir === agentDir,
    `packed install escaped isolated agent dir: ${installResult.agentDir}`,
  );
  assert(
    existsSync(join(agentDir, "settings.json")),
    `isolated settings were not written under ${agentDir}`,
  );
  ...
  assert(
    boot.status === 0,
    `packed Senpi boot failed: ${boot.stderr || boot.stdout}`,
  );
  assert(
    existsSync(capturePath),
    `mock provider did not capture the Senpi tool surface: ${capturePath}`,
  );
  const capture = JSON.parse(readFileSync(capturePath, "utf8"));
  const toolNames = capture.toolNames ?? [];
  const missing = expectedSenpiTools.filter((name) =>
    !toolNames.includes(name)
  );
  const retiredToolMarker = ["code", "graph"].join("");
  const retiredTools = toolNames.filter((name) =>
    name.toLowerCase().includes(retiredToolMarker)
  );
  assert(
    missing.length === 0,
    `packed Senpi boot is missing ast_grep tools: ${
      JSON.stringify(missing)
    }; visible=${JSON.stringify(toolNames)}`,
  );
  assert(
    retiredTools.length === 0,
    `packed Senpi boot exposed retired graph tools: ${
      JSON.stringify(retiredTools)
    }`,
  );
  transcript.push(
    `PROVIDER CAPTURE RAW\n${readFileSync(capturePath, "utf8").trimEnd()}`,
  );
  transcript.push(
    `PACKED BOOT PASS astGrepTools=${
      JSON.stringify(expectedSenpiTools)
    } retiredGraphTools=[]`,
  );
  return { agentDir, homeDir, projectDir, sessionDir, capturePath, toolNames };
}

function runPackedInstallerFailureScenarios(
  workRoot,
  extractedPlugin,
  transcript,
) {
  ...
  assert(
    ...
    `packed runtime manifest is missing: ${manifestPath}`,
  );
  ...
  const missingEnv = isolatedEnv(
    join(workRoot, "missing-profile"),
    missingHomeDir,
    missingAgentDir,
  );
  const missingInstall = runInstaller(
    extractedPlugin,
    missingProjectDir,
    missingEnv,
  );
  ...
  assert(
    missingInstall.status !== 0,
    "packed installer silently succeeded with missing ast-grep MCP runtime",
  );
  assert(
    missingOutput.includes("missing required runtime artifacts") &&
      missingOutput.includes(extractedPlugin),
  ...
  const missingRestored = runInstaller(
    extractedPlugin,
    missingProjectDir,
    missingEnv,
  );
  ...
  assert(
    missingRestored.status === 0,
    `C1 restored runtime did not install: ${
      missingRestored.stderr || missingRestored.stdout
    }`,
  );
  transcript.push(
    `PACKED INSTALLER C1 PASS deleted=${runtimeEntry} loudMissingFailure=true restored=true`,
  );
  ...
  const corruptEnv = isolatedEnv(
    join(workRoot, "corrupt-profile"),
    corruptHomeDir,
    corruptAgentDir,
  );
  const corruptInstall = runInstaller(
    extractedPlugin,
    corruptProjectDir,
    corruptEnv,
  );
  ...
  assert(
    corruptInstall.status !== 0,
    "packed installer silently succeeded with corrupted present ast-grep MCP runtime",
  );
  assert(
    corruptOutput.includes("ast-grep MCP runtime integrity error") &&
      corruptOutput.includes("sha256 mismatch"),
    `packed installer corruption failure was not an explicit integrity error: ${corruptOutput}`,
  );
  assert(
    !existsSync(join(corruptAgentDir, "settings.json")),
    "corrupted packed runtime was registered despite integrity failure",
  );
  writeFileSync(runtimeEntry, original);
  chmodSync(runtimeEntry, originalMode);
  const corruptRestored = runInstaller(
    extractedPlugin,
    corruptProjectDir,
    corruptEnv,
  );
  ...
  assert(
    corruptRestored.status === 0,
    `C2 restored runtime did not install: ${
      corruptRestored.stderr || corruptRestored.stdout
    }`,
  );
  transcript.push(
    `PACKED INSTALLER C2 PASS corrupted=${runtimeEntry} manifestPresent=true integrityFailure=true restored=true`,
  );
}
...
function createRuntimeCheckFixture(workRoot) {
  ...
  return {
    ...
    sourceEntry,
    targetEntry,
    manifestPath: join(dirname(targetEntry), "manifest.json"),
  };
}
...
function runRuntimeCheckRegressions(workRoot, transcript) {
  ...
  const invoke = (name) => {
    const run = runCommand(process.execPath, [
      realpathSync(fixture.fixtureScript),
      "--check",
    ], { cwd: fixture.fixtureRoot, timeout: 15_000 });
    cases.push({ name, run });
    transcript.push(`\n### ${name}\n${renderRun(run)}`);
    return run;
  };

  const staged = runCommand(process.execPath, [
    realpathSync(fixture.fixtureScript),
  ], { cwd: fixture.fixtureRoot, timeout: 15_000 });
  transcript.push(`\n### stage-fixture\n${renderRun(staged)}`);
  assert(
    staged.status === 0,
    `fixture staging failed: ${staged.stderr || staged.stdout}`,
  );
  assert(
    existsSync(fixture.manifestPath),
    `fixture staging did not create manifest: ${fixture.manifestPath}`,
  );

  const fresh = invoke("fresh");
  assert(
    fresh.status === 0 && fresh.stdout.includes("runtime is current"),
    "fresh runtime --check did not exit 0",
  );
  ...
  assert(
    corrupted.status === 1 &&
      corrupted.stderr.includes("does not match built sha256"),
    "corrupted sha --check did not fail clearly",
  );

  rmSync(fixture.targetEntry);
  const missing = invoke("missing");
  assert(
    missing.status === 1 && missing.stderr.includes("staged CLI is missing"),
    "missing runtime --check did not fail clearly",
  );
  ...
  assert(
    nonExecutable.status === 1 &&
      nonExecutable.stderr.includes("staged CLI is not executable"),
    "non-executable runtime --check did not fail clearly",
  );

  copyFileSync(fixture.sourceEntry, fixture.targetEntry);
  chmodSync(fixture.targetEntry, 0o755);
  transcript.push(
    "STAGE-SCRIPT FIXTURE REGRESSION PASS manifestCreated=true fresh=0 corrupted=1 missing=1 nonExecutable=1",
  );
  return cases;
}
...
async function main() {
  ...
  const transcript = [
    `# 2026-08-03 ast-grep MCP end-to-end QA`,
    `startedAt=${timestamp()}`,
    `repoRoot=${repoRoot}`,
  ];
  ...
  try {
    ...
    assert(
      senpiBin !== null,
      "senpi binary unavailable; packed boot cannot SKIP",
    );
    ...
    runPackedInstallerFailureScenarios(
      workRoot,
      packed.extractedPlugin,
      transcript,
    );
    ...
    transcript.push(
      `processesReferencingWorkRootBeforeCleanup=${
        JSON.stringify(liveBeforeCleanup)
      }`,
    );
    for (const processEntry of liveBeforeCleanup) {
      try {
        ...
        transcript.push(
          `sent SIGTERM pid=${processEntry.pid} command=${processEntry.command}`,
        );
      } catch (error) {
        transcript.push(
          `SIGTERM pid=${processEntry.pid} result=${
    ...
    transcript.push(
      `processesReferencingWorkRootAfterCleanup=${
        JSON.stringify(liveAfterCleanup)
      }`,
    );
    transcript.push(
      `realSenpiCredentialDigestObservedUnchanged=${
        credentialDigest(realAgentDir) === beforeRealCredentials
      }`,
    );
    transcript.push(
      "realSenpiCredentialDigestIsInformational=true (concurrent user sessions may write there; isolation is gated by explicit HOME/XDG/SENPI_CODING_AGENT_DIR paths under workRoot)",
    );
  ...
  if (result !== "PASS") {
    process.stderr.write(
      `ast-grep MCP e2e failed: ${failure ?? "unknown failure"}\n`,
    );
    process.exitCode = 1;
  }
}
