...
const QUICK_PROMPT =
  "You are team member 'quick'. MOCKROLE=quick. End your turn.";
...
function seedResumeTeamProject(sandbox) {
  ...
  writeFileSync(
    ...
    `${JSON.stringify(RESUME_TEAM_OMO_CONFIG, null, 2)}\n`,
  );
}
...
function memberDeliveredFromLead(memberLog) {
  return memberLog
    .split(/\r?\n/)
    .some((line) =>
      line.includes('"type":"team_message_delivered"') &&
      line.includes('"from":"lead"')
    );
}
...
function memberBooted(cwd) {
  ...
  return {
    runId,
    quickTask,
    booted: runId !== undefined && quickTask !== undefined &&
      taskRecord(cwd, quickTask) !== undefined &&
      childSessionHasAssistant(cwd, quickTask),
  };
}
...
async function runMemberResumeLane(senpiBin, outDir, startRun, checks) {
  ...
  try {
    ...
    const afterQuit = seeded.quickTask === undefined
      ? undefined
      : taskRecord(sandbox.cwd, seeded.quickTask);
    checks.resume_member_suspended_on_quit = result1.status === 0 &&
      seeded.booted === true &&
    ...
    const run2 = startRun({
      ...
      script: {
        lead: [
          toolCall("task_send", {
            team_run_id: "__TEAM_RUN_ID__",
            to: "quick",
            message: `${LEAD2QUICK_TOKEN} revive handshake`,
          }),
        ...
        quick: [
          toolCall("task_send", {
            to: "lead",
            message: `${QUICK2LEAD_TOKEN} member revived and reporting`,
          }),
    ...
    const delivered = await pollUntil(
      () => (seeded.runId === undefined
        ? { reserved: -1 }
        : inboxCounts(memberInboxDir(sandbox.cwd, seeded.runId, "lead"))),
      (v) => v.reserved >= 1,
      POLL_MS,
    );
    ...
    const quickTaskAfter = seeded.runId === undefined
      ? undefined
      : memberTaskId(sandbox.cwd, seeded.runId, "quick");
    const memberLog = seeded.quickTask === undefined
      ? ""
      : taskEventText(sandbox.cwd, seeded.quickTask);
    const leadInbox = seeded.runId === undefined
      ? { unread: -1, reserved: -1 }
      : inboxCounts(memberInboxDir(sandbox.cwd, seeded.runId, "lead"));
    ...
    const leadSteerLanded = existsSync(leadReceipt) &&
      readFileSync(leadReceipt, "utf8").includes("QUICK2LEAD");
    checks.resume_member_revived = result2.status === 0 &&
      revivedAfterSuspend(memberLog) &&
      quickTaskAfter !== undefined &&
      quickTaskAfter === seeded.quickTask;
    checks.resume_member_mailbox_identity = seeded.quickTask !== undefined &&
      sessionContainsText(sandbox.cwd, seeded.quickTask, LEAD2QUICK_TOKEN) &&
      quickTaskAfter === seeded.quickTask;
    checks.resume_lead_poller_running = delivered.reserved >= 1 &&
      memberDeliveredFromLead(memberLog);
    writeFileSync(
...
async function runShutdownLane(senpiBin, outDir, startRun, checks) {
  ...
  try {
    ...
    const readTeamState = () => {
      ...
      const state = runId === undefined ? undefined : readJsonIfPresent(
        join(
          sandbox.cwd,
          ".omo",
          "senpi-task",
      ...
      const quickTask = runId === undefined
        ? undefined
        : memberTaskId(sandbox.cwd, runId, "quick");
    ...
    const requested = await pollUntil(
      () => {
        ...
        return {
          runId,
          quickTask,
          pending: (state?.shutdownRequests ?? []).some((request) =>
            request.approvedAt === undefined && request.rejectedAt === undefined
          ),
          memberRecorded: quickTask !== undefined &&
            taskRecord(sandbox.cwd, quickTask) !== undefined,
        };
      },
    ...
    const approved = await pollUntil(
      () => {
        ...
        return {
          stamped: (state?.members ?? []).some((member) =>
            member.name === "quick" && member.status === "shutdown_approved"
          ),
          cancelled: quickTask !== undefined &&
            taskRecord(sandbox.cwd, quickTask)?.status === "cancelled",
        };
      },
    ...
    const logBefore = requested.quickTask === undefined
      ? ""
      : taskEventText(sandbox.cwd, requested.quickTask);
    checks.resume_shutdown_approved_setup = result1.status === 0 &&
    ...
    const memberAfter = requested.quickTask === undefined
      ? undefined
      : taskRecord(sandbox.cwd, requested.quickTask);
    const logAfter = requested.quickTask === undefined
      ? ""
      : taskEventText(sandbox.cwd, requested.quickTask);
    checks.resume_shutdown_approved_not_revived = result2.status === 0 &&
      requested.quickTask !== undefined &&
