...
export function analyzeParallelismRequests(requests) {
  ...
  const summaries = captures.filter((capture) =>
    capture?.event === "parallelism_summary"
  );
  const properties = isRecord(summaries[0]?.properties)
    ? summaries[0].properties
    : {};
  ...
  return {
    ...
    nestedCountsExact: properties.eval_execution_event_count === 1 &&
    ...
    outerEvalExcludedFromNonEval: properties.eval_outer_joined_calls === 1 &&
    ...
    localhostOnlyTelemetry: requests.every((request) =>
