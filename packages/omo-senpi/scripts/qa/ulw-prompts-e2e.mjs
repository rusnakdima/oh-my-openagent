...
const FORBIDDEN_TRANSCRIPT_MARKERS = [
  "update_plan",
  "multi_agent",
  "spawn_agent",
];
  ...
  for (const file of files.filter((path) => path.endsWith(".jsonl"))) {
    for (const line of readFileSync(file, "utf8").split("\n")) {
      ...
      if (
        ...
        entry.customType === ULTRAWORK_CUSTOM_TYPE
      ) {
        const record = {
...
function stagedSkillChecks() {
  ...
  const research = existsSync(researchPath)
    ...
    : "";
  const ultrawork = existsSync(ultraworkPath)
    ? readFileSync(ultraworkPath, "utf8")
    : "";
  return {
    ulwResearchExists: research.length > 0,
    ulwResearchNative: research.length > 0 &&
    ...
    ultraworkDescriptionOk: (ultrawork.match(/^description: (.*)$/m)?.[1] ?? "")
      .includes("injects the full directive as a hidden custom message"),
  };
}
...
function runSelfTest() {
  ...
  try {
    seedSandbox(sandbox);
    if (REQUIRED_DIRECTIVE_MARKERS.some((marker) => marker.length === 0)) {
      throw new Error("empty required marker");
    }
    if (digestDirectory(join(sandbox.root, "missing")) !== "absent") {
      throw new Error("missing directory digest should be absent");
    }
    const staged = stagedSkillChecks();
    if (!staged.ulwResearchExists) {
      throw new Error(
        "staged ulw-research skill missing; run sync-skills.mjs first",
      );
    }
    ...
    if (empty.userTexts.length !== 0 || empty.hiddenDirectives.length !== 0) {
      throw new Error(
        "inspectSession should report nothing for a missing agent dir",
      );
    }
    ...
    writeFileSync(
      ...
      `${
        JSON.stringify({
          ...
          message: {
            role: "user",
            content: [{ type: "text", text: "ulw please respond" }],
          },
        })
      }\n` +
        `${
    ...
    if (
      probed.userTexts.length !== 1 ||
      probed.userTexts[0] !== "ulw please respond"
    ) {
      throw new Error("inspectSession failed to read the user message text");
    }
    if (
      probed.hiddenDirectives.length !== 1 ||
      probed.visibleDirectives.length !== 0
    ) {
      throw new Error(
        "inspectSession failed to classify the hidden directive entry",
      );
    }
  } finally {
...
function main() {
  const beforeDigest = digestDirectory(
  ...
  try {
    ...
    const resolvedSenpi = senpiBin.includes("/")
      ? (existsSync(senpiBin) ? senpiBin : null)
      : findOnPath(senpiBin);
    ...
    writeFileSync(
      ...
      `${
        JSON.stringify(
          { steps: [{ type: "text", text: "ulw prompts e2e complete" }] },
          null,
          2,
        )
      }\n`,
    );
    const run = spawnSync(resolvedSenpi, [
      "-e",
      mockProviderEntry,
      "-p",
      "--provider",
    ...
    const missingMarkers = REQUIRED_DIRECTIVE_MARKERS.filter((marker) =>
      !transcript.includes(marker)
    );
    const forbiddenHits = FORBIDDEN_TRANSCRIPT_MARKERS.filter((marker) =>
      transcript.includes(marker)
    );
    const staged = stagedSkillChecks();
    const ultraworkInjected = run.status === 0 && missingMarkers.length === 0 &&
      forbiddenHits.length === 0;
    const session = inspectSession(sandbox.agentDir);
    const userTextClean = session.userTexts.length > 0 &&
      session.userTexts.every((text) => !text.includes("<ultrawork-mode>"));
    const hiddenInjectionOk = session.hiddenDirectives.length === 1 &&
      session.visibleDirectives.length === 0 &&
      REQUIRED_DIRECTIVE_MARKERS.every((marker) =>
        session.hiddenDirectives[0].content.includes(marker)
      );
    result = ultraworkInjected && staged.ulwResearchNative &&
        staged.ultraworkDescriptionOk && userTextClean && hiddenInjectionOk
      ? "PASS"
      : "FAIL";
    return printResult({
      result,
...
function printResult({
  ...
  const afterDigest = digestDirectory(
    join(process.env.HOME ?? "", ".senpi", "agent"),
  );
  console.log(
    JSON.stringify({
