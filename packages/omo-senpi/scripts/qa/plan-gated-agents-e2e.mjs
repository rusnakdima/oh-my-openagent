...
const HOST_VOLATILE_SETTINGS_KEYS = [
  "workflow-skills",
  "tipsHistory",
  "skills",
];
    ...
    parentSteps: [
      {
        type: "tool_call",
        name: "read",
        arguments: { path: join(skillsDir, "ulw-plan", "SKILL.md") },
      },
      {
        type: "tool_call",
        name: "task",
  ...
  return {
    parentSteps: [
      ...
      },
      {
        type: "tool_call",
        name: "read",
        arguments: { path: join(skillsDir, "start-work", "SKILL.md") },
      },
      {
        type: "tool_call",
        name: "task",
...
function seedScenario(pluginRoot, script) {
  ...
  writeFileSync(
    join(sandbox.agentDir, "trust.json"),
    `${JSON.stringify({ [sandbox.canonicalCwd]: true }, null, 2)}\n`,
  );
...
function main() {
  ...
  const script = args.scenario === "sequence"
    ? sequenceScript(skillsDir)
    : args.scenario === "read-unlock"
    ? readUnlockScript(skillsDir)
    : denialScript();
  const scenarioPrompt = SCENARIO_PROMPTS[args.scenario] ??
    SCENARIO_PROMPTS.denial;
  ...
  try {
    ...
    const missingRequestDenial = transcript.includes(
      "available only after the user explicitly requests",
    );
    const missingArtifactDenial = transcript.includes("no plan artifact");
    const startWorkDenial = transcript.includes(
      "is plan-gated and cannot be spawned:",
    );
    const anyDenial = missingRequestDenial || missingArtifactDenial ||
      startWorkDenial;
    ...
    const passed = Object.values(checks).every((value) => value === true) &&
      beforeCredentials === afterCredentials;
    console.log(JSON.stringify({
      ...
      stdoutTail: (run.stdout ?? "").split("\n").filter((line) =>
        line.includes("plan-gated") || line.includes("momus review complete")
      ).slice(0, 6),
