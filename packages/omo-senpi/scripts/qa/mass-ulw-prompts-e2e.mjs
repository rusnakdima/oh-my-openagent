...
const REQUIRED_POINTER_MARKERS = [
  "<omo-mass-ulw-pointer>",
  "mass-ulw/SKILL.md",
  "read tool",
  "dag tool",
];
  ...
  for (const file of files.filter((path) => path.endsWith(".jsonl"))) {
    for (const line of readFileSync(file, "utf8").split("\n")) {
      ...
      if (
        ...
        entry.customType === MASS_ULW_CUSTOM_TYPE
      ) {
        const record = {
...
function runSelfTest() {
  ...
  try {
    seedSandbox(sandbox);
    if (REQUIRED_POINTER_MARKERS.some((marker) => marker.length === 0)) {
    ...
    writeFileSync(
      ...
      `${
        JSON.stringify({
          ...
          message: {
            role: "user",
            content: [{ type: "text", text: "mass ulw please respond" }],
          },
        })
      }\n` +
        `${
          JSON.stringify({
            ...
            display: false,
            content:
              "<omo-mass-ulw-pointer>mass-ulw/SKILL.md</omo-mass-ulw-pointer>",
          })
        }\n`,
    );
    ...
    if (
      ...
      probed.userTexts[0] !== "mass ulw please respond"
    ) {
      throw new Error("inspectSession failed to read the user message text");
    }
    if (
      probed.hiddenPointers.length !== 1 || probed.visiblePointers.length !== 0
    ) {
      throw new Error(
        "inspectSession failed to classify the hidden pointer entry",
      );
    }
  } finally {
...
function main() {
  ...
  try {
    ...
    writeFileSync(
      ...
      `${
        JSON.stringify(
          { steps: [{ type: "text", text: "mass ulw prompts e2e complete" }] },
          null,
          2,
        )
      }\n`,
    ...
    const run = spawnSync(
      ...
      [
        ...
        "--model",
        "mock-1",
        "mass ulw please orchestrate the docs refresh",
      ],
      {
        cwd: sandbox.cwd,
        env: {
    ...
    const missingMarkers = REQUIRED_POINTER_MARKERS.filter((marker) =>
    ...
    const userTextClean = session.userTexts.length > 0 &&
      session.userTexts.every((text) =>
        !text.includes("<omo-mass-ulw-pointer>")
      );
    const hiddenInjectionOk = session.hiddenPointers.length === 1 &&
      session.visiblePointers.length === 0 &&
      REQUIRED_POINTER_MARKERS.every((marker) =>
        session.hiddenPointers[0].content.includes(marker)
      );
    result = pointerInjected && userTextClean && hiddenInjectionOk
      ? "PASS"
      : "FAIL";
    return printResult({
      result,
