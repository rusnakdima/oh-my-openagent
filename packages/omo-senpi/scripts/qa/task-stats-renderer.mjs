...
function scenario(name) {
  if (name === "foreground") {
    return {
      ...
      required: ["foreground completed", "ran:2m5s", "tools:7", "tps:64"],
    };
  }
  if (name === "background") {
    return {
  ...
  throw new Error(
    "usage: bun packages/omo-senpi/scripts/qa/task-stats-renderer.mjs <foreground|background|team>",
  );
}
...
for (const token of selected.required) {
  if (!plain.includes(token)) {
    throw new Error(
      `missing required renderer token: ${token} in ${JSON.stringify(plain)}`,
    );
  }
}
const [runningLine = "", completedLine = ""] = selected.lines.map((line) =>
  line.replace(/\u001b\[[0-9;]*m/gu, "")
);
for (const token of selected.runningForbidden ?? []) {
  if (runningLine.includes(token)) {
    throw new Error(
      `unexpected running-row token: ${token} in ${
        JSON.stringify(runningLine)
      }`,
    );
  }
}
for (const token of selected.completedRequired ?? []) {
  if (!completedLine.includes(token)) {
    throw new Error(
      `missing completed-row token: ${token} in ${
