...
function createFakePi() {
  ...
  return {
    ...
    registerFlag(name, options) {
      if (!flagValues.has(name)) flagValues.set(name, options?.default);
    },
    getFlag(name) {
...
if (typeof factory !== "function") {
  console.error(
    JSON.stringify({
      verdict: "FAIL",
      reason: "bundle has no default-export factory",
    }),
  );
  process.exit(1);
}
...
await pi.dispatch("input", {
  type: "input",
  text: "ulw prove the fan-out reminder",
  source: "interactive",
}, sessionCtx("s-1"));
const armed = pi.messages.some((m) =>
  m.message?.customType === "omo-ultrawork:directive"
);
check("ulw input arms session via hidden directive message", armed, {
...
const reminderFrom = (results) =>
  results.find((r) =>
    ...
    r.content.some((b) =>
      typeof b?.text === "string" && b.text.includes("<system-reminder>")
    )
  );

// First task-adding todo result in the armed session -> exactly one reminder transform.
const firstResults = await pi.dispatch(
  "tool_result",
  todoResult("init"),
  sessionCtx("s-1"),
);
...
check(
  "first todo init result carries appended <system-reminder>",
  firstText.includes("<system-reminder>") && firstText.includes("fan-out") &&
    firstText.includes("fresh"),
  { blocks: first?.content?.length, handlerResults: firstResults.length },
);

// Second todo result in the same session -> no duplicate.
const secondResults = await pi.dispatch(
  "tool_result",
  todoResult("append"),
  sessionCtx("s-1"),
);
check(
  "second todo result in same session is untransformed",
  reminderFrom(secondResults) === undefined,
  { handlerResults: secondResults.length },
);

// Unarmed session -> no reminder.
const unarmedResults = await pi.dispatch(
  "tool_result",
  todoResult("init"),
  sessionCtx("s-2"),
);
check(
  "unarmed session gets no reminder",
  reminderFrom(unarmedResults) === undefined,
  { handlerResults: unarmedResults.length },
);
...
console.log(
  JSON.stringify(
    {
      verdict,
      surface: "built plugin/extensions/omo.js via senpi loader contract",
      results,
    },
    null,
    2,
