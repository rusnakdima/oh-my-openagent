...
import {
  ...
  pollUntil,
  startSenpiRun,
} from "./team-e2e-runtime.mjs";
import {
  DURA_REVIVE_SCRIPT,
  DURA_SEED_SCRIPT,
  LEAD_SCRIPT,
  NOOP_SCRIPT,
} from "./team-e2e-scripts.mjs";

const scriptDir = dirname(fileURLToPath(import.meta.url));
const mockProviderEntry = join(scriptDir, "team-e2e-mock-provider.ts");
const memberExtensionEntry = join(
  scriptDir,
  "..",
  "..",
  "plugin",
...
export const TEAM_E2E_OMO_CONFIG = {
  ...
  agents: {
    fixture: {
      ...
      description: "team fixture agent",
...
export function resolveSenpi(operations = {}) {
  ...
  if (
    requested !== undefined && requested !== "" && isExecutablePath(requested)
  ) {
    return fileExists(requested) ? requested : null;
  }
  const names = platform === "win32" &&
      (requested === undefined || requested === "" ||
        requested.toLowerCase() === "senpi")
    ? ["senpi.exe", "senpi.cmd", "senpi"]
    : [requested || "senpi"];
  const pathDelimiter = operations.delimiter ??
    (platform === "win32" ? ";" : delimiter);
  const joinPath = operations.joinPath ??
...
function isExecutablePath(value) {
  return isAbsolute(value) || win32.isAbsolute(value) || value.includes("/") ||
    value.includes("\\");
}
...
function seedProject(sandbox) {
  ...
  writeFileSync(
    ...
    `${JSON.stringify(TEAM_E2E_OMO_CONFIG, null, 2)}\n`,
  );
}
...
async function runMain(senpiBin, outDir) {
  ...
  try {
    ...
    const active = startRun({
      senpiBin,
      sandbox,
      prompt: "drive injection-only team delivery",
      script: LEAD_SCRIPT,
      obsDir,
    });
    let observation;
    try {
      observation = await pollUntil(
        ...
        (value) =>
          value.memberEnvelopeEchoed && value.memberToLeadInjected &&
          value.leadInbox.unread === 0 && value.leadInbox.reserved === 0,
    ...
    writeFileSync(
      join(outDir, "main-observed.json"),
      `${JSON.stringify(observation, null, 2)}\n`,
    );
    return {
      checks: analyzeMain(run, sandbox, obsDir, observation),
      exit: run.status,
    };
  } finally {
...
function mainInjectionObserved(cwd, obsDir) {
  ...
  const quickTask = runId === undefined
    ? undefined
    : memberTaskId(cwd, runId, "quick");
...
async function runDuraRevive(senpiBin, outDir) {
  ...
  try {
    ...
    const active = startRun({
      senpiBin,
      sandbox,
      prompt: "durability revive drain drive",
      script: DURA_REVIVE_SCRIPT,
      obsDir,
    });
    let state;
    try {
    ...
    writeFileSync(
      join(outDir, "dura-inbox.json"),
      `${JSON.stringify(state, null, 2)}\n`,
    );
    return {
      duraBacklogSeeded:
        readText(join(obsDir, "dura-seeded.txt")) !== undefined,
      duraMessagesEnqueued: teamMessageEnqueues(send).length >= 2,
      duraUnreadDrainedToZero: state.counts.unread === 0 &&
...
async function runReclaim(senpiBin, outDir) {
  ...
  try {
    seedProject(sandbox);
    const seed = await runSenpi({
      senpiBin,
      sandbox,
      prompt: "seed an active team for reclaim",
      script: DURA_SEED_SCRIPT,
    });
    writeFileSync(join(outDir, "reclaim-seed-stdout.json.log"), seed.stdout);
    const runId = discoverRunIds(sandbox.cwd)[0];
    if (runId === undefined) {
      return { reclaimReservationRestored: false, reclaimNoLeak: false };
    }
    ...
    const reclaim = await runSenpi({
      senpiBin,
      sandbox,
      prompt: "boot a fresh session so session_start reclaims",
      script: NOOP_SCRIPT,
    });
    writeFileSync(join(outDir, "reclaim-boot-stdout.json.log"), reclaim.stdout);
    const after = inboxCounts(inbox);
    writeFileSync(
      join(outDir, "reclaim-inbox.json"),
      `${
        JSON.stringify(
          {
            runId,
            reservation: reservation.messageId,
            before,
            after,
            restored: existsSync(reservation.restoredPath),
          },
          null,
          2,
        )
    ...
    return {
      reclaimReservationRestored: before.reserved === 1 && after.unread === 1 &&
        existsSync(reservation.restoredPath),
...
export function createOutDir(configured = process.env.TEAM_E2E_OUT_DIR) {
  ...
  return {
    outDir: resolve(mkdtempSync(join(tmpdir(), "omo-senpi-team-e2e-"))),
    cleanup: true,
  };
}
...
async function main() {
  ...
  try {
    ...
    try {
      ...
      const crash = await runCrashRestartScenario({
        senpiBin,
        outDir,
        createSandbox,
        seedProject,
        startRun,
        memberExtensionEntry,
      });
      // Plan todo 22: member quit->resume revival + shutdown-approved non-revival (own module).
      const resume = await runTeamResumeScenarios({
        senpiBin,
        outDir,
        startRun,
      });
    ...
    const payload = {
      result: result === "PASS" && credentialIsolationClean && leakedPids === 0
        ? "PASS"
        : "FAIL",
      checks,
      failed,
...
function selfTest() {
  ...
  if (defaultOutDir.outDir.startsWith(scriptDir)) {
    throw new Error(
      "self-test: team e2e default capture directory must not live under scripts/qa",
    );
  }
  if (defaultOutDir.cleanup) {
    rmSync(defaultOutDir.outDir, { recursive: true, force: true });
  }
  verifyMainInjectionHelper();
  const scriptSource = readFileSync(
    join(scriptDir, "team-e2e-scripts.mjs"),
    "utf8",
  );
  ...
  if (DURA_REVIVE_SCRIPT.lead.at(-1)?.type !== "hang") {
    throw new Error(
      "self-test: durability drive must remain alive while inbox drain is observed",
    );
  }
    ...
    [
      JSON.stringify({
        ...
        result: {
          content: [{
            type: "text",
            text: "Created team 'e2eteam' (run-1) with 2 members.",
          }],
          details: {
            kind: "created",
            team_run_id: "run-1",
            members: [{ name: "quick", status: "running" }, {
              name: "fixture",
              status: "running",
            }],
          },
  ...
  if (create?.details?.team_run_id !== "run-1") {
    throw new Error("self-test: team_create details not parsed");
  }
  if (create.isError !== false) {
    throw new Error("self-test: top-level isError not read");
  }
  ...
  if (empty.unread !== 0 || empty.reserved !== 0) {
    throw new Error("self-test: missing inbox should be zeroed");
  }
  if (verdict({ a: true, b: true }).result !== "PASS") {
    throw new Error("self-test: all-true verdict should PASS");
  }
  if (verdict({ a: true, b: false }).result !== "FAIL") {
    throw new Error("self-test: any-false verdict should FAIL");
  }
  if (resolveSenpi === undefined) {
    throw new Error("self-test: resolveSenpi missing");
  }
...
function verifyMainInjectionHelper() {
  ...
  try {
    ...
    writeFileSync(
      join(runtime, "senpi-task-members.json"),
      JSON.stringify({ quick: "st_self_test" }),
    );
    const observed = mainInjectionObserved(root, join(root, "obs"));
    if (observed.quickTask !== "st_self_test") {
      throw new Error("self-test: member task lookup failed");
    }
  } finally {
