...
export async function runTaskResumeScenarios(
  { senpiBin, checks, capture, pids, outDir },
) {
...
export function witnessRevived(cwd, name) {
  ...
  return witness !== undefined &&
    revivedAfterSuspend(taskEventText(cwd, witness.task_id));
}
...
async function runHappyLane(ctx) {
  ...
  const seeded = await pollUntil(
    () => ({
      midAssistant: childSessionHasAssistant(
        sandbox.cwd,
        findTaskByName(sandbox.cwd, "midchild")?.task_id ?? "",
      ),
      finCompleted:
        findTaskByName(sandbox.cwd, "finchild")?.status === "completed",
    }),
  ...
  ctx.checks.resume_spawn_children = verdict(
    result1.status === 0 && seeded.midAssistant && seeded.finCompleted &&
      mid !== undefined && fin !== undefined,
  );
  ctx.checks.resume_quit_suspends_both = verdict(
    mid?.status === "running" && mid?.residency_state === "persisted_only" &&
      mid?.host_pid === undefined &&
  ...
  const revived = await pollUntil(
    () => ({
      midResident:
        findTaskByName(sandbox.cwd, "midchild")?.residency_state === "resident",
      finResident:
        findTaskByName(sandbox.cwd, "finchild")?.residency_state === "resident",
      midContinued: childSessionText(sandbox.cwd, mid?.task_id ?? "").includes(
        MIDTURN_CONTINUED_TOKEN,
      ),
      finPong: childSessionText(sandbox.cwd, fin?.task_id ?? "").includes(
        PONG_TOKEN,
      ),
    }),
  ...
  ctx.checks.resume_revived_resident = verdict(
    revived.midResident && revived.finResident,
  );
  ctx.checks.resume_midturn_continuation = verdict(
    midSession.includes(CONTINUATION_MARKER) &&
      midSession.includes(MIDTURN_CONTINUED_TOKEN),
  );
  ctx.checks.resume_finished_no_nudge = verdict(
    finSession.length > 0 && !finSession.includes(CONTINUATION_MARKER),
  );
  ...
  const logsBefore = {
    mid: taskEventText(sandbox.cwd, mid?.task_id ?? ""),
    fin: taskEventText(sandbox.cwd, fin?.task_id ?? ""),
  };
  ...
  ctx.checks.resume_unrelated_no_revive = verdict(
    result3.status === 0 &&
      ...
      findTaskByName(sandbox.cwd, "midchild")?.residency_state ===
        "persisted_only" &&
      findTaskByName(sandbox.cwd, "finchild")?.residency_state ===
        "persisted_only",
  );
  ctx.capture.resumeHappy = {
    sessionId,
    midTask: mid?.task_id,
    finTask: fin?.task_id,
  };
