...
const SCENARIOS = [
  {
    name: "A-classifier-refusal",
    primaryOutcome: "refusal",
    categories: "declared",
    expectDirective: true,
  },
  {
    name: "B-policy-rejection",
    primaryOutcome: "policy_error",
    categories: "declared",
    expectDirective: true,
  },
  {
    name: "C-transient-fallback",
    primaryOutcome: "transient",
    categories: "declared",
    expectDirective: false,
  },
  {
    name: "D-architect-disabled",
    primaryOutcome: "refusal",
    categories: "disabled",
    expectDirective: false,
  },
  {
    name: "E-builtin-default",
    primaryOutcome: "refusal",
    categories: "builtin",
    expectDirective: true,
  },
...
function seedScenario(scenario) {
  ...
  const settings = JSON.parse(
    readFileSync(join(sandbox.agentDir, "settings.json"), "utf8"),
  );
  settings.retry = {
    enabled: true,
    maxRetries: 1,
  ...
  writeFileSync(
    ...
    `${
      JSON.stringify(
        { categories: categoriesByShape[scenario.categories] },
        null,
        2,
      )
    }\n`,
...
function collectDirectives(entries) {
  return entries.filter((entry) =>
    collectCustomTypes(entry).includes(DIRECTIVE_TYPE)
  );
}

function collectNotices(entries) {
  return entries.filter((entry) =>
    collectCustomTypes(entry).includes(NOTICE_TYPE)
  );
}
  ...
  while (stack.length > 0) {
    ...
    if (Reflect.get(current, "customType") === customType) {
      ...
      return {
        ...
        display: Reflect.get(current, "display"),
      };
    }
    for (const nested of Object.values(current)) stack.push(nested);
  }
...
function runScenario(senpiBin, scenario) {
  ...
  try {
    ...
    const content = directives.length === 0
      ? ""
      : directiveContent(directives[0]);
    const notices = collectNotices(entries);
    const noticePayload = notices.length === 0
      ? undefined
      : findCustomPayload(notices[0], NOTICE_TYPE);
    const noticeOk = scenario.expectDirective
      ? notices.length === 1 &&
        ...
        String(noticePayload?.content ?? "").includes(
          'task(category: "architect")',
        )
      : notices.length === 0;
    ...
    const contentOk = scenario.expectDirective
      ? content.includes(PRIMARY) &&
        ...
        content.includes(
          "Decompose the current problem into independent parts",
        ) &&
    ...
    return {
      ...
      result: directives.length === expectedCount && contentOk && noticeOk &&
          reminderLeaks.length === 0 && exitOk
        ? "PASS"
        : "FAIL",
...
function runSelfTest() {
  const entries = [
    {
      type: "custom",
      message: {
        customType: DIRECTIVE_TYPE,
        content: `${PRIMARY} ${FALLBACK} task(category: "architect")`,
      },
    },
    { type: "user", content: "hello" },
  ];
  if (collectDirectives(entries).length !== 1) {
    throw new Error("self-test: directive entry not detected exactly once");
  }
  if (!directiveContent(collectDirectives(entries)[0]).includes(PRIMARY)) {
    throw new Error("self-test: directive content not read");
  }
  if (collectDirectives([{ type: "user", content: "hello" }]).length !== 0) {
    throw new Error("self-test: false positive");
  }
  const duplicated = [...entries, entries[0]];
  if (collectDirectives(duplicated).length !== 2) {
    throw new Error(
      "self-test: duplicate directives must be counted, not collapsed",
    );
  }
  if (SCENARIOS.filter((s) => s.expectDirective).length !== 3) {
    throw new Error("self-test: expected three positive scenarios");
  }
  const noticeEntry = {
    type: "custom_message",
  ...
  if (collectNotices([noticeEntry, { type: "user" }]).length !== 1) {
    throw new Error("self-test: notice entry not detected");
  }
  if (findCustomPayload(noticeEntry, NOTICE_TYPE)?.display !== true) {
    throw new Error("self-test: notice payload not read");
  }
  const leakingUser = {
    type: "message",
    message: {
  ...
  if (userMessagesLeakingReminder([leakingUser]).length !== 1) {
    throw new Error("self-test: reminder leak not detected");
  }
  if (
    userMessagesLeakingReminder([{
      type: "message",
  ...
  const liveChurn = changedFiles(beforeDigests, {
    ...beforeDigests,
    "settings.json": "x",
  });
  if (
    liveChurn.length !== 1 ||
    liveChurn.every((name) => CREDENTIAL_FILES.includes(name))
  ) {
    throw new Error(
      "self-test: live-session churn must be detected as non-credential",
    );
  }
  const credentialChange = changedFiles(beforeDigests, {
    ...beforeDigests,
    "auth.json": "x",
  });
  if (!credentialChange.some((name) => CREDENTIAL_FILES.includes(name))) {
    throw new Error(
      "self-test: credential change must trip the isolation gate",
    );
  }
  console.log(
    JSON.stringify(
...
function main() {
  ...
  if (resolved === null) {
    console.log(
      JSON.stringify(
        { result: "SKIP", reason: "senpi-binary-unavailable" },
        null,
        2,
      ),
    );
  ...
  const liveSessionActivity = changed.filter((name) =>
    LIVE_SESSION_FILES.includes(name)
  );
  const result = scenarios.every((s) => s.result === "PASS") && isolated
    ? "PASS"
    : "FAIL";
