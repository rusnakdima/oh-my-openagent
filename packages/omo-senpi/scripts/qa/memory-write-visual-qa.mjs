    ...
    steps: [
      {
        ...
        arguments: {
          ...
          file_text:
            "The deploy runbook lives in the team wiki under Runbooks/Deploy.\n",
        },
      },
...
try {
  function capture(label, inputs) {
    ...
    const senpiCmd =
      `${envPrefix} ${senpiBin} -e ${mockProviderEntry} --provider omo-mock --model mock-1`;
    const outDir = join(evidenceDir, label);
    const args = [
      qaScript,
      "--title",
      `memory-write-notice-${label}`,
      "--command",
      senpiCmd,
      "--cwd",
      sandbox.cwd,
      ...
      outDir,
      "--source-label",
      "senpi interactive mock-provider memory write",
    ];
    ...
    const transcript = existsSync(join(outDir, "terminal.txt"))
      ? readFileSync(join(outDir, "terminal.txt"), "utf8")
      : "";
    const ansi = existsSync(join(outDir, "terminal-ansi.txt"))
      ? readFileSync(join(outDir, "terminal-ansi.txt"), "utf8")
      : "";
    return { transcript, ansi };
  }

  // Collapsed row: typed prompt -> mock emits the memory tool call -> real tool executes.
  const collapsed = capture("collapsed", [
    "remember the deploy runbook location",
    "{Enter}",
  ]);
  record(
    "collapsed: notice title rendered",
    /Memory updated/.test(collapsed.transcript),
    "expects 'Memory updated' in row",
  );
  record(
    "collapsed: old line gone",
    !/committed locally/.test(collapsed.transcript),
    "'committed locally' must not appear",
  );
  record(
    "collapsed: command name gone",
    !/create committed|Memory create/.test(collapsed.transcript),
    "command name must not appear",
  );
  record(
    "collapsed: why line names path",
    /knowledge\/deploy\.md/.test(collapsed.transcript),
    "affected path visible",
  );
  record(
    "collapsed: bold title styling",
    /\[1m.*Memory updated/s.test(collapsed.ansi),
    "SGR bold around title",
  );
  ...
  record(
    "expanded: sha in detail",
    /[0-9a-f]{7} · /.test(expanded.transcript),
    "sha7 + separator in expanded detail",
  );
  record(
    "expanded: subject in detail",
    /Track the deploy runbook/.test(expanded.transcript),
    "commit subject in expanded detail",
  );
  ...
  for (const sandbox of sandboxes) {
    if (keepSandbox) {
      ...
      console.log(
        `cleanup: removed sandbox ${sandbox.root}; exists=${
