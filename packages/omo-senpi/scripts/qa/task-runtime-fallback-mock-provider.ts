...
const QUOTA_ERROR =
  `403: {"message":"You've reached your usage limit for this billing cycle. Your quota will be refreshed in the next cycle.","type":"access_terminated_error"}`;
...
export default function registerFallbackMockProvider(pi: ExtensionAPI): void {
    ...
    streamSimple(model, context) {
      ...
      return streamMessage(
        parentCalls === 1
          ...
          : assistant(model.id, "stop", [{
            type: "text",
            text: "parent observed fallback completion",
          }]),
      );
    },
  });
  ...
  pi.registerProvider("quotio-openai", {
    ...
    api: "openai-completions",
    models: [{
      ...mockModel("gpt-5.6-luna-fast", "Chain rung two"),
      reasoning: true,
    }],
    streamSimple(model, context) {
      return streamMessage(childReply(model.id));
  ...
  if (process.env.OMO_FALLBACK_DEBUG_DUMP === "1") {
    ...
    const dump = (label: string) => {
      const registry = (pi as unknown as {
        modelRegistry?: {
          getAll(): { provider: string; id: string }[];
          find(p: string, i: string): unknown;
        };
      }).modelRegistry;
      const ids = registry?.getAll().map((model) =>
        `${model.provider}/${model.id}`
      ) ?? [];
      fs.writeFileSync(
        `/tmp/fallback-dump-${label}.json`,
        JSON.stringify(
          {
            label,
            count: ids.length,
            ...
            findQuotio: registry?.find("quotio-openai", "gpt-5.6-luna-fast") !==
              undefined,
          },
          null,
          2,
...
function isChild(context: Context): boolean {
  return (context.messages ?? []).some((message) => {
    if (typeof message.content === "string") {
      return message.content.includes(CHILD_IDENTITY);
    }
    return message.content.some((part) =>
      part.text?.includes(CHILD_IDENTITY) === true
    );
  });
}
...
if (
  process.argv[1]?.endsWith("task-runtime-fallback-mock-provider.ts") &&
  process.argv.includes("--self-test")
) {
  if (!isChild({ messages: [{ content: `You are ${CHILD_IDENTITY}.` }] })) {
    throw new Error("child detection failed");
  }
  if (isChild({ messages: [{ content: "parent" }] })) {
    throw new Error("parent misclassified");
  }
