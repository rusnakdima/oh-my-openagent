...
const providerEntry = join(
  scriptDir,
  "openai-recommendations-mock-provider.ts",
);
...
function credentialDigests() {
  return Object.fromEntries(
    realAgentDirs.map((path) => [path, credentialDigest(path)]),
  );
}
...
function inspectAutomatic(records, outputEvents, captures) {
  ...
  for (const [category, expected] of Object.entries(automaticExpectations)) {
    ...
    checks[`${category}_model`] = actualModel === expected.model
      ? "PASS"
      : "FAIL";
    checks[`${category}_variant`] = actualVariant === expected.variant
      ? "PASS"
      : "FAIL";
    checks[`${category}_completed`] = record?.status === "completed"
      ? "PASS"
      : "FAIL";
  }
  for (const [agent, expected] of Object.entries(automaticAgentExpectations)) {
    ...
    checks[`${agent}_variant`] = actualVariant === expected.variant
      ? "PASS"
      : "FAIL";
    checks[`${agent}_completed`] = record?.status === "completed"
      ? "PASS"
      : "FAIL";
  }
  checks.architect_unavailable =
    records.every((record) => record.category !== "architect") &&
      outputEvents.some((event) =>
        containsText(event, 'No available model for category "architect"')
      )
      ? "PASS"
      : "FAIL";
  checks.child_models_exercised =
    captures.filter((capture) => capture.child === true).length === 6
      ? "PASS"
      : "FAIL";
...
function inspectOverride(records, captures) {
  ...
  return {
    checks: {
      ...
      override_variant: record?.resolved_model?.variant === "low"
        ? "PASS"
        : "FAIL",
      override_completed: record?.status === "completed" ? "PASS" : "FAIL",
      one_child_exercised:
        captures.filter((capture) => capture.child === true).length === 1
          ? "PASS"
          : "FAIL",
    },
    observed: {
...
function runScenario(senpiBin, scenario, evidenceDir) {
  ...
  try {
    ...
    if (
      fixture.configPath !== undefined && fixture.configDigest !== undefined
    ) {
      configUnchanged = existsSync(fixture.configPath) &&
        sha256(readFileSync(fixture.configPath)) === fixture.configDigest;
    }
  } finally {
  ...
  const checks = {
    ...
    real_agent_credentials_unchanged:
      JSON.stringify(beforeCredentials) === JSON.stringify(afterCredentials)
        ? "PASS"
        : "FAIL",
    real_omo_config_unchanged: beforeUserConfig === afterUserConfig
      ? "PASS"
      : "FAIL",
    sandbox_cleanup: cleanup,
  };
  const result = Object.values(checks).every((value) => value === "PASS")
    ? "PASS"
    : "FAIL";
  const verdict = {
    result,
...
function selfTest() {
  if (
    Object.keys(automaticExpectations).join(",") !==
      "artistry,writing,visual-engineering,quick"
  ) {
    throw new Error("automatic recommendation case set drifted");
  }
  if (
    Object.keys(automaticAgentExpectations).join(",") !== "explore,librarian"
  ) {
  ...
  try {
    if (
      ...
      throw new Error(
        "override fixture did not create an isolated project config",
      );
    }
...
function main(evidenceDir) {
  ...
  const results = scenarios.map((scenario) =>
    runScenario(senpiBin, scenario, evidenceDir)
  );
  const summary = {
    result: results.every((scenario) => scenario.result === "PASS")
      ? "PASS"
      : "FAIL",
