...
function hashFiles(root, excludePrefix = undefined) {
  ...
  const walk = (dir) => {
    for (const name of readdirSync(dir).toSorted()) {
      ...
      if (excludePrefix !== undefined && rel.startsWith(excludePrefix)) {
        continue;
      }
      files.set(
        rel,
...
function gitLines(repo, format) {
  const run = spawnSync("git", ["log", `--format=${format}`, "HEAD"], {
    cwd: repo,
    encoding: "utf8",
  });
  return run.status === 0
...
async function main() {
  ...
  if (senpiBin === null) {
    throw new Error("senpi binary not found (set SENPI_BIN)");
  }
  ...
  const credentialSnapshot = (dir) =>
    new Map(CREDENTIALS.map((name) => {
      const path = join(dir, name);
      return [
        path,
        existsSync(path)
          ? createHash("sha256").update(credentialBytes(path, name)).digest(
            "hex",
          )
          : "absent",
      ];
    }));
  const omoAgentCredentialsBefore = credentialSnapshot(
    join(homedir(), ".omo", "agent"),
  );
  const senpiCredentialsBefore = credentialSnapshot(
      ...
      JSON.stringify(
        {
          categories: {
            quick: {
              description: "QA mock quick category",
  ...
  const { FactsExtractorRunner } = await import(
    join(packageRoot, "src", "components", "memory", "facts-runner.ts")
  );
  const identity = {
    id: IDENTITY,
    safeSlug: IDENTITY,
  ...
  const seededFiles = readdirSync(identity.paths.factsQueue).filter((name) =>
    name.endsWith(".json") && name !== "consumed.json"
  );
  record(
    "seeded legacy backlog (4 queue entry files, one oversize)",
    seededFiles.length === 4,
    `${seededFiles.length} files`,
  );
  ...
  const runner = new FactsExtractorRunner({
    ...
    resolveModelRegistry: () => ({
      ...
      find: (
        provider,
        modelId,
      ) => (provider === "omo-mock" && modelId === "mock-1"
  ...
  record(
    "A first launch committed",
    launched.status === "committed",
    JSON.stringify(launched),
  );

  const runsDir = join(identity.paths.facts, "runs");
  const runNames = existsSync(runsDir)
    ? readdirSync(runsDir).filter((name) => name.startsWith("facts-"))
    : [];
  record(
    "A drain produced multiple capped runs from one launch",
    runNames.length >= 2,
  ...
  const trailers = gitLines(repo, "%(trailers:key=Generated-By,valueonly)").map(
    (line) => line.trim(),
  ).filter((line) => line.length > 0);
  const factsCommits = subjects.filter((subject) =>
    subject.startsWith("feat(facts)") || subject.includes("facts")
  );
  record(
    "B commits carry Generated-By: facts-extractor",
    trailers.length >= 2 &&
      trailers.every((value) => value === "facts-extractor"),
    `${trailers.length} trailers: ${[...new Set(trailers)].join(",")}`,
  );
  record(
    "B facts commits landed in the memory repo",
    factsCommits.length >= 2,
    `${factsCommits.length}: ${factsCommits.slice(0, 3).join(" | ")}`,
  );

  const consumed = JSON.parse(
    readFileSync(core.factsQueuePaths(identity.paths).consumedPath, "utf8"),
  );
  const consumedKeys = Object.entries(consumed.consumed).map((
    [conversationId, value],
  ) => `${conversationId}:${value.end_message_id}`);
  const failuresFile = JSON.parse(
    readFileSync(core.factsQueuePaths(identity.paths).failuresPath, "utf8"),
  );
  const parked = failuresFile.entries.filter((entryRecord) =>
    entryRecord.state === "parked"
  );
  const parkedKeys = parked.map((entryRecord) =>
    `${entryRecord.conversationId}:${entryRecord.end_message_id}`
  );
  const accounted = new Set([...consumedKeys, ...parkedKeys]);
  record(
    "C losslessness: every seeded entry is committed or parked",
    seeded.every((key) => accounted.has(key)),
    `seeded=${seeded.join(",")} consumed=${consumedKeys.join(",")} parked=${
      parkedKeys.join(",")
    }`,
  );
  record(
    "C backlog drained: only parked entries remain pending",
    (await queue.listPending()).every((entry) =>
      parkedKeys.includes(
        `${entry.conversationId}:${entry.range.end_message_id}`,
      )
    ),
    `pending=${
      (await queue.listPending()).map((entry) => entry.conversationId).join(",")
    }`,
  );

  const payloadsLeft = runNames.filter((name) =>
    existsSync(join(runsDir, name, "facts-payload.json"))
  );
  const terminal = runNames.filter((name) =>
    existsSync(join(runsDir, name, "final.json"))
  );
  record(
    "D every run is terminal",
    terminal.length === runNames.length,
    `${terminal.length}/${runNames.length}`,
  );
  record(
    "D no payload survives a terminal run",
    payloadsLeft.length === 0,
    payloadsLeft.join(",") || "none",
  );
  const skipped = warnings.filter((entry) =>
    JSON.stringify(entry).includes("skipped_overflow")
  );
  record(
    "D no skipped_overflow anywhere",
    skipped.length === 0,
    `${skipped.length}`,
  );

  record(
    "E oversize entry parked exactly once, not looped",
    parked.length === 1 && parked[0].lastReason === "payload_entry_oversize" &&
      parked[0].streak === 1,
  ...
  const oversizeFile = readdirSync(identity.paths.factsQueue).find((name) =>
    name.endsWith(".json") && name !== "consumed.json" &&
    name !== "failures.json" &&
    statSync(join(identity.paths.factsQueue, name)).size > CAP
  );
  record(
    "E oversize queue file retained untruncated",
    oversizeFile !== undefined,
    oversizeFile ?? "missing",
  );

  // A second launch must NOT relaunch the parked entry (no run-dir growth).
  const second = await runner.launchPending();
  const runNamesAfter = readdirSync(runsDir).filter((name) =>
    name.startsWith("facts-")
  );
  record(
    "E parked entry never relaunches",
    second.status === "empty" && runNamesAfter.length === runNames.length,
    `${second.status} runs=${runNamesAfter.length}`,
  );
  ...
  await failQueue.enqueue({
    identity: failIdentity.id,
    sessionId: "good-a",
    conversationId: "good-a",
    entries: [transcriptEntry("good-a-m1", bulk)],
  });
  await failQueue.enqueue({
    identity: failIdentity.id,
    sessionId: "fail-b",
    conversationId: "fail-b",
    entries: [
      transcriptEntry(
        "fail-b-m1",
        `FAILCHILD ${"f".repeat(Math.floor(CAP * 0.5))}`,
      ),
    ],
  });
  await failQueue.enqueue({
    identity: failIdentity.id,
    sessionId: "good-c",
    conversationId: "good-c",
    entries: [transcriptEntry("good-c-m1", bulk)],
  });
  ...
  const failRunNames = existsSync(failRunsDir)
    ? readdirSync(failRunsDir).filter((name) => name.startsWith("facts-"))
    : [];
  const failOutcomes = failRunNames.map((name) =>
    JSON.parse(readFileSync(join(failRunsDir, name, "final.json"), "utf8"))
      .outcome
  );
  record(
    "G mid-drain failure: newest batch commits, failing batch fails",
    failResult.status === "committed" && failOutcomes.includes("failed"),
    `${failResult.status} outcomes=${failOutcomes.join(",")}`,
  );
  record(
    "G drain stopped at the failure: no third run launched",
    failRunNames.length === 2,
    `runs=${failRunNames.length}`,
  );
  const failPending = (await failQueue.listPending()).map((entry) =>
    entry.conversationId
  ).toSorted();
  record(
    "G older good entry never attempted",
    failPending.includes("good-a") && failPending.includes("fail-b") &&
      !failPending.includes("good-c"),
    `pending=${failPending.join(",")}`,
  );
  const failLedger = JSON.parse(
    readFileSync(core.factsQueuePaths(failIdentity.paths).failuresPath, "utf8"),
  );
  const streakRecords = failLedger.entries.filter((entryRecord) =>
    entryRecord.conversationId === "fail-b"
  );
  const goodARecords = failLedger.entries.filter((entryRecord) =>
    entryRecord.conversationId === "good-a"
  );
  record(
    "G failure recorded exactly one streak increment",
    streakRecords.length === 1 && streakRecords[0].streak === 1 &&
      streakRecords[0].state === "backoff" &&
      streakRecords[0].lastReason === "child_exit" && goodARecords.length === 0,
    `${JSON.stringify(streakRecords)} good-a=${goodARecords.length}`,
  );
  ...
  console.log(
    gitLines(
      repo,
      "%h %s [Generated-By: %(trailers:key=Generated-By,valueonly)]",
    ).join("\n"),
  );
  console.log(`--- runs (${runNames.length}) ---`);
  for (const name of runNames.toSorted()) {
    const ledger = JSON.parse(
      readFileSync(join(runsDir, name, "ledger.json"), "utf8"),
    );
    const final = JSON.parse(
      readFileSync(join(runsDir, name, "final.json"), "utf8"),
    );
    console.log(
      `${name} outcome=${final.outcome} queued=${
        ledger.queued.map((key) =>
          `${key.conversationId}:${key.end_message_id}`
        ).join(",")
      }`,
    );
  }
  ...
  record(
    ...
    cleanup.length === 0 && !existsSync(root),
    root,
  );
  ...
  record(
    "F real ~/.omo/agent credentials byte-identical",
    credentialsIntact(
      omoAgentCredentialsBefore,
      join(homedir(), ".omo", "agent"),
    ),
    CREDENTIALS.join(","),
  );
  record(
    "F real ~/.senpi/agent credentials byte-identical",
    credentialsIntact(
      senpiCredentialsBefore,
      join(homedir(), ".senpi", "agent"),
    ),
    CREDENTIALS.join(","),
  );
  ...
  record(
    "F real ~/.omo/memory root untouched (live per-session repos excluded)",
    realMemoryBefore === realMemoryAfter,
    `${realMemoryBefore.slice(0, 16)} vs ${realMemoryAfter.slice(0, 16)}`,
  );
  const leakedIdentity = join(homedir(), ".omo", "memory", "agents", IDENTITY);
  record(
    "F driver identity never leaked into the real home",
    !existsSync(leakedIdentity),
    leakedIdentity,
  );
  const realAgentAfter = hashDir(join(homedir(), ".senpi", "agent"));
  record(
    "F real ~/.senpi/agent untouched",
    realAgentBefore === realAgentAfter,
    `${realAgentBefore.slice(0, 16)} vs ${realAgentAfter.slice(0, 16)}`,
  );

  console.log(
    JSON.stringify(
      { ok: failures.length === 0, total: results.length, failures },
      null,
      2,
    ),
  );
