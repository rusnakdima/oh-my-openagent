...
function recordOutcome(cwd) {
  ...
  if (sessionText.includes(MIDTURN_CONTINUED_TOKEN)) {
    return { kind: "continued", record };
  }
...
async function runScenario(options) {
  ...
  const runs = [];
  const summary = {
    pluginMode: options.pluginPath === undefined
  ...
  try {
    ...
    if (firstResult.status !== null) {
      throw new Error(
        `first parent was not hard-killed: ${firstResult.status}`,
      );
    }
    ...
    if (secondResult.status !== 0) {
      throw new Error(
        `replacement parent failed with status ${
          String(secondResult.status)
        }: ${secondResult.stderr}`,
      );
    }
    ...
    writeArtifact(
      join(options.evidenceDir, "parent-restart-transcript.txt"),
      transcript,
    );

    if (outcome.kind !== "continued") {
      throw new Error(
        outcome.record.error_message ??
          `task ended with ${outcome.record.status}`,
      );
    }
    if (outcome.record.task_id !== initialRecord.task_id) {
      throw new Error("task id changed across parent restart");
    }
  } finally {
    stateAbort.abort();
    ...
    writeArtifact(
      join(options.evidenceDir, "parent-restart-summary.json"),
  ...
  console.log(
    `PASS parent_restart_process_child_recovered task=${summary.taskId}`,
  );
}
...
function runSelfTest() {
  ...
  if (parsed.evidenceDir !== resolve("./evidence")) {
    throw new Error("evidence path parsing failed");
  }
  if (parsed.pluginPath !== resolve("./plugin")) {
    throw new Error("plugin path parsing failed");
  }
  const continued = recordOutcomeFromValues("running", MIDTURN_CONTINUED_TOKEN);
  const lost = recordOutcomeFromValues("lost", "");
  if (continued !== "continued" || lost !== "lost") {
    throw new Error("outcome classification failed");
  }
}
