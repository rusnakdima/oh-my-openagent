...
export function analyzeMain(
  ...
  const memberNames = (create?.details?.members ?? []).map((member) =>
    member.name
  ).sort();
  const leadToQuick = teamMessageEnqueues(sends).find((entry) =>
    entry.recipients.includes("quick")
  );
  ...
  const evidence = observedEvidence ??
    injectionEvidence(
      ...
      runId,
      quickTask,
      leadToQuick?.messageId,
      obsDir,
    );
  mkdirSync(obsDir, { recursive: true });
  writeFileSync(
    join(obsDir, "team-injection-evidence.json"),
    `${JSON.stringify(evidence, null, 2)}\n`,
  );
  return {
    createTwoMembersActive: create?.details?.kind === "created" &&
      (create.details.members?.length ?? 0) === 2,
    createListsActiveMembers:
      JSON.stringify(memberNames) === JSON.stringify(["fixture", "quick"]),
    ...
    leadInboxDrained: evidence.leadInbox.unread === 0 &&
      evidence.leadInbox.reserved === 0,
    noBlockingTeamWaitCalls:
      findResults(run.events, ["team", "wait"].join("_")).length === 0,
  };
}
...
export function teamMessageEnqueues(sendResults) {
  return sendResults.flatMap((result) => {
    ...
    if (team?.kind !== "to_members" || !Array.isArray(team.recipients)) {
      return [];
    }
...
export function injectionEvidence(cwd, runId, quickTask, leadMessage, obsDir) {
  ...
  return {
    ...
    quickTask,
    leadMessage,
    memberEnvelopeEchoed: quickTask !== undefined &&
      leadMessage !== undefined &&
      sessionContainsText(cwd, quickTask, leadMessage),
    memberToLeadInjected: existsSync(leadReceipt) &&
