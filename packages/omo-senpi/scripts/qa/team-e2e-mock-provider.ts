...
const { existsSync, readFileSync, readdirSync, mkdirSync, writeFileSync } =
...
export default function registerMockProvider(pi: ExtensionAPI): void {
  pi.registerProvider("omo-mock", {
    ...
    streamSimple: (streamModel, context, options) =>
      streamMockResponse(streamModel, context, options),
  });
}
...
function loadMockScript(cwd: string): MockScript {
  ...
  if (!existsSync(scriptPath)) {
    return { lead: [{ type: "text", text: "no script" }] };
  }
...
function detectRole(text: string): string {
  for (const [marker, role] of ROLE_MARKERS) {
    if (text.includes(marker)) return role;
  }
  return "lead";
}
...
function resolvePlaceholders(step: MockStep, text: string): MockStep {
  ...
  const teamRunId = firstMatch(text, /team_run_id"\s*:\s*"([^"]+)"/) ??
    firstMatch(text, /Created team '[^']*' \(([^)]+)\)/);
  const taskId = firstMatch(text, /Created task (\d+)/) ??
    firstMatch(text, /"id"\s*:\s*"(\d+)"/);
  ...
  return {
    ...step,
    arguments: JSON.parse(substituted) as Record<string, unknown>,
  };
}
...
function recordObservation(role: string, text: string): void {
  ...
  if (role === "quick" && text.includes("LEAD2QUICK")) {
    writeFileSync(
      join(obsDir, "quick-received.txt"),
      extractLine(text, "LEAD2QUICK"),
    );
  }
  if (role === "lead" && text.includes("QUICK2LEAD")) {
    const carriesCustomType = text.includes("senpi-task.team-message") ||
      text.includes("peer_message");
    writeFileSync(
      join(obsDir, "lead-received.txt"),
      `${carriesCustomType ? "custom-message" : "plain"}\n${
...
function seedDuraBacklog(cwd: string): void {
  ...
  for (const runId of readdirSync(runtimeRoot)) {
    ...
    const message = {
      ...
      to: "dura",
      kind: "message",
      body: "DURA-BACKLOG redeliver me",
      timestamp: Date.now(),
    };
    writeFileSync(
...
function streamMockResponse(
  ...
  const step = resolvePlaceholders(
...
function resolveEventWait(
  ...
  if (
    text.includes("senpi-task.team-member-liveness") ||
    text.includes("Team member liveness:")
  ) {
  ...
  if (boundary.type !== "tool_call") {
    throw new Error("liveness boundary did not resolve to a tool call");
  }
  return boundary;
}

function streamHangingResponse(
  callCount: number,
  options?: SimpleStreamOptions,
) {
  const stream = createLocalAssistantMessageEventStream();
...
function isTerminalAssistantMessageEvent(
  event: unknown,
): event is { type: "done"; message: AssistantMessage } | {
  type: "error";
...
function extractAssistantMessageResult(
  event: { type: "done"; message: AssistantMessage } | {
    type: "error";
    error: AssistantMessage;
  },
): AssistantMessage {
...
if (
  ...
  if (process.argv.includes("--self-test")) {
    if (detectRole("prompt with MOCKROLE=quick here") !== "quick") {
      throw new Error("quick role detection failed");
    }
    if (detectRole("plain lead prompt") !== "lead") {
      throw new Error("lead role detection failed");
    }
    ...
    if (
      resolved.type !== "tool_call" ||
      resolved.arguments.team_run_id !== "run-xyz"
    ) throw new Error("placeholder resolution failed");
    const withTask = resolvePlaceholders(
      {
        type: "tool_call",
        name: "task_update",
        arguments: { task_id: "__TASK_ID__" },
      },
      "Created task 7.",
    );
    if (withTask.type !== "tool_call" || withTask.arguments.task_id !== "7") {
      throw new Error("task id resolution failed");
    }
    ...
    if (
      waiting.type !== "tool_call" ||
      waiting.arguments.team_run_id !== "run-xyz"
    ) {
      throw new Error("liveness wait should create a real tool boundary");
    }
    const observed = resolveEventWait(
      { type: "wait_for_liveness" },
      "senpi-task.team-member-liveness",
    );
    if (observed.type !== "text") {
      throw new Error(
        "liveness wait should settle after the structured message",
      );
    }
