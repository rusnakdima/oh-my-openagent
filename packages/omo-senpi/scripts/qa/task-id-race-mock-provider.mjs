...
export function loadMockScript() {
  ...
  if (!isMockScript(parsed)) {
    throw new Error(`${scriptPath} must contain valid mock steps`);
  }
  return parsed;
}
...
export function waitForGoFile() {
  ...
  const timeoutMs = positiveTimeout(
    process.env.SENPI_QA_GATE_TIMEOUT_MS,
    120_000,
  );
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(
      () =>
        finish(
          new Error(
            `task-id race gate timed out after ${timeoutMs}ms waiting for ${goFile}`,
          ),
        ),
      timeoutMs,
    );
    try {
      watcher = watch(dirname(goFile), (_eventType, filename) => {
        if (
          filename === null || filename.toString() === goFile.split("/").at(-1)
        ) {
...
function positiveTimeout(value, fallback) {
  ...
  if (!Number.isFinite(parsed) || parsed <= 0) {
    throw new Error("SENPI_QA_GATE_TIMEOUT_MS must be a positive number");
  }
  return parsed;
}
...
function isMockScript(value) {
  return typeof value === "object" && value !== null &&
    Array.isArray(value.steps) && value.steps.every(isMockStep);
}
...
function stepToAssistantMessage(step, callNumber) {
  const content = step.type === "text"
    ...
    : [{
      type: "toolCall",
      id: step.id ?? `omo-race-tool-${callNumber}`,
      name: step.name,
