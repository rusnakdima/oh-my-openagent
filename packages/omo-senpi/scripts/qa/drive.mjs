...
export function credentialDigest(agentDir) {
  ...
  for (const name of CREDENTIAL_FILES) {
    ...
    hash.update("\0");
    hash.update(
      existsSync(path) ? credentialBytes(path, name) : Buffer.from("absent"),
    );
    hash.update("\0");
  }
...
export function seedSandbox(
  { cwd, agentDir, xdgConfigHome, homeDir, canonicalCwd },
) {
  mkdirp(cwd);
  mkdirp(agentDir);
...
function main() {
  ...
  try {
    ...
    ultraworkInjected = ultrawork.status === 0 &&
