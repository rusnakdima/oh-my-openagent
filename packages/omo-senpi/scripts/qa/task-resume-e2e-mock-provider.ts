...
export default function registerMockProvider(pi: ExtensionAPI): void {
  pi.registerProvider("omo-mock", {
    ...
    api: "openai-completions",
    models: [model],
    streamSimple: (_streamModel, context, options) =>
      streamMockResponse(context, options),
  });
}
...
function loadMockScript(cwd: string): MockScript {
  ...
  if (!existsSync(scriptPath)) {
    return { parentSteps: [{ type: "text", text: "no script" }] };
  }
...
function lastUserText(context: Context): string {
  const users = (context.messages ?? []).filter((message) =>
    message.role === "user"
  );
...
export function routeChildStep(context: Context): MockStep {
  ...
  if (lastUser.includes(CONTINUATION_MARKER)) {
    return { type: "text", text: MIDTURN_CONTINUED_TOKEN };
  }
  if (lastUser.includes(PING_TOKEN)) {
    return { type: "text", text: `${PONG_TOKEN} steerability proven` };
  }
  const thread = threadText(context);
  const hasAssistant = (context.messages ?? []).some((message) =>
  ...
  if (thread.includes("midturn-child")) {
    ...
    return hasAssistant ? { type: "hang" } : {
      type: "tool_call",
      name: "read",
      arguments: { path: "mock-script.json" },
    };
  }
  if (thread.includes("finished-child")) {
    return { type: "text", text: FINISHED_CHILD_TOKEN };
  }
  if (thread.includes("cancel-child")) {
    return { type: "text", text: CANCEL_CHILD_TOKEN };
  }
  if (thread.includes("lru-child")) {
    return { type: "text", text: LRU_CHILD_TOKEN };
  }
  if (thread.includes("ttl-child")) {
    return { type: "text", text: TTL_CHILD_TOKEN };
  }
...
function streamHangingResponse(options?: SimpleStreamOptions) {
  const stream = createStream();
  const aborted = stepToAssistantMessage(
    { type: "text", text: "hang aborted" },
    0,
  );
