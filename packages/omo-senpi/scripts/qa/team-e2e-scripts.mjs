#!/usr/bin/env node
const QUICK_PROMPT =
  "You are team member 'quick'. MOCKROLE=quick. Report to the lead when injected work arrives, then end your turn.";
const FIXTURE_PROMPT =
  "You are team member 'fixture'. MOCKROLE=fixture. Acknowledge, then finish.";
const DURA_PROMPT =
  "You are team member 'dura'. MOCKROLE=dura. Seed your backlog, then end your turn.";
const RCL_PROMPT =
  "You are team member 'rcl'. MOCKROLE=quick. Acknowledge, then finish.";
...
export const LEAD_SCRIPT = {
  lead: [
    ...
    toolCall("task_send", {
      ...
      to: "quick",
      message:
        "LEAD2QUICK injected handshake: report QUICK2LEAD after this arrives",
    }),
  ...
  quick: [
    ...
    toolCall("task_send", {
      to: "lead",
      message: "QUICK2LEAD member report delivered by injection",
    }),
    { type: "hang" },
  ],
...
export const DURA_REVIVE_SCRIPT = {
  lead: [
    ...
    toolCall("task_send", {
      team_run_id: "__TEAM_RUN_ID__",
      to: "dura",
      message: "DURA-DRAIN durability payload one",
    }),
    toolCall("task_send", {
      team_run_id: "__TEAM_RUN_ID__",
      to: "dura",
      message: "DURA-DRAIN durability payload two",
    }),
    { type: "hang" },
  ],
...
export function crashReplacementScript(unreadPath, processedPath) {
  return {
    quick: [
      ...
      toolCall("bash", {
        command: waitForPathCommand(processedPath, "present"),
      }),
...
function waitForPathCommand(path, mode) {
  ...
  const script =
    `const fs=require('fs');const p=process.argv[1];const until=Date.now()+25000;(function poll(){if(${hit})process.exit(0);if(Date.now()>until){console.error('timed out waiting for '+p);process.exit(1)}setTimeout(poll,100)})()`;
    ...
    toolCall("team_create", {
      inline_spec: {
        ...
        members: [{
          ...
          category: "quick",
          prompt:
            "You are team member 'crash'. MOCKROLE=quick. End your turn, then accept one crash-window injection.",
        }],
      },
    }),
    toolCall("task_send", {
      team_run_id: "__TEAM_RUN_ID__",
      to: "crash",
      message: "CRASH-ONCE inject exactly once",
    }),
    toolCall("task_list", { team_run_id: "__TEAM_RUN_ID__" }),
  ],
