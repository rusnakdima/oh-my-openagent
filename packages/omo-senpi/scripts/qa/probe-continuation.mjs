...
function main() {
  ...
  try {
    ...
    writeFileSync(
      ...
      JSON.stringify({ steps: [{ type: "text", text: "first turn" }] }),
    );
    ...
    if (
      transcript.includes("Continue the active omo-agent-toolkit ulw-loop run")
    ) {
      result = "PASS";
    ...
    for (let attempt = 0; attempt < 20; attempt += 1) {
      const capture = spawnSync(tmuxBin, [
        "capture-pane",
        "-pt",
        tmuxSession,
        "-S",
    ...
    continuationQaPath = "tmux";
    result =
      captureText.includes("Continue the active omo-agent-toolkit ulw-loop run")
        ? "PASS"
        : "FAIL";
    return print({ result, continuationQaPath, beforeDigest });
  } finally {
    if (tmuxBin !== null && tmuxSession !== null) {
      spawnSync(tmuxBin, ["kill-session", "-t", tmuxSession], {
        timeout: 10_000,
      });
    }
    rmSync(sandbox.root, { recursive: true, force: true });
