...
async function main() {
  ...
  const checks = {
    ...
    evalToolSucceeded: evalResults.length === 1 &&
      evalResults[0]?.isError === false,
...
function startPosthogSink() {
  ...
  return {
    ...
    ready: new Promise((resolveReady) => {
      server.on(
        "listening",
        () => resolveReady(`http://127.0.0.1:${server.address().port}`),
      );
    }),
    waitForSummary(timeoutMs) {
      if (requests.some((request) => containsSummary(request.body))) {
      ...
      return new Promise((resolveSummary, rejectSummary) => {
        const timer = setTimeout(
          () =>
            rejectSummary(
              new Error("local sink did not receive parallelism_summary"),
            ),
          timeoutMs,
        );
        summaryWaiters.push(() => {
...
function writeEvidence(evidenceDir, run, requests, checks, result, cleanup) {
  ...
  writeFileSync(
    join(evidenceDir, "qa5-sink-events.json"),
    `${JSON.stringify(sanitizeRequests(requests), null, 2)}\n`,
  );
  writeFileSync(
    join(evidenceDir, "qa5-cleanup.json"),
    `${JSON.stringify(cleanup, null, 2)}\n`,
  );
  writeFileSync(
    join(evidenceDir, "qa5-verdict.json"),
...
function sanitizeRequests(requests) {
  return requests.map((request) => ({
    ...
    events: (request.body?.batch ?? []).map((capture) => ({
      ...
      ...(capture?.event === "parallelism_summary"
...
function containsSummary(body) {
  return Array.isArray(body?.batch) &&
    body.batch.some((capture) => capture?.event === "parallelism_summary");
}
...
async function selfTest() {
  ...
  const body = JSON.stringify({
    batch: [{ event: "parallelism_summary", properties: {} }],
  });
  ...
  if (
    sink.requests.length !== 1 || sink.requests[0]?.body?.batch?.length !== 1
  ) {
    throw new Error("self-test local gzip sink did not record one event");
  }
  if (
    !EVAL_SCRIPT.lead[0]?.arguments?.code.includes("tool.read") ||
    !EVAL_SCRIPT.lead[0]?.arguments?.code.includes("tool.bash")
  ) {
