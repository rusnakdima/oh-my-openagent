...
function readTaskArtifacts(stateDir) {
  ...
  const eventsText = existsSync(eventsPath)
...
function assertScenario(run, artifacts, sessionTranscript) {
  ...
  const checks = {
    ...
    no_premature_capacity_line: !parentSurface.includes(
      "mock provider capacity exhausted",
    ),
    ...
    requested_model_preserved:
      artifacts.record.requested_model?.display === REQUESTED_MODEL,
    actual_model_recorded:
      artifacts.record.resolved_model?.display === FALLBACK_MODEL,
    configured_chain_consumed:
      Array.isArray(artifacts.record.fallback_models) &&
      artifacts.record.fallback_models.length === 0,
    fallback_event_once:
      count(artifacts.eventsText, '"type":"task_model_fallback"') === 1,
  };
  return {
...
function writeEvidence(evidenceDir, result, artifacts, isolation, cleanup) {
  ...
  writeFileSync(
    join(evidenceDir, "parent-session.jsonl"),
...
function main() {
  ...
  try {
    ...
    const childPid = typeof artifacts.record.pid === "number"
      ? artifacts.record.pid
      : undefined;
  ...
  const cleanup =
    `child_alive_before_cleanup=${report.childAlive}; sandbox_removed=${sandboxRemoved}`;
