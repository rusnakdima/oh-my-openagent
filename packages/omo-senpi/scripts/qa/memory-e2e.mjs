...
const ISOLATION_EXCLUDE = new Set([
  "sessions",
  "senpi-debug.log",
  "mcp-cache.json",
  "mcp-auth",
...
function runSenpi(sandbox, { prompt, env = {}, args = [], cwd }) {
  const fullArgs = [
    ...
    "mock-1",
    "--session-dir",
    join(sandbox.agentDir, "sessions"),
    ...args,
    prompt,
  ];
...
function waitFor(
  predicate,
  { timeoutMs = 90_000, intervalMs = 500, description } = {},
) {
...
function memoryRepos(memoryHome) {
  ...
  return readdirSync(agentsDir).map((name) => join(agentsDir, name, "repo"))
    .filter(existsSync);
}

function gitLog(repo) {
  const run = spawnSync("git", ["log", "--format=%s | %ae", "HEAD"], {
    cwd: repo,
    encoding: "utf8",
  });
...
async function scenario1() {
    ...
    parentSteps: [
      {
        ...
        arguments: {
          ...
          reason: "seed harness fact for QA",
        },
      },
  ...
  const first = runSenpi(sandbox, {
    prompt: "remember that senpi is a pi harness",
    env: { MOCK_DUMP_SYSTEM: dumpLog },
  });
  if (first.status !== 0) {
    return record("S1 first senpi run", false, first.stderr.slice(-400));
  }
  ...
  if (repos.length === 0) {
    return record(
      "S1 repo created",
      false,
      "no identity repo under OMO_MEMORY_HOME",
    );
  }
  const log = gitLog(repos[0]);
  record(
    "S1 commit landed",
    log.includes("seed harness fact for QA"),
    log || "<empty git log>",
  );
  record(
    "S1 author is identity",
    log.includes("@omo.local"),
    log.split("\n")[0] ?? "",
  );
  ...
  const second = runSenpi(sandbox, {
    prompt: "what do you remember",
    env: { MOCK_DUMP_SYSTEM: dumpLog },
  });
  if (second.status !== 0) {
    return record("S1 second senpi run", false, second.stderr.slice(-400));
  }
  const dump = readFileSync(dumpLog, "utf8");
  record(
    "S1 sentinel in next-run prompt",
    dump.includes("<!-- senpi-memory:"),
    dumpLog,
  );
  record(
    "S1 memory body in next-run prompt",
    dump.includes("senpi is a pi harness"),
    dumpLog,
  );
  return { sandbox };
}
...
async function scenario2() {
  ...
  writeFileSync(
    shimPath,
    `#!/bin/sh\nexec ${JSON.stringify(senpiBin)} -e ${
      JSON.stringify(mockProviderEntry)
    } "$@"\n`,
  );
    ...
    parentSteps: [
      {
        ...
        arguments: {
          ...
          file_path: "system/facts.md",
          description: "seed",
          file_text: "seed for dreaming",
          reason: "seed memory before reflection",
        },
      },
  ...
  if (first.status !== 0) {
    return record("S2 seeding run", false, first.stderr.slice(-400));
  }
    ...
    parentSteps: [
      {
        ...
        arguments: {
          ...
          edits: [{
            oldText: "seed for dreaming",
            newText: "seed for dreaming\ndreamed fact",
          }],
        },
      },
      {
        ...
        arguments: {
          command:
            "git add system/facts.md && git commit -m 'feat(reflection): dreamed a memory'",
        },
      },
  ...
  writeFileSync(
    join(repo, "mock-script.json"),
    `${JSON.stringify(childScript, null, 2)}\n`,
  );
  const scriptCommit = spawnSync("git", ["add", "mock-script.json"], {
    cwd: repo,
  });
  if (scriptCommit.status !== 0) {
    return record(
      "S2 child script staging",
      false,
      scriptCommit.stderr?.toString() ?? "",
    );
  }
  const scriptCommitResult = spawnSync("git", [
    "commit",
    "-m",
    "qa: child script",
  ], { cwd: repo });
  if (scriptCommitResult.status !== 0) {
    return record(
      "S2 child script commit",
      false,
      scriptCommitResult.stderr?.toString() ?? "",
    );
  }

  writeMockScript(sandbox, {
    parentSteps: [{ type: "text", text: "triggering turn" }],
    childSteps: [],
  });
  const run = runSenpi(sandbox, {
    prompt: "trigger one reflection",
    env: { SENPI_BIN: shimPath },
  });
  if (run.status !== 0) {
  ...
  const completion = await waitFor(
    () => {
      ...
      for (const agent of readdirSync(agentsDir)) {
        ...
        const files = readdirSync(completionsDir).filter((name) =>
          name.endsWith(".json")
        );
        if (files.length === 0) continue;
        const record_ = JSON.parse(
          readFileSync(join(completionsDir, files[0]), "utf8"),
        );
  ...
  if (completion instanceof Error) {
    console.error(
      "S2 completion wait failed. reflection dirs:",
      existsSync(join(memoryHome, "agents"))
        ? readdirSync(join(memoryHome, "agents")).map((a) =>
          readdirSync(join(memoryHome, "agents", a, "runtime"), {
            withFileTypes: true,
          }).map((d) => d.name).join(",")
        )
        : "none",
  ...
  record(
    "S2 outcome merged",
    completion.record.outcome === "merged",
    JSON.stringify(completion.record),
  );
  record(
    "S2 quick category recorded",
    completion.record.category === "quick",
    completion.record.category,
  );
  const log = gitLog(repo);
  record(
    "S2 merge commit landed",
    /merge\(reflection\)/.test(log),
    log.split("\n").slice(0, 3).join(" || "),
  );
  record(
    "S2 dreamed fact merged into parent repo",
    existsSync(join(repo, "system", "facts.md")) &&
      readFileSync(join(repo, "system", "facts.md"), "utf8").includes(
        "dreamed fact",
      ),
    "system/facts.md",
  );
...
async function scenario3And4() {
    ...
    parentSteps: [
      {
        ...
        arguments: {
          ...
          file_path: "system/facts.md",
          description: "qa fact",
          file_text: "searchable unique phrase zqxwvb",
          reason: "seed searchable memory",
        },
      },
  ...
  if (run.status !== 0) {
    return record("S3 seeding senpi run", false, run.stderr.slice(-400));
  }

  const bundleModule = await import("../../src/extension/index.ts");
  const { MemoryFakeExtensionAPI } = await import(
    "../../src/components/memory/memory.test-support.ts"
  );
  ...
  try {
    ...
    await composed(pi, {});
    const searchCommand = pi.commands.find((command) =>
      command.name === "search"
    );
    if (searchCommand === undefined) {
      return record(
        "S3 search command registered",
        false,
        `commands: ${pi.commands.map((c) => c.name).join(",")}`,
      );
    }
    const output = await searchCommand.options.handler(
      "zqxwvb",
      buildCommandContext(sandbox),
    );
    record(
      "S3 search finds prior message",
      typeof output === "string" && output.includes("zqxwvb"),
      String(output).slice(0, 200),
    );

    const palaceCommand = pi.commands.find((command) =>
      command.name === "palace"
    );
    if (palaceCommand === undefined) {
      return record("S4 palace command registered", false, "not registered");
    }
    await pi.dispatch(
      "session_start",
      {},
      buildCommandContext(sandbox, {
        sessionManager: {
          getSessionId: () => "palace-session",
          getEntries: () => [],
        },
      }),
    );
    let palacePath = "";
    const palaceOutput = await palaceCommand.options.handler(
      "",
      buildCommandContext(sandbox, {
        hasUI: false,
        output: (text) => {
    ...
    const htmlPathMatch = String(palacePath || palaceOutput).match(
      /(\/\S+\.html)/,
    );
    if (htmlPathMatch === null) {
      return record(
        "S4 palace path returned",
        false,
        String(palaceOutput).slice(0, 200),
      );
    }
    const htmlPath = htmlPathMatch[1];
    if (!existsSync(htmlPath)) {
      return record("S4 palace file exists", false, htmlPath);
    }
    const mode = statSync(htmlPath).mode & 0o777;
    const html = readFileSync(htmlPath, "utf8");
    record(
      "S4 palace file mode 0600",
      mode === 0o600,
    ...
    record(
      "S4 palace tabs",
      ["core", "external", "history", "reflection"].every((tab) =>
        tabValues.has(tab)
      ),
      [...tabValues].join(","),
    );
    record(
      "S4 palace persona entry",
      html.includes("system/persona.md"),
      htmlPath,
    );
  } finally {
...
async function scenario5() {
  ...
  writeMockScript(sandbox, {
    parentSteps: [{ type: "text", text: "plain answer" }],
    childSteps: [],
  });
  ...
  if (run.status !== 0) {
    return record("S5 disabled run", false, run.stderr.slice(-400));
  }
  const dump = existsSync(dumpLog) ? readFileSync(dumpLog, "utf8") : "";
  record(
    "S5 no sentinel when disabled",
    !dump.includes("<!-- senpi-memory:"),
    dumpLog,
  );
  record(
    "S5 no memory dirs when disabled",
    memoryRepos(join(sandbox.root, "memory")).length === 0,
    join(sandbox.root, "memory"),
  );
}
...
async function main() {
  ...
  const realSenpiAfter = hashDir(
    join(homedir(), ".senpi", "agent"),
    isolationExclude,
  );
  const realOmoMemoryAfter = hashDir(join(homedir(), ".omo", "memory"));
  record(
    "isolation: real ~/.senpi/agent untouched (volatile paths excluded)",
    realSenpiBefore === realSenpiAfter,
    `${realSenpiBefore.slice(0, 12)} vs ${realSenpiAfter.slice(0, 12)}`,
  );
  record(
    "isolation: real ~/.omo/memory untouched",
    realOmoMemoryBefore === realOmoMemoryAfter,
    `${realOmoMemoryBefore.slice(0, 12)} vs ${realOmoMemoryAfter.slice(0, 12)}`,
  );
