...
function seedScenario(script, { withMarker } = {}) {
  ...
  writeFileSync(
    ...
    `${JSON.stringify(OMO_CONFIG, null, 2)}\n`,
  );
  writeFileSync(
  ...
  if (withMarker === true) {
    ...
    writeFileSync(
      join(extDir, "marker.js"),
      `import { appendFileSync } from "node:fs"\nexport default function () { appendFileSync(${
        JSON.stringify(markerLog)
      }, "x\\n") }\n`,
    );
  }
  return {
...
function runMainFlow(senpiBin, checks, capture, pids) {
  ...
  const { run, events } = driveSenpi(
    senpiBin,
    scenario,
    "spawn a background child, keep working, then follow up and read its output",
    pids,
  );
  capture.main = {
    exit: run.status,
  ...
  const jsonl = taskId === undefined
    ? ""
    : readStoreJsonl(scenario.stateDir, taskId);
  ...
  checks.spawn_background = run.status === 0 && typeof taskId === "string" &&
      existsSync(join(scenario.stateDir, "tasks", `${taskId}.json`))
    ? "PASS"
    : "FAIL";
  checks.unconditional_wake = wake.ok ? "PASS" : "FAIL";
  checks.followup_revive =
    findRevived(events) && JSON.stringify(events).includes(CHILD_SECOND)
      ? "PASS"
      : "FAIL";
  checks.task_output_peek =
    findTranscript(events, CHILD_FIRST) && findPeekTaskOutput(events)
      ? "PASS"
      : "FAIL";
  checks.jsonl_sequence =
    matchesOrderedSubsequence(signatures, MAIN_FLOW_EXPECTED_SEQUENCE)
      ? "PASS"
      : "FAIL";
  checks.extension_suppression = markerCount(scenario.markerLog) === 1
    ? "PASS"
    : "FAIL";
...
function runBatchFlow(senpiBin, checks, capture, pids) {
  ...
  const { run, events } = driveSenpi(
    senpiBin,
    scenario,
    "fan out two synchronous child tasks",
    pids,
  );
  const taskIds = readStoreTaskIds(scenario.stateDir);
  const items = findBatchFanout(events, 2);
  capture.batchStdout = run.stdout ?? "";
  checks.batch_fanout_two_children = run.status === 0 && taskIds.length >= 2 &&
      batchChildrenCompleted(events, items)
    ? "PASS"
    : "FAIL";
  return scenario.sandbox;
}
...
function runSyncFlow(senpiBin, checks, capture, pids) {
  ...
  const { run, events } = driveSenpi(
    senpiBin,
    scenario,
    "run a synchronous task and return its answer",
    pids,
  );
  ...
  checks.sync_inline_no_notification =
    run.status === 0 && inline && noNotification ? "PASS" : "FAIL";
  return scenario.sandbox;
}
...
function runNegativeFlow(senpiBin, checks, capture, pids) {
  ...
  const { run, events } = driveSenpi(
    senpiBin,
    scenario,
    "route a task to a category that does not exist",
    pids,
  );
  capture.negativeStdout = run.stdout ?? "";
  checks.negative_category_error = findCategoryListingError(events)
    ? "PASS"
    : "FAIL";
  return scenario.sandbox;
}
...
async function main() {
  ...
  try {
    for (
      const runner of [runMainFlow, runBatchFlow, runSyncFlow, runNegativeFlow]
    ) {
  ...
  const allChangedRealPaths = changedRealPaths(
    beforeSnapshot,
  ...
  const { qaAttributedPaths, concurrentSessionPaths } =
    classifyRealSenpiChanges(allChangedRealPaths, sandboxTokens);
  ...
  const result =
    values.length > 0 && values.every((verdict) => verdict === "PASS")
      ? "PASS"
      : "FAIL";
  const payload = {
    result,
...
function writeEvidenceMaybe(outDir, capture, payload) {
  ...
  writeFileSync(
    join(outDir, "batch.stdout.json.log"),
    capture.batchStdout ?? "",
  );
  writeFileSync(join(outDir, "sync.stdout.json.log"), capture.syncStdout ?? "");
  writeFileSync(
    join(outDir, "negative.stdout.json.log"),
    capture.negativeStdout ?? "",
  );
}

function findPeekTaskOutput(events) {
  const output = JSON.stringify(events);
  return output.includes('"name":"task_output"') &&
    !output.includes('"block"') && !output.includes('"timeout_ms"');
}

function batchChildrenCompleted(events, items) {
  if (
    items.length !== 2 || !items.every((item) => item?.status === "completed")
  ) return false;
  const output = JSON.stringify(events);
  return output.includes(`${BATCH_FINAL} one`) &&
    output.includes(`${BATCH_FINAL} two`);
}
...
function runSelfTest() {
  const wakeEvents = parseJsonEvents(`banner\n${
    JSON.stringify({
      type: "custom",
      content:
        'task completion name:e2echild id:st_abc status:completed duration:3ms\nresult:"done"\nnext:Use task_send({ to: "st_abc", message: "..." }) to continue.',
    })
  }`);
  if (!findWakeNotification(wakeEvents, "st_abc").ok) {
    throw new Error("self-test: wake notification must be detected");
  }
  if (findWakeNotification(wakeEvents, "st_missing").ok) {
    throw new Error("self-test: wake must not match a foreign task id");
  }
  if (
    !findRevived(
      parseJsonEvents(
        JSON.stringify({
          type: "toolResult",
          details: { kind: "revived", task_id: "st_abc", run_epoch: 1 },
        }),
      ),
    )
  ) throw new Error("self-test: revived must be detected");
  if (
    !findTranscript(
      parseJsonEvents(
        JSON.stringify({
          type: "toolResult",
          content: `st_abc [completed] transcript via jsonl:\n${CHILD_FIRST}`,
        }),
      ),
      CHILD_FIRST,
    )
  ) throw new Error("self-test: transcript must be detected");
  if (
    !findPeekTaskOutput(
      parseJsonEvents(
        JSON.stringify({ name: "task_output", arguments: { mode: "tail" } }),
      ),
    )
  ) throw new Error("self-test: non-blocking output peek must be detected");
  if (
    findPeekTaskOutput(
      parseJsonEvents(
        JSON.stringify({ name: "task_output", arguments: { block: true } }),
      ),
    )
  ) {
    throw new Error(
      "self-test: legacy blocking output call must not count as a peek",
    );
  }
  const taskSendIndex = MAIN_SCRIPT.parentSteps.findIndex((step) =>
    step.type === "tool_call" && step.name === "task_send"
  );
  const taskOutputIndex = MAIN_SCRIPT.parentSteps.findIndex((step) =>
    step.type === "tool_call" && step.name === "task_output"
  );
  if (taskSendIndex < 0 || taskOutputIndex !== taskSendIndex + 1) {
    throw new Error(
      "self-test: task_output must be the next tool boundary after task_send",
    );
  }
  if (
    !findInlineFinal(
      parseJsonEvents(JSON.stringify({ type: "text", text: SYNC_FINAL })),
      SYNC_FINAL,
    )
  ) throw new Error("self-test: inline final must be detected");
  if (
    !findCategoryListingError(
      parseJsonEvents(
        JSON.stringify({
          type: "toolResult",
          content: "Unknown category. Available categories: quick, deep.",
        }),
      ),
    )
  ) throw new Error("self-test: category listing error must be detected");
  const batchItems = findBatchFanout(
    parseJsonEvents(JSON.stringify({
      type: "tool_execution_end",
      toolName: "task",
      result: {
        details: { items: [{ task_id: "st_1" }, { task_id: "st_2" }] },
      },
    })),
    2,
  );
  if (batchItems.length !== 2) {
    throw new Error("self-test: two-child batch fanout must be detected");
  }
  ...
  if (!batchChildrenCompleted(completedBatchEvents, completedItems)) {
    throw new Error(
      "self-test: completed batch children with both outputs must pass",
    );
  }
  ...
  if (
    batchChildrenCompleted(
      pendingBatchEvents,
      findBatchFanout(pendingBatchEvents, 2),
    )
  ) {
  ...
  if (
    batchChildrenCompleted(
      incompleteOutputBatchEvents,
      findBatchFanout(incompleteOutputBatchEvents, 2),
    )
  ) {
  ...
  if (!matchesOrderedSubsequence(signatures, MAIN_FLOW_EXPECTED_SEQUENCE)) {
    throw new Error("self-test: expected transition sequence must match");
  }
  if (
    matchesOrderedSubsequence(["running/resident"], MAIN_FLOW_EXPECTED_SEQUENCE)
  ) throw new Error("self-test: a partial sequence must not match");
  const logOnlyDelta = changedRealPaths(
    new Map([[SHARED_SENPI_LOG, "a"], ["settings.json", "x"]]),
    new Map([[SHARED_SENPI_LOG, "b"], ["settings.json", "x"]]),
  );
  if (logOnlyDelta.length !== 0) {
    throw new Error(
      "self-test: a shared-log-only delta must not count as pollution",
    );
  }
  const configDelta = changedRealPaths(
    new Map([["settings.json", "x"]]),
    new Map([["settings.json", "y"]]),
  );
  if (configDelta.length !== 1 || configDelta[0] !== "settings.json") {
    throw new Error("self-test: a real config change must be reported");
  }
  const removalDelta = changedRealPaths(
    new Map([["auth.json", "x"]]),
    new Map(),
  );
  if (removalDelta.length !== 1 || removalDelta[0] !== "auth.json") {
    throw new Error("self-test: a real config removal must be reported");
  }
