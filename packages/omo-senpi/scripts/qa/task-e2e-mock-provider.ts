...
const { existsSync, readFileSync, appendFileSync } = process.getBuiltinModule<
  FsModule
>("fs");
const env =
...
export function loadMockScript(cwd: string): MockScript {
  ...
  if (
    typeof override === "string" && override.length > 0 && existsSync(override)
  ) {
  ...
  if (!existsSync(scriptPath)) {
    return {
      parentSteps: [{ type: "text", text: "no script" }],
...
export function messagesContainChild(context: Context): boolean {
  for (const message of context.messages ?? []) {
    ...
    for (const part of message.content) {
      if (typeof part.text === "string" && part.text.includes(CHILD_IDENTITY)) {
        return true;
      }
    }
  }
...
function streamMockResponse(
  ...
  if (typeof dumpTarget === "string" && dumpTarget.length > 0) {
    ...
    const rendered = typeof systemPrompt === "string"
      ? systemPrompt
      : JSON.stringify(systemPrompt ?? null);
    appendFileSync(
      dumpTarget,
      `\n=== model=${streamModel.id} cwd=${
...
if (
  ...
  if (process.argv.includes("--self-test")) {
    ...
    const parent = stepToAssistantMessage({
      type: "tool_call",
      name: "task",
      arguments: {},
    }, 1);
    // then it stops with toolUse
    if (parent.stopReason !== "toolUse") {
    ...
    const childCtx: Context = {
      messages: [{
        role: "user",
        content: `You are ${CHILD_IDENTITY}. Task: x`,
      }],
    };
    if (!messagesContainChild(childCtx)) {
      throw new Error("child identity detection failed");
    }
    // given a parent context / when detected / then false
    const parentCtx: Context = {
      messages: [{ role: "user", content: "ulw parent prompt" }],
    };
    if (messagesContainChild(parentCtx)) {
      throw new Error("parent must not detect child identity");
    }
