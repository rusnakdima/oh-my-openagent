...
if (
  Number.isNaN(expectedCode) || sepIdx < 0 || sepIdx + 1 >= process.argv.length
) {
  console.error(
    "Usage: node expect-exit.mjs <expected-exit-code> -- <command...>",
  );
...
child.on("exit", (code) => {
  ...
  if (actual === expectedCode) {
    console.log(
      `expect-exit: exit code ${actual} matches expected ${expectedCode}`,
    );
    process.exit(0);
  } else {
    console.error(
      `expect-exit: exit code ${actual} does NOT match expected ${expectedCode}`,
    );
    process.exit(1);
  }
});
