...
import {
  findStaleRuntimePersona,
  stageRuntimePersonas,
} from "./persona-artifacts.mjs";
...
const supervisorEntryPath = join(
  packageRoot,
  "src",
  "components",
  "memory",
  "worker",
  "memory-run-supervisor.ts",
);
const supervisorOutputPath = join(
  pluginRoot,
  "extensions",
  "memory-run-supervisor.mjs",
);
const advisorRuntimeEntryPath = join(
  packageRoot,
  "src",
  "components",
  "init-deep-advisor",
...
const advisorRuntimeOutputPath = join(
  pluginRoot,
  "extensions",
...
export async function buildExtension(options = {}) {
  ...
  const taskOutput = options.taskOutputPath ??
    (options.outputPath === undefined
      ? taskOutputPath
      : join(dirname(output), "omo-task.js"));
  const memberOutput = options.memberOutputPath ??
    (options.outputPath === undefined
      ? memberOutputPath
      : join(dirname(output), "omo-member.js"));
  const memoryMcpOutput = options.memoryMcpOutputPath ??
    (options.outputPath === undefined
      ? memoryMcpOutputPath
      : join(dirname(output), "omo-memory-mcp.js"));
  const supervisorOutput = options.supervisorOutputPath ??
    (options.outputPath === undefined
      ? supervisorOutputPath
      : join(dirname(output), "memory-run-supervisor.mjs"));
  const advisorRuntimeOutput = options.advisorRuntimeOutputPath ??
    (options.outputPath === undefined
  ...
  const memberInputs = await buildEntry(
    memberEntryPath,
    memberOutput,
    buildDefines,
  );
  const memoryMcpInputs = await buildEntry(
    memoryMcpEntryPath,
    memoryMcpOutput,
    buildDefines,
  );
  const supervisorInputs = await buildEntry(
    supervisorEntryPath,
    supervisorOutput,
    buildDefines,
  );
  const advisorRuntimeInputs = await buildEntry(
    advisorRuntimeEntryPath,
...
async function buildEntry(entry, output, buildDefines) {
  ...
  try {
    run(resolveBunExecutable(), [
      ...
      "--minify-whitespace",
      `--metafile=${metafile}`,
      ...Object.entries(buildDefines).flatMap((
        [name, value],
...
export async function checkExtensionCurrent(options = {}) {
  ...
  if (currentTask === undefined) {
    return { ok: false, reason: "missing-output", output: taskOutput };
  }
  const currentMember = await readBuiltEntry(memberOutput);
  if (currentMember === undefined) {
    return { ok: false, reason: "missing-output", output: memberOutput };
  }
  const currentMemoryMcp = await readBuiltEntry(memoryMcpOutput);
  if (currentMemoryMcp === undefined) {
    return { ok: false, reason: "missing-output", output: memoryMcpOutput };
  }
  const currentSupervisor = await readBuiltEntry(supervisorOutput);
  if (currentSupervisor === undefined) {
    return { ok: false, reason: "missing-output", output: supervisorOutput };
  }
  ...
  const expectedAdvisorRuntimeOutput = join(
    tempRoot,
    "omo-init-deep-advisor.js",
  );
  try {
    ...
    if (
      !artifactsMatch(currentTask, await readFile(expectedTaskOutput, "utf8"))
    ) {
    ...
    if (
      !artifactsMatch(
        currentMember,
        await readFile(expectedMemberOutput, "utf8"),
      )
    ) {
    ...
    if (
      !artifactsMatch(
        currentMemoryMcp,
        await readFile(expectedMemoryMcpOutput, "utf8"),
      )
    ) {
    ...
    if (
      !artifactsMatch(
        currentSupervisor,
        await readFile(expectedSupervisorOutput, "utf8"),
      )
    ) {
    ...
    if (
      !artifactsMatch(
        currentAdvisorRuntime,
        await readFile(expectedAdvisorRuntimeOutput, "utf8"),
      )
    ) {
      return {
        ok: false,
    ...
    const stalePersona = await findStaleRuntimePersona(
      tempRoot,
      dirname(output),
      repoRoot,
    );
    if (stalePersona !== undefined) {
      return { ok: false, reason: "stale-output", output: stalePersona };
    }
...
async function digestBuildSources(metadata, entry, buildDefines) {
  const inputs = metadata !== null && typeof metadata === "object" &&
      metadata.inputs !== null &&
      typeof metadata.inputs === "object"
    ? Object.keys(metadata.inputs).sort()
    : [];
  ...
  for (const input of inputs) {
    ...
    hash.update(toPortableBuildPath(relative(repoRoot, inputPath))).update(
...
if (
  ...
  if (process.argv.includes("--check")) {
    ...
    if (!result.ok) {
      console.error(
        `omo-senpi extension build is not current: ${result.reason}`,
      );
