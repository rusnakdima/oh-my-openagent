...
const cli = process.env.OMO_SENPI_CLI_PATH ?? ${
    JSON.stringify(options.senpiCliPath)
  }
const plugin = process.env.OMO_PLUGIN_ROOT ?? ${
    JSON.stringify(options.pluginPath)
  }
...
function installLocalLauncher(options) {
  ...
  if ((options.platform ?? process.platform) === "win32") {
    writeFileSync(
      localLauncherCmdPath(options.homeDir),
      `@echo off\r
node "%~dp0omo" %*\r
`,
    );
...
function removeLegacyBuiltinShadows(packages, repoRoot, agentDir) {
  const shadowPaths = new Set(
    LEGACY_BUILTIN_SHADOW_PACKAGES.map((path) => resolve3(repoRoot, path)),
  );
  return packages.filter((entry) =>
    !shadowPaths.has(resolve3(agentDir, entry))
  );
}
async function removeSupersededOmoPackages(
  packages,
  currentPluginPath,
  agentDir,
) {
...
async function ensurePluginArtifacts(context) {
  if (context.allowBuild) {
    await context.runCommand("node", [
      join4(context.pluginPath, "scripts", "build-extension.mjs"),
    ...
    await context.runCommand("node", [
      join4(context.pluginPath, "scripts", "sync-skills.mjs"),
    ], { cwd: context.repoRoot });
    await context.runCommand("node", [
      join4(context.pluginPath, "scripts", "build-install.mjs"),
    ], { cwd: context.repoRoot });
    await context.runCommand("node", [
      join4(context.pluginPath, "scripts", "stage-lsp-daemon-runtime.mjs"),
    ], { cwd: context.repoRoot });
    await context.runCommand("node", [
      join4(context.pluginPath, "scripts", "stage-ast-grep-mcp-runtime.mjs"),
    ], { cwd: context.repoRoot });
    await context.runCommand("node", [
      join4(context.pluginPath, "scripts", "stage-agent-toolkit.mjs"),
    ], { cwd: context.repoRoot });
  }
...
async function verifyAstGrepRuntimeIntegrity(pluginPath, platform) {
  ...
  let actualSha256;
  try {
    actualSha256 = createHash("sha256").update(await readFile2(runtimeEntry))
      .digest("hex");
  } catch (error) {
    throw astGrepIntegrityError(
      runtimeEntry,
...
function isAstGrepRuntimeManifest(value) {
  ...
  return typeof value.sha256 === "string" &&
    /^[a-f0-9]{64}$/.test(value.sha256) && typeof value.mode === "number" &&
    Number.isInteger(value.mode) && typeof value.stagedAtUtc === "string" &&
...
function resolveInstallContext(options) {
  ...
  const repoRoot = resolve4(
    options.repoRoot ??
      (allowBuild
        ? findRepoRoot(dirname4(fileURLToPath(import.meta.url)))
        : dirname4(resolve4(options.pluginPath))),
  );
  const agentDir = resolve4(
    options.agentDir ??
      resolveAgentHome({ env, homeDir: env.HOME ?? homedir3() }),
  );
  const pluginPath = resolve4(
    options.pluginPath ?? join5(repoRoot, "packages", "omo-senpi", "plugin"),
  );
  return {
    env,
    repoRoot,
...
function findRepoRoot(importerDir) {
  ...
  for (let depth = 0; depth <= 7; depth += 1) {
    if (
      fileExistsSync(
        join5(current, "packages", "omo-senpi", "plugin", "package.json"),
      )
    ) {
      return current;
    }
