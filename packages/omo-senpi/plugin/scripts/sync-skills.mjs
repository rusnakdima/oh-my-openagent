...
const { sources: nativeSkillSources, names: nativeSkillNames } =
  createNativeSkillSources(repoRoot);
...
function stripNamedSections(content) {
  ...
  for (const line of lines) {
    ...
    if (
      strippingLevel !== undefined && currentLevel !== undefined &&
      currentLevel <= strippingLevel
    ) {
...
function applyTier1Adaptation(content) {
  return normalizeBlankLines(
    stripForbiddenGuidanceLines(
      stripNamedSections(rewriteEditionNaming(content)),
    ),
  );
}

function applyStartWorkOverlay(content) {
  return content.replace(/codex:<session_id>/g, "senpi:<session_id>").replace(
    /\bcodex:/g,
    "senpi:",
  );
}
...
const ulwPlanConsultationLanes =
...
function applyUlwPlanOverlay(content) {
  ...
  if (/^#\s+ulw-plan\s*$/m.test(rewritten)) {
    return insertAfterFrontmatter(
      rewritten,
      `${ulwPlanReviewOverride}${ulwPlanConsultationLanes}`,
    );
  }
  return rewritten;
}
...
export async function syncSkills() {
  ...
  for (const skillName of sharedSkillNames) {
    if (componentSkillNames.has(skillName) || nativeSkillNames.has(skillName)) {
      continue;
    }
    ...
    await adaptSkillTree(
      destination,
      (content) => applySharedTierAdaptation(skillName, content),
    );
  }
