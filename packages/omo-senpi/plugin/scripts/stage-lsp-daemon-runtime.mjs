...
export async function stageLspDaemonRuntime(options = {}) {
  ...
  const inputDigest = await computeInputDigest(
    root,
    options.inputCandidates ?? INPUT_CANDIDATES,
  );
  await mkdir(dirname(targetDist), { recursive: true });
  const tempParent = await mkdtemp(
    join(dirname(targetDist), ".tmp-omo-senpi-lsp-runtime-"),
  );
  ...
  try {
    ...
    return {
      ok: true,
      targetDist,
      manifestPath: join(targetDist, manifestName),
    };
  } catch (error) {
...
export async function verifyRuntimeDist(distDir) {
  ...
  if (actual !== expected) {
    throw new Error(
      `Senpi LSP runtime manifest output hash mismatch at ${
...
export async function checkRuntimeDistFresh(options = {}) {
  ...
  const expectedInputDigest = await computeInputDigest(
    root,
    options.inputCandidates ?? INPUT_CANDIDATES,
  );
  if (manifest.inputDigest !== expectedInputDigest) {
    throw new Error(
      `Senpi LSP runtime manifest is stale at ${
        join(targetDist, manifestName)
      }`,
    );
  }
...
async function readRuntimeManifest(distDir) {
  ...
  if (schemaVersion !== 1) {
    throw new Error(
      `Senpi LSP runtime manifest schemaVersion must be 1: ${manifestPath}`,
    );
  }
  if (typeof version !== "string" || version.length === 0) {
    throw new Error(
      `Senpi LSP runtime manifest version is invalid: ${manifestPath}`,
    );
  }
  if (
    typeof inputDigest !== "string" ||
    !/^sha256:[0-9a-f]{64}$/.test(inputDigest)
  ) {
    throw new Error(
      `Senpi LSP runtime manifest inputDigest is invalid: ${manifestPath}`,
    );
  }
  if (!Array.isArray(outputs)) {
    throw new Error(
      `Senpi LSP runtime manifest outputs must be an array: ${manifestPath}`,
    );
  }
  for (const output of outputs) {
    if (
      ...
      throw new Error(
        `Senpi LSP runtime manifest output entry is invalid: ${manifestPath}`,
      );
    }
    if (!/^[0-9a-f]{64}$/.test(output.sha256)) {
      throw new Error(
        `Senpi LSP runtime manifest output hash is invalid: ${manifestPath}`,
      );
    }
  }
  ...
  if (JSON.stringify(sorted) !== JSON.stringify(outputs)) {
    throw new Error(
      `Senpi LSP runtime manifest outputs must be sorted: ${manifestPath}`,
    );
  }
...
async function hashOutputs(distDir) {
  const files = (await listFiles(distDir))
    ...
    .sort((left, right) =>
      relative(distDir, left).localeCompare(relative(distDir, right))
    );
  const outputs = [];
  for (const file of files) {
    outputs.push({
      path: toPortablePath(relative(distDir, file)),
  ...
  for (const output of REQUIRED_OUTPUTS) {
    ...
    try {
      ...
      if (isErrno(error, "ENOENT")) {
        throw new Error(`Senpi LSP runtime is missing ${output} at ${distDir}`);
      }
      throw error;
    }
    if (!stats.isFile()) {
      throw new Error(`Senpi LSP runtime output is not a file: ${path}`);
    }
  }
}
  ...
  try {
    if (process.argv.includes("--check")) {
      ...
      if (!(await fileExists(defaultSourceDist))) {
        console.log(
          `runtime dist not built locally; skipping freshness check: ${defaultSourceDist}`,
        );
      } else {
