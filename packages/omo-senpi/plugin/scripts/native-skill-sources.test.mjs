...
describe("createNativeSkillSources", () => {
  ...
  test("#given each source entry #when the path is resolved #then it points to an existing directory under the native skills root", () => {
    for (const { name, source } of sources) {
      ...
      assert.equal(
        source,
        expectedPath,
        `source path for "${name}" must resolve under native skills root`,
      );
      assert.ok(
        existsSync(source),
        `source directory for "${name}" must exist at ${source}`,
      );
      assert.ok(
        statSync(source).isDirectory(),
        `source for "${name}" must be a directory`,
      );
    }
  });
  ...
  test("#given onboarding skill #when checked #then it is present in the registry at the correct position", () => {
    ...
    assert.equal(
      sources.indexOf(onboardingEntry),
      5,
      "onboarding must be at index 5 (alphabetical)",
    );
