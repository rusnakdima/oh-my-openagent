...
const defaultDirectiveEntry = join(
  codexPluginRoot,
  "components",
  "ulw-loop",
  "directive.md",
...
const installedPackageLock = join(
  codexPluginRoot,
  "node_modules",
  ".package-lock.json",
);
...
export async function stageAgentToolkit(options = {}) {
  ...
  const directiveEntry = resolve(
    options.directiveEntry ?? defaultDirectiveEntry,
  );
  ...
  const tempParent = await mkdtemp(
    join(dirname(targetDir), ".tmp-omo-senpi-agent-toolkit-"),
  );
  ...
  try {
    ...
    await writeFile(
      join(tempDir, "omo-agent-toolkit.cmd"),
      windowsShim,
      "utf8",
    );
    ...
    if (await stagedToolkitMatches(tempDir, targetDir)) {
      return {
        ...
        directiveEntry,
        targetDir,
        sha256: await sha256(sourceEntry),
      };
    }
...
async function stagedToolkitMatches(tempDir, targetDir) {
  try {
    ...
    if (
      topLevelEntries.toSorted().join("\n") !== stagedTopLevelEntries.join("\n")
    ) return false;
    if (ulwLoopEntries.join("\n") !== "cli.js") return false;
    const matches = await Promise.all(
      stagedArtifactPaths.map((path) =>
        filesEqual(join(tempDir, path), join(targetDir, path))
      ),
    );
    if (!matches.every(Boolean)) return false;
    return process.platform === "win32" ||
      ((await stat(join(targetDir, "omo-agent-toolkit"))).mode & 0o777) ===
        0o755;
  } catch (error) {
    if (
      isErrno(error, "ENOENT") || isErrno(error, "ENOTDIR") ||
...
export async function checkAgentToolkitFresh(options = {}) {
  ...
  await Promise.all(
    Object.values(paths).map((path) =>
      validateFile(path, "staged agent-toolkit artifact")
    ),
  );
  if (
    !(await readFile(paths.directive)).equals(await readFile(directiveEntry))
  ) {
    throw new Error(
      `agent-toolkit runtime stale: directive content differs: ${paths.directive}`,
    );
  }
  if (await readFile(paths.dispatcher, "utf8") !== dispatcher) {
    throw new Error(
      `agent-toolkit runtime stale: dispatcher content differs: ${paths.dispatcher}`,
    );
  }
  if (await readFile(paths.posix, "utf8") !== posixShim) {
    throw new Error(
      `agent-toolkit runtime stale: POSIX shim content differs: ${paths.posix}`,
    );
  }
  if (await readFile(paths.windows, "utf8") !== windowsShim) {
    throw new Error(
      `agent-toolkit runtime stale: Windows shim content differs: ${paths.windows}`,
    );
  }
  if (
    process.platform !== "win32" &&
    ((await stat(paths.posix)).mode & 0o777) !== 0o755
  ) {
    throw new Error(
      `agent-toolkit runtime stale: POSIX shim is not mode 755: ${paths.posix}`,
    );
  }
  ...
  if (sourceSha256 !== stagedSha256) {
    throw new Error(
      `agent-toolkit runtime stale: staged ulw-loop sha256 ${stagedSha256} does not match built sha256 ${sourceSha256}`,
    );
  }
  return {
    ok: true,
...
async function buildAggregateBundle() {
  ...
  const needsInstall = !(await filesEqual(packageLock, installedPackageLock)) ||
    !(await fileExists(compiler));
  if (needsInstall) {
...
async function probeSelfContainment(targetDir) {
  ...
  try {
    ...
    if (result.status !== 0) {
      throw new Error(
        `Self-containment probe failed for ${artifact} with exit code ${
          result.status ?? 1
        }`,
      );
    }
...
async function validateFile(path, label) {
  ...
  try {
    ...
    if (isErrno(error, "ENOENT")) {
      throw new Error(`Senpi ${label} is missing: ${path}`);
    }
    throw error;
  }
  ...
  try {
    if (process.argv.includes("--check")) {
      ...
      if (!(await fileExists(defaultSourceEntry))) {
        console.log(
          `runtime dist not built locally; skipping freshness check: ${defaultSourceEntry}`,
        );
      } else {
        const result = await checkAgentToolkitFresh();
        console.log(
          `Senpi agent-toolkit runtime is current: ${result.targetDir} sha256=${result.sha256}`,
        );
      }
    } else {
