...
export async function stageAstGrepMcpRuntime(options = {}) {
  ...
  const tempParent = await mkdtemp(
    join(dirname(targetDir), ".tmp-omo-senpi-ast-grep-runtime-"),
  );
...
export async function checkAstGrepMcpRuntimeFresh(options = {}) {
  ...
  const manifestPath = resolve(
    options.manifestPath ?? join(dirname(targetEntry), "manifest.json"),
  );
  ...
  try {
    ...
    if (isErrno(error, "ENOENT")) {
      throw new Error(
        `ast-grep-mcp runtime stale: staged CLI is missing: ${targetEntry}`,
      );
    }
    throw error;
  }
  if (!targetStat.isFile()) {
    throw new Error(
      `ast-grep-mcp runtime stale: staged CLI is not a file: ${targetEntry}`,
    );
  }
  try {
    ...
    if (isErrno(error, "EACCES")) {
      throw new Error(
        `ast-grep-mcp runtime stale: staged CLI is not executable: ${targetEntry}`,
      );
    }
    throw error;
  }
  ...
  if (platform !== "win32" && manifest.mode !== stagedMode) {
    throw new Error(
      `ast-grep-mcp runtime stale: manifest mode ${manifest.mode} does not match staged mode ${stagedMode}`,
    );
  }
  return {
    ok: true,
...
async function readManifest(manifestPath) {
  ...
  try {
    ...
    if (isErrno(error, "ENOENT")) {
      throw new Error(
        `ast-grep-mcp runtime stale: manifest is missing: ${manifestPath}`,
      );
    }
    throw error;
  }
  ...
  try {
    ...
    throw new Error(
      `ast-grep-mcp runtime stale: manifest is invalid JSON: ${manifestPath}`,
    );
  }
  if (
    parsed === null ||
    ...
    throw new Error(
      `ast-grep-mcp runtime stale: manifest is malformed: ${manifestPath}`,
    );
  }
  return parsed;
}
...
async function validateEntry(entry, kind) {
  ...
  try {
    ...
    if (isErrno(error, "ENOENT")) {
      throw new Error(`Senpi ast-grep MCP ${kind} CLI is missing: ${entry}`);
    }
    throw error;
  }
  if (!entryStat.isFile()) {
    throw new Error(`Senpi ast-grep MCP ${kind} CLI is not a file: ${entry}`);
  }
  ...
  try {
    if (process.argv.includes("--check")) {
      ...
      if (!(await fileExists(defaultSourceEntry))) {
        ...
        console.log(
          `Senpi ast-grep-mcp runtime is current: ${result.targetEntry} sha256=${result.sha256}`,
        );
      }
    } else {
