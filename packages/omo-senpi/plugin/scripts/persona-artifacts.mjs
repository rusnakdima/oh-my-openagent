...
export async function stageRuntimePersonas(repoRoot, outputDir) {
  await Promise.all(
    runtimePersonaSources(repoRoot).map(async ([name, source]) =>
      writeFile(join(outputDir, name), await readFile(source, "utf8"))
    ),
  );
}

export async function findStaleRuntimePersona(
  expectedDir,
  currentDir,
  repoRoot,
) {
  for (const [name] of runtimePersonaSources(repoRoot)) {
    ...
    const current = await readFile(join(currentDir, name), "utf8").catch(() =>
      undefined
    );
