...
function minifyBundleWithNode(output) {
  ...
  return new Promise((resolvePromise, reject) => {
    ...
    minifier.child.stdin.write(
      `${JSON.stringify({ id, output })}\n`,
      (error) => {
...
function getNodeMinifier() {
  ...
  lines.on("line", (line) => {
    ...
    minifier.idleTimer = setTimeout(
      () => closeNodeMinifier(minifier),
      MINIFIER_IDLE_MS,
    );
  });
  child.on("exit", (code, signal) => {
    ...
    const reason = new Error(
      `Node minifier exited before completing requests: code=${
        String(code)
...
async function digestBuildSources(metadata, options) {
  ...
  for (const input of inputs) {
    ...
    hash.update(toPortableBuildPath(relative(options.repoRoot, inputPath)))
...
function parseBuildArtifact(text) {
  ...
  const match = /^\/\/ omo:([A-Za-z0-9_-]{43}):([A-Za-z0-9_-]{43})$/.exec(
    marked.slice(0, newline),
  );
...
if (resolve(process.argv[1] ?? "") === resolve(BUILD_ARTIFACT_PATH)) {
  if (process.argv[2] === "--minify") {
    ...
    if (output === undefined) {
      throw new Error("missing bundle path for --minify");
    }
    await minifyBundleInProcess(output);
