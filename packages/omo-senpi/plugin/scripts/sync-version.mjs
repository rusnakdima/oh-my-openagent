...
export async function syncVersion(options = {}) {
  const adapterManifest = await readJson(
    options.adapterManifestPath ?? adapterManifestPath,
  );
  const pluginManifest = await readJson(
    options.pluginManifestPath ?? pluginManifestPath,
  );
  const version = adapterManifest.version;
  if (typeof version !== "string" || version.length === 0) {
    throw new Error(
      "Cannot sync omo-senpi plugin version: adapter package.json has no version",
    );
  }
...
if (
  ...
  if (!result.ok) {
    console.error(
      `omo-senpi plugin/package.json version mismatch: expected ${result.version}, found ${result.current}`,
    );
    process.exit(1);
  }
  console.log(
    `omo-senpi plugin/package.json version ${result.version}${
      result.changed ? " synced" : " current"
    }`,
  );
}
