...
function kernel(name) {
  ...
  if (value === undefined || value === null) {
    throw new Error(
      `dag library: the eval kernel global \`${name}\` is unavailable here.`,
    );
  }
  return value;
}
...
async function readDefinitionText(name, dirs) {
  if (!/^[A-Za-z0-9][A-Za-z0-9._-]*$/.test(name)) {
    throw new Error(
      `dag library: invalid definition name "${name}" (letters, digits, dot, dash, underscore).`,
    );
  }
  ...
  throw new Error(
    `dag library: definition "${name}" not found. Searched: ${dirs.join(", ")}`,
  );
}
...
function fillPlaceholders(value, vars) {
  if (typeof value === "string") {
    return value.replaceAll("{{key}}", vars.key).replaceAll(
      "{{date}}",
      vars.date,
  ...
  if (value !== null && typeof value === "object") {
    return Object.fromEntries(
      Object.entries(value).map((
        [field, entry],
      ) => [field, fillPlaceholders(entry, vars)]),
    );
  }
  return value;
}
...
export async function load(name, options) {
  ...
  if (typeof definition.key !== "string" || definition.key === "") {
    throw new Error(
      `dag library: definition "${name}" needs a non-empty string key (the base idempotency key).`,
    );
  }
  if (!Array.isArray(definition.nodes) || definition.nodes.length === 0) {
    throw new Error(
      `dag library: definition "${name}" needs a non-empty nodes array.`,
    );
  }
  ...
  const finalKey = suffix === ""
    ? definition.key
    : `${definition.key}-${suffix ?? utcVars("", now).datetime}`;
  return {
    ...fillPlaceholders(definition, utcVars(finalKey, now)),
    key: finalKey,
  };
}
...
function throwIfToolError(action, response) {
  ...
  const message = typeof error?.message === "string" && error.message !== ""
    ? error.message
    : undefined;
  const detail = code === undefined && message === undefined
    ? firstContentText(response) ??
      "the dag tool reported an error with no details."
...
export async function start(name, options) {
  ...
  const response = throwIfToolError(
    "start",
    await kernel("tool").dag({ action: "start", definition }),
  );
  const runId = response?.details?.run_id ?? response?.run_id;
  if (typeof runId !== "string" || runId === "") {
    throw new Error(
      "dag library: the dag start response did not include a run_id.",
    );
  }
