...
async function callDag(args) {
  ...
  if (
    proxy === undefined || proxy === null || typeof proxy.dag !== "function"
  ) {
    throw new Error(
      "The dag sdk requires the eval kernel's global `tool` proxy; it is unavailable here.",
    );
  }
...
class DagDefinitionBuilder {
  ...
  node(input) {
    if (
      input === undefined || input === null || typeof input.id !== "string" ||
      input.id === ""
    ) {
    ...
    if (this.ids.has(input.id)) {
      throw new Error(
        `Duplicate dag node id "${input.id}": every node id must be unique within a definition.`,
      );
    }
...
export function define(input) {
  if (
    input === undefined || input === null || typeof input.key !== "string" ||
    input.key === ""
  ) {
    throw new Error(
      "define() needs a non-empty string key: it is the run's idempotency key.",
    );
  }
...
export function retry(runId, nodeIds, opts) {
  ...
  if (nodeIds !== undefined) {
    if (Array.isArray(nodeIds)) {
      ...
      throw new Error(
        "retry() nodeIds must be a string or string[] when provided.",
      );
    }
  }
