...
const extensionEventHandlers = new WeakMap<
  FakeExtensionAPI,
  Map<string, Set<FakeExtensionEventHandler>>
>();
  ...
  pi.events = {
    ...
    on: (name, handler) => {
      const handlers = handlersByName.get(name) ??
        new Set<FakeExtensionEventHandler>();
...
export function emitFakeExtensionEvent(
  pi: FakeExtensionAPI,
  name: string,
  payload: unknown,
): void {
  for (const handler of extensionEventHandlers.get(pi)?.get(name) ?? []) {
    handler(payload);
  }
}

export function fakeExtensionEventHandlerCount(
  pi: FakeExtensionAPI,
  name: string,
): number {
