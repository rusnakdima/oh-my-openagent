...
function asGitMasterReadResultEvent(
  payload: unknown,
): GitMasterReadResultEvent | undefined {
  if (!isRecord(payload) || payload["type"] !== "tool_result") return undefined;
  if (payload["toolName"] !== "read" || payload["isError"] === true) {
    return undefined;
  }
  const filePath = readFilePath(payload["input"]);
  if (filePath === undefined) return undefined;
  if (!filePath.replaceAll("\\", "/").endsWith(GIT_MASTER_SKILL_PATH_SUFFIX)) {
    return undefined;
  }
    ...
    register(pi: SenpiExtensionAPI, ctx: ComponentContext): void {
      pi.on(
        ...
        (
          ...
          return {
            content: [...event.content, { type: "text", text: directive }],
          };
        },
      );
    },
