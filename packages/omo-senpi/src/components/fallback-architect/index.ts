...
const FALLBACK_ARCHITECT_DISABLED_FLAG =
...
export function createFallbackArchitectComponent(
  ...
  const hasArchitectCategory = options.hasArchitectCategory ??
    ((cwd: string, registry?: GateRegistry) =>
      hasActiveArchitectCategory(cwd, { registry }));

  return {
    name: "fallback-architect",
    register(pi: SenpiExtensionAPI, ctx: ComponentContext): void {
      const state: FallbackArchitectState = {
        refusalPending: false,
        active: undefined,
      };
      const isDisabled = (): boolean =>
        ctx.config.getFlag(FALLBACK_ARCHITECT_DISABLED_FLAG) === true;
      ...
      pi.on("model_select", (payload: unknown, eventCtx: unknown): void => {
        ...
        if (
          isFableFiveModel(payload.model) ||
          payload.source === "fallback-revert"
        ) {
        ...
        if (!isFableFiveModel(payload.previousModel) || !state.refusalPending) {
          return;
        }

        const cwd = extractCwd(eventCtx) ?? process.cwd();
        if (!hasArchitectCategory(cwd, extractRegistry(eventCtx))) {
          ctx.logger.info("omo-senpi fallback-architect skipped", {
            reason: "architect-category-inactive",
          });
          state.refusalPending = false;
          return;
        }
        ...
        state.refusalPending = false;
        ctx.logger.info("omo-senpi fallback-architect directive injected", {
          from,
          to,
        });
      });
      ...
      pi.on("input", (payload: unknown): FallbackArchitectInputResult => {
        if (isDisabled() || state.active === undefined) {
...
function isUserSourcedInput(
  payload: unknown,
): payload is Record<string, unknown> & { text: string } {
  return (
    isRecord(payload) &&
    payload["type"] === "input" &&
