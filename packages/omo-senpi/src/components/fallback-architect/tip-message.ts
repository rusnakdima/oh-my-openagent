...
export function buildFallbackTipText(
...
export const renderFallbackTip: MessageRenderer = (message, options, theme) => {
  const lines = (typeof message.content === "string" ? message.content : "")
    .split("\n").map(normalizeRendererText);
  const why = lines[0] ??
    "Fallback architect keeps the refused question moving.";
  const extra = lines.slice(1, -1).filter((line) => line.length > 0).map((
    text,
  ) => ({ text }));
  ...
  return buildNoticeBox(
    {
      ...
      why,
      extra,
      ...(expandedLine === undefined || expandedLine.length === 0
        ? {}
        : { expandedLine }),
    },
    options,
