...
export function isRefusalLikeMessage(message: unknown): boolean {
  ...
  if (
    ...
    !REFUSAL_ELIGIBLE_STOP_REASONS.has(stopReason)
  ) return false;

  const stopDetails = message["stopDetails"];
  if (
    isRecord(stopDetails) && typeof stopDetails["type"] === "string" &&
    REFUSAL_STOP_DETAIL_TYPES.has(stopDetails["type"])
  ) {
    return true;
  }
  ...
  return typeof errorMessage === "string" &&
    ANTHROPIC_POLICY_REFUSAL_PATTERN.test(errorMessage);
}

export function isFableFiveModel(
  model: unknown,
): model is FallbackModelDescriptor {
  return isModelDescriptor(model) && model.id === FABLE_FIVE_MODEL_ID;
}

export function isModelSelectEvent(
  payload: unknown,
): payload is FallbackModelSelectEvent {
...
export function isMessageEndEvent(
  payload: unknown,
): payload is FallbackMessageEndEvent {
  return isRecord(payload) && payload["type"] === "message_end" &&
