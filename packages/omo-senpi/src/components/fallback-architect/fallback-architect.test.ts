...
async function setup(
  options: { architect?: boolean; disabled?: boolean } = {},
...
function policyRejectionMessage(): Record<string, unknown> {
  return {
    ...
    stopReason: "error",
    errorMessage:
      "This request triggered restrictions on request content and was blocked under Anthropic's Usage Policy.",
  };
}
...
async function endMessage(
  ...
  await pi.dispatch("message_end", { type: "message_end", message }, {
    cwd: "/tmp/project",
  });
}
...
async function selectModel(
  ...
  await pi.dispatch("model_select", { type: "model_select", ...input }, {
    cwd: "/tmp/project",
  });
}
...
function directives(pi: FakeExtensionAPI): Record<string, unknown>[] {
  return pi.messages.map((call) => call.message).filter((m) =>
    m["customType"] === FALLBACK_ARCHITECT_DIRECTIVE_TYPE
  );
}

function reminders(pi: FakeExtensionAPI): Record<string, unknown>[] {
  return pi.messages.map((call) => call.message).filter((m) =>
    m["customType"] === FALLBACK_ARCHITECT_REMINDER_TYPE
  );
}

function notices(pi: FakeExtensionAPI): Record<string, unknown>[] {
  return pi.messages.map((call) => call.message).filter((m) =>
    m["customType"] === FALLBACK_ARCHITECT_NOTICE_TYPE
  );
}
  ...
  describe("#given a classifier refusal on claude-fable-5", () => {
    describe("#when the session falls back to a weaker model", () => {
      it("#then it injects exactly one hidden directive naming both models", async () => {
        ...
        expect(String(injected[0]?.["content"])).toContain(
      ...
      it("#then it also shows exactly one visible notice with structured model details", async () => {
        ...
        expect(String(shown[0]?.["content"])).toContain(
          "anthropic/claude-opus-5",
        );
      ...
      it("#then it registers a renderer for the visible notice message", async () => {
        const pi = await setup();
        expect(
          pi.messageRenderers.map((registration) => registration.customType),
        ).toContain(FALLBACK_ARCHITECT_NOTICE_TYPE);
      });
    });
  });
