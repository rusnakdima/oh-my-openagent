...
import { type SessionArming, sharedSessionArming } from "../ultrawork/index";
  ...
  return {
    ...
    register(pi: SenpiExtensionAPI, ctx: ComponentContext): void {
      ...
      const isReminded = (sessionId: string | undefined): boolean =>
        sessionId === undefined
          ? anonymousReminded
          : remindedSessionIds.has(sessionId);
      ...
      pi.on(
        ...
        (
          ...
          ctx.logger.info("omo-senpi todo-fanout-reminder injected", {
            sessionId,
            op: event.op,
          });
          return {
      ...
      pi.on(
        "session_shutdown",
        (_payload: unknown, eventCtx: unknown): void => {
          clearReminded(extractSessionId(eventCtx));
        },
      );
    },
...
function asTodoResultEvent(payload: unknown): TodoResultEvent | undefined {
  ...
  if (payload["toolName"] !== "todo" || payload["isError"] === true) {
    return undefined;
  }
  ...
  return {
    op: input["op"],
    content: content as ReadonlyArray<Record<string, unknown>>,
  };
}
...
function isRejectedCompaction(payload: unknown): boolean {
  return isRecord(payload) && "accepted" in payload &&
