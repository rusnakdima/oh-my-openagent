...
function memoryFileSystem() {
  ...
  const fileSystem: MigrationFileSystem & {
    ...
    copyFileSync(source, destination) {
      files.set(key(destination), this.readFileSync(source, "utf-8"));
    },
    existsSync(path) {
      return files.has(key(path)) || directories.has(key(path));
    },
    lstatSync() {
      return { isSymbolicLink: () => false };
    },
    mkdirSync(path) {
      directories.add(key(path));
      return undefined;
    },
    readFileSync(path) {
    ...
    unlinkSync(path) {
      if (!files.delete(key(path))) {
        throw fileError("ENOENT", `Missing ${path}`);
      }
    },
...
function withProcessPlatform<T>(
...
describe("runSenpiStartupMigration", () => {
  test("#given both legacy source groups without an OpenCode boot #when Senpi starts #then the core migration engine migrates both into the unified config", () => {
    ...
    // when
    const result = withProcessPlatform(
      "win32",
      () => runSenpiStartupMigration(migrationOptions(fileSystem)),
    );
    ...
    expect(
      parse(fileSystem.readFileSync("/home/alice/.omo/omo.jsonc", "utf-8")),
    ).toMatchObject({
      _migrations: [
  ...
  test("#given OpenCode and Senpi race on the first legacy migration #when Senpi holds the injected core lock #then exactly one caller migrates", () => {
    ...
    // when
    const senpiResult = withProcessPlatform(
      "win32",
      () =>
        runSenpiStartupMigration({
          ...options,
    ...
    // then
    if (opencodeResult === undefined) {
      throw new Error("Expected OpenCode race result");
    }
    expect(opencodeResult.status).toBe("locked");
    expect(
      opencodeResult.results.filter((entry) => entry.status === "migrated"),
    ).toHaveLength(0);
    expect(senpiResult.results.filter((entry) => entry.status === "migrated"))
      .toHaveLength(2);
    expect(
      parse(fileSystem.readFileSync("/home/alice/.omo/omo.jsonc", "utf-8"))
        ._migrations,
...
describe("createConfigStartupComponent", () => {
  test("#given migration results and config diagnostics #when session_start captures a UI #then it reports both once through that UI", async () => {
    ...
    const config: SenpiOmoConfigResult = {
      ...
      diagnostics: [{
        kind: "parse",
        message: "JSONC parse error",
        path: "/home/alice/.omo/omo.jsonc",
      }],
      layers: [],
      sources: [],
    };
    ...
    expect(notifications).toEqual([
      {
        message:
          "omo-senpi: migrated legacy configuration from /home/alice/.config/opencode/oh-my-openagent.jsonc",
        type: "info",
      },
      {
        message: "omo-senpi: configuration diagnostics: JSONC parse error",
        type: "warning",
      },
    ]);
    expect(logs).toEqual([]);
  ...
  test("#given a migration target with a conflicting legacy value #when session_start captures a UI #then it reports the skipped-conflict diagnostic once", async () => {
    ...
    fileSystem.files.set(
      "/home/alice/.config/opencode/oh-my-openagent.jsonc",
      '{"agents":{"finder":{"model":"provider/legacy"}}}',
    );
    fileSystem.files.set(
      "/home/alice/.omo/omo.jsonc",
      '{"[opencode]":{"agents":{"finder":{"model":"provider/kept"}}}}',
    );
    ...
    expect(notifications).toEqual([
      ...
      {
        message:
          'omo-senpi: configuration migration: skipped: [opencode].agents.finder.model legacy="provider/legacy" kept="provider/kept"',
        type: "warning",
      },
    ]);
    expect(logs).toEqual([]);
  ...
  test("#given config diagnostics without a UI #when session_start fires #then it logs a single fallback warning", async () => {
    ...
    const config: SenpiOmoConfigResult = {
      ...
      diagnostics: [{
        kind: "validation",
        message: "Invalid omo config",
        path: "/project/.omo/omo.jsonc",
      }],
      layers: [],
      sources: [],
    };
    ...
    // then
    expect(logs).toEqual([
      "warn:omo-senpi: configuration diagnostics: Invalid omo config",
    ]);
  });
});
