...
export interface ConfigStartupComponentOptions {
  ...
  readonly runMigration?: (
    options: SenpiStartupMigrationOptions,
...
export function runSenpiStartupMigration(
  options: SenpiStartupMigrationOptions,
): SenpiStartupMigrationResult {
  ...
  if (homeDir.length === 0) {
    return {
      error:
        "Cannot migrate configuration because no home directory is available",
  ...
  try {
    const batch = runMigrations({
      ...
      discover: () =>
        createLegacyConfigMigrationPlans({
          ...(options.backupTimestamp === undefined
            ? {}
            : { backupTimestamp: options.backupTimestamp }),
          cwd: options.cwd,
          ...(options.discoveryFileSystem === undefined
            ? {}
            : { fileSystem: options.discoveryFileSystem }),
          environment: options.environment ?? process.env,
          homeDir,
          pathOperations: options.pathOperations ??
            (options.platform === "win32" ? win32 : posix),
    ...
    return {
      ...(batch.status === "locked"
        ? { error: "Configuration migration is already running" }
        : {}),
...
export function createConfigStartupComponent(
  options: ConfigStartupComponentOptions = {},
): OmoSenpiComponent {
    ...
    register(pi: SenpiExtensionAPI, ctx: ComponentContext): void {
      ...
      pi.on("session_start", (_payload, eventCtx) => {
        ...
        for (const notice of notices) {
          notifyOrLog(eventCtx, ctx, notice.message, notice.type);
        }
      });
    },
  };
...
function notificationMessages(
  ...
  if (migration.error !== undefined) {
    messages.push({
      message: `omo-senpi: configuration migration: ${migration.error}`,
      type: "warning",
    });
  } else if (migration.migratedFrom.length > 0) {
    messages.push({
      message: `omo-senpi: migrated legacy configuration from ${
        migration.migratedFrom.join(", ")
      }`,
      type: "info",
    });
  } else if (migration.journalResumed) {
    messages.push({
      message: "omo-senpi: recovered an interrupted configuration migration",
      type: "info",
    });
  }
  const migrationDiagnostics = migration.results.flatMap((result) =>
    result.diagnostics
  );
  if (migrationDiagnostics.length > 0) {
    messages.push({
      message: `omo-senpi: configuration migration: ${
  ...
  if (config.diagnostics.length > 0) {
    messages.push({
      message: `omo-senpi: configuration diagnostics: ${
        config.diagnostics.map((diagnostic) => diagnostic.message).join("; ")
      }`,
      type: "warning",
    });
  }
...
function notificationUi(value: unknown): NotificationUi | undefined {
  if (typeof value !== "object" || value === null || !("ui" in value)) {
    return undefined;
  }
...
function isNotificationUi(value: unknown): value is NotificationUi {
  return typeof value === "object" && value !== null &&
    typeof Reflect.get(value, "notify") === "function";
}
