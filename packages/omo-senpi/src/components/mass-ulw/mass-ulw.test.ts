...
import {
  ...
  MASS_ULW_DISABLED_FLAG,
} from "./index";

type InputDispatchResult = { action: "continue" } | {
  action: "transform";
  text: string;
};
...
function expectSkillPointerInjection(
  pi: FakeExtensionAPI,
  result: unknown,
): void {
  ...
  describe("#given the keyword detector", () => {
    it("#when given trigger spellings #then each matches", () => {
      ...
      for (const text of triggers) {
        expect({ text, matched: isMassUlwInput(text) }).toEqual({
          text,
          matched: true,
        });
      }
