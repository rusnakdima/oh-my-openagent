...
function createLogger(): ComponentContext["logger"] & { entries: unknown[] } {
  ...
  return {
    info: (_message: string, details?: unknown) =>
      entries.push({ level: "info", details }),
    warn: (_message: string, details?: unknown) =>
      entries.push({ level: "warn", details }),
    error: (_message: string, details?: unknown) =>
      entries.push({ level: "error", details }),
    entries,
  };
}
...
describe("omo-senpi start-work-continuation", () => {
  ...
  it("#given /stop-continuation fired #when subsequent agent_end fires #then continuation suppressed (issue #6752 repro 2)", async () => {
    ...
    const stop = pi.commands.find((command) =>
      command.name === "stop-continuation"
    );
    expect(stop).toBeDefined();
    const stopHandler = stop?.options["handler"] as (
      args: string,
      ctx: unknown,
    ) => void;
  ...
  it("#given /stop-continuation #when a normal user message arrives #then stop state persists across the message and continuation stays suppressed", async () => {
    ...
    const stopHandler = pi.commands.find((command) =>
      command.name === "stop-continuation"
    )?.options[
      "handler"
  ...
  it("#given /stop-continuation followed by /resume-continuation #when agent_end fires #then continuation resumes", async () => {
    ...
    const commands = new Map(
      pi.commands.map((command) => [command.name, command]),
    );
    const commandCtx = { sessionManager: { getSessionId: () => "qa-s1" } };
    const stopHandler = commands.get("stop-continuation")
      ?.options["handler"] as (args: string, ctx: unknown) => void;
    const resumeHandler = commands.get("resume-continuation")
      ?.options["handler"] as (args: string, ctx: unknown) => void;
