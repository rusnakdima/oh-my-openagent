  ...
  return {
    ...
    register(pi: SenpiExtensionAPI, ctx: ComponentContext): void {
      ...
      pi.on("agent_end", async (_payload, eventCtx) => {
        ...
        const signature = `${work.work_id}:${
          work.updated_at ?? work.started_at
...
function registerStopContinuationCommands(
  ...
  pi.registerCommand("stop-continuation", {
    description:
      "Suppress start-work / ulw-loop continuation for this session until /resume-continuation.",
    handler: (_args: string, commandCtx: StopCommandContext) => {
      const sessionId = extractCommandSessionId(commandCtx);
      if (!sessionId) {
        commandCtx.ui?.notify(
          "stop-continuation: no active session id",
          "warning",
        );
        return;
      }
      ...
      onStop();
      ctx.logger.info("omo-senpi start-work-continuation stopped", {
        sessionId,
      });
      commandCtx.ui?.notify(
        "Continuation stopped. Use /resume-continuation to re-arm.",
        "info",
      );
    },
  });
  pi.registerCommand("resume-continuation", {
    description:
      "Re-arm start-work / ulw-loop continuation after /stop-continuation.",
    handler: (_args: string, commandCtx: StopCommandContext) => {
      const sessionId = extractCommandSessionId(commandCtx);
      if (!sessionId) {
        commandCtx.ui?.notify(
          "resume-continuation: no active session id",
          "warning",
        );
        return;
      }
      guard.clear(sessionId);
      ctx.logger.info("omo-senpi start-work-continuation resumed", {
        sessionId,
      });
...
function extractCommandSessionId(
  commandCtx: StopCommandContext,
): string | undefined {
...
const START_WORK_CONTINUATION_INJECTION_KEY =
...
function renderDirective(state: DirectiveState): string {
  ...
  const nextLabel = state.checklist.nextTaskLabel ??
    "none (final gate pending)";
  const finalGateHint = state.checklist.remaining === 0
...
function extractSessionManager(
  eventCtx: unknown,
