...
async function registerFooterScenario(input: {
  ...
  await createUlwLoopComponent({
    ...
    runCommand: async () => ({
      code: 0,
      stdout: input.outputs.shift() ?? activeStatus(),
    }),
    footerStatus: {
...
async function defaultFooterScenario(
  sessionId: string,
  outputs = [activeStatus(), activeStatus()],
) {
  ...
  await createUlwLoopComponent({
    ...
    runCommand: async () => ({
      code: 0,
      stdout: outputs.shift() ?? activeStatus(),
    }),
    footerStatus: { timers },
  }).register(pi, {
    logger: createLogger(),
...
describe("omo-senpi ulw-loop footer status", () => {
  it("reads the project goal store and publishes all four animation frames", async () => {
    ...
    try {
      ...
      const frames = scenario.ui.calls.filter((call) => call.key === "ulw-loop")
        .map((call) => call.text);
  ...
  it("uses an encoded session id and clears when the project goal completes", async () => {
    ...
    try {
      ...
      expect(
        scenario.ui.calls.some((call) =>
          call.key === "ulw-loop" && call.text !== undefined
        ),
      ).toBe(true);
      ...
      expect(scenario.ui.calls.at(-1)).toEqual({
        key: "ulw-loop",
        text: undefined,
      });
  ...
  it("publishes animated ultraworking status only when goal and ulw-loop are active", async () => {
    ...
    const frames = ui.calls.filter((call) => call.key === "ulw-loop").map((
      call,
    ) => call.text);
    expect(frames.map(visibleFrame)).toEqual([
      "⚡ ultraworking",
  ...
  it("keeps continuation and headless paths unchanged", async () => {
    ...
    expect(pi.messages[0]?.message["customType"]).toBe(
      "omo-senpi:ulw-continuation",
    );
    expect(
      ui.calls.some((call) =>
        call.key === "ulw-loop" && visibleFrame(call.text) === "⚡ ultraworking"
      ),
    ).toBe(true);
