...
describe("omo-senpi ulw-loop runtime", () => {
  it("#given OMO_BIN is set #when resolving the default omo binary #then Bun is not needed and PATH is ignored", () => {
    withEnv({
      OMO_AGENT_TOOLKIT_BIN: undefined,
      OMO_BIN: "/custom/omo",
      PATH: "",
    }, () => {
  ...
  it(
    "#given a temp omo binary #when default runOmoCommand executes status #then it captures stdout and cwd via Node-compatible spawning",
    async () => {
      const fake = createTempOmoBin(activeStatus("NODE-RUNNER"));
      try {
        const result = await __testInternals.runOmoCommand(fake.bin, [
          "ulw-loop",
          "status",
          "--json",
        ], { cwd: fake.dir });
        ...
        expect(result).toEqual({
          code: 0,
          stdout: `${activeStatus("NODE-RUNNER")}\n`,
        });
  ...
  it(
    "#given a child flooding stderr beyond the 64KiB pipe buffer #when runOmoCommand executes a .js target #then it completes within the 30s budget via process.execPath",
    async () => {
      const flood = createTempStderrFloodScript(
        128 * 1024,
        activeStatus("FLOOD"),
      );
      try {
        const startedAt = Date.now();
        const result = await __testInternals.runOmoCommand(flood.script, [
          "ulw-loop",
          "status",
          "--json",
        ], {
...
describe("omo-senpi ulw-loop resolveOmoBin toolkit-first chain", () => {
  it("#given only OMO_AGENT_TOOLKIT_BIN is set #when resolving #then it wins and PATH is ignored", () => {
    withEnv({
      OMO_AGENT_TOOLKIT_BIN: "/custom/omo-agent-toolkit",
      OMO_BIN: undefined,
      PATH: "",
    }, () => {
  ...
  it("#given all three links are present #when resolving #then OMO_AGENT_TOOLKIT_BIN beats the PATH toolkit and OMO_BIN", () => {
    ...
    try {
      withEnv({
        OMO_AGENT_TOOLKIT_BIN: "/custom/toolkit",
        OMO_BIN: "/custom/omo",
        PATH: fake.dir,
      }, () => {
  ...
  it("#given a whitespace-only OMO_AGENT_TOOLKIT_BIN #when resolving #then it falls through to OMO_BIN", () => {
    withEnv(
      { OMO_AGENT_TOOLKIT_BIN: "   ", OMO_BIN: "/custom/omo", PATH: "" },
      () => {
        expect(__testInternals.resolveOmoBin()).toBe("/custom/omo");
      },
    );
...
describe("omo-senpi ulw-loop default registration through the toolkit chain", () => {
  it(
    "#given a PATH omo-agent-toolkit and no envs #when the component registers with defaults #then the toolkit binary receives the status argv",
    async () => {
      const fake = createTempOmoBin(
        activeStatus("DEFAULT-REGISTRATION"),
        "omo-agent-toolkit",
      );
      const path = process.env.PATH
        ? `${fake.dir}${delimiter}${process.env.PATH}`
        : fake.dir;
      try {
        await withEnvAsync({
          OMO_AGENT_TOOLKIT_BIN: undefined,
  ...
  it(
    "#given PATH holds only a fake named omo and no envs #when the component registers with defaults #then it degrades without executing the stale binary",
    async () => {
