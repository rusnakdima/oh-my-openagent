...
export function createUlwLoopFooterStatus(
  options: UlwLoopFooterStatusOptions = {},
): UlwLoopFooterStatus {
  ...
  function stop(): void {
    ...
    if (published && runtime !== undefined) {
      runtime.ui.setStatus(STATUS_KEY, undefined);
    }
    published = false;
    frameIndex = 0;
  }
...
function goalPathsFromContext(value: unknown): readonly string[] {
  ...
  if (
    ...
    if (typeof sessionDir === "string") {
      paths.push(join(sessionDir, "extensions", "goal", goalFile));
    }
  }
  const cwd = value["cwd"];
