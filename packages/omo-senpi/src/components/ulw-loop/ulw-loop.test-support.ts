  ...
  return JSON.stringify({
    ...
    plan: {
      ...
      goals: [{
        id: "G001",
        status: "complete",
        successCriteria: [{ id: "C001", status: "pass" }],
      }],
    },
  });
}
...
export function createTempOmoBin(
  stdout = activeStatus(),
  name = "omo",
): { dir: string; bin: string; cleanup: () => void } {
  ...
  writeFileSync(
    ...
    [
      ...
      `writeFileSync(${
        JSON.stringify(join(dir, "cwd.txt"))
      }, realpathSync(process.cwd()))`,
      `writeFileSync(${
        JSON.stringify(join(dir, "runtime.json"))
      }, JSON.stringify({ bunVersion: process.versions.bun ?? null }))`,
      `writeFileSync(${
        JSON.stringify(join(dir, "argv.json"))
...
function resolveNodeExecutable(): string {
  ...
  if (executable.length === 0) {
    throw new Error("node did not report process.execPath");
  }
  cachedNodeExecutable = executable;
  return executable;
}

export async function registerWithRunner(
  outputs: string[],
  logger = createLogger(),
): Promise<{
...
export function isTransformResult(
  value: unknown,
): value is { action: "transform"; text: string } {
  return (
    typeof value === "object" &&
    value !== null &&
