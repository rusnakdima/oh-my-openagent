...
export function createReflectionCompletionApi(
  pi: SenpiExtensionAPI,
...
export function createMemoryReflectionLiveWiring(
  ...
  const footerLive = createMemoryFooterStatusLive({
    ...
    ...(options.footerTimers === undefined
      ? {}
      : { timers: options.footerTimers }),
  });

  async function onReflectionLaunched(
    identity: string,
    run: ReservedRun,
  ): Promise<void> {
    activeRuns.start(
  ...
  return {
    ...
    async bind(
      ...
      liveSession.current = api === undefined ? undefined : {
        sessionId,
        api,
        ...(ui === undefined ? {} : { ui }),
      ...
      if (liveSession.current !== undefined) {
        try {
          ...
          options.logger?.warn("memory reflection completion drain failed", {
