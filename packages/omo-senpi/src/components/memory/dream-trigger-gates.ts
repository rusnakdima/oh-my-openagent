...
export function resolveDreamTriggerSettings(
  ...
  const override = agentName === undefined
    ? undefined
    : settings.agents[agentName]?.dream;
  return {
    ...
    autoSelectMaxChars: override?.auto_select_max_chars ??
      base.auto_select_max_chars,
  };
}

export type DreamGateRejection =
  | "disabled"
  | "shutdown_disabled"
  | "too_soon"
  | "insufficient_volume";
...
export async function evaluateDreamGates(
  ...
  if (origin === "shutdown" && !settings.shutdownLaunch) {
    return { allowed: false, rejection: "shutdown_disabled" };
  }
  const lastDreamAtMs = await probe.lastDreamAtMs();
  if (
    lastDreamAtMs !== null &&
    probe.nowMs - lastDreamAtMs <= settings.minHoursBetween * 3_600_000
  ) {
    return { allowed: false, rejection: "too_soon" };
  }
  if (
    origin !== "pressure" &&
    (await probe.unreflectedBytes()) <= DREAM_VOLUME_GATE_BYTES
  ) {
...
export async function readLastDreamAtMs(
  runtimeDir: string,
): Promise<number | null> {
  let raw: string;
  try {
  ...
  try {
    ...
    if (!isRecord(parsed) || typeof parsed.last_dream_at !== "string") {
      return null;
    }
