...
function deletionProbe(remove: (path: string) => Promise<void>) {
  ...
  return {
    ...
    remove: async (path: string): Promise<void> => {
      ...
      observed.push({
        path: path.split(/[\\/]/).pop() ?? "",
        sentinelExisted: existsSync(join(runDir, "final.json")) ||
...
async function expectCleanTerminalRun(
  runDir: string,
  sentinel: "final.json" | "abandoned.json",
): Promise<void> {
...
describe("facts terminal payload deletion", () => {
  test(
    "#given a committed run #when it finalizes #then the payload is gone and the sentinel remains",
    async () => {
      // given
  ...
  test(
    "#given an extraction with no facts #when it finalizes #then the payload is deleted too",
    async () => {
      // given
      const { root, identity, queue } = await fixture();
      const runner = new FactsExtractorRunner(
  ...
  test(
    "#given a failing child #when the run finalizes #then the payload is deleted too",
    async () => {
      // given
      const { root, identity, queue } = await fixture();
      const runner = new FactsExtractorRunner(
  ...
  test(
    "#given a dirty parent repo #when the run finalizes parent_dirty #then the payload is deleted too",
    async () => {
      // given
      const { root, identity, queue } = await fixture();
      let dirty = true;
  ...
  test(
    "#given a run of unknown liveness #when reconcile abandons it #then the payload is deleted after the sentinel",
    async () => {
      // given
      const { root, identity, queue } = await fixture();
  ...
  test(
    "#given a finalizing run #when the payload is deleted #then a terminal sentinel already exists",
    async () => {
      ...
      // then
      expect(result.status).toBe("committed");
      expect(probe.observed.map((entry) => entry.path)).toContain(
        "facts-payload.json",
      );
  ...
  test(
    "#given a crash after the sentinel #when the next session reconciles #then maintenance removes the leftover payload",
    async () => {
  ...
  test(
    "#given a non-terminal run dir #when maintenance sweeps #then its payload is left untouched",
    async () => {
  ...
  test(
    "#given an unlink that fails #when the run finalizes #then the outcome stands and a warning is logged",
    async () => {
      // given
      const { root, identity, queue } = await fixture();
      ...
      expect(existsSync(payloadPath(runDir))).toBe(true);
      expect(collector.warnings.map((entry) => entry.message)).toContain(
        "facts run artifact cleanup failed",
      );
    },
    30_000,
  );
  ...
  test(
    "#given a payload already gone #when the run finalizes #then the ENOENT is tolerated silently",
    async () => {
      ...
      expect(result.status).toBe("committed");
      await expectCleanTerminalRun(await onlyRunDir(identity), "final.json");
      expect(collector.warnings.map((entry) => entry.message)).not.toContain(
        "facts run artifact cleanup failed",
      );
    },
    30_000,
