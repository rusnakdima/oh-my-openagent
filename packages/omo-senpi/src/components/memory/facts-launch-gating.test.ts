...
async function runDirNames(identity: MemoryIdentity): Promise<string[]> {
  return (await readdir(join(identity.paths.facts, "runs")).catch(() => []))
    .sort();
}
...
describe("facts launch gating on failure state", () => {
  test(
    "#given every pending endpoint is parked #when a launch is attempted #then nothing launches and no run dir is reserved",
    async () => {
      // given
  ...
  test(
    "#given one parked conversation beside a healthy one #when a launch runs #then only the healthy conversation's entry is carried",
    async () => {
      // given
      const { root, identity, queue } = await fixture();
      await enqueue(
      ...
      expect(pending.map((entry) => entry.conversationId)).toEqual([
        "session-1",
      ]);
    },
    30_000,
  ...
  test(
    "#given a backoff endpoint #when the clock is before nextEligibleAt #then the launch is empty, and at the instant itself it launches",
    async () => {
      // given
      const { root, identity, queue } = await fixture();
      await new FactsFailureStore({
        identityPaths: identity.paths,
        now: () => NOW,
      }).recordFailure({
        targets: [{
  ...
  test(
    "#given a legacy failure record anchored at snapshot line 0 #when a launch is attempted #then the pair still gates the entry",
    async () => {
  ...
  test(
    "#given a corrupt failures.json #when a launch is attempted #then the launch aborts with a typed warning and reserves nothing",
    async () => {
      // given
      const { root, identity, queue } = await fixture();
  ...
  test(
    "#given a reconcilable dead run whose endpoints are now parked #when reconcile runs #then the run reconciles but nothing relaunches",
    async () => {
