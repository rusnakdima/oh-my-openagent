...
async function fixtureRepo(): Promise<
  { dir: string; repo: GitMemoryRepo; head: string }
> {
  const dir = realpathSync.native(
    await mkdtemp(join(tmpdir(), "memory-rpc-snapshot-")),
  );
  tempDirs.push(dir);
...
async function estimateSystemTokensLegacy(
  ...
  const paths = (await repo.lsTree(head)).filter((path) =>
    path.startsWith("system/") && path.endsWith(".md")
  );
  const contents = await Promise.all(
    paths.map((path) => repo.show(head, path)),
  );
...
describe("estimateSystemTokens", () => {
  test("#given system and non-system markdown #when estimated from lsTreeSized #then the result matches the old per-path show loop", async () => {
    ...
    // when
    const estimate = await estimateSystemTokens(
      createMemoryRpcGitRepo(dir),
      head,
      new Map(),
    );
...
async function contextFixture(): Promise<MemoryIdentityContext> {
  const root = realpathSync.native(
    await mkdtemp(join(tmpdir(), "memory-rpc-snapshot-ctx-")),
  );
  tempDirs.push(root);
...
async function writeCompletion(
  ...
  await writeFile(
    join(dir, `${String(record.runId)}.json`),
...
describe("buildMemorySnapshot previousEntryAtISO", () => {
  test("#given a real two-commit repo #when a snapshot is built #then previousEntryAtISO carries the second-newest commit time", async () => {
    // given
    const dir = realpathSync.native(
      await mkdtemp(join(tmpdir(), "memory-rpc-two-commit-")),
    );
    tempDirs.push(dir);
  ...
  test("#given a repo with a single commit #when a snapshot is built #then previousEntryAtISO is omitted", async () => {
    // given
    const dir = realpathSync.native(
      await mkdtemp(join(tmpdir(), "memory-rpc-one-commit-")),
    );
    tempDirs.push(dir);
...
describe("buildMemorySnapshot lastConsolidationAtISO", () => {
  test("#given merged and failed completions #when a snapshot is built #then lastConsolidationAtISO is the newest merged finishedAt", async () => {
    ...
    await writeCompletion(
      context,
      completion("run-1", "merged", "2026-08-10T01:00:00.000Z"),
    );
    await writeCompletion(
      context,
      completion("run-2", "merged", "2026-08-11T05:00:00.000Z"),
    );
    await writeCompletion(
      context,
      completion("run-3", "failed", "2026-08-12T09:00:00.000Z"),
    );
    ...
    // then
    expect(snapshot.reflection.lastConsolidationAtISO).toBe(
  ...
  test("#given only non-merged completions #when a snapshot is built #then lastConsolidationAtISO is omitted", async () => {
    ...
    await writeCompletion(
      context,
      completion("run-1", "no_changes", "2026-08-10T01:00:00.000Z"),
    );
    await writeCompletion(
      context,
      completion("run-2", "failed", "2026-08-11T05:00:00.000Z"),
    );
