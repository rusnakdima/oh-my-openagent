...
function fixture(): {
  ...
  const payloadPaths = [
    join(payloadDir, "transcript.json"),
    join(payloadDir, "persona.md"),
  ];
  for (const dir of [worktree, gitCommonDir, payloadDir]) {
...
describe("reflection worker OS sandbox", () => {
  test.skipIf(
    ...
    () => {
      ...
      // when
      const allowedRun = transform({
        ...spawnArgs(setup.worktree),
        args: ["-c", `echo allowed > '${allowed}'`],
      });
      const allowedResult = Bun.spawnSync([
        allowedRun.command,
        ...allowedRun.args,
      ], { cwd: allowedRun.cwd, env: allowedRun.env });
      const deniedRun = transform({
        ...spawnArgs(setup.worktree),
        args: ["-c", `echo denied > '${denied}'`],
      });
      const deniedResult = Bun.spawnSync(
        [deniedRun.command, ...deniedRun.args],
        { cwd: deniedRun.cwd, env: deniedRun.env },
      );
      ...
      expect(deniedResult.stderr.toString()).toMatch(
  ...
  test(
    "#given Darwin payloads and a foreign agent root #when the profile is generated #then payload reads and network remain allowed while foreign reads are denied",
    () => {
      // given
      const setup = fixture();
      ...
      expect(profile).toContain(
        `(allow file-read* (literal ${
          JSON.stringify(realpathSync(setup.payloadPaths[0] ?? ""))
        }))`,
      );
      expect(profile).toContain(
        `(deny file-read* (subpath ${
          JSON.stringify(realpathSync(foreignRoot))
        }))`,
      );
      expect(transformed.env.TMPDIR).toBe(
        join(
          realpathSync(dirname(setup.payloadPaths[0] ?? "")),
          ".sandbox-tmp",
        ),
      );
    },
  ...
  test(
    "#given the Darwin profile #when git-shell children run inside it #then device nodes they stream through stay writable",
    () => {
      // given
      const setup = fixture();
      const transform = buildSandboxTransform({
  ...
  test(
    "#given Linux with bwrap available #when spawn arguments are transformed #then the root is read-only while worktree and git state are rebound writable",
    () => {
      // given
      const { setup, transform } = build("required", {
        platform: "linux",
      ...
      expect(transformed.args).toEqual([
        ...
        "/tmp",
        "--bind",
        realpathSync(setup.worktree),
        realpathSync(setup.worktree),
        "--bind",
        realpathSync(setup.gitCommonDir),
        realpathSync(setup.gitCommonDir),
        "--chdir",
        setup.worktree,
        "--",
  ...
  test(
    "#given a bare inner command available on the child PATH #when sandbox arguments are transformed #then the wrapper receives its absolute path",
    () => {
      // given
      const setup = fixture();
      const original = {
      ...
      // then
      expect(transformed.command).toBe("/usr/bin/bwrap");
      expect(transformed.args.slice(-4)).toEqual([
        "--",
        process.execPath,
        "-c",
        "exit 0",
  ...
  test(
    "#given a bare inner command missing from the child PATH #when sandbox arguments are transformed #then it degrades to identity with an explicit warning",
    () => {
      // given
      const setup = fixture();
      const original = {
      ...
      expect(transform.warning).toContain(
        'inner command "missing-senpi" is not absolute and could not be resolved',
      );
    },
    30_000,
  );
  ...
  test(
    "#given required policy without a platform sandbox #when the transform is built #then a typed unavailable error is thrown",
    () => {
      // given
      const setup = fixture();
      ...
      expect(buildRequired).toThrow(
        "required reflection sandbox unavailable on linux: bwrap not found",
      );
    },
    30_000,
  );
  ...
  test(
    "#given auto policy without a platform sandbox #when spawn arguments are transformed #then they pass through with an explicit warning",
    () => {
      // given
      ...
      expect(transformed).toBe(original);
      expect(transform.wasSandboxed).toBe(false);
      expect(transform.warning).toBe(
        "reflection sandbox unavailable on linux: bwrap not found; running unsandboxed because policy is auto",
      );
    },
    30_000,
  );
  ...
  test(
    "#given off policy #when the transform is built and used #then detection is skipped and spawn arguments pass through",
    () => {
      // given
