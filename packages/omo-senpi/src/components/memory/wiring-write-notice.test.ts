...
async function registerWith(writeNoticeEnabled: boolean): Promise<{
  ...
  const root = realpathSync.native(
    await mkdtemp(join(tmpdir(), "omo-memory-write-notice-wiring-")),
  );
  roots.push(root);
  ...
  await mkdir(join(identity.paths.transcripts, "session-write-notice"), {
    recursive: true,
  });
  ...
  createMemoryComponent({
    ...
    loadConfig: () =>
      loadedMemoryConfig(
        memorySettings({ write_notice: { enabled: writeNoticeEnabled } }),
      ),
    resolveCwd: () => cwd,
  }).register(pi, componentContext());
  const tool = pi.tools.find((registered) =>
    registered["name"] === MEMORY_TOOL_NAME
  );
  if (tool === undefined) throw new Error("memory tool was not registered");
  const renderResult = tool["renderResult"];
  if (typeof renderResult !== "function") {
    throw new Error("memory tool has no renderResult");
  }
  return {
...
const RESULT = {
  content: [{
    type: "text",
    text: "Memory create committed locally (a1b2c3d).",
  }],
  details: {
...
function lines(
  ...
  return renderResult(RESULT, { expanded: false, isPartial: false }, THEME, {
    isError: false,
  }).render(200);
}
...
describe("memory write notice wiring", () => {
  test(
    "#given the write notice is enabled #when the registered tool renders a committed result #then the notice replaces the local-commit line",
    async () => {
      // given
  ...
  test(
    "#given the write notice is disabled in config #when the registered tool renders #then the plain message survives",
    async () => {
      // given
