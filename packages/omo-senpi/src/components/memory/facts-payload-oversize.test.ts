...
import {
  FactsExtractorRunner,
  type FactsExtractorRunnerOptions,
} from "./facts-runner";
import {
  enqueue,
  fixture,
...
async function bloatPeoplePayload(identity: MemoryIdentity): Promise<void> {
  ...
  const aliases = Array.from(
    { length: 64 },
    (_, index) => `alias-${index}-${"w".repeat(2_048)}`,
  );
  await writeFile(
...
function launch(
  ...
  return new FactsExtractorRunner(
...
describe("facts oversize payload classification", () => {
  test(
    "#given one entry larger than the whole cap #when a launch runs #then it parks immediately while other conversations still ship",
    async () => {
      // given: session-1 (small, from the fixture) plus an oversize session-2 entry.
      const { root, identity, queue } = await fixture();
      await publish(queue, identity, "session-2", [
        message("m2", oversizeText()),
      ]);
      ...
      expect(ledger.queued.map((key) => key.conversationId)).toEqual([
        "session-1",
      ]);
      ...
      expect((await queue.listPending()).map((entry) => entry.conversationId))
        .toEqual(["session-2"]);
    },
    30_000,
  );
  ...
  test(
    "#given a parked oversize entry #when the same conversation queues a newer entry #then the newer entry stays blocked while others flow",
    async () => {
      ...
      expect((await store.readFailures()).entries).toMatchObject([{
        conversationId: "session-2",
        state: "parked",
      }]);
      await publish(queue, identity, "session-2", [
        message("m2", oversizeText()),
        message("m3", "Small tail."),
      ]);
      await enqueue(
        queue,
        identity,
      ...
      const latest = JSON.parse(
        await readFile(
          join(
            identity.paths.facts,
            "runs",
            runs[runs.length - 1] ?? "missing",
            "ledger.json",
          ),
          "utf8",
        ),
      ) as FactsRunLedger;
      expect(latest.queued.map((key) => key.conversationId)).toEqual([
        "session-3",
      ]);
      expect(
        (await queue.listPending()).map((entry) => entry.range.end_message_id)
          .sort(),
      ).toEqual(["m2", "m3"]);
    },
    30_000,
  ...
  test(
    "#given a people payload larger than the cap #when a launch runs #then nothing launches, frontiers take a normal streak, and the warning names bytes vs cap",
    async () => {
      // given
      const { root, identity, queue } = await fixture();
      await enqueue(
      ...
      expect(state.entries.map((record) => record.conversationId).sort())
        .toEqual(["session-1", "session-2"]);
      for (const record of state.entries) {
        expect(record).toMatchObject({
          state: "backoff",
          streak: 1,
          lastReason: "payload_envelope_oversize",
        });
        expect(record.nextEligibleAt).toBe(
          new Date(NOW.getTime() + 60_000).toISOString(),
        );
      }
      const warning = collector.warnings.find((entry) =>
        entry.message.includes("envelope")
      );
      expect(warning).toBeDefined();
      expect(warning?.fields.envelopeBytes).toBeGreaterThan(
        MAX_FACTS_PAYLOAD_BYTES,
      );
  ...
  test(
    "#given an oversize park #when /facts retry clears the record #then the entry is launchable again and re-parks while still oversize",
    async () => {
      // given
      const { root, identity, queue } = await fixture();
      await publish(queue, identity, "session-2", [
      ...
      const removed = await store.clearForRetry({
        conversationId: "session-2",
      });
      const afterRetry = await store.readFailures();
      const relaunch = await launch(root, identity, queue, {
        now: () => new Date(NOW.getTime() + 60_000),
      });
  ...
  test(
    "#given only oversize work #when classification runs #then no queue file, watermark, or run dir is written",
    async () => {
