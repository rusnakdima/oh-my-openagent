...
function recordingSteps(
  order: string[],
  overrides: Partial<ShutdownDrainSteps> = {},
): ShutdownDrainSteps {
  return {
    flushJournal: async () => {
      order.push("a");
...
describe("session shutdown drain budget", () => {
  ...
  test("#given a quit shutdown #when the drain runs #then steps run in the a, b, c-prime, c, d order", async () => {
    ...
    drain.registerEvaluator(({ signal }) => {
      order.push("d1");
      signals.push(signal);
    });
    drain.registerEvaluator(async () => {
      order.push("d2");
    });
  ...
  test("#given an aborted budget #when the stalled step finally settles #then no step that had not started ever starts", async () => {
    ...
    await drain.run({
      reason: "quit",
      sessionId: SESSION,
      deadlineAt: shutdownDeadlineAt(() => 0),
      now: () => clock,
    });
    releaseStall?.();
    await stalled;
  ...
  test("#given a bound session with journal rows #when session_shutdown fires #then the drain runs before the session is released", async () => {
    ...
    const root = realpathSync.native(
      await mkdtemp(join(tmpdir(), "omo-memory-shutdown-")),
    );
    tempDirs.push(root);
    ...
    createMemoryComponent({
      env,
      loadConfig: () =>
        loadedMemoryConfig(memorySettings({ agent: "drain-agent" })),
      now: () => 0,
      resolveCwd: () => cwd,
    ...
    const journal = new TranscriptJournal({
      journalDir: join(identity.paths.transcripts, SESSION),
    });
    ...
    // then
    const queued = await readdir(identity.paths.factsQueue).catch(() =>
      [] as string[]
    );
    expect(
      queued.filter((name) =>
      ...
      new Promise<never>((_, reject) => {
        setTimeout(
          () =>
            reject(
              new Error(
                "waited 5s for the bind-time reconcile warning, never fired",
              ),
            ),
          5_000,
        );
