...
export const MEMORY_REFLECTING_FRAMES = [
  "⠋",
  "⠙",
  "⠹",
  "⠸",
 ...
 */
export function createMemoryFooterLive(
  options: MemoryFooterLiveOptions = {},
): MemoryFooterLive {
  ...
  function publishFrame(): void {
    ...
    if (
      disposed || activeIdentity === undefined || activeUi === undefined ||
      frame === undefined
    ) return;
    activeUi.setStatus(
      MEMORY_STATUS_KEY,
      `mem:${activeIdentity} ${frame} reflecting`,
    );
  }
  ...
  async function recompute(
    ...
    try {
      ...
      if (disposed || text === undefined || activeIdentity !== undefined) {
        return;
      }
