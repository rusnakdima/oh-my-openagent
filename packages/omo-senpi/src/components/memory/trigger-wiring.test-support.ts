...
export async function fixture(options: {
  ...
  const journal = new TranscriptJournal({
    journalDir: join(identity.paths.transcripts, CONVERSATION),
  });
  ...
  const store = new ReflectionReservationStore({
    identity,
    config: {
      stepCount: options.stepCount ?? 1,
      onCompaction: options.onCompaction ?? true,
    },
    getJournal: async (conversationId) => {
      if (conversationId !== CONVERSATION) {
        throw new Error(`missing journal: ${conversationId}`);
      }
      return journal;
    },
  ...
  const ledger: MemoryPendingLedger = {
    pendingCompaction: false,
    configRestartNotified: false,
  };
  ...
  const wiring = createReflectionTriggerWiring({
    ...
    ...(options.withoutLaunchHandler === true ? {} : {
      onLaunch: options.onLaunch ??
        ((request: ReflectionRequest) => void launches.push(request)),
    }),
    logger: {
