...
import { createMemoryComponent, MEMORY_BINDING_CUSTOM_TYPE } from "./index";
import {
  componentContext,
  loadedMemoryConfig,
...
const BRANCH = [{ id: "message-1" }, { id: "message-2" }, {
  id: "message-3",
}] as const;
...
async function compactionFixture(): Promise<CompactionFixture> {
  const root = realpathSync.native(
    await mkdtemp(join(tmpdir(), "omo-memory-compaction-")),
  );
  roots.push(root);
  ...
  const store = new ReflectionReservationStore({
    ...
    getJournal: async (conversationId) => {
      ...
      const journal = new TranscriptJournal({
        journalDir: join(identity.paths.transcripts, conversationId),
      });
  ...
  return {
    ...
    readLedger: () => {
      if (boundLedger === undefined) {
        throw new Error("memory runtime was not created");
      }
      return boundLedger;
    },
    launches,
...
describe("compaction survival + trigger integration", () => {
  test(
    "#given a bound session with committed memory #when an accepted compaction settles successfully #then exactly one reflection launches and the sentinel block survives byte-identical",
    async () => {
      const { pi, identity, readLedger, launches, logs } =
        await compactionFixture();
      ...
      expect(pi.entries[0]?.data).toMatchObject({
        ...
        repoPathHash: createHash("sha256").update(identity.paths.repo, "utf8")
          .digest("hex"),
      });
  ...
  test(
    "#given a bound session #when compaction is rejected #then no flag is recorded and settle launches nothing",
    async () => {
  ...
  test(
    "#given compaction accepted mid-run #when the retried turn chain settles #then exactly one launch fires across all settles",
    async () => {
  ...
  test(
    "#given two consecutive accepted compactions #when the run settles #then the flag is consumed once with exactly one launch",
    async () => {
      const { pi, readLedger, launches, logs } = await compactionFixture();
