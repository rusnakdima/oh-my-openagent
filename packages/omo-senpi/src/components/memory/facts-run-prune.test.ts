...
describe("terminal facts run pruning", () => {
  test(
    "#given 25 terminal runs #when pruning keeps the last 20 #then the newest 20 survive deterministically",
    async () => {
      // #given
      ...
      for (let index = 0; index < 25; index += 1) {
        await seedTerminalRun({
          factsDir,
          runId: `facts-aaaa-${index}`,
          finishedAt: instant(index),
        });
      }
  ...
  test(
    "#given the newest run alone exceeds the byte cap #when pruning runs #then the newest is still kept",
    async () => {
      ...
      await seedTerminalRun({
        factsDir,
        runId: "facts-bbbb-2",
        finishedAt: instant(2),
        bytes: 8_192,
      });

      // #when
      const result = await pruneTerminalFactsRuns({
        factsDir,
        locksDir,
        maxTotalBytes: 1_024,
      });
  ...
  test(
    "#given terminal runs past the byte cap #when pruning runs #then only the newest-first prefix under the cap survives",
    async () => {
      // #given
      const { factsDir, locksDir } = await fixture();
  ...
  test(
    "#given runs finishing in the same instant #when pruning sorts them #then the run id breaks the tie deterministically",
    async () => {
      // #given
      const { factsDir, locksDir } = await fixture();
      await seedTerminalRun({
  ...
  test(
    "#given a non-terminal run dir #when pruning runs #then it is never pruned",
    async () => {
      // #given
      const { factsDir, locksDir } = await fixture();
  ...
  test(
    "#given a malformed sentinel #when pruning runs #then the mismatched run is never pruned",
    async () => {
      ...
      await writeFile(
        join(factsDir, "runs", "facts-ffff-3", "final.json"),
        "{ not json",
        "utf8",
      );
  ...
  test(
    "#given a live process holding a run's finalize lock #when pruning runs #then that run survives",
    async () => {
  ...
  test(
    "#given a leftover prune tombstone #when session-start maintenance sweeps #then the tombstone is removed",
    async () => {
      ...
      const tombstone = join(
        factsDir,
        "runs",
        ".prune-facts-hhhh-1-11111111-1111-4111-8111-111111111111",
      );
  ...
  test(
    "#given pruning frees a lower attempt name #when a higher attempt survives #then the sequence stays monotonic",
    async () => {
      ...
      expect([runName(first), runName(second)]).toEqual([
        "facts-" + digestOf(first) + "-1",
        "facts-" + digestOf(first) + "-2",
      ]);
  ...
  test(
    "#given every trace of a digest is pruned #when it reserves again #then the attempt name IS reused",
    async () => {
  ...
  test(
    "#given the facts-runs lock held by a live process #when a reservation runs #then it waits rather than racing the scan",
    async () => {
      // #given
      const { factsDir, locksDir } = await fixture();
  ...
  test(
    "#given a tombstone rename #when the deletion runs #then the facts-runs lock is already released",
    async () => {
