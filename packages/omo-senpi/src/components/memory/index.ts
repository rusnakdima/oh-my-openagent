...
import {
  hasMemoryCapabilities,
  missingMemoryCapabilities,
} from "./capabilities";
import {
...
const RESTART_REQUIRED_NOTICE =
  "restart required to apply memory config change";
  ...
  return {
    ...
    register(pi: SenpiExtensionAPI, ctx: ComponentContext): void {
      ...
      const wiring = createMemoryWiring({
        ...
        ...(options.refreshStatus === undefined
          ? {}
      ...
      pi.registerEntryRenderer(
        MEMORY_BINDING_CUSTOM_TYPE,
        renderMemoryBindingEntry,
      );
      const unsubscribeReload = pi.events?.on(
        CONFIG_WATCH_RELOADED,
        (payload) => {
          if (!isOmoConfigReload(payload)) return;
          const enabled = isEnabled(
            resolveMemoryConfig(loadConfig({ cwd })),
            ctx,
            env,
          );
      ...
      pi.on("session_start", (_payload, eventCtx) => {
        ...
        void wiring.afterBind(pi, surface.id, state.context, eventCtx).catch(
          (error: unknown) => {
            ctx.logger.warn("memory bind-time reconcile failed", {
              error: String(error),
            });
          },
        );
...
const CHILD_SENTINELS = [
  "SENPI_MEMORY_REFLECTION",
  "SENPI_MEMORY_FACTS",
] as const;

export function isMemoryChildProcess(
...
function readSessionSurface(value: unknown): SessionSurface {
  ...
  const manager = isRecord(value.sessionManager)
    ? value.sessionManager
    : undefined;
  const getSessionId = manager?.getSessionId;
  const getEntries = manager?.getEntries;
  const id = typeof getSessionId === "function"
    ? Reflect.apply(getSessionId, manager, [])
    : "unknown-session";
  const entries = typeof getEntries === "function"
    ? Reflect.apply(getEntries, manager, [])
    : [];
