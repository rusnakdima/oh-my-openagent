...
describe("registerMemoryUsage", () => {
  test("#given a read tool targeting reference/project/foo.md #when dispatched then flushed #then foo.md.count is 1", async () => {
    ...
    await pi.dispatch(
      "tool_call",
      toolCall("read", {
        path: join(repoDir, "reference", "project", "foo.md"),
      }),
      eventContext("session-1"),
    );
    const tracker = trackers.get(context.identity);
    await tracker?.flush();
    const ledger = await readMemoryUsageLedger(
      memoryUsagePaths(context.identityPaths).ledgerPath,
    );
    expect(ledger["reference/project/foo.md"]).toEqual({
      count: 1,
