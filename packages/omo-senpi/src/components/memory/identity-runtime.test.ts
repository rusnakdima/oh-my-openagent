  ...
  test.skipIf(process.platform !== "darwin" && process.platform !== "linux")(
    ...
    async () => {
      ...
      const root = await mkdtemp(
        join(tmpdir(), "omo-memory-identity-runtime-"),
      );
      roots.push(root);
      ...
      process.env.PATH = `${binDir}${process.platform === "win32" ? ";" : ":"}${
        previousPath ?? ""
      }`;
      ...
      try {
        ...
        const sandbox = (runtime.runner as unknown as {
          options: { sandbox: ReflectionSandbox };
        }).options.sandbox;
        ...
        expect(ctx.logs).toContainEqual({
          ...
          details: {
            ...
            runId: "reflection-run-visible",
            warning:
              'reflection sandbox unavailable: inner command "missing-senpi" is not absolute and could not be resolved; running unsandboxed',
          },
        });
      } finally {
...
async function renderReflectionSandboxAgentDir(envShape: {
  ...
  { readonly fakeHome: string; readonly renderedArgs: readonly string[] }
> {
  ...
  if (envShape.omo !== undefined) {
    injectedEnv.OMO_CODING_AGENT_DIR = envShape.omo;
  }
  if (envShape.senpi !== undefined) {
    injectedEnv.SENPI_CODING_AGENT_DIR = envShape.senpi;
  }
  const snapshot = {
    ...
    const runtime = createIdentityRuntime(identity, {
      ...
      resolveAgentDir: () =>
        resolveAgentHome({
          env: injectedEnv,
          homeDir: fakeHome,
          exists: (p) => p === sentinelPath,
        }),
    });
    const sandbox =
      (runtime.runner as unknown as { options: { sandbox: ReflectionSandbox } })
        .options.sandbox;
...
function sandboxGrantsAgentDir(
  ...
  if (process.platform === "darwin") {
    ...
    return typeof profile === "string" &&
      profile.includes(
        `(allow file-write* (subpath ${JSON.stringify(agentDirReal)}))`,
      );
  }
  for (let i = 0; i + 2 < renderedArgs.length; i++) {
    if (
      renderedArgs[i] === "--bind" && renderedArgs[i + 1] === agentDirReal &&
      renderedArgs[i + 2] === agentDirReal
    ) {
      return true;
    }
  }
...
describe("memory identity runtime agent-dir resolution", () => {
  test.skipIf(process.platform !== "darwin" && process.platform !== "linux")(
    ...
    async () => {
      ...
      // when
      const { renderedArgs } = await renderReflectionSandboxAgentDir({
        omo: omoAgentDir,
      });
      ...
      expect(sandboxGrantsAgentDir(renderedArgs, realpathSync(omoAgentDir)))
        .toBe(true);
    },
    30_000,
  );
  ...
  test.skipIf(process.platform !== "darwin" && process.platform !== "linux")(
    ...
    async () => {
      // given
      const senpiAgentDir = await mkdtemp(
        join(tmpdir(), "omo-senpi-agent-dir-"),
      );
      ...
      // then
      expect(sandboxGrantsAgentDir(renderedArgs, realpathSync(senpiAgentDir)))
        .toBe(true);
    },
    30_000,
  );
  ...
  test.skipIf(process.platform !== "darwin" && process.platform !== "linux")(
    ...
    async () => {
      ...
      expect(
        sandboxGrantsAgentDir(
          renderedArgs,
          realpathSync(join(fakeHome, ".omo")),
        ),
      ).toBe(true);
    },
    30_000,
