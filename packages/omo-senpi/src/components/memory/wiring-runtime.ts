...
import {
  createMemoryFactsWiring,
  type MemoryFactsWiring,
} from "./facts-wiring";
import {
...
import {
  createMemoryJournalWiring,
  type MemoryJournalWiring,
} from "./journal-wiring";
...
export interface MemoryRuntimeWiring {
  ...
  resolveModelRegistry(): ReturnType<
    MemoryIdentityRuntimeDeps["resolveModelRegistry"]
  >;
...
export function createMemoryRuntimeWiring(
  ...
  function resolveModelRegistry(): ReturnType<
    MemoryIdentityRuntimeDeps["resolveModelRegistry"]
  > {
  ...
  function factsWiringFor(identity: MemoryIdentityContext): MemoryFactsWiring {
    ...
    const sandboxPolicy =
      settings.agents[identity.identity]?.reflection?.sandbox ??
    ...
    const extractor = createExtractor({
      ...
      sandbox: buildFactsSandboxTransform({
        ...
        onWarning: (warning, spawnArgs) =>
          options.logger?.warn("memory facts sandbox degraded", {
    ...
    const runtime = create(identity, {
      ...
      ...(liveSession === undefined ? {} : {
        liveSession: () => {
          ...
          if (live === undefined || hooks.onLiveCompletion === undefined) {
            return live;
          }
          return {
            ...live,
            onCompletion: (runId: string) =>
              hooks.onLiveCompletion?.(identity.identity, runId),
          };
        },
      }),
    });
  ...
  function triggerSessionFor(
    eventCtx: unknown,
  ): ReflectionTriggerSession | undefined {
    ...
    return {
      ...
      enabled:
        resolveReflectionTriggerConfig(settings, identity.identity).enabled,
      engine: {
    ...
    return {
      ...
      getJournal: async (conversationId) =>
        new TranscriptJournal({
          journalDir: join(identity.identityPaths.transcripts, conversationId),
        }),
      store: runtime.store,
