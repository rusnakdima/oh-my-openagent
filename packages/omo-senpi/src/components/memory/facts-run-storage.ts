...
export async function reserveFactsRunDir(options: {
  ...
  const record = await createLockRecord("facts-runs", {
...
async function claimFactsRunDir(options: {
  ...
  const digest = createHash("sha256").update(
    JSON.stringify(queueKeys(options.entries)),
...
export async function writeFactsFinal(options: {
  ...
  await (options.write ?? writeRunJsonAtomic)(
    join(options.runDir, "final.json"),
    {
      version: 1,
      runId: options.runId,
...
export async function runLiveness(
  ...
  for (const [pid, expectedStart] of identities) {
    ...
    if (
      liveness === "unknown" || expectedStart === null ||
