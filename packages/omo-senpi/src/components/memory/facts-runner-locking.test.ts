...
describe("deterministic facts writer-lock interleavings", () => {
  test(
    "#given the lock releases through the injected retry boundary #when finalization retries #then the batch commits and the queue empties",
    async () => {
      // given
  ...
  test(
    "#given the lock remains held for all three attempts #when finalization exhausts retries #then no commit lands and one warning leaves queue and watermarks unchanged",
    async () => {
      // given
      const { root, identity, queue } = await fixture();
      ...
      expect(
        (await repo.log()).filter((commit) =>
          commit.trailers["Generated-By"] === "facts-extractor"
        ),
      ).toHaveLength(0);
    },
    30_000,
