...
import {
  type FactsFailureReadPort,
  hasFailureReader,
  readLaunchableFailures,
} from "./facts-launch-selection";
import {
...
export class FactsExtractorRunner {
  ...
  constructor(private readonly options: FactsExtractorRunnerOptions) {
    this.queue = options.queue ??
      new FactsQueue({ identityPaths: options.identity.paths });
    this.now = options.now ?? (() => new Date());
    const store = new FactsFailureStore({
      identityPaths: options.identity.paths,
      now: this.now,
    });
    ...
    this.terminal = new FactsTerminalWrites({
      ...
      ...(options.writeTerminalSentinel === undefined
        ? {}
        : { write: options.writeTerminalSentinel }),
      ...(options.removeRunArtifact === undefined
        ? {}
        : { remove: options.removeRunArtifact }),
  ...
  async launchPending(signal?: AbortSignal): Promise<FactsLaunchResult> {
    ...
    const operation = drainFactsLaunches(
      () => this.launchPendingOnce(signal),
      signal,
    );
  ...
  private async launchPendingOnce(
    signal?: AbortSignal,
    ...
    const ledger = await readLaunchableFailures(
      this.failureReader,
      (message, fields) => this.options.logger?.warn(message, fields),
    );
    ...
    const resolution = resolveReflectionModel(
      QUICK_CATEGORY,
      loaded.config,
      this.options.resolveModelRegistry(),
    );
    if (resolution.kind === "category_unavailable") {
      this.options.logger?.warn("facts extractor quick category unavailable", {
    ...
    const repo = new GitMemoryRepo({
      dir: this.options.identity.paths.repo,
    ...
    const capped = selectCappedFactsBatch({
      entries,
      envelope,
      now: this.now(),
    });
    ...
    const envelopeRefused = await classifyOversizePayload({
      ...
      ...(this.options.createPreflightId === undefined
        ? {}
        : { createFailureId: this.options.createPreflightId }),
    ...
    if (capped.selected.length === 0) {
      this.options.logger?.warn(
        "facts batch selection carried nothing within the payload cap",
        {
    ...
    try {
      ...
      if (child.timedOut || child.code !== 0) {
        const reason: FactsFailureReason = child.timedOut
          ? "deadline_exceeded"
          : "child_exit";
      ...
      const reason: FactsFailureReason =
  ...
  private async reconcileRuns(): Promise<boolean> {
    ...
    await sweepTerminalFactsRuns({
      factsDir: this.options.identity.paths.facts,
      ...(this.options.removeRunArtifact === undefined
        ? {}
        : { remove: this.options.removeRunArtifact }),
    ...
    const active = await reconcileFactsRuns({
      ...
      abandon: (runDir, ledger, reason) =>
        this.terminal.abandon(runDir, ledger, reason),
      warn: (message, fields) => this.options.logger?.warn(message, fields),
    });
    await this.prune();
    return active;
  ...
  private async prune(): Promise<void> {
    try {
      ...
      this.options.logger?.warn("facts run retention pruning failed", {
  ...
  private async finalizeRun(
    ...
    const ledger = await readRunJson<FactsRunLedger>(
      join(runDir, "ledger.json"),
    );
    const record = await createLockRecord("facts-finalize", {
      runId: ledger.runId,
    });
    ...
    const finalizeLock = runFinalizationLockPath(
      this.options.identity.paths.locks,
      ledger.runId,
    );
    const result = await withLock<FactsLaunchResult>(
      finalizeLock,
      record,
      async () => {
        ...
        if (existsSync(finalPath)) {
          return finalResult(
            await readRunJson<FactsFinalRecord>(finalPath),
          );
        }
        if (existsSync(join(runDir, "abandoned.json"))) {
          return {
            status: "failed",
            runId: ledger.runId,
          };
        ...
        return finalizeClaimedFactsRun({
          ...
          withWriterLock: (operation, attempt) =>
            this.withWriterLock(operation, attempt),
        });
      },
  ...
  private async withWriterLock<T>(
    ...
    if (this.options.withWriterLock !== undefined) {
      return this.options.withWriterLock(operation, attempt);
    }
    const record = await createLockRecord("memory-write", {
      runId: `facts-${attempt}`,
    });
    return withLock(
      memoryWriterLockPath(this.options.identity.paths.locks),
      record,
      operation,
      {
        waitTimeoutMs: WRITER_WAIT_MS,
