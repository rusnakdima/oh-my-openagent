...
export async function buildMemorySnapshot(
  ...
  const [repoState, journal, health, timeline, consolidation] = await Promise
    .all([
  ...
  return {
    ...
    reflection: {
      ...
      ...(health.lastOutcome === undefined
        ? {}
        : { lastOutcome: health.lastOutcome }),
      consecutiveFailures: health.streak,
      ...(health.fingerprint === undefined
        ? {}
        : { lastFailureFingerprint: health.fingerprint }),
      ...(consolidation === undefined
        ? {}
        : { lastConsolidationAtISO: consolidation }),
    },
    journal: {
      sessionId,
...
type TimelineState = Pick<
  MemoryRpcSnapshot["repo"],
  "entriesToday" | "previousEntryAtISO"
>;

async function readMemoryRpcTimeline(
...
async function countEntriesToday(
  repo: MemoryRpcGitRepo,
...
async function readLastConsolidationAt(
  ...
  const finishedAts = await Promise.all(
    names.map((name) => mergedFinishedAt(join(dir, name))),
  );
  let newest: string | undefined;
  for (const finishedAt of finishedAts) {
    if (finishedAt === undefined) continue;
    if (newest === undefined || Date.parse(finishedAt) > Date.parse(newest)) {
      newest = finishedAt;
    }
  }
  return newest;
...
async function mergedFinishedAt(path: string): Promise<string | undefined> {
  try {
    ...
    if (record.outcome !== "merged" || typeof record.finishedAt !== "string") {
      return undefined;
    }
    return Number.isFinite(Date.parse(record.finishedAt))
      ? record.finishedAt
      : undefined;
  } catch {
    return undefined;
...
export async function readMemoryRpcRepoState(
  ...
  if (head === null) {
    return { dirty: dirtyPaths > 0, dirtyPaths, systemTokensEstimate: 0 };
  }
  ...
  return {
    ...
    ...(committedAt === null
      ? {}
      : { committedAtISO: new Date(committedAt * 1_000).toISOString() }),
    dirty: dirtyPaths > 0,
    dirtyPaths,
...
export function memoryTreeStats(
  entries: readonly GitTreeSizedEntry[],
): MemoryTreeStats {
...
export async function readMemoryRpcJournalState(
  ...
  try {
    const raw: unknown = JSON.parse(
      await readFile(
        join(context.identityPaths.transcripts, sessionId, "state.json"),
        "utf8",
      ),
    );
    ...
    return {
      ...
      pendingCompaction: state.pending_compaction === true ||
        context.ledger.pendingCompaction,
...
export async function readMemoryRpcHealth(
  context: MemoryIdentityContext,
): Promise<HealthState> {
  try {
...
export function createMemoryRpcGitRepo(
  repoPath: string,
): MemoryRpcGitRepo & MemoryRpcTreeRepo {
...
async function countCommitsSince(
  ...
  if (result.code !== 0) {
    throw new Error(`git rev-list failed: ${result.stderr.trim()}`);
  }
  const count = Number.parseInt(result.stdout.trim(), 10);
  if (!Number.isSafeInteger(count) || count < 0) {
    throw new Error("git rev-list returned no count");
  }
  return count;
}
