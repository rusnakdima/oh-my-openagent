...
export async function fixture(): Promise<
  { readonly factsDir: string; readonly locksDir: string }
> {
...
export async function seedTerminalRun(options: {
  ...
  if (options.bytes !== undefined) {
    await writeFile(
      join(runDir, "child-stdout.log"),
      "x".repeat(options.bytes),
      "utf8",
    );
  }
  return runDir;
...
export async function seedNonTerminalRun(
  factsDir: string,
  runId: string,
): Promise<string> {
...
export function instant(index: number): string {
  return new Date(Date.parse("2026-08-01T00:00:00.000Z") + index * 60_000)
    .toISOString();
}
...
export async function holdLock(lockPath: string): Promise<void> {
  const child = spawn(process.execPath, [holdLockFixture, lockPath], {
  ...
  await new Promise<void>((resolve, reject) => {
    const timeout = setTimeout(
      () => reject(new Error("lock holder never reported ready")),
      10_000,
    );
    let output = "";
...
export async function finalizeReserved(
  runDir: string,
  at: string,
): Promise<void> {
