...
describe("memory journal wiring", () => {
  test("#given a 3-turn session with a tool call #when agent_settled fires #then rows are projected and steps equal 3", async () => {
    ...
    expect(
      existsSync(join(paths.transcripts, "session-alpha", "transcript.jsonl")),
    ).toBe(true);
  });
  ...
  test("#given turn 1 journaled before a crash #when session_start reconciles the full branch #then only missing rows are appended", async () => {
    ...
    const turn2 = [
      userEntry("u2", "second question"),
      assistantEntry("a2", { texts: ["answer two"] }),
    ];
    ...
    const recovered = await pi2.dispatch(
      "session_start",
      {},
      sessionCtx([...turn1, ...turn2]),
    );
    ...
    const settledAgain = await pi2.dispatch(
      "agent_settled",
      {},
      sessionCtx([...turn1, ...turn2]),
    );
  ...
  test("#given redacted and visible thinking blocks #when settled #then reasoning rows use the placeholder and source index", async () => {
    ...
    const entries = [
      ...
      assistantEntry("a1", {
        texts: ["done"],
        thinkings: [{ redacted: true }, "visible thought"],
      }),
    ];
    ...
    expect(rows[2]).toMatchObject({
      ...
      source_line_id: "a1:reasoning:0",
    });
    expect(rows[3]).toMatchObject({
      text: "visible thought",
      source_line_id: "a1:reasoning:1",
    });
  });
  ...
  test("#given an errored tool result #when settled #then the tool_call row joins resultOk false", async () => {
    ...
    const entries = [
      ...
      assistantEntry("a1", {
        toolCalls: [{ id: "c1", name: "read", args: { path: "/missing" } }],
      }),
  ...
  test("#given a malformed event context #when handlers fire #then the reconcile is a no-op", async () => {
    ...
    expect(await pi.dispatch("agent_settled", {}, undefined)).toEqual([{
      appended: 0,
      skipped: 0,
    }]);
    expect(await pi.dispatch("agent_settled", {}, { sessionManager: {} }))
      .toEqual([{ appended: 0, skipped: 0 }]);
  });

  test(
    "#given the journal lock is contended by a live foreign owner #when agent_settled fires #then the handler degrades to a no-op and warns instead of rejecting",
    async () => {
      ...
      try {
        ...
        const entries = [
          ...
          assistantEntry("a1", { texts: ["answer"] }),
        ];
        ...
        const result = await pi.dispatch(
          "agent_settled",
          {},
          sessionCtx(entries, "session-contended"),
        );
