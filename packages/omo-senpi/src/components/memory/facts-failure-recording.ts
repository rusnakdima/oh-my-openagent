...
export function queueEntryTargets(
  entries: readonly FactsQueueEntry[],
): readonly FactsFailureTarget[] {
...
export function ledgerTargets(
  queued: readonly FactsQueuedKey[],
): readonly FactsFailureTarget[] {
  return queued.map((key) => ({
    conversationId: key.conversationId,
    endMessageId: key.end_message_id,
    endSnapshotLine: typeof key.end_snapshot_line === "number"
      ? key.end_snapshot_line
      : 0,
  }));
}

/** Failures raised before a run dir exists have no runId to key idempotency on. */
export function preflightFailureId(
