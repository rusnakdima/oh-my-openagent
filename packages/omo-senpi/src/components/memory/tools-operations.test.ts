...
describe("memory tool execution", () => {
  test(
    "#given a bound identity #when memory create executes #then the engine commits and reports the short sha",
    async () => {
      // given
      ...
      const written = parseMemoryFile(
        await readFile(
          join(fixture.repo.dir, "system/human/prefs/coding.md"),
          "utf8",
        ),
      );
      expect(written.frontmatter.description).toBe(
        "The user's coding preferences.",
      );
      expect(written.body).toBe("Adds type hints to all Python code.");
      expect(await git(fixture.repo, ["log", "-1", "--format=%s"])).toBe(
        "Track coding preferences",
      );
  ...
  test(
    "#given a bound identity #when the engine rejects the change #then the memory error maps to an error result",
    async () => {
      // given
      const fixture = await boundFixture();
  ...
  test(
    "#given no repo exists yet #when the memory tool executes #then it lazily initializes with hooks and seeds",
    async () => {
      // given
      const root = realpathSync.native(
        await mkdtemp(join(tmpdir(), "omo-senpi-memory-lazy-init-")),
      );
      roots.push(root);
      const identityPaths = buildIdentityPaths(root, IDENTITY);
      const binding = createMemoryBinding({
        identity: IDENTITY,
      ...
      expect(await git(repo, ["show", "HEAD~1:system/persona.md"])).toContain(
        "description:",
      );
      const { existsSync } = await import("node:fs");
      expect(
        existsSync(join(identityPaths.repo, ".git", "hooks", "pre-commit")),
      ).toBe(true);
      expect(
        existsSync(join(identityPaths.repo, ".git", "hooks", "post-commit")),
      ).toBe(true);
    },
    30_000,
  );
...
describe("memory tool onCommit seam", () => {
  test(
    "#given a bound identity #when memory create commits #then onCommit receives the sha, first-line subject, and affected paths",
    async () => {
      // given
      const fixture = await boundFixture();
      const commits: Array<
        { sha: string; subject: string; affectedPaths: readonly string[] }
      > = [];
      const [memoryTool] = createMemoryTools(() => fixture.context, {
  ...
  test(
    "#given an engine rejection #when the tool executes #then onCommit is not called",
    async () => {
      // given
      const fixture = await boundFixture();
      const commits: Array<
