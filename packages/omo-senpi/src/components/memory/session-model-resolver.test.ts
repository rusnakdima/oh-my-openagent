...
describe("resolveMemorySessionModel", () => {
  describe("#given a senpi event context carrying the active model", () => {
    test("#when resolved #then it yields the provider and id", () => {
      ...
      // when / then
      expect(resolveMemorySessionModel(eventCtx)).toEqual({
        provider: "anthropic",
        id: "claude-opus-5",
      });
    });
  ...
  describe("#given a context that cannot supply a model", () => {
    test("#when the model is absent, malformed, or the context is not a record #then it resolves to undefined", () => {
      ...
      expect(resolveMemorySessionModel({ model: { provider: "anthropic" } }))
        .toBeUndefined();
      expect(resolveMemorySessionModel({ model: { id: "claude-opus-5" } }))
        .toBeUndefined();
      expect(resolveMemorySessionModel({ model: { provider: "", id: "x" } }))
        .toBeUndefined();
      expect(resolveMemorySessionModel({ model: { provider: "x", id: "" } }))
        .toBeUndefined();
      expect(resolveMemorySessionModel({ model: { provider: 1, id: 2 } }))
        .toBeUndefined();
    });
  });
});
