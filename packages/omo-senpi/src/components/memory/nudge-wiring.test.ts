...
async function fixture() {
  const root = realpathSync.native(
    await mkdtemp(join(tmpdir(), "omo-memory-nudge-")),
  );
  roots.push(root);
...
function eventContext(
  sessionId: string,
  entries: readonly SessionEntryFixture[] = [],
) {
  return {
    sessionManager: {
...
function disposition(
  inputId: string,
  value: "handled" | "queued" | "started" | "rejected",
) {
...
describe("createMemoryNudgeWiring", () => {
  test(
    "#given a fresh session #when two interactive inputs are accepted while extension and rejected inputs occur #then only the accepted interactive turns reach the threshold",
    async () => {
      // given
      ...
      await pi.dispatch(
        "input_disposition",
        disposition("first", "started"),
        ctx,
      );
      const beforeThreshold = await wiring.nudgeTurns(
        repo,
        "session-1",
        context.identity,
      );
      ...
      await pi.dispatch(
        "input_disposition",
        disposition("second", "queued"),
        ctx,
      );

      // then
      expect(beforeThreshold).toBeUndefined();
      expect(await wiring.nudgeTurns(repo, "session-1", context.identity)).toBe(
        2,
      );
      expect(pi.handlers.map((handler) => handler.event)).not.toContain(
        "agent_settled",
      );
  ...
  test(
    "#given a durable accepted-turn record and contaminated branch users #when a session resumes #then hydration uses only the durable baseline",
    async () => {
      // given
      const { context, repo } = await fixture();
      const pi = new MemoryFakeExtensionAPI();
      ...
      // then
      expect(await wiring.nudgeTurns(repo, "resumed", context.identity)).toBe(
        2,
      );
      expect(wiring.provenance("resumed")).toEqual({
        sessionId: "resumed",
        userTurns: 8,
      });
    },
  ...
  test(
    "#given one durable accepted turn and a second admitted input awaiting its started disposition #when the prompt compiles #then the projected accepted turn reaches the threshold",
    async () => {
      // given
      const { context, repo } = await fixture();
      const pi = new MemoryFakeExtensionAPI();
      ...
      // when
      const turns = await wiring.nudgeTurns(
        repo,
        "session-order",
        context.identity,
      );
  ...
  test(
    "#given a threshold on an identity with no repository yet #when nudge state resolves #then the fresh-session baseline is used without requiring git history",
    async () => {
      // given
      const { context } = await fixture();
      await rmEfaultTolerant(context.identityPaths.repo, {
        recursive: true,
        force: true,
        maxRetries: 10,
        retryDelay: 200,
      ...
      const repo = new GitMemoryRepo({
        dir: context.identityPaths.repo,
  ...
  test(
    "#given a threshold nudge #when an in-band memory commit lands #then the next compile state self-clears and disabled settings remain quiet",
    async () => {
      // given
      const { context, repo } = await fixture();
      const pi = new MemoryFakeExtensionAPI();
      ...
      await pi.dispatch("session_start", {}, ctx);
      expect(await wiring.nudgeTurns(repo, "session-clear", context.identity))
        .toBe(2);
      ...
      const cleared = await wiring.nudgeTurns(
        repo,
        "session-clear",
        context.identity,
      );
  ...
  test(
    "#given a bound non-auto identity #when a memory MCP tool call starts #then unforgeable identity and accepted-turn provenance are injected in place",
    async () => {
      // given
      const { context } = await fixture();
      const pi = new MemoryFakeExtensionAPI();
  ...
  test(
    "#given a bound identity #when an MCP-surface memory tool call starts #then provenance is injected under the catalog tool names",
    async () => {
      // given
      const { context } = await fixture();
      const pi = new MemoryFakeExtensionAPI();
      ...
      const call: {
        ...
        toolName: "mcp_omo-memory_memory_apply_patch",
        input: { reason: "save" },
      };
...
function acceptedTurns(
  over: Partial<AcceptedTurnsRecord> = {},
): AcceptedTurnsRecord {
  return {
    version: 1,
    sessionId: "session-1",
...
function renderAccepted(
  record: AcceptedTurnsRecord,
  expanded = false,
): string[] {
  return renderAcceptedTurnsEntry(
...
describe("renderAcceptedTurnsEntry", () => {
  ...
  test("#given a single accepted turn #when it renders #then the turn noun is singular", () => {
    // when
    const lines = renderAccepted(
      acceptedTurns({ priorUserTurns: 1, sessionBaselineTurns: 0 }),
    );
  ...
  test("#when rendered with a background theme #then every padded line carries customMessageBg", () => {
    const component = renderAcceptedTurnsEntry(
      { data: acceptedTurns() } as never,
      { expanded: false },
      {
        fg: (_color: ThemeColor, text: string) => text,
        bg: (_color: "customMessageBg", text: string) =>
          `<notice-bg>${text}</notice-bg>`,
      } as never,
    );
    for (const line of component!.render(120)) {
      expect(line).toMatch(/^<notice-bg>.*<\/notice-bg>$/u);
    }
  });
  ...
  test("#when coloured #then the title is muted and the secondary rows are dim", () => {
    ...
    // when
    renderAcceptedTurnsEntry({ data: acceptedTurns() } as never, {
      expanded: true,
    }, recorder.theme as never)!.render(120);
