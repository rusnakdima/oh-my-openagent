...
import {
  ...
  onlyRunDir,
  runnerOptions,
} from "./facts-runner.test-support";
...
describe("quick-pinned facts launch", () => {
  test(
    "#given quick cannot resolve #when pending facts are launched #then no child spawns and the queue stays intact with one warning",
    async () => {
      // given
  ...
  test(
    "#given one injected launch-preflight seam #when reflection and facts launch #then both surfaces route through it",
    async () => {
      // given
      ...
      const { root, identity, queue } = await fixture();
      const facts = new FactsExtractorRunner(
        runnerOptions(root, identity, queue, "fact", {
          resolveAndPreflightLaunch: observe,
        }),
      );
  ...
  test(
    "#given two pending queue entries #when one launch runs #then the supervised child consumes all entries in one trailer-bearing commit",
    async () => {
      // given
      const { root, identity, queue } = await fixture();
      await enqueue(
      ...
      expect(commit?.trailers["Omo-Facts-Batch"]).toBe(
        "11111111-1111-4111-8111-111111111111",
      );
      expect(
        await readFile(
          join(identity.paths.repo, "notes/facts/2026-08.md"),
          "utf8",
        ),
      ).toContain("fixture consumed 2 queue entries");
      const runDir = await onlyRunDir(identity);
      const ledger = JSON.parse(
        await readFile(join(runDir, "ledger.json"), "utf8"),
      );
      expect(JSON.parse(await readFile(join(runDir, "final.json"), "utf8")))
        .toMatchObject({ outcome: "committed" });
      expect(JSON.parse(await readFile(join(runDir, "outcome.json"), "utf8")))
        .toMatchObject({ childExit: { code: 0 } });
      ...
      expect(
        ledger.applyRecovery.paths.map((entry: { path: string }) => entry.path),
      ).toEqual(
        [...ledger.applyRecovery.paths].map((entry: { path: string }) =>
          entry.path
        ).sort(),
      );
    },
  ...
  test(
    "#given an extension-only quick primary and a child-visible fallback #when facts extraction launches #then it retries and commits with the fallback",
    async () => {
      // given
      const { root, identity, queue } = await fixture();
      ...
      const final = JSON.parse(
        await readFile(join(runDir, "final.json"), "utf8"),
      );

      // then
      expect({ status: result.status, attempted, detail: final.detail })
        .toEqual({
          status: "committed",
  ...
  test(
    "#given facts attempt two has a stale attempt-one outcome #when reconciled before the shared deadline #then the retry remains active",
    async () => {
      // given
      const { root, identity, queue } = await fixture();
  ...
  test(
    "#given a commit lands before queue cleanup crashes #when a fresh runner reconciles #then the batch receipt prevents a duplicate commit",
    async () => {
      // given
      const { root, identity, queue } = await fixture();
      class FlakyQueue extends FactsQueue {
        private fail = true;
        override async markConsumed(
          entries: Parameters<FactsQueue["markConsumed"]>[0],
        ): Promise<void> {
          if (this.fail) {
            this.fail = false;
      ...
      const first = new FactsExtractorRunner(
        runnerOptions(root, identity, flaky, "fact"),
      );
      await expect(first.launchPending()).rejects.toThrow(
        "injected cleanup crash",
      );
      const runDir = await onlyRunDir(identity);
      expect(
        await readFile(join(runDir, "final.json"), "utf8").catch(() =>
          undefined
        ),
      ).toBeUndefined();
      await writeFile(
        join(identity.paths.repo, "foreign.md"),
        "parent bytes\n",
      );

      // when: receipt probing must win before dirty-state recovery classification
      const recovered = new FactsExtractorRunner(
        runnerOptions(root, identity, queue, "fact"),
      );
      ...
      expect(
        (await repo.log()).filter((commit) =>
          commit.trailers["Omo-Facts-Batch"] !== undefined
        ),
      ).toHaveLength(1);
      expect(await readFile(join(identity.paths.repo, "foreign.md"), "utf8"))
        .toBe("parent bytes\n");
      expect(JSON.parse(await readFile(join(runDir, "final.json"), "utf8")))
        .toMatchObject({ outcome: "committed" });
    },
  ...
  test(
    "#given a legacy dirty run without an envelope #when retried after cleanup #then it fails closed, retains queue watermarks, and later commits",
    async () => {
      const { root, identity, queue } = await fixture();
      ...
      const consumedBefore = await readFile(paths.consumedPath, "utf8").catch(
        () => undefined,
      );
      ...
      expect(await readFile(paths.consumedPath, "utf8").catch(() => undefined))
        .toBe(consumedBefore);
      const firstRun = await onlyRunDir(identity);
      expect(JSON.parse(await readFile(join(firstRun, "final.json"), "utf8")))
        .toMatchObject({ outcome: "parent_dirty" });
      expect(
        JSON.parse(await readFile(join(firstRun, "ledger.json"), "utf8"))
          .applyRecovery,
      ).toBeUndefined();
      ...
      const retried = await new FactsExtractorRunner(
        runnerOptions(root, identity, queue, "fact", {
      ...
      expect(await queue.listPending()).toHaveLength(0);
      expect((await readdir(join(identity.paths.facts, "runs"))).length).toBe(
        2,
      );
    },
    30_000,
  ...
  test(
    "#given a valid empty extraction #when finalized #then no commit lands and the queue is consumed as no_facts",
    async () => {
      // given
      const { root, identity, queue } = await fixture();
      const runner = new FactsExtractorRunner(
        runnerOptions(root, identity, queue, "empty"),
      );
      ...
      const runDir = await onlyRunDir(identity);
      expect(JSON.parse(await readFile(join(runDir, "final.json"), "utf8")))
        .toMatchObject({ outcome: "no_facts" });
    },
    30_000,
  );

  test(
    "#given a schema-invalid project record carrying person #when finalized #then the queue is retained",
    async () => {
      // given
      const { root, identity, queue } = await fixture();
      const runner = new FactsExtractorRunner(
        runnerOptions(root, identity, queue, "malformed"),
      );
