...
function orderingProbe(runsDir: string) {
  const observed: {
    ...
    readonly sentinelExisted: boolean;
  }[] = [];
    ...
    recordFailure: async (request) => {
      ...
      observed.push({
        ...
        sentinelExisted: names.some((name) =>
          ...
          existsSync(join(runsDir, name, "abandoned.json"))
        ),
      });
    },
...
async function ledgerBatchId(runDir: string): Promise<string> {
  return JSON.parse(await readFile(join(runDir, "ledger.json"), "utf8"))
    .batchId;
}

describe("facts terminal failure recording", () => {
  test(
    "#given a child that exits non-zero #when the run finalizes #then one streak entry is recorded for the queued endpoint",
    async () => {
      ...
      const runner = new FactsExtractorRunner(
        runnerOptions(root, identity, queue, "fail"),
      );
  ...
  test(
    "#given a failing run #when the failure store is written #then no terminal sentinel exists yet",
    async () => {
      ...
      expect(probe.observed).toEqual([{
        failureId: await ledgerBatchId(runDir),
  ...
  test(
    "#given finalize crashed after the failure record #when reconcile replays the same runId #then the streak stays at one",
    async () => {
      // given: the record landed, the sentinel did not - exactly the crash window the ordering buys.
      const { root, identity, queue } = await fixture();
      const crashing = new FactsExtractorRunner(
        runnerOptions(root, identity, queue, "fail", {
          writeTerminalSentinel: async () => {
            throw new Error(
              "injected crash between failure record and sentinel",
            );
          },
        }),
      );
      ...
      const store = new FactsFailureStore({ identityPaths: identity.paths });
      expect((await store.readFailures()).entries[0]).toMatchObject({
        streak: 1,
      });

      // when: a fresh runner reconciles the same run dir, replaying the same runId
      await reconcileOnly(
        new FactsExtractorRunner(
          runnerOptions(root, identity, queue, "fail", {
      ...
      expect(state.entries[0]).toMatchObject({
        streak: 1,
        lastFailureId: await ledgerBatchId(runDir),
      });
    },
    30_000,
  );
  ...
  test(
    "#given a run of unknown liveness past its deadline #when reconcile abandons it #then the failure record precedes the sentinel",
    async () => {
      // given
      const { root, identity, queue } = await fixture();
      ...
      expect(probe.observed[0]).toEqual({
        failureId: ABANDONED_BATCH_ID,
        sentinelExisted: false,
      });
    },
    30_000,
  ...
  test(
    "#given a real store #when a run is abandoned #then the queued endpoint carries the unknown_liveness reason",
    async () => {
      // given
      const { root, identity, queue } = await fixture();
      ...
      // when
      await reconcileOnly(
        new FactsExtractorRunner(runnerOptions(root, identity, queue, "fail", {
          now: () => new Date("2026-08-11T12:00:00.000Z"),
        })),
      );

      // then
      const state = await new FactsFailureStore({
  ...
  test(
    "#given a parent_dirty finalize #when the run ends #then the endpoint is recorded with the parent_dirty reason",
    async () => {
      // given
      const { root, identity, queue } = await fixture();
      let dirty = true;
  ...
  test(
    "#given a recorded failure #when a later run commits #then the endpoint's record is cleared after consumption",
    async () => {
      // given
      const { root, identity, queue } = await fixture();
      const store = new FactsFailureStore({
        identityPaths: identity.paths,
        now: () => RECORDED_AT,
      });
  ...
  test(
    "#given a queued endpoint with a stale record #when the run yields no facts #then the record is cleared too",
    async () => {
      // given
      const { root, identity, queue } = await fixture();
      const store = new FactsFailureStore({
  ...
  test(
    "#given the quick category cannot resolve #when a launch is attempted #then a preflight-scoped failure is recorded",
    async () => {
      // given
      const { root, identity, queue } = await fixture();
      const runner = new FactsExtractorRunner({
      ...
      expect(state.entries[0]).toMatchObject({
        streak: 1,
        lastReason: "quick_category_unavailable",
      });
      expect(state.entries[0]?.lastFailureId).toMatch(
        /^preflight-[0-9a-f-]{36}$/,
      );
  ...
  test(
    "#given an aborted drain #when launch is refused before reservation #then nothing is recorded",
    async () => {
      // given
      const { root, identity, queue } = await fixture();
      ...
      // then
      expect(result.status).toBe("skipped");
      expect(existsSync(factsQueuePaths(identity.paths).failuresPath)).toBe(
        false,
      );
    },
    30_000,
  ...
  test(
    "#given five consecutive failing runs #when each finalizes #then the endpoint parks with a null eligibility",
    async () => {
      // given
      const { root, identity, queue } = await fixture();
      const store = new FactsFailureStore({ identityPaths: identity.paths });
  ...
  test(
    "#given retention keeps freeing the attempt name #when six launches fail #then the streak still parks the endpoint",
    async () => {
      ...
      for (let cycle = 1; cycle <= 6; cycle += 1) {
        ...
        expect(await readdir(join(identity.paths.facts, "runs"))).toEqual([
          `facts-busier-${cycle}`,
        ]);
      }
  ...
  test(
    "#given a run ledger #when the run reserves its directory #then queued endpoints carry their snapshot boundary",
    async () => {
      // given
      const { root, identity, queue } = await fixture();
      const runner = new FactsExtractorRunner(
