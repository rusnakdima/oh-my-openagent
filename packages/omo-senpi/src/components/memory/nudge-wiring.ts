...
export const renderAcceptedTurnsEntry: EntryRenderer<AcceptedTurnsRecord> = (
  entry,
  options,
  theme,
) => {
...
export function createMemoryNudgeWiring(
  options: MemoryNudgeWiringOptions,
): MemoryNudgeWiring {
  ...
  return {
    register(pi): void {
      ...
      pi.on("input", (payload, eventCtx) => {
        if (
          !isRecord(payload) || payload.type !== "input" ||
          payload.source === "extension"
        ) return;
        if (
          typeof payload.inputId !== "string" || payload.inputId.length === 0
        ) return;
        const sessionId = readSessionId(eventCtx);
        if (sessionId !== undefined) {
          pendingInputs.set(payload.inputId, sessionId);
        }
      });
      ...
      pi.on("input_disposition", (payload) => {
        ...
        if (
          payload.disposition !== "queued" && payload.disposition !== "started"
        ) return;
      ...
      pi.on("tool_call", (payload, eventCtx) => {
        if (
          !isRecord(payload) || !isMemoryToolName(payload.toolName) ||
    ...
    async nudgeTurns(repo, sessionId, identity): Promise<number | undefined> {
      ...
      const savedAt = lastSave === undefined
        ...
        : parseTurn(lastSave.trailers["Omo-Turn"]) ??
          state.sessionBaselineTurns;
      const pendingTurn =
        [...pendingInputs.values()].some((pendingSessionId) =>
    ...
    provenance(sessionId): MemoryToolProvenance | undefined {
      ...
      return state === undefined
        ? undefined
        : { sessionId, userTurns: state.priorUserTurns };
    },
  };
}
...
function findLatestAcceptedTurns(
  ...
  for (let index = entries.length - 1; index >= 0; index -= 1) {
    ...
    if (
      !isRecord(entry) || entry.type !== "custom" ||
      entry.customType !== ACCEPTED_TURNS_ENTRY_TYPE
    ) continue;
    if (
      isAcceptedTurnsRecord(entry.data) && entry.data.sessionId === sessionId
    ) return entry.data;
  }
  return undefined;
}
...
function readSession(
  eventCtx: unknown,
): { id: string; entries: readonly unknown[] } | undefined {
  ...
  if (typeof getSessionId !== "function" || typeof getEntries !== "function") {
    return undefined;
  }
  const id = Reflect.apply(getSessionId, manager, []);
  const entries = Reflect.apply(getEntries, manager, []);
  return typeof id === "string" && id.length > 0 && Array.isArray(entries)
    ? { id, entries }
    : undefined;
}
