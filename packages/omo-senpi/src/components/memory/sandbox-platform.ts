...
export function buildPathSandboxTransform<
  T extends ReflectionSpawnArgs | FactsSpawnArgs,
>(
  ...
  if (executable === undefined) {
    ...
    return identityTransform(
      `${input.surface} sandbox unavailable on ${platform}: ${reason}; running unsandboxed because policy is auto`,
    );
  }
  ...
  if (platform !== "darwin" && lockPaths.length > 0) {
    const reason =
      `${platform} sandbox cannot grant the agent lockfile paths (${
        lockPaths.join(", ")
      }); bwrap has no minimal grant for entries that do not exist yet`;
    if (input.policy === "required") {
      const error = new SandboxUnavailableError(platform, reason);
      if (input.errorRethrow !== undefined) input.errorRethrow(error);
      throw error;
  ...
  if (platform === "darwin") {
    ...
    if ("warning" in lockPathsResolution) {
      return identityTransform(lockPathsResolution.warning);
    }
    const tempDir = join(
      dirname(payloads[0] ?? canonicalPath(input.fallbackDir)),
      ".sandbox-tmp",
    );
    ...
    return guardedSandboxedTransform(
      input.surface,
      input.command,
      input.env,
      (spawnArgs, innerCommand) => ({
        ...spawnArgs,
        command: executable,
...
function buildDarwinProfile(input: {
  ...
  return [
    ...
    ...input.lockPaths.map((path) =>
      `(allow file-write* (literal ${seatbeltString(path)}) (subpath ${
        seatbeltString(path)
      }))`
    ),
    '(allow file-write* (literal "/dev/null"))',
    '(allow file-write* (literal "/dev/tty"))',
    ...input.payloads.map((path) =>
      `(allow file-read* (literal ${seatbeltString(path)}))`
    ),
    ...input.foreignRoots.map((path) =>
...
function defaultWhich(command: string): string | undefined {
  ...
  if (command === "sandbox-exec" && existsSync("/usr/bin/sandbox-exec")) {
...
function canonicalPath(path: string): string {
  ...
  for (;;) {
    if (existsSync(current)) {
      return join(realpathSync(current), ...segments.reverse());
    }
...
function resolveLockPaths(input: {
  ...
  const reason = `lock parent directories do not exist (${
    absentParents.join(", ")
  })`;
  if (input.policy === "required") {
  ...
  return {
    warning:
      `${input.surface} sandbox unavailable on ${input.platform}: ${reason}; running unsandboxed because policy is auto`,
  };
}
...
function guardedSandboxedTransform<
  T extends ReflectionSpawnArgs | FactsSpawnArgs,
>(
  ...
  if (innerCommand === undefined) {
    return identityTransform(
      `${surface} sandbox unavailable: inner command "${command}" is not absolute and could not be resolved; running unsandboxed`,
    );
  }
  return Object.assign((spawnArgs: T) => transform(spawnArgs, innerCommand), {
    wasSandboxed: true,
  });
}

function resolveInnerCommand(
  command: string,
  env: NodeJS.ProcessEnv,
): string | undefined {
