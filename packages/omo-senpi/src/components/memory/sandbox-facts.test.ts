...
describe("facts worker OS sandbox", () => {
  test(
    "#given Darwin with sandbox-exec available #when facts spawn arguments are transformed #then the wrapper keeps the inner command and grants the run directory",
    async () => {
      // given
      const setup = fixture();
      const transform = buildFactsSandboxTransform({
      ...
      expect(transformed.args[1] ?? "").toContain(
        `(allow file-write* (subpath ${
          JSON.stringify(realpathSync(setup.spawnArgs.paths.runDir))
        }))`,
      );
  ...
  test(
    "#given a missing facts inner command #when the transform is built #then its warning names the facts surface",
    async () => {
      // given
      const setup = fixture();
...
describe("facts sandbox agent lockfile grants", () => {
  test(
    "#given an agent dir whose lock directories do not exist yet #when the facts Darwin profile is built #then it renders both literal and subpath write grants for each lock path",
    async () => {
      // given
      const setup = fixture();
      ...
      expect(profile).toContain(
        `(allow file-write* (literal ${
          JSON.stringify(settingsLock)
        }) (subpath ${JSON.stringify(settingsLock)}))`,
      );
      expect(profile).toContain(
        `(allow file-write* (literal ${JSON.stringify(authLock)}) (subpath ${
          JSON.stringify(authLock)
        }))`,
      );
      expect(existsSync(settingsLock)).toBe(false);
      expect(existsSync(authLock)).toBe(false);
  ...
  test(
    "#given the facts Darwin profile #when its write grants are inspected #then the bare agent dir is never writable",
    async () => {
      // given
      const setup = fixture();
      ...
      expect(profile).not.toContain(
        JSON.stringify(join(setup.agentDirReal, "auth.json")),
      );
      expect(profile).not.toContain(
        JSON.stringify(join(setup.agentDirReal, "settings.json")),
      );
    },
    30_000,
  );
  ...
  test(
    "#given the facts Darwin profile #when the run directory grant is inspected #then the run dir stays the only writable subtree beside the lock paths and the sandbox temp dir",
    async () => {
      // given
      const setup = fixture();
      ...
      const subpathOnlyGrants = profile.split("\n").filter((line) =>
        line.startsWith("(allow file-write* (subpath ")
      );
      expect(subpathOnlyGrants).toEqual([
        `(allow file-write* (subpath ${JSON.stringify(runDirReal)}))`,
        `(allow file-write* (subpath ${
          JSON.stringify(join(runDirReal, ".sandbox-tmp"))
        }))`,
      ]);
    },
    30_000,
  ...
  test(
    "#given Linux with bwrap available and required policy #when lock grants are requested #then a typed unavailable error names the lock-grant gap",
    () => {
      // given
      const setup = fixture();
      const transform = buildFactsSandboxTransform({
  ...
  test(
    "#given Linux with bwrap available and auto policy #when lock grants are requested #then spawn arguments pass through with an explicit degradation warning",
    async () => {
      // given
      const setup = fixture();
      let warning: string | undefined;
  ...
  test.skipIf(
    ...
    async () => {
      ...
      const grantedRun = await transform({
        ...
        args: [
          "-c",
          `mkdir '${authLock}' && printf lock > '${authLock}/owner' && rm -rf '${authLock}'`,
        ],
      });
      const granted = Bun.spawnSync([grantedRun.command, ...grantedRun.args], {
        cwd: grantedRun.cwd,
        env: grantedRun.env,
      });
      ...
      const denied = Bun.spawnSync([deniedRun.command, ...deniedRun.args], {
        cwd: deniedRun.cwd,
        env: deniedRun.env,
      });
