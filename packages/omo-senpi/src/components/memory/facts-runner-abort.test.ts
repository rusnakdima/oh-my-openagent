...
describe("facts runner shutdown abort boundary", () => {
  test(
    "#given an aborted drain signal #when launch is attempted #then no run is reserved, no child spawns, and the batch stays retryable",
    async () => {
      // given
  ...
  test(
    "#given the abort fires at the batch-id hook mid-composite #when launch proceeds #then the reservation boundary refuses to start",
    async () => {
