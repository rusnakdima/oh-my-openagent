...
function state(
  overrides: Partial<ReflectionTranscriptState> = {},
...
describe("dream conversation scoring", () => {
  ...
  test("#given three equal scores #when ranked #then unreflected steps recency and conversation id break ties in order", () => {
    ...
    // then
    expect(ranked.map((candidate) => candidate.conversationId)).toEqual([
      "zeta",
      "alpha",
      "beta",
    ]);
...
describe("dream conversation packing", () => {
  test("#given an oversized middle candidate #when packed #then it is skipped and a later fitting candidate is selected", () => {
    // given
    const ranked = [
      scored("first", 10, 0, 0, 6),
      scored("oversized", 9, 0, 0, 11),
      scored("later", 8, 0, 0, 4),
    ];

    // when
    const result = packDreamCandidates(ranked, {
      maxConversations: 3,
      maxBytes: 10,
    });
    const countCapped = packDreamCandidates(ranked, {
      maxConversations: 1,
      maxBytes: 10,
    });
...
describe("dream journal selection", () => {
  for (const kind of ["reasoning", "error", "tool_call"] as const) {
    test(`#given a stale conversation with a newer ${kind} tail #when recent selection runs #then canonical activity wins`, async () => {
      ...
      const root = realpathSync.native(
        await mkdtemp(join(tmpdir(), "dream-selector-recency-")),
      );
      tempDirs.push(root);
  ...
  test("#given a stale conversation with a newer contextual tail #when automatic selection is capped #then canonical activity wins", async () => {
    // given
    const root = realpathSync.native(
      await mkdtemp(join(tmpdir(), "dream-selector-auto-recency-")),
    );
    tempDirs.push(root);
    const transcriptsDir = join(root, "transcripts");
    await writeJournal(transcriptsDir, "stale-with-tail", [
  ...
  test("#given UTF-8 journals with one fully reflected conversation #when volume and selection run twice #then bytes snapshots ranking and output stay deterministic", async () => {
    // given
    const root = realpathSync.native(
      await mkdtemp(join(tmpdir(), "dream-selector-")),
    );
    tempDirs.push(root);
    const transcriptsDir = join(root, "transcripts");
    const alpha = [
    ...
    const beta = [
      textEntry("beta", 0, "already reflected", "2026-08-10T10:00:00.000Z"),
    ];
    const gamma = [
      textEntry("gamma", 0, "focus later", "2026-08-10T09:00:00.000Z"),
    ];
    await writeJournal(
      transcriptsDir,
      "gamma",
      gamma,
      state({ steps_since_last_successful_reflection: 10 }),
    );
    await writeJournal(
      transcriptsDir,
      "alpha",
      alpha,
      state({ steps_since_last_successful_reflection: 20 }),
    );
    await writeJournal(
      transcriptsDir,
      "beta",
    ...
    expect(first).toEqual({
      conversationIds: ["gamma", "alpha"],
      totalBytes: alphaBytes + gammaBytes,
    });
