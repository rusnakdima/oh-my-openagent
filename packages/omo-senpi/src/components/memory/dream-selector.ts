...
export async function selectDreamConversations(
  ...
  return selectLoadedConversations(
    await loadConversations(options.transcriptsDir),
    options,
    mode,
  );
}
...
export async function selectRecentDreamConversations(
  ...
  const recent = (await loadConversations(options.transcriptsDir))
    .sort((left, right) => {
      const activity = Date.parse(right.lastActivity) -
        Date.parse(left.lastActivity);
      return activity === 0
        ? compareConversationIds(left.conversationId, right.conversationId)
        : activity;
    })
    .slice(0, recentN);
...
function selectLoadedConversations(
  ...
  const candidates = conversations.map(
    (conversation): DreamScoredConversation => {
      const scored = scoreDreamCandidate({
        searchMatchCount: countSearchMatches(conversation, mode.focus),
        stepsSinceLastSuccessfulReflection:
          conversation.stepsSinceLastSuccessfulReflection,
        lastActivity: conversation.lastActivity,
        distinctSourceCount: new Set(conversation.entries.map((entry) =>
          entry.source_message_id
        )).size,
        totalBytes: conversation.totalBytes,
        targetBytes,
        isCurrent:
          conversation.conversationId === options.currentConversationId,
        newestMessageCovered: conversation.reflectedThroughMessageId ===
...
async function loadConversations(
  ...
  const names = entries.filter((entry) => entry.isDirectory()).map((entry) =>
    entry.name
  ).sort(compareConversationIds);
  const conversations = await Promise.all(
    names.map(async (conversationId): Promise<LoadedConversation | null> => {
      const journalDir = join(transcriptsDir, conversationId);
      const journalEntries = await new TranscriptJournal({ journalDir })
        .readEntries();
  ...
  return conversations.filter((
    conversation,
  ): conversation is LoadedConversation => conversation !== null);
}
...
async function readReflectionState(
  ...
  const value = parsed && typeof parsed === "object"
    ...
    : {};
  const reflectedThrough =
    typeof value.reflected_through_message_id === "string" &&
  ...
  return deriveState({
    ...initialReflectionState(),
    ...(reflectedThrough === undefined
      ? {}
      : { reflected_through_message_id: reflectedThrough }),
...
function conversationByteLength(entries: readonly TranscriptEntry[]): number {
  const text = entries
    .map((entry) => {
      ...
      if (entry.kind === "tool_call" && entry.argsText !== undefined) {
        fields.push(entry.argsText);
      }
      if (entry.kind === "tool_call" && entry.resultText !== undefined) {
        fields.push(entry.resultText);
      }
      return fields.join("\n");
    })
  ...
  return searchTranscripts(
    {
      listConversations: () => [{
        ...
        messages: searchDocuments(conversation),
      }],
    },
    focus,
...
function searchDocuments(conversation: LoadedConversation): SearchDocument[] {
  ...
  return [...grouped].map(([messageId, entries]) => {
    ...
    return {
      ...
      content,
      reasoning,
      toolCalls: tools.map((entry) => ({
        name: entry.name,
        arguments: entry.argsText,
      })),
      toolReturn: tools.map((entry) => entry.resultText).filter((text) =>
        text !== undefined
      ).join("\n"),
    };
  });
