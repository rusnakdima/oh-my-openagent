...
import {
  type EntryRenderTheme,
  joinFields,
  noticeComponent,
} from "./worker/entry-renderers";
...
export function createMemoryWriteRenderResult(
  ...
  return (result, options, theme, context) => {
    ...
    return renderMemoryWriteNotice(
      notice,
      options,
      theme,
      (deps.now ?? Date.now)(),
  ...
  return noticeComponent(
    {
      ...
      extra: [
        ...(size === undefined
          ? []
          : [{ text: size, tone: "dim" as ThemeColor }]),
        ...(timeline === undefined
          ? []
          : [{ text: timeline.text, tone: timeline.tone }]),
      ],
      detail: joinFields([
        shortSha(notice.sha),
...
function titleLine(notice: MemoryWriteNotice): string {
  ...
  if (entries === undefined || !Number.isFinite(entries) || entries <= 0) {
    return "Memory updated";
  }
  return joinFields([
    "Memory updated",
    `${ordinal(Math.floor(entries))} entry today`,
  ]);
}
...
function whyLine(notice: MemoryWriteNotice): string {
  ...
  if (
    affected.length === 1 && only !== undefined && only.deletions === 0 &&
    only.insertions > 0
  ) {
    return `Added ${only.insertions} line${
      only.insertions === 1 ? "" : "s"
    } to ${normalizeRendererText(only.path)}.`;
  }
  const paths = affected.map((entry) => normalizeRendererText(entry.path)).join(
    ", ",
  );
  return `Updated ${affected.length} memory file${
    affected.length === 1 ? "" : "s"
  } (${paths}).`;
}
...
function timelineLine(
  ...
  const text = joinFields([
    ...
    consolidationAge === undefined
      ? undefined
      : `last consolidation ${consolidationAge}`,
    steps === undefined
      ? undefined
      : `${steps} step${steps === 1 ? "" : "s"} unreflected`,
  ]);
...
export function formatBytes(bytes: number): string {
  ...
  return bytes < DECIMAL_KB_LIMIT
    ? `${kilobytes.toFixed(1)}K`
    : `${Math.round(kilobytes)}K`;
}
