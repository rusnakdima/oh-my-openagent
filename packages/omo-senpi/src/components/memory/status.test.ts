...
function segmentRepo(
  overrides: Partial<GitRepoForStatus> = {},
): GitRepoForStatus {
  return {
    head: async () => "abcdef1234567890",
    headCommitTimestamp: async () =>
      Date.parse("2026-08-10T00:00:00.000Z") / 1000,
    lsTree: async () => [],
    show: async () => "",
    status: async () => "",
    ...overrides,
  ...
  for (let index = 0; index < (input.failures ?? 0); index += 1) {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        detail: "boom",
        finishedAt: new Date(
          Date.now() - ((input.failures ?? 0) - index) * 60_000,
        ).toISOString(),
...
describe("refreshMemoryStatus segments", () => {
  test(
    "#given a dirty memory worktree #when refresh runs #then the footer carries the dirty marker",
    async () => {
  ...
  test(
    "#given a clean worktree with a backlog of fourteen steps #when refresh runs #then the backlog segment renders",
    async () => {
      const context = segmentContext({
        sessionId: "session-a",
        backlogSteps: 14,
  ...
  test(
    "#given a zero backlog #when refresh runs #then no backlog segment renders",
    async () => {
      const context = segmentContext({
        sessionId: "session-a",
        backlogSteps: 0,
  ...
  test(
    "#given three consecutive reflection failures #when refresh runs #then the streak badge renders",
    async () => {
  ...
  test(
    "#given only two consecutive failures #when refresh runs #then no streak badge renders",
    async () => {
  ...
  test(
    "#given dirty, backlog, and streak all present #when refresh runs #then segments render in fixed order",
    async () => {
      const context = segmentContext({
        identity: "agent",
        sessionId: "session-a",
  ...
  test(
    "#given an assembled line over sixty characters #when refresh runs #then segments drop right to left until it fits",
    async () => {
  ...
  test(
    "#given a base line already past the width budget #when refresh runs #then every segment drops",
    async () => {
  ...
  test(
    "#given a repo with no HEAD #when refresh runs #then no git status work happens",
    async () => {
      const context = segmentContext({
        sessionId: "session-a",
        backlogSteps: 9,
  ...
  test(
    "#given footer rendering disabled #when refresh runs #then no git status call happens",
    async () => {
      const context = segmentContext({
        sessionId: "session-a",
        backlogSteps: 9,
  ...
  test(
    "#given a future commit timestamp with dirty state #when refresh runs #then the future-age guard still suppresses the footer",
    async () => {
      const context = segmentContext({});
      const recorder = recordingUi();
      ...
      await refreshMemoryStatus({
        ...
        gitRepo: segmentRepo({
          headCommitTimestamp: async () =>
            Date.parse("2026-08-10T00:02:00.000Z") / 1000,
          status: async () => " M x",
        }),
        now: () => SEGMENT_NOW,
        checkAdvisory: false,
  ...
  test(
    "#given git status throws #when refresh runs #then the base footer still renders",
    async () => {
      const context = segmentContext({});
      const recorder = recordingUi();
...
describe("refreshMemoryStatus", () => {
  test(
    "#given a committed HEAD ninety seconds old #when refresh runs #then footer shows system-clock-relative age",
    async () => {
  ...
  test(
    "#given committed HEAD ages across display buckets #when refresh runs #then compact labels follow the system clock",
    async () => {
      const now = Date.parse("2026-08-10T12:00:00.000Z");
      for (
        const [ageMs, expected] of [
          [30_000, "just now"],
          [2 * 60 * 60_000, "2h ago"],
  ...
  test("#given system memory exactly at the soft pressure threshold #when refresh runs #then one pressure dream request fires without changing the advisory notify", async () => {
    ...
    await refreshMemoryStatus({
      ...
      alreadyNotified: false,
      gitRepo: fakeRepo,
      requestPressureDream: async () => {
        pressureRequests.push(1);
      },
    });
  ...
  test(
    "#given system markdown under the advisory threshold #when refresh runs #then no advisory notify fires",
    async () => {
  ...
  test(
    "#given system markdown at or above the advisory threshold #when refresh runs #then one warning notify fires with token estimate",
    async () => {
  ...
  test(
    "#given footer rendering disabled at session bind #when memory is oversized #then advisory fires without a footer",
    async () => {
      const fakeRepo: GitRepoForStatus = {
        head: async () => "abcdef1234567890",
        headCommitTimestamp: async () =>
  ...
  test(
    "#given advisory checking disabled after first memory use #when memory is oversized #then footer renders without another warning",
    async () => {
      const fakeRepo: GitRepoForStatus = {
        head: async () => "abcdef1234567890",
        headCommitTimestamp: async () =>
  ...
  test(
    "#given an already-notified session #when refresh runs again over threshold #then no second notify fires",
    async () => {
  ...
  test(
    "#given a repo with no HEAD #when refresh runs #then no footer or advisory appears",
    async () => {
  ...
  test(
    "#given a committed HEAD without a readable commit timestamp #when refresh runs #then no footer appears",
    async () => {
      const fakeRepo: GitRepoForStatus = {
        head: async () => "abcdef1234567890",
        headCommitTimestamp: async () => null,
  ...
  test(
    "#given a commit timestamp later than the system clock #when refresh runs #then no footer appears",
    async () => {
      const fakeRepo: GitRepoForStatus = {
        head: async () => "abcdef1234567890",
        headCommitTimestamp: async () =>
  ...
  test(
    "#given system files with non-system markdown excluded #when refresh estimates tokens #then only system/**/*.md counts",
    async () => {
