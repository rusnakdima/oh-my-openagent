...
export interface MemoryIdentityRuntimeDeps {
  ...
  readonly cwd: () => string;
  readonly resolveModelRegistry: () =>
    | SenpiModelRegistryPort<SenpiModelPort>
    | undefined;
...
export function createIdentityRuntime(
  ...
  const settings = resolveMemorySettings(
    deps.loadConfig({ cwd: deps.cwd() }).config.memory,
  );
  ...
  const store = new ReflectionReservationStore({
    ...
    getJournal: async (conversationId: string) =>
      new TranscriptJournal({
        journalDir: `${identity.identityPaths.transcripts}/${conversationId}`,
      }),
    createRunId: () => `reflection-run-${++runCounter}`,
  });

  let builtSandbox: SandboxTransform | undefined;
  const resolveAgentDir = deps.resolveAgentDir ??
  ...
  const runner = new SenpiSubprocessRunner({
    ...
    ...(deps.resolveSessionModel === undefined
      ? {}
      : { resolveSessionModel: deps.resolveSessionModel }),
    ...(deps.resolveParentContextTokens === undefined
      ? {}
      : { resolveParentContextTokens: deps.resolveParentContextTokens }),
    ...(deps.resolveParentSessionFile === undefined
      ? {}
      : { resolveParentSessionFile: deps.resolveParentSessionFile }),
    ...(deps.resolveParentCacheReusable === undefined
      ? {}
      : { resolveParentCacheReusable: deps.resolveParentCacheReusable }),
  ...
  const launch = (run: ReservedRun): void => {
    void runner.launch(run).then((result) => {
      ...
      deps.logger?.warn("memory reflection launch failed", {
