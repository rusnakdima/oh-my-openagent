...
export function resolveFactsPeopleRouting(
  config: OmoConfig,
  identity: string,
): FactsPeopleRouting {
  ...
  return {
    ...
    maxEntryChars: override?.max_entry_chars ??
      memory?.people.max_entry_chars ?? 200,
  };
}
...
export async function applyFactsWithRetries(options: {
  ...
  for (let attempt = 1; attempt <= WRITER_ATTEMPTS; attempt += 1) {
    try {
      return await options.withWriterLock(
        async () => applyClaimed(options),
        attempt,
      );
    } catch (error) {
      if (!(error instanceof LockContentionError)) throw error;
      if (attempt === WRITER_ATTEMPTS) {
        options.logger?.warn("facts extractor memory-write lock exhausted", {
...
async function applyClaimed(options: {
  ...
  const receipt = await findFactsBatchReceipt(
    options.repo,
    options.ledger.batchId,
  );
  ...
  if (options.ledger.applyRecovery !== undefined) {
    ...
    try {
      recovery = await planFactsMutation(
        options.repo,
        batch,
        options.people,
        (tie) =>
  ...
  const result = await applyFactsRecovery(
    options.repo,
    recovery,
