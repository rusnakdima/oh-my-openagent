...
describe("createMemoryComponent", () => {
    ...
    for (
      const scenario of [
        ...
        { memory: memorySettings(), flags: { "omo-senpi-disabled": true } },
        {
          ...
          flags: { "omo-senpi-memory-disabled": true },
        },
      ]
    ) {
      ...
      createMemoryComponent({
        loadConfig: () => loadedMemoryConfig(scenario.memory),
      }).register(pi, ctx);
  ...
  test("#given the sentinel is absent or not exactly 1 #when registered #then memory stays enabled", () => {
    for (
      const env of [{}, { SENPI_MEMORY_REFLECTION: "0" }, {
  ...
  test("#given enabled memory #when session_start binds an auto identity #then it appends a hidden binding and performs no filesystem writes", async () => {
    ...
    expect(pi.entries).toEqual([{
      ...
      data: expect.objectContaining({
        identity: expect.stringMatching(/^project-[a-f0-9]{8}$/),
        boundAt: 123,
      }),
    }]);
    ...
    for (const latchedEnabled of [false, true]) {
      ...
      createMemoryComponent({
        ...
        loadConfig: () =>
          loadedMemoryConfig(
            memorySettings({
              enabled: states[Math.min(reads++, states.length - 1)],
            }),
          ),
        resolveCwd: () => cwd,
      }).register(pi, componentContext());
      await pi.dispatch("session_start", {}, sessionContext({ notifications }));
      const registrations = {
        handlers: pi.handlers.length,
        renderers: pi.entryRenderers.length,
        entries: pi.entries.length,
      };

      pi.events.emit("config-watch:reloaded", {
        registrationId: "omo",
        paths: ["omo.jsonc"],
      });
      pi.events.emit("config-watch:reloaded", {
