...
async function dispatchMcpWrite(
  writeNoticeEnabled: boolean,
  toolCallId: string,
): Promise<MemoryFakeExtensionAPI> {
  const root = realpathSync.native(
    await mkdtemp(join(tmpdir(), "omo-memory-mcp-notice-")),
  );
  roots.push(root);
  ...
  await mkdir(join(identity.paths.transcripts, SESSION_ID), {
    recursive: true,
  });
  await writeToolReceipt(identity.paths.toolReceipts, {
    version: 1,
    toolCallId,
    ...COMMIT,
  });
  ...
  createMemoryComponent({
    env,
    loadConfig: () =>
      loadedMemoryConfig(memorySettings({
...
describe("memory MCP notice wiring", () => {
  test(
    "#given the write notice is enabled #when an MCP memory tool_result carries a soul commit receipt #then both notices are appended",
    async () => {
      // when
  ...
  test(
    "#given the write notice is disabled in config #when the same tool_result fires #then only the soul notice is appended",
    async () => {
      // when
