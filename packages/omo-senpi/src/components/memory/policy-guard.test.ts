...
function registeredPolicy(setup: PolicyFixture): FilesystemPolicy {
  ...
  if (policy === undefined) {
    throw new Error("expected a registered filesystem policy");
  }
  return policy;
}
...
describe("memory filesystem policy guard", () => {
  ...
  test("#given the registered policy #when operations target the bound identity repo and worktrees #then they are allowed", async () => {
    ...
    expect(await check(policy, "read", canonical(setup.ownRepoFile))).toEqual({
      allow: true,
    });
    expect(
      await check(policy, "write", canonical(setup.ownWorktreeFile), "write"),
    ).toEqual({ allow: true });
    expect(
      await check(
        policy,
        "enumerate",
        canonical(setup.own.identityPaths.root),
        "ls",
      ),
    ).toEqual({ allow: true });
  });
  ...
  test("#given the registered policy #when an operation targets a path outside the agents root #then it is allowed", async () => {
    ...
    expect(await check(policy, "read", canonical(setup.outsideFile))).toEqual({
      allow: true,
    });
  });
  ...
  test("#given the policy guard module source #when imports are audited #then senpi references are type-only with no runtime import or any-cast", () => {
    const source = readFileSync(
      resolve(import.meta.dir, "policy-guard.ts"),
      "utf8",
    );
    ...
    const runtimeSenpiImports = source
      .split("\n")
      .filter((line) =>
        /^\s*import\s+(?!type\b).+\s+from\s+["']@code-yeongyu\/senpi["']/.test(
          line,
        )
      );
