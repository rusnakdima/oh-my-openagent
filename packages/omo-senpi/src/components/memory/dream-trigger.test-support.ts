...
export function triggerSettings(
  overrides: Partial<DreamTriggerSettings> = {},
...
export class FakeScheduler {
  ...
  cancel(handle: unknown): void {
    const entry = this.scheduled.find((candidate) =>
...
export async function fixture(options: {
  ...
  const text = options.conversationText === undefined
    ? "x".repeat(DREAM_VOLUME_GATE_BYTES + 1)
    : options.conversationText;
  if (text !== null) {
    await writeConversation(identity.paths.transcripts, CONVERSATION, text);
  }
  if (options.lastDreamAt !== undefined) {
    const dreamDir = join(identity.paths.runtime, "dream");
    await mkdir(dreamDir, { recursive: true });
    await writeFile(
      join(dreamDir, "state.json"),
      `${
        JSON.stringify({
          last_dream_at: options.lastDreamAt,
          lastRunId: "run-0",
        })
      }\n`,
    );
  ...
  const wiring = createDreamTriggerWiring({
    ...
    resolveSessionById: (
      sessionId,
    ) => (sessionId === CONVERSATION ? session : undefined),
...
export async function settle(
  fixture: Fixture,
  eventCtx: unknown = fixture.eventCtx,
): Promise<void> {
  await fixture.pi.dispatch(
    "agent_settled",
    { type: "agent_settled" },
    eventCtx,
  );
...
export function gateProbe(
  input: {
    readonly lastDreamAtMs?: number | null;
    readonly unreflectedBytes?: number;
  } = {},
): {
  ...
  return {
    ...
    probe: {
      ...
      unreflectedBytes: async () => {
        ...
        return input.unreflectedBytes === undefined
          ? DREAM_VOLUME_GATE_BYTES + 1
          : input.unreflectedBytes;
      },
    },
  };
}
