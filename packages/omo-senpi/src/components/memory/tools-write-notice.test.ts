...
async function writeMergedCompletion(
  fixture: BoundFixture,
  finishedAt: string,
): Promise<void> {
...
describe("memory write notice gathering", () => {
  test(
    "#given a committed write #when the tool result is built #then writeNotice carries raw sha, affected counts, size and timeline",
    async () => {
      // given
      const fixture = await boundFixture();
      await writeJournalState(
        fixture,
        JSON.stringify({ steps_since_last_successful_reflection: 7 }),
      );
      ...
      expect(textOf(result)).toMatch(
        /^Memory create committed locally \([0-9a-f]{7}\)\.$/,
      );
      ...
      expect(notice?.timeline.lastConsolidationAtISO).toBe(
  ...
  test(
    "#given the model-facing text #when a notice is attached #then content[0].text still carries only the tool message",
    async () => {
      // given
      const fixture = await boundFixture();
      const [memoryTool] = toolsFor(fixture);
  ...
  test(
    "#given no prior commit before this write #when gathered #then previousEntryAtISO is omitted and nothing throws",
    async () => {
      ...
      const single = await gatherMemoryWriteNotice(fixture.context, {
        ...
        buildSnapshot: async () =>
          ({ repo: {}, reflection: {}, journal: {} }) as never,
      });
      ...
      expect(single.affected).toEqual([{
  ...
  test(
    "#given no reflection completions directory #when gathered #then lastConsolidationAtISO is omitted and nothing throws",
    async () => {
      // given
      const fixture = await boundFixture();
      await rm(join(fixture.context.identityPaths.reflection, "completions"), {
        recursive: true,
        force: true,
      });
      const [memoryTool] = toolsFor(fixture);
      ...
      expect(result.details.writeNotice?.timeline.lastConsolidationAtISO)
        .toBeUndefined();
    },
    60_000,
  );
  ...
  test(
    "#given a corrupt journal state.json #when gathered #then unreflectedSteps is omitted and nothing throws",
    async () => {
      // given
      const fixture = await boundFixture();
      ...
      expect(result.isError).toBeUndefined();
      expect(result.details.writeNotice).toBeDefined();
      expect(result.details.writeNotice?.timeline.unreflectedSteps)
        .toBeUndefined();
    },
    60_000,
  );
  ...
  test(
    "#given git fails while reading line counts #when gathered #then affected is empty and nothing throws",
    async () => {
      // given
      const fixture = await boundFixture();
  ...
  test(
    "#given the whole snapshot read throws #when gathered #then size and timeline degrade without throwing",
    async () => {
      // given
      const fixture = await boundFixture();
  ...
  test(
    "#given the write-notice gate is off #when the tool commits #then no writeNotice is attached",
    async () => {
      // given
      const fixture = await boundFixture();
  ...
  test(
    "#given memory_apply_patch commits #when the result is built #then it carries the same notice payload shape",
    async () => {
      // given
      const fixture = await boundFixture();
      ...
      // then
      expect(result.isError).toBeUndefined();
      expect(result.details.writeNotice?.affected.map((entry) => entry.path))
        .toEqual(["knowledge/patched.md"]);
    },
    60_000,
  );
});
...
describe("memory tool result rendering", () => {
  ...
  test(
    "#given a committed write #when the row renders collapsed #then the local-commit line is replaced by the notice",
    async () => {
      // given
      const fixture = await boundFixture();
      const [memoryTool] = toolsFor(fixture);
      ...
      if (renderResult === undefined) {
        throw new Error("memory renderResult is missing");
      }
      ...
      expect(expanded.join("\n")).toContain(
        created.details.writeNotice?.sha.slice(0, 7) ?? "MISSING",
      );
    },
    60_000,
  );
