...
export function memoryUsagePaths(
  identityPaths: MemoryIdentityPaths,
): MemoryUsageLedgerPath {
  return {
...
export async function readMemoryUsageLedger(
  ledgerPath: string,
): Promise<MemoryUsageLedger> {
  try {
...
export async function incrementMemoryUsage(
  ...
  try {
    await incrementMemoryUsageBatch(
      paths,
      new Map([[relativePath, 1]]),
      now,
      record,
    );
  } catch (error) {
    logger?.warn("memory-usage ledger write failed", {
      relativePath,
      error: String(error),
    });
  }
