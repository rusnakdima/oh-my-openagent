...
export type ResolvedTriggerConfig = TriggerConfig & {
...
export interface ReflectionTriggerWiring {
  ...
  requestManualReflection(
    focus?: string,
    options?: ManualReflectionOptions,
  ): void;
  /** Resolves once every evaluation started so far has finished. */
  whenIdle(): Promise<void>;
}

export function createReflectionTriggerWiring(
  options: ReflectionTriggerWiringOptions,
): ReflectionTriggerWiring {
  ...
  function track(task: () => Promise<void>): void {
    const promise = task()
      .catch((error: unknown) => {
        options.logger?.warn("omo-senpi memory reflection trigger failed", {
  ...
  async function evaluateAndLaunch(
    session: ReflectionTriggerSession,
    event: ReflectionEvent,
  ): Promise<void> {
  ...
  function launch(request: ReflectionRequest): void {
    try {
      ...
      options.logger?.warn("omo-senpi memory reflection launch failed", {
        error: describe(error),
      });
    }
  }
  ...
  return {
    register(pi: SenpiExtensionAPI): void {
      ...
      pi.on("agent_settled", async (_payload, eventCtx) => {
        ...
        if (outcome === undefined || outcome.aborted || outcome.willRetry) {
          return;
        }
        ...
        try {
          if (session.ledger.pendingCompaction) {
            await session.engine.evaluate(session.conversationId, {
              kind: "compaction_accepted",
            });
            session.ledger.pendingCompaction = false;
          }
    ...
    requestManualReflection(
      focus?: string,
      manual: ManualReflectionOptions = {},
    ): void {
      ...
      track(() =>
        evaluateAndLaunch(session, {
          ...
          ...(manual.conversationIds === undefined
            ? {}
            : { conversationIds: manual.conversationIds }),
        })
      );
    },
 ...
 */
export function resolveReflectionTriggerConfig(
...
function readOutcome(payload: unknown): AgentRunOutcome {
  ...
  return {
    ...
    ...(abortSource === "user" || abortSource === "system"
      ? { abortSource }
      : {}),
  };
}
