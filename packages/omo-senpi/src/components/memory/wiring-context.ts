...
export function readUi(eventCtx: unknown): StatusUi | undefined {
  ...
  if (typeof ui.setStatus !== "function" || typeof ui.notify !== "function") {
    return undefined;
  }
  return {
    setStatus: (key, text) =>
      Reflect.apply(ui.setStatus as (...args: unknown[]) => unknown, ui, [
        key,
        text,
      ]),
    notify: (message, level) =>
      Reflect.apply(ui.notify as (...args: unknown[]) => unknown, ui, [
        message,
        level,
      ]),
  };
