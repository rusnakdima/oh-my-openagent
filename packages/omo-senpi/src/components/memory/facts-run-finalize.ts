...
import {
  applyFactsWithRetries,
  resolveFactsPeopleRouting,
} from "./facts-batch-apply";
...
export interface FinalizeClaimedFactsRunInput {
  ...
  readonly withWriterLock: <T>(
    operation: () => Promise<T>,
    attempt: number,
  ) => Promise<T>;
}

export async function finalizeClaimedFactsRun(
  ...
  const payload = await readRunJson<FactsPayload>(
    join(runDir, "facts-payload.json"),
  );
  const batch = {
  ...
  if (outcome.timedOut || outcome.childExit.code !== 0) {
    await fail(
      outcome.timedOut ? "deadline_exceeded" : "child_exit",
      "facts child did not exit successfully",
    );
  ...
  try {
    records = parseFactsExtractionJsonl(
      await readFile(join(runDir, "extraction.jsonl"), "utf8"),
    );
  } catch (error) {
...
function applyWithRetries(
  ...
  return applyFactsWithRetries({
    ...
    repo: input.repo,
    records,
    people: resolveFactsPeopleRouting(
      input.options.loadConfig().config,
      input.identity.id,
    ),
