...
function fixture(): {
  ...
  const root = realpathSync.native(
    mkdtempSync(join(tmpdir(), "omo-sandbox-absent-")),
  );
  roots.push(root);
  ...
  for (const dir of [worktree, gitCommonDir, dirname(payload)]) {
...
describe("reflection sandbox with not-yet-created paths", () => {
  test(
    "#given a runtime write dir that does not exist yet #when the Darwin transform is built #then no ENOENT is thrown and the profile grants that exact path",
    () => {
      // given
      const setup = fixture();
      const absentRuntimeWrite = join(
        setup.root,
        "runtime",
        "reflection-sessions",
      );
      ...
      expect(profile).toContain(
        `(allow file-write* (subpath ${JSON.stringify(absentRuntimeWrite)}))`,
      );
      ...
      expect(profile).not.toContain(
        `(allow file-write* (subpath ${JSON.stringify(setup.root)}))`,
      );
      expect(profile).not.toContain(
        `(allow file-write* (subpath ${
          JSON.stringify(dirname(absentRuntimeWrite))
        }))`,
      );
    },
    30_000,
  ...
  test(
    "#given absent payload and foreign-root entries #when the Darwin transform is built #then the rendered rules name the full intended paths",
    () => {
      // given
      const setup = fixture();
      ...
      // then
      expect(profile).toContain(
        `(allow file-read* (literal ${JSON.stringify(absentPayload)}))`,
      );
      expect(profile).toContain(
        `(deny file-read* (subpath ${JSON.stringify(absentForeignRoot)}))`,
      );
      ...
      expect(profile).not.toContain(
        `(deny file-read* (subpath ${
          JSON.stringify(dirname(absentForeignRoot))
        }))`,
      );
    },
    30_000,
  ...
  test(
    "#given a Linux runtime write dir that does not exist yet #when the bwrap transform is built #then that exact path is rebound writable",
    () => {
      // given
      const setup = fixture();
  ...
  test(
    "#given required policy and an absent runtime write dir #when the transform is built #then it still succeeds rather than failing closed on a raw ENOENT",
    () => {
      // given
      const setup = fixture();
      const absentRuntimeWrite = join(setup.root, "runtime", "reflection");
