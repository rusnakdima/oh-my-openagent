...
async function dispatch(
  ...
  const results = await setup.pi.dispatch(
    "tool_call",
...
describe("memory cross-identity guard", () => {
  test("#given a bound identity #when read targets a foreign identity file #then the call is blocked with the stable reason", async () => {
    ...
    expect(result).toEqual({
      block: true,
      reason:
        `cross-identity memory access denied: read to ${setup.foreignFile} belongs to another memory identity`,
    });
  });
  ...
  test("#given a bound identity #when write targets its own repo or worktree #then both calls are allowed", async () => {
    ...
    const worktreeFile = join(
      setup.own.identityPaths.worktrees,
      "reflection",
      "notes.md",
    );

    expect(await dispatch(setup, "write", { filePath: setup.ownFile }))
      .toBeUndefined();
    expect(await dispatch(setup, "edit", { file_path: worktreeFile }))
      .toBeUndefined();
  });
  ...
  test("#given no bound identity #when a file tool targets another identity #then the guard never blocks", async () => {
    ...
    expect(await dispatch(setup, "read", { path: setup.foreignFile }))
      .toBeUndefined();
  });
  ...
  test("#given a bound identity #when unknown tools or file tools without path arguments run #then they are allowed", async () => {
    ...
    expect(await dispatch(setup, "database_query", { path: setup.foreignFile }))
      .toBeUndefined();
  ...
  test("#given a relative traversal from the workspace #when a suffixed case-variant file tool resolves into foreign memory #then it is blocked", async () => {
    ...
    const result = await dispatch(setup, "vendor_READ", {
      target: relativeForeign,
    });

    expect(result).toEqual({
      block: true,
      reason:
        `cross-identity memory access denied: vendor_READ to ${relativeForeign} belongs to another memory identity`,
    });
  });
  ...
  test("#given paths contains own and foreign targets #when glob runs #then the foreign array member is blocked", async () => {
    ...
    const result = await dispatch(setup, "glob", {
      paths: [setup.ownFile, setup.foreignFile],
    });

    expect(result).toEqual({
      block: true,
      reason:
        `cross-identity memory access denied: glob to ${setup.foreignFile} belongs to another memory identity`,
    });
  });
  ...
  test("#given an outside symlink points into foreign memory #when a file and a not-yet-created descendant are targeted #then realpath and nearest-real-parent both block", async () => {
    ...
    expect(await dispatch(setup, "read", { path: existing })).toEqual({
      block: true,
      reason:
        `cross-identity memory access denied: read to ${existing} belongs to another memory identity`,
    });
    expect(await dispatch(setup, "write", { path: missing })).toEqual({
      block: true,
      reason:
        `cross-identity memory access denied: write to ${missing} belongs to another memory identity`,
    });
  });
  ...
  test("#given the identities root and one of its ancestors #when enumeration tools target them #then cross-identity enumeration is blocked", async () => {
    ...
    expect(await dispatch(setup, "ls", { path: setup.agentsRoot })).toEqual({
      block: true,
      reason:
        `cross-identity memory access denied: ls to ${setup.agentsRoot} belongs to another memory identity`,
    });
    expect(await dispatch(setup, "grep", { path: memoryRoot })).toEqual({
      block: true,
      reason:
        `cross-identity memory access denied: grep to ${memoryRoot} belongs to another memory identity`,
    });
  ...
  test("#given a namespaced apply-patch variant #when its patch directives touch foreign memory #then every directive path is guarded", async () => {
    ...
    const result = await dispatch(setup, "vendor_APPLY_PATCH", {
      input: patch,
    });
    ...
    expect(result).toEqual({
      block: true,
      reason:
        `cross-identity memory access denied: vendor_APPLY_PATCH to ${setup.foreignFile} belongs to another memory identity`,
    });
  });
  ...
  test("#given bash contains a forbidden foreign-root literal #when invoked repeatedly in one session #then it warns once and never blocks", async () => {
    ...
    expect(setup.context.logs).toEqual([{
      level: "warn",
      message:
        "omo-senpi memory guard advisory: bash command references another memory identity; shell text is not blocked because this soft guard is not a security boundary",
  ...
  test("#given separate sessions invoke bash with a forbidden root #when advisory state is tracked #then each session receives one warning", async () => {
    ...
    expect(await dispatch(setup, "bash", { command }, "session-a"))
      .toBeUndefined();
    expect(await dispatch(setup, "bash", { command }, "session-b"))
      .toBeUndefined();
