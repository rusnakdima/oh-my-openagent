...
function render(
  ...
  const component = renderResult(
    {
      content: [{ type: "text", text: details?.message ?? MESSAGE }],
      details,
    } as never,
  ...
  if (
    details?.writeNotice === undefined || options.isError === true ||
...
describe("memory write notice rendering", () => {
  ...
  describe("#given a successful single-file write", () => {
    ...
    test("#when the entry count needs an ordinal #then English suffixes are correct", () => {
      // when / then
      const ordinals = [1, 2, 3, 4, 11, 12, 13, 21, 22, 23, 101, 111].map((
        entriesToday,
      ) =>
        render({
          message: MESSAGE,
          writeNotice: notice({ timeline: { entriesToday } }),
        })[0]
      );
  ...
  describe("#given byte formatting", () => {
    test("#when the size crosses the 10K boundary #then one decimal is used below it and an integer above", () => {
      ...
      const small = render({
        message: MESSAGE,
        writeNotice: notice({
          size: { systemBytes: 2048, totalBytes: 9_932, fileCount: 3 },
        }),
      });
      const large = render({
        message: MESSAGE,
        writeNotice: notice({
          size: { systemBytes: 10_240, totalBytes: 1_048_576, fileCount: 3 },
        }),
      });
    ...
    test("#when exactly one file is tracked #then the noun is singular", () => {
      ...
      const lines = render({
        message: MESSAGE,
        writeNotice: notice({
          size: { systemBytes: 2048, totalBytes: 33_792, fileCount: 1 },
        }),
      });
  ...
  describe("#given a stale consolidation or a deep reflection backlog", () => {
    test("#when consolidation is a week old #then the timeline line turns warning toned", () => {
      ...
      // then
      expect(lines[3]).toBe(
        "last entry 5m ago · last consolidation 7d ago · 3 steps unreflected",
      );
    ...
    test("#when 25 steps are unreflected #then the timeline line turns warning toned", () => {
      ...
      // then
      expect(lines[3]).toBe(
        "last entry 5m ago · last consolidation 6d ago · 25 steps unreflected",
      );
  ...
  describe("#given degraded gathering", () => {
    ...
    test("#when the tree size read failed #then the size line is dropped entirely", () => {
      ...
      const lines = render({
        ...
        writeNotice: notice({
          size: undefined,
          affected: [{ path: "a.md", insertions: 2, deletions: 0 }],
        }),
      });
