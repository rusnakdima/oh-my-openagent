...
export function createMemoryBinding(input: {
  ...
  return {
    ...
    repoPathHash: createHash("sha256").update(input.repoPath, "utf8").digest(
      "hex",
    ),
...
export function findLatestMemoryBinding(
  entries: readonly SessionEntryLike[],
): MemorySessionBinding | undefined {
  for (let index = entries.length - 1; index >= 0; index -= 1) {
    const entry = entries[index];
    if (
      entry.type !== "custom" || entry.customType !== MEMORY_BINDING_CUSTOM_TYPE
    ) continue;
