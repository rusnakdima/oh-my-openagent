...
const DISABLED: FactsEnqueueResult = {
  enqueued: false,
...
export function createMemoryFactsWiring(
  options: MemoryFactsWiringOptions,
): MemoryFactsWiring {
  ...
  const fire = (
    ...
    void promise?.catch((error) => {
      options.logger?.warn(`facts extractor ${operation} failed`, {
        error: String(error),
      });
    });
  };
  ...
  const wiring: MemoryFactsWiring = {
    async enqueueSettled(
      ...
      try {
        const journal = createJournal(
          join(options.identityPaths.transcripts, conversationId),
        );
        ...
        options.logger?.warn("facts queue enqueue failed", {
          conversationId,
          error: String(error),
        });
        return DISABLED;
    ...
    async reconcilePending(): Promise<FactsQueueEntry[]> {
      ...
      try {
        ...
        options.logger?.warn("facts queue reconcile failed", {
          error: String(error),
        });
        return [];
      }
    ...
    async launchIfThresholdMet(signal?: AbortSignal): Promise<boolean> {
      ...
      try {
        ...
        options.logger?.warn("facts extractor launch failed", {
          error: String(error),
        });
      }
      return true;
    ...
    async markConsumed(entries: readonly FactsQueueEntry[]): Promise<void> {
      try {
        ...
        options.logger?.warn("facts queue consume failed", {
          error: String(error),
        });
      }
    },
