...
export const AVAILABLE_MODEL: SenpiModelPort = {
  provider: "omo-mock",
  id: "mock-1",
};
...
export const supervisorFixture = join(
  import.meta.dir,
  "worker",
  ...
  return {
    ...
    sandbox: (args) => ({
      ...
      args: [
        childFixture,
        mode === "model-fallback"
          ? args.args.includes("extension-only/primary")
            ? "model-not-found"
            : "fact"
          : mode,
      ],
 ...
 */
export async function runLedgers(
  identity: MemoryIdentity,
): Promise<FactsRunLedger[]> {
  ...
  const dated = await Promise.all(names.map(async (name) => ({
    ...
    ledger: JSON.parse(
      await readFile(join(runs, name, "ledger.json"), "utf8"),
    ) as FactsRunLedger,
  })));
  return dated
    .sort((
