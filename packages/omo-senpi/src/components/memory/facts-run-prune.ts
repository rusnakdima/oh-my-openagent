...
export async function pruneTerminalFactsRuns(
  ...
  for (const { name, tombstone } of claimed) {
    ...
    try {
      ...
      options.warn?.("facts run tombstone deletion failed", {
        tombstone,
...
async function readTerminalRuns(
  runsDir: string,
): Promise<readonly TerminalRun[]> {
...
async function readSentinel(
  runDir: string,
): Promise<{ readonly runId: string; readonly at: string } | undefined> {
  const final = await readRunJson<{ runId?: unknown; finishedAt?: unknown }>(
    join(runDir, "final.json"),
  )
    .catch(() => undefined);
  if (final !== undefined) {
    return typeof final.runId === "string" &&
        typeof final.finishedAt === "string"
      ? { runId: final.runId, at: final.finishedAt }
      : undefined;
  }
  const abandoned = await readRunJson<
    { runId?: unknown; abandonedAt?: unknown }
  >(join(runDir, "abandoned.json"))
    .catch(() => undefined);
  if (abandoned === undefined) return undefined;
  return typeof abandoned.runId === "string" &&
      typeof abandoned.abandonedAt === "string"
...
async function withFactsRunsLock<T>(
  locksDir: string,
  operation: () => Promise<T>,
): Promise<T> {
  const record = await createLockRecord("facts-runs", { runId: "facts-prune" });
  return withLock(factsRunsLockPath(locksDir), record, operation, {
    waitTimeoutMs: RUNS_LOCK_WAIT_MS,
  });
}
