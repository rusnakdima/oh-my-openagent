...
export function createMemoryWiring(options: MemoryWiringOptions): MemoryWiring {
  ...
  const skillsUsageTrackersRef: { current: Map<string, SkillsUsageTracker> } = {
    current: new Map(),
  };
  const memoryUsageTrackersRef: { current: Map<string, MemoryUsageTracker> } = {
    current: new Map(),
  };
  const reflectionLive = createMemoryReflectionLiveWiring(
    options,
    activeSession,
    lastEventCtx,
  );
  ...
  const { resolveContext, journalWiringFor, factsWiringFor, runtimeFor } =
    runtimeWiring;
  ...
  const nudgeWiring = createMemoryNudgeWiring({
    ...
    resolveSettings: (identity) => {
      const settings = resolveMemorySettings(
        options.loadConfig({ cwd: options.cwd() }).config.memory,
      );
      const override = settings.agents[identity]?.nudge;
      return {
        enabled: override?.enabled ?? settings.nudge.enabled,
        everyUserTurns: override?.every_user_turns ??
          settings.nudge.every_user_turns,
      };
    },
  });
  ...
  const dreamTriggerWiring = buildDreamTriggerWiring(
    options,
    runtimeWiring,
    activeSession,
  );
  ...
  function resolvePalacePeople(): PalacePeopleOptions | undefined {
    const people = resolveMemorySettings(
      options.loadConfig({ cwd: options.cwd() }).config.memory,
    ).people;
    return {
      enabled: people.enabled,
      limits: {
        maxEntries: people.max_entries,
        maxEntryChars: people.max_entry_chars,
      },
    };
  }
  ...
  function loadCommandSettings(): MemoryCommandSettings {
    ...
    return {
      settings: resolveMemorySettings(resolved.memory),
      config: resolved,
    };
  }
  ...
  return {
    ...
    async afterBind(pi, sessionId, identity, eventCtx): Promise<void> {
      ...
      await reflectionLive.bind(
        ...
        eventCtx,
        () => {
          void dreamTriggerWiring.requestPressureDream(sessionId).catch(
            (error: unknown) => {
              options.logger?.warn(
                "omo-senpi memory pressure dream trigger failed",
    ...
    async onSessionShutdown(input: ShutdownDrainInput): Promise<void> {
      reflectionLive.shutdown(
        options.sessions.get(input.sessionId)?.context?.identity,
      );
