...
describe("memory tool activation", () => {
  test("#given no bound identity #when either tool executes #then an actionable initialization error is returned", async () => {
    ...
    // when
    const memoryResult = await memoryTool.execute("call-1", {
      command: "create",
      reason: "x",
      file_path: "a.md",
      description: "d",
    });
    const patchResult = await applyPatchTool.execute("call-2", {
      reason: "x",
      input: "*** Begin Patch\n*** End Patch",
    });
  ...
  test("#given a resolver that binds after registration #when the tool executes #then activation follows binding", async () => {
    ...
    const holder: { current: MemoryIdentityContext | undefined } = {
      current: undefined,
    };
    const [memoryTool] = createMemoryTools(() => holder.current);

    // when
    const stale = await memoryTool.execute("call-1", {
      command: "create",
      reason: "early",
      file_path: "a.md",
      description: "d",
    ...
    const bound = await memoryTool.execute("call-2", {
      command: "create",
      reason: "Track a",
      file_path: "a.md",
      description: "d",
