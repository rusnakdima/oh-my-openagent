...
async function fixture(
  personaBody = "first",
): Promise<{ repo: CountingRepo; context: MemoryIdentityContext }> {
  const dir = realpathSync.native(
    await mkdtemp(join(tmpdir(), "memory-prompt-")),
  );
  tempDirs.push(dir);
...
async function fixtureAtSystemTokens(
  tokens: number,
): Promise<{ repo: CountingRepo; context: MemoryIdentityContext }> {
  const header = "---\ndescription: Persona\n---\n";
  return fixture(
    "A".repeat(tokens * 4 - Buffer.byteLength(header, "utf8") - 1),
  );
}
...
function eventContext(sessionId: string, branchLength: number): unknown {
  return {
    sessionManager: {
      ...
      getBranch: () =>
        Array.from({ length: branchLength }, (_, index) => ({ index })),
    },
  };
}
...
describe("MEMORY_PROMPT_TEMPLATE", () => {
  test(
    "#given the compiled-block cache key #when the template id is read #then it is the v3 stable template",
    () => {
...
describe("createMemoryPromptHandler", () => {
  test(
    "#given an unbound or disabled session #when before_agent_start dispatches #then the handler returns undefined and never touches the repo",
    async () => {
      // given
  ...
  test(
    "#given an event context without a session manager #when before_agent_start dispatches #then the handler returns undefined",
    async () => {
      // given
  ...
  test(
    "#given a bound identity #when before_agent_start dispatches #then the prompt gains a stable sentinel block and recall metadata arrives late",
    async () => {
      // given
      const { repo, context } = await fixture();
      const pi = new FakeExtensionAPI();
  ...
  test(
    "#given the same identity and HEAD across sessions and turns #when volatile notices change #then the system block stays byte-identical and notices travel as a late message",
    async () => {
      // given
      const { repo, context } = await fixture();
      const pi = new FakeExtensionAPI();
      pi.on(
        ...
        createMemoryPromptHandler({
          resolveContext: () => context,
          createRepo: () => repo,
          resolveNudgeTurns: async (_repo, sessionId) =>
            sessionId === "session-after-threshold" ? 12 : undefined,
          resolveSoulNotice: async (_repo, sessionId) =>
            sessionId === "session-after-threshold"
      ...
      // when
      const beforeThreshold = await dispatchEvent(
        pi,
        beforeAgentStart("BASE PROMPT"),
        eventContext("session-before-threshold", 2),
      );
      const afterThreshold = await dispatchEvent(
        pi,
        beforeAgentStart("BASE PROMPT"),
        eventContext("session-after-threshold", 12),
      );
      ...
      expect(beforeThreshold?.message?.content).toContain(
        "2 previous messages",
      );
      expect(afterThreshold?.message?.content).toContain(
  ...
  test(
    "#given committed system memory below the soft threshold #when before_agent_start compiles #then the block stays byte-identical to the pre-pressure format",
    async () => {
      // given
      const { repo, context } = await fixture();
      const pi = new FakeExtensionAPI();
  ...
  test(
    "#given committed system memory exactly at floor eighty percent of the advisory #when before_agent_start compiles #then one actionable pressure line carries N M and P",
    async () => {
      // given
      ...
      const pressureLines =
        result?.systemPrompt?.split("\n").filter((line) =>
          line.includes(MEMORY_PRESSURE_METADATA_TOKEN)
        ) ?? [];
  ...
  test(
    "#given nudge state at the threshold #when before_agent_start compiles #then the late message carries the behavioral nudge token",
    async () => {
      // given
      const { repo, context } = await fixture();
      const pi = new FakeExtensionAPI();
  ...
  test(
    "#given a reflection soul notice #when before_agent_start compiles #then the late message carries the soul token and short sha",
    async () => {
      // given
      const { repo, context } = await fixture();
      const pi = new FakeExtensionAPI();
      ...
      expect(result?.message?.content).toMatch(
        /- Soul updated by reflection a1b2c3d /,
      );
    },
    30_000,
  );
  ...
  test(
    "#given an out-of-band soul commit #when the prompt compiles repeatedly at the same HEAD #then the soul line appears exactly once",
    async () => {
      // given
      const { repo, context } = await fixture();
      const watermark = {
      ...
      pi.on(
        ...
        createMemoryPromptHandler({
          resolveContext: () => context,
          createRepo: () => repo,
          resolveSoulNotice: (repoArg) =>
            consumeSoulNoticeDelta(repoArg, watermark),
        }),
      );
      ...
      const third = await dispatchEvent(
        pi,
        beforeAgentStart("BASE PROMPT"),
        eventContext("session-1", 1),
      );
  ...
  test(
    "#given another extension already rewrote the system prompt #when the handler runs #then the foreign text survives and the block is appended",
    async () => {
      // given
      const { repo, context } = await fixture();
      ...
      const [foreign] = await foreignPi.dispatch(
        "before_agent_start",
        beforeAgentStart("BASE PROMPT"),
        eventContext("session-1", 0),
      );
      const foreignPrompt =
        (foreign as BeforeAgentStartEventResult).systemPrompt ?? "";
      const result = await dispatchEvent(
        memoryPi,
        beforeAgentStart(foreignPrompt),
        eventContext("session-1", 0),
      );
  ...
  test(
    "#given an unchanged HEAD #when the handler runs twice #then HEAD is re-checked each run while the block compiles once",
    async () => {
      // given
      const { repo, context } = await fixture();
      const pi = new FakeExtensionAPI();
  ...
  test(
    "#given a commit between runs #when the next dispatch happens #then the new content appears while the prior result keeps the old content",
    async () => {
      // given
      const { repo, context } = await fixture();
      const pi = new FakeExtensionAPI();
      ...
      await repo.commitWrite(["system/persona.md"], "update persona", {
        agentId: IDENTITY,
        authorName: "Prompt Agent",
      });
  ...
  test(
    "#given a prompt already carrying our sentinel block #when the handler runs #then the block is replaced, not duplicated",
    async () => {
      // given
      const { repo, context } = await fixture();
      const pi = new FakeExtensionAPI();
      ...
      const second = await dispatchEvent(
        pi,
        beforeAgentStart(first?.systemPrompt ?? ""),
        eventContext("session-1", 2),
      );

      // then
      expect(
        second?.systemPrompt?.match(
          new RegExp(`<!-- senpi-memory:${IDENTITY}:begin -->`, "g"),
        ),
      ).toHaveLength(1);
      expect(
        second?.systemPrompt?.match(
          new RegExp(`<!-- senpi-memory:${IDENTITY}:end -->`, "g"),
        ),
      ).toHaveLength(1);
