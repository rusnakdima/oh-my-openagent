...
export type FilesystemPolicyDecision = { allow: true } | {
  allow: false;
  reason: string;
};
...
export function hasFilesystemPolicySupport(
  ...
  const candidate: {
    on: SenpiExtensionAPI["on"];
    registerFilesystemPolicy?: unknown;
  } = api;
...
function buildMemoryFilesystemPolicy(
  ...
  return {
    ...
    check(
      ...
      if (ownRoots.some((root) => isWithin(root, target))) {
        return { allow: true };
      }
      if (agentsRoots.some((root) => isWithin(root, target))) {
        return {
          allow: false,
          reason:
            `cross-identity memory access denied: ${request.operation} ${request.canonicalPath}`,
        };
      }
      return { allow: true };
    },
...
function foreignIdentityRoots(
  ...
  try {
    ...
    return stableRoots(
      entries.filter((entry) => entry.name !== ownIdentity).map((entry) =>
        join(agentsRoot, entry.name)
      ),
    );
  } catch (error) {
...
function stableRoots(paths: readonly string[]): string[] {
  return uniquePaths(
    paths.flatMap((path) => [path, canonicalizeFromNearestExisting(path)]),
  );
}
