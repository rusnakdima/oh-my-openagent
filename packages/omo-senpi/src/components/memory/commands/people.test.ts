...
describe("people graph derivation", () => {
  test(
    "#given three cards with four RELATIONSHIP lines #when the graph is derived #then nodes and edges match those lines exactly",
    async () => {
      // given
  ...
  test(
    "#given a repository without any card #when the graph is derived #then it holds no node and no edge",
    async () => {
      // given
...
describe("person query resolution", () => {
  test(
    "#given an alias #when the query resolves #then it hits the owning slug",
    async () => {
      // given
      const identity = await peopleFixture();
      const graph = await derivePeopleGraph(
        fakeDeps(identity),
        identity,
        LIMITS,
      );
  ...
  test(
    "#given an unknown name #when the query resolves #then it misses with close slugs",
    async () => {
      // given
      const identity = await peopleFixture();
      const graph = await derivePeopleGraph(
...
describe("/people command", () => {
  test(
    "#given a fixture repository #when /people runs #then the derived roster and edge sets are surfaced through one notification",
    async () => {
      // given
      const identity = await peopleFixture();
      const deps = fakeDeps(identity);
      ...
      // then
      expect(new Set(graph.nodes.map((node) => node.slug))).toEqual(
        new Set(["human", "jane-doe", "nia-blank", "sam-rivers"]),
      );
      expect(
        new Set(
  ...
  test(
    "#given an unknown name #when /people runs #then the structural query miss and error notification preserve close slugs",
    async () => {
      // given
      const identity = await peopleFixture();
      const deps = fakeDeps(identity);
