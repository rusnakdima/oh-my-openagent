...
import { type MemfsSubcommand, requireExistingRepo } from "./memfs-shared";
...
export const memfsRestore: MemfsSubcommand = async (
  { deps, ctx, parsed, identity },
) => {
  const backups = await listRepoBackups(identity);
  if (backups.length === 0) {
    return respond(
      ctx,
      `no backups found for ${identity.identity} in ${identity.identityPaths.root}`,
      "error",
    );
  }
  ...
  if (requested === undefined) {
    ...
    const selected = await ctx.ui?.select?.("Restore memory backup", [
      ...backups,
    ]);
    if (selected === undefined) {
      return respond(ctx, "restore cancelled; nothing changed");
    }
    name = selected;
  } else {
    name = requested;
  ...
  if (!backups.includes(name)) {
    return respond(
      ctx,
      `unknown backup: ${name}; available: ${backups.join(", ")}`,
      "error",
    );
  }

  const result = await restoreRepoBackup(deps, identity, name);
  const safety = result.safetyBackup === undefined
    ? ""
    : `; safety backup at ${result.safetyBackup}`;
  return respond(
    ctx,
    `restored ${result.restored} into ${identity.identityPaths.repo}${safety}`,
  );
};
