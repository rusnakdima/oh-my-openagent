...
const NOT_CONFIGURED =
  "no memory repository configured; set one with /memory-repository set <url>";

export function registerMemoryRepositoryCommand(
  pi: SenpiExtensionAPI,
  deps: MemoryCommandDeps,
): void {
  pi.registerCommand("memory-repository", {
    description:
      "Configure the push-only mirror remote for the memory repository.",
    ...
    handler: async (
      ...
      if (subcommand === undefined) {
        return respond(
          ctx,
          `usage: /memory-repository <${SUBCOMMANDS.join("|")}> [url]`,
          "error",
        );
      }
      ...
      if (subcommand === "set") {
        if (value === undefined) {
          return respond(ctx, "usage: /memory-repository set <url>", "error");
        }
        ...
        return result.pushed
          ? respond(
            ctx,
            `memory repository set to ${target}; initial push succeeded`,
          )
          : respond(
            ctx,
      ...
      if (subcommand === "unset") {
        ...
        return respond(
          ctx,
          "memory repository unset; commits stay local from now on",
        );
      }
      ...
      if (subcommand === "status") {
        ...
        if (status.lastLogLines.length > 0) {
          lines.push("", "## Recent push log", ...status.lastLogLines);
        }
      ...
      if (subcommand === "push") {
        ...
        if (status.url === undefined) {
          return respond(ctx, NOT_CONFIGURED, "error");
        }
      ...
      return respond(
        ctx,
        `unknown /memory-repository subcommand: ${subcommand}; expected one of ${
          SUBCOMMANDS.join(", ")
        }`,
        "error",
      );
