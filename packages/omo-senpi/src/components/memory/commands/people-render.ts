...
export function parsePeopleCardLines(
  raw: string | undefined,
  limits: PeopleLimits,
): readonly string[] {
  const body = fileBody(raw);
  if (body === undefined) return [];
  return parsePeopleCard(body, limits).card.entries.map((entry) =>
    `${entry.prefix}: ${entry.content}`
  );
}

/** Observation groups of a person ledger, in file order. */
export function parsePeopleObservations(
  raw: string | undefined,
  limits: PeopleLimits,
...
export function renderRoster(graph: PalacePeople): string {
  ...
  lines.push(
    ...
    } across ${graph.nodes.length} card${graph.nodes.length === 1 ? "" : "s"}.`,
  );
...
export function renderPersonCard(
  ...
  const related = edges.filter((edge) =>
    edge.source === node.slug || edge.targetSlug === node.slug
  );
  if (related.length > 0) {
    lines.push("", "## Relationships");
    for (const edge of related) {
      lines.push(`${edge.source} ${edge.predicate} -> ${edge.target}`);
    }
  }
  ...
  for (const group of observations) {
    ...
    for (const entry of group.entries) {
      lines.push(`- [${entry.date}] ${entry.content}${suffixOf(entry)}`);
    }
  }
  return lines.join("\n");
}
