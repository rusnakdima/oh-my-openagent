...
describe("/init", () => {
  test(
    "#given no repository #when invoked #then the repo is initialized and a standardized instruction turn follows waitForIdle",
    async () => {
      // given
  ...
  test(
    "#given an existing repository #when invoked #then it refuses without overwriting and sends nothing",
    async () => {
      // given
      const { root, identity } = await tempIdentity();
      tempDirs.push(root);
  ...
  test(
    "#given an unbound session #when invoked #then an actionable error is returned",
    async () => {
      // given
      const pi = new MemoryFakeExtensionAPI();
