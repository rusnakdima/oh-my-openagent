...
describe("observation selection", () => {
  test(
    "#given more observations than the per-level cap #when they are selected #then each level keeps the newest twenty, newest first",
    async () => {
      // given
      ...
      // when
      const groups = await selectObservations(
        deps,
        identity,
        "jane-doe",
        LIMITS,
      ...
      expect(
        groups.find((group) => group.section === "Inductive")?.entries.length,
      ).toBe(1);
    },
    30_000,
  );
  ...
  test(
    "#given the all flag #when observations are selected #then every entry survives",
    async () => {
      // given
      const identity = await peopleFixture(25);
      const deps = fakeDeps(identity);
      ...
      // then
      expect(
        groups.find((group) => group.section === "Explicit")?.entries.length,
      ).toBe(25);
    },
    30_000,
  );
...
describe("/people command", () => {
  test(
    "#given a person with observations #when /people <name> runs #then parsed card fields and capped observation selection are surfaced",
    async () => {
      // given
      const identity = await peopleFixture(25);
      const deps = fakeDeps(identity);
      ...
      // when
      const card =
        parsePeopleCard(parseMemoryFile(JANE_CARD).body, LIMITS).card;
      ...
      expect(
        observations.find((group) => group.section === "Explicit")?.entries.map(
          (entry) => entry.date,
        ),
      ).toEqual(
        Array.from(
