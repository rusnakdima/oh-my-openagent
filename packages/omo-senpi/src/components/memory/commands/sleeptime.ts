...
export function resolveSleeptimeSettings(
  ...
  return {
    reflection: {
      enabled: resolved(
        effectiveReflection.enabled,
        reflection?.enabled !== undefined,
      ),
      stepCount: resolved(
      ...
      merge: resolved(
        effectiveReflection.merge,
        reflection?.merge !== undefined,
      ),
      category: resolved(
        effectiveReflection.category,
        reflection?.category !== undefined,
      ),
      ...
      sandbox: resolved(
        effectiveReflection.sandbox,
        reflection?.sandbox !== undefined,
      ),
    },
    nudge: {
      enabled: resolved(
        nudge?.enabled ?? settings.nudge.enabled,
        nudge?.enabled !== undefined,
      ),
    ...
    facts: {
      enabled: resolved(
        facts?.enabled ?? settings.facts.enabled,
        facts?.enabled !== undefined,
      ),
    ...
    dream: {
      enabled: resolved(
        ...
        dream?.enabled !== undefined,
      ),
      idleMinutes: resolved(
        dream?.idle_minutes ?? settings.dream.idle_minutes,
        dream?.idle_minutes !== undefined,
      ),
    ...
    people: {
      enabled: resolved(
        people?.enabled ?? settings.people.enabled,
        people?.enabled !== undefined,
      ),
      maxEntries: resolved(
        people?.max_entries ?? settings.people.max_entries,
        people?.max_entries !== undefined,
      ),
    ...
    soul: {
      editNotice: resolved(
        soul?.edit_notice ?? settings.soul.edit_notice,
        soul?.edit_notice !== undefined,
      ),
    },
  };
}
...
export function registerSleeptimeCommand(
  pi: SenpiExtensionAPI,
  deps: MemoryCommandDeps,
): void {
  pi.registerCommand("sleeptime", {
    description:
      "Show the resolved sleeptime memory settings for this identity.",
    argumentHint: "",
    handler: async (
      _args: string,
      ...
      const lines = [
        ...
        "",
        `Reflection: ${reflection.enabled.value ? "on" : "off"}${
          mark(reflection.enabled)
        }`,
        `Step trigger: ${
          reflection.stepCount.value > 0
            ? `every ${reflection.stepCount.value} steps`
            : "off"
        }${mark(reflection.stepCount)}`,
        `On compaction: ${reflection.onCompaction.value ? "on" : "off"}${
          mark(reflection.onCompaction)
        }`,
        `Merge policy: ${reflection.merge.value}${mark(reflection.merge)}`,
        `Category: ${reflection.category.value}${mark(reflection.category)}`,
        `Timeout: ${reflection.timeoutMinutes.value} minutes${
          mark(reflection.timeoutMinutes)
        }`,
        `Sandbox: ${reflection.sandbox.value}${mark(reflection.sandbox)}`,
        "",
        `Nudge: ${nudge.enabled.value ? "on" : "off"}${mark(nudge.enabled)}`,
        `Nudge every: every ${nudge.everyUserTurns.value} turns${
          mark(nudge.everyUserTurns)
        }`,
        "",
        `Facts: ${facts.enabled.value ? "on" : "off"}${mark(facts.enabled)}`,
        `Facts debounce: debounce ${facts.debounceSettles.value} settles${
          mark(facts.debounceSettles)
        }`,
        "",
        `Dream: ${dream.enabled.value ? "on" : "off"}${mark(dream.enabled)}`,
        `Dream idle: idle ${dream.idleMinutes.value} minutes${
          mark(dream.idleMinutes)
        }`,
        `Dream spacing: min ${dream.minHoursBetween.value}h between${
          mark(dream.minHoursBetween)
        }`,
        `Dream shutdown: shutdown launch ${
          dream.shutdownLaunch.value ? "on" : "off"
        }${mark(dream.shutdownLaunch)}`,
        `Dream select: select max ${dream.autoSelectMax.value}${
          mark(dream.autoSelectMax)
        }`,
        `Dream select chars: max ${dream.autoSelectMaxChars.value} chars${
          mark(dream.autoSelectMaxChars)
        }`,
        "",
        `People: ${people.enabled.value ? "on" : "off"}${mark(people.enabled)}`,
        `People entries: max ${people.maxEntries.value} entries${
          mark(people.maxEntries)
        }`,
        `People chars: max ${people.maxEntryChars.value} chars${
          mark(people.maxEntryChars)
        }`,
        "",
        `Soul: edit notice ${soul.editNotice.value ? "on" : "off"}${
          mark(soul.editNotice)
        }`,
        "",
        `Edit ${
          configPath ?? "your omo config file"
        } under memory.reflection, or memory.agents.${agentId} for this identity only.`,
