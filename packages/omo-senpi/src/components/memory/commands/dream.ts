...
export function registerDreamCommand(
  ...
  pi.registerCommand("dream", {
    ...
    handler: async (
      ...
      try {
        targetDoc = resolveTargetDoc(
          identity.identityPaths.repo,
          parsed.flags.get("to"),
        );
      } catch (error) {
      ...
      try {
        if (parsed.flags.has("recent")) {
          const recentN = positiveInteger(
            parsed.flags.get("recent"),
            "--recent",
          );
          const settings = resolveDreamTriggerSettings(
            deps.loadSettings().settings,
            identity.identity,
          );
          ...
          if (selected.conversationIds.length === 0) {
            throw new TypeError("--recent found no unreflected conversations");
          }
          ...
          if (
            !source.startsWith("transcript:") ||
            source.slice("transcript:".length).length === 0
          ) {
            throw new TypeError(
              "--from expects transcript:<path> to a senpi session JSONL file",
            );
          }
      ...
      if (!outcome.fired) {
        return respond(ctx, dreamRejection(outcome.rejection), "error");
      }
      return respond(
        ctx,
...
function resolveTargetDoc(
  ...
  let absolute: string;
  try {
    absolute = validateMemoryPath(repoDir, raw, {
      fieldName: "--to",
      toolPath: false,
    });
  } catch {
    throw new TypeError(
      "--to must be a memory-repo-relative document path without traversal",
    );
  }
  const path = relative(repoDir, absolute).split(sep).join("/");
  if (!path.endsWith(".md")) {
    throw new TypeError("--to must target a .md document in the memory repo");
  }
  return path;
}
...
function conversationList(value: string | true | undefined): string[] {
  const ids = requiredValue(value, "--conversation").split(",").map((id) =>
    id.trim()
  ).filter(Boolean);
  if (ids.length === 0) {
    throw new TypeError(
      "--conversation expects one or more comma-separated ids",
    );
  }
  return [...new Set(ids)];
}
...
function dreamRejection(rejection: string): string {
  if (rejection === "no_unreflected_content") {
    return "dream found no unreflected content to process";
  }
  if (rejection === "no_session") {
    return "dreaming is not available without a bound memory session";
  }
