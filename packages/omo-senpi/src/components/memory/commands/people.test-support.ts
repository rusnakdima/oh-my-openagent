...
function observationsFile(count: number): string {
  const lines = [
    "---",
    "description: Observations - Jane Doe",
    "---",
    "",
    "## Explicit",
    "",
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    lines.push(
      `- [2026-03-${day}] explicit note ${index + 1} <!-- src: s-${
        index + 1
      } -->`,
    );
  }
  lines.push(
    ...
    "## Inductive",
    "",
    "- [2026-01-05] tends to review in the morning <!-- pattern: cadence; confidence: low -->",
  );
  return `${lines.join("\n")}\n`;
}
...
export async function peopleFixture(
  ...
  await seededRepo(identity, [
    ...
    ...(observationCount === 0 ? [] : [{
      relativePath: "people/jane-doe/observations.md",
      content: observationsFile(observationCount),
    }]),
  ]);
  return identity;
}
