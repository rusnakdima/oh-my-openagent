...
function conversationsIn(
  records: readonly { readonly conversationId: string }[],
): string[] {
...
async function runRetry(
  ...
  try {
    ...
    return respond(
      ctx,
      `cannot retry: the failure ledger is unreadable (${
        error instanceof Error ? error.message : String(error)
      }); repair or remove failures.json first`,
      "error",
    );
  }
  ...
  if (matching.length === 0) {
    ...
    return respond(
      ctx,
      `no failure records to clear for ${scope}; nothing was retried`,
      "info",
    );
  }

  const filter: FactsFailureFilter = conversationId === undefined
    ? {}
    : { conversationId };
  ...
  return respond(
    ctx,
    `cleared ${removed} record${removed === 1 ? "" : "s"} for ${
      names.join(", ")
    }; one launch attempt was triggered`,
  );
}

export function registerFactsCommand(
  ...
  pi.registerCommand("facts", {
    ...
    handler: async (
      ...
      const conversationId = typeof conversationFlag === "string"
        ? conversationFlag
        : undefined;
