...
describe("people command people.enabled gate", () => {
  test(
    "#given people.enabled false #when /people runs #then it refuses without reading any record",
    async () => {
      // given
        ...
        fakeDeps(identity, {
          loadSettings: () => ({
            settings: memorySettings({
              people: { enabled: false, max_entries: 40, max_entry_chars: 200 },
            }),
          }),
        }),
      );
  ...
  test(
    "#given people.enabled true #when the suite registers #then /people is present",
    async () => {
      // given
      const identity = await peopleFixture();
      const pi = new MemoryFakeExtensionAPI();
