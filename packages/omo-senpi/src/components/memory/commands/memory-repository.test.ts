...
async function harness(options: { readonly seeded?: boolean } = {}) {
  ...
  tempDirs.push(root);
  const repo = options.seeded === false
    ? undefined
...
describe("/memory-repository", () => {
  test(
    "#given a credentialed url #when set runs #then config is written and output is redacted",
    async () => {
      // given
      ...
      expect(
        ctx.ui.notifications.every((entry) =>
          !entry.message.includes("s3cr3t-token")
        ),
      ).toBe(true);
    },
    20_000,
  ...
  test(
    "#given a configured mirror #when status runs #then the redacted url and ahead count render",
    async () => {
      // given
      const { repo, pi, ctx } = await harness();
  ...
  test(
    "#given an unreachable mirror #when push runs #then the failure is reported with a redacted detail",
    async () => {
      // given
      const { repo, pi, ctx } = await harness();
      await repo?.configSet(CONFIG_KEY, CREDENTIALED_URL);
