...
async function renderSystemSection(
  ...
  const systemPaths = paths.filter(isSystemMarkdown).sort((left, right) =>
  ...
  if (systemPaths.length === 0) {
    return [
      ...
      "",
      "_no system memory files committed_",
    ];
  }
  ...
  pi.registerCommand("memory", {
    ...
    handler: async (
      ...
      if (head === null) {
        return respond(
          ctx,
          `memory repository at ${identity.identityPaths.repo} has no commits yet; run /init`,
          "error",
        );
      }
      ...
      const externalPaths = paths.filter((path) =>
        !isSystemMarkdown(path) && !isSkillPath(path)
      );
