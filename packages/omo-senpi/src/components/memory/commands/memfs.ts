...
import { installHooks, MirrorSync } from "@oh-my-opencode/memory-core";
...
import {
  type MemfsSubcommand,
  type MemfsSubcommandInput,
  requireExistingRepo,
} from "./memfs-shared";
import {
...
const memfsStatus: MemfsSubcommand = async ({ deps, ctx, identity }) => {
  ...
  const mirror = await new MirrorSync({
    repo,
  ...
  const lines = [
    ...
    `Working tree: ${
      dirty
        ? `dirty — ${porcelain.length} entr${
          porcelain.length === 1 ? "y" : "ies"
        }`
        : "clean"
    }`,
    `Mirror: ${mirror.redactedUrl ?? "not configured"}${
      mirror.url === undefined ? "" : ` (${mirror.aheadCount} ahead)`
    }`,
  ];
  if (dirty) {
    lines.push(
...
const memfsInit: MemfsSubcommand = async ({ deps, ctx, identity }) => {
  if (hasGitRepo(identity)) {
    ...
    return respond(
      ctx,
      `memory repository already initialized at ${identity.identityPaths.repo}${
        head === null ? "" : ` (HEAD ${shortSha(head)})`
      }`,
    );
  }
  ...
  return respond(
    ctx,
    `initialized memory repository at ${identity.identityPaths.repo} (HEAD ${
      shortSha(sha)
    })`,
  );
};
...
const memfsSync: MemfsSubcommand = async ({ deps, ctx, identity }) => {
  ...
  if (!result.pushed) {
    return respond(
      ctx,
      `push to ${status.redactedUrl} failed: ${result.detail}`,
      "error",
    );
  }
...
const memfsRepair: MemfsSubcommand = async ({ deps, ctx, identity }) => {
  ...
  const lines = [
    ...
    `- scanned ${repaired.scanned} skill file${
      repaired.scanned === 1 ? "" : "s"
    }`,
  ];
  if (report.length > 0) {
    lines.push(...report.split("\n").map((line) => `- ${line}`));
  }
  return respond(ctx, lines.join("\n"));
};
...
const memfsReset: MemfsSubcommand = async ({ deps, ctx, parsed, identity }) => {
  ...
  if (isInteractive(ctx)) {
    ...
    if (confirmed !== true) {
      return respond(ctx, "reset aborted; nothing changed");
    }
  ...
  return respond(
    ctx,
    `reset complete; backup at ${backup}; new HEAD ${shortSha(sha)}`,
  );
};

const SKILLS_USAGE_FILENAME = "skills-usage.json";

async function renderSkillsUsage(
  runtimeDir: string,
  ...
  for (const [skillId, value] of entries) {
    ...
    const lastUsedAt = typeof entry.lastUsedAt === "string"
      ? entry.lastUsedAt
      : "unknown";
    lines.push(
      `  ${skillId}: ${count} read${
...
export function registerMemfsCommand(
  ...
  pi.registerCommand("memfs", {
    ...
    handler: async (
      ...
      const identity = subcommand === "init"
        ? identityForInit(deps, ctx)
        : requireIdentity(deps, ctx);
