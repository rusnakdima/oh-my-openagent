...
export function resolvePersonQuery(
  ...
  if (needle.length === 0) {
    return {
      kind: "miss",
      close: nodes.map((node) => node.slug),
    };
  }
...
function closeSlugs(
  ...
  return nodes
    .map((node) => ({
      slug: node.slug,
      score: closeness(node, needle, collapsed),
    }))
...
function closeness(
  ...
  return shared >= MIN_SHARED_PREFIX
    ? 1 + shared / Math.max(flat.length, collapsed.length)
    : 0;
}
...
function sharedPrefix(left: string, right: string): number {
  let index = 0;
  while (
    index < left.length && index < right.length && left[index] === right[index]
  ) index += 1;
  return index;
}
