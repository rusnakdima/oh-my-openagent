...
function resolveSessionsDir(
  ...
  return ctx.agentDir === undefined
    ? undefined
    : join(ctx.agentDir, "sessions");
}
...
export function registerSearchCommand(
  ...
  pi.registerCommand("search", {
    description:
      "Search past session transcripts and show scored, numbered results.",
    ...
    handler: async (
      ...
      if (query.length === 0) {
        return respond(
          ctx,
          "usage: /search <query> [--include-hidden]",
          "error",
        );
      }
      ...
      if (sessionsDir === undefined || !existsSync(sessionsDir)) {
        return respond(
          ctx,
          `no senpi sessions directory found at ${
            sessionsDir ?? "<unknown>"
          }; start a session first or check the agent directory`,
          "error",
        );
      }
      ...
      if (results.length === 0) {
        return respond(ctx, `No matches for "${query}".`);
      }

      const { terms, phrases } = parseQuery(query);
      const needles = [...phrases, ...terms];
      const lines = [
        `# Search: "${query}" — ${results.length} result${
          results.length === 1 ? "" : "s"
        }`,
        "",
      ];
      results.forEach((result, index) => {
        lines.push(
          `${
            index + 1
          }. session ${result.conversationId} · ${result.createdAt} · ${
            result.document.messageType ?? "unknown"
          }`,
