...
export function trackSendOrder(
...
export async function tempIdentity(
  root?: string,
): Promise<{ root: string; identity: MemoryCommandIdentity }> {
  const identityRoot = root ??
    (await mkdtemp(join(tmpdir(), "memory-commands-")));
  return {
    root: identityRoot,
    identity: {
...
export async function invoke(
  ...
  const handler = registration.options.handler as (
    args: string,
