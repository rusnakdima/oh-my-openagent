...
async function seedQueueArtifacts(
  identity: MemoryCommandIdentity,
): Promise<string[]> {
  ...
  const entryPath = join(
    layout.queueDir,
    "20260816T115900000Z-aaaaaaaaaaaa-bbbbbbbb.json",
  );
  await writeFile(
    entryPath,
  ...
  await writeFile(
    cursorPath,
    `${
      JSON.stringify({ version: 1, enqueued_through_snapshot_line: 4 }, null, 2)
    }\n`,
    "utf8",
  );
...
describe("/facts", () => {
  ...
  test("#given records for two conversations #when retrying one #then only its records are cleared", async () => {
    ...
    // when
    const text = await invokeFacts(
      factsDeps(identity),
      "retry --conversation conv-a",
    );

    // then
    const remaining = await store.readFailures();
    expect(remaining.entries.map((record) => record.conversationId)).toEqual([
      "conv-b",
    ]);
    expect(text).toContain("conv-a");
  ...
  test("#given queue files and watermarks #when retrying #then every queue artifact is byte-identical", async () => {
    ...
    const namesBefore =
      (await readdir(factsQueuePaths(identity.identityPaths).queueDir)).sort();
