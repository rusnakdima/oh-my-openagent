...
async function harness() {
  ...
  return {
    ...
    requests,
    pi,
    ctx: fakeCommandContext({ agentDir: root }),
  };
}
...
async function writeConversation(
  ...
  const journal = new TranscriptJournal({
    journalDir: join(transcriptsDir, conversationId),
    now: () => NOW,
  });
...
describe("/dream external transcript staging and target validation", () => {
  test("#given a senpi JSONL transcript with a malformed line #when imported twice #then message ids are staged once", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        JSON.stringify({
          ...
          message: {
            role: "assistant",
            content: [{ type: "text", text: "tabs are preferred" }],
          },
        }),
      ].join("\n"),
      "utf8",
    ...
    // when
    const first = await stageDreamTranscript(
      transcript,
      identity.identityPaths.transcripts,
      ctx.agentDir ?? "",
    );
    const second = await stageDreamTranscript(
      transcript,
      identity.identityPaths.transcripts,
      ctx.agentDir ?? "",
    );
    ...
    const secondDerivedId = createHash("sha1")
      ...
      .digest("hex")
      .slice(0, 16);
    expect(new Set(first.stagedMessageIds)).toEqual(
      new Set([firstDerivedId, secondDerivedId]),
    );
    expect(second.stagedMessageIds).toEqual([]);
    expect(new Set(second.skippedMessageIds)).toEqual(
      new Set(first.stagedMessageIds),
    );
    ...
    expect(textRows.map((row) => row.text).sort()).toEqual([
      "remember tabs",
      "tabs are preferred",
    ]);
    expect(
      textRows.find((row) => row.source_message_id === firstDerivedId)
        ?.captured_at,
    ).toBe(fileMtime.toISOString());
    expect(
      textRows.find((row) => row.source_message_id === secondDerivedId)
        ?.captured_at,
    ).toBe(fileMtime.toISOString());
  });
  ...
  test("#given non-session JSONL #when imported #then it is rejected without submitting a dream", async () => {
    ...
    await writeFile(
      transcript,
      `${JSON.stringify({ kind: "user", text: "not senpi" })}\n`,
      "utf8",
    );
  ...
  test("#given valid and escaping --to paths #when invoked #then only the memory-relative document is accepted", async () => {
    ...
    await seededRepo(identity, [
      {
        relativePath: "reference/style.md",
        content: "---\ndescription: Style\n---\nOriginal.\n",
      },
    ]);
    ...
    await invoke(
      pi,
      "dream",
      "--conversation sess-a --to reference/style.md",
      ctx,
    );
    const traversal = await invoke(
      pi,
      "dream",
      "--conversation sess-a --to ../escape",
      ctx,
    );
    const absolute = await invoke(
      pi,
      "dream",
      "--conversation sess-a --to /abs",
      ctx,
    );

    // then
    expect(requests).toEqual([{
      conversationIds: ["sess-a"],
