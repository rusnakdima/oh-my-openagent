...
export { type PersonQueryResolution, resolvePersonQuery } from "./people-query";
...
export async function selectObservations(
  ...
  const raw = await readPersonFile(
    deps,
    identity,
  ...
  return groups.map((group) => {
    ...
    return {
      ...
      entries: options.all ? sorted : sorted.slice(0, OBSERVATIONS_PER_LEVEL),
    };
  });
}
...
export function registerPeopleCommand(
  ...
  pi.registerCommand("people", {
    ...
    handler: (args: string, ctx: MemoryCommandContext): Promise<string> =>
      handlePeople(deps, args, ctx),
  });
}
...
async function handlePeople(
  ...
  if (people?.enabled === false) {
    return respond(
      ctx,
      "people records are disabled (memory.people.enabled)",
      "error",
    );
  }
  ...
  if (resolved.kind === "miss") {
    const close = resolved.close.length === 0
      ? "no people records exist yet"
      : `closest slugs: ${resolved.close.join(", ")}`;
    return respond(ctx, `no person matches "${name}"; ${close}`, "error");
  }

  const node = graph.nodes.find((candidate) =>
    candidate.slug === resolved.slug
  );
  if (node === undefined) {
    return respond(ctx, `no person matches "${name}"`, "error");
  }
  // `--ask` reasons over the capped evidence set, so the `--all` widening is a render-only flag.
  const observations = await selectObservations(
    deps,
    identity,
    node.slug,
    limits,
  ...
  return respond(
    ...
    renderPersonCard(
      node,
      parsePeopleCardLines(cardBody, limits),
      observations,
      graph.edges,
    ),
  );
  ...
  const evidence: PeopleAskEvidence = {
    ...
    observations: observations.flatMap((group) =>
      group.entries.map((entry) =>
        `[${group.section}] ${entry.date} ${entry.content}`
      )
    ),
...
async function readPersonFile(
  ...
  const working = await readFile(
    join(identity.identityPaths.repo, path),
    "utf8",
  ).catch(() => undefined);
...
function extractAsk(
  args: string,
): { readonly question: string | undefined; readonly rest: string } {
  ...
  const rest = `${args.slice(0, match.index)} ${
    args.slice((match.index ?? 0) + match[0].length)
  }`;
  return { question: question.length === 0 ? undefined : question, rest };
}

function byDateDescending(
  left: { readonly date: string },
  right: { readonly date: string },
): number {
...
function modelRegistryOf(
  ctx: MemoryCommandContext,
): Parameters<typeof createPeopleAskRunner>[0]["registry"] {
