...
function worstLevel(checks: readonly DoctorCheck[]): CheckLevel {
  return checks.reduce<CheckLevel>(
    (
      worst,
      check,
    ) => (LEVEL_ORDER[check.level] > LEVEL_ORDER[worst] ? check.level : worst),
    "ok",
  );
}
...
export function registerDoctorCommand(
  ...
  pi.registerCommand("doctor", {
    description:
      "Run deterministic memory health checks and repair skill frontmatter.",
    argumentHint: "",
    handler: async (
      _args: string,
      ...
      if (repository.level === "ok") {
        ...
        extra.push(
          `[info] skills: scanned ${repaired.scanned} skill file${
      ...
      if (level === "fail") {
        lines.push("", "fix the failing checks above, then re-run /doctor");
      } else if (level === "warn") {
        lines.push(
          "",
          "warnings do not block memory; re-run /doctor after addressing them",
        );
      }
