...
export async function stageDreamTranscript(
  ...
  if (!metadata.isFile()) {
    throw new TypeError(
      "--from transcript:<path> must name a senpi session JSONL file",
    );
  }
  if (metadata.size > MAX_TRANSCRIPT_BYTES) {
    throw new TypeError("--from transcript file exceeds the 64 MiB limit");
  }
  ...
  const capturedAtById = new Map(
    normalized.map((item) => [String(item.value.id), item.capturedAt]),
  );
  const projections = projectSessionEntries(
    normalized.map((item) => item.value),
  );
  const entries = projections.flatMap((projection) =>
    projectTranscriptEntries(
      projection,
      capturedAtById.get(projection.messageId) ?? fallbackCapturedAt,
    )
  );
  if (entries.length === 0) {
    throw new TypeError(
      "--from senpi session JSONL contains no stageable user or assistant messages",
    );
  }

  const conversationId = `from-transcript-${sha1(filePath).slice(0, 12)}`;
  const journal = new TranscriptJournal({
    journalDir: `${transcriptsDir}/${conversationId}`,
  });
  const existingMessageIds = new Set(
    (await journal.readEntries()).map((entry) => entry.source_message_id),
  );
  const fresh = entries.filter((entry) =>
    !existingMessageIds.has(entry.source_message_id)
  );
  const stagedMessageIds = unique(
    fresh.map((entry) => entry.source_message_id),
  );
  const skippedMessageIds = unique(
    entries
...
function normalizeMessage(
  ...
  if (
    row.type !== "message" || !isRecord(row.message) ||
    typeof row.message.role !== "string"
  ) return undefined;
  const suppliedId = stringOf(row.source_message_id);
  const text = messageText(row.message);
  const id = suppliedId ??
    sha1(`${filePath}:${lineIndex}:${text}`).slice(0, 16);
...
function messageText(message: Record<string, unknown>): string {
  ...
  for (const part of message.content) {
    ...
    if (part.type === "toolCall" && part.arguments !== undefined) {
      values.push(safeStringify(part.arguments));
    }
  }
  return values.join("\n");
}
