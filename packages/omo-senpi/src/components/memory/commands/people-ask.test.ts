...
describe("/people --ask", () => {
  test(
    "#given no evidence for the person #when --ask runs #then it abstains without launching a child",
    async () => {
      // given
      ...
      const text = await invoke(
        pi,
        "people",
        'nia-blank --ask "does nia ship on fridays?"',
        ctx,
      );

      // then
      expect(hasNoEvidence({ card: [], observations: [], searchHits: [] }))
        .toBe(true);
  ...
  test(
    "#given card and observation evidence #when --ask runs #then the quick child receives that evidence and its answer renders",
    async () => {
      // given
      ...
      const text = await invoke(
        pi,
        "people",
        'jane-doe --ask "how does jane review?"',
        ctx,
      );
