...
function nextEligibleMs(state: FactsOverview): number | undefined {
  const instants = backoff(state)
    .map((
      ...
      ? Number.NaN
      : Date.parse(record.nextEligibleAt))
    )
...
export function formatFactsAdvisory(state: FactsOverview): string | undefined {
  if (state.corrupt !== undefined) {
    return "facts: failure ledger unreadable (run /facts for detail)";
  }
...
export function factsRemediationHint(state: FactsOverview): string | undefined {
  ...
  const parkedCount = parked(state).length;
  if (parkedCount > 0) {
    return `${parkedCount} parked facts batch${
      parkedCount === 1 ? "" : "es"
    } need${
      parkedCount === 1 ? "s" : ""
    } a manual unpark; run /facts retry after fixing the cause`;
  }
  const backoffCount = backoff(state).length;
  if (backoffCount === 0) return undefined;
  const wait = nextEligibleMs(state);
  return `${backoffCount} facts batch${
    backoffCount === 1 ? " is" : "es are"
  } backing off${
    wait === undefined ? "" : `; next attempt in ${formatWait(wait)}`
  }`;
}

/** The full read-only `/facts` view. Never enters model context; callers notify + return it. */
export function formatFactsStatus(
  identity: string,
  state: FactsOverview,
): string {
  const lines = [
    `# Facts pipeline: ${identity}`,
    "",
    `queued: ${state.pending} pending`,
  ];
  if (state.corrupt !== undefined) {
    lines.push(
      "",
  ...
  lines.push(
    ...
    `backoff: ${backoffRecords.length}${
      wait === undefined ? "" : ` (next eligible ${formatWait(wait)})`
    }`,
    "",
  );
  ...
  for (const record of newestFirst.slice(0, MAX_LISTED_CONVERSATIONS)) {
    ...
    lines.push(
      `- ${record.conversationId}: ${record.state} after ${record.streak} (${record.lastReason})${
...
export async function readFactsOverview(
  input: ReadFactsOverviewInput,
): Promise<FactsOverview> {
