...
async function setup(): Promise<
  { sessionsDir: string; pi: MemoryFakeExtensionAPI }
> {
  const sessionsDir = realpathSync.native(
    await mkdtemp(join(tmpdir(), "memory-search-sessions-")),
  );
  ...
  await writeSession(sessionsDir, "sess-a", "sess-a", [
    {
      id: "msg-1",
      role: "user",
      text: "how do I run the linter before committing",
      timestamp: "2026-08-01T10:00:01.000Z",
    },
    {
      id: "msg-2",
      role: "assistant",
      text: "run bun run lint first, then commit",
  ...
  await writeSession(sessionsDir, "sess-b", "sess-b", [
    {
      id: "msg-9",
      role: "user",
      text: "tabs versus spaces preference",
      timestamp: "2026-08-02T09:00:00.000Z",
    },
  ]);
  const hiddenPath = await writeSession(
    sessionsDir,
    "sess-hidden",
    "sess-hidden",
    [
      {
        id: "msg-h",
        role: "user",
        text: "linter notes from a hidden session",
  ...
  registerSearchCommand(
    pi,
    fakeDeps(identity, { sessionsDir: () => sessionsDir }),
  );
...
describe("/search", () => {
  test("#given sessions on disk #when queried #then numbered results carry session id, date, role, snippet, and entry id", async () => {
    ...
    expect(text).toContain(
      "1. session sess-a · 2026-08-01T10:00:01.000Z · user",
    );
  ...
  test("#given a missing sessions directory #when invoked #then an actionable error is returned", async () => {
    ...
    registerSearchCommand(
      pi,
      fakeDeps(undefined, { sessionsDir: () => "/nonexistent/sessions" }),
    );
    const ctx = fakeCommandContext();
