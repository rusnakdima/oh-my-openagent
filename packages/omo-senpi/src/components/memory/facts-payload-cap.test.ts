...
describe("facts payload byte cap", () => {
  test(
    "#given a prepared facts spawn #when its payload file is read #then the bytes equal the shared measurement",
    async () => {
      // given
      ...
      expect(Buffer.byteLength(written, "utf8")).toBe(
        measureFactsPayloadBytes(payload),
      );
    },
    30_000,
  );
  ...
  test(
    "#given a backlog larger than the cap #when a launch runs #then the payload stays within the cap and only shipped endpoints are consumed",
    async () => {
      ...
      const result = await new FactsExtractorRunner({
        ...
        sandbox: (args) => {
          payloadBytes.push(
            Buffer.byteLength(readFileSync(args.paths.payload, "utf8"), "utf8"),
          );
      ...
      expect(ledgers[0]?.queued.map((key) => key.conversationId).sort())
        .toEqual(["session-1", "session-3"]);
      expect(ledgers[1]?.queued.map((key) => key.conversationId)).toEqual([
        "session-2",
      ]);
      ...
      expect(Object.keys(consumed.consumed).sort()).toEqual([
        "session-1",
        "session-2",
        "session-3",
      ]);
  ...
  test(
    "#given a conversation whose older entry is unconsumed #when the cap forbids both #then neither ships and the watermark stays put",
    async () => {
      ...
      await publish(queue, identity, "session-1", [
        first,
        second,
        message("m3", bulk()),
      ]);
      ...
      const ledgers = await runLedgers(identity);
      expect(
        ledgers.map((ledger) => ledger.queued.map((key) => key.end_message_id)),
      ).toEqual([["m1", "m2"], ["m3"]]);
      const consumed = JSON.parse(
        await readFile(factsQueuePaths(identity.paths).consumedPath, "utf8"),
      ) as FactsConsumedWatermark;
