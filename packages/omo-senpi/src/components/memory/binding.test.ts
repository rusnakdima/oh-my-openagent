...
describe("memory binding", () => {
  test("#given an identity repo and clock #when a binding is created #then it carries a stable path hash without exposing the path", () => {
    const binding = createMemoryBinding({
      identity: "agent-123",
      repoPath: "/private/memory/repo",
      boundAt: 42,
    });

    expect(binding).toEqual({
      identity: "agent-123",
      repoPathHash: expect.stringMatching(/^[a-f0-9]{64}$/),
      boundAt: 42,
    });
    ...
    const latest = findLatestMemoryBinding([
      {
        ...
        data: { identity: "old", repoPathHash: "a", boundAt: 1 },
      },
      {
        type: "custom",
        customType: MEMORY_BINDING_CUSTOM_TYPE,
      ...
      },
      {
        type: "custom",
        customType: MEMORY_BINDING_CUSTOM_TYPE,
        data: { identity: "new", repoPathHash: "b", boundAt: 2 },
      },
    ]);
