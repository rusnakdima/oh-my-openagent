...
export interface DreamTriggerSession {
  ...
  ) => Promise<Pick<TranscriptJournal, "captureReflectionSnapshot">>;
...
export type DreamFireRejection =
  | DreamGateRejection
  | "no_unreflected_content"
  | "no_session"
  | "aborted";
...
export function createDreamTriggerWiring(
  options: DreamTriggerWiringOptions,
): DreamTriggerWiring {
  const now = options.now ?? Date.now;
  const scheduler = options.scheduler ?? defaultScheduler;
  const timers = new Map<
    string,
    { readonly handle: unknown; readonly eventCtx: unknown }
  >();
  const inFlight = new Set<Promise<void>>();
  const launch = (
    session: DreamTriggerSession,
    origin: "idle" | "shutdown" | "manual" | "pressure",
    ...
    fireDream({
      ...
      request,
      now,
      warnLaunchFailure: (error) =>
        options.logger?.warn("omo-senpi memory dream launch failed", {
  ...
  function track(task: () => Promise<void>): void {
    const promise = task()
      .catch((error: unknown) => {
        options.logger?.warn("omo-senpi memory dream trigger failed", {
          error: describe(error),
        });
      })
      .finally(() => {
  ...
  function fire(conversationId: string): void {
    ...
    if (session === undefined || session.conversationId !== conversationId) {
      return;
    }
    track(async () => {
...
function isIdleNow(eventCtx: unknown): boolean {
  ...
  if (typeof isIdle !== "function" || typeof hasPending !== "function") {
    return false;
  }
  return Reflect.apply(isIdle, eventCtx, []) === true &&
    Reflect.apply(hasPending, eventCtx, []) === false;
}
