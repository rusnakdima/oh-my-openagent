...
import { type MemoryUsageTracker, registerMemoryUsage } from "./memory-usage";
...
export function registerMemoryStatic(input: {
  ...
  ) => ReflectionCompletionApi | undefined;
  readonly resolveContext: (
    sessionId: string,
  ) => MemoryIdentityContext | undefined;
  readonly journalWiringFor: (
    identity: MemoryIdentityContext,
  ) => MemoryJournalWiring;
  readonly factsWiringFor: (
    identity: MemoryIdentityContext,
  ) => MemoryFactsWiring;
  readonly runtimeFor: (
    identity: MemoryIdentityContext,
  ) => MemoryIdentityRuntime;
  readonly triggerSessionFor: Parameters<
    typeof createReflectionTriggerWiring
  >[0]["resolveSession"];
  ...
  const promptHandler = createPromptHandler({
    ...
    resolveCompileWarnTokens: () =>
      loadCommandSettings().settings.compile_warn_tokens,
    resolveNudgeTurns: (repo, sessionId, identity) =>
      nudgeWiring.nudgeTurns(repo, sessionId, identity),
    resolveSoulNotice: async (repo, sessionId, identity) => {
      const context = resolveContext(sessionId);
      if (context === undefined || context.identity !== identity) {
        return undefined;
      }
  ...
  registerMemoryToolSurface(
    ...
    () => (activeSession.current === undefined
      ? undefined
      : resolveContext(activeSession.current)),
    {
      exposure: toolExposure,
      onCommit: (commit) => {
        const context = activeSession.current === undefined
          ? undefined
          : resolveContext(activeSession.current);
      ...
      writeNotice: {
        get enabled(): boolean {
          return resolveWriteNoticeEnabled(
            loadCommandSettings,
            activeSession,
            resolveContext,
          );
  ...
  registerMemoryCommands(pi, {
    contextForSession: (sessionId) =>
      asCommandIdentity(resolveContext(sessionId)),
    resolveIdentity: () => (activeSession.current === undefined
      ? undefined
      : asCommandIdentity(resolveContext(activeSession.current))),
    ...
    reflectionSink: {
      request: async (request) => {
        if (activeSession.current === undefined) {
          throw new Error("no bound memory session");
        }
        ...
        const result = await runtime.store.evaluate(activeSession.current, {
          ...
          ...(request.recentN === undefined
            ? {}
            : { recentN: request.recentN }),
        ...
        return {
          disposition: result.status === "active" ? "reserved" : "pending",
          runId: result.run.runId,
        };
      },
    },
    dreamSink: {
      request: (request) => dreamTriggerWiring.requestManualDream(request),
    },
    factsSink: {
      ...
      reconcile: async () => {
        const identity = activeSession.current === undefined
          ? undefined
          : resolveContext(activeSession.current);
    ...
    sessionsDir: () =>
      join(
        options.env.SENPI_CODING_AGENT_DIR ??
...
function resolveWriteNoticeEnabled(
  ...
  try {
    ...
    const identity = activeSession.current === undefined
      ? undefined
      : resolveContext(activeSession.current)?.identity;
    const override = identity === undefined
      ? undefined
      : settings.agents[identity]?.write_notice;
...
function asCommandIdentity(
  identity: MemoryIdentityContext | undefined,
): MemoryCommandIdentity | undefined {
  if (identity === undefined) return undefined;
