...
export const MEMORY_NUDGE_METADATA_TOKEN =
...
export function createMemoryPromptHandler(
  ...
  return async (payload, eventCtx) => {
    ...
    const nudgeTurns = await options.resolveNudgeTurns?.(
      repo,
      session.id,
      context.identity,
    );
    const soulNotice = await options.resolveSoulNotice?.(
      repo,
      session.id,
      context.identity,
    );
    const block = await cache.compile(
      repo,
      `${MEMORY_PROMPT_TEMPLATE}:${context.identity}`,
      {
    ...
    const composed = options.searchExposure?.() === true
      ? `${pressureBlock}\n\n${MEMORY_TOOL_DISCOVERY_NOTE}`
      : pressureBlock;
    return {
      systemPrompt: replaceMemoryBlock(
        systemPrompt,
        markMemoryBlock(context.identity, composed),
      ),
      message: {
        customType: MEMORY_NOTICE_CUSTOM_TYPE,
        content: renderMemoryNotice(
          session.priorMessageCount,
          nudgeTurns,
          soulNotice,
        ),
        display: false,
...
async function addMemoryPressureMetadata(
  ...
  const softThreshold = Math.floor(
    MEMORY_PRESSURE_SOFT_RATIO * compileWarnTokens,
  );
  if (estimate < softThreshold) return block;
  const percentage = Math.floor((estimate / compileWarnTokens) * 100);
  const line =
    `- ${MEMORY_PRESSURE_METADATA_TOKEN} system/ ~${estimate}/${compileWarnTokens} tokens (${percentage}% of advisory); trim or demote stale system/ blocks via the memory tool or run /dream`;
...
function renderMemoryNotice(
  ...
  return [
    ...
    ...(soulNotice === undefined ? [] : [
      `- ${MEMORY_SOUL_METADATA_TOKEN} reflection ${
        soulNotice.sha.slice(0, 7)
      } since your last run`,
    ]),
    "</memory_notice>",
  ].join("\n");
...
function readSystemPrompt(payload: unknown): string | undefined {
  ...
  return typeof payload.systemPrompt === "string"
...
function readPromptSession(eventCtx: unknown): MemoryPromptSession | undefined {
  ...
  const manager = isRecord(eventCtx.sessionManager)
    ? eventCtx.sessionManager
    : undefined;
  if (manager === undefined) return undefined;
  const getSessionId = manager.getSessionId;
  const getBranch = manager.getBranch;
  if (typeof getSessionId !== "function" || typeof getBranch !== "function") {
    return undefined;
  }
  const id = Reflect.apply(getSessionId, manager, []);
  const branch = Reflect.apply(getBranch, manager, []);
  if (typeof id !== "string" || id.length === 0 || !Array.isArray(branch)) {
    return undefined;
  }
