  ...
  for (;;) {
    try {
      ...
      await new Promise((resolve) =>
        setTimeout(resolve, TEMP_ROOT_RELEASE_POLL_MS)
      );
    }
  }
}
...
describe("memory pressure dream wiring", () => {
  test(
    "#given committed system memory at soft pressure #when bind refresh computes the advisory estimate #then it attempts one pressure reservation",
    async () => {
      const root = realpathSync.native(
        await mkdtemp(join(tmpdir(), "omo-memory-pressure-dream-wiring-")),
      );
      roots.push(root);
      ...
      const wiring = createMemoryWiring({
        ...
        loadConfig: () =>
          loadedMemoryConfig(memorySettings({ compile_warn_tokens: 100 })),
        cwd: () => root,
        env: {},
      ...
      const boundedObservation = Promise.race([
        ...
        new Promise<never>((_resolve, reject) => {
          timeout = setTimeout(
            () => reject(new Error("pressure reservation was not observed")),
            5_000,
          );
        }),
      ]);
...
describe("memory pressure compile wiring", () => {
  test(
    "#given a bound fixture repo below pressure #when a committed write crosses the threshold #then the next real compile refresh adds pressure metadata without truncating memory",
    async () => {
      const root = realpathSync.native(
        await mkdtemp(join(tmpdir(), "omo-memory-pressure-wiring-")),
      );
      roots.push(root);
      ...
      const belowPrompt =
        (below as { systemPrompt?: string } | undefined)?.systemPrompt ?? "";
      const pressuredPrompt =
        (pressured as { systemPrompt?: string } | undefined)?.systemPrompt ??
          "";
...
describe("memory footer wiring", () => {
  test("#given committed memory bound without a visible footer #when memory tools return #then only the first result shows relative age", async () => {
    ...
    await pi.dispatch(
      "session_start",
      { type: "session_start" },
      sessionContext(fixture.sessionId),
    );
    ...
    await pi.dispatch(
      "tool_result",
      memoryResult("mcp_omo-memory_memory"),
      toolContext,
    );
    await pi.dispatch(
      "tool_result",
      memoryResult("mcp_omo-memory_memory_apply_patch"),
      toolContext,
    );
  ...
  test("#given bind reconciliation is pending #when a footer publishes #then bind completion does not erase it", async () => {
    ...
    const bindCompletion = wiring.afterBind(
      pi,
      fixture.sessionId,
      fixture.context,
      eventCtx,
...
async function createFixture(): Promise<{
  ...
  const root = realpathSync.native(
    await mkdtemp(join(tmpdir(), "omo-memory-footer-wiring-")),
  );
  roots.push(root);
...
function expectRelativeStatus(
  ...
  expect(call?.text).toMatch(
    new RegExp(`^mem:${identity} (?:just now|[1-9]\\d*[mhd] ago)$`),
  );
}

async function reflectionFixture(
  settings: ReturnType<typeof memorySettings> | undefined,
): Promise<{
  readonly pi: MemoryFakeExtensionAPI;
  readonly evaluations: { count: number };
}> {
  const root = realpathSync.native(
    await mkdtemp(join(tmpdir(), "omo-memory-wiring-")),
  );
  roots.push(root);
  const identity = createMemoryIdentityContext({
    identity: "agent-test",
  ...
  describe("#given a pending completion and a bound session with a real UI callback", () => {
    describe("#when afterBind drains the identity completion directory", () => {
      test("#then the callback receives the completion notification payload and level", async () => {
        // given
        const root = realpathSync.native(
          await mkdtemp(join(tmpdir(), "omo-memory-wiring-notify-")),
        );
        roots.push(root);
        const identity = createMemoryIdentityContext({
        ...
        await writeFile(
          join(completionsDir, "run-notify.json"),
          `${
            JSON.stringify({
              schemaVersion: 1,
              runId: "run-notify",
        ...
        expect(notifications).toEqual([{
          message:
            "Delivered 1 memory reflection completions; 1 need attention.",
          level: "warning",
        }]);
      });
    });
...
async function liveFooterHarness(): Promise<{
  ...
  const wiring = createMemoryWiring({
    sessions: new Map([[fixture.sessionId, {
...
describe("memory footer live wiring", () => {
  test("#given a spinner animating mid-session #when the footer is cleared #then the interval is released and nothing repaints", async () => {
    ...
    expect(
      harness.statusCalls.some((call) =>
        call.text?.includes("reflecting") === true
      ),
    ).toBe(true);
