...
export function conversationFrontiers(
  ...
  for (const entry of entries) {
    ...
    if (
      current === undefined ||
      entry.range.end_snapshot_line > current.range.end_snapshot_line
    ) {
 ...
 */
export async function classifyOversizePayload(
  input: OversizeClassificationInput,
): Promise<boolean> {
  ...
  if (input.envelopeOversized) {
    ...
    input.warn?.(
      "facts payload envelope exceeds the byte cap; nothing can be launched",
      {
        envelopeBytes,
        maxBytes,
        conversations: new Set(input.pending.map((entry) =>
  ...
  for (const entry of input.oversized) {
    const entryBytes = measureFactsPayloadBytes({
      ...input.envelope,
      entries: [entry],
    });
