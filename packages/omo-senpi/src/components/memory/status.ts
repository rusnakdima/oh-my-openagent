...
export async function refreshMemoryStatus(
  ...
  if (input.showFooter !== false) {
    ...
    if (age !== null) {
      const segments = await readMemoryStatusSegments(
        repo,
        input.context,
        input.sessionId,
      );
  ...
  if (input.checkAdvisory === false || input.alreadyNotified) {
    return { notified: false, footerShown };
  }

  const estimate = await estimateSystemTokens(repo, head);
  if (
    estimate >= Math.floor(MEMORY_PRESSURE_SOFT_RATIO * input.compileWarnTokens)
  ) {
    try {
  ...
  if (estimate < input.compileWarnTokens) {
    return { notified: false, footerShown };
  }
