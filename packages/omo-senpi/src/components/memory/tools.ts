...
export const MemoryToolParams = Type.Object({
  ...
  new_string: Type.Optional(
    Type.String({ description: "Replacement text. Required by str_replace." }),
  ),
  ...
  insert_text: Type.Optional(
    Type.String({ description: "Text to insert. Required by insert." }),
  ),
  ...
  file_text: Type.Optional(
    Type.String({ description: "Initial body text for create." }),
  ),
});
...
export type MemoryToolExecutionResult =
  & AgentToolResult<MemoryToolResultDetails>
  & { readonly isError?: boolean };

export type MemoryToolDefinition<TParams extends TSchema> =
  & Omit<ToolDefinition<TParams, MemoryToolResultDetails>, "execute">
  & {
    readonly execute: (
      toolCallId: string,
...
export function createMemoryTools(
  ...
  return [
    createMemoryTool(resolveContext, options),
    createMemoryApplyPatchTool(resolveContext, options),
  ];
}
...
export function registerMemoryTools(
  ...
  for (const tool of createMemoryTools(resolveContext, options)) {
...
export function registerMemoryToolSurface(
  ...
  if (
    options.exposure === "search" && typeof pi.registerMcpServer === "function"
  ) {
    pi.registerMcpServer(MEMORY_MCP_SERVER_NAME, {
      command: process.execPath,
      args: [
        join(dirname(fileURLToPath(import.meta.url)), "omo-memory-mcp.js"),
      ],
      exposure: "search",
      enabled: true,
    });
...
function createMemoryTool(
  ...
  return {
    ...
    promptSnippet:
      "memory - edit omo memory blocks (create/str_replace/insert/delete/rename/update_description); auto-commits each change",
    promptGuidelines: [
    ...
    execute: async (_toolCallId, params) => {
      ...
      if (context === undefined) {
        return errorResult(`${MEMORY_TOOL_NAME}: ${UNBOUND_IDENTITY_MESSAGE}`);
      }
      try {
        ...
        return okResult(
          result.message,
          await writeNoticeFor(context, result.commit, options),
        );
      } catch (error) {
...
function createMemoryApplyPatchTool(
  ...
  return {
    ...
    promptSnippet:
      "memory_apply_patch - apply a codex-style patch to omo memory files; auto-commits the change",
    promptGuidelines: [
    ...
    execute: async (_toolCallId, params) => {
      ...
      if (context === undefined) {
        return errorResult(
          `${MEMORY_APPLY_PATCH_TOOL_NAME}: ${UNBOUND_IDENTITY_MESSAGE}`,
        );
      }
      try {
        const { repo, lock, author } = await prepareEngine(context, options);
...
function renderResultFor(options: MemoryToolsOptions) {
  return createMemoryWriteRenderResult({
    enabled: () => options.writeNotice?.enabled === true,
  });
}
...
async function writeNoticeFor(
  ...
  if (commit === undefined || options.writeNotice?.enabled !== true) {
    return undefined;
  }
  try {
...
export async function gatherMemoryWriteNotice(
  ...
  return {
    ...
    ...(size === undefined ? {} : { size }),
    timeline: {
      ...(repo?.entriesToday === undefined
        ? {}
        : { entriesToday: repo.entriesToday }),
      ...(repo?.previousEntryAtISO === undefined
        ? {}
        : { previousEntryAtISO: repo.previousEntryAtISO }),
      ...(snapshot?.reflection.lastConsolidationAtISO === undefined ? {} : {
        lastConsolidationAtISO: snapshot.reflection.lastConsolidationAtISO,
      }),
      ...(unreflectedSteps === undefined || unreflectedSteps <= 0
        ? {}
        : { unreflectedSteps }),
    },
  };
...
async function readSnapshot(
  ...
  try {
    if (deps.buildSnapshot !== undefined) {
      return await deps.buildSnapshot(context, sessionId);
    }
...
async function readAffectedFiles(
  repoPath: string,
  sha: string,
): Promise<readonly MemoryWriteAffectedFile[]> {
  try {
...
export function parseNumstat(
  stdout: string,
): readonly MemoryWriteAffectedFile[] {
...
async function prepareEngine(
  ...
  return prepareMemoryEngineSession(
    context.identity,
    context.identityPaths,
    options,
  );
}
