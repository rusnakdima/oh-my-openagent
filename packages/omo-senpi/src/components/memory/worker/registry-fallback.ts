...
export function selectRegistryFallbackModels(
  available: unknown,
): readonly RegistryFallbackCandidate[] {
  if (!Array.isArray(available)) return [];
  const candidates = available
    .map(parseCandidate)
    .filter((candidate): candidate is RegistryCandidate =>
      candidate !== undefined
    )
    .filter((candidate) => !NON_CHAT_ID.test(candidate.id))
    .filter((candidate) =>
      candidate.contextWindow === undefined ||
      candidate.contextWindow >= MIN_CONTEXT_WINDOW
    );
...
export function readModelPricing(
  entry: unknown,
): ReflectionModelPricing | undefined {
...
function compareCandidates(
  ...
  if (left.inputCost !== undefined && right.inputCost !== undefined) {
    if (left.inputCost !== right.inputCost) {
      return left.inputCost - right.inputCost;
    }
