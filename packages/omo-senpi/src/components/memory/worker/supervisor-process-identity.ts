...
export function parseSupervisorChildExit(
  text: string,
): SupervisorChildExit | undefined {
  ...
  try {
    ...
    const signal = typeof value.signal === "string" || value.signal === null
      ? value.signal
      : undefined;
    return code === undefined || signal === undefined || signal === "MODEL_PID"
      ? undefined
      : { code, signal };
  } catch {
    return undefined;
...
export function readSupervisorClockNow(): number {
  const clockDir = testSeamsEnabled()
    ? process.env.OMO_MEMORY_SUPERVISOR_CLOCK_PATH
    : undefined;
  if (clockDir === undefined) return Date.now();
  return readInjectedClock(clockDir);
}

export function scheduleSupervisorDeadline(
  instant: number,
  callback: () => void,
): CancelSupervisorDeadline {
  const clockDir = testSeamsEnabled()
    ? process.env.OMO_MEMORY_SUPERVISOR_CLOCK_PATH
    : undefined;
...
function recordTestTermination(action: string, targetPid: number): void {
  ...
  writeFileSync(
    `${runDir}/${action}-${process.pid}.json`,
    `${JSON.stringify({ targetPid })}\n`,
    "utf8",
  );
}
...
function spawnTerminationCommand(
  ...
  if (executable === undefined) {
    throw new TypeError("termination command is required");
  }
  if (synchronous) {
...
export function signalSupervisorProcessGroup(
  pid: number,
  ...
  if (injected !== undefined) {
    spawnTerminationCommand(
      injected,
      ["--signal-group", String(pid), signal],
      false,
    );
    return;
  }
...
function taskkillTree(pid: number, synchronous: boolean): void {
  const command = testCommand("OMO_MEMORY_SUPERVISOR_TASKKILL_COMMAND") ??
    ["taskkill"];
  spawnTerminationCommand(
    command,
    ["/pid", String(pid), "/T", "/F"],
...
export function recordSupervisorGracefulDeadline(
...
export function terminateSupervisorChildGracefully(
  ...
  if (platform === "win32") {
    ...
    wrapper.kill();
  } else if (wrapper.pid !== undefined) {
    signalSupervisorProcessGroup(wrapper.pid, "SIGTERM");
  }
}
...
export async function getSupervisorProcessStart(
  pid: number,
): Promise<string | null> {
  if (process.platform === "linux") return await readLinuxStartIdentity(pid);
  if (process.platform === "darwin" || process.platform === "freebsd") {
    const value = await readCommand("/bin/ps", [
      "-o",
      "lstart=",
      "-p",
      String(pid),
