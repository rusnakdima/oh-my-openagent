...
export const resolveAndPreflightMemoryLaunch: ResolveAndPreflightMemoryLaunch =
  async (input) => {
    ...
    if (preflight.kind === "none_visible") {
      throw new Error(
        `No ${input.surfaceName} model candidate is visible to the discovery-disabled child: ${
          preflight.rejected.map((item) => `${item.model} (${item.cause})`)
            .join(", ")
        }`,
      );
    }
