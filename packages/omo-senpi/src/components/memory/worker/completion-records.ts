...
export async function ensureReflectionCompletion(
  ...
  if (existing !== null) {
    if (!sameCompletion(existing, desired)) {
      throw new Error(
        `Reflection completion record mismatch for ${desired.runId}`,
      );
    }
    return existing;
  }
...
function isCompletionRecord(
  value: unknown,
): value is ReflectionCompletionRecord {
  ...
  return record.schemaVersion === 1 &&
    ...
    (record.trigger === "manual" || record.trigger === "compaction" ||
      record.trigger === "step-count" || record.trigger === "dream") &&
    (record.trigger === "dream"
      ? record.origin === "manual" || record.origin === "idle" ||
        record.origin === "shutdown" || record.origin === "pressure"
    ...
    (((delivery as Record<string, unknown>).status === "pending") ||
      ((delivery as Record<string, unknown>).status === "consumed"));
}
