...
export async function failReservationRun(
  ...
  const claimed = await withRunFinalizationClaim(
    ...
    ledger.runId,
    async () =>
      withRunTerminalGate(runDir, ledger.runId, async () => {
  ...
  const claimed = await withRunFinalizationClaim(
    ...
    async () =>
      withRunTerminalGate(runDir, ledger.runId, async () => {
        ...
        const precedence = await checkRunAbandonmentPrecedence(
          runDir,
          current.runId,
          context,
        );
        if (precedence.decision === "finalize") {
          return finalizeClaimedOutcome(
            context,
            runDir,
            precedence.ledger.runId,
          );
        }
        ...
        if (active?.runId === precedence.ledger.runId) {
          ...
          if (transition.launch !== undefined) {
            context.launch?.(transition.launch);
          }
        }
        return {
          runId: precedence.ledger.runId,
          outcome: "abandoned_unknown" as const,
        };
      }),
  );
  return claimedValue(claimed);
...
async function finalizeClaimedOutcome(
  ...
  if (!runOutcomeMatchesLedger(ledger, outcome)) {
    throw new Error(
      `Run outcome attempt ${outcome.attempt ?? "legacy"} does not match ${
        ledger.attempt ?? "legacy"
      }`,
    );
  }
  const decision = await resolveFinalizationDecision(
    context,
    runDir,
    ledger,
    outcome,
