...
if (payloadPath === undefined || extractionPath === undefined) {
  throw new Error("facts paths are required");
}
if (process.env.SENPI_MEMORY_FACTS !== "1") {
  throw new Error("facts sentinel is required");
}
...
if (mode === "fact") {
  ...
  await writeFile(
    extractionPath,
    '{"scope":"project","person":{"name":"Mina","aliases":[]},"text":"bad","date":"2026-08-10"}\n',
  );
} else if (mode === "fail") {
  process.exitCode = 7;
