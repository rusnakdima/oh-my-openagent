...
if (runDir === undefined) {
  throw new Error("termination fixture requires a run directory");
}
const child = JSON.parse(
  readFileSync(join(runDir, "child-started.json"), "utf8"),
...
if (signalGroupIndex >= 0) {
  ...
  if (!Number.isInteger(targetPid)) {
    throw new Error("taskkill fixture requires a pid");
  }
  await writeFile(
    join(runDir, "taskkill-invocation.json"),
    `${JSON.stringify({ args })}\n`,
    "utf8",
  );
