...
const transcript = await json(transcriptPath) as {
  request?: { trigger?: string; origin?: string };
};

if (process.env.SENPI_MEMORY_REFLECTION !== "1") {
  throw new Error("reflection sentinel is required");
}
if (
  transcript.request?.trigger !== "dream" ||
  transcript.request.origin !== "manual"
) {
  throw new Error("dream request payload is required");
}
const emptyLedgers = process.env.DREAM_FIXTURE_EMPTY_LEDGERS === "1";
const expectedSkills = emptyLedgers
  ? {}
  : { review: { count: 2, lastUsedAt: "2026-08-01T00:00:00.000Z" } };
const expectedState = emptyLedgers
  ? {}
  : { last_dream_at: "2026-07-01T00:00:00.000Z", lastRunId: "prior-run" };
if (
  JSON.stringify(await json(skillsUsagePath)) !== JSON.stringify(expectedSkills)
) {
  throw new Error("skills usage payload mismatch");
}
if (
  JSON.stringify(await json(dreamStatePath)) !== JSON.stringify(expectedState)
) {
...
if (targetPath !== undefined) {
  ...
  await writeFile(
    join(memoryDir, "system", "dream-dispatch.json"),
    `${JSON.stringify(policy, null, 2)}\n`,
  );
  if (policy.people.enabled) {
    const entries = Array.from(
      { length: policy.people.max_entries },
      (_, index) =>
        `ATTRIBUTE: ${String(index + 1).padStart(2, "0")} ${
          "x".repeat(policy.people.max_entry_chars)
        }`.slice(0, policy.people.max_entry_chars),
    );
    await mkdir(join(memoryDir, "people", "fixture"), { recursive: true });
    await writeFile(
      join(memoryDir, "people", "fixture", "card.md"),
      `${entries.join("\n")}\n`,
    );
  }
}
