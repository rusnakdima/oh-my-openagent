...
if (mode === undefined || runDir === undefined) {
  throw new Error("mode and runDir are required");
}
...
if (mode === "model-not-found") {
  ...
  process.on("SIGTERM", () => {
    ...
    terminating = true;
    void writeMarker("child-terminated.json", { signal: "SIGTERM" }).then(() =>
      process.exit(0)
    );
  });
