...
afterEach(async () =>
  Promise.all(
    harnesses.splice(0).map((item) =>
      rmEfaultTolerant(item.root, {
        recursive: true,
        force: true,
        maxRetries: 10,
        retryDelay: 200,
...
describe("live reflection run finalization", () => {
  test(
    "#given a successful supervised run #when the parent finalizer completes #then final is published after integration and the merge carries its run receipt",
    async () => {
      // given
      ...
      const process = Bun.spawn(["git", "log", "-1", "--format=%B"], {
        cwd: item.identity.paths.repo,
        stdout: "pipe",
      });
