...
export async function readReflectionHealth(
  ...
  const stale = newestFailureAt !== undefined &&
    Number.isFinite(newestFailureAt) &&
    now - newestFailureAt > REFLECTION_HEALTH_STALE_MS;
  const recent = (stale ? [] : failuresBeforeSuccess.slice(0, 3)).map(
    fingerprintOf,
  );
  const dominant = dominantFingerprint(recent);
  const lastFailure = bounded.find((record) => record.outcome === "failed");
  const lastSuccess = bounded.find((record) =>
    record.outcome === "merged" || record.outcome === "no_changes"
  );
  const streakSinceISO = stale
    ? undefined
    : failuresBeforeSuccess.at(-1)?.finishedAt;
...
export function reflectionFailureFingerprint(
...
async function readHealthRecord(
  path: string,
): Promise<HealthRecord | undefined> {
  try {
    const parsed: unknown = JSON.parse(await readFile(path, "utf8"));
    if (
      !isRecord(parsed) || typeof parsed.finishedAt !== "string" ||
