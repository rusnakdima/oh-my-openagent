...
import {
  classifyRunProcess,
  signalRecordedProcessGroup,
  waitUntil as waitForTime,
} from "./run-liveness";
import {
  parseReservationRunLedger,
  type ReservationRunLedger,
} from "./reservation-run-ledger";
import { type SentinelWaitResult, waitForRunSentinel } from "./run-sentinel";

export type ReflectionRunReconcileResult = Pick<
...
type ReconcileContext =
  & Required<Pick<ReflectionRunReconciliationOptions, "now" | "hostname">>
...
export async function reconcileReflectionRuns(
  ...
  for (const name of await directoryNames(runsDir)) {
    ...
    if (
      existsSync(join(runDir, "final.json")) ||
      existsSync(join(runDir, "abandoned.json"))
    ) continue;
    if (!existsSync(join(runDir, "ledger.json"))) continue;
    const ledger = parseReservationRunLedger(
      await readRunJson<unknown>(join(runDir, "ledger.json")),
    );
    const result = await reconcileRun(context, runDir, ledger);
    if (result !== undefined) {
      results.push({ runId: result.runId, outcome: result.outcome });
    }
  }
  return results;
}

async function reconcilePrelaunch(
  context: ReconcileContext,
): Promise<ReflectionRunReconcileResult | undefined> {
  const active = (await context.reservation.readState()).active;
  if (
    active?.reservedAt === undefined || active.launcherPid === undefined ||
    active.launcherHostname === undefined
  ) return undefined;
  ...
  if (
    context.now() - Date.parse(active.reservedAt) <= 60_000 ||
    active.launcherHostname !== context.hostname()
  ) return undefined;
  const liveness = (context.getPidLiveness ?? getPidLiveness)(
    active.launcherPid,
  );
  let dead = liveness === "dead";
  if (
    !dead && liveness === "alive" && active.launcherProcessStart !== null &&
    active.launcherProcessStart !== undefined
  ) {
    const actual =
      await (context.getProcessStartIdentity ?? getProcessStartIdentity)(
        active.launcherPid,
      );
  ...
  if (existsSync(prelaunchPath)) {
    const prelaunch = parseRunPrelaunchArtifact(
      await readRunJson<unknown>(prelaunchPath),
    );
    if (prelaunch.runId !== active.runId) {
      throw new Error("Reflection prelaunch run id does not match reservation");
    }
    const repo = new GitMemoryRepo({
      dir: context.identity.paths.repo,
      agentId: context.identity.id,
    });
...
async function reconcileRun(
  ...
  if (ledger.launching === true && context.now() <= ledger.hardDeadlineAt) {
    return undefined;
  }
  const supervisor = await classifyRunProcess(
    ledger.pid,
    ledger.processStart,
    context,
  );
  if (supervisor === "alive" || supervisor === "unknown") {
    const wait = context.waitForOutcome ??
      ((path, deadlineAt) => waitForRunSentinel(path, deadlineAt, context.now));
    await wait(outcomePath, ledger.deadlineAt);
    const refreshed = parseReservationRunLedger(
      await readRunJson<unknown>(join(runDir, "ledger.json")),
    );
    ...
    if (
      refreshed.launching === true && context.now() <= refreshed.hardDeadlineAt
    ) return undefined;
    const freshSupervisor = await classifyRunProcess(
      refreshed.pid,
      refreshed.processStart,
      context,
    );
    if (freshSupervisor === "unknown" || freshSupervisor === "absent") {
      return await abandonReservationRun(context, runDir, refreshed);
    }
    return freshSupervisor === "alive"
      ? undefined
      : await reconcileDeadSupervisor(context, runDir, refreshed);
  }
...
async function reconcileDeadSupervisor(
  ...
  let child = await classifyRunProcess(
    ledger.childPid,
    ledger.childProcessStart,
    context,
  );
  if (child === "unknown") {
    return await abandonReservationRun(context, runDir, ledger);
  }
  if (child === "dead" || child === "absent") {
    return await failReservationRun(context, runDir, ledger, "failed");
  }
  const wait = context.waitUntil ??
    ((deadlineAt) => waitForTime(deadlineAt, context.now));
  await wait(ledger.hardDeadlineAt);
  child = await classifyRunProcess(
    ledger.childPid,
    ledger.childProcessStart,
    context,
  );
  ...
  return child === "dead"
    ? await failReservationRun(context, runDir, ledger, "timed_out")
    : undefined;
}
