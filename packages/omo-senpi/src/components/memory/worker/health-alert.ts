...
export const renderReflectionHealthEntry: EntryRenderer<ReflectionHealthEntry> =
  (entry, options, theme) => {
    ...
    return noticeComponent(
      {
        glyph: "✗",
        title: `Memory reflection failing · ${health.streak} run${
          health.streak === 1 ? "" : "s"
        } in a row`,
        tone: "error",
        ...
        detail: joinFields([
          ...
          optionalRendererText(health.lastDetail) === undefined
            ? undefined
            : detailExcerpt(health.lastDetail ?? ""),
...
export function registerReflectionHealthRenderer(
...
function recommendationWhy(recommendation: string): string {
  ...
  if (text.length === 0) {
    return "Reflection has failed repeatedly and needs attention.";
  }
...
export async function emitReflectionHealthAlert(
  ...
  if (
    health.recentFailureFingerprints.filter((item) =>
      item === health.fingerprint
    ).length < 2
  ) return false;
  if (!once(`${live.sessionId}:${health.fingerprint}`)) return false;
  const failure = health.lastFailure;
  const recommendation = reflectionRemediation(
    failure?.reason,
    failure?.detail,
  );
  const entry: ReflectionHealthEntry = {
    ...
    sinceISO: health.streakSinceISO ?? failure?.finishedAt ??
  ...
  safeNotify(
    live,
    `Memory reflection has failed ${health.streak} times (${health.fingerprint}). ${recommendation}`,
    "warning",
  );
  return true;
}
