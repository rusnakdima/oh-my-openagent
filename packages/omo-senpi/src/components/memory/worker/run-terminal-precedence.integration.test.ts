...
async function fixture() {
  const root = realpathSync.native(
    await mkdtemp(join(tmpdir(), "run-terminal-precedence-")),
  );
  roots.push(root);
  ...
  return new Promise((resolve, reject) => {
    ...
    child.once("close", (code, signal) => {
      if (code === 0 && signal === null) resolve();
          ...
          new Error(`supervisor exited with code ${code} signal ${signal}`),
        );}
    });
  });
}
...
describe("run terminal precedence", () => {
  test(
    "#given abandonment owns the gate when the shipped supervisor child finishes successfully #when publication follows #then the matching outcome wins",
    async () => {
      ...
      const reachedStateRead = new Promise<void>((resolve) => {
        stateReadReached = resolve;
      });
      ...
      expect(
        JSON.parse(
          await readFile(join(item.runDir, "publishing.json"), "utf8"),
        ),
      ).toMatchObject({
        version: 1,
        runId: "run-1",
      ...
      const outcome = JSON.parse(
        await readFile(join(item.runDir, "outcome.json"), "utf8"),
      );
      ...
      const finalized = await reconcileReflectionRuns({
        identity: item.identity,
        reservation,
      });
