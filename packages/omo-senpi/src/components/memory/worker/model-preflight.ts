...
export async function preflightMemoryModels(
  input: ModelPreflightInput,
): Promise<ModelPreflightResult> {
  const cacheKey = await modelCatalogCacheKey(
    input.launch,
    input.configSources,
  );
  ...
  const current =
    cached !== undefined && now - cached.probedAt < CATALOG_CACHE_TTL_MS
      ? cached
      : undefined;
  let visible = current?.visible;
  if (visible === undefined) {
    try {
      visible = await probeChildModels(
        input.launch,
        input.env,
        input.timeoutMs ?? PREFLIGHT_TIMEOUT_MS,
      );
      catalogCache.set(cacheKey, { visible, probedAt: now });
    } catch (error) {
      input.warn?.(
        "memory child model preflight failed; falling back to reactive model retries",
        {
  ...
  const candidates = input.candidates.filter((candidate) =>
    visible.has(candidate.model)
  );
  const rejected = input.candidates
    .filter((candidate) => !visible.has(candidate.model))
    .map((candidate): ModelPreflightRejection => ({
      model: candidate.model,
      cause: "model_not_visible",
    }));
  ...
  return {
    kind: "filtered",
    candidates: asMemoryModelChain(candidates),
    rejected,
  };
}
...
async function probeChildModels(
  ...
  if (timedOut) {
    throw new Error(`model catalog probe timed out after ${timeoutMs}ms`);
  }
  if (code !== 0) {
    throw new Error(
      `model catalog probe exited with code ${code}: ${
  ...
  if (models.size === 0) {
    throw new Error("model catalog probe returned no parseable model ids");
  }
  return models;
}
...
function parseModelCatalog(output: string): ReadonlySet<string> {
  ...
  const headerIndex = lines.findIndex((line) =>
    /^provider\s{2,}model(?:\s{2,}|$)/i.test(line)
  );
  if (headerIndex >= 0) {
    for (const line of lines.slice(headerIndex + 1)) {
      const columns = line.split(/\s{2,}/);
      if (
        columns.length >= 2 && isProviderPart(columns[0] ?? "") &&
        isModelIdPart(columns[1] ?? "")
      ) {
...
async function modelCatalogCacheKey(
  ...
  const sourceMtimes = await Promise.all(
    sources.filter((source) => source.exists).map(async (source) => {
      try {
...
function asMemoryModelChain(
  candidates: readonly ReflectionModelCandidate[],
): MemoryModelChain {
  const first = candidates[0];
  if (first === undefined) {
    throw new Error("model preflight requires at least one visible candidate");
  }
