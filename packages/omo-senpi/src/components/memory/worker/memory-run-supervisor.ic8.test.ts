...
describe("memory run supervisor IC-8 containment", () => {
  test(
    "#given a connected model child #when the supervisor is killed and test cleanup runs #then cleanup terminates the child group and releases the socket",
    async () => {
      // given
      const { runDir, clockPath, childExited, exitSocketAccepted } =
  ...
  test(
    "#given injected Windows and a child that exits during grace #when the hard deadline arrives #then the supervisor cancels forced taskkill",
    async () => {
      // given
      ...
      if (process.platform === "win32") {
        expect(outcome.childExit).toEqual({ code: null, signal: "SIGTERM" });
  ...
  for (const platform of IC8_PLATFORMS) {
    test(
      `#given the injected ${platform} branch #when the absolute deadline instants are advanced #then graceful and hard tree termination use those instants`,
      async () => {
        // given
        ...
        const nativeWindowsGraceWins = platform === "win32" &&
        ...
        await waitForPath(
          join(
            runDir,
            `${
              platform === "win32" ? "win32-graceful" : "posix-SIGTERM"
            }-${supervisor.pid}.json`,
          ),
        );
        expect(existsSync(join(runDir, "taskkill-invocation.json"))).toBe(
          false,
        );
    ...
    test(
      `#given the injected ${platform} branch and a released child #when the supervisor dies abruptly #then the bootstrap alone enforces the persisted deadline`,
      async () => {
        // given
        const { runDir, clockPath, childExited } = await makeRun("graceful");
        const supervisor = launchSupervisor(runDir, clockPath, platform);
