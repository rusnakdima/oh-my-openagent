...
export async function runReflectionChild(
  ...
  if (graceMs < 0 || maxOutputBytes <= 0) {
    throw new TypeError("reflection spawn limits are invalid");
  }
  ...
  if (
    !Number.isFinite(prepared.hardDeadlineAt) || prepared.hardDeadlineAt <= 0
  ) {
  ...
  return runSupervisedChild({
    ...
    ledger: {
      ...
      ...(metadata.targetDoc === undefined
        ? {}
        : { targetDoc: metadata.targetDoc }),
      ...(metadata.systemTokenBudget === undefined
        ? {}
        : { systemTokenBudget: metadata.systemTokenBudget }),
      ...(metadata.systemTokenTarget === undefined
        ? {}
        : { systemTokenTarget: metadata.systemTokenTarget }),
...
export async function runFactsChild(
  ...
  const maxOutputBytes = options.maxOutputBytes ?? DEFAULT_MAX_OUTPUT_BYTES;
  if (graceMs < 0 || maxOutputBytes <= 0) {
    throw new TypeError("facts spawn limits are invalid");
  }
  const prepared = await (options.sandbox ?? passthroughFactsSandbox)(
    spawnArgs,
  );
  const launchedAt = (options.now ?? Date.now)();
...
async function runSupervisedChild(input: {
  ...
  const supervisor = spawn(process.execPath, [
    input.supervisorPath ?? defaultSupervisorPath(),
    input.runDir,
  ], {
    detached: true,
    stdio: "ignore",
  ...
  const hasCompleteOutcome = () => {
    ...
    try {
      return runOutcomeMatchesLedger(
        ledger,
        JSON.parse(readFileSync(outcomePath, "utf8")) as RunOutcome,
      );
    } catch {
      return false;
    }
  ...
  void waitForRunCompletion(
    ...
    launchPath,
    hasCompleteOutcome,
    input.hardDeadlineAt + input.terminationGraceMs +
      OUTCOME_PUBLICATION_MARGIN_MS,
    Date.now,
    outcomeWait.signal,
  ...
  if (result.kind === "close" && !hasCompleteOutcome()) {
    throw new Error(
      `memory run supervisor exited with ${
        result.exit.code ?? result.exit.signal ?? "unknown status"
      }`,
    );
  }
  ...
  if (!runOutcomeMatchesLedger(ledger, outcome)) {
    throw new Error(
      `memory run outcome attempt ${
        outcome.attempt ?? "legacy"
      } does not match attempt ${input.attempt}`,
    );
  }
  return {
    code: outcome.childExit.code,
    signal: isNodeSignal(outcome.childExit.signal)
      ? outcome.childExit.signal
      : null,
