...
async function expectSingleRecoveryWinner(
  dir: string,
  initial: string,
): Promise<void> {
  ...
  const ownWinners = claims.filter((claim, index) =>
    claim.claimantPid === [1111, 2222][index]
  );
  expect(ownWinners).toHaveLength(1);
  expect(new Set(claims.map((claim) => claim.kind))).toEqual(
    new Set([ownWinners[0]?.kind]),
  );
...
describe("run terminal claim", () => {
  test(
    "#given a claim file left empty by a crash #when arbitration retries #then it recovers with one valid claim",
    async () => {
      // given
      ...
      // when
      const claim = await claimRunTerminal(
        dir,
      ...
      expect(readRunTerminalClaim(dir)).toMatchObject({
        kind: "abandon",
        runId: "run-1",
        attempt: 1,
      });
  ...
  test(
    "#given a live publish claim #when abandonment claims #then the publisher keeps the claim",
    async () => {
      // given
      const dir = await runDir();
      await claimRunTerminal(
        dir,
        { runId: "run-1", attempt: 1 },
        "publish",
        {},
  ...
  test(
    "#given two claimants inspected the same malformed claim #when recovery races #then only one replacement wins",
    async () => {
  ...
  test(
    "#given two claimants inspected the same confirmed-dead publish claim #when recovery races #then only one replacement wins",
    async () => {
      await expectSingleRecoveryWinner(
        await runDir(),
        `${
          JSON.stringify({
            version: 1,
