...
describe("selectRegistryFallbackModels", () => {
  describe("#given priced chat models", () => {
    ...
    test("#when two models share a price #then the larger context window breaks the tie", () => {
      // given
      const available = [
        priced("x", "small-ctx", 1, 128_000),
        priced("x", "big-ctx", 1, 1_000_000),
      ];

      // when / then
      expect(selectRegistryFallbackModels(available)[0]?.model).toBe(
        "x/big-ctx",
      );
    });
  });
  ...
  describe("#given models reflection cannot use", () => {
    test("#when the id names a non-chat modality #then it is excluded", () => {
      ...
      const available = [
        priced("openai", "text-embedding-005", 0.001),
        priced("openai", "chat-model", 1),
      ];

      // when / then
      expect(
        selectRegistryFallbackModels(available).map((entry) => entry.model),
      ).toEqual(["openai/chat-model"]);
    });
...
describe("readModelPricing", () => {
  test("#given a registry entry #when read #then only numeric cost fields survive", () => {
    // when / then
    expect(readModelPricing({ cost: { input: 1, cacheRead: 0.1 } })).toEqual({
      input: 1,
      cacheRead: 0.1,
    });
