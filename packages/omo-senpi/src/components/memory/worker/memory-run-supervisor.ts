...
function writeBootstrapStatus(status: SupervisorChildExit): void {
  try {
    ...
    if (
      ...
      !["EPIPE", "EBADF"].includes(String(error.code))
    ) throw error;
  }
}
...
async function runChildBootstrap(runDir: string): Promise<void> {
  ...
  const manifest = await readRunJson<RunLaunchManifest>(
  ...
  const cancelTerm = scheduleSupervisorDeadline(manifest.hardDeadlineAt, () => {
    if (platform === "win32") {
      terminateSupervisorChildGracefully(platform, child);
    } else signalSupervisorProcessGroup(process.pid, "SIGTERM");
  });
  const cancelKill = scheduleSupervisorDeadline(
    manifest.hardDeadlineAt + manifest.terminationGraceMs,
    () => {
...
async function runSupervisor(runDir: string): Promise<void> {
    ...
    control.on("data", (chunk: string) => {
      ...
      for (const line of bootstrapStatus.split("\n")) {
        try {
          ...
          if (
            message.signal === "MODEL_PID" && typeof message.code === "number"
          ) modelPid = message.code;
        } catch (error) {
  ...
  if (bootstrap.pid === undefined) {
    throw new Error("child bootstrap did not receive a pid");
  }
  await updateRunLedger(ledgerPath, {
  ...
  if (platform === "win32") {
    hardKillFinished = new Promise<void>((resolve) => {
      finishHardKill = resolve;
    });
  }
  const cancelTerm = scheduleSupervisorDeadline(manifest.hardDeadlineAt, () => {
    timedOut = true;
  ...
  const wrapperExit = await new Promise<SupervisorChildExit>(
    (resolve, reject) => {
      let settled = false;
  ...
  const outcome: RunOutcome = {
    ...
    timedOut: timedOut ||
      (Number.isFinite(clockNow) && clockNow >= manifest.hardDeadlineAt),
  };
