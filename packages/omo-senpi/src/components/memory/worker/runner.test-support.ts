...
import type { ReflectionCompletionApi } from "./completion";
...
function isChainMode(childMode: string): boolean {
  return childMode === "model-fallback" || childMode === "model-exhausted" ||
...
export async function createRunnerHarness(options: {
  ...
  const reserved = await store.evaluate("conversation-a", {
    kind: "settled",
    success: true,
  });
  if (!reserved || reserved.status !== "active") {
    throw new Error("expected active reflection reservation");
  }
  ...
  await writeFile(
    ...
    `import { appendFileSync } from "node:fs"\nappendFileSync(${
      JSON.stringify(preflightProbeLog)
    }, "probe\\n")\nprocess.stdout.write(${
      JSON.stringify(`${
        (options.preflightModels ?? models).map((candidate) =>
  ...
  const runner = new SenpiSubprocessRunner({
    ...
    ...(options.resolveSessionModel === undefined
      ? {}
      : { resolveSessionModel: options.resolveSessionModel }),
    ...(options.resolveParentContextTokens === undefined
      ? {}
      : { resolveParentContextTokens: options.resolveParentContextTokens }),
    ...(options.resolveParentSessionFile === undefined
      ? {}
      : { resolveParentSessionFile: options.resolveParentSessionFile }),
    ...(options.resolveParentCacheReusable === undefined
      ? {}
      : { resolveParentCacheReusable: options.resolveParentCacheReusable }),
    ...
    sandbox: (spawnArgs) => {
      ...
      const mode = options.childMode === "model-fallback"
        ? spawnArgs.args.includes("extension-only/primary")
          ? "model-not-found"
          : "commit"
  ...
  return {
    ...
    reserveAgain: async () => {
      ...
      if (next.status !== "active") {
        throw new Error("expected another active reservation");
      }
      return next.run;
    },
  };
