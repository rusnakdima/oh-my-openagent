...
async function harness(
  options: Parameters<typeof createRunnerHarness>[0],
...
describe("SenpiSubprocessRunner integration", () => {
  test(
    "#given conflicting base and agent reflection settings #when launched #then execution uses the agent category merge and timeout",
    async () => {
      // given
  ...
  test(
    "#given a stub child that commits in its reflection worktree #when launched #then it merges records notifies and advances the cursor",
    async () => {
      // given
      const item = await harness({ childMode: "commit" });
      const parent = new GitMemoryRepo({
        dir: item.identity.paths.repo,
      ...
      expect(
        await readFile(
          join(item.identity.paths.repo, "system", "reflected.md"),
          "utf8",
        ),
      ).toContain("reflection stub");
      ...
      expect(existsSync(join(spawn?.paths.sessionDir ?? "", "launch.json")))
        .toBe(false);
      expect(
        JSON.parse(
          await readFile(
            join(spawn?.paths.sessionDir ?? "", "outcome.json"),
            "utf8",
          ),
        ),
      ).toMatchObject({
      ...
      expect(
        JSON.parse(
          await readFile(
            join(spawn?.paths.sessionDir ?? "", "ledger.json"),
            "utf8",
          ),
        ),
      ).toMatchObject({
  ...
  test(
    "#given a child sleeping beyond an injected hard deadline #when launched #then the process group times out and cleanup leaves the cursor retryable",
    async () => {
      // given
      const item = await harness({
        childMode: "timeout",
  ...
  test(
    "#given an extension-only primary and a child-visible fallback #when the primary is missing in the clean child #then reflection retries and records the fallback",
    async () => {
      // given
      ...
      expect(result.outcome).toBe("merged");
      expect(
        item.spawnCalls.map((spawn) =>
          spawn.args[spawn.args.indexOf("--model") + 1]
        ),
      ).toEqual([
      ...
      expect(new Set(item.spawnCalls.map((spawn) => spawn.hardDeadlineAt)).size)
        .toBe(1);
  ...
  test(
    "#given the primary model's providers are all cooling down #when the child dies with a 503 #then reflection relaunches on the next candidate and merges",
    async () => {
      // given
  ...
  test(
    "#given no candidate is visible #when a fresh probe and then its cached negative are used #then only the fresh verdict fails closed",
    async () => {
      // given
      const item = await harness({
        childMode: "commit",
      ...
      expect(fresh.detail).toContain(
        "No reflection model candidate is visible",
      );
  ...
  test(
    "#given every child-visible candidate misses its model or auth #when the chain is exhausted #then the failed outcome fingerprints every attempted cause",
    async () => {
      // given
      ...
      expect(result.detail).toContain(
        "model_not_visible:extension-only/primary",
      );
      expect(result.detail).toContain("auth_missing:kimi-coding");
      expect(result.detail).toContain(
        "attempted:extension-only/primary,kimi-coding/fallback",
      );
  ...
  test(
    "#given empty categories and no quick model #when launched twice in one session #then both fail without spawning and only one unsuppressed warning appears",
    async () => {
      // given
      const item = await harness({
        childMode: "commit",
      ...
      expect(
        item.api.entries.every((entry) =>
          entry.customType !== REFLECTION_LAUNCHED_ENTRY_TYPE
        ),
      ).toBe(true);
      expect(item.notifications).toHaveLength(1);
  ...
  test(
    "#given a zero-exit child that modifies linked-worktree git administration #when completion validates #then it fails cleans up and leaves the cursor unmoved",
    async () => {
      // given
