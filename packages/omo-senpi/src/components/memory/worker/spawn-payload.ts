...
export async function prepareReflectionSpawn(
  input: PrepareReflectionSpawnInput,
): Promise<ReflectionSpawnArgs> {
  const sessionDir = join(
    input.reflectionSessionsDir,
    safeRunId(input.run.runId),
  );
  ...
  if (
    isDream &&
    (input.systemTokenBudget === undefined ||
      input.systemTokenTarget === undefined)
  ) {
    throw new TypeError(
      "dream spawn requires a system token budget and target",
    );
  }
  ...
  await Promise.all([
    ...
    writeFile(
      persona,
      isDream ? loadDreamPersona().markdown : loadReflectionPersona().markdown,
      "utf8",
    ),
    writeFile(
      prompt,
      buildTaskPrompt(input.run, input.worktree.dir, transcript),
      "utf8",
    ),
    ...(dreamPaths === undefined ? [] : [
      ...
      writeFile(
        dreamPaths.dreamPolicy,
        `${
          JSON.stringify({ version: 1, people: input.peoplePolicy }, null, 2)
        }\n`,
        "utf8",
      ),
  ...
  return {
    ...
    model: input.model,
    ...(input.thinking === undefined ? {} : { thinking: input.thinking }),
    ...(input.nextAttempt === undefined
      ? {}
      : { nextAttempt: input.nextAttempt }),
    kind: input.run.request.trigger === "dream" ? "dream" : "reflection",
    trigger: input.run.request.trigger,
    ...(input.run.request.trigger === "dream"
      ? { origin: input.run.request.origin }
      : {}),
    mergePolicy: input.mergePolicy,
    ...(input.run.request.targetDoc === undefined
      ? {}
      : { targetDoc: input.run.request.targetDoc }),
    ...(isDream
      ? {
...
export async function prepareReflectionForkSpawn(
  input: PrepareReflectionSpawnInput,
): Promise<ReflectionSpawnArgs> {
...
export async function prepareFactsSpawn(
  input: PrepareFactsSpawnInput,
  ...
  await writeFile(payload, serializeFactsPayload(input.payload), {
    encoding: "utf8",
    mode: 0o600,
  });
...
function resolveDreamTarget(worktree: string, targetDoc: string): string {
  if (
    isAbsolute(targetDoc) || /^[a-zA-Z]:[\\/]/.test(targetDoc) ||
    !targetDoc.endsWith(".md")
  ) {
    throw new TypeError(
      "dream target must be a memory-repo-relative .md document",
    );
  }
  const target = resolve(worktree, targetDoc);
  const confined = relative(resolve(worktree), target);
  if (
    !confined || confined === ".." || confined.startsWith(`..${sep}`) ||
    isAbsolute(confined) ||
...
function buildTaskPrompt(
  ...
  const target = run.request.targetDoc === undefined ? [] : [
    `Document maintenance target: ${run.request.targetDoc}`,
    "Modify no memory document except this target.",
  ];
  return [
...
async function writeSystemTokenEstimate(
  repoDir: string,
  destination: string,
): Promise<void> {
  ...
  await writeFile(
    ...
    `${
      JSON.stringify(
        { totalTokens: estimate.totalTokens, files: estimate.files },
        null,
        2,
      )
    }\n`,
...
async function copyJsonOrEmpty(
  source: string,
  destination: string,
): Promise<void> {
  let content = "{}\n";
...
function safeRunId(runId: string): string {
  const safe = basename(runId.trim()).replace(/[^a-zA-Z0-9._-]+/g, "-").replace(
    /^-+|-+$/g,
    "",
  );
  if (!safe || safe === "." || safe === "..") {
    throw new TypeError("runId must contain a safe identifier");
  }
