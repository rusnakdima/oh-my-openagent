...
export async function claimRunTerminal(
  ...
  for (;;) {
    ...
    if (existing === undefined) {
      ...
      const replacement = await recoverClaim(
        runDir,
        path,
        snapshot,
        "invalid",
    ...
    if (
      existing.kind !== "publish" || !claimantIsConfirmedDead(existing, seams)
    ) return existing;
...
export function readRunTerminalClaim(runDir: string): RunTerminalClaim {
  return parseRunTerminalClaim(
    JSON.parse(readFileSync(runTerminalClaimPath(runDir), "utf8")),
  );
}
...
async function recoverClaim(
  ...
  const record = await createLockRecord("reflection-finalize", {
    runId: identity.runId,
  });
  return withLock(
    join(runDir, "terminal-claim-recovery.lock"),
    record,
    async () => {
      ...
      if (reason === "invalid") {
        ...
        if (
          current.claim.kind !== "publish" ||
          !claimantIsConfirmedDead(current.claim, seams)
...
function claimantIsConfirmedDead(
  claim: RunTerminalClaim,
...
function syncDirectory(path: string): void {
  ...
  try {
    ...
    if (
      ...
      ["EISDIR", "EPERM"].includes(errorCode(error) ?? "")
    ) return;
    throw error;
  }
  try {
    ...
    if (
      process.platform !== "win32" ||
      !["EPERM", "EACCES", "EINVAL"].includes(errorCode(error) ?? "")
    ) throw error;
  } finally {
    closeSync(directory);
  }
