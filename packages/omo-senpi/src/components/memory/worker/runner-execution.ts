...
export async function executeReflectionRun(input: {
  ...
  const repo = new GitMemoryRepo({
    dir: options.identity.paths.repo,
  ...
  try {
    ...
    const hardDeadlineAt = input.now() +
      (options.deadlineMs ?? reflection.timeout_minutes * 60_000);
    ...
    let launched = false;
    await (options.resolveAndPreflightLaunch ??
      resolveAndPreflightMemoryLaunch)({
        ...
        attempt: async (candidate, attemptNumber, nextAttempt) => {
          const parentSessionFile = input.route?.route === "fork"
            ? options.resolveParentSessionFile?.()
            : undefined;
          const parentCwd = input.route?.route === "fork"
            ? options.cwd
            : undefined;
  ...
  const finalized = await finalizeRecordedOutcome(
...
async function finalizeExecutionFailure(
  ...
  const runDir = join(
    input.options.identity.paths.reflection,
    "runs",
    input.run.runId,
  );
  if (existsSync(join(runDir, "ledger.json"))) {
    ...
    const finalized = error instanceof MemoryModelExhaustedError
      ? await overrideFailedReservationRun(
        input.finalizationContext(),
        runDir,
        ledger,
        error.message,
  ...
  const discarded = worktree === undefined
    ? undefined
    : await discardWorktree(worktree);
  return {
    outcome: "failed",
    reason: discarded !== undefined && !cleanupSucceeded(discarded)
      ? "cleanup_failed"
      : "spawn_failed",
