...
export const MEMORY_WORKLOAD_PROFILES: Readonly<
  Record<MemoryLaunchSurface, MemoryWorkloadProfile>
> = {
  reflection: {
    inputTokens: 44_502,
    cacheReadTokens: 643_584,
    outputTokens: 6_996,
    turns: 21,
  },
  dream: {
...
export function estimateForkCost(input: ForkCostInput): number {
  ...
  for (let turn = 1; turn < turns; turn += 1) {
    total += (parentContextTokens + INJECTED_PROMPT_TOKENS +
      PER_TURN_NEW_TOKENS * turn) * cacheRead;
  }
...
export function estimateQuickCost(input: QuickCostInput): number {
  ...
  const cacheRead = pricing.cacheRead ?? pricing.input;
  const tokens = profile.inputTokens * pricing.input +
...
export function chooseMemoryLaunchRoute(
  input: MemoryLaunchRouteInput,
): MemoryLaunchRoute {
  const { surface, quick, session } = input;
  if (quick === undefined) {
    if (session === undefined) {
      throw new Error("chooseMemoryLaunchRoute requires a quick candidate");
    }
  ...
  if (input.parentContextTokens === undefined) {
    return pick("quick", quick, "unknown_context");
  }
