...
export async function resolveFinalizationDecision(
  ...
  let ledger = parseReservationRunLedger(
    await readRunJson<unknown>(ledgerPath),
  );
  if (ledger.runId !== inputLedger.runId || child.runId !== ledger.runId) {
    throw new Error("Finalization run id changed");
  }
  const succeeded = !child.timedOut && child.childExit.code === 0 &&
    child.childExit.signal === null;
  if (!succeeded) {
    const detail =
      (await readRunTextTail(join(runDir, "child-stderr.log"), 64 * 1024))
        .trim() || undefined;
  ...
  if (validated === undefined && existsSync(worktree.dir)) {
    ...
    if (validation.status === "valid") {
      if (
        ledger.targetDoc !== undefined &&
        validation.changedPaths.some((path) => path !== ledger.targetDoc)
      ) {
        const decision = {
          outcome: "failed" as const,
          reason: "invalid_target",
          detail: "Dream changed paths outside targetDoc",
        };
        await checkpointDecision(ledgerPath, decision);
        await cleanupOrThrow(context, ledger);
        return decision;
      ...
      const decision = {
        outcome: validation.status as Extract<
          ReflectionOutcome,
    ...
    if (ledger.mergePolicy === "auto") {
      ...
      if (legacy.status === "integrated") {
        ...
        const decision = {
          ...
          integrationSha: legacy.integrationSha,
          ...budget,
        };
  ...
  if (probe.status === "integrated") {
    ...
    const decision = {
      outcome: "merged" as const,
      integrationSha: probe.integrationSha,
      ...budget,
    };
    await checkpointIntegrated(ledgerPath, decision);
    await cleanupOrThrow(context, ledger);
  ...
  const decision: DurableFinalizationDecision = {
    ...
    ...(integrated.integrationSha === undefined
      ? {}
...
function validatedFromLedger(
  ledger: ReservationRunLedger,
): ValidatedReflectionTip | undefined {
  return ledger.validatedTipSha === undefined ? undefined : {
    tipSha: ledger.validatedTipSha,
...
function decisionFromLedger(
  ...
  return {
    ...
    ...(ledger.finalizeReason === undefined
      ? {}
      : { reason: ledger.finalizeReason }),
    ...(ledger.finalizeDetail === undefined
      ? {}
      : { detail: ledger.finalizeDetail }),
    ...(ledger.integrationSha === undefined
      ? {}
      : { integrationSha: ledger.integrationSha }),
  };
}

async function checkpointDecision(
  path: string,
  ...
  await updateRunLedger(path, {
    ...
    ...(decision.detail === undefined
      ? {}
      : { finalizeDetail: decision.detail }),
  });
}

async function checkpointIntegrated(
  path: string,
  decision: DurableFinalizationDecision,
): Promise<void> {
  await updateRunLedger(path, {
...
async function validateIntegratedDreamBudget(
  ...
  if (
    ledger.kind !== "dream" || ledger.origin === undefined ||
    ledger.systemTokenTarget === undefined
  ) return {};
  ...
  return {
    detail:
      `Committed system/ estimate is ${validation.totalTokens} tokens; dream target is below ${validation.targetTokens} tokens`,
  };
}
...
async function cleanupOrThrow(
  ...
  const cleanup = await cleanupReflectionWorktree(
...
async function writerLock<T>(
  context: RunFinalizationContext,
  operation: () => Promise<T>,
): Promise<T> {
  if (context.withWriterLock !== undefined) {
    return await context.withWriterLock(operation);
  }
  const record = await createLockRecord("memory-write");
  return await withLock(
    memoryWriterLockPath(context.identity.paths.locks),
    record,
    operation,
    {
      waitTimeoutMs: 5_000,
