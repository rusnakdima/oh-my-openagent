...
describe("supervised child outcome authority", () => {
  test(
    "#given a matching durable outcome and a later supervisor failure #when the parent completes #then the outcome remains authoritative",
    async () => {
      // given
      const runDir = await mkdtemp(
        join(tmpdir(), "supervisor-outcome-authority-"),
      );
      roots.push(runDir);
      ...
      const result = await runReflectionChild({
        ...
        supervisorPath: join(
          import.meta.dir,
          "__fixtures__",
          "supervisor-outcome-then-fail.ts",
        ),
      });
  ...
  test(
    "#given an outcome before launch cleanup and a supervisor failure #when the parent completes #then the incomplete outcome is rejected",
    async () => {
      // given
      const runDir = await mkdtemp(
        join(tmpdir(), "supervisor-incomplete-outcome-"),
      );
      roots.push(runDir);
      const payloadDir = join(runDir, "payload");
      await mkdir(payloadDir);
      ...
      const run = runReflectionChild({
        ...
        supervisorPath: join(
          import.meta.dir,
          "__fixtures__",
          "supervisor-incomplete-outcome-then-fail.ts",
        ),
      });
  ...
  test(
    "#given a matching durable outcome while the supervisor stays alive #when the parent completes #then the outcome wins without waiting for close",
    async () => {
      // given
      const runDir = await mkdtemp(
        join(tmpdir(), "supervisor-outcome-linger-"),
      );
      roots.push(runDir);
      const payloadDir = join(runDir, "payload");
      await mkdir(payloadDir);
      ...
      const result = await runReflectionChild({
        ...
        supervisorPath: join(
          import.meta.dir,
          "__fixtures__",
          "supervisor-outcome-then-linger.ts",
        ),
      });
      await writeFile(join(runDir, "release"), "");
      expect(
        await waitForRunSentinel(
          join(runDir, "released.json"),
          Date.now() + 5_000,
          Date.now,
        ),
      ).toBe("present");
  ...
  test(
    "#given a supervisor that never publishes #when the parent deadline expires #then publication timeout is reported",
    async () => {
      // given
