...
export function reflectionLaunchedText(
  launched: ReflectionLaunchedEntry,
): string {
  return `memory reflection started run:${
    normalizeRendererText(launched.runId)
  } trigger:${
    normalizeRendererText(launched.trigger)
  } (+${launched.backlogSteps} steps)`;
}

// In-flight work reads as `accent`, matching the running state in statusThemeColor.
export const renderReflectionLaunchedEntry: EntryRenderer<
  ...
  return noticeComponent(
    {
      ...
      title: joinFields([
        ...
        runLabel(launched.runId),
      ]),
      tone: "accent",
      why: `The outcome lands in this transcript when the run settles - ${
        triggerPhrase(launched.trigger)
      } after ${launched.backlogSteps} new step${
        launched.backlogSteps === 1 ? "" : "s"
      }.`,
      extra: [
        {
          text: joinFields([
...
function triggerPhrase(trigger: ReflectionTrigger): string {
  return trigger === "manual"
    ? "triggered manually"
    : `triggered by ${normalizeRendererText(trigger)}`;
}

export const renderReflectionCompletionEntry: EntryRenderer<
...
> = (entry, options, theme) => {
  ...
  const payoff = joinFields([
    record.filesChanged !== undefined && record.filesChanged > 0
      ? `${record.filesChanged} file${
        record.filesChanged === 1 ? "" : "s"
      } changed`
      : undefined,
    record.mergedCommitSha === undefined
      ? undefined
      : `commit ${normalizeRendererText(record.mergedCommitSha).slice(0, 7)}`,
  ...
  return noticeComponent(
    {
      ...
      title: joinFields([
        `Memory reflection ${outcomeLabel(record.outcome)}`,
        runLabel(record.runId),
      ]),
      tone: outcomeThemeColor(record.outcome),
      why: outcomeSummary(record.outcome),
      extra: payoff.length === 0
        ? []
        : [{ text: payoff, tone: outcomeThemeColor(record.outcome) }],
      detail: joinFields([
...
export const renderReflectionSummaryEntry: EntryRenderer<
  ReflectionCompletionSummary
> = (entry, options, theme) => {
  ...
  return noticeComponent(
    {
      ...
      extra: clean || fingerprint === undefined ? [] : [{
        text: `most common ${detailExcerpt(fingerprint)}`,
        tone: "warning",
      }],
      detail: joinFields([
        optionalRendererText(summary.oldestISO) === undefined
          ? undefined
          : `oldest ${normalizeRendererText(summary.oldestISO)}`,
        optionalRendererText(summary.newestISO) === undefined
          ? undefined
          : `newest ${normalizeRendererText(summary.newestISO)}`,
      ]),
    },
    options,
    theme,
...
export function registerReflectionCompletionRenderer(
  api: ReflectionCompletionApi,
): void {
  api.registerEntryRenderer(
    REFLECTION_COMPLETION_ENTRY_TYPE,
...
function formatDuration(durationMs: number): string {
  ...
  return `${minutes}m${
    String(Math.round(seconds - minutes * 60)).padStart(2, "0")
  }s`;
}
