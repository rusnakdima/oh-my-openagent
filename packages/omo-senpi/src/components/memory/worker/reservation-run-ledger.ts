...
export function parseReservationRunLedger(
  value: unknown,
): ReservationRunLedger {
  if (!isRecord(value)) throw new Error("Invalid reservation run ledger");
  const trigger = value.trigger;
  const kind = value.kind;
  if (value.version !== 1 || typeof value.runId !== "string") {
    throw new Error("Invalid reservation run ledger identity");
  }
  if (kind !== "reflection" && kind !== "dream") {
    throw new Error("Invalid reservation run kind");
  }
  if (
    trigger !== "step-count" && trigger !== "compaction" &&
    trigger !== "manual" && trigger !== "dream"
  ) {
    throw new Error("Invalid reservation run trigger");
  }
  if ((kind === "dream") !== (trigger === "dream")) {
    throw new Error("Reservation run kind and trigger disagree");
  }
  if (
    trigger === "dream" && value.origin !== "manual" &&
    value.origin !== "idle" && value.origin !== "shutdown" &&
    value.origin !== "pressure"
  ) {
  ...
  if (requiredStrings.some((field) => typeof value[field] !== "string")) {
    throw new Error("Invalid reservation run paths");
  }
  ...
  if (Number(value.deadlineAt) !== hardDeadlineAt + terminationGraceMs) {
    throw new Error("Reservation run deadline mismatch");
  }
  if (value.mergePolicy !== "auto" && value.mergePolicy !== "integration") {
    throw new Error("Invalid reservation merge policy");
  }
  if (
    value.targetDoc !== undefined &&
    (kind !== "dream" || typeof value.targetDoc !== "string")
  ) {
  ...
  for (const field of ["systemTokenBudget", "systemTokenTarget"] as const) {
    if (
      ...
      (kind !== "dream" || !Number.isInteger(value[field]) ||
  ...
  if (
    (value.systemTokenBudget === undefined) !==
      (value.systemTokenTarget === undefined)
  ) {
    throw new Error("Dream token budget metadata is incomplete");
  }
  if (
    !optionalPid(value.pid) || !optionalIdentity(value.processStart) ||
    !optionalPid(value.childPid) || !optionalIdentity(value.childProcessStart)
  ) {
  ...
  if (value.model !== undefined && typeof value.model !== "string") {
    throw new Error("Invalid reservation run model");
  }
  if (value.thinking !== undefined && typeof value.thinking !== "string") {
    throw new Error("Invalid reservation run thinking");
  }
  if (value.category !== undefined && typeof value.category !== "string") {
    throw new Error("Invalid reservation run category");
  }
  if (
    value.conversationIds !== undefined &&
  ...
  if (
    value.commonConfigSnapshot !== null &&
    typeof value.commonConfigSnapshot !== "string"
  ) {
    throw new Error("Invalid common config snapshot");
  }
  if (
    value.finalizePhase !== undefined &&
  ...
  if (
    value.validatedChangedPaths !== undefined &&
    (!Array.isArray(value.validatedChangedPaths) ||
      !value.validatedChangedPaths.every((path) => typeof path === "string"))
  ) {
    throw new Error("Invalid validated changed paths");
  }
  if (
    value.finalizeOutcome !== undefined &&
    !isReflectionOutcome(value.finalizeOutcome)
  ) {
...
export function worktreeFromLedger(
  identity: MemoryIdentity,
  ledger: ReservationRunLedger,
): ReflectionWorktree {
  return {
    parent: new GitMemoryRepo({
