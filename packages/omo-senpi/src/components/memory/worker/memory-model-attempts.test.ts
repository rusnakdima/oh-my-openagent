...
function child(
  overrides: Partial<ReflectionChildResult>,
...
describe("runMemoryModelAttempts", () => {
  test("#given an exact model-not-found startup error #when a fallback exists #then it retries the fallback", async () => {
    ...
    // when
    const result = await runMemoryModelAttempts(
      candidates,
      async (candidate) => {
        ...
        return candidate.model === "extension-only/primary"
          ? child({
            stderr:
              'Error: Model "extension-only/primary" not found. Use --list-models to see available models.',
          })
          : child({ code: 0 });
      },
    );
    ...
    const result = await runMemoryModelAttempts(
      ...
      async (candidate) => {
        ...
        return candidate.model === "extension-only/primary"
          ? child({
            stderr:
              '503: {"message":"All providers are temporarily cooling down"}',
          })
          : child({ code: 0 });
      },
    );
  ...
  test("#given every candidate has a retryable model or auth miss #when the chain is exhausted #then every candidate and cause are carried by a typed signal", async () => {
    ...
    // when
    const attempt = runMemoryModelAttempts(
      allMissing,
      async (candidate) =>
        candidate.model.startsWith("extension-only/")
          ? child({
            stderr:
              'Error: Model "extension-only/primary" not found. Use --list-models to see available models.',
          })
    ...
    await expect(attempt).rejects.toEqual(
      new MemoryModelExhaustedError([
        {
          candidate: allMissing[0],
          miss: { kind: "model_not_visible", id: "extension-only/primary" },
        },
        {
          candidate: allMissing[1],
          miss: { kind: "auth_missing", provider: "anthropic" },
        },
      ]),
    );
  });
