...
async function launchDream(
  ...
  const root = realpathSync.native(
    await mkdtemp(join(tmpdir(), "memory-dream-dispatch-")),
  );
  roots.push(root);
  ...
  await repo.init({
    seedFiles: [
      ...
      ...(options.targetDoc === undefined ? [] : [{
        relativePath: options.targetDoc,
        content: "---\ndescription: Style\n---\nOriginal.\n",
      }]),
    ],
  });
  ...
  if (options.seedLedgers !== false) {
    ...
    await writeFile(
      join(identity.paths.runtime, "skills-usage.json"),
      `${
        JSON.stringify({
    ...
    await writeFile(
      join(identity.paths.runtime, "dream", "state.json"),
      `${
        JSON.stringify({
  ...
  const loaded: SenpiOmoConfigResult = {
    config,
    diagnostics: [],
    layers: [],
    sources: [],
  ...
  const runner = new SenpiSubprocessRunner({
    ...
    loadConfig: () => loaded,
    cwd: root,
    env: options.seedLedgers === false
      ? { DREAM_FIXTURE_EMPTY_LEDGERS: "1" }
      : {},
...
describe("dream worker dispatch", () => {
  test(
    "#given a dream with people disabled #when the worker launches #then it selects dream inputs and touches no people path",
    async () => {
      // given
      const expectedPolicy = {
        version: 1,
        people: { enabled: false, max_entries: 7, max_entry_chars: 80 },
      };
      ...
      expect(await readFile(item.spawn.paths.persona, "utf8")).toBe(
        loadDreamPersona().markdown,
      );
      expect(basename(item.spawn.env.SKILLS_USAGE_PATH ?? "")).toBe(
        "skills-usage.json",
      );
      expect(basename(item.spawn.env.DREAM_STATE_PATH ?? "")).toBe(
        "dream-state.json",
      );
      expect(basename(item.spawn.env.DREAM_POLICY_PATH ?? "")).toBe(
        "dream-policy.json",
      );
      expect(
        JSON.parse(
          await readFile(item.spawn.env.DREAM_POLICY_PATH ?? "", "utf8"),
        ),
  ...
  test(
    "#given absent dream ledgers #when the worker launches #then it supplies readable empty objects",
    async () => {
      // given
      ...
      expect(
        JSON.parse(
          await readFile(item.spawn.env.SKILLS_USAGE_PATH ?? "", "utf8"),
        ),
      ).toEqual({});
      expect(
        JSON.parse(
          await readFile(item.spawn.env.DREAM_STATE_PATH ?? "", "utf8"),
        ),
      ).toEqual({});
    },
    30_000,
  ...
  test(
    "#given custom people limits #when the dream writes fixture entries #then it honors the resolved limits exactly",
    async () => {
      // given
      ...
      const lines = (await readFile(
        join(item.identity.paths.repo, "people", "fixture", "card.md"),
        "utf8",
      )).replace(/\r\n/g, "\n").trim().split("\n");
      expect(lines).toHaveLength(limits.max_entries);
      expect(lines.every((line) => line.length === limits.max_entry_chars))
        .toBe(true);
    },
    30_000,
  );
  ...
  test(
    "#given a memory document target #when the dream completes #then only that document is merged",
    async () => {
      // given
      ...
      const commits = await repo.log({
        range: `${item.baseSha}..HEAD`,
        includePaths: true,
      });
      const ledger = JSON.parse(
        await readFile(
          join(item.spawn.paths.sessionDir, "ledger.json"),
          "utf8",
        ),
      );
      ...
      // then
      expect(item.result.outcome).toBe("merged");
      expect(await readFile(join(item.identity.paths.repo, targetDoc), "utf8"))
        .toContain("Maintained by dream");
      expect(new Set(commits.flatMap((commit) => commit.paths ?? []))).toEqual(
        new Set([targetDoc]),
      );
      expect(item.spawn.env.DREAM_TARGET_PATH).toBe(
        join(item.spawn.paths.worktree, targetDoc),
      );
