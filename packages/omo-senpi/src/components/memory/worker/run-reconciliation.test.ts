...
async function fixture(trigger: "step-count" | "dream" = "step-count") {
  const root = realpathSync.native(
    await mkdtemp(join(tmpdir(), "reflection-reconcile-")),
  );
  roots.push(root);
  ...
  const worktree = await createReflectionWorktree(
    repo,
    reserved.run.runId,
    identity.paths.worktrees,
  );
...
async function commitOrphanWorktree(
  item: Awaited<ReturnType<typeof fixture>>,
): Promise<void> {
  await mkdir(join(item.worktree.dir, "system"), { recursive: true });
  await writeFile(
    join(item.worktree.dir, "system", "orphan.md"),
    "---\ndescription: Orphan\n---\nrecovered\n",
  );
  await item.worktree.exec.run(["add", "system/orphan.md"], {
    cwd: item.worktree.dir,
    timeoutMs: 30_000,
  });
  await item.worktree.exec.run(["commit", "-m", "reflection orphan"], {
    cwd: item.worktree.dir,
    timeoutMs: 30_000,
  });
}
...
describe("reflection and dream run reconciliation", () => {
  test(
    "#given an orphaned successful dream worktree #when session-start reconciliation runs #then it merges clears the reservation records completion and publishes final",
    async () => {
      // given
      ...
      expect(
        await readFile(
          join(item.identity.paths.repo, "system", "orphan.md"),
          "utf8",
        ),
      ).toContain("recovered");
      ...
      expect(
        JSON.parse(
          await readFile(
            join(
              item.identity.paths.reflection,
              "completions",
              "run-orphan.json",
            ),
            "utf8",
      ...
      expect(
        JSON.parse(await readFile(join(item.runDir, "final.json"), "utf8")),
      ).toMatchObject({
        version: 1,
        runId: "run-orphan",
        outcome: "merged",
  ...
  test(
    "#given attempt two is live with a stale attempt-one outcome #when reconciled #then the retry remains active and its worktree stays intact",
    async () => {
      // given
      ...
      const results = await reconcileReflectionRuns({
        ...
        getProcessStartIdentity: async (pid) =>
          pid === 222 ? "supervisor-two" : "child-two",
      });
  ...
  test(
    "#given attempt two is being launched with a stale attempt-one outcome #when reconciled before the shared deadline #then the retry remains active",
    async () => {
      // given
      const item = await fixture();
      await writeRunJsonAtomic(join(item.runDir, "ledger.json"), {
  ...
  test(
    "#given a failed child outcome with a valid commit #when reconciled #then output is never merged and the cursor remains retryable",
    async () => {
      // given
      const item = await fixture();
      await commitOrphanWorktree(item);
      await writeFile(
        join(item.runDir, "child-stderr.log"),
        "model child failed validation\n",
      );
      await writeRunJsonAtomic(join(item.runDir, "outcome.json"), {
        version: 1,
        runId: "run-orphan",
      ...
      expect(
        Bun.file(join(item.identity.paths.repo, "system", "orphan.md")).size,
      ).toBe(0);
  ...
  test(
    "#given genuinely gone processes with unknown liveness and no outcome #when the bounded watch expires #then the run is abandoned without cleanup or cursor advance",
    async () => {
      // given
      const item = await fixture();
      await writeRunJsonAtomic(join(item.runDir, "ledger.json"), {
      ...
      expect(
        JSON.parse(await readFile(join(item.runDir, "abandoned.json"), "utf8")),
      ).toMatchObject({
        version: 1,
        runId: "run-orphan",
        outcome: "abandoned_unknown",
  ...
  test(
    "#given a dead identified supervisor and live identified child #when the shared deadline elapses #then reconciliation signals on the persisted timeline and cleans only after confirmed death",
    async () => {
      // given
      const item = await fixture();
      await writeRunJsonAtomic(join(item.runDir, "ledger.json"), {
        ...
        childPid: 333,
        childProcessStart: "child-start",
      });
      let childAlive = true;
      const signals: Array<
        { pid: number; signal: NodeJS.Signals; at: number }
      > = [];
      let now = 900;
      ...
      const results = await reconcileReflectionRuns({
        ...
        now: () => now,
        getPidLiveness: (pid) =>
          pid === 222 ? "dead" : childAlive ? "alive" : "dead",
        getProcessStartIdentity: async (pid) =>
          pid === 333 ? "child-start" : null,
        waitUntil: async (deadlineAt) => {
          now = deadlineAt;
        },
  ...
  test(
    "#given old pre-launch reservations with unverifiable or reused launcher pids #when reconciled #then only confirmed pid reuse releases the reservation",
    async () => {
      // given
      const unknown = await fixture();
      await rmEfaultTolerant(unknown.runDir, {
        recursive: true,
        force: true,
        maxRetries: 10,
        retryDelay: 200,
      ...
      expect((await unknown.journal.getState()).reflected_completed_steps).toBe(
        0,
      );
    },
    30_000,
  ...
  test(
    "#given an old prelaunch worktree without a ledger and a confirmed-dead launcher #when reconciled #then resources and reservation are released",
    async () => {
      // given
      const item = await fixture();
