...
export function requireFinalizedResult(
  ...
  if (result === undefined) {
    throw new Error("Reflection finalization claim is busy");
  }
  if (
    result.outcome === "abandoned_unknown" || result.completion === undefined
  ) {
    throw new Error(
      `Reflection finalization did not publish completion for ${result.runId}`,
    );
  }
  return {
    runId: result.runId,
