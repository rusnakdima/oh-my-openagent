...
export function prepareReflectionCandidateSpawn(input: ReflectionSpawnInput) {
  const builder = input.fork === undefined
  ...
  return builder({
    ...(input.fork === undefined ? {} : {
      ...
      ...(input.fork.parentCwd === undefined
        ? {}
        : { parentCwd: input.fork.parentCwd }),
    }),
    run: input.run,
    ...
    peoplePolicy: {
      enabled:
        input.config.memory?.agents[input.identity.id]?.people?.enabled ??
          input.config.memory?.people.enabled ??
          true,
      max_entries:
        input.config.memory?.agents[input.identity.id]?.people?.max_entries ??
          input.config.memory?.people.max_entries ??
          40,
      max_entry_chars: input.config.memory?.agents[input.identity.id]?.people
        ?.max_entry_chars ??
        input.config.memory?.people.max_entry_chars ??
        200,
    },
    systemTokenBudget,
    systemTokenTarget: Math.floor(
      MEMORY_PRESSURE_SOFT_RATIO * systemTokenBudget,
    ),
