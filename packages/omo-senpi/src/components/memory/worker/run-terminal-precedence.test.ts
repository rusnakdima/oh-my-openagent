...
describe("run abandonment precedence", () => {
  test(
    "#given a publisher that finishes during liveness classification #when precedence is checked #then abandonment is vetoed",
    async () => {
      // given
  ...
  test(
    "#given a publish claimant whose pid is confirmed dead #when precedence is checked #then abandonment reclaims the terminal claim",
    async () => {
      // given
      const { runDir } = await fixture();
  ...
  test(
    "#given a publish claimant whose pid liveness is unknown #when precedence is checked #then the run stays active",
    async () => {
      // given
      const { runDir } = await fixture();
      await claimRunTerminal(
  ...
  test(
    "#given a genuinely dead run with unknown liveness #when precedence is checked #then it still abandons",
    async () => {
      // given
      const { runDir } = await fixture();
