...
function categoryUnavailableDetail(
  ...
  const base =
    `Reflection category "${category}" could not resolve a usable model (cause: ${resolution.cause})`;
  const providers = resolution.missingProviders?.join(", ");
  return providers === undefined || providers.length === 0
    ? base
    : `${base}; missing providers: ${providers}`;
}
...
export class SenpiSubprocessRunner implements ReflectionRunner {
  ...
  async launch(run: ReservedRun): Promise<ReflectionRunResult> {
    ...
    const reflection = resolveAgentReflectionSettings(
    ...
    const route = this.chooseLaunchRoute(
      run,
      resolution,
      registry,
      sessionModel,
  ...
  private chooseLaunchRoute(
    ...
    const surface: MemoryLaunchSurface = run.request.trigger === "dream"
      ? "dream"
      : "reflection";
    ...
    return chooseMemoryLaunchRoute({
      ...
      ...(sessionModel === undefined || sessionCost === undefined ? {} : {
        session: {
          ...
          ...(sessionModel.thinking === undefined
            ? {}
            : { thinking: sessionModel.thinking }),
          cost: sessionCost,
        },
      }),
  ...
  private async appendLaunched(
    ...
    const states = this.options.getTranscriptState === undefined
      ? []
      : await Promise.all(run.request.conversationIds.map((id) =>
        this.options.getTranscriptState?.(id)
      ));
    const backlogSteps = states.reduce(
      (sum, state) =>
        sum + (state?.steps_since_last_successful_reflection ?? 0),
      0,
    );
  ...
  private async mergedMetadata(
    runId: string,
  ): Promise<{ mergedCommitSha?: string; filesChanged?: number }> {
    try {
      const ledger = parseReservationRunLedger(
        await readRunJson<unknown>(
          join(
            this.options.identity.paths.reflection,
            "runs",
            runId,
            "ledger.json",
          ),
      ...
      return {
        ...(ledger.integrationSha === undefined
          ? {}
          : { mergedCommitSha: ledger.integrationSha }),
  ...
  private notifyCategoryUnavailable(
    config: OmoConfig,
    resolution: Extract<
      ReflectionModelResolution,
      { readonly kind: "category_unavailable" }
    ...
    const live = this.options.liveSession?.();
    if (
      !live?.ui || !shouldWarnCategoryUnavailable(config, resolution.category)
    ) return;
    if (!this.warnedCategory(`${live.sessionId}:${resolution.category}`)) {
      return;
    }
    const providers = resolution.missingProviders?.join(", ");
    safeNotify(
