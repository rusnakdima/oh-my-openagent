...
async function makeRun(options: {
  ...
  const runDir = realpathSync.native(
    await mkdtemp(join(tmpdir(), "memory-run-supervisor-")),
  );
  roots.push(runDir);
  await mkdir(runDir, { recursive: true, mode: 0o700 });
  await writeFile(
    join(runDir, "ledger.json"),
    `${JSON.stringify({ version: 1, runId: "run-a", kind: "reflection" })}\n`,
  );
  await writeFile(
...
async function waitForLedgerChild(
  runDir: string,
): Promise<{ readonly childPid: number }> {
  const path = join(runDir, "ledger.json");
  return await waitForFilesystemState(
    runDir,
    async () => {
      const value = JSON.parse(await readFile(path, "utf8")) as Record<
        ...
        unknown
      >;
      return typeof value.childPid === "number"
...
async function readOutcome(runDir: string): Promise<Outcome> {
  ...
  return JSON.parse(
    await readFile(join(runDir, "outcome.json"), "utf8"),
  ) as Outcome;
}
...
describe("memory run supervisor", () => {
  test(
    "#given an unrelated active handle #when the durable outcome is published #then the standalone supervisor exits",
    async () => {
      // given
      const runDir = await makeRun({ mode: "inspect" });
      const keepalivePath = join(
        import.meta.dir,
        "__fixtures__",
        "supervisor-keepalive.ts",
      );
  ...
  test(
    "#given a retryable model miss and a next attempt #when the supervisor publishes the outcome #then the ledger advances first",
    async () => {
      // given
  ...
  test(
    "#given a fixture launch manifest #when supervised #then identity is durable before the command starts and the outcome is published once",
    async () => {
      // given
      const runDir = await makeRun({ mode: "inspect" });
      ...
      const observation = JSON.parse(
        await readFile(join(runDir, "child-observation.json"), "utf8"),
      ) as Record<string, unknown>;
      expect(observation.pid).toBe(supervisor.pid);
      expect(
        typeof observation.processStart === "string" ||
          observation.processStart === null,
      ).toBe(true);
      expect(typeof observation.childPid).toBe("number");
      expect(
        typeof observation.childProcessStart === "string" ||
          observation.childProcessStart === null,
      ).toBe(true);
      expect(outcome).toMatchObject({
        version: 1,
        runId: "run-a",
      ...
      expect((await readdir(runDir)).filter((name) => name === "outcome.json"))
        .toEqual(["outcome.json"]);
    },
    60_000,
  );

  test(
    "#given a detached supervisor launched by a parent #when the parent is killed #then the supervisor still publishes the child outcome",
    async () => {
      // given
      const runDir = await makeRun({ mode: "inspect" });
      ...
      const ready = await new Promise<string>((resolve, reject) => {
        const timeout = setTimeout(
          () =>
            reject(new Error(`waited ${WAIT_MS}ms for parent launch receipt`)),
          WAIT_MS,
        );
  ...
  test(
    "#given a bootstrap waiting for release #when its parent pipe closes #then it exits without starting the manifest command",
    async () => {
      // given
      const runDir = await makeRun({ mode: "inspect" });
      const bootstrap = spawn(process.execPath, [
        supervisorPath,
        "--child-bootstrap",
        runDir,
      ], {
  ...
  test(
    "#given a released child #when the supervisor is killed abruptly #then the recorded live process group can be terminated",
    async () => {
      // given
      ...
      expect(JSON.parse(await readFile(join(runDir, "ledger.json"), "utf8")))
        .toMatchObject({ childPid });
    },
    60_000,
  );

  test(
    "#given a child that exits during termination grace #when the absolute deadline arrives #then timeout is recorded without escalation",
    async () => {
      // given
      const runDir = await makeRun({
        mode: "graceful",
  ...
  test(
    "#given a child that ignores SIGTERM #when termination grace expires #then the process group is killed",
    async () => {
      // given
      const runDir = await makeRun({
        mode: "stubborn",
