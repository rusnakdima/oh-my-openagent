...
export async function writeRunJsonAtomic(
  path: string,
  value: unknown,
  mode = 0o600,
): Promise<void> {
...
async function syncDirectory(path: string): Promise<void> {
  try {
    ...
    if (
      ...
      !["EISDIR", "EPERM"].includes(String(code))
    ) throw error;
  }
}

export async function readRunTextTail(
  path: string,
  maxBytes: number,
): Promise<string> {
  if (!Number.isInteger(maxBytes) || maxBytes <= 0) {
    throw new TypeError("run output limit must be positive");
  }
  let file;
  try {
...
export async function updateRunLedger(
  path: string,
...
export function parseRunPrelaunchArtifact(
  value: unknown,
): RunPrelaunchArtifact {
  if (
    !isRecord(value) ||
    value.version !== 1 ||
