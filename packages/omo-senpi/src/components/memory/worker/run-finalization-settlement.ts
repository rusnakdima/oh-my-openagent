...
export async function settleReservationRun(
  ...
  const current = parseReservationRunLedger(
    await readRunJson<unknown>(ledgerPath),
  );
  if (current.runId !== inputLedger.runId) {
    throw new Error("Finalization ledger run id changed");
  }
  const finalizedAt = current.finalizedAt ??
  ...
  await updateRunLedger(ledgerPath, {
    ...
    ...(decision.integrationSha === undefined
      ? {}
  ...
  const existing = await readReflectionCompletion(
    completionsDir,
    current.runId,
  );
  ...
  const completion = await ensureReflectionCompletion(completionsDir, {
    ...
    durationMs: Math.max(
      0,
      Date.parse(finalizedAt) - Date.parse(current.startedAt),
    ),
    ...(decision.outcome === "merged" && decision.integrationSha !== undefined
      ? { mergedCommitSha: decision.integrationSha }
      : {}),
    ...(current.validatedChangedPaths === undefined
      ? {}
      : { filesChanged: current.validatedChangedPaths.length }),
    consecutiveFailures: decision.outcome === "failed"
      ? healthBefore.streak + 1
      : 0,
...
async function readLegacyConversationIds(
  ...
  try {
    const payload = await readRunJson<Record<string, unknown>>(
      join(runDir, "transcript-payload.json"),
    );
