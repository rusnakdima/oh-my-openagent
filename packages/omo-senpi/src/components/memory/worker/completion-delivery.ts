...
export async function consumePendingReflectionCompletions(
  ...
  let names: string[];
  try {
    names = (await readdir(completionsDir)).filter((name) =>
      name.endsWith(".json")
    ).sort();
  } catch (error) {
  ...
  for (const name of names) {
    ...
    if (record?.identity === identity && record.delivery.status === "pending") {
      pending.push(record);
    }
  }
  ...
  const stale = pending.filter((record) =>
    Date.parse(record.finishedAt) < cutoff
  );
  const consumed: ReflectionCompletionRecord[] = [];
  for (const record of fresh.slice(0, DETAILED_DRAIN_LIMIT)) {
    consumed.push(
      await deliverReflectionCompletion(completionsDir, record, live, false),
    );
  }
  ...
  if (collapsed.length > 0) {
    ...
    for (const record of collapsed) {
      consumed.push(
        await markDelivered(completionsDir, record, live.sessionId),
      );
    }
  }
  for (const record of stale) {
    consumed.push(await markDelivered(completionsDir, record, live.sessionId));
  }
  ...
  if (fresh.length > 0) {
    safeNotify(
      live,
      drainMessage(fresh),
      fresh.some(isUnsuccessful) ? "warning" : "info",
    );
  }
  return consumed;
}
...
export async function deliverReflectionCompletion(
  ...
  if (notify) {
    safeNotify(
      live,
      completionMessage(delivered),
      completionLevel(delivered.outcome),
    );
  }
  return delivered;
}
...
function summarize(
  records: readonly ReflectionCompletionRecord[],
...
function drainMessage(records: readonly ReflectionCompletionRecord[]): string {
  ...
  return failures === 0
    ? `Delivered ${records.length} memory reflection completion${
      records.length === 1 ? "" : "s"
    }.`
...
export function safeNotify(
  ...
  try {
    ...
    live.logger?.warn("memory reflection notification failed", {
...
function completionMessage(record: ReflectionCompletionRecord): string {
  if (record.outcome === "merged") {
    return `Memory reflection ${record.runId} merged.`;
  }
  if (record.outcome === "no_changes") {
    return `Memory reflection ${record.runId} completed with no changes.`;
  }
  if (record.outcome === "timed_out") {
    return `Memory reflection ${record.runId} timed out; its transcript cursor was not advanced.`;
  }
