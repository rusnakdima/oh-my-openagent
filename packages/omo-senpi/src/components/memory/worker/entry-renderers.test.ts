...
function launched(
  over: Partial<ReflectionLaunchedEntry> = {},
): ReflectionLaunchedEntry {
  return {
    schemaVersion: 1,
...
function completion(
  over: Partial<ReflectionCompletionRecord> = {},
...
describe("memory reflection entry rendering", () => {
  test("#given every registered memory notice renderer #when rendered #then each emits a custom-message background block", () => {
    const cases = [
      renderReflectionLaunchedEntry({ data: launched() } as never, {
        expanded: false,
      }, BACKGROUND_THEME as never),
      renderReflectionCompletionEntry({ data: completion() } as never, {
        expanded: false,
      }, BACKGROUND_THEME as never),
      renderReflectionSummaryEntry(
        {
      ...
      renderReflectionHealthEntry(
        {
          data: {
            ...
            recommendation: "Retry reflection.",
          },
        } as never,
        { expanded: false },
        BACKGROUND_THEME as never,
    ...
    for (const component of cases) {
      expectNoticeBackground(component!.render(WIDE));
    }
  });
  describe("#given a launched reflection", () => {
    ...
    test("#when a single backlog step is pending #then the step noun is singular", () => {
      ...
      const lines = render(
        renderReflectionLaunchedEntry,
        launched({ backlogSteps: 1 }),
      );

      // then
      expect(lines[1]).toBe(
        "The outcome lands in this transcript when the run settles - triggered by step-count after 1 new step.",
      );
    });
    ...
    test("#when triggered manually #then the why line reads as a manual trigger", () => {
      ...
      const lines = render(
        renderReflectionLaunchedEntry,
        launched({ trigger: "manual" }),
      );

      // then
      expect(lines[1]).toBe(
        "The outcome lands in this transcript when the run settles - triggered manually after 25 new steps.",
      );
    });
    ...
    test("#when in flight #then the title is accent toned and the secondary rows are dim", () => {
      ...
      // when
      render(renderReflectionLaunchedEntry, launched(), {
        expanded: true,
  ...
  describe("#given a completed reflection", () => {
    test("#when the outcome merged #then it reads as success with a prose summary", () => {
      ...
      // when
      const lines = render(renderReflectionCompletionEntry, completion(), {
        theme: recorder.theme,
      });
    ...
    test("#when merge metadata is present and expanded #then the detail row carries identity and trigger", () => {
      ...
      // then
      expect(lines[3]).toBe(
        "category quick · identity project-a1b2c3d4 · trigger step-count",
      );
    });
    ...
    test("#when the outcome timed out #then it reads as warning", () => {
      ...
      // then
      expect(lines[0]).toBe(
        bold("⚠ Memory reflection timed out · reflection-run-2"),
      );
  ...
  describe("#given a collapsed backlog summary", () => {
    ...
    test("#when nothing failed #then it stays muted and says so", () => {
      ...
      // when
      const lines = render(renderReflectionSummaryEntry, {
        ...summary,
        failedCount: 0,
      }, { theme: recorder.theme });
    ...
    test("#when exactly one completion collapsed #then the noun is singular", () => {
      ...
      // then
      expect(lines[0]).toBe(
        bold("● Memory reflection · 1 older completion collapsed"),
      );
    });
    ...
    test("#when it renders expanded #then the detail row carries the time range", () => {
      ...
      // then
      expect(lines[3]).toBe(
        "oldest 2026-08-11T04:00:00.000Z · newest 2026-08-13T08:00:00.000Z",
      );
    });
  });
  ...
  describe("#given a reflection failure streak", () => {
    ...
    test("#when the recommendation is a fragment #then the why line is a full sentence", () => {
      // when
      const lines = render(renderReflectionHealthEntry, {
        ...health,
  ...
  describe("#given a narrow terminal", () => {
    ...
    test("#when a very long run id is rendered #then the identifier itself is excerpted", () => {
      ...
      const lines = render(
        renderReflectionCompletionEntry,
        completion({
          runId: "reflection-run-with-an-extremely-long-identifier",
        }),
      );

      // then
      expect(lines[0]).toBe(
        bold("● Memory reflection merged · reflection-run-with-an-ex..."),
      );
    });
    ...
    test("#when coloured output is truncated #then no terminal reset leaks into the middle of the why span", () => {
      ...
      // then
      expect(lines.join("\n")).toContain(
        "[dim]Reflection did not finish; the transcript cursor was",
      );
      expect(lines.join("\n")).not.toContain("\u001b[0m");
    });
    ...
    test("#when the title is bolded at a narrow width #then the bold escapes wrap the whole fitted title", () => {
      ...
      expect(lines[0].indexOf("\u001b[1m")).toBeLessThan(
        lines[0].indexOf("\u001b[22m"),
      );
    });
    ...
    test("#when rendered at hostile widths #then no line ever exceeds the terminal width", () => {
      ...
      for (const width of [1, 5, 20, 40, 60, 80, 100]) {
        for (
          const line of render(renderReflectionCompletionEntry, record, {
            width,
            expanded: true,
          })
        ) {
