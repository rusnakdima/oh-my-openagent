...
describe("reflection completion flow", () => {
  ...
  test("#given a legacy completion without enrichment fields #when it renders #then backward compatibility is preserved", () => {
    ...
    // when
    const renderer = api.renderers.find((item) =>
      item.customType === REFLECTION_COMPLETION_ENTRY_TYPE
    )?.renderer;
  ...
  test("#given an existing consumed completion #when a retry ensures the same pending record #then consumed delivery is preserved", async () => {
    ...
    const root = realpathSync.native(
      await mkdtemp(join(tmpdir(), "reflection-completion-")),
    );
    roots.push(root);
    const api = new CapturedCompletionApi();
    ...
    expect(JSON.parse(await readFile(join(root, "run-offline.json"), "utf8")))
      .toEqual(ensured);
  });
  ...
  test("#given an existing completion with a different outcome #when ensured #then corruption is rejected without overwrite", async () => {
    ...
    roots.push(root);
    await recordReflectionCompletion(root, record());
    const mismatched: ReflectionCompletionRecord = {
      ...record(),
      outcome: "failed",
    };
  ...
  test("#given no live source session #when completion is recorded #then it remains durable and pending", async () => {
    ...
    const persisted: unknown = JSON.parse(
      await readFile(join(root, "run-offline.json"), "utf8"),
    );
  ...
  test("#given a worker completion record #when the landed palace collector reads reflection outcomes #then runId outcome and finishedAt match its contract", async () => {
    ...
    await recordReflectionCompletion(
      join(paths.reflection, "completions"),
      record(),
    );
  ...
  test("#given a pending offline completion #when its source session starts #then appendEntry notify and consumed state happen without a model message", async () => {
    ...
    // when
    const consumed = await consumePendingReflectionCompletions(
      root,
      "agent-test",
      {
        sessionId: "conversation-a",
    ...
    expect(consumed[0]?.delivery).toMatchObject({
      status: "consumed",
      sessionId: "conversation-a",
    });
  });
  ...
  describe("#given target foreign and missing-identity pending completions in one drained directory", () => {
    describe("#when the target identity drains the backlog", () => {
      test("#then only target records are delivered counted and consumed", async () => {
        ...
        const targetRecords = Array.from(
          { length: 8 },
          (_, index): ReflectionCompletionRecord => ({
            ...record(),
            runId: `target-${index}`,
            finishedAt: new Date(now - (index + 2) * 60_000).toISOString(),
          }),
        );
        const foreignRecords = Array.from(
          { length: 2 },
          (_, index): ReflectionCompletionRecord => ({
            ...record(),
            runId: `foreign-${index}`,
        ...
        for (const pending of [...targetRecords, ...foreignRecords]) {
          await recordReflectionCompletion(root, pending);
        }
        ...
        await writeFile(
          join(root, "missing-identity.json"),
        ...
        expect(
          api.entries.filter((entry) =>
            entry.customType === REFLECTION_COMPLETION_ENTRY_TYPE
          ),
        ).toHaveLength(5);
        expect(
          api.entries.filter((entry) =>
          ...
          expect.arrayContaining(
            targetRecords.slice(0, 5).map((pending) =>
        ...
        expect(
          api.entries.filter((entry) =>
            entry.customType === REFLECTION_SUMMARY_ENTRY_TYPE
          ),
        ).toEqual([{
        ...
        for (const pending of foreignRecords) {
          expect(
            JSON.parse(
              await readFile(join(root, `${pending.runId}.json`), "utf8"),
            ),
          ).toMatchObject({
            identity: "agent-foreign",
            delivery: { status: "pending" },
        ...
        expect(
          JSON.parse(
            await readFile(join(root, "missing-identity.json"), "utf8"),
          ),
        ).toMatchObject({
          runId: "missing-identity",
          delivery: { status: "pending" },
      ...
      test("#then five details one summary and one notify are emitted once while stale records are silently consumed", async () => {
        ...
        const records = Array.from(
          ...
          (_, index): ReflectionCompletionRecord => ({
            ...
            ...(index === 5 || index === 6
              ? { reason: "child_exit", detail: "same failure" }
              : {}),
            finishedAt: new Date(
              now -
                (index < 6 ? index * 60_000 : (8 + index) * 24 * 60 * 60_000),
            ).toISOString(),
          }),
        );
        ...
        // when
        const first = await consumePendingReflectionCompletions(
          root,
          "agent-test",
          live,
          now,
        );
        const second = await consumePendingReflectionCompletions(
          root,
          "agent-test",
          live,
          now,
