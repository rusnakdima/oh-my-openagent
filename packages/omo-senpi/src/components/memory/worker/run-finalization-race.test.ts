...
async function fixture(attempt: number) {
  ...
  await new GitMemoryRepo({ dir: identity.paths.repo, agentId: identity.id })
    .init();
  ...
  const ledger: ReservationRunLedger = {
    ...
    baseSha: await new GitMemoryRepo({
...
describe("run finalization publication races", () => {
  test(
    "#given outcome publishes during abandonment state read #when terminalized #then matching outcome wins and abandoned is never published",
    async () => {
      // given
  ...
  test(
    "#given publication is scheduled after every unknown-liveness read #when abandonment writes #then the matching publisher wins atomically",
    async () => {
      // given
      const item = await fixture(1);
      ...
      const result = await abandonReservationRun(
        {
          ...
          getPidLiveness: () => {
            publication ??= (async () => {
              await claimRunTerminal(item.runDir, {
                runId: ledger.runId,
  ...
  test(
    "#given attempt two and a stale attempt-one outcome #when current attempt is confirmed dead #then stale outcome is ignored",
    async () => {
      // given
      ...
      expect(JSON.parse(await Bun.file(join(item.runDir, "final.json")).text()))
        .toMatchObject({
          outcome: "failed",
        });
    },
