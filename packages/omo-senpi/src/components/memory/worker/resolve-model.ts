...
import {
  chooseReflectionLaunchModel,
  type ReflectionLaunchCandidate,
} from "./model-cost";
import {
  readModelPricing,
  selectRegistryFallbackModels,
...
export type ReflectionThinkingLevel =
  | "off"
  | "minimal"
  | "low"
  | "medium"
...
export function resolveReflectionModel(
  ...
  if (resolution.kind !== "resolved") {
    ...
    const pinnedSelector = typeof pinned === "string" && pinned.includes("/")
      ? pinned
      : undefined;
    if (
      resolution.kind === "model_unavailable" && pinnedSelector !== undefined
    ) {
    ...
    return {
      ...
      category,
      cause: resolution.kind === "not_found"
        ? "not_found"
  ...
  const resolvedFallbacks = (resolution.spec.fallback_models ?? []).map(
    (fallback): ReflectionModelCandidate => {
      const fallbackThinking = normalizeThinking(
        fallback.reasoning_effort ?? fallback.variant,
      );
      return {
...
function resolveBeyondCategory(
  ...
  | Extract<ReflectionModelResolution, { readonly kind: "resolved" }>
  | undefined {
  const candidates = registry === undefined
    ? []
    : selectRegistryFallbackModels(registry.getAvailable());
  ...
  return {
    ...
    ...(thinking === undefined ? {} : { thinking }),
    source: decision.choice === "inherit"
      ? "session_inherit"
      : "registry_fallback",
    fallbacks: decision.choice === "inherit"
      ? []
      : candidates.slice(1).map((entry) => ({ model: entry.model })),
  };
}
...
function sessionCandidate(
  ...
  const cost = registry === undefined
    ? undefined
    : readModelPricing(registry.find(session.provider, session.id));
  return {
...
function configuredFallbackModels(
  ...
  const ordered = [
    ...configured.slice(selectedIndex + 1),
    ...configured.slice(0, selectedIndex),
  ];
...
export function shouldWarnCategoryUnavailable(
  config: OmoConfig,
  category: string,
): boolean {
