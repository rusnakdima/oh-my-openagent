...
export function estimateFreshCost(
...
export function chooseReflectionLaunchModel(
  input: ReflectionLaunchInput,
): ReflectionLaunchChoice {
  const { fresh, session } = input;
  if (fresh === undefined && session === undefined) {
    throw new Error(
      "chooseReflectionLaunchModel requires at least one candidate",
    );
  }
  if (session === undefined && fresh !== undefined) {
    return pick("fresh", fresh, "only_candidate");
  }
  if (fresh === undefined && session !== undefined) {
    return pick("inherit", session, "only_candidate");
  }
  if (fresh === undefined || session === undefined) {
    throw new Error(
      "chooseReflectionLaunchModel requires at least one candidate",
  ...
  const inheritCost = estimateInheritCost(
    session,
    input.prefixTokens,
    input.workloadTokens,
    input.cacheReusable,
  );
  if (freshCost === undefined || inheritCost === undefined) {
    return pick("fresh", fresh, "no_pricing");
  }
