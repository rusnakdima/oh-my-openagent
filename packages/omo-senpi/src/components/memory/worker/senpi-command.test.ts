...
function runtime(
  overrides: Partial<SenpiLaunchRuntime> = {},
): SenpiLaunchRuntime {
  return {
    isBunBinary: false,
...
describe("resolveSenpiLaunch", () => {
  test(
    "#given a Windows npm shim #when resolved #then Node launches the adjacent Senpi CLI",
    async () => {
      // given
      ...
      expect(launch).toEqual({
        command: "C:\\node.exe",
        prefixArgs: [realpathSync.native(cli)],
      });
    },
    30_000,
  );
  ...
  test(
    "#given no executable but an installed CLI #when resolved #then the current interpreter launches the CLI",
    async () => {
      // given
      const root = await tempRoot();
  ...
  test(
    "#given package lookup fails in a script-hosted Senpi process #when resolved #then the current entry script is retained",
    async () => {
      // given
      const root = await tempRoot();
  ...
  test(
    "#given no executable installed CLI or current entry #when resolved #then it fails instead of launching a bare interpreter",
    () => {
      // given
      const resolve = () =>
        resolveSenpiLaunch(
  ...
  test(
    "#given the real restricted PATH fallback #when launched #then Senpi prints its version",
    () => {
      // given
