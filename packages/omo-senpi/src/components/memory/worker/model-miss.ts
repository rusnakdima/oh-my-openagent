...
const MODEL_NOT_FOUND_PATTERN =
  /^Error: Model "([^"]+)" not found\. Use --list-models to see available models\.$/m;
const API_KEY_NOT_FOUND_PATTERN =
  /^(?:Error:\s*)?No API key found for\s+([^\s.]+)/m;
const HTTP_STATUS_PATTERN = /(?:^|\s)(\d{3})\s*:/;
const PROVIDER_DETAIL_MAX_CHARS = 200;

export function classifyRetryableModelMiss(
  result: ModelMissResult,
): RetryableModelMiss | undefined {
  ...
  const statusCode = Number.parseInt(
    HTTP_STATUS_PATTERN.exec(detail)?.[1] ?? "",
    10,
  );
