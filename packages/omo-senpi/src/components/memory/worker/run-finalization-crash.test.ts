...
async function fixture(integrate = true) {
  ...
  await writeFile(
    join(worktree.dir, "system", "learned.md"),
    "---\ndescription: Learned\n---\nlearned\n",
  );
  ...
  if (validation.status !== "valid") {
    throw new Error(`expected valid worktree: ${validation.status}`);
  }
  if (integrate) {
...
async function receiptCount(repoDir: string): Promise<number> {
  ...
  return log.stdout.split("\0").filter((body) =>
    body.includes("Omo-Run: run-1")
  ).length;
}

describe("reflection finalization crash recovery", () => {
  test(
    "#given integration landed before its checkpoint #when finalization retries #then receipt recovery settles exactly once",
    async () => {
      // given
  ...
  test(
    "#given merge cleanup and settlement completed before final publication #when retried #then completion and final repair without duplicate settlement",
    async () => {
      // given
      const item = await fixture();
  ...
  test(
    "#given a pressure dream merges above its token target #when finalization validates committed system memory #then the merge records a budget_not_met warning",
    async () => {
      const item = await fixture(false);
      await writeFile(
        join(item.worktree.dir, "system", "learned.md"),
        "L".repeat(400),
      );
      const childRepo = new GitMemoryRepo({
        dir: item.worktree.dir,
        agentId: item.identity.id,
      });
      await childRepo.commitWrite(
        ["system/learned.md"],
        "dream retained too much",
        {
          agentId: item.identity.id,
      ...
      expect(result).toMatchObject({
        ...
        detail:
          "Committed system/ estimate is 108 tokens; pressure dream target is below 80 tokens",
      });
  ...
  test(
    "#given parent_dirty was checkpointed before cleanup #when finalization retries #then the durable decision and user edit are preserved",
    async () => {
      // given
      const item = await fixture(false);
      await writeFile(
        join(item.repo.dir, "system", "base.md"),
        "---\ndescription: Base\n---\nuser edit\n",
      );
      ...
      expect(await readFile(join(item.repo.dir, "system", "base.md"), "utf8"))
        .toContain("user edit");
