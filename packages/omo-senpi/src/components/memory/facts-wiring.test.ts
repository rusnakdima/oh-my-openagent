...
async function fixture(): Promise<MemoryIdentityPaths> {
  const dir = realpathSync.native(
    await mkdtemp(join(tmpdir(), "omo-memory-facts-wiring-")),
  );
  tempDirs.push(dir);
...
async function seedJournal(
  ...
  const journal = new TranscriptJournal({
    journalDir: join(paths.transcripts, SESSION),
  });
...
describe("facts wiring settle enqueue", () => {
  test(
    "#given debounce four #when four settles complete #then exactly one all-pending extractor launch fires",
    async () => {
      // given
      ...
      await Promise.race([
        ...
        new Promise<never>((_, reject) =>
          setTimeout(
            () => reject(new Error("facts launch did not fire")),
            2_000,
          )
        ),
      ]);
  ...
  test(
    "#given facts enabled and a new journal delta #when settle runs #then one queue file is published",
    async () => {
      // given
      const paths = await fixture();
      await seedJournal(paths, 2);
  ...
  test(
    "#given the same cursor #when settle runs twice #then no duplicate entry is published",
    async () => {
      // given
      const paths = await fixture();
      await seedJournal(paths, 2);
  ...
  test(
    "#given facts disabled #when settle runs #then nothing is enqueued",
    async () => {
      // given
      const paths = await fixture();
      await seedJournal(paths, 2);
  ...
  test(
    "#given an empty journal #when settle runs #then nothing is enqueued",
    async () => {
      // given
      const paths = await fixture();
      const wiring = wiringFor(paths, true);
  ...
  test(
    "#given a pre-aborted drain signal #when final-delta enqueue is requested #then no journal read or queue mutation starts",
    async () => {
      const paths = await fixture();
  ...
  test(
    "#given abort arrives while the final journal delta is being read #when enqueue reaches the queue boundary #then no queue mutation starts",
    async () => {
      const paths = await fixture();
      const controller = new AbortController();
      const wiring = createMemoryFactsWiring({
...
describe("facts wiring shutdown launch", () => {
  test(
    "#given a pre-aborted drain signal at a met threshold #when launch is requested #then no extractor spawn starts",
    async () => {
      const paths = await fixture();
  ...
  test(
    "#given abort arrives during the threshold probe #when launch reaches the extractor boundary #then no extractor spawn starts",
    async () => {
      const paths = await fixture();
      await seedJournal(paths, 1);
      const controller = new AbortController();
...
describe("facts wiring session_start reconcile", () => {
  test(
    "#given a leftover queue file from a crashed run #when reconcile runs #then it is reported launchable",
    async () => {
  ...
  test(
    "#given facts disabled #when reconcile runs #then no leftover work is reported",
    async () => {
      // given
      const paths = await fixture();
      await seedJournal(paths, 2);
  ...
  test(
    "#given a consumed batch #when reconcile runs #then nothing is left to relaunch",
    async () => {
      // given
      const paths = await fixture();
      await seedJournal(paths, 2);
