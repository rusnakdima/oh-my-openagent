...
function inlineJson(html: string): Record<string, unknown> {
  ...
  if (match?.[1] === undefined) {
    throw new Error("palace html has no inline data script");
  }
  ...
  if (parsed === null || typeof parsed !== "object") {
    throw new Error("palace data is not an object");
  }
...
describe("palace people graph", () => {
  test(
    "#given committed people cards #when the graph is collected #then nodes carry slug, name and kind from card frontmatter",
    async () => {
      ...
      expect(people?.nodes.map((node) => node.slug).sort()).toEqual([
        "human",
        "jane-doe",
        "sam-rivers",
      ]);
  ...
  test(
    "#given RELATIONSHIP card lines #when the graph is collected #then edges match those lines and resolve to known slugs",
    async () => {
      const fixture = await createPalaceFixture();
      await seedPalacePeople(fixture);
  ...
  test(
    "#given a card line that is not a RELATIONSHIP #when the graph is collected #then it produces no edge",
    async () => {
      const fixture = await createPalaceFixture();
      await seedPalacePeople(fixture);
      ...
      expect(people?.edges.some((edge) => edge.predicate === "senior-engineer"))
        .toBe(false);
  ...
  test(
    "#given an uncommitted people card #when the graph is collected #then the node is labelled as absent from the system prompt",
    async () => {
      const fixture = await createPalaceFixture();
      await seedPalacePeople(fixture);
      ...
      expect(people?.nodes.find((node) => node.slug === "jane-doe")?.state)
        .toBe("committed");
    },
    30_000,
  );
  ...
  test(
    "#given a repo without a people directory #when the graph is collected #then only the primary human node is present and no edges are derived",
    async () => {
      const fixture = await createPalaceFixture();
  ...
  test(
    "#given people.enabled false #when the graph is collected #then no people section is produced",
    async () => {
      const fixture = await createPalaceFixture();
      await seedPalacePeople(fixture);
  ...
  test(
    "#given a card whose entry exceeds the configured limit #when the graph is collected #then the diagnostic is surfaced with its card path",
    async () => {
      const fixture = await createPalaceFixture();
      await seedPalacePeople(fixture);
      ...
      expect(
        people?.diagnostics.some((entry) =>
          entry.path === "people/jane-doe/card.md"
        ),
      ).toBe(true);
    },
    30_000,
  ...
  test(
    "#given people.enabled true #when the palace is generated #then the inline payload carries the derived graph",
    async () => {
      const fixture = await createPalaceFixture();
      await seedPalacePeople(fixture);

      const result = await generatePalaceHtml(fixture.context, {
        people: { enabled: true, limits: LIMITS },
      });
  ...
  test(
    "#given people.enabled false #when the palace is generated #then the panel and its tab are absent",
    async () => {
      const fixture = await createPalaceFixture();
      await seedPalacePeople(fixture);

      const result = await generatePalaceHtml(fixture.context, {
        people: { enabled: false, limits: LIMITS },
      });
      const html = await readFile(result.path, "utf8");
      const data = inlineJson(html);
  ...
  test(
    "#given a people card carrying a script-breakout payload #when the palace is generated #then the raw payload never reaches the document",
    async () => {
      const fixture = await createPalaceFixture();
