...
import { type PalaceEntryState, UNCOMMITTED_LABEL } from "./collectors";
...
export async function collectPeople(
  ...
  const committed = head === null
    ? []
    : (await repo.lsTree(head)).filter(isCardPath);
  ...
  const paths = [...new Set([...committed, ...working])].sort(
    byPrimaryHumanFirst,
  );
  ...
  for (const path of paths) {
    const state: PalaceEntryState =
      dirty.has(path) || head === null || !committedSet.has(path)
        ? UNCOMMITTED_LABEL
        : "committed";
    ...
    for (const entry of parsed.card.entries) {
      ...
      if (edge === undefined) {
        diagnostics.push({
          path,
          message:
            `RELATIONSHIP entry is not '<predicate>: <target>': ${entry.content}`,
        });
        continue;
      }
...
function parseFile(
  raw: string,
): ReturnType<typeof parseMemoryFile> | undefined {
  try {
...
async function workingCardPaths(repoDir: string): Promise<readonly string[]> {
  const paths: string[] = [];
  const human = await stat(join(repoDir, PRIMARY_HUMAN_CARD_PATH)).catch(() =>
    undefined
  );
  if (human?.isFile() === true) paths.push(PRIMARY_HUMAN_CARD_PATH);
  const slugs = await readdir(join(repoDir, "people"), { withFileTypes: true })
    .catch(() => []);
...
function isCardPath(path: string): boolean {
  ...
  if (!path.startsWith(PEOPLE_PREFIX) || !path.endsWith(CARD_SUFFIX)) {
    return false;
  }
