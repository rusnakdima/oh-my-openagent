...
export interface PalaceData {
  ...
  readonly external: {
    readonly entries: readonly PalaceExternalEntry[];
...
export async function collectPalaceData(
  ...
  return {
    ...
    external: {
      entries: external,
      tree: renderExternalProjection(external.map((entry) => entry.path)),
    },
    history,
    reflection,
...
export function renderPalaceHtml(data: PalaceData): string {
  return PALACE_TEMPLATE.replace(
    PALACE_DATA_PLACEHOLDER,
    () => encodePalaceData(data),
  )
    .replace(
      PALACE_PEOPLE_TAB,
      () => (data.people === undefined ? "" : PALACE_PEOPLE_TAB),
    )
    .replace(
      PALACE_PEOPLE_PANEL,
      () => (data.people === undefined ? "" : PALACE_PEOPLE_PANEL),
    );
}
