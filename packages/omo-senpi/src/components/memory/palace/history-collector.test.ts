...
describe("palace history collector", () => {
  test(
    "#given reflection and ordinary commits #when history is collected #then reflection commits are tagged and caps are declared",
    async () => {
      ...
      const reflection = history.commits.find((commit) =>
        commit.subject.startsWith("feat(reflection)")
      );
      expect(reflection?.isReflection).toBe(true);
      expect(reflection?.diff).toContain("learned thing");
      expect(history.commits.filter((commit) => commit.isReflection))
        .toHaveLength(1);
  ...
  test("#given merge and chore reflection subjects #when matched against the reflection pattern #then only reflection-scoped commits match", () => {
    expect(REFLECTION_COMMIT_PATTERN.test("feat(reflection): learn")).toBe(
      true,
    );
    expect(REFLECTION_COMMIT_PATTERN.test("fix(reflection): repair")).toBe(
      true,
    );
    expect(REFLECTION_COMMIT_PATTERN.test("chore(reflection): prune")).toBe(
      true,
    );
    expect(REFLECTION_COMMIT_PATTERN.test("merge(reflection): run 12")).toBe(
      true,
    );
    expect(REFLECTION_COMMIT_PATTERN.test("feat(memory): unrelated")).toBe(
      false,
    );
    expect(REFLECTION_COMMIT_PATTERN.test("docs: mentions reflection later"))
      .toBe(false);
  });
});
