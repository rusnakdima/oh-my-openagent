...
describe("palace reflection collector", () => {
  test(
    "#given journal state and completion records #when reflection data is collected #then cursor state and recent outcomes are returned newest first",
    async () => {
      const fixture = await createPalaceFixture();
      await fixture.writeCompletion("run-old", {
        runId: "run-old",
        outcome: "merged",
        finishedAt: "2026-01-01T00:00:00.000Z",
      });
      await fixture.writeCompletion("run-new", {
        runId: "run-new",
        outcome: "failed",
      ...
      expect(reflection.outcomes.map((entry) => entry.runId)).toEqual([
        "run-new",
        "run-old",
      ]);
  ...
  test(
    "#given more completion records than the limit #when reflection data is collected #then only the newest N are kept",
    async () => {
      const fixture = await createPalaceFixture();
