...
export type PalaceContextResolver = (
...
export function registerPalaceCommand(
  ...
  pi.registerCommand("palace", {
    description:
      "Generate the memory palace HTML viewer for the bound identity.",
    handler: (_args: string, ctx: PalaceCommandContext) =>
      runPalaceCommand(resolve, ctx, resolvePeople),
  });
}
...
export async function runPalaceCommand(
  ...
  const { path } = await generatePalaceHtml(
    identityContext,
  ...
  const result = await ctx.exec(opener.command, [...opener.args, path]).catch(
    () => undefined,
  );
