...
export const REFLECTION_COMMIT_PATTERN =
  /^(?:feat|fix|chore)\(reflection\)|^merge\(reflection\)/;
...
async function readMetadata(
  repo: GitMemoryRepo,
  exec: GitExec,
): Promise<readonly CommitMetadata[]> {
  ...
  for (const record of raw.split(RECORD_SEPARATOR)) {
    ...
    if (
      sha === undefined || author === undefined || date === undefined ||
      subject === undefined
    ) continue;
...
function capDiff(
  ...
  if (capped.length > HISTORY_PER_DIFF_CAP) {
    capped = `${
      capped.slice(0, HISTORY_PER_DIFF_CAP)
    }\n\n[diff truncated - exceeded ${
      Math.round(HISTORY_PER_DIFF_CAP / 1024)
    }KB]`;
    truncated = true;
  }
  if (totalSoFar + capped.length > HISTORY_TOTAL_PAYLOAD_CAP) {
