...
describe("palace entry collector", () => {
  test(
    "#given a committed memory repo #when core entries are collected #then system files carry projection paths and bodies",
    async () => {
      ...
      expect(core.every((entry) => entry.path.startsWith("system/"))).toBe(
        true,
      );
    },
    30_000,
  ...
  test(
    "#given committed binary and text assets #when external entries are collected #then binaries report size only and never content",
    async () => {
      const fixture = await createPalaceFixture();

      const external = await collectExternal(fixture.repo, fixture.head);

      const binary = external.find((entry) =>
        entry.path === "reference/logo.png"
      );
      expect(binary?.binary).toBe(true);
      expect(binary?.byteSize).toBeGreaterThan(0);
      expect(binary?.body).toBeUndefined();
      const text = external.find((entry) =>
        entry.path === "reference/notes.md"
      );
  ...
  test(
    "#given a dirty working tree #when core entries are collected #then uncommitted files are labelled as absent from the system prompt",
    async () => {
      const fixture = await createPalaceFixture();
      ...
      expect(core.find((entry) => entry.path === "system/persona.md")?.state)
        .toBe("committed");
    },
    30_000,
  );
