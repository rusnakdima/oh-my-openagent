...
import { type PalaceCommandContext, registerPalaceCommand } from "./command";
import {
...
async function runPalace(
  ...
  if (registration === undefined) {
    throw new Error("palace command was not registered");
  }
  const handler = registration.options.handler;
  if (typeof handler !== "function") {
    throw new Error("palace command has no handler");
  }
...
describe("palace command", () => {
  ...
  test("#given a tmux or ssh session #when /palace runs #then the path is printed instead of opened", async () => {
    const fixture = await createPalaceFixture();
    const tmux = commandContext({
      env: { TMUX: "/tmp/tmux-1000/default,1,0" },
    });
    const ssh = commandContext({
      env: { SSH_CONNECTION: "10.0.0.2 22 10.0.0.1 22" },
    });
