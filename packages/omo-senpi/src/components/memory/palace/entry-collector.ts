...
export async function collectCore(
  ...
  const committed = head === null
    ? []
    : (await repo.lsTree(head)).filter(isSystemMarkdown);
  ...
  for (const path of paths.sort()) {
    const state = dirty.has(path) || head === null
      ? UNCOMMITTED_LABEL
      : "committed";
...
export async function collectExternal(
  ...
  const committed = head === null
    ? []
    : (await repo.lsTree(head)).filter(isExternal);
  const dirty = await dirtyPaths(repo);
  ...
  for (const path of paths.sort()) {
    ...
    entries.push({
      ...
      state,
      ...(measured.binary || measured.text === undefined
        ? {}
        : { body: measured.text }),
    });
  }
...
> {
  if (state === UNCOMMITTED_LABEL) {
    ...
    const text = await readFile(join(repo.dir, path), "utf8").catch(() =>
      undefined
    );
...
function decide(
  ...
  const binary = text === undefined || isBinaryText(text) ||
    Buffer.byteLength(text, "utf8") !== byteSize;
  return binary
    ? { binary: true, byteSize }
    : { binary: false, byteSize, text };
}
