...
const seatbeltTest = test.skipIf(
...
async function runUnderFactsSandbox(
  setup: ScratchFixture,
  script: string,
): Promise<number | null> {
  ...
  const run = Bun.spawnSync([transformed.command, ...transformed.args], {
    cwd: transformed.cwd,
    env: transformed.env,
  });
  return run.exitCode;
...
describe("facts sandbox absent lock parent", () => {
  test(
    "#given an agent dir that does not exist at all (fresh machine) #when the required-policy facts sandbox is built #then a typed unavailable error names the missing parent instead of a raw ENOENT",
    async () => {
      // given
  ...
  test(
    "#given an agent dir that does not exist at all (fresh machine) #when the auto-policy facts sandbox is built #then it degrades to the unsandboxed identity transform with an explicit warning naming the missing parent",
    async () => {
      // given
      const root = mkdtempSync(join(tmpdir(), "omo-facts-absent-agent-"));
      roots.push(root);
...
describe("facts sandbox lock-grant invariants (real seatbelt)", () => {
  seatbeltTest(
    "#given the real Darwin seatbelt profile rendered by the facts sandbox #when a child runs the full mkdir/touch/rm/rmdir lifecycle on both granted locks #then every operation succeeds and leaves no lock directory behind",
    async () => {
      // given
      ...
      const lifecycle = (lock: string) =>
        `mkdir '${lock}' && touch '${lock}/owner' && rm '${lock}/owner' && rmdir '${lock}'`;

      // when
      const exitCode = await runUnderFactsSandbox(
        setup,
        `${lifecycle(settingsLock)} && ${lifecycle(authLock)}`,
      );
  ...
  seatbeltTest(
    "#given the same profile #when a child mkdirs a THIRD lock path (models.json.lock) #then the deny-by-default wall rejects it",
    async () => {
      // given
      const setup = scratchFixture();
  ...
  seatbeltTest(
    "#given the same profile #when a child appends to auth.json and to settings.json #then both appends are denied and the files stay byte-identical",
    async () => {
      // given
      const setup = scratchFixture();
      ...
      // when
      const settingsDenied = await runUnderFactsSandbox(
        setup,
        `printf tampered >> '${setup.settingsFile}'`,
      );
      const authDenied = await runUnderFactsSandbox(
        setup,
        `printf tampered >> '${setup.authFile}'`,
      );
  ...
  seatbeltTest(
    "#given an agent dir where NEITHER lock directory exists yet #when the facts profile is rendered #then the build succeeds and carries both lock grants",
    async () => {
      // given
      const setup = scratchFixture();
      const settingsLock = join(setup.agentDirReal, "settings.json.lock");
