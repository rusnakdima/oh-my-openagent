 ...
 */
export async function cleanupTerminalFactsRun(
  options: FactsRunCleanupOptions,
): Promise<void> {
  ...
  for (const name of DISPOSABLE) {
    ...
    try {
      ...
      options.warn?.("facts run artifact cleanup failed", {
        path,
  ...
  for (const name of names.sort()) {
    ...
    if (name.startsWith(PRUNE_TOMBSTONE_PREFIX)) {
      try {
        ...
        options.warn?.("facts run tombstone cleanup failed", {
          path: runDir,
          error: describe(error),
        });
      }
    ...
    if (!DISPOSABLE.some((artifact) => existsSync(join(runDir, artifact)))) {
      continue;
    }
...
export function isTerminalRunDir(runDir: string): boolean {
  return existsSync(join(runDir, "final.json")) ||
