...
export function scoreDreamCandidate(
  input: DreamScoreInput,
): Pick<DreamScoredConversation, "operands" | "score"> {
  const ageHours =
    Math.max(0, input.now.getTime() - Date.parse(input.lastActivity)) /
    3_600_000;
  const recency = Number.isFinite(ageHours)
    ? Math.min(1, Math.exp(-ageHours / 168))
    : 0;
  const operands: DreamScoreOperands = {
    searchHits: Math.min(10, Math.max(0, input.searchMatchCount)) / 10,
    unreflectedSteps:
      Math.min(50, Math.max(0, input.stepsSinceLastSuccessfulReflection)) / 50,
    recency,
