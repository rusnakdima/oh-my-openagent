...
async function makeStagedEntry(): Promise<string> {
  const runtimeDir = await mkdtemp(
    join(tmpdir(), "omo-senpi-ast-grep-runtime-"),
  );
...
function recordingLogger(): ComponentLogger & {
...
describe("createAstGrepComponent", () => {
  it.each([
    ...
    {
      context: "child",
      env: { SENPI_CODING_AGENT_SESSION_DIR: "/tmp/senpi-child" },
    },
  ])(
    "#given a staged runtime in a $context session #when registered #then declares the Anthropic-safe _ast_grep MCP server",
    async ({ env }) => {
  ...
  it("#given OMO_AST_GREP_PROJECT_CWD #when registered #then passes the explicit project cwd through", async () => {
    ...
    expect(pi.mcpServers[0]?.config.env).toEqual({
      OMO_AST_GREP_PROJECT_CWD: "/explicit/project",
    });
  });
  ...
  it("#given the staged entry is absent #when registered #then skips cleanly and warns", async () => {
    ...
    const missingEntry = join(
      tmpdir(),
      `missing-omo-ast-grep-${process.pid}-${Date.now()}`,
      "cli.js",
    );
    const component = createAstGrepComponent({
      resolveEntry: () => missingEntry,
    });
