...
export function createAstGrepComponent(
  ...
  return {
    ...
    register(pi: SenpiExtensionAPI, ctx: ComponentContext): void {
      if (typeof pi.registerMcpServer !== "function") {
        ctx.logger.info(
          "omo-senpi ast-grep skipped: senpi ExtensionAPI does not expose registerMcpServer",
          {
...
function resolvePackagedAstGrepEntry(
  importerUrl: string = import.meta.url,
): string {
  const packagedEntry = fileURLToPath(
    new URL("../runtime/ast-grep-mcp/cli.js", importerUrl),
  );
  if (existsSync(packagedEntry)) return packagedEntry;

  const sourceTreeEntry = fileURLToPath(
    new URL("../../../plugin/runtime/ast-grep-mcp/cli.js", importerUrl),
  );
