...
export function resolveSenpiDaemonRuntime(
  ...
  if (!hasCliOverride || !hasVersionOverride) {
    ...
    return {
      cliPath: packagedRuntime.cliPath,
      version: validateDaemonVersion(packagedRuntime.version),
    };
  }
  ...
  if (!stat.isFile()) {
    throw new Error("OMO_LSP_DAEMON_CLI must point to a regular file");
  }
  return {
    cliPath: cliOverride,
    version: validateDaemonVersion(versionOverride),
  };
}

export function resolveSenpiPackagedDaemonRuntime(
  importerUrl: string = import.meta.url,
): SenpiDaemonRuntime {
  const cliPath = fileURLToPath(
    new URL("../runtime/lsp-daemon/dist/cli.js", importerUrl),
  );
  const packageJsonPath = fileURLToPath(
    new URL("../runtime/lsp-daemon/dist/package.json", importerUrl),
  );
  if (!existsSync(cliPath)) {
    throw new Error(`Senpi packaged LSP daemon CLI is missing: ${cliPath}`);
  }
  ...
  if (
    ...
    throw new Error(
      `Senpi packaged LSP daemon version is missing: ${packageJsonPath}`,
    );
  }
...
function validateDaemonVersion(version: string): string {
  if (!DAEMON_VERSION_PATTERN.test(version)) {
    throw new Error(
      "LSP daemon version must match [A-Za-z0-9][A-Za-z0-9._+-]{0,127}",
    );
  }
  return version;
}
