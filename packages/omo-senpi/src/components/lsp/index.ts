...
import {
  callPackagedDaemonTool,
  clearPackagedDaemonToolClientCache,
} from "./daemon-tool-client.js";
import {
...
const LSP_POST_EDIT_DIAGNOSTICS_ENABLED_FLAG =
...
export function createLspComponent(
  options: LspComponentOptions = {},
): OmoSenpiComponent {
  const postEditState = options.postEdit?.state ??
    createLspPostEditSessionState();
  const runPostEditDiagnostics = options.postEdit?.runDiagnostics ??
    runLspDiagnosticsForPostEdit;
  return {
    name: "lsp",
    register(pi, ctx) {
      registerLspFlags(pi);
      ...
      if (
        ctx.config.getFlag(LSP_POST_EDIT_DIAGNOSTICS_ENABLED_FLAG) !== false
      ) {
        pi.on(
          "tool_result",
          (event, eventCtx) =>
            handlePostEditDiagnosticsToolResult(
              event,
              eventCtx,
...
export async function handlePostEditDiagnosticsToolResult(
  ...
  const result = await appendPostEditDiagnostics(
    event,
    runDiagnostics,
    state.getOrCreate(sessionIdFromContext(ctx)),
  );
...
async function runLspDiagnosticsForPostEdit(
  filePath: string,
): Promise<PostEditDiagnosticsOutcome> {
  const result = await callPackagedDaemonTool("lsp_diagnostics", {
    filePath,
    severity: "error",
  });
...
function notConfiguredAvailability(
  details: unknown,
  ...
  return typeof extension === "string" && extension.length > 0
    ? { extension }
    : undefined;
}
...
function isWidgetContext(value: unknown): value is WidgetContext {
  return isRecord(value) &&
    (value["ui"] === undefined || isRecord(value["ui"]));
}
