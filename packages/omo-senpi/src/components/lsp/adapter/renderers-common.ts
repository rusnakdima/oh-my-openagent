...
export function locText(loc: Location | LocationLink): string {
  if ("targetUri" in loc) {
    return `${shorten(uriToPath(loc.targetUri), PATH_BUDGET)}:${
      loc.targetRange.start.line + 1
    }:${loc.targetRange.start.character}`;
  }
  return `${shorten(uriToPath(loc.uri), PATH_BUDGET)}:${
...
export function renderPositionCall(
  toolName: string,
  args: PositionArgs,
  theme: Theme,
): string {
  ...
  const loc = theme.fg(
    "accent",
    `${shorten(args.filePath, PATH_BUDGET)}:${args.line}:${args.character}`,
  );
  return head + loc;
}
