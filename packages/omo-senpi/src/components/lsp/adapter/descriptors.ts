...
const PositionParams = Type.Object({
  ...
  character: Type.Number({
    description: "0-based column of the symbol on that line",
  }),
});
...
export const lsp_prepare_rename = defineTool({
  ...
  description:
    "Check if rename is valid at a given position. Use BEFORE lsp_rename.",
...
export const lsp_rename = defineTool({
  ...
  label: "LSP Rename",
  description:
    "Rename symbol across the entire workspace. APPLIES changes to all files.",
...
async function descriptorOnlyExecute(): Promise<never> {
  throw new Error(
    "Senpi LSP descriptors must be wrapped with the packaged daemon runtime before execution",
  );
}
