...
export function renderPrepareRenameCall(
  args: PositionArgs,
  theme: Theme,
): Text {
...
export function renderPrepareRenameResult(
  ...
  if (details?.error) {
    return new Text(
      theme.fg("warning", details.error.split("\n")[0] ?? "error"),
      0,
      0,
    );
  }
...
export function renderRenameResult(
  ...
  if (!details?.apply) {
    return new Text(
      theme.fg("dim", result.content[0]?.text ?? "No edit applied"),
      0,
      0,
    );
  }
  ...
  const summary = theme.fg(
    "success",
    `Applied ${apply.totalEdits} edit${apply.totalEdits === 1 ? "" : "s"}`,
  ) +
    theme.fg(
      "muted",
      ` to ${apply.filesModified.length} file${
        apply.filesModified.length === 1 ? "" : "s"
      }`,
    );
  ...
  if (!options.expanded) {
    ...
    if (apply.filesModified.length > COLLAPSED_HEAD) {
      lines.push(
        theme.fg(
          "dim",
          `  … ${apply.filesModified.length - COLLAPSED_HEAD} more`,
        ),
      );
    }
    return new Text(lines.join("\n"), 0, 0);
  ...
  if (apply.filesModified.length > EXPANDED_HEAD) {
    lines.push(
      theme.fg("dim", `… ${apply.filesModified.length - EXPANDED_HEAD} more`),
    );
  }
  return new Text(lines.join("\n"), 0, 0);
}
