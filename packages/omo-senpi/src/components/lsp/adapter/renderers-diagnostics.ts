...
function diagSeverityKey(
  severity?: number,
): "error" | "warning" | "muted" | "dim" {
  switch (severity) {
    case 1:
      return "error";
    case 2:
...
export function renderDiagnosticsResult(
  ...
  if (counts.warning > 0) {
    badges.push(theme.fg("warning", `W:${counts.warning}`));
  }
  ...
  if (!options.expanded) {
    ...
    if (uniqueDiagnosticFiles.length > COLLAPSED_HEAD) {
      lines.push(
        theme.fg(
          "dim",
          `  … ${uniqueDiagnosticFiles.length - COLLAPSED_HEAD} more files`,
        ),
      );
    }
    return new Text(lines.join("\n"), 0, 0);
  ...
  if (total > EXPANDED_HEAD) {
    lines.push(
      theme.fg("dim", `… ${total - EXPANDED_HEAD} more diagnostics not shown`),
    );
  }
  return new Text(lines.join("\n"), 0, 0);
}
