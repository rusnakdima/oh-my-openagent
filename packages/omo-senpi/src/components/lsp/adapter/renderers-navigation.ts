...
export function renderGotoDefinitionCall(
  args: PositionArgs,
  theme: Theme,
): Text {
...
export function renderFindReferencesCall(
  args: PositionArgs & { includeDeclaration?: boolean },
  theme: Theme,
): Text {
...
export function renderFindReferencesResult(
  ...
  if (!options.expanded) {
    const head = unique(details.references, (r) => r.uri).slice(
      0,
      COLLAPSED_HEAD,
    );
    ...
    for (const ref of head) {
      lines.push(
        theme.fg("muted", `  ${shorten(uriToPath(ref.uri), PATH_BUDGET)}`),
      );
    }
    if (fileCount > COLLAPSED_HEAD) {
      lines.push(
        theme.fg("dim", `  … ${fileCount - COLLAPSED_HEAD} more files`),
      );
    }
