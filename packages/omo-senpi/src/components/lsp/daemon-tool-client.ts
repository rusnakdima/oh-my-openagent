...
interface DaemonClientModule {
  readonly callToolViaDaemon: (
    ...
    options?: {
      readonly context?: LspRequestContext;
...
export async function callPackagedDaemonTool(
  ...
  return client.callToolViaDaemon(toDaemonToolName(name), args, {
    context,
...
async function loadPackagedDaemonClient(
  importerUrl: string,
): Promise<DaemonClientModule> {
...
async function loadPackagedDaemonClientOnce(
  clientPath: string,
): Promise<DaemonClientModule> {
  const loaded: unknown = await import(pathToFileURL(clientPath).href);
  if (!isDaemonClientModule(loaded)) {
    throw new Error(
      `Packaged LSP daemon runtime is missing public client exports: ${clientPath}`,
    );
  }
  return loaded;
}
