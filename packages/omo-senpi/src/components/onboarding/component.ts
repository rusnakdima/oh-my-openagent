...
} from "../telemetry/product-identity";
    ...
    register(pi: SenpiExtensionAPI, _ctx: ComponentContext): void {
      ...
      pi.on("session_start", (rawPayload: unknown, rawEventCtx: unknown) => {
        ...
        pi.sendMessage({
          ...
          content:
            `Read the onboarding skill at ${skillsRoot}/onboarding/SKILL.md with the read tool and follow it. Greet the user first.`,
          display: false,
        }, { triggerTurn: true, deliverAs: "followUp" });
        if (force) onboardConsumed = true;
        pi.appendEntry?.("omo-onboarding:started", {
