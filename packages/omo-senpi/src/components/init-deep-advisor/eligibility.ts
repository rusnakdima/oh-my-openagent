...
export function computeEligibility(
  ...
  if (snapshot.kind === "missing") {
    ...
    if (
      !shouldProposeInit(
        coverage?.missingRatio ?? null,
        existsSync(join(root, "AGENTS.md")),
      )
    ) {
      return null;
    }
    if (currentHead === lastProposedHead || Date.now() < cooldownUntil) {
      return null;
    }
    return {
  ...
  if (
    !shouldProposeRefresh(drift, currentHead, lastProposedHead, cooldownUntil)
  ) return null;
  if (drift.kind === "stale") {
...
function driftEligibility(
  drift: Extract<DriftMetrics, { kind: "valid" }>,
): EligibilityResult {
  const data = {
