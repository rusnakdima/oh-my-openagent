...
import { createInitDeepAdvisorComponent, runAdvisor } from "./component";
import {
  advisorStateDir,
  componentContext,
  createHarness,
...
describe("createInitDeepAdvisorComponent", () => {
  for (const choice of [...choices, undefined]) {
    test(`#given an eligible repo #when ${choice ?? "the dialog times out"} #then the correct response state is applied`, async () => {
      ...
      // then
      expect(select).toHaveBeenCalledWith("Init-deep", choices, {
        timeout: 60_000,
      });
      if (choice === "Run now") {
        ...
        expect(advisorState.readCooldownUntil(stateDir, repo)).toBeGreaterThan(
          Date.now(),
        );
  ...
  test("#given a non-startup session #when session_start fires #then no selector opens", () => {
    ...
    const handler =
      pi.handlers.find((entry) => entry.event === "session_start")!.handler;
  ...
  test("#given a non-UI mode #when startup runs #then proposal state and UI remain untouched", async () => {
    // given
    const root = makeCoverageRepo();
    const { pi, select, eventCtx } = createHarness(
      root,
      "Skip this time",
      false,
    );
    ...
    // then
    expect(select).not.toHaveBeenCalled();
    expect(advisorState.readLastProposedHead(advisorStateDir(), repo))
      .toBeNull();
  ...
  test("#given every eligibility trigger #when proposed data is built #then each discriminated payload is exact", () => {
    ...
    const cases: Array<
      { eligibility: EligibilityResult; expected: OmoInitDeepProposedData }
    > = [
      {
        eligibility: {
          trigger: "coverage-gap",
          coverage: { missingRatio: 0.75, candidateDirs: 4, coveredDirs: 1 },
        },
        expected: {
          repo: "repo",
          trigger: "coverage-gap",
      ...
      ...(["commit-and-touch", "loc-churn", "snapshot-age"] as const).map(
        (
          ...
          expected: OmoInitDeepProposedData;
        } => ({
    ...
    for (const entry of cases) {
      expect(buildProposedData("repo", entry.eligibility, "local")).toEqual(
        entry.expected,
      );
    }
  });
