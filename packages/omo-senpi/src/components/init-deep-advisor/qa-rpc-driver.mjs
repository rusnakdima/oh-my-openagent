...
const FROZEN_PLUGIN =
  "/Volumes/mengmotaStorage/local-workspaces/omo-wt/feat-senpi-onboarding-init-deep-advisor/packages/omo-senpi/plugin/extensions/omo.js";
const mode = process.argv[2];
if (mode !== "e" && mode !== "f") {
  throw new Error("usage: qa-rpc-driver.mjs <e|f>");
}
if (process.env.SENPI_BIN !== FROZEN_PLUGIN) {
  throw new Error(`SENPI_BIN must be the frozen plugin path: ${FROZEN_PLUGIN}`);
}
if (!existsSync(FROZEN_PLUGIN)) {
  throw new Error(`frozen plugin does not exist: ${FROZEN_PLUGIN}`);
}
...
async function scenarioE() {
  ...
  try {
    ...
    if (
      proposal.lastProposedHead !== gitText(fixture.repo, ["rev-parse", "HEAD"])
    ) {
    ...
    return {
      ...
      evidence: {
        ...
        settledTurnIndex: run.settledTurnIndex,
        session: basename(run.session),
      },
    };
  } finally {
...
async function scenarioF() {
  ...
  try {
    ...
    for (let index = 1; index <= 40; index += 1) {
      ...
      writeFileSync(
        file,
        `${readFileSync(file, "utf8")}export const drift${index} = ${index}\n`,
      );
    ...
    if (proposedHead !== gitText(fixture.repo, ["rev-parse", "HEAD"])) {
      throw new Error("launch 1 did not persist proposed HEAD");
    }

    const second = await launchRpc(fixture, false);
    if (second.transcript.includes("omo-init-deep-advisor:run")) {
      throw new Error("same-HEAD launch injected init-deep");
    }

    writeFileSync(
      `${fixture.repo}/advance.ts`,
      "export const advance = true\n",
    );
    gitText(fixture.repo, ["add", "-A"]);
    commit(fixture.repo, "advance");
    const newHead = gitText(fixture.repo, ["rev-parse", "HEAD"]);
    if (newHead === proposedHead) {
      throw new Error("fixture HEAD did not advance");
    }
    writeFutureCooldown(fixture);
    const third = await launchRpc(fixture, false);
    if (third.transcript.includes("omo-init-deep-advisor:run")) {
      throw new Error("cooldown launch injected init-deep");
    }
    ...
    return {
      ...
      checks: {
        ...
        allSettledCorrelated: [first, second, third].every((run) =>
          run.settledTurnIndex >= run.turnIndex
        ),
        settingsOnlyFrozenPlugin: true,
        sandboxRemovedInFinally: true,
      },
      evidence: {
        ...
        proposedHead,
        newHead,
        turns: [first, second, third].map((run) => ({
          turnIndex: run.turnIndex,
          settledTurnIndex: run.settledTurnIndex,
          session: basename(run.session),
        })),
