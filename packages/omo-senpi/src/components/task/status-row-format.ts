...
const SUSPENDED_RESIDENCIES: ReadonlySet<ResidencyState> = new Set([
  "persisted_only",
  "rpc_detached",
]);
...
function statusLabel(record: TaskRecord): string {
  return isSuspended(record)
    ? "suspended"
    : normalizeRendererText(record.status);
}
...
export function formatTaskRow(record: TaskRecord): string {
  const identity = taskIdentityLabel({
  ...
  if (identity !== normalizeRendererText(record.task_id)) {
    parts.push(`(${normalizeRendererText(record.task_id)})`);
  }
  parts.push(recordStatusTarget(record));
  parts.push(
    `mode:${normalizeRendererText(record.execution_mode)}`,
...
export function buildWidgetRows(
  ...
  const shown = selected.slice(0, MAX_WIDGET_ROWS).map((record) =>
    formatCompactTaskRow(record, WIDGET_LINE_MAX, true)
  );
...
function liveStatsTokens(stats: TaskRunStats | undefined): string[] {
  ...
  if (stats.tokens_per_second !== undefined) {
    tokens.push(`${stats.tokens_per_second} tok/s`);
  }
  return tokens;
}
...
function formatLiveBackgroundRow(
  ...
  const frame = SPINNER_FRAMES[
    Math.floor(now / LIVE_STATUS_REFRESH_MS) % SPINNER_FRAMES.length
  ] ?? SPINNER_FRAMES[0];
  ...
  const minimumPartsWidth = rendererVisibleWidth(
    `${frame} ${excerptRendererText(fullIdentity, LIVE_IDENTITY_MIN)} · ${
      excerptRendererText(fullTarget, LIVE_TARGET_MIN)
    } · ${excerptRendererText(fullActivity, LIVE_ACTIVITY_MIN)} · ${elapsed}`,
  );
  ...
  const activityWidth = Math.min(
    rendererVisibleWidth(fullActivity),
    LIVE_ACTIVITY_MIN + remainingWidth,
  );
  remainingWidth -= Math.max(0, activityWidth - LIVE_ACTIVITY_MIN);
  const targetWidth = Math.min(
    rendererVisibleWidth(fullTarget),
    LIVE_TARGET_MIN + remainingWidth,
  );
  remainingWidth -= Math.max(0, targetWidth - LIVE_TARGET_MIN);
  const identityWidth = Math.min(
    LIVE_IDENTITY_MAX,
    LIVE_IDENTITY_MIN + remainingWidth,
  );
  const context = [
...
export function backgroundWidgetRows(
  ...
  const shown = selected.slice(0, MAX_WIDGET_ROWS).map((record) =>
    record.status === "completed"
      ...
      : formatLiveBackgroundRow(
        record,
        activity.get(record.task_id) ?? "running",
        now,
        boundedWidth,
        liveStats?.(record.task_id),
      )
  );
  const overflow = selected.length - MAX_WIDGET_ROWS;
  if (overflow > 0) shown.push(`+${overflow} more`);
...
function formatCompactTaskRow(
  ...
  const identityWidth = Math.max(
    0,
    maxWidth - rendererVisibleWidth(context) - 1,
  );
