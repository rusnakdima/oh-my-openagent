...
export function wireHarness(sessionId?: string, options: HarnessOptions = {}) {
  ...
  const childListeners = new Map<
    string,
    Set<(event: ManagedChildEvent) => void>
  >();
  ...
  const engine = {
    ...
    lifecycle: {
      ...
      cleanupExpiredRecords: async () => {
        ...
        return {
          deleted: options.cleanupDeleted ?? [],
          retained: [] as readonly string[],
        };
      },
    ...
    manager: {
      ...
      list: (
        ...
        Object.values(records)
          .filter((record) =>
            scope.scope === "all" ||
            record.parent_session_id === scope.session_id
          )
          .map((record) => ({ record })),
    ...
    notifier: {
      reconcileUnnotifiedNotifications: (
        input: { sessionId: string; parentState: unknown },
      ) => {
    ...
    } as unknown as TaskEngine["memberLiveness"],
