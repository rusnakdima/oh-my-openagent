...
import {
  createDagReloadSource,
  type DagReloadRunManager,
} from "./dag-reload-source";

const SESSION_ID = "parent-session";

function managerOf(
  runs: ReadonlyArray<
    { readonly runId: string; readonly status: string; readonly name?: string }
  >,
  ...
  return {
    list: (parentSessionId) =>
      parentSessionId === SESSION_ID
        ? runs.map(({ runId, status }) => ({ runId, status }))
        : [],
    snapshot: (runId) => {
      if (runId === options.snapshotThrowsFor) throw new Error("pruned");
      return {
        name: runs.find((entry) => entry.runId === runId)?.name ?? runId,
      };
    },
  };
}
