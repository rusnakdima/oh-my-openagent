...
function harness(options: { readonly withCoordinator?: boolean } = {}) {
  ...
  const created: Array<
    { readonly input: LeadPollerFactoryInput; readonly poller: FakePoller }
  > = [];
  ...
  const lifecycle = createLeadPollerLifecycle({
    ...
    coordinator: options.withCoordinator === false ? undefined : {
  ...
  return {
    ...
    get intervalDisposals() {
      return intervalDisposals;
    },
    setSessionId: (value: string | undefined) => {
      sessionId = value;
    },
    setState: (value: ReturnType<TaskRuntimeContext["parentState"]>) => {
      state = value;
    },
    setSessionFile: (value: string | undefined) => {
      sessionFile = value;
    },
    setTeams: (value: typeof teams) => {
      teams = value;
    },
  };
}
...
describe("lead poller lifecycle", () => {
  test("#given no shared coordinator #when team mail injects #then fallback is hidden custom steer", async () => {
    ...
    h.created[0]?.input.coordinator.enqueue({
      ...
      source: "team-message",
      content:
        '<peer_message from="worker" to="lead" messageId="m1">ready</peer_message>',
    } as LeadInjection);
  ...
  test("#given owned and foreign teams #when ticks repeat #then only one owned poller is created and reused", async () => {
    ...
    // then
    expect(h.created.map((entry) => entry.poller.teamRunId)).toEqual([
      "run-owned",
    ]);
    expect(h.created[0]?.poller.polls).toBe(2);
    expect(h.mapReads).toEqual(["/tmp/runtime/run-owned"]);
    expect(h.created[0]?.input.eventTaskId(messageFrom("alpha"))).toBe(
      "st_alpha",
    );
  });
  ...
  test("#given an owned team whose members are suspended #when the lead session shuts down and resumes #then the same poller keeps the team", async () => {
    ...
    expect(h.created[0]?.poller.polls).toBe(2);
    expect(h.lifecycle.resolveLeadPoller("run-owned")).toBe(
      h.created[0]?.poller,
    );
  });
  ...
  test("#given coordinator delivery states #when an injection enqueues #then scheduling follows streaming idle and transition rules", async () => {
    ...
    // then
    expect(h.injected.map((entry) => entry.key)).toEqual([
      "stream",
      "idle",
      "transition",
    ]);
