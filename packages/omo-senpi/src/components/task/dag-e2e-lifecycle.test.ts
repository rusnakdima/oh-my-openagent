  ...
  start(spec: ManagedStartSpec): Promise<ManagedChildHandle> {
    ...
    const handle: ManagedChildHandle = {
      ...
      abort: () => {
        ...
        return this.abortError === undefined
          ? Promise.resolve()
...
async function runtimeFixture(
  ...
  const project = options.project ??
    fs.mkdtempSync(join(tmpdir(), "omo-dag-lifecycle-"));
  ...
  const createStore = spyOn(dagEngine, "createDagFileStore").mockImplementation(
    (config, storeOptions = {}) =>
  ...
  const attached = options.attach === false
    ? Promise.resolve()
    : runtime.attach();
  ...
  return {
    ...
    async start(key, nodes = [{ id: "work" }]) {
      ...
      if (started.details.kind !== "started") {
        throw new Error(`expected dag start, received ${started.details.kind}`);
      }
...
function events(
  ...
  runId: DagRunId,
): readonly DagRunEvent[] {
  return fixture.runtime.manager.history({
    runId,
    parentSessionId: sessionId,
    limit: 256,
  }).events;
...
function expectOk<T>(value: unknown): T {
  ...
  if (!envelope.ok) {
    throw new Error(
      `expected ok RPC envelope, received ${JSON.stringify(envelope)}`,
    );
  }
...
class ReferenceDagViewer {
  ...
  readonly gaps: Array<
    { readonly expected: number; readonly received: number }
  > = [];
  ...
  async catchUp(): Promise<DagRpcSubscribeHandshake> {
    ...
    while (page.hasMore) {
      page = expectOk<DagRpcHistoryPage>(
        await this.rpc.call("omo.dag.history", {
          runId: this.runId,
    ...
    for (
      const event of this.#buffer.splice(0).sort((left, right) =>
        left.seq - right.seq
      )
    ) this.apply(event);
  ...
  private apply(event: DagRpcEvent): void {
    ...
    if (event.seq !== expected) {
      this.gaps.push({ expected, received: event.seq });
    }
...
describe("assembled DAG lifecycle end to end", () => {
  test("#given a live run #when its session compacts #then events continue without a pause and the TUI refreshes after the post-compact event", async () => {
    ...
    expect(persisted.some((event) => event.type === "dag.run.paused")).toBe(
      false,
    );
  ...
  test("#given a pending adapter run #when shutdown persists a pause and a fresh adapter attaches #then it claims, resumes, and completes the run", async () => {
    ...
    const project = fs.mkdtempSync(
      join(tmpdir(), "omo-dag-lifecycle-restart-"),
    );
    ...
    const paused = store.readCheckpoint<
      DagRunRecordV1 & { readonly previousLeaseHolderPid?: number }
    >(runId);
    expect(paused?.status).toBe("paused");
    expect(store.readEvents(runId, 0, { limit: 256 }).events.at(-1)).toEqual(
    ...
    expect(
      store.readEvents(runId, 0, { limit: 256 }).events.map((event) =>
        event.type
      ),
    ).toContain("dag.run.resumed");
  ...
  test("#given an eval-cell waiter that is abandoned #when a later cell re-attaches and calls done #then the shipped handle returns the result", async () => {
    ...
    // then
    expect(result.status).toBe("completed");
    expect(result.nodes.work).toEqual(
      expect.objectContaining({
        state: "completed",
    ...
    expect(
      events(fixture, runId).filter((event) =>
        event.type === "dag.run.cancelled"
      ),
    ).toHaveLength(0);
  });
  ...
  test("#given a shipped handle for a live child #when it cancels the run #then no abort rejection escapes and a later handle settles", async () => {
    ...
    const fixture = await runtimeFixture({
      runner: new ControlledRunner(
        new DOMException("intentional child abort", "AbortError"),
      ),
    });
    ...
    try {
      ...
      const after = await fixture.runtime.manager.attach(runId, sessionId)
        .done();
      ...
      expect(after.nodes.live).toEqual(
        expect.objectContaining({
          state: "cancelled",
          reason: "assembled cancel",
        }),
      );
      expect(after.nodes.after).toEqual(
        expect.objectContaining({
          state: "cancelled",
          reason: "assembled cancel",
        }),
  ...
  test("#given two terminal DAG notifications in one streaming flush window #when the window flushes #then exactly one batched steer is delivered", async () => {
    ...
    const deliveries: Array<
      { readonly content: string; readonly deliverAs: string }
    > = [];
  ...
  test("#given bursty adapter mutations with an identical final state #when the snapshot window flushes #then omo.dag.updated emits that state once", async () => {
    ...
    const firstWindow = fixture.rpc.events.filter((event) =>
      event.name === "omo.dag.updated"
    );
    fixture.runtime.sync();
    fixture.runtime.sync();
    fixture.bridgeTimers.flush(50);
    const secondWindow = fixture.rpc.events.filter((event) =>
      event.name === "omo.dag.updated"
    );
    ...
    expect(
      (firstWindow[0]?.data as { runs: readonly { status: string }[] }).runs[0]
        ?.status,
    ).toBe("running");
  });
});
...
describe("assembled DAG retry lifecycle end to end", () => {
  test(
    "#given a diamond run whose middle node fails #when the dag TOOL retries it #then wait settles completed and the untouched nodes keep their original children",
    async () => {
      // given
      const fixture = await runtimeFixture();
      ...
      const byNode = (index: number): string =>
        String(fixture.runner.children[index]?.spec.taskId);
      ...
      const waited = await runDagTool(toolDeps(fixture.runtime), {
        action: "wait",
        run_id: runId,
      });
      ...
      if (waited.details.kind !== "waited") {
        throw new Error(`expected wait, received ${waited.details.kind}`);
      }
      expect(waited.details.result.status).toBe("completed");
      expect(waited.details.result.nodes.left).toEqual(
        expect.objectContaining({ state: "completed", output: "left retried" }),
      );
      expect(waited.details.result.nodes.join).toEqual(
        expect.objectContaining({ state: "completed", output: "join output" }),
      );
      const record = fixture.runtime.manager.record(runId, sessionId);
      expect(record.nodes.find((node) => String(node.id) === "root")?.taskId)
        .toBe(rootTaskId);
      expect(record.nodes.find((node) => String(node.id) === "right")?.taskId)
        .toBe(rightTaskId);
      expect(
        events(fixture, runId).some((event) =>
