...
export function dagUpdatedPayload(
  ...
  return {
    ...
    runs: (truncatedRuns === 0 ? runs : runs.slice(0, DAG_MAX_RUN_SNAPSHOTS))
      .map(runSnapshotPayload),
  ...
  return {
    ...
    nodes: run.nodes.map((node) => ({
      ...
      ...(node.error === undefined ? {} : {
        last_error: { code: node.error.code, message: node.error.message },
      }),
    })),
    edges: run.edges.map((edge) => ({ from: edge.from, to: edge.to })),
    waves: run.waves.map((wave) => ({
      index: wave.index,
      node_ids: wave.nodeIds,
    })),
