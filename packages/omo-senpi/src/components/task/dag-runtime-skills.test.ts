...
class CapturingRunner implements ManagedRunner {
  readonly starts: Array<
    { readonly spec: ManagedStartSpec; readonly settle: () => void }
  > = [];
  ...
  start(spec: ManagedStartSpec): Promise<ManagedChildHandle> {
    ...
    this.starts.push({
      spec,
      settle: () =>
        outcome.resolve({ status: "completed", finalResponse: "done" }),
    });
...
describe("createDagRuntime skill wiring", () => {
  test("#given a shared task skill loader #when a skilled node starts #then dispatch receives the materialized prompt", async () => {
    ...
    engine.runtime.captureFrom({
      sessionManager: { getSessionId: () => "session-shared-skill" },
    });
    ...
    expect(snapshot.diagnostics).not.toContainEqual(
      expect.objectContaining({ kind: "missing_skill" }),
    );
