...
import { type CommandManager, registerTaskCommands } from "./commands";
...
function fakeManager(
...
): CommandManager & { cancelled: string[] } {
  ...
  return {
    ...
    list: (scope) => {
      ...
      return listed(
        records.filter((rec) =>
          rec.parent_session_id === scope.session_id ||
          rec.root_session_id === scope.session_id
        ),
      );
    },
    cancelTask: (idOrName) => {
...
describe("registerTaskCommands", () => {
  ...
  it("#given a task with a description #when /task-kill lists options #then the human label leads and the id trails", async () => {
    ...
    // then
    expect(ui.selectCalls[0]?.options[0]).toBe(
      "Audit the waiting line (st_kill) running",
    );
  });
