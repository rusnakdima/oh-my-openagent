...
async function givenActiveOwnedTeam(
  h: ReturnType<typeof harness>,
): Promise<void> {
  mkdirSync(resolveTeamRuntimeDirs(h.stateDir, TEAM_RUN_ID).runtimeDir, {
    recursive: true,
  });
  await saveRuntimeState(
    activeTeamRuntime(),
    toTeamCoreConfig(h.settings, teamStorageBaseDir(h.stateDir)),
  );
}
