...
import { type ParentTransition, TaskRuntimeContext } from "./runtime-context";
import {
  createSessionTransitionBridge,
  type FlushingNotifier,
} from "./session-transition-bridge";
...
function capturingNotifier(): ParentNotifier & {
  readonly enqueued: ParentNotifierMessage[];
} {
...
function recordingRuntime(): {
  runtime: { setTransition(t: ParentTransition): void };
  transitions: ParentTransition[];
} {
  const transitions: ParentTransition[] = [];
  return {
    runtime: { setTransition: (transition) => transitions.push(transition) },
    transitions,
  };
}
...
describe("createSessionTransitionBridge transition marking", () => {
  it("#given a session about to switch #when onBeforeSwitch fires #then the runtime enters the switching transition", () => {
    ...
    const bridge = createSessionTransitionBridge({
      runtime,
      notifier: stubFlusher(),
    });
  ...
  it("#given no pending transition #when a fresh session starts #then no flush is attempted", () => {
    ...
    const notifier: FlushingNotifier = {
      flushBuffered: (input) => (flushes.push(input), { kind: "empty" }),
    };
    const bridge = createSessionTransitionBridge({
...
describe("createSessionTransitionBridge buffered-completion round trip", () => {
  ...
  it("#given a completion buffered before a switch #when a different session replaces it #then the completion is dropped, not delivered", () => {
    ...
    expect(notifier.enqueued).toHaveLength(0);
    expect(store.events.some((event) => event.type === "notification_dropped"))
      .toBe(true);
