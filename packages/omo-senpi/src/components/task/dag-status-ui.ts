...
import { type DagStatusRunSnapshot, runRows } from "./dag-status-row-format";
...
export function createDagStatusUi(deps: DagStatusUiDeps): DagStatusUi {
  ...
  function render(): void {
    ...
    const rows = runs.flatMap((run) =>
      runRows(run, liveActivity.get(run.runId))
    );
    pruneActivity(runs);
    if (rows.length === 0) {
      clearLiveRefresh();
    ...
    if (runs.some((run) => !TERMINAL_RUN_STATUSES.has(run.status))) {
      scheduleLiveRefresh();
  ...
  function readSnapshot(
    runId: string,
    sessionId: string,
  ): DagStatusRunSnapshot | undefined {
    try {
