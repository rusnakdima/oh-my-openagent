...
export interface StatusUiManager {
  ...
  subscribeChild?(
    taskId: string,
    listener: (event: ManagedChildEvent) => void,
  ): () => void;
...
export function createTaskStatusUi(deps: TaskStatusUiDeps): TaskStatusUi {
  ...
  function renderCachedRecords(): void {
    ...
    const background = deps.manager.wasBackground === undefined
      ...
      : records.filter((record) =>
        deps.manager.wasBackground?.(record.task_id) === true
      );
    ...
    if (
      deps.manager.wasBackground !== undefined &&
      background.some((record) => !isTerminal(record.status))
    ) {
      scheduleLiveRefresh();
  ...
  function syncChildSubscriptions(records: readonly TaskRecord[]): void {
    if (
      deps.manager.subscribeChild === undefined ||
      deps.manager.wasBackground === undefined
    ) return;
    ...
    for (const taskId of activeBackgroundIds) {
      ...
      subscriptions.set(
        taskId,
        deps.manager.subscribeChild(taskId, (event) => {
...
function activityFromEvent(event: ManagedChildEvent): string | undefined {
  if (
    event.type === "tool_execution_start" && typeof event.toolName === "string"
  ) {
    return excerptRendererText(
      formatToolActivity(event.toolName, event.args ?? event.input),
      32,
    );
  }
...
function scopedRecords(
  ...
  return manager.list({ scope: "parent-session", session_id: sessionId }).map((
    entry,
  ) => entry.record);
}
