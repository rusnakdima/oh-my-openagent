...
import {
  ...
  dagRunStartedEvent,
} from "../../../../senpi-task/src/dag/events";
...
function wire(
  target: Engine,
  sessionId: () => string | undefined = () => parentSessionId,
): Handlers {
...
function expectOk<T>(envelope: unknown): T {
  ...
  if (!typed.ok) {
    throw new Error(`expected ok envelope, received ${JSON.stringify(typed)}`);
  }
  return typed.value;
}
...
function expectError(
  ...
  if (typed.ok) {
    throw new Error(
      `expected error envelope, received ${JSON.stringify(typed)}`,
    );
  }
  return typed.error;
}
...
describe("dag rpc handlers", () => {
  describe("#given a session that owns dag runs", () => {
    it(
      "#when omo.dag.list is called #then it returns session-scoped summaries newest first",
      async () => {
        // given
        let clock = 1_000;
        const target = engine({ now: () => clock });
        const first = await startRun(target, {
          key: "alpha",
          name: "alpha run",
        });
        clock = 2_000;
        const second = await startRun(target, {
          key: "beta",
          name: "beta run",
        });
        clock = 3_000;
    ...
    it("#when omo.dag.list requests a limit above the maximum #then the limit clamps to 256", async () => {
      ...
      const value = expectOk<
        { readonly runs: readonly unknown[]; readonly limit: number }
      >(
    ...
    it("#when omo.dag.history pages a run #then it walks the ledger with an exclusive sinceSeq", async () => {
      ...
      // when
      const firstPage = expectOk<DagRpcHistoryPage>(
        await handlers.call("omo.dag.history", { runId, limit: 3 }),
      );
      const secondPage = expectOk<DagRpcHistoryPage>(
        await handlers.call("omo.dag.history", {
          runId,
          sinceSeq: firstPage.nextSinceSeq,
          limit: 3,
        }),
      );
    ...
    it("#when omo.dag.subscribe is called #then highWaterSeq is captured before the page is read", async () => {
      ...
      // when
      const handshake = expectOk<DagRpcSubscribeHandshake>(
        await racing.call("omo.dag.subscribe", { runId }),
      );

      // then
      expect(handshake.highWaterSeq).toBe(4);
      expect(handshake.page.events.map((event) => event.seq)).toEqual([
        1,
        2,
        3,
        4,
    ...
    it("#when history filters by lane and type #then the filters reach the ledger read", async () => {
      ...
      // when
      const boundary = expectOk<DagRpcHistoryPage>(
        await handlers.call("omo.dag.history", { runId, lane: "boundary" }),
      );
      const started = expectOk<DagRpcHistoryPage>(
        await handlers.call("omo.dag.history", {
          runId,
          types: ["dag.run.started"],
        }),
      );
  ...
  describe("#given a request the engine must reject", () => {
    it("#when a foreign session asks for a run #then every run-scoped handler answers run_not_owned", async () => {
      ...
      // when
      const snapshot = expectError(
        await handlers.call("omo.dag.snapshot", { runId }),
      );
      const history = expectError(
        await handlers.call("omo.dag.history", { runId }),
      );
      const subscribe = expectError(
        await handlers.call("omo.dag.subscribe", { runId }),
      );
      const listed = expectOk<{ readonly runs: readonly unknown[] }>(
        await handlers.call("omo.dag.list", {}),
      );
    ...
    it("#when the run id is unknown #then the envelope carries run_not_found instead of throwing", async () => {
      ...
      // when
      const snapshot = expectError(
        await handlers.call("omo.dag.snapshot", { runId: "run-missing" }),
      );
      const history = expectError(
        await handlers.call("omo.dag.history", { runId: "run-missing" }),
      );
      const subscribe = expectError(
        await handlers.call("omo.dag.subscribe", { runId: "run-missing" }),
      );
    ...
    it("#when params are malformed #then the envelope carries invalid_arguments", async () => {
      ...
      // when
      const missingRunId = expectError(
        await handlers.call("omo.dag.snapshot", {}),
      );
      const badSince = expectError(
        await handlers.call("omo.dag.history", { runId, sinceSeq: -1 }),
      );
      const badLimit = expectError(
        await handlers.call("omo.dag.list", { limit: 0 }),
      );
      const badLane = expectError(
        await handlers.call("omo.dag.history", { runId, lane: "sideways" }),
      );
      const badStatus = expectError(
        await handlers.call("omo.dag.list", { statuses: ["exploded"] }),
      );
      const badPayload = expectError(
        await handlers.call("omo.dag.subscribe", "not-an-object"),
      );
    ...
    it("#when the ledger read fails #then the envelope carries history_unavailable", async () => {
      ...
      // then
      expect([history.code, subscribe.code]).toEqual([
        "history_unavailable",
        "history_unavailable",
      ]);
