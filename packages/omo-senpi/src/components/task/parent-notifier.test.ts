...
type SentUserMessage = {
  ...
  readonly options: { deliverAs?: "steer" | "followUp" } | undefined;
};

function fakePi(): SenpiExtensionAPI & {
  readonly sent: SentMessage[];
  readonly userSent: SentUserMessage[];
} {
...
function coordinatorWithManualFlush(
  delivered: Delivered[],
): { coordinator: IdleInjectionCoordinator; runFlush: () => void } {
...
describe("createParentNotifier batched injection delivery", () => {
  ...
  test("#given no coordinator is wired #when a completion enqueues #then it falls back to a direct steer on the renderer channel", () => {
    ...
    expect(pi.sent[0]?.options).toMatchObject({
      triggerTurn: true,
      deliverAs: "steer",
    });
