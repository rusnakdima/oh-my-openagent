...
const bridgeSource = fs.readFileSync(
  join(import.meta.dir, "dag-rpc-bridge.ts"),
  "utf8",
);
const handlerSource = fs.readFileSync(
  join(import.meta.dir, "dag-rpc-handlers.ts"),
  "utf8",
);
const eventTypesSource = fs.readFileSync(
  join(repoRoot, "packages", "senpi-task", "src", "dag", "types.ts"),
  "utf8",
);

function extract(pattern: RegExp): readonly string[] {
  return [
    ...new Set([...doc.matchAll(pattern)].map((match) => match[1] as string)),
  ];
}
    ...
    describe("#when checked against the shipped bridge and handler sources", () => {
      it("#then each name exists as a string literal in the source", () => {
        ...
        for (const name of wireNames) {
          const shipped = bridgeSource.includes(`"${name}"`) ||
            handlerSource.includes(`"${name}"`);
  ...
  describe("#given the documented overflow payload fields", () => {
    describe("#when compared with the shipped event union", () => {
      it("#then the field names match exactly", () => {
        ...
        const sourceFields = [...sourceBlock.matchAll(/readonly ([A-Za-z]+):/g)]
          .map((match) => match[1]);
        const docFields = [...docBlock.matchAll(/`([A-Za-z]+)`/g)].map((
          match,
        ) => match[1]);
