...
export function registerDagCommands(
  ...
  manager: DagCommandManager,
): void {
  pi.registerCommand("dag", {
    description: "List dag runs, or show one run's wave tree.",
    handler: (args: string, ctx: DagCommandContext) =>
      runDagCommand(manager, args, ctx),
  });
}

async function runDagCommand(
  manager: DagCommandManager,
  args: string,
  ctx: DagCommandContext,
): Promise<void> {
  const ui = ctx.ui;
...
function summaryRow(summary: DagCommandRunSummary): string {
  ...
  const tokens = [
    `${
      normalizeRendererText(counts.completed.toString())
    }/${counts.total} done`,
  ];
  if (counts.running > 0) tokens.push(`${counts.running} running`);
  if (counts.failed > 0) tokens.push(`${counts.failed} failed`);
  return `${normalizeRendererText(summary.name)} (${
    normalizeRendererText(summary.runId)
  }) ${normalizeRendererText(summary.status)} ${tokens.join(", ")}`;
}
...
function renderDetail(
  ...
  if (snapshot === undefined) {
    ui.notify(
      `No dag run "${normalizeRendererText(runId)}" in this session.`,
      "warning",
    );
    return;
  }
...
function readSnapshot(
  ...
  runId: string,
  sessionId: string,
): DagCommandRunSnapshot | undefined {
  try {
...
function detailText(
  snapshot: DagCommandRunSnapshot,
  manager: DagCommandManager,
): string[] {
  ...
  if (snapshot.criticalPath.length > 0) {
    lines.push(
      `critical path: ${
        snapshot.criticalPath.map((id) => normalizeRendererText(id)).join(
          " -> ",
        )
      }`,
    );
  ...
  for (const bottleneck of snapshot.bottlenecks) {
    lines.push(
      `bottleneck: ${
        normalizeRendererText(bottleneck.nodeId)
      } blocks ${bottleneck.blockedCount}`,
    );
  }
  return lines;
}
...
function summaryHeader(snapshot: DagCommandRunSnapshot): string {
  ...
  return `${normalizeRendererText(snapshot.name)} (${
    normalizeRendererText(snapshot.runId)
  }) ${
    normalizeRendererText(snapshot.status)
  } ${snapshot.nodes.length} nodes, ${snapshot.waves.length} waves${amended}`;
}
...
function dependentsByNode(
  snapshot: DagCommandRunSnapshot,
): Map<string, string[]> {
  const dependents = new Map<string, string[]>();
  for (const node of snapshot.nodes) {
    dependents.set(node.id, [...node.dependsOn]);
  }
...
function nodeRow(
  ...
  const parts = [
    normalizeRendererText(node.label ?? node.id),
    normalizeRendererText(node.state),
  ...
  if (dependsOn.length > 0) {
    parts.push(
      `after ${dependsOn.map((id) => normalizeRendererText(id)).join(", ")}`,
    );
  }
  if (isCritical) parts.push(CRITICAL_MARKER);
  if (node.error !== undefined) {
    parts.push(
      `error: ${normalizeRendererText(node.error.code)} ${
        normalizeRendererText(node.error.message)
      }`,
    );
  }
  return parts.join(" ");
...
function routeLabel(route: DagCommandRoute): string {
  if (route.kind === "agent") {
    return `agent:${normalizeRendererText(route.agent)}`;
  }
...
function resolvedModel(
  ...
  const record = node.taskId === undefined
    ? undefined
    : manager.taskRecord(node.taskId);
  const resolved = record?.resolved_model?.display ?? record?.model ??
    (node.route.kind === "agent" ? node.route.model : undefined);
