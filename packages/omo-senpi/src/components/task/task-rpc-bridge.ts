...
export function wireTaskRpcBridge(
  ...
  const recordsForSession = (sessionId: string) => {
    ...
    return {
      records: [
        ...
        ...(terminalSlots === 0
          ? []
          : all.filter((record) => !isLive(record)).slice(-terminalSlots)),
      ],
      truncatedTasks,
    };
  };
  ...
  const emit = (
    selection = activeSessionId === undefined
      ? undefined
      : recordsForSession(activeSessionId),
  ): void => {
    const sessionId = activeSessionId;
    if (disposed || sessionId === undefined || pi.rpc?.emit === undefined) {
      return;
    }
    ...
    const data = {
      ...
      ...(selection.truncatedTasks === 0
        ? {}
        : { truncated_tasks: selection.truncatedTasks }),
    };
  ...
  const syncSubscriptions = (records: readonly TaskRecord[]): void => {
    const liveIds = new Set(
      records
        .filter((record) =>
          isLive(record) && record.residency_state === "resident"
        )
    ...
    for (const record of records) {
      if (!liveIds.has(record.task_id) || subscriptions.has(record.task_id)) {
        continue;
      }
      ...
      const unsubscribe = engine.manager.subscribeChild(
        record.task_id,
        (event: ManagedChildEvent) => {
          if (!progress.accept(event)) return;
          liveProgress.set(
            record.task_id,
            liveProgressSnapshot(progress.details()),
          );
          emit();
        },
      );
  ...
  registerTaskHandlers(
    pi,
    engine,
    () => (disposed ? undefined : activeSessionId),
  );
...
function registerTaskHandlers(
  ...
  handle("omo.task.send", async (data) => {
    ...
    if (record === undefined || record.parent_session_id !== sessionId) {
      return notFound();
    }
