...
function rpcBus(
  options: { readonly dropLiveSeq: number; readonly duplicateLiveSeq: number },
) {
  ...
  const publish = (name: string, data: unknown): void => {
    ...
    const deliveries =
      name === "omo.dag.event" && event.seq === options.duplicateLiveSeq
        ? 2
        : 1;
...
class ReferenceDagViewer {
  ...
  async catchUp(): Promise<DagRpcSubscribeHandshake> {
    ...
    this.rpc.listen("omo.dag.activity", (value) => {
      ...
      if (activity.runId === this.runId) {
        this.activityByNode.set(activity.nodeId, activity);
      }
    });

    const handshake = expectOk<DagRpcSubscribeHandshake>(
      await this.rpc.call("omo.dag.subscribe", {
        runId: this.runId,
    ...
    while (this.liveBuffer.length > 0) {
      const buffered = this.liveBuffer.splice(0).sort((left, right) =>
        left.seq - right.seq
      );
  ...
  private async applyLive(event: DagRpcEvent): Promise<void> {
    ...
    if (event.seq > this.lastAppliedSeq + 1) {
      await this.recoverThrough(event.seq - 1);
    }
    this.apply(event);
  }
  ...
  private async history(
    sinceSeq: number,
    throughSeq: number,
  ): Promise<DagRpcHistoryPage> {
    this.historyRequests += 1;
    return expectOk<DagRpcHistoryPage>(
  ...
  private applyPage(events: readonly DagRpcEvent[]): void {
    for (
      const event of [...events].sort((left, right) => left.seq - right.seq)
    ) this.apply(event);
  }
  ...
  private apply(event: DagRpcEvent): void {
    ...
    if (event.seq !== this.lastAppliedSeq + 1) {
      this.gaps.push({
        expected: this.lastAppliedSeq + 1,
        received: event.seq,
      });
    }
    this.seen.add(key);
...
describe("dag viewer reference consumer", () => {
  it("#given a 200-event live run #when catch-up races live delivery #then application is gap-free and duplicate-free", async () => {
    ...
    while (journal.snapshot().checkpointSeq < 150) {
    ...
    const bridge = createDagRpcBridge(bus.pi, {
      liveRuns:
        () => [{ runId, status: "running", subscribe: journal.subscribe }],
      timers: timers.seam,
    });
    bridge.attach();
    ...
    const tappedManager = {
      ...
      history(params: DagHistoryParams) {
        historyReads += 1;
        const targetSeq = historyReads === 1
          ? 175
          : historyReads === 2
          ? 200
          : undefined;
        if (targetSeq !== undefined) {
          while (journal.snapshot().checkpointSeq < targetSeq) {
            journal.append(dagRunStartedEvent({ generation: 1 }));
          }
          bridge.publishActivity(
            activity(
              runId,
              historyReads === 1 ? "catching up" : "still running",
            ),
          );
          timers.flushOne();
        }
    ...
    registerDagRpcHandlers(bus.pi, {
      manager: tappedManager,
    ...
    expect(viewer.recoveryQueries).toEqual([{
      sinceSeq: 175,
      throughSeq: 176,
    }]);
    ...
    expect(viewer.lastAppliedSeq).toBe(
      manager.snapshot(runId, parentSessionId).lastSeq,
    );
