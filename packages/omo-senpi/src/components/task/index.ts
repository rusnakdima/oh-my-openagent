...
export function createTaskComponent(
  ...
  return {
    ...
    register(pi: SenpiExtensionAPI, ctx: ComponentContext): void {
      ...
      if (missing.length > 0) {
        ctx.logger.warn(
          "omo-senpi task component skipped: missing ExtensionAPI capabilities",
          { missing },
        );
        return;
      }
          ...
          evaluateSpawnPolicy(
            {
              ...
              resolveSkillInvocations: (sessionId: string) =>
                skillInvocations.stateFor(sessionId),
            },
            node.subagentType,
            node.prompt,
      ...
      registerTaskTools(
        ...
        engine,
        teamTools.service,
        teamTools.leadPollers.resolveDefaultTeamRunId,
        skillInvocations,
        dagRuntime,
      );
...
function sessionCwd(pi: SenpiExtensionAPI): string {
  return typeof pi.cwd === "string" && pi.cwd.length > 0
    ? pi.cwd
    : process.cwd();
}
...
function registerTaskFlags(pi: SenpiExtensionAPI): void {
  pi.registerFlag(TASK_ENABLED_FLAG, {
    ...
    default: true,
    description:
      "Enable the omo-senpi task engine (use --no-omo-task to disable).",
  });
  ...
  pi.registerTool({
    ...createTaskSendTool({
      ...
      teamRouting: {
        ...
        ...(resolveDefaultTeamRunId !== undefined
...
export function wireDagLifecycle(
  ...
  runtime: Pick<
    DagRuntime,
    "attach" | "detach" | "pauseForShutdown" | "dispose"
  >,
...
function registerTeamTools(
  ...
  for (const tool of buildLeadTeamTools({ service: context.service })) {
