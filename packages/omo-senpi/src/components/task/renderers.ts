...
import {
  ...
  type NoticeSpec,
  type NoticeTone,
} from "@oh-my-opencode/senpi-task/notice-box";
...
export const renderCategoryUnavailable: MessageRenderer<
...
export const renderTaskCompletion: MessageRenderer<
...
> = (message, options, theme) => {
    ...
    return buildNoticeBox(
      {
        ...
        tone: "dim",
        why: "Task completion details are unavailable.",
      },
      options,
      theme,
    );
  ...
  const components = details.map((detail) =>
    buildNoticeBox(completionNotice(detail), options, theme)
  );
  return {
    render: (width) =>
...
export const renderTeamMemberLiveness: MessageRenderer<
  TeamMemberLivenessDetails
> = (message, options, theme) => {
  const details = message.details;
    ...
    return buildNoticeBox(
      {
        ...
        tone: "warning",
        why: "Team member liveness details are unavailable.",
      },
      options,
      theme,
    );
  ...
  const deliveryKey =
    "deliveryKey" in details && typeof details.deliveryKey === "string"
      ? details.deliveryKey
      : undefined;
  return buildNoticeBox(
    {
      title: `⚠ Team member unavailable · ${
        normalizeRendererText(details.memberName)
      }`,
      tone:
        details.lastKnownState === "error" || details.lastKnownState === "lost"
          ? "error"
          : "warning",
      why:
        `The team member is no longer making progress; its last known state was ${state}.`,
      extra: details.reason === undefined
        ? []
        : [{ text: normalizeRendererText(details.reason), tone: "warning" }],
      ...noticeExpandedLine(joinNoticeFields([
        deliveryKey === undefined
          ? undefined
          : `delivery ${normalizeRendererText(deliveryKey)}`,
      ])),
    },
    options,
    theme,
...
function completionNotice(detail: CompletionDetails): NoticeSpec {
  ...
  const resultFile = detail.final_response_file === undefined
    ? undefined
    : normalizeRendererText(detail.final_response_file);
  return {
    title: `${statusGlyph(detail.status)} Task complete · ${
      normalizeRendererText(detail.name)
    }`,
    ...
    extra: [{
      text: joinNoticeFields([
        ...
        detail.run_stats?.tool_calls === undefined
          ? undefined
          : `tools ${detail.run_stats.tool_calls}`,
        detail.run_stats?.tokens_per_second === undefined
          ? undefined
          : `tps ${detail.run_stats.tokens_per_second}`,
      ]),
    }],
    ...noticeExpandedLine(joinNoticeFields([
...
function formatDuration(durationMs: number): string {
  ...
  if (seconds < 60) {
    return `${seconds.toFixed(2).replace(/0+$/u, "").replace(/\.$/u, "")}s`;
  }
