...
const TERMINAL_NODE_STATES: ReadonlySet<string> = new Set([
  "completed",
  "failed",
  "cancelled",
  "skipped",
...
function runHeaderRow(run: DagStatusRunSnapshot): string {
  ...
  return `${icon} ${name} ${normalizeRendererText(run.status)} ${
    waveLabel(run)
  } ${countsLabel(run)}`;
}
...
function waveLabel(run: DagStatusRunSnapshot): string {
  ...
  const states = new Map(
    run.nodes.map((node) => [node.id, node.state] as const),
  );
  const openIndex = run.waves.findIndex((wave) =>
    wave.nodeIds.some((nodeId) =>
      !TERMINAL_NODE_STATES.has(states.get(nodeId) ?? "pending")
    )
  );
...
function nodeRow(
  ...
  const label = excerptRendererText(
  ...
  if (live !== undefined) {
    parts.push(excerptRendererText(normalizeRendererText(live), ACTIVITY_MAX));
  }
  return parts.join(" ");
}
...
function routeLabel(route: DagStatusRoute): string {
  if (route.kind === "agent") {
    ...
    return route.model === undefined
      ? `agent:${agent}`
      : `agent:${agent}(${normalizeRendererText(route.model)})`;
  }
