...
function node(
  overrides: Partial<DagStatusNode> & {
    id: string;
    state: DagStatusNode["state"];
  },
): DagStatusNode {
  return {
    label: overrides.id,
...
function snapshot(
  overrides: Partial<DagStatusRunSnapshot> & { runId: string },
...
function fakeManager(
  initial: readonly DagStatusRunSnapshot[],
  sessionId = "session-a",
): FakeManager {
  let runs = initial;
  ...
  return {
    ...
    snapshot: (runId, parentSessionId) => {
      ...
      if (found === undefined || parentSessionId !== sessionId) {
...
function activity(
  overrides: Partial<DagStatusActivityEvent> & {
    runId: string;
    nodeId: string;
  },
): DagStatusActivityEvent {
  return {
    taskId: "st_1",
    activity: "running",
    turns: 1,
...
describe("createDagStatusUi.syncNow", () => {
  it("#given a run transitioning across node states #when syncing after each transition #then the rows track the states", () => {
      ...
      snapshot({
        ...
        nodes: [
          ...
          node({
            ...
            state: "pending",
            dependsOn: ["plan"],
            route: { kind: "agent", agent: "coder" },
          }),
        ],
        waves: [{ index: 0, nodeIds: ["plan"] }, {
          index: 1,
          nodeIds: ["build"],
        }],
      }),
    ...
    expect(pendingRows.slice(1)).toEqual([
      "  ○ plan category:quick",
      "  ○ build agent:coder",
    ]);
    expect(runningRows.slice(1)).toEqual([
      "  ▶ plan category:quick",
      "  ○ build agent:coder",
    ]);
    expect(settledRows.slice(1)).toEqual([
      "  ✓ plan category:quick",
      "  ✗ build agent:coder",
    ]);
    ...
    const dagUi = createDagStatusUi({
      manager: fakeManager([
        snapshot({
          ...
          status: "paused",
          nodes: [
            node({ id: "a", state: "skipped" }),
            node({ id: "b", state: "cancelled" }),
          ],
        }),
      ]),
      runtime: {
  ...
  it("#given a non-tui run mode #when syncing #then nothing renders", () => {
    ...
    dagUi.syncNow();
    dagUi.onActivity(
      activity({ runId: "dag_1", nodeId: "a", activity: "read package.json" }),
    );
  ...
  it("#given every run terminal #when syncing #then the widget is cleared", () => {
    ...
    manager.set([
      snapshot({
        runId: "dag_1",
        status: "completed",
        nodes: [node({ id: "a", state: "completed" })],
      }),
    ]);
    dagUi.syncNow();
...
describe("createDagStatusUi.onActivity", () => {
  it("#given repeated activity for one node #when rendering #then only the latest activity shows", () => {
    ...
    dagUi.onActivity(
      activity({ runId: "dag_1", nodeId: "a", activity: "edit manager.ts" }),
    );
    timers.advance(250);
