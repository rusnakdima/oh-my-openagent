...
function stateDirConfig(deps: TeamServiceDeps): StateDirConfig {
  return {
    ...
    ...(deps.settings.state_dir !== undefined
      ? { task: { state_dir: deps.settings.state_dir } }
      : {}),
  };
}

function createTaskEventAppender(
  stateDir: StateDirConfig,
): (taskId: string, event: PersistedTaskEvent) => void {
...
function requireLeadSession(deps: TeamServiceDeps): string {
  ...
  if (leadSessionId === undefined || leadSessionId.length === 0) {
    throw new Error(
      "team tools require an active lead session; none was captured yet",
    );
  }
...
function toTeams(
  rows: Awaited<ReturnType<typeof listActiveTeams>>,
...
async function assertOwnedTeam(
...
export function createTeamService(deps: TeamServiceDeps): TeamToolsService {
  ...
  const config: TeamCoreConfig = toTeamCoreConfig(
    deps.settings,
    teamStorageBaseDir(stateDir),
  );
  const appendTaskEvent = deps.appendTaskEvent ??
    createTaskEventAppender(stateDir);
  ...
  const runtimeDir = (teamRunId: string) =>
    resolveTeamRuntimeDirs(stateDir, teamRunId).runtimeDir;
  ...
  const service: TeamToolsService = {
    async createTeam(input) {
      ...
      const { spec, source } = await resolveTeamSpec(
        input,
        ports,
        deps.cwd,
        omoTeams,
    ...
    sendMessage: async (teamRunId, input) => {
      ...
      return sendTeamMessage(input, {
        ...
        ...(deps.now !== undefined ? { now: deps.now } : {}),
        ...(deps.newMessageId !== undefined
          ? { newMessageId: deps.newMessageId }
          : {}),
      });
    },
    status: async (teamRunId) => {
      await assertOwnedTeam(deps, config, teamRunId);
      return refreshTeamMemberStatuses(teamRunId, {
        manager: deps.manager,
        config,
        runtimeDir: runtimeDir(teamRunId),
      });
    },
    ...
    approveShutdown: async (teamRunId, member) => {
      ...
      return approveShutdown(teamRunId, member, {
        config,
        sendMessage: makeShutdownMessenger(deps.manager, stateDir, teamRunId),
        cancelMemberTask: makeCancelMemberTask(
          deps.manager,
          stateDir,
          teamRunId,
        ),
 ...
 */
export function createTeamMailboxReconciler(
  deps: TeamServiceDeps,
): () => Promise<void> {
  const stateDir = stateDirConfig(deps);
  const config: TeamCoreConfig = toTeamCoreConfig(
