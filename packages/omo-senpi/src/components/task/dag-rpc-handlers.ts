...
type ParseResult<TValue> = { readonly value: TValue } | {
  readonly error: string;
};

export function registerDagRpcHandlers(
  pi: SenpiExtensionAPI,
  deps: DagRpcHandlerDeps,
): void {
  ...
  handle("omo.dag.list", (data) => {
    ...
    if (sessionId === undefined) {
      return ok<DagRpcListValue>({ runs: [], limit: input.value.limit });
    }
    return attempt(() => {
      ...
      const runs = deps.manager
        ...
        .filter((run) =>
          input.value.statuses === undefined ||
          input.value.statuses.includes(run.status)
        )
  ...
  handle("omo.dag.history", (data) => {
    ...
    if (sessionId === undefined) return notOwned(input.value.runId);
    return attempt(() =>
      deps.manager.history(historyQuery(input.value, sessionId))
    );
  });
...
function errorCode(error: unknown): DagRpcErrorCode {
  ...
  if (
    code === "run_not_found" || code === "run_not_owned" ||
...
function parseList(value: unknown): ParseResult<ListParams> {
  ...
  if (!Array.isArray(value.statuses)) {
    return { error: "statuses must be an array of run statuses." };
  }
  const statuses: string[] = [];
  for (const status of value.statuses) {
    if (typeof status !== "string" || !RUN_STATUSES.has(status)) {
      return {
        error: `statuses must contain only ${[...RUN_STATUSES].join(", ")}.`,
      };
    }
...
function parseRunScoped(
  value: unknown,
): ParseResult<{ readonly runId: string }> {
  if (!isRecord(value)) return { error: "Request must be an object." };
...
function parseHistory(value: unknown): ParseResult<HistoryParams> {
  ...
  if (
    ...
    (typeof value.lane !== "string" || !EVENT_LANES.has(value.lane))
  ) {
  ...
  return {
    value: {
      ...
      ...(throughSeq.value === undefined
        ? {}
        : { throughSeq: throughSeq.value }),
    },
  };
}
...
function parseTypes(
  ...
  if (!Array.isArray(value)) {
    return { error: "types must be an array of event types." };
  }
  const types: string[] = [];
  for (const type of value) {
    if (typeof type !== "string" || type.trim().length === 0) {
      return { error: "types must contain only event types." };
    }
    types.push(type);
  }
  return { value: types };
...
function fail<TValue>(
  code: DagRpcErrorCode,
  message: string,
): DagRpcEnvelope<TValue> {
