...
function retryTargets(
  params: DagToolInput,
): { readonly kind: "ok"; readonly nodeIds?: readonly string[] } | {
  readonly kind: "error";
  readonly result: DagToolResult;
} {
  if (params.node_id !== undefined && params.node_ids !== undefined) {
    return {
      ...
      result: failure(
        "invalid_arguments",
        "action=retry accepts node_id OR node_ids, never both.",
      ),
    };
  }
  const selected = params.node_id !== undefined
    ? [params.node_id]
    : params.node_ids;
  if (
    ...
    return {
      ...
      result: failure(
        "invalid_arguments",
        "action=retry accepts prompt only alongside exactly one node id.",
      ),
    };
  }
  return {
    kind: "ok",
    ...(selected === undefined ? {} : { nodeIds: selected }),
  };
}
...
export async function retryAction(
  ...
  if (deps.retry === undefined) {
    return failure(
      "run_still_active",
      `Dag run ${runId} has no retry surface wired in this session.`,
    );
  }
  ...
  const scope = nodeIds === undefined
    ? "every failed or cancelled node"
    : nodeIds.join(", ");
  return toolResult(
    `Retried ${scope} on dag run ${runId}; it is now ${snapshot.status}.`,
    {
      kind: "retried",
      run_id: runId,
...
export async function sendAction(
  ...
  if (message === undefined || message.trim().length === 0) {
    return failure(
      "invalid_arguments",
      "action=send requires a non-empty message.",
    );
  }
  if (deps.send === undefined) {
    return failure(
      "node_not_continuable",
      `Dag run ${runId} has no send surface wired in this session.`,
    );
  }
  const sent = await deps.send(runId as DagRunId, nodeId.trim(), message);
  return toolResult(
    `Delivered the message to dag node ${sent.nodeId} (${sent.delivery}).`,
    {
      kind: "sent",
      run_id: runId,
      node_id: sent.nodeId,
      ...
      ...(sent.queuePosition === undefined
        ? {}
        : { queue_position: sent.queuePosition }),
    },
  );
}
...
export async function amendAction(
  ...
  if (input === undefined) {
    return failure(
      "invalid_arguments",
      "action=amend requires a definition carrying the edited graph.",
    );
  }
  ...
  if (deps.amend === undefined) {
    return failure(
      "invalid_amendment",
      `Dag run ${runId} has no amend surface wired in this session.`,
    );
  }
