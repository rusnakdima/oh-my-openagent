...
function withGitMasterAttribution(
  ...
  return (names, cwd) => {
    ...
    if (!resolvedSkills.some((skill) => skill.name === GIT_MASTER_SKILL_NAME)) {
      return resolution;
    }
...
export function createTaskSkillLoader(
  options: TaskSkillLoaderOptions = {},
): SkillLoader {
  const homeDir = options.homeDir ?? homedir();
  const agentDir = options.agentDir ??
    resolveAgentHome({ env: process.env, homeDir });
  const pluginSkillsDirs = options.pluginSkillsDirs ??
    packagedSkillDirs(import.meta.url);
