...
export function buildMemberPorts(
  omoConfig: OmoConfig,
  agentNames: ReadonlySet<string>,
): SenpiTeamMemberPorts {
  const categoryNames = new Set<string>([
    ...Object.keys(DEFAULT_CATEGORIES),
    ...Object.keys(omoConfig.categories ?? {}),
  ]);
  return {
...
export async function resolveTeamSpec(
  ...
  if (teamName === undefined) {
    throw new SenpiTeamSpecError(
      "no team_name or inline_spec provided",
      "INVALID_SPEC",
      "unknown",
    );
  }
  ...
  if (failure !== undefined) {
    throw new SenpiTeamSpecError(failure.message, "INVALID_SPEC", teamName);
  }

  const declared = registry.teams.map((candidate) => candidate.name);
  const declaredSuffix = declared.length > 0
    ? ` Declared teams: ${declared.sort().join(", ")}.`
    : "";
  const brokenSuffix = registry.errors.length > 0
    ? ` (${registry.errors.length} team spec(s) failed to load: ${
      registry.errors.map((error) => error.name).sort().join(", ")
    }.)`
    : "";
  throw new SenpiTeamSpecError(
