...
function fakeTimers() {
  ...
  return {
    ...
    advance(ms: number) {
      ...
      for (;;) {
        const due = [...timers.values()].filter((timer) =>
          timer.dueAt <= target
        ).sort((a, b) =>
          a.dueAt - b.dueAt
        )[0];
...
function fakePi() {
  ...
  const pi = {
    on() {},
    rpc: {
      emit: (name: string, data: unknown) => void emitted.push({ name, data }),
    },
    registerTool() {},
    registerCommand() {},
    registerFlag() {},
...
function wire(
  ...
  return {
    ...
    emitted,
    timers,
    addRun: (run: DagBridgeRun) => void owned.push(run),
  };
}
...
function wireSnapshots(
  initial: readonly DagBridgeRunSnapshot[],
  parentSessionId: string | null = "ses_parent",
) {
  const timers = fakeTimers();
  const { pi, emitted } = fakePi();
  let current = [...initial];
  ...
  return {
    ...
    emitted,
    timers,
    setSnapshots: (next: readonly DagBridgeRunSnapshot[]) =>
      void (current = [...next]),
  };
}
...
describe("dag rpc bridge", () => {
  describe("#given a journal that fans out post-durability", () => {
    it("#when events arrive #then every event is emitted exactly once on omo.dag.event in seq order", () => {
      ...
      expect(events.map((entry) => (entry.data as DagBridgeRunEvent).seq))
        .toEqual([1, 2, 3]);
      expect(events.map((entry) => (entry.data as DagBridgeRunEvent).type))
        .toEqual([
          "dag.run.created",
          "dag.wave.started",
    ...
    it("#when a run starts mid-session and sync runs #then its events reach the ledger too", () => {
      ...
      // then
      const events = emittedNames(emitted, "omo.dag.event").map((entry) =>
        entry.data as DagBridgeRunEvent
      );
  ...
  describe("#given unsequenced activity telemetry", () => {
    it("#when a node bursts activity #then it coalesces to the latest payload on omo.dag.activity only", () => {
      ...
      expect((activity[0]?.data as DagBridgeActivityEvent).activity).toBe(
        "running tests",
      );
  ...
  describe("#given the omo.dag.updated snapshot channel", () => {
    ...
    it("#when a real mutation lands #then a fresh snapshot reflecting it is emitted", () => {
      // given
      const { bridge, emitted, timers, setSnapshots } = wireSnapshots([
        runSnapshot("dag_1"),
      ]);
      bridge.attach();
      timers.advance(DAG_SNAPSHOT_DEBOUNCE_MS);
      ...
      const latest = updates[1]?.data as {
        runs: {
          status: string;
          nodes: { id: string; state: string; task_id?: string }[];
        }[];
      };
      expect(latest.runs[0]?.status).toBe("completed");
      expect(latest.runs[0]?.nodes.map((node) => [node.id, node.state]))
        .toEqual([
          ["plan", "completed"],
          ["build", "completed"],
        ]);
    ...
    it("#when more runs exist than the cap #then truncated_runs reports the overflow", () => {
      ...
      const all = Array.from(
        { length: DAG_MAX_RUN_SNAPSHOTS + overflow },
        (_, index) => runSnapshot(`dag_${String(index).padStart(4, "0")}`),
      );
      ...
      expect(payload.runs.at(-1)?.run_id).toBe(
        all[DAG_MAX_RUN_SNAPSHOTS - 1]?.runId,
      );
  ...
  describe("#given the retry, steer, and amend event vocabulary", () => {
    ...
    it("#when a retried node's attempt changes #then the whole-payload fingerprint redraws the snapshot channel", () => {
      ...
      expect(updates).toHaveLength(2);
      const latest = updates[1]?.data as {
        runs: { nodes: { attempt: number; last_error?: unknown }[] }[];
      };
    ...
    it("#when only a node's last error changes #then the snapshot channel redraws", () => {
      ...
      expect(updates).toHaveLength(2);
      const latest = updates[1]?.data as {
        runs: { nodes: { last_error?: { code: string; message: string } }[] }[];
      };
      expect(latest.runs[0]?.nodes[0]?.last_error).toEqual({
        code: "task_cancelled",
        message: "cancelled by operator",
      });
    });
    ...
    it("#when the definition is amended #then amend_count reaches the wire and redraws", () => {
      ...
      setSnapshots([
        runSnapshot("dag_1", {
          amendHistory: [{ at: "2026-08-14T00:01:00.000Z" }],
        }),
      ]);
      bridge.notifyStoreMutation();
      timers.advance(DAG_SNAPSHOT_DEBOUNCE_MS);
      ...
      const updates = emittedNames(emitted, "omo.dag.updated");
      expect(updates).toHaveLength(2);
      expect((updates[0]?.data as { runs: Record<string, unknown>[] }).runs[0])
        .not.toHaveProperty("amend_count");
      expect(
        (updates[1]?.data as { runs: { amend_count?: number }[] }).runs[0]
          ?.amend_count,
      ).toBe(1);
    });
  });
