...
import {
  createRpcModelAdmission,
  PROBE_TIMEOUT_MS,
} from "@oh-my-opencode/senpi-task/rpc-model-admission";
import {
  buildRpcModelCatalogSpawn,
  type RpcSpawnDescriptor,
...
const senpiPackageDir = dirname(
  createRequire(import.meta.url).resolve("@code-yeongyu/senpi/package.json"),
);
const senpiRpcEntry = join(senpiPackageDir, "dist", "rpc-entry.js");

function buildPinnedCatalogSpawn(
  spec: RpcRunnerSpec,
...
describe("task RPC launch profile parity", () => {
  ...
  test(
    "#given an explicit provider extension #when process model admission runs #then its model is visible without credentials",
    async () => {
      // given
  ...
  test(
    "#given a model known only through parent resources #when its provider extension is not forwarded #then admission rejects before launch",
    async () => {
      // given
      const admit = createAdmission();
      ...
      await expect(admission).rejects.toMatchObject({
        failure: {
          ...
          message: expect.stringMatching(
            /omo-mock\/mock-1.*probed catalog has/,
          ),
        },
      });
    },
