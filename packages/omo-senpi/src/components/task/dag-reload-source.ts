...
const TERMINAL_DAG_RUN_STATUSES: ReadonlySet<string> = new Set([
  "completed",
  "failed",
  "cancelled",
]);
...
export function createDagReloadSource(
  deps: DagReloadSourceDeps,
  ...
  return {
    liveRuns(): readonly ReloadGuardDagRun[] {
      ...
      for (const summary of deps.manager.list(sessionId)) {
        ...
        live.push({
          runId: summary.runId,
          name: runName(deps, summary.runId, sessionId),
