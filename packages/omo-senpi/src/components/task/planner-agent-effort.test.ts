...
function resolvedPlan(
  resolution: ReturnType<ReturnType<typeof createTaskChildPlanner>>,
) {
  if (resolution.kind !== "resolved") {
    throw new Error(`Expected a resolved plan, got ${resolution.kind}`);
  }
...
describe("agent plans carrying configured effort", () => {
  ...
  test("#given an agent with plain string models #when planned #then no variant is invented", () => {
    // given
    const config = {
      agents: { explore: { models: ["openai/plain"] } },
    } satisfies OmoConfig;
    const agents = mapOmoConfigAgents(config);
