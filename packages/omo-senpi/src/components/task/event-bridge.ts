...
import { type ReloadGuardDagSource, wireReloadGuard } from "./reload-guard";
...
type EventBridgeState = {
  ...
  readonly resumptionChannels: Pick<
    ResumptionChannelEmitter,
    "emitSessionStart" | "emitShutdown"
  >;
...
export function wireEventBridge(
  ...
  pi.on("session_start", async (_payload, eventCtx) => {
    ...
    const livenessRecords = new Map<
      string,
      ReturnType<typeof engine.manager.get>
    >();
    ...
    if (sessionId !== undefined) {
      for (
        const { record } of engine.manager.list({
    ...
    if (cleanup.deleted.length > 0) {
      ctx.logger.info("senpi-task ttl cleanup", {
  ...
  pi.on("session_shutdown", async (payload, eventCtx) => {
    ...
    await engine.lifecycle.suspendOnSessionShutdown({
      parentSessionId,
      reason,
    });
  });
  ...
  pi.on("agent_end", async (_payload, eventCtx) => {
    ...
    await engine.memberLiveness.acknowledgePersisted(
      () =>
        liveContext.sessionManager?.getSessionFile?.() ??
          engine.runtime.sessionFile(),
    );
  });
...
async function reconcileTeamMailboxBestEffort(
  ctx: ComponentContext,
  state: EventBridgeState,
): Promise<void> {
  try {
    ...
    ctx.logger.warn(
      "omo-senpi task session-start team mailbox reclaim failed",
      {
...
async function tickLeadPollersBestEffort(
  ctx: ComponentContext,
  state: EventBridgeState,
): Promise<void> {
  try {
