...
const SKILL_INVOCATION_TRACKER_EVENTS = [
  "input",
  "tool_result",
  "session_shutdown",
];
const DAG_LIFECYCLE_EVENTS = [
  "session_start",
  "session_before_switch",
  "session_shutdown",
  "session_shutdown",
...
async function seedTeamRuntime(
  cwd: string,
...
describe("omo-senpi task component wiring", () => {
  it("#given an explicit team member process #when the task component registers #then no lead task surface is wired", () => {
    ...
    try {
      // when
      createTaskComponent({ resolveCwd: () => tempProject() }).register(
        pi,
        ctxFor(pi, logger),
      );
    } finally {
  ...
  it("#given a fake ExtensionAPI boot #when the task component registers #then tools, commands, the completion renderer, and event handlers are wired", () => {
    ...
    expect(pi.commands.map((entry) => entry.name).sort()).toEqual(
      [...TASK_COMMANDS].sort(),
    );
    ...
    expect(pi.handlers.map((handler) => handler.event).sort()).toEqual(
      [
        ...TASK_EVENTS,
        ...SKILL_INVOCATION_TRACKER_EVENTS,
        ...DAG_LIFECYCLE_EVENTS,
        "session_start",
      ].sort(),
    );
  });
  ...
  it("#given task and DAG lifecycle handlers #when startup and shutdown dispatch #then task reconciliation precedes DAG resume while DAG pause precedes task suspension and DAG disposal", async () => {
    ...
    const pi = new FakeExtensionAPI();
    const order: string[] = [];
    const wireDagLifecycle = Reflect.get(
      taskComponentModule,
      "wireDagLifecycle",
    );
    ...
    wireDagLifecycle(pi, {
      attach: async () => {
        order.push("dag-resume");
      },
      detach: () => undefined,
      pauseForShutdown: () => {
        order.push("dag-pause");
      },
      dispose: () => {
        order.push("dag-dispose");
      },
    }, () => {
      pi.on("session_start", () => {
        order.push("task-reconcile");
      });
      pi.on("session_shutdown", () => {
        order.push("task-suspend");
      });
    });
  ...
  it("#given a fake ExtensionAPI boot #when the task component registers #then only injection-driven lead team tools are wired", () => {
    ...
    for (const teamTool of TEAM_TOOL_NAMES) {
      expect(registered).toContain(teamTool);
    }
  ...
  it("#given the omo-task flag is false #when the component registers #then only the unconditional hygiene sweep handler is wired", () => {
    ...
    expect(logger.entries).toContainEqual({
      level: "info",
      message: "omo-senpi task component disabled by flag",
    });
  });
  ...
  it("#given a malformed omo.json #when the component registers #then it boots with defaults and still wires the tools", () => {
    ...
    // when
    createTaskComponent({ resolveCwd: () => project }).register(
      pi,
      ctxFor(pi, logger),
    );
  ...
  it("#given a terminal member owned by lead A #when lead B reconciles then lead A reconciles #then only the owning lead receives replay", async () => {
    ...
    expect(pi.messages[0]?.message.customType).toBe(
      "senpi-task.team-member-liveness",
    );
  });
  ...
  it("#given a liveness send before host persistence #when the process crashes and restarts #then no early ack suppresses replay and agent_end later commits the exact persisted marker", async () => {
    ...
    expect(store.load(terminal.task_id)?.notification.liveness_notified_epoch)
      .toBeUndefined();
    ...
    if (replayMessage === undefined) {
      throw new Error("expected replay liveness message");
    }
    writeFileSync(
      sessionFile,
      `${JSON.stringify({ type: "custom_message", ...replayMessage })}\n`,
      "utf8",
    );
  ...
  it("#given an ExtensionAPI missing registerMessageRenderer #when the component registers #then it skips with one warning and never crashes", () => {
    ...
    const pi = new FakeExtensionAPI();
    const logger = createLogger();
    (pi as { registerMessageRenderer?: unknown }).registerMessageRenderer =
      undefined;
    ...
    const skipWarnings = logger.entries.filter(
      (entry) =>
        entry.level === "warn" &&
        entry.message.includes("missing ExtensionAPI capabilities"),
    );
