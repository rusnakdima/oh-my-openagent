...
type ResidencyManager = Pick<
  TaskManager,
  "forget" | "getResidentHandle" | "residentTaskIds"
>;
...
export function createManagerResidencyRegistry(
  getManager: () => ResidencyManager,
): ResidencyRegistry {
  return {
    ...
    terminate: () => {
      ...
      if (handle.terminate === undefined) {
        return Promise.reject(
          new TypeError(`rpc resident ${handle.task_id} has no terminate port`),
        );
      }
