...
type DagWakeStatus =
  (typeof TERMINAL_EVENT_STATUSES)[keyof typeof TERMINAL_EVENT_STATUSES];
...
export function createDagWake(deps: DagWakeDeps): DagWake {
  ...
  return {
    ...
    onSessionStart(parentSessionId) {
      ...
      for (const injection of entries.values()) {
        deps.coordinator.enqueue(injection);
      }
...
function buildRunInjection(
  ...
  if (event.type === PAUSED_EVENT_TYPE) {
    return buildPausedInjection(run, event.reason);
  }
...
function buildPausedInjection(
  ...
  return {
    ...
    content:
      `DAG "${run.name}" paused${cause}: it will resume when the session restarts.`,
    display: false,
    details: {
      runId: run.runId,
      name: run.name,
...
function buildSummary(
  ...
  const summary =
    `DAG "${name}" ${status}: ${counts.completed} completed, ${counts.failed} failed, ${counts.cancelled} cancelled, ${counts.skipped} skipped (${counts.total} total)`;
  if (firstFailure === undefined) return summary;
  const node = firstFailure.nodeId === undefined
    ? ""
    : ` at ${firstFailure.nodeId}`;
