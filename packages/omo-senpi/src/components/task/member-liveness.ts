...
export const TEAM_MEMBER_LIVENESS_MESSAGE_TYPE =
...
export function createTeamMemberLivenessNotifier(
  ...
  const maxPersistenceRetries = deps.maxPersistenceRetries ??
    DEFAULT_MAX_RETRIES;
  ...
  const notifyTerminal = (record: TaskRecord): void => {
    ...
    const key =
      `${DELIVERY_KEY_PREFIX}${record.task_id}:${record.notification.run_epoch}`;
    if (delivered.has(key)) return;
    delivered.add(key);
    pendingAcknowledgement.set(key, record);
    const deliveryDetails: TeamMemberLivenessDeliveryDetails = {
      ...details,
      deliveryKey: key,
    };
    const injection = {
    ...
    try {
      ...
      if (isPromiseLike(result)) {
        void result.catch((error: unknown) => failDelivery(key, record, error));
      }
    } catch (error) {
      failDelivery(key, record, error);
    }
  ...
  const checkPersistence = async (retriesRemaining: number): Promise<void> => {
    try {
      ...
      for (const deliveryKey of [...pendingAcknowledgement.keys()]) {
        if (await markerIndex.contains(sessionFile, deliveryKey)) {
...
export function livenessDeliveryKeysFromSessionText(
  text: string,
): readonly string[] {
...
export function livenessDetails(
  record: TaskRecord,
): TeamMemberLivenessDetails | undefined {
  const memberName = parseTeamMemberTaskIdentity(record)?.memberName;
  if (
    memberName === undefined ||
    (record.status !== "error" && record.status !== "lost" &&
      record.killed !== true)
  ) {
    return undefined;
  }
...
function messageDeliveryKeys(
  ...
  if (
    message.customType !== WAKE_MESSAGE_TYPE || !Array.isArray(message.details)
  ) return [];
  const keys = new Set<string>();
  for (const entry of message.details) {
    if (
      !isRecord(entry) || entry.customType !== TEAM_MEMBER_LIVENESS_MESSAGE_TYPE
    ) continue;
...
function readDeliveryKey(value: unknown): string | undefined {
  ...
  return typeof key === "string" && key.startsWith(DELIVERY_KEY_PREFIX)
    ? key
    : undefined;
}
