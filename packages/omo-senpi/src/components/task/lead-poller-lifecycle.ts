...
export type LeadPollerLifecycleDeps = {
  ...
  readonly runtime: Pick<
    TaskRuntimeContext,
    "sessionId" | "sessionFile" | "parentState"
  >;
...
export function createLeadPollerLifecycle(
  deps: LeadPollerLifecycleDeps,
): LeadPollerLifecycle {
  ...
  const synchronizeOwnedPollers = async (): Promise<
    readonly ActiveTeamSummary[]
  > => {
    if (stopped) return [];
    ...
    for (const [teamRunId, entry] of pollers) {
      if (entry.ownerSessionId === sessionId && ownedIds.has(teamRunId)) {
        continue;
      }
    ...
    if (sessionId === undefined || deps.runtime.sessionFile() === undefined) {
      return owned;
    }
    for (const team of owned) {
      ...
      if (
        stopped || deps.runtime.sessionId() !== sessionId ||
        deps.runtime.sessionFile() === undefined
      ) break;
      const poller = createPoller({
        teamRunId: team.teamRunId,
        config: deps.config,
        coordinator: sink,
        ...(deps.deliveryJournal !== undefined
          ? { deliveryJournal: deps.deliveryJournal }
          : {}),
  ...
  const resolveLeadPoller = (teamRunId: string): LeadPollerPort | undefined => {
    if (
      stopped || deps.runtime.sessionFile() === undefined ||
      isTransition(deps.runtime.parentState())
    ) return undefined;
    const entry = pollers.get(teamRunId);
    if (
      entry === undefined || entry.ownerSessionId !== deps.runtime.sessionId()
    ) return undefined;
    return entry.poller;
  };
  ...
  > => {
    ...
    if (explicit !== undefined) {
      return owned.some((team) => team.teamRunId === explicit)
        ...
        : {
          ok: false,
          reason: `Team ${explicit} is not owned by the current session.`,
        };
    }
    const onlyTeam = owned[0];
    if (owned.length === 1 && onlyTeam !== undefined) {
      return { ok: true, teamRunId: onlyTeam.teamRunId };
    }
    if (owned.length === 0) {
      return {
        ok: false,
        reason: "No active team is owned by the current session.",
      };
    }
    return { ok: false, reason: multipleOwnedReason(owned) };
  };

  const resolveDefaultTeamRunId = async (): Promise<
    DefaultTeamRunIdResolution
  > => {
    const owned = await synchronize();
    const onlyTeam = owned[0];
    if (owned.length === 1 && onlyTeam !== undefined) {
      return { kind: "resolved", teamRunId: onlyTeam.teamRunId };
    }
  ...
  function createInjectionSink(
    input: LeadPollerLifecycleDeps,
  ): LeadInjectionSink {
    return {
      enqueue(injection) {
...
function multipleOwnedReason(owned: readonly ActiveTeamSummary[]): string {
  const listed = owned.map((team) => `${team.teamRunId} ('${team.teamName}')`)
    .join(", ");
