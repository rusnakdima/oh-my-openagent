...
export function taskSnapshot(
  ...
  return {
    ...
    ...(record.child_session_id === undefined ? {} : {
      child_session_id: boundedRequired(
...
export function liveProgressSnapshot(details: ToolProgressDetails) {
  return {
    ...
    ...(details.currentTool === undefined
      ? {}
      : { current_tool: details.currentTool }),
    ...(details.lastAssistantLine === undefined
      ? {}
      : { last_assistant_line: details.lastAssistantLine }),
    turns: details.turns,
    ...
    ...(details.tokensPerSecond === undefined
      ? {}
      : { tokens_per_second: details.tokensPerSecond }),
  };
}

export function parseTaskSend(
  value: unknown,
): { readonly value: { readonly to: string; readonly message: string } } | {
  ...
  if (!validTaskId(value.to)) {
    return { error: "to must be a task id of at most 256 characters." };
  }
  ...
  if (value.message.length > MAX_CONTROL_MESSAGE_LENGTH) {
    return {
      error:
        `message must be at most ${MAX_CONTROL_MESSAGE_LENGTH} characters.`,
    };
  }
...
export function parseTaskCancel(
  ...
  if (!validTaskId(value.task_id)) {
    return { error: "task_id must be a task id of at most 256 characters." };
  }
  ...
  if (
    ...
    value.reason.length > MAX_CONTROL_REASON_LENGTH
  ) {
    return {
      error: `reason must be at most ${MAX_CONTROL_REASON_LENGTH} characters.`,
    };
  }
  return {
    value: {
...
export function parseTaskOutput(
  ...
  if (
    value.mode !== undefined && value.mode !== "status" &&
    value.mode !== "tail" && value.mode !== "full"
  ) {
    return { error: "mode must be status, tail, or full." };
  }
  if (
    value.tail_lines !== undefined &&
    (!Number.isInteger(value.tail_lines) || Number(value.tail_lines) < 1)
  ) {
    return { error: "tail_lines must be a positive integer." };
  }
  if (
    typeof value.tail_lines === "number" && value.tail_lines > MAX_TAIL_LINES
  ) {
  ...
  return {
    value: {
      ...
      ...(value.tail_lines === undefined
        ? {}
        : { tail_lines: Number(value.tail_lines) }),
    },
  };
}
...
function boundedOutputSnapshot(snapshot: TaskOutputSnapshot) {
  ...
  return {
    ...
    description: description.value,
    task_summary:
      bounded(snapshot.task_summary, MAX_SNAPSHOT_TEXT_LENGTH).value,
    execution_mode: boundedRequired(
    ...
    parent_session_id: boundedRequired(
      snapshot.parent_session_id,
      MAX_TASK_ID_LENGTH,
    ),
    root_session_id: boundedRequired(
      snapshot.root_session_id,
      MAX_TASK_ID_LENGTH,
    ),
    child_session_id:
      bounded(snapshot.child_session_id, MAX_TASK_ID_LENGTH).value,
    final_response: finalResponse.value,
    error_message: errorMessage.value,
    ...(description.truncated ? { description_truncated: true } : {}),
    ...(finalResponse.truncated ? { final_response_truncated: true } : {}),
    ...
    ...(snapshot.resolved_model === undefined ? {} : {
      resolved_model: {
        ...
        provider: boundedRequired(
          snapshot.resolved_model.provider,
          MAX_SNAPSHOT_TEXT_LENGTH,
        ),
        model_id: boundedRequired(
          snapshot.resolved_model.model_id,
          MAX_SNAPSHOT_TEXT_LENGTH,
        ),
        display: boundedRequired(
          snapshot.resolved_model.display,
          MAX_SNAPSHOT_TEXT_LENGTH,
        ),
        variant:
          bounded(snapshot.resolved_model.variant, MAX_SNAPSHOT_TEXT_LENGTH)
            .value,
        ...
        reasoning:
          bounded(snapshot.resolved_model.reasoning, MAX_SNAPSHOT_TEXT_LENGTH)
            .value,
      },
    }),
    ...(snapshot.suspended === undefined ? {} : {
      suspended: {
        explanation: boundedRequired(
          snapshot.suspended.explanation,
          MAX_SNAPSHOT_TEXT_LENGTH,
        ),
      },
    }),
    ...(snapshot.lost === undefined ? {} : {
      lost: {
        ...snapshot.lost,
        explanation: boundedRequired(
          snapshot.lost.explanation,
          MAX_SNAPSHOT_TEXT_LENGTH,
        ),
        session_dir: boundedRequired(
          snapshot.lost.session_dir,
          MAX_SNAPSHOT_TEXT_LENGTH,
        ),
      },
    }),
