...
export function createDagRuntime(deps: DagRuntimeDeps): DagRuntime {
  ...
  const baseStore = createDagFileStore({
    ...
    task: {
      ...(deps.engine.settings.state_dir === undefined
        ? {}
        : { state_dir: deps.engine.settings.state_dir }),
  ...
  const schedulers = new Map<
    DagRunId,
    { readonly scheduler: DagScheduler; running?: Promise<DagRunRecordV1> }
  >();
  ...
  const materializeSkills = (
    input: Parameters<ReturnType<typeof createDagSkillMaterializer>>[0],
  ) =>
    createDagSkillMaterializer({
      store,
      cwd: deps.engine.runtime.cwd(),
      loadSkills: deps.engine.loadSkills,
    })(input);
  ...
  const schedulerOptions = {
    ...
    ...(deps.nodeSpawnPolicy === undefined
      ? {}
      : { nodeSpawnPolicy: deps.nodeSpawnPolicy }),
    ...(dagSettings?.subscriber_ring === undefined
      ? {}
      : { subscriberRing: dagSettings.subscriber_ring }),
  };

  const subscribe = (
    runId: DagRunId,
    listener: (event: DagRunEvent) => void,
  ): () => void => {
    const listeners = runListeners.get(runId) ??
      new Set<(event: DagRunEvent) => void>();
  ...
  const publishSchedulerEvent = (event: DagRunEvent): void => {
    ...
    for (const listener of runListeners.get(event.runId) ?? []) {
      deliverDurableEvent(listener, event);
    }
  };
  const stopObservingSchedulers = observeDagSchedulers(
    taskManager,
    (scheduler) => {
  ...
  const controller = (
    ...
    running?: Promise<DagRunRecordV1>;
  } => {
  ...
  const send = async (
    ...
    const sessionId = deps.engine.runtime.sessionId() ?? "";
    coreManager.record(runId, sessionId);
    return await controller(runId, sessionId).scheduler.sendToNode(
      runId,
      nodeId as DagNodeId,
      message,
    );
  ...
  const amend = async (
    ...
    const sessionId = deps.engine.runtime.sessionId() ?? "";
    const amended = await coreManager.amend({
      runId,
      definition,
    ...
    adoptReentry(
      runId,
      reenterDagRun({ ...schedulerOptions, initialRecord: amended }),
    );
    return coreManager.snapshot(runId, sessionId);
  };
  ...
  const queryManager = {
    list: (sessionId: string, options?: { readonly limit?: number }) =>
      manager.list(sessionId, options),
    snapshot: (runId: string, sessionId: string) =>
      manager.snapshot(runId as DagRunId, sessionId),
    history: (params: Parameters<typeof manager.history>[0]) =>
      manager.history(params),
  };
  const bridge = createDagRpcBridge(deps.pi, {
    liveRuns: () =>
      runsForActiveSession(manager, deps.engine).map((summary) => ({
        runId: summary.runId,
    ...
    ...(dagSettings?.heartbeat_ms === undefined
      ? {}
      : { heartbeatMs: dagSettings.heartbeat_ms }),
    ...(deps.bridgeTimers === undefined ? {} : { timers: deps.bridgeTimers }),
  });
  registerDagRpcHandlers(deps.pi, {
  ...
  const statusUi = createDagStatusUi({
    ...
    ...(deps.statusUiTimers === undefined
      ? {}
      : { timers: deps.statusUiTimers }),
  });

  const syncActivitySubscriptions = (): void => {
    const wanted = new Map<
      string,
      { readonly runId: DagRunId; readonly nodeId: string }
    >();
    if (activeSessionId !== undefined) {
      for (const summary of manager.list(activeSessionId)) {
        if (TERMINAL_RUN_STATUSES.has(summary.status)) continue;
        for (
          const node of manager.record(summary.runId, activeSessionId).nodes
        ) {
          if (
            node.taskId !== undefined && node.state !== "completed" &&
            node.state !== "failed" &&
    ...
    for (const [taskId, owner] of wanted) {
      ...
      activityTaskSubscriptions.set(
        taskId,
        deps.engine.manager.subscribeChild(
          taskId,
          (event: ManagedChildEvent) => {
            ...
            const activity: Parameters<typeof bridge.publishActivity>[0] = {
              ...
              ...(details.currentTool === undefined
                ? {}
                : { currentTool: details.currentTool }),
              ...(details.lastAssistantLine === undefined
                ? {}
                : { lastAssistantLine: details.lastAssistantLine }),
              turns: details.turns,
            ...
            statusUi.onActivity({
              ...activity,
              activity: activity.currentTool ?? activity.lastAssistantLine ??
                activity.activity,
            });
          },
        ),
  ...
  const wake = deps.coordinator === undefined ? undefined : createDagWake({
  ...
  const notifyPaused = (
    event: DagRunEvent & { readonly type: "dag.run.paused" },
  ): void => {
  ...
  const recovery = createDagRecovery({
    ...
    reattach: (runId, taskId) => {
      ...
      recoveryTaskSubscriptions.set(
        key,
        deps.engine.manager.subscribeChild(taskId, () => mutationListener()),
      );
    },
  });
  ...
  const runtime: DagRuntime = {
    ...
    async attach() {
      ...
      if (sessionId !== undefined) {
        ...
        for (const run of manager.list(sessionId)) {
          ensureScheduled(run.runId, sessionId);
        }
      }
      statusUi.syncNow();
    },
    ...
    detach() {
      ...
      const detachedListeners = new Map(
        [...runListeners].map((
          [runId, listeners],
        ) => [runId, new Set(listeners)]),
      );
      const stopping = [...schedulers].map(([runId, owned]) =>
        owned.scheduler.cancel(runId, "runtime detached")
      );
      void Promise.allSettled(stopping).then(() =>
        removeListeners(runListeners, detachedListeners)
      );
      clearSubscriptions(recoveryTaskSubscriptions);
  ...
  return new Proxy(manager, {
    get(target, property) {
      if (property === "startOwned") {
        return (
          spec: Parameters<TaskManager["startOwned"]>[0],
          owner: Parameters<TaskManager["startOwned"]>[1],
        ) => {
          if (admissionStopped(owner.runId)) return stoppedAdmission();
          return target.startOwned(
