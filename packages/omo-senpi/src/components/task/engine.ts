...
import {
  ...
  type TeamMemberLivenessNotifier,
...
export function composeTaskEngine(deps: ComposeTaskEngineDeps): TaskEngine {
  ...
  const memberLiveness = createTeamMemberLivenessNotifier({
    ...
    wasDelivered: (record) => {
      try {
        ...
        return (fresh.notification.liveness_notified_epoch ?? -1) >=
      ...
      try {
        ...
        baseStore.mutate(record.task_id, (fresh) => {
          if (
            ...
            fresh.notification.run_epoch !== capturedEpoch
          ) return fresh;
          if (
            (fresh.notification.liveness_notified_epoch ?? -1) >= capturedEpoch
          ) return fresh;
          return {
            ...fresh,
            notification: {
              ...fresh.notification,
              liveness_notified_epoch: capturedEpoch,
            },
          };
        });
      } catch (error) {
  ...
  const getManager = (): TaskManager => {
    if (managerRef === undefined) {
      throw new Error("task manager accessed before composition finished");
    }
    return managerRef;
  };
  ...
  const storeChain = createTaskStoreChain({
    ...
    terminal: {
      ...
      notifyOwnedMemberLiveness: (record) =>
        void notifyOwnedMemberLiveness(record),
