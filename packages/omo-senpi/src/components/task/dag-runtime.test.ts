...
class ScriptedRunner implements ManagedRunner {
  ...
  start(spec: ManagedStartSpec): Promise<ManagedChildHandle> {
    ...
    const handle: ManagedChildHandle = {
      ...
      abort: () => {
        ...
        if (this.#abortError !== undefined) {
          queueMicrotask(() => {
            void Promise.reject(this.#abortError);
          });
        }
        return Promise.resolve();
      },
    ...
    this.handles.push({
      ...
      settle: (output) =>
        outcome.resolve({ status: "completed", finalResponse: output }),
      fail: (message) =>
        outcome.resolve({
          status: "error",
          failure: { kind: "child-turn-failed", message },
        }),
    });
...
class ManualTimers {
  readonly #timers = new Map<
    number,
    { readonly callback: () => void; readonly ms: number }
  >();
  #next = 0;
...
function pauseForShutdown(runtime: ReturnType<typeof createDagRuntime>): void {
  ...
  if (
    !("pauseForShutdown" in runtime) ||
    typeof runtime.pauseForShutdown !== "function"
  ) return;
...
describe("assembled DAG runtime", () => {
  test(
    "#given a running DAG node #when its assembled child emits progress #then RPC and the live widget share one activity subscription that tears down",
    async () => {
      // given
      const cwd = fs.mkdtempSync(join(tmpdir(), "omo-senpi-dag-activity-"));
      cleanupRoots.push(cwd);
      const runner = new ScriptedRunner();
      const rpcEvents: Array<
        { readonly name: string; readonly data: unknown }
      > = [];
      ...
      const pi = Object.assign(new FakeExtensionAPI(), {
        rpc: {
          emit: (name: string, data: unknown) => {
            ...
            if (
              name === "omo.dag.event" && typeof data === "object" &&
              data !== null &&
              "type" in data && data.type === "dag.node.task-attached"
            ) taskAttached.resolve();
          },
          handle: () => undefined,
        },
      });
      ...
      // then
      expect(rpcEvents.filter((event) => event.name === "omo.dag.activity"))
        .toEqual([
          {
      ...
      expect(runner.handles[0]?.listenerCount()).toBe(
  ...
  test("#given an unknown node skill #when the assembled runtime creates the run #then snapshot records missing_skill and dispatch keeps the prompt", async () => {
    ...
    engine.runtime.captureFrom({
      sessionManager: { getSessionId: () => "session-missing-skill" },
    });
    ...
    await within(runner.whenStarted(1));
    const snapshot = runtime.manager.snapshot(
      started.snapshot.runId,
      "session-missing-skill",
    );
  ...
  test(
    "#given a terminal wake buffered during compaction #when the assembled runtime attaches on session start #then it redelivers exactly once",
    async () => {
      // given
      const cwd = fs.mkdtempSync(
        join(tmpdir(), "omo-senpi-dag-wake-redelivery-"),
      );
      cleanupRoots.push(cwd);
      const runner = new ScriptedRunner();
      ...
      runner.handles[0]?.settle("done");
      await within(
        runtime.wait(started.snapshot.runId, "session-wake-redelivery"),
      );
  ...
  test(
    "#given a live task component runtime #when a two-node dag runs #then scheduler, artifacts, rpc, widget, and one wake all observe the same run",
    async () => {
      // given
      ...
      const coordinator = new IdleInjectionCoordinator((message) => {
        wakeDeliveries.push(message.content);
      });
      const engine = composeTaskEngine({
        pi,
        omoConfig: loadOmoConfig({ cwd }).config,
      ...
      if (started.details.kind !== "started") {
        throw new Error("expected dag start");
      }
      ...
      expect(result.nodes.plan).toEqual(
        expect.objectContaining({ state: "completed", output: "plan output" }),
      );
      expect(result.nodes.build).toEqual(
        expect.objectContaining({ state: "completed", output: "build output" }),
      );
      expect(
        fs.readFileSync(
          join(engine.stateDir, "dag", "results", runId, "plan.txt"),
          "utf8",
        ),
      ).toBe("plan output");
      expect(
        fs.readFileSync(
          join(engine.stateDir, "dag", "results", runId, "build.txt"),
          "utf8",
        ),
      ).toBe("build output");
      const persisted = runtime.manager.record(
        runId,
        "session-dag",
      ) as unknown as {
      ...
      expect(persisted.nodes.every((node) => node.resultArtifact !== undefined))
        .toBe(true);
      expect(rpcEvents.some((event) => event.name === "omo.dag.event")).toBe(
        true,
      );
      expect(rpcEvents.some((event) => event.name === "omo.dag.updated")).toBe(
        true,
      );
      expect(
        widgetRows.some((rows) =>
          rows.some((row) => row.includes("assembled two node"))
        ),
      ).toBe(true);
      ...
      if (cancellable.details.kind !== "started") {
        throw new Error("expected cancellable dag start");
      }
      ...
      const cancelled = await runDagTool(
        ...
        {
          action: "cancel",
          run_id: cancellable.details.run_id,
          reason: "stop proof",
        },
      );
      if (cancelled.details.kind !== "cancelled") {
        throw new Error("expected dag cancellation");
      }
      ...
      if (copyFailure.details.kind !== "started") {
        throw new Error("expected copy-failure dag start");
      }
      ...
      const blockedStatsPath = join(
        resultsDir,
        copyFailure.details.run_id,
        "copy.stats.json",
      );
      ...
      const failedCopyRecord = runtime.manager.record(
        copyFailure.details.run_id as DagRunId,
        "session-dag",
      ) as unknown as {
        readonly diagnostics: readonly { readonly kind: string }[];
      };
      expect(
        failedCopyRecord.diagnostics.some((diagnostic) =>
          diagnostic.kind === "journal_corrupt"
        ),
      ).toBe(true);
    },
    { timeout: 15_000 },
  ...
  test("#given a live-shaped in-process child whose abort rejects #when the assembled runtime cancels #then no unhandled rejection escapes and a later run still completes", async () => {
    ...
    try {
      ...
      const survived = await within(
        runtime.wait(survivor.snapshot.runId, sessionId),
      );
      ...
      expect(survived.nodes.survivor).toEqual(
        expect.objectContaining({ output: "runtime survived" }),
      );
  ...
  test("#given a throwing durable-event subscriber #when a run publishes checkpoints #then the scheduler and other subscribers still reach completion", async () => {
    ...
    engine.runtime.captureFrom({
      sessionManager: { getSessionId: () => "session-subscriber" },
    });
    const runtime = createDagRuntime({ pi, engine, logger: logger() });
    await runtime.attach();
    ...
    // then
    expect(result.status).toBe("completed");
    expect(result.nodes.survivor).toEqual(
      expect.objectContaining({ state: "completed", output: "survived" }),
    );
    runtime.dispose();
  });
  ...
  test("#given a live adapter DAG #when shutdown pauses it and a later adapter starts #then the paused event is durable and the run is claimed, resumed, and completed", async () => {
    ...
    const firstEngine = composeTaskEngine({
      ...
      sharedParentTools: () => [],
      runnerFactories: {
        inProcess: () => new ScriptedRunner(),
        process: () => new ScriptedRunner(),
      },
    });
    firstEngine.runtime.captureFrom({
      sessionManager: { getSessionId: () => sessionId },
    });
    const firstRuntime = createDagRuntime({
      pi: firstPi,
      engine: firstEngine,
      logger: logger(),
    });
    ...
    const paused = pausedStore.readCheckpoint<
    ...
    pausedStore.writeCheckpoint(runId, {
      ...paused,
    ...
    const resumedEngine = composeTaskEngine({
      ...
      sharedParentTools: () => [],
      runnerFactories: {
        inProcess: () => resumedRunner,
        process: () => resumedRunner,
      },
    });
    resumedEngine.runtime.captureFrom({
      sessionManager: { getSessionId: () => sessionId },
    });
    const resumedRuntime = createDagRuntime({
      pi: resumedPi,
    ...
    // then
    expect(result.status).toBe("completed");
    expect(result.nodes.resume).toEqual(
      expect.objectContaining({ state: "completed", output: "resumed output" }),
    );
    expect(
      dagEvents(cwd, runId).some((event) => event.type === "dag.run.resumed"),
    ).toBe(true);
      ...
      rpc: {
        emit: (name: string, data: unknown) => {
          if (
            name !== "omo.dag.event" || typeof data !== "object" ||
            data === null ||
            !("type" in data) || data.type !== "dag.stream.overflow"
          ) return;
          overflow.resolve(
            data as Extract<DagRunEvent, { type: "dag.stream.overflow" }>,
          );
        },
        handle: () => undefined,
      },
  ...
  test("#given a paused adapter DAG whose lease holder PID is still live #when another adapter starts #then it never claims or resumes the run", async () => {
    ...
    secondEngine.runtime.captureFrom({
      sessionManager: { getSessionId: () => sessionId },
    });
    ...
    // then
    expect(secondRuntime.manager.record(runId, sessionId).status).toBe(
      "paused",
    );
    expect(
      dagEvents(cwd, runId).some((event) => event.type === "dag.run.resumed"),
...
describe("dag runtime node spawn policy", () => {
  test("#given a runtime with a denying node spawn policy #when an agent-routed node is admitted #then the node fails with the denial and no child starts", async () => {
    ...
    const runtime = createDagRuntime({
      ...
      nodeSpawnPolicy: () => ({
        kind: "deny",
        message: "momus requires a plan gate",
      }),
    });
    runtime.attach();
    ...
    const result = await runtime.wait(
      started.details.run_id as DagRunId,
      "session-dag",
    );
    ...
    if (review?.state !== "failed") {
      throw new Error("expected the denied node to fail");
    }
...
describe("assembled DAG runtime control verbs", () => {
  async function controlFixture(
    ...
    const nodeEvents: Array<
      { readonly type: string; readonly nodeId?: string; readonly to?: string }
    > = [];
    ...
    const whenNode = (
      ...
      const satisfied = () =>
        nodeEvents.some((event) => event.nodeId === nodeId && predicate(event));
    ...
    return {
      ...
      whenAttached,
      whenState,
      runId: started.details.run_id as DagRunId,
    };
  }

  test(
    "#given a failed node #when the runtime retry entry point runs #then a fresh scheduler re-registers and the run completes",
    async () => {
      // given
      const { runner, runtime, sessionId, runId } = await controlFixture(
        "retry-reentry",
        [
          { id: "plan" },
      ...
      expect(result.status).toBe("completed");
      expect(result.nodes.plan).toEqual(
        expect.objectContaining({ state: "completed", output: "plan retried" }),
      );
      expect(result.nodes.build).toEqual(
        expect.objectContaining({ state: "completed", output: "build output" }),
      );
  ...
  test(
    "#given a run paused for shutdown and resumed in the SAME process #when a node is retried #then admission is un-latched and the retry completes",
    async () => {
      // given
      const { runner, runtime, sessionId, runId, whenAttached, whenState } =
        await controlFixture("retry-after-pause", [
          { id: "solo" },
      ...
      // then
      expect(result.status).toBe("completed");
      expect(result.nodes.solo).toEqual(
        expect.objectContaining({ state: "completed", output: "solo retried" }),
      );
      expect(result.nodes.next).toEqual(
        expect.objectContaining({ state: "completed", output: "next output" }),
      );
      runtime.dispose();
    },
    { timeout: 20_000 },
  ...
  test(
    "#given a run resumed by retry #when attach re-runs the schedulable gate #then the re-registered scheduler is reused instead of a second one starting the node twice",
    async () => {
      // given
      const { runner, runtime, sessionId, runId, whenAttached } =
        await controlFixture("retry-registration", [{ id: "solo" }]);
      await within(runner.whenStarted(1));
      runner.handles[0]?.fail("solo blew up");
  ...
  test(
    "#given a running node #when the runtime send entry point runs #then the message is steered to its child",
    async () => {
      // given
      const { runner, runtime, sessionId, runId, whenAttached } =
        await controlFixture("send-steer", [{ id: "live" }]);
      await within(runner.whenStarted(1));
      await within(whenAttached("live"), 5_000);

      // when
      const sent = await within(
        runtime.send(runId, "live", "focus on the failing test"),
        5_000,
      );
  ...
  test(
    "#given a settled run #when the runtime amend entry point edits a node #then only the changed node re-runs and the run re-enters",
    async () => {
      // given
      const { runner, runtime, sessionId, runId } = await controlFixture(
        "amend-reentry",
      ...
      const failed = await within(runtime.wait(runId, sessionId), 5_000);
      expect(failed.status).toBe("failed");
      const planTaskId = runtime.manager.record(runId, sessionId).nodes.find((
        node,
      ) => String(node.id) === "plan")?.taskId;
      ...
      expect(result.nodes.build).toEqual(
        expect.objectContaining({
          state: "completed",
          output: "build amended",
        }),
      );
      expect(
        runtime.manager.record(runId, sessionId).nodes.find((node) =>
          String(node.id) === "plan"
        )?.taskId,
      ).toBe(planTaskId);
      runtime.dispose();
