...
describe("createSkillInvocationTracker - plan-artifact reference counting", () => {
  ...
  test("#given three reads of plan A and one edit of plan B #when references are queried #then counts accumulate per path with the most-referenced first", async () => {
    ...
    await pi.dispatch(
      "tool_result",
      toolResult("read", { path: "/repo/.omo/plans/alpha.md" }),
      CTX_A,
    );
    await pi.dispatch(
      "tool_result",
    ...
    await pi.dispatch(
      "tool_result",
      toolResult("edit", { path: "/repo/.omo/plans/beta.md" }),
      CTX_A,
    );
  ...
  test("#given apply_patch events mentioning plan paths #when they arrive #then each distinct path counts once per event", async () => {
    ...
    await pi.dispatch(
      "tool_result",
      toolResult("apply_patch", { input: patch }),
      CTX_A,
    );
  ...
  test("#given two plans touched once each #when references are queried #then the most recently touched wins the tie until one pulls ahead", async () => {
    ...
    await pi.dispatch(
      "tool_result",
      toolResult("read", { path: "/repo/.omo/plans/beta.md" }),
      CTX_A,
    );
    ...
    expect(tied.map((ref) => ref.path)).toEqual([
      "/repo/.omo/plans/beta.md",
      "/repo/.omo/plans/alpha.md",
    ]);
    ...
    expect(
      tracker.stateFor("sess-a").planArtifactReferences().map((
        ref,
      ) => [ref.path, ref.count]),
    ).toEqual([
  ...
  test("#given windows and posix spellings of the same plan path #when both arrive #then they merge into one normalized reference", async () => {
    ...
    await pi.dispatch(
      "tool_result",
      toolResult("read", { path: "C:\\repo\\.omo\\plans\\win.md" }),
      CTX_A,
    );
    await pi.dispatch(
      "tool_result",
  ...
  test("#given the same plan suffix under two worktree roots #when both arrive #then the roots stay distinct", async () => {
    ...
    await pi.dispatch(
      "tool_result",
      toolResult("read", { path: "/wt/one/.omo/plans/a.md" }),
      CTX_A,
    );
    await pi.dispatch(
      "tool_result",
      toolResult("read", { path: "/wt/two/.omo/plans/a.md" }),
      CTX_A,
    );
    ...
    expect(refs.map((ref) => ref.path).sort()).toEqual([
      "/wt/one/.omo/plans/a.md",
      "/wt/two/.omo/plans/a.md",
    ]);
  });
});
