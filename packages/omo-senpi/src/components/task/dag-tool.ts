...
import { type DagToolInput, DagToolParams } from "./dag-tool-params";
...
export type { DagToolDefinitionInput, DagToolInput } from "./dag-tool-params";
export type {
  DagToolDeps,
  DagToolDetails,
  DagToolError,
...
function fromManagerError(error: DagManagerError): DagToolResult {
  const code: DagToolErrorCode = error.code === "invalid_definition"
    ? "invalid_definition"
    : error.code;
...
async function startAction(
  ...
  if (input === undefined) {
    return failure(
      "invalid_definition",
      "action=start requires a definition. Provide definition.key, definition.name, and definition.nodes.",
    );
  }
  ...
  const warningText = warnings.length === 0
    ? ""
    : ` Advisory warnings (fix the definition and re-start under a new key to clear them): ${
      warnings.join(" | ")
    }`;
  return toolResult(
    `${verb} dag run ${result.snapshot.runId} (${result.snapshot.counts.total} nodes).${warningText}`,
    {
      kind: "started",
...
async function waitAction(
  ...
  if (deps.wait === undefined) {
    return toolResult(
      `Dag run ${runId} is ${snapshot.status}; no wait surface is wired in this session.`,
      {
        kind: "waited",
        run_id: runId,
        result: {
...
async function cancelAction(
  ...
  const snapshot = deps.manager.snapshot(runId as DagRunId, parentSessionId);
  return toolResult(
    `Cancelled dag run ${runId}${reason === undefined ? "" : ` (${reason})`}.`,
    {
      kind: "cancelled",
      run_id: runId,
      snapshot,
...
export async function runDagTool(
  ...
  try {
    ...
    if (runId === undefined) {
      return failure(
        "run_not_found",
        `action=${params.action} requires run_id.`,
      );
    }
    switch (params.action) {
      case "attach": {
        const handle = deps.manager.attach(
          runId as DagRunId,
          deps.parentSessionId(),
        );
        const snapshot = handle.snapshot();
        return toolResult(
          `Attached to dag run ${runId} (${snapshot.status}).`,
          {
            kind: "attached",
            run_id: runId,
            snapshot,
      ...
      case "snapshot": {
        ...
        return toolResult(
          `Dag run ${runId} is ${snapshot.status} (${snapshot.counts.completed}/${snapshot.counts.total} nodes complete).`,
          {
            kind: "snapshot",
            run_id: runId,
            snapshot,
...
export function createDagTool(
  deps: DagToolDeps,
): ToolDefinition<typeof DagToolParams, DagToolDetails> {
  return {
    name: DAG_TOOL_NAME,
    label: "Dag",
