...
describe("buildWidgetRows", () => {
  it("#given active and completed records #when selecting rows #then only resident team members follow active rows before the cap", () => {
    const alpha = record({
      task_id: "st_alpha",
      name: "team:12345678-1234-1234-1234-123456789abc:alpha",
      task_summary: "settled alpha",
      status: "completed",
    });
    const beta = record({
      task_id: "st_beta",
      name: "team:12345678-1234-1234-1234-123456789abc:beta",
      task_summary: "settled beta",
      status: "completed",
    });
    const ordinary = record({
      task_id: "st_ordinary",
      task_summary: "ordinary task",
      status: "completed",
    });
    const stale = record({
      task_id: "st_stale",
      name: "team:12345678-1234-1234-1234-123456789abc:stale",
      task_summary: "stale member",
      status: "completed",
    });
    const active = Array.from(
      { length: 4 },
      (_value, index) => record({ task_id: `st_${index}`, status: "running" }),
    );
    const rows = buildWidgetRows(
      [alpha, ordinary, stale, ...active, beta],
      new Set([alpha.task_id, beta.task_id, ordinary.task_id]),
    );
  ...
  it("#given a 137-column active task #when building its widget row #then it remains one physical line", () => {
    ...
    for (const columns of [70, 72, 120]) {
      expect(rendererVisibleWidth(row)).toBeLessThanOrEqual(columns);
    }
  ...
  it("#given a record with a task_summary #when building the widget row #then the summary leads over name and description", () => {
    const row = buildWidgetRows([
      record({
        ...
        task_summary: "Audit auth session flow",
        status: "running",
        category: "quick",
      }),
    ])[0] ?? "";
...
describe("backgroundWidgetRows", () => {
  ...
  it("#given a wide terminal #when a live task row renders #then the longer summary and adjacent metadata remain visible", () => {
    const row = backgroundWidgetRows(
      ...
      new Map([["st_wide", "running read src/library.ts"]]),
      now,
      () => stats,
      220,
    )[0] ?? "";
    ...
    expect(row).toContain(
      "Plan the complete Spider-Man media library migration",
    );
    expect(row).toContain(
      "category:unspecified-high(anthropic/claude-opus-5:xhigh)",
    );
...
describe("formatTaskRow", () => {
  ...
  it("#given a description #when formatting #then the human label leads", () => {
    ...
    expect(row).toStartWith(
      "Audit the waiting line (st_described) category:quick",
    );
  });
  ...
  it("#given no resolved model #when formatting #then raw model remains", () => {
    ...
    expect(row).toBe(
      "st_legacy agent:explore(anthropic/claude-sonnet-4-6) mode:in-process status:running",
    );
  });
  ...
  it("#given empty resolved detail labels #when formatting #then they are omitted", () => {
    ...
    expect(row).toBe(
      "st_empty category:ultrabrain(google/gemini-3.1-pro) mode:in-process status:running",
    );
  });
  ...
  it("#given malformed running progress #when formatting #then the excerpt is width-safe", () => {
    ...
    const progress = progressIndex >= 0
      ? row.slice(progressIndex + progressPrefix.length)
      : "";
