...
function record(
  overrides: Partial<TaskRecord> & Pick<TaskRecord, "task_id" | "status">,
): TaskRecord {
  return {
...
function dagSource(
  runs: ReadonlyArray<
    { readonly runId: string; readonly name: string; readonly status: string }
  >,
): ReloadGuardDagSource {
  return {
    liveRuns: () =>
      runs.filter((entry) =>
...
describe("reload guard", () => {
  it("#given a running resident child #when the veto is evaluated #then reload is cancelled with an actionable reason", () => {
    const veto = evaluateReloadVeto(
      managerOf([
        record({ task_id: "st_1", status: "running", name: "explore-auth" }),
      ]),
    );
  ...
  it("#given a wired guard #when senpi emits session_before_reload #then the handler returns the cancel result", async () => {
    ...
    wireReloadGuard(
      pi,
      managerOf([
        record({ task_id: "st_9", status: "running", name: "deep-refactor" }),
      ]),
    );

    expect(pi.handlers.map((entry) => entry.event)).toEqual([
      "session_before_reload",
    ]);
    const results = await pi.dispatch("session_before_reload", {
  ...
  it("#given a running child #when the veto is consulted twice (senpi pre-check + internal check) #then both consultations return the identical result", async () => {
    ...
    wireReloadGuard(
      pi,
      managerOf([
        record({ task_id: "st_7", status: "running", name: "deep-refactor" }),
      ]),
    );

    const first = await pi.dispatch("session_before_reload", {
      type: "session_before_reload",
    });
    const second = await pi.dispatch("session_before_reload", {
      type: "session_before_reload",
    });
  ...
  it("#given a wired guard with no running children #when senpi emits session_before_reload #then reload proceeds", async () => {
    ...
    wireReloadGuard(
      pi,
      managerOf([record({ task_id: "st_1", status: "completed" })]),
    );
  ...
  it("#given a wired guard with a live dag run #when senpi emits session_before_reload #then the handler cancels the reload", async () => {
    ...
    wireReloadGuard(
      pi,
      managerOf([]),
      dagSource([{ runId: "dag_9", name: "release", status: "running" }]),
    );
