...
export function createCategoryConfigGenerations(
  mask: MaskCategoryModel = identityMask,
  ...
  return {
    observe: ({ omoConfig, registry }) => {
      ...
      const next = {
        categories,
        userCategoryCount:
          userNames.filter((name) => !BUILTIN_CATEGORY_NAMES.includes(name))
            .length,
        builtinOverriddenCount:
          userNames.filter((name) => BUILTIN_CATEGORY_NAMES.includes(name))
            .length,
      };
...
function identityMask(
  provider: string,
  modelId: string,
): { readonly provider: string; readonly model_id: string } {
...
function canonicalCategories(
  ...
  const entries = BUILTIN_CATEGORY_NAMES.map((name) =>
    [name, encodeCategory(name, omoConfig, registry, mask)] as const
  );
  ...
  switch (resolution.kind) {
    case "resolved": {
      ...
      const reasoning = resolution.spec.reasoning ??
        resolution.spec.reasoningEffort ?? resolution.spec.variant ?? "none";
