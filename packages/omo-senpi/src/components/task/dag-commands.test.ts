...
function node(
  overrides: Partial<DagCommandNode> & { id: string },
): DagCommandNode {
  return {
    state: "completed",
...
function fakeManager(
  ...
  return {
    ...
    snapshot: (runId, parentSessionId) => {
      ...
      const owner = summaries.find((summary) => summary.runId === runId)
        ?.parentSessionId;
      if (owner !== undefined && owner !== parentSessionId) {
        throw new Error(`dag run "${runId}" belongs to another session`);
      }
      return found;
    },
...
function summary(
  overrides: Partial<DagCommandRunSummary> & { runId: string },
): DagCommandRunSummary {
  return {
    name: "run",
...
async function invoke(
  ...
  const handler = command.options["handler"] as (
    args: string,
    ctx: unknown,
  ) => Promise<void>;
...
describe("registerDagCommands", () => {
  ...
  it("#given runs in two sessions #when /dag runs with no argument #then this session's runs print with statuses", async () => {
    ...
    const done = summary({
      ...
      name: "nightly sweep",
      status: "completed",
      counts: { total: 3, completed: 3, running: 0, failed: 0 },
    });
    const other = summary({
      runId: "dag_other",
      name: "foreign",
  ...
  it("#given a diamond run #when /dag <id> runs #then waves group nodes with dependency annotations", async () => {
    ...
    registerDagCommands(
      pi,
      fakeManager([summary({ runId: "dag_diamond", name: "release audit" })], {
        dag_diamond: snapshot,
      }),
    );
    const { ctx, ui } = commandCtx("session-a");
  ...
  it("#given a diamond run #when /dag <id> runs #then per-node state, route, model, attempt, duration and error render", async () => {
    ...
    const tasks: Record<string, DagCommandTaskRecord> = {
      st_left: {
        model: "anthropic/claude-opus-4-1",
        resolved_model: { display: "opus-4.1-high" },
      },
    };
    registerDagCommands(
      pi,
      fakeManager([summary({ runId: "dag_diamond" })], {
        dag_diamond: snapshot,
      }, tasks),
    );
    const { ctx, ui } = commandCtx("session-a");
  ...
  it("#given a diamond run #when /dag <id> runs #then the critical path and bottleneck are marked", async () => {
    ...
    expect(lines.find((line) => line.includes("deep review"))).toContain(
      "*critical*",
    );
    expect(lines.find((line) => line.includes("quick lint"))).not.toContain(
      "*critical*",
    );
  ...
  it("#given a retried run #when /dag <id> runs #then only re-run nodes carry the xN badge and the failure line names its code", async () => {
    ...
    expect(lines.find((line) => line.includes("collect inputs"))).not.toContain(
      "x2",
    );
    expect(lines.find((line) => line.includes("quick lint"))).toContain(
      "error: task_error lint exited 1",
    );
  });
  ...
  it("#given an amended run #when /dag <id> runs #then the header carries an amended xN marker", async () => {
    ...
    // then
    const header =
      ui.notifications.map((entry) => entry.message).join("\n").split(
        "\n",
      )[0] ?? "";
  ...
  it("#given a run that was never amended #when /dag <id> runs #then no amended marker renders", async () => {
    ...
    registerDagCommands(
      pi,
      fakeManager([summary({ runId: "dag_diamond" })], {
        dag_diamond: { ...diamondSnapshot(), amendHistory: [] },
      }),
    );
    const { ctx, ui } = commandCtx("session-a");
  ...
  it("#given an unknown run id #when /dag <id> runs #then it notifies instead of throwing", async () => {
    ...
    registerDagCommands(
      pi,
      fakeManager([summary({ runId: "dag_known" })], {
        dag_known: diamondSnapshot(),
      }),
    );
    const { ctx, ui } = commandCtx("session-a");
  ...
  it("#given a session with zero runs #when /dag runs #then the empty state prints", async () => {
    ...
    registerDagCommands(
      pi,
      fakeManager([
        summary({ runId: "dag_other", parentSessionId: "session-b" }),
      ]),
    );
    const { ctx, ui } = commandCtx("session-a");
