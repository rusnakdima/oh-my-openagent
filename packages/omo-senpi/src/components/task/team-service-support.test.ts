...
import {
  ...
  cleanupProjects,
  makeManager,
} from "../../../../senpi-task/src/manager/__fixtures__/manager-fakes";
...
describe("makeShutdownMessenger", () => {
  test("#given a mapped running member #when a shutdown notice is sent #then the notice steers into the running turn", async () => {
    ...
    const runtimeDir =
      resolveTeamRuntimeDirs({ project_dir: project }, "run-1").runtimeDir;
    mkdirSync(runtimeDir, { recursive: true });
    writeFileSync(
      join(runtimeDir, "senpi-task-members.json"),
      `${JSON.stringify({ beta: started.task_id })}\n`,
    );

    const messenger = makeShutdownMessenger(
      manager,
      { project_dir: project },
      "run-1",
    );
    ...
    expect(handle.steerCalls).toEqual([
      "[team shutdown_request] finish current work",
    ]);
