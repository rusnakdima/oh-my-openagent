...
async function activeTeamHarness(sessionId?: string) {
  ...
  const stateDir = {
    project_dir: cwd,
    ...(engine.settings.state_dir !== undefined
      ? { task: { state_dir: engine.settings.state_dir } }
      : {}),
  };
  ...
  const creating = await createRuntimeState(
    spec,
    "lead-session",
    "project",
    config,
...
describe("createTeamService lead messaging", () => {
  test("#given a mapped recipient task #when the lead sends #then the correlation event is persisted on the recipient task", async () => {
    ...
    const runtimeDir =
      resolveTeamRuntimeDirs(stateDir, runtimeState.teamRunId).runtimeDir;
    writeFileSync(
    ...
    // when
    await service.sendMessage(runtimeState.teamRunId, {
      from: "lead",
      to: "beta",
      body: "continue",
    });
    ...
    const eventLog = readFileSync(
      join(store.stateDir, "logs", `${MEMBER_TASK_ID}.jsonl`),
      "utf8",
    );
  ...
  test("#given an active runtime member without a sidecar entry #when the lead sends #then delivery succeeds without a correlation event", async () => {
    ...
    expect(
      existsSync(
        join(
          resolveTeamMemberInboxDir(stateDir, runtimeState.teamRunId, "beta"),
          `${MESSAGE_ID}.json`,
        ),
      ),
    ).toBe(true);
    expect(
      existsSync(
        join(
          createTaskRecordStore(stateDir).stateDir,
          "logs",
          `${MEMBER_TASK_ID}.jsonl`,
        ),
      ),
    ).toBe(false);
  });
...
describe("createTeamService named-team lookup", () => {
  test("#given an unknown team_name with declared teams #when createTeam runs #then the error lists the declared teams", async () => {
    ...
    // when / then
    await expect(service.createTeam({ teamName: "missing-team" })).rejects
      .toThrow(/missing-team[\s\S]*declared-a[\s\S]*declared-b/);
  });
});
...
describe("createTeamService ownership guard", () => {
  test("#given a team owned by another session #when scoped service methods are called #then they reject with an ownership error", async () => {
    ...
    await expect(service.listTasks(runtimeState.teamRunId, {})).rejects.toThrow(
      "is not owned by the current session",
    );
    await expect(service.getTask(runtimeState.teamRunId, "1")).rejects.toThrow(
      "is not owned by the current session",
    );
    await expect(
      service.createTask(runtimeState.teamRunId, {
        subject: "s",
        description: "d",
        status: "pending",
      }),
    ...
    await expect(
      service.sendMessage(runtimeState.teamRunId, {
        from: "lead",
        to: "beta",
        body: "x",
      }),
    ).rejects.toThrow("is not owned by the current session");
    await expect(service.status(runtimeState.teamRunId)).rejects.toThrow(
      "is not owned by the current session",
    );
    await expect(service.deleteTeam({ teamRunId: runtimeState.teamRunId }))
      .rejects.toThrow("is not owned by the current session");
    await expect(service.requestShutdown(runtimeState.teamRunId, "beta"))
      .rejects.toThrow("is not owned by the current session");
    await expect(service.approveShutdown(runtimeState.teamRunId, "beta"))
      .rejects.toThrow("is not owned by the current session");
    await expect(
      service.rejectShutdown(runtimeState.teamRunId, "beta", "keep going"),
    ).rejects.toThrow("is not owned by the current session");
  });
