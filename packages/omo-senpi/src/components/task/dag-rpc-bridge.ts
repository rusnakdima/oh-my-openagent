...
export function createDagRpcBridge(
  pi: SenpiExtensionAPI,
  deps: DagRpcBridgeDeps,
): DagRpcBridge {
  const timers = deps.timers ?? globalTimers;
  const now = deps.now ?? Date.now;
  const heartbeatMs = deps.heartbeatMs ?? DAG_DEFAULT_HEARTBEAT_MS;
  const activityCoalesceMs = deps.activityCoalesceMs ??
    DAG_ACTIVITY_COALESCE_MS;
  const snapshotDebounceMs = deps.snapshotDebounceMs ??
  ...
  const beat = (): void => {
    ...
    emit(DAG_HEARTBEAT_CHANNEL, {
      ...
      runs: runs.map((run) => ({
        runId: run.runId,
        headSeq: headSeq.get(run.runId) ?? 0,
      })),
    });
    scheduleHeartbeat();
  };
  ...
  const flushSnapshot = (): void => {
    ...
    if (parentSessionId === undefined || deps.runSnapshots === undefined) {
      return;
    }
  ...
  return {
    ...
    publishActivity(event) {
      ...
      if (activityFlush === undefined) {
        activityFlush = timers.set(flushActivity, activityCoalesceMs);
      }
    },
    dispose() {
      if (disposed) return;
