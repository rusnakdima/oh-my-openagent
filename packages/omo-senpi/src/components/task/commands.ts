...
async function runTasksCommand(
  ...
  const text = records.length === 0
    ? `No tasks in ${scopeLabel}.`
    : records.map(formatTaskRow).join("\n");
...
async function runTaskKillCommand(
  ...
  const cancellable = collect(manager, scopeFor(ctx, false)).filter((record) =>
