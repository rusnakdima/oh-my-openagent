...
import type {
  BinaryResolutionState,
  CommentCheckerComponentOptions,
} from "./types";
import { getString, normalizeFeedbackText } from "./utils";

export function createCommentCheckerComponent(
  options: CommentCheckerComponentOptions = {},
): OmoSenpiComponent {
  ...
  return {
    ...
    register(pi: SenpiExtensionAPI, ctx: ComponentContext): void {
      ...
      pi.on("tool_result", async (payload, eventContext) => {
        ...
        if (
          event === undefined || event.isError ||
          !isMutationToolName(event.toolName)
        ) {
          return undefined;
        }
        ...
        const absolutePath = isAbsolute(rawPath)
          ? rawPath
          : resolve(toolContext.cwd, rawPath);
        ...
        return {
          content: [
            ...event.content,
            {
              type: "text",
              text:
                `${COMMENT_CHECKER_FEEDBACK_HEADER} ${absolutePath}:\n${message}`,
            },
          ],
        };
      });
...
function ensureBinaryPath(
  ...
  try {
    ...
    state.logger.warn(
      "omo-senpi comment-checker binary resolution failed; component disabled for this session",
      { error },
    );
    nextBinaryPath = null;
  }
  ...
  if (nextBinaryPath === null) {
    ...
    if (!state.missingBinaryNoticeLogged) {
      state.logger.warn(
        "omo-senpi comment-checker binary unavailable; component disabled for this session",
      );
