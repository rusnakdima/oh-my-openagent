...
import {
  COMMENT_CHECKER_ENV_KEY,
  COMMENT_CHECKER_PACKAGE_NAME,
} from "./constants";
import type { SenpiCommentCheckerBinaryResolverOptions } from "./types";
import { isCommentCheckerPackage } from "./utils";

export function resolveSenpiCommentCheckerBinary(
  options: SenpiCommentCheckerBinaryResolverOptions = {},
): string | null {
  const checkExists = options.existsSync ?? existsSync;
  const env = options.env ?? process.env;
  const envBinaryPath = env[COMMENT_CHECKER_ENV_KEY]?.trim();
  if (
    envBinaryPath !== undefined && envBinaryPath.length > 0 &&
    isAbsolute(envBinaryPath) && checkExists(envBinaryPath)
  ) {
  ...
  const binaryName = (options.platform ?? process.platform) === "win32"
    ? "comment-checker.exe"
    : "comment-checker";
...
function resolvePackageApiBinary(
  input: PackageApiBinaryResolverInput,
): string | null {
  try {
    const requireModule = input.requireModule ??
      createRequire(input.importMetaUrl);
