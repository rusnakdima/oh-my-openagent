...
function parseEdits(
  value: unknown,
): readonly { old_string: string; new_string: string }[] {
...
export function parseToolResultContext(value: unknown): ToolResultContextLike {
  ...
  const sessionManager = isRecord(value.sessionManager)
