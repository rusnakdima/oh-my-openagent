...
const DOCS_URL =
  "https://github.com/code-yeongyu/oh-my-openagent/blob/dev/docs/reference/senpi-telemetry.md";
const NOTICE =
  `omo-senpi sends anonymous usage telemetry (no prompts, no paths). Docs: ${DOCS_URL} - opt out: DO_NOT_TRACK=1`;
...
function enabled(
  ...
  return isTelemetryClientEnabled({
    env,
...
function configEnabled(
  ...
  try {
    ...
    diagnostics(
      new Error(loaded.diagnostics.map(({ message }) => message).join("; ")),
    );
    return false;
  } catch (error) {
    diagnostics(error);
...
function notificationUi(value: unknown): NotificationUi | undefined {
  ...
  const ui = value["ui"];
  return isRecord(ui) && typeof ui["notify"] === "function"
    ? ui as NotificationUi
    : undefined;
}
