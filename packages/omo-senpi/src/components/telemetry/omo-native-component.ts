...
export function createOmoNativeTelemetryComponent(
  ...
  return {
    ...
    register(pi, ctx) {
      ...
      const detachDelegation = createOmoNativeDelegationCapture({
        ...
        hashSessionId: options.hashSessionId ?? hashSessionId,
        observers: options.taskTerminalObservers ??
          sharedTaskTerminalObservers(),
      ...
      pi.on("turn_end", (payload, eventCtx) => {
        ...
        const sessionHash = state.sessionHash ??
          hashForContext(eventCtx, options.hashSessionId);
...
function sharedTransportFactory(
  ...
  return (apiKey, transportOptions) => {
    ...
    const wrapped: TelemetryTransport = {
      ...
      flush: transport.flush === undefined
        ? undefined
...
function installCaptureFacade(
  ...
  const sessionId =
    template.event === "daily_active" || template.event === "session_started"
      ? template.properties?.$session_id
      : undefined;
  state.sessionHash = typeof sessionId === "string"
    ? sessionId
    : state.sessionHash;
...
function forwardValidatedCapture(
  transport: TelemetryTransport,
  message: TelemetryCaptureMessage,
): void {
...
function resolveTaskStateDir(pi: { readonly cwd?: string }): string {
  return resolveStateDir({
    project_dir: typeof pi.cwd === "string" && pi.cwd.length > 0
      ? pi.cwd
      : process.cwd(),
  });
}
  ...
  if (isRecord(value) && isRecord(value.sessionManager)) {
    ...
    if (typeof getter === "function") {
      ...
      if (typeof sessionId === "string" && sessionId.length > 0) {
        return (hash ?? hashSessionId)(sessionId);
      }
    }
  }
...
function isTurnEndEvent(value: unknown): value is TurnEndEvent {
  return isRecord(value) && value.type === "turn_end" &&
    typeof value.turnIndex === "number" && isRecord(value.message);
}
