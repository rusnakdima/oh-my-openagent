  ...
  return {
    ...
    factory: () => {
      ...
      return {
        ...
        flush: async () => {
          flushes[index] = (flushes[index] ?? 0) + 1;
        },
        shutdown: async () => {
          shutdowns[index] = (shutdowns[index] ?? 0) + 1;
        },
      };
    },
  };
...
function writeInventory(agentDir: string, input: {
  ...
  writeFileSync(
    ...
    JSON.stringify({ providers: input.providers ?? {} }),
  );
  writeFileSync(
...
describe("OmO Native session telemetry", () => {
  it.each([
    ...
    "#given %s #when session_start fires #then native and legacy telemetry both emit nothing",
    async (name, optOut, configEnabled) => {
      await withTempAgentDir(async (agentDir) => {
        ...
        expect(
          existsSync(
            getTelemetryActivityStateFilePath(getOmoNativeStateDir(env)),
          ),
        ).toBe(false);
        expect(
          existsSync(
            getTelemetryActivityStateFilePath(getSenpiTelemetryStateDir(env)),
          ),
        ).toBe(false);
      });
    },
  ...
  it.each([
    ...
    "#given %s #when session_start loads config #then telemetry matches the resolved setting",
    async (_name, config, expectedEnabled) => {
      await withTempAgentDir(async (agentDir) => {
        // given
        writeInventory(agentDir);
        const home = join(agentDir, "home");
        ...
        await pi.dispatch("session_start", {
          type: "session_start",
          reason: "startup",
        }, sessionCtx("config", cwd));

        // then
        expect(
          recorder.messages.some(({ event }) => event === "session_started"),
  ...
  it("#given enabled telemetry #when two clients start today and one starts tomorrow #then native daily_active emits once per UTC day", async () => {
    await withTempAgentDir(async (agentDir) => {
      ...
      const tomorrow = register(agentDir, recorder, {
      ...
      await stale.dispatch("session_start", {
        type: "session_start",
        reason: "resume",
      }, sessionCtx("stale"));
      await tomorrow.dispatch("session_start", {
        type: "session_start",
        reason: "new",
      }, sessionCtx("tomorrow"));
      ...
      // then
      expect(
        recorder.messages.filter(({ event }) => event === "daily_active").map((
          { properties },
        ) => properties?.day_utc),
      ).toEqual([
        "2026-07-03",
      ...
      expect(
        recorder.messages.filter(({ event }) =>
          event === SENPI_TELEMETRY_EVENT_NAME
        ),
      ).toHaveLength(2);
    });
  });
  ...
  it("#given real-shape providers/defaultProvider/defaultModel fixtures #when session_started emits #then inventory is masked and the key set is exact", async () => {
    await withTempAgentDir(async (agentDir) => {
      ...
      writeInventory(agentDir, {
        providers: {
          anthropic: {
            models: [{ id: "claude-sonnet-5" }, { id: "private-model" }],
          },
      ...
      await pi.dispatch("session_start", {
        type: "session_start",
        reason: "fork",
      }, sessionCtx("inventory"));

      // then
      const captured = recorder.messages.find(({ event }) =>
  ...
  it("#given an injected IANA time zone #when session_started is captured #then timezone is present, at most 64 characters, and category_config is captured once afterwards with the same $session_id", async () => {
    await withTempAgentDir(async (agentDir) => {
      ...
      // then
      const started = recorder.messages.find(({ event }) =>
        event === "session_started"
      );
      expect(started?.properties?.timezone).toBe("Asia/Seoul");
      expect(String(started?.properties?.timezone).length).toBeLessThanOrEqual(
        64,
      );
      const configs = recorder.messages.filter(({ event }) =>
        event === "category_config"
      );
      expect(configs).toHaveLength(1);
      expect(configs[0]?.properties?.$session_id).toBe(
        started?.properties?.$session_id,
      );
      ...
      expect(
        recorder.messages.map(({ event }) => event).indexOf("category_config"),
      ).toBeGreaterThan(
  ...
  it("#given a host context without a model registry #when session_start fires #then session_started still ships and no category_config is captured", async () => {
    await withTempAgentDir(async (agentDir) => {
      ...
      await pi.dispatch("session_start", {
        type: "session_start",
        reason: "startup",
      }, sessionCtx("registryless"));

      // then
      expect(recorder.messages.some(({ event }) => event === "session_started"))
        .toBe(true);
      expect(
        recorder.messages.filter(({ event }) => event === "category_config"),
      ).toEqual([]);
    });
  });
  ...
  it("#given a throwing time zone provider #when session_start fires #then session_started ships without a timezone property", async () => {
    await withTempAgentDir(async (agentDir) => {
      ...
      // then
      expect(
        recorder.messages.find(({ event }) => event === "session_started")
          ?.properties,
  ...
  it("#given an existing same-day legacy marker #when the native session path runs #then its bytes stay unchanged and native uses its separate marker", async () => {
    await withTempAgentDir(async (agentDir) => {
      ...
      const legacyStateFile = getTelemetryActivityStateFilePath(
        getSenpiTelemetryStateDir(env),
      );
      ...
      expect(recorder.messages.map(({ event }) => event)).toContain(
        "daily_active",
      );
      expect(recorder.messages.map(({ event }) => event)).not.toContain(
  ...
  it.each([
    ...
    "#given %s #when session_start fires #then counts are zero, defaults are omitted, and one diagnostic is reported",
    async (_name, modelsContent) => {
      await withTempAgentDir(async (agentDir) => {
        // given
        mkdirSync(agentDir, { recursive: true });
        if (modelsContent !== undefined) {
          writeFileSync(join(agentDir, "models.json"), modelsContent);
        }
        writeFileSync(
          join(agentDir, "settings.json"),
          JSON.stringify({
        ...
        // then
        const properties = recorder.messages.find(({ event }) =>
          event === "session_started"
        )?.properties;
        ...
        expect(
          diagnostics.filter(({ event }) =>
            event === "omo_native_inventory_read_failed"
          ),
        ).toHaveLength(1);
      });
    },
