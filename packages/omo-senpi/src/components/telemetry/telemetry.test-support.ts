...
export function createRejectingTransportRecorder(
  messages: TelemetryCaptureMessage[],
...
export function createHangingTransportRecorder(
  messages: TelemetryCaptureMessage[],
): TelemetryTransportFactory {
  return () => ({
    capture(message) {
...
export function readStamp(stateDir: string): unknown {
  return JSON.parse(
    readFileSync(getTelemetryActivityStateFilePath(stateDir), "utf-8"),
  );
}
