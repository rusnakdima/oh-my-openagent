...
const BUILTIN_CATEGORY_NAMES: readonly string[] = BUILTIN_CATEGORY_DEFAULTS.map(
  ({ name }) => name,
);
...
export type CategoryConfigCapture = {
   ...
   */
  readonly observe: (
    input: { readonly registry: TaskModelRegistry; readonly source: string },
  ) => void;
};

export function createCategoryConfigCapture(
  options: CategoryConfigCaptureOptions,
): CategoryConfigCapture {
  let generation = -1;
      ...
      options.captureEvent("category_config", {
        ...
        ...Object.fromEntries(
          Object.entries(categories).map((
            [name, value],
          ) => [categoryPropertyName(name), value]),
        ),
        combo_fingerprint: createHash("sha256").update(form).digest("hex")
...
function encodeCategories(
  ...
  return Object.fromEntries(
    BUILTIN_CATEGORY_NAMES.map((
      name,
    ) => [name, encodeCategory(name, omoConfig, registry)]),
  );
}
  ...
  switch (resolution.kind) {
    case "resolved": {
      ...
      const depth = chainDepth(
        name,
        resolution.modelSelection.fallbackEntry?.model,
      );
...
function categoryCounts(
  omoConfig: OmoConfig,
): { readonly user: number; readonly overridden: number } {
  const configured = Object.keys(omoConfig.categories ?? {});
  return {
    user: configured.filter((name) => !BUILTIN_CATEGORY_NAMES.includes(name))
      .length,
    overridden:
      configured.filter((name) => BUILTIN_CATEGORY_NAMES.includes(name)).length,
  };
}
