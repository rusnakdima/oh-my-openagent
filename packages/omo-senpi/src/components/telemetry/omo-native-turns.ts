...
type TurnCompletedProperty =
  (typeof OMO_NATIVE_PROPERTY_ALLOWLISTS.turn_completed)[number];
...
export function createOmoNativeTurnHandler(
  ...
  return (event) => {
    ...
    if (
      ![input, output, cacheRead, cacheWrite, reasoning, total, costTotal]
        .every(({ valid }) => valid)
    ) {
      options.diagnostics?.({
        ...
        error: new Error(
          "turn_end assistant usage contained missing or invalid values",
        ),
        errorKind: "error",
      });
    }
...
export function maskModel(
  ...
  const modelIsKnown = Array.isArray(providerModels) &&
    providerModels.some((known) => known === modelId);
  return {
