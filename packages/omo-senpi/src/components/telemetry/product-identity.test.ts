...
function readStampedWorkspaceVersion(): string {
  ...
  throw new Error(
    "packages/omo-senpi/package.json must expose a string version",
  );
}
...
describe("OmO Native product identity", () => {
  test("#given the native product #when config is created #then identity derivation and effective geoip settings are fixed", () => {
    ...
    expect(
      getTelemetryApiKey(
        { POSTHOG_API_KEY: "env-project-key" },
        config.defaultApiKey,
      ),
    ).toBe("env-project-key");
  });
  ...
  test("#given an explicit agent directory #when the native state path is resolved #then it is nested under omo-senpi", () => {
    ...
    expect(getOmoNativeStateDir()).toBe(
      join(agentDir, "omo-senpi", "omo-native"),
    );
  });
  ...
  test("#given a known provider with an unknown custom model #when masked #then only model_id becomes custom", () => {
    ...
    expect(maskProviderAndModel(provider, knownModel ?? "")).toEqual({
      provider,
      model_id: knownModel,
    });
    expect(maskProviderAndModel(provider, "user-defined-model")).toEqual({
      provider,
      model_id: "custom",
    });
  ...
  test("#given every provider and model rung in CATEGORY_FALLBACK_CHAINS #when masked #then no shipped rung collapses to custom, while an arbitrary user provider and model still mask to custom", () => {
    ...
    const shippedRungs = Object.values(CATEGORY_FALLBACK_CHAINS).flatMap((
      chain,
    ) =>
      chain.flatMap((rung) =>
        rung.providers.map((provider) => ({ provider, model: rung.model }))
      )
    );
    ...
    expect(maskProviderAndModel("deepseek", "claude-opus-5").model_id).toBe(
      "custom",
    );
    // and: arbitrary user-configured providers and models never leave the machine
    expect(maskProviderAndModel("my-gateway", "my-finetune")).toEqual({
      provider: "custom",
      model_id: "custom",
    });
    expect(maskProviderAndModel("anthropic", "my-finetune")).toEqual({
      provider: "anthropic",
      model_id: "custom",
    });
  });
  ...
  test("#given senpi-task builtins #when telemetry allowlists are loaded #then names exactly match imported sources", () => {
    const categoryNames = BUILTIN_CATEGORY_DEFAULTS.map((definition) =>
      definition.name
    );

    expect([...CURATED_AGENTS].sort()).toEqual(
      [...CURATED_READONLY_AGENT_NAMES].sort(),
    );
  ...
  test("#given the tracked-call cap #when the widest wave histogram is encoded #then it stays inside the 64 character privacy limit", () => {
    ...
    const worstCaseHistogram = Array.from(
      { length: bucketCount },
      () => widestBucketValue,
    ).join(":");
    ...
    expect(worstCaseHistogram).toHaveLength(
      bucketCount * 4 + (bucketCount - 1),
    );
    expect(worstCaseHistogram.length).toBeLessThanOrEqual(64);
    expect(
      OMO_NATIVE_EVENT_SCHEMAS.parallelism_summary.non_eval_wave_size_histogram
        .type,
    ).toBe("string");
    expect(OMO_NATIVE_PROPERTY_ALLOWLISTS.parallelism_summary).toContain(
  ...
  test("#given the native event schemas #when inspected #then delegation_completed and category_config exist with exactly the planned property sets and enum vocabularies", () => {
    ...
    expect(Object.keys(delegation).sort()).toEqual([
      ...
      "stats_status",
      "status",
      "task_send_queued_count",
      "task_send_running_count",
      "task_seq",
      "token_status",
      "tool_calls",
      "total_tokens",
    ...
    expect(Object.keys(categoryConfig).sort()).toEqual([
      "$session_id",
      "builtin_overridden_count",
      "cat_architect",
      "cat_artistry",
      "cat_deep",
      "cat_quick",
      "cat_ultrabrain",
      "cat_unspecified_high",
      "cat_unspecified_low",
      "cat_visual_engineering",
      "cat_writing",
      "combo_fingerprint",
      "config_generation",
      "source",
    ...
    expect(delegation.status.values).toEqual([
      "completed",
      "error",
      "cancelled",
      "interrupted",
    ...
    expect(delegation.owner_kind.values).toEqual([
      "plain_child",
      "dag_node",
      "team_member",
      "unknown",
    ]);
    expect(delegation.background_mode.values).toEqual([
      "foreground",
      "background",
      "promoted",
      "unknown",
    ...
    expect(delegation.model_source.values).toEqual([
      "category",
      "explicit",
      "agent",
      "none",
    ]);
    expect(delegation.token_status.values).toEqual([
      "complete",
      "partial",
      "unavailable",
    ]);
    expect(delegation.cost_status.values).toEqual([
      "reported",
      "unavailable",
      "invalid",
    ]);
    expect(delegation.stats_status.values).toEqual([
      "complete",
      "partial",
      "unavailable",
    ]);
    expect(delegation.duration_status.values).toEqual([
      "monotonic",
      "wall_clock",
      "unavailable",
    ]);
    expect(delegation.category.values).toEqual([
      ...BUILTIN_CATEGORY_NAMES,
      "custom",
      "none",
    ]);
    expect(delegation.agent_type.values).toEqual([
      ...CURATED_AGENTS,
      "custom",
      "none",
    ]);
    expect(categoryConfig.source.values).toEqual(
      OMO_NATIVE_EVENT_SCHEMAS.session_started.reason.values,
    );
    ...
    expect(OMO_NATIVE_EVENT_SCHEMAS.session_started.timezone).toEqual({
      type: "string",
    });
  });
  ...
  test("#given static telemetry inventories #when inspected #then they and every property allowlist are frozen", () => {
    ...
    for (const properties of Object.values(OMO_NATIVE_EVENT_SCHEMAS)) {
      ...
      for (const schema of Object.values(properties)) {
        ...
        if ("values" in schema) {
          expect(Object.isFrozen(schema.values)).toBe(true);
        }
      }
    }
    expect(Object.isFrozen(OMO_NATIVE_PROPERTY_ALLOWLISTS)).toBe(true);
    for (
      const [eventName, properties] of Object.entries(
        OMO_NATIVE_PROPERTY_ALLOWLISTS,
      )
    ) {
      expect(Object.isFrozen(properties)).toBe(true);
      expect(properties.join("\n")).toBe(
        Object.keys(
          OMO_NATIVE_EVENT_SCHEMAS[
            eventName as keyof typeof OMO_NATIVE_EVENT_SCHEMAS
          ],
        ).join("\n"),
      );
    }
