...
import {
  DELEGATION_REASONING_EFFORTS,
  DELEGATION_START_REASONS,
...
export type DelegationProperties = Readonly<
  Record<string, string | number | boolean>
>;

const BUILTIN_CATEGORY_NAMES: ReadonlySet<string> = new Set(
 ...
 */
export function projectDelegationCompleted(
  input: DelegationProjectionInput,
): DelegationProperties {
  const { record } = input.edge;
  const masked = maskProviderAndModel(
    record.resolved_model?.provider ?? "",
    record.resolved_model?.model_id ?? "",
  );
  ...
  return {
    ...
    execution_mode: record.execution_mode === "process"
      ? "process"
      : "in-process",
...
function durationOf(
  record: TaskRecord,
): { readonly ms: number; readonly status: string } {
  const measured = record.run_stats?.runtime_ms;
  if (
    record.run_stats?.duration_status === "monotonic" &&
  ...
  if (!Number.isFinite(elapsed) || elapsed < 0) {
    return { ms: 0, status: "unavailable" };
  }
...
function statsStatusOf(record: TaskRecord): string {
  ...
  if (
    stats.token_status === "complete" && stats.duration_status === "monotonic"
  ) return "complete";
  return "partial";
}
...
function ownerKindOf(record: TaskRecord): string {
  ...
  if (
    "member_scoped_tool_names" in spec &&
    spec.member_scoped_tool_names !== undefined
  ) return "team_member";
  if ("member_env" in spec && spec.member_env !== undefined) {
    return "team_member";
  }
  return "plain_child";
}
