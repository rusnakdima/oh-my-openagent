...
const GENERATOR_URL = new URL(
  "../../../../../script/telemetry-schema-block.mjs",
  import.meta.url,
);
...
async function loadGenerator(): Promise<SchemaBlockGenerator> {
  ...
  if (typeof generator !== "function") {
    throw new Error(
      "Telemetry schema generator does not export generateTelemetrySchemaBlock",
    );
  }
...
function extractGeneratedBlock(document: string): string | undefined {
  ...
  const endIndex = document.indexOf(
    END_SENTINEL,
    beginIndex + BEGIN_SENTINEL.length,
  );
  if (endIndex < 0) return undefined;
  if (
    document.indexOf(BEGIN_SENTINEL, beginIndex + BEGIN_SENTINEL.length) >= 0
  ) return undefined;
  if (document.indexOf(END_SENTINEL, endIndex + END_SENTINEL.length) >= 0) {
    return undefined;
  }
