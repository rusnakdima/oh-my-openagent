...
export const OMO_NATIVE_POSTHOG_API_KEY =
  "phc_r6UYQzNZcGYSzKw4PxCiVrZepGqV3dw9qcvcKtRNUWAn";
...
export const BUILTIN_CATEGORY_NAMES = Object.freeze(
  BUILTIN_CATEGORY_DEFAULTS.map(({ name }) => name),
);
...
export const OMO_NATIVE_EVENT_SCHEMAS = Object.freeze(
  {
    ...
    session_started: Object.freeze({
      ...
      default_model: enumProperty(
        [...new Set(Object.values(KNOWN_MODELS).flat()), "custom"] as const,
      ),
      default_provider: enumProperty([...KNOWN_PROVIDERS, "custom"] as const),
      memory_bucket: enumProperty(
        ["lt_8_gb", "8_15_gb", "16_31_gb", "32_63_gb", "64_plus_gb"] as const,
      ),
      model_count: NUMBER_PROPERTY,
      provider_count: NUMBER_PROPERTY,
      providers: STRING_PROPERTY,
      reason: enumProperty(
        ["startup", "reload", "new", "resume", "fork"] as const,
      ),
    ...
    prompt_submitted: Object.freeze({
      ...
      invocation_stage: enumProperty(
        ["none", "first_arm", "remention", "post_compact_rearm"] as const,
      ),
      ...
      keyword_occurrence_bucket: enumProperty(
        ["1", "2", "3_5", "6_plus"] as const,
      ),
      keyword_ultrawork_full: BOOLEAN_PROPERTY,
      keyword_ulw_abbrev: BOOLEAN_PROPERTY,
      keyword_variant: enumProperty(
        ["none", "ulw", "ultrawork", "both"] as const,
      ),
      prompt_length_bucket: enumProperty(
        ["lt_100", "100_500", "500_2000", "gte_2000"] as const,
      ),
      queue_mode: enumProperty(
        ["immediate", "follow_up", "steer", "other"] as const,
      ),
      real_prompt_ordinal_bucket: enumProperty(
        ["1", "2_3", "4_10", "11_25", "26_plus"] as const,
      ),
    ...
    delegation_started: Object.freeze({
      ...
      name: enumProperty(
        [...BUILTIN_CATEGORY_NAMES, ...CURATED_AGENTS, "custom"] as const,
      ),
    }),
    feature_used: Object.freeze({
      "$session_id": STRING_PROPERTY,
      feature: enumProperty(
        ["goal_tool", "team_create", "memory_tool"] as const,
      ),
    }),
...
export const OMO_NATIVE_PROPERTY_ALLOWLISTS = Object.freeze(
  Object.fromEntries(
    ...
    readonly [EventName in keyof typeof OMO_NATIVE_EVENT_SCHEMAS]:
      readonly (keyof typeof OMO_NATIVE_EVENT_SCHEMAS[EventName] & string)[];
  },
);
...
export function maskProviderAndModel(
  provider: string,
  modelId: string,
): { provider: string; model_id: string } {
  const knownProvider = isKnownProvider(provider);
  const knownModels: readonly string[] | undefined = knownProvider
