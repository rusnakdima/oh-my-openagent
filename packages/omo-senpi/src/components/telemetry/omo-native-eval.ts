...
export const REJECTED_EVAL_EXECUTION_ROLLUP: EvalExecutionRollup = Object
  .freeze({
...
export function parseEvalExecutionEvent(
  value: unknown,
): EvalExecutionParseResult {
  ...
  if (
    ...
    (kernelDurationMs !== undefined &&
      finiteNonnegative(kernelDurationMs) === undefined) ||
  ...
  const overflow = overflowValue === undefined
    ? undefined
    : parseToolAggregate(overflowValue);
  if (
...
export function addEvalExecutionRollup(
  ...
  return {
    ...
    measuredExecutionDurationMsSum: left.measuredExecutionDurationMsSum +
      right.measuredExecutionDurationMsSum,
    nestedToolCallCount: left.nestedToolCallCount + right.nestedToolCallCount,
    nestedToolCallOkCount: left.nestedToolCallOkCount +
      right.nestedToolCallOkCount,
    nestedToolCallErrorCount: left.nestedToolCallErrorCount +
      right.nestedToolCallErrorCount,
    nestedToolCallPendingCount: left.nestedToolCallPendingCount +
      right.nestedToolCallPendingCount,
    measuredNestedToolDurationMsSum: left.measuredNestedToolDurationMsSum +
      right.measuredNestedToolDurationMsSum,
    truncatedExecutionCount: left.truncatedExecutionCount +
      right.truncatedExecutionCount,
  };
}
