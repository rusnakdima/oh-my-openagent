...
describe("omo-senpi telemetry daily activity", () => {
  ...
  it.each([
    ...
    ["product anonymous telemetry opt-out", {
      OMO_SENPI_SEND_ANONYMOUS_TELEMETRY: "0",
    }],
    ["global opt-out", { OMO_DISABLE_POSTHOG: "1" }],
  ])(
    "#given %s #when session_start fires #then opt-out suppresses emission and stamp writes",
    async (_name, optOutEnv) => {
      await withTempAgentDir(async (agentDir) => {
        // given
        const transport = createTransportRecorder();
        const env = {
        ...
        expect(existsSync(getTelemetryActivityStateFilePath(stateDir))).toBe(
          false,
        );
      });
    },
