...
export function createParallelTelemetryRegistry(): ParallelTelemetryRegistry {
  ...
  return {
    record: (sessionId, observation) => {
      const state = observation.kind === "start"
        ? ensure(sessionId)
        : sessions.get(sessionId);
      state?.observations.push(observation);
      if (
        observation.kind === "start" && isEvalToolName(observation.toolName)
      ) {
...
export function registerOmoNativeParallelTelemetry(
  ...
  pi.events?.on(
    SENPI_EVAL_EXECUTION_EVENT,
    (payload) => registry.recordEvalExecution(payload),
  );

  pi.on(
    "tool_execution_start",
    (payload: unknown, eventContext: unknown): void => {
      const atMs = now();
      recordExecution(
        registry,
        "start",
