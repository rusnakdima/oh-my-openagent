...
type PromptProperty =
  (typeof OMO_NATIVE_PROPERTY_ALLOWLISTS.prompt_submitted)[number];
...
export function createOmoNativePromptComponent(
  options: OmoNativePromptComponentOptions,
): OmoSenpiComponent {
  return {
    name: "telemetry",
    register(pi: SenpiExtensionAPI, _ctx: ComponentContext): void {
      ...
      pi.on(
        "input",
        (payload: unknown, eventCtx: unknown): { action: "continue" } => {
          const input = parseInput(payload);
          if (
            input === undefined || completed.has(input.inputId) ||
            pending.has(input.inputId)
          ) {
          ...
          const ordinal = nextRealPromptOrdinal(
            realPromptOrdinals,
            sessionId,
            input.source,
          );
      ...
      const emit = (
        ...
        options.client.captureEvent(
          "prompt_submitted",
          properties(prompt, disposition),
        );
      };
    },
  };
...
function parseInput(value: unknown): {
  ...
  if (
    ...
    typeof text !== "string" || !isInputSource(source)
  ) return undefined;
...
function parseDisposition(
  value: unknown,
): { inputId: string; disposition: Disposition } | undefined {
  if (!isRecord(value) || value["type"] !== "input_disposition") {
    return undefined;
  }
  const inputId = value["inputId"];
  const disposition = value["disposition"];
  if (typeof inputId !== "string" || !isDisposition(disposition)) {
    return undefined;
  }
  return { inputId, disposition };
}

function nextRealPromptOrdinal(
...
function keywordVariant(classification: UltraworkClassification): string {
  if (classification.matchedUlw && classification.matchedUltrawork) {
    return "both";
  }
...
function isDisposition(value: unknown): value is Disposition {
  return value === "queued" || value === "started" || value === "handled" ||
    value === "rejected";
}
