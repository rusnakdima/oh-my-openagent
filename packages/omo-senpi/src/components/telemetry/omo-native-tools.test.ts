...
function eventProperties(
  ...
  return events.filter((event) => event.name === name).map((event) =>
    event.properties
  );
}
...
describe("OmO Native tool telemetry", () => {
  test("#given a builtin SKILL.md #when a namespaced Read result arrives #then the exact builtin skill is emitted", async () => {
    ...
    await pi.dispatch(
      "tool_result",
      result("builtin:Read", { path }),
      context(),
    );

    expect(events).toEqual([{
      name: "skill_loaded",
      properties: { $session_id: "hashed:session-a", skill_name: "ulw-plan" },
    }]);
  });
  ...
  test("#given outside, custom, traversal, symlink, and injected skill paths #when read results arrive #then no skill name is emitted", async () => {
    ...
    for (
      const path of [
        ...
        custom,
        injected,
        join(skillsRoot, "..", "..", "etc", "passwd"),
        join(symlinkPath, "SKILL.md"),
      ]
    ) {
  ...
  test("#given canonical Windows paths outside the builtin root #when matched #then no skill name is emitted", () => {
    ...
    for (
      ...
      expect(builtinSkillNameFromCanonicalPaths(root, path, win32))
        .toBeUndefined();
    }
  });
  ...
  test("#given subagent and unknown category spawns #when task completes #then allowlisted names pass and unknown names are custom", async () => {
    ...
    await pi.dispatch(
      "tool_result",
      result("task", { prompt: "scan", subagent_type: "explore" }),
      context(),
    );
    await pi.dispatch(
      "tool_result",
      result("task", { prompt: "invent", category: "deep-injected" }),
      context(),
    );
  ...
  test("#given tool_result without input #when handled #then it is ignored with a diagnostic", async () => {
    ...
    await expect(pi.dispatch("tool_result", result("read"), context())).resolves
      .toEqual([undefined]);
