...
type SpawnTarget = {
  readonly kind: "category" | "subagent";
  readonly name: string;
};

type ToolTelemetryOptions = {
  readonly captureEvent: (
    name: OmoNativeEventName,
...
export function registerOmoNativeToolTelemetry(
  ...
  pi.on("tool_result", (payload: unknown, eventContext: unknown): void => {
    ...
    if (matchesToolName(event.toolName, "read") && !event.isError) {
      const skillName = builtinSkillName(
        event.input.path,
        eventContext,
        skillsRoot,
      );
      if (skillName !== undefined) {
        options.captureEvent("skill_loaded", {
          ...shared,
    ...
    if (matchesToolName(event.toolName, "task") && !event.isError) {
      ...
      const batchSizeBucket = bucketBatchSize(
        Array.isArray(event.input.tasks) ? event.input.tasks.length : 1,
      );
      const background = event.input.run_in_background === true;
      for (const target of targets) {
        const allowlist = target.kind === "category"
          ? BUILTIN_CATEGORIES
          : BUILTIN_AGENTS;
...
function readToolResult(
  ...
  if (
    !isRecord(value) || value.type !== "tool_result" ||
    typeof value.toolName !== "string"
  ) return undefined;
  if (!isRecord(value.input)) {
    diagnostics?.("omo-native tool_result missing input", {
...
function builtinSkillName(
  ...
  try {
    ...
    const cwd = isRecord(eventContext) && typeof eventContext.cwd === "string"
      ? eventContext.cwd
      : process.cwd();
    const target = realpathSync(resolve(cwd, rawPath));
    return builtinSkillNameFromCanonicalPaths(root, target, {
      isAbsolute,
      relative,
      sep,
    });
...
export function builtinSkillNameFromCanonicalPaths(
  ...
  const comparableTarget = windows
    ? windowsPathWithoutNamespace(target)
    : target;
  const pathFromRoot = path.relative(comparableRoot, comparableTarget);
  if (
    pathFromRoot === "" || pathFromRoot.startsWith(`..${path.sep}`) ||
    pathFromRoot === ".." || path.isAbsolute(pathFromRoot)
  ) return undefined;
  ...
  return fileName === "SKILL.md" && BUILTIN_SKILLS.has(skillName)
    ? skillName
    : undefined;
}
...
function spawnTarget(
  ...
  const category = itemCategory ??
    (itemSubagent === undefined ? identifier(parent.category) : undefined);
  const subagent = itemSubagent ??
    (itemCategory === undefined ? identifier(parent.subagent_type) : undefined);
...
function featureForTool(
  toolName: string,
): "goal_tool" | "team_create" | "memory_tool" | undefined {
  if (matchesToolName(toolName, "create_goal")) return "goal_tool";
  if (matchesToolName(toolName, "team_create")) return "team_create";
  if (
    matchesToolName(toolName, "memory_apply_patch") ||
    matchesToolName(toolName, "memory")
  ) return "memory_tool";
  return undefined;
}
...
function matchesToolName(toolName: string, expected: string): boolean {
  ...
  return normalized === suffix || normalized.endsWith(`_${suffix}`) ||
    normalized.endsWith(`:${suffix}`) || normalized.endsWith(`/${suffix}`);
}
