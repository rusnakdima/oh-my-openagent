...
import {
  type OmoNativeInventoryDiagnostic,
  readOmoNativeInventory,
...
export type OmoNativeSessionOptions = {
  readonly diagnostics?: (
    input: TelemetryDiagnosticInput | OmoNativeInventoryDiagnostic,
  ) => void;
  readonly env?: TelemetryEnv;
  readonly hashSessionId?: (rawId: string) => string;
  readonly isConfigEnabled?: (
...
export function createOmoNativeSessionComponent(
  ...
  return {
    ...
    register(pi, ctx) {
      pi.on("session_start", (payload, eventCtx) => {
        ...
        if (
          !isTelemetryClientEnabled({ env, product }) ||
          !configEnabled(options, eventCtx, env)
        ) return;

        const osProvider = options.osProvider ??
          getDefaultTelemetryOsProvider();
        ...
        const inventory = readOmoNativeInventory(
          resolveAgentHome({ env }),
          options.diagnostics,
        );
        ...
        client.captureEvent("session_started", {
          ...
          ...(inventory.defaultProvider === undefined
            ? {}
            : { default_provider: inventory.defaultProvider }),
          ...(inventory.defaultModel === undefined
            ? {}
            : { default_model: inventory.defaultModel }),
        });
...
function configEnabled(
  ...
  const cwd = extractString(eventCtx, "cwd");
  if (options.isConfigEnabled !== undefined) {
    return options.isConfigEnabled(cwd, env);
  }
  try {
...
function sessionReason(payload: unknown): string {
  ...
  return reason !== undefined && SESSION_REASONS.has(reason)
    ? reason
    : "startup";
}
