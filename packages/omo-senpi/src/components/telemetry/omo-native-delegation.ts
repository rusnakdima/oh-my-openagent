 ...
 */
export function createOmoNativeDelegationCapture(
...
function capture(
  ...
  if (record.task_seq !== undefined) {
    priorStatus.set(record.task_seq, record.status);
  }
}
...
function startReasonOf(
  ...
  if (epoch === 0) {
    return record.owner?.kind === "dag" ? "dag_retry" : "initial_spawn";
  }
  const revived = events.some((event) =>
    event.type === "revived" && event.runEpoch === epoch
  );
  if (!revived) return record.owner?.kind === "dag" ? "dag_retry" : "unknown";
  const previous = record.task_seq === undefined
    ? undefined
    : priorStatus.get(record.task_seq);
  switch (previous) {
    case "completed":
 ...
 */
function countSteers(
  events: readonly PersistedEventLine[],
  runEpoch: number,
): DelegationSteerCounts {
  let running = 0;
  let queued = 0;
...
function readEventLogText(path: string): string | undefined {
  try {
    ...
    return text.length > MAX_EVENT_LOG_BYTES
      ? text.slice(-MAX_EVENT_LOG_BYTES)
      : text;
  } catch {
    return undefined;
  }
...
function parseEventLine(line: string): PersistedEventLine | undefined {
  try {
    ...
    const runEpoch = isRecord(payload) && typeof payload.run_epoch === "number"
      ? payload.run_epoch
      : undefined;
