...
function createPi(
  events?: FakeEvents,
): SenpiExtensionAPI & { dispatch(name: string, payload?: unknown): void } {
...
function createComponent() {
  return createConfigWatchComponent({
    ...
    resolveTargets: () => [{
      path: "/project/.omo",
      kind: "dir",
      filterGlobs: ["omo.jsonc", "omo.json"],
    }],
...
describe("createConfigWatchComponent", () => {
  ...
  it("skips without the optional events capability", () => {
    ...
    expect(logs).toEqual([{
      level: "warn",
      message: "config-watch skipped: events capability missing",
      details: undefined,
    }]);
  });
  ...
  it("logs omo reload and rejection outcomes with paths and errors", () => {
    ...
    events.emit("config-watch:reloaded", {
      registrationId: "omo",
    ...
    expect(logs).toEqual([
      {
        level: "info",
        message: "omo config hot-reloaded",
        details: { paths: ["/project/.omo/omo.jsonc"], pathCount: 1 },
      },
      {
        level: "warn",
  ...
  it(
    "refreshes targets after rejection without replacing the sticky validator, deferring the re-registration",
    async () => {
      const events = new FakeEvents();
      const pi = createPi(events);
      const validate = () => ({
  ...
  it(
    "caps deferred re-registration retries when the host rejects deterministically",
    async () => {
      const events = new FakeEvents();
      const logs: Array<{ level: string; message: string; details?: unknown }> =
        [];
      ...
      expect(
        logs.filter((entry) =>
          entry.message === "omo config hot-reload retry budget exhausted"
        ),
      ).toHaveLength(1);
    },
    10_000,
  ...
  it(
    "resets the rejection retry budget when the registration payload changes",
    async () => {
      const events = new FakeEvents();
      let version = 1;
      createConfigWatchComponent({
      ...
      expect(events.registrations.get("omo")?.targets[0]?.path).toBe(
        "/project/v2",
      );
    },
    10_000,
