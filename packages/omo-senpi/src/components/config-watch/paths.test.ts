...
function fixtureEnv(
  fixture: Fixture,
): { HOME: string; XDG_CONFIG_HOME: string; SENPI_CODING_AGENT_DIR: string } {
  return {
...
function targetFor(
  ...
  return paths.some((target) =>
    target.path === path && target.filterGlobs.includes(filter)
  );
}
...
describe("resolveOmoConfigWatchTargets", () => {
  it("#given nested project configs #when resolving targets #then watches user scope, every existing .omo directory, and each cwd-to-home ancestor", () => {
    ...
    expect(resolveUserOmoConfigDirectory(fixtureEnv(fixture))).toBe(
      userConfigDirectory,
    );
    expect(findProjectConfigPathsFarthestFirst(fixture.cwd, fixture.homeDir, {
      existsSync: (path) =>
        [workConfigPath, projectConfigPath, cwdConfigPath].includes(path),
      readFileSync: () => "",
    ...
    expect(targetFor(targets, join(fixture.workDir, ".omo"), "/omo.json")).toBe(
      true,
    );
    expect(targetFor(targets, join(fixture.projectDir, ".omo"), "/omo.jsonc"))
      .toBe(true);
    expect(targetFor(targets, join(fixture.cwd, ".omo"), "/omo.jsonc")).toBe(
      true,
    );
    expect(
      targets.filter((target) => target.filterGlobs.includes("/.omo")).map((
        target,
      ) => target.path),
    ).toEqual([
      fixture.cwd,
  ...
  it("#given a newly created ancestor .omo directory #when resolving targets #then keeps watching its config files after a rejected creation", () => {
    ...
    const creationTarget = targets.find(
      (target) =>
        target.path === fixture.projectDir &&
        target.filterGlobs.includes("/.omo"),
    );
    ...
    expect(creationTarget?.filterGlobs).toEqual([
      "/.omo",
      "/.omo/omo.jsonc",
      "/.omo/omo.json",
    ]);
  ...
  it("#given an existing ~/.omo user config directory #when resolving targets #then it is watched for both config filenames", () => {
    ...
    expect(targetFor(resolution.targets, userConfigDirectory, "/omo.jsonc"))
      .toBe(true);
    expect(targetFor(resolution.targets, userConfigDirectory, "/omo.json"))
      .toBe(true);
  });
  ...
  it("#given ancestor creation targets #when resolving targets #then every creation glob is root-anchored", () => {
    ...
    const creationTargets = targets.filter((target) =>
      target.filterGlobs.some((glob) => glob.endsWith(".omo"))
    );
    expect(creationTargets.length).toBeGreaterThan(0);
    for (const target of creationTargets) {
      expect(target.filterGlobs).toEqual([
        "/.omo",
        "/.omo/omo.jsonc",
        "/.omo/omo.json",
      ]);
  ...
  it("#given the user config parent creation target #when resolving targets #then its glob is root-anchored", () => {
    ...
    const userCreationTarget = targets.find((target) =>
      target.path === fixture.homeDir && target.filterGlobs.includes("/omo")
    );
    expect(userCreationTarget?.filterGlobs).toEqual(["/omo"]);
  });
  ...
  it("#given an existing .omo directory holding runtime state #when resolving targets #then its config globs are root-anchored so the subtree is not scanned", () => {
    ...
    const omoDirectory = join(fixture.projectDir, ".omo");
    mkdirSync(join(omoDirectory, "senpi-task", "children", "st_abc"), {
      recursive: true,
    });
    writeFileSync(
      join(
