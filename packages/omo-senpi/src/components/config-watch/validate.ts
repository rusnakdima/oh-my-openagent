...
export type ConfigWatchValidation = { ok: true } | {
  ok: false;
  errors: string[];
};
...
function containingConfigDirectory(
  path: string,
  userConfigDirectory: string,
): string | null {
  const resolvedPath = resolve(path);
  const resolvedUserConfigDirectory = resolve(userConfigDirectory);
  if (isEqualToOrDescendantOf(resolvedPath, resolvedUserConfigDirectory)) {
    return resolvedUserConfigDirectory;
  }
...
function isAttributableDiagnostic(
  ...
  if (
    diagnostic.path === MERGED_OMO_CONFIG_DIAGNOSTIC_PATH ||
    diagnostic.kind === "model_catalog_cycle"
  ) return true;

  const diagnosticConfigDirectory = containingConfigDirectory(
    diagnostic.path,
    userConfigDirectory,
  );
  return changedPaths.some((changedPath) => {
    ...
    return containingConfigDirectory(changedPath, userConfigDirectory) ===
      diagnosticConfigDirectory;
  });
}
 ...
 */
export function createOmoConfigValidator(
  options: CreateOmoConfigValidatorOptions,
): OmoConfigValidator {
  ...
  let baseline = new Set(
    loadConfig({ cwd: options.cwd, env, platform }).diagnostics.map(
  ...
  return {
    validate(changedPaths: readonly string[]): ConfigWatchValidation {
      const diagnostics =
        loadConfig({ cwd: options.cwd, env, platform }).diagnostics;
      const fingerprintedDiagnostics = diagnostics.map(
        (diagnostic): FingerprintedDiagnostic => ({
          diagnostic,
          fingerprint: fingerprintDiagnostic(diagnostic),
        }),
      );
      const diagnosticsByFingerprint = new Map(
        fingerprintedDiagnostics.map((
          entry,
        ) => [entry.fingerprint, entry.diagnostic]),
      );

      for (const fingerprint of unresolvedRejected) {
        if (!diagnosticsByFingerprint.has(fingerprint)) {
          unresolvedRejected.delete(fingerprint);
        }
      }
      ...
      if (unresolvedRejected.size > 0) {
        const errors = [...unresolvedRejected]
          ...
          .filter((diagnostic): diagnostic is SenpiConfigDiagnostic =>
      ...
      baseline = new Set(
        fingerprintedDiagnostics.map((entry) => entry.fingerprint),
      );
      return { ok: true };
    },
  };
