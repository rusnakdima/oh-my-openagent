...
export const OMO_CONFIG_FILE_FILTER_GLOBS = [
  "/omo.jsonc",
  "/omo.json",
] as const;
...
export const OMO_CONFIG_DIRECTORY_FILTER_GLOBS = [
  "/.omo",
  "/.omo/omo.jsonc",
  "/.omo/omo.json",
] as const;
...
function isSenpiRestrictedTarget(
  path: string,
...
function findAncestorDirectories(
  ...
  const stopDir = containsPath(resolvedHomeDir, startDir)
    ? resolvedHomeDir
    : null;
...
function creationTarget(path: string): OmoConfigWatchTarget {
  return {
    path,
    kind: "dir",
    filterGlobs: [...OMO_CONFIG_DIRECTORY_FILTER_GLOBS],
  };
}
...
function userConfigCreationTarget(path: string): OmoConfigWatchTarget {
  return {
    path,
    kind: "dir",
    filterGlobs: [...USER_OMO_CONFIG_DIRECTORY_FILTER_GLOBS],
  };
}
...
export function resolveOmoConfigWatchTargetResolution(
  ...
  const ancestorDirectories = findAncestorDirectories(
    options.cwd,
    resolveHomeDir(env),
  );
  const resolvedConfigPaths = resolveOmoConfigPaths({
    cwd: options.cwd,
    env,
    platform,
  });
  ...
  if (isExistingDirectory(userConfigDirectory)) {
    ...
    if (isExistingDirectory(userConfigParent)) {
      targets.push(userConfigCreationTarget(userConfigParent));
    }
  }
  ...
  for (const ancestorDirectory of ancestorDirectories) {
    ...
    if (
      configuredProjectDirectories.has(omoDirectory) ||
      isExistingNonSymlinkDirectory(omoDirectory)
    ) {
  ...
  for (const ancestorDirectory of ancestorDirectories) {
    targets.push(creationTarget(ancestorDirectory));
  }

  const senpiProtectedPaths = resolveSenpiProtectedPaths(env);
  const permittedTargets = targets.filter((target) =>
    !isSenpiRestrictedTarget(target.path, senpiProtectedPaths)
  );
  ...
  const userConfigCreationWatched = permittedTargets.some(
    (target) =>
      target.path === userConfigDirectory ||
      target.path === dirname(userConfigDirectory),
  );
  return {
    targets: permittedTargets,
    userConfigCreationWatched,
    userConfigCreationDiscovery: userConfigCreationWatched
      ? "watched"
      : "reload_required",
  };
}
