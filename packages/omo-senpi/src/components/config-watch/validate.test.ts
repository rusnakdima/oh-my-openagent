...
function configPath(
  directory: string,
  fileName: "omo.jsonc" | "omo.json" = "omo.jsonc",
): string {
...
function rejectedErrors(
  validation: ReturnType<
    ReturnType<typeof createOmoConfigValidator>["validate"]
  >,
): string[] {
...
describe("createOmoConfigValidator", () => {
  ...
  it("#given a missing optional user config #when validating its path #then accepts", () => {
    ...
    expect(
      validator.validate([join(fixture.xdgConfigHome, "omo", "omo.jsonc")]),
    ).toEqual({ ok: true });
  });
  ...
  it("#given a changed Senpi model catalog cycle #when validating #then rejects the resolved Senpi view diagnostic", () => {
    ...
    writeConfig(
      changedPath,
      '{"models":{"loop":{"model":"loop"}},"[senpi]":{"categories":{"quick":{"model":"loop"}}}}',
    );
    ...
    expect(validation.ok).toBe(false);
    expect(rejectedErrors(validation).join("\n")).toContain(
      'Model catalog entry "loop" references itself',
    );
  });
