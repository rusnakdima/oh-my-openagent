...
export interface ConfigWatchComponentOptions {
  ...
  readonly resolveTargetResolution?: (
    options: { readonly cwd: string },
...
function isReloaded(value: unknown): value is ConfigWatchReloaded {
  return isRecord(value) && typeof value.registrationId === "string" &&
...
export function createConfigWatchComponent(
  options: ConfigWatchComponentOptions = {},
): OmoSenpiComponent {
  ...
  const resolveTargetResolution = options.resolveTargetResolution ??
    ((request: { readonly cwd: string }): OmoConfigWatchTargetResolution => {
      if (options.resolveTargets === undefined) {
        return resolveOmoConfigWatchTargetResolution(request);
      }
      return {
  ...
  return {
    ...
    register(pi: SenpiExtensionAPI, ctx: ComponentContext): void {
      ...
      const createRegistration = (): ConfigWatchRegistration => {
        ...
        if (
          resolution.userConfigCreationDiscovery === "reload_required" &&
          !userConfigReloadWarningLogged
        ) {
          userConfigReloadWarningLogged = true;
          ctx.logger.warn(
            "config-watch user config discovery requires reload",
            {
              userConfigCreationDiscovery:
                resolution.userConfigCreationDiscovery,
            },
          );
        }
        return {
      ...
      const unsubscribes = [
        ...
        events.on(CONFIG_WATCH_RELOADED, (payload) => {
          if (
            !isReloaded(payload) ||
            payload.registrationId !== OMO_REGISTRATION_ID
          ) return;
          ctx.logger.info("omo config hot-reloaded", {
            paths: payload.paths,
