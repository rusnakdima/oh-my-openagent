...
const taskTargetPattern =
  /\b(subagent_type|category)["']?\s*[=:]\s*["']([a-z0-9-]+)["']/g;
...
function listDirectoryNames(path: string): string[] {
  if (!existsSync(path)) {
    throw new Error(
      ...
      } does not exist; run packages/omo-senpi/plugin/scripts/sync-skills.mjs`,
    );
  }
...
function expectFrontmatterField(
  frontmatter: string,
  field: string,
  path: string,
): void {
  ...
  expect(
    ...
    `${relative(repoRoot, path)} frontmatter must include ${field}`,
  ).toBe(true);
}
...
describe("OMO Senpi scoped skill sync", () => {
  test("#given the telemetry builtin skill allowlist #when compared with packaged skills #then it stays exact and frozen", () => {
    ...
    expect([...telemetrySkillNames].sort()).toEqual(
      listDirectoryNames(skillsRoot),
    );
  });
  ...
  test("#given synced skill output #when inspected #then exactly 24 roots exist with valid names", () => {
    ...
    for (const skillName of expectedSkillNames) {
      ...
      expect(
        ...
        `${relative(repoRoot, skillFile)} must exist`,
      ).toBe(true);
      expect(
        statSync(skillFile).isFile(),
        `${relative(repoRoot, skillFile)} must be a file`,
      ).toBe(true);
      expect(
        namePattern.test(skillName),
        `${skillName} must match ${namePattern.source}`,
      ).toBe(true);
    }
  });
  ...
  test("#given synced skill roots #when frontmatter is parsed #then every root skill has name, description, and valid values", () => {
    for (const skillName of expectedSkillNames) {
      ...
      expect(
        ...
        } frontmatter name must equal ${skillName}`,
      ).toBe(skillName);

      const descriptionLine =
        frontmatter.match(/^description:\s*(.*)$/m)?.[0] ?? "";
      expect(
        descriptionLine.length,
        `${
          relative(repoRoot, skillFile)
        } description line must be <= 1024 chars`,
  ...
  test("#given codex-derived and native skill roots #when scanned #then no Codex or multi-agent harness guidance survives", () => {
    ...
    for (
      const skillName of [
        ...Object.keys(CODEX_DERIVED_SKILL_NAMES),
        ...Object.keys(NATIVE_SENPI_SKILL_NAMES),
      ]
    ) {
  ...
  test("#given native senpi skills #when synced output is compared #then they ship verbatim modulo blank-line normalization", () => {
    for (const skillName of Object.keys(NATIVE_SENPI_SKILL_NAMES)) {
      ...
      expect(
        ...
        `${relative(repoRoot, sourceFile)} must exist`,
      ).toBe(true);
      ...
      expect(
        shipped,
        `${
          relative(repoRoot, shippedPath)
        } must ship the native source verbatim`,
      ).toBe(source);
    }
  });
  ...
  test("#given start-work skill #when inspected #then session ids reference senpi, not codex", () => {
    ...
    expect(
      content.includes("senpi:<session_id>"),
      "start-work must reference senpi:<session_id>",
    ).toBe(true);
    expect(
      content.includes("codex:<session_id>"),
      "start-work must not reference codex:<session_id>",
    ).toBe(false);
  });
  ...
  test("#given start-work skill #when inspected #then the senpi banner advertises senpi watcher tools, not a codex wait idiom", () => {
    ...
    expect(
      /\bmonitor\b/.test(content),
      "start-work must name the senpi tool that arms a lane watcher",
    ).toBe(true);
    expect(
      /\bkill_bash\b/.test(content),
      "start-work must name the senpi tool that tears a watcher down",
    ).toBe(true);
    expect(
      /\bwait_agent\b/.test(content),
      "start-work must not carry the codex wait_agent polling idiom",
    ).toBe(false);
  });

  test("#given synced skill tree #when inspected #then no codex-only display metadata is packaged", () => {
    const openaiFiles = listFiles(skillsRoot).filter((file) =>
  ...
  test("#given ported orchestration skills #when scanned #then no foreign-harness delegation guidance survives", () => {
    ...
    const foreignDelegationPattern =
      /\b(?:multi_agent|spawn_agent|lazycodex)\b/i;
    const leaks: string[] = [];
    ...
    for (const skillName of portedOrchestrationSkillNames) {
      ...
      for (const file of listFiles(skillRoot)) {
        ...
        if (foreignDelegationPattern.test(content)) {
          leaks.push(
            `${relative(repoRoot, file)}: foreign delegation tool guidance`,
          );
        }
        if (skillName === "ulw-plan" && /\boracle\b/i.test(content)) {
          leaks.push(
            ...
            }: oracle reviewer does not exist in omo-senpi`,
          );
        }
      }
    }
  ...
  test("#given shipped task examples #when targets are scanned #then every agent and category exists in Senpi", () => {
    ...
    for (
      const file of listFiles(skillsRoot).filter((path) => path.endsWith(".md"))
    ) {
      const content = readFileSync(file, "utf8");
  ...
  test("#given frontend skill #when inspected #then materialized design references exist", () => {
    ...
    expect(
      existsSync(refsDir),
      "frontend/references/design must exist after materialization",
    ).toBe(true);
  });
});
