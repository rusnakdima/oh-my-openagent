...
function readObjectRecord(
  ...
  for (const [entryKey, entryValue] of Object.entries(value)) {
    if (!isJsonObject(entryValue)) {
      throw new Error(`${key}.${entryKey} must be an object`);
    }
    result[entryKey] = entryValue;
  }
  return result;
...
describe("omo-senpi package shape", () => {
  test("#given the senpi adapter manifest #when audited #then it declares the package contract", async () => {
    ...
    const peerDependenciesMeta = readObjectRecord(
      manifest,
    ...
    expect(readString(manifest, "version")).toBe(
      readString(rootManifest, "version"),
    );
    ...
    expect(peerDependenciesMeta["@code-yeongyu/senpi"]).toMatchObject({
      optional: true,
    });
    ...
    expect(Object.keys(readStringRecord(rootManifest, "dependencies"))).not
      .toContain(["vscode", "jsonrpc"].join("-"));
  });
  ...
  test("#given root workspace metadata #when audited #then the senpi adapter is registered", async () => {
    ...
    const typecheckPackages = readString(
      readStringRecord(rootManifest, "scripts"),
      "typecheck:packages",
    );
    ...
    expect(typecheckPackages).toContain(
