...
function createRecordingLogger(): ComponentLogger & {
...
describe("composeOmoSenpiExtension", () => {
    ...
    const components: OmoSenpiComponent[] = [
      {
        ...
        register(api, ctx) {
          ...
          api.on("session_start", async () => {
            api.sendUserMessage(
              `flag=${String(ctx.config.getFlag("omo-senpi-alpha-disabled"))}`,
              {
                deliverAs: "followUp",
              },
            );
  ...
  it("#given a component using optional registerMcpServer on an old API #when composed #then skips that component and registers later components", async () => {
    // given
    const pi: Omit<FakeExtensionAPI, "registerMcpServer"> & {
    ...
    // when
    await composeOmoSenpiExtension(components, { logger })(
      pi as unknown as FakeExtensionAPI,
    );
