...
function resolveDefaultBaseDir(importerUrl: string = import.meta.url): string {
  ...
  const sourceTreeDir = fileURLToPath(
    new URL("../../plugin/runtime/dag", importerUrl),
  );
  return existsSync(sourceTreeDir) ? sourceTreeDir : packagedDir;
}

export function createDagSdkRootProvisioning(
  options: DagSdkRootProvisioningOptions = {},
): () => void {
