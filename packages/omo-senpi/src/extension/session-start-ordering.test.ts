...
import {
  ...
  processStartTime,
} from "../components/init-deep-advisor";
import {
  makeCoverageRepo,
  resetTestHome,
  setTestHome,
...
import {
  ...
  createLogger,
  sessionEventCtx,
} from "../components/ulw-loop/ulw-loop.test-support";
...
describe("session_start component ordering", () => {
  ...
  test("#given real onboarding state and an active ulw loop #when startup then agent_end fire #then onboarding is preserved and the first-session advisor stays detached and suppressed", async () => {
    ...
    for (const component of components) {
      await component.register(pi, componentContext);
    }
    ...
    const markerPath = join(
      getOmoNativeStateDir(process.env),
      "onboarding-completed",
    );
    expect(statSync(markerPath).mtimeMs).toBeGreaterThanOrEqual(
      processStartTime,
    );
