...
const NIGHTLY = {
  ...
  nodes: [
    {
      id: "audit",
      prompt: "Audit docs for {{key}} on {{date}}",
      category: "explore",
    },
    {
      id: "verify",
      prompt: "Verify at {{datetime}}",
      category: "quick",
      dependsOn: ["audit"],
    },
  ],
...
function installGlobals(
  ...
  reply: (args: DagCall) => unknown = () => ({
    details: { kind: "started", run_id: "run-42" },
  }),
): DagCall[] {
  const calls: DagCall[] = [];
  Reflect.set(
    globalThis,
    "read",
    async (path: string) => readFileSync(path, "utf8"),
  );
  Reflect.set(
    globalThis,
    "env",
    (key?: string) => (key === undefined ? envMap : envMap[key]),
  );
...
describe("dag definition library", () => {
  ...
  describe("#given a stored definition in OMO_DAG_LIBRARY", () => {
    it("#then load rotates the key with an explicit suffix and fills {{key}} and {{date}} placeholders", async () => {
      ...
      >;
      expect(nodes[0]!.prompt).toMatch(
        /^Audit docs for nightly-audit-20260818 on \d{8}$/,
      );
    ...
    it("#then load with an empty suffix keeps the stored key so the run stays idempotent", async () => {
      ...
      expect((await library.load("nightly", { suffix: "" })).key).toBe(
        "nightly-audit",
      );
    });
  });
  ...
  describe("#given definitions scattered across the search dirs", () => {
    it("#then OMO_DAG_LIBRARY shadows $PWD/.omo/dags which shadows $HOME/.omo/dags", async () => {
      const base = {
        key: "k",
        nodes: [{ id: "a", prompt: "p", category: "quick" }],
      };
      await writeFile(
        join(homeDags, "only-home.json"),
        JSON.stringify({ ...base, name: "from-home" }),
      );
      await writeFile(
        join(pwdDags, "shadowed.json"),
        JSON.stringify({ ...base, name: "from-pwd" }),
      );
      await writeFile(
        join(libDir, "shadowed.json"),
        JSON.stringify({ ...base, name: "from-lib" }),
      );
      installGlobals(envMap);
      const library = await loadLibrary();

      expect((await library.load("only-home", { suffix: "" })).name).toBe(
        "from-home",
      );
      expect((await library.load("shadowed", { suffix: "" })).name).toBe(
        "from-lib",
      );
    });
    ...
    it("#then on win32 the library env splits on semicolons so drive-letter paths survive", async () => {
      ...
      try {
        ...
        expect((await library.load("nightly", { suffix: "" })).name).toBe(
          "Nightly audit",
        );
      } finally {
    ...
    it("#then a definition without a nodes array fails before any host round-trip", async () => {
      await writeFile(
        join(libDir, "broken.json"),
        JSON.stringify({ key: "broken" }),
      );
      installGlobals(envMap);
      const library = await loadLibrary();
  ...
  describe("#given a stubbed tool.dag", () => {
    it("#then start loads, rotates, and starts the run in one call and the handle wires done/cancel", async () => {
      ...
      const started = calls[0]!.definition as {
        key: string;
  ...
  describe("#given the dag tool refuses the start with a details.kind=error envelope", () => {
    it("#then start throws the tool's own code and human message", async () => {
      const conflict =
        'dag run key "nightly-audit-t1" already exists with a different definition';
      await writeFile(join(libDir, "nightly.json"), JSON.stringify(NIGHTLY));
      installGlobals(envMap, () => ({
      ...
      const rejection =
        expect(library.start("nightly", { suffix: "t1" })).rejects;
