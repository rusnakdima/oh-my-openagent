...
} from "../components/memory/nudge-wiring";
...
describe("omo-memory MCP server", () => {
  ...
  test("#given tools/list #then exactly the two memory tools are exposed", async () => {
    ...
    const tools =
      (result?.result as { tools?: { name: string }[] } | undefined)?.tools ??
        [];
  ...
  test("#given a fresh project #when create then str_replace run through tools/call #then the memory repo records them", async () => {
    ...
    const created = await handleMemoryMcpRequest(
      {
        ...
        params: {
          ...
          arguments: {
            command: "create",
            reason: "Record preference",
            file_path: "system/human/preferences.md",
            description: "User preferences",
            file_text: "theme: dark",
          },
        },
      },
      { cwd, env },
    );
    expect((created?.result as { isError?: boolean } | undefined)?.isError)
      .toBeFalsy();
    ...
    expect((replaced?.result as { isError?: boolean } | undefined)?.isError)
      .toBeFalsy();
    ...
    const profile = parseMemoryFile(
      readFileSync(join(repoDir, "system/human/preferences.md"), "utf8"),
    );
    const repo = new GitMemoryRepo({
      dir: repoDir,
      agentId: String(identityDir),
    });
    expect(profile.frontmatter.description).toBe("User preferences");
    expect(profile.body.trim()).toBe("theme: light");
    expect((await repo.log({ limit: 2 })).map((entry) => entry.subject))
      .toEqual(["Switch theme", "Record preference"]);
  });
  ...
  test("#given injected provenance for a non-auto identity #when an MCP memory call runs #then it writes that bound repo with durable turn trailers", async () => {
    ...
    // then
    expect((result?.result as { isError?: boolean } | undefined)?.isError)
      .toBeFalsy();
    ...
    expect(await nudge.nudgeTurns(repo, "session-mcp", identityId))
      .toBeUndefined();
    expect(readdirSync(join(String(env.OMO_MEMORY_HOME), "agents"))).toEqual([
      identityId,
    ]);
  });
  ...
  test("#given injected provenance with a toolCallId #when an MCP memory call commits #then an out-of-band receipt lands under runtime/tool-receipts", async () => {
    ...
    expect((result?.result as { isError?: boolean } | undefined)?.isError)
      .toBeFalsy();
    const key = createHash("sha256").update("call-receipt-1").digest("hex")
      .slice(0, 32);
    const receipt = JSON.parse(
      readFileSync(join(paths.toolReceipts, `${key}.json`), "utf8"),
  ...
  test("#given no injected toolCallId #when an MCP memory call commits #then no receipt is written", async () => {
    ...
    const created = await handleMemoryMcpRequest(
      {
        ...
        params: {
          ...
          arguments: {
            command: "create",
            reason: "standalone write",
            file_path: "notes.md",
            description: "Note",
            file_text: "standalone",
          },
        },
      },
      { cwd, env },
    ...
    expect(parseMemoryFile(await repo.show("HEAD", "notes.md")).body.trim())
      .toBe("standalone");
  ...
  test("#given a failing MCP memory call with a toolCallId #when the call errors #then no receipt is written", async () => {
    ...
    // then
    expect((result?.result as { isError?: boolean } | undefined)?.isError).toBe(
      true,
    );
    const key = createHash("sha256").update("call-failed-1").digest("hex")
      .slice(0, 32);
