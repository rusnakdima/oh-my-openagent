    ...
    const identity = provenance === undefined
      ...
      : {
        ...
        paths: buildIdentityPaths(
          dirname(dirname(dirname(provenance.repoPath))),
...
function readMcpProvenance(
  args: Record<string, unknown>,
): McpMemoryProvenance | undefined {
...
async function writeCommitReceipt(
  ...
  try {
    ...
    process.stderr.write(
      `omo-memory: failed to write tool receipt: ${errorMessage(error)}\n`,
    );
  }
}
