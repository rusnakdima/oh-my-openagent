...
async function settingsPackages(agentDir: string): Promise<readonly string[]> {
  ...
  if (!isRecord(parsed) || !Array.isArray(parsed.packages)) {
    throw new Error("expected settings packages");
  }
  return parsed.packages.filter((entry): entry is string =>
    typeof entry === "string"
  );
}
...
describe("runSenpiInstaller source refresh", () => {
  ...
  test("#given current and stale OMO entries #when refreshing #then the current package keeps its precedence", async () => {
    ...
    await writeFile(
      join(agentDir, "settings.json"),
      JSON.stringify({
        packages: ["before", pluginPath, stalePlugin, "after"],
      }),
    );
    ...
    // then
    expect(await settingsPackages(agentDir)).toEqual([
      "before",
      pluginPath,
      "after",
    ]);
