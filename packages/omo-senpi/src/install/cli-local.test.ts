...
async function makePackagedPlugin(): Promise<string> {
  ...
  await writeFixtureFile(
    ...
    `${
      JSON.stringify(
        {
          sha256: createHash("sha256").update(await readFile(astGrepRuntime))
            .digest("hex"),
          mode: 0o755,
...
async function buildInstaller(pluginPath: string): Promise<void> {
  ...
  if (exitCode !== 0) {
    throw new Error(`installer build failed: ${stdout}${stderr}`);
  }
}
...
describe("cli-local", () => {
  test("#given isolated agent dir #when install then uninstall are invoked #then stdout is one-line JSON and settings round-trip", async () => {
    ...
    const uninstalledSettings = JSON.parse(
      await readFile(join(agentDir, "settings.json"), "utf8"),
    ) as { packages?: string[] };

    // then
    expect(install.exitCode).toBe(0);
    const installResult = JSON.parse(install.stdout) as {
  ...
  test("#given invalid positional arg #when invoked #then it exits non-zero with one-line JSON error", async () => {
    ...
    expect(JSON.parse(result.stdout)).toMatchObject({
      ok: false,
      error: expect.stringContaining("install|uninstall"),
    });
