...
export async function runSenpiInstaller(
  options: SenpiInstallOptions = {},
): Promise<SenpiInstallResult> {
  ...
  const packages = dedupePackages(
    await removeSupersededOmoPackages(
      removeLegacyBuiltinShadows(
  ...
  const backupPath = await writeSettingsAtomically(
...
export async function runSenpiUninstaller(
  options: SenpiInstallOptions = {},
): Promise<SenpiInstallResult> {
  const context = resolveInstallContext(options);
  const settings = await readSettings(context.settingsPath);
  ...
  const repoRoot = resolve(
    options.repoRoot ??
      (allowBuild
        ...
        : dirname(resolve(options.pluginPath))),
  );
  const agentDir = resolve(
    options.agentDir ??
      resolveAgentHome({ env, homeDir: env.HOME ?? homedir() }),
  );
  const pluginPath = resolve(
    options.pluginPath ?? join(repoRoot, "packages", "omo-senpi", "plugin"),
  );
  return {
    env,
    repoRoot,
...
function findRepoRoot(importerDir: string): string {
  ...
  for (let depth = 0; depth <= 7; depth += 1) {
    if (
      fileExistsSync(
        join(current, "packages", "omo-senpi", "plugin", "package.json"),
      )
    ) return current;
    current = resolve(current, "..");
  }
  throw new Error(
    "Unable to locate packages/omo-senpi/plugin/package.json from installer module",
  );
}
