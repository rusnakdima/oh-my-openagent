...
async function backupFiles(agentDir: string): Promise<readonly string[]> {
  return (await readdir(agentDir)).filter((entry) =>
    entry.startsWith("settings.json.") && entry.endsWith(".backup")
  );
}

async function makePluginFixture(
  options: { readonly runtime?: boolean } = { runtime: true },
): Promise<string> {
  ...
  await writeFixtureFile(
    join(pluginPath, "extensions", "omo-task.js"),
    "export const createTaskComponent = () => ({})\n",
  );
  await writeFixtureFile(
    join(pluginPath, "extensions", "omo-member.js"),
    "export default {}\n",
  );
  await writeFixtureFile(
    join(pluginPath, "extensions", "memory-run-supervisor.mjs"),
    "export {}\n",
  );
  await writeFixtureFile(
    join(pluginPath, "extensions", "reflection-persona.md"),
    "# reflection persona fixture\n",
  );
  await writeFixtureFile(
    join(pluginPath, "extensions", "dream-persona.md"),
    "# dream persona fixture\n",
  );
  await writeFixtureFile(
    join(pluginPath, "extensions", "facts-persona.md"),
  ...
  for (const skillName of requiredSkillNames) {
    await writeFixtureFile(
      join(pluginPath, "skills", skillName, "SKILL.md"),
      `# ${skillName}\n`,
    );
  }
  await writeFixtureFile(
  ...
  if (options.runtime !== false) {
    ...
    await writeFixtureFile(
      ...
      `${
        JSON.stringify(
          {
            sha256: createHash("sha256").update(astGrepRuntimeBytes).digest(
              "hex",
            ),
            mode: 0o755,
    ...
    await writeFixtureFile(
      join(pluginPath, "runtime", "agent-toolkit", "cli.js"),
      "export {}\n",
    );
    await writeFixtureFile(
      join(pluginPath, "runtime", "agent-toolkit", "ulw-loop", "cli.js"),
      "console.log('ulw-loop')\n",
    );
    await writeFixtureFile(
      join(pluginPath, "runtime", "agent-toolkit", "omo-agent-toolkit"),
      "#!/bin/sh\n",
    );
    await writeFixtureFile(
      join(pluginPath, "runtime", "agent-toolkit", "omo-agent-toolkit.cmd"),
      "@echo off\r\n",
    );
    await writeFixtureFile(
    ...
    await writeFixtureFile(
      join(pluginPath, "runtime", "lsp-daemon", "dist", "index.js"),
      "export {}\n",
    );
    await writeFixtureFile(
      join(pluginPath, "runtime", "lsp-daemon", "dist", "index.d.ts"),
      "export {}\n",
    );
    await writeFixtureFile(
      join(pluginPath, "runtime", "lsp-daemon", "dist", "daemon-client.js"),
      "export {}\n",
    );
    await writeFixtureFile(
      join(pluginPath, "runtime", "lsp-daemon", "dist", "daemon-client.d.ts"),
      "export {}\n",
    );
    await writeFixtureFile(
      join(pluginPath, "runtime", "lsp-daemon", "dist", "package.json"),
...
describe("runSenpiInstaller", () => {
  ...
  test("#given a POSIX packed runtime with a mismatched manifest mode #when installing #then integrity fails", async () => {
    ...
    await expect(install).rejects.toThrow(
      "mode mismatch: manifest=493 actual=",
    );
  });
  ...
  test("#given packed ast-grep runtime differs from its manifest #when installing #then integrity failure leaves settings unchanged", async () => {
    ...
    // then
    await expect(install).rejects.toThrow(
      "ast-grep MCP runtime integrity error",
    );
  ...
  test("#given packed plugin missing the lazy task runtime #when installing #then settings stay unchanged", async () => {
    ...
    await expect(install).rejects.toThrow("missing required runtime artifacts");
    await expect(readFile(join(agentDir, "settings.json"), "utf8")).rejects
      .toThrow();
  });
});
...
describe("runSenpiUninstaller", () => {
  test("#given mixed package settings #when uninstalling #then only the omo-senpi plugin path is removed", async () => {
    ...
    // when
    const result = await runSenpiUninstaller({
      env: { SENPI_CODING_AGENT_DIR: agentDir },
      repoRoot,
      pluginPath,
    });
