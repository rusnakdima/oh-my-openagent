...
async function makeRuntimeFixture(): Promise<{
  ...
  await writeText(
    join(repoRoot, "packages", "lsp-daemon", "src", "index.ts"),
    "export const daemon = true\n",
  );
  await writeText(
    join(repoRoot, "packages", "lsp-core", "src", "index.ts"),
    "export const core = true\n",
  );
  await writeText(
    join(repoRoot, "packages", "mcp-stdio-core", "src", "index.ts"),
...
describe("Senpi LSP daemon runtime staging", () => {
  ...
  test("#given a tampered staged output #when verified #then manifest mismatch is rejected", async () => {
    ...
    await writeFile(
      join(fixture.targetDist, "cli.js"),
  ...
  test("#given an existing staged runtime #when replacement fails after backup #then prior bytes are restored", async () => {
    ...
    await expect(failure).rejects.toThrow(
      "Injected Senpi runtime staging failure",
    );
    expect(await readFile(join(fixture.targetDist, "cli.js"), "utf8")).toBe(
      "old cli\n",
    );
  });
});
