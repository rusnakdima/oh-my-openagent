...
describe("omo-senpi local-path runtime dependencies", () => {
  test("#given a symlinked plugin without host hoisting #when Node imports its runtime dependencies #then each resolves from the real path", () => {
    ...
    const runtimeImports = externalImports.filter((specifier) =>
      !peerPackages.has(specifier)
    );

    expect(
      runtimeImports.filter((specifier) => !runtimePackages.has(specifier)),
    ).toEqual([]);
    ...
    const isolatedProbePath = join(
      isolatedPluginRoot,
      "extensions",
      "runtime-dependency-probe.mjs",
    );
    ...
    const importedExtensionUrl = pathToFileURL(
      join(hostPluginPath, "extensions", "runtime-dependency-probe.mjs"),
    ).href;
    const result = spawnSync(
      ...
      [
        "--input-type=module",
        "-e",
        `await import(${JSON.stringify(importedExtensionUrl)})`,
      ],
