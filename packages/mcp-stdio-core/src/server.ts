...
async function handleParseError<HandlerOptions>(
  ...
  const response = config.parseErrorResponse?.(message.message) ??
...
async function handleRequest<HandlerOptions>(
  ...
  const method = isPlainRecord(parsed) && typeof parsed["method"] === "string"
    ? parsed["method"]
    : null;
