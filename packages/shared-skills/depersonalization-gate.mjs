  ...
  async function walk(dir) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        entry.isFile() && TEXT_EXTENSIONS.has(fileExtension(entry.name))
      ) {
        out.push(full);
      }
    }
...
export async function runDepersonalizationGate(
...
async function main() {
  ...
  const scanDirs = rootIndex !== -1 && args[rootIndex + 1]
    ? [args[rootIndex + 1]]
    : DEFAULT_SCAN_DIRS;
  const baseDir = rootIndex !== -1 && args[rootIndex + 1]
    ? args[rootIndex + 1]
    : here;
