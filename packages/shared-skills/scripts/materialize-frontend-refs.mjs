...
import {
  frontendSkillRoot,
  thirdPartyMaterializeMap,
  upstreamsRoot,
} from "./frontend-refs-manifest.mjs";
...
function materializedContent(relTarget, sourcePath) {
  ...
  const isDesignpowersSkillReference =
    relTarget.startsWith("references/designpowers/vendor/skills/") &&
    relTarget.endsWith("/reference.md") &&
    isSkillMarkdownSourcePath(sourcePath);
  if (relTarget.endsWith("/SKILL.md") || isDesignpowersSkillReference) {
    return normalizeSkillFrontmatter(content);
  }
  return content;
}
...
export function materializeFrontendRefs({ strict = false } = {}) {
  ...
  const requiredUpstreams = new Set(
    Object.values(map).map((entry) => entry.upstream),
  );

  for (const upstream of requiredUpstreams) {
    if (upstreamPopulated(upstream)) continue;
    const message =
      `[materialize] upstream submodule not initialized: ${upstream}`;
    if (strict) {
      throw new Error(
        `${message} (run: git submodule update --init packages/shared-skills/upstreams/${upstream})`,
      );
    }
    process.stderr.write(
      `${message} - skipping frontend ref materialization\n`,
    );
    return { materialized: 0, skipped: true };
  }

  rmSync(join(frontendSkillRoot, "references", "ui-ux-db"), {
    recursive: true,
    force: true,
  });
  rmSync(join(frontendSkillRoot, "references", "designpowers", "vendor"), {
    recursive: true,
    force: true,
  });
  ...
  for (const [relTarget, { upstream, source }] of Object.entries(map)) {
    ...
    if (!existsSync(sourcePath)) {
      throw new Error(
        `[materialize] missing upstream source: ${upstream}/${source} for ${relTarget}`,
      );
    }
  ...
  process.stdout.write(
    `[materialize] wrote ${materialized} frontend reference files from submodules\n`,
  );
