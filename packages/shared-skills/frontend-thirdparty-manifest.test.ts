...
describe("frontend third-party manifest partition", () => {
  ...
  const keptDesign: string[] = (designOriginals as string[]).map((name) =>
    `references/design/${name}`
  );
  ...
  test("no ui-ux-db file is committed", () => {
    ...
    const trackedUiUxDb = [...tracked].filter((relPath) =>
  ...
  test("no designpowers vendor file is committed", () => {
    ...
    const trackedDesignpowersVendor = [...tracked].filter((relPath) =>
  ...
  test("third-party manifest covers design brand, taste-skill, ui-ux-db, and designpowers", () => {
    ...
    const designCount =
      thirdParty.filter((relPath) => relPath.startsWith("references/design/"))
        .length;
    const uiUxDbCount =
      thirdParty.filter((relPath) => relPath.startsWith("references/ui-ux-db/"))
        .length;
    const designpowersCount =
      thirdParty.filter((relPath) =>
        relPath.startsWith("references/designpowers/vendor/")
      ).length;
