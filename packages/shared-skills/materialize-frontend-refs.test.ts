...
import {
  isSkillMarkdownSourcePath,
  materializeFrontendRefs,
} from "./scripts/materialize-frontend-refs.mjs";
import {
  brandStems,
  designpowersThirdPartyRelativePaths,
  frontendSkillRoot,
  tasteSkillArtifactFiles,
  thirdPartyRelativePaths,
  uiUxDbScripts,
  upstreamsRoot,
} from "./scripts/frontend-refs-manifest.mjs";
...
describe("materialize-frontend-refs", () => {
  ...
  test("reproduces every brand design file and the ui-ux-db scripts", () => {
    ...
    for (const brand of brandStems as string[]) {
      expect(
        existsSync(
          join(frontendSkillRoot, "references", "design", `${brand}.md`),
        ),
      ).toBe(true);
    }
    for (const script of uiUxDbScripts as string[]) {
      expect(
        existsSync(
          join(frontendSkillRoot, "references", "ui-ux-db", "scripts", script),
        ),
      ).toBe(true);
    }
  });
  ...
  test("materializes designpowers skills as references instead of nested skill entrypoints", () => {
    if (result.skipped) return;
    const referencePaths = designpowersThirdPartyRelativePaths().filter((
      relPath,
    ) => relPath.endsWith("/reference.md"));
    const nestedSkillPaths = designpowersThirdPartyRelativePaths().filter((
      relPath,
  ...
  test("recognizes upstream skill sources across platform path separators", () => {
    expect(isSkillMarkdownSourcePath("skills/design-review/SKILL.md")).toBe(
      true,
    );
    expect(isSkillMarkdownSourcePath("skills\\design-review\\SKILL.md")).toBe(
      true,
    );
    expect(isSkillMarkdownSourcePath("skills/design-review/reference.md")).toBe(
      false,
    );
  });
  ...
  test("materializes every taste-skill artifact byte-identical to its upstream source", () => {
    ...
    for (const [fileName, source] of Object.entries(tasteSkillArtifactFiles)) {
      const upstream = readFileSync(
        join(upstreamsRoot, "taste-skill", source),
        "utf8",
      );
      const materialized = readFileSync(
        join(frontendSkillRoot, "references", "design", fileName),
        "utf8",
      );
  ...
  test("materialized brand reference is byte-identical to its upstream source", () => {
    ...
    const materialized = readFileSync(
      join(frontendSkillRoot, "references", "design", "apple.md"),
      "utf8",
    );
    expect(materialized).toBe(upstream);
  });
