...
function trackedFrontendReferenceFiles(): string[] {
  ...
  if (override !== undefined) {
    return override.split(",").map((value) => value.trim());
  }
...
describe("DMCA provenance gate", () => {
  const keptDesign = new Set(
    (designOriginals as string[]).map((name) => `references/design/${name}`),
  );
  ...
  test("designpowers skills match upstream after frontmatter normalization", () => {
    ...
    for (const skillName of includedDesignpowersSkills) {
      const upstream = readFileSync(
        join(upstreamsRoot, "designpowers", "skills", skillName, "SKILL.md"),
        "utf8",
      );
      ...
      if (normalizeSkillFrontmatter(upstream) !== materialized) {
        mismatches.push(skillName);
      }
    }
  ...
  test("each ATTRIBUTION pin equals the live submodule HEAD", () => {
    ...
    const attributionPath = process.env.ATTRIBUTION_OVERRIDE ??
      join(repoRoot, attributionRel);
    const attribution = readFileSync(attributionPath, "utf8");
    const pins = [
      ...attribution.matchAll(/Pinned upstream commit:\s*([0-9a-f]{40})/g),
    ].map((match) => match[1]);
    const heads = [
      "open-design",
      "taste-skill",
  ...
  test("no submodule gitlink lives under any shipped skills/ directory", () => {
    ...
    const paths = [...gitmodules.matchAll(/path\s*=\s*(.+)/g)].map((match) =>
      match[1].trim()
    );
