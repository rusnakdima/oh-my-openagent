...
const typescriptPackageDir = dirname(
  repoRequire.resolve("typescript/package.json"),
);
...
describe("#given the no-excuse script is executed from an installed ~/.codex-style skill cache", () => {
  beforeAll(() => {
    ...
    symlinkSync(
      ...
      join(callerDir, "node_modules", "typescript"),
      "junction",
    );
    writeFileSync(
      join(callerDir, "clean.ts"),
      "export const answer: number = 42\n",
    );
    writeFileSync(
      join(callerDir, "violating.ts"),
      "const x = foo as any\nexport default x\n",
    );
    ...
    writeFileSync(
      join(callerWithoutTypescriptDir, "clean.ts"),
      "export const answer: number = 42\n",
    );
  });
  ...
  test(
    "#when the caller project provides typescript #then it resolves the caller's typescript and exits 0",
    () => {
  ...
  test(
    "#when a checked file violates the rules #then violations are still reported with exit 1",
    () => {
  ...
  test(
    "#when the caller project genuinely lacks typescript #then it fails with a clear error and exit 2",
    () => {
      ...
      expect(result.stderr).toContain(
        'cannot resolve "typescript" from the caller project',
      );
    },
    60_000,
  );
