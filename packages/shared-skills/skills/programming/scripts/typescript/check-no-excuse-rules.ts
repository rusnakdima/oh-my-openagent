...
function loadTypescriptFromCaller(): TsModule {
  ...
  const callerRequire = createRequire(
    path.join(process.cwd(), "no-excuse-anchor.cjs"),
  );
  try {
    const api: Partial<TsApiModule> = callerRequire(
      "typescript/unstable/async",
    );
    const ast: Partial<TsAstModule> = callerRequire("typescript/unstable/ast");
    if (
      typeof api.API === "function" && typeof ast.isAsExpression === "function"
    ) {
...
function discoverFiles(inputs: string[]): string[] {
  ...
  for (const input of inputs) {
    ...
    if (fs.statSync(resolved).isFile()) {
      if (isIncludedFile(resolved) && !isDeclarationFile(resolved)) {
        files.push(resolved);
      }
      continue;
    }
    const walk = (dir: string): void => {
      for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
        if (entry.isDirectory()) {
          ...
          isIncludedFile(entry.name) && !isDeclarationFile(entry.name)
        ) {
...
): Promise<ReadonlyMap<string, tsTypes.SourceFile>> {
  ...
  function visit(node: tsTypes.Node): void {
    ...
    if (ts.isAsExpression(node)) {
      ...
      if (typeText === "any") {
        ...
        violations.push({
          ...
          filePath,
          ...p,
          message: "`as any` — narrow with type guards or redesign the types",
        });
      }
      if (typeText === "unknown") {
        const p = pos(node);
        violations.push({
          ...
          filePath,
          ...p,
          message: "`as unknown` — redesign the types",
        });
      }
    }
    ...
    if (ts.isEnumDeclaration(node)) {
      ...
      violations.push({
        ...
        filePath,
        ...p,
        message: "`enum` — use `as const` object + literal union type",
      });
    }
    ...
    if (ts.isNonNullExpression(node)) {
      ...
      violations.push({
        ...
        filePath,
        ...p,
        message: "`x!` — use narrowing or optional chaining",
      });
    }
    ...
    if (ts.isThrowStatement(node) && node.expression) {
      ...
      if (
        ...
        violations.push({
          ...
          filePath,
          ...p,
          message: "`throw literal` — throw an Error subclass",
        });
      }
      if (ts.isTemplateExpression(expr)) {
        ...
        violations.push({
          ...
          filePath,
          ...p,
          message: "`throw template` — throw an Error subclass",
        });
      }
    }
      ...
      if (hasExport) {
        ...
        if (!(flags & ts.NodeFlags.Const)) {
          ...
          violations.push({
            ...
            filePath,
            ...p,
            message: "`export let/var` — use `export const`",
          });
        }
      }
    }
    ...
    // ── : any in annotations ──
    if (
      ts.isTypeReferenceNode(node) || node.kind === ts.SyntaxKind.AnyKeyword
    ) {
      if (node.kind === ts.SyntaxKind.AnyKeyword && !lineHasOptOut(node)) {
        ...
        if (parent && ts.isAsExpression(parent)) {
          ...
          violations.push({
            ...
            filePath,
            ...p,
            message: "`: any` annotation — use `unknown` and narrow",
          });
        } else if (
          parent && (
          ...
          violations.push({
            ...
            filePath,
            ...p,
            message: "`(): any` return — use a specific type",
          });
        }
      }
    }
      ...
      if (!CATCH_OK_RE.test(catchLineText)) {
        ...
        if (stmts.length === 0) {
          ...
          violations.push({
            ...
            ...p,
            message:
              "empty `catch` block — handle, re-throw, or remove the try/catch",
          });
          ...
          const hasRethrow = blockText.includes(`throw ${varName}`) ||
            blockText.includes(`throw new`);
          if (!hasInstanceof && !hasRethrow) {
            const p = pos(node);
            violations.push({
              ruleId: "catch-without-narrowing",
              filePath,
              ...p,
              message:
                "`catch` without `instanceof` narrowing or re-throw — narrow the error type or re-throw",
            });
          }
        }
      }
