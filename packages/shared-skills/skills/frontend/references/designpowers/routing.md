...
`designpowers` routes design operating-layer guidance inside the existing
frontend skill. It is not a replacement for `/frontend`, `/visual-qa`,
`/ulw-plan`, `/start-work`, `/review-work`, or any OpenAgent skill, and it must
not create a second planner, builder, verification harness, or orchestration
API.
...
When planning is needed, `designpowers` supplies design-specific context and
lets `/ulw-plan` own the plan artifact. Do not write a separate design plan. The
Prometheus plan should receive:
...
- verification expectations: frontend checks, visual QA artifacts, persona
  walkthroughs, and review-work sign-off;
- explicit Must Not Have constraints, including prohibited bridge/canvas
  tooling.

## Execution Through `/start-work`

When a plan is approved or selected, `/start-work` remains the orchestrator.
`designpowers` only enriches worker prompts with design context from
`.omo/frontend-design/state.md` and the selected plan. Worker prompts should
carry:
...
- the design debt rule: unresolved accessibility debt cannot disappear into a
  summary.

Direct implementation outside `/start-work` is not part of this routing contract
when a Prometheus plan is active.
...
`designpowers` must not replace the frontend `DESIGN.md` gate, taste routing,
React tooling, Lighthouse 100 workflow, browser QA, or performance discipline.

## Visual Checks Through `/visual-qa`

`/visual-qa` owns objective rendered evidence. Run it before accepting visual or
design-quality claims. `designpowers` adds design judgment only after that
evidence exists:
...
The same build must satisfy objective visual evidence and design judgment,
unless remaining gaps are explicitly recorded and accepted by the user.

## Final Review Through `/review-work`

Use `/review-work` as the final gate for significant implementation work. The
...
`designpowers` does not approve its own work. It prepares design context so
`/review-work` can evaluate whether the delivered UI satisfies the full request.

## Prohibited Routes

The following are guardrails only: framesmith, Figma bridge tooling,
`figma-bridge`, canvas adapters, and `canvas_evaluate` are not available
integration paths. Do not add scripts, hooks, a scheduler, fake direct calls, or
a competing planner/build harness.
