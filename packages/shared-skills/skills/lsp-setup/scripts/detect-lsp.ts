  ...
  while (stack.length > 0) {
    ...
    for (const entry of entries) {
      ...
      if (kind === "dir") {
        ...
        visited += 1;
        const ext = (BASENAME_EXTENSIONS[entry] ?? extname(entry))
          .toLowerCase();
...
function pathDirectories(): readonly string[] {
  return (process.env["PATH"] ?? "").split(delimiter).filter((dir: string) =>
    dir.length > 0
  );
}
...
function resolveExecutable(command: string): string | null {
  const extensions = process.platform === "win32"
    ...
    : [""];
  const bases = command.includes("/") || command.includes(sep)
    ? [command]
...
function readConfigState(root: string): readonly ConfigFileState[] {
  return PROJECT_CONFIG_FILES.map((relative: string): ConfigFileState => {
    ...
    if (!existsSync(path)) {
      return { path: relative, exists: false, serverIds: [] };
    }
    return {
      path: relative,
      exists: true,
      serverIds: parseConfiguredServerIds(path),
    };
  });
}
...
function renderReport(
  ...
  const configSummary = configState
    .map((state) =>
      `${state.path} (${
        state.exists ? `present: ${state.serverIds.length} server(s)` : "absent"
      })`
    )
    .join(", ");
  ...
  for (const result of results) {
    ...
    const state = result.installed
      ...
      : "NOT installed";
    const config = result.configuredIn.length > 0
      ? `configured in ${result.configuredIn.join(", ")}`
      : "builtin-default";
    lines.push(
      `[${mark}] ${
        result.server.language.padEnd(12)
      } server=${result.server.serverId}  exe=${result.executable}  ${state}  ${config}`,
    );
    if (!result.installed) {
      lines.push(`        install: ${result.server.installHint}`);
    }
  }
  ...
  lines.push(
    ...
    missing.length === 0
      ...
      : `${missing.length}/${results.length} server(s) NOT installed: ${
        missing.map((m) => m.server.language).join(", ")
      }`,
...
function main(): void {
  ...
  const root = args.find((arg: string) => !arg.startsWith("--")) ??
    process.cwd();

  if (!existsSync(root)) {
    process.stderr.write(
      `detect-lsp: target directory does not exist: ${root}\n`,
    );
    process.exit(2);
  }
  ...
  if (wantsJson) {
    process.stdout.write(
      `${JSON.stringify({ root, configState, results }, null, 2)}\n`,
    );
    return;
  }
