...
class EngineNotFoundError extends Error {
  constructor(public readonly target: string) {
    super(
      `lsp-tools-mcp engine not found (looked for ${target}). Run verify-lsp.ts inside the omo repo/worktree.`,
    );
...
async function run(filePath: string, timeoutMs: number): Promise<number> {
  ...
  try {
    ...
    const result = await context.runWithRequestContext(
      ...
      () =>
        tools.executeLspDiagnostics(
          { filePath: absolute, severity: "all" },
          signal,
        ),
    );
    const details = isDiagnosticsDetails(result.details)
      ? result.details
      : null;
    ...
    if (details?.errorKind === "missing_dependency") {
      process.stdout.write(
        `FAIL ${absolute}: language server not installed\n${text}\n`,
      );
      return 1;
    }
    if (
      result.isError === true || details?.errorKind === "invalid_path" ||
