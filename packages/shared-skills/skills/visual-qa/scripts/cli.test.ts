...
describe("runImageDiff", () => {
  test("#given two identical PNG files #when diffed #then similarity is 100", () => {
    ...
    writeFileSync(
      reference,
      encodeRgbaPng(2, 2, solidRgba(2, 2, [1, 2, 3, 255])),
    );
...
describe("cli entry", () => {
  test("#given the Node bundle is spawned for image-diff without Bun on PATH #when it runs #then it prints JSON and exits zero", () => {
    ...
    writeFileSync(
      reference,
      encodeRgbaPng(2, 2, solidRgba(2, 2, [0, 0, 0, 255])),
    );
    writeFileSync(
      actual,
      encodeRgbaPng(2, 2, solidRgba(2, 2, [255, 255, 255, 255])),
    );
    ...
    const proc = Bun.spawnSync({
      cmd: [
        nodePath,
        join(import.meta.dir, "visual-qa.mjs"),
        "image-diff",
        reference,
        actual,
      ],
