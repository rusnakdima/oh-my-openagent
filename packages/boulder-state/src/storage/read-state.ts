...
function normalizeState(state: Record<string, unknown>): void {
  ...
  const sessionOrigins =
    state.session_origins && typeof state.session_origins === "object" &&
  ...
  if (
    !state.task_sessions || typeof state.task_sessions !== "object" ||
    Array.isArray(state.task_sessions)
  ) {
    state.task_sessions = {};
  }
...
function normalizeSessionFields(target: Record<string, unknown>): void {
  const sessionIds = Array.isArray(target.session_ids)
    ? target.session_ids.filter((sessionId): sessionId is string =>
  ...
  const sessionOrigins =
    target.session_origins && typeof target.session_origins === "object" &&
      !Array.isArray(target.session_origins)
      ? normalizeSessionOrigins(
        target.session_origins as Record<string, unknown>,
      )
      : {};
  target.session_origins = sessionOrigins;
}

function normalizeSessionOrigins(
  sessionOrigins: Record<string, unknown>,
): Record<string, unknown> {
  return Object.fromEntries(
    Object.entries(sessionOrigins).map((
      [sessionId, origin],
    ) => [normalizeSessionId(sessionId), origin]),
  );
}
...
export function getBoulderWorks(state: BoulderState): BoulderWorkState[] {
  if (state.works && typeof state.works === "object") {
    return Object.values(state.works).filter((work): work is BoulderWorkState =>
      work != null
    );
  }
...
export function getActiveWorks(directory: string): BoulderWorkState[] {
  ...
  return getBoulderWorks(state).filter((work) =>
    work.status !== "completed" && work.status !== "abandoned"
  );
}
...
export function getWorkForSession(
  directory: string,
  sessionId: string,
): BoulderWorkState | null {
  const state = readBoulderState(directory);
  ...
  return state.session_ids.includes(normalizedSessionId)
    ? buildWorkFromMirror(state)
    : null;
}

export function getWorkResumeOptions(
  directory: string,
): BoulderWorkResumeOption[] {
  const state = readBoulderState(directory);
  if (!state) {
  ...
  return getBoulderWorks(state)
    ...
    .map((work) => {
      const progress = getPlanProgress(
        resolveBoulderPlanPathForWork(directory, work),
      );
      return {
        ...
        status: work.status && isValidWorkStatus(work.status)
          ? work.status
          : "active",
