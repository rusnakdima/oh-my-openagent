...
Harness-neutral pure-TypeScript core package (`@oh-my-opencode/utils`). Consumed
by both adapters (`packages/omo-opencode`, `packages/omo-codex`) and 15+ sibling
Core packages. Barrel-exports 36 modules across runtime shims, config tooling,
file utilities, prompt gating, git parsing, migration maps, and codegraph
primitives.
...
- **No path aliases:** relative imports only; strict `ESNext` + `bundler`
  moduleResolution.
- **Runtime portability:** `runtime/spawn.ts` wraps both Bun and Node APIs so
  downstream packages can run under Electron-hosted OpenCode.
- **Prompt-async-gate is critical infrastructure:** every internal prompt
  dispatch across the plugin must route through `dispatchInternalPrompt()` to
  prevent duplicate injections and race conditions. See the root
  [`AGENTS.md`](../../AGENTS.md) "Internal message injection is dangerous"
  section.
