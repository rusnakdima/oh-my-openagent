...
export function runProcessWithTreeTimeout(
  ...
  return new Promise((resolvePromise) => {
    ...
    const startTermination = (): Promise<ProcessTreeTerminationReport> => {
      ...
      treeTermination = treeTermination.then((report) => {
        try {
          ...
          stderr += `[process-tree] termination observer failed: ${
    ...
    const settle = async (
      ...
      removeTimeoutSignal();
      const termination = treeTermination === undefined
        ? null
    ...
    if (options.timeoutSignal?.aborted === true) {
      ...
      removeTimeoutSignal = () =>
        timeoutSignal.removeEventListener("abort", triggerTimeout);
    }
