...
describe("findSgBinarySync", () => {
  ...
  it("prefers ast-grep over sg on darwin without probing the noisy sg alias", () => {
    ...
    const result = findSgBinarySync({
      ...
      runVersionProbeSync: (binaryPath) => {
        ...
        if (binaryPath === sgAlias) {
          throw new Error(
            "the sg alias probe must never run when ast-grep passes",
          );
        }
  ...
  it("keeps the probed binary's stderr out of the parent process", () => {
    ...
    writeFileSync(
      ...
      [
        ...
        `  platform: "darwin",`,
        `  which: (commandName) => (commandName === "sg" ? ${
          JSON.stringify(fakeSg)
        } : null),`,
        `})`,
  ...
  it("prefers ast-grep over Linux setgroups sg collisions", () => {
    ...
    const result = findSgBinarySync({
      ...
      env: {},
      fileExists: (filePath) =>
        filePath === linuxSetgroups || filePath === astGrep,
      homeDir: NO_RUNTIME_HOME,
      platform: "linux",
      runVersionProbeSync: (
        binaryPath,
      ) => (binaryPath === linuxSetgroups
        ? "sg from shadow-utils"
        : "ast-grep 0.43.0"),
      which: (
        commandName,
      ) => (commandName === "ast-grep" ? astGrep : linuxSetgroups),
    });
