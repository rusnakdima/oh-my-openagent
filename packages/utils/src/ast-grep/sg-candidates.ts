...
function omoRuntimeCandidates(options: {
  ...
  if (options.runtimeDir !== undefined) {
    paths.push(join(options.runtimeDir, binaryName));
  }
  const codexHome = nonEmptyValue(options.env["CODEX_HOME"]);
  if (codexHome !== null) {
    paths.push(join(codexHome, "runtime", "ast-grep", slug, binaryName));
  }
  paths.push(
    join(options.homeDir, ".omo", "runtime", "ast-grep", slug, binaryName),
  );
...
function skillBinCandidates(options: {
  ...
  const names = [
    astGrepBinaryName(options.platform),
    sgBinaryName(options.platform),
  ];
  ...
  if (options.packageDir !== undefined) {
    directories.push(join(options.packageDir, "bin"));
  }
  return directories.flatMap((directory) =>
    names.map((name) => candidate("skill-bin", join(directory, name)))
  );
}
...
function homebrewCandidates(platform: NodeJS.Platform): readonly SgCandidate[] {
  const prefixes =
    platform === "darwin" || platform === "linux" || platform === "win32"
      ? HOMEBREW_PREFIXES[platform]
      : [];
  const names = [astGrepBinaryName(platform), sgBinaryName(platform)];
  return prefixes.flatMap((prefix) =>
    names.map((name) => candidate("homebrew", join(prefix, name)))
  );
}
