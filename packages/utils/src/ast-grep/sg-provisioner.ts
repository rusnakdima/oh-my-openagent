...
export type SgProvisionErrorCode =
  | "bad_checksum"
  | "download_failed"
  | "extract_failed"
...
async function downloadAsset(
  url: string,
  fetchImpl: SgProvisionOptions["fetchImpl"],
  signal: AbortSignal,
): Promise<Buffer> {
  ...
  try {
    ...
    throw new SgProvisionError(
      "download_failed",
      `failed to download ast-grep ${SG_PINNED_VERSION} from ${url}: ${
  ...
  if (!response.ok) {
    throw new SgProvisionError(
      "download_failed",
      `failed to download ast-grep ${SG_PINNED_VERSION} from ${url}: HTTP ${response.status}`,
    );
  }
  try {
    ...
    throw new SgProvisionError(
      "download_failed",
      `failed to read ast-grep ${SG_PINNED_VERSION} download from ${url}: ${
        describeFailure(error)
      }`,
      { cause: error },
    );
...
function findEndOfCentralDirectory(zip: Buffer): number {
  ...
  throw new SgProvisionError(
    "extract_failed",
    "downloaded ast-grep asset is not a zip archive",
  );
}
...
function listZipEntries(zip: Buffer): readonly ZipCentralEntry[] {
  ...
  for (let index = 0; index < entryCount; index += 1) {
    if (
      cursor + 46 > zip.length || zip.readUInt32LE(cursor) !== CENTRAL_SIGNATURE
    ) {
      throw new SgProvisionError(
        "extract_failed",
        "downloaded ast-grep zip central directory is corrupt",
      );
    }
    ...
    entries.push({
      ...
      name: zip.subarray(cursor + 46, cursor + 46 + nameLength).toString(
        "utf8",
      ),
...
function decompressZipEntry(raw: Buffer, entry: ZipCentralEntry): Buffer {
  ...
  throw new SgProvisionError(
    "extract_failed",
    `ast-grep zip entry ${entry.name} uses unsupported compression method ${entry.method}`,
  );
}
...
function readZipEntryBytes(zip: Buffer, entry: ZipCentralEntry): Buffer {
  if (
    ...
    throw new SgProvisionError(
      "extract_failed",
      `ast-grep zip entry ${entry.name} uses unsupported zip64 extensions`,
    );
  }
  if (zip.readUInt32LE(entry.localHeaderOffset) !== LOCAL_SIGNATURE) {
    throw new SgProvisionError(
      "extract_failed",
      `ast-grep zip entry ${entry.name} has a corrupt local header`,
    );
  }
  ...
  const bytes = decompressZipEntry(
    zip.subarray(dataStart, dataStart + entry.compressedSize),
    entry,
  );
  if (bytes.length !== entry.uncompressedSize) {
    throw new SgProvisionError(
      "extract_failed",
      `ast-grep zip entry ${entry.name} inflated to ${bytes.length} bytes, expected ${entry.uncompressedSize}`,
    );
  }
  return bytes;
}

function extractStandaloneSgBinary(
  zip: Buffer,
  ...
  for (const preferred of preferredNames) {
    const entry = entries.find((candidate) =>
      zipEntryBaseName(candidate.name) === preferred
    );
    if (entry !== undefined) return readZipEntryBytes(zip, entry);
  }
  throw new SgProvisionError(
    "extract_failed",
    `ast-grep release zip has no standalone ${
      preferredNames.join(" or ")
    } binary`,
  );
}
...
function assertInsideTarget(targetDir: string, filePath: string): void {
  ...
  if (relativePath.startsWith("..") || isAbsolute(relativePath)) {
    throw new SgProvisionError(
      "write_failed",
      `refusing to write ast-grep binary outside targetDir: ${filePath}`,
    );
  }
}

export async function provisionSgBinary(
  options: SgProvisionOptions,
): Promise<string> {
  ...
  if (asset === undefined) {
    throw new SgProvisionError(
      "unsupported_platform",
      `ast-grep ${SG_PINNED_VERSION} has no asset for ${slug}`,
    );
  }
  ...
  try {
    ...
    throw new SgProvisionError(
      "write_failed",
      `failed to provision ast-grep ${SG_PINNED_VERSION} into ${targetDir}: ${
        describeFailure(error)
      }`,
      { cause: error },
    );
