...
export type RunAstGrepSkillInstall = (
  options: AstGrepSkillInstallOptions,
) => Promise<AstGrepSkillInstallResult>;
...
type TimedInvocationOutcome = AstGrepInstallSpawnOutcome | {
  readonly kind: "timed-out";
};

export function astGrepRuntimeDir(
  baseDir: string,
...
function scriptPathForPlatform(
  skillDir: string,
...
function invocationsForPlatform(
  scriptPath: string,
  platform: NodeJS.Platform,
): readonly InstallInvocation[] {
...
function failedReason(
  outcome: Exclude<
    ...
    { readonly kind: "exit" } | { readonly kind: "timed-out" }
  >,
): string {
  return outcome.error.message;
}

export async function runAstGrepSkillInstall(
  options: AstGrepSkillInstallOptions,
): Promise<AstGrepSkillInstallResult> {
