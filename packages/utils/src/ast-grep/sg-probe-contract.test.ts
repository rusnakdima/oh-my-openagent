...
const TEST_PLATFORM: NodeJS.Platform = process.platform === "win32"
  ? "win32"
  : "linux";
...
function runWindowsStubProbe(binaryPath: string): string {
  ...
  if ((fixture.mode & 0o111) === 0) {
    const error = new Error(`spawn ${binaryPath} EACCES`) as Error & {
      code: string;
    };
    error.code = "EACCES";
    throw error;
  ...
  if (fixture.body.includes("sleep 60")) {
    const error = new Error(`spawn ${binaryPath} ETIMEDOUT`) as Error & {
      code: string;
    };
...
function isolated(overrides: SgResolverOptions): SgResolverOptions {
  return {
    ...
    runVersionProbeSync: TEST_PLATFORM === "win32"
      ? runWindowsStubProbe
      : undefined,
    which: () => null,
    ...overrides,
...
describe("sg probe contract: a failing probe never wins", () => {
  it("#given a broken env override and a working PATH ast-grep #when resolving #then the override loses to PATH", () => {
    ...
    const brokenOverride = writeStub(
      join(root, "override", "custom-override"),
      "#!/bin/sh\nexit 42\n",
    );
    const pathAstGrep = workingAstGrep(
    ...
    const result = resolveSgBinarySync(
      isolated({
        ...
        which: (
          commandName,
        ) => (commandName === "ast-grep" ? pathAstGrep : null),
      }),
    );
  ...
  it("#given a broken env override and nothing else #when resolving #then BINARY_NOT_FOUND is returned", () => {
    ...
    // when
    const result = resolveSgBinarySync(
      isolated({ env: { [SG_PATH_ENV_KEY]: brokenOverride } }),
    );
  ...
  it("#given a cached env candidate that loses its executable bit #when revalidating #then EACCES rejects it", () => {
    ...
    expect(revalidated.found ? null : revalidated.error.code).toBe(
      "BINARY_NOT_FOUND",
    );
  ...
  it(
    "#given a PATH ast-grep that hangs #when the 5s probe times out #then it is rejected and the next candidate wins",
    () => {
      // given: an `ast-grep`-named stub that never returns, and a working `sg` behind it on PATH
      const root = tempDir("contract-timeout");
      const hangingAstGrep = writeStub(
        join(root, "path", binaryName("ast-grep")),
        "#!/bin/sh\nsleep 60\n",
      );
      ...
      const result = resolveSgBinarySync(
        isolated({
          which: (
            commandName,
          ) => (commandName === "ast-grep" ? hangingAstGrep : workingSg),
        }),
      );
  ...
  it("#given an ast-grep-named candidate whose probe reports another tool #when resolving #then the name grants no exemption", () => {
    ...
    const result = resolveSgBinarySync(
      isolated({
        which: (commandName) => (commandName === "ast-grep" ? impostor : null),
      }),
    );
  ...
  it("#given packageDir is supplied #when the package-local bin holds ast-grep #then the skill-bin tier activates", () => {
    ...
    const packageLocal = workingAstGrep(
      join(packageDir, "bin", binaryName("ast-grep")),
    );
    const pathAstGrep = workingAstGrep(
      join(root, "path", binaryName("ast-grep")),
    );
