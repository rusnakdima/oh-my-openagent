...
function hasObjectSessionPath(
  input: unknown,
): input is ObjectPathPromptInput & { readonly path: { readonly id: string } } {
...
export async function dispatchInternalPrompt<TInput = PromptAsyncInput>(
  ...
  const dispatchTimeoutMs = args.dispatchTimeoutMs ??
    DEFAULT_PROMPT_DISPATCH_TIMEOUT_MS;
    ...
    if (args.mode === "async") {
      ...
      if (resolved.route === "live") {
        const originalSession =
          (args.client as PromptAsyncClient<TInput>).session;
          ...
          try {
            ...
            if (isPreSendConnectionFailure(error)) {
              ...
              if (typeof originalSession?.promptAsync === "function") {
                const fallbackDispatch = originalSession.promptAsync.bind(
                  originalSession,
                );
        ...
        try {
          ...
          if (isPreSendConnectionFailure(error)) {
            ...
            if (typeof originalSession?.prompt === "function") {
              const fallbackDispatch = originalSession.prompt.bind(
                originalSession,
              );
              return fallbackDispatch(dispatchInput);
            }
  ...
  if (!dispatch) {
    log(`[prompt-async-gate] ${sessionName} unavailable`, {
      sessionID,
      source,
    });
  ...
  const queueBehavior = args.queueBehavior ??
    (args.mode === "sync" ? "defer" : "enqueue");
  ...
  if (queueBehavior === "defer") {
    ...
    const recentDispatchResult = coalesceRecentSemanticPromptDispatch({
      sessionID,
      dedupeKey,
      source,
    });
    ...
    const deferResult = await dispatchAfterSessionIdle({
      ...
      dispatch: (dispatchInput) =>
        dispatchWithPathCompatibility(dispatch, dispatchInput),
    });
    if (
  ...
  if (args.queue !== false) {
    ...
    return enqueueInternalPrompt({
      ...
      checkToolState: args.checkToolState !== false,
      shouldDispatch: args.shouldDispatch,
      dispatch: async (_dispatchInput: unknown) =>
        dispatchWithPathCompatibility(dispatch, input),
    });
  }
...
export function isInternalPromptDispatchAccepted(
...
export function releasePromptAsyncReservation(
  ...
  if (
    !reservationSourceMatches(
      existing.source,
      expectedSource,
      options?.reservedByPrefix,
      options?.supersedeTransientRetryOwners,
    )
  ) {
    log(
