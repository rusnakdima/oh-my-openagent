...
async function listPosixTreePids(rootPid: number): Promise<readonly number[]> {
  try {
    const rows = parseProcessRows(
      await execFileText("ps", ["-eo", "pid=,ppid=,pgid="]),
    );
    ...
    while (changed) {
      ...
      for (const row of rows) {
        if (
          ...
          (row.pgid !== rootPid && !selected.has(row.ppid))
        ) continue;
...
function parseProcessRows(output: string): readonly ProcessRow[] {
  ...
  for (const line of output.split(/\r?\n/)) {
    ...
    if (
      pidText === undefined || ppidText === undefined || pgidText === undefined
    ) continue;
