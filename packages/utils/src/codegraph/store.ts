...
export function writeCodegraphSourceMetadata(
  dataDir: string,
  sourceDir: string,
): void {
  writeFileSync(
    ...
    `${
      JSON.stringify(
        { sourceDir, version: CODEGRAPH_PROJECT_SOURCE_METADATA_VERSION },
        null,
        2,
      )
    }\n`,
...
function directorySize(path: string): number {
  ...
  return readdirSync(path).reduce(
    (total, entry) => total + directorySize(join(path, entry)),
    0,
  );
}
...
function readRecordedSourceDir(projectDir: string): string | null {
  ...
  try {
    ...
    return typeof sourceDir === "string" && sourceDir.trim().length > 0
      ? sourceDir
      : null;
  } catch (error) {
...
function isNonFatalSourceMetadataError(error: unknown): boolean {
  ...
  return typeof code === "string" &&
    ["EACCES", "ENOENT", "ENOTDIR", "EPERM"].includes(code);
}
...
export function pruneCodegraphStore(
  options: PruneCodegraphStoreOptions,
): PruneCodegraphStoreResult {
  const projectsDir = join(
    codegraphDataRoot(options.homeDir ?? homedir()),
    "projects",
  );
