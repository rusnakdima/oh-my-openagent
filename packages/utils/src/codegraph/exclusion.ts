...
export type CodegraphProjectExclusionReason =
  | "custom-root"
  | "omo-state"
  | "tmp-root";
...
function resolveConfiguredRoot(path: string, homeDir: string): string {
  ...
  return realpathIfPossible(
    isAbsolute(expanded) ? expanded : join(homeDir, expanded),
  );
}
...
function defaultExcludedRoots(
  platform: NodeJS.Platform,
  tmpdir: string,
): readonly string[] {
  return platform === "win32"
    ? [tmpdir]
    : [...POSIX_DEFAULT_EXCLUDED_ROOTS, tmpdir];
}
