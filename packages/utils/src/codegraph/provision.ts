...
function forcedBadChecksumOptions(options: EnsureCodegraphProvisionedOptions): {
  ...
  return {
    ...
    installDir: options.installDir ??
      join(options.lockDir, "codegraph-force-bad-checksum"),
    manifest: {
      assets: {
        [key]: {
...
async function readMarker(
  ...
  try {
    ...
    if (
      typeof raw === "object" && raw !== null && "binPath" in raw &&
      "version" in raw
    ) {
      const binPath = raw.binPath;
      return raw.version === version && typeof binPath === "string" &&
          existsSync(binPath)
        ? binPath
        : null;
    }
...
async function installExtractedBundle(
  extractDir: string,
  installDir: string,
  ...
  if (roots.length !== 1) {
    throw new Error(
      `CodeGraph archive should contain one root directory, found ${roots.length}`,
    );
  }
...
async function installAsset(layout: {
  ...
  try {
    ...
    if (actualChecksum !== asset.sha256) {
      throw new Error(
        `checksum mismatch for ${
          basename(asset.url)
        }: expected ${asset.sha256}, got ${actualChecksum}`,
      );
    }

    if (!asset.url.endsWith(".tar.gz") && !asset.url.endsWith(".tgz")) {
      throw new Error(
        `unsupported CodeGraph archive type for ${basename(asset.url)}`,
      );
    }
    await writeFile(archivePath, bytes);
    await extractTarGz(archivePath, extractDir);
    const destination = await installExtractedBundle(
      extractDir,
      installDir,
    ...
    await writeFile(
      markerPath(installDir, version),
      `${JSON.stringify({ binPath: destination, version })}\n`,
    );
    return destination;
  } finally {
...
export async function ensureCodegraphProvisioned(
  ...
  const installDir = forced?.installDir ?? options.installDir ??
    defaultInstallDir();
  const manifest = forced?.manifest ?? options.manifest ??
    CODEGRAPH_PROVISION_MANIFEST;
  const activePlatformKey = forced?.platformKey ?? options.platformKey ??
    platformKey();
  const downloader = forced?.downloader ?? options.downloader ??
    ((asset) => defaultDownloader(asset, options.downloadTimeoutMs));
  ...
  try {
    ...
    if (manifest.version !== options.version) {
      return {
        error:
          `manifest version ${manifest.version} does not match requested ${options.version}`,
        provisioned: false,
      };
    }
