...
export function buildCodegraphNodeSkipHint(
  support: CodegraphNodeSupport,
): string {
  const detail = support.reason === "too-new"
  ...
  return `CodeGraph MCP skipped: ${detail}. Use Node ${CODEGRAPH_MIN_NODE_MAJOR}-${
    CODEGRAPH_BLOCKED_NODE_MAJOR - 1
  } (e.g. Node 22 LTS) or set ${CODEGRAPH_UNSAFE_NODE_ENV}=1 to override.\n`;
}
