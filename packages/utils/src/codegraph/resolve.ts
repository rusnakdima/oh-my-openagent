...
export function codegraphCommandRequiresSupportedLocalNode(
  ...
  return resolution.source !== "bundled" && resolution.source !== "env" &&
    resolution.source !== "provisioned";
}
...
const CODEGRAPH_NODE_CANDIDATES = [
  "node24",
  "node22",
  "node20",
  "node",
...
export function resolveCodegraphNodeSupport(
  ...
  return evaluateCodegraphNodeSupport({
    env,
    nodeVersion: nodeVersion(runtime) ?? "0.0.0",
  });
}

function defaultProvisionedBin(
  homeDir: string,
  fileExists: (filePath: string) => boolean,
): string | null {
  return resolvePinnedCodegraphBin(join(homeDir, ".omo", "codegraph"), {
    fileExists,
  });
}
...
function resolveBundledShim(
  ...
  try {
    ...
    const candidates = [
      join(packageRoot, "bin", "codegraph.js"),
      join(packageRoot, "npm-shim.js"),
    ];
...
export function resolveCodegraphCommand(
  ...
  const configuredBin = env[CODEGRAPH_ENV_BIN]?.trim() ||
    env[CODEGRAPH_LEGACY_ENV_BIN]?.trim();
  ...
  const bundled = resolveBundledShim(
    options.requireResolve ?? defaultRequireResolve,
    fileExists,
  );
  ...
  const provisioned = options.provisioned?.() ??
    defaultProvisionedBin(options.homeDir ?? homedir(), fileExists);
