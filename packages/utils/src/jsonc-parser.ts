...
function getPluginConfigCacheKey(
  dir: string,
  options: DetectPluginConfigFileOptions,
): string {
  const basenames = [...options.basenames].join(",");
  const legacyBasenames = options.legacyBasenames
    ? [...options.legacyBasenames].join(",")
    : "";
...
export function detectPluginConfigFile(
  ...
  const firstExistingLegacyResult = legacyResults.find((result) =>
    result.format !== "none"
  );
  ...
  if (canonicalResult.format !== "none") {
    detectionResult = {
      ...canonicalResult,
      ...(firstExistingLegacyResult === undefined
        ? {}
    ...
    detectionResult = {
      format: "none",
      path: join(dir, `${canonicalBasename}.json`),
    };
  }
