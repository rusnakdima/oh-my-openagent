...
describe("deepMerge", () => {
  describe("basic merging", () => {
    test.each(
      ...
      ] satisfies Array<[string, AnyObject, AnyObject, AnyObject]>,
    )("%s", (_label, base, override, expected) => {
      // when
  ...
  describe("edge cases", () => {
    test.each(
      [
        ...
        ["returns override when base is undefined", undefined, { a: 1 }, {
          a: 1,
        }],
        ["returns base when override is undefined", { a: 1 }, undefined, {
          a: 1,
        }],
      ] satisfies Array<
        [
  ...
  describe("array handling", () => {
    test.each(
      [
        ["replaces arrays instead of merging them", { arr: [1, 2] }, {
          arr: [3, 4, 5],
        }, { arr: [3, 4, 5] }],
        ["replaces nested arrays", { a: { arr: [1, 2, 3] } }, {
          a: { arr: [4] },
        }, { a: { arr: [4] } }],
      ] satisfies Array<[string, AnyObject, AnyObject, AnyObject]>,
    )("%s", (_label, base, override, expected) => {
  ...
  describe("prototype pollution protection", () => {
    ...
    test.each(
      ...
      ] satisfies Array<[string, AnyObject]>,
    )("ignores %s key", (key, override) => {
      // given
      const base: AnyObject = { a: 1 };
