...
describe("replaceToolArgs", () => {
  describe("#given a mutable output.args object", () => {
    it("#when patching a single property #then the output.args contains the patched value", () => {
      // given
      const output = {
        args: { command: "git status", timeout: 30 } as Record<string, unknown>,
      };
    ...
    it("#when patching multiple properties #then all patched values are present", () => {
      // given
      const output = {
        args: { url: "http://old.com", format: "text" } as Record<
          string,
          unknown
        >,
      };
  ...
  describe("#given a frozen output.args object", () => {
    it("#when patching a single property #then no TypeError is thrown and the value is updated", () => {
      // given
      const output = {
        args: Object.freeze({ command: "git status", timeout: 30 }) as Record<
          string,
          unknown
        >,
      };
      ...
      // when / then
      expect(() => replaceToolArgs(output, { command: "git log" })).not
        .toThrow();
      expect(output.args.command).toBe("git log");
      expect(output.args.timeout).toBe(30);
    });
    ...
    it("#when patching with Object-typed value #then no TypeError is thrown", () => {
      // given
      const output = {
        args: Object.freeze({ todos: "[]" }) as Record<string, unknown>,
      };
    ...
    it("#when patching url on frozen webfetch args #then no TypeError is thrown", () => {
      ...
      // when / then
      expect(() => replaceToolArgs(output, { url: "http://redirected.com" }))
        .not.toThrow();
    ...
    it("#when patching command with env prefix on frozen bash args #then no TypeError is thrown", () => {
      // given
      const output = {
        args: Object.freeze({ command: "git rebase --continue" }) as Record<
          string,
          unknown
        >,
      };
      ...
      expect(() =>
        replaceToolArgs(output, {
          command: "GIT_EDITOR=: git rebase --continue",
        })
      ).not.toThrow();
    ...
    it("#when patching prompt on frozen task args #then no TypeError is thrown", () => {
      ...
      // when / then
      expect(() =>
        replaceToolArgs(output, { prompt: "[DIRECTIVE] Do the thing" })
      ).not.toThrow();
    ...
    it("#when stripping null bytes from frozen command #then no TypeError is thrown", () => {
      ...
      const output = {
        args: Object.freeze({ command: frozenCommand }) as Record<
          string,
          unknown
        >,
      };
      ...
      // when / then
      expect(() => replaceToolArgs(output, { command: "echo hello" })).not
        .toThrow();
    ...
    it("#when replacing truncated question labels on frozen args #then no TypeError is thrown", () => {
      ...
      const output = {
        args: Object.freeze({
          questions: [{
            question: "Pick",
            options: [{ label: "A very long label that should be truncated" }],
          }],
        }) as Record<string, unknown>,
      };
      const truncated = {
        questions: [{
          question: "Pick",
          options: [{ label: "A very long label that sho..." }],
        }],
      };
      ...
      expect(
        (output.args.questions as Array<{ options: Array<{ label: string }> }>)[
          0
        ].options[0].label,
      ).toBe(
    ...
    it("#when replacing modifiedInput from PreToolUse hook on frozen args #then no TypeError is thrown", () => {
      // given
      const output = {
        args: Object.freeze({ filePath: "/old/path.ts" }) as Record<
          string,
          unknown
        >,
      };
      ...
      // when / then
      expect(() => replaceToolArgs(output, { filePath: "/new/path.ts" })).not
        .toThrow();
