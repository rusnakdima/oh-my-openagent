...
function parseRuleFrontmatter<T>(content: string): FrontmatterResult<T> {
  ...
  try {
    const data = parseRuleYaml(
      normalized.slice(openingLength, closing.start),
    ) as T;
    return {
      data,
...
function parseGlobValue(
  ...
  if (!rawValue) {
    ...
    return parsed.values.length > 0
      ? { value: parsed.values, consumed: parsed.consumed }
      : { value: "", consumed: 1 };
  }
...
function parseMultilineArray(
  lines: readonly string[],
  currentIndex: number,
): { readonly values: readonly string[]; readonly consumed: number } {
  const values: string[] = [];
  let consumed = 1;
...
function parseInlineArray(value: string): string[] {
  ...
  if (closing === -1) return [];
  return splitCommaSeparated(value.slice(1, closing)).map(parseString).filter(
    Boolean,
  );
}
...
): { readonly start: number; readonly bodyStart: number } | null {
