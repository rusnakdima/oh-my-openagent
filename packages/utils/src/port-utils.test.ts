...
function getRequiredPropertyDescriptor(
  target: object,
  propertyName: string,
): PropertyDescriptor {
  ...
  for (const addresses of Object.values(networkInterfaces())) {
    ...
    for (const address of addresses) {
      if (
        ...
        address.address !== DEFAULT_HOSTNAME
      ) {
...
function startTrackedServer(
  ...
  return new Promise<Server>((resolve, reject) => {
    ...
    try {
      ...
      reject(
        new Error("Expected server startup failure to throw an Error instance"),
      );
    }
  });
}
...
async function closeAllTrackedServers(): Promise<void> {
  await Promise.all(
    Array.from(trackedServers).map((server) => closeTrackedServer(server)),
  );
}

async function getReleasedPort(
  hostname: string = DEFAULT_HOSTNAME,
): Promise<number> {
...
async function findContiguousAvailableStart(
  ...
  for (
    let seedAttempt = 0;
    seedAttempt < CONTIGUOUS_SEARCH_SEEDS;
    seedAttempt++
  ) {
    const seedPort = await getReleasedPort(hostname);
    const maxStartPort = Math.min(
      65_535 - portCount + 1,
      seedPort + CONTIGUOUS_SEARCH_WINDOW,
    );

    for (
      let candidatePort = seedPort;
      candidatePort <= maxStartPort;
      candidatePort++
    ) {
...
async function startAlternateInterfaceBlockerWithDefaultHostFree(
  hostname: string,
  ...
  for (
    ...
    for (
      ...
      try {
        ...
        const defaultHostProbe = await startTrackedServer(
          candidatePort,
          DEFAULT_HOSTNAME,
        );
...
async function captureDefaultListenHostname(
  port: number,
): Promise<string | undefined> {
  const listenDescriptor = getRequiredPropertyDescriptor(
    Server.prototype,
    "listen",
  );
  const closeDescriptor = getRequiredPropertyDescriptor(
    Server.prototype,
    "close",
  );
  ...
  Object.defineProperty(Server.prototype, "listen", {
    configurable: true,
    value: function listenAndCaptureHostname(
      this: Server,
  ...
  Object.defineProperty(Server.prototype, "close", {
    configurable: true,
    value: function closeCapturedServer(
      this: Server,
...
async function runTimedOutAvailabilityProbe(
  port: number,
): Promise<TimeoutProbeResult> {
  const setTimeoutDescriptor = getRequiredPropertyDescriptor(
    globalThis,
    "setTimeout",
  );
  const listenDescriptor = getRequiredPropertyDescriptor(
  ...
  Object.defineProperty(globalThis, "setTimeout", {
    configurable: true,
    value: (callback: () => void): ReturnType<typeof setTimeout> =>
  ...
  Object.defineProperty(Server.prototype, "close", {
    configurable: true,
    value: function closeTimedOutServer(
      this: Server,
      callback?: (error?: Error) => void,
    ): Server {
      closeCallCount++;
...
async function runAvailabilityProbe<Result>(
  ...
  Object.defineProperty(Server.prototype, "listen", {
    configurable: true,
    value: function listenWithUnavailableProbes(
      this: Server,
      requestedPort: number,
    ): Server {
      ...
      if (requestedPort < startPort + unavailableProbeCount) {
        queueMicrotask(() => {
          this.emit(
            "error",
            Object.assign(
              new Error(`mocked port ${requestedPort} unavailable`),
              { code: "EADDRINUSE" },
            ),
          );
        });
  ...
  Object.defineProperty(Server.prototype, "close", {
    configurable: true,
    value: function closeMockedServer(
      this: Server,
      callback?: (error?: Error) => void,
    ): Server {
      queueMicrotask(() => callback?.());
...
describe("port-utils", () => {
  ...
  describe("#given isPortAvailable", () => {
    ...
    test("#when another interface owns the port #then default probing does not bind all interfaces", async () => {
      ...
      const blocker = await startAlternateInterfaceBlockerWithDefaultHostFree(
        alternateHostname,
      );
      if (!blocker) {
        const port = await getReleasedPort();
  ...
  describe("#given findAvailablePort", () => {
    ...
    test("#when the first three ports are blocked #then returns the next free port", async () => {
      ...
      const { errorMessage, probedPorts, selectedResult: selectedPort } =
      ...
      expect(probedPorts).toEqual(
        Array.from({ length: 4 }, (_, offset) => startPort + offset),
      );
    });
    ...
    test("#when every attempted port is blocked #then throws", async () => {
      ...
      expect(errorMessage).toBe(
        `No available port found in range ${startPort}-${
  ...
  describe("#given getAvailableServerPort", () => {
    ...
    test("#when the preferred port is blocked #then returns the next port with auto-selection", async () => {
      ...
      const { errorMessage, probedPorts, selectedResult } =
        await runAvailabilityProbe(
          preferredPort,
          1,
