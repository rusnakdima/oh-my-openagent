...
async function importFreshTarZipEntryListingModule(): Promise<
  TarZipEntryListingModule
> {
  return await import(
    `./tar-zip-entry-listing?test=${Date.now()}-${Math.random()}`
  );
}
...
describe("parseTarListingOutput", () => {
  ...
  describe("#given tar output with any unparsed lines", () => {
    it("#when parsing the output #then throws immediately (fail-closed)", async () => {
      ...
      const { parseTarListingOutput } =
        await importFreshTarZipEntryListingModule();
      const listedOutput = [
      ...
      const thrownError = captureThrownError(() =>
        parseTarListingOutput(listedOutput)
      );
    ...
    it("#when parsing the output #then rejects the listing", async () => {
      ...
      const thrownError = captureThrownError(() =>
        parseTarListingOutput(
          ["unknown format 1", "unknown format 2"].join("\n"),
        )
      );
