...
describe("runProcessWithTreeTimeout output decoding", () => {
  ...
  test.skipIf(process.platform === "win32")(
    "#given a child and SIGTERM-resistant grandchild #when the command times out #then SIGKILL is observed and no process survives",
    async () => {
      // given
      ...
      const script = [
        ...
        `const grandchild = spawn(process.execPath, ["--eval", ${
      ...
      expect(
        result.termination?.attempts.some((attempt) =>
