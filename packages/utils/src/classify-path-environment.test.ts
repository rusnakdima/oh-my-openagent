...
describe("classifyPathEnvironment", () => {
  ...
  it("classifies OneDrive path on windows style", () => {
    expect(classifyPathEnvironment("C:\\Users\\x\\OneDrive\\foo")).toBe(
      "onedrive",
    );
  });

  it("classifies macOS Desktop path as desktop-sync", () => {
    expect(classifyPathEnvironment("/Users/x/Desktop/foo")).toBe(
      "desktop-sync",
    );
  });

  it("classifies /Volumes path as network-drive", () => {
    expect(classifyPathEnvironment("/Volumes/NetworkShare/foo")).toBe(
      "network-drive",
    );
  });
...
describe("describePathClassification", () => {
  it("returns human-readable descriptions", () => {
    ...
    expect(describePathClassification("desktop-sync")).toBe(
      "Desktop sync (macOS)",
    );
