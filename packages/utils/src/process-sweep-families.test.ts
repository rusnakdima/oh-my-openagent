...
describe("process sweep family matrix", () => {
  ...
  const posixTable: readonly ProcessInfo[] = [
    ...
    {
      command: `node ${omoRoot}/components/codegraph/dist/serve.js`,
      pid: 301,
      ppid: 1,
    },
    ...
    {
      command:
        `node ${omoRoot}/node_modules/@colbymchenry/codegraph/bin/codegraph.js serve --mcp`,
      pid: 302,
      ppid: 9999,
    },
    // codegraph detached upstream daemon
    {
      command: `${installDir}/bin/codegraph serve --mcp --path /tmp/proj`,
      pid: 303,
      ppid: 1,
    },
    // lsp-daemon mcp proxy, orphaned (ppid 1)
    {
      command: `node ${omoRoot}/components/lsp-daemon/dist/cli.js mcp`,
      pid: 304,
      ppid: 1,
    },
    ...
    {
      command: `node ${omoRoot}/components/lsp-daemon/dist/cli.js daemon`,
      pid: 306,
      ppid: 1,
    },
    // dev-shape lsp-daemon mcp proxy via bun + src/cli.ts, dead parent
    {
      command: `bun ${omoRoot}/components/lsp-daemon/src/cli.ts mcp`,
      pid: 307,
      ppid: 9999,
    },
    // lsp-daemon mcp proxy outside any owned root
    {
      command: `node /tmp/other-plugin/components/lsp-daemon/dist/cli.js mcp`,
      pid: 308,
      ppid: 1,
    },
  ];
  ...
  it("#given a posix process table spanning every family #when classifying #then each family selects exactly its own zombies", () => {
    ...
    const proxies = selectOrphanedLspDaemonProxies(posixTable, {
      ownedRoots: [omoRoot],
      platform: "linux",
    });
    ...
    // then
    expect(
      codegraph.map((processInfo) => [processInfo.pid, processInfo.matchKind]),
    ).toEqual([
      [301, "serve-wrapper"],
    ...
    expect(
      proxies.map((processInfo) => [processInfo.pid, processInfo.matchKind]),
    ).toEqual([
  ...
  it("#given a windows process table spanning every family #when classifying #then windows shapes are selected per family", () => {
    ...
    const table: readonly ProcessInfo[] = [
      ...
      {
        command: `node ${winRoot}\\components\\codegraph\\dist\\serve.js`,
        pid: 311,
        ppid: 1,
      },
      {
        command:
          `node.exe ${winRoot}\\components\\lsp-daemon\\dist\\cli.js mcp`,
        pid: 312,
        ppid: 1,
      },
      {
      ...
      {
        command:
          `node.exe ${winRoot}\\components\\lsp-daemon\\dist\\cli.js daemon`,
        pid: 314,
        ppid: 1,
      },
    ];
    ...
    // when
    const codegraph = selectZombieCodegraphProcesses(table, {
      ownedRoots: [winRoot],
      platform: "win32",
    });
    const proxies = selectOrphanedLspDaemonProxies(table, {
      ownedRoots: [winRoot],
      platform: "win32",
    });
  ...
  it("#given a proxy-shaped command with a daemon token alongside mcp #when selecting proxies #then the daemon server shape is never matched", () => {
    ...
    const table: readonly ProcessInfo[] = [
      {
        command: `node ${omoRoot}/components/lsp-daemon/dist/cli.js mcp daemon`,
        pid: 320,
        ppid: 1,
      },
      {
  ...
  it("#given an mcp token that belongs to an unrelated cli #when selecting proxies #then it is ignored", () => {
    ...
    const table: readonly ProcessInfo[] = [
      {
        command: `node ${omoRoot}/components/other-tool/dist/cli.js mcp`,
        pid: 330,
        ppid: 1,
      },
      {
        command:
          `/usr/bin/python3 ${omoRoot}/components/lsp-daemon/dist/cli.js mcp`,
        pid: 331,
        ppid: 1,
      },
    ];
...
describe("orphaned lsp-daemon proxy sweep", () => {
  it("#given an orphaned proxy and a live-parent proxy #when sweeping #then only the orphan is killed and the family stamp is written", async () => {
    ...
    try {
      ...
      expect(result.stampFile).toBe(
        join(homeDir, ".omo", "lsp-daemon", "lsp-proxy-sweep.stamp"),
      );
      expect(existsSync(result.stampFile)).toBe(true);
      expect(
        existsSync(join(homeDir, ".omo", "codegraph", "zombie-sweep.stamp")),
      ).toBe(false);
    } finally {
...
describe("stale lsp-daemon version sweep", () => {
  it("#given a stale attested daemon and a live current daemon #when sweeping #then only the stale daemon is killed", async () => {
    ...
    try {
      ...
      // then
      expect(result.action).toBe("swept");
      expect(result.killed.map((target) => [target.pid, target.version]))
        .toEqual([[710, "0.0.1"]]);
  ...
  it("#given a stale version on Windows #when sweeping #then it is spared because ownership cannot be proven", async () => {
    ...
    try {
      ...
      // then
      expect(result.killed).toEqual([]);
      expect(result.spared.map((target) => [target.pid, target.reason]))
        .toEqual([
  ...
  it("#given non-version entries in the base dir #when sweeping #then they are ignored", async () => {
    ...
    try {
      ...
      // then
      expect(result.candidates.map((target) => target.version)).toEqual([
        "0.0.1",
      ]);
      expect(result.killed).toEqual([]);
    } finally {
...
describe("lsp-daemon cli attestation", () => {
  it("#given a linux /proc cmdline with node cli.js daemon #when attesting #then it passes", async () => {
    ...
    const cmdline = Buffer.from(
      "node\u0000/usr/lib/omo/lsp-daemon/dist/cli.js\u0000daemon\u0000",
    );

    // when / then
    expect(
      await attestLspDaemonCliProcess(801, "linux", {
  ...
  it("#given a linux /proc cmdline for the mcp proxy shape #when attesting #then it fails", async () => {
    // given
    const cmdline = Buffer.from(
      "node\u0000/usr/lib/omo/lsp-daemon/dist/cli.js\u0000mcp\u0000",
    );

    // when / then
    expect(
      await attestLspDaemonCliProcess(802, "linux", {
        readProcFile: () => Promise.resolve(cmdline),
      }),
    ).toBe(false);
  });
  ...
  it("#given a macOS ps command line with node cli.js daemon #when attesting #then it passes", async () => {
    ...
    expect(
      await attestLspDaemonCliProcess(804, "darwin", {
        executeForStdout: () =>
          Promise.resolve("node /usr/lib/omo/lsp-daemon/dist/cli.js daemon\n"),
      }),
    ).toBe(true);
    expect(
      await attestLspDaemonCliProcess(805, "darwin", {
        executeForStdout: () =>
          Promise.resolve("node /usr/lib/omo/lsp-daemon/dist/cli.js mcp\n"),
      }),
    ).toBe(false);
    expect(
      await attestLspDaemonCliProcess(806, "darwin", {
