...
function isArchiveAbsolutePath(filePath: string): boolean {
  ...
  return isAbsolute(normalizedPath) || /^[A-Za-z]:\//.test(normalizedPath) ||
...
function resolveContainedPath(
  ...
  if (isArchiveAbsolutePath(normalizedPath)) {
    throw new Error(
      `Unsafe archive entry: ${errorLabel} uses an absolute path (${filePath})`,
    );
  }

  if (containsTraversalSegment(normalizedPath)) {
    throw new Error(
      `Unsafe archive entry: ${errorLabel} contains path traversal (${filePath})`,
    );
  }
...
export function validateArchiveEntries(
  ...
  for (const entry of entries) {
    const resolvedEntryPath = resolveContainedPath(
      resolvedDestDir,
      entry.path,
      "path",
    );
    ...
    if (!entry.linkPath) {
      throw new Error(
        `Unsafe archive entry: ${
          entry.type === "symlink" ? "symlink" : "hard link"
        } target missing for ${entry.path}`,
      );
    }

    const normalizedLinkPath = normalizeArchivePath(entry.linkPath);
    const linkTypeLabel = entry.type === "symlink"
      ? "symlink target"
      : "hard link target";
