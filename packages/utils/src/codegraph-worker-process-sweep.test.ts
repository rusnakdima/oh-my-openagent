...
describe("CodeGraph worker process selection", () => {
  it("#given quoted real-world POSIX worker commands #when selecting zombies #then platform and provisioned workers are matched", () => {
    ...
    const processes = [
      {
        command:
          `${claudeRoot}/node_modules/@colbymchenry/codegraph-darwin-arm64/node --liftoff-only ${claudeRoot}/node_modules/@colbymchenry/codegraph-darwin-arm64/lib/dist/bin/codegraph.js status --json`,
        pid: 92635,
        ppid: 1,
      },
      {
        command:
          `${provisionedRoot}/node --liftoff-only ${provisionedRoot}/lib/dist/bin/codegraph.js status --json`,
        pid: 93383,
        ppid: 1,
      },
    ];
  ...
  it("#given an orphaned npm shim still owns a platform child #when selecting zombies #then the whole worker family is selected", () => {
    ...
    const processes = [
      {
        command:
          `node ${root}/node_modules/@colbymchenry/codegraph/npm-shim.js status --json`,
        pid: 94100,
        ppid: 1,
      },
      {
        command:
          `${root}/node_modules/@colbymchenry/codegraph-darwin-arm64/node --liftoff-only ${root}/node_modules/@colbymchenry/codegraph-darwin-arm64/lib/dist/bin/codegraph.js status --json`,
        pid: 94101,
        ppid: 94100,
      },
    ];
    ...
    // then
    expect(zombies.map((processInfo) => processInfo.pid)).toEqual([
      94100,
      94101,
    ]);
  });
  ...
  it("#given a live worker owns npm-shim and platform children #when selecting zombies #then none of its family is condemned", () => {
    ...
    const processes = [
      {
        command:
          `node ${root}/components/codegraph/dist/cli.js hook session-start-worker`,
        pid: 94200,
        ppid: 200,
      },
      {
        command:
          `node ${root}/node_modules/@colbymchenry/codegraph/npm-shim.js sync`,
        pid: 94201,
        ppid: 94200,
      },
      {
        command:
          `${root}/node_modules/@colbymchenry/codegraph-darwin-arm64/node --liftoff-only ${root}/node_modules/@colbymchenry/codegraph-darwin-arm64/lib/dist/bin/codegraph.js sync`,
        pid: 94202,
        ppid: 94201,
      },
  ...
  it("#given Windows platform and provisioned worker commands #when selecting zombies #then both shapes are matched", () => {
    // given
    const pluginRoot =
      "C:\\Users\\runner\\.codex\\plugins\\cache\\sisyphuslabs\\omo\\4.19.2";
  ...
  it("#given the default home roots #when discovering owned roots #then the Claude OMO install is included", () => {
    ...
    const roots = discoverCodegraphOwnedRoots({
      codexHome: join(homeDir, ".missing-codex"),
      homeDir,
    });
  ...
  it("#given a detached provisioned daemon #when selecting zombies #then daemon classification remains intact", () => {
    ...
    const command =
      `${root}/node --liftoff-only ${root}/lib/dist/bin/codegraph.js serve --mcp --path ${projectRoot}`;

    // when
    const zombies = selectZombieCodegraphProcesses([{
      command,
      pid: 95200,
      ppid: 1,
    }], {
    ...
    // then
    expect(
      zombies.map(({ daemonProjectRoot, matchKind }) => ({
        daemonProjectRoot,
        matchKind,
      })),
    ).toEqual([
