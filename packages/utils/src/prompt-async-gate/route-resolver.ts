...
export type PromptDispatchRouteResolver = {
  readonly tryResolveDispatchClientSync?: (
    client: unknown,
    sessionID: string,
  ) => PromptDispatchRouteResult | undefined;
  readonly resolveDispatchClient?: (
    client: unknown,
    sessionID: string,
  ) => Promise<PromptDispatchRouteResult>;
...
export function configurePromptDispatchRouteResolver(
  resolver: PromptDispatchRouteResolver | undefined,
): void {
...
export async function resolveDispatchClient(
  ...
  return routeResolver.resolveDispatchClient?.(client, sessionID) ??
    identityRoute(client);
}
