...
export function setExpiredReservationHandler(
...
function pruneExpiredReservations(now = Date.now()): void {
  ...
  for (const [sessionID, reservation] of promptAsyncReservations) {
    if (
      typeof reservation.expiresAt === "number" && reservation.expiresAt <= now
    ) {
...
export function getActiveReservation(
  sessionID: string,
): PromptAsyncReservation | undefined {
  pruneExpiredReservations();
  return promptAsyncReservations.get(sessionID);
}

export function getPromptReservation(
  sessionID: string,
): PromptAsyncReservation | undefined {
  return promptAsyncReservations.get(sessionID);
}

export function setPromptReservation(
  sessionID: string,
...
export function isTransientRetryReservationOwner(
...
export function reservationSourceMatches(
  ...
  if (
    supersedeTransientRetryOwners &&
    isTransientRetryReservationOwner(reservationSource)
  ) {
    return true;
  }
  ...
  const prefixes = typeof expectedPrefix === "string"
    ? [expectedPrefix]
    : expectedPrefix;
  return prefixes
