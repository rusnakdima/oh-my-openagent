  ...
  if (Array.isArray(value)) {
    ...
    try {
      return value.map((entry) =>
        canonicalizePromptInputForDedupe("", entry, seen, depth + 1)
      );
    } finally {
      seen.delete(value);
    }
  ...
  try {
    ...
    for (const entryKey of Object.keys(value).sort()) {
      canonicalEntries.push([
        entryKey,
        canonicalizePromptInputForDedupe(
          entryKey,
          value[entryKey],
          seen,
          depth + 1,
...
function hasContinuationPromptIntent(input: unknown): boolean {
  if (
    !isPlainRecord(input) || !isPlainRecord(input.body) ||
    !Array.isArray(input.body.parts)
  ) {
    return false;
  }
...
function stringifyPromptInputForDedupe(input: unknown): string {
  try {
    const serialized = JSON.stringify(
      normalizePromptInputForSemanticDedupe(input),
    );
...
export function coalesceRecentSemanticPromptDispatch(args: {
  ...
  const recentDispatch = getRecentPromptDispatch(
    args.sessionID,
    args.dedupeKey,
  );
  if (!recentDispatch) {
