...
export async function dispatchAfterSessionIdle<TInput>(args: {
  ...
  if (existing) {
    log(
      `[prompt-async-gate] ${sessionName} skipped because session is reserved`,
      {
        sessionID,
        source,
  ...
  try {
    const canReadStatus = checkStatus &&
    ...
    if (sessionActive) {
      log(
        `[prompt-async-gate] ${sessionName} skipped because session is active`,
        { sessionID, source },
      );
    ...
    if (
      ...
      log(
        `[prompt-async-gate] ${sessionName} skipped because latest assistant is still active`,
        {
          sessionID,
          source,
        },
    ...
    if (shouldDispatch?.() === false) {
      log(`[prompt-async-gate] ${sessionName} cancelled before dispatch`, {
        sessionID,
        source,
      });
    ...
    log(`[prompt-async-gate] ${sessionName} dispatching`, {
      sessionID,
      source,
    });
