...
export function messageFinish(message: unknown): string | true | undefined {
  ...
  return typeof message.finish === "string" && message.finish.length > 0
    ? message.finish
    : undefined;
}
...
function toInternalInitiatorTextPartLike(
  part: unknown,
): InternalInitiatorTextPartLike {
...
function toInternalInitiatorMessageLike(
  message: unknown,
): InternalInitiatorMessageLike | undefined {
  if (!isRecord(message)) {
    return undefined;
  }
...
export function messageIsSyntheticOrInternalUser(message: unknown): boolean {
  ...
  return initiatorMessage !== undefined &&
    isSyntheticOrInternalUserMessage(initiatorMessage);
}

export function messageIsTerminalNoReplyUser(message: unknown): boolean {
  const initiatorMessage = toInternalInitiatorMessageLike(message);
  return initiatorMessage !== undefined &&
    isTerminalNoReplyUserMessage(initiatorMessage);
}
...
function partIsQuestionTool(part: unknown): boolean {
  ...
  return toolName !== undefined &&
    QUESTION_TOOL_NAMES.has(toolName.toLowerCase());
}
...
export function messageHasQuestionTool(message: unknown): boolean {
  return isRecord(message) && Array.isArray(message.parts) &&
    message.parts.some(partIsUnansweredQuestionTool);
}

export function messageHasWaitingTool(message: unknown): boolean {
  return isRecord(message) && Array.isArray(message.parts) &&
    message.parts.some(partIsWaitingOnTool);
}

export function messageHasUnresolvedTool(message: unknown): boolean {
  return isRecord(message) && Array.isArray(message.parts) &&
    message.parts.some(partIsUnresolvedTool);
}

export function messageHasSubstantiveAssistantOutput(
  message: unknown,
): boolean {
  return isRecord(message) && Array.isArray(message.parts) &&
    message.parts.some(partHasSubstantiveAssistantOutput);
}
