...
function setPromptQueue(
  sessionID: string,
  queue: QueuedInternalPrompt[],
): void {
  if (queue.length === 0) {
...
export function schedulePromptQueueDrain(
  sessionID: string,
  delayMs: number,
): void {
...
function removePromptQueueEntry(
  sessionID: string,
  entry: QueuedInternalPrompt,
): void {
  const queue = promptQueues.get(sessionID);
...
export function releaseInFlightPromptMatchingDedupe(
  sessionID: string,
  dedupeKey: string,
): void {
  const inFlight = promptQueueInFlight.get(sessionID);
...
): Promise<InternalPromptDispatchResult | undefined> {
...
export async function enqueueInternalPrompt(
  entry: QueuedInternalPrompt,
  ...
  const existingIndex = queue.findIndex((queued) =>
    queued.dedupeKey === entry.dedupeKey
  );
  if (existingIndex >= 0) {
