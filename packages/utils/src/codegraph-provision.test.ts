...
describe("ensureCodegraphProvisioned", () => {
  ...
  it(
    "extracts a verified release archive and installs bin/codegraph",
    async () => {
      // given
      ...
      expect(result).toEqual({
        binPath: join(installDir, "bin", "codegraph"),
        provisioned: true,
      });
      expect(readFileSync(join(installDir, "bin", "codegraph"), "utf8"))
        .toContain("codegraph fixture");
  ...
  it("declares pinned Windows npm package assets in the default manifest", () => {
    ...
    expect(win32X64).toEqual({
      ...
      sha256:
        "ef64c878acb129885c2d8306ddec6674af865810b4c0f6a9ba9fcd61e21ff9d8",
      url:
    ...
    expect(win32Arm64).toEqual({
      executableName: "codegraph.cmd",
      sha256:
        "19e0237ea5d8928f705d60e339eb319e7ec37490a69585712933c1534f3c0bc2",
      url:
  ...
  it("extracts a verified npm tgz archive and installs bin/codegraph.cmd", async () => {
    ...
    // then
    expect(result).toEqual({
      binPath: join(installDir, "bin", "codegraph.cmd"),
      provisioned: true,
    });
    expect(readFileSync(join(installDir, "bin", "codegraph.cmd"), "utf8"))
      .toContain("codegraph fixture");
  ...
  it("serializes concurrent calls with a per-host lock", async () => {
    ...
    // then
    expect(
      results.every((result: { readonly provisioned: boolean }) =>
        result.provisioned
      ),
    ).toBe(true);
    expect(downloads).toBe(1);
