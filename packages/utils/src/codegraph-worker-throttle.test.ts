...
describe("CodeGraph worker sweep cadence", () => {
  it("#given a fresh daemon sweep stamp #when an orphaned worker appears #then worker reaping still runs", async () => {
    ...
    const homeDir = mkdtempSync(
      join(tmpdir(), "omo-codegraph-worker-throttle-"),
    );
    ...
    try {
      ...
      const workerCommand =
        `${installDir}/node --liftoff-only ${installDir}/lib/dist/bin/codegraph.js status --json`;
      const daemonCommand =
        `${installDir}/node --liftoff-only ${installDir}/lib/dist/bin/codegraph.js serve --mcp --path /tmp/project`;
  ...
  it("#given a live daemon lock and an orphaned worker #when both sweep lanes run #then the worker is killed and the daemon is spared", async () => {
    ...
    try {
      ...
      writeFileSync(
        join(projectRoot, ".codegraph", "daemon.pid"),
        `${daemonPid}\n`,
      );
      const workerCommand =
        `${installDir}/node --liftoff-only ${installDir}/lib/dist/bin/codegraph.js sync`;
      const daemonCommand =
