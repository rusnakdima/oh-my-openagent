...
describe("ensureCodegraphProvisioned version upgrades", () => {
  for (const staleVersion of ["1.0.1", "1.4.1"] as const) {
    it(
      `#given a managed ${staleVersion} runtime #when provisioning the current pin #then it replaces the stale runtime`,
      async () => {
        // given
        const root = mkdtempSync(
          join(tmpdir(), "omo-codegraph-provision-stale-marker-"),
        );
        ...
        writeFileSync(
          staleMarkerPath,
          `${JSON.stringify({ binPath: staleBin, version: staleVersion })}\n`,
        );
        ...
        try {
          ...
          expect(readFileSync(staleBin, "utf8")).toContain(
            "upgraded codegraph",
          );
          expect(
            JSON.parse(
  ...
  it("#given a stale managed runtime #when the replacement archive fails verification #then the previous runtime remains intact", async () => {
    // given
    const root = mkdtempSync(
      join(tmpdir(), "omo-codegraph-provision-atomic-upgrade-"),
    );
    const installDir = join(root, "install");
    const staleBin = join(installDir, "bin", "codegraph");
    mkdirSync(join(installDir, "bin"), { recursive: true });
  ...
  it(
    "#given a managed binary whose marker version does not match the requested pin #when provisioning #then it replaces the stale runtime",
    async () => {
      // given
      const root = mkdtempSync(
        join(tmpdir(), "omo-codegraph-provision-upgrade-"),
      );
      const installDir = join(root, "install");
      const staleBin = join(installDir, "bin", "codegraph");
      ...
      writeFileSync(
        markerPath,
        `${JSON.stringify({ binPath: staleBin, version: "1.0.1" })}\n`,
      );
      const archive = createArchive(root);
      let downloads = 0;
