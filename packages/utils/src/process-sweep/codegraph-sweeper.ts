...
export interface SweepCodegraphZombiesOptions
  extends CodegraphOwnedRootsOptions, ProcessFamilySweepOptions {
...
export interface SweepCodegraphZombiesResult
  extends ProcessFamilySweepResult<CodegraphZombieProcess> {
...
export async function sweepCodegraphZombies(
  options: SweepCodegraphZombiesOptions = {},
): Promise<SweepCodegraphZombiesResult> {
  const homeDir = options.homeDir ?? options.env?.["HOME"] ??
    options.env?.["USERPROFILE"] ?? homedir();
  ...
  const collectCandidates = (): Promise<readonly CodegraphZombieProcess[]> => {
    candidatesPromise ??= provider().then((processes) =>
      selectZombieCodegraphProcesses(processes, {
        ownedRoots,
    ...
    runProcessFamilySweep<CodegraphZombieProcess>(
      {
        ...
        collect: async () => {
          const candidates = (await collectCandidates()).filter((candidate) =>
            candidate.matchKind !== "upstream-daemon"
          );
    ...
    runProcessFamilySweep<CodegraphZombieProcess>(
      {
        ...
        collect: async () => {
          ...
          const { killList, spared } = partitionByDaemonStaleness(
            candidates,
            options.log,
          );
...
function partitionByDaemonStaleness(
  ...
  readonly killList: readonly CodegraphZombieProcess[];
  ...
  for (const candidate of candidates) {
    const staleness = evaluateDaemonStaleness(
      candidate.pid,
      candidate.daemonProjectRoot ?? candidate.matchedRoot,
    );
    if (staleness.stale) {
      log?.(
        `CodeGraph daemon sweep sweeping stale daemon pid ${candidate.pid} (${staleness.reason})`,
      );
      killList.push(candidate);
      continue;
    }
    log?.(
      `CodeGraph daemon sweep spared live daemon pid ${candidate.pid} (${staleness.reason})`,
    );
...
function aggregateAction(
  workerAction: ProcessSweepAction,
  daemonAction: ProcessSweepAction,
): ProcessSweepAction {
