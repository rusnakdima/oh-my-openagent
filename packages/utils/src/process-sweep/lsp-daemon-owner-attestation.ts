...
export async function attestLspDaemonOwner(
  ...
  try {
    ...
    const currentOwner = parseLspDaemonOwner(
      JSON.parse(await readText(target.ownerPath)),
    );
    if (currentOwner === null || !sameOwner(currentOwner, target.owner)) {
      return false;
    }
...
export function parseLspDaemonOwner(
  value: unknown,
): LspDaemonOwnerIdentity | null {
  ...
  if (kind !== "unix" || typeof path !== "string" || path.length === 0) {
    return null;
  }
...
export function createLspDaemonOwnerPingRequest(
  authToken: string,
  ...
  return {
    ...
    method: "omo/ping",
    params: {
      _omo: { protocolVersion: OMO_DAEMON_PROTOCOL_VERSION, token: authToken },
    },
  };
}
...
async function pingLspDaemonOwner(
  ...
  return new Promise((resolvePromise) => {
    ...
    socket.once("connect", () => {
      socket.write(
        `${JSON.stringify(createLspDaemonOwnerPingRequest(authToken))}\n`,
      );
    });
    socket.on("data", (chunk: string) => {
      ...
      try {
        ...
        if (
          !isRecord(response) || response["id"] !== OWNER_PING_REQUEST_ID ||
          "error" in response
        ) {
          return finish(null);
        }
...
function sameOwner(
  left: LspDaemonOwnerIdentity,
  right: LspDaemonOwnerIdentity,
): boolean {
