...
export function enumerateProcesses(
  ...
  return platform === "win32"
    ...
    : enumeratePosixProcesses();
}

export function createDefaultProcessKiller(
  platform: NodeJS.Platform = process.platform,
): ProcessKiller {
...
function enumeratePosixProcesses(): Promise<ProcessInfo[]> {
  return execFileText("ps", ["-eo", "pid=,ppid=,command="]).then(
    parsePosixProcessTable,
  );
}
...
function enumerateWindowsProcesses(): Promise<ProcessInfo[]> {
  ...
  return execFileText("powershell.exe", ["-NoProfile", "-Command", command])
    .then(parseWindowsProcessTable);
}
...
function createWindowsKiller(): ProcessKiller {
  return {
    isAlive: defaultIsProcessAlive,
    kill: (pid) =>
      execFileVoid("taskkill.exe", ["/PID", String(pid), "/T", "/F"]),
    terminate: (pid) =>
      execFileVoid("taskkill.exe", ["/PID", String(pid), "/T"]),
  };
}
