...
const LSP_DAEMON_CLI_SUFFIXES = [
  "/lsp-daemon/dist/cli.js",
  "/lsp-daemon/src/cli.ts",
] as const;
...
export function selectOrphanedLspDaemonProxies(
  ...
  for (const processInfo of processes) {
    const matchedRoot = matchLspDaemonProxyCommand(
      processInfo.command,
      roots,
      platform,
    );
...
function matchLspDaemonProxyCommand(
  ...
  for (const root of roots) {
    ...
    for (const suffix of LSP_DAEMON_CLI_SUFFIXES) {
      if (
        hasExecutableTokenUnderRootWithSuffix(normalizedCommand, root, suffix)
      ) return root;
    }
  }
  return null;
