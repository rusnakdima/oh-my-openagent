...
import { buildCodegraphEnv, CODEGRAPH_INSTALL_DIR_ENV } from "../codegraph/env";
 ...
 */
export function discoverCodegraphOwnedRoots(
  options: CodegraphOwnedRootsOptions = {},
): string[] {
  ...
  for (
    const root of readCodexPluginCacheRoots(
...
function readCodexPluginCacheRoots(codexHome: string): string[] {
  ...
  for (const publisher of safeReadDir(cacheRoot)) {
    ...
    for (const version of safeReadDir(omoRoot)) {
      roots.push(join(omoRoot, version));
    }
  }
  return roots;
}
