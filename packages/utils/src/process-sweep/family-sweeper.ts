...
export interface ProcessFamilySweepResult<
  TTarget extends { readonly pid: number },
  TSpared extends TTarget = TTarget,
> {
...
export interface FamilySweepConfig<
  TTarget extends { readonly pid: number },
  TSpared extends TTarget = TTarget,
> {
...
export async function runProcessFamilySweep<
  TTarget extends { readonly pid: number },
  TSpared extends TTarget = TTarget,
>(
...
async function killTargets<TTarget extends { readonly pid: number }>(
  ...
  for (const target of targets) {
    if (
      !(await passesSignalAttestation(
        target,
        config.attestBeforeSignal,
        options.log,
        "SIGTERM",
      ))
    ) continue;
...
async function passesSignalAttestation<
  TTarget extends { readonly pid: number },
>(
  target: TTarget,
  ...
  try {
    ...
    if (!attested) {
      log?.(
        `process sweep sparing pid ${target.pid}: identity changed before ${signal}`,
      );
    }
    return attested;
  } catch (error) {
    log?.(
      `process sweep sparing pid ${target.pid}: identity attestation failed before ${signal}: ${
        String(error)
      }`,
    );
    return false;
...
async function safelyTerminate(
  ...
  try {
    ...
    context.log?.(
      `${context.familyLabel} failed to terminate pid ${pid}: ${message}`,
    );
    return false;
  }
}
...
async function safelyKill(pid: number, context: KillContext): Promise<boolean> {
  try {
    ...
    context.log?.(
      `${context.familyLabel} failed to kill pid ${pid}: ${message}`,
    );
    return false;
  }
}

function isSweepThrottled(
  stampFile: string,
  nowMs: number,
  throttleMs: number,
): boolean {
