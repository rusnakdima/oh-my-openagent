...
export function hasExecutableToken(
  command: string,
  expectedPath: string,
): boolean {
  let searchFrom = 0;
  for (;;) {
    ...
    if (
      command.slice(tokenStart, tokenEnd) === expectedPath &&
 ...
 */
export function hasExecutableTokenUnderRootWithSuffix(
  command: string,
  root: string,
  suffix: string,
): boolean {
  ...
  for (;;) {
    ...
    if (
      token.endsWith(suffix) && token.startsWith(`${root}/`) &&
      tokenLooksExecutable(command, tokenStart)
    ) return true;
...
export function tokenLooksExecutable(
  command: string,
  tokenStart: number,
): boolean {
  ...
  for (;;) {
    ...
    if (!previousToken.startsWith("-")) {
      ...
      return /^node\d*(\.exe)?$/i.test(executableName) ||
  ...
  for (const root of roots) {
    ...
    normalized.add(
      normalizeForComparison(
        resolvePathForPlatform(trimmed, platform),
        platform,
      ),
    );
  }
...
export function resolvePathForPlatform(
  value: string,
  platform: NodeJS.Platform,
): string {
