...
export type CodegraphProcessMatchKind =
  | "serve-wrapper"
  | "upstream-codegraph"
  | "upstream-daemon";
...
const UPSTREAM_PACKAGE_PATTERN =
  /\/@colbymchenry\/codegraph(?:-(?:darwin|linux|win32)-(?:arm64|x64))?\//g;
...
export function selectZombieCodegraphProcesses(
  ...
  const daemonMatches = new Map<
    number,
    { readonly projectRoot: string; readonly root: string }
  >();
  const workerMatches = new Map<
    number,
    { readonly kind: CodegraphProcessMatchKind; readonly root: string }
  >();
  ...
  for (const processInfo of processes) {
    ...
    const worker = matchOwnedCodegraphCommand(
      processInfo.command,
      roots,
      platform,
    );
  ...
  for (const processInfo of processes) {
    ...
    if (
      !isOrphaned(processInfo, livePids) &&
      !hasOrphanedWorkerAncestor(processInfo, ancestry)
    ) continue;
    zombies.push({
      ...processInfo,
...
const STANDALONE_LAUNCHER_SUFFIXES = [
  "/bin/codegraph",
  "/bin/codegraph.exe",
] as const;
...
function upstreamPackagePathIsUnderRoot(
  command: string,
  root: string,
): boolean {
  ...
  for (;;) {
    ...
    if (
      command.slice(tokenStart).startsWith(`${root}/`) &&
      tokenLooksExecutable(command, tokenStart)
    ) return true;
  }
}
...
function hasOrphanedWorkerAncestor(
  ...
  ancestry: {
    ...
    readonly workerMatches: ReadonlyMap<
      number,
      { readonly kind: CodegraphProcessMatchKind; readonly root: string }
    >;
  },
