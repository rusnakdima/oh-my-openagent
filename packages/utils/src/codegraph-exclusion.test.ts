...
describe("CodeGraph project exclusion policy", () => {
  it("excludes default tmp roots and OMO state directories from CodeGraph projects", () => {
    // given
    const tmpWorkspace = process.platform === "win32"
      ? null
      : mkdtempSync(join("/tmp", "omo-codegraph-excluded-"));

    try {
      // then
      if (tmpWorkspace !== null) {
        expect(
          shouldExcludeCodegraphProject(tmpWorkspace, {
            platform: "linux",
      ...
      expect(
        shouldExcludeCodegraphProject(
          "/private/tmp/omo-codegraph-scratch/repo",
          {
            platform: "linux",
            tmpdir: "/tmp/omo-current-temp",
          },
      ...
      expect(
        shouldExcludeCodegraphProject(
          "C:\\Users\\x\\AppData\\Local\\Temp\\Repo",
          {
            platform: "win32",
      ...
      expect(
        shouldExcludeCodegraphProject("/Users/alice/repo", {
          platform: "darwin",
          tmpdir: "/var/folders/ab/xyz/T",
        }),
      ...
      expect(
        shouldExcludeCodegraphProject(
          "/Users/alice/repo/.omo/ulw-research/run/clones/repo",
        ),
      ).toEqual({
        excluded: true,
        matchedRoot: ".omo",
      ...
      if (tmpWorkspace !== null) {
        rmSync(tmpWorkspace, { force: true, recursive: true });
      }
    }
  });
