...
async function runControlledExitOne(
  directory: string,
  executable: string,
  fixture: string,
  mode: string,
...
describe("bounded sg NDJSON runner", () => {
  it("#given complete chunked NDJSON #when sg exits #then every record is parsed", async () => {
    ...
    const result = await spawnSgRunner({
      sgPath: executable,
      args: [fixture, "complete"],
      workdir: directory,
      maxMatches: 10,
    });
  ...
  it(
    "#given sg run exit 1 with an operational diagnostic #when stdout is empty #then the call rejects as SG_FAILED",
    async () => {
      const { directory, executable, fixture } = fixtureRunner();
      const outcome = await runControlledExitOne(
        directory,
        executable,
        fixture,
        "exit-1-controlled-error",
        "exit-1-error",
      );
  ...
  it(
    "#given an ERROR line after the retained stderr cap #when sg exits 1 #then the call rejects as SG_FAILED",
    async () => {
      const { directory, executable, fixture } = fixtureRunner();
      const outcome = await runControlledExitOne(
        directory,
        executable,
        fixture,
        "exit-1-controlled-flood",
        "exit-1-flood",
      );

      expect(outcome).toMatchObject({
        status: "rejected",
        error: { code: "SG_FAILED", durationMs: expect.any(Number) },
      });
    },
    4_000,
  );
  ...
  it(
    "#given an ERROR prefix split across stderr writes after the cap #when sg exits 1 #then the call rejects as SG_FAILED",
    async () => {
      const { directory, executable, fixture } = fixtureRunner();
      const outcome = await runControlledExitOne(
        directory,
        executable,
        fixture,
        "exit-1-controlled-split",
        "exit-1-split",
      );

      expect(outcome).toMatchObject({
        status: "rejected",
        error: { code: "SG_FAILED" },
      });
    },
    4_000,
  );
  ...
  it("#given sg run exit 1 with a warning only #when stdout is empty #then it remains a valid no-match", async () => {
    ...
    expect(result).toMatchObject({
      records: [],
      exitCode: 1,
      stderr: "WARNING: no matches in ignored files",
    });
  });
  ...
  it(
    "#given records exceeding the aggregate payload cap #when the next record arrives #then output is truncated and the child is terminated",
    async () => {
      const { directory, executable, fixture } = fixtureRunner();
      ...
      const running = spawnSgRunner({
        sgPath: executable,
        args: [fixture, "payload-cap", marker],
        workdir: directory,
        maxMatches: 500,
      });
      await ready;
      ...
      expect(Buffer.byteLength(JSON.stringify(result.records), "utf8"))
        .toBeLessThanOrEqual(MAX_MCP_PAYLOAD_BYTES);
  ...
  it("#given invalid UTF-8 and a split multibyte boundary #when stderr is decoded #then returned UTF-8 stays within the byte cap", async () => {
    const { directory, executable, fixture } = fixtureRunner();
    const invalid = await spawnSgRunner({
      sgPath: executable,
      args: [fixture, "stderr-invalid"],
      workdir: directory,
    });
    const boundary = await spawnSgRunner({
      sgPath: executable,
      args: [fixture, "stderr-boundary"],
      workdir: directory,
    });

    expect(Buffer.byteLength(invalid.stderr)).toBeLessThanOrEqual(
      MAX_STDERR_BYTES,
    );
    expect(boundary.stderr).toBe("a".repeat(MAX_STDERR_BYTES - 1));
    expect(Buffer.byteLength(boundary.stderr)).toBeLessThanOrEqual(
      MAX_STDERR_BYTES,
    );
  });

  it.skipIf(process.platform === "win32")(
    "#given a child that ignores SIGTERM #when aborted #then SIGKILL follows the grace period and no process remains",
    async () => {
      const { directory, executable, fixture } = fixtureRunner();
      ...
      await terminated;
      expect(await withTimeout(outcome, "aborted child exit")).toMatchObject({
        status: "rejected",
        error: { code: "ABORTED" },
      });
  ...
  it.skipIf(process.platform !== "win32")(
    "#given a hung child on Windows #when aborted #then child.kill terminates it without POSIX grace signaling",
    async () => {
      const { directory, executable, fixture } = fixtureRunner();
      ...
      expect(await withTimeout(outcome, "Windows aborted child exit"))
        .toMatchObject({ status: "rejected", error: { code: "ABORTED" } });
  ...
  it.skipIf(process.platform === "win32")(
    "#given a hung child #when the whole-call budget expires #then timeout termination is bounded",
    async () => {
      const { directory, executable, fixture } = fixtureRunner();
      ...
      await terminated;
      expect(await withTimeout(outcome, "timed-out child exit")).toMatchObject({
        status: "rejected",
        error: { code: "TIMEOUT" },
      });
      const pid = Number(readFileSync(`${marker}.pid`, "utf8"));
  ...
  it.skipIf(process.platform !== "win32")(
    "#given a hung child on Windows #when the whole-call budget expires #then child.kill terminates it without POSIX grace signaling",
    async () => {
      const { directory, executable, fixture } = fixtureRunner();
      ...
      expect(await withTimeout(outcome, "Windows timed-out child exit"))
        .toMatchObject({ status: "rejected", error: { code: "TIMEOUT" } });
      expect(existsSync(`${marker}.term`)).toBe(false);
      const pid = Number(readFileSync(`${marker}.pid`, "utf8"));
      expect(() => process.kill(pid, 0)).toThrow();
    },
