...
describe.skipIf(!SG_AVAILABLE)(
  "search tool: live fixture (ub §5/§6 contract)",
  () => {
    it("#given a fixture repo with console.log #when searching #then returns ub §5 success payload", async () => {
      ...
      expect(payload.matches.every((m) => m.path.startsWith("src/"))).toBe(
        true,
      );
      expect(
        payload.matches.every((m) => m.metavariables.single.MSG !== undefined),
      ).toBe(true);
    ...
    it("#given maxMatches=1 #when searching #then result is truncated with limit warning", async () => {
      ...
      expect(payload.warnings).toContain(
        "Result limit reached; narrow paths or globs.",
      );
    });
    ...
    it("#given includeHidden=true #when searching #then args include --no-ignore hidden and results return", async () => {
      ...
      expect(payload.matches.some((m) => m.path.includes(".hidden"))).toBe(
        true,
      );
    });
  },
