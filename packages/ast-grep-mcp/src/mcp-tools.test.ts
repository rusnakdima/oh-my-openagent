...
import {
  AST_GREP_ERROR_CODES,
  type AstGrepMcpOptions,
...
function stubOptions(
  overrides: Partial<AstGrepMcpOptions> = {},
): AstGrepMcpOptions {
  return { resolveSgPath: () => SG_PATH, ...overrides };
}

function payloadOf(
  response: Awaited<ReturnType<typeof handleAstGrepMcpRequest>>,
...
describe("ast_grep MCP tools/call dispatch", () => {
  it("#given a known tool #when called #then the matching execute function receives coerced args and sgPath", async () => {
      ...
      const response = await handleAstGrepMcpRequest(
        {
          ...
          id: name,
          method: "tools/call",
          params: { name, arguments: argsFor[name] },
        },
        options,
      );
      // then
    ...
    expect(seen.map((entry) => entry.tool)).toEqual([
      "search",
      "rewrite",
      "scan",
    ]);
  ...
  it("#given search tools/call with an UNKNOWN property #when dispatched #then additionalProperties:false is enforced", async () => {
    ...
    const response = await handleAstGrepMcpRequest(
      {
        ...
        params: {
          name: "search",
          arguments: { ...VALID_SEARCH_ARGS, unknown: true },
        },
      },
      stubOptions({
        executors: {
    ...
    expect(response?.result?.isError).toBe(true);
    const payload = payloadOf(response) as {
      error: { code: string; message: string; language: string };
    };
    expect(payload.error.code).toBe("INVALID_ARGUMENT");
    ...
    const response = await handleAstGrepMcpRequest(
      {
        ...
        id: 4,
        method: "tools/call",
        params: { name: "mcp__ast_grep__search", arguments: {} },
      },
      stubOptions(),
    );
    ...
    const response = await handleAstGrepMcpRequest(
      {
        ...
        id: 6,
        method: "tools/call",
        params: { name: "search", arguments: VALID_SEARCH_ARGS },
      },
      stubOptions({
        executors: {
          search: async () => {
  ...
  it("#given no resolvable sg binary #when a tool is called #then BINARY_NOT_FOUND isError with install hints", async () => {
    const response = await handleAstGrepMcpRequest(
      ...
      {
        resolveSgPath: () => {
          throw Object.assign(new Error("ast-grep binary not found"), {
            hints: ["brew install ast-grep"],
          });
        },
      },
    );
