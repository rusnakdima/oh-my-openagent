...
function nodeByteRange(value: unknown): { start: number; end: number } | null {
  ...
  const range = object(
    (value as Record<string, unknown>).range,
    "metavariable range",
  );
...
function normalizeMetavariables(
  ...
  const singles = meta.single === undefined
    ? {}
    : object(meta.single, "single metavariables");
  const multis = meta.multi === undefined
    ? {}
    : object(meta.multi, "multi metavariables");
...
export function normalizeMatch(
  ...
  for (const [key, value] of Object.entries(raw)) {
    if (!consumed.has(key)) normalized[key] = value;
  }
...
export function normalizeRecords(
  ...
  return records
    ...
    .sort((left, right) => {
      const pathOrder = left.path < right.path
        ? -1
