...
export function extractApplyPatchEdits(
  ...
  const patch = args === undefined
    ? undefined
    : getString(args, ["patchText", "input", "patch", "command"]);
  if (patch === undefined) return [];

  return parseApplyPatchRequests(patch);
}

export function getApplyPatchMetadataFiles(
  details: unknown,
): ApplyPatchFileMetadata[] {
  ...
  const result = isRecord(resultDetails)
    ? readApplyPatchMetadataFiles(resultDetails["files"])
    : [];
  if (result.length > 0) return result;

  const metadataDetails = details["metadata"];
  return isRecord(metadataDetails)
    ? readApplyPatchMetadataFiles(metadataDetails["files"])
    : [];
}

export function readApplyPatchMetadataFiles(
  value: unknown,
): ApplyPatchFileMetadata[] {
