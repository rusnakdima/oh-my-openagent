...
export function resolveCommentCheckerBinary(
  input: ResolveCommentCheckerBinaryInput,
): string | null {
  const packageName = input.packageName ?? "@code-yeongyu/comment-checker";

  if (
    input.cachedBinaryPath !== null && input.existsSync(input.cachedBinaryPath)
  ) {
...
export async function runCommentChecker(
  ...
  try {
    ...
    const completed = Promise.all(
      [stdoutPromise, stderrPromise, exitCodePromise] as const,
    );
