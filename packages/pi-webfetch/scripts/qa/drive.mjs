...
const FIXTURE_HTML =
  "<html><body><h1>Hello Webfetch</h1><p>Alpha <strong>Beta</strong></p><script>bad()</script></body></html>";
...
function runRpcScenario(piBin, sandbox, fixtureUrl, report) {
          ...
          steps: [
            {
              type: "tool_call",
              name: "webfetch",
              arguments: { url: fixtureUrl, format: "markdown" },
            },
  ...
  return new Promise((resolveScenario) => {
    ...
    child.stdin.write(
      `${
        JSON.stringify({ type: "prompt", message: "fetch the fixture page" })
      }\n`,
    );
  });
}
...
function runSelfTest() {
  ...
  try {
    ...
    if (!existsSync(webfetchExtensionEntry)) {
      throw new Error("webfetch extension entry missing");
    }
...
async function main() {
  ...
  try {
    ...
    report.markdownConverted =
      sandboxText.includes(EXPECTED_MARKDOWN_HEADING) &&
      sandboxText.includes("Alpha **Beta**");
    report.scriptTagStripped = !sandboxText.includes("bad()");

    if (report.markdownConverted && report.scriptTagStripped) {
      report.result = "PASS";
    ...
    if (report.result === "PASS" || report.result === "SKIP") {
      rmSync(sandbox.root, { recursive: true, force: true });
    }
