...
- Vendored source: keep diffs against upstream intentional and reviewable. Tests
  ...
  (so `server.close()` would otherwise hang), and the two oversized tests plus
  the Cloudflare-challenge test drop the undici-specific server-side
  socket-close proxy in favor of asserting the production contract (reject
  oversized, retry past the challenge).
...
- Not wired into any OpenCode/Codex/omo-senpi component. Ships as a standalone
  Pi package surface (`pi.extensions` manifest field).
