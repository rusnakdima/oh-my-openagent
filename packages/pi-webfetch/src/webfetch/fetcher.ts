...
export async function fetchUrl(options: FetchOptions): Promise<FetchResult> {
  ...
  const timeout = setTimeout(
    () =>
      controller.abort(
        new WebfetchTimeoutError(`Request timed out after ${timeoutSeconds}s`),
      ),
    timeoutSeconds * 1000,
  );
...
export function validateUrl(url: string): void {
  if (!url.startsWith("http://") && !url.startsWith("https://")) {
    throw new InvalidWebfetchUrlError(
...
async function rejectOversizedContentLength(response: Response): Promise<void> {
  ...
  if (
    contentLength &&
    Number.parseInt(contentLength, 10) > MAX_RESPONSE_SIZE_BYTES
  ) {
    await cancelBody(response);
    throw new WebfetchResponseTooLargeError(
