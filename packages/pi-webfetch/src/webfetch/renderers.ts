...
export function renderWebfetchCall(args: unknown, theme: Theme): Text {
  ...
  const timeout = webfetchArgs.timeout === undefined
    ? ""
    : theme.fg("dim", ` ${webfetchArgs.timeout}s`);
...
export function renderWebfetchResult(
    ...
    if (isWebfetchProgressDetails(details)) {
      return new Text(
        theme.fg(
          "warning",
          `Fetching ${
            shorten(details.url, URL_BUDGET)
          } as ${details.format} (${details.timeoutSeconds}s)`,
        ),
        0,
        0,
      );
  ...
  if (!isWebfetchDetails(details)) {
    return new Text(
      theme.fg("muted", truncateToWidth(text, PREVIEW_WIDTH)),
      0,
      0,
    );
  }

  const statusKey = details.status >= 200 && details.status < 300
    ? "success"
    : "warning";
  const status = theme.fg(
    statusKey,
    `${details.status} ${details.statusText || "OK"}`,
  );
  ...
  const header = `${status} ${theme.fg("muted", "•")} ${format} ${
    theme.fg("muted", "•")
  } ${size}${converted}`;
  ...
  const lines = [
    ...
    "",
    ...collectLines(text, 24).map((line) =>
      theme.fg("toolOutput", truncateToWidth(line, PREVIEW_WIDTH))
    ),
  ];
...
function collectNonEmptyTrimmedLines(text: string, limit: number): string[] {
  ...
  while (lines.length < limit && start <= text.length) {
    ...
    const line = (newlineIndex === -1
      ? text.slice(start)
      : text.slice(start, newlineIndex)).trim();
    if (line.length > 0) {
      lines.push(line);
    }
    if (newlineIndex === -1) {
...
function isWebfetchProgressDetails(
  details: WebfetchRenderDetails | unknown,
): details is WebfetchProgressDetails {
  return typeof details === "object" && details !== null &&
    "phase" in details && details.phase === "fetching";
}

function isWebfetchDetails(
  details: WebfetchRenderDetails | unknown,
): details is WebfetchDetails {
  return typeof details === "object" && details !== null &&
