...
function textContent(
  result: Awaited<ReturnType<typeof executeWebfetch>>,
): string {
...
describe("webfetch", () => {
  it("#given url fetch #when execution starts #then emits progress details for the TUI", async () => {
    ...
    expect(updates[0]).toMatchObject({
      content: [{
        type: "text",
        text: `Fetching ${server.baseUrl}/ready as text (timeout 7s)`,
      }],
      details: {
        phase: "fetching",
  ...
  it("#given html page #when fetching text #then returns readable text without tags", async () => {
    ...
    const server = await createFixtureServer((_request, response) => {
      ...
      response.end(
        "<main><h1>Title</h1><p>One&nbsp;Two</p><style>.x{}</style></main>",
      );
    });

    // when
    const result = await executeWebfetch({
      url: `${server.baseUrl}/text`,
      format: "text",
    });
  ...
  it("#given invalid scheme #when fetching #then rejects before network access", async () => {
    // given / when / then
    await expect(
      executeWebfetch({ url: "file:///tmp/secret", format: "markdown" }),
    ).rejects.toThrow(
  ...
  it("#given oversized content length #when fetching #then rejects before buffering the body", async () => {
    ...
    // is what this proves.
    await expect(
      executeWebfetch({ url: `${server.baseUrl}/large`, format: "text" }),
    ).rejects.toThrow(
  ...
  it("#given oversized stream #when fetching #then rejects once the byte cap is exceeded", async () => {
    ...
    await expect(
      executeWebfetch({ url: `${server.baseUrl}/stream`, format: "text" }),
    ).rejects.toThrow(
      "Response too large (exceeds 5MB limit)",
    );
  });
