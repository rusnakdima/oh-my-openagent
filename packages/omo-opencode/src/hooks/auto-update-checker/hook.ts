...
interface AutoUpdateCheckerDeps {
  ...
  updateAndShowConnectedProvidersCacheStatus:
    typeof updateAndShowConnectedProvidersCacheStatus;
    ...
    event: ({ event }: { event: { type: string; properties?: unknown } }) => {
      ...
      scheduleDeferredStartupCheck(() => {
        ...
        void (async () => {
          ...
          if (localDevVersion) {
            if (showStartupToast) {
              deps.showLocalDevToast(ctx, displayVersion, isSisyphusEnabled)
          ...
          if (showStartupToast) {
            deps.showVersionToast(ctx, displayVersion, getToastMessage(false))
              .catch(ignoreToastError);
          }

          deps.runBackgroundUpdateCheck(ctx, autoUpdate, getToastMessage).catch(
            (err) => {
              deps.log(
