...
import { ACCEPTED_PACKAGE_NAMES, CACHE_DIR, PACKAGE_NAME } from "../constants";
...
function safeUnlink(filePath: string): void {
  try {
    ...
    if (err instanceof Error) {
      log(
        `[auto-update-checker] Failed to cleanup temp file: ${filePath}`,
        err,
      );
    } else {
      log(
...
function getPackageNameFromIntent(pluginInfo: PluginEntryInfo): string {
  return ACCEPTED_PACKAGE_NAMES.find((packageName) =>
    pluginInfo.entry === packageName ||
    pluginInfo.entry.startsWith(`${packageName}@`)
  ) ?? PACKAGE_NAME;
}
...
export function syncCachePackageJsonToIntent(
  ...
  if (!fs.existsSync(cachePackageJsonPath)) {
    ...
    return {
      ...writeCachePackageJson(cachePackageJsonPath, {
        dependencies: { [packageName]: intentVersion },
      }),
  ...
  const currentIsSemver = EXACT_SEMVER_REGEX.test(
