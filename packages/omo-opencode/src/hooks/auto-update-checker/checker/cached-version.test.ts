...
describe("getCachedVersion (GH-3257)", () => {
  ...
  it("returns the version when the package is installed under oh-my-opencode", () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({ name: "oh-my-opencode", version: "3.16.0" }),
    );
  ...
  it("prefers the loaded module's package.json over flat-install candidates", () => {
    ...
    writeFileSync(
      sandboxPkgJson,
      JSON.stringify({ name: "oh-my-openagent", version: "3.17.5" }),
    );
    ...
    writeFileSync(
      ...
      JSON.stringify({ name: "oh-my-opencode", version: "3.17.6" }),
    );
  ...
  it("falls back to installed candidates when module-relative lookup throws a non-Error", () => {
    ...
    writeFileSync(
      join(legacyDir, "package.json"),
      JSON.stringify({ name: "oh-my-opencode", version: "3.18.0" }),
    );
  ...
  it("tries the next candidate when reading a candidate throws a non-Error", () => {
    ...
    writeFileSync(
      join(aliasDir, "package.json"),
      JSON.stringify({ name: "oh-my-openagent", version: "3.18.1" }),
    );
