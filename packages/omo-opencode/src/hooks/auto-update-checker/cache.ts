...
function removeFromTextBunLock(
  lockPath: string,
...
function removeSpecifierRootDirs(
  cacheDir: string,
  packageNames: readonly string[],
): boolean {
...
export function invalidatePackage(
  ...
  try {
    const acceptedPackageNames = options.acceptedPackageNames ??
      ACCEPTED_PACKAGE_NAMES;
    ...
    const packageNames = getInvalidationPackageNames(
      targetPackageName,
      defaultPackageName,
    ...
    if (!packageRemoved && !specifierRemoved && !lockRemoved) {
      log(
        `[auto-update-checker] Package not found, nothing to invalidate: ${targetPackageName}`,
      );
      return false;
    }
