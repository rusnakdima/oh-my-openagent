...
function findInjectedStatusPart(
  messages: MessageWithParts[],
): TransformPart | undefined {
...
function buildStatusLine(records: MonitorRecord[]): string {
  return `${MONITOR_STATUS_PREFIX} ${
    records.map(formatMonitorRecord).join(", ")
  } - call monitor_stop to stop`;
}
...
export function createMonitorStatusInjectorHook(
  ...
  return {
    "experimental.chat.messages.transform": async (
      ...
      const activeMonitors = monitorManager.list(sessionID).filter(
        isActiveMonitor,
      );
      ...
      output.messages.splice(
        ...
        0,
        createInjectedMessage(sessionID, statusLine),
      );
    },
  };
}
