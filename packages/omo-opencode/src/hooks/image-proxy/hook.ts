...
const DESCRIPTION_UNAVAILABLE = '<image-description status="unavailable">\n' +
...
export function createImageProxyHook(
  ...
  return {
    "chat.message": async (
      ...
      const effectiveModel = readModelIdentity(output.message.model) ??
        input.model;
      ...
      try {
        ...
        replacementText = `<image-description>\n${
