...
async function latestMainSessionUserMessageIsInProgress(
  ctx: BabysitterContext,
  sessionID: string,
  now: number,
): Promise<boolean> {
  try {
    ...
    for (let index = messages.length - 1; index >= 0; index--) {
      ...
      if (role === "user") {
        ...
        return createdAt !== undefined &&
          now - createdAt <= USER_MESSAGE_IN_PROGRESS_WINDOW_MS;
      }
    ...
    log(`[${HOOK_NAME}] Failed to inspect recent main session user activity`, {
      sessionID,
      error: String(error),
    });
    return false;
...
function getTaskLastActivityAt(
  task: { progress?: { lastUpdate?: Date; lastMessageAt?: Date } },
): Date | undefined {
  ...
  return lastUpdate.getTime() > lastMessageAt.getTime()
    ? lastUpdate
    : lastMessageAt;
}
...
export function createUnstableAgentBabysitterHook(
  ...
  const eventHandler = async (
    ...
    const tasks = options.backgroundManager.getTasksByParentSession(
      mainSessionID,
    );
    ...
    const now = Date.now();
    if (
      await latestMainSessionUserMessageIsInProgress(ctx, mainSessionID, now)
    ) {
      log(
        `[${HOOK_NAME}] Skipped reminder: main session has recent user activity`,
        { sessionID: mainSessionID },
      );
      return;
    }
