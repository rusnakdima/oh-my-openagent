...
type BabysitterContext = Parameters<
  typeof createUnstableAgentBabysitterHook
>[0];
...
describe("unstable-agent-babysitter hook", () => {
    ...
    try {
      ...
      await hook.event({
        event: {
          ...
          properties: { sessionID: "main-1", info: { role: "user" } },
        },
      });
      await hook.event({
        event: { type: "session.idle", properties: { sessionID: "main-1" } },
  ...
  test("#given a reminder was already sent before a stop event #when assistant activity resumes within cooldown #then no duplicate reminder is injected", async () => {
    ...
    try {
      ...
      await hook.event({
        event: { type: "session.stop", properties: { sessionID: "main-1" } },
      });
      currentNow += 1_000;
      await hook.event({
        event: {
          type: "message.updated",
          properties: { sessionID: "main-1", info: { role: "assistant" } },
        },
      });
      await hook.event({
        event: { type: "session.idle", properties: { sessionID: "main-1" } },
