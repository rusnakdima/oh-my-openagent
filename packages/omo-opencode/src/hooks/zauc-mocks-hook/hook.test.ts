...
const mockGetLocalDevVersion = mock<(directory: string) => string | null>(() =>
  null
);
...
describe("createAutoUpdateCheckerHook", () => {
  it("skips startup toasts and checks in CLI run mode", async () => {
    ...
    expect(mockUpdateAndShowConnectedProvidersCacheStatus).not
      .toHaveBeenCalled();
  ...
  it("runs all startup checks on normal session.created", async () => {
    ...
    expect(mockUpdateAndShowConnectedProvidersCacheStatus)
