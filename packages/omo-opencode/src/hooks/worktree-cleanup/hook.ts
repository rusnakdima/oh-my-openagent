...
export function createWorktreeCleanupHook(
  ...
  return {
    event: async (
      ...
      log("[worktree-cleanup] Checking worktree cleanup for session", {
        sessionID,
      });
      ...
      if (clean) {
        log("[worktree-cleanup] Worktree is clean, removing", {
          sessionID,
          worktreePath,
        });
        try {
          await removeWorktreeFn(worktreePath);
          log("[worktree-cleanup] Worktree removed successfully", {
            sessionID,
            worktreePath,
          });
        } catch (err) {
          log("[worktree-cleanup] Failed to remove worktree", {
            sessionID,
            worktreePath,
            error: String(err),
          });
        ...
        log(
          "[worktree-cleanup] Worktree has uncommitted changes, leaving on disk",
          {
            sessionID,
            worktreePath,
          },
