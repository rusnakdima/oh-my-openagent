  ...
  return {
    "chat.message": async (
      ...
      if (isSyntheticOrInternalOnlyTextParts(output.parts)) {
        log(`[keyword-detector] Skipping synthetic/internal text message`, {
      ...
      if (looksLikeSlashCommand(promptText)) {
        log(`[keyword-detector] Skipping slash command invocation`, {
          sessionID: input.sessionID,
        });
        return;
      }
      ...
      if (isPlannerAgent(currentAgent)) {
        ...
        detectedKeywords = detectedKeywords.filter(
          (k) =>
            k.type !== "ultrawork" && k.type !== "hyperplan" &&
            k.type !== "hyperplan-ultrawork" && k.type !== "openspec" &&
            k.type !== "search" && k.type !== "analyze" &&
            k.type !== "refactor" && k.type !== "test" && k.type !== "fix" &&
            k.type !== "review",
        );
        if (preFilterCount > detectedKeywords.length) {
          log(`[keyword-detector] Filtered mode keywords for planner agent`, {
            sessionID: input.sessionID,
            agent: currentAgent,
          });
        }
      ...
      const isNonMainSession = mainSessionID &&
        input.sessionID !== mainSessionID;

      if (detectedKeywords.length === 0) {
        // #5806: Re-inject ultrawork prompt if ultrawork was previously activated
        // but the current message omits the keyword.
        if (
          ultraworkActivatedSessions.has(input.sessionID) && !isNonMainSession
        ) {
          ...
          if (!cleanText.includes(ultraworkMessage.trim())) {
            ...
            if (textPartIndex !== -1) {
              ...
              log(
                `[keyword-detector] Ultrawork re-injected on follow-up message (previously activated, keyword omitted)`,
                {
                  sessionID: input.sessionID,
                },
              );
        ...
        if (
          defaultMode?.ultrawork && !isNonMainSession &&
          !defaultModeUltraworkInjectedSessions.has(input.sessionID)
        ) {
          defaultModeUltraworkInjectedSessions.add(input.sessionID);

          log(
            `[keyword-detector] Default ultrawork mode auto-activated (injected via system prompt)`,
            { sessionID: input.sessionID },
          );
          ...
          ctx.client.tui
            .showToast({
              body: {
                ...
                message:
                  "Default ultrawork mode enabled. All agents at your disposal.",
      ...
      detectedKeywords = filterAlreadyInjectedKeywords(
        detectedKeywords,
        cleanText,
      );
      if (detectedKeywords.length === 0) {
        log(`[keyword-detector] Skipping already injected keyword messages`, {
          sessionID: input.sessionID,
        });
        return;
      }
        ...
        ctx.client.tui
          .showToast({
            body: {
              ...
              message:
                "Adversarial planning engaged. 5 hostile members will cross-critique.",
              variant: "success" as const,
              duration: 3000,
            },
          })
      ...
      const hasHyperplanUltrawork = detectedKeywords.some((k) =>
        k.type === "hyperplan-ultrawork"
      );
      if (hasHyperplanUltrawork) {
        ...
        log(`[keyword-detector] Hyperplan Ultrawork mode activated`, {
          sessionID: input.sessionID,
        });
        ctx.client.tui
          .showToast({
            body: {
              ...
              message:
                "Ultrawork execution with adversarial hyperplan workflow.",
              variant: "success" as const,
              duration: 3000,
            },
          })
