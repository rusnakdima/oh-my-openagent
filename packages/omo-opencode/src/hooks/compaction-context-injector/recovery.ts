...
export function createRecoveryLogic(
  ...
  const recoverCheckpointedAgentConfig = async (
    ...
    const currentPromptConfig = await resolveSessionPromptConfig(
      ctx,
      sessionID,
    );
    ...
    try {
      const promptResult = await dispatchInternalPrompt({
        ...
        input: {
          ...
          body: {
            ...
            parts: [
              withInternalNoReplyMarker(
                createInternalAgentContinuationTextPart(AGENT_RECOVERY_PROMPT),
              ),
            ],
          },
      ...
      if (!isInternalPromptDispatchAccepted(promptResult)) {
        ...
        log(
          `[compaction-context-injector] Recovery skipped by promptAsync gate`,
          {
            sessionID,
            reason,
  ...
  const maybeWarnAboutNoTextTail = async (sessionID: string): Promise<void> => {
    ...
    const recentlyCompacted = tailState.lastCompactedAt !== undefined &&
