...
function resetTestCache(): void {
  ...
  mkdirSync(join(TEST_OPENCODE_CACHE_DIR, "node_modules", "oh-my-opencode"), {
    recursive: true,
  });
  writeFileSync(
...
describe("invalidatePackage", () => {
  ...
  it("invalidates the installed package from the OpenCode cache directory", async () => {
    const rootSpecifierDir = join(
      TEST_OPENCODE_CACHE_DIR,
      "oh-my-opencode@latest",
    );
    const rootAcceptedSpecifierDir = join(
      TEST_OPENCODE_CACHE_DIR,
      "oh-my-openagent@latest",
    );
    const packagesSpecifierDir = join(
      TEST_OPENCODE_CACHE_DIR,
      "packages",
      "oh-my-opencode@latest",
    );
    const packagesAcceptedSpecifierDir = join(
      TEST_OPENCODE_CACHE_DIR,
      "packages",
      "oh-my-openagent@latest",
    );
    const otherSpecifierDir = join(
      TEST_OPENCODE_CACHE_DIR,
      "packages",
      "other@latest",
    );
    mkdirSync(
      join(TEST_OPENCODE_CACHE_DIR, "node_modules", "oh-my-openagent"),
      { recursive: true },
    );
    mkdirSync(join(rootSpecifierDir, "node_modules", "oh-my-opencode"), {
      recursive: true,
    });
    mkdirSync(
      join(rootAcceptedSpecifierDir, "node_modules", "oh-my-openagent"),
      { recursive: true },
    );
    mkdirSync(join(packagesSpecifierDir, "node_modules", "oh-my-opencode"), {
      recursive: true,
    });
    mkdirSync(
      join(packagesAcceptedSpecifierDir, "node_modules", "oh-my-openagent"),
      { recursive: true },
    );
    ...
    expect(
      existsSync(
        join(TEST_OPENCODE_CACHE_DIR, "node_modules", "oh-my-opencode"),
      ),
    ).toBe(false);
    expect(
      existsSync(
    ...
    const packageJson = JSON.parse(
      readFileSync(join(TEST_OPENCODE_CACHE_DIR, "package.json"), "utf-8"),
    ) as {
    ...
    const bunLock = JSON.parse(
      readFileSync(join(TEST_OPENCODE_CACHE_DIR, "bun.lock"), "utf-8"),
    ) as {
      workspaces?: { ""?: { dependencies?: Record<string, string> } };
      packages?: Record<string, unknown>;
    };
    expect(bunLock.workspaces?.[""]?.dependencies?.["oh-my-opencode"]).toBe(
      "latest",
    );
    ...
    const explicitSpecifierDir = join(
      TEST_OPENCODE_CACHE_DIR,
      "some-other-package@latest",
    );
    const acceptedSpecifierDir = join(
      TEST_OPENCODE_CACHE_DIR,
    ...
    const explicitBunLock = JSON.parse(
      readFileSync(join(TEST_OPENCODE_CACHE_DIR, "bun.lock"), "utf-8"),
    ) as {
      packages?: Record<string, unknown>;
    };
