...
function createValidRgbaPng(
  width: number,
  height: number,
  options: TestPngOptions = {},
): string {
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let i = 0; i < rowBytes; i++) {
      ...
      const c = i >= 4 && previousUnfilteredRow
        ? previousUnfilteredRow[i - 4]
        : 0;
  ...
  const idatChunks = options.splitIdatAt === undefined
...
describe("resizeImageFallback", () => {
  describe("#given a valid RGBA PNG larger than the target", () => {
    ...
    it("#when rows use PNG filter type 4 #then preserves unfiltered pixel data", () => {
      ...
      expect(readOutputPixel(result.resizedDataUrl, 1)).toEqual([
        255,
        0,
        127,
        255,
  ...
  describe("#given unsupported PNG metadata", () => {
    ...
    it("#when IHDR requests interlacing #then returns null", () => {
      ...
      //#when
      const result = resizeImageFallback(interlacedPng, "image/png", {
        width: 2,
        height: 1,
      });
