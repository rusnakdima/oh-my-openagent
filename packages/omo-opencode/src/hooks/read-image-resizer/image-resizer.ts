...
export async function resizeImage(
  ...
  try {
    ...
    if (!sharpModule) {
      log(
        "[read-image-resizer] sharp unavailable, attempting pure-JS fallback",
      );
    ...
    if (
      resizedBuffer.length > ANTHROPIC_MAX_FILE_SIZE && canAdjustQuality(format)
    ) {
    ...
    return {
      resizedDataUrl: `data:${mimeType};base64,${
