...
export function replaceEmptyTextParts(
  ...
  if (isSqliteBackend()) {
    log(
      "[auto-compact] Disabled on SQLite backend: replaceEmptyTextParts (use async variant)",
    );
    return false;
  }
...
export async function replaceEmptyTextPartsAsync(
  ...
  try {
    ...
    log("[auto-compact] replaceEmptyTextPartsAsync failed", {
      error: String(error),
    });
    return false;
  }
...
export async function findMessagesWithEmptyTextPartsFromSDK(
  ...
  try {
    ...
    for (const msg of messages) {
      ...
      const hasEmpty = msg.parts.some((p) =>
