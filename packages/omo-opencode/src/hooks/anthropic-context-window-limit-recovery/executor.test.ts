...
const asClient = (client: MockClient): Client =>
  unsafeTestValue<Client>(client);
...
function createFakeTimeouts(): FakeTimeouts {
  ...
  const timers = new Map<
    number,
    { id: number; time: number; callback: TimerCallback; args: unknown[] }
  >();
...
describe("executeCompact lock management", () => {
  ...
  beforeEach(() => {
    ...
    mockClient = {
      session: {
        status: mock(() =>
          Promise.resolve({ data: { [sessionID]: { type: "idle" } } })
        ),
  ...
  test("does not start summarize recovery while the original session loop is still busy", async () => {
    ...
    mockClient.session.status = mock(() =>
      Promise.resolve({ data: { [sessionID]: { type: "busy" } } })
    );
    autoCompactState.errorDataBySession.set(sessionID, {
  ...
  test("clears lock when fixEmptyMessages path executes", async () => {
    ...
    const readMessagesSpy = spyOn(messagesReader, "readMessages")
      .mockReturnValue([]);
    autoCompactState.errorDataBySession.set(sessionID, {
