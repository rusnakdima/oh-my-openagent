...
import {
  getInboxDir,
  resolveBaseDir,
} from "../../features/team-mode/team-registry/paths";
import {
...
async function createTemporaryBaseDir(): Promise<string> {
  const baseDir = await mkdtemp(
    path.join(tmpdir(), "team-member-error-handler-"),
  );
...
function createRuntimeStateWithPendingMessage(
  teamRunId: string,
  messageId: string,
): RuntimeState {
...
function createRuntimeStateWithLeader(
  teamRunId: string,
...
async function seedReservedMessage(
  teamRunId: string,
  config: TeamModeConfig,
  messageId: string,
): Promise<void> {
...
describe("createTeamMemberErrorHandler", () => {
  ...
  test("#given fallback retry replaces a member sessionId #when stale session.error settles #then replacement stays running without a member_error announcement", async () => {
    ...
    await seedRuntimeState(
      createRuntimeStateWithLeader(teamRunId, [messageId]),
      config,
    );
    ...
    const finalWorker = finalState.members.find((member) =>
      member.name === "worker"
    );
    ...
    const { listUnreadMessages } = await import(
      "../../features/team-mode/team-mailbox/inbox"
    );
    expect(await listUnreadMessages(teamRunId, "lead", config)).toEqual([]);

    const workerInboxEntries = await readdir(
      getInboxDir(resolveBaseDir(config), teamRunId, "worker"),
    );
  ...
  test("falls back to disk lookup when the registry points the member session at the wrong teamRunId", async () => {
    ...
    // then
    const correctRuntimeState = await loadRuntimeState(
      correctTeamRunId,
      config,
    );
  ...
  test("injects a member_error announcement into the lead inbox when a non-lead member errors", async () => {
    ...
    expect(leadInboxEntries.some((entry) => entry.endsWith(".json"))).toBe(
      true,
    );
  ...
  test("requeues pending live-delivery messages when the recipient session errors before idle ack", async () => {
    ...
    const messageId = randomUUID();
    await seedRuntimeState(
      createRuntimeStateWithPendingMessage(teamRunId, messageId),
      config,
    );
    await seedReservedMessage(teamRunId, config, messageId);
    const handler = createTeamMemberErrorHandler(config);
    ...
    await handler({
      event: {
        ...
        properties: {
          sessionID: "member-session",
          error: new Error("late prompt failure"),
        },
      },
    });
  ...
  test("#given session.error arrives after peer message reached history #when pending live delivery exists #then it does not requeue a duplicate peer message", async () => {
                ...
                parts: [
                  {
                    type: "text",
                    text:
                      `<peer_message from="lead" messageId="${messageId}" kind="message">pending live delivery</peer_message>`,
                  },
                ],
              },
            ],
    ...
    await handler({
      event: {
        ...
        properties: {
          sessionID: "member-session",
          error: new Error("late session.error after accepted prompt"),
        },
      },
    });
