...
async function sendCompletionToLead(
  teamRunId: string,
  config: TeamModeConfig,
  body: string,
  timestamp: number,
...
describe("createTeamIdleWakeHint leader delivery", () => {
  test(
    "#given repeated member completions to an idle leader #when each cycle idles after delivery #then every completion wakes the leader",
    async () => {
      // given
      ...
      expect(promptInputs.map((input) => input.path.id)).toEqual(
        Array(COMPLETION_CYCLE_COUNT).fill("lead-session"),
      );
      expect(promptInputs.at(-1)?.body.parts[0]?.text).toContain(
        `${COMPLETION_CYCLE_COUNT} new team messages`,
      );
      ...
      expect(unreadMessages.map((message) => message.body)).toEqual(
        completionBodies,
      );
