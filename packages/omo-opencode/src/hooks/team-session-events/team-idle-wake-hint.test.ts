...
function createLeaderRuntimeState(
  teamRunId: string,
  pendingInjectedMessageIds: string[],
): RuntimeState {
  return {
...
async function seedLeadUnreadMessage(
  ...
  await sendMessage(
    ...
    teamRunId,
    config,
    { isLead: false, activeMembers: ["lead"] },
  );
}
...
describe("createTeamIdleWakeHint", () => {
  ...
  test("sends a trigger-only wake hint when new unread mail exists", async () => {
    ...
    expect(promptInput.body.parts[0]?.text).not.toContain(
      "second message body",
    );
  });
  ...
  test(
    "acks pending messages on idle, moves files to processed, and clears pending ids",
    async () => {
      // given
      const baseDir = await createTemporaryBaseDir();
      const config = createConfig(baseDir);
      ...
      expect(processedEntries.sort()).toEqual(
        messageIds.map((messageId) => `${messageId}.json`).sort(),
      );
  ...
  test("acks pending reserved live-delivery messages on idle", async () => {
    ...
    await seedReservedUnreadMessage(
      teamRunId,
      config,
      messageId,
      "live delivery body",
  ...
  test("#given pending live delivery is requeued by session.error #when stale idle handler continues #then it does not ack the requeued message", async () => {
    ...
    try {
      await waitForEvent(
        idleHandlerReachedStatus.promise,
        "stale idle handler status check",
      );
      await errorHandler({
        event: {
          type: "session.error",
  ...
  test("#given a pending live-delivery ack and later unread message #when member idles after the live reply #then it wakes the member for the unread message", async () => {
    ...
    await seedRuntimeState(
      createRuntimeState(teamRunId, [pendingMessageId]),
      config,
    );
    await seedReservedUnreadMessage(
      teamRunId,
  ...
  test("acks pending lead messages on idle without sending a wake hint", async () => {
    ...
    await seedRuntimeState(
      createLeaderRuntimeState(teamRunId, messageIds),
      config,
    );
