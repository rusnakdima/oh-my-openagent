...
type TodoWriter = (
  input: { sessionID: string; todos: TodoSnapshot[] },
) => Promise<void>;
...
function extractTodoArgument(value: unknown): TodoSnapshot[] {
  ...
  try {
    ...
    log(`[${HOOK_NAME}] Failed to parse todowrite todos`, {
      error: String(err),
    });
    return [];
  }
...
export function createCompactionTodoPreserverHook(
  ...
  const restore = async (sessionID: string): Promise<void> => {
    ...
    if (
      hasCurrent && !shouldRestoreOverCurrentTodos({ snapshot, currentTodos })
    ) {
      snapshots.delete(sessionID);
  ...
  const beforeToolExecute = async (
    ...
    log(
      `[${HOOK_NAME}] Replaced late Atlas bootstrap todowrite with restored snapshot`,
      {
