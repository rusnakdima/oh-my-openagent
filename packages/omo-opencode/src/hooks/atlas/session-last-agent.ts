...
async function getLastAgentFromSessionMessages(
  ...
  try {
    ...
    const messages = deps.normalizeSDKResponse(response, [] as Array<{
      ...
      const leftTime =
        (left as { info?: { time?: { created?: number } } }).info?.time
          ?.created ?? Number.NEGATIVE_INFINITY;
      const rightTime =
        (right as { info?: { time?: { created?: number } } }).info?.time
          ?.created ?? Number.NEGATIVE_INFINITY;
...
export async function getLastAgentFromSession(
  ...
  if (client) {
    const sdkAgent = await getLastAgentFromSessionMessages(
      sessionID,
      client,
      resolvedDeps,
    );
  ...
  try {
      ...
      .map((fileName) => {
        try {
          ...
          return {
            ...
            id: parsed.id,
            agent: parsed.agent,
            createdAt: typeof parsed.time?.created === "number"
              ? parsed.time.created
    ...
    for (const message of messages) {
      if (!message) continue;
      if (
        resolvedDeps.isCompactionMessage({ agent: message.agent }) ||
        resolvedDeps.hasCompactionPartInStorage(message?.id)
      ) {
        continue;
      }
