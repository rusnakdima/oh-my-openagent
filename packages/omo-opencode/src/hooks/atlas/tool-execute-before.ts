...
function parseTrackedTaskFromPrompt(
  prompt: string,
): TrackedTopLevelTaskRef | null {
  const lines = prompt.split(/\r?\n/);
  const taskHeaderIndex = lines.findIndex((line) =>
    TASK_SECTION_HEADER_PATTERN.test(line.trim())
  );
...
export function createToolExecuteBeforeHandler(input: {
  ...
  const { ctx, pendingFilePaths, pendingTaskRefs, pendingPlanSnapshots } =
    input;
  const resolveIsCallerOrchestrator = input.isCallerOrchestrator ??
    ((sessionID) => isCallerOrchestrator(sessionID, ctx.client));
  ...
  return async (toolInput, toolOutput): Promise<void> => {
    ...
    if (isWriteOrEditToolName(toolInput.tool)) {
      const filePath = (toolOutput.args.filePath ?? toolOutput.args.path ??
      ...
      if (
        planPath && resolve(filePath) === resolve(planPath) &&
        pendingPlanSnapshots
      ) {
        try {
          if (existsSync(planPath)) {
            pendingPlanSnapshots.set(
              toolInput.callID,
    ...
    if (toolInput.tool === "task") {
      if (toolInput.callID) {
        ...
        if (requestedSessionId) {
          ...
          const prompt = typeof toolOutput.args.prompt === "string"
          ...
          if (resolvedTask) {
            if (!taskFromPrompt) {
              log(
                `[${HOOK_NAME}] TASK section parse failed; falling back to current top-level task`,
                {
                  sessionID: toolInput.sessionID,
            ...
            const hasExistingClaim = [...pendingTaskRefs.values()].some((
              ...
              pendingTaskRef.task.key === trackedTask.key
            ));
