...
\`\`\``;
  ...
  test("returns missing when the output only repeats the verdict instruction", () => {
    // given
    const output =
      "Please emit VERDICT: APPROVE or VERDICT: REJECT before finishing.";
...
describe("Atlas final verification approval gate", () => {
  const env = registerFinalWaveTestEnvironment({
    resetAgentRegistration: true,
  });

  const resolveParentSessionID = (taskSessionID: string): string => {
    if (taskSessionID === "ses_final_wave_review") {
      return "atlas-final-wave-session";
    }
  ...
  function createMockPluginInput() {
    return createFinalWaveMockPluginInput({
...
`,
    ...
    const toolOutput = {
      title: "Sisyphus Task",
      output:
        `Tasks [4/4 compliant] | Contamination [CLEAN] | Unaccounted [CLEAN] | VERDICT: APPROVE
...
session_id: ses_final_wave_review
    ...
    await hook["tool.execute.after"]({ tool: "task", sessionID }, toolOutput);
    expect(toolOutput.output.length).toBeGreaterThan(
      outputLengthBeforeApproval,
    );
