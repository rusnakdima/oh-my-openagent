...
function getCachedBoulderState(
  directory: string,
): ReturnType<typeof readBoulderState> {
  const now = Date.now();
  ...
  boulderStateCache.set(directory, {
    state,
    expiresAt: now + BOULDER_CACHE_TTL_MS,
  });
  return state;
}
  ...
  sessionState.pendingRetryTimer = setTimeout(async () => {
    try {
      ...
      const currentProgress = getPlanProgress(
        resolveBoulderPlanPath(ctx.directory, currentBoulder),
      );
      ...
      const loggedError = error instanceof Error ? error : String(error);
      log(`[${HOOK_NAME}] Failed during boulder continuation retry`, {
        sessionID,
        error: loggedError,
      });
