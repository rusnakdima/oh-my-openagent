...
export async function handleCompletedBoulderIdle(input: {
  ...
  if (options?.isContinuationStopped?.(sessionID)) {
    log(
      `[${HOOK_NAME}] Boulder completion nudge skipped because continuation stopped`,
      {
        sessionID,
        plan: boulderState.plan_name,
      },
  ...
  const elapsedMilliseconds = work.elapsed_ms ??
    (Date.now() - new Date(work.started_at).getTime());
  ...
  const taskBreakdown = Object.values(work.task_sessions ?? {})
    ...
    .map((task) => {
      if (typeof task.elapsed_ms === "number") {
        return `- ${task.task_label} ${task.task_title}: ${
          formatDurationHuman(task.elapsed_ms)
        }`;
      }
  ...
  const prompt = BOULDER_COMPLETE_PROMPT
    ...
    .replace(
      /{TASK_BREAKDOWN}/g,
      taskBreakdown.length > 0 ? taskBreakdown : "- (no task timings)",
    );
  ...
  if (atlasAgent && isAgentRegistered(atlasAgent)) {
    if (
      !(await shouldPromptAfterSessionIdle(
        ctx.client,
        sessionID,
        options?.idleSettleMs,
      ))
    ) {
      log(
        `[${HOOK_NAME}] Boulder completion nudge skipped because session is active`,
        { sessionID },
      );
      return;
    }
    ...
    if (!isInternalPromptDispatchAccepted(promptResult)) {
      ...
      log(
        `[${HOOK_NAME}] Boulder completion nudge skipped by promptAsync gate`,
        {
          sessionID,
