...
describe("createToolExecuteAfterHandler background launch detection", () => {
  ...
  beforeEach(() => {
    testDirectory = join(
      tmpdir(),
      `atlas-background-launch-${crypto.randomUUID()}`,
    );
  ...
  describe("#given a call_omo_agent background launch result", () => {
    ...
    describe("#when a background task launch belongs to the active boulder task", () => {
      it("#then it should persist the delegated session without transforming the launch output", async () => {
        ...
        spyOn(client.session, "get").mockImplementation((input) =>
          Promise.resolve(
            createSessionGetResult(
              input?.path?.id === childSessionID ? sessionID : undefined,
            ),
          ) as never
        );
...
`,
        ...
        expect(
          readBoulderState(testDirectory)?.session_origins
            ?.[`opencode:${childSessionID}`],
        ).toBe("appended");
        expect(
          readBoulderState(testDirectory)?.task_sessions?.["todo:1"]
            ?.session_id,
        ).toBe(`opencode:${childSessionID}`);
      });
      ...
      it("#then background_output plugin retrieval requires completion gate when tracked task is still unchecked", async () => {
        ...
        const planPath = join(
          testDirectory,
          "background-output-plugin-retrieval.md",
        );
...
`,
      ...
      it("#then it should not track an extracted child session outside active lineage", async () => {
        ...
        spyOn(client.session, "get").mockImplementation((input) =>
          Promise.resolve(
            createSessionGetResult(
              input?.path?.id === childSessionID
                ? "ses_unrelated_parent"
                : undefined,
            ),
          ) as never
        );
...
`,
        ...
        await beforeHandler(
          {
            tool: "task",
            sessionID,
            callID: "call-bg-task-unrelated-launcher",
          },
          { args: { prompt: "Implement auth flow" } },
        );
      ...
      it("#then it should append launched child to the session-resolved work", async () => {
        ...
        spyOn(client.session, "get").mockImplementation((input) =>
          Promise.resolve(
            createSessionGetResult(
              input?.path?.id === childSessionID ? parentSessionID : undefined,
            ),
          ) as never
        );
        ...
        expect(boulderState?.works?.["work-b"]?.session_ids).toContain(
          `opencode:${childSessionID}`,
        );
        expect(boulderState?.works?.["work-a"]?.session_ids).not.toContain(
          `opencode:${childSessionID}`,
        );
      });
    });
