...
describe("Atlas final-wave approval gate regressions", () => {
  ...
  const resolveParentSessionID = (taskSessionID: string): string => {
    if (taskSessionID === "ses_nested_scope_review") {
      return "atlas-nested-final-wave-session";
    }
    if (taskSessionID.startsWith("ses_parallel_review_")) {
      return "atlas-parallel-final-wave-session";
    }
  ...
  function createHandler(sessionID: string) {
    return createFinalWaveAfterHandlerHarness({
      sessionID,
      ctx: createFinalWaveMockPluginInput({
...
`,
    ...
    const toolOutput = {
      title: "Sisyphus Task",
      output:
        `Tasks [1/1 compliant] | Contamination [CLEAN] | Unaccounted [CLEAN] | VERDICT: APPROVE
