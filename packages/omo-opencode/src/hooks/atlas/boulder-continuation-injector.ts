...
export async function injectBoulderContinuation(input: {
  ...
  const hasRunningBgTasks = backgroundManager
    ? backgroundManager.getTasksByParentSession(sessionID).some((
      t: { status: string },
    ) => ACTIVE_BACKGROUND_TASK_STATUSES.has(t.status))
    : false;
  ...
  const preferredSessionContext = preferredTaskSessionId
    ? `\n\n[Preferred reuse session for current top-level plan task${
      preferredTaskTitle ? `: ${preferredTaskTitle}` : ""
    }: ${preferredTaskSessionId}]`
    : "";
  const prompt = BOULDER_CONTINUATION_PROMPT.replace(/{PLAN_NAME}/g, planName) +
    `\n\n[Status: ${
      total - remaining
    }/${total} completed, ${remaining} remaining]` +
  ...
  const continuationAgent = resolvedContinuationAgent
    ? stripAgentListSortPrefix(resolvedContinuationAgent)
    : resolvedContinuationAgent;
  ...
  try {
    log(`[${HOOK_NAME}] Injecting boulder continuation`, {
      sessionID,
      planName,
      remaining,
    });

    const promptContext = await resolveRecentPromptContextForSession(
      ctx,
      sessionID,
    );
    ...
    if (promptResult.status === "failed") {
      if (isAmbiguousPostDispatchPromptFailure(promptResult)) {
        ...
        log(
          `[${HOOK_NAME}] Boulder continuation prompt failed after dispatch may have been accepted`,
          {
            sessionID,
