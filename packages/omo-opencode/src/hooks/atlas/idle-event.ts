...
export async function handleAtlasSessionIdle(input: {
  ...
  if (shouldAbortForNoToolProgress(sessionState)) {
    ...
    log(
      `[${HOOK_NAME}] Aborting boulder continuation after repeated no-tool-progress iterations`,
      {
        sessionID,
        plan: boulderState.plan_name,
  ...
  if (sessionState.promptFailureCount >= MAX_CONSECUTIVE_PROMPT_FAILURES) {
    const timeSinceLastFailure = sessionState.lastFailureAt !== undefined
  ...
  if (
    ...
    log(`[${HOOK_NAME}] Skipped: continuation cooldown active`, {
      sessionID,
      cooldownRemaining: CONTINUATION_COOLDOWN_MS -
  ...
  if (
    ...
    log(`[${HOOK_NAME}] Skipped: session became active during idle settle`, {
      sessionID,
    });
    return;
  }
