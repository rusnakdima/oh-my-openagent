...
describe("atlas background task retry", () => {
  ...
  const capturedTimers = new Map<
    number,
    { callback: () => Promise<void> | void; cleared: boolean }
  >();
  ...
  beforeEach(() => {
    ...
    globalThis.setTimeout = ((
      ...
      if (
        normalizedDelay === RETRY_DELAY_MS &&
        normalizedDelay !== DEFAULT_PROMPT_DISPATCH_TIMEOUT_MS
      ) {
    ...
    globalThis.clearTimeout =
      ((id?: number | ReturnType<typeof setTimeout>) => {
        const timerEntry = typeof id === "number"
          ? capturedTimers.get(id)
          : undefined;
        if (timerEntry) {
        ...
        client: {
          session: {
            get: async ({ path }: { path: { id: string } }) => ({
              data: {
                id: path.id,
                parentID: path.id === descendantSessionID
                  ? sessionID
                  : undefined,
              },
            }),
