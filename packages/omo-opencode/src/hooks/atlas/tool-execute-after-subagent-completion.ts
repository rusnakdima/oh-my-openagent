...
function isBackgroundLaunchOutput(output: string): boolean {
  return output.includes("Background task launched") ||
    output.includes("Background task continued") ||
    output.includes("Background delegate launched") ||
    output.includes("Background agent task launched");
}

function isBackgroundOutputIncompleteReport(
  toolName: string,
  output: string,
): boolean {
  ...
  const fullSessionStatusReport = new RegExp(
    `^# Full Session Output\\b[\\s\\S]*^Status:\\s*${incompleteStatus}\\s*$`,
    "m",
  );
...
export async function handleSubagentCompletionAfter(input: {
  ...
  const outputStr = typeof toolOutput.output === "string"
    ? toolOutput.output
    : "";
  ...
  const extractedSessionId = metadataSessionId ??
  ...
  const workScopedBoulderState = sessionWork
    ? {
      ...
      session_origins: sessionWork.session_origins
        ? { ...sessionWork.session_origins }
        : {},
      agent: sessionWork.agent,
      worktree_path: sessionWork.worktree_path,
      task_sessions: sessionWork.task_sessions
        ? { ...sessionWork.task_sessions }
        : {},
    }
    : boulderState;
  ...
  if (currentTask && subagentSessionId && !shouldSkipTaskSessionUpdate) {
    if (sessionWork) {
      startTaskTimer(ctx.directory, sessionWork.work_id, {
        ...
        agent: typeof toolOutput.metadata?.agent === "string"
          ? toolOutput.metadata.agent
          : undefined,
        category: typeof toolOutput.metadata?.category === "string"
  ...
  if (sessionState) {
    if (
      sessionState.activeContinuationPlanPath !== undefined &&
