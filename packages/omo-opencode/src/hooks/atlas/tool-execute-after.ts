...
export function createToolExecuteAfterHandler(input: {
  ...
  toolOutput: ToolExecuteAfterOutput | undefined,
) => Promise<void> {
  const {
    ctx,
    pendingFilePaths,
  ...
  const collectGitDiffStatsImpl = input.collectGitDiffStats ??
    collectGitDiffStats;
  ...
  return async (toolInput, toolOutput): Promise<void> => {
    ...
    if (
      toolInput.sessionID && isTangibleProgressTool(toolInput.tool) &&
      didToolMakeProgress(toolOutput)
    ) {
    ...
    if (
      await handleDirectWorkToolAfter({
        ctx,
        pendingFilePaths,
        pendingPlanSnapshots,
        toolInput,
    ...
    const isPluginToolWithSession = toolInput.tool !== "task" &&
      !!metadataSessionId;
