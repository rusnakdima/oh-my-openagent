...
const FAILURE_TITLE_PATTERN =
  /(?:\berror\b|\bfailed\b|\bfailure\b|\bdenied\b|\brejected\b)/i;
const FAILURE_OUTPUT_PATTERN =
  /^\s*(?:error|failed|failure|denied|rejected)\b/i;
...
export function didToolMakeProgress(output: ToolProgressOutput): boolean {
  ...
  return !FAILURE_TITLE_PATTERN.test(title) &&
    !FAILURE_OUTPUT_PATTERN.test(body);
}
...
export function resetStallStateForPlanChange(
  state: SessionState,
  planPath: string,
): void {
  ...
  state.awaitingToolProgressAfterContinuation = false;
  if (
    state.stalledContinuationReason &&
...
export function markContinuationInjectedAwaitingToolProgress(
  state: SessionState,
): void {
...
export function updateNoToolProgressIterations(state: SessionState): number {
  ...
  state.awaitingToolProgressAfterContinuation = false;
  state.iterationsSinceLastToolProgress =
    (state.iterationsSinceLastToolProgress ?? 0) + 1;
...
export function markContinuationStalled(
  state: SessionState,
  planName: string,
  planPath: string,
): void {
  state.stalledContinuationReason =
    `Boulder continuation stalled for plan "${planName}": ${MAX_BOULDER_CONTINUATION_NO_TOOL_PROGRESS} consecutive continuation iterations produced no successful bash/edit/write tool progress.`;
  state.stalledContinuationPlanPath = planPath;
}
