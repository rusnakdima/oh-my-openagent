...
export async function syncBackgroundLaunchSessionTracking(input: {
  ...
  const extractedSessionId = metadataSessionId ??
    extractSessionIdFromOutput(toolOutput.output);
  const lineageSessionIDs = trackedWork?.session_ids ??
  ...
  const trackedSessionId = subagentSessionId ??
...
async function resolveFallbackTrackedSessionId(input: {
  ...
  try {
    ...
    const normalizedLineageSessionIDs = input.lineageSessionIDs.map((
      sessionID,
