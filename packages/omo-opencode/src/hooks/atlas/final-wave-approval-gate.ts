...
export function classifyFinalWaveVerdict(
  output: string,
): "approve" | "reject" | "missing" {
  const verdicts = [...output.matchAll(VERDICT_PATTERN)].map((match) =>
    match[1]?.toLowerCase()
  );
...
export function shouldPauseForFinalWaveApproval(input: {
  ...
  const shouldPause = input.sessionState.approvedFinalWaveTaskCount >=
    planState.pendingFinalWaveTaskCount;
  if (shouldPause) {
    clearFinalWaveApprovalTracking(input.sessionState);
  }
