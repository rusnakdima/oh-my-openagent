...
import {
  parseCheckedTopLevelTaskKeys,
  readCheckedTaskKeysFromPlan,
} from "./tool-execute-after-plan-tasks";
...
export async function handleDirectWorkToolAfter(input: {
  ...
  const { ctx, pendingFilePaths, pendingPlanSnapshots, toolInput, toolOutput } =
    input;
  ...
  if (filePath && toolInput.sessionID) {
    ...
    if (sessionWork) {
      const planPath = resolveBoulderPlanPathForWork(
        ctx.directory,
        sessionWork,
      );
      if (
        resolve(filePath) === resolve(planPath) && planSnapshot !== undefined
      ) {
