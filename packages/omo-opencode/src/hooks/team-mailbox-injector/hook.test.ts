...
describe("createTeamMailboxInjector", () => {
  ...
  it("prepends an envelope as a user-role message for a member session", async () => {
    ...
    await sendMessage(
      ...
      TeamModeConfigSchema.parse({ base_dir: baseDir, enabled: true }),
      { isLead: true, activeMembers: ["lead", "member-a"] },
    );
  ...
  it("falls back to disk lookup when the registry points the session at the wrong teamRunId", async () => {
    ...
    await seedRuntimeState(
      baseDir,
      createRuntimeState("session-member", correctTeamRunId),
    );
    await seedRuntimeState(
      baseDir,
      createRuntimeState("other-session", wrongTeamRunId),
    );
    await sendMessage(
      {
        version: 1,
