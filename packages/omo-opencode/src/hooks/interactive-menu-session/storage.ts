...
export function loadInteractiveMenuSessionState(
  sessionId: string,
): InteractiveMenuSessionState | null {
  ensureDir();
  ...
  try {
    ...
    ) as InteractiveMenuSessionState;
  } catch {
    return null;
  }
}

export function saveInteractiveMenuSessionState(
  sessionId: string,
  state: InteractiveMenuSessionState,
): void {
  ensureDir();
  const path = `${INTERACTIVE_MENU_STORAGE_DIR}/${sessionId}.json`;
