...
function debugLog(...args: unknown[]) {
  if (DEBUG) {
    const msg = `[${new Date().toISOString()}] [comment-checker:cli] ${
      args.map((a) =>
...
export function resolveCommentCheckerPathFromPath(
  binaryName: string,
  which: (binary: string) => string | null = bunWhich,
): string | null {
  try {
...
export type { CheckResult, HookInput };
