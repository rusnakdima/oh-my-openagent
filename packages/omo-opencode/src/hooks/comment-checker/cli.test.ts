...
function createScriptBinary(
  ...
  writeFileSync(
    ...
    process.platform === "win32"
      ? windowsScriptContent ?? scriptContent
      : scriptContent,
  );
...
describe("comment-checker CLI", () => {
  describe("lazy initialization", () => {
    test("#given PATH-only binary #when resolving from PATH #then returns Bun.which result", async () => {
      ...
      // when
      const result = cliModule.resolveCommentCheckerPathFromPath(
        "comment-checker.exe",
        (binary: string) => {
          calls.push(binary);
  ...
  describe("runCommentChecker", () => {
    ...
    test("sends SIGKILL after grace period when process ignores SIGTERM", async () => {
      // given
      const { runCommentChecker } = await import("./cli");
      const binaryPath = createScriptBinary(
        `#!/bin/sh
if [ "$1" != "check" ]; then
  exit 1
fi
  ...
  describe("processWithCli semaphore", () => {
    ...
    test("allows second call after first call completes", async () => {
      ...
      await processWithCli(
        { tool: "write", sessionID: "ses-first", callID: "call-1" },
        firstPendingCall,
        { output: "" },
        "/fake",
        undefined,
      ...
      await processWithCli(
        { tool: "write", sessionID: "ses-second", callID: "call-2" },
        secondPendingCall,
        { output: "" },
        "/fake",
        undefined,
