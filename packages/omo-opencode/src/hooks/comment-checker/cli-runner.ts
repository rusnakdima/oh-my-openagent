...
export function initializeCommentCheckerCli(
  debugLog: (...args: unknown[]) => void,
): void {
  ...
  await withCommentCheckerLock(
    async () => {
      ...
      if (now - lastWarned < DEDUP_WINDOW_MS) {
        debugLog(
          "dedup: skipping comment warning within dedup window for session",
      ...
      const result = await (deps.runCommentChecker ?? runCommentChecker)(
        hookInput,
        cliPath,
        customPrompt,
      );
