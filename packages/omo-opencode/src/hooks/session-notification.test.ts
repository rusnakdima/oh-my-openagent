...
function createShellChain(
  result: MockShellResult,
  shouldReject = false,
): MockShellChain {
  const promise =
    (shouldReject
      ? Promise.reject(Object.assign(new Error("command failed"), result))
      : Promise.resolve(result)) as MockShellChain;
    ...
    return createShellChain(
      {
        ...
        exitCode: options.reject?.(commandString, values) ? 1 : 0,
      },
    ...
    return createMockInput(
      createShellMock({
        capture: (cmdStr) => {
          ...
          if (
            cmdStr.includes("osascript") || cmdStr.includes("notify-send") ||
