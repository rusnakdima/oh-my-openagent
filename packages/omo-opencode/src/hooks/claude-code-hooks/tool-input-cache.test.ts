...
describe("tool-input-cache", () => {
  ...
  afterEach(async () => {
    const modulePath = fileURLToPath(
      new URL("./tool-input-cache.ts", import.meta.url),
    );
  ...
  test("#given cached entries from multiple sessions #when clearing one session #then only matching entries are removed", async () => {
    ...
    expect(cacheModule.getToolInput("ses_b", "Read", "call-2")).toEqual({
      path: "b",
    });
  });
  ...
  test("#given cleanup timer started #when stop cleanup runs #then interval is cleared and cache is emptied", async () => {
    //#given
    const intervalHandle = unsafeTestValue<ReturnType<typeof setInterval>>({
      unref: mock(() => {}),
    });
    ...
    globalThis.clearInterval = unsafeTestValue<typeof clearInterval>(
      clearIntervalMock,
    );
    ...
    cacheModule.cacheToolInput("ses_stop", "Read", "call-stop", {
      path: "stop",
    });
    ...
    expect(cacheModule.getToolInput("ses_stop", "Read", "call-stop"))
      .toBeNull();
  });
});
