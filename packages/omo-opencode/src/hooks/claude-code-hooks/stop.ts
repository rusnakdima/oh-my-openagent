import type { ClaudeHooksConfig, StopInput, StopOutput } from "./types";
  ...
  for (const matcher of matchers) {
    ...
    for (const hook of matcher.hooks) {
      ...
      if (isHookCommandDisabled("Stop", hookName, extendedConfig ?? null)) {
        log("Stop hook command skipped (disabled by config)", {
          command: hookName,
        });
        continue;
      }
      ...
      if (result.exitCode === 2) {
        const reason = normalizeHookText(result.stderr) ??
          normalizeHookText(result.stdout) ?? "Blocked by stop hook";
        return {
          block: true,
          reason,
          injectPrompt: reason,
      ...
      if (result.stdout) {
        try {
          ...
          if (isBlock) {
            ...
            const injectPrompt = normalizeHookText(output.inject_prompt) ??
              reason;
            return {
              block: true,
              reason,
