...
export async function executePreToolUseHooks(
  ...
  for (const matcher of matchers) {
    ...
    for (const hook of matcher.hooks) {
      ...
      if (result.exitCode === 2) {
        return {
          decision: "deny",
          reason: normalizeHookText(result.stderr) ??
            normalizeHookText(result.stdout) ?? "Hook blocked the operation",
        ...
        try {
          ...
          if (output.hookSpecificOutput?.permissionDecision) {
            ...
            if (legacyDecision === "approve" || legacyDecision === "allow") {
              ...
              legacyDecision === "block" || legacyDecision === "deny"
            ) {
              decision = "deny";
          ...
          if (decision === "deny" || decision === "ask") {
            return {
              ...
              stopReason: normalizeHookText(output.stopReason) ??
                accumulatedCommonFields.stopReason,
              suppressOutput: output.suppressOutput ??
                accumulatedCommonFields.suppressOutput,
              systemMessage: normalizeHookText(output.systemMessage) ??
                accumulatedCommonFields.systemMessage,
            };
          }

          // "allow" — accumulate modifiedInput and common fields, continue to next hook
          if (modifiedInput) {
            accumulatedModifiedInput = {
              ...accumulatedModifiedInput,
              ...modifiedInput,
            };
            Object.assign(
              stdinData.tool_input,
              objectToSnakeCase(modifiedInput),
            );
          }
          ...
          if (output.stopReason !== undefined) {
            accumulatedCommonFields.stopReason = normalizeHookText(
              output.stopReason,
            );
          }
          ...
          if (output.systemMessage !== undefined) {
            accumulatedCommonFields.systemMessage = normalizeHookText(
  ...
  return {
    ...
    ...(accumulatedModifiedInput
      ? { modifiedInput: accumulatedModifiedInput }
      : {}),
    ...(Object.keys(accumulatedCommonFields).length > 0
      ? accumulatedCommonFields
      : {}),
  };
}
