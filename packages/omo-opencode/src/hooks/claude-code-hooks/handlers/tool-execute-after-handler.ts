...
function buildTranscriptToolOutput(
  outputText: string,
  metadata: unknown,
  ...
  for (
    const key of ["noopEdits", "deduplicatedEdits", "firstChangedLine"] as const
  ) {
...
export function createToolExecuteAfterHandler(
  ...
  return async (
    ...
    const cachedInput =
      getToolInput(input.sessionID, input.tool, input.callID) || {};
    ...
    output.output = appendHookSections(output.output, [
      ...(result.warnings ?? []),
      ...(normalizeHookText(result.additionalContext) === undefined
        ? []
