      ...
      if (shouldBypass && stopResult.block) {
        ...
        if (promptResult.status === "failed") {
          if (isAmbiguousPostDispatchPromptFailure(promptResult)) {
            log(
              "Prompt injected from Stop hook may have been accepted before ambiguous failure",
              {
                sessionID,
            ...
            log("Failed to inject prompt from Stop hook", {
              error: String(promptResult.error),
            });
          }
        } else if (!isInternalPromptDispatchAccepted(promptResult)) {
          log("Skipped prompt injection from Stop hook", {
            sessionID,
            status: promptResult.status,
          });
        }
      } else if (stopResult.block) {
        log("Stop hook returned block", {
          sessionID,
...
export function disposeSessionEventHandler(
  contextCollector?: ContextCollector,
): void {
