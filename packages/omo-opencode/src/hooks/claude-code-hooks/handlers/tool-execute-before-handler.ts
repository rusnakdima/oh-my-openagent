...
function resolvePreToolUseCwd(
  ...
  const trackedWorktree = nonBlankString(
    getWorkForSession(ctx.directory, input.sessionID)?.worktree_path,
  );
  if (trackedWorktree) {
...
export function createToolExecuteBeforeHandler(
  ...
  return async (
    ...
    if (
      input.tool.trim() === "todowrite" && typeof output.args.todos === "string"
    ) {
      let parsed: unknown;
      try {
      ...
      log("todowrite: parsed todos string to array", {
        sessionID: input.sessionID,
      });
    }
      ...
      ctx.client.tui
        ...
        .catch((error: unknown) => {
          if (error instanceof Error) {
            log("PreToolUse hook toast failed", {
              sessionID: input.sessionID,
              error: error.message,
            });
          } else {
