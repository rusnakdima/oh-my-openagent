const { afterEach, beforeEach, describe, expect, mock, spyOn, test } = require(
  "bun:test",
);
...
const { clearClaudeHooksConfigCache, loadClaudeHooksConfig } = await import(
  "./config"
);
...
describe("mergePluginHooksConfigs", () => {
  const {
    mergePluginHooksConfigs,
    setPluginHooksConfigs,
    clearClaudeHooksConfigCache: _clearCache,
    resetPluginHooksState,
  } = require("./config");
  const { setAdditionalAllowedMcpEnvVars, resetAdditionalAllowedMcpEnvVars } =
    require("../../features/claude-code-mcp-loader/configure-allowed-env-vars");
  ...
  test("#given empty plugin hooks #when merged #then returns base unchanged", () => {
    ...
    const base = {
      Stop: [{
        matcher: "*",
        hooks: [{ type: "command" as const, command: "echo stop" }],
      }],
    };
        ...
        PreToolUse: [
          {
            ...
            hooks: [{
              ...
              allowedEnvVars: ["MY_TOKEN", "SECRET_KEY"],
            }],
          },
        ],
      },
...
describe("setPluginHooksConfigs", () => {
  ...
  const { resetAdditionalAllowedMcpEnvVars } = require(
    "../../features/claude-code-mcp-loader/configure-allowed-env-vars",
  );
  let originalWorkingDirectory = "";
