...
function resolveParticipantFromRegistry(
  sessionID: string,
...
export function createTeamToolGating(
  _ctx: PluginInput,
  config: TeamModeConfig | undefined,
): Hooks {
  return {
    "tool.execute.before": async (
      ...
      if (
        toolName === "team_approve_shutdown" ||
        toolName === "team_reject_shutdown"
      ) {
        if (
          !isLeadOfTargetTeam(participant, teamRunId) &&
          !isTargetMember(participant, teamRunId, memberName)
        ) {
          throw new Error(
