...
async function resolveOrProvisionCommand(
  ...
  if (!nodeSupport.supported) {
    deps.log(
      "[codegraph-bootstrap] CodeGraph unsupported on this Node runtime; skipping bootstrap",
      {
  ...
  if (!provisioned.provisioned || provisioned.binPath === undefined) {
    deps.log(
      "[codegraph-bootstrap] CodeGraph unavailable; skipping bootstrap",
      {
...
async function runBootstrap(
  ...
  try {
    ...
    if (!autoInit && !existsSync(codegraphPath)) {
      deps.log(
        "[codegraph-bootstrap] CodeGraph auto_init disabled and .codegraph not present; skipping bootstrap",
        {
          projectRoot,
        },
      );
    ...
    const nodeSupport = deps.nodeSupport();
    if (
      command.source !== "bundled" && command.source !== "env" &&
    ...
    const status = await deps.runCommand(projectRoot, command.command, [
      ...command.argsPrefix,
      "status",
      "--json",
    ], {
    ...
    if (decision.kind === "skip") {
      deps.log(
        "[codegraph-bootstrap] CodeGraph status failed; skipping bootstrap",
        { projectRoot, reason: decision.reason },
      );
      return;
    }

    const actionArgs = command.argsPrefix.concat(
      decision.kind === "init" ? ["init"] : ["sync"],
    );
    const action = await deps.runCommand(
      projectRoot,
      command.command,
      actionArgs,
    ...
    if (error instanceof Error) {
      deps.log("[codegraph-bootstrap] Bootstrap failed", {
        error: error.message,
        projectRoot,
      });
      return;
  ...
  return {
    event(input: CodegraphBootstrapEventInput): void {
      try {
        ...
        const projectRoot = resolveCodegraphProjectRoot(
        ...
        if (exclusion.excluded) {
          deps.log(
            "[codegraph-bootstrap] CodeGraph project excluded; skipping bootstrap",
            {
        ...
        if (error instanceof Error) {
          deps.log("[codegraph-bootstrap] Event handler failed", {
            error: error.message,
          });
          return;
        }
