...
describe("CodeGraph command runner", () => {
  ...
  test("#given ambient provider tokens #when command runner spawns CodeGraph #then child env only gets safe and controlled variables", async () => {
    ...
    const workspace = mkdtempSync(
      join(tmpdir(), "omo-codegraph-opencode-env-"),
    );
  ...
  test("#given the command spawns a hanging descendant #when the timeout expires #then the process tree is reaped", async () => {
    // given
    const workspace = mkdtempSync(
      join(tmpdir(), "omo-codegraph-opencode-timeout-"),
    );
    ...
    if (address === null || typeof address === "string") {
      throw new Error("readiness server has no TCP port");
    }
    ...
    try {
      ...
      const result = await withTimeout(
        resultPromise,
        5_000,
        "command did not stop after the timeout signal",
      );
      descendantPid = Number.parseInt(
        readFileSync(join(workspace, "survivor.pid"), "utf8").trim(),
        10,
      );
