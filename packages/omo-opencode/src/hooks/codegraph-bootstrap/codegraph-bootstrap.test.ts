...
function createDeps(
  ...
  return {
    ...
    runCommand: async (_projectRoot, command, args) => {
      ...
      if (args[0] === "status") {
        return { exitCode: 0, stdout: "not initialized", timedOut: false };
      }
...
describe("createCodegraphBootstrapHook", () => {
  test("#given a non-session-created event #when event fires #then it does nothing", () => {
    // given
    const events: string[] = [];
    const hook = createCodegraphBootstrapHook({ directory: "/repo" }, {
      enabled: true,
    }, createDeps(events));
  ...
  test("#given session.created includes a worktree path #when it fires #then bootstrap runs once for that project", async () => {
    ...
    const hook = createCodegraphBootstrapHook({ directory: "/fallback" }, {
      auto_provision: true,
      enabled: true,
    }, createDeps(events));
    ...
    // when
    hook.event({
      event: { type: "session.created", properties: { worktree: "/repo" } },
    });
    hook.event({
      event: { type: "session.created", properties: { worktree: "/repo" } },
    });
    ...
    // then
    expect(events.filter((event) => event === `prepare:${projectRoot}`))
      .toHaveLength(1);
  ...
  test("#given binary resolution fails and auto provision fails #when background work runs #then it logs and skips without throwing", async () => {
    ...
    expect(events).toContain(
      "log:[codegraph-bootstrap] CodeGraph unavailable; skipping bootstrap",
    );
  ...
  test("#given a PATH CodeGraph binary but the host Node is unsupported #when background work runs #then it leaves the project untouched", async () => {
    ...
    const workspace = mkdtempSync(
      join(tmpdir(), "omo-codegraph-opencode-unsupported-node-"),
    );
    const homeDir = mkdtempSync(
      join(tmpdir(), "omo-codegraph-opencode-unsupported-node-home-"),
    );
    const events: string[] = [];
    const hook = createCodegraphBootstrapHook(
      { directory: workspace },
      { auto_provision: false, enabled: true },
      {
        ...
        prepareWorkspace: (projectRoot) =>
          prepareCodegraphWorkspace(projectRoot, { homeDir }),
        resolveCommand: () => ({
          argsPrefix: [],
    ...
    try {
      // when
      hook.event({
        event: { type: "session.created", properties: { worktree: workspace } },
      });
      await waitForBackground();

      // then
      expect(events).toContain(
        "log:[codegraph-bootstrap] CodeGraph unsupported on this Node runtime; skipping bootstrap",
      );
      expect(existsSync(join(workspace, ".codegraph"))).toBe(false);
      expect(existsSync(join(workspace, ".git", "info", "exclude"))).toBe(
        false,
      );
    } finally {
  ...
  test("#given CodeGraph is missing and auto provision is enabled on unsupported Node #when background work runs #then it does not provision or mutate the project", async () => {
    // given
    const workspace = mkdtempSync(
      join(tmpdir(), "omo-codegraph-opencode-unsupported-provision-"),
    );
    const homeDir = mkdtempSync(
      join(tmpdir(), "omo-codegraph-opencode-unsupported-provision-home-"),
    );
    const events: string[] = [];
    const hook = createCodegraphBootstrapHook(
      { directory: workspace },
  ...
  test("#given CodeGraph is unavailable and auto provisioning is disabled #when background work runs #then it leaves the project untouched", async () => {
    // given
    const workspace = mkdtempSync(
      join(tmpdir(), "omo-codegraph-opencode-unavailable-"),
    );
    const homeDir = mkdtempSync(
      join(tmpdir(), "omo-codegraph-opencode-unavailable-home-"),
    );
    const events: string[] = [];
    const hook = createCodegraphBootstrapHook(
      { directory: workspace },
  ...
  test("#given the project is under an excluded root #when session.created fires #then bootstrap is skipped and not scheduled", async () => {
    ...
    expect(events.some((event) => event.startsWith("run:"))).toBe(false);
    expect(events).toContain(
      "log:[codegraph-bootstrap] CodeGraph project excluded; skipping bootstrap",
    );
  });
  ...
  test("#given the real exclusion defaults #when the project lives under the OS tmpdir #then bootstrap is skipped", async () => {
    ...
    const events: string[] = [];
    const workspace = mkdtempSync(
      join(tmpdir(), "omo-codegraph-opencode-excluded-"),
    );
    const hook = createCodegraphBootstrapHook(
      { directory: workspace },
      { enabled: true },
