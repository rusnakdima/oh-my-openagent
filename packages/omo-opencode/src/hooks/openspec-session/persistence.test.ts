...
beforeEach(async () => {
  tmp = join(
    tmpdir(),
    `openspec-persistence-test-${Date.now()}-${
...
describe("writeSessionRecord and readSessionRecord", () => {
  ...
  it("overwrites existing record", async () => {
    writeSessionRecord(tmp, "session-1", {
      specName: "spec-a",
      injectedAt: 1000,
    });
    writeSessionRecord(tmp, "session-1", {
...
describe("loadAllSessionRecords", () => {
  ...
  it("skips malformed JSON files", async () => {
    ...
    await writeFile(
      join(tmp, "good.json"),
      JSON.stringify({ specName: "good", injectedAt: 1, version: 1 }),
    );
    const all = loadAllSessionRecords(tmp);
  ...
  it("handles session IDs with special characters", () => {
    ...
    writeSessionRecord(tmp, "session%2Fwith%2Fslashes", {
      specName: "slash-spec",
      injectedAt: 3000,
    });
    const all = loadAllSessionRecords(tmp);
