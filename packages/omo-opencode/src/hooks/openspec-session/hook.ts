...
function extractSpecName(messageText: string): string {
  ...
  const cleaned = firstSentence.replace(
    /^(plan|build|create|implement|make|write|design|develop)\s+(a?\s*|the\s*)/i,
    "",
  );
...
async function buildIdleContinuationPrompt(
  ...
  lines.push("");
  lines.push(
    "Choose the next concrete action to advance the spec. Avoid repeating work already done.",
  );
  ...
  return {
    ...
    "chat.message": async (
      ...
      if (specs.length === 0 && autoCreate) {
        ...
        writeSessionRecord(sessionStoreDir, input.sessionID, {
          specName,
          injectedAt: Date.now(),
        });
        try {
          await injectSpecContext(
            output,
            specName,
            specDir,
            projectDir,
          ...
          log(
            `[${HOOK_NAME}] Auto-created spec "${specName}" for session ${input.sessionID}`,
          );
        } catch (err) {
          log(
            `[${HOOK_NAME}] Failed to inject auto-created spec context:`,
            err,
          );
        }
        return;
      ...
      try {
        ...
        log(
          `[${HOOK_NAME}] Injected spec context for session ${input.sessionID}, spec: ${specName}`,
        );
      } catch (err) {
      ...
      if (activeSpec) {
        ...
        if (content) {
          ...
          if (updated) {
            ...
            log(
              `[${HOOK_NAME}] Wrote task completion back to ${activeSpec}/tasks.md`,
            );
          }
        }
      } else {
        ...
        for (const specName of specs) {
          ...
          if (updated) {
            await writeSpecFile(tasksFile, updated);
            log(
              `[${HOOK_NAME}] Wrote task completion back to ${specName}/tasks.md`,
            );
            break;
          }
        }
    ...
    event: async (
      ...
      if (input.event.type === "session.idle") {
        ...
        const promptText = await buildIdleContinuationPrompt(
          specName,
          projectDir,
          specDir,
        );
