...
function createMockController(projectDir: string, specDir = "openspec") {
  return {
    propose: async (_specName: string, _description?: string) => ({
      success: true,
      message: "mocked",
    }),
    verify: async (
      _specName?: string,
    ) => [{ specName: "mock", valid: true, files: [], taskStats: undefined }],
    apply: async (specName: string, _sessionID?: string) =>
      applySpec(projectDir, specDir, specName, _sessionID),
    archive: async (_specName: string) => ({
      success: true,
      message: "mocked",
    }),
    status:
      async () => [{ specName: "mock", valid: true, taskStats: undefined }],
...
beforeEach(async () => {
  ...
  await writeFile(
    join(specRoot, "test-spec", "tasks.md"),
    "| [~] | In-progress task |\n| [x] | Done |\n",
  );
});
...
describe("createOpenSpecSessionHook", () => {
  ...
  describe("chat.message handler", () => {
    it("returns early when autoInject=false", async () => {
      ...
      const output = {
        parts: [{ type: "text", text: "Hello" }] as Array<
    ...
    it("skips second message for same session", async () => {
      ...
      const output2 = {
        parts: [{ type: "text", text: "Hello again" }] as Array<
          { type: string; text?: string; [key: string]: unknown }
        >,
      };
    ...
    it("adds projectDir to injectedSessions when injection fires", async () => {
      ...
      const output1 = {
        parts: [{ type: "text", text: "First" }] as Array<
          { type: string; text?: string; [key: string]: unknown }
        >,
      };
      ...
      const output2 = {
        parts: [{ type: "text", text: "Second" }] as Array<
          { type: string; text?: string; [key: string]: unknown }
        >,
      };
    ...
    it("auto-creates spec when no specs exist and autoCreate=true", async () => {
      ...
      const emptyTmp = join(
        tmpdir(),
        `openspec-auto-create-test-${Date.now()}-${
      ...
      const autoCreateController = {
        propose: async (name: string, desc?: string) => {
          ...
          await writeFile(
            join(emptySpecRoot, name, "spec.md"),
            `# ${name}\n\n${desc ?? ""}\n`,
          );
          await writeFile(join(emptySpecRoot, name, "plan.md"), "# Plan\n\n");
          await writeFile(
            join(emptySpecRoot, name, "tasks.md"),
            "| [ ] | Task 1 |\n",
          );
    ...
    it("does NOT auto-create when autoCreate=false (default)", async () => {
      ...
      const emptyTmp = join(
        tmpdir(),
        `openspec-no-auto-create-${Date.now()}-${
          Math.random().toString(36).slice(2)
        }`,
      );
      const emptySpecRoot = join(emptyTmp, "openspec");
      ...
      const noAutoCreateController = {
        propose: async (name: string) => {
          proposeCalls.push(name);
          return { success: true };
        },
        verify: async () => [{
          specName: "mock",
      ...
      const output = {
        parts: [{ type: "text", text: "Hello world" }] as Array<
          { type: string; text?: string; [key: string]: unknown }
        >,
      };
  ...
  describe("tool.execute.after handler — taskWriteBack", () => {
    ...
    it("does not write to tasks.md when task_update fires without completion keyword", async () => {
      ...
      const tasksAfter = await readFile(
        join(specRoot, "test-spec", "tasks.md"),
        "utf-8",
      );
    ...
    it("applies only the first matching spec when multiple specs exist", async () => {
      ...
      await writeFile(
        join(specRoot, "another-spec", "tasks.md"),
        "| [~] | Another in-progress |\n",
      );
      ...
      const tasksA = await readFile(
        join(specRoot, "test-spec", "tasks.md"),
        "utf-8",
      );
      const tasksB = await readFile(
        join(specRoot, "another-spec", "tasks.md"),
        "utf-8",
      );
