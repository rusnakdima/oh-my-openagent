  ...
  return {
    ...
    async propose(specName: string, description?: string): Promise<{
      ...
      if (await specExists(specPath)) {
        return {
          success: false,
          message:
            `Spec "${specName}" already exists at ${specDir}/${specName}/.`,
        };
      }
      ...
      const specContent = description
        ...
        : `# ${specName}\n`;
      const planContent =
        `# ${specName} — Technical Plan\n\n(Not yet written)\n`;
      const taskLines =
        `# Tasks\n\n| Status | Description |\n| ------ | ----------- |\n`;

      await writeSpecFile(
        resolveSpecFile(projectDir, specDir, specName, "spec.md"),
        specContent,
      );
      await writeSpecFile(
        resolveSpecFile(projectDir, specDir, specName, "plan.md"),
        planContent,
      );
      await writeSpecFile(
        resolveSpecFile(projectDir, specDir, specName, "tasks.md"),
        taskLines,
      );
    ...
    async archive(
      ...
      if (await specExists(destPath)) {
        return {
          success: false,
          message: `Archived spec "${specName}" already exists in ARCHIVE/.`,
        };
      }
      ...
      return {
        success: true,
        message:
          `Spec "${specName}" archived to ${specDir}/ARCHIVE/${specName}/`,
      };
    },
      ...
      Array<
        {
          specName: string;
          valid: boolean;
          taskStats?: import("../../tools/openspec/store").TaskStats;
        }
      >
    > {
