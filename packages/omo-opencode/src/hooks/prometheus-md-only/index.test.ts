...
describe("prometheus-md-only", () => {
  ...
  describe("with Prometheus agent in message storage", () => {
    ...
    test("should not double-inject warning if already present", async () => {
      ...
      const promptWithWarning =
        `Some prompt ${PLANNING_CONTEXT_OPEN} already here`;
      const output = {
      ...
      // then
      const occurrences =
        (output.args.prompt as string).split(PLANNING_CONTEXT_OPEN).length - 1;
  ...
  describe("cross-platform path validation", () => {
    ...
    test("should allow nested project path with .omo (Windows real-world case)", async () => {
      ...
      const output = {
        args: {
          filePath:
            "xauusd-dxy-plan\\.omo\\drafts\\supabase-email-templates.md",
        },
      };
