...
export const BTW_TOOL_GUARD_DENIAL_MESSAGE =
  "/btw side questions are read-only and cannot use tools.";
...
function hasParentSession(response: unknown): boolean {
  ...
  return typeof sessionInfo.parentID === "string" &&
    sessionInfo.parentID.length > 0;
}

async function isPrimaryInteractiveSession(
  deps: BtwToolGuardDeps,
  sessionID: string,
): Promise<boolean> {
...
async function isBtwAnswerTurn(
  deps: BtwToolGuardDeps,
  sessionID: string,
): Promise<boolean> {
  try {
    ...
    const latestUserMessage = findLatestUserMessage(
      normalizeSessionMessages(response),
    );
    return latestUserMessage ? isBtwMarked(latestUserMessage) : false;
  } catch (error) {
    log(
      "[btw-tool-guard] Failed to resolve session messages, falling back to local turn state",
      {
        sessionID,
        error: String(error),
