...
function makeClient(
  session: ReturnType<typeof makeRecordingSession>["session"],
) {
  return { session };
}
...
describe("dispatchInternalPrompt live-server-route routing", () => {
  ...
  test("#given registered inProcessClient and live route available #when dispatchInternalPrompt runs #then promptAsync is called on the live client not the original", async () => {
    ...
    initLiveServerRoute({
      ...
      directory: "/test",
      inProcessClient: orig,
    });
    _setLiveClientForTesting(live);

    //#when
    const result = await dispatchInternalPrompt(
      makeBaseDispatchArgs("ses_live_dispatch", orig),
    );
  ...
  test("#given a child session in subagentSessions #when dispatchInternalPrompt runs #then the original client is used and live client is untouched", async () => {
    ...
    //#when
    const result = await dispatchInternalPrompt(
      makeBaseDispatchArgs("ses_child", orig),
    );
  ...
  test("#given an unregistered client #when dispatchInternalPrompt runs #then the original client is used and no probe fetch is made", async () => {
    ...
    _setFetchImplementationForTesting(
      (async (_url: unknown, _opts?: unknown) => {
        fetchCallCount += 1;
        return new Response(JSON.stringify([]), { status: 200 });
      }) as typeof fetch,
    );
    initLiveServerRoute({
      serverUrl: new URL("http://127.0.0.1:9999"),
      directory: "/test",
      inProcessClient: makeClient(origSession.session),
    });

    //#when
    const result = await dispatchInternalPrompt(
      makeBaseDispatchArgs("ses_unregistered", unregistered),
    );
  ...
  test("#given live routing is disabled via setLiveParentWakeRoutingDisabled #when dispatchInternalPrompt runs #then the original client is used", async () => {
    ...
    //#when
    const result = await dispatchInternalPrompt(
      makeBaseDispatchArgs("ses_flag_disabled", orig),
    );
  ...
  test("#given live dispatch rejects with a pre-send connection failure #when dispatchInternalPrompt runs #then fallback dispatches exactly once on original client and route is marked unavailable", async () => {
    ...
    let liveCalls = 0;
    let origCalls = 0;
    const connError = Object.assign(new TypeError("fetch failed"), {
      code: "ECONNREFUSED",
    });
    const liveSession = {
    ...
    //#when
    const result = await dispatchInternalPrompt(
      makeBaseDispatchArgs("ses_fallback_conn", orig),
    );
  ...
  test(
    "#given live dispatch hangs past dispatchTimeoutMs #when dispatchInternalPrompt runs #then result is failed with timeout error original client is not called and route is marked unavailable",
    async () => {
      //#given
      let liveCalls = 0;
      let origCalls = 0;
  ...
  test(
    "#given live route active and queueBehavior enqueue #when session is initially busy then becomes idle #then queued entry dispatches via live client",
    async () => {
      //#given
      let status = "busy";
      let liveCalls = 0;
      ...
      await new Promise<void>((resolve, reject) => {
        const timeoutId = setTimeout(
          () => reject(new Error("timed out waiting for queued prompt")),
          2_000,
        );
        promptSeen.then(() => {
