...
describe("dispatchInternalPrompt semantic dedupe", () => {
  ...
  test("#given the same semantic prompt is built with different object key order #when it repeats during the broad hold #then the duplicate is coalesced", async () => {
    ...
    try {
      ...
      expect(second).toEqual({
        status: "queued",
        queuedBy: "test:semantic-order:first",
        position: 0,
      });
      expect(promptCalls).toEqual(["prompt"]);
    } finally {
    ...
    try {
      ...
      const first = await dispatchInternalPrompt({
        ...
        input: {
          ...
          body: {
            parts: [{
              ...createInternalAgentTextPart("internal route A"),
              synthetic: true,
            }],
          },
        },
      ...
      const second = await dispatchInternalPrompt({
        ...
        input: {
          ...
          body: {
            parts: [{
              ...createInternalAgentTextPart("internal route B"),
              synthetic: true,
            }],
          },
        },
    ...
    try {
      ...
      const first = await dispatchInternalPrompt({
        ...
        input: {
          ...
          body: {
            parts: [
              createInternalAgentContinuationTextPart("continue from route A"),
            ],
            agent: "sisyphus",
            model: "openai/gpt-5",
          },
      ...
      const second = await dispatchInternalPrompt({
        ...
        input: {
          ...
          body: {
            parts: [
              createInternalAgentContinuationTextPart("continue from route B"),
            ],
            agent: "atlas",
