...
export const INTERRUPTED_TOOL_ERROR =
  "[Tool execution was interrupted before it produced output]";
...
export function repairUnpairedToolParts(
  ...
  log(
    "[tool-pair-validator] Settled unpaired tool parts into a terminal error state",
    {
      assistantMessageID: getMessageID(message.info),
...
export function diagnoseSubAgentUnpairedToolParts(
  message: MessageWithParts,
  sessionID: string,
): void {
  const unpaired = findUnpairedToolParts(message.parts);
