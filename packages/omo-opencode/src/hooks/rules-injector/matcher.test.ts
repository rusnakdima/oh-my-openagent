...
describe("shouldApplyRule", () => {
  ...
  it("#given repeated glob metadata #when matching many files #then compiles each pattern once", () => {
    ...
    for (let index = 0; index < 20; index += 1) {
      shouldApplyRule(
        metadata,
        `${projectRoot}/src/file-${index}.ts`,
        projectRoot,
      );
      shouldApplyRule(
        metadata,
        `${projectRoot}/test/file-${index}.ts`,
        projectRoot,
      );
    }
...
describe("rule duplicate helpers", () => {
  it("#given real path cache #when path exists #then reports duplicate", () => {
    ...
    // when / then
    expect(isDuplicateByRealPath("/workspace/project/AGENTS.md", cache)).toBe(
      true,
    );
    expect(isDuplicateByRealPath("/workspace/project/src/AGENTS.md", cache))
      .toBe(false);
  });
