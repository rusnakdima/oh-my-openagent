...
function isSameFailedModel(
  ...
  return state.providerID.toLowerCase() === providerID.toLowerCase() &&
    canonicalizeModelIDForDuplicateCheck(state.modelID) ===
      canonicalizeModelIDForDuplicateCheck(modelID);
}
...
export function createModelFallbackStateController(input: {
  ...
  function setPendingModelFallback(
    ...
    if (!fallbackChain?.length) {
      log(
        `[model-fallback] No fallback chain for agent: ${agentName} (key: ${agentKey})`,
      );
      return false;
    }
    ...
    if (!existing) {
      ...
      log(
        `[model-fallback] Set pending fallback for session: ${sessionID}, agent: ${agentName}`,
      );
      return true;
    }
    ...
    if (existing.pending) {
      log(
        `[model-fallback] Pending fallback already armed for session: ${sessionID}`,
      );
      return false;
    }
    ...
    if (
      ...
      isSameFailedModel(existing, currentProviderID, currentModelID)
    ) {
      log(
        `[model-fallback] Ignoring duplicate fallback arm for already handled model in session: ${sessionID}`,
      );
      return false;
    }
    ...
    if (existing.attemptCount >= existing.fallbackChain.length) {
      log(
        `[model-fallback] Fallback chain exhausted for session: ${sessionID}`,
      );
      return false;
    }
