...
export function getNextReachableFallback(
  ...
  while (state.attemptCount < state.fallbackChain.length) {
    ...
    if (!isReachable(fallback)) {
      log(
        "[model-fallback] Skipping unreachable fallback for session: " +
          sessionID + ", attempt: " + attemptCount + ", model: " +
          fallback.model,
      );
      continue;
    }
    ...
    if (isNoOpFallback) {
      log(
        "[model-fallback] Skipping no-op fallback for session: " + sessionID +
          ", attempt: " + attemptCount + ", model: " + fallback.model,
      );
      continue;
    }
    ...
    state.pending = false;
    log(
      "[model-fallback] Using fallback for session: " + sessionID +
        ", attempt: " + attemptCount + ", model: " + fallback.model,
    );
