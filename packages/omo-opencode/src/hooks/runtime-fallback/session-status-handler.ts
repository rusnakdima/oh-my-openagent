...
export function createSessionStatusHandler(
  ...
  return async (props: Record<string, unknown> | undefined) => {
    ...
    const retryMessage = typeof status.message === "string"
      ? status.message
      : "";
    ...
    if (!retrySignal) {
      ...
      const matchesRetryablePattern = RETRYABLE_ERROR_PATTERNS.some((pattern) =>
    ...
    const retryKey = `${retryModel}:${
      extractRetryAttempt(status.attempt, retryMessage)
    ...
    if (sessionRetryInFlight.has(sessionID)) {
      if (timeoutEnabled) {
        log(
          `[${HOOK_NAME}] Overriding in-flight retry due to provider auto-retry signal`,
          {
            sessionID,
            model,
          },
        ...
        log(
          `[${HOOK_NAME}] session.status retry skipped - retry already in flight`,
          { sessionID },
        );
    ...
    if (!state) {
      ...
      if (!initialModel) {
        sessionStatusRetryKeys.delete(sessionID);
        log(
          `[${HOOK_NAME}] session.status retry missing model info, cannot fallback`,
          { sessionID },
        );
        return;
      }
    ...
    if (state.pendingFallbackModel) {
      if (state.pendingFallbackPromptMayHaveBeenAccepted) {
        log(
          `[${HOOK_NAME}] session.status retry skipped (pending fallback prompt may already be accepted)`,
          {
            sessionID,
      ...
      if (timeoutEnabled) {
        ...
        log(
          `[${HOOK_NAME}] session.status retry skipped (pending fallback in progress)`,
          {
            sessionID,
            pendingFallbackModel: state.pendingFallbackModel,
          },
    ...
    log(
      `[${HOOK_NAME}] Detected provider auto-retry signal in session.status`,
      {
        sessionID,
