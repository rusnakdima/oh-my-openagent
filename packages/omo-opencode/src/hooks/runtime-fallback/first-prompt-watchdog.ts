...
import { DEFAULT_FIRST_PROMPT_WATCHDOG_MS, HOOK_NAME } from "./constants";
...
export function observeEventForWatchdog(
  ...
  if (
    ...
    const hasTextDelta = props.field === "text" &&
      typeof props.delta === "string";
    const hasNonEmptySessionPart = typeof part?.sessionID === "string" &&
      Object.keys(part).length > 0;
    if (
      sessionID &&
      (hasPartType || hasTopLevelType || hasTextDelta || hasNonEmptySessionPart)
    ) {
...
export function createFirstPromptWatchdog(
  ...
  const fire = async (
    ...
    if (!subagentSessions.has(sessionID)) {
      log(
        `[${HOOK_NAME}] ${SOURCE}: session no longer a subagent at fire time, skipping`,
        { sessionID },
      );
      return;
    }
    ...
    if (fallbackModels.length === 0) {
      log(
        `[${HOOK_NAME}] ${SOURCE}: subagent silent past ${watchdogMs}ms with no fallback configured`,
        {
          sessionID,
          model,
          agent: resolvedAgent,
    ...
    if (!state) {
      ...
      if (!initialModel) {
        log(
          `[${HOOK_NAME}] ${SOURCE}: no model info available, cannot dispatch fallback`,
          { sessionID },
        );
        return;
      }
    ...
    log(
      `[${HOOK_NAME}] ${SOURCE}: subagent silent past ${watchdogMs}ms, dispatching fallback`,
      {
        sessionID,
  ...
  return {
    onUserMessage(sessionID, model, agent) {
      ...
      log(`[${HOOK_NAME}] ${SOURCE}: armed for subagent`, {
        sessionID,
        model,
        agent,
        watchdogMs,
    ...
    onAssistantProgress(sessionID) {
      ...
      cancel(sessionID);
      log(`[${HOOK_NAME}] ${SOURCE}: cancelled (assistant progress observed)`, {
        sessionID,
      });
    },
    onSessionTerminal(sessionID) {
      if (!sessionID || !armed.has(sessionID)) return;
      cancel(sessionID);
      log(`[${HOOK_NAME}] ${SOURCE}: cancelled (session terminal)`, {
        sessionID,
      });
    },
    dispose() {
