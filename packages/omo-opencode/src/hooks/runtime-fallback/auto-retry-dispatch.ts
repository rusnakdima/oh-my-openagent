...
export function createAutoRetryDispatcher(
  ...
  return async (
    ...
    const hadAwaitingFallbackResult = sessionAwaitingFallbackResult.has(
      sessionID,
    );
    const fallbackState = sessionStates.get(sessionID);
    const isCurrentFallbackGeneration = () =>
      sessionStates.get(sessionID) === fallbackState;
    const staleGenerationOutcome = (): AutoRetryDispatchOutcome => {
      log(
        `[${HOOK_NAME}] Auto-retry skipped for stale fallback generation (${source})`,
        {
          sessionID,
        },
      );
    ...
    const previousPendingFallbackPromptMayHaveBeenAccepted = fallbackState
    ...
    try {
      ...
      const originalRetryMetadata = resolveOriginalUserRetryMetadata(
        messagesResp,
      );
      ...
      const retryParts = fetchedParts.length > 0
        ...
        : (() => {
          log(
            `[${HOOK_NAME}] No user message parts found for auto-retry (${source}); using synthetic continuation`,
            {
              sessionID,
              hint:
                "This can occur when the working directory contains .git and messages are not yet persisted",
            },
          );
      ...
      const retryMessageID = usingFetchedUserParts
      ...
      if (!isCurrentFallbackGeneration()) return staleGenerationOutcome();
      let promptResult = await dispatchRetryPrompt(
      ...
      if (promptResult.status === "active") {
        log(
          `[${HOOK_NAME}] Session active, queueing fallback dispatch (${source})`,
          {
            sessionID,
          },
        );
        promptResult = await dispatchRetryPrompt(
          `runtime-fallback:${source}:active-queue`,
        );
        if (!isCurrentFallbackGeneration()) return staleGenerationOutcome();
      ...
      if (promptResult.status === "reserved") {
        ...
        for (let attempt = 0; attempt < MAX_RESERVED_RETRIES; attempt++) {
          ...
          log(
            `[${HOOK_NAME}] Session reserved, retrying fallback dispatch in ${delay}ms (${source})`,
            {
              sessionID,
              attempt: attempt + 1,
        ...
        if (!isInternalPromptDispatchAccepted(reservedResult)) {
          log(
            `[${HOOK_NAME}] Auto-retry skipped by promptAsync gate after reserved retries (${source})`,
            {
              sessionID,
          ...
          return {
            accepted: false,
            status: "blocked",
            reason: `prompt gate returned ${reservedResult.status}`,
          };
        }
        acceptedStatus = "queued";
      } else if (!isInternalPromptDispatchAccepted(promptResult)) {
        log(
          `[${HOOK_NAME}] Auto-retry skipped by promptAsync gate (${source})`,
          {
            sessionID,
        ...
        return {
          accepted: false,
          status: "blocked",
          reason: `prompt gate returned ${promptResult.status}`,
        };
      }
      if (!isCurrentFallbackGeneration()) return staleGenerationOutcome();
      sessionAwaitingFallbackResult.add(sessionID);
      ...
      if (
        !retryDispatched && !retryMayHaveBeenAccepted && ownsFallbackGeneration
      ) {
        if (hadAwaitingFallbackResult) {
          sessionAwaitingFallbackResult.add(sessionID);
        } else {
        ...
        if (fallbackState) {
          fallbackState.currentModel = previousCurrentModel ??
            fallbackState.currentModel;
          if (hadAwaitingFallbackResult) {
