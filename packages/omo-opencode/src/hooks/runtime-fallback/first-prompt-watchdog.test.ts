...
import {
  ...
  type FirstPromptWatchdog,
  observeEventForWatchdog,
} from "./first-prompt-watchdog";
...
function createDeps(
  pluginConfig: HookDeps["pluginConfig"] = undefined,
): HookDeps {
  return {
    ctx: createContext(),
    config: {
...
describe("first-prompt-watchdog", () => {
  ...
  function getFakeTimers(): FakeTimers {
    if (!fakeTimers) {
      throw new Error(
        "Fake timers must be installed before advancing watchdog time",
      );
    }
    return fakeTimers;
  }
...
function createRecordingWatchdog(
  calls: RecordedWatchdogCalls,
): FirstPromptWatchdog {
  return {
...
describe("observeEventForWatchdog", () => {
  ...
  const assistantProgressParts: ReadonlyArray<
    readonly [
      string,
      {
  ...
  it.each(assistantProgressParts)(
    "#given a message.updated assistant event whose only part is type=%s #when observed #then onAssistantProgress is called (model is *working*, not silent)",
    (
      _label: string,
      part: {
        readonly type: string;
  ...
  it.each(assistantProgressParts)(
    "#given a message.part.updated event whose part is type=%s #when observed #then onAssistantProgress is called",
    (
      _label: string,
      part: {
        readonly type: string;
    ...
    observeEventForWatchdog(
      {
        ...
        properties: {
          info: {
            sessionID,
            role: "assistant",
            error: { name: "RateLimitError", message: "429" },
          },
        },
      },
      createRecordingWatchdog(calls),
  ...
  const terminalEventTypes: ReadonlyArray<readonly [string]> = [
    ["session.idle"],
    ["session.stop"],
    ["session.deleted"],
    ["session.error"],
