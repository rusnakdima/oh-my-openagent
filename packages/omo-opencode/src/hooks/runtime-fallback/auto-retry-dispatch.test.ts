...
describe("createAutoRetryDispatcher reserved-session retry (#5109)", () => {
      ...
      data: assistantIsActive
        ? [
          ...
          },
          {
            ...
            parts: [{
              type: "reasoning",
              text: "still resolving failed stream",
            }],
          },
        ]
        : [
        ...
        ? [
          {
            ...
            parts: [{
              type: "reasoning",
              text: "still resolving prior fallback",
            }],
          },
        ]
        : [
  ...
  test("#given manual model change replaces state during a failed dispatch #when rollback runs #then the replacement state is preserved", async () => {
    ...
    deps.sessionStates.set(
      sessionID,
      createFallbackState("anthropic/claude-opus-4-7"),
    );
    deps.sessionAwaitingFallbackResult.add(sessionID);
  ...
  test("#given a manual reset starts a new retry while the old dispatcher is awaiting messages #when the old generation completes #then it cannot clear the new generation in-flight marker", async () => {
    ...
    // when
    const staleRetry = helpers.autoRetryWithFallback(
      sessionID,
      "openai/gpt-5.4",
      undefined,
      "session.error",
    ...
    const currentRetry = helpers.autoRetryWithFallback(
      sessionID,
    ...
    if (!releaseStaleLookup) {
      throw new Error("stale message lookup did not start");
    }
    releaseStaleLookup();
    ...
    if (!releaseCurrentLookup) {
      throw new Error("current message lookup did not start");
    }
