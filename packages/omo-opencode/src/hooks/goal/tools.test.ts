...
function makeDeps(
  controller: Partial<GoalController> = {},
  sessionID?: string,
) {
  return {
    controller: {
...
describe("createGoalTools", () => {
  test("create_goal calls controller.setGoal with current session", async () => {
    const setGoal = (sessionID: string, objective: string) =>
      makeGoal({ sessionID, objective });
    const deps = makeDeps({ setGoal }, "s1");
    const tools = createGoalTools(deps);

    const result = await tools.create_goal.execute(
      { objective: "Ship" },
      {} as never,
    );
  ...
  test("create_goal uses explicit session_id", async () => {
    ...
    await tools.create_goal.execute(
      { objective: "Ship", session_id: "s2" },
      {} as never,
    );
  ...
  test("update_goal completes goal", async () => {
    ...
    const result = await tools.update_goal.execute(
      { status: "complete" },
      {} as never,
    );
