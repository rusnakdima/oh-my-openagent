...
function createGoal(
  objective: string,
  usage?: { timeUsedSeconds: number; tokensUsed: number },
): Goal {
  return {
    id: "goal-1",
    sessionID: "ses-1",
...
describe("buildContinuationPrompt", () => {
  test("propagates dynamic accumulated usage", () => {
    ...
    const prompt = buildContinuationPrompt(
      createGoal("continuation-objective-sentinel", usage),
    );
...
describe("buildResumePrompt", () => {
  test("uses the same escaped dynamic objective payload as continuation", () => {
    ...
    const continuationPayload = extractObjectivePayload(
      buildContinuationPrompt(goal),
    );
    const resumePayload = extractObjectivePayload(
      buildResumePrompt({ ...goal, status: "paused" }),
    );
