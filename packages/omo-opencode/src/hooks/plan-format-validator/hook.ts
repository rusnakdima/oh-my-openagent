...
function analyzeStructuredSections(content: string): PlanFormatStats {
  ...
  for (const line of lines) {
    ...
    if (SECTION_BOUNDARY_HEADING.test(line)) {
      const name = HEADING_TODOS.test(line)
        ? "todo"
  ...
  return {
    ...
    hasMalformedRows: sections.some((item) =>
      item.rawCount !== item.validCount
    ),
...
export function createPlanFormatValidatorHook(_ctx: PluginInput) {
  return {
    "tool.execute.after": async (
      ...
      log(
        `[plan-format-validator] Plan ${filePath}: ${parsedCount}/${formatStats.rawCount} tasks parsed`,
        {
