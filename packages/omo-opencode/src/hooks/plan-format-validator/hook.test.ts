...
type ToolExecuteAfter = NonNullable<
  PlanFormatValidatorHook["tool.execute.after"]
>;
...
function createFixture(content: string, options: FixtureOptions = {}) {
  ...
  const hook = createPlanFormatValidatorHook(
    unsafeTestValue<PluginInput>({ directory }),
  );
