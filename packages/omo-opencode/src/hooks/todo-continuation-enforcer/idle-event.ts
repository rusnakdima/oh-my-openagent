...
export async function handleSessionIdle(args: {
  ...
  if (state.allTodosCompletedAt) {
    log(`[${HOOK_NAME}] Skipped: all todos were already completed`, {
      sessionID,
      allTodosCompletedAt: state.allTodosCompletedAt,
    });
    return;
  }
  ...
  if (handedBackSyncSessions.has(sessionID)) {
    log(`[${HOOK_NAME}] Skipped: sync subagent already handed back to parent`, {
      sessionID,
    });
    return;
  }
  ...
  if (state.tokenLimitDetected) {
    log(
      `[${HOOK_NAME}] Skipped: token limit error detected, retry would worsen context overflow`,
      { sessionID },
    );
    return;
  }
  ...
  if (state.unrecoverableErrorDetected) {
    log(
      `[${HOOK_NAME}] Skipped: non-retryable request error detected, re-injecting would rebuild the same request`,
      { sessionID },
    );
    return;
  }
  ...
  if (state.abortDetectedAt) {
    ...
    if (timeSinceAbort < ABORT_WINDOW_MS) {
      log(
        `[${HOOK_NAME}] Skipped: abort detected via event ${timeSinceAbort}ms ago`,
        { sessionID },
      );
  ...
  try {
    ...
    prefetchedMessages = normalizeSDKResponse(
      messagesResp,
      [] as MessageWithInfo[],
    );
    if (isLastAssistantMessageAborted(prefetchedMessages)) {
      log(
        `[${HOOK_NAME}] Skipped: last assistant message was aborted (API fallback)`,
        { sessionID },
      );
      return;
    }
    if (hasUnansweredQuestion(prefetchedMessages)) {
      log(`[${HOOK_NAME}] Skipped: pending question awaiting user response`, {
        sessionID,
      });
      return;
    }
    if (latestAssistantTurnBlocksInternalPrompt(prefetchedMessages)) {
      log(`[${HOOK_NAME}] Skipped: pending internal continuation response`, {
        sessionID,
      });
      return;
    }
    ...
    const loggedError = error instanceof Error
      ...
      : String(error);
    log(`[${HOOK_NAME}] Messages fetch failed, skipping continuation`, {
      sessionID,
      error: loggedError,
    });
    return;
  ...
  const effectiveCooldown = CONTINUATION_COOLDOWN_MS *
    2 ** Math.min(state.consecutiveFailures, 5);
  if (
    state.lastInjectedAt &&
    Date.now() - state.lastInjectedAt < effectiveCooldown
  ) {
  ...
  try {
    const messageInfoResult = await resolveLatestMessageInfo(
      ctx,
      sessionID,
      prefetchedMessages,
    );
    ...
    const loggedError = error instanceof Error
      ? { name: error.name, message: error.message }
      : String(error);
    log(`[${HOOK_NAME}] Failed to fetch messages for agent check`, {
      sessionID,
      error: loggedError,
    });
  }
  ...
  if (latestMessageWasCompaction) {
    log(`[${HOOK_NAME}] Skipped: latest message is a compaction marker`, {
      sessionID,
    });
    return;
  }
  ...
  if (
    resolvedAgentName &&
    skipAgents.some((s) =>
      getAgentConfigKey(s) === getAgentConfigKey(resolvedAgentName)
    )
  ) {
  ...
  if (
    (compactionGuardActive || encounteredCompaction) && !resolvedInfo?.agent
  ) {
    log(
      `[${HOOK_NAME}] Skipped: compaction occurred but no agent info resolved`,
      { sessionID },
    );
    return;
  }
  if (compactionGuardActive) {
    log(
      `[${HOOK_NAME}] Skipped: compaction guard still armed for current epoch`,
      {
        sessionID,
        observedCompactionEpoch,
        currentCompactionEpoch: state.recentCompactionEpoch,
