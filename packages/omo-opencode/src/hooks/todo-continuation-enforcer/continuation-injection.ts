...
export async function injectContinuation(args: {
  ...
  if (handedBackSyncSessions.has(sessionID)) {
    log(
      `[${HOOK_NAME}] Skipped injection: sync subagent already handed back to parent`,
      { sessionID },
    );
    return;
  }
  ...
  const promptAgent =
    registeredAgentName !== undefined && registeredAgentName !== agentConfigKey
      ? registeredAgentName
  ...
  if (
    promptAgent &&
    skipAgents.some((s) =>
      getAgentConfigKey(s) === getAgentConfigKey(promptAgent)
    )
  ) {
  ...
  if (!promptAgent) {
    ...
    if (
      compactionState && isCompactionGuardActive(compactionState, Date.now())
    ) {
  ...
  const todoList = incompleteTodos.map((todo) =>
...
${todoList}`;
  ...
  if (hasBackgroundWorkBeforeDispatch) {
    log(
      `[${HOOK_NAME}] Skipped injection: background tasks running before prompt`,
      { sessionID },
    );
    return;
  }
  ...
  if (injectionState?.continuationBlockReason) {
    log(
      `[${HOOK_NAME}] Skipped injection: continuation paused at turn boundary`,
      {
        sessionID,
  ...
  try {
    ...
    if (!isInternalPromptDispatchAccepted(promptResult)) {
      log(`[${HOOK_NAME}] Injection skipped by promptAsync gate`, {
        sessionID,
    ...
    log(`[${HOOK_NAME}] Injection successful`, {
      sessionID,
      status: promptResult.status,
    });
    if (injectionState) {
    ...
    if (injectionState) {
      ...
      if (isTokenLimitError(errorObj)) {
        ...
        log(
          `[${HOOK_NAME}] Non-retryable request error detected during injection, stopping continuation`,
          { sessionID },
        );
      }
    }
