  ...
  test(
    ...
    async () => {
      ...
      const scheduledTimeouts: Array<
        { delay: number; callback: TimerCallback; args: unknown[] }
      > = [];
      const fakeSetTimeout = (
        ...
        scheduledTimeouts.push({
          delay: typeof delay === "number" && Number.isFinite(delay)
            ? delay
            : 0,
          callback,
          args: callbackArgs,
      ...
      Reflect.set(
        globalThis,
        "setInterval",
        () => (scheduledTimeouts.length + 1) as FakeTimerID,
      );
      ...
      try {
        ...
        const countdownTimer = scheduledTimeouts.find((timer) =>
          timer.delay === 2000
        );
