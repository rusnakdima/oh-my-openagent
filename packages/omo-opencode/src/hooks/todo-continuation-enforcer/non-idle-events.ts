...
function hasInternalSystemDirective(
  parts: InternalInitiatorTextPartLike[] | undefined,
): boolean {
  return (parts ?? []).some(
    (part) =>
      part.type === "text" &&
...
function markContinuationResponseObserved(
...
function pauseForGenuineUserInterruption(args: {
  ...
  log(`[${HOOK_NAME}] Paused continuation after genuine user interruption`, {
    sessionID,
  });
}
    ...
    if (role === "user") {
      ...
      if (isSyntheticOrInternalOnlyTextParts(parts)) {
        ...
        if (state?.countdownStartedAt && hasInternalSystemDirective(parts)) {
          sessionStateStore.cancelCountdown(sessionID);
          log(
            `[${HOOK_NAME}] Cancelled countdown for internal continuation message`,
            { sessionID },
          );
        }
        log(`[${HOOK_NAME}] Ignoring synthetic/internal user message event`, {
          sessionID,
        });
        return;
      }
      ...
      const shouldDeferClassification = state !== undefined &&
        (hasAcceptedContinuationLifecycle(state) ||
          state.unrecoverableErrorDetected === true);
      if (
        parts === undefined && state && shouldDeferClassification && messageID
      ) {
        state.pendingUserMessageID = messageID;
        log(
          `[${HOOK_NAME}] Deferred user interruption classification until message part`,
          {
            sessionID,
            messageID,
          },
      ...
      if (state) {
        ...
        if (part?.messageID && part.messageID === state.pendingUserMessageID) {
          ...
          if (isSyntheticOrInternalOnlyTextParts([part])) {
            log(
              `[${HOOK_NAME}] Ignoring synthetic/internal split user message`,
              {
