...
export interface WorktreeIsolationDeps {
   ...
   * Defaults to the real resolver.
   */
  containsGitRedirectTo?: (command: string, mainCheckoutPath: string) => boolean
}

const defaultDeps: WorktreeIsolationDeps = {}

export function createWorktreeIsolationHook(deps: WorktreeIsolationDeps = defaultDeps): Hooks {
  const {
    sessionDirectory = process.cwd(),
    getWorkForSession: getWorkForSessionFn = getWorkForSession,
    resolveMainRepoRootSync: resolveMainRepoRootSyncFn = resolveMainRepoRootSync,
  ...
  return {
    "tool.execute.before": async (
      ...
      const worktreePath = getWorktreePathForSession(sessionID, sessionDirectory, getWorkForSessionFn)
      ...
      if (BLOCKED_CWD_TOOLS.has(toolLower)) {
        ...
        if (command && mainRepoRoot && containsGitRedirectToFn(command, mainRepoRoot)) {
...
function getWorktreePathForSession(
  sessionID: string,
  cwd: string,
  getWorkForSessionFn: (cwd: string, sessionID: string) => { worktree_path?: string } | null,
): string | null {
  try {
 ...
 */
function resolveWriteTargetPath(input: unknown, output: unknown): string | null {
