...
describe("worktree-isolation resolver", () => {
  ...
  describe("containsGitRedirectTo", () => {
    ...
    it("#given git -C redirect to main repo #when containsGitRedirectTo #then return true", () => {
      expect(containsGitRedirectTo("git -C /home/user/main-repo status", mainRepo)).toBe(true)
      expect(containsGitRedirectTo("git -C /home/user/main-repo log --oneline", mainRepo)).toBe(true)
    })

    it("#given git with -c var -C redirect #when containsGitRedirectTo #then return true", () => {
      expect(
        containsGitRedirectTo("git -c user.name=test -C /home/user/main-repo status", mainRepo),
      ).toBe(true)
    })

    it("#given --git-dir=<path> redirect #when containsGitRedirectTo #then return true", () => {
      expect(containsGitRedirectTo("git --git-dir=/home/user/main-repo/.git status", mainRepo)).toBe(true)
      expect(containsGitRedirectTo("git --git-dir /home/user/main-repo/.git log", mainRepo)).toBe(true)
    })

    it("#given --work-tree=<path> redirect #when containsGitRedirectTo #then return true", () => {
      expect(containsGitRedirectTo("git --work-tree=/home/user/main-repo status", mainRepo)).toBe(true)
      expect(containsGitRedirectTo("git --work-tree /home/user/main-repo diff", mainRepo)).toBe(true)
    })

    it("#given GIT_DIR=<path> env var #when containsGitRedirectTo #then return true", () => {
      expect(containsGitRedirectTo("GIT_DIR=/home/user/main-repo/.git git status", mainRepo)).toBe(true)
      expect(
        containsGitRedirectTo("git status; export GIT_DIR=/home/user/main-repo/.git; git log", mainRepo),
      ).toBe(true)
    })

    it("#given GIT_WORK_TREE=<path> env var #when containsGitRedirectTo #then return true", () => {
      expect(
        containsGitRedirectTo("GIT_WORK_TREE=/home/user/main-repo git status", mainRepo),
      ).toBe(true)
    })

    it("#given cd into main checkout #when containsGitRedirectTo #then return true", () => {
      expect(containsGitRedirectTo("cd /home/user/main-repo && git status", mainRepo)).toBe(true)
      expect(containsGitRedirectTo("cd /home/user/main-repo; ls", mainRepo)).toBe(true)
    })

    it("#given cd into worktree (not main) #when containsGitRedirectTo #then return false", () => {
      expect(
        containsGitRedirectTo("cd /home/user/worktrees/feature && git status", mainRepo),
      ).toBe(false)
    })
