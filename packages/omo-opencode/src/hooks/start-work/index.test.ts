...
<user-request>${userRequest}</user-request>`
  ...
  describe("chat.message handler", () => {
    ...
    test("should inject resume info when existing boulder state found", async () => {
      ...
      expect(readBoulderState(testDir)?.session_ids).toContain(
    ...
    test("should prefer the plan most recently referenced in the current session", async () => {
      ...
      const hook = createStartWorkHook(
        unsafeTestValue<Parameters<typeof createStartWorkHook>[0]>({
          directory: testDir,
          client: {
            session: {
              messages: async () => ({
    ...
    test("should wrap multiple plans message in system-reminder tag", async () => {
      ...
      // then - should use system-reminder tag format and ask for a decision without writing state
  ...
  describe("session agent management", () => {
    ...
    test("#given start-work hands the session to Atlas but background work is still running #when that work finishes #then Atlas resumes via retry for the same session", async () => {
      ...
      globalThis.setTimeout = unsafeTestValue<typeof setTimeout>(
        (callback: Function, delay?: number, ...args: unknown[]) => {
      ...
      globalThis.clearTimeout = unsafeTestValue<typeof clearTimeout>(
  ...
  describe("worktree support", () => {
    ...
    test("should inject worktree path when --worktree flag is valid", async () => {
      ...
      expect(readBoulderState(testDir)?.worktree_path).toBe(
        "/validated/worktree",
      );
    });
    ...
    test("should store worktree_path in boulder when --worktree is valid", async () => {
      ...
      const output = {
        parts: [{
          type: "text",
          text: createStartWorkPrompt({ userRequest: "--worktree /valid/wt" }),
        }],
      };
    ...
    test("should show worktree plan progress and path when the mirrored plan exists", async () => {
      ...
      const mainPlanPath = join(
        ...
        ".omo",
        "plans",
        "resume-worktree-plan.md",
      );
      const worktreeDir = join(
        testDir,
        "..",
        `resume-worktree-${randomUUID()}`,
      );
      ...
      try {
        // when
        await hook["chat.message"](
          { sessionID: "session-worktree-progress" },
          output,
        );
  ...
  describe("retry-path idempotence (#4480)", () => {
    ...
    test("should not append contextInfo twice when hook fires more than once", async () => {
      ...
      const firstMarkerCount =
        (firstRunText.match(/<!-- omo-start-work-context -->/g) ?? []).length;
      ...
      const secondMarkerCount =
        (secondRunText.match(/<!-- omo-start-work-context -->/g) ?? []).length;
