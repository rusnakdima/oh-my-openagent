...
export function buildMultipleActiveWorksContext(params: {
  ...
  const optionList = resumeOptions
    .map((option, index) => {
      ...
      return `${
        index + 1
      }. ${option.plan_name} - ${option.progress.completed}/${option.progress.total} (${percent}%) - elapsed: ${
        formatElapsedHuman(option.elapsed_ms)
      } - worktree: ${
        option.worktree_path ?? "current directory"
      } - sessions: ${option.session_count}`;
    })
    .join("\n");
...
Looking for new plans...`;
  ...
  const shouldRewriteState = existingState.agent !== activeAgent ||
