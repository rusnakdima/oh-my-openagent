...
<session-context></session-context>`;
  ...
  test("#given raw chat session id #when processing start-work template #then boulder stores opencode-prefixed id", async () => {
    // given
    writeFileSync(
      join(testDir, ".omo", "plans", "work.md"),
      "# Work\n- [ ] First task\n",
    );
...
${args}
  ...
  test.each([
    ...
    "#given $flag #when processing #then the hook injects exactly that PR delivery branch",
    async ({ flag, flags }) => {
      ...
      await hook["chat.message"](
        { sessionID: `delivery-${flag.slice(2)}` },
        output,
      );
  ...
  test("#given --make-pr flag #when parsing plan name #then flag does not leak into boulder plan selection", async () => {
    ...
    const output = {
      parts: [{
        type: "text",
        text: createStartWorkPromptWithArgs("work --make-pr"),
      }],
    };
