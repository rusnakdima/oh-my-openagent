...
describe("ensureNotepadScaffold", () => {
  ...
  test("#given already-scaffolded dir #when called again #then all 4 files unchanged (idempotent)", () => {
    ...
    const notepadDir = join(directory, ".omo", "notepads", "alpha");
    const learningsBefore = readFileSync(
      join(notepadDir, "learnings.md"),
      "utf8",
    );
    const decisionsBefore = readFileSync(
      join(notepadDir, "decisions.md"),
      "utf8",
    );
    const issuesBefore = readFileSync(join(notepadDir, "issues.md"), "utf8");
    const problemsBefore = readFileSync(
      join(notepadDir, "problems.md"),
      "utf8",
    );
    ...
    // then
    const learningsAfter = readFileSync(
      join(notepadDir, "learnings.md"),
      "utf8",
    );
    const decisionsAfter = readFileSync(
      join(notepadDir, "decisions.md"),
      "utf8",
    );
  ...
  test("#given nested missing parent dirs #when called #then creates full .omo/notepads/alpha path", () => {
    ...
    expect(
      existsSync(join(directory, ".omo", "notepads", "alpha", "learnings.md")),
    ).toBe(true);
    expect(
      existsSync(join(directory, ".omo", "notepads", "alpha", "decisions.md")),
    ).toBe(true);
    expect(
      existsSync(join(directory, ".omo", "notepads", "alpha", "issues.md")),
    ).toBe(true);
    expect(
      existsSync(join(directory, ".omo", "notepads", "alpha", "problems.md")),
    ).toBe(true);
  });
});
