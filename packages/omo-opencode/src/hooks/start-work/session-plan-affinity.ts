...
const PLAN_PATH_PATTERN =
  /[A-Za-z0-9_./\\:~-]*\.(?:sisyphus|omo)[\\/]plans[\\/][A-Za-z0-9._/\\~-]+\.md/gi;
...
function extractPlanPathsFromValue(
  directory: string,
  value: unknown,
): string[] {
  ...
  if (value && typeof value === "object") {
    return Object.values(value).flatMap((item) =>
      extractPlanPathsFromValue(directory, item)
    );
  }
...
function extractPlanPathsFromInput(
  directory: string,
...
export async function findRecentSessionPlanPath(input: {
  ...
  const availablePlansByKey = new Map(
    input.availablePlans.map((
      planPath,
    ) => [normalizePlanPathKey(planPath), planPath]),
  );
    ...
    for (
      ...
      for (let partIndex = parts.length - 1; partIndex >= 0; partIndex -= 1) {
        ...
        const matchedPlan = planCandidates
          .map((planPath) =>
            availablePlansByKey.get(normalizePlanPathKey(planPath))
          )
