...
export function buildStartWorkContextInfo(params: {
  ...
  if (
    !explicitPlanName && resumeOptions.length === 0 &&
    getActiveWorks(directory).length === 0
  ) {
  ...
  if (
    shouldDiscoverPlans({ existingState, explicitPlanName, preferredPlanPath })
  ) {
    return buildPlanDiscoveryContext({
      contextInfo,
      sessionId,
...
function buildSelectedContextInfo(params: {
  ...
  if (existingState && !getPlanProgress(existingState.active_plan).isComplete) {
    log(
      `[${HOOK_NAME}] Ignoring unrelated active boulder state for this session`,
      {
        sessionID: sessionId,
