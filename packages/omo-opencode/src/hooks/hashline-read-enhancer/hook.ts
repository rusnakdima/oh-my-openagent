...
function isTextFile(output: string): boolean {
  ...
  return COLON_READ_LINE_PATTERN.test(firstLine) ||
...
function transformOutput(output: string): string {
  ...
  const fileStart = lines.findIndex((line) =>
    line === FILE_OPEN_TAG || line.startsWith(FILE_OPEN_TAG)
  );
...
function extractLineCount(metadata: unknown): number | undefined {
  ...
  const objectMeta = metadata as Record<string, unknown>;
  const candidates = [
    objectMeta.lineCount,
    objectMeta.linesWritten,
    objectMeta.lines,
  ];
  for (const candidate of candidates) {
    if (
...
async function appendWriteHashlineOutput(
  ...
  if (metadataLineCount !== undefined) {
    output.output =
      `${WRITE_SUCCESS_MARKER} ${metadataLineCount} lines written.`;
    return;
  }
  ...
  return {
    "tool.execute.after": async (
      ...
      if (!isReadTool(input.tool)) {
        if (
          isWriteTool(input.tool) && typeof output.output === "string" &&
