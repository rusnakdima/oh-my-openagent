...
describe("category-skill-reminder hook", () => {
  ...
  test.each([
    {
      delegationTool: "task",
      order: "before",
      tools: ["task", "edit", "edit", "edit"],
    },
    {
      delegationTool: "task",
      order: "after",
      tools: ["edit", "edit", "edit", "task"],
    },
    {
      delegationTool: "call_omo_agent",
      order: "before",
      tools: ["call_omo_agent", "edit", "edit", "edit"],
    },
    {
      delegationTool: "call_omo_agent",
      order: "after",
      tools: ["edit", "edit", "edit", "call_omo_agent"],
    },
  ])(
  ...
  test("#given a queued reminder and no real user text #when messages transform retries #then pending is retained", async () => {
    ...
    await transformMessages(hook, [
      createUserTurn(sessionID, "internal", { synthetic: true }),
    ]);
    const realMessages = [
      createUserTurn(sessionID, "real user text", { id: "msg_real" }),
    ];
  ...
  test("#given multiple real and synthetic user texts #when a reminder is pending #then exactly one is inserted before the latest real text", async () => {
    ...
    const latestTurn = createUserTurn(
      sessionID,
    ...
    const messages = [
      ...
      latestTurn,
      createUserTurn(sessionID, "synthetic tail", {
        id: "msg_tail",
        synthetic: true,
      }),
    ];
  ...
  test("#given a reminder is inserted #when transforms and tools repeat without assistant completion #then the same bytes appear only once", async () => {
    ...
    updateSessionAgent(sessionID, "Sisyphus");
    const firstOutput = await useTools({
      hook,
      sessionID,
      tools: ["edit", "edit", "edit"],
    });
    ...
    const secondOutput = await useTools({
      hook,
      sessionID,
      tools: ["edit", "edit", "edit"],
    });
    const secondMessages = [
      createUserTurn(sessionID, "second continuation", { id: "msg_second" }),
    ];
    ...
    expect(JSON.stringify(findReminderParts(firstMessages)[0])).toBe(
      injectedBytes,
    );
  ...
  test("#given a reminder is pending #when the session is deleted #then pending state and counts are cleared", async () => {
    ...
    const freshMessages = [
      createUserTurn(sessionID, "after fresh threshold", { id: "msg_fresh" }),
    ];
  ...
  test("#given mixed-case tool names #when the threshold is reached #then counting and delegation remain case-insensitive", async () => {
    ...
    const counted = await useTools({
      hook,
      sessionID: "case-count",
      tools: ["EDIT", "Edit", "edit"],
    });
    const delegated = await useTools({
      hook,
      sessionID: "case-delegate",
      tools: ["TASK", "edit", "edit", "edit"],
    });
    const countedMessages = [createUserTurn("case-count", "continue counted")];
    const delegatedMessages = [
      createUserTurn("case-delegate", "continue delegated"),
    ];
  ...
  test.each([
    ...
    },
    {
      ...
      expected: [
        "**⚡ YOUR SKILLS (PRIORITY)**",
        "react-19",
        'load_skills=["react-19"',
      ],
    },
    {
      name: "no skills",
    ...
    "#given $name #when the reminder fires #then it formats the available skills",
    async ({ skills, expected }) => {
      ...
      if (reminder?.type !== "text") {
        throw new Error("expected reminder text part");
      }
