...
export async function runPreemptiveCompactionIfNeeded(args: {
  ...
  const totalInputTokens = (cached.tokens.input ?? 0) +
  ...
  try {
    ...
    ctx.client.tui.showToast({
      body: {
        ...
        message: `Context window is above ${
          Math.round(PREEMPTIVE_COMPACTION_THRESHOLD * 100)
        }% and auto-compaction could not run. The session may grow large. Error: ${errorMessage}`,
        variant: "warning",
        duration: 10000,
      },
