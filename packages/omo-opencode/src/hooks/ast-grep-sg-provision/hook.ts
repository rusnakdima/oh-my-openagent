...
async function runProvision(
  targetDir: string,
  deps: AstGrepSgProvisionDeps,
): Promise<void> {
  try {
    const binaryPath = await deps.provisionSgBinary({
      arch: deps.arch,
      platform: deps.platform,
      targetDir,
    });
    deps.log("[ast-grep-sg-provision] Provisioning finished", {
      binaryPath,
      targetDir,
    });
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    deps.log("[ast-grep-sg-provision] Provisioning failed", {
      error: message,
      targetDir,
    });
  }
...
export function createAstGrepSgProvisionHook(
  depsOverride: Partial<AstGrepSgProvisionDeps> = {},
) {
  const deps: AstGrepSgProvisionDeps = { ...defaultDeps, ...depsOverride };
  const platform = deps.platform ?? process.platform;
  const targetDir = astGrepRuntimeDir(
    join(deps.homeDir(), ".omo"),
    platform,
  ...
  return {
    event(input: AstGrepSgProvisionEventInput): void {
      try {
        ...
        const message = error instanceof Error ? error.message : String(error);
        deps.log("[ast-grep-sg-provision] Event handler failed", {
          error: message,
        });
      }
    },
