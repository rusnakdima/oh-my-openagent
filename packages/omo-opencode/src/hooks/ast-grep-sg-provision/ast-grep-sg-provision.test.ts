...
function createDeps(
  events: string[],
  overrides: Partial<AstGrepSgProvisionDeps> = {},
): AstGrepSgProvisionDeps {
  return {
    findSgBinary: () => null,
    homeDir: () => "/home/test",
...
describe("createAstGrepSgProvisionHook", () => {
  ...
  test("#given sg is missing #when session starts #then provisioning is scheduled for the OMO runtime directory", async () => {
    // given
    const events: string[] = [];
    const hook = createAstGrepSgProvisionHook(
      createDeps(events, { arch: "x64", platform: "linux" }),
    );
    ...
    expect(events).toContain(
      `provision:${
        join("/home/test", ".omo", "runtime", "ast-grep", "linux-x64")
      }`,
    );
  });
  ...
  test("#given provisioning fails #when background work runs #then the session hook logs and never throws", async () => {
    ...
    // when
    expect(() => hook.event({ event: { type: "session.created" } })).not
      .toThrow();
    await Promise.resolve();
