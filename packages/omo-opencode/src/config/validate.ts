...
import { type OhMyOpenCodeConfig, OhMyOpenCodeConfigSchema } from "./schema";
...
function schemaMessages(
  ...
  const unknownKeyMessages = findUnknownKeyPaths(
    OhMyOpenCodeConfigSchema,
    rawConfig,
  )
    .map((path) =>
      `${shortPath(configPath)}: Unknown config key: ${formatIssuePath(path)}`
    );
...
function parseConfig(
  ...
  for (const [key, value] of Object.entries(rawConfig)) {
    ...
    if (!result.success) {
      log(
        `[config] Skipping invalid config section "${key}": ${
          result.error.issues.map((i) => i.message).join(", ")
        }`,
      );
      continue;
    }
    const section = Object.entries(result.data).find(([parsedKey]) =>
      parsedKey === key
    );
    if (section !== undefined) {
      config = Object.assign(config, Object.fromEntries([section]));
    }
  }
  return config;
}

function parseConfigView(
  path: string,
  rawConfig: Record<string, unknown>,
): LoadedConfigView {
  return {
...
function protectUserFields(
  ...
  const userPlaywrightMcpArgs = userConfig?.browser_automation_engine
    ?.playwright_mcp_args;
  const browserAutomationEngine = config.browser_automation_engine === undefined
    ? undefined
    : (() => {
      const { playwright_mcp_args: _projectPlaywrightMcpArgs, ...browser } =
  ...
  return {
    ...
    ...(browserAutomationEngine === undefined ? {} : {
      browser_automation_engine: userPlaywrightMcpArgs === undefined
        ...
        : {
          ...browserAutomationEngine,
          playwright_mcp_args: userPlaywrightMcpArgs,
        },
    }),
  };
}

function materializeAgentModelChains(
  config: OhMyOpenCodeConfig,
): OhMyOpenCodeConfig {
...
function migrateRalphLoopConfig(
  config: OhMyOpenCodeConfig,
): OhMyOpenCodeConfig {
  ...
  const enabled = typeof legacy.enabled === "boolean"
    ? legacy.enabled
    : undefined;
  const defaultMaxIterations = typeof legacy.default_max_iterations === "number"
    ? legacy.default_max_iterations
    : undefined;
  ...
  log(
    "[config] ralph_loop is deprecated and will be removed in a future release. Use goal instead.",
  );
  ...
  return {
    ...
    goal: {
      ...
      default_max_iterations: existingGoal?.default_max_iterations ??
...
export function validatePluginConfig(
  ...
  const views = chain.views.map((view) =>
    parseConfigView(view.path, view.config)
  );
  const chainMessages = chain.diagnostics.map((diagnostic) =>
    `${shortPath(diagnostic.path)}: ${diagnostic.message}`
  );
  const messages = [
    ...chainMessages,
    ...views.flatMap((view) => view.messages),
  ];
  ...
  return {
    ...
    messages,
    path: chainMessages.length > 0
      ? chain.diagnostics[0]?.path ?? null
      : firstFailingView?.path ?? firstView?.path ?? null,
