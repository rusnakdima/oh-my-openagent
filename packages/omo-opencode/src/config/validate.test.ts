...
describe("validatePluginConfig", () => {
  ...
  it("#given an invalid ancestor omo harness block #when validating from a child #then reports the schema path", () => {
    withOmoConfig("ancestor-invalid", (fixture) => {
      ...
      writeProjectConfig(fixture, {
        "[opencode]": { agents: { sisyphus: { model: 123 } } },
      });
      ...
      expect(
        result.messages.some((message) =>
          message.includes("agents.sisyphus.model")
        ),
      ).toBe(true);
    });
  });
  ...
  it("#given user and project omo views #when validating #then preserves plugin merge and disabled-set semantics", () => {
    withOmoConfig("merge", (fixture) => {
      ...
      expect(result.config.categories?.deep).toMatchObject({
        model: "user/deep",
        temperature: 0.2,
        prompt_append: "project appendix",
      });
      expect(result.config.claude_code).toMatchObject({
        mcp: true,
        commands: true,
      });
    ...
    withOmoConfig("protected-user-base", (fixture) => {
      ...
      writeProjectConfig(fixture, {
        "[opencode]": {
          ...
          browser_automation_engine: {
            provider: "playwright",
            playwright_mcp_args: ["--project-base"],
          },
        },
        profiles: {
          kimi: {
            "[opencode]": {
              ...
              browser_automation_engine: {
                provider: "playwright",
                playwright_mcp_args: ["--project-profile"],
              },
            },
          },
        },
      ...
      expect(result.config.browser_automation_engine?.playwright_mcp_args)
        .toEqual(["--user-base"]);
    });
  });
        ...
        profiles: {
          kimi: {
            "[opencode]": {
              ...
              browser_automation_engine: {
                provider: "playwright",
                playwright_mcp_args: ["--user-profile"],
              },
            },
          },
        },
  ...
  it("#given unknown opencode keys #when validating #then reports and strips every unknown path without losing known values", () => {
    withOmoConfig("unknown-keys", (fixture) => {
      ...
      expect(
        result.messages.some((message) =>
          message.includes("Unknown config key: unknown_switch")
        ),
      ).toBe(true);
      expect(
        result.messages.some((message) =>
          message.includes(
            "Unknown config key: tui.sidebar.unknown_sidebar_switch",
          )
        ),
      ).toBe(true);
      ...
      expect(
        Object.hasOwn(
          result.config.tui?.sidebar ?? {},
          "unknown_sidebar_switch",
        ),
      ).toBe(false);
      expect(result.config.tui?.sidebar.enabled).toBe(false);
    });
  ...
  it("#given an OCX profile overlay #when OCX_PROFILE changes #then flips the resolved sisyphus model", () => {
    withOmoConfig("ocx-profile", (fixture) => {
      writeUserConfig(fixture, {
        ...
        profiles: {
          kimi: {
            "[opencode]": { agents: { sisyphus: { model: "kimi/model" } } },
          },
        },
      });
