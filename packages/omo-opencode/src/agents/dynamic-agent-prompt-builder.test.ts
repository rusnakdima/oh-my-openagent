...
describe("buildCategorySkillsDelegationGuide", () => {
  ...
  it("propagates category metadata and skill source branches", () => {
    const categories: AvailableCategory[] = [
      {
        name: "SENTINEL_CATEGORY_A",
        description: "SENTINEL_CATEGORY_DESCRIPTION_A",
      },
      {
        name: "SENTINEL_CATEGORY_B",
        description: "SENTINEL_CATEGORY_DESCRIPTION_B",
      },
    ];
    const skills: AvailableSkill[] = [
      {
        name: "SENTINEL_PLUGIN_SKILL",
        description: "unused",
        location: "plugin",
      },
      { name: "SENTINEL_USER_SKILL", description: "unused", location: "user" },
      {
        name: "SENTINEL_PROJECT_SKILL",
        description: "unused",
        location: "project",
      },
    ];
...
describe("buildUltraworkSection", () => {
  it("propagates category, skill, and agent inputs through their rendering branches", () => {
    const categories: AvailableCategory[] = [
      {
        name: "SENTINEL_CATEGORY",
        description: "SENTINEL_CATEGORY_DESCRIPTION",
      },
    ];
    const skills: AvailableSkill[] = [
      {
        name: "SENTINEL_PLUGIN_SKILL",
        description: "SENTINEL_PLUGIN_DESCRIPTION",
        location: "plugin",
      },
      {
        name: "SENTINEL_USER_SKILL",
        description: "SENTINEL_USER_DESCRIPTION",
        location: "user",
      },
    ];
...
describe("buildParallelDelegationSection", () => {
  const deepCategory: AvailableCategory = {
    name: "deep",
    description: "SENTINEL_DEEP",
  };
  const highCategory: AvailableCategory = {
    name: "unspecified-high",
    description: "SENTINEL_HIGH",
  };
  const otherCategory: AvailableCategory = {
    name: "quick",
    description: "SENTINEL_QUICK",
  };

  it("enables only the non-Claude delegation-category branches", () => {
    const deepResult = buildParallelDelegationSection("google/gemini-3.1-pro", [
      deepCategory,
    ]);
    const highResult = buildParallelDelegationSection("openai/gpt-5.4", [
      highCategory,
    ]);

    const claudeResult = buildParallelDelegationSection(
      "anthropic/CLAUDE-opus-4-7",
      [deepCategory],
    );
    const unrelatedCategoryResult = buildParallelDelegationSection(
      "openai/gpt-5.4",
      [otherCategory],
    );
...
describe("buildNonClaudePlannerSection", () => {
  it("selects the planner branch from the model family", () => {
    ...
    const claudeResult = buildNonClaudePlannerSection(
