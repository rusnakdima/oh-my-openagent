...
describe("getAtlasPromptSource routes each model family to its dedicated variant", () => {
  ...
  test("Gemini models route to gemini", () => {
    ...
    expect(getAtlasPromptSource("google-vertex/gemini-2.5-flash")).toBe(
      "gemini",
    );
    expect(getAtlasPromptSource("github-copilot/gemini-2.0-pro")).toBe(
      "gemini",
    );
  });
  ...
  test("Claude Opus 4.7 routes to opus-4-7", () => {
    ...
    expect(getAtlasPromptSource("github-copilot/claude-opus-4.7")).toBe(
      "opus-4-7",
    );
  });
