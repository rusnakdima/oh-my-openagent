...
10 files. Sisyphus-Junior is a focused task executor spawned by `delegate-task`
when category routing requires it. Runs in subagent mode with its own fallback
chain. Does not delegate further; executes directly.
...
- Fallback chain: claude-sonnet-5 -> kimi-k3 -> gpt-5.6-sol medium -> minimax-m3
