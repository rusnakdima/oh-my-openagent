...
export function createSisyphusJuniorAgentWithOverrides(
  ...
  const model = overrideModel ?? defaultModel ?? systemDefaultModel ??
    SISYPHUS_JUNIOR_DEFAULTS.model;
  const temperature = override?.temperature ??
    SISYPHUS_JUNIOR_DEFAULTS.temperature;
  ...
  const migratedOverride = override
    ...
    : undefined;
  const userPermission = (migratedOverride?.permission ?? {}) as Record<
    string,
    PermissionValue
  >;
  ...
  if (isGptModel(model)) {
    return {
      ...base,
      reasoningEffort: override?.reasoningEffort ?? "medium",
    } as AgentConfig;
  }
