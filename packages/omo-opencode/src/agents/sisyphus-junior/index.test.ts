...
describe("createSisyphusJuniorAgentWithOverrides", () => {
  ...
  describe("constrained fields", () => {
    ...
    test("prompt override is ignored in favor of the routed prompt builder", () => {
      ...
      expect(result.prompt).toBe(
        buildSisyphusJuniorPrompt(systemDefaultModel, false),
      );
    });
  });
  ...
  describe("tool safety (task blocked, call_omo_agent allowed)", () => {
    ...
    test("tools override migrates boolean tools to permission (issue #5193)", () => {
      // given
      const override = {
        tools: { grep: false, write: false, read: true } as Record<
          string,
          boolean
        >,
      };
  ...
  describe("prompt composition", () => {
    test("no variant force-denies apply_patch", () => {
      ...
      const gptGenericResult = createSisyphusJuniorAgentWithOverrides(
        gptGenericOverride,
      );
      const claudeResult = createSisyphusJuniorAgentWithOverrides(
        claudeOverride,
      );
      ...
      expect(gptGenericResult.permission ?? {}).not.toHaveProperty(
        "apply_patch",
      );
...
describe("buildSisyphusJuniorPrompt", () => {
  ...
  test("routes each model id to its prompt source", () => {
    ...
    // when / then
    expect(cases.map(([model]) => getSisyphusJuniorPromptSource(model)))
      .toEqual(
