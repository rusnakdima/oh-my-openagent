...
Model-specific prompt variants for the Sisyphus main orchestrator. Parent
`sisyphus-agent-factory.ts` routes to the correct variant based on active model.
...
| `grok-4.ts`                | Shared Grok 4.5/4.6 variant: lean, verification-loop-centered, done-contract calibration      |
...
- Contains "gpt-5.5" or "gpt-5.6" -> shared `gpt-5-5.ts` prompt family with
...
- Grok 4.5 / 4.6 (digit-bounded, so grok-4.20 and fast tiers stay out) -> shared
  `grok-4.ts` prompt with model-aware identity
