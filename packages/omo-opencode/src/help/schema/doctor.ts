...
export const DoctorIssueSchema = z
  .object({
    ...
    affects: z.array(z.string()).optional().describe(
      "Components or areas affected",
    ),
    severity: z.enum(["error", "warning"]).describe(
...
export const SystemInfoSchema = z
  .object({
    opencodeVersion: z.string().nullable().describe(
      "Installed OpenCode version",
    ),
    opencodePath: z.string().nullable().describe("Path to OpenCode binary"),
    pluginVersion: z.string().nullable().describe(
      "oh-my-openagent plugin version",
    ),
    loadedVersion: z.string().nullable().describe(
      "Loaded plugin version at runtime",
    ),
