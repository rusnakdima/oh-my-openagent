...
type MockHandlerArgs = ChatMessageHandlerArgs & {
  readonly _appliedSessions: string[];
};
...
describe("createChatMessageHandler - cache warning behavior", () => {
  ...
  test("does not show provider cache warning when provider-models cache exists", async () => {
    ...
    const providerModelsCachePath = join(
      getOmoOpenCodeCacheDir(),
      "provider-models.json",
    );
    mkdirSync(getOmoOpenCodeCacheDir(), { recursive: true });
    writeFileSync(
      providerModelsCachePath,
      JSON.stringify({
        models: {
          openai: [{ id: "gpt-5.4" }],
        },
    ...
    args.ctx = {
      client: {
        tui: {
          showToast: async (
            input: { body: { title: string; message: string } },
          ) => {
...
describe("createChatMessageHandler - /start-work integration", () => {
  ...
  beforeEach(() => {
    ...
    writeFileSync(
      join(testDir, ".omo", "plans", "worker-plan.md"),
      "# Plan\n- [ ] Task 1",
    );
  ...
  test("smoke: resolves quoted human-readable plan names through the full /start-work chat.message path", async () => {
    // given
    writeFileSync(
      join(testDir, ".omo", "plans", "my-feature-plan.md"),
      "# Plan\n- [ ] Task 1",
    );
    updateSessionAgent("test-session", "prometheus");
    const args = createMockHandlerArgs();
