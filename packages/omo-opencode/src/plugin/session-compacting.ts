...
type SessionCompactingHook = NonNullable<
...
function unrefTimer(
  timer: TimerHandleWithOptionalUnref,
  sessionID: string,
): void {
  ...
  try {
    ...
    log(
      "[session-compacting] duplicate autocontinue guard timer unref failed",
      {
        sessionID,
        error: errorText,
      },
...
export function createSessionCompactingHandler(
  ...
  return async (
    ...
    await runCompactionStep(
      "compactionContextInjector.capture",
      input.sessionID,
      async () => {
    ...
    await runCompactionStep(
      "compactionTodoPreserver.capture",
      input.sessionID,
      async () => {
    ...
    await runCompactionStep(
      "compactionContextInjector.inject",
      input.sessionID,
      () => {
...
export function createCompactionAutocontinueHandler(
  ...
  const duplicateGuardMs = options.duplicateGuardMs ??
    DEFAULT_AUTOCONTINUE_DUPLICATE_GUARD_MS;
  ...
  return async (
    ...
    await runCompactionStep(
      "compactionContextInjector.restore",
      input.sessionID,
      async () => {
    ...
    await runCompactionStep(
      "compactionTodoPreserver.restore",
      input.sessionID,
      async () => {
