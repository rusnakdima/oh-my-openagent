...
export function createCommandExecuteBeforeHandler(args: {
  ...
  return async (input, output): Promise<void> => {
    ...
    if (
      ...
      switch (parsed.kind) {
        case "propose": {
          const result = await hooks.openspecSession.propose(
            parsed.specName,
            parsed.description,
          );
        ...
        case "verify": {
          ...
          for (const r of results) {
            const files = r.files
              .map((f) =>
                `  ${f.name}: ${f.exists && f.nonEmpty ? "ok" : "MISSING"}`
              )
              .join("\n");
            ...
            output.parts.push({
              type: "text",
              text:
                `Spec: ${r.specName} | Valid: ${r.valid}\n${files}\n${stats}`,
            });
          }
          break;
        }
        case "apply": {
          const result = await hooks.openspecSession.apply(
            parsed.specName,
            sessionID,
          );
          output.parts.push({ type: "text", text: result.message });
