...
import { clearGlobalTuiModel, setSelectedGlobalModel } from "../../shared/session-model-state"
...
function makeIO(): { input: ChatMessageInput; output: ChatMessageHandlerOutput } {
  return {
...
describe("applyGlobalModelToChatMessage (global model → all OMO agent modes)", () => {
  ...
  beforeEach(() => {
    ...
    for (const name of ["sisyphus", "atlas", "hephaestus", "multimodal-looker", "librarian"]) {
  ...
  it("#given a global pick and registered agent #when chat.message fires #then model is overridden for that mode", () => {
    ...
    applyGlobalModelToChatMessage(input, output, unsafeTestValue<OhMyOpenCodeConfig>({}))

    expect(output.message.model).toEqual({ providerID: "anthropic", modelID: "claude-opus-5" })
  })
  ...
  it("#given a non-OMO agent #when chat.message fires #then model is untouched", () => {
    setSelectedGlobalModel({ providerID: "anthropic", modelID: "claude-opus-5" })
    const { output } = makeIO()
    const input: ChatMessageInput = { sessionID: "ses_global_apply", agent: "some-external-agent" }
  ...
  it("#given vision-required agent and a text-only pick #when chat.message fires #then agent keeps its model", () => {
    ...
    setSelectedGlobalModel({ providerID: "deepseek", modelID: "deepseek-reasoner" })
    const { output } = makeIO()
    const input: ChatMessageInput = { sessionID: "ses_global_apply", agent: "multimodal-looker" }
