...
const ASSISTANT_PREFILL_RECOVERY_TEXT =
...
type MessagesTransformHooks = {
  ...
  contextInjectorMessagesTransform?:
    CreatedHooks["contextInjectorMessagesTransform"];
...
const MESSAGES_TRANSFORM_HOOKS = [
  ...
  {
    key: "contextInjectorMessagesTransform",
    name: "contextInjectorMessagesTransform",
  },
  ...
  { key: "btwContextStrip", name: "btwContextStrip" },
...
function readModelIdentifier(info: unknown): ModelIdentifier | undefined {
  ...
  const providerID = nestedModel
    ? readStringField(nestedModel, "providerID") ??
      readStringField(info, "providerID")
    : readStringField(info, "providerID");
  const modelID = nestedModel
    ? readStringField(nestedModel, "modelID") ??
...
function providerCanExposeUnsupportedAssistantPrefill(
  providerID: string,
  modelID: string,
): boolean {
...
function shouldRepairAssistantPrefillForModel(
  model: ModelIdentifier | undefined,
): boolean {
  if (!model) {
    return false;
  }
  ...
  if (
    !providerCanExposeUnsupportedAssistantPrefill(providerID, model.modelID)
  ) {
    return false;
  }

  const modelID = normalizeAssistantPrefillModelID(model.modelID);
  return ASSISTANT_PREFILL_UNSUPPORTED_MODEL_PREFIXES.some((prefix) =>
...
function hasInternalContinuationTrigger(messages: MessageWithParts[]): boolean {
  return findLastUserTurn(messages)?.parts.some(
...
function createAssistantPrefillRecoveryMessage(
  ...
  const sessionID = getSessionID(lastAssistantMessage) ??
...
function ensureUserTurnAfterAssistantTail(
  ...
  output.messages.push(
    createAssistantPrefillRecoveryMessage(lastMessage, output.messages),
  );
}
...
async function runMessagesTransformHookSafely<I, O>(
  hookName: string,
  handler:
    | ((input: I, output: O) => unknown | Promise<unknown>)
    | null
    | undefined,
  input: I,
  output: O,
...
export function createMessagesTransformHandler(args: {
  ...
  return async (input, output): Promise<void> => {
    for (const hook of MESSAGES_TRANSFORM_HOOKS) {
      const handler = args.hooks[hook.key]
