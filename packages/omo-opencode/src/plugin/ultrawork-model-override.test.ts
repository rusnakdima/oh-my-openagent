...
let resolveUltraworkOverride:
  (typeof import("./ultrawork-model-override"))["resolveUltraworkOverride"];
let detectUltrawork:
  (typeof import("./ultrawork-model-override"))["detectUltrawork"];
let applyUltraworkModelOverrideOnMessage:
  (typeof import("./ultrawork-model-override"))[
    "applyUltraworkModelOverrideOnMessage"
  ];

async function importFreshUltraworkModelOverrideModule(): Promise<
  typeof import("./ultrawork-model-override")
> {
  return import(
    `./ultrawork-model-override?test=${Date.now()}-${Math.random()}`
  );
}
...
describe("resolveUltraworkOverride", () => {
  ...
  test("should return null when agent has no ultrawork config", () => {
    //#given
    const config = unsafeTestValue<
      Parameters<typeof resolveUltraworkOverride>[0]
    >({
  ...
  test("should resolve display name to config key", () => {
    ...
    //#when
    const result = resolveUltraworkOverride(
      config,
...
describe("applyUltraworkModelOverrideOnMessage", () => {
  ...
  function createConfig(
    ...
    return unsafeTestValue<
      Parameters<typeof applyUltraworkModelOverrideOnMessage>[0]
    >({
      agents: {
        [agentName]: { ultrawork },
      },
      ...
      provider: {
        list: async () => ({
          data: {
            all: [{
              id: "anthropic",
              models: { "claude-opus-4-7": { variants: { max: {} } } },
            }],
          },
        }),
      },
      ...
      provider: {
        list: async () => ({
          data: {
            all: [{
              id: "anthropic",
              models: { "claude-haiku-4-5": { variants: { high: {} } } },
            }],
          },
        }),
      },
