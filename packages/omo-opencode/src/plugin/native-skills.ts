...
function getGeneratedClientFromPluginClient(
  client: PluginContext["client"],
): unknown {
  return getObjectProperty(client, "_client");
}

export function getPluginInputNativeSkills(
  ctx: PluginContext,
...
export function createNativeSkills(
  input: {
  ...
  const load = async () => {
    ...
    try {
      ...
      const result = await client.app.skills({ directory: input.directory }, {
        throwOnError: true,
      });
      const parsed = NativeSkillEntriesSchema.safeParse(
        getObjectProperty(result, "data"),
      );
      if (!parsed.success) {
        log("[native-skills] v2 sdk nativeSkills parse failed", {
