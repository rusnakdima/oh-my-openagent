...
function createIdleTrackingEventHandler(
  dispatchCalls: EventInput[],
...
describe("createEventHandler - idle deduplication", () => {
  ...
  it("#given a readiness retry is pending #when session.idle arrives through the plugin handler #then tmux retry spawns the pane", async () => {
    ...
    const executeActions = mock(
      async (actions: Array<{ type: string; sessionId: string }>) => {
    ...
    const { TmuxSessionManager } = await import(
      `../features/tmux-subagent/manager?test=${crypto.randomUUID()}`
    );
  ...
  it("#given session.status already emitted a synthetic idle #when real session.idle follows immediately #then hooks run once", async () => {
    ...
    expect(
      (dispatchCalls[0]?.event.properties as { sessionID?: string } | undefined)
        ?.sessionID,
    ).toBe(sessionId);
  });
  ...
  it("does not forward session.created to tmux session manager when tmux integration is disabled", async () => {
    ...
    await eventHandler(asEventHandlerInput({
      event: {
        type: "session.created",
        properties: {
          info: { id: "ses_tmux_disabled", parentID: "ses_parent" },
        },
      },
    }));
    ...
    const eventHandler = createEventHandler({
      ...
      managers: createEventHandlerManagers({
        ...
        tmuxSessionManager: {
          ...
          getTrackedPaneId: (
            sessionID: string,
          ) => (sessionID === "ses_openclaw_created" ? "%9" : undefined),
        },
      }),
      hooks: createEventHandlerHooks({}),
    });
    ...
    const eventHandler = createEventHandler({
      ...
      managers: createEventHandlerManagers({
        ...
        tmuxSessionManager: {
          ...
          getTrackedPaneId: (
            sessionID: string,
          ) => (sessionID === "ses_subagent" ? "%10" : undefined),
        },
      }),
      hooks: createEventHandlerHooks({}),
    });
    ...
    const eventHandler = createEventHandler({
      ...
      managers: createEventHandlerManagers({
        ...
        tmuxSessionManager: {
          ...
          getTrackedPaneId: (
            sessionID: string,
          ) => (sessionID === "ses_openclaw_idle" ? "%3" : undefined),
        },
      }),
      hooks: createEventHandlerHooks({}),
    });
