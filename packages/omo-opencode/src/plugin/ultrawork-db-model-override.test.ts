...
let scheduleDeferredModelOverride:
  (typeof import("./ultrawork-db-model-override"))[
    "scheduleDeferredModelOverride"
  ];

async function importFreshUltraworkDbModelOverrideModule(): Promise<
  typeof import("./ultrawork-db-model-override")
> {
  return import(
    `./ultrawork-db-model-override?test=${Date.now()}-${Math.random()}`
  );
}
...
describe("scheduleDeferredModelOverride", () => {
  ...
  beforeEach(async () => {
    ...
    ({ scheduleDeferredModelOverride } =
      await importFreshUltraworkDbModelOverrideModule());
  });
  ...
  test("should fall back to setTimeout when message never appears", async () => {
    ...
    //#then
    const fallbackFailureCall = logSpy.mock.calls.find(
  ...
  test("should log when microtask retries are exhausted before setTimeout fallback", async () => {
    ...
    //#then
    const retryExhaustedCall = logSpy.mock.calls.find(
      (call: readonly unknown[]) => {
        const message = call[0];
        const metadata = call[1];
        return (
