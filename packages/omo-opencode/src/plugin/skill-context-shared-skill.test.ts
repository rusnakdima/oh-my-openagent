...
const BLOCKED_MIXED_CASE_SKILL_DESCRIPTION =
...
function createNativeSkills(
  directory: string,
  skills: Awaited<
    ReturnType<NonNullable<SkillLoadOptions["nativeSkills"]>["all"]>
  >,
...
async function expectMixedCaseBlockedSkillFiltered(args: {
  ...
  expect(skillContext.mergedSkills.map((skill) => skill.name)).not.toContain(
    "Blocked-Skill",
  );
  expect(skillContext.availableSkills.map((skill) => skill.name)).not.toContain(
    "Blocked-Skill",
  );
  ...
  await expectSkillUnavailable(
    skillTool,
    createToolContext(args.directory),
    "Blocked-Skill",
  );
}
...
describe("plugin-wired shared skills", () => {
  ...
  test("#given a local skill shadows a bundled shared skill #when the plugin-wired skill tool executes #then the local skill wins", async () => {
    ...
    // when
    const plainOutput = toolResultToText(
      await skillTool.execute({ name: "ulw-plan" }, toolContext),
    );
  ...
  test("#given the shared skill is loaded #when skill context is built #then it appears under its bare name", async () => {
    ...
    // then
    expect(skillContext.mergedSkills.some((skill) => skill.name === "ulw-plan"))
      .toBe(true);
    expect(
      skillContext.mergedSkills.some((skill) =>
        skill.name.startsWith("shared/")
      ),
    ).toBe(false);
    expect(
      skillContext.availableSkills.some((skill) => skill.name === "ulw-plan"),
    ).toBe(true);
    expect(
      skillContext.availableSkills.some((skill) =>
        skill.name.startsWith("shared/")
      ),
    ).toBe(false);
  });
  ...
  test("#given mixed-case native skill disabled by lowercase skills.disable #when plugin-wired skill surfaces consume context #then it is absent from tool and delegate loads", async () => {
    ...
    expect(skillTool.description).not.toContain(
      BLOCKED_NATIVE_SKILL_DESCRIPTION,
    );
    await expectSkillUnavailable(
      skillTool,
      createToolContext(testDirectory),
      "Blocked-Native",
    );
    expect(resolved.content).toBeUndefined();
    expect(resolved.contents).toEqual([]);
