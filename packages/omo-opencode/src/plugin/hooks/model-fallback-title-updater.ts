...
export function createModelFallbackTitleUpdater(ctx: PluginContext) {
  ...
  const fallbackTitleState = new Map<
    string,
    { baseTitle?: string; lastKey?: string }
  >();
  ...
  return async (input: ModelFallbackTitleInput): Promise<void> => {
    ...
    if (!existing.baseTitle) {
      const sessionResp = await ctx.client.session.get({
        path: { id: input.sessionID },
      }).catch(() => null);
      const sessionInfo = sessionResp
        ? normalizeSDKResponse<{ readonly title?: string } | null>(
          sessionResp,
          null,
    ...
    const newTitle =
      `${existing.baseTitle} [fallback: ${input.providerID}/${input.modelID}${variantLabel}]`;
