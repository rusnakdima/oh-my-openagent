...
export type SessionHooks = {
  preemptiveCompaction:
    | ReturnType<typeof createPreemptiveCompactionHook>
    | null;
  ...
  anthropicContextWindowLimitRecovery:
    | ReturnType<typeof createAnthropicContextWindowLimitRecoveryHook>
    | null;
  ...
  interactiveBashSession:
    | ReturnType<typeof createInteractiveBashSessionHook>
    | null;
  interactiveMenuSession:
    | ReturnType<typeof createInteractiveMenuSessionHook>
    | null;
  ...
  sisyphusJuniorNotepad:
    | ReturnType<typeof createSisyphusJuniorNotepadHook>
    | null;

  hephaestusAgentsMdInjector:
    | ReturnType<typeof createHephaestusAgentsMdInjectorHook>
    | null;
  questionLabelTruncator:
    | ReturnType<typeof createQuestionLabelTruncatorHook>
    | null;
...
export function createSessionHooks(args: {
  ...
  let sessionNotification: ReturnType<typeof createSessionNotification> | null =
    null;
  if (isHookEnabled("session-notification")) {
    const forceEnable = pluginConfig.notification?.force_enable ?? false;
    const externalNotifier = detectExternalNotificationPlugin(ctx.directory);
    if (
      externalNotifier.detected && externalNotifier.pluginName && !forceEnable
    ) {
      log(getNotificationConflictWarning(externalNotifier.pluginName));
    } else {
      sessionNotification = safeHook(
        "session-notification",
        () =>
          createSessionNotification(ctx, {
            longRunningSessionMinutes:
              pluginConfig.notification?.long_running_session_minutes ?? 0,
          }),
      );
    }
  }
  ...
  const imageProxy = isHookEnabled("image-proxy") && !multimodalLookerDisabled
    ? safeHook("image-proxy", () =>
      createImageProxyHook(ctx, {
  ...
  const enableFallbackTitle = pluginConfig.experimental?.model_fallback_title ??
    false;
  ...
  const modelFallback =
    isModelFallbackConfigEnabled && isHookEnabled("model-fallback")
  ...
  const anthropicContextWindowLimitRecovery =
    isHookEnabled("anthropic-context-window-limit-recovery")
      ? safeHook(
  ...
  const interactiveMenuSession = isHookEnabled("interactive-menu-session") &&
      isTmuxIntegrationEnabled(pluginConfig)
    ? safeHook(
  ...
  const hephaestusAgentsMdInjector =
    isHookEnabled("hephaestus-agents-md-injector")
      ? safeHook(
