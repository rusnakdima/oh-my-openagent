...
function createFactories(
  createSkillTool: (options: SkillLoadOptions) => typeof fakeTool,
...
describe("#given disabled native skills in the registry skill context", () => {
  test("#when core tools register the skill tool #then disabled skills are passed to the skill tool factory", () => {
    ...
    expect(createSkillTool.mock.calls[0]?.[0].disabledSkills).toBe(
      disabledSkills,
    );
  });
});
...
describe("#given core skill tools are registered", () => {
  test("#when core tools are created #then skill task and skill_mcp share the runtime skill resolver", () => {
    const createSkillTool = mock((options: SkillLoadOptions) => fakeTool);
    const createDelegateTask = mock((
      options: Parameters<ToolRegistryFactories["createDelegateTask"]>[0],
    ) => fakeTool);
    const createSkillMcpTool = mock((
      options: Parameters<ToolRegistryFactories["createSkillMcpTool"]>[0],
    ) => fakeTool);
    const factories = {
    ...
    expect(createDelegateTask.mock.calls[0]?.[0].getLoadedSkills).toBe(
      skillResolver,
    );
    expect(createSkillMcpTool.mock.calls[0]?.[0].getLoadedSkills).toBe(
      skillResolver,
    );
  });
});
