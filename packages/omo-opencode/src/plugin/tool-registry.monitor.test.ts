...
function createRegistry(pluginConfig: OhMyOpenCodeConfig) {
  return createToolRegistry({
    ctx: { directory: "/tmp/monitor", client: {} } as Parameters<
      typeof createToolRegistry
    >[0]["ctx"],
    pluginConfig,
    managers: unsafeTestValue<
      Parameters<typeof createToolRegistry>[0]["managers"]
    >({
      backgroundManager: {},
      tmuxSessionManager: {},
      skillMcpManager: {},
...
describe("monitor tool registry wiring", () => {
  test("#given monitor is disabled #when registry is created #then monitor tools are absent", () => {
    const result = createRegistry(
      createPluginConfig({ monitor: { enabled: false } }),
    );
  ...
  test("#given monitor is enabled #when registry is created #then all monitor tools are present with normalized schemas", () => {
    const result = createRegistry(
      createPluginConfig({ monitor: { enabled: true } }),
    );
