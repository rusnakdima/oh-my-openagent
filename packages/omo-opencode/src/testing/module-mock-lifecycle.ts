...
function isInternalStackPath(candidatePath: string): boolean {
  ...
  return normalizedPath.includes("/test-setup.ts") ||
    normalizedPath.includes("/src/testing/module-mock-lifecycle.ts");
}

export function getCallerUrlFromStack(
  stack: string,
  fallbackUrl: string,
): string {
...
export function installModuleMockLifecycle(
  ...
  const loadOriginalModule = options.loadOriginalModule ??
    defaultLoadOriginalModule;
  const shouldPreserveActiveMocksOnRestore =
    options.shouldPreserveActiveMocksOnRestore ?? (() => {
      return new Error().stack?.includes("/test-setup.ts") ?? false;
    });
  const preserveOwners = new Set<string>();
  let handledPreserveCleanup = false;

  function replayActiveMocks(
    ownerFilter?: (ownerUrl: string) => boolean,
  ): void {
  ...
  function restoreUnpreservedModuleMocks(options: RestoreOptions = {}): void {
    ...
    for (const [restoreSpecifier, activeMockStack] of activeMocks.entries()) {
      const preservedMocks = activeMockStack.filter((activeMock) =>
        preserveOwners.has(activeMock.ownerUrl)
      );
      if (preservedMocks.length > 0) {
        activeMocks.set(restoreSpecifier, preservedMocks);
        removedMocks = removedMocks ||
          preservedMocks.length !== activeMockStack.length;
        continue;
      }
  ...
  function removeActiveMocksForTestFile(callerUrl: string): void {
    for (const [restoreSpecifier, activeMockStack] of activeMocks.entries()) {
      const remainingMocks = activeMockStack.filter((activeMock) =>
        activeMock.ownerUrl !== callerUrl
      );
  ...
  function hasActiveMocksForTestFile(callerUrl: string): boolean {
    for (const activeMockStack of activeMocks.values()) {
      if (
        activeMockStack.some((activeMock) => activeMock.ownerUrl === callerUrl)
      ) {
        return true;
      }
    }
  ...
  function restoreModuleMocksForTestFile(callerUrl: string): void {
    ...
    for (const [restoreSpecifier, activeMockStack] of activeMocks.entries()) {
      ...
      if (previousActiveMock) {
        delegateModule(
          previousActiveMock.specifier,
          previousActiveMock.factory,
        );
        continue;
      }
  ...
  mockApi.module = (specifier: string, factory: MockModuleFactory): unknown => {
    ...
    const nextActiveMockStack = activeMockStack.filter((activeMock) =>
      activeMock.ownerUrl !== callerUrl
    );
