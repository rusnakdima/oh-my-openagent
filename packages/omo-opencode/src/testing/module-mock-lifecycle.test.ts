...
import {
  getCallerUrlFromStack,
  installModuleMockLifecycle,
  normalizeStackPath,
} from "./module-mock-lifecycle";
...
describe("installModuleMockLifecycle", () => {
  test("#given a Windows stack path #when normalizing caller path #then it becomes a file URL rooted at the drive", () => {
    ...
    const stackPath = String
      .raw`D:\a\oh-my-openagent\oh-my-openagent\src\hooks\example.test.ts`;

    // when
    const callerUrl = normalizeStackPath(stackPath);

    // then
    expect(callerUrl).toBe(
      "file:///D:/a/oh-my-openagent/oh-my-openagent/src/hooks/example.test.ts",
    );
  });
  ...
  test("#given Windows internal stack frames #when selecting caller url #then test setup and lifecycle frames are ignored", () => {
    ...
    const stack = [
      "Error",
      String
        .raw`    at mock.restore (D:\a\oh-my-openagent\oh-my-openagent\src\testing\module-mock-lifecycle.ts:287:23)`,
      String
        .raw`    at afterEach (D:\a\oh-my-openagent\oh-my-openagent\test-setup.ts:72:10)`,
      String
        .raw`    at module code (D:\a\oh-my-openagent\oh-my-openagent\src\hooks\example.test.ts:12:3)`,
    ].join("\n");
  ...
  test("restores the original module exports on mock.restore", () => {
    ...
    const moduleCalls: Array<
      { specifier: string; value: Record<string, unknown> }
    > = [];
    const mockApi = {
  ...
  test("#given no active module mocks #when mock.restore runs #then it delegates to Bun restore", () => {
    ...
    const mockApi = {
      module: (
        _specifier: string,
        _factory: () => Record<string, unknown>,
      ) => {},
      restore: mock(() => {
        events.push("delegate:restore");
      }),
  ...
  test("#given preserved active module mock #when global test setup cleans up #then preserved mock stays active", () => {
    ...
    // when
    mockApi.module("./dependency", () => ({ named: "mocked" }));
    lifecycle.preserveModuleMocksForTestFile(
      "file:///repo/tests/example.test.ts",
    );
    mockApi.restore();
    lifecycle.restoreModuleMocks();
  ...
  test("captures the original module only once per resolved specifier", () => {
    ...
    const mockApi = {
      module: mock(
        (_specifier: string, _factory: () => Record<string, unknown>) => {},
      ),
      restore: mock(() => {}),
    };
  ...
  test("#given preserved unresolved module mock #when scoped restore runs #then Bun restore clears the active mock", () => {
    ...
    mockApi.module("virtual:missing", () => ({ named: "mocked" }));
    lifecycle.preserveModuleMocksForTestFile(
      "file:///repo/tests/preserved.test.ts",
    );
    lifecycle.restoreModuleMocksForTestFile(
      "file:///repo/tests/preserved.test.ts",
    );
