...
describe("2026-08 reasoning unification migration", () => {
  test("#given the canonical legacy fixture #when transformed #then rules one through eight match exact expected output", () => {
    ...
    // then
    expect(REASONING_UNIFICATION_MIGRATION_ID).toBe(
      "2026-08-reasoning-unification",
    );
    expect(result.document).toEqual(fixtureExpected);
    expect(result.diagnostics).toContain(
      'conflict: categories.conflict dropped variant="high" kept reasoningEffort="xhigh"',
    );
  });
  ...
  test("#given canonical reasoning outranking legacy keys #when transformed #then the journal reports reasoning as the winner", () => {
    ...
    const journal =
      result.diagnostics.find((entry) => entry.includes("conflict")) ?? "";
  ...
  test("#given an existing user omo config #when planned and executed #then dry-run, backup, journaled execution, and marker use the existing engine", () => {
    ...
    try {
      ...
      if (plan === undefined) {
        throw new Error("Expected reasoning migration plan");
      }
      ...
      const migrated = executeLegacyConfigMigrationPlan(plan, {
        ...
        onBoundary: (boundary) => {
          ...
          const journal = JSON.parse(
            readFileSync(
              join(homeDir, ".omo", ".migration-journal.json"),
              "utf-8",
            ),
      ...
      const document = JSON.parse(readFileSync(targetPath, "utf-8")) as Record<
        string,
        unknown
      >;
      ...
      expect(journalDiagnostics).toContain(
        'conflict: categories.conflict dropped variant="high" kept reasoningEffort="xhigh"',
      );
      expect(
        readdirSync(join(homeDir, ".omo")).some((name) =>
          name.startsWith("omo.jsonc.bak.")
        ),
      ).toBe(true);
    } finally {
  ...
  test("#given typed harness blocks, profiles, and unrelated opencode values #when transformed #then typed records recurse and opencode rewrites only known agent and category keys", () => {
    ...
    const input = {
      "[senpi]": {
        categories: { deep: { model: "p/m high", variant: "low" } },
      },
      "[codex]": {
        agents: { oracle: { model: "p/m", reasoningEffort: "none" } },
      },
      "[opencode]": {
        agents: {
          oracle: { variant: "high", native: { variant: "untouched" } },
        },
      ...
      profiles: {
        focused: {
          categories: { deep: { model: "p/m(xhigh)", maxTokens: 1 } },
        },
      },
    };
    ...
    expect(result.document).toEqual({
      "[senpi]": {
        categories: { deep: { model: "p/m:high", reasoning: "low" } },
      },
      "[codex]": { agents: { oracle: { model: "p/m", reasoning: "off" } } },
      "[opencode]": {
        agents: {
          oracle: { reasoning: "high", native: { variant: "untouched" } },
        },
        categories: { quick: { textVerbosity: "low" } },
        provider: { variant: "untouched" },
      },
      profiles: {
        focused: {
          categories: { deep: { model: "p/m:xhigh", max_tokens: 1 } },
        },
      },
    });
  });
