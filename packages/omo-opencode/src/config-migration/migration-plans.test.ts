...
describe("legacy config migration plans", () => {
  test("#given user, profile, config.jsonc, project, and sidecar fixtures #when planning and executing #then distinct group markers and exact backup destinations preserve every source", () => {
    ...
    const fixtureRoot = mkdtempSync(
      join(tmpdir(), "omo-config-migration-plan-"),
    );
    ...
    writeFileSync(
      rootPath,
      JSON.stringify({ agents: { oracle: { model: "reverted-old-model" } } }),
    );
    writeFileSync(
      sidecarPath,
      JSON.stringify({
        appliedMigrations: ["model-version:reverted-old-model->new-model"],
      }),
    );
    writeFileSync(
      profilePath,
      JSON.stringify({ agents: { oracle: { model: "kimi-model" } } }),
    );
    writeFileSync(
      configJsoncPath,
      JSON.stringify({ codegraph: { excluded_roots: ["/generated"] } }),
    );
    writeFileSync(
      configSidecarPath,
      JSON.stringify({ appliedMigrations: ["legacy-config-jsonc"] }),
    );
    writeFileSync(
      projectPath,
    ...
    try {
      ...
      const opencodeUserPlan = plans.find((plan) =>
        plan.id === OPENCODE_CONFIG_MIGRATION_ID &&
        plan.sources.some((source) => source.path === canonicalRootPath)
      );
      const configJsoncPlan = plans.find((plan) =>
        plan.id === CONFIG_JSONC_MIGRATION_ID &&
        plan.sources.some((source) => source.path === canonicalConfigJsoncPath)
      );
      const projectPlan = plans.find((plan) =>
        plan.id === OPENCODE_CONFIG_MIGRATION_ID &&
        plan.sources.some((source) =>
          source.path.endsWith("oh-my-opencode.json")
        )
      );
      if (
        opencodeUserPlan === undefined || configJsoncPlan === undefined ||
        projectPlan === undefined
      ) {
        throw new Error(
          "Expected user, config.jsonc, and project migration plans",
        );
      }
      const configResult = executeLegacyConfigMigrationPlan(configJsoncPlan, {
        env: { HOME: homeDir },
      });
      const opencodeResult = executeLegacyConfigMigrationPlan(
        opencodeUserPlan,
        { env: { HOME: homeDir } },
      );
      const projectResult = executeLegacyConfigMigrationPlan(projectPlan, {
        env: { HOME: homeDir },
      });
      ...
      expect(userDocument["[opencode]"]).toEqual({
        agents: { oracle: { model: "reverted-old-model" } },
      });
      ...
      expect(opencodeUserPlan.sources.map((source) => source.backupPath))
        .toEqual([
          join(
            canonicalHomeDir,
            ".omo",
          ...
          join(
            ...
            ".config",
            "opencode",
            "oh-my-openagent.jsonc.migrations.json",
          ),
          join(
            canonicalHomeDir,
            ".omo",
      ...
      expect(configJsoncPlan.sources.map((source) => source.backupPath))
        .toEqual([
          join(
            canonicalHomeDir,
            ".omo",
  ...
  test("#given overlapping legacy [omo] and [senpi] blocks #when previewing and migrating config.jsonc #then both results report the transform conflict", () => {
    // given
    const fixtureRoot = mkdtempSync(
      join(tmpdir(), "omo-config-migration-diagnostics-"),
    );
    const homeDir = join(fixtureRoot, "home");
    const configJsoncPath = join(homeDir, ".omo", "config.jsonc");
    ...
    try {
      ...
      const plan = plans.find((candidate) =>
        candidate.id === CONFIG_JSONC_MIGRATION_ID
      );
      if (plan === undefined) {
        throw new Error("Expected config.jsonc migration plan");
      }

      // when
      const dryRun = executeLegacyConfigMigrationPlan(plan, {
        dryRun: true,
        env: { HOME: homeDir },
      });
      ...
      // then
      expect(dryRun.diagnostics).toContain(
        "conflict: [senpi] legacy [omo] kept [senpi]",
      );
      expect(migrated.diagnostics).toContain(
        "conflict: [senpi] legacy [omo] kept [senpi]",
      );
    } finally {
      rmSync(fixtureRoot, { force: true, recursive: true });
