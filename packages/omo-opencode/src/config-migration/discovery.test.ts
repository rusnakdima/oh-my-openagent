...
function memoryFileSystem(
  ...
  const files = new Set(
    paths.map((path) => win32.normalize(path).toLowerCase()),
  );
  return {
    existsSync: (path): boolean =>
      files.has(win32.normalize(path).toLowerCase()),
...
function shortPathMemoryFileSystem(
  ...
  const existsSync = (path: string): boolean => {
    ...
    return files.has(key) ||
      [...files].some((file) => file.startsWith(`${key}\\`));
  };
  return {
    existsSync,
    readdirSync: (directory): string[] => {
    ...
    realpathSync: (path): string => {
      ...
      return win32.normalize(path).replace(
        normalizedShortHome,
        normalizedLongHome,
      );
    },
...
describe("legacy config discovery", () => {
  test("#given a custom active profile, default config, Tauri config, and walked project config #when discovering sources #then roots are deduplicated and separated from config.jsonc", () => {
    ...
    const fileSystem: ConfigMigrationDiscoveryFileSystem = {
      ...
      readdirSync: (path): string[] =>
        path === "/home/alice/.config/opencode/profiles"
          ? ["kimi", "quiet", "__test"]
          : [],
    ...
    expect(opencodeGroup.sources.find((source) => source.profile === "kimi"))
      .toMatchObject({
    ...
    expect(
      opencodeGroup.sources.filter((source) =>
        source.path.includes("/profiles/kimi/")
      ).length,
    ).toBe(1);
    expect(
      opencodeGroup.sources.some((source) => source.path.includes("__test")),
    ).toBe(false);
  ...
  test("#given an injected Windows filesystem #when its short path expands through realpath #then discovery keeps the canonical source within the config root", () => {
    // given
    const shortHome =
      "C:\\Users\\RUNNER~1\\AppData\\Local\\Temp\\omo-config-migrate-A1B2\\home";
    const longHome =
      "C:\\Users\\runner\\AppData\\Local\\Temp\\omo-config-migrate-A1B2\\home";
    const sourcePath = win32.join(
      shortHome,
      ".config",
      "opencode",
    ...
    // when
    const groups = withProcessPlatform(
      "win32",
      () =>
        discoverLegacyConfigGroups({
          ...
          environment: {
            HOME: shortHome,
            OPENCODE_CONFIG_DIR: win32.join(shortHome, ".config", "opencode"),
          },
          fileSystem: shortPathMemoryFileSystem(shortHome, longHome, [
            sourcePath,
          ]),
          homeDir: shortHome,
    ...
    try {
      ...
      const wsl = discoverLegacyConfigGroups({
        ...
        fileSystem: {
          ...
          readdirSync: (path): string[] =>
            path === "/mnt/c/Users/Alice/.config/opencode/profiles"
              ? ["kimi"]
              : [],
          realpathSync: (path): string => path.replace(/\/$/, ""),
        },
      ...
      expect(windows[0].sources.find((source) => source.profile === "kimi"))
        .toMatchObject({
          baseRoot: "C:\\USERS\\ALICE\\APPDATA\\ROAMING\\OPENCODE",
          isActiveProfile: true,
        });
      expect(wsl[0].sources.find((source) => source.profile === "kimi"))
        .toMatchObject({
