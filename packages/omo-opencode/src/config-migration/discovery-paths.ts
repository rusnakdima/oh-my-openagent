...
export function isWindowsPathOperations(
...
export function usesWindowsPathSemantics(
  options: ConfigMigrationDiscoveryOptions,
): boolean {
  return options.platform === "win32" ||
    (options.fileSystem === undefined && process.platform === "win32");
}

export function hostPathOperations(
  options: ConfigMigrationDiscoveryOptions,
...
function comparisonPath(
  ...
  return usesWindowsPathSemantics(options)
...
export function canonicalPath(
  ...
  const resolved =
    usesWindowsPathSemantics(options) && win32.isAbsolute(normalized)
      ? normalized
  ...
  try {
    ...
    if (
      ...
      (Reflect.get(error, "code") === "ENOENT" ||
        Reflect.get(error, "code") === "ENOTDIR")
    ) {
      return resolved;
    }
    throw error;
...
export function profileDirectories(
  root: string,
  options: ConfigMigrationDiscoveryOptions,
): readonly string[] {
...
function isWithin(
  ...
  return relative === "" ||
    (!relative.startsWith("..") && !pathOperations.isAbsolute(relative));
}
...
export function projectDirectories(
  ...
  for (;;) {
    ...
    if (stopAtHome && pathKey(current, options) === pathKey(homeDir, options)) {
      break;
    }
