...
function loaded(
  source: DiscoveredLegacyConfigSource,
  value: unknown,
  ...
  test("#given root and profile configs, config.jsonc, and a project config #when transformed #then the golden unified documents parse through OmoConfigSchema", () => {
    ...
    try {
      ...
      const opencodeGroup = groups.find((group) =>
        group.id === OPENCODE_CONFIG_MIGRATION_ID
      );
      const configJsoncGroup = groups.find((group) =>
        group.id === CONFIG_JSONC_MIGRATION_ID
      );
      if (opencodeGroup === undefined || configJsoncGroup === undefined) {
        throw new Error("Expected both migration groups");
      }
      const root = opencodeGroup.sources.find((source) =>
        source.kind === "user-config"
      );
      const focused = opencodeGroup.sources.find((source) =>
        source.profile === "focused"
      );
      const kimi = opencodeGroup.sources.find((source) =>
        source.profile === "kimi"
      );
      const project = opencodeGroup.sources.find((source) =>
        source.kind === "project-config"
      );
      const configJsonc = configJsoncGroup.sources.find((source) =>
        source.kind === "config-jsonc"
      );
      if (
        root === undefined || focused === undefined || kimi === undefined ||
        project === undefined ||
      ...
      expect(userDocument).toEqual({
        ...
        profiles: {
          focused: {
            "[opencode]": { categories: { deep: { model: "focused-model" } } },
          },
          kimi: {
            "[opencode]": { agents: { oracle: { model: "kimi-model" } } },
          },
        },
      });
      expect(projectDocument.document["[opencode]"]).toEqual({
  ...
  test("#given an identical profile subtree and a sidecar recording a reverted migration #when transformed #then only the profile delta and user-left value are retained", () => {
    ...
    const result = transformOpenCodeSources({
      ...
      sources: [
        loaded(root, {
          agents: {
            oracle: { model: "reverted-old-model" },
            atlas: { model: "same" },
          },
        }),
        loaded(profile, {
