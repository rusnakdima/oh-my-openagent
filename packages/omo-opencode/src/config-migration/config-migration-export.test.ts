...
const CONFIG_CORE_ENTRY_POINT = join(
  PACKAGES_PATH,
  "omo-config-core",
  "src",
  "index.ts",
...
const NEGATIVE_FIXTURE_ENTRY_POINT = join(
  import.meta.dir,
  "..",
  "..",
  "test",
  "fixtures",
  "config-migration",
  "opencode-side-effect-import.ts",
);
...
function workspacePackages(): ReadonlyMap<string, string> {
  ...
  for (const entry of readdirSync(PACKAGES_PATH, { withFileTypes: true })) {
    ...
    const name = workspacePackageName(
...
function resolveModule(
  ...
  if (resolved === undefined) {
    throw new Error(`Cannot resolve ${specifier} from ${sourcePath}`);
  }
  return resolved;
}
...
function resolveWorkspaceImport(
  ...
  for (const [packageName, packageSourcePath] of WORKSPACE_PACKAGES) {
    if (specifier === packageName) {
      return resolveModule(packageSourcePath, specifier, sourcePath);
    }
    const prefix = `${packageName}/`;
    if (specifier.startsWith(prefix)) {
      return resolveModule(
        join(packageSourcePath, specifier.slice(prefix.length)),
        specifier,
        sourcePath,
      );
    }
...
describe("config-migration public subpath", () => {
  ...
  test("#given the config-migration entry point #when its local module graph is audited #then it imports neither OpenCode SDK modules nor plugin runtime code", () => {
    ...
    for (const modulePath of modules) {
      ...
      for (const specifier of importSpecifiers(source)) {
        if (OPENCODE_IMPORT.test(specifier)) {
          offenders.push(`${modulePath} imports ${specifier}`);
        }
      }
      if (
        modulePath.includes(PLUGIN_RUNTIME_DIRECTORY) ||
        modulePath.includes(PLUGIN_CONFIG_DIRECTORY)
      ) {
  ...
  test("#given a fixture with a side-effect OpenCode SDK import #when auditing its module graph #then reports the forbidden import", () => {
    ...
    for (const modulePath of modules) {
      for (
        const specifier of importSpecifiers(readFileSync(modulePath, "utf-8"))
      ) {
        if (OPENCODE_IMPORT.test(specifier)) {
          offenders.push(`${modulePath} imports ${specifier}`);
        }
