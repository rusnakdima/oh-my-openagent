...
export type LegacyConfigMigrationPlan = {
  readonly id: string;
  readonly inspect: (
    sources: Parameters<MigrationTransform>[0],
  ) => ConfigMigrationTransformResult;
...
export type CreateLegacyConfigMigrationPlansOptions =
  & ConfigMigrationDiscoveryOptions
  & {
...
function descriptors(
  ...
  return sources.map((source) => ({
    backupPath: source.projectRoot === undefined
      ? pathOperations.join(
        userBackupRoot,
        safeRelativePath(source.path, homeDir, options),
      )
      : pathOperations.join(
        source.projectRoot,
        ".omo",
        `migration-backup-${timestamp}`,
        pathOperations.basename(source.path),
      ),
    path: source.path,
  }));
}
...
function openCodeInspection(
  ...
  return input.scopes.reduce<ConfigMigrationTransformResult>(
    (result, scope) => {
      const next = transformOpenCodeSources({
...
function existingOmoConfigPath(
  directory: string,
  options: ConfigMigrationDiscoveryOptions,
): string | undefined {
...
export function createLegacyConfigMigrationPlans(
  ...
  const pathOperations = hostPathOperations(options);
  const userTargetPath = pathOperations.join(
    options.homeDir,
    ".omo",
    "omo.jsonc",
  );
  const userSources = openCodeGroup.sources.filter((source) =>
  ...
  const plans = mergeOpenCodeInputs(openCodeInputs, options).map((input) =>
    openCodePlan(input, options, timestamp)
  );
  const legacyPlans = configJsoncGroup.id !== CONFIG_JSONC_MIGRATION_ID ||
      configJsoncGroup.sources.length === 0
    ? plans
    : [
      ...plans,
      {
        id: CONFIG_JSONC_MIGRATION_ID,
        inspect: (loaded: Parameters<MigrationTransform>[0]) =>
        ...
        targetPath: userTargetPath,
        transform: (loaded: Parameters<MigrationTransform>[0]) =>
          transformConfigJsoncSources({
            discovered: configJsoncGroup.sources,
            sources: loaded,
          }),
  ...
  addReasoningTarget(
    existingOmoConfigPath(
      options.pathOperations.join(options.homeDir, ".omo"),
      options,
    ),
  );
  for (const projectRoot of projectDirectories(options)) {
    addReasoningTarget(
      existingOmoConfigPath(
        options.pathOperations.join(projectRoot, ".omo"),
        options,
      ),
    );
  }
