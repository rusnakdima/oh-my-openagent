import {
  AGENT_NAME_MAP,
  migrateAgentNames,
} from "@oh-my-opencode/utils/migration/agent-names";
...
function replaceDisabledAgents(value: unknown): unknown {
  ...
  return value.map((entry) => {
    ...
    return AGENT_NAME_MAP[entry.toLowerCase()] ?? AGENT_NAME_MAP[entry] ??
      entry;
  });
}
...
function transformLegacyKeys(
  ...
  if (isPlainRecord(agents)) {
    ...
    config.agents =
      migrateModelVersions(namedAgents, appliedMigrations).migrated;
  }
  const categories = config.categories;
  if (isPlainRecord(categories)) {
    config.categories =
      migrateModelVersions(categories, appliedMigrations).migrated;
  }
  ...
  delete config.lsp;
  if (
    isPlainRecord(config.experimental) &&
    Object.hasOwn(config.experimental, "hashline_edit")
  ) {
    if (config.hashline_edit === undefined) {
      config.hashline_edit = config.experimental.hashline_edit;
    }
...
function loadedConfigs(
  ...
  const metadataByPath = new Map(
    discovered.map((source) => [source.path, source]),
  );
  ...
  for (const source of sources) {
    ...
    if (
      metadata === undefined || !isPlainRecord(source.value) ||
      metadata.kind === "migration-sidecar"
    ) continue;
    configs.push({
      metadata,
      value: transformLegacyKeys(
        source.value,
        historyByConfigPath[metadata.configPath] ?? [],
      ),
    });
  }
  return configs;
...
function transformUserSources(
  ...
  const rootConfigs = configs.filter((config) =>
    config.metadata.kind === "user-config"
  );
  const rootByDirectory = new Map<string, Record<string, unknown>>();
  for (
    const root of new Set(rootConfigs.map((config) => config.metadata.baseRoot))
  ) {
    if (root === undefined) continue;
    rootByDirectory.set(
      root,
      mergedConfigs(
        rootConfigs.filter((config) => config.metadata.baseRoot === root),
      ),
    );
  }
  const profiles: Record<string, unknown> = {};
  for (
    const profile of new Set(configs.map((config) => config.metadata.profile))
  ) {
    if (profile === undefined) continue;
    const profileConfigs = configs.filter((config) =>
      config.metadata.kind === "profile-config" &&
      config.metadata.profile === profile
    );
    const differences = profileConfigs.map((config) => ({
      metadata: config.metadata,
      value: deepDifference(
        rootByDirectory.get(config.metadata.baseRoot ?? "") ?? {},
        config.value,
      ),
    }));
    const difference = mergedConfigs(differences);
    if (Object.keys(difference).length > 0) {
      profiles[profile] = { "[opencode]": difference };
    }
  }
  return documentWithHistory(
    mergedConfigs(rootConfigs),
    profiles,
    discovered.filter((source) => source.projectRoot === undefined),
    sources,
  );
}
...
function transformProjectSources(
  ...
  const discovered = input.discovered.filter((source) =>
...
export function transformOpenCodeSources(
  input: TransformOpenCodeSourcesInput,
): ConfigMigrationTransformResult {
