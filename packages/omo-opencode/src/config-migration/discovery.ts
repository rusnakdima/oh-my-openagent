...
export const OPENCODE_CONFIG_MIGRATION_ID =
...
function addConfigAndSidecar(
  ...
  if (
    !seen.has(configKey) &&
    discoveryFileSystem(options).existsSync(source.configPath)
  ) {
    seen.add(configKey);
    sources.push({
      ...source,
      configPath,
      kind: source.projectRoot === undefined
  ...
  if (
    !seen.has(sidecarKey) &&
    discoveryFileSystem(options).existsSync(sidecarPath)
  ) {
    seen.add(sidecarKey);
    sources.push({
      ...source,
...
function discoverOpenCodeSources(
  options: ConfigMigrationDiscoveryOptions,
): readonly DiscoveredLegacyConfigSource[] {
  const sources: DiscoveredLegacyConfigSource[] = [];
  const seen = new Set<string>();
  for (const root of configRoots(options)) {
    for (
      const [index, configPath] of configPaths(root.path, options).entries()
    ) {
    ...
    for (const profile of profileDirectories(root.path, options)) {
      const profileDirectory = options.pathOperations.join(
        root.path,
        "profiles",
        profile,
      );
      for (
        const [index, configPath] of configPaths(profileDirectory, options)
          .entries()
      ) {
        addConfigAndSidecar(sources, seen, {
          baseRoot: root.path,
          ...
          ...(root.activeProfile !== undefined &&
              (root.activeProfile === profile || (
                options.pathOperations === win32 &&
                root.activeProfile.toLowerCase() === profile.toLowerCase()
              ))
  ...
  for (
    const [directoryIndex, projectRoot] of projectDirectories(options).entries()
  ) {
    const configDirectory = options.pathOperations.join(
      projectRoot,
      ".opencode",
    );
    for (
      const [fileIndex, configPath] of configPaths(configDirectory, options)
        .entries()
    ) {
      addConfigAndSidecar(sources, seen, {
        configPath,
...
function discoverConfigJsoncSources(
  options: ConfigMigrationDiscoveryOptions,
): readonly DiscoveredLegacyConfigSource[] {
  const configPath = options.pathOperations.join(
    options.homeDir,
    ".omo",
    "config.jsonc",
  );
...
export function discoverLegacyConfigGroups(
  options: ConfigMigrationDiscoveryOptions,
): readonly LegacyConfigMigrationGroup[] {
  return [
    {
      id: OPENCODE_CONFIG_MIGRATION_ID,
      sources: discoverOpenCodeSources(options),
    },
    {
      id: CONFIG_JSONC_MIGRATION_ID,
      sources: discoverConfigJsoncSources(options),
    },
  ];
}
