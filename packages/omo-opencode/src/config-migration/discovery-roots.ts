...
function isWindowsPath(path: string): boolean {
  return /^[a-z]:[\\/]/i.test(path) ||
    /^\/mnt\/[a-z]\/users\//i.test(path.replaceAll("\\", "/"));
}
...
function configBaseDirectory(options: ConfigMigrationDiscoveryOptions): string {
  ...
  if (
    configured !== undefined && !(isWsl(options) && isWindowsPath(configured))
  ) return configured;
...
function defaultTauriRoots(
  options: ConfigMigrationDiscoveryOptions,
): readonly string[] {
  if (options.tauriConfigDirs !== undefined) return options.tauriConfigDirs;
  if (options.platform === "darwin") {
    const supportDir = options.pathOperations.join(
      options.homeDir,
      "Library",
      "Application Support",
    );
    return TAURI_IDENTIFIERS.map((identifier) =>
      options.pathOperations.join(supportDir, identifier)
    );
  }
  const base = configBaseDirectory(options);
  return TAURI_IDENTIFIERS.map((identifier) =>
    options.pathOperations.join(base, identifier)
  );
}
...
function windowsRoots(
  ...
  return [
    ...
    ...TAURI_IDENTIFIERS.map((identifier) =>
      pathOperations.join(appData, identifier)
    ),
  ];
}
...
function activeProfileRoot(
  ...
  if (
    options.pathOperations.basename(profilesDirectory).toLowerCase() !==
      "profiles"
  ) {
...
export function configRoots(
  ...
  const candidates: ConfigRoot[] = [
    ...
    ...defaultTauriRoots(options).map((path, index) => ({
      path,
      precedence: index + 2,
    })),
    ...windowsRoots(options).map((path, index) => ({
      path,
      precedence: index + 4,
    })),
  ];
