...
export const REASONING_UNIFICATION_MIGRATION_ID =
...
function hasModelSettings(
  ...
  return [
    ...
    ...(opencode
      ? []
      : ["provider_options", "providerOptions", "textVerbosity"]),
  ].some((key) => record[key] !== undefined);
}

function primaryModelRef(
  record: Readonly<Record<string, unknown>>,
  opencode: boolean,
): unknown | undefined {
...
function conflictDiagnostic(
  ...
  const present = (["reasoning", "reasoningEffort", "variant"] as const).filter(
  ...
  const droppedText = dropped.map(([key, value]) =>
    `${key}=${JSON.stringify(value)}`
  ).join(" ");
  return `conflict: ${path.join(".")} dropped ${droppedText} kept ${keptKey}=${
    JSON.stringify(keptValue)
  }`;
}
...
function normalizeDefinition(
  ...
  if (opencode) {
    for (
      const key of ["maxTokens", "providerOptions", "textVerbosity"] as const
    ) {
  ...
  const shouldCombine = fallbackModels !== undefined ||
    (kind === "agent" && value["model"] !== undefined &&
...
function normalizeDefinitions(
  ...
  for (const [name, definition] of Object.entries(value)) {
    result[name] = normalizeDefinition(
      definition,
      kind,
      [...path, name],
      diagnostics,
...
function normalizeTypedBlock(
  ...
  if (result["categories"] !== undefined) {
    result["categories"] = normalizeDefinitions(
      result["categories"],
      "category",
      [...path, "categories"],
      diagnostics,
  ...
  if (result["agents"] !== undefined) {
    result["agents"] = normalizeDefinitions(result["agents"], "agent", [
      ...path,
      "agents",
    ], diagnostics);
  }
  ...
  for (const harness of ["[senpi]", "[codex]"] as const) {
    if (result[harness] !== undefined) {
      result[harness] = normalizeTypedBlock(
        result[harness],
        [...path, harness],
        diagnostics,
        false,
  ...
  if (result["[opencode]"] !== undefined) {
    result["[opencode]"] = normalizeOpenCodeBlock(result["[opencode]"], [
      ...path,
      "[opencode]",
    ], diagnostics);
  }
  if (recurseProfiles && isPlainRecord(result["profiles"])) {
    ...
    for (const [name, profile] of Object.entries(result["profiles"])) {
      profiles[name] = normalizeTypedBlock(
        profile,
        [...path, "profiles", name],
        diagnostics,
        false,
...
function normalizeOpenCodeBlock(
  ...
  const result = copyRecord(value);
  if (result["categories"] !== undefined) {
    result["categories"] = normalizeOpenCodeDefinitions(
      result["categories"],
      "category",
      [...path, "categories"],
      diagnostics,
  ...
  if (result["agents"] !== undefined) {
    result["agents"] = normalizeOpenCodeDefinitions(result["agents"], "agent", [
      ...path,
      "agents",
    ], diagnostics);
  }
...
export function transformReasoningUnification(
  document: unknown,
  ...
  return {
    ...
    document: isPlainRecord(document)
      ? normalizeTypedBlock(document, [], diagnostics, true) as Record<
        string,
        unknown
      >
      : {},
