...
function replayAgentConfigSideEffects(params: {
  ...
  if (
    params.configuredDefaultAgent && typeof params.defaultAgent === "string"
  ) {
...
export function createConfigHandler(deps: ConfigHandlerDeps) {
  ...
  const configHandler: (config: Record<string, unknown>) => Promise<void> =
    async (config) => {
      ...
      applyProviderConfig({
        config,
        modelCacheState,
        trustedVisionCapableModels: collectTrustedVisionCapableModels(
          pluginConfig,
        ),
      });
      clearFormatterCache();
      ...
      const pluginComponentsLoadFailed =
        pluginComponents.retryableLoadFailure === true;
      ...
      if (
        !pluginComponentsLoadFailed &&
        agentConfigSnapshot?.cacheKey === agentCacheKey
      ) {
      ...
      log("[config-handler] config handler applied", {
        ...
        commandCount:
          Object.keys((config.command as Record<string, unknown>) ?? {})
            .length,
      });
    };
