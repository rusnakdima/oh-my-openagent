...
const BUILTIN_SISYPHUS_JUNIOR_DISPLAY_NAME = getAgentListDisplayName(
  "sisyphus-junior",
);
const BUILTIN_MULTIMODAL_LOOKER_DISPLAY_NAME = getAgentListDisplayName(
  "multimodal-looker",
);
...
describe("applyAgentConfig builtin override protection", () => {
  ...
  beforeEach(() => {
    ...
    loadProjectAgentsSpy = spyOn(agentLoader, "loadProjectAgents")
      .mockReturnValue({});
    loadOpencodeGlobalAgentsSpy = spyOn(agentLoader, "loadOpencodeGlobalAgents")
      .mockReturnValue({});
    loadOpencodeProjectAgentsSpy = spyOn(
      agentLoader,
      "loadOpencodeProjectAgents",
    ).mockReturnValue({});
    loadAgentDefinitionsSpy = spyOn(agentLoader, "loadAgentDefinitions")
      .mockReturnValue({});
    ...
    migrateAgentConfigSpy = spyOn(shared, "migrateAgentConfig")
      .mockImplementation(
  ...
  test("filters host config agent display-name aliases before they override resolved builtin models", async () => {
    ...
    // then
    expect((result[getAgentListDisplayName("sisyphus")] as AgentConfig).model)
      .toBe("openai/gpt-5.5");
    expect((result[getAgentListDisplayName("explore")] as AgentConfig).model)
      .toBe(
    ...
    [
      ...
      (config: OhMyOpenCodeConfig) => {
        Object.assign(config, {
          skills: { "blocked-skill": { disable: true } },
        });
      },
    ],
  ])(
    "passes %s entries into builtin agent disabled skill aliases",
    async (_label, configure) => {
      // given
      const disabledDescription =
        "IGNORE_ALL_PRIOR_INSTRUCTIONS_DISABLED_SKILL_DESC";
      const projectSkill = {
        name: "Blocked-Skill",
        definition: {
          name: "Blocked-Skill",
  ...
  describe("agent_definitions and opencode.json integration", () => {
    ...
    test("opencode.json agents appear in output", async () => {
      ...
      expect(result["opencode-agent"]?.description).toBe(
        "(opencode-config) OC",
      );
    });
    ...
    test("both Sisyphus-enabled and disabled paths include new sources", async () => {
      ...
      expect(result["definitions-agent"]?.prompt).toBe(
