...
} from "../features/opengateway-provider";
...
describe("config handler OpenGateway provider injection", () => {
  ...
  beforeEach(async () => {
    mock.restore();
    tempDataDir = mkdtempSync(
      path.join(tmpdir(), "opengateway-config-handler-"),
    );
    ...
    spyOn(shared, unsafeTestValue("readConnectedProvidersCache"))
    ...
    ({ createConfigHandler } = await import(
      `./config-handler?opengateway=${Date.now()}-${Math.random()}`
    ));
  });
  ...
  test("registers the provider before provider model limits are harvested", async () => {
    ...
    const cachedOpenGatewayKeys = [
      ...modelCacheState.modelContextLimitsCache.keys(),
    ].filter((key) => key.startsWith(`${OPENGATEWAY_PROVIDER_ID}/`));
