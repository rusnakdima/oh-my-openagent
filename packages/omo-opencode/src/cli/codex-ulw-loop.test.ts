...
describe("Codex ulw-loop routing", () => {
  test("prefers the Codex-local omo-ulw-loop bin so a global omo can reach ulw-loop without recursion", () => {
    ...
    expect(command).toEqual({
      executable: join(binDir, "omo-ulw-loop"),
      argsPrefix: [],
    });
  });
