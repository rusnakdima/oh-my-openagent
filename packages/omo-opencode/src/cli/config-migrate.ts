...
function reports(
  result: ReturnType<typeof runOpenCodeStartupMigration>,
  ...
  return result.results.map((entry) => ({
    ...
    ...(entry.preview === undefined ? {} : {
      targetPath: entry.preview.targetPath,
      transform: entry.preview.transform,
    }),
  }));
}

function printText(
  report: readonly MigrationReport[],
  ...
  for (const entry of report) {
    ...
    if (entry.transform !== undefined) {
      output(`transform: ${JSON.stringify(entry.transform, null, 2)}`);
    }
    if (entry.movePlan.length > 0) {
      output(`move-plan: ${JSON.stringify(entry.movePlan, null, 2)}`);
    }
    for (const diagnostic of entry.diagnostics) {
      output(`conflict: ${diagnostic}`);
    }
  }
}
...
export function runConfigMigrate(options: ConfigMigrateOptions = {}): number {
  ...
  if (options.json) {
    ...
    if (report.every((entry) => entry.status === "skipped")) {
