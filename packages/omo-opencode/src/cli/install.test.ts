...
describe("install CLI - binary check behavior", () => {
  ...
  test("non-TUI mode: should show warning but continue when OpenCode binary not found", async () => {
    ...
    isOpenCodeInstalledSpy = spyOn(configManager, "isOpenCodeInstalled")
      .mockResolvedValue(false);
    getOpenCodeVersionSpy = spyOn(configManager, "getOpenCodeVersion")
      .mockResolvedValue(null);

    // given mock npm fetch
    globalThis.fetch = unsafeTestValue<typeof fetch>(
      mock(() =>
        Promise.resolve({
          ok: true,
  ...
  test("non-TUI mode: should create opencode.json with plugin even when binary not found", async () => {
    ...
    expect(config.plugin.some((p: string) => p.includes("oh-my-openagent")))
      .toBe(true);
    expect(config.plugin.some((p: string) => p.includes("oh-my-opencode")))
      .toBe(false);
