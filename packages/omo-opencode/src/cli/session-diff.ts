import {
  ...
  readSessionMessages,
  readSessionTodos,
} from "../tools/session-manager/storage";
...
export async function runSessionDiff(
  ...
  try {
    const [messagesA, messagesB, todosA, todosB, infoA, infoB] = await Promise
      .all([
    ...
    const result: SessionDiffResult = {
      ...
      timeRange: {
        aStart: firstA?.time
          ? new Date(firstA.time.created).toISOString()
          : undefined,
        aEnd: lastA?.time
          ? new Date(lastA.time.created).toISOString()
          : undefined,
        bStart: firstB?.time
          ? new Date(firstB.time.created).toISOString()
          : undefined,
        bEnd: lastB?.time
          ? new Date(lastB.time.created).toISOString()
          : undefined,
      },
    };
...
function formatDiff(diff: SessionDiffResult): void {
  ...
  console.log(
    `  ${diff.sessionA}: ${
      color.yellow(diff.messageCount.a.toString())
    } messages`,
  );
  console.log(
    `  ${diff.sessionB}: ${
      color.yellow(diff.messageCount.b.toString())
    } messages`,
  );
  console.log(
    `  Diff: ${diff.messageCount.diff >= 0 ? "+" : ""}${
      color.yellow(diff.messageCount.diff.toString())
    }`,
  );
  console.log();

  if (
    diff.todoChanges.added.length > 0 ||
    diff.todoChanges.completed.length > 0 ||
    diff.todoChanges.cancelled.length > 0 || diff.todoChanges.removed.length > 0
  ) {
    console.log(color.bold("Todos:"));
    if (diff.todoChanges.added.length > 0) {
      console.log(
        `  ${color.green("+ Added:")} ${diff.todoChanges.added.length}`,
      );
      diff.todoChanges.added.forEach((t) =>
        console.log(`    ${color.green("+")} ${t}`)
      );
    }
    if (diff.todoChanges.completed.length > 0) {
      console.log(
        `  ${color.cyan("✓ Completed:")} ${diff.todoChanges.completed.length}`,
      );
      diff.todoChanges.completed.forEach((t) =>
        console.log(`    ${color.cyan("✓")} ${t}`)
      );
    }
    if (diff.todoChanges.cancelled.length > 0) {
      console.log(
        `  ${color.red("× Cancelled:")} ${diff.todoChanges.cancelled.length}`,
      );
      diff.todoChanges.cancelled.forEach((t) =>
        console.log(`    ${color.red("×")} ${t}`)
      );
    }
    if (diff.todoChanges.removed.length > 0) {
      console.log(
        `  ${color.red("- Removed:")} ${diff.todoChanges.removed.length}`,
      );
      diff.todoChanges.removed.forEach((t) =>
        console.log(`    ${color.red("-")} ${t}`)
      );
    }
    console.log();
  ...
  if (
    diff.agentsUsed.common.length > 0 || diff.agentsUsed.uniqueToA.length > 0 ||
    diff.agentsUsed.uniqueToB.length > 0
  ) {
    console.log(color.bold("Agents:"));
    console.log(
      `  ${color.green("Common:")} ${
        diff.agentsUsed.common.join(", ") || "(none)"
      }`,
    );
    if (diff.agentsUsed.uniqueToA.length > 0) {
      console.log(
        `  ${color.yellow(`${diff.sessionA} only:`)} ${
          diff.agentsUsed.uniqueToA.join(", ")
        }`,
      );
    }
    if (diff.agentsUsed.uniqueToB.length > 0) {
      console.log(
        `  ${color.yellow(`${diff.sessionB} only:`)} ${
          diff.agentsUsed.uniqueToB.join(", ")
        }`,
      );
    }
    console.log();
  ...
  if (diff.timeRange.aStart || diff.timeRange.bStart) {
    ...
    if (diff.timeRange.aStart) {
      console.log(
        `  ${diff.sessionA}: ${diff.timeRange.aStart}${
          diff.timeRange.aEnd ? ` → ${diff.timeRange.aEnd}` : ""
        }`,
      );
    }
    if (diff.timeRange.bStart) {
      console.log(
        `  ${diff.sessionB}: ${diff.timeRange.bStart}${
          diff.timeRange.bEnd ? ` → ${diff.timeRange.bEnd}` : ""
        }`,
      );
    }
  }
