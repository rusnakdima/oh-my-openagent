...
export function formatDefault(result: DoctorResult): string {
  ...
  if (allIssues.length === 0) {
    if (result.target === "codex" && result.codex) {
      ...
      const packageVer = result.codex.packageVersion ??
        result.codex.installerVersion;
      lines.push(
        ` ${color.green(SYMBOLS.check)} ${
          color.green(
            `LazyCodex OK (codex ${codex} · omo ${pluginVer} · ${packageName} ${packageVer})`,
          )
        }`,
      );
      return lines.join("\n");
    ...
    const totalStr = `${issueCount + warnCount} ${
      issueCount + warnCount === 1 ? "issue" : "issues"
    }`;
