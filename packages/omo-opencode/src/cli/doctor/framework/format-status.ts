...
export function formatStatus(result: DoctorResult): string {
  ...
  if (result.target === "codex" && result.codex) {
    ...
    lines.push(
      `  Config     ${result.codex.configPath} ${
        result.codex.config.pluginEnabled
          ? color.green("(enabled)")
          : color.red("(disabled)")
      }`,
    );
    ...
    lines.push(
      `  Agents     ${
        result.codex.agents.length > 0
          ? result.codex.agents.join(" · ")
          : "none"
      }`,
    );
    return lines.join("\n");
  ...
  lines.push(
    ` ${padding}Tools      LSP ${lspMark} ${lspText} · AST-Grep ${astGrepMark} · gh ${ghMark}${
      ghUser ? ` (${ghUser})` : ""
    }`,
  );
