...
describe("spawnWithTimeout", () => {
  ...
  describe("#given a nonexistent command", () => {
    ...
    it("does not consume process streams more than once", async () => {
      // given
      const unreadableCommand = process.platform === "win32"
        ? ["cmd", "/c", "exit", "9009"]
        : ["false"];

      // when
      const result = await spawnWithTimeout(unreadableCommand, {
        stdout: "pipe",
        stderr: "pipe",
      }, 2000);
  ...
  describe("#given spawn throws a non-Error value", () => {
    ...
    it("rethrows the unknown value", async () => {
      ...
      spyOn(spawnWithWindowsHideModule, "spawnWithWindowsHide")
        .mockImplementation(() => {
          throw unknownFailure;
        });

      // when
      const result = await spawnWithTimeout(["test-command"], {
        stdout: "pipe",
        stderr: "pipe",
      }).then(
        () => "resolved",
  ...
  describe("#given a spawned process exposes an already-used pipe", () => {
    ...
    it("treats the pipe as empty output", async () => {
      ...
      spyOn(spawnWithWindowsHideModule, "spawnWithWindowsHide")
        .mockImplementation(() => spawnedProcess);
