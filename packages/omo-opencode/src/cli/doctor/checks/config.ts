...
import {
  findLegacyConfigLeftovers,
  legacyConfigLeftoverWarning,
} from "./legacy-config-leftovers";
...
function collectModelResolutionIssues(config: OmoConfig): DoctorIssue[] {
  ...
  for (const invalidAgent of invalidAgentOverrides) {
    issues.push({
      ...
      description:
        `Override '${invalidAgent.userOverride}' must be in provider/model format.`,
      severity: "warning",
  ...
  for (const invalidCategory of invalidCategoryOverrides) {
    issues.push({
      ...
      description:
        `Override '${invalidCategory.userOverride}' must be in provider/model format.`,
      severity: "warning",
  ...
  if (availableModels.cacheExists) {
    ...
    if (unknownProviders.length > 0) {
      ...
      issues.push({
        ...
        description: `Provider(s) not found in OpenCode model cache: ${
...
export async function checkConfig(): Promise<CheckResult> {
  ...
  if (!validation.exists) {
    return {
      ...
      message: issues.length > 0
        ...
        : "No custom config found; defaults are used",
      details: undefined,
      issues,
    };
  }
  ...
  if (!validation.valid) {
    ...
    return {
      name: CHECK_NAMES[CHECK_IDS.CONFIG],
      status: "fail",
      message: `Configuration invalid (${issues.length} issue${
        issues.length > 1 ? "s" : ""
      })`,
