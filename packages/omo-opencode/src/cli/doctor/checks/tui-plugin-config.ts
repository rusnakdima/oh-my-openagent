...
function fileEntryPackageJsonPath(entry: string): string {
  ...
  // If the path points to a file (e.g. dist/index.js), look for package.json in the parent directory
  if (path.endsWith("/index.js") || path.endsWith("\\index.js")) {
    return join(dirname(path), "package.json");
  }
...
function packageJsonExportsTui(pkgJsonPath: string): boolean | null {
  ...
  try {
    ...
    if (
      parsed.exports == null || typeof parsed.exports !== "object" ||
...
export function isServerPluginEntry(entry: unknown): entry is string {
  ...
  if (
    typeof entry === "string" && entry.startsWith("file:") &&
    isOurFilePluginEntry(entry)
  ) return true;
  return false;
}
...
export function isTuiPluginEntry(entry: unknown): boolean {
  return typeof entry === "string" &&
    (isPackagePluginEntry(entry) ||
      (entry.startsWith("file:") && isOurFilePluginEntry(entry)));
}
...
function isCanonicalNamedTuiPluginEntry(entry: unknown): boolean {
  const canonicalPrefix = `${PLUGIN_NAME}/${TUI_SUBPATH}`;
  return typeof entry === "string" &&
    (entry === canonicalPrefix || entry.startsWith(`${canonicalPrefix}@`));
}
...
export function detectServerPluginRegistration(): ServerPluginInfo {
  ...
  if (!configPath) {
    return {
      ...
      configPath: null,
      entry: null,
      packageExportsTui: null,
    };
  }

  try {
    const parsed = parseJsonc<OpenCodeConfigShape>(
    ...
    return {
      ...
      packageExportsTui: serverEntry === undefined
        ? null
        : packageExportsTuiForServerEntry(serverEntry),
    };
  } catch (error) {
...
export function detectTuiPluginRegistration(): TuiPluginInfo {
  ...
  try {
    const parsed = parseJsonc<TuiConfigShape>(
      readFileSync(tuiJsonPath, "utf-8"),
    );
    const plugins = parsed.plugin ?? [];
    return {
...
export async function checkTuiPluginConfig(): Promise<CheckResult> {
  ...
  if (tui.hasNamedTuiEntry) {
    ...
    issues.push({
      ...
      fix:
        `Remove "${PLUGIN_NAME}/${TUI_SUBPATH}" and "${LEGACY_PLUGIN_NAME}/${TUI_SUBPATH}" from the "plugin" array in ${tui.configPath}, then add "${desiredEntry}".`,
  ...
  if (
    server.registered && server.packageExportsTui === false &&
    tui.hasPackageTuiEntry
  ) {
    issues.push({
      title: "TUI plugin package does not expose ./tui",
      description:
        `The installed ${
          server.entry ?? PLUGIN_NAME
        } package registered in opencode.json does not export "${TUI_EXPORT_SUBPATH}", ` +
        "but tui.json contains the package entry for TUI loading.",
      fix: `Remove "${
        server.entry ?? PLUGIN_NAME
      }" from the "plugin" array in ${tui.configPath}, or update the installed package to a version that exports "${TUI_EXPORT_SUBPATH}".`,
  ...
  if (server.registered && !tui.registered) {
    if (server.packageExportsTui === false) {
      return {
        ...
        status: "pass",
        message:
          "Server plugin registered; TUI subpath not shipped by this package version",
        details: details.length > 0 ? details : undefined,
        issues,
      };
    }
    ...
    issues.push({
      ...
      description:
        ...
        `("${
          server.entry ?? PLUGIN_NAME
        }") is missing from tui.json. The Roles · ` +
        "Models sidebar section and TUI-only commands will not appear.",
      fix:
        "Re-run the installer (`npx oh-my-openagent install`) to auto-write tui.json, " +
        `or add "${
          server.entry ?? PLUGIN_NAME
        }" to the "plugin" array in ${tui.configPath}.`,
      affects: ["TUI sidebar", "TUI commands"],
      severity: "warning",
    });
    return {
  ...
  if (!server.registered && tui.registered) {
    issues.push({
      ...
      fix:
        "Re-run the installer (`npx oh-my-openagent install`) to auto-write opencode.json, " +
        `or add "${PLUGIN_NAME}" to the "plugin" array in ${
          server.configPath ?? "opencode.json"
        }.`,
