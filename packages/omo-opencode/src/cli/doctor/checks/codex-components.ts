...
import { type CodexDoctorDeps, gatherCodexSummary } from "./codex";
...
const REINSTALL_FIX =
  "Reinstall: npx lazycodex-ai install (or upgrade: codex plugin marketplace upgrade sisyphuslabs)";
...
export async function checkCodexComponents(
  deps: CodexComponentsDoctorDeps = {},
): Promise<CheckResult> {
  const env = deps.env ?? process.env;
  const platform = deps.platform ?? process.platform;
  const arch = deps.arch ?? process.arch;
  const codexHome = resolve(
    deps.codexHome ?? env["CODEX_HOME"] ?? join(homedir(), ".codex"),
  );
  ...
  const { referencedCount, broken } = await auditBundleTargets(
    summary.pluginRoot,
  );
  details.push(
    broken.length === 0
  ...
  for (const target of broken) {
    issues.push({
      ...
      description: `Referenced by ${target.referencedBy} but ${
        target.reason === "missing" ? "missing from" : "zero bytes in"
      } the installed plugin bundle.`,
      fix: REINSTALL_FIX,
      severity: "error",
      affects: [
        target.referencedBy.endsWith(".mcp.json")
          ? "MCP servers"
          : "plugin hooks",
      ],
    });
  ...
  const sg = findSgBinarySync({
    ...
    ...(deps.sgRunVersionProbeSync === undefined
      ? {}
  ...
  if (sg === null) {
    ...
    issues.push({
      ...
      description:
        `sg was not found via the ${SG_PATH_ENV_KEY} override, the Codex runtime dir (${runtimeSgPath}), or PATH. The ast-grep skill runs degraded until sg is provisioned.`,
      fix:
        "Start a Codex session so LazyCodex bootstrap can provision the ast-grep skill runtime, then rerun: npx lazycodex-ai doctor (or omo doctor).",
      severity: "warning",
    ...
    details.push(
      `ast_grep: ok (${
        describeSgSource(sg, env, runtimeSgDir, platform)
      }: ${sg})`,
    );
  }
  ...
  return {
    ...
    status,
    message: status === "pass"
      ? "Codex component checks passed"
      : `${issues.length} Codex component issue(s) detected`,
    details,
    issues,
  };
}
  ...
  for (const hookManifestPath of await findHookManifestPaths(pluginRoot)) {
    ...
    for (const command of commands) {
      for (const relativePath of extractPluginRootPaths(command)) {
        referencedCount += 1;
        const baseRoot = relativePath.startsWith("components/")
          ? pluginRoot
          : hookPluginRoot;
        recordBrokenTarget(
          broken,
          await classifyBundleTarget(pluginRoot, baseRoot, relativePath, false),
          {
...
function recordBrokenTarget(
  ...
  const entry: BundleTargetIssue = {
    ...classified,
    referencedBy: origin.referencedBy,
  };
  if (
    broken.some((existing) =>
...
async function classifyBundleTarget(
  ...
  const bundleRootPrefix = bundleRootPath.endsWith(sep)
    ? bundleRootPath
    : `${bundleRootPath}${sep}`;
  if (
    targetPath !== bundleRootPath && !targetPath.startsWith(bundleRootPrefix)
  ) {
  ...
  if (size === undefined) {
    return {
      relativePath: normalizeRelative(bundleRoot, targetPath),
      reason: "missing",
    };
  }
  if (size === 0) {
...
function describeSgSource(
  ...
  if (sgPath === join(runtimeSgDir, sgBinaryName(platform))) {
    return "runtime dir";
  }
  return "PATH";
}

async function readBootstrapStateSummary(
  codexHome: string,
): Promise<BootstrapStateSummary | null> {
  ...
  const lastStatus =
    raw["lastStatus"] === "success" || raw["lastStatus"] === "degraded"
      ? raw["lastStatus"]
      : null;
  return {
    completedForVersion: typeof raw["completedForVersion"] === "string"
      ? raw["completedForVersion"]
      : null,
    lastStatus,
...
function parseDegradedEntries(
  value: unknown,
): readonly BootstrapDegradedEntry[] {
...
function bootstrapDetails(
  ...
  const lines = [
    `bootstrap: ${statusLabel}@${completed} (lastStatus=${state.lastStatus})`,
  ];
  lines.push(...degradedDetailLines(state.degraded));
  return lines;
}

function degradedDetailLines(
  entries: readonly BootstrapDegradedEntry[],
): string[] {
  return entries.map(
    (entry) =>
      `degraded component=${entry.component} reason=${entry.reason}${
