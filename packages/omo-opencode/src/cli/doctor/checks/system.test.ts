...
const mockGetLatestPluginVersion = mock(async (
...
describe("system check", () => {
  ...
  describe("#given malformed config JSONC", () => {
    it("marks the config invalid without throwing", async () => {
      ...
      //#when
      const { gatherSystemInfo: freshGatherSystemInfo } = await import(
        `./system?malformed=${Date.now()}`
      );
  ...
  describe("#given cache directory contains spaces", () => {
    it("uses a quoted cache directory in mismatch fix command", async () => {
      ...
      //#then
      const mismatchIssue = result.issues.find((issue) =>
        issue.title === "Loaded plugin version mismatch"
      );
    ...
    it("includes Bun postinstall trust guidance in the update fix command", async () => {
      ...
      //#then
      const outdatedIssue = result.issues.find((issue) =>
        issue.title === "Loaded plugin is outdated"
      );
  ...
  describe("#given OpenCode plugin entry uses legacy package name", () => {
    it("adds a warning for a bare legacy entry", async () => {
      ...
      //#then
      const legacyEntryIssue = result.issues.find((issue) =>
        issue.title === "Using legacy package name"
      );
