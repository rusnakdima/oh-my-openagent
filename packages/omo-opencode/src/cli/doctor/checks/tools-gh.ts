  ...
  try {
    ...
    if (result.exitCode === 0) {
      ...
      return {
        ...
        scopes: scopesMatch?.[1]?.split(/,\s*/).map((scope) =>
          scope.trim()
        ).filter(Boolean) ?? [],
        error: null,
      };
...
export async function getGhCliInfo(
  dependencies: GhCliDependencies = {},
): Promise<GhCliInfo> {
...
export async function fixGhAuth(): Promise<FixResult> {
  return {
    success: false,
    message:
      "gh auth login is interactive — run 'gh auth login' manually to authenticate",
  };
}
