...
function createTaggedInstallCandidates(rootDir: string): InstallCandidate[] {
  ...
  for (const entryName of readdirSync(packagesDir).sort()) {
    const packageName = ACCEPTED_PACKAGE_NAMES.find((name) =>
      entryName.startsWith(`${name}@`)
    );
...
export function getLoadedPluginVersion(): LoadedVersionInfo {
  ...
  const resolvedFallback = candidateExists
    ? null
    : resolveInstalledPackageJsonPath();
  const installedPackagePath = resolvedFallback?.packageJsonPath ??
    candidateInstalledPath;
  const resolvedPackageName = resolvedFallback?.packageName ??
