...
import {
  ...
  type CodexComponentsDoctorDeps,
...
const MCP_TARGETS = [
  ...
  "components/context7/dist/cli.js",
...
async function createInstalledFixture(
  ...
  await writeFile(
    ...
    JSON.stringify({ name: "omo", version: PLUGIN_VERSION }),
  );
  await writeFile(
    join(codexHome, "config.toml"),
    ['[plugins."omo@sisyphuslabs"]', "enabled = true", ""].join("\n"),
  );
            ...
            hooks: [
              {
                type: "command",
                command:
                  'node "${PLUGIN_ROOT}/components/telemetry/dist/cli.js" hook',
                commandWindows:
                  'powershell.exe -File "${PLUGIN_ROOT}\\components\\bootstrap\\scripts\\bootstrap.ps1"',
              },
              {
                type: "command",
                command:
                  'node "${PLUGIN_ROOT}/components/ultrawork/dist/cli.js" hook',
              },
              {
                type: "command",
                command: 'node "${PLUGIN_ROOT}/scripts/auto-update.mjs"',
              },
            ],
          },
        ],
    ...
    JSON.stringify({
      mcpServers: {
        codegraph: {
          command: "node",
          args: [
            join(pluginRoot, "components", "codegraph", "dist", "serve.js"),
          ],
        },
        context7: {
          command: "node",
          args: ["./components/context7/dist/cli.js", "mcp"],
        },
        lsp: {
          command: "node",
  ...
  for (const target of [...HOOK_TARGETS, WINDOWS_HOOK_TARGET, ...MCP_TARGETS]) {
    ...
    await writeBundleFile(
      pluginRoot,
      target,
      zeroed.has(target) ? "" : "// installed component payload\n",
    );
  }

  const runtimeSgPath = join(
    codexHome,
    "runtime",
    "ast-grep",
    `${TEST_PLATFORM}-${TEST_ARCH}`,
    "sg",
  );
  ...
  const bootstrapState = options.bootstrapState === undefined
    ? {
      ...
      lastAttemptAt: 1_770_000_000_000,
      lastStatus: "success",
      degraded: [],
    }
...
function buildDeps(
  fixture: Fixture,
  overrides: Partial<CodexComponentsDoctorDeps> = {},
): CodexComponentsDoctorDeps {
  return {
...
describe("codex components doctor check", () => {
  test("#given a complete installed plugin #when checking components #then passes with sg source and completed bootstrap", async () => {
    ...
    expect(result.details).toContain(
      `ast_grep: ok (runtime dir: ${fixture.runtimeSgPath})`,
    );
    expect(result.details).toContain(
      `bootstrap: completed@${PLUGIN_VERSION} (lastStatus=success)`,
    );
    expect(
      result.details?.some((detail) => detail.startsWith("dist targets: ok")),
    ).toBe(true);
  });
  ...
  test("#given a missing hook dist target #when checking components #then fails naming the relative path", async () => {
    ...
    const issue = result.issues.find((entry) =>
      entry.title.includes("components/telemetry/dist/cli.js")
    );
  ...
  test("#given a zero-byte mcp dist target #when checking components #then fails and reports the zero-byte state", async () => {
    // given
    const fixture = await createInstalledFixture({
      zeroByteTargets: ["components/context7/dist/cli.js"],
    });
    ...
    expect(result.status).toBe("fail");
    const issue = result.issues.find((entry) =>
      entry.title.includes("components/context7/dist/cli.js")
    );
    expect(issue).toBeDefined();
    expect(issue?.severity).toBe("error");
  ...
  test("#given an absolute installed mcp dist target is missing #when checking components #then fails instead of treating the rewrite as drift", async () => {
    // given
    const fixture = await createInstalledFixture({
      omitTargets: ["components/codegraph/dist/serve.js"],
    });
    ...
    expect(result.status).toBe("fail");
    const issue = result.issues.find((entry) =>
      entry.title.includes("components/codegraph/dist/serve.js")
    );
    expect(issue).toBeDefined();
    expect(issue?.severity).toBe("error");
    expect(issue?.description).toContain(".mcp.json");
  ...
  test("#given a missing commandWindows target #when checking components #then the windows hook path is validated too", async () => {
    // given
    const fixture = await createInstalledFixture({
      omitTargets: [WINDOWS_HOOK_TARGET],
    });
    ...
    expect(result.status).toBe("fail");
    expect(
      result.issues.some((entry) => entry.title.includes(WINDOWS_HOOK_TARGET)),
    ).toBe(true);
  });
  ...
  test("#given an OMO_AST_GREP_SG_PATH override #when checking components #then reports the env override source", async () => {
    ...
    expect(result.status).toBe("pass");
    expect(result.details).toContain(
      `ast_grep: ok (env override OMO_AST_GREP_SG_PATH: ${overridePath})`,
    );
  });
  ...
  test("#given no bootstrap state file #when checking components #then surfaces bootstrap pending as info detail without issues", async () => {
    ...
    expect(
      result.details?.some((detail) =>
        detail.includes("bootstrap pending — start a Codex session")
      ),
    ).toBe(true);
    expect(result.details?.some((detail) => detail.includes("state none")))
      .toBe(true);
  });
  ...
  test("#given completedForVersion behind the installed version #when checking components #then reports bootstrap pending with both versions", async () => {
    ...
    // then
    expect(result.status).toBe("pass");
    const pending = result.details?.find((detail) =>
      detail.includes("bootstrap pending — start a Codex session")
    );
  ...
  test("#given a degraded bootstrap ledger #when checking components #then surfaces every degraded entry verbatim", async () => {
    ...
    const fixture = await createInstalledFixture({
      bootstrapState: {
        ...
        degraded: [
          {
            component: "ast_grep",
            reason: "download failed: checksum mismatch",
    ...
    // then
    expect(result.details).toContain(
      `bootstrap: degraded@${PLUGIN_VERSION} (lastStatus=degraded)`,
    );
    expect(result.details).toContain(
      "degraded component=ast_grep reason=download failed: checksum mismatch hint=npx lazycodex-ai doctor",
    );
    expect(result.details).toContain(
      "degraded component=omo-agent-toolkit reason=payload has no dist/cli",
    );
  });
    ...
    await writeBundleFile(
      ...
      JSON.stringify({
        mcpServers: {
          local_dev: {
            command: "node",
            args: ["../../../../dev-packages/custom-mcp/dist/cli.js", "mcp"],
          },
        },
      }),
    );
  ...
  test("#given both a missing dist and missing sg #when checking components #then fail status wins and both surfaces report", async () => {
    ...
    expect(result.issues.map((issue) => issue.severity).sort()).toEqual([
      "error",
      "warning",
    ]);
  });
