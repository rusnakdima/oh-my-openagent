...
describe("config check", () => {
  describe("checkConfig", () => {
      ...
      try {
        ...
        expect(
          result.issues.some((issue) =>
            issue.title === "Invalid category override: deep"
          ),
        ).toBe(true);
    ...
    it("reports invalid ancestor plugin config from the current directory", async () => {
      ...
      const testRootDir = join(
        tmpdir(),
        `omo-doctor-layered-config-${Date.now()}-${
          Math.random().toString(36).slice(2)
        }`,
      );
      const projectDir = join(testRootDir, "project");
      ...
      try {
        ...
        expect(
          result.issues.some((issue) =>
            issue.description.includes("agents.sisyphus.model")
          ),
        ).toBe(true);
      } finally {
        process.chdir(originalCwd);
    ...
    it("does not flag configured custom providers as unavailable when they exist in opencode.json", async () => {
      ...
      const testRootDir = join(
        tmpdir(),
        `omo-doctor-custom-provider-${Date.now()}-${
          Math.random().toString(36).slice(2)
        }`,
      );
      ...
      try {
        ...
        writeFileSync(
          ...
          JSON.stringify(
            {
              "[opencode]": {
                agents: { sisyphus: { model: "kiro/claude-opus-4-6" } },
              },
            },
            null,
            2,
        ...
        const result = await config.checkConfig();
        const providerIssue = result.issues.find((issue) =>
          issue.title === "Model override uses unavailable provider"
        );
    ...
    it("does not flag reasoningEffort: 'max' as an invalid configuration", async () => {
      ...
      const testRootDir = join(
        tmpdir(),
        `omo-doctor-reasoning-effort-max-${Date.now()}-${
          Math.random().toString(36).slice(2)
        }`,
      );
      const projectDir = join(testRootDir, "project");
      ...
      try {
        ...
        //#when running the consolidated config check
        const result = await config.checkConfig();
        const invalidConfig = result.issues.find((issue) =>
