...
export function findLegacyConfigLeftovers(
  options: LegacyConfigLeftoverOptions,
): readonly string[] {
...
export function legacyConfigLeftoverWarning(
  paths: readonly string[],
): DoctorIssue | undefined {
  if (paths.length === 0) return undefined;
  return {
    affects: ["configuration migration"],
    description: `Legacy configuration remains at ${
      paths.join(", ")
    }. It is not part of the unified OMO config chain.`,
    fix:
      "Run `oh-my-openagent config migrate` to move it into ~/.omo/omo.jsonc.",
    severity: "warning",
