...
import {
  type CodexInstallationDetection,
  detectCodexInstallation,
} from "@oh-my-opencode/omo-codex/install/codex-installation-detection";
import { resolveCodexInstallerBinDir } from "@oh-my-opencode/omo-codex/install/install-codex";
import {
  parseHookStateHeaderKey,
  splitTomlSections,
} from "@oh-my-opencode/omo-codex/install/codex-config-toml-sections";
...
export async function gatherCodexSummary(
  deps: CodexDoctorDeps = {},
): Promise<CodexDoctorSummary> {
  ...
  const detection =
    await (deps.detectCodexInstallation ?? detectCodexInstallation)();
  const pluginRoot = await resolveInstalledPluginRoot(codexHome);
  const manifest = pluginRoot === null
    ? null
    : await readJson(join(pluginRoot, ".codex-plugin", "plugin.json"));
  const installSnapshot = pluginRoot === null
    ? null
    : await readJson(join(pluginRoot, "lazycodex-install.json"));
  ...
  return {
    ...
    codexAppId: detection.found && "appId" in detection
      ? detection.appId
      : null,
    marketplaceName: MARKETPLACE_NAME,
    pluginName: PLUGIN_NAME,
    pluginVersion,
    pluginVersionStamped: pluginVersion !== null &&
...
export async function checkCodex(
  ...
  return {
    ...
    message: status === "pass"
      ? "Codex checks passed"
      : `${issues.length} Codex issue(s) detected`,
    details: [
      ...
      `Plugin: ${summary.pluginName}@${summary.pluginVersion ?? "unknown"}${
        summary.pluginVersionStamped ? "" : " (placeholder, not stamped)"
      }`,
      `Distribution: ${summary.packageName ?? "unknown"}@${
        summary.packageVersion ?? "unknown"
      }`,
      `Config: ${summary.configPath}`,
      `Enabled plugin: ${
        summary.config.pluginEnabled ? "omo@sisyphuslabs" : "missing"
      }`,
      `Companion plugin: ${formatCompanionPluginStatus(summary.config)}`,
      `Linked bins: ${
        summary.linkedBins.length > 0 ? summary.linkedBins.join(", ") : "none"
      }`,
      `Agents: ${
        summary.agents.length > 0 ? summary.agents.join(", ") : "none"
      }`,
    ],
    issues,
  };
...
function buildCodexIssues(summary: CodexDoctorSummary): DoctorIssue[] {
  ...
  if (summary.pluginRoot === null) {
    ...
    issues.push({
      ...
      description:
        `The installed OMO Codex plugin reports the placeholder version ${
          summary.pluginVersion ?? "unknown"
        }${
          summary.packageVersion === null
            ? " and no distribution snapshot was found"
            : ""
        }. This usually means it was installed through the Codex app plugin UI instead of the CLI installer, so its version does not reflect the real release. Your CLI is oh-my-openagent ${summary.installerVersion}.`,
      fix: "Run: npx lazycodex-ai install",
      severity: "warning",
  ...
  if (
    summary.pluginRoot !== null &&
    !summary.linkedBins.includes("omo-agent-toolkit")
  ) {
    issues.push({
  ...
  if (!summary.config.pluginEnabled) {
    issues.push({
      ...
      description:
        'Expected [plugins."omo@sisyphuslabs"] enabled = true in Codex config.',
      fix: "Run: npx lazycodex-ai install",
      severity: "error",
      affects: ["plugin loading"],
    });
  ...
  if (
    !summary.config.pluginsFeatureEnabled ||
    !summary.config.pluginHooksFeatureEnabled
  ) {
    issues.push({
  ...
  if (
    summary.config.pluginEnabled && hasCompanionLifecycleSurface(summary.config)
  ) {
    issues.push({
      ...
      fix:
        'If LazyCodex is your primary Codex workflow, set [plugins."codex@openai-codex"] enabled = false and remove stale [hooks.state."codex@openai-codex:..."] SessionStart/Stop entries if the warning remains.',
      severity: "warning",
...
async function resolveInstalledPluginRoot(
  ...
  const candidates = versions.filter((entry) => entry.isDirectory()).map((
    entry,
  ) => entry.name).sort(compareVersionsDescending);
  return candidates.length === 0
    ? null
    : join(pluginRoot, candidates[0] ?? DEFAULT_PLUGIN_VERSION);
}

async function readCodexConfigSummary(
  configPath: string,
  ...
  return {
    ...
    pluginEnabled: settingEnabled(
      sectionBody(content, 'plugins."omo@sisyphuslabs"'),
      "enabled",
    ),
    pluginsFeatureEnabled: featureEnabled(content, "plugins"),
    pluginHooksFeatureEnabled: featureEnabled(content, "plugin_hooks"),
    companionPluginEnabled: settingEnabled(
      sectionBody(content, `plugins.${JSON.stringify(COMPANION_PLUGIN_KEY)}`),
      "enabled",
    ),
    companionLifecycleHookStateEvents: readCompanionLifecycleHookStateEvents(
      content,
    ),
  };
}
...
async function readLinkedAgents(codexHome: string): Promise<readonly string[]> {
  ...
  return entries.filter((entry) => entry.isFile() || entry.isSymbolicLink())
    .map((entry) => basename(entry.name, ".toml")).sort();
}
...
function hasCompanionLifecycleSurface(config: CodexConfigSummary): boolean {
  return config.companionPluginEnabled ||
    config.companionLifecycleHookStateEvents.length > 0;
}

function companionLifecycleSurfaceDescription(
  ...
  const pluginText = config.companionPluginEnabled
    ? `${COMPANION_PLUGIN_KEY} is enabled`
    : `${COMPANION_PLUGIN_KEY} appears disabled`;
...
function formatCompanionPluginStatus(config: CodexConfigSummary): string {
  if (config.companionPluginEnabled) {
    const suffix = config.companionLifecycleHookStateEvents.length > 0
      ? ` (${
        formatCompanionLifecycleEvents(config.companionLifecycleHookStateEvents)
      } hook trust)`
      : "";
    return `${COMPANION_PLUGIN_KEY} enabled${suffix}`;
  }
  if (config.companionLifecycleHookStateEvents.length > 0) {
    return `stale ${COMPANION_PLUGIN_KEY} ${
      formatCompanionLifecycleEvents(config.companionLifecycleHookStateEvents)
    } hook trust`;
  }
  return "none";
...
function readCompanionLifecycleHookStateEvents(
  content: string,
): readonly string[] {
  ...
  for (const section of splitTomlSections(content)) {
    ...
    const event = key === null
      ? null
      : companionLifecycleEventFromHookStateKey(key);
...
function companionLifecycleEventFromHookStateKey(key: string): string | null {
  ...
  return eventName !== undefined && COMPANION_LIFECYCLE_EVENTS.has(eventName)
    ? eventName
    : null;
}

function compareCompanionLifecycleEvents(left: string, right: string): number {
  return companionLifecycleEventOrder(left) -
    companionLifecycleEventOrder(right);
}
