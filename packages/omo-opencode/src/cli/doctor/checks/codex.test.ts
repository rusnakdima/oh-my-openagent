...
async function createPlatformBin(
  binDir: string,
  name: string,
  target: string,
): Promise<void> {
...
async function createInstalledCodexHome(): Promise<
  {
...
> {
  ...
  await mkdir(
    join(codexHome, ".tmp", "marketplaces", "sisyphuslabs", "plugins", "omo"),
    { recursive: true },
  );
  ...
  await writeFile(
    ...
    JSON.stringify({ name: "omo", version: "4.7.5" }),
  );
  await writeFile(
    join(pluginRoot, "lazycodex-install.json"),
    JSON.stringify({ packageName: "lazycodex-ai", version: "4.7.5" }),
  );
  await writeFile(
  ...
  await createPlatformBin(
    binDir,
    "omo-rules",
    join(pluginRoot, "components", "rules", "dist", "cli.js"),
  );
...
describe("codex doctor checks", () => {
  ...
  test("#given missing Codex config #when checking Codex doctor #then fails with install guidance", async () => {
    ...
    expect(result.issues.map((issue) => issue.title)).toContain(
      "Codex is not installed",
    );
    expect(result.issues.map((issue) => issue.title)).toContain(
      "OMO Codex plugin is not installed",
    );
    expect(result.issues.map((issue) => issue.title)).toContain(
      "Codex plugin is not enabled",
    );
    expect(result.issues.map((issue) => issue.title)).toContain(
      "LazyCodex marketplace is not configured",
    );
  });
  ...
  test("#given installed plugin without the omo-agent-toolkit runtime bin #when checking Codex doctor #then reports the missing canonical command", async () => {
    ...
    expect(result.status).toBe("fail");
    expect(result.issues.map((issue) => issue.title)).toContain(
      "omo-agent-toolkit runtime command is not linked",
    );
  });
  ...
  test("#given generated omo-agent-toolkit wrapper points at a deleted runtime target #when checking runtime wrapper #then warns with reinstall guidance", async () => {
    ...
    await writeFile(
      ...
      process.platform === "win32"
        ? [
          "@echo off",
          "rem OMO_GENERATED_RUNTIME_WRAPPER",
          `"%%BUN_BINARY%%" "${missingCliPath}" %%*`,
          "",
        ].join("\r\n")
        : [
          "#!/bin/sh",
          "# OMO_GENERATED_RUNTIME_WRAPPER",
          `exec "$BUN_BINARY" "${missingCliPath}" "$@"`,
          "",
        ].join("\n"),
    );
    ...
    const issue = result.issues.find((entry) =>
      entry.title === "omo-agent-toolkit runtime wrapper target is missing"
    );
  ...
  test("#given a marker-bearing legacy omo wrapper #when checking runtime wrapper #then warns with the self-cleaning reinstall command", async () => {
    ...
    const legacyPath = join(
      binDir,
      process.platform === "win32" ? "omo.cmd" : "omo",
    );
    await writeFile(
      legacyPath,
      `${
    ...
    expect(result.status).toBe("warn");
    const issue = result.issues.find((entry) =>
      entry.title === "Legacy omo runtime wrapper is still installed"
    );
    expect(issue?.severity).toBe("warning");
    expect(issue?.fix).toBe(
      "Run: npx --yes lazycodex-ai@latest install --no-tui",
    );
  });
  ...
  test("#given installed LazyCodex #when checking Codex doctor #then details include Codex-specific health surfaces", async () => {
    ...
    expect(result.details).toContain(
      "Linked bins: omo-agent-toolkit, omo-rules",
    );
  ...
  test("#given LazyCodex is primary and Codex Companion lifecycle hooks are configured #when checking Codex doctor #then warns without disabling anything", async () => {
    ...
    expect(result.details).toContain(
      "Companion plugin: codex@openai-codex enabled (SessionStart, Stop hook trust)",
    );
    const issue = result.issues.find((entry) =>
      entry.title ===
        "Codex Companion lifecycle hooks may conflict with LazyCodex"
    );
    expect(issue).toBeDefined();
    expect(issue?.severity).toBe("warning");
    ...
    expect(issue?.fix).toContain(
      '[plugins."codex@openai-codex"] enabled = false',
    );
  });
  ...
  test("#given an un-stamped placeholder plugin bundle with no install snapshot #when checking Codex doctor #then it warns and points at the installer version", async () => {
    ...
    await writeFile(
      join(pluginRoot, ".codex-plugin", "plugin.json"),
      JSON.stringify({ name: "omo", version: "0.1.0" }),
    );
    ...
    // then
    expect(result.status).toBe("warn");
    const stampIssue = result.issues.find((issue) =>
      issue.title === "Codex plugin bundle is not version-stamped"
    );
