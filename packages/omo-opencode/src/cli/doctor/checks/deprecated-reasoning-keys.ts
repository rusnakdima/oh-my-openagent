  ...
  for (const [key, child] of Object.entries(value)) {
    ...
    if (replacement !== undefined) {
      issues.push({
        ...
        fix:
          `Replace ${key} with ${replacement}, or run: oh-my-openagent config migrate`,
        severity: "warning",
        affects: [path],
      });
      continue;
    ...
    if (
      TUNING_CONTAINERS.has(key) || isHarnessBlock(key) || prefix.length > 0
    ) {
...
export async function checkDeprecatedReasoningKeys(): Promise<CheckResult> {
  ...
  return {
    ...
    ...(scanned.length > 0
      ? { details: scanned.map((path) => `Scanned: ${path}`) }
      : {}),
    issues,
  };
}
...
export async function fixDeprecatedReasoningKeys(): Promise<FixResult> {
  ...
  if (homeDir === undefined || homeDir.length === 0) {
    return {
      success: false,
      message: "Cannot migrate: no home directory available",
    };
  }
  ...
  const skipped =
    result.results.filter((r) =>
      r.status === "skipped" || r.status === "planned"
    ).length;

  if (fixedItems.length === 0) {
    return {
      success: true,
      message: `No deprecated keys to migrate (${skipped} skipped)`,
      fixed: [],
    };
  }
  const fixed = fixedItems.flatMap((r) =>
    r.preview?.backupMoves.map((m) => m.from) ?? []
  );
  return {
    success: true,
    message: `Migrated ${fixedItems.length} deprecated key(s)`,
    fixed,
  };
}
