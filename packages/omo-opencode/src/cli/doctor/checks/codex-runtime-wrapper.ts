...
export async function checkCodexRuntimeWrapper(
  deps: CodexRuntimeWrapperDoctorDeps = {},
): Promise<CheckResult> {
  ...
  const wrapperPath = join(
    binDir,
    platform === "win32" ? "omo-agent-toolkit.cmd" : "omo-agent-toolkit",
  );
  const legacyWrapperPath = join(
    binDir,
    platform === "win32" ? "omo.cmd" : "omo",
  );
  const [wrapper, legacyWrapper] = await Promise.all([
    readRuntimeWrapper(wrapperPath),
    readRuntimeWrapper(legacyWrapperPath),
  ]);
  ...
  if (wrapper?.includes(RUNTIME_WRAPPER_MARKER) === true) {
    ...
    if (targetPath !== null && !existsSync(targetPath)) {
      issues.push({
        ...
        description:
          `Generated omo-agent-toolkit runtime wrapper at ${wrapperPath} points to missing target ${targetPath}.`,
  ...
  if (legacyWrapper?.includes(RUNTIME_WRAPPER_MARKER) === true) {
    issues.push({
      ...
      description:
        `Generated legacy omo runtime wrapper remains at ${legacyWrapperPath} and may execute an outdated cached CLI.`,
      fix: `Run: ${REINSTALL_COMMAND}`,
      severity: "warning",
  ...
  return {
    ...
    message: issues.length > 0
      ? `${issues.length} Codex runtime wrapper issue(s) detected`
      : "Codex runtime wrapper checks passed",
