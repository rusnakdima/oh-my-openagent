...
function getLoadedPackageName(installedPackagePath: string): string {
  ...
  const packageName = nodeModulesIndex >= 0
    ? parts[nodeModulesIndex + 1]
    : undefined;
  return packageName !== undefined &&
      ACCEPTED_PACKAGE_NAMES.some((acceptedName) =>
        acceptedName === packageName
      )
    ? packageName
    : PUBLISHED_PACKAGE_NAME;
}
...
export async function gatherSystemInfo(
  ...
  const opencodeVersion = binaryInfo
    ...
    : null;
  const pluginVersion = pluginInfo.pinnedVersion ??
...
export async function checkSystem(
  ...
  if (
    ...
    issues.push({
      ...
      description:
        `Detected ${systemInfo.opencodeVersion}; required >= ${MIN_OPENCODE_VERSION}.`,
  ...
  if (!pluginInfo.registered) {
    ...
    issues.push({
      ...
      fix: "Clear the OpenCode plugin cache and reinstall:\n" +
  ...
  if (pluginInfo.entry && !pluginInfo.isLocalDev) {
    ...
    if (isLegacyName) {
      ...
      issues.push({
        ...
        description:
          `Your opencode.json references "${LEGACY_PLUGIN_NAME}" which has been renamed to "${PLUGIN_NAME}". The old name may stop working in a future release.`,
        fix:
          `Update your opencode.json plugin entry: "${pluginInfo.entry}" → "${suggestedEntry}"`,
        severity: "warning",
  ...
  if (
    ...
    issues.push({
      ...
      description:
        `Cache expects ${loadedInfo.expectedVersion} but loaded ${loadedInfo.loadedVersion}.`,
  ...
  if (
    ...
    const loadedPackageName = getLoadedPackageName(
      loadedInfo.installedPackagePath,
    );
    issues.push({
      title: "Loaded plugin is outdated",
      description:
        `Loaded ${systemInfo.loadedVersion}, latest ${latestVersion}.`,
      fix:
        `Update: cd "${loadedInfo.cacheDir}" && bun add ${loadedPackageName}@${installTag}\n` +
