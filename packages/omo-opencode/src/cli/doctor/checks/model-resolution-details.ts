...
export function buildModelResolutionDetails(options: {
  ...
  if (options.available.cacheExists) {
    ...
    details.push(
      `  Sample: ${options.available.providers.slice(0, 6).join(", ")}${
        options.available.providers.length > 6 ? "..." : ""
      }`,
    );
  ...
  for (const agent of options.info.agents) {
    ...
    details.push(
      `  ${marker} ${agent.name}: ${display} [capabilities: ${
        formatCapabilityResolutionLabel(
          agent.capabilityDiagnostics?.resolutionMode,
        )
      }]`,
    );
  }
  ...
  for (const category of options.info.categories) {
    ...
    details.push(
      `  ${marker} ${category.name}: ${display} [capabilities: ${
        formatCapabilityResolutionLabel(
          category.capabilityDiagnostics?.resolutionMode,
        )
      }]`,
    );
  }
