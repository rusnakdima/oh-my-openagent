...
const originalOmoSendAnonymousTelemetry =
...
function resetEnv(): void {
  ...
  if (originalOmoSendAnonymousTelemetry === undefined) {
    ...
    process.env.OMO_SEND_ANONYMOUS_TELEMETRY =
      originalOmoSendAnonymousTelemetry;
  }
}
...
describe("checkTelemetry", () => {
  it("reports enabled status and last daily active date", async () => {
    // given
    const dataHomePath = join(
      tmpdir(),
      `doctor-telemetry-${Date.now()}-${Math.random()}`,
    );
    const stateDir = join(dataHomePath, "oh-my-opencode");
    mkdirSync(stateDir, { recursive: true });
    writeFileSync(
      join(stateDir, "posthog-activity.json"),
      `${JSON.stringify({ lastActiveDayUTC: "2026-06-28" })}\n`,
    );
    process.env.XDG_DATA_HOME = dataHomePath;
    delete process.env.OMO_DISABLE_POSTHOG;
    delete process.env.OMO_SEND_ANONYMOUS_TELEMETRY;
    const { checkTelemetry } = await import(
      `./telemetry?enabled=${Date.now()}`
    );
