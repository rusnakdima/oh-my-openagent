...
export async function checkTeamMode(): Promise<CheckResult> {
  ...
  const baseDirMessage = baseDirExists
    ? `base dir: ok`
    : `base dir: missing (plugin init will create it on first use)`;

  return {
    name: CHECK_NAMES[CHECK_IDS.TEAM_MODE],
    status: deps.tmuxAvailable && deps.gitAvailable ? "pass" : "warn",
    message: `team_mode: enabled | tmux: ${
      deps.tmuxAvailable ? "ok" : "missing"
    } | git: ${
      deps.gitAvailable ? "ok" : "missing"
    } | ${baseDirMessage} | declared: ${teamCount} | runtime dirs: ${runtimeCount}`,
    details: undefined,
    issues: [],
  };
}
