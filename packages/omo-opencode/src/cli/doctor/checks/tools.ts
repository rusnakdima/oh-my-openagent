...
export function buildToolIssues(summary: ToolsSummary): DoctorIssue[] {
  ...
  if (!summary.astGrepCli) {
    issues.push({
      ...
      fix:
        "The ast-grep skill resolves sg automatically; run omo doctor or lazycodex-ai doctor to check provisioning.",
      severity: "warning",
...
export async function checkTools(): Promise<CheckResult> {
  ...
  const invalidUserMcpServers = userMcpServers.filter((server) =>
    !server.valid
  );
  ...
  if (invalidUserMcpServers.length > 0) {
    issues.push({
      ...
      description:
        `${invalidUserMcpServers.length} user MCP server(s) have invalid config format.`,
      severity: "warning",
  ...
  return {
    ...
    details: [
      ...
      `LSP: ${
        summary.lspServers.length > 0
          ? `${summary.lspServers.length} server(s)`
          : "none"
      }`,
      `GH CLI: ${summary.ghCli.installed ? "installed" : "missing"}${
        summary.ghCli.authenticated ? " (authenticated)" : ""
      }`,
