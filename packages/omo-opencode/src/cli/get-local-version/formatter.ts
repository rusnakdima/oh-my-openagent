  ...
  switch (info.status) {
    ...
    case "outdated":
      ...
      lines.push(
        `  ${color.dim("Run:")} ${
          color.cyan(
            `cd ~/.config/opencode && bun update ${PUBLISHED_PACKAGE_NAME}`,
          )
        }`,
      );
      break;
    ...
    case "dev":
      ...
      lines.push(
        `  ${color.dim("Installed from source; update checks are skipped")}`,
      );
      break;
    case "pinned":
      lines.push(
        `  ${SYMBOLS.pin} ${
          color.magenta(`Version pinned to ${info.pinnedVersion}`)
        }`,
      );
    ...
    case "pinned-mismatch":
      lines.push(
        `  ${SYMBOLS.warn} ${
          color.yellow(
            `Version pinned to ${info.pinnedVersion} but running ${info.currentVersion}`,
          )
        }`,
      );
      lines.push(
