...
function resolveCompatibleFallbackSettings(
  ...
  return {
    ...
    ...(compatibility.reasoningEffort
      ? {
        reasoningEffort: compatibility
          .reasoningEffort as FallbackModelObject["reasoningEffort"],
      }
      : {}),
    ...(compatibility.temperature !== undefined
      ? { temperature: compatibility.temperature }
      : {}),
    ...(compatibility.topP !== undefined ? { top_p: compatibility.topP } : {}),
    ...(compatibility.maxTokens !== undefined
      ? { maxTokens: compatibility.maxTokens }
      : {}),
    ...(compatibility.thinking !== undefined
      ? { thinking: compatibility.thinking as FallbackModelObject["thinking"] }
      : {}),
  };
}

function toCompatibleModelConfig(
  model: string,
  desired: Pick<CompatibleFallbackSettings, "variant">,
): AgentConfig {
  ...
  const compatible = resolveCompatibleFallbackSettings(
    providerID,
    modelID,
    desired,
  );
  return compatible.variant
    ? { model, variant: compatible.variant }
    : { model };
}
...
function toFallbackModelObject(
  ...
  const compatible = resolveCompatibleFallbackSettings(provider, modelID, {
    ...
    reasoningEffort: entry.reasoningEffort as
      | FallbackModelObject["reasoningEffort"]
      | undefined,
...
function attachFallbackModels<T extends AgentConfig | CategoryConfig>(
  ...
  const uniqueFallbacks = collectAvailableFallbacks(
    fallbackChain,
    availability,
  );
  const primaryIndex = uniqueFallbacks.findIndex((entry) =>
...
function attachAllFallbackModels<T extends AgentConfig | CategoryConfig>(
  ...
  const fallbackModels = uniqueFallbacks.filter((entry) =>
    entry.model !== config.model
  );
  if (fallbackModels.length === 0) {
    return config;
...
export function generateModelConfig(config: InstallConfig): GeneratedOmoConfig {
    ...
    return {
      ...
      agents: Object.fromEntries(
        Object.entries(CLI_AGENT_MODEL_REQUIREMENTS)
          .filter(([role, req]) =>
            !(role === "sisyphus" && req.requiresAnyModel)
          )
          .map(([role]) => [role, { model: ULTIMATE_FALLBACK }]),
      ),
      categories: Object.fromEntries(
        Object.keys(CLI_CATEGORY_MODEL_REQUIREMENTS).map((
          cat,
        ) => [cat, { model: ULTIMATE_FALLBACK }]),
      ),
    };
  }
  ...
  for (const [role, req] of Object.entries(CLI_AGENT_MODEL_REQUIREMENTS)) {
    if (role === "librarian") {
      ...
      if (resolved) {
        const agentConfig = toCompatibleModelConfig(resolved.model, {
          variant: resolved.variant,
        });
        agents[role] = attachFallbackModels(
          agentConfig,
          req.fallbackChain,
          avail,
        );
  ...
  for (const [cat, req] of Object.entries(CLI_CATEGORY_MODEL_REQUIREMENTS)) {
    ...
    if (resolved) {
      const variant = resolved.variant ?? req.variant;
      const categoryConfig = toCompatibleModelConfig(resolved.model, {
        variant,
      });
