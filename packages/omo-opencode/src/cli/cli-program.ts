...
import {
  ...
  isSenpiPlatformEnabled,
  SENPI_PLATFORM_ENV_FLAG,
} from "./senpi-platform-flag";
...
export function resolveInstallArgs(
  ...
  const defaultPlatform =
    process.env.OMO_EDITION === "codex" || invocationName === "lazycodex" ||
      invocationName === "lazycodex-ai"
      ? "codex"
      : undefined;
...
program
  ...
  .addOption(
    new Option(
      "--platform <platform>",
      `Install target platform: ${availableInstallPlatforms().join(", ")}`,
    ).choices(availableInstallPlatforms()).hideHelp(),
  )
...
program
  ...
  .option(
    "--bailian-coding-plan <value>",
    "Bailian Coding Plan subscription: no, yes (default: no)",
  )
  .option(
    "--minimax-cn-coding-plan <value>",
    "MiniMax Coding Plan (minimaxi.com) subscription: no, yes (default: no)",
  )
  .option(
    "--minimax-coding-plan <value>",
    "MiniMax Coding Plan (minimax.io) subscription: no, yes (default: no)",
  )
  .option(
  ...
  .option(
    "--codex-autonomous",
    "Configure Codex with approval never, full filesystem access, and network enabled",
  )
  .option(
    "--no-codex-autonomous",
    "Leave existing Codex permission settings unchanged",
  )
...
`,
  ...
  .action(async (options: InstallCommandOptions) => {
    ...
    const args = resolveInstallArgs({
      ...options,
      platform: options.platform ?? rootOptions.platform,
    });
...
program
  ...
  .option(
    "-w, --worktree <name-or-path>",
    "Git worktree: create or reuse a worktree under ~/.omo/worktrees/<name>, or use an explicit path",
  )
  .option(
    "-p, --port <port>",
...
program
  ...
  .addOption(
    new Option(
      "--platform <platform>",
      "Doctor target platform: opencode, codex",
...
`,
  ...
  .action(async (options: DoctorCommandOptions) => {
    const rootOptions = program.opts<RootCommandOptions>();
    const rootDoctorPlatform =
      rootOptions.platform === "opencode" || rootOptions.platform === "codex"
        ? rootOptions.platform
        : undefined;
    ...
    const doctorOptions: DoctorOptions = {
      ...
      target: resolveDoctorTarget(
        process.env.OMO_INVOCATION_NAME,
        options.platform ?? rootDoctorPlatform,
      ),
    };
...
program
  ...
  .option(
    "--dry-run",
    "Print the transform, backup move plan, and conflicts without new migration writes",
  )
  ...
  .option(
    "--path <key>",
    "Focus on a specific config path (e.g., agents.sisyphus)",
  )
  .addHelpText(
    "after",
    `
...
const isCodexPlatform = invocationName === "lazycodex" ||
  invocationName === "lazycodex-ai";
...
sessionCommand
  ...
  .action(async (options) => {
    const exitCode = await runSessionList({
      tag: options.tag,
      limit: options.limit,
    });
    process.exit(exitCode);
...
`,
  ...
  .action(async (sessionA: string, sessionB: string, options) => {
    const exitCode = await runSessionDiff(sessionA, sessionB, {
      json: options.json ?? false,
    });
    process.exit(exitCode);
  });
