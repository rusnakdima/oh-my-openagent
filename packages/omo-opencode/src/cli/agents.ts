...
export function resolveAgentsInfo(): AgentInfo[] {
  ...
  return AGENT_ORDER.map((name) => {
    ...
    const provenance: AgentInfo["provenance"] = userOverride
      ? "config override"
      : "fallback chain";
...
export async function runAgentsCommandAsync(options: {
  ...
  let agents = options.live
    ? await resolveLiveAgentsInfo()
    : resolveAgentsInfo();
  ...
  if (options.json) {
    console.log(
      JSON.stringify(
        {
          agents,
          summary: { total: agents.length, overridden: overriddenCount },
        },
        null,
        2,
      ),
    ...
    const note = options.live && agents[0]?.provenance !== "live"
...
export async function runSetModelCommand(model: string): Promise<number> {
  if (!model || model.trim() === "") {
    console.error(
      "Error: model argument cannot be empty. Use 'omo agents set-model <model>' to set a model.",
    );
...
`,
  ...
  agentsCmd
    ...
    .description(
      "Set the same model for all built-in agents in the user config",
    )
    .action(async (model) => {
