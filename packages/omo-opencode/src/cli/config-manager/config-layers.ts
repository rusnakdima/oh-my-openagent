...
function inferScope(view: OmoOpenCodeConfigView, projectCwd: string): "project" | "user" | "defaults" {
  const homeDir = homedir()
  const normalizedHome = posix.resolve(homeDir)
  const normalizedPath = posix.resolve(view.path)

  if (normalizedPath.startsWith(normalizedHome + "/.omo/") || normalizedPath === posix.join(normalizedHome, ".omo", "omo.jsonc")) {
    return "user"
  }
...
export function listConfigLayers(cwd: string = process.cwd()): ConfigLayerInfo[] {
...
export function explainConfigKey(key: string, cwd: string = process.cwd()): ExplainResult | null {
  const chain = loadOmoOpenCodeConfigChain(cwd)
...
export function printConfigLayers(cwd: string = process.cwd()): void {
  ...
  for (const layer of layers) {
    console.log(`  ${layer.index}. ${formatScope(layer.scope)}: ${shortPath(layer.path)}`)
  }
}

export function printExplainResult(key: string, cwd: string = process.cwd()): void {
  ...
  if (result.defaultValue !== undefined && JSON.stringify(result.value) !== JSON.stringify(result.defaultValue)) {
