...
function createProc(
  ...
  return {
    ...
    exitCode,
    stdout: options.output?.stdout !== undefined
      ? new Blob([options.output.stdout]).stream()
      : undefined,
    stderr: options.output?.stderr !== undefined
...
describe("runBunInstallWithDetails", () => {
  ...
  beforeEach(async () => {
    getOpenCodeCacheDirSpy = spyOn(dataPath, "getOpenCodeCacheDir")
    ...
    describe("#when bun install runs with proxy environment variables set", () => {
      it("#then forwards process.env so child bun install inherits proxy settings (issue #3528)", async () => {
        ...
        try {
          ...
          const spawnOptions = callArgs?.[1] as {
            env?: Record<string, string | undefined>;
          } | undefined;
          expect(spawnOptions?.env).toBeDefined();
          expect(spawnOptions?.env?.https_proxy).toBe(
            "http://proxy.example.com:3128",
          );
          expect(spawnOptions?.env?.http_proxy).toBe(
            "http://proxy.example.com:3128",
          );
        } finally {
          ...
          expect(outcome).toEqual(
            {
              ...
              timedOut: true,
              error:
                `bun install timed out after 60 seconds. Try running manually: cd "${EXPECTED_WORKSPACE_DIR}" && bun i`,
