  ...
  it("detects OpenCode Go from the existing omo config", () => {
    ...
    writeFileSync(
      ...
      JSON.stringify(
        {
          "[opencode]": {
            agents: { atlas: { model: "opencode-go/kimi-k2.6" } },
          },
        },
        null,
        2,
...
describe("addPluginToOpenCodeConfig - single package writes", () => {
  ...
  beforeEach(() => {
    ...
    getPluginNameWithVersionSpy = spyOn(
      pluginNameWithVersion,
      "getPluginNameWithVersion",
    ).mockResolvedValue("oh-my-openagent");
  });
  ...
  it("updates a version-pinned legacy entry to the requested version", async () => {
    // given
    const getPluginNameWithVersionSpy = spyOn(
      pluginNameWithVersion,
      "getPluginNameWithVersion",
    ).mockResolvedValue("oh-my-openagent@3.16.0");
    writeFileSync(
      testConfigPath,
      JSON.stringify({ plugin: ["oh-my-opencode@3.15.0"] }, null, 2) + "\n",
      "utf-8",
    );
  ...
  it("preserves a canonical entry when the same version is re-installed", async () => {
    ...
    const getPluginNameWithVersionSpy = spyOn(
      pluginNameWithVersion,
      "getPluginNameWithVersion",
    ).mockResolvedValue("oh-my-openagent@3.10.0");
    writeFileSync(
      testConfigPath,
  ...
  it("blocks a downgrade for a version-pinned canonical entry", async () => {
    ...
    const getPluginNameWithVersionSpy = spyOn(
      pluginNameWithVersion,
      "getPluginNameWithVersion",
    ).mockResolvedValue("oh-my-openagent@3.15.0");
    writeFileSync(
      testConfigPath,
      JSON.stringify({ plugin: ["oh-my-openagent@3.16.0"] }, null, 2) + "\n",
      "utf-8",
    );
  ...
  it("mirrors an existing source plugin entry into profile configs", async () => {
    ...
    writeFileSync(
      testConfigPath,
      JSON.stringify({ plugin: [sourcePlugin] }, null, 2) + "\n",
      "utf-8",
    );
    ...
    const savedProfileConfig = JSON.parse(
      readFileSync(profileConfigPath, "utf-8"),
    );
  ...
  it("uses the parent source plugin entry when OPENCODE_CONFIG_DIR points at a profile", async () => {
    ...
    expect(result.configPath).toBe(
      join(realpathSync(profileDir), "opencode.json"),
    );
    const savedProfileConfig = JSON.parse(
      readFileSync(profileConfigPath, "utf-8"),
    );
