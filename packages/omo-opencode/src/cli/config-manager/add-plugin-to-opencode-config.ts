...
function detectConfigFormatInDir(
  configDir: string,
): { readonly format: ConfigFormat; readonly path: string } {
...
function listProfileConfigDirs(rootConfigDir: string): string[] {
  ...
  return readdirSync(profilesDir, { withFileTypes: true })
    ...
    .filter((profileDir) =>
      detectConfigFormatInDir(profileDir).format !== "none"
    );
}

function getConfigTargets(): ConfigTarget[] {
  const primaryConfigDir = getConfigDir();
  const rootConfigDir = getParentConfigDirForProfile(primaryConfigDir) ??
    primaryConfigDir;
  const targetDirs = new Set<string>([primaryConfigDir]);

  if (
    rootConfigDir !== primaryConfigDir &&
    detectConfigFormatInDir(rootConfigDir).format !== "none"
  ) {
...
function isSourceOmoPluginEntry(plugin: string): boolean {
  ...
  return /\/(omo(?:-[^/]*)?|oh-my-opencode|oh-my-openagent)\/(src|dist)\/index\.(ts|js)$/
    .test(normalized);
}
...
function writePluginEntryToTarget(params: {
  ...
  const { target, currentVersion, fallbackEntry, preferredSourceEntry } =
    params;
