...
function pluginEntries(config: ConfigShape): string[] {
  return Array.isArray(config.plugin)
    ? config.plugin.filter((entry): entry is string =>
      typeof entry === "string"
    )
    : [];
}
...
function desiredTuiEntry(serverEntry: string): string | null {
  if (
    serverEntry === PLUGIN_NAME || serverEntry.startsWith(`${PLUGIN_NAME}@`)
  ) {
    return serverEntry;
  }
  if (
    serverEntry === LEGACY_PLUGIN_NAME ||
    serverEntry.startsWith(`${LEGACY_PLUGIN_NAME}@`)
  ) {
    return serverEntry;
  }
...
function readTuiConfig(
  tuiJsonPath: string,
): { config: ConfigShape; malformed: boolean } {
...
export function ensureTuiPluginEntry(
  opts: { configDir?: string } = {},
): EnsureTuiPluginEntryResult {
  const configDir = opts.configDir ??
    getOpenCodeConfigDir({ binary: "opencode", version: null });
  const serverConfig = readServerConfig(configDir);
  const serverEntry = serverConfig
    ? pluginEntries(serverConfig).find(isServerPluginEntry)
    : undefined;
  if (!serverEntry) {
  ...
  const plugins = pluginEntries(config).filter((entry) =>
    !isNamedTuiPluginEntry(entry)
  );
  ...
  writeFileAtomically(
    tuiJsonPath,
    formatConfig({ ...config, plugin: [...plugins, desiredEntry] }),
  );
