...
describe("install platform resolution", () => {
  ...
  test("rejects --platform=senpi while the senpi platform flag is disabled", () => {
    ...
    // when / then
    expect(() =>
      resolveInstallArgs({ tui: true, platform: "senpi" }, invocationName)
    ).toThrow(/OMO_ENABLE_SENPI_PLATFORM/);
  });
  ...
  test("defines Commander choices so invalid --platform values are rejected", async () => {
    // given
    const cliProgramSource = await Bun.file(
      new URL("./cli-program.ts", import.meta.url),
    ).text();
  ...
  test("defines root --platform so npx can pass it before install", async () => {
    ...
    // when
    const rootBlock = cliProgramSource.match(
...
describe("OMO_EDITION install routing", () => {
  ...
  test("a non-codex OMO_EDITION leaves name-based routing unchanged", () => {
    withEdition("opencode", () => {
      // when / then
      expect(resolveInstallArgs({ tui: true }, "omo-agent-toolkit").platform)
        .toBeUndefined();
      expect(resolveInstallArgs({ tui: true }, "lazycodex-ai").platform).toBe(
        "codex",
      );
    });
  });
  ...
  test("unset OMO_EDITION keeps today's name-based routing exactly", () => {
    withEdition(undefined, () => {
      ...
      expect(resolveInstallArgs({ tui: true }, "omo-agent-toolkit").platform)
        .toBeUndefined();
      expect(resolveInstallArgs({ tui: true }, "oh-my-opencode").platform)
        .toBeUndefined();
      expect(resolveInstallArgs({ tui: true }, "lazycodex").platform).toBe(
        "codex",
      );
      expect(resolveInstallArgs({ tui: true }, "lazycodex-ai").platform).toBe(
        "codex",
