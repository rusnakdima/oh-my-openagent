...
describe("installAstGrepForOpenCode", () => {
  test("#given OpenCode install finished #when ast-grep provisioning runs #then it targets the OMO runtime directory", async () => {
    // given
    const calls: Array<
      { readonly skillDir: string; readonly targetDir: string }
    > = [];
    ...
    expect(calls).toEqual([{
      skillDir: join("/repo/packages/shared-skills/skills", "ast-grep"),
      targetDir: join("/home/test", ".omo", "runtime", "ast-grep", "linux-x64"),
    }]);
  });
