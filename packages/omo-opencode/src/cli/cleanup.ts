...
export function resolveCleanupPlatform(
  ...
  return invocationName === "lazycodex" || invocationName === "lazycodex-ai"
    ? "codex"
    : undefined;
}
...
export async function cleanup(options: CleanupOptions): Promise<number> {
  ...
  if (result.configChanged) {
    ...
    console.log(
      `- No managed Codex config blocks found in ${result.configPath}`,
    );
  }
  ...
  for (const skippedPath of result.skippedPaths) {
    console.log(
      `- Skipped cleanup target ${skippedPath.path}: ${skippedPath.reason}`,
    );
  }
  ...
  if (result.projectCleanup.changed) {
    console.log(
      `- Repaired project-local Codex config ${result.projectCleanup.configPath}`,
    );
  }
