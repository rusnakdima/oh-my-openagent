...
export function resolveCodexUlwLoopCommand(
  input: ResolveCodexUlwLoopCommandInput = {},
): CodexUlwLoopCommand | null {
  ...
  const legacyLocalBin = resolveLegacyLocalOmoBin(
    env,
    homeDir,
    input.currentExecutablePaths ??
      [process.argv[1]].filter((value): value is string =>
        typeof value === "string"
      ),
  );
...
function resolveLocalUlwLoopBin(
...
function resolveLegacyLocalOmoBin(
  env: NodeJS.ProcessEnv,
  homeDir: string,
  currentExecutablePaths: readonly string[],
): string | null {
  const candidates = resolveCodexComponentBinCandidates({
    executableName: "omo",
    env,
...
function isCurrentExecutable(
  ...
  return currentExecutablePaths.some((currentPath) =>
    realpathOrSelf(currentPath) === candidateRealPath
  );
}
