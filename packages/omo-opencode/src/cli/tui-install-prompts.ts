...
export async function promptInstallPlatform(
  ...
  const options: Option<InstallPlatform>[] = [
    {
      value: "opencode",
      label: "OpenCode",
      hint: "Install OpenCode plugin only",
    },
    {
      value: "codex",
      label: "Codex",
      hint: "Install Codex harness adapter only",
    },
    {
      value: "both",
      label: "Both",
      hint: "Install OpenCode plugin and Codex adapter",
    },
  ];
  if (isSenpiPlatformEnabled()) {
    options.push({
      value: "senpi",
      label: "Senpi",
      hint: "Install Senpi harness adapter only",
    });
  }
...
export async function promptInstallConfig(
  ...
  const codexAutonomous = await resolveCodexAutonomous(
    hasCodex,
    codexAutonomousOverride,
  );
    ...
    options: [
      {
        value: "no",
        label: "No",
        hint: `Will use ${ULTIMATE_FALLBACK} as fallback`,
      },
      {
        value: "yes",
        label: "Yes (standard)",
        hint: "Claude Opus 5 for orchestration",
      },
      {
        value: "max20",
        label: "Yes (max20 mode)",
        hint: "Higher Claude usage limits for orchestration",
      },
    ],
  ...
  const openai = await selectOrCancel({
    ...
    options: [
      { value: "no", label: "No", hint: "Oracle will use fallback models" },
      {
        value: "yes",
        label: "Yes",
        hint: "GPT-5.6 Sol for Oracle (high-IQ debugging)",
      },
    ],
  ...
  const opencodeZen = await selectOrCancel({
    ...
    options: [
      { value: "no", label: "No", hint: "Will use other configured providers" },
      {
        value: "yes",
        label: "Yes",
        hint: "opencode/claude-opus-5, opencode/gpt-5.6-sol, etc.",
      },
    ],
  ...
  const kimiForCoding = await selectOrCancel({
    ...
    options: [
      { value: "no", label: "No", hint: "Will use other configured providers" },
      {
        value: "yes",
        label: "Yes",
        hint: "Kimi K3 for Sisyphus/Prometheus fallback",
      },
    ],
  ...
  const bailianCodingPlan = await selectOrCancel({
    ...
    options: [
      { value: "no", label: "No", hint: "Will use other configured providers" },
      {
        value: "yes",
        label: "Yes",
        hint: "Qwen, GLM, and Kimi fallback route",
      },
    ],
  ...
  const minimaxCnCodingPlan = await selectOrCancel({
    ...
    options: [
      { value: "no", label: "No", hint: "Will use other configured providers" },
      {
        value: "yes",
        label: "Yes",
        hint: "Enables MiniMax-M3 fallback models via minimaxi.com",
      },
    ],
  ...
  const minimaxCodingPlan = await selectOrCancel({
    ...
    options: [
      { value: "no", label: "No", hint: "Will use other configured providers" },
      {
        value: "yes",
        label: "Yes",
        hint: "Enables MiniMax-M3 fallback models via minimax.io",
      },
    ],
