...
export async function getContinuationState(
  ...
  return {
    ...
    hasActiveBackgroundTaskMarker:
      marker?.sources["background-task"]?.state === "active",
...
async function hasActiveBoulderContinuation(
  ...
  const normalizedTrackedSessionIDs = boulder.session_ids.map((
    trackedSessionID,
  ) => normalizeSessionId(trackedSessionID));
  if (!normalizedTrackedSessionIDs.includes(normalizedSessionID)) {
    return false;
  }

  const sessionOrigin = boulder.session_origins?.[sessionID] ??
    boulder.session_origins?.[normalizedSessionID];
