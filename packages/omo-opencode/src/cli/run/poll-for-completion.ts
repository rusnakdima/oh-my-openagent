...
export async function pollForCompletion(
  ...
  const rawMinStabilizationMs = options.minStabilizationMs ??
    MIN_STABILIZATION_MS;
  const minStabilizationMs = rawMinStabilizationMs > 0
    ? rawMinStabilizationMs
    : MIN_STABILIZATION_MS;
    ...
    if (eventState.lastEventTimestamp !== null) {
      ...
      if (timeSinceLastEvent > eventWatchdogMs) {
        ...
        if (mainSessionStatus === "idle") {
          ...
          mainSessionStatus === "busy" || mainSessionStatus === "retry"
        ) {
          eventState.mainSessionIdle = false;
        }
