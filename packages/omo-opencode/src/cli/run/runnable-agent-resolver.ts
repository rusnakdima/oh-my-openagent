...
export interface RunAgentDisplayNameConfig {
  readonly agents?: Readonly<
    Record<string, { readonly displayName?: string } | undefined>
  >;
}
...
export async function resolveRunnableRunAgent(
  ...
  try {
    ...
    const agents = normalizeSDKResponse(
      agentsRes,
      [] as readonly AgentListItem[],
      {
        preferResponseOnMissingData: true,
      },
    );
    const exactAgent = agents.find((agent) => agent.name === resolvedAgent)
      ?.name;
    if (exactAgent) return exactAgent;

    const resolvedConfigKey = getAgentConfigKey(resolvedAgent);
    const configuredDisplayName = config.agents?.[resolvedConfigKey]
      ?.displayName;
