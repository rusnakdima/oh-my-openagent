import {
  createOpencode as createOpencodeSdk,
  createOpencodeClient as createOpencodeClientSdk,
...
export type ServerConnectionDeps<TClient> = {
  createOpencode: (
    options: { signal: AbortSignal; port: number; hostname: string },
...
export async function createServerConnectionWithDeps<TClient>(
  ...
  try {
    const selected = await deps.getAvailableServerPort(
      DEFAULT_SERVER_PORT,
      "127.0.0.1",
    );
    ...
    const defaultPortIsAvailable = await deps.isPortAvailable(
      DEFAULT_SERVER_PORT,
      "127.0.0.1",
    );
  ...
  try {
    ...
    const { port: retryPort } = await deps.getAvailableServerPort(
      selectedPort + 1,
      "127.0.0.1",
    );
    console.log(
