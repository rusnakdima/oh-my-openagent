...
describe("executeOnCompleteHook", () => {
  ...
  function createHookDeps(
    spawnImpl: typeof spawnWithWindowsHideModule.spawnWithWindowsHide = () =>
      createProc(0),
    platform: NodeJS.Platform = "linux",
    env: NodeJS.ProcessEnv = { SHELL: "/bin/sh" },
  ) {
    const spawnCalls: Array<
      Parameters<typeof spawnWithWindowsHideModule.spawnWithWindowsHide>
    > = [];
    return {
      deps: {
  ...
  it("hook stdout and stderr are logged to file logger", async () => {
    ...
    expect(stdoutCall?.[1]).toEqual({
      command: "echo test",
      stdout: "hook output",
    });
    expect(stderrCall?.[1]).toEqual({
      command: "echo test",
      stderr: "hook warning",
    });
  });
