...
import {
  closeThinkBlockIfNeeded,
  ensureThinkBlockOpen,
} from "./event-think-block";
...
export function handleMessagePartDelta(
  ...
  const partType = props?.partID
    ? state.partTypesById[props.partID]
    : undefined;
...
export function handleMessageUpdated(
  ...
  if (
    agent !== state.currentAgent || model !== state.currentModel ||
