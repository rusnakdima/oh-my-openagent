...
describe("checkCompletionConditions continuation coverage", () => {
  ...
  it("returns false when current session is an appended descendant of an active boulder session with unchecked plan items", async () => {
    ...
    ctx.client.session.get = unsafeTestValue<
      RunContext["client"]["session"]["get"]
    ...
    ctx.client.session.messages = unsafeTestValue<
      RunContext["client"]["session"]["messages"]
    >(mock(async ({ path }: { path: { id: string } }) => ({
  ...
  it("returns true when current session is only in lineage and is not explicitly tracked in boulder", async () => {
    ...
    >(mock(async ({ path }: { path: { id: string } }) => ({
      data: {
        id: path.id,
        parentID: path.id === "lineage-only-session"
          ? "root-session"
          : undefined,
      },
    })));
  ...
  it("returns true when appended descendant has agent mismatch and atlas would not continue it", async () => {
    ...
    const planPath = join(
      ...
      ".omo",
      "plans",
      "lineage-agent-mismatch-plan.md",
    );
    mkdirSync(join(directory, ".omo", "plans"), { recursive: true });
    writeFileSync(planPath, "- [ ] unfinished task\n", "utf-8");
    writeBoulderStateFile(directory, planPath, [
      "root-session",
      "mismatch-subagent-session",
    ], {
      "root-session": "direct",
    ...
    >(mock(async ({ path }: { path: { id: string } }) => ({
      data: {
        id: path.id,
        parentID: path.id === "mismatch-subagent-session"
          ? "root-session"
          : undefined,
      },
    })));
  ...
  it("returns true when mismatched descendant was already appended into boulder session_ids", async () => {
    ...
    const planPath = join(
      ...
      ".omo",
      "plans",
      "appended-mismatch-plan.md",
    );
    mkdirSync(join(directory, ".omo", "plans"), { recursive: true });
    writeFileSync(planPath, "- [ ] unfinished task\n", "utf-8");
    writeBoulderStateFile(directory, planPath, [
      "root-session",
      "appended-mismatch-session",
    ], {
      "root-session": "direct",
    ...
    >(mock(async ({ path }: { path: { id: string } }) => ({
      data: {
        id: path.id,
        parentID: path.id === "appended-mismatch-session"
          ? "root-session"
          : undefined,
      },
    })));
  ...
  it("returns true when appended descendant cannot prove lineage because parent lookup fails", async () => {
    ...
    const planPath = join(
      ...
      ".omo",
      "plans",
      "appended-unresolved-lineage-plan.md",
    );
    mkdirSync(join(directory, ".omo", "plans"), { recursive: true });
    writeFileSync(planPath, "- [ ] unfinished task\n", "utf-8");
    writeBoulderStateFile(directory, planPath, [
      "root-session",
      "ses_appended_descendant",
    ], {
      "root-session": "direct",
  ...
  it("returns true when multi-session tracked child is missing provenance and lineage cannot be proven", async () => {
    ...
    const planPath = join(
      ...
      ".omo",
      "plans",
      "unknown-origin-multi-session-plan.md",
    );
    mkdirSync(join(directory, ".omo", "plans"), { recursive: true });
    writeFileSync(planPath, "- [ ] unfinished task\n", "utf-8");
    writeBoulderStateFile(directory, planPath, [
  ...
  it("returns false when directly tracked child session has a tracked ancestor and mismatched agent metadata", async () => {
    ...
    const planPath = join(
      ...
      ".omo",
      "plans",
      "multi-tracked-direct-child-plan.md",
    );
    mkdirSync(join(directory, ".omo", "plans"), { recursive: true });
    writeFileSync(planPath, "- [ ] unfinished task\n", "utf-8");
    writeBoulderStateFile(directory, planPath, [
    ...
    >(mock(async ({ path }: { path: { id: string } }) => ({
      data: {
        id: path.id,
        parentID: path.id === "ses_direct_child"
          ? "ses_root_tracked"
          : undefined,
      },
    })));
  ...
  it("returns false when latest appended descendant message is compaction but previous real agent still matches atlas", async () => {
    ...
    const planPath = join(
      ...
      ".omo",
      "plans",
      "compaction-descendant-plan.md",
    );
    mkdirSync(join(directory, ".omo", "plans"), { recursive: true });
    writeFileSync(planPath, "- [ ] unfinished task\n", "utf-8");
    writeBoulderStateFile(directory, planPath, [
      "root-session",
      "ses_child_after_compaction",
    ], {
      "root-session": "direct",
    ...
    >(mock(async ({ path }: { path: { id: string } }) => ({
      data: {
        id: path.id,
        parentID: path.id === "ses_child_after_compaction"
          ? "root-session"
          : undefined,
      },
    })));
  ...
  it("returns true for untracked descendant continuation on SQLite-shaped misordered messages because lineage alone is no longer sufficient", async () => {
    ...
    const planPath = join(
      ...
      ".omo",
      "plans",
      "sqlite-ordered-descendant-plan.md",
    );
    mkdirSync(join(directory, ".omo", "plans"), { recursive: true });
    writeFileSync(planPath, "- [ ] unfinished task\n", "utf-8");
    writeBoulderStateFile(directory, planPath, ["root-session"]);
    ...
    >(mock(async ({ path }: { path: { id: string } }) => ({
      data: {
        id: path.id,
        parentID: path.id === "ses_sqlite_descendant"
          ? "root-session"
          : undefined,
      },
    })));
  ...
  it("returns false when appended tracked descendant has no persisted messages but in-memory session agent matches atlas", async () => {
    ...
    const planPath = join(
      ...
      ".omo",
      "plans",
      "session-agent-fallback-plan.md",
    );
    mkdirSync(join(directory, ".omo", "plans"), { recursive: true });
    writeFileSync(planPath, "- [ ] unfinished task\n", "utf-8");
    writeBoulderStateFile(directory, planPath, [
    ...
    >(mock(async ({ path }: { path: { id: string } }) => ({
      data: {
        id: path.id,
        parentID: path.id === "ses_appended_child"
          ? "ses_root_tracked"
          : undefined,
      },
    })));
