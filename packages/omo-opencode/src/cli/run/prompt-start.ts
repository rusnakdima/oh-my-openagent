...
async function readMainSessionStatus(
  ctx: RunContext,
): Promise<SessionStatus["type"] | null> {
  ...
  try {
    ...
    if (ctx.verbose) {
      console.error(
        `[run] failed to read session status while waiting for prompt start: ${
          String(error)
        }`,
      );
    }
...
async function hasPersistedMessages(ctx: RunContext): Promise<boolean> {
  ...
  try {
    ...
    if (ctx.verbose) {
      console.error(
        `[run] failed to read session messages while waiting for prompt start: ${
          String(error)
        }`,
      );
    }
...
export async function waitForPromptStart(
  ...
  const pollIntervalMs = options.pollIntervalMs ??
    DEFAULT_PROMPT_START_POLL_INTERVAL_MS;
  ...
  while (!abortController.signal.aborted) {
    ...
    if (eventState.mainSessionError) {
      throw new Error(
        `Session errored before prompt started: ${
          eventState.lastError ?? "unknown error"
        }`,
      );
    }
