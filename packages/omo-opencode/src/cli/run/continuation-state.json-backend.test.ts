...
describe("getContinuationState JSON backend descendant coverage", () => {
    ...
    const state = await getContinuationState(directory, "ses_child_session", {
      session: {
        get: async ({ path }: { path: { id: string } }) => ({
          data: {
            id: path.id,
            parentID: path.id === "ses_child_session"
              ? "ses_root_session"
              : undefined,
          },
        }),
  ...
  test("prefers earliest JSON agent by time.created instead of filename order for first-message fallback helpers", async () => {
    ...
    const { findFirstMessageWithAgent } = await import(
      "../../features/hook-message-injector"
    );

    // when
    const result = findFirstMessageWithAgent(
      join(TEST_MESSAGE_STORAGE, sessionID),
    );
  ...
  test("prefers newest JSON agent by time.created even when filenames look reversed and timestamps tie-break by filename only", async () => {
    ...
    mkdirSync(messageDir, { recursive: true });
    writeFileSync(
      join(messageDir, "msg_a91f00ab_000001.json"),
      JSON.stringify({
        agent: "atlas",
        model: { providerID: "openai", modelID: "gpt-5.4" },
        time: { created: 100 },
    ...
    writeFileSync(
      join(messageDir, "msg_f0e1d2c3_000002.json"),
      JSON.stringify({
        agent: "compaction",
        model: { providerID: "openai", modelID: "gpt-5.4" },
        time: { created: 200 },
    ...
    writeFileSync(
      join(messageDir, "msg_d4c3b2a1_000003.json"),
      JSON.stringify({
