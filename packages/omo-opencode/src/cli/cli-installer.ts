...
export async function runCliInstaller(
  ...
  if (hasOpenCode) {
    ...
    printSuccess(
      `Plugin ${isUpdate ? "verified" : "added"} ${SYMBOLS.arrow} ${
        color.dim(pluginResult.configPath)
      }`,
    );
    try {
  ...
  if (config.hasCodex) {
    ...
    try {
      ...
      printSuccess(
        `Codex plugin installed ${SYMBOLS.arrow} ${
          color.dim(codexResult.configPath)
        }`,
      );
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      ...
      printWarning(
        `Codex install failed (OpenCode install is still complete): ${message}`,
      );
    }
    console.log();
  }
  ...
  if (config.hasSenpi) {
    ...
    try {
      ...
      printSuccess(
        `Senpi adapter installed ${SYMBOLS.arrow} ${
          color.dim(senpiResult.settingsPath)
        }`,
      );
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
  ...
  printBox(
    `${color.bold("Pro Tip:")} Include ${color.cyan("ultrawork")} (or ${
      color.cyan("ulw")
    }) in your prompt.\n` +
...
async function maybePromptForGitHubStars(
  ...
  try {
    const answer = await readline.question(
      `${SYMBOLS.star} ${color.yellow("Star the repos on GitHub?")} ${
        color.dim("[y/N]")
      } `,
    );
  ...
  printWarning(
    "Could not star every repository. Make sure GitHub CLI is installed and authenticated.",
  );
