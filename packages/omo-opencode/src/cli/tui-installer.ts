...
import {
  promptInstallConfig,
  promptInstallPlatform,
} from "./tui-install-prompts";
import {
  detectCodexInstallation,
  formatCodexInstallationWarning,
...
export async function runTuiInstaller(
  ...
  const selectedPlatform = await promptInstallPlatform(
    args.platform ?? "opencode",
  );
  if (!selectedPlatform) return 1;

  const hasOpenCode = selectedPlatform === "opencode" ||
    selectedPlatform === "both";
  const detected = hasOpenCode ? detectCurrentConfig() : {
    isInstalled: false,
  ...
  if (hasOpenCode) {
    ...
    if (!installed) {
      ...
      const unsupportedVersionMessage = getUnsupportedOpenCodeVersionMessage(
        openCodeVersion,
      );
  ...
  if (config.hasOpenCode && !config.hasClaude) {
    p.log.info(
      `${
        color.bold("Note:")
      } Sisyphus agent performs best with Claude Opus 5.\n` +
  ...
  if (config.hasCodex) {
    ...
    try {
      ...
      spinner.stop(
        `Codex plugin installed to ${color.cyan(codexResult.configPath)}`,
      );
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      ...
      p.log.warn(
        `Codex install failed (OpenCode install remains successful): ${message}`,
      );
    }
  }
  ...
  if (config.hasSenpi) {
    ...
    try {
      ...
      spinner.stop(
        `Senpi adapter installed to ${color.cyan(senpiResult.settingsPath)}`,
      );
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
  ...
  p.note(
    `Include ${color.cyan("ultrawork")} (or ${
      color.cyan("ulw")
    }) in your prompt.\n` +
