...
function computeDiff(
  ...
  for (const key of allKeys) {
    ...
    if (hasInUser && !hasInDefaults) {
      ...
      results.push(
        ...computeDiff(userVal, defaultVal, unknownPaths, [...path, key]),
      );
...
export function runConfigDiff(
  options: { json?: boolean; path?: string },
): number {
  const validation = validatePluginConfig(process.cwd());
  const defaults = OhMyOpenCodeConfigSchema.parse({}) as Record<
    string,
    unknown
  >;
  const userConfig = validation.valid
    ? (validation.config as unknown as Record<string, unknown>)
    : {};

  const unknownPaths = findUnknownKeyPaths(
