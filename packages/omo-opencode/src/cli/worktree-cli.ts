...
async function resolveWorktreePath(name: string, explicitPath?: string): Promise<string> {
...
function parseWorktreeListLine(line: string): WorktreeEntry | null {
  ...
  const isDetached = remainder.startsWith("(") && remainder.includes("detached")
...
export async function runWorktreeList(): Promise<number> {
  ...
  if (result.code !== 0) {
    console.error(color.red(`git worktree list failed: ${result.stderr.trim()}`))
    return 1
  }
  ...
  console.log(`${headerPath.padEnd(padPath)} ${headerBranch.padEnd(padBranch)} ${headerMain}`)
...
export async function runWorktreeCreate(name: string, options?: { path?: string }): Promise<number> {
  if (!(await isGitAvailable())) {
    console.error(color.red("git is not available"))
    return 1
  }
  ...
  if (result.code !== 0) {
    console.error(color.red(`git worktree add failed: ${result.stderr.trim()}`))
    return 1
  }
...
export async function runWorktreeDelete(name: string): Promise<number> {
  ...
  const result = await runGit(["worktree", "remove", "--force", worktreePath], repoRoot)

  if (result.code !== 0) {
    console.error(color.red(`git worktree remove failed: ${result.stderr.trim()}`))
    return 1
  }
...
export async function runWorktreePrune(): Promise<number> {
  ...
  if (result.code !== 0) {
    console.error(color.red(`git worktree prune failed: ${result.stderr.trim()}`))
    return 1
  }
  ...
  if (orphans.length === 0) {
    ...
    console.log(color.green(`git worktree prune completed. ${orphans.length} orphan(s) removed.`))
  }
