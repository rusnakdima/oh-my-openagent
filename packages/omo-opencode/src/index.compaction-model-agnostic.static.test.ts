...
describe("experimental.session.compacting", () => {
  test("does not hardcode a model and uses output.context", () => {
    ...
    expect(
      `${content}\n${compactionContent}`.includes('modelID: "claude-opus-4-7"'),
    ).toBe(false);
  ...
  test("registers autocontinue restores before OpenCode synthetic continue", () => {
    ...
    //#then
    expect(hookIndex).toBeGreaterThanOrEqual(0);
    expect(compactionContent.includes("compactionContextInjector?.restore"))
      .toBe(true);
    expect(compactionContent.includes("compactionTodoPreserver?.restore")).toBe(
      true,
    );
  });
});
