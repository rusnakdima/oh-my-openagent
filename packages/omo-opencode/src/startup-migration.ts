...
function skippedConflictCount(results: readonly MigrationRunResult[]): number {
  return results.flatMap((result) => result.diagnostics).filter((diagnostic) =>
    diagnostic.startsWith("skipped:")
  ).length;
}
...
export function runOpenCodeStartupMigration(
  ...
  try {
    const batch = runMigrations({
      ...(options.afterMigrations === undefined
        ? {}
        : { afterMigrations: options.afterMigrations }),
