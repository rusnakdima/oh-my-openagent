...
describe("shared skills package manifest", () => {
  test("#given root package metadata #when shared-skills package is required #then workspace and tarball entries include it", async () => {
    ...
    const devDependency =
      rootPackageJson.devDependencies["@oh-my-opencode/shared-skills"];
  ...
  test("#given shared user skills #when copied into the package #then frontmatter and resource directories are preserved", async () => {
    ...
    const skillFiles = await Promise.all(
      copiedSkills.map(async (skillName) => ({
        ...
        content: await Bun.file(
          `packages/shared-skills/skills/${skillName}/SKILL.md`,
        ).text(),
      })),
    );
    ...
    expect(
      (await stat("packages/shared-skills/skills/debugging/references"))
        .isDirectory(),
    ).toBe(true);
    expect(
      (await stat(
        "packages/shared-skills/skills/coding-agent-sessions/scripts",
      )).isDirectory(),
    ).toBe(true);
    expect(
      (await stat(
        "packages/shared-skills/skills/coding-agent-sessions/references",
      )).isDirectory(),
    ).toBe(true);
    expect(
      (await stat("packages/shared-skills/skills/programming/references"))
        .isDirectory(),
    ).toBe(true);
    expect(
      (await stat("packages/shared-skills/skills/programming/scripts"))
        .isDirectory(),
    ).toBe(true);
  });
  ...
  test("#given coding-agent-sessions skill #when package exclusions are read #then dev fixtures and caches are excluded", async () => {
    ...
    const ignoreRules = new Set(
      (await Bun.file(
        "packages/shared-skills/skills/coding-agent-sessions/.npmignore",
      ).text())
        .split(/\r?\n/)
        .map((line) => line.trim())
    ...
    expect(
      (await stat(
        "packages/shared-skills/skills/coding-agent-sessions/scripts/find-agent-sessions.py",
      )).isFile(),
    ).toBe(true);
    expect(
      (await stat(
        "packages/shared-skills/skills/coding-agent-sessions/scripts/agent_sessions/cli.py",
      )).isFile(),
    ).toBe(true);
    expect(
      (await stat(
