...
export function createPluginInterface(args: {
  ...
  return {
    ...
    "chat.params": async (input: unknown, output: unknown) => {
      ...
      const agentName = typeof chatParamsInput.agent === "string"
        ? chatParamsInput.agent
        : chatParamsInput.agent?.name;
      const providerID = chatParamsInput.model?.providerID;
      const rawModelID = chatParamsInput.model?.modelID ??
        chatParamsInput.model?.id;
      const modelID = typeof rawModelID === "string" ? rawModelID : undefined;
      if (
        chatParamsInput.message && typeof providerID === "string" &&
        modelID !== undefined
      ) {
