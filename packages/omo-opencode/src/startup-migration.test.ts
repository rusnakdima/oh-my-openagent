...
import {
  ...
  parseFile,
} from "../../omo-config-core/src/migration/migration-test-support";
...
class CrossDeviceBackupFileSystem extends MemoryMigrationFileSystem {
  ...
  override renameSync(oldPath: string, newPath: string): void {
    if (
      this.backupRenameErrorCode !== undefined &&
      newPath.includes("/migration-backup-")
    ) {
      const error = new Error(
        `${this.backupRenameErrorCode}: backup rename failed '${oldPath}' -> '${newPath}'`,
      );
      Object.defineProperty(error, "code", {
        value: this.backupRenameErrorCode,
      });
      throw error;
    }
...
function run(
  fileSystem: MemoryMigrationFileSystem,
  options: { readonly onBoundary?: (boundary: MigrationBoundary) => void } = {},
) {
...
describe("runOpenCodeStartupMigration", () => {
  test("#given both legacy source groups #when startup migration runs #then it migrates, moves, and reports that the chain must reload", () => {
    ...
    fileSystem.files.set(
      rootSourcePath,
      JSON.stringify({ agents: { oracle: { model: "anthropic/legacy" } } }),
    );
    fileSystem.files.set(
      configJsoncPath,
    ...
    expect(
      fileSystem.existsSync(
        `${homeDir}/.omo/migration-backup-2026-07-27T12-34-56-789Z-opencode-config/.config/opencode/oh-my-openagent.json`,
      ),
    ).toBe(true);
    expect(
      fileSystem.existsSync(
        `${homeDir}/.omo/migration-backup-2026-07-27T12-34-56-789Z-opencode-config/.omo/config.jsonc`,
      ),
    ).toBe(true);
  });
  ...
  test("#given a legacy source on another filesystem #when its backup rename throws EXDEV #then startup copies and removes the source", () => {
    ...
    expect(fileSystem.existsSync(`${homeDir}/.omo/.migration-journal.json`))
      .toBe(false);
  });
  ...
  test("#given no legacy source #when startup migration runs #then it does not create a target or backups", () => {
    ...
    expect(
      [...fileSystem.files.keys()].filter((path) =>
        path.includes("migration-backup")
      ),
    ).toEqual([]);
  });
  ...
  test("#given an invalid transformed document #when startup migration runs #then validation aborts before target and source changes", () => {
    ...
    fileSystem.files.set(
      configJsoncPath,
      JSON.stringify({ codegraph: { excluded_roots: [42] } }),
    );
  ...
  test("#given a journal recorded the target write #when recovery crosses filesystems #then it copies the backup before evaluating migration sources", () => {
    ...
    const crashed = run(fileSystem, {
      onBoundary: (boundary) => {
        if (boundary === "target-recorded") {
          throw new Error("crash after target recorded");
        }
      },
    });
    expect(crashed.error).toBe("crash after target recorded");
    expect(
      parseFile(fileSystem, `${homeDir}/.omo/.migration-journal.json`)
        .targetWritten,
    ).toBe(true);
    fileSystem.backupRenameErrorCode = "EXDEV";
