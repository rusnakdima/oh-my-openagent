...
import {
  buildShellAwareGitPrefix,
  injectGitMasterConfig,
  parseBashEnvPrefix,
} from "./git-master-template-injection";
...
describe("injectGitMasterConfig", () => {
  it("propagates the configured Unix env prefix to template commands", () => {
    ...
    expectCommandsPrefixed(
      result,
      buildShellAwareGitPrefix(gitEnvPrefix, "unix"),
    );
  });
  ...
  it("uses PowerShell routing when PowerShell is detected", () => {
    ...
    expect(result).toContain(
      buildShellAwareGitPrefix(gitEnvPrefix, "powershell"),
    );
    for (const command of TEMPLATE_COMMANDS) expect(result).toContain(command);
  });
...
describe("parseBashEnvPrefix", () => {
  ...
  it("parses multiple assignments", () => {
    expect(parseBashEnvPrefix("CI=true DEBIAN_FRONTEND=noninteractive"))
      .toEqual({ CI: "true", DEBIAN_FRONTEND: "noninteractive" });
  });
...
describe("buildShellAwareGitPrefix", () => {
  ...
  it("renders multiple assignments", () => {
    expect(buildShellAwareGitPrefix("CI=true GIT_MASTER=1", "powershell")).toBe(
      "$env:CI='true'; $env:GIT_MASTER='1';",
    );
  });
