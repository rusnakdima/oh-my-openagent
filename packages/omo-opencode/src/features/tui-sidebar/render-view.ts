    ...
    case "live":
      return [
        section("ULW", theme, [
          ...
          text(
            { fg: theme.accent },
            `active ${truncate(activeGoalLabel(loop.activeGoal))}`,
          ),
        ]),
      ];
    default:
      ...
      return [
        section(
          ...
          agents.agents.map((agent) =>
            text(
              { fg: theme.text },
              `${truncate(agent.name)} (${agent.mode ?? "subagent"}) — ${
                truncate(agent.model ?? "—")
              } ${agent.status}`,
            )
          ),
        ),
  ...
  switch (agents.kind) {
    ...
    case "list":
      return [
        "Agents",
        ...agents.agents.map((agent) =>
          `${agent.name} (${agent.mode ?? "subagent"}) — ${
            agent.model ?? "—"
          } ${agent.status}`
        ),
      ];
      ...
      return [
        section(
          ...
          jobs.jobs.map((job) =>
            text(
              { fg: theme.text },
              `${truncate(job.title)} ${job.status} ${job.toolCalls ?? 0} ${
                job.lastTool ?? "none"
              }`,
            )
          ),
  ...
  switch (jobs.kind) {
    ...
    case "list":
      return jobs.jobs.flatMap((job) => [
        "Jobs",
        `${job.title} ${job.status} calls ${job.toolCalls ?? 0} last ${
          job.lastTool ?? "none"
        }`,
      ]);
    default:
  ...
  return [
    section("Config", theme, [
      ...
      ...messages.map((message) =>
        text({ fg: theme.textMuted }, truncate(message))
      ),
    ]),
  ...
  switch (roster.kind) {
    ...
    case "rows":
      return roster.rows.map((row) => {
        ...
        if (row.model === "—" && !row.hasOverride) {
          return `${labelPart} ${modePart}`;
        }
...
function truncate(value: string): string {
  return value.length <= LABEL_MAX
    ? value
    : `${value.slice(0, LABEL_MAX - 3)}...`;
}
