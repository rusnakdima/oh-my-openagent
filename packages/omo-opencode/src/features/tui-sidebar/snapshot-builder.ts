...
export type SessionAgentResolver = (
  sessionID: string,
...
async function activeAgentsFromStatuses(
  ...
  return Promise.all(
    rows.map(async (row) => {
      const name = (await sessionAgentResolver(row.sessionID, client)) ??
        row.sessionID;
      const effective = getEffectiveModelForAgent(name);
      const fallbackModel = getSessionModel(row.sessionID);
      const model = formatAgentModel(effective) ??
        (fallbackModel
...
function redactLoopText(
  loop: TuiRuntimeSnapshot["loop"],
): TuiRuntimeSnapshot["loop"] {
  if (loop === null) {
    return null;
  }
