...
export class TaskToastManager {
  ...
  private buildTaskListMessage(newTask: TrackedTask): string {
    ...
    if (isFallback) {
      ...
      lines.push(
        t("toast.fallback_prefix", { model: newTask.modelInfo!.model, suffix }),
      );
      lines.push("");
    }
    ...
    if (running.length > 0) {
      lines.push(
        t("toast.task_list_running", { count: running.length }) +
          concurrencyInfo,
      );
    ...
    if (queued.length > 0) {
      ...
      for (const task of queued) {
        ...
        const isNew = task.id === newTask.id ? t("toast.task_list_new") : "";
        lines.push(
          `${bgIcon} ${task.description} (${taskId})${skillsInfo} - ${
   ...
   */
  showCompletionToast(
    task: { id: string; description: string; duration: string },
  ): void {
    const tuiClient = this.client as ClientWithTui;
    if (!tuiClient.tui?.showToast) return;
    ...
    let message = t("toast.task_completion_message", {
      description: task.description,
      duration: task.duration,
    });
    if (remaining.length > 0 || queued.length > 0) {
      message += `\n\n${
        t("toast.task_completion_remaining", {
          running: remaining.length,
  ...
  showToast(
    opts: { title: string; message: string; variant: string; duration: number },
  ): void {
    const tuiClient = this.client as ClientWithTui;
    if (!tuiClient.tui?.showToast) return;
