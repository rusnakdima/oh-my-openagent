...
export function createContextInjectorHook(collector: ContextCollector) {
  return {
    "chat.message": async (
      ...
      const result = injectPendingContext(
        collector,
        input.sessionID,
        output.parts,
      );
...
function getSessionIDFromMessageInfo(info: Message): string | undefined {
  return "sessionID" in info && typeof info.sessionID === "string"
    ? info.sessionID
    : undefined;
}
    ...
    "experimental.chat.messages.transform": async (_input, output) => {
      ...
      if (!isRealUserMessage(lastUserMessage)) {
        log(
          ...
          {
            sessionID: getSessionIDFromMessageInfo(lastUserMessage.info) ??
              getMainSessionID(),
          },
        );
        return;
      }
      const messageSessionID = getSessionIDFromMessageInfo(
      ...
      if (textPartIndex === -1) {
        log(
          "[context-injector] No text part found in last user message, skipping injection",
          {
            sessionID,
