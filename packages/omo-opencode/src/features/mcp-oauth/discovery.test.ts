...
describe("discoverOAuthServerMetadata", () => {
  ...
  test("returns endpoints from PRM + AS discovery", () => {
    ...
    const prmUrl = new URL("/.well-known/oauth-protected-resource", resource)
      .toString();
    const authServer = "https://auth.example.com";
    const asUrl = new URL("/.well-known/oauth-authorization-server", authServer)
      .toString();
    const calls: string[] = [];
  ...
  test("falls back to RFC 8414 when PRM returns 404", () => {
    ...
    const prmUrl = new URL("/.well-known/oauth-protected-resource", resource)
      .toString();
    const asUrl = new URL("/.well-known/oauth-authorization-server", resource)
      .toString();
    const calls: string[] = [];
    const fetchMock = async (input: string | URL) => {
      const url = typeof input === "string" ? input : input.toString();
  ...
  test("throws when PRM, path-suffixed AS, and root AS all return 404", () => {
    ...
    const fetchMock = async (input: string | URL) => {
      const url = typeof input === "string" ? input : input.toString();
      if (
        url === prmUrl || url.includes(".well-known/oauth-authorization-server")
      ) {
        return new Response("not found", { status: 404 });
      }
      return new Response("not found", { status: 404 });
