...
type ParentWakePromptDispatchInput = {
  ...
  readonly hasRecordedPromptAfterDispatch: (
    wake: PendingParentWake,
  ) => Promise<boolean>;
  readonly trackDispatchedWake: (
    wake: PendingParentWake,
    dispatchedAt: number,
  ) => void;
...
export async function sendParentWakePrompt(
  ...
  try {
    ...
    const promptResult = await dispatchInternalPrompt({
      ...
      checkStatus: input.forceNoReply !== true &&
        input.skipPromptGateStatusCheck !== true,
    ...
    if (promptResult.status === "failed") {
      if (isAmbiguousPostDispatchPromptFailure(promptResult)) {
        ...
        if (await input.hasRecordedPromptAfterDispatch(dispatchedWake)) {
          ...
          input.trackDispatchedWake(
            createTrackedDispatchedWake(input.latestWake, input.forceNoReply),
            dispatchStartedAt,
          );
          log(
    ...
    if (
      ...
      if (
        dispatchedWake &&
        isRedundantParentWake(input.latestWake, dispatchedWake)
      ) {
        log(
    ...
    log("[background-agent] Sent deferred parent wake:", {
      sessionID: input.sessionID,
    });
    ...
    const errorText = error instanceof Error
      ...
      : getErrorText(error) || String(error);
    input.requeueWake(input.latestWake);
    input.scheduleFlush();
...
function markRetainedNoReplyAdmission(
  input: ParentWakePromptDispatchInput,
  dispatchStartedAt: number,
): void {
  if (
    input.retainPendingWake !== true || input.forceNoReply !== true ||
    !input.latestWake.shouldReply
  ) {
    return;
  }
...
function createTrackedDispatchedWake(
  wake: PendingParentWake,
  forceNoReply: boolean | undefined,
): PendingParentWake {
