...
export function pruneStaleTasksAndNotifications(args: {
  ...
  for (const [taskId, task] of tasks.entries()) {
    ...
    const sessionStatus = task.sessionId
      ? args.sessionStatuses?.[task.sessionId]?.type
      : undefined;
    if (
      task.status === "running" && sessionStatus !== undefined &&
...
async function interruptStaleTask(args: {
  ...
  if (!aborted) {
    log(
      "[background-agent] Task stale interruption skipped because session abort failed:",
      {
        taskId: task.id,
        sessionID,
        reason,
  ...
  task.error =
    `Stale timeout (${reason} for ${staleMinutes}min${errorSuffix}). This is a FINAL cancellation - do NOT create a replacement task. If the timeout is too short, increase 'background_task.${timeoutConfigKey}' in .omo/omo.jsonc.`;
...
export async function checkAndInterruptStaleTasks(args: {
  ...
  for (const task of tasks) {
    ...
    const sessionMissing = sessionStatuses !== undefined &&
    ...
    const sessionGone = sessionMissing &&
      (task.consecutiveMissedPolls ?? 0) >= MIN_SESSION_GONE_POLLS;
    const shouldSkipInactivityTimeout = task.teamRunId !== undefined &&
      !sessionGone;
    ...
    if (!task.progress?.lastUpdate) {
      ...
      if (shouldRefreshFromSessionActivity) {
        ...
        if (
          activityRefresh.type === "activity" &&
          now - activityRefresh.activityTime <= effectiveTimeout
        ) continue;
      }
    ...
    if (shouldRefreshFromSessionActivity) {
      ...
      const refreshedLastUpdate = task.progress?.lastUpdate.getTime() ??
        (activityRefresh.type === "activity"
          ? activityRefresh.activityTime
          : undefined);
      if (
        refreshedLastUpdate !== undefined &&
        now - refreshedLastUpdate <= effectiveStaleTimeout
      ) continue;
