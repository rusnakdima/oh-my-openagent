...
type ParentWakeNotifierClient = ConstructorParameters<
...
function createNotifier(
  messages: readonly SessionMessageStub[],
): ParentWakeNotifier {
...
describe("ParentWakeNotifier assistant error history", () => {
  ...
  test("requeues late session.error when history only shows the injected wake plus an assistant error turn with no parts", async () => {
    ...
    try {
      ...
      expect(
        notifier.getPendingParentWakes().get("parent-assistant-error-history")
          ?.notifications,
      ).toEqual([FINAL_WAKE]);
      expect(
        notifier.getDispatchedParentWakes().has(
