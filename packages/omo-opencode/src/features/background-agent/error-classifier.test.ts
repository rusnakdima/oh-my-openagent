...
describe("isAbortedSessionError", () => {
  describe("#given error with aborted message", () => {
    ...
    test("returns true for object with message containing aborted", () => {
      expect(isAbortedSessionError({ message: "The session was aborted" }))
        .toBe(true);
    });
  });
...
describe("getErrorText", () => {
  ...
  describe("#given object with message property", () => {
    ...
    test("prefers message over name", () => {
      expect(getErrorText({ name: "CustomError", message: "error message" }))
        .toBe("error message");
    });
  });
...
describe("getSessionErrorMessage", () => {
  ...
  describe("#given missing or invalid properties", () => {
    ...
    test("returns undefined when data.message is not string", () => {
      expect(getSessionErrorMessage({ error: { data: { message: null } } }))
        .toBe(undefined);
    });
  });
});
...
describe("isTerminalSessionError", () => {
  ...
  describe("#given name and message together", () => {
    ...
    test("does NOT classify as terminal when only a generic error name is present", () => {
      expect(isTerminalSessionError({ name: "ProviderNotFoundError" })).toBe(
        false,
      );
    });
  });
