...
type ParentWakeNotifierForMarkerTest = {
  ...
  readonly requeueDispatchedParentWakeAfterEmptyAssistantTurn: (
    sessionID: string,
  ) => boolean;
};
...
async function waitForBackgroundTaskMarkerState(
  ...
  throw new Error(
    `Timed out waiting for background-task marker ${expectedState}; last state: ${
...
describe("BackgroundManager run continuation marker parent-wake races", () => {
  test("#given no active child tasks but a completion parent wake is preparing #when refreshing the run marker #then the marker remains active", () => {
    ...
    const internals = unsafeTestValue<BackgroundManagerMarkerInternals>(
      manager,
    );
    internals.parentWakeNotifier.reserveNotificationPreparation(
      parentSessionID,
    );
    ...
    try {
      ...
      expect(marker?.sources["background-task"]?.reason).toBe(
        BACKGROUND_COMPLETION_WAKE_PENDING_REASON,
      );
    } finally {
      internals.parentWakeNotifier.releaseNotificationPreparation(
        parentSessionID,
      );
      manager.shutdown();
    }
  ...
  test("#given a completion parent wake is queued #when dispatch accepts it #then the marker returns idle", async () => {
    ...
    try {
      ...
      expect(queuedMarker?.sources["background-task"]?.reason).toBe(
        BACKGROUND_COMPLETION_WAKE_PENDING_REASON,
      );
      ...
      // then
      const dispatchedMarker = readContinuationMarker(
        directory,
        parentSessionID,
      );
  ...
  test("#given parent notifications are disabled #when final child completion releases preparation #then the marker returns idle", async () => {
    ...
    try {
      // when
      const completed = await internals.tryCompleteTask(
        task,
        "marker regression",
      );
  ...
  test("#given a completion parent wake is queued on the scheduled timer #when the timer flush accepts it #then the marker returns idle", async () => {
    ...
    try {
      ...
      await waitForBackgroundTaskMarkerState(
        directory,
        parentSessionID,
        "idle",
      );
  ...
  test("#given a dispatched completion wake is requeued after session.error #when the prompt failure requeue returns #then the marker is active immediately", async () => {
    ...
    try {
      ...
      // when
      const requeued = await internals.parentWakeNotifier
        .requeueDispatchedParentWake(
          parentSessionID,
          "session.error",
        );
      ...
      expect(requeuedMarker?.sources["background-task"]?.reason).toBe(
        BACKGROUND_COMPLETION_WAKE_PENDING_REASON,
      );
    } finally {
      manager.shutdown();
  ...
  test("#given a dispatched completion wake is requeued after an empty assistant turn #when the retry is queued #then the marker is active immediately", async () => {
    ...
    try {
      ...
      // when
      const requeued = internals.parentWakeNotifier
        .requeueDispatchedParentWakeAfterEmptyAssistantTurn(parentSessionID);
