...
const PARENT_WAKE_SESSION_ACTIVITY_IN_PROGRESS_WINDOW_MS =
  PARENT_WAKE_TOOL_CALL_DEFER_MAX_MS;
...
function formatAttemptModelSummary(
  attempt: Pick<BackgroundTaskAttempt, "providerId" | "modelId"> | undefined,
): string | undefined {
...
function getPreviousAttempt(
  ...
  const attemptIndex = task.attempts.findIndex((attempt) =>
...
export type OnSubagentSessionDeleted = (
  event: SubagentSessionDeletedEvent,
) => Promise<void>;
...
export class BackgroundManager {
  ...
  private completedTaskSummaries: Map<
    string,
  ...
  private readonly scheduledFlushSettledWaiters = new Map<
    string,
    Array<() => void>
  >();
  ...
  constructor(config: BackgroundManagerConfig) {
    ...
    this.modelFallbackControllerAccessor = options
    ...
    this.parentWakeNotifier = new ParentWakeNotifier(
      {
        ...
        enqueueNotificationForParent: this.enqueueNotificationForParent.bind(
          this,
        ),
        onPendingWakeRequeued: (sessionID) =>
          this.updateBackgroundTaskMarker(sessionID),
        onScheduledFlushSettled: (sessionID) =>
      ...
      },
      {
        ...
        userMessageInProgressWindowMs:
          PARENT_WAKE_USER_MESSAGE_IN_PROGRESS_WINDOW_MS,
        parentSessionActivityInProgressWindowMs:
          PARENT_WAKE_SESSION_ACTIVITY_IN_PROGRESS_WINDOW_MS,
      },
    );
  ...
  private async abortSessionWithLogging(
    ...
    try {
      ...
      if (!aborted) {
        log(
          `[background-agent] Session abort did not complete during ${reason}:`,
          {
            sessionID,
          },
        );
  ...
  async assertCanSpawn(parentSessionID: string): Promise<SubagentSpawnContext> {
    const spawnContext = await resolveSubagentSpawnContext(
      this.client,
      parentSessionID,
      this.directory,
    );
  ...
  async reserveSubagentSpawn(parentSessionID: string): Promise<{
    ...
    const descendantCount = this.registerRootDescendant(
      spawnContext.rootSessionID,
    );
    let settled = false;
  ...
  private addTask(task: BackgroundTask): void {
    ...
    const taskIDs = this.tasksByParentSession.get(task.parentSessionId) ??
      new Set<string>();
    taskIDs.add(task.id);
  ...
  private restoreTaskAfterSkippedResume(
    ...
    skippedStatus: Exclude<
      PromptAsyncGateResult["status"],
  ...
  async launch(input: LaunchInput): Promise<BackgroundTask> {
    ...
    try {
      ...
      if (input.parentSessionId) {
        const pending = this.pendingByParent.get(input.parentSessionId) ??
          new Set();
        pending.add(task.id);
      ...
      while (queue && queue.length > 0) {
        ...
        try {
          await this.concurrencyManager.acquire(
            item.rawConcurrencyKey ?? key,
            item.task.id,
          );
        } catch (error) {
          if (
            item.task.status === "cancelled" || item.task.status === "error" ||
        ...
        try {
          ...
          if (item.task.currentAttemptID) {
            ...
            item.task.error = error instanceof Error
              ? error.message
              : String(error);
          ...
          this.enqueueNotificationForParent(
            item.task.parentSessionId,
            () => this.notifyParentSession(item.task),
          ).catch((err) => {
  ...
  private async startTask(item: QueueItem): Promise<void> {
    ...
    const attemptID = item.attemptID ??
      ensureCurrentAttempt(task, input.model).attemptId;
    ...
    const boundAttempt = bindAttemptSession(
      task,
      attemptID,
      sessionID,
      input.model,
    ...
    if (task.retryNotification) {
      ...
      const failedSessionID = previousAttempt?.sessionId ??
        task.retryNotification.previousSessionID;
      ...
      const failedModel = formatAttemptModelSummary(previousAttempt) ??
        task.retryNotification.failedModel;
      const failedModelLine = failedModel
        ? `\n- Failed model: \`${failedModel}\``
        : "";
      const failedError = previousAttempt?.error ??
        task.retryNotification.failedError;
      const failedErrorLine = failedError ? `\n- Error: ${failedError}` : "";
      const retryModel = formatAttemptModelSummary(boundAttempt) ??
        task.retryNotification.nextModel;
      const parentPromptContext = await this.resolveParentWakePromptContext(
        task,
      );
...
**Retry link:** ${retrySessionUrl}${failedSessionLine}${failedModelLine}${failedErrorLine}${
          retryModel ? `\n- Model: \`${retryModel}\`` : ""
        }
...
The fallback retry session is now created and can be inspected directly.
    ...
    promptWithRetryInDirectory(this.client, {
      ...
      if (resolvedTask && !resolvedTask.isCurrent) {
        log(
          "[background-agent] Ignoring prompt error from stale attempt session",
          {
            sessionID,
      ...
      if (existingTask) {
        ...
        const errorMessage = errorInfo.message ??
          (error instanceof Error ? error.message : String(error));
        const terminalError = errorMessage.includes("agent.name") ||
            errorMessage.includes("undefined") || isAgentNotFoundError(error)
  ...
  getTask(id: string): BackgroundTask | undefined {
    return this.tasks.get(id) ?? this.completedTaskArchive.get(id) ??
      getRegisteredBackgroundTask(id);
  }
  ...
  hasActiveChildTasks(sessionID: string): boolean {
    return this.getTasksByParentSession(sessionID).some((t) =>
  ...
  hasPendingParentWake(sessionID: string): boolean {
    return this.hasUndeliveredParentWake(sessionID) ||
  ...
  private resolveTaskAttemptBySession(
    ...
    | { task: BackgroundTask; attemptID?: string; isCurrent: boolean }
    | undefined {
  ...
  private getConcurrencyKeyFromInput(input: LaunchInput): string {
    return this.concurrencyManager.getConcurrencyKey(
      this.getRawConcurrencyKeyFromInput(input),
    );
  }
  ...
  private getRawConcurrencyKeyFromTask(
    task: Pick<BackgroundTask, "model" | "agent">,
  ): string {
    return task.model
  ...
  async resume(input: ResumeInput): Promise<BackgroundTask> {
    ...
    dispatchInternalPrompt({
      ...
      if (promptResult.status !== "dispatched") {
        ...
        this.restoreTaskAfterSkippedResume(
          existingTask,
          resumeSnapshot,
          promptResult.status,
        );
  ...
  private async checkSessionTodos(sessionID: string): Promise<boolean> {
    const observedIncompleteTodos = this.observedIncompleteTodosBySession.get(
      sessionID,
    );
  ...
  private async requeueDispatchedParentWake(
    sessionID: string,
    reason: string,
  ): Promise<boolean> {
    return this.parentWakeNotifier.requeueDispatchedParentWake(
      sessionID,
      reason,
    );
  }
  ...
  private shouldHoldDispatchedParentWakeForTextDelta(
    ...
    const candidate = `${
      this.parentWakeTextDeltaBuffers.get(key) ?? ""
    }${partInfo.delta}`;
    const expectedInternalWakeText =
      createInternalAgentTextPart(wake.notifications.join("\n\n")).text;
    const expectedVisibleInternalWakeText = expectedInternalWakeText.replace(
      /<\/?system-reminder>/g,
      "",
    );
    const shouldHold = expectedInternalWakeText.startsWith(candidate) ||
  ...
  handleEvent(event: Event): void {
    ...
    if (event.type === "message.updated") {
      ...
      if (isEmptyNoProgressAssistantTurnInfo(info)) {
        const dispatchedWake = this.parentWakeNotifier
          .getDispatchedParentWakes().get(sessionID);
        if (dispatchedWake) {
          this.parentWakeNotifier
            .requeueDispatchedParentWakeAfterEmptyAssistantTurn(sessionID);
          return;
        }
      }
      ...
      void this.tryFallbackRetry(task, errorInfo, "message.updated").catch(
        (error) => {
          log(
            "[background-agent] Error handling message.updated fallback retry:",
            {
              error,
              taskId: task.id,
            },
    ...
    if (
      ...
      const isInternalWakePart = isInternalInitiatorTextPart(
        partInfo,
        sessionID,
      );
      const dispatchedWake = this.parentWakeNotifier.getDispatchedParentWakes()
        .get(sessionID);
      const holdDispatchedWakeForTextDelta = this
      ...
      const hasParentWakeOutput =
        hasOutputSignalFromPart(partInfo, sessionID) &&
        !isUserPart &&
        !isInternalWakePart &&
      ...
      if (
        !isUserPart && !isInternalWakePart && !holdDispatchedWakeForTextDelta
      ) {
        this.parentWakeNotifier.recordParentSessionActivity(sessionID);
      }
      ...
      if (partInfo?.type === "tool" || partInfo?.tool) {
        const countedToolPartIDs = task.progress.countedToolPartIDs ??
          new Set<string>();
        const shouldCountToolCall = !partInfo.id ||
        ...
        const circuitBreaker = this.cachedCircuitBreakerSettings ??
          resolveCircuitBreakerSettings(this.config);
        ...
        if (partInfo.tool) {
          ...
          if (circuitBreaker.enabled) {
            const loopDetection = detectRepetitiveToolUse(
              task.progress.toolCallWindow,
            );
            if (loopDetection.triggered) {
              log(
                "[background-agent] Circuit breaker: consecutive tool usage detected",
                {
                  taskId: task.id,
                  agent: task.agent,
                  sessionID,
              ...
              void this.cancelTask(task.id, {
                ...
                reason:
                  `Subagent called ${loopDetection.toolName} ${loopDetection.repeatedCount} consecutive times (threshold: ${circuitBreaker.consecutiveThreshold}). This usually indicates an infinite loop. The task was automatically cancelled to prevent excessive token usage.`,
              });
              return;
            }
          }
        ...
        if (task.progress.toolCalls >= maxToolCalls) {
          ...
          void this.cancelTask(task.id, {
            source: "circuit-breaker",
            reason:
              `Subagent exceeded maximum tool call limit (${maxToolCalls}). This usually indicates an infinite loop. The task was automatically cancelled to prevent excessive token usage.`,
          });
        }
      }
    }
    ...
    if (event.type === "session.idle") {
      ...
      const sessionID = resolveSessionEventID(props);
      if (sessionID) {
        void this.enqueueNotificationForParent(
          sessionID,
          () => this.flushPendingParentWake(sessionID),
        ).catch((error) => {
          log("[background-agent] Failed to flush pending parent wake:", {
            sessionID,
            error,
          });
        });
    ...
    if (event.type === "session.error") {
      ...
      if (
        ...
        void this.requeueDispatchedParentWake(sessionID, "session.error")
          ...
          .catch((error) => {
            log(
              "[background-agent] Failed to requeue dispatched parent wake:",
              { sessionID, error },
            );
          });
        return;
      ...
      for (const task of tasksToCancel.values()) {
        ...
        if (task.status === "running" || task.status === "pending") {
          void this.cancelTask(task.id, {
            ...
            log(
              "[background-agent] Failed to cancel task on session.deleted:",
    ...
    if (event.type === "session.status") {
      ...
      void this.tryFallbackRetry(task, errorInfo, "session.status").catch(
        (error) => {
          log(
            "[background-agent] Error handling session.status fallback retry:",
            {
              error,
              taskId: task.id,
            },
  ...
  private async interruptTaskFromAsyncPromptFailure(
    ...
    if (notificationParentSessionID) {
      this.parentWakeNotifier.reserveNotificationPreparation(
        notificationParentSessionID,
      );
    }
    const releaseNotificationPreparation = (): void => {
      if (notificationParentSessionID) {
        this.parentWakeNotifier.releaseNotificationPreparation(
          notificationParentSessionID,
        );
    ...
    this.enqueueNotificationForParent(
      ...
      log("[background-agent] Failed to notify on async prompt failure:", {
        taskId: task.id,
        error: err,
      });
  ...
  private async handleSessionErrorEvent(args: {
    ...
    if (!task.fallbackChain && task.sessionId) {
      const sessionFallbackChain = this.modelFallbackControllerAccessor
        ?.getSessionFallbackChain(task.sessionId);
    ...
    if (sessionId) {
      ...
      if (sessionStillAlive && !isTerminalSessionError(errorInfo)) {
        this.logger(
          "[background-agent] session.error received but session still alive, treating as transient:",
          {
            taskId: task.id,
            sessionId,
      ...
      if (sessionStillAlive && isTerminalSessionError(errorInfo)) {
        this.logger(
          "[background-agent] Finalizing task after terminal session.error (session shell alive but will never produce output):",
          {
            taskId: task.id,
            sessionId,
            errorName,
...
**ID:** \`${task.id}\`
**Description:** ${task.description}${sourceText}${failedSessionLine}${failedModelLine}${failedErrorLine}${
...
The task was re-queued on a fallback model after a retryable failure.
    ...
    const timer = setTimeout(() => {
      ...
      if (task.parentSessionId) {
        ...
        const runningOrPendingSiblings = siblings.filter(
          (sibling) =>
            sibling.id !== taskId &&
            (sibling.status === "running" || sibling.status === "pending"),
        );
        const completedAtTimestamp = task.completedAt?.getTime();
        const reachedTaskTtl = completedAtTimestamp !== undefined &&
          (Date.now() - completedAtTimestamp) >= TASK_TTL_MS;
        if (
          runningOrPendingSiblings.length > 0 &&
          rescheduleCount < MAX_TASK_REMOVAL_RESCHEDULES && !reachedTaskTtl
        ) {
  ...
  async cancelTask(
    ...
    if (wasRunning && abortSession && task.sessionId) {
      const aborted = await this.abortSessionWithLogging(
        task.sessionId,
  ...
  private async tryCompleteTask(
    ...
    try {
      ...
      if (task.sessionId) {
        ...
        await this.onSubagentSessionDeleted?.({ sessionID: task.sessionId })
          .catch((error) => {
            log(
              "[background-agent] onSubagentSessionDeleted callback failed:",
              {
                taskId: task.id,
                sessionID: task.sessionId,
                error: String(error),
  ...
  private async notifyParentSession(task: BackgroundTask): Promise<void> {
    ...
    if (pendingSet) {
      ...
      remainingCount = Array.from(this.tasks.values())
        .filter((t) =>
          t.parentSessionId === task.parentSessionId && t.id !== task.id &&
          (t.status === "running" || t.status === "pending")
        )
        .length;
      allComplete = remainingCount === 0;
    ...
    if (this.enableParentSessionNotifications) {
      ...
      const isTaskFailure = task.status === "error" ||
        task.status === "cancelled" || task.status === "interrupt";
      const shouldReply = (this.config?.wakeOnEachCompletion ?? false) ||
        allComplete || isTaskFailure;
      ...
      if (shouldDeferNotification) {
        ...
        log(
          "[background-agent] Queued notification while parent session is active:",
          {
            taskId: task.id,
            allComplete,
            isTaskFailure,
        ...
        log(
          "[background-agent] Queued notification for short-debounce flush to idle parent:",
          {
            taskId: task.id,
            allComplete,
            isTaskFailure,
      ...
      log(
        "[background-agent] Parent session notifications disabled, skipping prompt injection:",
        {
          taskId: task.id,
          parentSessionID: task.parentSessionId,
        },
  ...
  private async resolveParentWakePromptContext(
    task: BackgroundTask,
  ): Promise<ParentWakePromptContext> {
    ...
    const resolvedTools = resolveInheritedPromptTools(
      task.parentSessionId,
      tools,
    );
    return {
  ...
  private async isSessionActive(sessionID: string): Promise<boolean> {
    ...
    return isOpenCodeSessionActive(
      resolved.client as Parameters<typeof isOpenCodeSessionActive>[0],
      sessionID,
    );
  }
  ...
  private recordScheduledFlushSettled(sessionID: string): void {
    this.updateBackgroundTaskMarker(sessionID);
    this.scheduledFlushSettledCounts.set(
      sessionID,
      (this.scheduledFlushSettledCounts.get(sessionID) ?? 0) + 1,
    );
  ...
  awaitScheduledFlush(sessionID: string, sinceCount: number): Promise<void> {
    return new Promise<void>((resolve) => {
      const arm = (): void => {
        if (
          (this.scheduledFlushSettledCounts.get(sessionID) ?? 0) > sinceCount
        ) {
          resolve();
          return;
        }
  ...
  private pruneStaleTasksAndNotifications(
    ...
    pruneStaleTasksAndNotifications({
      ...
      onTaskPruned: (taskId, task, errorMessage) => {
        ...
        if (wasPending) {
          const key = this.concurrencyManager.getConcurrencyKey(
            this.getRawConcurrencyKeyFromTask(task),
          );
          const queue = this.queuesByKey.get(key);
          if (queue) {
            const index = queue.findIndex((item) => item.task.id === taskId);
  ...
  private async failCrashedTask(
    ...
    this.enqueueNotificationForParent(
      ...
      log("[background-agent] Error in notifyParentSession for crashed task:", {
        taskId: task.id,
        error: err,
      });
    });
    ...
    try {
      ...
      if (typeof sessionStatusMethod !== "function") {
        ...
        try {
          ...
          if (!this.loggedSessionStatusUnavailable) {
            log("[background-agent] Error polling session statuses:", {
              error,
            });
            this.loggedSessionStatusUnavailable = true;
          }
      ...
      for (const task of this.tasks.values()) {
        ...
        try {
          ...
          const sessionGoneFromStatus = allStatuses !== undefined &&
            !sessionStatus;
  ...
  async shutdown(): Promise<void> {
    ...
    const abortRequests: Array<
      { sessionID: string; promise: Promise<unknown> }
    > = [];
    ...
    if (abortRequests.length > 0) {
      const abortResults = await Promise.allSettled(
        abortRequests.map((request) => request.promise),
      );
  ...
  private enqueueNotificationForParent(
    ...
    const previous = this.notificationQueueByParent.get(parentSessionID) ??
      Promise.resolve();
    ...
    const current = previous
      .catch((error) => {
        log(
          "[background-agent] Continuing notification queue after previous failure:",
          {
            parentSessionID,
            error,
          },
