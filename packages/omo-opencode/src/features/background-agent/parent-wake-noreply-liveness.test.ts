...
function releaseParentWakeHold(sessionID: string): void {
  releasePromptAsyncReservation(
    sessionID,
    "test:simulate-expired-parent-wake-hold",
    {
...
describe("parent wake noReply admission liveness (issues #4874/#5086)", () => {
  test("#given all-complete wake admitted as noReply during history deferral #then reply liveness is retained and resumes once safe", async () => {
    ...
    notifier.queuePendingParentWake("parent-1", FINAL_WAKE, {
      agent: "sisyphus",
    }, true);
    ...
    try {
      ...
      expect(promptAsyncCalls).toHaveLength(1);
      expect(notifier.getPendingParentWakes().get("parent-1")?.shouldReply)
        .toBe(true);
      expect(notifier.getPendingParentWakeTimers().has("parent-1")).toBe(true);
      ...
      expect(JSON.stringify(promptAsyncCalls[1]?.body.parts)).toContain(
        "ALL BACKGROUND TASKS COMPLETE",
      );
  ...
  test("#given noReply admission of a reply-required wake #then the dispatched tracker does not claim reply coverage", async () => {
    ...
    try {
      ...
      expect(promptAsyncCalls).toHaveLength(1);
      expect(notifier.getDispatchedParentWakes().get("parent-1")?.shouldReply)
        .toBe(false);
    } finally {
      Date.now = originalDateNow;
      notifier.shutdown();
  ...
  test("#given failure wake while parent is unsafe #then it stays pending without admission and resumes with a reply once safe", async () => {
    ...
    notifier.queuePendingParentWake("parent-1", FAILURE_WAKE, {
      agent: "sisyphus",
    }, true);
    ...
    const { notifier, promptAsyncCalls } = createNotifier({
      ...
      messagesProvider: () => [
        ...SAFE_MESSAGES,
        {
          info: {
            role: "user",
            time: { created: userMessageFresh ? 99_900 : 80_500 },
          },
...
describe("parent wake admitted-consumption drop (duplicate ALL-COMPLETE regression)", () => {
  test("#given admitted wake consumed by live turn output #when stale tool-call deferral would force a resume #then no duplicate reply dispatch and the wake is dropped", async () => {
    ...
    try {
      ...
      expect(promptAsyncCalls[0]?.body.noReply).toBe(true);
      expect(
        notifier.getPendingParentWakes().get("parent-1")?.noReplyAdmittedAt,
      ).toBeDefined();
...
describe("BackgroundManager parent wake recent-activity admission liveness", () => {
  ...
  test("#given completion admitted during fresh parent activity #then the parent resumes with a reply once activity goes stale", async () => {
    ...
    const pendingByParent = Reflect.get(manager, "pendingByParent") as Map<
      string,
      Set<string>
    >;
    ...
    const parentWakeNotifier = Reflect.get(
      manager,
      "parentWakeNotifier",
    ) as ParentWakeNotifier;
    ...
    try {
      ...
      expect(promptAsyncCalls[0]?.body.noReply).toBe(true);
      expect(
        parentWakeNotifier.getPendingParentWakes().get("parent-1")?.shouldReply,
      ).toBe(true);
      // regression: the reply-required wake must be scheduled for re-flush
      expect(parentWakeNotifier.getPendingParentWakeTimers().has("parent-1"))
        .toBe(true);
      ...
      expect(parentWakeNotifier.getPendingParentWakes().has("parent-1")).toBe(
        false,
      );
    } finally {
      Date.now = originalDateNow;
