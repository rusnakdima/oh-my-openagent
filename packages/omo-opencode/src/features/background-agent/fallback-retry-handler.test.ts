...
const readProviderModelsCacheMock = mock((): ProviderModelsCache | null =>
  null
);
const shouldRetryErrorMock = mock(() => true);
const getNextFallbackMock = mock((chain: FallbackEntry[], attempt: number) =>
  chain[attempt]
);
const hasMoreFallbacksMock = mock((chain: FallbackEntry[], attempt: number) =>
  attempt < chain.length
);
const selectFallbackProviderMock = mock((providers: string[]) => providers[0]);
const transformModelForProviderMock = mock((_provider: string, model: string) =>
  model
);
...
describe("tryFallbackRetry", () => {
  ...
  beforeEach(() => {
    ...
    selectFallbackProviderMock.mockImplementation((providers: string[]) =>
      providers[0]
    );
    readProviderModelsCacheMock.mockReturnValue(null);
    readConnectedProvidersCacheMock.mockReturnValue(null);
    getNextFallbackMock.mockImplementation((
      chain: FallbackEntry[],
      attempt: number,
    ) => chain[attempt]);
    hasMoreFallbacksMock.mockImplementation((
      chain: FallbackEntry[],
      attempt: number,
    ) => attempt < chain.length);
    transformModelForProviderMock.mockImplementation((
      _provider: string,
      model: string,
    ) => model);
  });
  ...
  describe("#given disconnected fallback providers with connected preferred provider", () => {
    test("skips explicit-provider fallback entries when none of their providers are connected", async () => {
      ...
      expect(selectFallbackProviderMock.mock.calls.length).toBe(
        providerCallsBefore,
      );
    });
  });
  ...
  describe("#team-mode fallback", () => {
    test("throws TeamModeFallbackError when teamRunId is set but onSessionCreated is absent", async () => {
      ...
      await expect(tryFallbackRetry(args)).rejects.toThrow(
        TeamModeFallbackError,
      );
      await expect(
        tryFallbackRetry(
