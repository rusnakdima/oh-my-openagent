...
type ParentWakeNotifierClientForTest = ConstructorParameters<
  typeof ParentWakeNotifier
>[0]["client"];
type PromptAsyncCall = Parameters<
  ParentWakeNotifierClientForTest["session"]["promptAsync"]
>[0];
...
function createNotifier(args: {
  ...
  readonly promptAsyncImpl?: (
    call: PromptAsyncCall,
    attempt: number,
  ) => Promise<unknown>;
} = {}): {
  ...
  const client: ParentWakeNotifierClientForTest = {
    session: {
      messages: async () => {
        messagesAttempt += 1;
        return args.sessionMessagesImpl?.(messagesAttempt) ??
          { data: sessionMessages };
      },
      status: async () => ({ data: {} }),
      promptAsync: async (call: PromptAsyncCall) => {
        promptAsyncCalls.push(call);
        return args.promptAsyncImpl?.(call, promptAsyncCalls.length) ??
          { data: {} };
      },
    },
  };
...
describe("ParentWakeNotifier late error recovery", () => {
  test("#given session.error arrives after the recovery window #when no assistant output accepted the wake #then the final wake is already requeued", async () => {
    ...
    notifier.queuePendingParentWake(sessionID, FINAL_WAKE, {
      agent: "sisyphus",
    }, true);
    ...
    try {
      ...
      expect(notifier.getDispatchedParentWakes().get(sessionID)?.notifications)
        .toEqual([FINAL_WAKE]);
      await waitForTimer();
      ...
      expect(notifier.getPendingParentWakes().get(sessionID)?.notifications)
        .toEqual([FINAL_WAKE]);
  ...
  test("#given assistant output appears after the recovery window #when the wake timer inspects history #then the dispatched wake is cleared", async () => {
    ...
    try {
      ...
      // then
      expect(notifier.getDispatchedParentWakes().has(sessionID)).toBe(false);
      expect(notifier.getDispatchedParentWakeTimers().has(sessionID)).toBe(
        false,
      );
    } finally {
      notifier.shutdown();
