...
function createManager(
  sessionMessages: readonly SessionMessageForTest[] = [],
): {
  ...
  const client = unsafeTestValue<PluginInput["client"]>({
    session: {
      status: async () => ({
        data: { "parent-session-empty-wake": { type: "idle" } },
      }),
...
function waitForPendingWake(
  internals: BackgroundManagerInternals,
  sessionID: string,
): Promise<void> {
  return waitUntil(
    () => internals.parentWakeNotifier.getPendingParentWakes().has(sessionID),
    600,
  );
}
    ...
    const falseCases: readonly unknown[] = [
      {
        ...
        tokens: { ...EMPTY_UNKNOWN_ASSISTANT_INFO.tokens, input: 1 },
      },
      {
        ...EMPTY_UNKNOWN_ASSISTANT_INFO,
        tokens: { ...EMPTY_UNKNOWN_ASSISTANT_INFO.tokens, output: 1 },
      },
      {
        ...EMPTY_UNKNOWN_ASSISTANT_INFO,
        tokens: {
...
describe("BackgroundManager parent wake empty-turn recovery", () => {
  test("#given dispatched parent wake #when OpenCode records a zero-token empty unknown assistant turn #then the wake is requeued", async () => {
    ...
    expect(
      internals.parentWakeNotifier.getDispatchedParentWakes().has(sessionID),
    ).toBe(true);
    ...
    expect(
      internals.parentWakeNotifier.getPendingParentWakes().get(sessionID)
        ?.notifications,
    ).toEqual([
      notification,
    ]);
  ...
  test("#given dispatched parent wake has coalesced notifications #when OpenCode records an empty assistant turn #then notification slots are preserved", async () => {
    ...
    expect(promptCalls).toHaveLength(1);
    expect(
      internals.parentWakeNotifier.getDispatchedParentWakes().get(sessionID)
        ?.notifications,
    ).toEqual([
      firstNotification,
      secondNotification,
    ...
    const completedEmptyHistory: readonly SessionMessageForTest[] = [
      ...
      },
      {
        ...
        parts: [{
          type: "step-finish",
          reason: "unknown",
          tokens: EMPTY_UNKNOWN_ASSISTANT_INFO.tokens,
        }],
      },
    ];
