...
function createBackgroundManagerWithOptions(
  options: Partial<ConstructorParameters<typeof BackgroundManager>[0]>,
): BackgroundManager {
  const client = {
    session: {
      prompt: async () => ({}),
...
function getConcurrencyManager(manager: BackgroundManager): ConcurrencyManager {
  return (cast<{ concurrencyManager: ConcurrencyManager }>(manager))
    .concurrencyManager;
}
...
function getPendingByParent(
  ...
  return (cast<{ pendingByParent: Map<string, Set<string>> }>(manager))
    .pendingByParent;
}
...
function getPendingNotifications(
  ...
  return (cast<{ pendingNotifications: Map<string, string[]> }>(manager))
...
function getPendingParentWakeTimers(
  manager: BackgroundManager,
): Map<string, ReturnType<typeof setTimeout>> {
  return (cast<{
    parentWakeNotifier: {
      getPendingParentWakeTimers: () => Map<
        string,
...
function getCompletionTimers(
  ...
  >(manager)).completionTimers;
}

function getRootDescendantCounts(
  manager: BackgroundManager,
): Map<string, number> {
  return (cast<{ rootDescendantCounts: Map<string, number> }>(manager))
...
function getPreStartDescendantReservations(
  ...
  return (cast<{ preStartDescendantReservations: Set<string> }>(manager))
...
async function processKeyForTest(
  ...
  return (cast<{ processKey: (key: string) => Promise<void> }>(manager))
    .processKey(key);
}
...
function pruneStaleTasksAndNotificationsForTest(
  ...
  (cast<{ pruneStaleTasksAndNotifications: () => void }>(manager))
    .pruneStaleTasksAndNotifications();
}
...
function stubNotifyParentSession(manager: BackgroundManager): void {
  (cast<{ notifyParentSession: () => Promise<void> }>(manager))
...
function waitForCoalescedFlush(
  manager: BackgroundManager,
  sessionID: string,
): Promise<void> {
  const api = cast<{
  ...
  const flushArmed = getPendingParentWakes(manager).has(sessionID) ||
    getPendingParentWakeTimers(manager).has(sessionID);
  if (flushArmed) {
    return settled;
  }
  const drained = waitUntil(
    () =>
      !getPendingParentWakes(manager).has(sessionID) ||
      getDispatchedParentWakes(manager).has(sessionID),
    600,
  );
  return Promise.race([settled, drained]);
}

function waitForParentWakeRequeue(
  manager: BackgroundManager,
  sessionID: string,
): Promise<void> {
...
function waitForParentWakeErrorSettle(
  manager: BackgroundManager,
  sessionID: string,
): Promise<void> {
  return waitUntil(
...
describe("BackgroundManager tmux callback ordering", () => {
  test("starts promptAsync before a blocking tmux callback resolves", async () => {
    ...
    try {
      ...
      await waitUntil(
        () =>
          events.includes("session.create") && events.includes("promptAsync"),
        600,
      );
  ...
  test("registers launch bootstrap before first prompt and clears it after completion", async () => {
    ...
    const client = {
      session: {
        ...
        promptAsync: async () => {
          const bootstrap = getDelegatedChildSessionBootstrap(
            "ses_background_bootstrap",
          );
    ...
    try {
      ...
      await (cast<
        {
          startTask: (
            item: { task: BackgroundTask; input: typeof input },
          ) => Promise<void>;
        }
      >(manager))
      ...
      expect(getDelegatedChildSessionBootstrap("ses_background_bootstrap"))
        .toBeDefined();
...
describe("BackgroundManager retry observability", () => {
  test("queues a parent-visible retry notification when fallback retry is scheduled", async () => {
    ...
    const retryingCall =
      cast<Array<[string, string, Record<string, unknown>, boolean]>>(
  ...
  test("queues a second parent-visible notification once the retry session ID is created", async () => {
    ...
    //#then
    const retryReadyCall = cast<
      Array<
        [string, string, Record<string, unknown>, boolean, number | undefined]
      >
    >(
      queuePendingParentWake.mock.calls,
    ).find((call) => call[1].includes("[BACKGROUND TASK RETRY SESSION READY]"));
    const retryReadyNotification = retryReadyCall?.[1];
    const expectedRetryLink = `http://127.0.0.1:4096/${
      Buffer.from(tmpdir()).toString("base64url")
    }/session/ses_retry_created`;
    ...
    expect(retryReadyNotification).toContain(
      "genai-proxy-openai/gpt-5.6-luna-fast",
    );
    expect(retryReadyNotification).toContain(
  ...
  test("builds retry-ready links from the parent session directory when it differs from the manager directory", async () => {
    ...
    const manager = new BackgroundManager({
      pluginContext: createPluginInput(client, managerDirectory),
    });
    (cast<{
      queuePendingParentWake: (
        sessionId: string,
    ...
    //#then
    const retryReadyNotification = cast<
      Array<
        [string, string, Record<string, unknown>, boolean, number | undefined]
      >
    >(
      ...
      .map((call) => call[1])
      .find((notification) =>
        notification.includes("[BACKGROUND TASK RETRY SESSION READY]")
      );
    const expectedRetryLink = `http://127.0.0.1:4096/${
      Buffer.from(parentDirectory).toString("base64url")
    }/session/ses_retry_created_parent_dir`;
    expect(retryReadyNotification).toBeDefined();
    expect(retryReadyNotification).toContain(expectedRetryLink);
...
describe("BackgroundManager.notifyParentSession - aborted parent", () => {
  ...
  test("should queue notification when promptAsync aborts while parent is idle", async () => {
    ...
    expect(pendingWake?.notifications[0]).toContain(
  ...
  test("should prefer parent session variant over child task variant in parent notification promptAsync body", async () => {
    ...
    const client = {
      session: {
        prompt: async () => ({}),
        promptAsync: async (
          args: { path: { id: string }; body: Record<string, unknown> },
        ) => {
...
describe("BackgroundManager.injectPendingNotificationsIntoChatMessage", () => {
  test("should defer queued notifications without mutating user text", () => {
    ...
    expect(getPendingParentWakes(manager).get("session-parent")?.notifications)
      .toEqual([
        "<system-reminder>queued-one</system-reminder>\n\n<system-reminder>queued-two</system-reminder>",
      ]);
    expect(getPendingParentWakes(manager).get("session-parent")?.shouldReply)
      .toBe(false);
    expect(getPendingNotifications(manager).get("session-parent"))
      .toBeUndefined();
...
describe("BackgroundManager.tryCompleteTask", () => {
  ...
  test("#given the final child is mid-teardown #when its session abort is still pending #then hasPendingParentWake keeps reporting an owed wake", async () => {
    ...
    try {
      ...
      expect(getDispatchedParentWakes(manager).has(parentSessionId)).toBe(
        false,
      );
  ...
  test("should clean pendingByParent even when promptAsync notification fails", async () => {
    ...
    // then
    expect(task.status).toBe("completed");
    expect(getPendingByParent(manager).get(task.parentSessionId))
      .toBeUndefined();
  });
  ...
  test("should avoid overlapping promptAsync calls when tasks complete concurrently", async () => {
    ...
    const client = {
      session: {
        ...
        promptAsync: async (
          args: { path: { id: string }; body: PromptAsyncBody },
        ) => {
...
describe("BackgroundManager.resume running-task guard", () => {
  test("rejects a running task without changing parent context or dispatching a prompt", async () => {
    ...
    try {
      ...
      expect(task.parentModel).toEqual({
        providerID: "openai",
        modelID: "gpt-5.4",
      });
...
describe("BackgroundManager.resume promptAsync gate state", () => {
  test("restores completed task state when resume prompt is skipped because the session is active", async () => {
    ...
    const client = {
      session: {
        status: async () => ({
          data: { "session-active-resume": { type: "busy" } },
        }),
        promptAsync: async () => {
          promptCallCount += 1;
          return {};
    ...
    expect(getPendingByParent(manager).get("parent-session-new"))
      .toBeUndefined();
...
describe("BackgroundManager - Non-blocking Queue Integration", () => {
  ...
  beforeEach(() => {
    ...
    manager = new BackgroundManager({
      pluginContext: createPluginInput(mockClient),
    });
  });
  ...
  describe("launch() returns immediately with pending status", () => {
    ...
    test("should reject wrapper-only agent names after sanitization", async () => {
      ...
      await expectRejectsWithMessage(
        result,
        "Agent parameter is required after sanitization",
      );
    });
    ...
    test("should keep agent when launch has model and keep agent without model", async () => {
      ...
      manager.shutdown();
      manager = new BackgroundManager({
        pluginContext: createPluginInput(customClient),
      });
  ...
  describe("task transitions pending→running when slot available", () => {
    ...
    test("should transition first task to running immediately", async () => {
      ...
      const task = await manager.launch(input);
      await waitUntil(
        () => manager.getTask(task.id)?.status === "running",
        600,
      );
        ...
        manager = new BackgroundManager(
          {
            ...
            onSubagentSessionCreated: async () => {
              const activeTaskID = taskID ??
                Array.from(getTaskMap(manager).keys())[0];
  ...
  describe("multiple keys process in parallel", () => {
    test("should process different concurrency keys in parallel", async () => {
      ...
      await waitUntil(
        () =>
          manager.getTask(task1.id)?.status === "running" &&
          manager.getTask(task2.id)?.status === "running",
        600,
      );
    ...
    test("should remove cancelled pending model task from provider queue", async () => {
      ...
      // when
      const cancelled = await manager.cancelTask(task3.id, {
        abortSession: false,
        skipNotification: true,
      });
      ...
      const providerQueuedTaskIds =
        providerQueue?.map((item) => item.task.id) ?? [];
      expect(cancelled).toBe(true);
      expect(providerQueuedTaskIds).not.toContain(task3.id);
      expect(getQueuesByKey(manager).get("anthropic/claude-haiku-4.5"))
        .toBeUndefined();
  ...
  describe("TTL uses queuedAt for pending, startedAt for running", () => {
    ...
    test("should have different timestamps for queuedAt and startedAt", async () => {
      ...
      const runningTask = tasks.find((t) =>
        t.status === "running" && t.id !== task2.id
      );
  ...
  describe("manual verification scenario", () => {
    test("should handle 10 tasks with limit 5 returning immediately", async () => {
      ...
      await waitUntil(
        () =>
          tasks.filter((t) => manager.getTask(t.id)?.status === "running")
            .length >= 5,
        600,
      );
      ...
      const runningCount =
        updatedTasks.filter((t) => t?.status === "running").length;
      const pendingCount =
        updatedTasks.filter((t) => t?.status === "pending").length;
    ...
    const client = {
      session: {
        ...
        get: async () => ({
          data: {
            id: "session-running",
            time: { updated: fixedTime - 300_000 },
          },
        }),
      },
    };
...
describe("BackgroundManager.handleEvent - session.error", () => {
  ...
  const mockVerifySessionExists = (
    manager: BackgroundManager,
    sessionExists: boolean,
  ): void => {
    verifySessionExistsSpy?.mockRestore();
    const spy = spyOn(
      cast<{ verifySessionExists: (sessionID: string) => Promise<boolean> }>(
        manager,
      ),
      "verifySessionExists",
    );
  ...
  const stubProcessKey = (manager: BackgroundManager) => {
    (cast<{ processKey: (key: string) => Promise<void> }>(manager)).processKey =
      async () => {};
  };
  ...
  test("does not terminate task on session.error when session is still alive", async () => {
    ...
    expect(
      logCalls.some((call) =>
        call.message.includes("session.error received but session still alive")
      ),
    ).toBe(true);
  ...
  test("terminates task and notifies parent on terminal session.error (e.g. no provider available) even when session is still alive", async () => {
    ...
    expect(
      logCalls.some((call) =>
        call.message.includes("Finalizing task after terminal session.error")
      ),
    ).toBe(true);
  ...
  test.each([
    ...
    async (_case: string, errorName: string, errorMessage: string) => {
      ...
      (cast<{ notifyParentSession: (task: BackgroundTask) => Promise<void> }>(
        manager,
      )).notifyParentSession = notifyParentSession;
      const concurrencyManager = getConcurrencyManager(manager);
  ...
  test("terminates task when agent-not-found arrives as async session.error after promptAsync accept", async () => {
    ...
    expect(task.error).toBe(
      'Agent "missing-agent" not found. Make sure the agent is registered in your opencode.json or provided by a plugin.',
    );
    expect(task.completedAt).toBeInstanceOf(Date);
    expect(task.concurrencyKey).toBeUndefined();
    expect(concurrencyManager.getCount(concurrencyKey)).toBe(0);
  ...
  test("requeues dispatched parent wake when the wake prompt fails through session.error", async () => {
    ...
    const client = {
      session: {
        status: async () => ({
          data: { "parent-session-wake": { type: "idle" } },
        }),
        messages: async () => ({ data: [] }),
        promptAsync: async (
          args: { path: { id: string }; body: Record<string, unknown> },
    ...
    //#then
    expect(promptCalls).toHaveLength(1);
    expect(getDispatchedParentWakes(manager).has("parent-session-wake")).toBe(
      false,
    );
    expect(
      getPendingParentWakes(manager).get("parent-session-wake")?.notifications,
    ).toEqual([
      "<system-reminder>done</system-reminder>",
    ]);
  ...
  test("pins the registered parent agent alias before dispatching a deferred parent wake", async () => {
    ...
    const client = {
      session: {
        status: async () => ({
          data: { "parent-session-alias": { type: "idle" } },
        }),
        messages: async () => ({ data: [] }),
        promptAsync: async (
          args: { path: { id: string }; body: Record<string, unknown> },
    ...
    //#then
    expect(promptCalls).toHaveLength(1);
    expect(promptCalls[0]?.body.agent).toBe(
      "\u200B\u200B\u200B\u200BAtlas - Plan Executor",
    );
  ...
  test("keeps dispatched parent wake tracked when message.updated only records the injected user wake", async () => {
    ...
    const client = {
      session: {
        status: async () => ({
          data: { "parent-session-user-update": { type: "idle" } },
        }),
        messages: async () => ({ data: [] }),
        promptAsync: async (
          args: { path: { id: string }; body: Record<string, unknown> },
    ...
    //#then
    expect(promptCalls).toHaveLength(1);
    expect(getDispatchedParentWakes(manager).has("parent-session-user-update"))
      .toBe(true);
  ...
  test("requeues dispatched parent wake when a late error follows the injected user wake part", async () => {
    ...
    const client = {
      session: {
        status: async () => ({
          data: { "parent-session-part-wake": { type: "idle" } },
        }),
        messages: async () => ({ data: [] }),
        promptAsync: async (
          args: { path: { id: string }; body: Record<string, unknown> },
    ...
    expect(getDispatchedParentWakes(manager).has("parent-session-part-wake"))
      .toBe(true);
    ...
    expect(getDispatchedParentWakes(manager).has("parent-session-part-wake"))
      .toBe(false);
    expect(
      getPendingParentWakes(manager).get("parent-session-part-wake")
        ?.notifications,
  ...
  test("requeues dispatched parent wake when split text deltas stream the injected user wake", async () => {
    ...
    const client = {
      session: {
        status: async () => ({
          data: { "parent-session-delta-wake": { type: "idle" } },
        }),
        messages: async () => ({ data: [] }),
        promptAsync: async (
          args: { path: { id: string }; body: Record<string, unknown> },
    ...
    expect(getDispatchedParentWakes(manager).has("parent-session-delta-wake"))
      .toBe(true);
    ...
    expect(getDispatchedParentWakes(manager).has("parent-session-delta-wake"))
      .toBe(false);
    expect(
      getPendingParentWakes(manager).get("parent-session-delta-wake")
        ?.notifications,
    ).toEqual([notification]);
  ...
  test("does not requeue dispatched parent wake after real assistant text delta output", async () => {
    ...
    const client = {
      session: {
        status: async () => ({
          data: { "parent-session-real-delta": { type: "idle" } },
        }),
        messages: async () => ({ data: [] }),
        promptAsync: async (
          args: { path: { id: string }; body: Record<string, unknown> },
    ...
    expect(getDispatchedParentWakes(manager).has("parent-session-real-delta"))
      .toBe(true);
    ...
    //#then
    expect(promptCalls).toHaveLength(1);
    expect(getPendingParentWakes(manager).has("parent-session-real-delta"))
      .toBe(false);
  ...
  test("does not requeue dispatched parent wake when session history already contains assistant output after the wake", async () => {
    ...
    expect(getPendingParentWakes(manager).has("parent-session-wake")).toBe(
      false,
    );
...
describe("BackgroundManager queue processing - error tasks are skipped", () => {
  test("does not start tasks with status=error", async () => {
    ...
    (cast<{ startTask: (item: unknown) => Promise<void> }>(manager)).startTask =
      async () => {
        startCalled = true;
      };
...
describe("BackgroundManager.completionTimers - Memory Leak Fix", () => {
  ...
  test("should start per-task cleanup timers independently of sibling completion", async () => {
    ...
    getTaskMap(manager).set(taskA.id, taskA);
    getTaskMap(manager).set(taskB.id, taskB);
    (cast<{ pendingByParent: Map<string, Set<string>> }>(manager))
      .pendingByParent.set(
        "parent-session",
        new Set([taskA.id, taskB.id]),
      );
...
describe("BackgroundManager regression fixes - resume and aborted notification", () => {
  test("should keep resumed task in memory after previous completion timer deadline", async () => {
    ...
    await waitUntil(
      () => getTaskMap(manager).has(task.id) && !completionTimers.has(task.id),
      600,
    );
  ...
  test("should start cleanup timer even when promptAsync aborts", async () => {
    ...
    await (cast<
      { notifyParentSession: (task: BackgroundTask) => Promise<void> }
    >(manager)).notifyParentSession(task);
  ...
  test("should keep completed task retrievable after scheduled removal", () => {
    ...
    getTaskMap(manager).set(task.id, task); //#when
    (cast<{ removeTask: (task: BackgroundTask) => void }>(manager)).removeTask(
      task,
    );
  ...
  test("should resolve a completed task registered by an earlier plugin manager instance", () => {
    ...
    (cast<{ addTask: (task: BackgroundTask) => void }>(firstManager)).addTask(
      task,
    );
  ...
  test("should redact active task prompts resolved from an earlier plugin manager instance", () => {
    ...
    expect(registeredTask?.progress?.countedToolPartIDs).toEqual(
      new Set(["part-1"]),
    );
  ...
  test("should resolve archived completed task from an earlier plugin manager instance", () => {
    ...
    getTaskMap(firstManager).set(task.id, task); //#when
    (cast<{ removeTask: (task: BackgroundTask) => void }>(firstManager))
      .removeTask(task);
  ...
  test("should cap completed task archive size at 100 entries", () => {
    ...
    for (let index = 0; index < 120; index += 1) {
      ...
      (cast<{ removeTask: (task: BackgroundTask) => void }>(manager))
        .removeTask(task);
    }

    //#then
    const archive = cast<Map<string, unknown>>(
      Reflect.get(manager, "completedTaskArchive"),
    );
...
describe("BackgroundManager - tool permission spread order", () => {
  test("startTask respects explore agent restrictions", async () => {
    ...
    await (cast<
      {
        startTask: (
          item: { task: BackgroundTask; input: import("./types").LaunchInput },
        ) => Promise<void>;
      }
    >(manager))
      .startTask({ task, input });
  ...
  test("startTask updates tracked session agent when launch falls back to general", async () => {
    ...
    try {
      ...
      expect(
        getDelegatedChildSessionBootstrap("session-manager-fallback")?.tools
          ?.call_omo_agent,
      ).toBe(true);
    } finally {
      manager.shutdown();
...
describe("BackgroundManager attempt lifecycle bindings", () => {
  test("startTask binds the created session to the queued attempt ID and mirrors task projection", async () => {
    ...
    //#then
    const activeAttempt = task.attempts?.find((attempt) =>
