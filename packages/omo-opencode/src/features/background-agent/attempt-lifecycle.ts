...
type TerminalAttemptStatus = Extract<
  BackgroundTaskStatus,
  "completed" | "error" | "cancelled" | "interrupt"
>;

function toAttemptModel(
  model: DelegatedModelConfig | undefined,
): Pick<BackgroundTaskAttempt, "providerId" | "modelId" | "variant"> {
  return {
...
): DelegatedModelConfig | undefined {
...
function getAttemptIndex(task: BackgroundTask, attemptID: string): number {
  return task.attempts?.findIndex((attempt) =>
...
): status is TerminalAttemptStatus {
  return status === "completed" || status === "error" ||
    status === "cancelled" || status === "interrupt";
}
...
export function projectTaskFromCurrentAttempt(
  task: BackgroundTask,
): BackgroundTask {
...
export function findAttemptBySession(
  task: BackgroundTask,
  sessionID: string,
): BackgroundTaskAttempt | undefined {
