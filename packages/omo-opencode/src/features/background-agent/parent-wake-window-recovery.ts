...
export async function handleDispatchedParentWakeWindowElapsed(
  ...
  if (
    await input.sessionInspector.hasAssistantOrToolOutputAfterDispatchedWake(
      input.sessionID,
      input.wake,
    )
  ) {
    ...
    log(
      "[background-agent] Cleared dispatched parent wake after observing assistant output:",
      {
  ...
  if (retryCount >= MAX_NO_ASSISTANT_OUTPUT_RETRIES) {
    input.dispatchedTracker.clearWake(input.sessionID);
    log(
      "[background-agent] Stopped retrying parent wake after repeated no-output dispatch:",
      {
        sessionID: input.sessionID,
        retryCount,
      },
  ...
  input.scheduleFlush();
  log(
    "[background-agent] Requeued dispatched parent wake after no assistant output:",
    {
      sessionID: input.sessionID,
...
export function logParentWakeWindowRecoveryError(
  sessionID: string,
  error: unknown,
): void {
  const errorText = error instanceof Error
    ...
    : String(error);
  log(
    "[background-agent] Failed to inspect dispatched parent wake after recovery window:",
    {
      sessionID,
      error: errorText,
    },
