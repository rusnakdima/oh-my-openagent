...
function createAtlasHarness(): {
  ...
  readonly createdSessions: Array<
    { readonly id: string; readonly body: SessionCreateArgs["body"] }
  >;
  ...
  const client = {
    session: {
      get: async ({ path }: SessionGetArgs) => {
        ...
        return {
          error: { status: 404, message: `session ${path.id} not found` },
        };
      },
      create: async (args: SessionCreateArgs) => {
        const id = sessionIDs[createdSessions.length] ??
          `ses_extra_${createdSessions.length}`;
  ...
  const manager = new BackgroundManager({
    pluginContext: createPluginInput(client, directory),
  });
  ...
  return {
    ...
    createdSessions,
    promptCalls,
    markSessionMissing: (sessionID: string) =>
      sessionAlive.set(sessionID, false),
  };
}

async function launchAtlasOracleSubagent(
...
describe("Atlas-spawned subagent runtime fallback", () => {
  test("retries oracle subagent on OpenAI usage_limit_reached and registers the fallback session", async () => {
    ...
    expect(createdSessions[1]?.body?.model).toEqual({
      providerID: "github-copilot",
  ...
  test("surfaces non-retryable oracle subagent errors without creating a fallback session", async () => {
    //#given
    const { manager, createdSessions, markSessionMissing } =
      createAtlasHarness();
    const taskID = await launchAtlasOracleSubagent(manager);
