...
function formatAttemptTimeline(task: BackgroundTaskNotificationTask): string {
  ...
  const lines = task.attempts
    .map((attempt) => {
      const attemptLines = [
        `  - Attempt ${attempt.attemptNumber} — ${attempt.status.toUpperCase()} — ${
          formatAttemptModel(attempt)
...
export function buildBackgroundTaskNotificationText(input: {
  ...
  if (allComplete) {
    const succeededTasks = completedTasks.filter((t) =>
      t.status === "completed"
    );
    ...
    const resultCollectionInstruction =
...
**${remainingCount} task${
    remainingCount === 1 ? "" : "s"
  } still in progress.** You WILL be notified when ALL complete.
${
    isFailure
