...
describe("ConcurrencyManager.getConcurrencyLimit", () => {
  ...
  test("should prioritize model-specific over provider-specific over default", () => {
    ...
    // when
    const modelLimit = manager.getConcurrencyLimit(
      "anthropic/claude-sonnet-4-6",
    );
    const providerLimit = manager.getConcurrencyLimit(
...
describe("ConcurrencyManager.acquire/release", () => {
  ...
  test("should queue multiple tasks and process in order", async () => {
    ...
    // when
    const order: string[] = [];
    const task1 = manager.acquire("model-a").then(() => {
      order.push("1");
    });
    const task2 = manager.acquire("model-a").then(() => {
      order.push("2");
    });
    const task3 = manager.acquire("model-a").then(() => {
      order.push("3");
    });
  ...
  test("should use model-specific limit for acquire", async () => {
    ...
    // when
    let resolved = false;
    const waitPromise = manager.acquire("anthropic/claude-sonnet-4-6").then(
      () => {
        resolved = true;
      },
    );
...
describe("ConcurrencyManager.cleanup", () => {
  test("cancelWaiter should cancel a raw model waiter stored under a normalized provider key", async () => {
    ...
    const errors: Error[] = [];
    const waiter = manager.acquire(rawKey, "queued-task").catch(
      (error: Error) => {
        errors.push(error);
      },
    );
  ...
  test("cancelWaiters should cancel all raw model waiters stored under a normalized provider key", async () => {
    ...
    const errors: Error[] = [];
    const firstWaiter = manager.acquire(rawKey, "queued-task-1").catch(
      (error: Error) => {
        errors.push(error);
      },
    );
    const secondWaiter = manager.acquire(rawKey, "queued-task-2").catch(
      (error: Error) => {
        errors.push(error);
      },
    );
