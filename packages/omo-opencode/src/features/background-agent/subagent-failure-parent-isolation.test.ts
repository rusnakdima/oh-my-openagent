...
function createManager(): {
  ...
  const client = {
    session: {
      ...
      status: async () => ({
        data: {
          "main-session": { type: "idle" },
          "subagent-session": { type: "idle" },
        },
      }),
      get: async (input: { path: { id: string } }) => ({
        data: input.path.id === "subagent-session"
          ? null
          : { id: input.path.id },
      }),
      prompt: async () => ({}),
...
async function flushPendingParentWakeForTest(
  manager: BackgroundManager,
  sessionID: string,
): Promise<void> {
...
describe("BackgroundManager subagent failure parent isolation", () => {
  test("#given nested background wake prompt errors in a subagent session #when the subagent is also a parent task #then the main session is not notified or cancelled", async () => {
    ...
    getPendingByParent(manager).set(
      nestedFailure.parentSessionId,
      new Set([nestedFailure.id]),
    );
    ...
    expect(JSON.stringify(promptAsyncCalls[0]?.body.parts)).toContain(
