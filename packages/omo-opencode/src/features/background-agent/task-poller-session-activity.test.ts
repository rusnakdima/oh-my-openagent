...
describe("checkAndInterruptStaleTasks persisted session activity", () => {
  const mockClient = unsafeTestValue<
    Parameters<typeof checkAndInterruptStaleTasks>[0]["client"]
  >({
    session: {
  ...
  const mockConcurrencyManager = unsafeTestValue<
    Parameters<typeof checkAndInterruptStaleTasks>[0]["concurrencyManager"]
  >({
    release: mock(() => {}),
  });
  ...
  test("cancels a busy task when persisted session activity is also stale", async () => {
    ...
    await checkAndInterruptStaleTasks({
      ...
      getSessionActivity: async () => ({
        type: "activity",
        activity: stalePersistedActivity,
      }),
    });
