...
import {
  getParentWakeMessageActivityAt,
  getParentWakeMessageCreatedAt,
} from "./parent-wake-message-activity";
import type { PendingParentWake } from "./parent-wake-dedupe";

export function latestAssistantTurnIsCompletedEmptyNoProgress(
  ...
  for (let index = messages.length - 1; index >= 0; index--) {
    ...
    if (role === "assistant") {
      const info = isRecord(message) && isRecord(message.info)
        ? message.info
        : message;
...
export function latestAssistantTurnHasToolBlock(
  messages: readonly unknown[],
): boolean {
  for (let index = messages.length - 1; index >= 0; index--) {
    const message = messages[index];
  ...
  for (let index = messages.length - 1; index >= 0; index--) {
    ...
    if (role === "assistant") {
      ...
      return message.parts.some((part) =>
        partHasFreshToolActivity(part, now, maxAgeMs)
      );
    }
    if (role === "user" && !messageIsSyntheticOrInternalUser(message)) {
      return false;
...
export function createEmptyAssistantTurnRetryDedupeKey(
...
function partHasFreshToolActivity(
  part: unknown,
  now: number,
  maxAgeMs: number,
): boolean {
...
function messageHasFreshActivity(
  message: unknown,
  now: number,
  maxAgeMs: number,
): boolean {
