...
export type SessionActivityResolver = (
  sessionID: string,
...
export async function getSessionActivityFromClient(
  ...
  try {
    ...
    if (
      ...
      log("[background-agent] Failed to read session activity:", {
        sessionID,
    ...
    const sessionInfo = isRecord(response) && "data" in response
      ? response.data
      : response;
