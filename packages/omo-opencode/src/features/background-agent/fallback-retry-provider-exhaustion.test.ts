...
function createDeps(): Partial<FallbackRetryHandlerDeps> {
  return {
    ...
    getNextFallback: mock((chain: FallbackEntry[], attempt: number) =>
      chain[attempt]
    ),
    hasMoreFallbacks: mock((chain: FallbackEntry[], attempt: number) =>
      attempt < chain.length
    ),
    selectFallbackProvider: mock((providers: string[]) => providers[0]),
    transformModelForProvider: mock((_provider: string, model: string) =>
      model
    ),
  };
}
