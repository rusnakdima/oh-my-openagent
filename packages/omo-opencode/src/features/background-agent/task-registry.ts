...
function cloneProgress(
  ...
  return {
    ...progress,
    countedToolPartIDs: progress.countedToolPartIDs
      ? new Set(progress.countedToolPartIDs)
      : undefined,
  };
}

function cloneAttempts(
  attempts: BackgroundTask["attempts"],
): BackgroundTask["attempts"] {
  if (!attempts) {
    return undefined;
  }
...
export function getRegisteredBackgroundTask(
  taskID: string,
