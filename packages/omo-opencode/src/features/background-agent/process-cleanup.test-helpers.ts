...
type ProcessCleanupSignal = Parameters<
  typeof __getProcessCleanupSignalListenerForTesting
>[0];
...
export function getNewListener(
  ...
  const listener = process
    .listeners(signal)
    .find((registeredListener) =>
      !existingListeners.includes(registeredListener)
    );
