...
export function createTask(input: LaunchInput): BackgroundTask {
  return buildTaskRecord(
    input,
    `bg_${crypto.randomUUID().slice(0, 8)}`,
    new Date(),
  );
}
...
export async function resumeTask(
  ...
  ctx: Pick<
    SpawnerContext,
    "client" | "concurrencyManager" | "directory" | "onTaskError"
  >,
): Promise<void> {
  ...
  promptWithRetryInDirectory(client, {
    ...
    if (isAgentNotFoundError(error) && task.agent !== FALLBACK_AGENT) {
      log(
        "[background-agent] Resume agent not found, retrying with fallback agent",
        {
          original: task.agent,
          fallback: FALLBACK_AGENT,
          taskId: task.id,
      ...
      try {
        ...
        log(
          "[background-agent] Resume fallback agent also failed:",
          retryError,
        );
        onTaskError(
          task,
