...
export function getParentWakeSessionHistoryDeferralDecision(input: {
  ...
  if (!input.messages) {
    log(
      "[background-agent] Deferred parent wake because parent messages could not be inspected:",
      {
  ...
  const latestAssistantBlocksPrompt = latestAssistantTurnBlocksInternalPrompt(
    messages,
  );
  const latestAssistantHasUnansweredQuestion =
    latestAssistantTurnHasUnansweredQuestion(messages);
  if (!latestAssistantBlocksPrompt) {
    delete input.wake.toolCallDeferralStartedAt;
    delete input.wake.allowEmptyAssistantTurnRetry;
    return {
      defer: false,
      skipPromptGateToolStateCheck: strippedOwnAdmittedDeposit,
    };
  }
  const now = input.now ?? Date.now();
  input.wake.toolCallDeferralStartedAt ??= now;
  if (
    input.wake.allowEmptyAssistantTurnRetry &&
    latestAssistantTurnIsCompletedEmptyNoProgress(messages)
  ) {
    log(
      "[background-agent] Retrying parent wake after completed empty assistant turn:",
      { sessionID: input.sessionID },
    );
  ...
  if (latestAssistantHasUnansweredQuestion) {
    log(
      "[background-agent] Deferred parent wake because latest assistant question awaits user response:",
      {
        sessionID: input.sessionID,
      },
    );
  ...
  if (
    ...
    log(
      "[background-agent] Holding parent wake during stale tool-call deferral:",
      { sessionID: input.sessionID },
    );
  ...
  if (
    ...
    log(
      "[background-agent] Retrying parent wake after stale unknown-finish assistant output:",
      {
        sessionID: input.sessionID,
      },
    );
  ...
  log(
    "[background-agent] Deferred parent wake because latest assistant turn blocks internal prompts:",
    {
      sessionID: input.sessionID,
    },
  );
...
export function hasAssistantOutputAfterParentWakeAdmission(input: {
  ...
  return input.messages.some((message) => {
    ...
    const createdAt = getParentWakeMessageCreatedAt(message);
    return createdAt !== undefined && createdAt >= admittedAt &&
...
function getParentWakeMessageRole(
  message: ParentWakeSessionMessage,
): string | undefined {
  return message.info?.role ?? message.role;
}

function parentWakeMessageHasOutput(
  message: ParentWakeSessionMessage,
): boolean {
  const role = getParentWakeMessageRole(message);
  if (role !== "assistant" && role !== "tool") {
...
function parentWakeMessageContainsNotification(
  message: ParentWakeSessionMessage,
  ...
  return message.parts?.some((part) =>
    ...
    wake.notifications.some((notification) => part.text?.includes(notification))
  ) ?? false;
}
