...
function getTasks(manager: BackgroundManager): Map<string, BackgroundTask> {
  return unsafeTestValue<Map<string, BackgroundTask>>(
    Reflect.get(manager, "tasks"),
  );
}
...
function getPendingByParent(
  ...
  return unsafeTestValue<Map<string, Set<string>>>(
    Reflect.get(manager, "pendingByParent"),
  );
}
...
async function notifyParentSessionForTest(
  ...
  const notifyParentSession = unsafeTestValue<
    (task: BackgroundTask) => Promise<void>
  >(Reflect.get(manager, "notifyParentSession"));
...
async function flushPendingParentWakeForTest(
  ...
  const flushPendingParentWake = unsafeTestValue<
    (sessionID: string) => Promise<void>
  >(Reflect.get(manager, "flushPendingParentWake"));
