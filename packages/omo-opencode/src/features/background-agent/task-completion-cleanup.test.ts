...
function createTask(
  ...
  const { id: _ignoredID, parentSessionId: _ignoredParentSessionID, ...rest } =
    overrides;
...
async function flushParentWake(
  manager: BackgroundManager,
  sessionID = "parent-1",
): Promise<void> {
  if (!fakeTimers) {
    throw new Error(
      "Fake timers must be installed before flushing parent wakes",
    );
  }
  ...
  if (pendingTimer) {
    ...
    const flushPendingParentWake = Reflect.get(
      manager,
      "flushPendingParentWake",
...
async function waitForDeferredWake(
  manager: BackgroundManager,
  promptAsyncCalls: PromptAsyncCall[],
): Promise<void> {
  if (!fakeTimers) {
    throw new Error(
      "Fake timers must be installed before waiting for parent wakes",
    );
  }

  for (
    let attempts = 0;
    attempts < 12 && promptAsyncCalls.length === 0;
    attempts += 1
  ) {
...
async function waitForRequeuedParentWake(
  manager: BackgroundManager,
  sessionID: string,
): Promise<void> {
  if (!fakeTimers) {
  ...
  for (
    let attempts = 0;
    attempts < 12 &&
    (getPendingParentWakes(manager).get(sessionID)?.notifications.length ??
        0) === 0;
    attempts += 1
  ) {
  ...
  describe("#given background tasks for same parent", () => {
    ...
    test("#when partial completion arrives while parent session is busy #then notification waits until idle without waking a reply", async () => {
      ...
      const notificationPayload = JSON.stringify(
        promptAsyncCalls[0]?.body.parts,
      );
      expect(notificationPayload).toContain("BACKGROUND TASK RESULT READY");
      expect(notificationPayload).not.toContain(
    ...
    test("#when retry no-reply notification batches with final completion #then idle flush sends one reply wake", async () => {
      ...
      managerUnderTest = manager;
      const queuePendingParentWake = Reflect.get(
        manager,
    ...
    test("#when stale tool-call history keeps blocking an all-complete wake #then the wake is admitted as noReply with reply liveness retained", async () => {
      ...
      expect(notificationPayload).toContain("ALL BACKGROUND TASKS COMPLETE");
      expect(getPendingParentWakes(manager).get("parent-1")?.shouldReply).toBe(
        true,
      );
    });
