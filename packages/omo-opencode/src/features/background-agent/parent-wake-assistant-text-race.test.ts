...
describe("ParentWakeNotifier — assistant text dispatch race", () => {
  test("#given stale text becomes fresh assistant activity before dispatch #when flushing parent wake #then wake is recorded without forking a reply", async () => {
    ...
    const client = unsafeTestValue<ParentWakeClient>({
      session: {
        messages: async () => {
          messageReadCount += 1;
          return {
            data: messageReadCount === 1
              ? staleAssistantMessages
              : freshAssistantMessages,
          };
        },
        status: async () => ({
          data: { "parent-stale-then-fresh-text": { type: "idle" } },
        }),
    ...
    try {
      ...
      expect(
        notifier.getPendingParentWakes().has("parent-stale-then-fresh-text"),
      ).toBe(true);
    } finally {
