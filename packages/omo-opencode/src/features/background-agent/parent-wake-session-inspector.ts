...
export class ParentWakeSessionInspector {
  ...
  async hasRecordedPromptMessageAfterDispatchedWake(
    sessionID: string,
    wake: PendingParentWake,
  ): Promise<boolean> {
    const messages = await this.loadMessages(sessionID);
  ...
  async hasAssistantOutputAfterAdmittedWake(
    sessionID: string,
    wake: PendingParentWake,
  ): Promise<boolean> {
    const messages = await this.loadMessages(sessionID);
  ...
  private async loadMessages(
    sessionID: string,
  ): Promise<ParentWakeSessionMessage[] | undefined> {
    try {
