...
export class TaskHistory {
  ...
  formatForCompaction(parentSessionID: string): string | null {
    ...
    if (olderOmittedCount > 0) {
      lines.push(
        `- ${olderOmittedCount} older delegated sessions omitted from compaction summary.`,
      );
    }
...
function formatCompactionEntry(entry: TaskHistoryEntry): string {
  const description = compactInline(
    entry.description,
  ...
  const parts = [
    ...
    entry.sessionID
      ? ` | session: \`${compactInline(entry.sessionID, 120)}\``
      : "",
  ];
...
function appendBudgetSummary(lines: string[], omittedCount: number): void {
  const summary =
    `- ${omittedCount} delegated sessions omitted to stay within compaction budget.`;
  ...
  while (
    lines.length > 0 &&
    !appendWithinBudget(lines, summary, MAX_COMPACTION_TOTAL_CHARS)
  ) {
    lines.pop();
  }
}
