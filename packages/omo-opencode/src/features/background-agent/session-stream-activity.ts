...
function buildPartInfo(
  ...
  return {
    id: getStringField(source, "id") ?? getStringField(source, "partID") ??
      getStringField(source, "callID"),
    sessionID: getStringField(source, "sessionID") ??
      getStringField(fallback, "sessionID"),
    ...
    synthetic: getBooleanField(source, "synthetic") ??
      getBooleanField(fallback, "synthetic"),
...
export function resolveMessagePartInfo(
  properties: unknown,
  ...
  return nestedPart
    ? buildPartInfo(nestedPart, props)
    : buildPartInfo(props, undefined);
}
...
export function resolveSessionNextPartInfo(
  eventType: string,
  properties: unknown,
): MessagePartInfo | undefined {
...
export function isMessagePartForSession(
  partInfo: MessagePartInfo | undefined,
  sessionID: string,
): boolean {
  return !partInfo?.sessionID || partInfo.sessionID === sessionID;
}

export function hasOutputSignalFromPart(
  partInfo: MessagePartInfo | undefined,
  sessionID?: string,
): boolean {
  if (!partInfo) return false;
  if (partInfo.sessionID && sessionID && partInfo.sessionID !== sessionID) {
    return false;
  }
...
export function isInternalInitiatorTextPart(
  partInfo: MessagePartInfo | undefined,
  sessionID?: string,
): boolean {
  if (!partInfo) return false;
...
export function hasParentWakeOutputSignalFromPart(
  partInfo: MessagePartInfo | undefined,
  sessionID?: string,
): boolean {
