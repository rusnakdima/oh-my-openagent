...
const { describe, it, expect, mock, spyOn, beforeEach, afterEach } = require(
  "bun:test",
);
...
describe("checkAndInterruptStaleTasks", () => {
  ...
  it("should keep never-updated task running when stale abort returns SDK error", async () => {
    ...
    mockClient.session.abort.mockImplementationOnce(() =>
      Promise.resolve({ error: { message: "still running" } })
    );
