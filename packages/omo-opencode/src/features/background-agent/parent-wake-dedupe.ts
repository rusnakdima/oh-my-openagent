...
export function resolveParentWakePromptContext(
  promptContext: ParentWakePromptContext,
...
export function cloneParentWake(wake: PendingParentWake): PendingParentWake {
  ...
  return {
    ...
    ...(wake.dispatchedAt !== undefined
      ? { dispatchedAt: wake.dispatchedAt }
      : {}),
    ...(wake.noReplyAdmittedAt !== undefined
      ? { noReplyAdmittedAt: wake.noReplyAdmittedAt }
      : {}),
...
export function isRedundantParentWake(
...
export function mergeParentWakeNotifications(
  existingNotifications: readonly string[],
...
function parentWakePromptContextMatches(
  left: PendingParentWake,
  right: PendingParentWake,
): boolean {
  return JSON.stringify(left.promptContext) ===
    JSON.stringify(right.promptContext);
}

function parentWakeReplyModeIsCovered(
  latestWake: PendingParentWake,
  dispatchedWake: PendingParentWake,
): boolean {
...
function parentWakeNotificationsAreCovered(
  latestWake: PendingParentWake,
  dispatchedWake: PendingParentWake,
): boolean {
  const dispatchedNotifications = new Set(dispatchedWake.notifications);
  return latestWake.notifications.every((notification) =>
    dispatchedNotifications.has(notification)
  );
}
...
export function isFailureParentWake(wake: PendingParentWake): boolean {
  return wake.shouldReply &&
    wake.notifications.some((notification) =>
      getSystemReminderHeaderLines(notification).some(
        isBackgroundTaskFailureHeader,
      )
    );
}
...
function isBackgroundTaskProgressNotification(notification: string): boolean {
  if (
    !getSystemReminderHeaderLines(notification).some(
      isBackgroundTaskProgressHeader,
    )
  ) {
    return false;
  }
  ...
  return notification.split("\n").some((line) =>
    /^\*\*\d+ tasks? still in progress\.\*\* You WILL be notified when ALL complete\.$/
      .test(line.trim())
  );
}
...
function getSystemReminderHeaderLines(notification: string): string[] {
  ...
  const headerStartIndex =
    lines[firstContentLineIndex]?.trim() === "<system-reminder>"
