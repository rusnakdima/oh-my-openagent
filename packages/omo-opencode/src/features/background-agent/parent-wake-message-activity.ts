...
function latestTimeActivity(
  time: ParentWakeMessageTime | undefined,
): number | undefined {
  if (!time) {
    return undefined;
  }
...
export function getParentWakeMessageCreatedAt(
  message: ParentWakeMessageActivity,
): number | undefined {
  return timestampFromUnknown(
    message.info?.time?.created ?? message.time?.created,
  );
}

export function getParentWakeMessageActivityAt(
  message: ParentWakeMessageActivity,
): number | undefined {
  ...
  for (const part of message.parts ?? []) {
    ...
    if (
      partActivity !== undefined &&
      (latest === undefined || partActivity > latest)
    ) {
      latest = partActivity;
    }
  }
