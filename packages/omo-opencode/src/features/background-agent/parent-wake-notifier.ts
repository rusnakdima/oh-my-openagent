...
import type {
  ParentWakeNotifierDeps,
  ParentWakeNotifierOptions,
} from "./parent-wake-notifier-types";
import {
...
export class ParentWakeNotifier {
  ...
  async requeueDispatchedParentWake(
    ...
    if (
      await this.sessionInspector.hasAssistantOrToolOutputAfterDispatchedWake(
        sessionID,
        wake,
      )
    ) {
  ...
  requeueDispatchedParentWakeAfterEmptyAssistantTurn(
    sessionID: string,
  ): boolean {
    const wake = this.dispatchedTracker.getWake(sessionID);
    if (!wake) {
    ...
    log(
      "[background-agent] Requeued dispatched parent wake after empty assistant turn:",
      { sessionID },
    );
    return true;
  }
