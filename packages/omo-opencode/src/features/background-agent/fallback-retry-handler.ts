...
export async function tryFallbackRetry(args: {
  ...
  const canUseProviderExhaustionFallback = deps
    .isProviderExhaustionFallbackEligible(errorInfo);
  const canRetry =
  ...
  const connectedProviders = providerModelsCache?.connected ??
    deps.readConnectedProvidersCache();
  ...
  while (fallbackChain && selectedAttemptCount < fallbackChain.length) {
    ...
    const candidateModelID = deps.transformModelForProvider(
      candidateProviderID,
      candidate.model,
    );
    const isNoOpFallback = !!task.model &&
      candidateProviderID.toLowerCase() ===
        task.model.providerID.toLowerCase() &&
      canonicalizeModelID(candidateModelID) ===
        canonicalizeModelID(task.model.modelID);
    if (isNoOpFallback) {
  ...
  task.retryNotification = {
    previousSessionID,
    failedModel: previousModel
      ? `${previousModel.providerID}/${previousModel.modelID}`
      : undefined,
  ...
  if (task.teamRunId && !task.onSessionCreated) {
    deps.log(
      "[background-agent] team-mode fallback denied: task has teamRunId but no onSessionCreated; cannot preserve team membership",
      {
        taskId: task.id,
  ...
  if (previousSessionID) {
    await abortWithTimeout(client, previousSessionID).catch((err) => {
      log(
        `[background-agent] Failed to abort previous session ${previousSessionID}`,
        {
