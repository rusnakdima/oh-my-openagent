...
function createHarness(opts: {
  ...
  const client = {
    session: {
      status: async () => ({
        data: { "parent-1": { type: active ? "busy" : "idle" } },
      }),
      messages: async () => ({ data: [] }),
    ...
    session: {
      ...
      messages: (
        input: { path: { id: string }; query: { directory: string } },
      ) => Promise<unknown>;
  ...
  const injector = new MonitorOutputInjector({
    ...
    ...(opts.maxActiveDeferMs === undefined
      ? {}
      : { maxActiveDeferMs: opts.maxActiveDeferMs }),
    now: () => currentTime,
...
describe("monitor terminal batch parent wake", () => {
  test("terminal batch dispatches as a reply-producing prompt", async () => {
    ...
    expect(
      hasInternalNoReplyMarker(firstPartText(harness.calls[0] as DispatchCall)),
    ).toBe(false);
  });
  ...
  test("streaming batch stays a no-reply observation", async () => {
    ...
    harness.injector.queueBatch(
      createRecord({ status: "running", exitCode: undefined }),
      createBatch(1, true),
    );
    await harness.injector.flushMonitor("mon_1");
  ...
  test("over-ceiling terminal batch dispatches ahead of queued streaming batches while the parent is busy", async () => {
    ...
    expect(harness.calls[0]?.checkStatus).toBe(false);
    expect(
      harness.injector.getPendingBatches("mon_1").map((batch) =>
        batch.batchSeq
      ),
    ).toEqual([1, 2]);
  });
