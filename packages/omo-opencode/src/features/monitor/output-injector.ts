...
import type {
  DispatchedMonitorOutput,
  MonitorOutputInjectorDeps,
  PendingMonitorOutput,
} from "./output-injector-types";
import type { MonitorRecord, OutputBatch } from "./types";

const SAME_SOURCE_RETRY_MS = 2_000;

export class MonitorOutputInjector {
  private readonly pendingOutputs: Map<string, PendingMonitorOutput> =
    new Map();
  private readonly dispatchedOutputs: Map<string, DispatchedMonitorOutput> =
    new Map();
  ...
  constructor(private readonly deps: MonitorOutputInjectorDeps) {
    if (deps.postDispatchHoldMs <= 0) {
      throw new Error(
        "MonitorOutputInjector requires a nonzero postDispatchHoldMs",
      );
    }
  }
  ...
  queueBatch(record: MonitorRecord, batch: OutputBatch): void {
    ...
    if (pending) {
      ...
      if (
        !pending.batches.some((existingBatch) =>
          this.createSource(record, existingBatch) === source
        )
      ) {
  ...
  async flushMonitor(monitorId: string): Promise<void> {
    ...
    const hasForcedTerminalBatch = this.hasTerminalBatchOverActiveDeferCeiling(
      pending,
    );
    ...
    if (sessionActive && hasForcedTerminalBatch) {
      ...
      for (const batch of pending.batches) {
        const destination =
          this.shouldForceActiveDispatch(pending.record, batch)
    ...
    if (
      await latestAssistantTurnBlocksMonitorOutput(
        this.deps.client,
        this.deps.directory,
        sessionID,
      )
      ...
      this.scheduleFlush(monitorId);
      log(
        "[monitor] Deferred output injection because latest assistant turn blocks internal prompts:",
        { sessionID, monitorId },
      );
      return;
    }
    ...
    if (
      await isUserMessageInProgress(
        this.deps.client,
        this.deps.directory,
        sessionID,
        this.now(),
        this.deps.userMessageInProgressWindowMs,
      )
    ) {
      this.scheduleFlush(monitorId);
      log(
        "[monitor] Deferred output injection because user message just arrived:",
        { sessionID, monitorId },
      );
      return;
    }
    ...
    while (pending.batches.length > 0) {
      ...
      const forceActiveDispatch = this.shouldForceActiveDispatch(
        pending.record,
        batch,
      );
    ...
    try {
      ...
      if (promptResult.status === "failed") {
        if (
          ...
          await hasAcceptedMessageAfterDispatchedMonitorOutput(
            this.deps.client,
            this.deps.directory,
            sessionID,
            {
          ...
          log(
            "[monitor] Treated failed monitor output prompt as accepted after observing session history:",
            {
              sessionID,
              monitorId: record.id,
      ...
      if (
        promptResult.status === "reserved" && promptResult.reservedBy === source
      ) {
        if (this.dispatchedOutputs.has(source)) {
          log(
            "[monitor] Suppressed duplicate monitor output during promptAsync gate hold:",
            {
              sessionID,
              monitorId: record.id,
              batchSeq: batch.batchSeq,
      ...
      if (!isInternalPromptDispatchAccepted(promptResult)) {
        ...
        log(
          "[monitor] Deferred output injection skipped by promptAsync gate:",
          {
            sessionID,
            monitorId: record.id,
            batchSeq: batch.batchSeq,
      ...
      this.scheduleFlush(record.id);
      log("[monitor] Failed to send deferred monitor output:", {
        sessionID,
        monitorId: record.id,
        batchSeq: batch.batchSeq,
        error,
  ...
  private scheduleFlush(
    monitorId: string,
    delayMs = this.deps.pendingRetryMs,
  ): void {
    this.deps.scheduleFlush?.(
      monitorId,
      delayMs,
      () => this.flushMonitor(monitorId),
    );
  }
