...
export async function loadMonitorSessionMessages(
  ...
  try {
    ...
    log(
      "[monitor] Failed to inspect parent session messages for output injection safety:",
      { sessionID, error },
    );
    return [];
  }
...
export async function isUserMessageInProgress(
  ...
  const messages = await loadMonitorSessionMessages(
    client,
    directory,
    sessionID,
  );
...
export function monitorMessageHasOutput(
  message: MonitorSessionMessage,
): boolean {
  const role = getMessageRole(message);
  if (role !== "assistant" && role !== "tool") {
...
export function monitorMessageContainsBatch(
  message: MonitorSessionMessage,
