...
} from "../background-agent/process-cleanup";
...
export class MonitorManager implements MonitorManagerContract {
  ...
  constructor(private readonly options: MonitorManagerOptions) {
    ...
    this.registerCleanup = options.deps?.registerManagerForCleanup ??
      registerManagerForCleanup;
    this.unregisterCleanup = options.deps?.unregisterManagerForCleanup ??
      unregisterManagerForCleanup;
  ...
  async start(opts: MonitorStartOpts): Promise<MonitorRecord> {
    ...
    if (!filterResult.filter) {
      throw new Error(
        `monitor_start match_pattern rejected: ${
          filterResult.error ?? "invalid pattern"
        }`,
      );
    }

    const monitoredProcess = spawnMonitorProcess(
      this.options.deps,
      opts.command,
      this.config.max_runtime_ms,
    );
    const ring = new MonitorRingBuffer({
      ringMaxLines: this.config.ring_max_lines,
    });
    ...
    observeProcessExit(
      state,
      this.logger,
      (parentSessionId) => this.enforceTerminalRetention(parentSessionId),
    );
    return { ...record };
  }
  ...
  handleEvent(event: MonitorManagerEvent): void {
    ...
    if (event.type === "session.deleted") {
      void this.stopSessionMonitors(event.sessionId).catch((error) => {
        this.logger("[monitor] Failed to stop monitors for deleted session", {
  ...
  private scheduleFlush(
    ...
    const timer = this.scheduler.setTimer(() => {
      ...
      void operation().catch((error) => {
        this.logger("[monitor] Failed to flush monitor output", {
          monitorId,
          error,
        });
      });
  ...
  private addMonitor(state: InternalMonitorState): void {
    ...
    const ids =
      this.monitorsByParentSession.get(state.record.parentSessionId) ??
        new Set<MonitorId>();
  ...
  private assertSessionCapacity(sessionId: string): void {
    ...
    if (activeCount >= this.config.max_monitors_per_session) {
      throw new Error(
        `max_monitors_per_session reached for session ${sessionId}`,
      );
    }
  }
  ...
  private flushSessionMonitors(sessionId: string): void {
    ...
    for (const id of ids) {
      ...
      void state.injector.flushMonitor(id).catch((error) => {
        this.logger(
          "[monitor] Failed to flush monitor output on session idle",
          { sessionId, monitorId: id, error },
        );
      });
    }
  }
