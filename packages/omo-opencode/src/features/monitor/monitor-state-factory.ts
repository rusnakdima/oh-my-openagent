...
import { type MonitoredProcess, spawnMonitoredProcess } from "./process";
...
export function createMonitorState(
  input: MonitorStateFactoryInput,
): InternalMonitorState {
  const {
    record,
    monitoredProcess,
  ...
  pipeline.onBatch((batch) => {
    ...
    const outputBatch: OutputBatch = {
      ...
      stillRunning: state.record.status === "running" ||
...
export function observeProcessExit(
  ...
  void state.process.exited.then((result) => {
    ...
    logger("[monitor] monitored process failed", {
      monitorId: state.record.id,
      error,
    });
    onTerminal(state.record.parentSessionId);
