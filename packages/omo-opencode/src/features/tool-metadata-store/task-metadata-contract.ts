...
function readSessionIdFromMetadata(
  ...
  return readString(metadata.sessionId) ?? readString(metadata.sessionID) ??
...
function readTaskIdFromMetadata(
  ...
  return readString(metadata.taskId) ?? readString(metadata.taskID) ??
...
export function extractTaskLink(
  ...
  if (isRecord(metadata)) {
    ...
    if (
      metadataLink.sessionId || metadataLink.taskId ||
      metadataLink.backgroundTaskId || metadataLink.agent ||
  ...
  if (
    parsed.sessionId || parsed.taskId || parsed.backgroundTaskId ||
    parsed.agent || parsed.category
  ) {
