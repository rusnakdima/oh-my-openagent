...
describe("recoverToolMetadata", () => {
  ...
  test("#given metadata stored under a diverging session id #when recovering with the same call id #then the payload is recovered once", () => {
    // given
    const payload = {
      title: "Background explore",
      metadata: { sessionId: "ses_child_explore" },
    };
    storeToolMetadata("ses_child_explore", "call_abc123", payload);

    // when
    const recovered = recoverToolMetadata("ses_parent_main", {
      callID: "call_abc123",
    });
    const consumedAgain = recoverToolMetadata("ses_child_explore", {
      callID: "call_abc123",
    });
  ...
  test("#given the same call id pending in multiple foreign sessions #when recovering #then the ambiguous match is declined", () => {
    ...
    // when
    const recovered = recoverToolMetadata("ses_other", {
      callID: "call_collide",
    });

    // then
    expect(recovered).toBeUndefined();
    expect(recoverToolMetadata("ses_a", { callID: "call_collide" })).toEqual({
      title: "A",
    });
    expect(recoverToolMetadata("ses_b", { callID: "call_collide" })).toEqual({
      title: "B",
    });
  });
});
