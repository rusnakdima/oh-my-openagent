...
describe("checked-in opengateway-models.json", () => {
  ...
  test("every entry has a usable name, limits, costs and text output modality", () => {
    ...
    for (const [id, entry] of entries) {
      ...
      expect(typeof entry.cost.cache_read, `${id} cost.cache_read`).toBe(
        "number",
      );
      expect(typeof entry.cost.cache_write, `${id} cost.cache_write`).toBe(
        "number",
      );
      expect(entry.modalities.output, `${id} modalities.output`).toEqual([
        "text",
      ]);
      expect(entry.modalities.input, `${id} modalities.input`).toContain(
        "text",
      );
    }
  });
  ...
  test("every entry is tool-capable and flags attachments only for image inputs", () => {
    ...
    for (const [id, entry] of entries) {
      ...
      expect(entry.attachment, `${id} attachment`).toBe(
