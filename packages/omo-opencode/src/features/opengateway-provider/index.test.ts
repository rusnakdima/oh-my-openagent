...
describe("applyOpenGatewayProviderConfig", () => {
  ...
  test("injects the provider when the API key env var is set", () => {
    ...
    expect(Object.keys(provider?.models ?? {}).length).toBeGreaterThanOrEqual(
      60,
    );
  });
  ...
  test("injects the provider when auth.json holds an opengateway entry", () => {
    ...
    writeAuthFile(
      JSON.stringify({
        [OPENGATEWAY_PROVIDER_ID]: { type: "api", key: "sk-from-auth" },
      }),
    );
    const config: Record<string, unknown> = {};
    ...
    expect(Object.keys(readOpenGateway(config)?.models ?? {}).length)
  ...
  test("preserves every user-set value and fills only the missing ones", () => {
    ...
    expect(Object.keys(provider?.models ?? {}).length).toBe(
      catalogIds.length + 1,
    );
  });
  ...
  test("keeps a user override of a catalog model instead of the catalog entry", () => {
    ...
    const provider = readOpenGateway(config);
    expect(provider?.models?.[overriddenId]).toEqual({
      name: "Pinned",
      limit: { context: 1234, output: 56 },
    });
  ...
  test("does not share catalog objects with the injected config", () => {
    ...
    const injectedModels = readOpenGateway(config)?.models as Record<
      string,
      { name: string }
    >;
    ...
    const secondModels = readOpenGateway(secondConfig)?.models as Record<
      string,
      { name: string }
    >;
