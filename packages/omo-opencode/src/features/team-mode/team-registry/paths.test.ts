...
describe("paths", () => {
  ...
  test("discoverTeamSpecs prefers project scope", async () => {
    ...
    // when
    const teamSpecs = await discoverTeamSpecs(
      TeamModeConfigSchema.parse({ base_dir: userBaseDir }),
      projectRoot,
      {
        log: captureLog,
      },
  ...
  test("ensureBaseDirs swallows EPERM from chmod and logs a warning instead of aborting team-mode init", async () => {
    ...
    let chmodCalls = 0;
    const chmodWithEperm: typeof chmod = async (
      target,
      _mode,
    ): Promise<void> => {
      chmodCalls += 1;
      const eperm = Object.assign(
        new Error(`EPERM: operation not permitted, chmod '${target}'`),
        {
          code: "EPERM",
          syscall: "chmod",
          path: target,
    ...
    expect("code" in firstWarning ? firstWarning.code : undefined).toBe(
      "EPERM",
    );
    expect("path" in firstWarning ? firstWarning.path : undefined).toContain(
      baseDir,
    );
  });
});
