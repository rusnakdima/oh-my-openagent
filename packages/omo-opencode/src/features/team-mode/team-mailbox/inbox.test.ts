...
const { TeamModeConfigSchema } = await import(
  "../../../config/schema/team-mode"
);
...
describe("listUnreadMessages", () => {
  test("returns FIFO messages while skipping malformed, processed, and dot files", async () => {
    ...
    const config = TeamModeConfigSchema.parse({
      base_dir: await createBaseDirectory(),
    });
    ...
    await writeFile(
      path.join(inboxDir, "earlier.json"),
      JSON.stringify({
        version: 1,
        messageId: randomUUID(),
        from: "m3",
