...
async function createCallerSession(
  sessionName: string,
): Promise<
  { callerSessionId: string; callerPaneId: string; socketPath: string }
> {
  ...
  if (!createdSession.success) {
    throw new Error(
      `failed to create caller tmux session: ${
        createdSession.stderr || createdSession.stdout
      }`,
    );
  }
  ...
  if (!callerSessionId || !callerPaneId) {
    throw new Error(
      `failed to parse caller session identifiers: ${createdSession.stdout}`,
    );
  }

  const socketPathResult = await runTmuxCommand([
    "display-message",
    "-p",
    "-t",
    callerPaneId,
  ...
  if (!socketPathResult.success || socketPathResult.stdout.length === 0) {
    throw new Error(
      `failed to resolve tmux socket path: ${
        socketPathResult.stderr || socketPathResult.stdout
      }`,
    );
  }
...
async function listWindows(sessionId: string): Promise<TmuxWindow[]> {
  const listedWindows = await runTmuxCommand([
    "list-windows",
    "-t",
    sessionId,
    "-F",
  ...
  if (!listedWindows.success) {
    throw new Error(
      `failed to list tmux windows: ${
        listedWindows.stderr || listedWindows.stdout
      }`,
    );
  }
...
async function invokeCreateTeamLayout(
  ...
  const result = await Promise.resolve(
    Reflect.apply(createTeamLayout, undefined, [
      teamRunId,
      members,
      tmuxManager,
    ]),
...
describe("team-mode live tmux smoke", () => {
  beforeEach(async () => {
    ...
    const { callerSessionId, callerPaneId, socketPath } =
      await createCallerSession(callerSessionName);
  ...
  test.skipIf(!LIVE)(
    "#given a real caller tmux session and two mock members #when createTeamLayout runs #then teammate panes appear in the caller window and cleanup leaves the session intact",
    async () => {
      // given
      ...
      // when
      const layoutResult = await invokeCreateTeamLayout(
        layoutModule,
        teamRunId,
        members,
        state.tmuxManager,
      ...
      const panesAppeared = await waitForCondition(async () => {
        const panes = await runTmuxCommand([
          "list-panes",
          "-t",
          state.callerSessionId,
          "-F",
          "#{pane_id}",
        ]);
        return panes.success &&
          Object.values(layoutResult.focusPanesByMember).every((paneId) =>
            panes.stdout.split("\n").includes(paneId)
          );
      });
      const windowsUnchangedBeforeCleanup = await waitForCondition(async () => {
        const windows = await listWindows(state.callerSessionId);
        return windows.map((window) => window.id).join(",") ===
          initialWindows.map((window) => window.id).join(",");
      });

      await invokeRemoveTeamLayout(
        layoutModule,
        teamRunId,
        state.tmuxManager,
        layoutResult,
      ...
      const callerSessionStillAlive = await runTmuxCommand([
        "has-session",
        "-t",
        state.callerSessionId,
      ]);
