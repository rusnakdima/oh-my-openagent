...
function defaultRunTmuxCommand(
  _tmuxPath: string,
  args: Array<string>,
  _options?: unknown,
) {
  ...
  if (
    command === "display" && args.includes("#{session_name}:#{window_index}")
  ) {
  ...
  if (command === "display") {
    return Promise.resolve(
      createTmuxCommandResult(displaySessionId, displaySuccess),
    );
  }

  if (command === "list-panes") {
    const windowTarget = args[2] ?? "";
    const allPanes = panesByWindow.get(windowTarget) ??
  ...
  if (command === "split-window") {
    ...
    const matchedEntry = Array.from(panesByWindow.entries()).find(([, panes]) =>
      panes.includes(targetPane ?? "")
    );
    if (matchedEntry) {
...
async function loadLayoutModule() {
  const deps: TeamLayoutDeps = {
    ...
    resolveCallerTmuxSession: async () => {
      if (
        !process.env.TMUX_PANE || !displaySuccess ||
        !/^\$[0-9]+$/.test(displaySessionId)
      ) {
        return null;
      }
  ...
  return {
    ...
    createTeamLayout: (
      teamRunId: string,
      members: Parameters<typeof createTeamLayout>[1],
      tmuxMgr: Parameters<typeof createTeamLayout>[2],
    ) => {
...
describe("team-layout-tmux", () => {
  ...
  beforeEach(() => {
    ...
    spyOn(sharedTmuxModule, "isServerRunning").mockImplementation(
      isServerRunningMock,
    );
    spyOn(sharedTmuxModule, "runTmuxCommand").mockImplementation(
      runTmuxCommandMock,
    );
    spyOn(resolveCallerTmuxSessionModule, "resolveCallerTmuxSession")
  ...
  test("creates teammate panes in the caller window and sends attach via send-keys", async () => {
    ...
    expect(commands.filter((args) => args[0] === "split-window")).toHaveLength(
      2,
    );
    ...
    const paneOptionCalls = commands.filter((args) =>
      args[0] === "set-option" && args[1] === "-p"
    );
    expect(paneOptionCalls).toContainEqual([
      "set-option",
      "-p",
      "-t",
      "%1",
  ...
  test("#given env auth set #when createTeamLayout runs #then send-keys attach commands omit secrets while split-window forwards pane env", async () => {
    ...
    try {
      ...
      // then
      const commands = getCommands();
      const sendKeys = commands.filter((args) => args[0] === "send-keys").map((
        args,
      ) => args.join(" "));
      ...
      expect(splitWindow).toContain(
        `OPENCODE_SERVER_PASSWORD=${fixturePassword}`,
      );
      expect(splitWindow).toContain(
        `OPENCODE_SERVER_USERNAME=${fixtureUsername}`,
      );
    } finally {
  ...
  test("#given no OPENCODE_SERVER_PASSWORD #when createTeamLayout runs #then attach command has no env-prefix", async () => {
    ...
    try {
      ...
      // then
      const sendKeys = getCommands().filter((args) => args[0] === "send-keys")
        .map((args) => args.join(" "));
      const attach = sendKeys.find((s) => s.includes("opencode attach"));
      expect(attach).toBeDefined();
      expect(attach).not.toContain("OPENCODE_SERVER_PASSWORD");
  ...
  test("uses caller window main-vertical layout with caller pane as primary", async () => {
    ...
    // then
    const commands = getCommands();
    const selectLayoutArgs = commands.filter((args) =>
    ...
    expect(Object.keys(result?.focusPanesByMember ?? {}).sort()).toEqual([
      "m1",
      "m2",
      "m3",
    ]);
  ...
  test("#given caller inside tmux #when createTeamLayout runs #then it never steals focus or mutates window border options", async () => {
    ...
    expect(
      commands.some((args) =>
        args[0] === "select-pane" && !args.includes("-T")
      ),
    ).toBe(false);
    expect(
      commands.some((args) => args[0] === "set-option" && args[1] !== "-p"),
    ).toBe(false);
  });
  ...
  test("#given delegated pane startup #when split-window is called #then -d flag is always present to prevent terminal probe replies leaking into caller pane (fix #2887)", async () => {
    ...
    const commands = getCommands();
    const splitWindowCalls = commands.filter((args) =>
      args[0] === "split-window"
    );
  ...
  test("#given ownedSession=false and the first kill-window fails #when removeTeamLayout runs #then the second kill-window still fires", async () => {
    ...
    runTmuxCommandMock.mockImplementation(
      (_tmuxPath: string, args: Array<string>, _options?: unknown) => {
        if (args[0] === "kill-window") {
          killWindowCallCount += 1;
          return Promise.resolve(
            createTmuxCommandResult("", killWindowCallCount > 1),
          );
        }
        ...
        if (command === "new-session") {
          return Promise.resolve(
            createTmuxCommandResult(`@${nextWindowNumber++}`),
          );
        }
        if (command === "new-window") {
          return Promise.resolve(
            createTmuxCommandResult(
              `@${nextWindowNumber++} %${nextPaneNumber++}`,
            ),
          );
        }
        if (command === "split-window") {
          return Promise.resolve(
            createTmuxCommandResult(`%${nextPaneNumber++}`),
          );
        }
  ...
  describe("createTeamLayout - focus/grid window topology", () => {
    test("#given caller inside tmux #when createTeamLayout runs #then uses the caller window without a new session", async () => {
      ...
      expect(commands.filter((args) => args[0] === "new-window").length).toBe(
        0,
      );
      expect(
        commands.some((args) =>
          args[0] === "split-window" &&
          args.includes(process.env.TMUX_PANE ?? "")
        ),
      ).toBe(true);
    });
    ...
    test("#given 3 members #when createTeamLayout runs #then focusPanesByMember contains 3 distinct pane ids", async () => {
      ...
      expect(new Set(Object.values(result?.focusPanesByMember ?? {})).size)
        .toBe(3);
    });
    ...
    test("#given layout created #when createTeamLayout runs #then it records focus panes only", async () => {
      ...
      expect(
        commands.some((args) =>
          args[0] === "send-keys" && args.includes("Enter")
        ),
      ).toBe(true);
    });
  });
