...
function buildPromptGenerationParams(
  model: PromptGenerationModel | undefined,
): Omit<TeamMemberPromptBody, "parts" | "agent" | "model" | "variant"> {
  if (!model) {
    return {};
  }
...
export function buildMemberPromptBody(
  ...
  const normalizedAgent = member.subagent_type
    ? stripAgentListSortPrefix(member.subagent_type)
    : undefined;
  const launchAgent = resolveRegisteredAgentName(normalizedAgent) ??
    normalizedAgent;
