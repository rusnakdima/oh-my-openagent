...
describe("team-session-registry", () => {
  ...
  test("unregisters a single session by sessionId", () => {
    ...
    expect(lookupTeamSession("ses_beta")).toEqual({
      teamRunId: "team-1",
      memberName: "worker-1",
      role: "member",
    });
  ...
  test("unregisters every session that belongs to the given teamRunId", () => {
    ...
    expect(lookupTeamSession("ses_gamma")).toEqual({
      teamRunId: "team-2",
      memberName: "solo",
      role: "member",
    });
  ...
  test("registering the same sessionId twice overwrites the previous entry", () => {
    ...
    registerTeamSession("ses_alpha", {
      teamRunId: "team-2",
      memberName: "promoted-lead",
      role: "lead",
    });

    // then
    expect(lookupTeamSession("ses_alpha")).toEqual({
      teamRunId: "team-2",
      memberName: "promoted-lead",
      role: "lead",
    });
