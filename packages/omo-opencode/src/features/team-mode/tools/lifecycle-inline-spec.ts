...
import { type TeamSpec, TeamSpecSchema } from "@oh-my-opencode/team-core/types";

export const TEAM_CREATE_USAGE =
  'team_create requires exactly one of teamName or inline_spec. Use team_create({ teamName: "existing-team" }) or team_create({ inline_spec: { name: "team-name", members: [{ name: "worker", category: "quick", prompt: "Do the assigned work." }] } }).';
...
function omitEmptyStringArgs(rawArgs: unknown): unknown {
  ...
  return Object.fromEntries(
    Object.entries(rawArgs).filter(([, value]) => value !== ""),
  );
}
...
export const TeamCreateArgsSchema = z.preprocess(
  ...
  z.object({
    ...
    const optionCount = Number(value.teamName != null) +
      Number(value.inline_spec != null);
    if (optionCount !== 1) {
      ctx.addIssue({
        code: "custom",
        message: "Provide exactly one of teamName or inline_spec.",
      });
    }
  }),
);
...
export function resolveDefaultInlineCategory(
  userCategories?: CategoriesConfig,
): string | undefined {
  const userCategoryName = Object.entries(userCategories ?? {}).find((
    [, categoryConfig],
  ) => categoryConfig.disable !== true)?.[0];
...
export function parseInlineTeamSpec(
  ...
  const parsedSpecResult = TeamSpecSchema.safeParse(
    normalizeTeamSpecInput(specObject, options),
  );
  if (!parsedSpecResult.success) {
    throw new Error(
      `Invalid inline_spec for team_create: ${
        formatTeamSpecIssues(parsedSpecResult.error)
      }. Provide an object with name and members array. Example: team_create({ inline_spec: { name: "project-analysis-team", members: [{ name: "structure-analyst", category: "quick", prompt: "Analyze project structure." }] } }).`,
    );
  }
