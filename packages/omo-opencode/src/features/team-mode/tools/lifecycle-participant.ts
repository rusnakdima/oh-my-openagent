...
export function sanitizeRuntimeState(
  runtimeState: RuntimeState,
): Omit<RuntimeState, "members"> & {
  members: Array<
    Omit<
      RuntimeState["members"][number],
      "lastInjectedTurnMarker" | "pendingInjectedMessageIds"
    >
  >;
} {
  return {
...
export async function findParticipantRuntime(
  sessionID: string,
  config: TeamModeConfig,
  deps: TeamRuntimeStoreDeps,
  ...
  if (registryEntry) {
    const runtimeState = await deps.loadRuntimeState(
      ...
      config,
    ).catch(resolveRuntimeStateLoadFailure);
    if (
      runtimeState && ACTIVE_RUNTIME_STATUSES.has(runtimeState.status) &&
      resolveRegisteredParticipant(runtimeState, registryEntry)
    ) {
      return runtimeState;
    }
  }
  ...
  for (const activeTeam of await deps.listActiveTeams(config)) {
    const runtimeState = await deps.loadRuntimeState(
      activeTeam.teamRunId,
      config,
    ).catch(resolveRuntimeStateLoadFailure);
    if (!runtimeState || !ACTIVE_RUNTIME_STATUSES.has(runtimeState.status)) {
      continue;
    }
    if (runtimeState.leadSessionId === sessionID) return runtimeState;
    if (runtimeState.members.some((member) => member.sessionId === sessionID)) {
      return runtimeState;
    }
  }
}
...
export async function resolveParticipant(
  ...
  config: TeamModeConfig,
  deps: TeamRuntimeStoreDeps,
): Promise<{ runtimeState: RuntimeState; participant?: TeamParticipant }> {
  const registryEntry = lookupTeamSession(sessionID);
  const runtimeState = await deps.loadRuntimeState(teamRunId, config);
  if (registryEntry?.teamRunId === teamRunId) {
    const participant = resolveRegisteredParticipant(
      runtimeState,
      registryEntry,
    );
  ...
  if (runtimeState.leadSessionId === sessionID) {
    return {
      ...
      participant: {
        role: "lead",
        memberName: getLeadMemberName(runtimeState),
      },
    };
  }
  const member = runtimeState.members.find((candidate) =>
  ...
  return member
    ? { runtimeState, participant: { role: "member", memberName: member.name } }
    : { runtimeState };
}
