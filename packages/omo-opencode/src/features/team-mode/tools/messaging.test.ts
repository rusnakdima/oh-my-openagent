...
const mockClient: LiveDeliveryClient = {
  session: {
    promptAsync: async () => {
      throw new Error("live delivery disabled in fixture");
    },
  },
};
...
function parseToolResult(result: string | ToolResult): TeamSendToolResult {
  return TeamSendToolResultSchema.parse(
...
describe("createTeamSendMessageTool", () => {
  test("resolveTeamRuntimeDetails preserves Error fallback for missing runtime state", async () => {
    ...
    // when
    const runtimeDetails = await resolveTeamRuntimeDetails(
      "team-run-missing",
      "session-missing",
      config,
      {
  ...
  test("routes a member message to one recipient", async () => {
    ...
    const inboxDir = getInboxDir(
      resolveBaseDir(fixture.config),
      fixture.teamRunId,
      "m2",
    );
    const [messageFile] = (await readdir(inboxDir)).filter((entry) =>
      entry.endsWith(".json")
    );
    const message = MessageSchema.parse(
      JSON.parse(await readFile(path.join(inboxDir, messageFile), "utf8")),
    );
  ...
  test("#given recipient tries path traversal #when team_send_message runs #then it rejects without creating an escaped inbox", async () => {
    // given
    const fixture = await createTeamFixture();
    const escapedInboxRoot = path.join(
      resolveBaseDir(fixture.config),
      "runtime",
      "escape",
    );
  ...
  test("persists optional message metadata from tool arguments", async () => {
    ...
    const result = await fixture.tool.execute({
      ...
      references: [{
        path: "src/features/team-mode/tools/messaging.ts",
  ...
  test("gates broadcast to the lead and fans out to active members", async () => {
    ...
    const memberOneInbox = await readdir(
      getInboxDir(resolveBaseDir(fixture.config), fixture.teamRunId, "m1"),
    );
    const memberTwoInbox = await readdir(
      getInboxDir(resolveBaseDir(fixture.config), fixture.teamRunId, "m2"),
    );
    expect(
      memberOneInbox.filter((entry) =>
        entry.endsWith(".json") && !entry.startsWith(".")
      ),
    ).toHaveLength(1);
    expect(
      memberTwoInbox.filter((entry) =>
        entry.endsWith(".json") && !entry.startsWith(".")
      ),
    ).toHaveLength(1);
  });
  ...
  test("live delivery targets the recipient worktree when available", async () => {
    // given
    const fixture = await createTeamFixture();
    const { loadRuntimeState: loadState, saveRuntimeState: saveState } =
      await import("../team-state-store/store");
  ...
  test("#given runtime marks recipient running #when team_send_message sends a peer message #then it leaves the unread mailbox path without promptAsync", async () => {
    ...
    const inboxEntries = await readdir(
      getInboxDir(resolveBaseDir(fixture.config), fixture.teamRunId, "m2"),
    );
    expect(
      inboxEntries.filter((entry) =>
        entry.endsWith(".json") && !entry.startsWith(".")
      ),
    ).toHaveLength(1);
    expect(inboxEntries.some((entry) => entry.startsWith(".delivering-"))).toBe(
      false,
    );
  });
  ...
  test("#given recipient OpenCode session is busy #when team_send_message attempts live delivery #then it releases the message for later mailbox injection", async () => {
    ...
    const client = {
      session: {
        status: async () => ({
          data: { [fixture.memberTwoSessionId]: { type: "busy" } },
        }),
        promptAsync: async () => {
          promptCalls += 1;
        },
  ...
  test("live delivery uses the registered agent alias when the runtime stores a config-key agent name", async () => {
    ...
    try {
      ...
      await waitForEvent(
        settleTimerScheduled.promise,
        "live delivery idle-settle timer",
      );
      await delivery;

      // then
      expect(calls).toHaveLength(1);
      expect(calls[0]?.agent).toBe(
        "\u200B\u200B\u200B\u200BAtlas - Plan Executor",
      );
    } finally {
  ...
  test("live delivery reapplies category routing and advanced model params for category members", async () => {
    ...
    expect(SessionCategoryRegistry.get(fixture.memberTwoSessionId)).toBe(
      "quick",
    );
  ...
  test("prefers the team session registry when the runtime member session has not been persisted yet", async () => {
    ...
    const memberOne = runtimeState.members.find((member) =>
      member.name === "m1"
    );
  ...
  test("keeps live-delivered messages reserved until the recipient idles", async () => {
    ...
    const { loadRuntimeState: loadState } = await import(
      "../team-state-store/store"
    );
    const runtimeState = await loadState(fixture.teamRunId, fixture.config);
    const recipient = runtimeState.members.find((member) =>
      member.name === "m2"
    );
  ...
  test("broadcast still queues for members whose session has not spawned yet", async () => {
    ...
    const pendingMember = stateBefore.members.find((member) =>
      member.name === "m2"
    );
  ...
  test("#given dispatchInternalPrompt fails ambiguously #when pending mark fails #then the reservation is committed to processed", async () => {
    ...
    expect(inboxEntries.filter((entry) => entry.startsWith(".delivering-")))
      .toHaveLength(0);
  ...
  test("reserves the message during live delivery so concurrent listings cannot surface it", async () => {
    ...
    const reservingClient = {
      session: {
        promptAsync: async () => {
          unreadDuringDelivery = await listUnreadMessages(
            fixture.teamRunId,
            "m2",
            fixture.config,
          );
  ...
  test("#given transform already listed a peer message while recipient becomes idle #when live delivery races it #then only the live prompt receives the message", async () => {
    ...
    let loadCount = 0;
    let transformResult:
      | Awaited<ReturnType<typeof pollAndBuildInjection>>
      | undefined;
    const deps = {
  ...
  test("hides the message from the inbox from the moment it is written for a live recipient", async () => {
    ...
    const unreadImmediately = await listUnreadMessages(
      fixture.teamRunId,
      "m2",
      fixture.config,
    );
