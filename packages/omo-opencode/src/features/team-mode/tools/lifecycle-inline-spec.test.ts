...
function createRuntimeState(
  ...
  return {
    ...
    members: spec.members.map((member) => ({
      name: member.name,
      sessionId: member.name === spec.leadAgentId
        ? undefined
        : `${member.name}-session`,
      tmuxPaneId: undefined,
      agentType: member.name === spec.leadAgentId
        ? "leader"
        : "general-purpose",
      status: "running",
      color: member.color,
...
const createTeamRunMock = mock(
  async (spec: TeamSpec, leadSessionId: string) => {
...
async function loadCreateTeamCreateTool(): Promise<
  typeof import("./lifecycle").createTeamCreateTool
> {
...
function createTeamCreateToolForTest(
  ...
  return factory(config, {} as never, {} as never, undefined, executorConfig, {
    ...
    loadTeamSpec: async () => {
      throw new Error(
        "loadTeamSpec should not be called for inline_spec tests",
      );
    },
    listActiveTeams: async () => [],
    loadRuntimeState: async () => {
      throw new Error(
        "loadRuntimeState should not be called when no active teams exist",
      );
    },
  });
}
...
describe("createTeamCreateTool inline_spec normalization", () => {
  ...
  test("accepts inline_spec objects and auto-assigns missing member names", async () => {
      ...
      members: [
        {
          kind: "category",
          category: "quick",
          prompt: "Quick scout the workspace for entrypoints.",
        },
    ...
    expect(
      result.runtimeState.members.map((member: { name: string }) =>
        member.name
      ),
    ).toEqual(["lead", "quick-1", "atlas-1"]);
  });
  ...
  test("exposes inline_spec object shape in the tool schema", async () => {
    ...
    const inlineSpecObjectSchema = inlineSpecSchema.options.find((option) =>
      option.type === "object"
    );

    // then
    expect(inlineSpecSchema.type).toBe("union");
    expect(inlineSpecSchema.options.map((option) => option.type)).toEqual([
      "object",
      "string",
    ]);
    ...
    expect(inlineSpecObjectSchema?.shape.members.element.shape.category.type)
      .toBe("optional");
    expect(
      inlineSpecObjectSchema?.shape.members.element.shape.subagent_type.type,
    ).toBe("optional");
    expect(teamCreateTool.args.inline_spec.description).toContain(
      "members must be a flat array",
    );
  });
  ...
  test("accepts stringified inline_spec values from tool calling", async () => {
      ...
      members: [
        {
          kind: "category",
          category: "quick",
          prompt: "Quick scout: survey ccapi workspace structure.",
        },
        { kind: "category", category: "deep", prompt: "Deep dive ccapi-cf." },
        {
          kind: "category",
          category: "deep",
          prompt: "Deep dive ccapi-cf-proxy.",
        },
      ],
    });
    ...
    ).toEqual(["lead", "quick-1", "deep-1", "deep-2"]);
  ...
  test("accepts category members written with natural inline prompt fields", async () => {
    ...
    const inlineSpec = {
      ...
      description:
        "Analyze the codebase from structure, core logic, and quality angles.",
      members: [
        {
          ...
          loadSkills: [],
          systemPrompt:
            "Focus on directory layouts, module boundaries, and architectural organization.",
        },
        {
          ...
          loadSkills: [],
          systemPrompt:
            "Focus on initialization flows, plugin architecture, hooks, tools, and MCP integration.",
        },
        {
          ...
          loadSkills: [],
          systemPrompt:
            "Focus on tests, CI/CD, build scripts, conventions, and anti-pattern enforcement.",
        },
      ],
    };
    ...
    await teamCreateTool.execute(
      { inline_spec: inlineSpec },
      createToolContext("lead-session", "Sisyphus"),
    );
    const firstCall = createTeamRunMock.mock.calls[0];
  ...
  test("explains how to call team_create when arguments are empty", async () => {
    ...
    expect(errorMessage).toContain(
      "team_create requires exactly one of teamName or inline_spec",
    );
  ...
  test("explains how to shape inline_spec when members are missing", async () => {
    ...
    try {
      await teamCreateTool.execute({
        inline_spec: { name: "project-analysis-team" },
      }, createToolContext("lead-session", "Sisyphus"));
    } catch (error) {
      errorMessage = error instanceof Error ? error.message : String(error);
    }
      ...
      members: [
        {
          ...
          category: "quick",
          prompt: "Analyze project structure and report concrete files.",
        },
        {
          name: "Agent 2: Core Logic Analyst",
          category: "quick",
          prompt: "Analyze initialization flow and report concrete functions.",
        },
        {
          name: "Agent 3: Quality/Process Analyst",
          category: "quick",
          prompt: "Analyze tests, builds, CI, and conventions.",
        },
      ],
    };
        ...
        members: [
          {
            ...
            category: "quick",
            subagent_type: "",
            prompt: "Temporary smoke test member.",
          },
        ],
        lead: {
          name: "",
    ...
    expect(firstCall?.[0]).toMatchObject({
      ...
      members: [
        ...
        },
        {
          ...
          kind: "category",
          category: "analysis",
          prompt: "Role: Quality/Process Analyst\ntests, builds, CI/CD",
        },
      ],
    });
  });
