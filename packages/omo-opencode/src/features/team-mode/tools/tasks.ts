...
async function resolveSenderName(
  ...
  deps: TeamTaskToolDeps,
): Promise<string> {
  const runtimeState: RuntimeState = await deps.loadRuntimeState(
    teamRunId,
    config,
  );
  const matchedMember = runtimeState.members.find((member) =>
  ...
  throw new Error(
    `team member not found for session ${sessionID ?? "unknown"}`,
  );
}
...
export function createTeamTaskCreateTool(
  ...
  deps: TeamTaskToolDeps = defaultDeps,
): ToolDefinition {
  void client;
  ...
  return tool({
    ...
    args: {
      ...
      blockedBy: tool.schema.array(tool.schema.string()).optional().describe(
        "Blocking task IDs",
      ),
    },
...
export function createTeamTaskListTool(
  ...
  return tool({
    ...
    execute: async (args: TeamTaskListArgs): Promise<string> => {
      const tasks = await deps.listTasks(args.teamRunId, config, {
        status: args.status,
        owner: args.owner,
      });
