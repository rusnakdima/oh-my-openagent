...
const createTaskMock = mock(
  async () => ({ id: "1", subject: "task one" } as Task),
);
const listTasksMock = mock(
  async () => [{ id: "1", status: "pending" } as Task],
);
const claimTaskMock = mock(
  async () => ({ id: "1", status: "claimed" } as Task),
);
...
const getTaskMock = mock(
  async () => ({ id: "1", status: "completed" } as Task),
);
...
describe("team task tools", () => {
  ...
  test("create -> list -> claim -> complete flow", async () => {
    ...
    expect(listTasksMock).toHaveBeenCalledWith("team-run-1", config, {
      status: "pending",
      owner: "member-a",
    });
  ...
  test("cross-owner update rejected", async () => {
    // given
    const config = createConfig();
    updateTaskStatusMock.mockImplementationOnce(async () => {
      throw new Error("CrossOwnerUpdateError");
    });
    const updateTool = createTeamTaskUpdateTool(config, mockClient, deps);

    // when
    const result = updateTool.execute({
      teamRunId: "team-run-1",
      taskId: "1",
      status: "in_progress",
      owner: "member-b",
  ...
  test("blockedBy enforcement", async () => {
    // given
    const config = createConfig();
    claimTaskMock.mockImplementationOnce(async () => {
      throw new Error("blocked by 2");
    });
    const updateTool = createTeamTaskUpdateTool(config, mockClient, deps);
