      ...
      member.name === recipientName
        ? {
          ...
          pendingInjectedMessageIds: Array.from(
            new Set([...member.pendingInjectedMessageIds, messageId]),
          ),
        }
        : member
    )),
...
export async function loadRuntimeStateForLiveDelivery(
  ...
  try {
    ...
    await releaseReservationsForRecipients(
      teamRunId,
      deliveredTo,
      messageId,
      config,
    ...
    log(
      "[team-mailbox] live delivery unavailable after pre-reserve, released recipients to inbox",
      {
        teamRunId,
        messageId,
        deliveredTo,
