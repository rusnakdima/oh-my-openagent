...
let aggregateStatusImplementation:
  typeof import("../team-runtime/status").aggregateStatus = async () => {
    throw new Error("aggregateStatusImplementation not set");
  };

let discoverTeamSpecsImplementation:
  typeof import("../team-registry/paths").discoverTeamSpecs = async () => {
    throw new Error("discoverTeamSpecsImplementation not set");
  };

let loadTeamSpecImplementation:
  typeof import("../team-registry/loader").loadTeamSpec = async () => {
    throw new Error("loadTeamSpecImplementation not set");
  };

let listActiveTeamsImplementation:
  typeof import("../team-state-store/store").listActiveTeams = async () => {
    throw new Error("listActiveTeamsImplementation not set");
  };

const deps = {
  aggregateStatus: (
    ...args: Parameters<typeof aggregateStatusImplementation>
  ) => aggregateStatusImplementation(...args),
  discoverTeamSpecs: (
    ...args: Parameters<typeof discoverTeamSpecsImplementation>
  ) => discoverTeamSpecsImplementation(...args),
  loadTeamSpec: (...args: Parameters<typeof loadTeamSpecImplementation>) =>
    loadTeamSpecImplementation(...args),
  listActiveTeams: (
    ...args: Parameters<typeof listActiveTeamsImplementation>
  ) => listActiveTeamsImplementation(...args),
};
...
describe("query tools", () => {
  test("team_status returns aggregated team status", async () => {
    ...
    const result = JSON.parse(
      await tool.execute({ teamRunId: "team-run-1" }, createMockContext()),
    );
