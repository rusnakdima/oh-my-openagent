...
export function getLatestShutdownRequest(
  ...
  for (
    let index = runtimeState.shutdownRequests.length - 1;
    index >= 0;
    index -= 1
  ) {
...
export const createTeamRunMock = mock(
...
export const deleteTeamMock = mock(async (
  ...
  if (
    !options?.force &&
    runtimeState.members.some((member) =>
      member.agentType !== "leader" && member.status !== "shutdown_approved" &&
      member.status !== "completed" && member.status !== "errored"
    )
  ) {
...
export const requestShutdownOfMemberMock = mock(
  async (
    teamRunId: string,
...
export const approveShutdownMock = mock(
  async (teamRunId: string, memberName: string) => {
    const runtimeState = requireRuntime(teamRunId);
...
export const rejectShutdownMock = mock(
  async (
    teamRunId: string,
    memberName: string,
    ...
    const request = getLatestShutdownRequest(
      requireRuntime(teamRunId),
      memberName,
    );
    if (request) {
...
export const listActiveTeamsMock = mock(async () =>
  Array.from(runtimes.values()).map((runtimeState) => ({
...
export const loadRuntimeStateMock = mock(async (teamRunId: string) =>
  clone(requireRuntime(teamRunId))
);
