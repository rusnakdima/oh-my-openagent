...
const TeamDeleteArgsSchema = z.object({
  teamRunId: z.string().min(1),
  force: z.boolean().optional(),
});
const TeamShutdownRequestArgsSchema = z.object({
  teamRunId: z.string().min(1),
  targetMemberName: z.string().min(1),
});
const TeamApproveShutdownArgsSchema = z.object({
  teamRunId: z.string().min(1),
...
export function createTeamDeleteTool(
  ...
  return tool({
    description:
      "Delete a completed or shutdown-approved team run. Pass force=true to tear it down even while members are still active.",
    args: {
      teamRunId: tool.schema.string(),
    ...
    async execute(rawArgs, toolContext) {
      ...
      const { runtimeState, participant } = await resolveParticipant(
        args.teamRunId,
        runtimeContext.sessionID,
        config,
        deps,
      );
      const isOrphanedForceDelete = args.force === true &&
        runtimeState.status === "orphaned";
      const isStuckDeletingForceDelete = args.force === true &&
        runtimeState.status === "deleting";
      const isForceBypass =
        (isStuckDeletingForceDelete || isOrphanedForceDelete) &&
...
export function createTeamShutdownRequestTool(
  ...
  return tool({
    ...
    args: {
      teamRunId: tool.schema.string(),
      targetMemberName: tool.schema.string(),
    },
    async execute(rawArgs, toolContext) {
      ...
      await deps.requestShutdownOfMember(
        args.teamRunId,
        args.targetMemberName,
        participant.memberName,
        config,
      );
      return JSON.stringify({
        teamRunId: args.teamRunId,
        targetMemberName: args.targetMemberName,
        requesterName: participant.memberName,
    ...
    async execute(rawArgs, toolContext) {
      ...
      if (
        ...
        (participant.role !== "lead" &&
          participant.memberName !== args.memberName)
      ) {
        throw new Error(
          "team_approve_shutdown: caller must be target member or team lead",
        );
      }
      await deps.approveShutdown(
        args.teamRunId,
        args.memberName,
        participant.memberName,
        config,
      ...
      return JSON.stringify({
        ...
        approverName: participant.memberName,
  ...
  return tool({
    ...
    async execute(rawArgs, toolContext) {
      ...
      if (
        ...
        throw new Error(
          "team_reject_shutdown: caller must be target member or team lead",
        );
      }
      await deps.rejectShutdown(
        args.teamRunId,
        args.memberName,
        participant.memberName,
        args.reason,
      ...
      return JSON.stringify({
        teamRunId: args.teamRunId,
        memberName: args.memberName,
        rejectedBy: participant.memberName,
        reason: args.reason,
