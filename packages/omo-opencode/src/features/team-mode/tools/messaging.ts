...
export function createTeamSendMessageTool(
  ...
  return tool({
    ...
    args: {
      ...
      kind: tool.schema.enum(MESSAGE_TOOL_KINDS).optional().default("message")
        .describe("Message kind"),
      correlationId: tool.schema.string().optional().describe(
        "Optional UUID correlation ID. Do not use task IDs like 'task-1'.",
      ),
      ...
      references: tool.schema.array(tool.schema.object({
        ...
        "Optional references as [{ path, description? }]",
      ),
    },
    execute: async (rawArgs, context) => {
      ...
      const targetDirectory = typeof runtimeContext.directory === "string"
        ? runtimeContext.directory
        : process.cwd();

      const teamRuntime = await resolveTeamRuntimeDetails(
        args.teamRunId,
        sessionID,
        config,
        deps,
      ...
      try {
        ...
        log(
          "[team-mailbox] deliverLive top-level error (message already in inbox, safe to ignore)",
          {
            error: liveError instanceof Error
              ? liveError.message
              : String(liveError),
