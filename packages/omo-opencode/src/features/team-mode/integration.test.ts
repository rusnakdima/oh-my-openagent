...
mock.module(
  "./team-runtime/resolve-member",
  () => ({ resolveMember: resolveMemberMock }),
);
...
const { createTask, claimTask, listTasks, updateTaskStatus } = await import(
  "./team-tasklist"
);
const { resumeAllTeams } = await import("./team-state-store/resume");
const { loadRuntimeState, saveRuntimeState } = await import(
  "./team-state-store/store"
);

const temporaryDirectories: string[] = [];
type MockClient = ExecutorContext["client"] & {
  session: { get: ReturnType<typeof mock> };
};

function createConfig(
  baseDir: string,
  overrides: Partial<TeamModeConfig> = {},
): TeamModeConfig {
...
function createClient(aliveSessionIds: ReadonlySet<string>): MockClient {
  return {
    session: {
      get: mock(async ({ path: { id } }: { path: { id: string } }) =>
        aliveSessionIds.has(id) ? { data: { id } } : {
          error: Object.assign(new Error("session not found"), {
            status: 404,
          }),
        }
      ),
...
function createManager(
  ...
  let launchCount = 0;
  manager.launch = mock((input: LaunchInput) =>
    launchImpl?.(input) ?? Promise.resolve({
      id: `task-${++launchCount}`,
...
describe("team-mode integration", () => {
  test(
    "C-10.1 creates a single-member echo team, delivers mail, surfaces unread status, and deletes runtime",
    async () => {
      // given
      const baseDir = await createBaseDir();
      const config = createConfig(baseDir);
      const manager = createManager();
      const runtime = await createTeamRun(
        createSpec("echo-team", "echo", [{
          kind: "subagent_type",
          name: "echo",
          subagent_type: "atlas",
        ...
        "ses_lead",
        createContext(baseDir, manager, new Set(["ses_lead"])),
        config,
        manager,
      );
      ...
      const delivered = await sendMessage(
        ...
        runtime.teamRunId,
        config,
        { isLead: true, activeMembers: ["echo"] },
      );
      ...
      expect(
        await exists(
          getRuntimeStateDir(resolveBaseDir(config), runtime.teamRunId),
        ),
      ).toBe(false);
    },
    30000,
  ...
  test(
    "C-10.2 runs a 2-member pipeline where worker claims and completes a lead-created task",
    async () => {
      // given
      const baseDir = await createBaseDir();
      const config = createConfig(baseDir);
      ...
      const createdTask = await createTask(runtime.teamRunId, {
        subject: "X",
        description: "Ship X",
        blocks: [],
        blockedBy: [],
      ...
      // when
      const claimedTask = await claimTask(
        runtime.teamRunId,
        createdTask.id,
        "worker",
        config,
      ...
      const completedTasks = await listTasks(runtime.teamRunId, config, {
        status: "completed",
      });
  ...
  test(
    "C-10.3 resumes alive teams, orphans dead leads, fails stuck creating teams, and cleans deleting runs",
    async () => {
      // given
      const baseDir = await createBaseDir();
      ...
      const aliveRuntime = await createTeamRun(
        createSpec("alive-team", "lead", [{
          kind: "subagent_type",
          name: "lead",
          subagent_type: "sisyphus",
          backendType: "in-process",
      ...
      const deadRuntime = await createTeamRun(
        createSpec("dead-team", "lead", [{
          kind: "subagent_type",
          name: "lead",
          subagent_type: "atlas",
          backendType: "in-process",
      ...
      const stuckRuntime = await createTeamRun(
        createSpec("stuck-team", "lead", [{
          kind: "subagent_type",
          name: "lead",
          subagent_type: "atlas",
          backendType: "in-process",
      ...
      const deletingRuntime = await createTeamRun(
        createSpec("deleting-team", "lead", [{
          kind: "subagent_type",
          name: "lead",
          subagent_type: "atlas",
          backendType: "in-process",
      ...
      await saveRuntimeState({
        ...(await loadRuntimeState(stuckRuntime.teamRunId, config)),
        status: "creating",
        createdAt: Date.now() - 40 * 60 * 1000,
      }, config);
      await saveRuntimeState({
        ...(await loadRuntimeState(deletingRuntime.teamRunId, config)),
        status: "deleting",
      }, config);
      ...
      expect((await loadRuntimeState(aliveRuntime.teamRunId, config)).status)
        .toBe("active");
      expect((await loadRuntimeState(deadRuntime.teamRunId, config)).status)
        .toBe("orphaned");
      expect((await loadRuntimeState(stuckRuntime.teamRunId, config)).status)
        .toBe("failed");
      expect(
        await exists(
          getRuntimeStateDir(resolveBaseDir(config), deletingRuntime.teamRunId),
        ),
      ).toBe(false);
    },
    30000,
  ...
  test(
    "C-10.5 end-to-end: createTeamRun persists category-aware routing and team_send_message reapplies it on promptAsync",
    async () => {
      ...
      const leadMember = runtime.members.find((member) =>
        member.name === "lead"
      );
      const workerMember = runtime.members.find((member) =>
        member.name === "worker"
      );
      ...
      const toolContext = {
        ...
        ReturnType<typeof createTeamSendMessageTool>["execute"]
      >[1];
      ...
      const persistedRuntime = await loadRuntimeState(
        runtime.teamRunId,
        config,
      );
      const persistedWorker = persistedRuntime.members.find((member) =>
        member.name === "worker"
      );
      ...
      expect(recorded[0]?.model).toEqual({
        providerID: "openai",
        modelID: "gpt-5.6-luna-fast",
      });
  ...
  test(
    "C-10.4 keeps member spawn concurrency within max_parallel_members",
    async () => {
      // given
      const baseDir = await createBaseDir();
      const launchLimit = 2;
      ...
      const run = createTeamRun(
        ...
        "ses_lead",
        createContext(baseDir, manager, new Set(["ses_lead"])),
        createConfig(baseDir, { max_parallel_members: launchLimit }),
        manager,
      );
      try {
