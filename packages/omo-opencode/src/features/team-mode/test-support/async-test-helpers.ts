...
export function createLaunchConcurrencyProbe(
  options: LaunchConcurrencyProbeOptions,
): LaunchConcurrencyProbe {
  ...
  const snapshot = (): LaunchConcurrencySnapshot => ({
    launchCount,
    inFlight,
    maxInFlight,
  });
  ...
  return {
    async launch(input: LaunchInput): Promise<BackgroundTask> {
      ...
      if (launchId === options.launchLimit) {
