...
async function seedRuntimeState(
  ...
  const updatedRuntimeState = {
    ...runtimeState,
    members: runtimeState.members.map((member, index) =>
      index === 0
        ? { ...member, sessionId: leadSessionId, status: "running" as const }
        : {
          ...member,
          sessionId: memberSessionIds[index - 1],
...
describe("aggregateStatus", () => {
  ...
  test("surfaces stale locks from claims directory", async () => {
    ...
    const config = createConfig(baseDir);
    const teamRunId = await seedRuntimeState(
      baseDir,
      "team-gamma",
      "lead-3",
      [],
    ...
    const claimsDir = path.join(
      getTasksDir(resolveBaseDir(config), teamRunId),
      "claims",
    );
    await mkdir(claimsDir, { recursive: true });
    const claimedTask = await createTask(teamRunId, createTaskInput(), config);
    await writeFile(
      path.join(claimsDir, `${claimedTask.id}.lock`),
      "owner\n999999\n1\n",
    );
  ...
  test("aggregates members plus tasks plus unread counts", async () => {
    ...
    temporaryDirectories.push(baseDir);
    const config = createConfig(baseDir);
    const teamRunId = await seedRuntimeState(baseDir, "team-alpha", "lead-1", [
      "session-a",
      "session-b",
    ]);
    ...
    expect(Object.keys(result.members[0] ?? {})).toEqual(
      expect.arrayContaining(["name", "unreadMessages"]),
    );
