...
export type DeleteTeamBackgroundManager = Pick<
  BackgroundManager,
  "getTasksByParentSession" | "cancelTask"
>;
...
const FORCE_COMPLETABLE_MEMBER_STATUSES = new Set<
  RuntimeState["members"][number]["status"]
>([
  "pending",
  "running",
  "idle",
...
const FORCE_BYPASS_DELETING_STATUSES = new Set<RuntimeState["status"]>([
  "creating",
  "orphaned",
]);
...
function getTeamBackgroundTasks(
  ...
  return bgMgr.getTasksByParentSession(runtimeState.leadSessionId)
    .filter((task) =>
      task.teamRunId === runtimeState.teamRunId ||
      task.parentMessageId?.startsWith(teamMessageMarkerPrefix)
    );
}
...
export async function deleteTeam(
  ...
  const nonLeadMembers = runtimeState.members.filter((member) =>
    member.agentType !== "leader"
  );

  if (
    options?.force !== true &&
    nonLeadMembers.some((member) =>
      !DELETABLE_MEMBER_STATUSES.has(member.status)
    )
  ) {
  ...
  if (bgMgr) {
    ...
    if (options?.force !== true) {
      return await deleteTeamResources(
        teamRunId,
        config,
        runtimeState,
        tmuxMgr,
    ...
    await Promise.all(teamTasks.map((task) =>
      bgMgr.cancelTask(task.id, {
...
async function deleteTeamResources(
  ...
  if (options?.force === true) {
    await transitionRuntimeState(teamRunId, (currentRuntimeState) => ({
      ...
      members: currentRuntimeState.members.map((member) => (
        member.agentType === "leader" ||
          !FORCE_COMPLETABLE_MEMBER_STATUSES.has(member.status)
          ? member
  ...
  if (runtimeState.status !== "deleting" && runtimeState.status !== "deleted") {
    if (
      options?.force === true &&
      FORCE_BYPASS_DELETING_STATUSES.has(runtimeState.status)
    ) {
  ...
  const removedWorktrees = await removeWorktrees(
    runtimeState.members.map((member) => member.worktreePath),
  );
  ...
  await removeWorktrees([
    getRuntimeStateDir(resolveBaseDir(config), teamRunId),
  ]);
  ...
  sweepStaleTeamSessions(new Set(activeTeams.map((team) => team.teamRunId)))
    .catch(ignoreStaleTeamSessionSweepFailure);
