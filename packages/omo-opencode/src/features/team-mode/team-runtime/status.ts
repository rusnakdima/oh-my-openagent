...
function getPrimaryModelKey(
  bgMgr: TeamBackgroundManager | undefined,
...
function resolveConcurrencyCounts(
  bgMgr: TeamBackgroundManager | undefined,
  leadSessionId: string | undefined,
): ConcurrencyCounts {
  ...
  const running =
    tasksByParent.filter((task) => task.status === "running").length;
  const queued =
    tasksByParent.filter((task) => task.status === "pending").length;
...
export async function aggregateStatus(
  ...
  const unreadCounts = await Promise.all(
    runtimeState.members.map(async (member) => ({
      member,
      unreadMessages:
        (await listUnreadMessages(teamRunId, member.name, config)).length,
    })),
  );
  const tasks = await listTasks(teamRunId, config);
  const teamBackgroundManager: TeamBackgroundManager | undefined = bgMgr;
  const concurrencyCounts = resolveConcurrencyCounts(
    teamBackgroundManager,
    runtimeState.leadSessionId,
  );
  const teamRunIdSpecific = teamBackgroundManager?.listTasksByParentSession?.(
    runtimeState.leadSessionId ?? teamRunId,
  )?.length;
  const baseDir = resolveBaseDir(config);
  const claimsDir = path.join(getTasksDir(baseDir, teamRunId), "claims");
  const staleLockEntries = await readdir(claimsDir, { withFileTypes: true })
    .catch((error: unknown) => {
  ...
  const staleLockPaths = await Promise.all(
    staleLockEntries
      ...
      .map(async (entry) => {
        ...
        return (await detectStaleLock(lockPath, 300_000))
          ? lockPath
          : undefined;
      }),
  );
  ...
  return {
    ...
    bounds: runtimeState.bounds,
    staleLocks: staleLockPaths.filter((lockPath): lockPath is string =>
      lockPath !== undefined
    ),
  };
}
