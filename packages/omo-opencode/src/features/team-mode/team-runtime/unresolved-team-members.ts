...
export function hasUnresolvedTeamMembers(
  members: readonly RuntimeStateMember[],
): boolean {
  return members.some((member) => member.sessionId === undefined);
}

export function assertNoUnresolvedTeamMembers(
  members: readonly RuntimeStateMember[],
): void {
  ...
  const summary = unresolved
    ...
    .join(", ");
  throw new Error(
    `team runtime cannot transition to active with unresolved member(s): ${summary}`,
  );
}
