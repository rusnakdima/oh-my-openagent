...
const { approveShutdown, deleteTeam, rejectShutdown, requestShutdownOfMember } =
...
describe("team-runtime shutdown", () => {
  ...
  test("refuses team deletion while non-lead members are still active", async () => {
    ...
    expect(
      runtimeState.members.filter((member) => member.agentType !== "leader")
  ...
  test("approves shutdown requests and notifies the lead", async () => {
    ...
    const leadInboxMessages = await readInboxMessages(
      fixture.teamRunId,
      "lead",
      fixture.config,
    );
    const approvedRequest = runtimeState.shutdownRequests.find((
      shutdownRequest,
    ) => shutdownRequest.memberId === "member-a");
    expect(approvedRequest?.approvedAt).toEqual(expect.any(Number));
    expect(
      runtimeState.members.find((member) => member.name === "member-a")?.status,
    ).toBe("shutdown_approved");
  ...
  test("rejects shutdown requests and replies to the original requester", async () => {
    ...
    const rejectedRequest = runtimeState.shutdownRequests.find((
      shutdownRequest,
    ) => shutdownRequest.memberId === "member-a");
  ...
  test("rejects shutdown requests from a lead caller as the lead member", async () => {
    ...
    // then
    const requesterInboxMessages = await readInboxMessages(
      fixture.teamRunId,
      "member-b",
      fixture.config,
    );
  ...
  test("deletes team runtime resources after all non-lead members are approved", async () => {
    ...
    expect(result.removedWorktrees.sort()).toEqual(
      [...fixture.worktreePaths].sort(),
    );
    await Promise.all(fixture.worktreePaths.map(async (worktreePath) => {
    ...
    const runtimeStateDirectory = getRuntimeStateDir(
  ...
  test("force deletes a team stuck in 'creating' status", async () => {
    ...
    const originalTransitionRuntimeState =
      runtimeStateStore.transitionRuntimeState;
    spyOn(runtimeStateStore, "transitionRuntimeState").mockImplementation(
      async (teamRunId, transition, config) => {
        const currentRuntimeState = await runtimeStateStore.loadRuntimeState(
          teamRunId,
          config,
        );
        transitionedStatuses.push(transition(currentRuntimeState).status);
        return await originalTransitionRuntimeState(
          teamRunId,
          transition,
          config,
        );
    ...
    // when
    await deleteTeam(fixture.teamRunId, fixture.config, undefined, undefined, {
      force: true,
    });
  ...
  test("force removes lead member worktree if present", async () => {
    ...
    const fixture = await createFixture();
    temporaryDirectories.push(fixture.baseDir);
    const leadWorktreePath = path.join(
      fixture.baseDir,
      "fixture-worktrees",
      "lead",
    );
    ...
    // then
    expect(result.removedWorktrees.sort()).toEqual(
      [leadWorktreePath, ...fixture.worktreePaths].sort(),
    );
  ...
  test("cancels team background tasks before deleting when force=true", async () => {
    ...
    const runtimeStatusesDuringCancellation: Array<
      { teamStatus: string; memberStatuses: string[] }
    > = [];
    ...
    const bgMgr = {
      getTasksByParentSession: () => [
        ...
        },
        {
          id: "team-task-b",
          sessionId: "session-b",
          parentMessageId: `team-create:${fixture.teamRunId}:member-b`,
        },
      ],
