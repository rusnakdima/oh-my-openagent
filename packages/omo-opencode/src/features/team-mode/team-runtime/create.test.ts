...
function createSpec(memberCount: number, withWorktrees = false): TeamSpec {
  return {
    ...
    members: Array.from({ length: memberCount }, (_, index) => ({
      ...
      color: `color-${index + 1}`,
      ...(withWorktrees
        ? { worktreePath: `./worktrees/member-${index + 1}` }
        : {}),
    })),
  };
}
...
function createContext(
  ...
  client: { session: { create: ReturnType<typeof mock> } };
} {
  return {
    client: {
      session: { create: mock(async () => ({ data: { id: "forbidden" } })) },
    } as ExecutorContext["client"] & {
      session: { create: ReturnType<typeof mock> };
    },
    manager,
    directory: baseDir,
  };
...
function createManager(
  ...
  manager: BackgroundManager;
  launchMock: ReturnType<typeof mock>;
  cancelTaskMock: ReturnType<typeof mock>;
} {
...
describe("createTeamRun", () => {
  ...
  test("spawns 3 members through BackgroundManager.launch without direct session creation", async () => {
    ...
    let launchCount = 0;
    const { manager, launchMock } = createManager(
      baseDir,
      async () => ({
        id: `task-${++launchCount}`,
    ...
    expect(runtimeState.members.map((member) => member.sessionId)).toEqual([
      "session-1",
      "session-2",
      "session-3",
    ]);
    expect(
      (launchMock.mock.calls as Array<[LaunchInput]>).every(([input]) =>
  ...
  test("#given a new team runtime #when createTeamRun succeeds #then it registers the run for session cleanup", async () => {
    ...
    const baseDir = await mkdtemp(
      path.join(tmpdir(), "team-runtime-session-cleanup-"),
    );
    temporaryDirectories.push(baseDir);
  ...
  test("registers a member session as soon as launch reports the real sessionId", async () => {
    // given
    const baseDir = await mkdtemp(
      path.join(tmpdir(), "team-runtime-session-lineage-"),
    );
    temporaryDirectories.push(baseDir);
  ...
  test("persists the resolved subagent_type and model on each spawned runtime member", async () => {
    // given
    const baseDir = await mkdtemp(
      path.join(tmpdir(), "team-runtime-subagent-type-"),
    );
    temporaryDirectories.push(baseDir);
    let launchCount = 0;
    const { manager } = createManager(
    ...
    expect(runtimeState.members.map((member) => ({
      ...
      {
        name: "member-1",
        subagent_type: "member-1-agent",
        model: { providerID: "openai", modelID: "gpt-5.6-luna-fast" },
      },
      {
        name: "member-2",
        subagent_type: "member-2-agent",
        model: { providerID: "openai", modelID: "gpt-5.6-luna-fast" },
      },
      {
        name: "member-3",
        subagent_type: "member-3-agent",
        model: { providerID: "openai", modelID: "gpt-5.6-luna-fast" },
      },
    ]);
  });
  ...
  test("member prompt only documents member-safe communication tools", async () => {
    // given
    const baseDir = await mkdtemp(
      path.join(tmpdir(), "team-runtime-member-prompt-"),
    );
    temporaryDirectories.push(baseDir);
    ...
    const firstPrompt =
      (launchMock.mock.calls as Array<[LaunchInput]>)[0]?.[0].prompt ?? "";
  ...
  test("rolls back launched members in reverse order when a later spawn fails", async () => {
    // given
    const baseDir = await mkdtemp(
      path.join(tmpdir(), "team-runtime-rollback-"),
    );
    temporaryDirectories.push(baseDir);
    let launchCount = 0;
    ...
    expect(
      (cancelTaskMock.mock.calls as Array<[string]>).map(([taskId]) => taskId),
  ...
  test("#given member launch throws a non-Error value #when createTeamRun rolls back #then it preserves the fallback error message", async () => {
    // given
    const baseDir = await mkdtemp(
      path.join(tmpdir(), "team-runtime-non-error-"),
    );
    temporaryDirectories.push(baseDir);
    const { manager } = createManager(
      baseDir,
      async () => Promise.reject("launch failed as string"),
    );
    ...
    try {
      ...
      expect(error).toHaveProperty(
        "message",
        "Failed to create team run 'alpha-team': launch failed as string",
      );
    }
  });
  ...
  test("removes all created worktrees when spawn fails after worktree creation", async () => {
    // given
    const baseDir = await mkdtemp(
      path.join(tmpdir(), "team-runtime-worktree-"),
    );
    temporaryDirectories.push(baseDir);
    let launchCount = 0;
    ...
    // then
    expect(await pathExists(path.resolve(baseDir, "./worktrees/member-1")))
      .toBe(false);
    expect(await pathExists(path.resolve(baseDir, "./worktrees/member-2")))
      .toBe(false);
  });
  ...
  test("returns the existing runtime on repeated calls with the same spec and lead session", async () => {
    // given
    const baseDir = await mkdtemp(
      path.join(tmpdir(), "team-runtime-idempotent-"),
    );
    temporaryDirectories.push(baseDir);
    let launchCount = 0;
    const { manager, launchMock } = createManager(
    ...
    // when
    const firstRuntime = await createTeamRun(
      spec,
      "lead-session",
      context,
      createConfig(baseDir),
    ...
    const secondRuntime = await createTeamRun(
      spec,
      "lead-session",
      context,
      createConfig(baseDir),
  ...
  test("#given an existing active runtime with unresolved members #when createTeamRun runs again #then it creates a fresh runtime", async () => {
    // given
    const baseDir = await mkdtemp(
      path.join(tmpdir(), "team-runtime-unresolved-existing-"),
    );
    temporaryDirectories.push(baseDir);
    let launchCount = 0;
    const { manager, launchMock } = createManager(
  ...
  test("never exceeds max_parallel_members while spawning", async () => {
    // given
    const baseDir = await mkdtemp(
      path.join(tmpdir(), "team-runtime-parallel-"),
    );
    temporaryDirectories.push(baseDir);
    const launchLimit = 4;
    ...
    const { manager } = createManager(
      baseDir,
      async (input) => launchProbe.launch(input),
    );
    ...
    const run = createTeamRun(
      ...
      "lead-session",
      createContext(baseDir, manager),
      createConfig(baseDir, launchLimit),
      manager,
    );
    try {
  ...
  test("reuses the caller session for the lead when the lead matches the caller agent", async () => {
    // given
    const baseDir = await mkdtemp(
      path.join(tmpdir(), "team-runtime-caller-lead-"),
    );
    temporaryDirectories.push(baseDir);
    let launchCount = 0;
    ...
    expect(launchMock.mock.calls[0]?.[0]).toMatchObject({
      description: "Create team member alpha-team/member-1",
    });
    ...
    expect(
      runtimeState.members.map((member) => ({
        name: member.name,
  ...
  test("persists the reused caller lead's subagent_type so live deliveries can pin it", async () => {
    // given
    const baseDir = await mkdtemp(
      path.join(tmpdir(), "team-runtime-caller-lead-pin-"),
    );
    temporaryDirectories.push(baseDir);
    ...
    // then
    const leadMember = runtimeState.members.find((member) =>
      member.name === "lead"
    );
  ...
  test("reuses the caller session for the lead even when the lead subagent_type differs", async () => {
    // given
    const baseDir = await mkdtemp(
      path.join(tmpdir(), "team-runtime-explicit-lead-"),
    );
    temporaryDirectories.push(baseDir);
    let launchCount = 0;
    const { manager, launchMock } = createManager(baseDir, async (input) => ({
