...
function getLayoutCleanupTarget(
  ...
  if (
    ...
    runtimeState.tmuxLayout.paneIds.length > 0
  ) {
    return runtimeState.tmuxLayout;
  }

  const paneIds = runtimeState.members.flatMap((member) => {
    const ids = [member.tmuxPaneId, member.tmuxGridPaneId].filter((
      paneId,
    ): paneId is string => Boolean(paneId));
...
export async function cleanupTeamRunResources(args: {
  ...
  for (const resource of [...args.resources].reverse()) {
    if (resource.taskId) {
      try {
        ...
        cleanupReport.errors.push(
          `cancel ${resource.taskId}: ${normalizeError(cancelError).message}`,
        );
      }
    }
    ...
    if (resource.worktreePath) {
      try {
        ...
        cleanupReport.errors.push(
          `worktree ${resource.worktreePath}: ${
  ...
  if (args.createdLayout && args.tmuxMgr) {
    try {
      ...
      cleanupReport.errors.push(
        `layout ${args.teamRunId}: ${normalizeError(layoutError).message}`,
      );
    }
  }
  ...
  await transitionRuntimeState(
    ...
    cleanupReport.errors.push(
      `state ${args.teamRunId}: ${normalizeError(transitionError).message}`,
    );
    return undefined;
  });
