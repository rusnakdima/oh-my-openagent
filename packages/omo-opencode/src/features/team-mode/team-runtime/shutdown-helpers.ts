...
export const DELETABLE_MEMBER_STATUSES = new Set<
...
export function getRuntimeMember(
...
export function getLeadMemberName(runtimeState: RuntimeState): string {
  ...
  if (!leadMember) {
    throw new Error(
      `team '${runtimeState.teamRunId}' is missing a lead member`,
    );
  }
...
export function findLatestShutdownRequestIndex(
  ...
  for (
    ...
    if (
      ...
      shutdownRequest.requesterName !== requesterName
    ) continue;
    return index;
  }
...
export async function removeWorktrees(
  memberPaths: Array<string | undefined>,
): Promise<string[]> {
