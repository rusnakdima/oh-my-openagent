...
type BackgroundTaskInput =
  & Pick<BackgroundTask, "id" | "sessionId" | "teamRunId">
  & {
...
describe("deleteTeam cancels only this team's background tasks", () => {
  ...
  test("uses leadSessionId as the getTasksByParentSession key when force deleting", async () => {
    ...
    const cancelTaskMock = mock(async (
      _taskId: string,
      _options?: Parameters<DeleteTeamBackgroundManager["cancelTask"]>[1],
    ) => true);
    const bgMgr = {
  ...
  test("leaves unrelated sibling tasks on the same lead session alive when force deleting", async () => {
    ...
    const getTasksByParentSessionMock = mock(() => [
      ...
      createBackgroundTask({
        id: "delegate-task-x",
        sessionId: "session-x",
        parentMessageId: "delegate-task:plan-refactor",
      }),
      createBackgroundTask({
        id: "background-task-y",
        sessionId: "session-y",
        parentMessageId: "delegate-task:background-task-y",
      }),
      createBackgroundTask({
        id: "team-task-other",
        sessionId: "session-other",
        parentMessageId: "team-create:other-team-id:member-a",
      }),
    ]);
    const cancelTaskMock = mock(async (
      _taskId: string,
