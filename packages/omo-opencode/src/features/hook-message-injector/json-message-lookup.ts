...
function parseStoredMessageWithTime(
  value: unknown,
): StoredMessageWithTime | null {
  if (!isRecord(value)) {
    return null;
  }
...
function readMessageEntry(
  ...
  try {
    ...
    return {
      ...
      createdAt: typeof msg.time?.created === "number"
        ? msg.time.created
        : missingCreatedAt,
    };
  } catch (error) {
...
function isUsableMessageEntry(entry: MessageEntry): boolean {
  return !entry.hasCompactionMarker &&
...
export function findNearestMessageWithFields(
  ...
  try {
    ...
    for (const entry of messages) {
      ...
      if (
        entry.msg.agent && entry.msg.model?.providerID &&
