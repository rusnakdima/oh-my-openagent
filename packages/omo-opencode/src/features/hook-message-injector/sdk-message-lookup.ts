...
export async function fetchSDKMessages(
  ...
  try {
    ...
    const normalized = normalizeSDKResponse(response, emptyMessages, {
...
export function findNearestMessageWithFieldsFromMessages(
  ...
  for (const message of sortedMessages) {
    const stored = message.stored;
    if (
      stored?.agent || (stored?.model?.providerID && stored?.model?.modelID)
    ) {
      return stored;
    }
  }
...
export function findFirstMessageWithAgentFromMessages(
  messages: readonly SDKMessage[],
): string | null {
