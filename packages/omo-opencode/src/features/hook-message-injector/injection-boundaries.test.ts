...
} from "../../testing/module-mock-lifecycle";
...
const { getCompactionPartStorageDir } = await import(
  "../../shared/compaction-marker"
);
...
function createMockClient(messages: readonly MockSDKMessage[]): {
  readonly session: {
    readonly messages: () => Promise<
      { readonly data: readonly MockSDKMessage[] }
    >;
  };
} {
  return {
...
function listJsonFiles(directory: string): string[] {
  return readdirSync(directory).filter((fileName: string) =>
...
describe("hook message injection boundaries", () => {
  ...
  it("uses nearest message fallback and existing nested session directory when original context is partial", () => {
    ...
    writeFileSync(
      join(nestedMessageDir, "msg_existing.json"),
      JSON.stringify({
        id: "msg_existing",
        agent: "sisyphus",
        model: {
    ...
    const injectedFile = messageFiles.find((fileName) =>
      fileName !== "msg_existing.json"
    );
  ...
  it("#given original context and older fallback data #when injecting through the facade #then writes the original context metadata", () => {
    ...
    writeFileSync(
      join(messageDir, "msg_previous.json"),
      JSON.stringify({
        id: "msg_previous",
    ...
    // then
    expect(result).toBe(true);
    const injectedFile = listJsonFiles(messageDir).find((fileName) =>
      fileName !== "msg_previous.json"
    );
    expect(injectedFile).toBeDefined();
    const message = readJsonFile<{
      readonly agent: string;
  ...
  it("falls back to direct message directory when project directory listing is unreadable", () => {
    ...
    const unreadableProjectDir = join(
      TEST_MESSAGE_STORAGE,
      "project-without-read",
    );
    ...
    try {
      // when
      const result = injectHookMessage(
        "ses_unreadable_project",
        "test content",
        {
          agent: "atlas",
          model: { providerID: "openai", modelID: "gpt-5" },
      ...
      // then
      expect(result).toBe(true);
      expect(existsSync(join(TEST_MESSAGE_STORAGE, "ses_unreadable_project")))
        .toBe(true);
    } finally {
  ...
  it("does not leave message metadata when part write fails", () => {
    ...
    try {
      ...
      expect(listJsonFiles(join(TEST_MESSAGE_STORAGE, "ses_part_failure")))
        .toHaveLength(0);
    } finally {
...
describe("hook message context resolution boundaries", () => {
  it("uses JSON ordering and skips compaction marker messages for first-agent lookup", () => {
    ...
    const compactionMessageID =
      "msg_test_injector_first_agent_compaction_marker";
  ...
  it("uses SDK lookups for SQLite backend", async () => {
    ...
    // when
    const result = await resolveMessageContext(
      "ses_sqlite",
  ...
  it("uses JSON lookups for stable backend", async () => {
    ...
    const messageDir = createMessageDir();
    writeFileSync(
      join(messageDir, "msg_early.json"),
      JSON.stringify({
        agent: "atlas",
        time: { created: 10 },
      }),
    ...
    const result = await resolveMessageContext(
      "ses_json",
      unsafeTestValue(createMockClient([])),
      messageDir,
    );
