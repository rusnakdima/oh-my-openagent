...
import { adaptTuiPromptRef, currentTuiSessionID } from "./tui-session-bridge";
...
export async function openBtwPicker(args: {
  ...
  async function refreshStaleSelection(sessionID: string): Promise<void> {
    args.api.ui.toast({
      variant: "warning",
      message:
        `BTW session ${sessionID} no longer exists. Refreshing the list.`,
    });
  ...
  async function select(value: string): Promise<void> {
    ...
    if (selection.type === "new") {
      const currentSessionID = currentTuiSessionID(args.api);
      const promptRef = currentSessionID === selection.parentSessionID
