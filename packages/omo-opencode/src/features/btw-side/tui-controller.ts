...
import { abortBtwSide, deleteBtwSide } from "./tui-side-removal";
...
export function createBtwSideController(
  ...
  function attachPromptRef(
    sessionID: string,
    promptRef: BtwPromptRef | undefined,
  ): void {
    promptQueue.attach(sessionID, promptRef);
  }

  function latestSideForParent(
    parentSessionID: string,
  ): BtwSideRecord | undefined {
  ...
  async function startFromPrompt(
    ...
    try {
      const sideSession = await dependencies.createSession(
  ...
  function handleSessionDeleted(sessionID: string): void {
    ...
    if (detachedSides.length > 0) {
      const closingSideID = currentState.phase === "closing" &&
          sessionID === currentState.parentSessionID
