...
import { type BtwSideMetadata, getBtwSideMetadata } from "./metadata";
...
function prependBoundaryPart(message: MessageWithParts): void {
  if (
    message.parts.some(
      (part) =>
        part.type === "text" && part.text.includes(BTW_BOUNDARY_SENTINEL),
    )
  ) {
    return;
  }
...
export function createBtwSideContextInjectorHook(args: {
  ...
  async function resolveMetadata(
    sessionID: string,
  ): Promise<BtwSideMetadata | undefined> {
  ...
  async function loadParentContext(
    ...
    try {
      ...
      const parentMessages = normalizeSDKResponse<MessageWithParts[]>(
        response,
        [],
      );
