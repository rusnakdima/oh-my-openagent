...
export function parseBtwSideMetadata(
  value: unknown,
): BtwSideMetadata | undefined {
  if (!isRecord(value)) return undefined;
  if (value.version !== BTW_SIDE_METADATA_VERSION) return undefined;
  if (
    typeof value.parent_session_id !== "string" ||
    value.parent_session_id.length === 0
  ) {
    return undefined;
  }
  if (
    typeof value.boundary_message_id !== "string" ||
    value.boundary_message_id.length === 0
  ) {
    return undefined;
  }
  return {
...
export function getBtwSideMetadata(
  session: {
