...
export async function registerBtwSideTui<Node>(
  ...
  const adoptionGuard = createBtwAdoptionGuard(() => currentTuiSessionID(api));
  ...
  const unregisterSlashCommand = api.command?.register(() => [
    {
    ...
    slots: {
          ...
          ref: (promptRef) => {
            ...
            if (promptRef) {
              ...
              void (async () => {
                ...
                const isRelated = state.phase === "creating"
      ...
      session_prompt_right: (
        ...
        if (
          ...
          const sideLabel = sideNumber === undefined
            ? "BTW side"
            : `BTW #${sideNumber}`;
          label = `${sideLabel} · ${
            parentTuiStatusLabel(api, state.parentSessionID)
          } · esc esc return · ctrl+/ picker · ctrl+c delete`;
        } else if (
          state.phase === "closing" &&
          value.session_id === state.sideSessionID
        ) {
