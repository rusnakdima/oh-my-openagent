...
describe("TmuxPollingManager never-activated pane hard timeout (#4773, #5071)", () => {
  test("closes a never-focused pane with no session status once SESSION_TIMEOUT_MS elapses", async () => {
    ...
    sessions.set(
      "ses-unfocused-timed-out",
      createTrackedSession("ses-unfocused-timed-out", pastHardTimeout),
    );

    const closedSessionIds: string[] = [];
    const manager = new TmuxPollingManager(
      createStatuslessClient(),
      sessions,
      async (sessionId) => {
  ...
  test("keeps a young never-focused pane with no session status open", async () => {
    ...
    sessions.set(
      "ses-unfocused-young",
      createTrackedSession("ses-unfocused-young", thirtySecondsAgo),
    );
