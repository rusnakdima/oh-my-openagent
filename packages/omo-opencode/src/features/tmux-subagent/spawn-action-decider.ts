...
export function decideSpawnActions(
  ...
  if (agentAreaWidth < minAgentPaneWidth && currentCount > 0) {
    return {
      ...
      actions: [],
      reason:
        `window too small for agent panes: ${state.windowWidth}x${state.windowHeight}`,
    };
  }
  ...
  if (currentCount === 0) {
    ...
    if (
      canSplitPane(virtualMainPane, initialSplitDirection, minAgentPaneWidth)
    ) {
      return {
        canSpawn: true,
        actions: [
    ...
    return {
      canSpawn: false,
      actions: [],
      reason: "mainPane too small to split",
    };
  }
...
export function decideCloseAction(
  ...
  const paneExists = state.agentPanes.some((pane) =>
    pane.paneId === mapping.paneId
  );
