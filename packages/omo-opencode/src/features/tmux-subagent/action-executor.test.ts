...
const mockSpawnTmuxPane = mock(async (): Promise<SpawnPaneResult> => ({
  success: true,
  paneId: "%7",
}));
...
describe("executeAction", () => {
  beforeEach(() => {
    ...
    mockSpawnTmuxPane.mockImplementation(async () => ({
      success: true,
      paneId: "%7",
    }));
  });
  ...
  test("does not apply layout when spawn fails", async () => {
    // given
    mockSpawnTmuxPane.mockImplementation(async (): Promise<
      SpawnPaneResult
    > => ({ success: false }));
