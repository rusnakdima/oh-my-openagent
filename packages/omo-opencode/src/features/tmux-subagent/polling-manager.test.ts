...
describe("TmuxPollingManager overlap", () => {
  test("skips overlapping pollSessions executions", async () => {
    ...
    const manager = new TmuxPollingManager(
      unsafeTestValue<import("../../tools/delegate-task/types").OpencodeClient>(
        client,
      ),
      sessions,
      async () => {},
    ...
    //#when
    const firstPoll =
      (unsafeTestValue<{ pollSessions: () => Promise<void> }>(manager))
        .pollSessions();
  ...
  test("keeps active sessions when status API returns a raw status map without data wrapper", async () => {
    ...
    // then
    expect(closedSessionIds).toEqual([]);
    expect(sessions.get("ses-1")?.lastSeenAt.getTime()).toBeGreaterThanOrEqual(
      now,
    );
  });
  ...
  test("does not close immediately when first status is delayed after focused activation", async () => {
    ...
    try {
      ...
      const manager = new TmuxPollingManager(
        unsafeTestValue<
          import("../../tools/delegate-task/types").OpencodeClient
        >(client),
        sessions,
        async (sessionId) => {
          closedSessionIds.push(sessionId);
...
describe("TmuxPollingManager cmux eager-activated sessions", () => {
  test("skips activateSessionPane for sessions pre-marked as attachActivated=true", async () => {
    ...
    const activateSessionPaneMock = async (
      _tracked: TrackedSession,
    ): Promise<boolean> => {
    ...
    const client = {
      session: {
        status: async () => ({
          data: { "ses-cmux-eager": { type: "running" } },
        }),
        messages: async () => ({ data: [] }),
      },
    };
    ...
    await (unsafeTestValue<{ pollSessions: () => Promise<void> }>(manager))
      .pollSessions();
