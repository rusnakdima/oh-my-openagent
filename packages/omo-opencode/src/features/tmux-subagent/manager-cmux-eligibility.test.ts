...
describe("TmuxSessionManager cmux eligibility", () => {
  ...
  test("#given CMUX_SOCKET_PATH and TMUX_PANE without TMUX #when child session is created #then inline spawn reaches the source pane", async () => {
    // given
    const queryWindowState = mock(async (): Promise<WindowState> =>
      windowState
    );
    const executeActions = mock(
      async (actions: PaneAction[]): Promise<ExecuteActionsResult> => {
        const results = actions.map((
          action,
        ): { action: PaneAction; result: ActionResult } => ({
          action,
    ...
    const deps = {
      ...
      executeActions,
      executeAction: mock(async (): Promise<ActionResult> => ({
        success: true,
      })),
      log: mock(() => undefined),
    } satisfies Partial<TmuxUtilDeps>;
    const manager = new TmuxSessionManager(context, config, deps);
    unsafeTestValue<{ staleSweepCompleted: boolean }>(manager)
      .staleSweepCompleted = true;
  ...
  test("#given cmux fake TMUX #when isolation is window #then unsupported container spawn stays disabled", async () => {
    ...
    const queryWindowState = mock(async (): Promise<WindowState | null> =>
      null
    );
    const context = unsafeTestValue<PluginInput>({
      directory: "/tmp/omo-project",
