import type { TmuxPaneInfo, WindowState } from "./types";
...
export async function queryWindowStateWithDeps(
  sourcePaneId: string,
  deps: QueryWindowStateDeps,
  ...
  if (result.exitCode !== 0) {
    deps.log("[pane-state-querier] list-panes failed", {
...
export async function queryWindowState(
  ...
  return queryWindowStateWithDeps(sourcePaneId, {
    getTmuxPath,
    runTmuxCommand,
    log,
  });
