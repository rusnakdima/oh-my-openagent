...
function stubClientRetry(
  ...
  spyOn(
    unsafeTestValue<ManagerWithPrivateRetry>(manager),
    "getOrCreateClientWithRetry",
...
describe("SkillMcpManager post-request OAuth retry", () => {
  it("retries the operation after a 401 refresh succeeds", async () => {
    // given
    const refresh = mock(
      async () => ({ accessToken: "refreshed-token" } satisfies OAuthTokenData),
    );
  ...
  it("propagates the auth error without retry when refresh fails", async () => {
    ...
    // when / then
    await expect(
      manager.callTool(createInfo(), createContext(), "test-tool", {}),
