...
const OPENCODE_IMPORT_RE =
  /from\s+['"](@opencode-ai\/[^'"]+|opencode\/[^'"]+)['"]/;
...
describe("package opencode coupling grep gate", () => {
  test(
    "#given package source files #when audited #then no file imports from @opencode-ai/* or opencode/",
    async () => {
      // given
      ...
      for (const filePath of files) {
        ...
        for (let i = 0; i < lines.length; i++) {
          ...
          if (match) {
            offenders.push(
              `${relativeWorkspacePath(filePath)}:${i + 1}  imports ${
                match[1]
              }`,
            );
          }
  ...
  test(
    "#given package source files #when audited #then no file uses Bun.spawn, Bun.file, Bun.write, Bun.which, or Bun.hash",
    async () => {
      // given
      const files = await listPackageSourceFiles();
      const offenders: string[] = [];
      ...
      for (const filePath of files) {
        ...
        for (let i = 0; i < lines.length; i++) {
          ...
          if (match) {
            offenders.push(
              `${relativeWorkspacePath(filePath)}:${i + 1}  uses ${match[0]}`,
            );
          }
        }
      }
