...
const MAINTAINER_LOCAL_PATH_RE =
  /file:\/\/\/(?:Users|home)\/|(?:^|[\s(`'"])(?:\/Users|\/home)\//;

function collectMarkdownFiles(): string[] {
  const output = Bun.spawnSync(["git", "ls-files", "*.md"], {
    cwd: WORKSPACE_ROOT,
    stdout: "pipe",
  });
  expect(output.exitCode).toBe(0);
  return output.stdout.toString("utf-8").trim().split("\n").filter(Boolean).map(
    (filePath) => resolve(WORKSPACE_ROOT, filePath),
  );
}
...
function countPrecedingBackslashes(
  ...
  return line[index - count - 1] === "\\"
    ? countPrecedingBackslashes(line, index, count + 1)
    : count;
}
...
function findClosingBracket(
  ...
  const nextDepth = char === "[" && isUnescaped
    ? depth + 1
    : char === "]" && isUnescaped
    ? depth - 1
    : depth;
...
function readAngleBracketTarget(
  line: string,
  index: number,
  target = "",
): string | undefined {
...
function skipLeadingWhitespace(line: string, index: number): number {
  ...
  return char && /\s/.test(char)
    ? skipLeadingWhitespace(line, index + 1)
    : index;
}
...
function collectInlineTargets(
  ...
  const labelStart = line[index] === "!" && line[index + 1] === "["
    ? index + 1
    : line[index] === "["
    ? index
    : -1;
  ...
  const nestedTargets = collectInlineTargets(
    line.slice(labelStart + 1, labelEnd),
    lineNumber,
  );
  const destStart = line[labelEnd + 1] === "("
    ? skipLeadingWhitespace(line, labelEnd + 2)
    : -1;
  const target = destStart === -1
    ? undefined
    : line[destStart] === "<"
    ? readAngleBracketTarget(line, destStart + 1)
    : readMarkdownTarget(line, destStart);
  const currentTargets = target
    ? [...nestedTargets, { line: lineNumber, target }]
    : nestedTargets;
  return [
    ...currentTargets,
    ...collectInlineTargets(line, lineNumber, labelEnd + 1),
  ];
}
...
function collectLinkedTargets(
  ...
  return stripInlineCodeSpans(
    stripIndentedCodeBlocks(stripFencedCodeBlocks(markdown)),
    ...
    return referenceTarget
      ? [...targets, { line: lineIndex + 1, target: referenceTarget }]
      : targets;
  });
}
...
describe("markdown local link audit", () => {
  test("#given external markdown links #when resolving targets #then http and https links are ignored", () => {
    expect(
      resolveMarkdownTarget("docs/AGENTS.md", "http://example.com/readme.md"),
    ).toBeUndefined();
    expect(
      resolveMarkdownTarget("docs/AGENTS.md", "https://example.com/readme.md"),
    ).toBeUndefined();
  });

  test("#given relative file uri markdown link #when resolving target #then it resolves from the markdown file", () => {
    expect(
      resolveMarkdownTarget("docs/AGENTS.md", "file://./guide/overview.md"),
    ).toBe(resolve("docs/guide/overview.md"));
  });

  test("#given reference-style markdown links #when collecting targets #then link definitions are audited", () => {
    expect(
      collectLinkedTargets("[Guide][guide]\n\n[guide]: ./guide/overview.md"),
    ).toEqual([
  ...
  test("#given separated reference-style markdown links #when collecting targets #then each definition target is audited", () => {
    expect(
      collectLinkedTargets(
        "[Guide][guide] and [Docs][]\n\n[guide]: ./guide/overview.md\n[Docs]: docs/AGENTS.md",
      ),
    ).toEqual([
      { line: 3, target: "./guide/overview.md" },
  ...
  test("#given image-wrapped markdown link #when collecting targets #then wrapper target is audited", () => {
    expect(
      collectLinkedTargets(
        "[![License](https://img.shields.io/badge/license-SUL--1.0-white)](LICENSE.md)",
      ),
    ).toEqual([
      {
        line: 1,
        target: "https://img.shields.io/badge/license-SUL--1.0-white",
      },
  ...
  test("#given nested markdown links #when collecting targets #then each nested target is audited", () => {
    expect(collectLinkedTargets("[[Guide](./guide/overview.md)](README.md)"))
      .toEqual([
  ...
  test("#given inline code spans #when collecting targets #then links inside code spans are ignored", () => {
    expect(
      collectLinkedTargets(
        "Use `create_memory_object_stream[T](max_buffer_size=N)` then [Guide](./guide/overview.md)",
      ),
    ).toEqual([
      { line: 1, target: "./guide/overview.md" },
    ]);
  ...
  test("#given angle-bracket-wrapped destination with spaces #when collecting targets #then the spaced target is audited", () => {
    expect(collectLinkedTargets("[Release notes](<./docs/release notes.md>)"))
      .toEqual([
  ...
  test(
    "#given checked-in markdown #when local links are audited #then every local target exists",
    async () => {
      const offenders = (await Promise.all(
        collectMarkdownFiles().map(async (filePath) => {
          return collectLinkedTargets(await readFile(filePath, "utf-8"))
            .flatMap((linkedTarget) => {
              ...
              return targetPath && !existsSync(targetPath)
                ? [
                  ...
                  }:${linkedTarget.line} missing ${linkedTarget.target}`,
                ]
                : [];
            });
        }),
  ...
  test(
    "#given checked-in markdown #when maintainer-local paths are audited #then none are present",
    async () => {
      const offenders = (await Promise.all(
        collectMarkdownFiles().map(async (filePath) => {
          return collectMaintainerLocalPathLines(
            await readFile(filePath, "utf-8"),
          ).map((line) => `${relativeWorkspacePath(filePath)}:${line}`);
        }),
      )).flat();
      expect(offenders.sort()).toEqual([]);
    },
