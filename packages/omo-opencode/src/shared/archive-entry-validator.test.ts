...
describe("validateArchiveEntries", () => {
  it("rejects absolute paths and traversal entries", () => {
    ...
    const rejectTraversalPath = () =>
      validateArchiveEntries(
        [{ path: "nested/../../evil.txt", type: "file" }],
        destDir,
      );
  ...
  it("accepts contained files, directories, and symlinks", () => {
    ...
    //#when
    const validateContainedEntries = () =>
...
describe.skipIf(
  ...
  () => {
    ...
    it("extracts safe tar and zip archives into the destination directory", async () => {
      ...
      //#then
      expect(readFileSync(join(tarDestDir, "bin", "tool.txt"), "utf8")).toBe(
        "safe",
      );
      expect(readFileSync(join(zipDestDir, "bin", "tool.txt"), "utf8")).toBe(
        "safe",
      );
      expect(lstatSync(join(tarDestDir, "bin", "tool-link")).isSymbolicLink())
        .toBe(true);
      expect(lstatSync(join(zipDestDir, "bin", "tool-link")).isSymbolicLink())
        .toBe(true);
    });
  },
);
