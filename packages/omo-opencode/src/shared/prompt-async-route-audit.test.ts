...
const snippetDirectory = mkdtempSync(
  path.join(tmpdir(), "prompt-async-route-audit-"),
);
let snippetIndex = 0;
afterAll(async () => {
  await parser.close();
...
const PROMPT_GATE_FILE = path.join(
  SOURCE_ROOT,
  "shared",
  "prompt-async-gate.ts",
);
...
function getPropertyName(
  node: ts.PropertyName | ts.MemberName | ts.Expression,
): string | null {
...
function detectRawPromptInSourceFile(sourceFile: ts.SourceFile): boolean {
  ...
  const visit = (node: ts.Node): void => {
    ...
    const isRawPromptAccess = isRawPromptPropertyAccess(node) &&
      !isTypeofPromptCheck(node);
    if (
      isRawPromptAccess || isPromptBindingPattern(node) ||
      isReflectApplyPromptCall(node)
    ) {
      detected = true;
      return;
    }
...
function objectLiteralHasQueueBehavior(
...
function findPromptGateCallsWithoutQueueBehaviorInSourceFile(
  sourceFile: ts.SourceFile,
): number[] {
...
async function findPromptGateCallsWithoutQueueBehavior(
  _filePath: string,
  contents: string,
): Promise<number[]> {
  return findPromptGateCallsWithoutQueueBehaviorInSourceFile(
    await parseSnippet(contents),
  );
}

function findPromptRetryCallsWithoutQueueBehaviorInSourceFile(
  sourceFile: ts.SourceFile,
): number[] {
  const offenders: number[] = [];
  ...
  const visit = (node: ts.Node): void => {
    if (
      ...
      guardedNames.has(callExpressionName(node.expression) ?? "")
    ) {
      const optionsArgument = node.arguments[2];
      if (
        !optionsArgument || !ts.isObjectLiteralExpression(optionsArgument) ||
        !objectLiteralHasQueueBehavior(optionsArgument, sourceFile)
      ) {
        offenders.push(
          sourceFile.getLineAndCharacterOfPosition(node.getStart(sourceFile))
            .line + 1,
...
describe("production prompt injection routes", () => {
  ...
  test("#given optional-chain promptAsync call #when audit scans snippet #then it is flagged", async () => {
    // given
    const snippet =
      "await client.session?.promptAsync({ body: { text: 'hi' } })";
...
`;
    ...
    // when
    const offenders = await findPromptGateCallsWithoutQueueBehavior(
      "audit-snippet.ts",
      snippet,
    );
  ...
  test("#given production TypeScript sources #when prompt routes are audited #then only the shared gate may call raw OpenCode prompt APIs", async () => {
    // given
    const files = [
      ...await listSourceFiles(SOURCE_ROOT),
      ...await listPackageSourceFiles(),
    ];
    const candidates = await Promise.all(files.map(async (filePath) => {
      if (
        PROMPT_GATE_FILES.has(filePath) || RAW_PROMPT_ALLOWLIST.has(filePath)
      ) {
        return null;
      }
    ...
    const parsedSourceFiles = await parser.parse(
      candidates.filter((filePath): filePath is string => filePath !== null),
    );
  ...
  test("#given production TypeScript sources #when prompt gate callers are audited #then every route declares queue behavior explicitly", async () => {
    ...
    for (const filePath of candidates) {
      ...
      const missingLines = findPromptGateCallsWithoutQueueBehaviorInSourceFile(
        sourceFile,
      );
  ...
  test("#given production TypeScript sources #when model-suggestion prompt wrappers are audited #then every retry caller declares queue behavior explicitly", async () => {
    ...
    const candidates = await Promise.all(files.map(async (filePath) => {
      const contents = await readFile(filePath, "utf8");
      return contents.includes("promptWithModelSuggestionRetry") ||
    ...
    for (const filePath of candidates) {
      ...
      const missingLines = findPromptRetryCallsWithoutQueueBehaviorInSourceFile(
        sourceFile,
      );
      for (const line of missingLines) {
        offenders.push(`${relativeSourcePath(filePath)}:${line}`);
