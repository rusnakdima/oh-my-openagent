...
describe("parseFallbackModelEntry (extended)", () => {
  it("parses provider/model string", () => {
    const result = parseFallbackModelEntry(
...
describe("flattenToFallbackModelStrings", () => {
  ...
  it("passes through plain strings", () => {
    expect(flattenToFallbackModelStrings(["anthropic/claude-sonnet-4-6"]))
      .toEqual([
        "anthropic/claude-sonnet-4-6",
      ]);
  });
...
describe("findMostSpecificFallbackEntry", () => {
  it("picks exact match over prefix match", () => {
    ...
    const result = findMostSpecificFallbackEntry(
      "openai",
      "gpt-5.4-preview",
      chain,
    );
  ...
  it("returns undefined when no entry matches", () => {
    ...
    expect(findMostSpecificFallbackEntry("openai", "gpt-5.4", chain))
      .toBeUndefined();
  });
...
describe("parseFallbackModelEntry: non-string input (issue #4145)", () => {
  ...
  test("returns undefined for null and undefined", () => {
    ...
    expect(parseFallbackModelEntry(undefinedInput, "anthropic"))
      .toBeUndefined();
  });
