...
export function resolveFirstAvailableFallback(
  ...
  for (const entry of fallbackChain) {
    for (const provider of entry.providers) {
      const matchedModel = fuzzyMatchModel(entry.model, availableModels, [
        provider,
      ]);
    ...
    for (const entry of fallbackChain) {
      if (entry.providers.some((p) => connectedSet.has(p))) {
        log(
          ...
          {
            chain: fallbackChain.map((entryItem) => entryItem.model),
            availableCount: availableModels.size,
          },
        );
        return true;
  ...
  if (connectedProviders) {
    ...
    for (const provider of providers) {
      if (connectedSet.has(provider)) {
        log("[isAnyProviderConnected] provider connected via cache", {
          provider,
        });
        return true;
      }
