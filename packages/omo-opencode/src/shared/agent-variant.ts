...
export function resolveVariantForModel(
  ...
  const agentChain = AGENT_MODEL_REQUIREMENTS[stripped]?.fallbackChain as
    | ChainEntry[]
    | undefined;
  const categoryChain = categoryName
    ? CATEGORY_MODEL_REQUIREMENTS[categoryName]?.fallbackChain as
      | ChainEntry[]
      | undefined
    : undefined;
