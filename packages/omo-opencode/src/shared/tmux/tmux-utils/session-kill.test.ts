...
async function loadKillTmuxSessionIfExists(): Promise<
  typeof import("./session-kill").killTmuxSessionIfExists
> {
  const module = await import(
    `${sessionKillSpecifier}?test=${crypto.randomUUID()}`
  );
...
describe("killTmuxSessionIfExists", () => {
  ...
  it("#given omo-agents session does NOT exist #when killTmuxSessionIfExists called #then NO kill-session invocation and returns false", async () => {
    // given
    const killTmuxSessionIfExists = await loadKillTmuxSessionIfExists();
    runTmuxCommandMock.mockResolvedValueOnce({
      success: false,
      output: "",
      stdout: "",
      stderr: "",
    ...
    expect(runTmuxCommandMock.mock.calls).toEqual([["tmux", [
      "has-session",
      "-t",
      "omo-agents",
    ]]]);
