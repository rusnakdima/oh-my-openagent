const ATTACH_SERVER_URL_PATTERN =
  /\bopencode\s+attach\s+(?:"([^"]+)"|'([^']+)'|(\S+))/;
...
export type SweepAttachPaneDeps = {
  ...
  readonly listCandidatePanes: (
    tmux: string,
  ) => Promise<readonly TmuxAttachPane[]>;
...
async function buildRuntimeAttachPaneDeps(): Promise<SweepAttachPaneDeps> {
  ...
  return {
    ...
    isServerRunning: (serverUrl) =>
      serverHealth.isServerRunning(serverUrl, {
...
function isOmoAttachPane(pane: TmuxAttachPane): boolean {
  return pane.attachServerUrl.length > 0 ||
    OMO_ATTACH_PANE_TITLE_PREFIXES.some((prefix) =>
      pane.title.startsWith(prefix)
    );
}

export async function sweepStaleOmoAttachPanesWith(
  deps: SweepAttachPaneDeps,
): Promise<number> {
  ...
  for (const pane of candidatePanes) {
    ...
    const rawServerUrl = pane.attachServerUrl ||
      extractAttachServerUrl(pane.commandLine);
