...
async function loadCloseTmuxPane(): Promise<
  typeof import("./pane-close").closeTmuxPane
> {
  const module = await import(
    `${paneCloseSpecifier}?test=${crypto.randomUUID()}`
  );
...
describe("closeTmuxPane runner integration", () => {
  ...
  it("#given pane exists #when closeTmuxPane called #then delegates send-keys and kill-pane to shared runner", async () => {
    ...
    const paneCloseCalls = runTmuxCommandMock.mock.calls.filter((
      call: [string, string[]],
    ) => call[1].includes("%42"));
