...
const tmuxPathResolverSpecifier = import.meta.resolve(
  "../../../tools/interactive-bash/tmux-path-resolver",
);
...
async function loadSweepStaleOmoAgentSessions(): Promise<
  typeof import("./stale-session-sweep").sweepStaleOmoAgentSessions
> {
  const module = await import(
    `${staleSessionSweepSpecifier}?test=${crypto.randomUUID()}`
  );
...
function registerModuleMocks(): void {
  ...
  mock.module(
    sessionKillSpecifier,
    () => ({ killTmuxSessionIfExists: killTmuxSessionIfExistsMock }),
  );
  mock.module(
    tmuxPathResolverSpecifier,
