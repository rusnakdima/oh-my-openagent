  ...
  if (existsSync(targetPath)) {
    if (legacyStat.isDirectory() && lstatSync(targetPath).isDirectory()) {
      ...
      for (const entry of readdirSync(legacyPath)) {
        copiedChild = copyMissingEntries(
...
export function migrateLegacyWorkspaceDirectory(
  directory: string,
  ...
  try {
    const migrated = copyMissingEntries(
      legacyDirectory,
      targetDirectory,
      targetDirectory,
      skipped,
    ...
    log(
      "[legacy-workspace-migration] Failed to migrate legacy workspace directory",
      {
        legacyDirectory,
        targetDirectory,
        error,
