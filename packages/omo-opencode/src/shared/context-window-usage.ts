...
const usageCacheByClient = new WeakMap<
  object,
  Map<string, Map<string, Promise<ContextWindowUsage | null>>>
>();
...
function createModelCacheKey(
  ...
  return `${
    modelCacheState.anthropicContext1MEnabled ? "1m" : "200k"
  }|${cachedLimits}`;
}
...
export function invalidateContextWindowUsageCache(
  ctx: PluginInput,
  sessionID?: string,
): void {
...
export function trackLargestOutput(
  sessionID: string,
  outputBytes: number,
): void {
...
async function fetchContextWindowUsage(
  ...
  const fetchTimeoutMs = contextWindowUsageFetchTimeoutMsForTesting ??
  ...
  try {
    ...
    const messages = normalizeSDKResponse(response, [] as MessageWrapper[], {
    ...
    const actualLimit = lastAssistant.providerID !== undefined
      ? resolveActualContextLimit(
    ...
    log(
      "[dynamic-truncator] fetchContextWindowUsage failed; falling back to null",
      {
        sessionID,
