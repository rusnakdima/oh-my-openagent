...
function mergeConnectedProviders(
  ...
  const droppedPreviousProvider = previous.some((provider) =>
    !fetchedSet.has(provider)
  );
...
export function createConnectedProvidersCacheStore(
  ...
  async function updateConnectedProvidersCache(client: {
    ...
    try {
      ...
      const reportedProviderIDs = new Set(
        allProviders.map((provider) => provider.id),
      );
      const connected = mergeConnectedProviders(
        previousConnected ?? previousProviderModels?.connected,
        fetchedConnected,
        reportedProviderIDs,
      );
      ...
      for (const provider of allProviders) {
        if (provider.models) {
          ...
          const modelMetadata = Object.entries(provider.models).map(
            ([modelID, rawMetadata]) => {
      ...
      const mergedModelsByProvider = mergeProviderModels(
        previousProviderModels?.models,
        modelsByProvider,
        connected,
        reportedModelProviderIDs,
      );

      log("[connected-providers-cache] Extracted models from provider list", {
        providerCount: Object.keys(mergedModelsByProvider).length,
        totalModels: Object.values(mergedModelsByProvider).reduce(
...
export function findProviderModelMetadata(
  providerID: string,
  modelID: string,
  cache: ProviderModelsCache | null = defaultConnectedProvidersCacheStore
    .readProviderModelsCache(),
