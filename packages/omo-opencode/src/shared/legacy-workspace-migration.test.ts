...
describe("migrateLegacyWorkspaceDirectory", () => {
  ...
  beforeEach(() => {
    testDirectory = join(
      tmpdir(),
      `omo-workspace-migration-${Date.now()}-${
  ...
  test("#given legacy workspace with nested state and no target #when migrating #then copies the tree to .omo", () => {
    ...
    const legacyNotepadDirectory = join(
      testDirectory,
      ".sisyphus",
      "notepads",
      "work",
    ...
    expect(
      readFileSync(join(testDirectory, ".omo", "plans", "work.md"), "utf-8"),
    ).toBe("# Plan");
    expect(
      readFileSync(
        join(testDirectory, ".omo", "notepads", "work", "notes.md"),
        "utf-8",
      ),
    ).toBe("note");
    expect(existsSync(join(testDirectory, ".sisyphus", "plans", "work.md")))
      .toBe(true);
  });
  ...
  test("#given target has other files #when migrating #then copies only missing legacy files", () => {
    ...
    mkdirSync(join(testDirectory, ".sisyphus", "plans"), { recursive: true });
    mkdirSync(join(testDirectory, ".omo", "notepads", "work"), {
      recursive: true,
    });
  ...
  test("#given legacy workspace contains symlinks #when migrating #then skips symlinks without copying target contents", () => {
    ...
    expect(existsSync(join(testDirectory, ".omo", "plans", "linked.md"))).toBe(
      false,
    );
  });
