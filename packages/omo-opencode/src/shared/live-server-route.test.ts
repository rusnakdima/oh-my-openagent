...
describe("live-server-route", () => {
  ...
  describe("resolveDispatchClient — identity passthrough", () => {
    test("#given different client object #when resolveDispatchClient called #then returns in-process without probing", async () => {
      ...
      initLiveServerRoute({
        ...
        directory: "/tmp/test",
        inProcessClient: fakeInProcessClient,
      });

      const differentClient = { _marker: "different" } as unknown;

      //#when
      const result = await resolveDispatchClient(
        differentClient,
        "ses_identity",
      );
  ...
  describe("resolveDispatchClient — 200 probe → live + client cached", () => {
    test("#given probe returns 200 #when resolveDispatchClient called twice #then returns live route and caches same client object", async () => {
      ...
      //#when
      const result1 = await resolveDispatchClient(
        fakeInProcessClient,
        "ses_live1",
      );
      const result2 = await resolveDispatchClient(
        fakeInProcessClient,
        "ses_live2",
      );
  ...
  describe("resolveDispatchClient — timeout → in-process bounded <2s", () => {
    test(
      "#given probe never resolves #when resolveDispatchClient called #then returns in-process within 2500ms",
      async () => {
        //#given
  ...
  describe("resolveDispatchClient — multiple server() registrations (multi-instance serve)", () => {
    test("#given two registrations with different in-process clients #when resolveDispatchClient called with the first client #then it still routes live instead of identity passthrough", async () => {
      ...
      initLiveServerRoute({
        serverUrl: FAKE_SERVER_URL,
        directory: "/tmp/instance-one",
        inProcessClient: firstClient,
      });
      _setLiveClientForTesting(fakeLiveClient);
      initLiveServerRoute({
        serverUrl: FAKE_SERVER_URL,
        directory: "/tmp/instance-two",
        inProcessClient: secondClient,
      });

      //#when
      const first = await resolveDispatchClient(
        firstClient,
        "ses_instance_one",
      );
      const second = await resolveDispatchClient(
        secondClient,
        "ses_instance_two",
      );
  ...
  describe("tryResolveDispatchClientSync — synchronous fast path", () => {
    test("#given an unregistered client #when tryResolveDispatchClientSync called #then it returns identity passthrough without awaiting", () => {
      ...
      //#when
      const result = tryResolveDispatchClientSync(
        unregistered,
        "ses_sync_identity",
      );
  ...
  describe("markLiveRouteUnavailable", () => {
    test("#given available route #when markLiveRouteUnavailable called #then subsequent resolve returns in-process", async () => {
      ...
      const before = await resolveDispatchClient(
        fakeInProcessClient,
        "ses_before",
      );
      ...
      const { fetch: fakeFetch2, callCount: callCount2 } = makeFakeFetch([{
        ok: true,
        status: 200,
      }]);
      _setFetchImplementationForTesting(fakeFetch2);
      const after = await resolveDispatchClient(
        fakeInProcessClient,
        "ses_after",
      );
  ...
  describe("session affinity", () => {
    function makeUrlAwareFakeFetch(
      handler: (url: string) => FakeFetchResponse,
    ): {
      fetch: typeof fetch;
      urls: () => string[];
    } {
    ...
    test("#given healthy listener that does not own the session #when resolving dispatch client #then falls back to in-process", async () => {
      ...
      expect(urls().some((url) => url.includes("/session/ses_not_owned"))).toBe(
        true,
      );
    });
    ...
    test("#given listener that owns the session #when resolving twice #then routes live and caches the affinity probe", async () => {
      //#given
      const { fetch: fakeFetch, urls } = makeUrlAwareFakeFetch(() => ({
        ok: true,
        status: 200,
      }));
      _setFetchImplementationForTesting(fakeFetch);
      ...
      expect(first.route).toBe("live");
      expect(second.route).toBe("live");
      const affinityProbes = urls().filter((url) =>
        url.includes("/session/ses_owned")
      );
    ...
    test("#given fresh health probe but unknown session affinity #when sync resolution runs #then defers to the async path", async () => {
      ...
      const { fetch: fakeFetch } = makeUrlAwareFakeFetch(() => ({
        ok: true,
        status: 200,
      }));
      _setFetchImplementationForTesting(fakeFetch);
      ...
      const syncResult = tryResolveDispatchClientSync(
        fakeInProcessClient,
        "ses_unknown_affinity",
      );
