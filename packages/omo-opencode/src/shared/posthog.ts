...
let activityStateProviderOverride:
...
export function __setTransportFactoryForTesting(
  provider: TelemetryTransportFactory,
): void {
...
function createCoreCompatibleTelemetryEnv(
...
export function getPostHogDistinctId(): string {
  return getTelemetryDistinctId(
    createOpencodeTelemetryProductConfig().machineIdPrefix,
    resolveOsProvider(),
  );
}

export function createCliPostHog(
  options: CreatePostHogOptions = {},
): PostHogClient {
