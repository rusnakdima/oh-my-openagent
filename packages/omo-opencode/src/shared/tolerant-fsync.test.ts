...
describe("tolerantFsync (async)", () => {
  ...
  it("#given fsync throws EPERM #when called #then resolves without throwing", async () => {
    const handle = fakeHandleWithSyncError(
      makeFsError("EPERM", "operation not permitted, fsync"),
    );
    await expect(tolerantFsync(handle, "test:async-eperm")).resolves
      .toBeUndefined();
  });
  ...
  it("#given fsync throws EACCES #when called #then resolves without throwing", async () => {
    ...
    await expect(tolerantFsync(handle, "test:async-eacces")).resolves
      .toBeUndefined();
  });
  ...
  it("#given fsync throws ENOTSUP #when called #then resolves without throwing", async () => {
    ...
    await expect(tolerantFsync(handle, "test:async-enotsup")).resolves
      .toBeUndefined();
  });
  ...
  it("#given fsync throws EINVAL #when called #then resolves without throwing", async () => {
    ...
    await expect(tolerantFsync(handle, "test:async-einval")).resolves
      .toBeUndefined();
  });
  ...
  it("#given fsync throws EIO #when called #then propagates the error", async () => {
    ...
    await expect(tolerantFsync(handle, "test:async-eio")).rejects.toThrow(
      "EIO: simulated",
    );
  });
  ...
  it("#given fsync throws ENOSPC #when called #then propagates the error", async () => {
    ...
    await expect(tolerantFsync(handle, "test:async-enospc")).rejects.toThrow(
      "ENOSPC: simulated",
    );
  });
  ...
  it("#given fsync throws EPERM #when called #then tracker records one skip", async () => {
    ...
    await tolerantFsync(
      handle,
      "atomicWrite:/Users/x/Library/Mobile Documents/com~apple~CloudDocs/file.txt",
    );
  ...
  it("#given fsync throws EIO #when called #then tracker remains empty", async () => {
    ...
    await expect(tolerantFsync(handle, "atomicWrite:/tmp/file.txt")).rejects
      .toThrow("EIO: simulated");
...
describe("tolerantFsyncSync (synchronous)", () => {
  it("#given fsyncSync throws EPERM #when called #then returns without throwing", () => {
    ...
    expect(() => tolerantFsyncSync(123, "test:sync-eperm", fakeFsync)).not
      .toThrow();
  });
  ...
  it("#given fsyncSync throws EACCES #when called #then returns without throwing", () => {
    ...
    expect(() => tolerantFsyncSync(123, "test:sync-eacces", fakeFsync)).not
      .toThrow();
  });
  ...
  it("#given fsyncSync throws EIO #when called #then propagates the error", () => {
    ...
    expect(() => tolerantFsyncSync(123, "test:sync-eio", fakeFsync)).toThrow(
      "EIO: simulated",
    );
  });
