...
async function captureWithEnv(
  ...
  posthogModule.__setTransportFactoryForTesting(
    createCapturingTransportFactory(captured),
  );
...
describe("telemetry before/after parity", () => {
  it("pins event name, distinct ID prefix, state file path, env matrix, and payload keys", async () => {
    ...
    const xdgDataHome = join(
      tmpdir(),
      `telemetry-parity-${Date.now()}-${Math.random()}`,
    );
    ...
    posthogModule.__setOsProviderForTesting(
      createParityOsProvider("telemetry-parity-host"),
    );
    posthogModule.__setActivityStateProviderForTesting(() => ({
      dayUTC: "2026-06-28",
      captureDaily: true,
    }));
    const expectedStateFilePath = join(
      xdgDataHome,
      "oh-my-opencode",
      "posthog-activity.json",
    );

    // when
    const distinctId = posthogModule.getPostHogDistinctId();
    const stateFilePath = getTelemetryActivityStateFilePath(
      resolveTelemetryStateDir(product),
    );
    const client = posthogModule.createCliPostHog();
  ...
  it("preserves the legacy env opt-out matrix including yes as enabled", async () => {
    // given / when
    const disabledByPostHogFlag = await captureWithEnv({
      OMO_DISABLE_POSTHOG: "1",
    });
    const disabledBySendZero = await captureWithEnv({
      OMO_SEND_ANONYMOUS_TELEMETRY: "0",
    });
    const enabledBySendYes = await captureWithEnv({
      OMO_SEND_ANONYMOUS_TELEMETRY: "yes",
    });
    const disabledBySendNo = await captureWithEnv({
