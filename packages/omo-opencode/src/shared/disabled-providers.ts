...
export function filterDisabledProviderModels<
  T extends string | FallbackModelObject,
>(
  models: readonly T[],
  disabled: readonly string[],
): T[] {
...
function applyToHolder(
  ...
  if (normalizedChain) {
    const filteredChain = filterDisabledProviderModels(
      normalizedChain,
      disabled,
    );
    if (filteredChain.length !== normalizedChain.length) {
      log(
        `[${HOOK_NAME}] Filtered disabled-provider entries from fallback chain`,
        {
          label,
    ...
    holder.fallback_models = filteredChain.length === 0
      ? undefined
      : filteredChain;
  }

  if (
    typeof holder.model === "string" &&
    isProviderDisabled(holder.model, disabled)
  ) {
 ...
 */
export function applyDisabledProviders(
