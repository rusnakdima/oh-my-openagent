...
type ConnectedProvidersCacheModule =
  typeof import("./connected-providers-cache");

async function importFreshConnectedProvidersCacheModule(): Promise<
  ConnectedProvidersCacheModule
> {
  return await import(
    new URL(
      `./connected-providers-cache.ts?real-connected-providers-cache-test=${Date.now()}-${Math.random()}`,
      import.meta.url,
    ).href
  );
}
...
function createTestCacheContext(
  createConnectedProvidersCacheStore:
    ConnectedProvidersCacheModule["createConnectedProvidersCacheStore"],
) {
  const fakeUserCacheRoot = mkdtempSync(
    join(tmpdir(), "connected-providers-user-cache-"),
  );
...
describe("updateConnectedProvidersCache", () => {
  ...
  test("does not remove unrelated files in the cache directory", async () => {
    ...
    const sentinelPath = join(
      realCacheDir,
      "connected-providers-cache.test-sentinel.json",
    );
