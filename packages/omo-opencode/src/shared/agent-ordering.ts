...
export function validateAgentOrder(
  agentOrder: readonly string[] | undefined,
): AgentOrderValidation {
  const order: string[] = [];
...
export function resolveAgentOrderDisplayNames(
  agentOrder: readonly string[] | undefined,
): string[] {
  return validateAgentOrder(agentOrder).order.map((configKey) =>
    getAgentListDisplayName(configKey)
  );
}
