...
describe("external-plugin-detector", () => {
  ...
  describe("detectExternalNotificationPlugin", () => {
    ...
    test("should detect opencode-notifier with version suffix", () => {
      ...
      fs.writeFileSync(
        path.join(opencodeDir, "opencode.json"),
        JSON.stringify({
          plugin: ["oh-my-opencode", "opencode-notifier@1.2.3"],
        }),
      );
    ...
    test("should detect @mohak34/opencode-notifier", () => {
      ...
      fs.writeFileSync(
        path.join(opencodeDir, "opencode.json"),
        JSON.stringify({
          plugin: ["oh-my-opencode", "@mohak34/opencode-notifier"],
        }),
      );
    ...
    test("should safely handle tuple-format plugin entries without crashing (fixes #3122)", () => {
      ...
      expect(result.allPlugins).not.toContain(["advanced-tuple-plugin", {
        debug: true,
      }]);
    });
  ...
  describe("detectDuplicateOmoPlugin", () => {
    test("#given a source plugin and active profile package alias #when detecting duplicates #then it reports the self-conflict", async () => {
      // given
      const projectConfigDir = path.join(tempDir, ".opencode");
      const profileConfigDir = path.join(
        tempHomeDir,
        ".config",
        "opencode",
        "profiles",
      ...
      const { detectDuplicateOmoPlugin: detectDuplicateOmoPluginFresh } =
        await importFreshExternalPluginDetectorModule();
    ...
    test("#given both package names from the rename window #when detecting duplicates #then it treats them as the same OMO plugin", async () => {
      ...
      fs.writeFileSync(
        path.join(opencodeDir, "opencode.json"),
        JSON.stringify({
          plugin: ["oh-my-opencode", "npm:oh-my-openagent@latest"],
        }),
      );
      const { detectDuplicateOmoPlugin: detectDuplicateOmoPluginFresh } =
        await importFreshExternalPluginDetectorModule();
