...
describe("global-model-store", () => {
  ...
  it("#given a persisted pick #when readPersistedGlobalModel #then round-trips the model", () => {
    ...
    expect(readPersistedGlobalModel()).toEqual({ providerID: "anthropic", modelID: "claude-opus-5" })
  })
  ...
  it("#given a persisted pick #when read from a fresh process cache #then still returns the model", () => {
    ...
    _resetGlobalModelStoreCacheForTesting()
    expect(readPersistedGlobalModel()).toEqual({ providerID: "openai", modelID: "gpt-5.4" })
  })
  ...
  it("#given invalid store content #when readPersistedGlobalModel #then degrades to null", () => {
    ...
    mkdirSync(join(tempDataHome, "opencode/storage/oh-my-openagent"), { recursive: true })
    writeFileSync(storePath, "{not json", "utf-8")
    expect(readPersistedGlobalModel()).toBeNull()

    writeFileSync(storePath, JSON.stringify({ version: 99, providerID: "x", modelID: "y" }), "utf-8")
    expect(readPersistedGlobalModel()).toBeNull()

    writeFileSync(storePath, JSON.stringify({ version: 1, providerID: "", modelID: "y" }), "utf-8")
    expect(readPersistedGlobalModel()).toBeNull()
  })
  ...
  it("#when persistGlobalModel #then write is atomic (no .tmp leftover) and readable", () => {
    ...
    const raw = JSON.parse(readFileSync(storePath, "utf-8")) as { version: number; providerID: string; modelID: string }
