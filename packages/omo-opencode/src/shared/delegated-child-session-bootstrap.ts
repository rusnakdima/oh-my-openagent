...
const delegatedChildSessionBootstraps = new Map<
  string,
  DelegatedChildSessionBootstrap
>();

function cloneRetryParts(
  parts: DelegatedChildSessionRetryPart[],
): DelegatedChildSessionRetryPart[] {
...
export function registerDelegatedChildSessionBootstrap(_args: {
  ...
  _args.modelFallbackControllerAccessor?.setSessionFallbackChain(
    _args.sessionID,
    fallbackChain,
  );
  if (_args.category) {
...
export function getDelegatedChildSessionBootstrap(
  _sessionID: string,
): DelegatedChildSessionBootstrap | undefined {
