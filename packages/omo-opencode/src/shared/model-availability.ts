...
export function fuzzyMatchModel(
  ...
  if (matches.length === 0) {
    log("[fuzzyMatchModel] WARNING: no match found", {
      target,
      availableCount: available.size,
      providers,
    });
  ...
  const exactMatch = matches.find((model) =>
    normalizeModelName(model) === targetNormalized
  );
  if (exactMatch) {
...
export async function fetchAvailableModels(
  ...
  if (connectedProvidersUnknown) {
    if (client?.model?.list) {
      ...
      try {
        ...
        const models = normalizeSDKResponse(
          modelsResult,
  ...
  if (providerModelsCache) {
    ...
    if (providerCount === 0) {
      log(
        "[fetchAvailableModels] provider-models cache empty, falling back to models.json",
      );
    } else {
      log(
        "[fetchAvailableModels] using provider-models cache (whitelist-filtered)",
      );
      ...
      log(
        "[fetchAvailableModels] provider-models cache produced no models for connected providers, falling back to models.json",
      );
    }
  }

  log(
    "[fetchAvailableModels] provider-models cache not found, falling back to models.json",
  );
  const cacheFile = join(getOpenCodeCacheDir(), "models.json");

  if (!existsSync(cacheFile)) {
    log(
      "[fetchAvailableModels] models.json cache file not found, falling back to client",
    );
  } else {
    try {
      ...
      log(
        "[fetchAvailableModels] parsed models from models.json (NO whitelist filtering)",
        {
          count: modelSet.size,
          connectedProviders: connectedProvidersList.slice(0, 5),
        },
