...
function getWslLinuxHomeDir(windowsConfigRoot?: string): string | null {
  ...
  const user = process.env.USER?.trim() || process.env.LOGNAME?.trim() ||
    ...
    (windowsConfigRoot
      ? getWindowsUserFromConfigRoot(windowsConfigRoot)
      : undefined);
...
function getCliDefaultConfigDir(): string {
  ...
  const shouldIgnoreWindowsXdg = envXdgConfig !== undefined &&
    envXdgConfig.length > 0 &&
  ...
  const configDir = isWslEnvironment()
    ? posix.join(xdgConfig, "opencode")
...
export function getOpenCodeConfigDirs(
