import { spawn, type SpawnedProcess, spawnSync } from "./bun-spawn-shim";
    ...
    switch (extractor) {
      ...
      case "pwsh":
        proc = spawn([
          ...
          "-Command",
          `Expand-Archive -Path '${
            escapePowerShellPath(archivePath)
          }' -DestinationPath '${escapePowerShellPath(destDir)}' -Force`,
        ], {
          stdout: "ignore",
          stderr: "pipe",
        });
...
async function listZipEntries(archivePath: string) {
  if (process.platform === "win32") {
    ...
    return listZipEntriesWithPowerShell(
      archivePath,
