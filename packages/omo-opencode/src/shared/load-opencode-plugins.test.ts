...
async function importFreshLoadOpencodePluginsModule(): Promise<
  LoadOpencodePluginsModule
> {
  const modulePath = `${
    fileURLToPath(new URL("./load-opencode-plugins.ts", import.meta.url))
  ...
  describe("#given OPENCODE_CONFIG_DIR points at an active profile", () => {
    describe("#when loading plugins for the project", () => {
      it("#then includes plugin entries from the profile config directory", async () => {
        ...
        const projectPlugin = `file://${
          join(projectDirectory, "src", "index.ts")
        }`;
        const profilePlugin = `file://${
          join(profileDirectory, "profile-plugin.ts")
        }`;
        ...
        expect(result.indexOf(projectPlugin)).toBeLessThan(
          result.indexOf(profilePlugin),
        );
      });
    });
  });
  ...
  describe("#given XDG_CONFIG_HOME is set", () => {
    describe("#when loading plugins for the project", () => {
      it("#then discovers plugins from the XDG config directory (not just hardcoded ~/.config)", async () => {
        ...
        writeOpencodeConfig(projectDirectory, [projectPlugin]);
        writeFileSync(
          join(xdgOpencodeDir, "opencode.json"),
          JSON.stringify({ plugin: [xdgPlugin] }),
        );
