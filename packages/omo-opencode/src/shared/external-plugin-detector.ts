...
function matchesKnownPlugin(
  ...
  for (const known of knownPlugins) {
    ...
    if (
      normalized === `npm:${known}` || normalized.startsWith(`npm:${known}@`)
    ) return known;
    if (
...
export function detectDuplicateOmoPlugin(
  directory: string,
): DuplicateOmoPluginResult {
  const plugins = loadOpencodePlugins(directory);
  const duplicatePlugins = plugins.filter((plugin) =>
    matchesOmoPlugin(plugin) !== null
  );
...
export function getDuplicateOmoPluginWarning(
  duplicatePlugins: readonly string[],
): string {
  const formattedPlugins = duplicatePlugins.map((plugin) => `   - ${plugin}`)
    .join("\n");
