...
const log = mock(() => {});
const getServerBasicAuthHeader = mock(() =>
  "Basic b3BlbmNvZGU6dGVzdHBhc3N3b3Jk"
);
const fetchImplementation = mock(async (): Promise<Response> =>
  new Response(null, { status: 200 })
);

async function loadOpencodeHttpApi(): Promise<OpencodeHttpApi> {
  const opencodeHttpApi = await import(
    `${opencodeHttpApiSpecifier}?test=${crypto.randomUUID()}`
  );
  opencodeHttpApi._setFetchImplementationForTesting(fetchImplementation);
  opencodeHttpApi._setLogImplementationForTesting(log);
  opencodeHttpApi._setServerBasicAuthHeaderResolverForTesting(
...
describe("patchPart", () => {
  beforeEach(() => {
    ...
    getServerBasicAuthHeader.mockReturnValue(
      "Basic b3BlbmNvZGU6dGVzdHBhc3N3b3Jk",
    );
