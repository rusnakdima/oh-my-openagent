...
describe("dist bundle prompt content", () => {
  test(
    "#given the built plugin bundle #when its runtime agent factory is executed #then the bundled prompt equals its source artifact",
    async () => {
      const distIndex = Bun.file(DIST_INDEX);
      expect(
        await distIndex.exists(),
        `${DIST_INDEX} must exist before this CI-only test runs`,
      ).toBe(true);
