...
describe("live server route directory context", () => {
  test("#given generated live client #when session status is requested #then registered directory reaches the listener", async () => {
    ...
    const fakeFetch: typeof fetch = async (input) => {
      ...
      seenUrls.push(url);
      if (
        url.includes("/global/health") || url.includes("/session/ses_directory")
      ) {
    ...
    try {
      ...
      expect(new URL(statusUrl ?? SERVER_URL).searchParams.get("directory"))
        .toBe("/tmp/project with spaces");
    } finally {
