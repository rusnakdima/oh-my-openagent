...
const { describe, it, expect, beforeEach, afterEach, beforeAll, spyOn } =
  require("bun:test");
import {
  existsSync,
  mkdtempSync,
...
describe("fetchAvailableModels", () => {
  ...
  it("#given cache file with models #when fetchAvailableModels called with connectedProviders #then returns Set of model IDs", async () => {
    writeModelsCache({
      ...
      google: {
        id: "google",
        models: { "gemini-3.1-pro": { id: "gemini-3.1-pro" } },
      },
    });
  ...
  it("#given cache file with various providers #when fetchAvailableModels called with all providers #then extracts all IDs correctly", async () => {
    writeModelsCache({
      ...
      opencode: {
        id: "opencode",
        models: { "gpt-5-nano": { id: "gpt-5-nano" } },
      },
    });
...
describe("fuzzyMatchModel", () => {
  ...
  it("should normalize claude version separators for future versions", () => {
    ...
    const result = fuzzyMatchModel("claude-sonnet-5-1", available, [
      "github-copilot",
    ]);
  ...
  it("should normalize dash and dot version separators for non-Claude model families", () => {
    ...
    expect(fuzzyMatchModel("kimi-k2.6", available, ["kimi-for-coding"])).toBe(
      "kimi-for-coding/kimi-k2-6",
    );
    expect(fuzzyMatchModel("glm-4-6", available, ["zai-coding-plan"])).toBe(
      "zai-coding-plan/glm-4.6",
    );
    expect(fuzzyMatchModel("gpt-5.4", available, ["openai"])).toBe(
      "openai/gpt-5-4",
    );
  });
...
describe("fetchAvailableModels with provider-models cache (whitelist-filtered)", () => {
  ...
  function writeProviderModelsCache(
    data: { models: Record<string, string[] | any[]>; connected: string[] },
  ) {
  ...
  it("should prefer provider-models cache over models.json", async () => {
    ...
    writeModelsCache({
      opencode: {
        models: { "big-pickle": {}, "gpt-5-nano": {}, "gpt-5.4": {} },
      },
  ...
  it("should fall back to models.json when provider-models cache is empty", async () => {
    ...
    const match = fuzzyMatchModel("google/gemini-3-flash", availableModels, [
      "google",
    ]);
...
describe("fallback model availability", () => {
  ...
  beforeEach(() => {
    // given
    tempDir = mkdtempSync(join(tmpdir(), "opencode-test-"));
    connectedProvidersCacheSpy = spyOn(
      connectedProvidersCache,
  ...
  it("returns first resolved fallback model from chain", () => {
    ...
    // when
    const result = resolveFirstAvailableFallback(
      fallbackChain,
      availableModels,
    );
