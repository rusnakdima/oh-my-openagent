...
export const LIVE_ROUTE_DISPATCH_LOG =
  "[live-server-route] dispatch via live listener";
export const LIVE_ROUTE_UNAVAILABLE_LOG =
...
export function _setFetchImplementationForTesting(
  ...
  try {
    ...
    if (response.status === 401 || response.status === 403) {
      if (!registration.warnedOnce) {
        ...
        log(
          "[live-server-route] listener requires auth we cannot satisfy; live wake routing disabled",
        );
      }
      registration.available = false;
...
async function probeSessionAffinity(
  ...
  try {
    ...
    let response: Response;
    try {
      response = await getFetch()(probeUrl, {
        headers,
    ...
    if (response.status === 404) {
      ...
      log(
        "[live-server-route] live listener does not own session; falling back to in-process client",
        { sessionID },
      );
      return false;
    }
...
function getFreshSessionAffinity(
  registration: RouteRegistration,
  sessionID: string,
): boolean | undefined {
...
async function resolveSessionAffinity(
  registration: RouteRegistration,
  sessionID: string,
): Promise<boolean | undefined> {
...
function getFreshProbeAvailability(
  registration: RouteRegistration,
): boolean | undefined {
  const available = registration.available;
  if (
    available === undefined ||
    Date.now() - registration.probeTimestamp >= PROBE_TTL_MS
  ) {
    return undefined;
  }
  return available;
...
export function tryResolveDispatchClientSync(
  client: unknown,
  sessionID: string,
...
export async function resolveDispatchClient(
  client: unknown,
  sessionID: string,
): Promise<RouteResult> {
