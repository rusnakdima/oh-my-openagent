...
function shouldReleaseReservationAfterFailedAsyncPrompt(
  error: unknown,
): boolean {
  return parseModelSuggestionFromCore(error) !== null ||
    isAgentResolutionError(error);
}
...
export async function promptWithModelSuggestionRetry(
  ...
  try {
    const promptResult = await dispatchInternalPrompt({
      ...
      settleMs: 0,
      ...(options.queueBehavior
        ? { queueBehavior: options.queueBehavior }
        : {}),
      ...(options.checkStatus !== undefined
        ? { checkStatus: options.checkStatus }
        : {}),
      ...(options.checkToolState !== undefined
        ? { checkToolState: options.checkToolState }
        : {}),
    });
