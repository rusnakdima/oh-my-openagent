...
import { _resetGlobalModelStoreCacheForTesting, persistGlobalModel } from "./global-model-store"
...
describe("global model applies to all agent modes (repro /models bug)", () => {
  ...
  afterEach(() => {
    ...
    const mockRestore = (connectedCache.readProviderModelsCache as unknown as { mockRestore?: () => void }).mockRestore
    if (typeof mockRestore === "function") mockRestore.call(connectedCache.readProviderModelsCache)
  })
  ...
  it("persisted cross-process pick is honored when heap is empty (TUI-process pick)", () => {
    ...
    expect(getEffectiveModelForAgent("sisyphus")).toEqual({ providerID: "openai", modelID: "gpt-5.4" })
    expect(getEffectiveModelForAgent("librarian")).toEqual({ providerID: "openai", modelID: "gpt-5.4" })
  })
