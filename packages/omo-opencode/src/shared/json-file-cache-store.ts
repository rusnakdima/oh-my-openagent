...
export function createJsonFileCacheStore<TValue>(
  ...
  function read(): TValue | null {
    ...
    if (!existsSync(cacheFile)) {
      memoryValue = null;
      log(`[${options.logPrefix}] ${options.cacheLabel} file not found`, {
        cacheFile,
      });
      return null;
    }
    ...
    try {
      ...
      memoryValue = value;
      log(
        `[${options.logPrefix}] Read ${toLogLabel(options.cacheLabel)}`,
      ...
      memoryValue = null;
      log(
        `[${options.logPrefix}] Error reading ${
          toLogLabel(options.cacheLabel)
        }`,
        {
          error: String(error),
        },
  ...
  function write(value: TValue): void {
    ...
    try {
      ...
      log(
        `[${options.logPrefix}] ${options.cacheLabel} written`,
        options.describe(value),
      );
    } catch (error) {
      log(
        `[${options.logPrefix}] Error writing ${
          toLogLabel(options.cacheLabel)
        }`,
        {
          error: String(error),
