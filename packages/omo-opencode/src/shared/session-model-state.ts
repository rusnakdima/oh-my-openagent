...
function getBuiltinFallback(name: string): SessionModel | null {
  const req = CATEGORY_MODEL_REQUIREMENTS[name] ??
    AGENT_MODEL_REQUIREMENTS[name];
...
export function getProviderDefaultModel(): SessionModel | null {
  ...
  for (const [providerID, modelEntries] of Object.entries(cache.models)) {
    ...
    const modelID = typeof first === "string"
      ? first
...
// NO Fallbacks: global model is the ONLY model for all agents
// Priority: only getSelectedGlobalModelLive() - fails if no global pick
// Global model applies to ALL modes (primary|subagent|all) — no mode filter.

export function getEffectiveModelForAgent(
  agentName: string,
): SessionModel | null {
  // ONLY return the global model - no provider defaults, no builtin fallbacks
  return getSelectedGlobalModelLive();
}
