...
export function logLegacyPluginStartupWarning(
  ...
  const checkForLegacyPluginEntryFn = deps.checkForLegacyPluginEntry ??
    checkForLegacyPluginEntry;
  const logFn = deps.log ?? log;
  const migrateLegacyPluginEntryFn = deps.migrateLegacyPluginEntry ??
  ...
  if (migrated) {
    console.warn(
      `[oh-my-openagent] Auto-migrated opencode.json: ${
        result.legacyEntries.join(", ")
      } -> ${suggestedEntries.join(", ")}`,
    );
  } else {
    console.warn(
      `[oh-my-openagent] Could not auto-migrate. Please manually update your opencode.json:` +
        ` ${
          result.legacyEntries.map((e, i) =>
            `"${e}" -> "${suggestedEntries[i]}"`
          ).join(", ")
        }`,
    );
  }
