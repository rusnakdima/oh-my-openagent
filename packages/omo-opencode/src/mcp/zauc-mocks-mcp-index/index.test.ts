...
function mockLocalMcps(): void {
  mock.module("../lsp", () => ({
    createLspMcpConfig: () => ({
      type: "local",
      command: ["node", "dist/cli.js", "mcp"],
      enabled: true,
    }),
  }));
  mock.module("../codegraph", () => ({
    createCodegraphMcpConfig: () => ({
      type: "local",
...
describe("createBuiltinMcps", () => {
  test("should return all MCPs when disabled_mcps is empty", () => {
    // given
    mockLocalMcps();
    const { createBuiltinMcps } = require(
      "../index",
  ...
  test("should omit codegraph when it is listed in disabled_mcps", () => {
    ...
    // when
    const result = createBuiltinMcps(["codegraph"], {
  ...
  test("should keep codegraph registered but disabled when its binary is absent", async () => {
    // given
    mock.restore();
    const { createBuiltinMcps } = await import(
      `../index?codegraph-missing=${Date.now()}-${Math.random()}`
    );
  ...
  test("should resolve enabled local MCP runtime commands before registration", async () => {
    ...
    const { createBuiltinMcps } = await import(
      `../index?runtime=${Date.now()}-${Math.random()}`
    );
    ...
    const result = createBuiltinMcps([], undefined, {
      ...
      resolveExecutable: (commandName: string) => {
        if (commandName === "node") {
          return { command: nodePath, available: true };
        }
    ...
    if (result.lsp?.type !== "local") {
      throw new Error("expected local MCP config");
    }
