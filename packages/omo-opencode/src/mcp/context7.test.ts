...
async function importFreshContext7Module(): Promise<
  typeof import("./context7")
> {
  return import(`./context7?test=${Date.now()}-${Math.random()}`);
}

async function loadContext7WithApiKey(
  value: string | undefined,
): Promise<typeof import("./context7")["context7"]> {
...
describe("context7 MCP config", () => {
  ...
  test("#given real CONTEXT7_API_KEY #when config is loaded #then bearer auth header is used", async () => {
    ...
    expect(config.headers).toEqual({
      Authorization: "Bearer ctx7sk_test_real_value",
    });
