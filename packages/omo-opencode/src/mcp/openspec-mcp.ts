...
import { createOpenSpecMcpTools, handleOpenSpecMcpRequest } from "./openspec-tools"
...
export async function handleOpenSpecMcpRequestRpc(
  ...
  if (method === "initialize") {
    return successResponse(id, {
      capabilities: { tools: {} },
      serverInfo: { name: OPENSEPC_MCP_SERVER_NAME, version: OPENSEPC_MCP_SERVER_VERSION },
      protocolVersion: "2024-11-05",
    })
  }

  if (method === "tools/list") {
    const tools = createOpenSpecMcpTools({ specDir: options.specDir ?? "openspec" })
    return successResponse(id, { tools })
  }

  if (method === "tools/call") {
    const params = input["params"] as { name: string; arguments?: Record<string, unknown> } | undefined
