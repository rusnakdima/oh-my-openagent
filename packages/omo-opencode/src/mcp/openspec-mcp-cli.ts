...
async function main(): Promise<void> {
  ...
  try {
    mcpModule = require(join(thisDir, "openspec-mcp.js")) as typeof import("./openspec-mcp.js")
  } catch {
    // Try as ESM
    try {
      mcpModule = await import(`${join(thisDir, "openspec-mcp.js")}?t=${Date.now()}`)
    } catch (err) {
      stderr.write(`Error: Could not load openspec-mcp module: ${err instanceof Error ? err.message : String(err)}\n`)
      process.exitCode = 1
      return
    }
  }
  ...
  if (typeof mcpModule?.runOpenSpecMcpServer !== "function") {
    stderr.write("Error: openspec-mcp module does not export runOpenSpecMcpServer\n")
    process.exitCode = 1
    return
  }
