...
type AncestorCliResolverOptions = {
  ...
  readonly isSourceCandidateAvailable?: (
    input: SourceCandidateAvailabilityInput,
  ) => boolean;
};
...
export function createAncestorCliCandidates(
  options: AncestorCliResolverOptions,
  ...
  while (true) {
    ...
    if (!seenPaths.has(sourceCliPath)) {
      ...
      candidates.push({
        ...
        root: currentDirectory,
        path: sourceCliPath,
        exists: runtime.available && options.pathExists(sourceCliPath) &&
