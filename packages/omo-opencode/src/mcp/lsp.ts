...
import {
  type AncestorCliCandidate,
  createAncestorCliCandidates,
  resolveJavaScriptRuntime,
} from "./shared/ancestor-cli-resolver";
...
function findBootstrapRoot(
  candidates: readonly AncestorCliCandidate[],
  pathExists: (path: string) => boolean,
): string {
  return candidates.find((candidate) =>
    pathExists(resolve(candidate.root, "package.json"))
  )?.root ?? process.cwd();
}
...
function readDaemonPackageVersion(root: string): string | null {
  try {
    const packageJson = JSON.parse(
      readFileSync(resolve(root, PACKAGE_REL, "package.json"), "utf-8"),
    );
...
function createBootstrapCandidate(
  ...
  return {
    ...
    exists: runtime.available && npm.available &&
      pathExists(packageManifestPath),
    runtimeAvailable: runtime.available,
  };
}

function resolveLspCommand(
  options: LspMcpConfigOptions = {},
): AncestorCliCandidate {
  ...
  const candidates = moduleDirectory
    ? createAncestorCliCandidates({
      ...
      isSourceCandidateAvailable: ({ root }) =>
        pathExists(resolve(root, LSP_TOOLS_PACKAGE_REL, DIST_CLI_REL)) &&
        readDaemonPackageVersion(root) !== null,
    })
    : [];

  const distCandidate = candidates.find((candidate) =>
    hasCliSuffix(candidate.path, DIST_CLI_REL) && candidate.exists
  );
  if (distCandidate) {
  ...
  const sourceCandidate = candidates.find(
    (candidate) =>
      hasCliSuffix(candidate.path, SOURCE_CLI_REL) && candidate.exists,
  );
  if (sourceCandidate) {
    return sourceCandidate;
  }

  return createBootstrapCandidate(
    findBootstrapRoot(candidates, pathExists),
    pathExists,
    resolveExecutable,
  );
}

export function createLspMcpConfig(
  options: LspMcpConfigOptions = {},
): LocalMcpConfig {
  const resolvedCommand = resolveLspCommand(options);
  const cwd = resolve(options.cwd ?? process.cwd());
  const configDir = getOpenCodeConfigDir({ binary: "opencode" });
  const sourceVersion = hasCliSuffix(resolvedCommand.path, SOURCE_CLI_REL)
    ? readDaemonPackageVersion(resolvedCommand.root)
    : null;
  ...
  return {
    ...
    environment: {
      ...
      LSP_TOOLS_MCP_INSTALL_DECISIONS: resolve(
        configDir,
