...
describe("createLspMcpConfig", () => {
  it("resolves bundled dist cli from module root when cwd is unrelated", () => {
    ...
    mkdirSync(join(packageRoot, "packages", "lsp-daemon", "dist"), {
      recursive: true,
    });
  ...
  it("uses the bun daemon source cli when the engine dist is already built", () => {
    ...
    mkdirSync(join(packageRoot, "packages", "lsp-daemon", "src"), {
      recursive: true,
    });
    mkdirSync(join(packageRoot, "packages", "lsp-tools-mcp", "dist"), {
      recursive: true,
    });
    ...
    expect(config.environment?.OMO_LSP_DAEMON_VERSION).toBe(
  ...
  it("prefers an ancestor dist cli before an earlier source cli candidate", () => {
    ...
    const nearerSourceCliPath = join(
      packageRoot,
      "nested",
      "src",
      "mcp",
    ...
    const nearerToolsDistPath = join(
      packageRoot,
      "nested",
      "src",
      "mcp",
    ...
    const ancestorDistCliPath = join(
      packageRoot,
      "packages",
      "lsp-daemon",
      "dist",
  ...
  it("uses the nearest source cli when no dist cli exists in the ancestor walk", () => {
    ...
    const ancestorSourceCliPath = join(
      packageRoot,
      "packages",
      "lsp-daemon",
      "src",
    ...
    const ancestorToolsDistPath = join(
      packageRoot,
      "packages",
      "lsp-tools-mcp",
      "dist",
    ...
    writeFileSync(nearerToolsDistPath, "#!/usr/bin/env node\n", "utf-8");
    writeFileSync(
      ancestorSourceCliPath,
      "console.log('ancestor-source')\n",
      "utf-8",
    );
  ...
  it("does not resolve the MCP command from the opened workspace", () => {
    ...
    const workspaceRoot = createTemporaryDirectory(
      "omo-lsp-malicious-workspace-",
    );
    const moduleFilePath = join(packageRoot, "dist", "index.js");
    const workspaceCliPath = join(
      workspaceRoot,
      "packages",
      "lsp-daemon",
      "dist",
    ...
    mkdirSync(join(workspaceRoot, "packages", "lsp-daemon", "dist"), {
      recursive: true,
    });
    writeFileSync(
      join(packageRoot, "package.json"),
  ...
  it("returns a vendored package bootstrap command when no LSP cli entrypoint exists", () => {
    ...
    expect(script.indexOf("packages/lsp-tools-mcp")).toBeLessThan(
      script.indexOf("packages/lsp-daemon"),
    );
  ...
  it("disables the MCP config when no runtime can launch any LSP candidate", () => {
    ...
    // then
    expect(config.enabled).toBe(false);
    const projectConfigPaths = config.environment?.LSP_TOOLS_MCP_PROJECT_CONFIG
      .split(delimiter);
    ...
    expectPathSuffix(config.environment?.LSP_TOOLS_MCP_USER_CONFIG, [
      "opencode",
      "lsp.json",
    ]);
  ...
  it("matches user config path suffixes across POSIX and Windows separators", () => {
    ...
    const windowsUserConfig =
      "C:\\Users\\RUNNER~1\\AppData\\Local\\Temp\\omo-test-home\\.config\\opencode\\lsp.json";
...
function expectPathSuffix(
  pathValue: string | undefined,
  expectedSuffix: readonly string[],
): void {
  expect(pathValue?.split(/[\\/]+/).slice(-expectedSuffix.length)).toEqual(
    expectedSuffix,
  );
}

function writeDaemonPackageJson(
  packageRoot: string,
  version: string = DAEMON_PACKAGE_VERSION,
): void {
  mkdirSync(join(packageRoot, "packages", "lsp-daemon"), { recursive: true });
  const packageJson = {
