...
function parseResponses(
  chunks: string[],
): Array<{ id: unknown; result?: unknown; error?: unknown }> {
  return chunks
    .join("")
    .trim()
    .split("\n")
    ...
    .map((line) =>
      JSON.parse(line) as { id: unknown; result?: unknown; error?: unknown }
    );
}
...
beforeEach(async () => {
  tmp = join(
    tmpdir(),
    `openspec-cli-test-${Date.now()}-${Math.random().toString(36).slice(2)}`,
  );
  specDir = "specs";
...
describe("runOpenSpecMcpServer", () => {
  it("#given initialize request #when server starts #then returns protocol version + capabilities", async () => {
    ...
    expect(
      (responses[0].result as { serverInfo: { name: string; version: string } })
        .serverInfo.name,
    ).toBe("openspec");
    expect(
      (responses[0].result as { capabilities: { tools: unknown } }).capabilities
        .tools,
    ).toEqual({});
  });
  ...
  it("#given tools/call openspec_status #when spec exists #then returns task counts", async () => {
      ...
      inputStream([
        ...
        },
        {
          ...
          params: {
            ...
            arguments: { spec_name: "status-spec" },
          },
        },
      ]),
      collectingWritable(chunks),
    ...
    const text =
      (statusResp?.result as { content: Array<{ text: string }> }).content[0]
        .text;
  ...
  it("#given tools/call openspec_propose #when spec does not exist #then creates files", async () => {
    ...
    const text =
      (proposeResp?.result as { content: Array<{ text: string }> }).content[0]
        .text;
  ...
  it("#given tools/call openspec_verify #when spec is valid #then returns verification results", async () => {
      ...
      inputStream([
        ...
        },
        {
          ...
          params: {
            ...
            arguments: { spec_name: "verify-spec" },
          },
        },
      ]),
      collectingWritable(chunks),
    ...
    const text =
      (verifyResp?.result as { content: Array<{ text: string }> }).content[0]
        .text;
    ...
    const server = runOpenSpecMcpServer(
      inputStream([
        ...
        },
        {
          ...
          id: 2,
          method: "tools/call",
          params: { name: "openspec_verify", arguments: {} },
        },
      ]),
      collectingWritable(chunks),
      { specDir, cwd: tmp },
  ...
  it("#given tools/call openspec_read #when spec file exists #then returns file content", async () => {
    ...
    const server = runOpenSpecMcpServer(
      inputStream([
        ...
        },
        {
          ...
          params: {
            name: "openspec_read",
            arguments: { file: "spec.md", spec_name: "read-spec" },
          },
        },
      ]),
      collectingWritable(chunks),
    ...
    const text =
      (readResp?.result as { content: Array<{ text: string }> }).content[0]
        .text;
    ...
    const server = runOpenSpecMcpServer(
      inputStream([
        ...
        },
        {
          ...
          params: {
            name: "openspec_read",
            arguments: { file: "tasks.md", spec_name: "empty-spec" },
          },
        },
      ]),
      collectingWritable(chunks),
  ...
  it("#given tools/call openspec_archive #when spec exists #then moves spec to ARCHIVE", async () => {
      ...
      inputStream([
        ...
        },
        {
          ...
          params: {
            ...
            arguments: { spec_name: "archive-spec" },
          },
        },
      ]),
      collectingWritable(chunks),
    ...
    const text =
      (archiveResp?.result as { content: Array<{ text: string }> }).content[0]
        .text;
    ...
    const server = runOpenSpecMcpServer(
      inputStream([
        ...
        },
        {
          ...
          params: {
            name: "openspec_archive",
            arguments: { spec_name: "nonexistent" },
          },
        },
      ]),
      collectingWritable(chunks),
  ...
  it("#given tools/call with unknown method #then returns isError content", async () => {
    ...
    const server = runOpenSpecMcpServer(
      inputStream([
        ...
        },
        {
          ...
          id: 2,
          method: "tools/call",
          params: { name: "unknown_method", arguments: {} },
        },
      ]),
      collectingWritable(chunks),
      { specDir, cwd: tmp },
    ...
    await server;
    const responses = parseResponses(chunks);
    const unknownResp = responses.find((r) => r.id === 2) as
    ...
    const resultContent =
      (unknownResp?.result as { content: Array<{ text: string }> }).content[0];
  ...
  it("#given invalid JSON-RPC #then returns parse error", async () => {
    ...
    const server = runOpenSpecMcpServer(readable, collectingWritable(chunks), {
      specDir,
      cwd: tmp,
    });
