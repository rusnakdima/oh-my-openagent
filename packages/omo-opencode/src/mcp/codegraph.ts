...
export type CodegraphMcpConfigOptions = {
  ...
  readonly nodeVersionForExecutable?:
    ResolveCodegraphCommandOptions["nodeVersion"];
...
function isProjectExcluded(
  config: Partial<CodegraphConfig> | undefined,
  options: CodegraphMcpConfigOptions,
): boolean {
...
export function createCodegraphMcpConfig(
  options: CodegraphMcpConfigOptions = {},
): LocalMcpConfig {
  ...
  const resolvedCommand = resolveCodegraphCommand({
    ...
    provisioned: options.provisioned ??
      (() =>
        provisionedBinFromInstallDir(options.config?.install_dir, fileExists)),
  ...
  const enabled = !isProjectExcluded(options.config, options) &&
    resolvedCommand.exists &&
    (resolvedCommand.source === "bundled" || resolvedCommand.source === "env" ||
