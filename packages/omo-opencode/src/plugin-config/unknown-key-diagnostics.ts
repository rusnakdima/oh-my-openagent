...
function unwrap(schema: Schema): Schema {
  ...
  if (def.type === "pipe") {
    return def.in === undefined ? schema : unwrap(def.in);
  }
  if (def.type === "lazy") {
    return def.getter === undefined ? schema : unwrap(def.getter());
  }
  return schema;
}
...
function matchesShape(schema: Schema, value: unknown): boolean {
  ...
  if (def.type === "object" || def.type === "record") {
    return isPlainRecord(value);
  }
  if (def.type === "array" || def.type === "tuple") return Array.isArray(value);
  if (def.type === "union") {
    return def.options?.some((option) => matchesShape(option, value)) ?? false;
  }
  if (def.type === "intersection") {
    return def.left !== undefined && def.right !== undefined &&
      matchesShape(def.left, value) && matchesShape(def.right, value);
  }
...
function unknownPaths(
  ...
  value: unknown,
  path: UnknownKeyPath,
): readonly UnknownKeyPath[] {
  const unwrapped = unwrap(schema);
  const def = definition(unwrapped);

  if (
    def.type === "object" && isPlainRecord(value) && def.shape !== undefined
  ) {
    const catchall =
      def.catchall !== undefined && definition(def.catchall).type !== "never"
        ? def.catchall
        : undefined;
    return Object.entries(value).flatMap(([key, entry]) => {
      ...
      if (catchall !== undefined) {
        return unknownPaths(catchall, entry, [...path, key]);
      }
      return [[...path, key]];
    });
  }
  ...
  if (def.type === "record" && isPlainRecord(value)) {
    ...
    if (valueSchema !== undefined) {
      return Object.entries(value).flatMap(([key, entry]) =>
        unknownPaths(valueSchema, entry, [...path, key])
      );
    }
  }
  ...
  if (def.type === "array" && Array.isArray(value)) {
    ...
    if (element !== undefined) {
      return value.flatMap((entry, index) =>
        unknownPaths(element, entry, [...path, index])
      );
    }
  }
  ...
  if (def.type === "tuple" && Array.isArray(value)) {
    return value.flatMap((entry, index) => {
      ...
      return item === undefined || item === null
        ? []
        : unknownPaths(item, entry, [...path, index]);
    });
  }
  ...
  if (def.type === "union") {
    ...
    return candidates.reduce<readonly UnknownKeyPath[] | undefined>(
      (best, candidate) => (
  ...
  if (
    def.type === "intersection" && def.left !== undefined &&
    def.right !== undefined
  ) {
    return [
      ...unknownPaths(def.left, value, path),
      ...unknownPaths(def.right, value, path),
    ];
  }
...
export function findUnknownKeyPaths(
  schema: Schema,
  value: unknown,
): readonly UnknownKeyPath[] {
