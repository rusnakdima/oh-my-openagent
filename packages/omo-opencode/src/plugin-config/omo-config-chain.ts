...
const NON_PLUGIN_FIELDS = new Set([
  "legacy_migrations",
  "models",
  "task",
  "teams",
...
function baseView(
  ...
  for (const [key, value] of Object.entries(config)) {
    if (
      key === "profiles" || (key.startsWith("[") && key.endsWith("]")) ||
      NON_PLUGIN_FIELDS.has(key)
    ) continue;
    result[key] = value;
  }
  return result;
...
function modelInput(
  ...
  const agents = isPlainRecord(view.agents)
    ? Object.fromEntries(
      Object.entries(view.agents).flatMap(([name, definition]) => {
  ...
  const categories = isPlainRecord(view.categories)
    ? Object.fromEntries(
      Object.entries(view.categories).flatMap(([name, definition]) => {
        const fields = recordFields(definition, [
          "description",
          "model",
          "fallback_models",
...
function modelView(view: Readonly<Record<string, unknown>>): {
  ...
  return {
    config: {
      ...(resolved.view.agents === undefined
        ? {}
        : { agents: resolved.view.agents }),
      ...(resolved.view.categories === undefined
        ? {}
        : { categories: resolved.view.categories }),
    },
...
export function loadOmoOpenCodeConfigChain(
  ...
  appendViews(
    views,
    loaded.layers,
    (config) => baseView(profile(config, selectedProfile)),
  );
  appendViews(
    views,
    loaded.layers,
    (config) => block(profile(config, selectedProfile)),
  );
  ...
  for (const layer of loaded.layers) {
    ...
    protectedUserView = mergeOmoConfigRecords(
      protectedUserView,
      block(layer.config),
    );
  }
  for (const layer of loaded.layers) {
    ...
    protectedUserView = mergeOmoConfigRecords(
      protectedUserView,
      block(profile(layer.config, selectedProfile)),
    );
  }
