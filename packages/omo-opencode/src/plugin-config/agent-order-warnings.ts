...
function formatAgentOrderWarningValues(values: readonly string[]): string {
  const displayedValues = values.slice(0, MAX_AGENT_ORDER_WARNING_VALUES).map(
    (value) => {
...
export function addAgentOrderWarnings(
  configPath: string,
  ...
  if (validation.invalid.length > 0) {
    messages.push(
      `unknown agent names ignored: ${
        formatAgentOrderWarningValues(validation.invalid)
      }`,
    );
  }
  ...
  if (validation.duplicates.length > 0) {
    messages.push(
      `duplicate agent names ignored: ${
        formatAgentOrderWarningValues(validation.duplicates)
      }`,
    );
  }
