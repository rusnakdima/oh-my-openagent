...
export async function formatFullSession(
  ...
  if (options.sinceMessageId) {
    const index = filteredMessages.findIndex((message) =>
      message.id === options.sinceMessageId
    );
    if (index === -1) {
    ...
    for (const part of message.parts ?? []) {
      if (part.type === "text" && part.text) {
        ...
        const input = part.input === undefined
          ? ""
          : truncateText(JSON.stringify(part.input), thinkingMaxChars);
