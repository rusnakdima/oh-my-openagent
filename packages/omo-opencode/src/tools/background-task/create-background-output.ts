  ...
  return tool({
    ...
    args: {
      task_id: tool.schema
        .string()
        .describe(
          "background task ID (`bg_...`) from launch/completion; not a session ID (`ses_...`).",
        ),
      block: tool.schema
        .boolean()
        .optional()
      ...
      timeout: tool.schema.number().optional().describe(
        "Max wait time in ms (default: 60000, max: 600000)",
      ),
      full_session: tool.schema.boolean().optional().describe(
        "Return full session messages with filters (default: false)",
      ),
      ...
      try {
        ...
        if (shouldBlock && isTaskActiveStatus(task.status)) {
          ...
          while (Date.now() - startTime < timeoutMs) {
            ...
            await delay(
              Math.min(
                BACKGROUND_OUTPUT_POLL_INTERVAL_MS,
                Math.max(1, remainingMs),
              ),
            );

            const currentTask = await getTaskWithMissingRetry(
              manager,
              args.task_id,
            );
            if (!currentTask) {
          ...
          if (isTaskActiveStatus(resolvedTask.status)) {
            const finalCheck = await getTaskWithMissingRetry(
              manager,
              args.task_id,
            );
            if (finalCheck) {
        ...
        const includeToolResults = isActive ||
          (args.include_tool_results ?? false);
