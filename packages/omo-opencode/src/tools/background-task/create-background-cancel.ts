...
export function createBackgroundCancel(
  ...
  return tool({
    ...
    args: {
      taskId: tool.schema.string().optional().describe(
        "Task ID to cancel (required if all=false)",
      ),
      all: tool.schema.boolean().optional().describe(
        "Cancel all running background tasks (default: false)",
      ),
    },
    async execute(args: BackgroundCancelArgs, toolContext) {
      try {
        ...
        if (cancelAll) {
          ...
          const cancellableTasks = tasks.filter((t: { status: string }) =>
            ...
            .map(
              (t) =>
                `| \`${t.id}\` | ${t.description} | ${t.status} | ${
                  t.sessionID ? `\`${t.sessionID}\`` : "(not started)"
                } |`,
            )
            .join("\n");
...
Status: ${task.status}`;
        ...
        return `[ERROR] Error cancelling task: ${
