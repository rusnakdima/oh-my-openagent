const { describe, test, expect, beforeEach, afterEach, mock, spyOn } = require(
  "bun:test",
);
...
describe("executeSyncContinuation - toast cleanup error paths", () => {
  ...
  test("marks and aborts resumed sync session after successful handback", async () => {
    ...
    const { handedBackSyncSessions } = require(
  ...
  test("removes toast when abort happens", async () => {
    ...
    const deps = {
      pollSyncSession: async (
        ...
        _client: unknown,
        input: {
          toastManager?: { removeTask: (id: string) => void } | null;
          taskId?: string;
        },
      ) => {
  ...
  test("no crash when toastManager is null", async () => {
    ...
    const { _resetTaskToastManagerForTesting } = require(
      "../../features/task-toast-manager/manager",
    );
    _resetTaskToastManagerForTesting();
  ...
  test("preserves restricted tool permissions for resumed explore sessions", async () => {
    ...
    const mockClient = {
      session: {
        ...
        prompt: async (
          input: { path: { id: string }; body: Record<string, unknown> },
        ) => {
