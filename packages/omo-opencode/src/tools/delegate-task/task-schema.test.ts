...
function createDelegateTask(
  ...args: Parameters<typeof import("./tools").createDelegateTask>
): ReturnType<typeof import("./tools").createDelegateTask> {
  return requireFresh<typeof import("./tools")>("./tools").createDelegateTask(
    ...args,
  );
}
...
describe("createDelegateTask schema", () => {
  ...
  test("#given task schema #when describing async mode #then it names background task ids explicitly", () => {
    ...
    //#when
    const runInBackgroundSchema = unsafeTestValue<{ description?: string }>(
