...
import {
  getTerminalSessionError,
  isSessionComplete,
} from "./sync-session-turns";
...
function abortSyncSession(
  ...
  void client.session.abort({
    ...
    log("[task] Failed to abort sync session", {
      sessionID,
      reason,
      error: String(error),
    });
...
async function fetchSessionMessages(
  ...
  const rawData = (messagesResult as { data?: unknown })?.data ??
    messagesResult;
...
export async function pollSyncSession(
  ...
  const maxPollTimeMs = Math.max(
    timeoutMs ?? getDefaultSyncPollTimeoutMs(),
    50,
  );
  ...
  const isAwaitingChildContinuation = (
    ...
    if (
      childWaitAssistantId === undefined ||
      currentAssistantId !== childWaitAssistantId
    ) {
      return false;
    }
  ...
  while (true) {
    ...
    if (ctx.abort?.aborted) {
      ...
      if (finalMessages) {
        const hasNewMessages = input.anchorMessageCount === undefined ||
          finalMessages.length > input.anchorMessageCount;
        if (hasNewMessages && isSessionComplete(finalMessages)) {
          log("[task] Abort detected after session already completed", {
            sessionID: input.sessionID,
          });
          return null;
        }
    ...
    try {
      ...
      const errorMessage = error instanceof Error
        ? error.message
        : String(error);
      log("[task] Poll status fetch failed, checking messages", {
        sessionID: input.sessionID,
        error: errorMessage,
      });
      ...
      if (consecutiveStatusFailures >= MAX_CONSECUTIVE_STATUS_FAILURES) {
        const msg =
          `Session status unavailable after ${consecutiveStatusFailures} attempts. The session may have been deleted server-side.`;
    ...
    try {
      ...
      const errorMessage = error instanceof Error
        ? `${error.name}: ${error.message}`
        : String(error);
      log("[task] Poll messages fetch failed, retrying", {
        sessionID: input.sessionID,
        error: errorMessage,
      });
      continue;
    ...
    if (
      ...
      messages.length <= input.anchorMessageCount
    ) {
      continue;
    }
    ...
    if (sessionError) {
      log("[task] Poll detected terminal session error", {
        sessionID: input.sessionID,
        sessionError,
      });
      return sessionError;
    ...
    if (isSessionComplete(messages)) {
      const currentAssistantId = [...messages].reverse().find((m) =>
        m.info?.role === "assistant"
      )?.info?.id;
      if (isAwaitingChildContinuation(currentAssistantId)) {
        continue;
      }
      log("[task] Poll complete - terminal finish detected", {
        sessionID: input.sessionID,
        pollCount,
      });
      break;
    ...
    if (
      lastAssistant?.info?.id && lastAssistant.info.id !== lastSeenAssistantId
    ) {
      lastSeenAssistantId = lastAssistant.info.id;
      assistantTurnCount++;
      if (assistantTurnCount >= maxTurns) {
        log(
          "[task] Max assistant turns reached, aborting to prevent infinite loop",
          {
            sessionID: input.sessionID,
            assistantTurnCount,
            maxTurns,
  ...
  if (timedOut) {
    log("[task] Poll inactivity timeout reached", {
      sessionID: input.sessionID,
      pollCount,
    });
