...
function resolveAvailableModelEntry(
  ...
  const matchedModel = fuzzyMatchModel(fullModel, availableModels, [
  ...
  if (typeof entry === "string") {
    return parsedModel.variant
      ? `${matchedModel}(${parsedModel.variant})`
      : matchedModel;
  }
...
export function resolveCategoryConfig(
  ...
  if (
    configuredUserConfig?.models && availableModels && availableModels.size > 0
  ) {
  ...
  const requiredModel = categoryReq?.requiresModel ??
  ...
  if (requiredModel && availableModels && !hasExplicitUserConfig) {
    if (!isModelAvailable(requiredModel, availableModels)) {
      log(
        `[resolveCategoryConfig] Category ${categoryName} requires ${requiredModel} but not available`,
      );
      return null;
    }
  }
