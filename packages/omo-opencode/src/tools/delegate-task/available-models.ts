...
function hasModelList(client: OpencodeClient): client is ModelListClient {
  return "model" in client && isRecord(client.model) &&
    typeof client.model.list === "function";
}
...
export async function getAvailableModelsForDelegateTask(
  ...
  if (providerModelsCache?.models) {
    ...
    for (
      const [providerID, models] of Object.entries(providerModelsCache.models)
    ) {
  ...
  const connectedProviders = connectedProvidersCache
