...
function isSkillAllowedForTargetAgent(
  skill: LoadedSkill,
...
async function loadNativeSkillEntries(
  ...
  try {
    ...
    log(
      "[skill-resolver] nativeSkills.all() failed; falling back to disk-only skills",
      {
        error: errorMessage,
      },
    );
...
async function loadBaseSkills(
  ...
  if (options.getLoadedSkills) {
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : String(err);
      log(
        "[skill-resolver] getLoadedSkills() failed; falling back to disk-discovered skills",
        {
          error: errorMessage,
        },
      );
...
export async function resolveSkillContent(
  ...
  { content: string | undefined; contents: string[]; error: string | null }
> {
  if (skills.length === 0) {
  ...
  const mergeNativeEntries = async (): Promise<void> => {
    ...
    nativeEntries = await loadNativeSkillEntries(
  ...
  for (const name of skills) {
    ...
    if (!skill && options.browserProvider === undefined) {
      ...
      if (
        skill && options.disabledSkills &&
        isDisabledSkillAlias(skill, options.disabledSkills)
      ) {
        skill = undefined;
      }
    }
    ...
    if (!isSkillAllowedForTargetAgent(skill, options.targetAgent)) {
      log(
        "[skill-resolver] filtered agent-restricted skill for delegate target",
        {
          skill: skill.name,
