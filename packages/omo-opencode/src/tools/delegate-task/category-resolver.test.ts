...
describe("resolveCategoryExecution", () => {
  ...
  beforeEach(() => {
    ...
    hasConnectedProvidersSpy = spyOn(
      connectedProvidersCache,
      "hasConnectedProvidersCache",
    ).mockReturnValue(false);
    hasProviderModelsSpy = spyOn(
      connectedProvidersCache,
  ...
  test.skip("promotes object-style fallback model settings to categoryModel when fallback becomes initial model", async () => {
    //#given
    const cacheSpy = spyOn(connectedProvidersCache, "readProviderModelsCache")
      .mockReturnValue({
  ...
  test.skip("does not inherit hardcoded fallbackChain when user configures a category model [regression #3040]", async () => {
    ...
    expect(result.actualModel).toBe(
      "animal-gateway-xai/grok-4-fast-non-reasoning",
    );
    expect(result.categoryModel).toEqual({
  ...
  test("routes non-family models to the legacy deep append", async () => {
    ...
    expect(result.categoryPromptAppend).not.toBe(
      DEEP_CATEGORY_PROMPT_APPEND_GPT_5_5,
    );
  });
  ...
  test("applyCategoryParams propagates category tools config (issue #5182)", () => {
    ...
    expect((result as unknown as { tools?: Record<string, boolean> }).tools)
      .toEqual({ grep: false, read: true });
  });
});
