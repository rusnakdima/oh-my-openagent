...
function buildCategoryRecord<TValue>(
  ...
  return Object.fromEntries(
    BUILTIN_CATEGORIES.map((
      definition,
    ) => [definition.name, selector(definition)]),
  );
}
...
export const CATEGORY_CALLER_GUIDANCE: Record<string, string | undefined> =
  buildCategoryRecord(
    (definition) => definition.callerGuidance,
  );

export const CATEGORY_PROMPT_APPEND_RESOLVERS: Record<
  string,
...
export const BUILTIN_CATEGORY_REQUIRES_MODEL: Record<string, string> = Object
  .fromEntries(
    BUILTIN_CATEGORIES
