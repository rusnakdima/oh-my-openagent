...
describe("fetchSyncResult", () => {
  ...
  test("deliverableTag: returns tagged turn even when a newer untagged turn exists", async () => {
    ...
    //#when
    const result = await fetchSyncResult(mockClient, "ses_test", undefined, {
      deliverableTag: "plan",
    });
  ...
  test("strict abort recovery: requires latest assistant text output", async () => {
    ...
    //#when
    const result = await fetchSyncResult(mockClient, "ses_test", 0, {
      strictAbortRecovery: true,
    });
          ...
          data: [
            { info: { id: "msg_001", role: "user", time: { created: 1000 } } },
            {
              ...
              parts: [
                {
                  type: "reasoning",
                  text: "<plan>internal draft sketch</plan>",
                },
