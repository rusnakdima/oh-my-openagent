...
function isInternalAllCompleteWake(message: SessionMessage): boolean {
  return isTerminalNoReplyUserMessage(message) &&
    getTextParts(message).includes(ALL_BACKGROUND_TASKS_COMPLETE_MARKER);
}

function getLastSessionTurns(
  ...
  for (let i = messages.length - 1; i >= 0; i--) {
    ...
    if (
      !lastRelevantUser && msg.info?.role === "user" &&
      !isInternalAllCompleteWake(msg)
    ) {
...
export function isSessionComplete(
  ...
  if (
    lastAssistant.parts?.some((part) =>
      part.type && PENDING_TOOL_PART_TYPES.has(part.type)
    )
  ) return false;
...
export function getTerminalSessionError(
  messages: readonly SessionMessage[],
): string | null {
  const { lastAssistant, lastRelevantUser } = getLastSessionTurns(messages);
  if (
    lastRelevantUser?.info?.id && lastAssistant?.info?.id &&
    lastAssistant.info.id <= lastRelevantUser.info.id
  ) {
    return null;
  }
