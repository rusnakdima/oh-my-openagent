...
function isUnexpectedEofError(error: unknown): boolean {
  ...
  return lowered.includes("unexpected eof") ||
...
export async function sendSyncPrompt(
  ...
  const loweredReasoning = applySessionPromptParams(
    input.sessionID,
    input.categoryModel,
  );
  const promptModel = input.categoryModel?.reasoning !== undefined
    ? {
      ...input.categoryModel,
      reasoningEffort: loweredReasoning.reasoningEffort,
    }
    : input.categoryModel;
  ...
  try {
    await deps.promptWithModelSuggestionRetry(
      client,
      routePromptRetry(promptArgs, input.directory),
      {
        queueBehavior: "defer",
        checkStatus: false,
    ...
    const errorMessage = promptError instanceof Error
      ...
      : String(promptError);
    if (
      errorMessage.includes("agent.name") || errorMessage.includes("undefined")
    ) {
