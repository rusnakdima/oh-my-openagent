...
export async function registerSyncSessionSideEffects(input: {
  ...
  registerDelegatedChildSessionBootstrap({
    ...
    promptText: buildTaskPrompt(
      input.args.prompt,
      input.agentToUse,
      input.executorCtx.sisyphusAgentConfig?.tdd,
    ),
    ...
    modelFallbackControllerAccessor:
      input.executorCtx.modelFallbackControllerAccessor,
  });
...
export function cleanupSyncSessionSideEffects(
  ...
  executorCtx.modelFallbackControllerAccessor?.clearSessionFallbackChain(
    sessionID,
  );
