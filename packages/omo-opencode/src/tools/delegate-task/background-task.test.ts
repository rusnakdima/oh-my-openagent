...
const { SessionCategoryRegistry } = require(
  "../../shared/session-category-registry",
);

describeFn(
  "executeBackgroundTask output/session metadata compatibility",
  () => {
    beforeEachFn(() => {
    ...
    testFn(
      "does not emit synthetic pending session metadata when session id is unresolved",
      async () => {
        ...
        const result = await executeBackgroundTask(
          ...
          },
          {
            ...
            callID: "call_1",
            metadata: async (value: { metadata: Record<string, unknown> }) =>
    ...
    testFn(
      "emits task metadata session_id when real session id is available",
      async () => {
        ...
        expectFn(metadataCalls[0].metadata.backgroundTaskId).toBe(
          "bg_resolved",
        );
      },
    );
    ...
    testFn(
      "keeps continuation taskId out of visible background metadata",
      async () => {
    ...
    testFn(
      "captures late-resolved session id and emits synced metadata",
      async () => {
    ...
    testFn(
      "passes question-deny session permission when launching delegate task",
      async () => {
    ...
    testFn(
      "strips leading zwsp from agent name before launching background task",
      async () => {
        ...
        //#then
        expectFn(launchCalls).toHaveLength(1);
        expectFn((launchCalls[0] as { agent: string }).agent).toBe(
          "sisyphus-junior",
        );
      },
    );
    ...
    testFn(
      "keeps launched background task alive when parent aborts before session id resolves",
      async () => {
    ...
    testFn(
      "registers late session category even when parent aborts before session id resolves",
      async () => {
        ...
        expectFn(result).toContain("Background task launched");
        expectFn(SessionCategoryRegistry.get("ses_abort_category")).toBe(
          "quick",
        );
      },
    );
    ...
    testFn(
      "prefers child terminal status over parent abort while waiting for session id",
      async () => {
    ...
    testFn(
      "reports failure when manager marks task as error during session startup",
      async () => {
          ...
          getTask: () => {
            ...
            if (reads >= 2) {
              return {
                ...
                status: "error",
                error: "crash between session creation and prompt send",
              };
            }
            return { sessionId: undefined, status: "pending" };
          },
    ...
    testFn(
      "keeps sibling background launch alive when two tasks start concurrently",
      async () => {
    ...
    testFn(
      "strips legacy ZWSP-prefixed agent names from persisted background task launch input (GH-3259)",
      async () => {
