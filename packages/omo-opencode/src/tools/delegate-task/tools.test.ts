...
const { describe, test, expect, beforeEach, afterEach, spyOn, mock } = require(
  "bun:test",
);
import {
...
const TEST_CONNECTED_PROVIDERS = [
  "anthropic",
  "google",
  "openai",
  "kimi-for-coding",
...
type DelegateTaskArgsWithSerializedSkills =
  & Omit<DelegateTaskArgs, "load_skills">
  & {
    load_skills: string;
  };
...
type ResolvedCategoryConfig = NonNullable<
  ReturnType<typeof resolveCategoryConfig>
>;

function expectResolvedCategoryConfig(
  result: ReturnType<typeof resolveCategoryConfig>,
...
function requireResolvedCategoryConfig(
  result: ReturnType<typeof resolveCategoryConfig>,
): ResolvedCategoryConfig {
  if (result === null) {
    throw new Error("Expected category config to resolve");
...
function requireCapturedLaunchInput(
  value: CapturedLaunchInput | undefined,
): CapturedLaunchInput {
  if (value === undefined) {
...
describe("sisyphus-task", () => {
  ...
  beforeEach(() => {
    ...
    cacheSpy = spyOn(connectedProvidersCache, "readConnectedProvidersCache")
      .mockReturnValue(["anthropic", "google", "openai", "kimi-for-coding"]);
  ...
  describe("load_skills parsing", () => {
    test("parses valid JSON string into array before validation", async () => {
      ...
      const mockClient = {
        ...
        model: {
          list: async () => ({ data: [{ provider: "openai", id: "gpt-5.5" }] }),
        },
        session: {
      ...
      const resolveSkillContentSpy = spyOn(executor, "resolveSkillContent")
        .mockImplementation(async (_skills, options) => {
    ...
    test.skip(
      ...
      async () => {
        ...
        const mockClient = {
          ...
          model: {
            list: async () => ({
              data: [{ provider: "openai", id: "gpt-5.5" }],
            }),
          },
          session: {
            create: async () => ({ data: { id: "test-session" } }),
        ...
        // then
        expect(requireCapturedLaunchInput(launchInput).agent).toBe(
          "Sisyphus-Junior",
        );
      },
      { timeout: 10000 },
  ...
  describe("background metadata sessionId", () => {
    test("should wait for background sessionId and set metadata for TUI toolcall counting", async () => {
      ...
      const sessionIdCall = metadataCalls.find((c) =>
        c.metadata?.sessionId === "ses_child"
      );
  ...
  describe("category variant", () => {
    ...
    test.skip(
      ...
      async () => {
        ...
        const mockClient = {
          ...
          model: {
            list:
              async () => [{ provider: "anthropic", id: "claude-opus-4-8" }],
          },
          session: {
            create: async () => ({ data: { id: "test-session" } }),
            prompt: async () => ({ data: {} }),
        ...
        const tool = createDelegateTask({
          ...
          availableModelsOverride: createTestAvailableModels(),
          userCategories: {
            "unspecified-high": { model: "kimi-for-coding/k3", variant: "max" },
          },
        });
    ...
    test.skip(
      "DEFAULT_CATEGORIES explicit high model passes to sync prompt request WITHOUT userCategories",
      async () => {
  ...
  describe("skills parameter", () => {
    test(
      "#given load_skills omitted #when executing #then defaults to [] and proceeds (fixes #4119)",
      async () => {
        // given
        const { createDelegateTask } = require("./tools");
        let promptBody: CapturedPromptBody = {};
  ...
  describe.skip("run_in_background parameter", () => {
    test(
      "#given category without run_in_background #when executing #then defaults to sync and proceeds (fixes #4119)",
      async () => {
        // given
        const { createDelegateTask } = require("./tools");
        let promptBody: CapturedPromptBody = {};
    ...
    test(
      "#given subagent_type without run_in_background #when executing #then defaults to sync and proceeds (fixes #4119)",
      async () => {
        // given
        const { createDelegateTask } = require("./tools");
        let promptBody: CapturedPromptBody = {};
    ...
    test(
      "#given task_id without run_in_background #when executing #then defaults to sync continuation (fixes #4119)",
      async () => {
            ...
            messages: async (input: { path: { id: string } }) => {
              ...
              return {
                data: [{
                  ...
                  parts: [{
                    type: "text",
                    text: "sync-continuation-result-sentinel",
                  }],
                }],
              };
            },
    ...
    test(
      ...
      async () => {
        ...
        const taskStates = new Map([
          ["bg_tool_first", {
            reads: 0,
            abortOnFirstRead: true,
            sessionId: "ses_tool_first",
          }],
          ["bg_tool_second", {
            reads: 0,
            abortOnFirstRead: false,
            sessionId: "ses_tool_second",
          }],
        ]);
        let launchCount = 0;
        const mockManager = {
              ...
              return {
                data: [
                  ...
                  },
                  {
                    ...
                    parts: [{
                      type: "text",
                      text: "continued-task-result-sentinel",
                    }],
                  },
                ],
              };
  ...
  describe.skip("sync mode new task (run_in_background=false)", () => {
    ...
    test(
      "#given sync prompt returns ambiguous EOF #when sync task runs #then it waits for the accepted session result",
      async () => {
        // given
        const { createDelegateTask } = require("./tools");
        let promptCalls = 0;
            ...
            messages: async () => ({
              data: [
                ...
                },
                {
                  ...
                  parts: [{
                    type: "text",
                    text: "accepted-despite-eof-sentinel",
                  }],
                },
              ],
            }),
            status: async () => ({
              data: { ses_sync_ambiguous_eof: { type: "idle" } },
            }),
            abort: async () => ({}),
          },
          config: {
  ...
  describe.skip("unstable agent forced background mode", () => {
              ...
              data: [
                {
                  ...
                  parts: [{
                    type: "text",
                    text: "gemini-forced-bg-result-sentinel",
                  }],
                },
              ],
            }),
              ...
              data: [
                {
                  ...
                  parts: [{
                    type: "text",
                    text: "minimax-forced-bg-result-sentinel",
                  }],
                },
              ],
            }),
    ...
    test(
      "user gemini category with run_in_background=false should force background but wait for result",
      async () => {
              ...
              data: [
                {
                  ...
                  parts: [{
                    type: "text",
                    text: "artistry-forced-bg-result-sentinel",
                  }],
                },
              ],
            }),
              ...
              data: [
                {
                  ...
                  parts: [{
                    type: "text",
                    text: "custom-unstable-result-sentinel",
                  }],
                },
              ],
            }),
  ...
  describe.skip("delegate task with short skill name", () => {
    ...
    test("resolves short named discovered skill and flows content into prompt", async () => {
      ...
      const unique = `delegate-shortname-${Date.now()}-${
  ...
  describe("buildSystemContent", () => {
    ...
    test("combines plan agent prepend with skill content", () => {
      ...
      expect(systemContent.indexOf(planPrepend)).toBeLessThan(
        systemContent.indexOf(skillContent),
      );
    });
      ...
      const mockClient = {
        ...
        session: {
          ...
          messages: async () => ({
            data: [{
              info: { role: "assistant" },
              parts: [{ type: "text", text: "plan-created-sentinel" }],
            }],
          }),
  ...
  describe("subagent_type model extraction (issue #1225)", () => {
    ...
    test.skip(
      "sync mode passes matched agent model to prompt request",
      async () => {
  ...
  describe("subagent task permission", () => {
    ...
    test(
      ...
      async () => {
        ...
        expect(result).toContain(
          'Cannot delegate to coordinator agent "prometheus" via task()',
        );
      },
      { timeout: 20000 },
    );
