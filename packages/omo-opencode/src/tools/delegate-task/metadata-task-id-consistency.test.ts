...
function makeMockCtx(): ToolContextWithMetadata & {
  ...
  return {
    ...
    metadata: async (
      ...
      captured.push(input as CapturedMetadata);
    },
    captured,
  };
}
...
describe("taskId and backgroundTaskId metadata consistency", () => {
  describe("#given sync-task runs", () => {
    test("#when publishing metadata #then taskId equals sessionId", async () => {
      ...
      await executeSyncTask(
        ...
        {
          client: {
            session: { create: async () => ({ data: { id: "ses_sync" } }) },
          },
          directory: "/tmp",
      ...
      const meta = ctx.captured.find((m: CapturedMetadata) =>
      ...
      await executeUnstableAgentTask(
        ...
        unsafeTestValue({
          ...
          client: {
            session: {
              status: async () => ({
                data: { ses_unstable_xyz: { type: "idle" } },
              }),
              messages: async () => ({
                data: [{
  ...
  describe("#given background-continuation runs", () => {
    ...
    test("#when resumed task has category #then metadata.category equals task.category", async () => {
      ...
      const meta = ctx.captured.find((item: CapturedMetadata) =>
  ...
  describe("#given stock task title metadata contract", () => {
    ...
    test("#when sync continuation publishes metadata #then title equals description without resume prefix", async () => {
      ...
      const ctx = makeMockCtx();
      const args: DelegateTaskArgs = {
        description: "continue sync",
        prompt: "keep going",
        load_skills: [],
        run_in_background: false,
        task_id: "ses_sync_title",
      };
  ...
  describe("#given background_output runs", () => {
    test("#when publishing metadata #then backgroundTaskId is task.id not task_id", async () => {
      const { createBackgroundOutput } = require(
        "../background-task/create-background-output",
      );
      const ctx = makeMockCtx();
      const manager = {
        getTask: (id: string) => ({
      ...
      const bgOutput = createBackgroundOutput(
        unsafeTestValue(manager),
        unsafeTestValue(client),
      );
      await bgOutput.execute(
        unsafeTestValue({ task_id: "bg_output_xyz" }),
        unsafeTestValue(ctx),
      );

      const meta = ctx.captured.find((m: CapturedMetadata) =>
        m.metadata?.backgroundTaskId
      )!;
      expect(meta).toBeDefined();
