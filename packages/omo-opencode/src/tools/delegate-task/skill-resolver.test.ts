...
function makeNativeSkill(name: string, description: string, content: string) {
  return {
    name,
    description,
    location: `/fake/native/${name}/SKILL.md`,
    content,
  };
}
...
describe("resolveSkillContent — nativeSkills integration", () => {
  ...
  it("#given a plugin runtime skill reachable through getLoadedSkills #when delegate load_skills resolves #then returns its content without native fallback", async () => {
    const nativeSkills = {
      all: mock(
        () => [
          makeNativeSkill("native-only", "Native only", "NATIVE_ONLY_BODY"),
        ],
      ),
      get: () => undefined,
      dirs: () => [],
    ...
    const result = await resolveSkillContent(["plugin-runtime-skill"], {
      ...
      getLoadedSkills: async () => [
        makeLoadedSkill("plugin-runtime-skill", "PLUGIN_RUNTIME_BODY"),
      ],
    });
