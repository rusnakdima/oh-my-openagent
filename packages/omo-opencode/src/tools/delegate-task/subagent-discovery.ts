...
export function mergeWithClaudeCodeAgents(
  ...
  const toAgentInfoList = (
    record: Record<
      string,
      { mode?: string; hidden?: boolean; model?: AgentInfo["model"] }
    >,
  ): AgentInfo[] =>
...
function matchesRequestedAgent(
  ...
  const listedAgentName = stripAgentListSortPrefix(agent.name).trim()
    .toLowerCase();
  const listedAgentConfigKey = getAgentConfigKey(agent.name).trim()
    .toLowerCase();
...
export function findPrimaryAgentMatch(
  ...
  return agents.find((agent) =>
    agent.mode === "primary" && matchesRequestedAgent(agent, requestedAgentName)
  );
}
...
export function findCallableAgentMatch(
  ...
  return agents.find((agent) =>
    isTaskCallableAgentMode(agent.mode) && isVisibleToTask(agent) &&
