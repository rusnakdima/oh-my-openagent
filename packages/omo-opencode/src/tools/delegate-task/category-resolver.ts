...
export async function resolveCategoryExecution(
  ...
  const systemDefaultModel = systemDefaultModelMaybe ??
    systemDefaultModelOrInherited;
  const {
    client,
    userCategories,
    sisyphusJuniorModel,
  ...
  const availableModels = await getAvailableModelsForDelegateTask(
    client,
    availableModelsOverride,
  );
  ...
  if (!resolved) {
    ...
    const requiredModel = requirement?.requiresModel ??
    ...
    if (configuredModels && availableModels.size > 0) {
      const configuredChain = configuredModels.map((entry) =>
        getConfiguredModel(entry)
      ).join(" -> ");
      return categoryResolutionError(
        `Configured model chain is unavailable for category "${categoryName}": ${configuredChain}`,
      );
    }

    if (categoryExists && requiredModel) {
      return categoryResolutionError(
        `Category "${categoryName}" requires model "${requiredModel}" which is not available.
...
Available categories: ${allCategoryNames}`,
  ...
  const categoryResolvedModel = hasCanonicalModels
  ...
  if (canonicalChain && canonicalChain.length > 0) {
    ...
    if (!systemDefaultModel) {
      ...
      return categoryResolutionError(
        `No model selected in TUI for category "${args.category}".
...
Available categories: ${categoryNames.join(", ")}`,
    ...
    if (parsedModel) {
      ...
      categoryModel = applyCategoryParams({
        ...parsedModel,
        variant: variantToUse ?? parsedModel.variant,
      }, resolved.config);
      modelInfo = {
    ...
    if (systemDefaultModel) {
      ...
      if (resolution && "skipped" in resolution) {
        ...
        const parsedModel = parseModelString(actualModel);
        const variantToUse = userCategories?.[args.category!]?.variant ??
          resolvedVariant ?? resolved.config.variant;
...
Available categories: ${categoryNames.join(", ")}`,
  ...
  if (!categoryModel && !actualModel) {
    ...
    const categoryNames = Object.keys(enabledCategories);
    return categoryResolutionError(
      `No model available for category "${args.category}". Select a model in the TUI picker first.
Available categories: ${categoryNames.join(", ")}`,
    );
  }
