...
The `task` tool implementation delegates work to subagents via background or
sync sessions. It resolves categories, models, skills, and manages both async
and synchronous execution flows. Harness-neutral task selection and retry
primitives are extracted to
[`packages/delegate-core/`](../../../../../packages/delegate-core); this
directory keeps OpenCode session/tool execution wiring. Built-in categories are
defined alongside user category overrides.
...
Each file handles one step. `sync-continuation.ts` handles session continuation
(resume with task_id).
...
`background-continuation.ts` handles `task_id` resume for existing background
tasks.
...
`model-string-parser.ts` handles `"model variant"` format (e.g.,
`"gpt-5.5 medium"` → model=`gpt-5.5`, variant=`medium`).

## UNSTABLE AGENT TRACKING

`unstable-agent-task.ts` marks tasks from categories/agents known to be unstable
(e.g., free models). Enables `unstableAgentBabysitter` hook monitoring.
