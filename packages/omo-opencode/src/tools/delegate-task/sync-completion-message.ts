...
function formatModelID(
  model: DelegatedModelConfig | ParentContext["model"] | undefined,
): string | undefined {
...
export function buildSyncTaskCompletion(input: {
  ...
  if (actualModelStr && parentModelStr && actualModelStr !== parentModelStr) {
    modelRoutingNote =
      `\n⚠️  Model routing: parent used ${parentModelStr}, this subagent used ${actualModelStr} (via category: ${
        input.args.category ?? "unknown"
      })`;
  } else if (actualModelStr) {
    modelRoutingNote = `\nModel: ${actualModelStr}${
      input.args.category ? ` (category: ${input.args.category})` : ""
    }`;
  }
