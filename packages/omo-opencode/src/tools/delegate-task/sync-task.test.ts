...
type AddTaskArg = Parameters<
  import("../../features/task-toast-manager/manager").TaskToastManager[
    "addTask"
  ]
>[0];
...
describe("executeSyncTask - cleanup on error paths", () => {
  ...
  beforeEach(() => {
    ...
    deleteCalls = [];
    addCalls = [];
    const { clearAllDelegatedChildSessionBootstrap } = require(
      "../../shared/delegated-child-session-bootstrap",
    );
    clearAllDelegatedChildSessionBootstrap();

    clearRequireCache("./sync-task");

    const { initTaskToastManager, _resetTaskToastManagerForTesting } = require(
      "../../features/task-toast-manager/manager",
    );
    ...
    const { subagentSessions } = require(
  ...
  test("does not recover from non-abort poll error containing abort-like words", async () => {
    ...
    const deps = {
      ...
      pollSyncSession: async () =>
        "Task aborted: subagent exceeded 5 assistant turns without completing",
      fetchSyncResult: async () => {
  ...
  test("#given fallback chain exhausted #when all retries fail #then returns final error", async () => {
    ...
    const deps = {
      ...
      sendSyncPrompt: async (
        ...
        attemptedModels.push(input.categoryModel);
        return promptErrors[attemptedModels.length - 1] ??
          "Unexpected extra retry";
      },
      pollSyncSession: async () => null,
      fetchSyncResult: async () => ({
        ok: true as const,
  ...
  test("retries sync session on retryable runtime session error using next fallback model", async () => {
    ...
    const deps = {
      createSyncSession: async () => {
        const sessionID = createdSessions.length === 0
          ? "ses_first"
          : "ses_second";
    ...
    const mockCtx = {
      ...
      metadata: (
        ...
        metadataCalls.push(input as CapturedMetadata);
      },
    };
  ...
  test("#given sync poll hits subscription quota exhaustion #when a fallback chain exists #then retries on the next fallback model without changing generic stop semantics", async () => {
    ...
    const deps = {
      ...
      pollSyncSession: async (
        ...
        return input.sessionID === "ses_quota_primary" ? pollError : null;
      },
      fetchSyncResult: async (_client: unknown, sessionID: string) => ({
        ok: true as const,
        textContent: `Result from ${sessionID}`,
      }),
      isProviderExhaustionFallbackEligible: (error: { message?: string }) =>
        error.message === pollError,
    };
  ...
  test("registers child-session bootstrap before sync prompt and clears it after completion", async () => {
    ...
    const { executeSyncTask } = require("./sync-task");
    const { getDelegatedChildSessionBootstrap } = require(
      "../../shared/delegated-child-session-bootstrap",
    );
    ...
    expect(getDelegatedChildSessionBootstrap("ses_bootstrap_sync"))
      .toBeUndefined();
  });
  ...
  test("replays sync session side effects for retry-created sessions", async () => {
    ...
    expect(result).toContain("Result from ses_second");
    expect(onSyncSessionCreated.mock.calls.map((call: unknown[]) => call[0]))
      .toEqual([
        {
          sessionID: "ses_first",
    ...
    expect(addTaskCalls.map((task) => task.sessionID)).toEqual([
      "ses_first",
      "ses_second",
    ]);
    expect(addTaskCalls.map((task) => task.id)).toEqual([
      "sync_ses_firs",
      "sync_ses_firs",
    ]);
  });
  ...
  test("depth regression: does not silently fall back to childDepth: 1 when manager methods are present", async () => {
    ...
    const taskMeta = metadataCalls.find((c) =>
      c.metadata?.spawnDepth !== undefined
    );
