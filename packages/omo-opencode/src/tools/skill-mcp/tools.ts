...
export function createSkillMcpTool(
  ...
  return tool({
    description:
      `${SKILL_MCP_DESCRIPTION} Optional cdp_url connects Playwright to a runtime CDP endpoint.`,
    args: {
      mcp_name: tool.schema.string().describe(
        "Name of the MCP server from skill config",
      ),
      tool_name: tool.schema.string().optional().describe("MCP tool to call"),
      resource_name: tool.schema.string().optional().describe(
        "MCP resource URI to read",
      ),
      prompt_name: tool.schema.string().optional().describe(
        "MCP prompt to get",
      ),
