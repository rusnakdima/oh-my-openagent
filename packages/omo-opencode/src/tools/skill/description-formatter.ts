...
function shouldSuppressBuiltinCommandAlias(
  command: CommandInfo,
  skills: SkillInfo[],
): boolean {
  ...
  return skills.some((skill) =>
    normalizeSkillName(skill.name) === normalizedCommandName
  );
}
...
function deduplicateCommandsForPathAliasedSkills(
  ...
  return commands.filter((command) =>
    !shouldSuppressBuiltinCommandAlias(command, skills)
  );
}
...
export function formatCombinedDescription(
  ...
  const availableSkills = options.includeSkills
    ? deduplicatePathAliasedSkills(skills ?? [])
    : [];
  const availableCommands = deduplicateCommandsForPathAliasedSkills(
    commands ?? [],
    availableSkills,
  );
...
Priority: project > user > opencode > builtin/plugin${
    options.includeSkills ? " | Skills listed before commands" : ""
  }
