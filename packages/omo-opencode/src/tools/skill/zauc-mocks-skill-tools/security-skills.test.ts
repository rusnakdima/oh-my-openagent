...
describe("skill tool - bundled security skills", () => {
  it("loads security-research and security-review when the plugin skill context pre-seeds them", async () => {
    const { builtinToLoadedSkill } = await import(
      "../../../features/opencode-skill-loader/merger/builtin-skill-converter"
    );
    const { securityResearchSkill, securityReviewSkill } = await import(
      "../../../features/builtin-skills/skills/index"
    );
