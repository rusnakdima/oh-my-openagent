...
describe("slashcommand command discovery plugin integration", () => {
  ...
  it("discovers marketplace plugin commands and skills as command items", () => {
    ...
    const pluginCommand = commands.find((command) =>
      command.name === "daplug:run-prompt"
    );
    const pluginSkill = commands.find((command) =>
      command.name === "daplug:plugin-plan"
    );
...
`,
    ...
    const ancestorCommand = commands.find((command) =>
      command.name === "ancestor"
    );
...
`,
    ...
    const commands = discoverCommandsSync(projectDir);
    const duplicates = commands.filter((command) =>
...
`,
    ...
    const commands = discoverCommandsSync(projectDir);
    const nestedCommand = commands.find((command) =>
      command.name === "refactor/code"
    );
  ...
  it("keeps builtin start-work routed to Atlas during static discovery", () => {
    ...
    // when
    const commands = discoverCommandsSync(projectDir);
    const startWorkCommand = commands.find((command) =>
      command.name === "start-work"
    );
