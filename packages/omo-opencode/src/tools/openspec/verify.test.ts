...
beforeEach(async () => {
  tmp = join(
    tmpdir(),
    `openspec-verify-test-${Date.now()}-${Math.random().toString(36).slice(2)}`,
  );
...
async function makeSpec(
  ...
  if (files.spec !== undefined) {
    await writeFile(join(dir, "spec.md"), files.spec);
  }
  if (files.plan !== undefined) {
    await writeFile(join(dir, "plan.md"), files.plan);
  }
  if (files.tasks !== undefined) {
    await writeFile(join(dir, "tasks.md"), files.tasks);
  }
  return dir;
}
...
describe("verifySpec", () => {
  ...
  it("marks spec as invalid when spec.md is missing", async () => {
    ...
    expect(results[0].valid).toBe(false);
    expect(results[0].files.find((f) => f.name === "spec.md")?.exists).toBe(
      false,
    );
  });
  ...
  it("marks spec as invalid when plan.md is empty", async () => {
    ...
    expect(results[0].valid).toBe(false);
    expect(results[0].files.find((f) => f.name === "plan.md")?.nonEmpty).toBe(
      false,
    );
  });
  ...
  it("marks spec as invalid when tasks.md is missing", async () => {
    ...
    expect(results[0].valid).toBe(false);
    expect(results[0].files.find((f) => f.name === "tasks.md")?.exists).toBe(
      false,
    );
  });
  ...
  it("returns task stats for valid specs", async () => {
    ...
    expect(results[0].taskStats).toEqual({
      open: 1,
      in_progress: 1,
      completed: 1,
      blocked: 0,
  ...
  it("verifies all specs when specName is omitted", async () => {
    const { verifySpec } = await import("./verify");
    await makeSpec("spec-a", {
      spec: "# A",
      plan: "# A",
      tasks: "| [ ] | A |",
    });
    await makeSpec("spec-b", {
      spec: "# B",
      plan: "# B",
      tasks: "| [x] | B |",
    });
