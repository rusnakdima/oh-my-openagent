...
export function createHashlineEditTool(ctx?: PluginContext): ToolDefinition {
  return tool({
    ...
    args: {
      ...
      delete: tool.schema.boolean().optional().describe(
        "Delete file instead of editing",
      ),
      rename: tool.schema.string().optional().describe(
      ...
      edits: tool.schema
        .array(
          tool.schema.object({
            ...
            pos: tool.schema.string().optional().describe(
              "Primary anchor in LINE#ID format",
            ),
            end: tool.schema.string().optional().describe(
            ...
            lines: tool.schema
              ...
              .describe(
                "Replacement or inserted lines as newline-delimited string. null deletes with replace",
              ),
          }),
        )
