...
24 files. Implements the `hashline_edit` tool — hash-anchored file editing where
every line reference includes a content hash (`LINE#ID`). Validates hashes
before applying edits, rejecting stale references.
...
`lines: null` or `lines: []` with `replace` = delete. `delete: true` at tool
level = delete file.
