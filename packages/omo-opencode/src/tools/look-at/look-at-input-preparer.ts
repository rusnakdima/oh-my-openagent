...
function getTemporaryConversionPath(error: unknown): string | null {
  ...
  if (
    typeof temporaryOutputPath === "string" && temporaryOutputPath.length > 0
  ) {
...
function readJsonTextPart(filePath: string): ReadJsonTextPartResult {
  ...
  try {
    ...
    return {
      ok: false,
      error: `Error: Failed to read JSON file ${filePath}: ${message}`,
    };
  }
  return {
    ok: true,
...
export function prepareLookAtInput(args: LookAtArgs): PrepareLookAtInputResult {
  ...
  const imageDataList = args.image_data_list ??
  ...
  for (const filePath of filePaths) {
    ...
    if (needsConversion(mimeType)) {
      ...
      try {
        ...
        const failedConversionPath = getTemporaryConversionPath(
          conversionError,
        );
    ...
    if (needsConversion(mimeType)) {
      ...
      try {
        ...
        return {
          ok: false,
          error:
            `Error: Failed to convert Base64 image format. ${conversionError}`,
        };
      }
    }
