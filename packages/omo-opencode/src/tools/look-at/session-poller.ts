...
async function getSessionStatus(client: Client, sessionID: string): Promise<{
  ...
  try {
    ...
    if (statusResult.error) {
      log(
        `[look_at] session.status returned error (falling back to messages):`,
        statusResult.error,
      );
      return { supported: false, type: null };
    }
...
export async function waitForLookAtSessionResult(
  ...
  { messages: unknown[]; outcome: AssistantOutcome; statusType: string | null }
> {
  ...
  while (Date.now() - startTime < timeout) {
    ...
    const supportedButNeverSeen = status.supported && statusType === null &&
      !hasEverSeenSessionInStatus &&
      !options?.allowStableIdleWithoutActivity;
    const isActive = supportedButNeverSeen ||
      (statusType !== null && !isTerminal);
    const { messages, error: messagesError } = await getSessionMessages(
      client,
      sessionID,
    );
    ...
    if (isActive) {
      ...
      stableIdlePolls = currentMessageCount === lastIdleMessageCount
        ? stableIdlePolls + 1
        : 1;
      ...
      const canConcludeIdle = sawNonIdleStatus ||
        !status.supported ||
        (
          Boolean(options?.allowStableIdleWithoutActivity) &&
          (outcome.hasAssistant ||
            Boolean(options?.allowEmptyStableIdleWithoutActivity))
        );
