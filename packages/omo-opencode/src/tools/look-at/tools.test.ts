...
describe("look-at tool", () => {
              ...
              data: [
                {
                  ...
                  parts: [{
                    type: "text",
                    text: "eof-ambiguity-result-sentinel",
                  }],
                },
              ],
            };
  ...
  describe("createLookAt unhandled error resilience", () => {
    ...
    // then returns error string instead of crashing
    test(
      "catches session.messages throw and returns error string",
      async () => {
        let statusCalls = 0;
        const mockClient = {
          app: {
          ...
          session: {
            ...
            status: async () => {
              ...
              return {
                data: {
                  ses_msg_throw: { type: statusCalls <= 1 ? "busy" : "idle" },
                },
              };
            },
            messages: async () => {
              throw new Error("unexpected-server-error-sentinel");
            },
            abort: async () => ({ data: {} }),
          },
        };
    ...
    test("handles non-Error thrown objects gracefully", async () => {
      const mockClient = {
        session: {
          get: async () => ({ data: { directory: "/project" } }),
          create: async () => {
            throw "string-error-thrown-sentinel";
          },
        },
      };
    ...
    test("sends JSON files as text instead of unsupported application/json file parts", async () => {
      ...
      try {
        ...
        expect(
          promptBody?.parts.some((part) =>
            part.type === "file" && part.mime === "application/json"
          ),
        ).toBe(false);
        expect(
          promptBody?.parts.some((part) =>
            part.type === "text" && part.text?.includes('{"hello":"world"}')
          ),
        ).toBe(true);
      } finally {
  ...
  describe("createLookAt with image_data", () => {
    ...
    test("sends data URL when image_data provided", async () => {
      ...
      const filePart = promptBody.parts.find((p: LookAtPart) =>
        p.type === "file"
      )!;
  ...
  describe("createLookAt prompt conditional on Read availability", () => {
    const captureLastPromptBody = () => {
      const captured: { body: LookAtPromptBody } = {
        body: undefined as unknown as LookAtPromptBody,
      };
      const mockClient = {
        app: {
          agents: async () => ({ data: [] }),
  ...
  describe("createLookAt multi-file processing", () => {
    test("schema exposes file_paths as an optional string array", () => {
      const lookAtTool = createLookAt(
        { client: {}, directory: "/project" } as never,
      );
      const filePathsSchema = lookAtTool.args.file_paths;
      const filePathsDef =
        typeof filePathsSchema === "object" && filePathsSchema !== null
          ? Reflect.get(filePathsSchema, "def")
          : undefined;
      const filePathsInnerType =
        typeof filePathsDef === "object" && filePathsDef !== null
          ? Reflect.get(filePathsDef, "innerType")
          : undefined;
      const filePathsInnerDef =
        typeof filePathsInnerType === "object" && filePathsInnerType !== null
          ? Reflect.get(filePathsInnerType, "def")
          : undefined;
      const filePathsElement =
        typeof filePathsInnerDef === "object" && filePathsInnerDef !== null
          ? Reflect.get(filePathsInnerDef, "element")
          : undefined;
      const filePathsElementDef =
        typeof filePathsElement === "object" && filePathsElement !== null
    ...
    test("schema exposes image_data_list as an optional string array", () => {
      ...
      const imageDataListDef =
        typeof imageDataListSchema === "object" && imageDataListSchema !== null
          ? Reflect.get(imageDataListSchema, "def")
          : undefined;
      const imageDataListInnerType =
        typeof imageDataListDef === "object" && imageDataListDef !== null
          ? Reflect.get(imageDataListDef, "innerType")
          : undefined;
      const imageDataListInnerDef =
        typeof imageDataListInnerType === "object" &&
          imageDataListInnerType !== null
          ? Reflect.get(imageDataListInnerType, "def")
          : undefined;
      const imageDataListElement = typeof imageDataListInnerDef === "object" &&
          imageDataListInnerDef !== null
        ? Reflect.get(imageDataListInnerDef, "element")
        : undefined;
      const imageDataListElementDef =
        typeof imageDataListElement === "object" &&
          imageDataListElement !== null
      ...
      expect(Reflect.get(imageDataListInnerDef as object, "type")).toBe(
        "array",
      );
      expect(Reflect.get(imageDataListElementDef as object, "type")).toBe(
        "string",
      );
    });
    ...
    test("builds two file parts and plural prompt text for multi-file paths", async () => {
      ...
      const fileParts = promptBody!.parts.filter((part) =>
        part.type === "file"
      );

      expect(fileParts).toHaveLength(2);
      expect(fileParts.map((part) => part.filename)).toEqual([
        "first.png",
        "second.jpg",
      ]);
    });
