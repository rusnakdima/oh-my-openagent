...
export function buildLookAtPrompt(
  ...
  const labels = isPlural
    ? `\n\nAttached files/images:\n${
      fileParts
        .map((filePart, index) =>
          `File ${index + 1}: ${sanitizeFilename(filePart.filename)}`
        )
        .join("\n")
    }`
    : "";
  const sourceClause = READ_ENABLED
    ...
    : `The attached ${subjectNoun} ${
      isPlural ? "are" : "is"
    } already included in this message. Analyze ${pronoun} directly from the attachment. Do NOT attempt to load by path — the ${subjectNoun} cannot be loaded by path.`;

  return `Analyze ${
    isPlural ? "these files/images" : "this file/image"
  } and extract the requested information.${labels}
