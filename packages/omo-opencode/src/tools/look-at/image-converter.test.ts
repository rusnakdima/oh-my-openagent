...
describe("image-converter command execution safety", () => {
  ...
  test("uses execFileSync with argument arrays for conversion commands", async () => {
    ...
    const [firstCommand, firstArgs] = execFileSyncSpy.mock.calls[0] as [
      string,
      string[],
    ];
  ...
  test("removes temporary conversion directory during cleanup", async () => {
    ...
    writeFileSync(inputPath, "fake-heic-data");
    const { convertImageToJpeg, cleanupConvertedImage } =
      await loadImageConverter();
  ...
  test("uses magick command on non-darwin platforms to avoid convert.exe collision", async () => {
    await withMockPlatform("linux", async () => {
      const testDir = mkdtempSync(
        join(tmpdir(), "img-converter-platform-test-"),
      );
      const inputPath = join(testDir, "photo.heic");
      writeFileSync(inputPath, "fake-heic-data");
      const { convertImageToJpeg, cleanupConvertedImage } =
      ...
      const [command, args] = execFileSyncSpy.mock.calls[0] as [
        string,
        string[],
      ];
  ...
  test("applies timeout when executing conversion commands", async () => {
    ...
    const options = execFileSyncSpy.mock.calls[0]?.[2] as
      | { timeout?: number }
      | undefined;
  ...
  test("attaches temporary output path to conversion errors", async () => {
    await withMockPlatform("linux", async () => {
      const testDir = mkdtempSync(
        join(tmpdir(), "img-converter-failure-test-"),
      );
      const inputPath = join(testDir, "photo.heic");
      writeFileSync(inputPath, "fake-heic-data");
      const { convertImageToJpeg } = await loadImageConverter();
      ...
      try {
        ...
        expect(
          typeof temporaryOutputPath === "string" &&
            temporaryOutputPath.endsWith("converted.jpg"),
        ).toBe(true);
      }
  ...
  test("preserves conversion error when temporary output cleanup fails", async () => {
    await withMockPlatform("linux", async () => {
      const testDir = mkdtempSync(
        join(tmpdir(), "img-converter-cleanup-failure-test-"),
      );
      const inputPath = join(testDir, "photo.heic");
