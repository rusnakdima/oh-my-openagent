...
export function normalizeArgs(args: LookAtArgsWithAlias): LookAtArgs {
  ...
  const imageDataListFromSingular = !args.image_data_list &&
    hasNonEmptyString(imageData);

  const normalized: NormalizedLookAtArgs = {
    file_path: filePath,
    file_paths: args.file_paths ??
      (filePathsFromSingular ? [filePath] : undefined),
    image_data: imageData,
    image_data_list: args.image_data_list ??
      (imageDataListFromSingular ? [imageData] : undefined),
    goal: args.goal ?? "",
    _normalized_file_paths_from_singular: filePathsFromSingular || undefined,
    _normalized_image_data_list_from_singular: imageDataListFromSingular ||
      undefined,
  };
...
export function validateArgs(args: LookAtArgs): string | null {
  ...
  const filePathsFromSingular =
    normalizedArgs._normalized_file_paths_from_singular === true;
  const imageDataListFromSingular =
    normalizedArgs._normalized_image_data_list_from_singular === true;
