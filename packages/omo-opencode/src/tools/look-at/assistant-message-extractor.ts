    ...
    info: isObject(info)
      ? {
        ...
        time: isObject(info["time"])
          ? {
            created: typeof info["time"]["created"] === "number"
              ? info["time"]["created"]
              : undefined,
          }
          : undefined,
...
function normalizeThinkingText(text: string): string | null {
  const answerMatches = [
    ...text.matchAll(/<answer\b[^>]*>([\s\S]*?)<\/answer>/gi),
  ]
    .map((match) => match[1]?.trim())
    .filter((value): value is string => Boolean(value));
  const withoutThinking = text.replace(/<think\b[^>]*>[\s\S]*?<\/think>/gi, "")
    .trim();
  const normalized = answerMatches.length > 0
    ? answerMatches.join("\n")
    : withoutThinking;
...
function getTextParts(message: SessionMessage): MessagePart[] {
  ...
  return message.parts
    ...
    .map((part) => ({
      ...
      reasoning: typeof part["reasoning"] === "string"
        ? part["reasoning"]
        : undefined,
      reasoningContent: typeof part["reasoning_content"] === "string"
...
function extractTextFromParts(parts: MessagePart[]): string | null {
  ...
  for (const part of parts) {
    ...
    const reasoningText = asText(part.reasoningContent) ??
      asText(part.reasoning);
    if (reasoningText) reasoningCandidates.push(reasoningText);
  }

  const primaryText = textCandidates.map(normalizeThinkingText).filter((
    text,
  ): text is string => text !== null).join("\n");
  if (primaryText) return primaryText;

  return reasoningCandidates.map(normalizeThinkingText).filter((
    text,
  ): text is string => text !== null).join("\n") || null;
}
...
export function extractLatestAssistantOutcome(
  messages: unknown,
): AssistantOutcome {
  ...
  const allParts = Array.isArray(lastAssistantMessage.parts)
  ...
  const errorName = errorPart && typeof errorPart["error"] === "string"
    ? errorPart["error"]
    : null;
