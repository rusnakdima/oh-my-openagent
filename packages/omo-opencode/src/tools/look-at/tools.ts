...
export function createLookAt(ctx: PluginInput): ToolDefinition {
  return tool({
    ...
    args: {
      file_path: tool.schema.string().optional().describe(
        "Absolute path to the file to analyze",
      ),
      file_paths: tool.schema.array(tool.schema.string()).optional().describe(
        "Absolute paths to the files to analyze",
      ),
      image_data: tool.schema.string().optional().describe(
        "Base64 encoded image data (for clipboard/pasted images)",
      ),
      image_data_list: tool.schema.array(tool.schema.string()).optional()
        .describe(
          "Base64 encoded image data entries (for multiple clipboard/pasted images)",
        ),
      goal: tool.schema.string().describe(
        "What specific information to extract from the file",
      ),
    },
    async execute(rawArgs: LookAtArgs, toolContext) {
      ...
      try {
        ...
        if (missingFilePath) {
          log(
            `[look_at] Missing file while analyzing ${sourceDescription}:`,
            error,
          );
        ...
        const errorMessage = error instanceof Error
          ...
          : String(error);
        log(
          `[look_at] Unexpected error analyzing ${sourceDescription}:`,
          error,
        );
