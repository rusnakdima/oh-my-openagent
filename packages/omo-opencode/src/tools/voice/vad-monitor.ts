...
export function createVADMonitor(
  ...
  const inputDevice: string[] = isWindows ? ["-t", "waveaudio"] : ["-d"]; // -d = default input device on Linux/macOS
    ...
    while (stderrBuffer.includes("\n")) {
      ...
      if (line.includes("Samples amplitude")) {
        ...
        if (match) {
          ...
          lastLevel = Number.isNaN(amplitude)
            ? 0
        ...
        if (match) {
          ...
          lastLevel = Number.isNaN(db)
            ? 0
            : Math.round(Math.pow(10, db / 20) * 32767);
        }
      }
      ...
      if (lastLevel < threshold) {
        if (silenceStart === null) {
          ...
          if (!stopped) {
            ...
            log(
              `[voice] VAD detected silence (${silenceDurationMs}ms threshold)`,
            );
            resolveVAD("silence");
          }
        }
  ...
  proc.on("error", (err) => {
    if (!stopped) {
      ...
      log(
        `[voice] VAD monitor unavailable: ${err.message}. Relying on max_duration timeout.`,
      );
