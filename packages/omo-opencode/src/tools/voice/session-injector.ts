...
export async function injectTranscription(
  opts: InjectTranscriptionOptions,
): Promise<void> {
  log(
    `[voice] Injecting transcription into session ${opts.sessionID}: "${
      opts.text.substring(0, 50)
    }..."`,
  );
  ...
  const result = await dispatchInternalPrompt({
    ...
    source: "voice-input",
    dedupeKey: `voice:${opts.sessionID}:${Date.now()}:${
