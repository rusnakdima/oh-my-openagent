...
export function createCloudflareDeepgramProvider(
  config: CloudflareDeepgramConfig,
): STTProvider {
  return {
    ...
    validateConfig() {
      ...
      if (!accountId) {
        return {
          valid: false,
          error:
            "Cloudflare account ID not found. Set CF_ACCOUNT_ID environment variable.",
        };
      }
      if (!apiToken) {
        return {
    ...
    async transcribe(audio: AudioBuffer): Promise<string> {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Cloudflare Workers AI error ${response.status}: ${errorText}`,
        );
      }
