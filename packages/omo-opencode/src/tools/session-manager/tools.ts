...
export function createSessionManagerTools(
  ...
  const session_list: ToolDefinition = tool({
    ...
    args: {
      ...
      from_date: tool.schema.string().optional().describe(
        "Filter sessions from this date (ISO 8601 format)",
      ),
      to_date: tool.schema.string().optional().describe(
        "Filter sessions until this date (ISO 8601 format)",
      ),
      project_path: tool.schema.string().optional().describe(
  ...
  const session_read: ToolDefinition = tool({
    ...
    args: {
      ...
      include_todos: tool.schema.boolean().optional().describe(
        "Include todo list if available (default: false)",
      ),
      include_transcript: tool.schema.boolean().optional().describe(
        "Include transcript log if available (default: false)",
      ),
      limit: tool.schema.number().optional().describe(
        "Maximum number of messages to return (default: all messages)",
      ),
      from_end: tool.schema.boolean().optional().describe(
        "Read messages from the END of the session (default: false). Pass true to get the most-recent / final assistant message in the output. Recommended when you want the result of a completed task.",
      ),
    },
  ...
  const session_search: ToolDefinition = tool({
    ...
    args: {
      ...
      case_sensitive: tool.schema.boolean().optional().describe(
  ...
  const session_tag: ToolDefinition = tool({
    ...
    args: {
      ...
      tags: tool.schema.array(tool.schema.string()).describe(
        "Tags to add, remove, or replace",
      ),
      action: tool.schema.enum(["add", "remove", "replace"]).describe(
        "Tag operation: add (append), remove (delete), or replace (overwrite)",
      ),
    },
    execute: async (args: SessionTagArgs, _context) => {
      try {
        ...
        const result = await resolvedDeps.setSessionTags(
          args.session_id,
          args.tags,
          args.action,
        );
        ...
        const actionPast = args.action === "add"
          ? "Added"
          : args.action === "remove"
          ? "Removed"
          : "Replaced";
        return `${actionPast} tags on session ${args.session_id}: ${
  ...
  const session_branch: ToolDefinition = tool({
    ...
    args: {
      session_id: tool.schema.string().describe(
        "Parent session ID to branch from",
      ),
      title: tool.schema.string().optional().describe(
        "Title for the new branch session",
      ),
      tags: tool.schema.array(tool.schema.string()).optional().describe(
        "Initial tags for the new branch",
      ),
    },
    execute: async (args: SessionBranchArgs, _context) => {
      try {
        const parentMeta = await resolvedDeps.getSessionMetadata(
          args.session_id,
        );
        ...
        const result = await ctx.client.session.create({
          body: {
            ...
            title: args.title ??
              `Branch of ${parentMeta.title ?? args.session_id}`,
            tags: args.tags,
