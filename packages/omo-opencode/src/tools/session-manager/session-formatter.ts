...
export async function formatSessionList(sessionIDs: string[]): Promise<string> {
  ...
  const infos = (await Promise.all(sessionIDs.map((id) => getSessionInfo(id))))
    .filter(
  ...
  for (const msg of messages) {
    ...
    for (const part of msg.parts) {
      if (part.type === "text" && part.text) {
        ...
        (part.type === "tool_use" || part.type === "tool") && part.tool
      ) {
...
export function formatSessionInfo(info: SessionInfo): string {
  const lines = [
    ...
    `Date Range: ${info.first_message?.toISOString() ?? "N/A"} to ${
      info.last_message?.toISOString() ?? "N/A"
    }`,
    `Agents Used: ${info.agents_used.join(", ") || "none"}`,
    `Has Todos: ${
      info.has_todos ? `Yes (${info.todos?.length ?? 0} items)` : "No"
    }`,
    `Has Transcript: ${
      info.has_transcript ? `Yes (${info.transcript_entries} entries)` : "No"
    }`,
  ];
