...
import {
  type BashPermissionAskInput,
  checkMonitorCommandPermission,
} from "../../features/monitor/permission";
...
function getEffectiveMode(
  requestedMode: MonitorMode | undefined,
  ...
  if (requestedMode === "live_safe" && !liveModeEnabled) {
    return {
      mode: "idle",
      note:
        'requested mode "live_safe" was coerced to "idle" because monitor.live_mode_enabled is false',
    };
  }
  ...
  return tool({
    ...
    args: {
      command: tool.schema.string().describe(
        "Shell command to run in the background monitor",
      ),
      label: tool.schema.string().optional().describe(
        "Safe human-facing label for the monitor",
      ),
      mode: tool.schema
        .union([tool.schema.literal("idle"), tool.schema.literal("live_safe")])
        .optional()
        .describe(
          "Delivery mode. idle is safe default; live_safe requires monitor.live_mode_enabled.",
        ),
      match_pattern: tool.schema
        .string()
        .optional()
        .describe(
          "Optional JavaScript regex. Matching lines are delivered automatically.",
        ),
    },
    async execute(args: MonitorStartArgs, toolContext) {
      ...
      if (!filterResult.filter) {
        return `[ERROR] monitor_start match_pattern rejected: ${
      ...
      const effectiveMode = getEffectiveMode(
        args.mode,
        monitorConfig.live_mode_enabled,
      );
      ...
      try {
        ...
        return `[ERROR] monitor_start failed for label: ${
          args.label ?? "(manager-assigned label)"
        }`;
      }
    },
  });
