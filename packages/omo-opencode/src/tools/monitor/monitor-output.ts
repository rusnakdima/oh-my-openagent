...
Returns both output lines and counters so agents can detect dropped lines. Unknown or unauthorized monitor IDs return a not_found result instead of throwing.`,
    args: {
      monitor_id: tool.schema.string().describe(
        "Monitor ID to read output from",
      ),
      stream: tool.schema
        .enum(["matched", "unmatched", "all"])
        .optional()
        .describe(
          "Which retained stream to return: matched, unmatched, or all. Defaults to all.",
        ),
      since_sequence: tool.schema.number().optional().describe(
        "Return only lines with seq greater than this value",
      ),
      limit: tool.schema.number().optional().describe(
        "Maximum number of retained lines to return",
      ),
    },
