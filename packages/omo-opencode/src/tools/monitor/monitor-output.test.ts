...
function createToolContext(sessionID = CALLING_SESSION_ID): ToolContext {
  return {
    ...
    metadata: (
      _input: { title?: string; metadata?: Record<string, unknown> },
    ) => {},
    ask: (
      _input: {
        permission: string;
...
function createRecord(
  id: string,
  parentSessionId = CALLING_SESSION_ID,
): MonitorRecord {
  return {
    id,
    command: "bun test",
...
function createFakeManager(
  ...
  lines: OutputLine[],
): MonitorManager & { calls: GetOutputCall[] } {
  const calls: GetOutputCall[] = [];

  return {
    calls,
    start: (_opts: MonitorStartOpts) =>
      Promise.resolve(records[0] ?? createRecord("mon_created")),
    ...
    getOutput: (id: string, opts: MonitorOutputQuery): MonitorOutputResult => {
      ...
      const streamFilteredLines = stream === "all"
        ? lines
        : lines.filter((line) => line.text.startsWith(stream));
      const sinceSequence = opts.since_sequence;
      const sinceFilteredLines = sinceSequence === undefined
        ? streamFilteredLines
        : streamFilteredLines.filter((line) => line.seq > sinceSequence);
      const limitedLines = opts.limit === undefined
        ? sinceFilteredLines
        : sinceFilteredLines.slice(-opts.limit);
...
describe("monitor_output tool", () => {
  test("returns only unmatched lines when stream is unmatched", async () => {
    ...
    const result = parseMonitorOutput(
      await monitorOutput.execute(
        { monitor_id: "mon_1", stream: "unmatched" },
        createToolContext(),
      ),
    );
  ...
  test("returns only lines newer than since_sequence", async () => {
    ...
    const result = parseMonitorOutput(
      await monitorOutput.execute(
        { monitor_id: "mon_2", since_sequence: 5 },
        createToolContext(),
      ),
    );
    ...
    expect(manager.calls).toEqual([{
      id: "mon_2",
      opts: { stream: "all", since_sequence: 5 },
    }]);
  });
  ...
  test("caps returned lines with limit", async () => {
    ...
    expect(manager.calls).toEqual([{
      id: "mon_3",
      opts: { stream: "all", limit: 3 },
    }]);
  });

  test("returns counters with the output lines", async () => {
    //#given
    const manager = createFakeManager([createRecord("mon_4")], [
      createLine(9, "matched: final"),
    ]);
    const monitorOutput = createMonitorOutput(manager);

    //#when
    const result = parseMonitorOutput(
      await monitorOutput.execute({ monitor_id: "mon_4" }, createToolContext()),
    );
  ...
  test("does not expose output from another session's monitor", async () => {
    ...
    const manager = createFakeManager([
      createRecord("foreign_monitor", OTHER_SESSION_ID),
    ], [
    ...
    const result = parseMonitorOutput(
      await monitorOutput.execute(
        { monitor_id: "foreign_monitor" },
        createToolContext(CALLING_SESSION_ID),
      ),
    );
