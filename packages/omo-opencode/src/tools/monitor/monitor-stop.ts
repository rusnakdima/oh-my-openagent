...
export function createMonitorStop(
  manager: MonitorManager,
  _ctx?: unknown,
): ToolDefinition {
  const stoppedMonitorIds = new Set<string>();

  const formatResult = (result: MonitorStopResult): string =>
  ...
  return tool({
    ...
    async execute(
      args: MonitorStopArgs,
      toolContext: MonitorToolContext,
    ): Promise<string> {
      ...
      if (
        record.status === "stopped" || stoppedMonitorIds.has(args.monitor_id)
      ) {
        return formatResult({
          status: "already-stopped",
          monitor_id: args.monitor_id,
