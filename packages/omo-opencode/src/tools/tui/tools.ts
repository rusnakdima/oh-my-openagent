...
export function createTuiSnapshotTool(_ctx: PluginInput): ToolDefinition {
  return tool({
    ...
    args: {
      ...
      window: tool.schema.string().optional().describe(
        "Window name (defaults to session name)",
      ),
      pane: tool.schema.number().optional().default(0).describe(
      ...
      format: tool.schema
        ...
        .default("readable")
        .describe(
          "Output format: 'json' for raw data, 'readable' for human-friendly",
        ),
    },
    execute: async (args) => {
...
export function createTuiClickTool(_ctx: PluginInput): ToolDefinition {
  return tool({
    description: "Simulate a mouse click in the TUI. " +
      ...
      if (target.text) {
        ...
        if (!snapshot.parsedLayout) {
          return JSON.stringify({
            error: "No layout parsed - cannot find element by text",
          });
        }
        ...
        if (!element) {
          return JSON.stringify({
            error: `Element with text "${target.text}" not found`,
          });
        }
      }

      const success = element
        ? await clickOnElement(args.session, element, {
          windowName: args.window,
          paneIndex: args.pane ?? 0,
        })
        : await (async () => {
...
export function createTuiKeysTool(_ctx: PluginInput): ToolDefinition {
  return tool({
    description: "Send keyboard keys to the TUI. " +
...
export function createTuiTypeTool(_ctx: PluginInput): ToolDefinition {
  return tool({
    description:
      "Type text into the TUI. Each character is sent individually followed by no key.",
    args: {
      session: tool.schema.string().describe("The tmux session name"),
...
export function createTuiFindTool(_ctx: PluginInput): ToolDefinition {
  return tool({
    description: "Find UI elements in the current TUI layout. " +
    ...
    args: {
      ...
      text: tool.schema.string().optional().describe(
...
export function createTuiTools(
  ctx: PluginInput,
