...
type SessionWithPrompt = {
  prompt: (
    opts: { path: { id: string }; body: Record<string, unknown> },
  ) => Promise<unknown>;
};

function hasPrompt(
  session: PluginInput["client"]["session"],
): session is PluginInput["client"]["session"] & SessionWithPrompt {
...
export async function executeSync(
  ...
  try {
    const session = await deps.createOrGetSession(
      args,
      toolContext,
      ctx,
      model,
    ...
    const promptAgent = normalizeAgentForPrompt(normalizedSubagentType) ??
    ...
    try {
      ...
      if (promptResult.status === "failed") {
        if (promptMayHaveBeenAccepted) {
          log(
            "[call_omo_agent] Prompt returned an ambiguous error after dispatch; waiting for completion",
            {
              sessionID,
    ...
    if (sessionID && createdSessionForExecution) {
      ...
      if (typeof ctx.client.session.abort === "function") {
        void ctx.client.session.abort({ path: { id: sessionID } }).catch(
          (error: unknown) => {
            log(
              `[call_omo_agent] Failed to abort completed sync session:`,
              error,
            );
          },
        );
