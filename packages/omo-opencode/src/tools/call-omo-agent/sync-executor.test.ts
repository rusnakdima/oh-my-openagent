...
describe("executeSync", () => {
  ...
  test("registers child-session bootstrap and tracked prompt state before sync prompt dispatch", async () => {
    //#given
    const executeSync = await importExecuteSync();
    const { _resetForTesting, getSessionAgent } = require(
      "../../features/claude-code-session-state",
    );
    const {
      clearAllDelegatedChildSessionBootstrap,
      getDelegatedChildSessionBootstrap,
    } = require("../../shared/delegated-child-session-bootstrap");
    const { clearSessionTools, getSessionTools } = require(
      "../../shared/session-tools-store",
    );
    const deps = createDependencies({
      createOrGetSession: mock(async () => ({
        sessionID: "ses-call-bootstrap",
        isNew: true,
      })),
    });
    const toolContext = createToolContext();
    ...
    try {
      ...
      expect(observed[0]?.bootstrap?.retryParts[0]?.text).toContain(
        "collect bootstrap evidence",
      );
      expect(observed[0]?.bootstrap?.tools?.question).toBe(false);
      expect(observed[0]?.bootstrap?.fallbackChain?.[0]?.model).toBe("gpt-5.4");
      expect(getDelegatedChildSessionBootstrap("ses-call-bootstrap"))
        .toBeUndefined();
  ...
  test("#given sync prompt returns ambiguous EOF after dispatch #when executeSync runs #then it waits for the existing session result", async () => {
    ...
    const deps = createDependencies({
      createOrGetSession: mock(async () => ({
        sessionID: "ses-ambiguous-prompt",
        isNew: true,
      })),
      waitForCompletion: mock(async () => {}),
  ...
  test("strips legacy ZWSP-prefixed agent names from persisted sync prompt body (GH-3259)", async () => {
    ...
    //#then
    expect(recorder.getCapturedInput()?.body.agent).toBe(
