...
export async function resolveOrCreateSessionId(
  ...
  const parentSession = await ctx.client.session
    ...
    .catch((err: unknown) => {
      log("[call_omo_agent] Failed to get parent session", {
        error: String(err),
      });
      return null;
    });
  ...
  if (createResult.error) {
    ...
    if (errorStr.toLowerCase().includes("unauthorized")) {
      return {
        ok: false,
        error:
          `Error: Failed to create session (Unauthorized). This may be due to:
...
Original error: ${createResult.error}`,
    ...
    return {
      ok: false,
      error: `Error: Failed to create session: ${createResult.error}`,
    };
  }
