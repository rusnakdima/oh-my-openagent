...
function buildBlockedTmuxCommandMessage(
  command: string,
  parts: string[],
): string {
...
export async function executeInteractiveBash(
  args: InteractiveBashArgs,
): Promise<string> {
  try {
    ...
    const proc = spawnWithWindowsHide([
      ...resolveTmuxExecutable(tmuxPath),
      ...parts,
    ], {
      stdout: "pipe",
      stderr: "pipe",
...
export const interactive_bash: ToolDefinition = tool({
  ...
  args: {
    tmux_command: tool.schema.string().describe(
      "The tmux command to execute (without 'tmux' prefix)",
    ),
  },
