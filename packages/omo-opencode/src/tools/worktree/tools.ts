...
import {
  detectWorktreePath,
  resolveMainRepoRoot,
} from "../../hooks/start-work/worktree-detector";
...
export interface WorktreeToolsDeps {
  ...
  getWorkForSession?: (
    cwd: string,
    sessionID: string,
  ) => { worktree_path?: string; session_ids?: string[] } | null;
...
const realGetWorkForSession: (
  cwd: string,
  sessionID: string,
) => BoulderState | null = getWorkForSession;
const realWriteBoulderState: (cwd: string, state: BoulderState) => void =
  writeBoulderState;
const realDetectWorktreePath: (directory: string) => string | null =
  detectWorktreePath;
const realResolveMainRepoRoot: (directory: string) => string | null =
  resolveMainRepoRoot;
...
export function createWorktreeTools(deps: WorktreeToolsDeps = defaultDeps): {
  ...
  const detectWorktreePathFn = deps.detectWorktreePath ??
    realDetectWorktreePath;
  ...
  const enterWorktreeTool: ToolDefinition = tool({
    ...
    execute: async (args: { path: string }, context: ToolContext) => {
      ...
      log(
        `[worktree] enter_worktree: ${worktreePath} for session ${sessionID}`,
      );
  ...
  const exitWorktreeTool: ToolDefinition = tool({
    ...
    execute: async (_args: Record<string, never>, context: ToolContext) => {
      ...
      const resolveMainRepoRootFn = deps.resolveMainRepoRoot ??
        realResolveMainRepoRoot;
      ...
      if (!existing) {
        throw new Error(
          "[exit_worktree] No boulder state found for this session. Are you inside a worktree?",
        );
      }
