...
export type ModelsDevCatalogs = Readonly<
  Record<
    string,
    { readonly models?: Readonly<Record<string, ModelsDevModel>> } | undefined
  >
>;
...
export type OpenGatewayCatalog = Readonly<
  Record<string, OpenGatewayModelEntry>
>;
...
const LEGACY_FAMILY_PATTERN =
  /gpt-5[.-][23](?![-\s]codex(?:\b|[-._]))(?:\b|[-._])/i;
...
function baseTier(source: ModelsDevModel | undefined): {
  ...
  const smallest = contextTiers.reduce<
    (typeof contextTiers)[number] | undefined
  >(
    (
      best,
      tier,
    ) => (best === undefined || (tier.tier?.size ?? 0) < (best.tier?.size ?? 0)
      ? tier
      : best),
    undefined,
  );
  return {
    cache_read: smallest?.cache_read,
    cache_write: smallest?.cache_write,
  };
}
...
function toEntry(
  ...
  return {
    ...
    cost: {
      ...
      cache_read: source?.cost?.cache_read ?? tier.cache_read ??
        override?.cost.cache_read ?? 0,
      cache_write: source?.cost?.cache_write ?? tier.cache_write ??
        override?.cost.cache_write ?? 0,
    },
    limit: {
...
export function buildOpenGatewayCatalog(
  ...
  for (const item of response.data ?? []) {
    ...
    const source = (providerKey === undefined
      ? undefined
      : modelsDev[providerKey]?.models?.[upstreamId]) ?? openRouter[item.id];
    ...
    if (source !== undefined && source.tool_call !== true) {
      continue;
    }
    ...
    if (
      isRetiredModelReference(item.id) || isRetiredModelReference(entry.name)
    ) {
      continue;
    }
...
export function serializeOpenGatewayCatalog(
  catalog: OpenGatewayCatalog,
): string {
...
async function main(): Promise<void> {
  const [gateway, modelsDev] = await Promise.all([
    fetchJson(OPENGATEWAY_MODELS_URL),
    fetchJson(MODELS_DEV_URL),
  ]);
  const catalog = buildOpenGatewayCatalog(
    gateway as OpenGatewayCatalogResponse,
    modelsDev as ModelsDevCatalogs,
  );
  const count = Object.keys(catalog).length;
  if (count === 0) {
    throw new Error(
      "OpenGateway catalog came back empty; refusing to overwrite the checked-in JSON",
    );
  }
