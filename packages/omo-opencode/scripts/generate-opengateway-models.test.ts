...
describe("buildOpenGatewayCatalog", () => {
  ...
  test("maps the x-ai style owner alias through the owner table", () => {
    ...
    expect(catalog["qwen/qwen4-max"]?.limit).toEqual({
      context: 262144,
      output: 32768,
    });
  });
  ...
  test("derives attachment from image input modality only", () => {
    ...
    expect(catalog["qwen/qwen4-max"]?.modalities).toEqual({
      input: ["text"],
      output: ["text"],
    });
  });
  ...
  test("defaults missing cost and limit metadata to zero-cost and floor limits", () => {
    ...
    const sparse: ModelsDevCatalogs = {
      deepseek: {
        models: { "deepseek-v9": { name: "DeepSeek V9", tool_call: true } },
      },
    };
    ...
    expect(catalog["deepseek/deepseek-v9"]?.limit).toEqual({
      context: 4096,
      output: 4096,
    });
  });
