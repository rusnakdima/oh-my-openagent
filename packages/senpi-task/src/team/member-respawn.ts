...
export class TeamMemberRespawnLaunchError extends Error {
  ...
  constructor(
    code: TeamMemberRespawnLaunchErrorCode,
...
function parseTeamMemberTaskName(
  name: string | undefined,
): TeamMemberTaskIdentity | undefined {
  if (name === undefined) return undefined;
  const match = /^team:([0-9a-f-]{36}):([a-z0-9-]+)$/.exec(name);
  const teamRunId = match?.[1];
  const memberName = match?.[2];
  return teamRunId === undefined || memberName === undefined
    ? undefined
    : { teamRunId, memberName };
}
...
export function createTeamMemberRespawnLaunchResolver(
  ...
  const config = toTeamCoreConfig(
    ...
    teamStorageBaseDir(options.stateDir),
  );
  const inheritedExtensions = [
    ...new Set(options.memberExtension.inheritedExtensions ?? []),
  ];
  const extensions = assembleMemberExtensions(
    options.memberExtension.entryPath,
    inheritedExtensions,
  );
  ...
  return async (record: TaskRecord) => {
    ...
    if (
      runtime.status !== "active" && runtime.status !== "shutdown_requested"
    ) {
      throw new TeamMemberRespawnLaunchError("runtime_inactive", identity);
    }
    const member = runtime.members.find((entry) =>
      entry.name === identity.memberName
    );
    if (member === undefined || member.status === "shutdown_approved") {
      throw new TeamMemberRespawnLaunchError("member_missing", identity);
    }
    const map = await readMemberTaskMap(
      resolveTeamRuntimeDirs(options.stateDir, identity.teamRunId).runtimeDir,
    );
