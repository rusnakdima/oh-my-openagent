...
describe("team tasklist orchestration", () => {
  test("#given a task input #when createTeamTask runs #then it persists a pending task with an id", async () => {
    ...
    // when
    const task = await createTeamTask(ctx, {
      subject: "build",
      description: "do it",
      status: "pending",
    });
  ...
  test(
    "#given two tasks with a dependency #when the blocked task is claimed before its blocker completes #then it is rejected until the blocker is completed",
    async () => {
      // given
      const ctx = tempContext();
      const blocker = await createTeamTask(ctx, {
        subject: "a",
        description: "blocker",
        status: "pending",
      });
      ...
      expect((rejected as TeamTaskBlockedByError).blockers).toEqual([
        blocker.id,
      ]);
  ...
  test("#given several tasks #when listTeamTasks runs with an owner filter #then only that owner's tasks are returned in id order", async () => {
    // given
    const ctx = tempContext();
    const first = await createTeamTask(ctx, {
      subject: "a",
      description: "x",
      status: "pending",
    });
    await createTeamTask(ctx, {
      subject: "b",
      description: "y",
      status: "pending",
    });
