...
function assertNoCallerTeamLead(
  options: NormalizeSenpiTeamSpecOptions | undefined,
  teamName: string,
): void {
...
function assertNoRawLeadField(rawSpec: unknown, teamName: string): void {
  if (
    isPlainRecord(rawSpec) && rawSpec.lead !== undefined &&
    rawSpec.lead !== null
  ) {
    throw new SenpiTeamSpecError(
...
function clampMemberTaskSummaries(members: readonly unknown[]): unknown[] {
  return members.map((member) => {
    if (!isPlainRecord(member) || typeof member.task_summary !== "string") {
      return member;
    }
...
function assertNoReservedMemberName(
...
export function normalizeSenpiTeamSpec(
  ...
  if (!isPlainRecord(normalized)) {
    throw new SenpiTeamSpecError(
      `Team '${teamName}' spec must be an object like { name?, members: [...] }; received ${
        describeReceived(rawSpec)
      }. Pass the spec as a nested object, or as a valid JSON string of that object.`,
      "INVALID_SPEC",
      teamName,
    );
  }
  ...
  if (Array.isArray(preNormalized.members)) {
    const members = clampMemberTaskSummaries(
      remapAgentAliasKind(preNormalized.members),
    );
  ...
  if (!parsed.success) {
    ...
    const detail = firstIssue
      ? `${firstIssue.path.join(".") || "spec"}: ${firstIssue.message}`
      : parsed.error.message;
    throw new SenpiTeamSpecError(
      `Invalid team '${teamName}' spec (${detail}).`,
      "INVALID_SPEC",
      teamName,
    );
  }
