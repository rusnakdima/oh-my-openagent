...
export function resolveTeamRuntimeDirs(
...
export function resolveTeamMemberInboxDir(
  config: StateDirConfig,
  teamRunId: string,
  memberName: string,
): string {
 ...
 */
export function resolveProjectTeamSpecPath(
  projectRoot: string,
  teamName: string,
): string {
...
export async function ensureTeamRuntimeDirs(
  ...
  for (const memberName of memberNames) {
    await mkdir(resolveTeamMemberInboxDir(config, teamRunId, memberName), {
      recursive: true,
      mode: 0o700,
    });
  }
