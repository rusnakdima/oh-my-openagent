...
async function givenTeamWithSuspendedMember() {
  ...
  const created = await createTeam(
    ...
    "project",
    {
      ...
      spawnDepth: 1,
      memberExtension: {
        entryPath: TRUSTED_ENTRY,
        inheritedExtensions: [TRUSTED_INHERITED],
      },
    },
  );
  if (created.memberTaskIds.alpha === undefined) {
    throw new Error("expected alpha task id");
  }
  ...
  await writeMemberTaskMap(
    resolveTeamRuntimeDirs(stateDir, created.runtimeState.teamRunId).runtimeDir,
    {
      alpha: taskId,
    },
  );
...
describe("team member revival across session resume", () => {
  ...
  test("#given a suspended member whose team run was deleted #when the owning session reconciles #then it stays suspended with a deferred team_inactive outcome", async () => {
    ...
    const h = await givenTeamWithSuspendedMember();
    rmSync(
      resolveTeamRuntimeDirs(h.stateDir, h.created.runtimeState.teamRunId)
        .runtimeDir,
      {
        recursive: true,
        force: true,
