...
} from "@oh-my-opencode/team-core/config";
...
describe("member extension tools", () => {
  test("#given member and lead recipients #when task_send runs #then each durable inbox receives its unchanged message", async () => {
    ...
    const deps = {
      ...
      members: ["alice", "bob"],
      appendEvent: (_taskId: string, event: PersistedTaskEvent) =>
    ...
    // when
    const memberResult = await runMemberTaskSend(deps, {
      to: "bob",
      message: "member note",
    });
    const leadResult = await runMemberTaskSend(deps, {
      to: "lead",
      message: "lead note",
    });

    // then
    expect(memberResult.details).toEqual({
      kind: "team_message",
      message_id: "99999999-9999-4999-8999-999999999999",
      to: "bob",
    });
    expect(leadResult.details).toEqual({
      kind: "team_message",
      message_id: "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa",
      to: "lead",
    });
    expect(
      (await listUnreadMessages(TEAM_RUN_ID, "bob", harness.config)).map((
        entry,
      ) => entry.body),
    ).toEqual(["member note"]);
    expect(
      (await listUnreadMessages(TEAM_RUN_ID, "lead", harness.config)).map((
        entry,
      ) => entry.body),
    ).toEqual(["lead note"]);
  ...
  test("#given an unknown recipient #when task_send runs #then it rejects without writing mail", async () => {
    ...
    // when / then
    await expect(runMemberTaskSend(deps, { to: "nobody", message: "nope" }))
      .rejects.toThrow("Unknown team recipient");
  });
  ...
  test("#given malformed member identity env #when parsed #then typed configuration errors reject malformed values", () => {
    ...
    for (
      const identity of [
        ...
        `${TEAM_RUN_ID}::`,
        `::alice`,
        `${TEAM_RUN_ID}::alice::extra`,
      ]
    ) {
      expect(() =>
        parseMemberExtensionEnv({ ...baseEnv, SENPI_TASK_MEMBER: identity })
      ).toThrow(MemberExtensionConfigError);
    }
    expect(() =>
      parseMemberExtensionEnv({
        ...baseEnv,
        SENPI_TASK_MEMBER_TASK_ID: "st_bad",
      })
    ).toThrow(MemberExtensionConfigError);
  });
});
