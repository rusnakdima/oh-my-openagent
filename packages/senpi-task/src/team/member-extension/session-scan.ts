...
export async function sessionJsonlContainsMessage(
  sessionDir: string,
  messageId: string,
): Promise<boolean> {
  let entries: string[];
  try {
    entries = (await readdir(sessionDir)).filter((name) =>
      name.endsWith(".jsonl")
    ).toSorted();
  } catch (error) {
...
function containsEnvelopeMarker(value: unknown, messageId: string): boolean {
  if (typeof value === "string") {
    return value.includes("<peer_message ") &&
      value.includes(`messageId="${messageId}"`);
  }
  if (Array.isArray(value)) {
    return value.some((entry) => containsEnvelopeMarker(entry, messageId));
  }
  if (!isRecord(value)) return false;
  return Object.values(value).some((entry) =>
    containsEnvelopeMarker(entry, messageId)
  );
}
