...
export function resolveMemberExtensionEntryPath(
  extensionUrl = import.meta.url,
): string {
  return fileURLToPath(
    new URL(`./${MEMBER_EXTENSION_BUNDLE_NAME}`, extensionUrl),
  );
}

export function parseMemberExtensionEnv(
...
async function startRuntime(runtime: ActiveRuntime): Promise<void> {
  ...
  try {
    ...
    runtime.pollTimer = setInterval(
      () => runSafely("poll", runtime.poller.pollOnce()),
      MEMBER_POLL_INTERVAL_MS,
    );
    runtime.ackTimer = setInterval(
      () => runSafely("ack", runtime.poller.checkPendingAcks()),
...
function runSafely(operation: string, promise: Promise<void>): void {
  promise.catch((error: unknown) => {
    log("senpi-task member extension poll failed", {
      operation,
      error: String(error),
    });
  });
...
function parseMembers(value: unknown): readonly string[] {
  if (
    !Array.isArray(value) ||
    !value.every((member) =>
      typeof member === "string" && MEMBER_NAME_PATTERN.test(member)
    )
  ) {
    throw new MemberExtensionConfigError(
