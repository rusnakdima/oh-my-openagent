...
export type MemberStatusPort = {
  get(
    ...
    | { readonly status: TaskStatus; readonly child_session_id?: string }
    | undefined;
  getResidentHandle(
    taskId: string,
  ...
  return transitionRuntimeState(
    ...
    (state) => ({
      ...
      members: state.members.map((member) => {
        ...
        const sessionId = deps.manager.getResidentHandle(taskId)?.sessionId ??
          record.child_session_id ?? member.sessionId;
        return {
          ...member,
