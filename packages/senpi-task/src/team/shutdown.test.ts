...
import {
  type RuntimeStateMember,
  TeamSpecSchema,
} from "@oh-my-opencode/team-core/types";

import { type TeamCoreConfig, toTeamCoreConfig } from "./runtime-config";
import {
  approveShutdown,
  rejectShutdown,
  requestShutdown,
...
function recordingMessenger(): {
  sent: ShutdownOutboundMessage[];
  send: (message: ShutdownOutboundMessage) => Promise<void>;
} {
...
function recordingCanceller(): {
  cancelled: string[];
  cancelMemberTask: (memberName: string) => Promise<void>;
} {
  const cancelled: string[] = [];
  return {
    cancelled,
    cancelMemberTask: (memberName) => {
...
describe("team shutdown protocol", () => {
  test("#given an active member #when requestShutdown runs #then a pending request is recorded and a shutdown_request message is sent to the member", async () => {
    ...
    // when
    await requestShutdown(teamRunId, "alpha", {
      config,
  ...
  test("#given an unknown member #when requestShutdown runs #then a typed unknown_member error is raised and nothing is sent", async () => {
    ...
    try {
      await requestShutdown(teamRunId, "ghost", {
        config,
        sendMessage: messenger.send,
      });
    } catch (error) {
  ...
  test("#given a pending request #when approveShutdown runs #then the member becomes shutdown_approved, the task is cancelled, and the request is marked approved", async () => {
    ...
    // when
    await approveShutdown(teamRunId, "alpha", {
      config,
      sendMessage: messenger.send,
      cancelMemberTask: canceller.cancelMemberTask,
    });
    ...
    expect(
      messenger.sent.some((message) =>
        message.kind === "shutdown_approved" && message.to === "alpha"
      ),
    ).toBe(true);
  });
  ...
  test("#given a completed member with a pending request #when approveShutdown runs #then the completed status is preserved (not overwritten)", async () => {
    ...
    const state = await loadRuntimeState(teamRunId, config);
    expect(
      state.members.find((candidate) => candidate.name === "alpha")?.status,
    ).toBe("completed");
    expect(state.shutdownRequests[0]?.approvedAt).toBeGreaterThan(0);
  });
  ...
  test("#given a pending request #when rejectShutdown runs #then the member status is left running and the request records the reason", async () => {
    ...
    // when
    await rejectShutdown(teamRunId, "alpha", "still needed for the release", {
      config,
      sendMessage: messenger.send,
    });
    ...
    expect(state.shutdownRequests[0]?.rejectedReason).toBe(
      "still needed for the release",
    );
    expect(
      messenger.sent.some((message) =>
        message.kind === "shutdown_rejected" &&
        message.body === "still needed for the release"
      ),
    ).toBe(true);
  });
  ...
  test("#given no pending request #when rejectShutdown runs #then a typed no_pending_request error is raised", async () => {
    ...
    let rejected: unknown;
    try {
      await rejectShutdown(teamRunId, "alpha", "no", {
        config,
        sendMessage: messenger.send,
      });
    } catch (error) {
  ...
  test("#given a rejected request #when requestShutdown runs again #then a fresh pending request is allowed", async () => {
    ...
    await rejectShutdown(teamRunId, "alpha", "later", {
      config,
      sendMessage: messenger.send,
    });
