 ...
 */
export async function reconcileTeamMailboxOnSessionStart(
  ...
  for (const team of teams) {
    try {
      const runtimeDir =
        resolveTeamRuntimeDirs(deps.stateDir, team.teamRunId).runtimeDir;
      ...
      await reclaimStaleTeamReservations(
        team.teamRunId,
        recipients,
        deps.config,
        staleTtlMs,
      ...
      log("senpi-task team session-start reclaim skipped one team", {
