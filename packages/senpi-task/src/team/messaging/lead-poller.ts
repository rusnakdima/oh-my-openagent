...
import { type Message, MessageSchema } from "@oh-my-opencode/team-core/types";
...
export function createLeadPoller(deps: LeadPollerDeps): LeadPoller {
  ...
  let stopped = false;
  const state: LeadPollState = {
    pending,
    isStopped: () => stopped,
    markerIndex: createSessionMarkerIndex(),
  };
...
async function settlePending(
  ...
  for (const delivery of [...state.pending.values()]) {
    ...
    switch (delivery.phase) {
      case "awaiting_flush":
        continue;
      case "awaiting_persistence":
        releaseWhenMissing = false;
        break;
      case "recovery":
        releaseWhenMissing = true;
        break;
      default: {
    ...
    const persisted = await state.markerIndex.contains(
      sessionFile,
    ...
    state.pending.delete(delivery.message.messageId);
    deps.deliveryJournal?.markReported(
      deps.teamRunId,
      delivery.message.messageId,
    );
  ...
  try {
    deps.coordinator.enqueue({
      ...
      onFlushed: () => {
        ...
        if (current === delivery && current.phase === "awaiting_flush") {
          current.phase = "awaiting_persistence";
        }
      },
    });
  } catch (error) {
...
async function recoverReservations(
  ...
  try {
    entries = (await readdir(inboxDir(deps)))
      .filter((name) =>
        name.startsWith(RESERVED_PREFIX) && name.endsWith(RESERVED_SUFFIX)
      )
      .toSorted();
  } catch (error) {
  ...
  for (const entry of entries) {
    ...
    if (sessionFile === undefined) {
      state.pending.set(message.messageId, {
        message,
        reservation,
        phase: "recovery",
      });
    } else if (
      await state.markerIndex.contains(sessionFile, message.messageId)
    ) {
      await commitDeliveryReservation(reservation);
      deps.deliveryJournal?.markReported(deps.teamRunId, message.messageId);
      appendDeliveredEvent(deps, message);
