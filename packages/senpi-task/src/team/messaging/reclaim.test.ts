...
async function reserveOne(
  ...
  await sendMessage(message, TEAM_RUN_ID, config, {
    isLead: false,
    activeMembers: [recipient],
  });
  ...
  if (options.age === true) {
    ...
    await utimes(
      reservedPath(stateDir, recipient, message.messageId),
      past,
      past,
    );
  }
...
describe("reclaimStaleTeamReservations", () => {
  test("#given a stale reservation older than the ttl #when reclaimed #then it is restored to unread", async () => {
    ...
    // when
    const reclaimed = await reclaimStaleTeamReservations(
      TEAM_RUN_ID,
      ["beta"],
      config,
      STALE_TTL_MS,
    ...
    expect(existsSync(reservedPath(stateDir, "beta", message.messageId))).toBe(
      false,
    );
  });
