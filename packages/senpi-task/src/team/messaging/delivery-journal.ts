...
export type LeadDeliveryJournal = {
  ...
  takeOldestUnreported(
    teamRunId: string,
    filter?: { readonly from?: string },
  ): Message | undefined;
...
export function createLeadDeliveryJournal(
  options: LeadDeliveryJournalOptions = {},
): LeadDeliveryJournal {
  ...
  return {
    record(teamRunId, message) {
      const list = (entries.get(teamRunId) ?? []).filter((candidate) =>
    ...
    takeOldestUnreported(teamRunId, filter = {}) {
      ...
      const index = list.findIndex((candidate) =>
        filter.from === undefined || candidate.from === filter.from
      );
