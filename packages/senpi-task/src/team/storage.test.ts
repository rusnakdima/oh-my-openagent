...
describe("team storage layout", () => {
  ...
  test("#given a team run id #when runtime dirs are resolved #then runtime and tasks dirs live under the base dir", () => {
    ...
    expect(dirs.tasksDir).toBe(
      join(dirs.baseDir, "runtime", teamRunId, "tasks"),
    );
  ...
  test("#given the discovery-only project spec path #when resolved #then it maps to the omo-compatible .omo/teams path", () => {
    ...
    // then
    expect(specPath).toBe(
      join(projectDir, ".omo", "teams", "research-team", "config.json"),
    );
  ...
  test("#given a team run #when runtime dirs are ensured #then the directories are created at the right paths", async () => {
    ...
    // when
    const dirs = await ensureTeamRuntimeDirs(config, teamRunId, [
      "finder",
      "quick-1",
    ]);
    ...
    expect(existsSync(resolveTeamMemberInboxDir(config, teamRunId, "finder")))
      .toBe(true);
    expect(existsSync(resolveTeamMemberInboxDir(config, teamRunId, "quick-1")))
      .toBe(true);
  });
});
