 ...
 */
export const DELETABLE_MEMBER_STATUSES: ReadonlySet<
  RuntimeStateMember["status"]
> = new Set([
  "completed",
  "shutdown_approved",
  "errored",
...
export function isMemberDeletable(
  status: RuntimeStateMember["status"],
): boolean {
  return DELETABLE_MEMBER_STATUSES.has(status);
}

export function findRuntimeMember(
  state: RuntimeState,
  memberName: string,
): RuntimeStateMember | undefined {
...
export function isUnresolvedRequest(
  request: RuntimeState["shutdownRequests"][number] | undefined,
): boolean {
  return request !== undefined && request.approvedAt === undefined &&
