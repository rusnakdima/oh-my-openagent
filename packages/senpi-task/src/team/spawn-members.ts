...
import {
  projectMemberStatus,
  type RuntimeMemberStatus,
} from "./member-projection";
import {
...
): Promise<SpawnMembersResult> {
  const spawned = new Map<string, SpawnedMember>();
  let failure: Error | undefined;
  let nextIndex = 0;
  const workerCount = Math.max(
    1,
    Math.min(input.maxParallel, input.spec.members.length),
  );
...
function toSpawnFailure(
  ...
  const wrapped = new SenpiTeamRuntimeError(
    `member '${memberName}' failed to start: ${
...
async function spawnOneMember(
  ...
  if (member.worktreePath !== undefined) {
    await mkdir(member.worktreePath, { recursive: true });
  }
  ...
  const sessionId =
    input.manager.getResidentHandle(result.task_id)?.sessionId ??
      record?.child_session_id;
  const status = projectMemberStatus(
    record?.status ?? (result.status === "pending" ? "pending" : "running"),
  );
...
function buildMemberPrompt(spec: TeamSpec, member: TeamMember): string {
  const role = member.prompt ??
    `You are team member '${member.name}' in team '${spec.name}'.`;
  return [
...
function describeStartResult(
  result: Exclude<StartResult, { kind: "started" }>,
): string {
  switch (result.kind) {
    case "depth_denied":
