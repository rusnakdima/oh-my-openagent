...
export class FakeDestruction {
  readonly calls: Array<
    { readonly taskId: string; readonly cause: DestroyCause }
  > = [];
...
export class FakeTeamManager {
  ...
  readonly cancelled: Array<
    { readonly taskId: string; readonly reason?: string }
  > = [];
  readonly #records = new Map<string, TaskRecord>();
  readonly #getHooks = new Map<
    string,
    (record: TaskRecord, readCount: number) => TaskRecord
  >();
  ...
  start(spec: ManagerStartSpec): Promise<StartResult> {
    ...
    const behavior = this.#options.behaviors?.[index] ??
      this.#options.defaultBehavior ?? { kind: "ok" };
    ...
    this.#records.set(
      taskId,
      buildRecord(taskId, spec, status, behavior.resolvedModel),
    );
    return Promise.resolve({
      kind: "started",
      task_id: taskId,
      ...
      ...(behavior.resolvedModel !== undefined
        ? { resolved_model: behavior.resolvedModel }
        : {}),
    });
  }
  ...
  async cancelTask(idOrName: string, reason?: string): Promise<CancelOutcome> {
    ...
    if (record === undefined) {
      return { kind: "not_found", reason: `no task ${idOrName}` };
    }
  ...
  setGetHook(
    taskId: string,
    hook: (record: TaskRecord, readCount: number) => TaskRecord,
  ): void {
  ...
  setResidency(
    taskId: string,
    residencyState: TaskRecord["residency_state"],
  ): void {
    const record = this.#records.get(taskId);
    if (record !== undefined) {
      this.#records.set(taskId, { ...record, residency_state: residencyState });
    }
  }
}
