...
const DELIVER_STATES: readonly ParentState[] = [{ kind: "idle" }, {
  kind: "streaming",
}];
...
function runningWithHandle(state: ChaosState): string[] {
  return state.taskIds.filter((id) =>
    load(state, id)?.status === "running" &&
    state.harness.engines.some((engine) =>
      engine.manager.getResidentHandle(id) !== undefined
    )
  );
}

function ownerFor(state: ChaosState, taskId: string) {
  return state.harness.engines.find((engine) =>
    engine.manager.getResidentHandle(taskId) !== undefined
  );
}

function fakeHandleFor(state: ChaosState, taskId: string) {
  for (const engine of state.harness.engines) {
    const handle = engine.inProcessRunner.handles.get(taskId) ??
      engine.processRunner.handles.get(taskId);
...
function byStatus(
  state: ChaosState,
  predicate: (record: TaskRecord) => boolean,
): string[] {
  return state.taskIds.filter((id) => {
...
async function settleWith(
  ...
  if (outcome === "clean") {
    handle.settle({ status: "completed", finalResponse: "clean exit" });
  } else {handle.settle({
      status: "error",
      failure: {
        kind: "child-prompt-failed",
        message: "terminated by signal SIGKILL",
      },
    });}
...
async function actSteer(state: ChaosState): Promise<void> {
  const id = pick(
    ...
    byStatus(
      state,
      (record) =>
        record.status === "running" && record.residency_state === "resident",
    ),
  );
  if (id === undefined) return;
...
async function actInterrupt(state: ChaosState): Promise<void> {
  const id = pick(
    state,
    byStatus(state, (record) => record.status === "running"),
  );
  if (id === undefined) return;
...
async function actCancel(state: ChaosState): Promise<void> {
  ...
  if (id === undefined) return;
  await ownerFor(state, id)?.manager.cancelTask(
    id,
    state.rng.bool() ? "user aborted" : undefined,
  );
}
...
async function actCancelPending(state: ChaosState): Promise<void> {
  const id = pick(
    state,
    byStatus(state, (record) => record.status === "pending"),
  );
  if (id === undefined) return;
  await state.harness.manager.cancelTask(
    id,
    state.rng.bool() ? "cancelled while queued" : undefined,
  );
}
...
async function actRevive(state: ChaosState): Promise<void> {
  const id = pick(
    ...
    byStatus(
      state,
      (record) =>
        REVIVABLE.has(record.status) && record.residency_state === "resident",
    ),
  );
  if (id === undefined) return;
...
function checkDelta(
  ...
  if (delta !== expectedDelta(result)) {
    state.seamBreaches.push(
      `notify result ${result.kind} produced ${delta} enqueue(s), expected ${
...
function unobservedNotifying(state: ChaosState): string[] {
  return byStatus(state, (record) => {
    if (
      !NOTIFYING_TERMINALS.has(record.status) ||
      !(state.background.get(record.task_id) ?? false)
    ) return false;
    return !state.observedEdges.has(
      `${record.task_id}:${record.notification.run_epoch}`,
    );
  });
}
...
function notifyEdge(
  ...
  const result = state.harness.notifier.notifyTerminal({
    record,
    parentState,
...
async function actNotify(state: ChaosState): Promise<void> {
  ...
  if (id === undefined) return;
  const parentState = state.rng.bool(0.6)
    ? state.rng.pick(DELIVER_STATES)
    : state.rng.pick(BUFFER_STATES);
...
async function actReplayNotify(state: ChaosState): Promise<void> {
  ...
  if (result.kind !== "skipped") {
    state.seamBreaches.push(
      `replay of already-notified ${id} epoch ${record.notification.run_epoch} was ${result.kind}, expected skipped`,
    );
  }
}
...
async function actFlush(state: ChaosState): Promise<void> {
  const before = state.harness.parentNotifier.calls.length;
  const result = state.harness.notifier.flushBuffered({
    sessionId: CHAOS_SESSION,
    replaced: state.rng.bool(0.3),
  });
  const delta = state.harness.parentNotifier.calls.length - before;
  const expected = result.kind === "flushed" ? 1 : 0;
  if (delta !== expected) {
    state.seamBreaches.push(
      `flush result ${result.kind} produced ${delta} enqueue(s), expected ${expected}`,
    );
  }
}
...
async function actReconcile(state: ChaosState): Promise<void> {
  ...
  state.harness.notifier.reconcileFailedNotifications({
    sessionId: CHAOS_SESSION,
    parentState: { kind: "idle" },
  });
}
...
type Action = {
  ...
  readonly run: (state: ChaosState) => Promise<void>;
...
export async function applyRandomAction(state: ChaosState): Promise<void> {
  ...
  const action = state.rng.weighted(
    ACTIONS.map((entry) => ({ value: entry, weight: entry.weight })),
  );
