...
export function createObservingStore(
  backing: TaskRecordStore,
  shared?: StoreObservations,
): ObservingStore {
  ...
  const observeWrite = (
    ...
    if (after !== null) {
      observeLifecycleWrite(backing, observations, before, after);
    }
...
function observeReplace(
  ...
  if (nextNotified > previous.notification.notified_epoch) {
    ...
    observations.notifyCommits.set(
      key,
      (observations.notifyCommits.get(key) ?? 0) + 1,
    );
  }
  if (
    ...
    observations.breaches.push({
      invariant: 2,
      detail:
        `replace overwrote terminal ${previous.status} -> ${record.status} for ${record.task_id} at epoch ${record.notification.run_epoch}`,
    });
  }
}
...
function observeLifecycleWrite(
  ...
  if (before !== null) {
    if (after.notification.run_epoch < before.notification.run_epoch) {
      observations.lifecycleBreaches.push(
        `run_epoch regressed for ${after.task_id}: ${before.notification.run_epoch} -> ${after.notification.run_epoch}`,
      );
    }
    const becameSuspended = after.residency_state === "persisted_only" ||
      after.residency_state === "rpc_detached";
    if (
      becameSuspended &&
      after.notification.run_epoch !== before.notification.run_epoch
    ) {
      observations.lifecycleBreaches.push(
        `suspension changed run_epoch for ${after.task_id}: ${before.notification.run_epoch} -> ${after.notification.run_epoch}`,
      );
    }
  }
  if (after.status === "lost" && hasRecoveryArtifact(backing, after)) {
    observations.lifecycleBreaches.push(
      `recoverable task ${after.task_id} transitioned to lost`,
    );
  }
  if (
    after.killed === true || after.status === "cancelled" ||
    after.residency_state === "evicted" || after.residency_state === "disposed"
  ) {
    observations.forbiddenResidency.add(after.task_id);
  } else if (
    after.residency_state === "resident" &&
    observations.forbiddenResidency.has(after.task_id)
  ) {
    observations.lifecycleBreaches.push(
      `deliberately stopped task ${after.task_id} regained residency`,
    );
  }
}

function hasRecoveryArtifact(
  backing: TaskRecordStore,
  record: TaskRecord,
): boolean {
  if (record.spawn_spec !== undefined && isSpawnSpecV1(record.spawn_spec)) {
    return true;
  }
  const sessions = join(
    backing.stateDir,
...
function observeResidentCounts(
  backing: TaskRecordStore,
  observations: StoreObservations,
): void {
  ...
  for (const record of backing.list().records) {
    ...
    counts.set(
      ...
      (counts.get(record.parent_session_id) ?? 0) + 1,
    );
  }
  for (const [parent, count] of counts) {
    observations.maxResidentsByParent.set(
      parent,
      Math.max(count, observations.maxResidentsByParent.get(parent) ?? 0),
    );
  }
}
...
function observeTransition(
  ...
  if (
    before === null || !isTerminalStatus(before.status) ||
    !STATUS_CHANGING.has(transition.type)
  ) return;
  ...
  if (result.applied || result.audit.type !== "late_transition_ignored") {
    observations.breaches.push({
      invariant: 2,
      detail:
        `late ${transition.type} on terminal ${before.status} for ${before.task_id} was ${
          result.applied ? "applied" : "not logged as late"
        } (audit=${result.audit.type})`,
    });
  }
}
