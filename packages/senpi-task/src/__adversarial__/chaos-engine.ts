...
export type LifecycleChaosObservations = {
  readonly actionCounts: Record<
    | "suspend_session"
    | "resume_session"
    | "crash_mid_suspend"
    | "sibling_reconcile_race"
    | "mass_revive_at_cap",
    number
  >;
...
function hasSession(store: TaskRecordStore, taskId: string): boolean {
  ...
  return existsSync(directory) &&
    readdirSync(directory).some((entry) => entry.endsWith(".jsonl"));
}
...
function observeLaunch(
  ...
  if (
    ...
    !hasSession(store, taskId)
  ) {
    observations.terminalRelaunches.push(`${engineId}:${taskId}`);
  }
  if (record?.pid !== undefined && processes.isAlive(record.pid)) {
    observations.pidBreaches.push(
      `replacement for ${taskId} spawned before retained pid ${record.pid} was terminated`,
    );
  }
}
...
export class ChaosRunner implements ManagedRunner {
  ...
  #launch(taskId: string): ManagedChildHandle {
    ...
    const handle: ManagedChildHandle = {
      ...
      ...(pid === undefined ? {} : { pid }),
      terminate: async () => {
        if (pid !== undefined) this.#processes.signal(pid, "SIGTERM");
      },
      dispose: async () => {
        if (disposed) return;
        disposed = true;
