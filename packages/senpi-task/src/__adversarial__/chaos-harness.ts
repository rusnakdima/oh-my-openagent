...
import {
  createChaosNotifier,
  createNotificationEpochTracker,
  instrumentCompletionNotifier,
  ...
  return {
    ...
    advance() {
      ...
      for (const [controller, abortAtStep] of scheduled) {
        ...
        controller.abort(
          new DOMException("chaos parent wait aborted", "AbortError"),
        );
      }
    },
    abortAll() {
      for (const controller of scheduled.keys()) {
        controller.abort(
          new DOMException("chaos parent wait drained", "AbortError"),
        );
      }
      scheduled.clear();
    },
...
function observePendingCancellations(
  store: TaskRecordStore,
  taskIds: Set<string>,
): TaskRecordStore {
  return {
    ...store,
    appendEvent: (taskId, event: PersistedTaskEvent) => {
      if (
        event.type === "cancelled" && typeof event.payload === "object" &&
        event.payload !== null &&
...
export function buildHarness(options: ChaosHarnessOptions): ChaosHarness {
  ...
  const observed = createObservingStore(
    createTaskRecordStore({ project_dir: project }),
  );
  const pendingCancelledTaskIds = new Set<string>();
  const store = observePendingCancellations(
    observed.store,
  ...
  const lifecycleObservations: LifecycleChaosObservations = {
    actionCounts: {
      ...
      resume_session: 0,
      crash_mid_suspend: 0,
      sibling_reconcile_race: 0,
      mass_revive_at_cap: 0,
    },
    liveHandleBreaches: [],
    pidBreaches: [],
    reclamationBreaches: [],
  ...
  if (primary === undefined || probeEngine === undefined) {
    throw new Error("chaos engine unavailable");
  }
  ...
  const parentNotifier = createChaosNotifier(
    store,
    observed.observations,
    notificationEpochs,
  );
  const notifier = instrumentCompletionNotifier(
...
function makeHarnessResult(input: HarnessResultInput): ChaosHarness {
  ...
  return {
    ...
    parentNotifier: input.parentNotifier,
    retryScheduler: input.retryScheduler,
    waiters: input.waiters,
    runner: primary.inProcessRunner,
    registry: primary.registry,
    engines: input.engines,
    probeEngine: input.probeEngine,
    processes: input.processes,
    lifecycleObservations: input.lifecycleObservations,
    observations: input.observed,
    store,
    pendingCancelledTaskIds: input.pendingCancelledTaskIds,
    residencyMax: input.options.residencyMax,
      ...
      for (const [taskId, ids] of owners) {
        if (ids.length > 1) {
          ...
          input.lifecycleObservations.liveHandleBreaches.push(
            `${taskId} live in ${ids.join(",")} host=${
              record?.host_pid ?? "none"
            } status=${record?.status ?? "missing"}/${
              record?.residency_state ?? "missing"
            } after ${trace}`,
          );
        }
      }
