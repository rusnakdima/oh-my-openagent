...
describe("W1-V chaos bench", () => {
  ...
  beforeAll(() => {
    ...
    console.log(
      `[chaos] seed=${seedLabel} base=${baseSeed} iterations=${ITERATIONS}`,
    );
  });
  ...
  test(
    ...
    async () => {
      ...
      const seamHarness = buildHarness({
        concurrency: 1,
        residencyMax: 1,
        maxDepth: 1,
      });
      ...
      for (let iteration = 0; iteration < ITERATIONS; iteration += 1) {
        ...
        for (const violation of report.violations) {
          failures.push(
            `iter ${iteration} seed=${seed}: inv${violation.invariant} ${violation.detail}`,
          );
        }
        for (const record of rejections.slice(rejectionsBefore)) {
          failures.push(
            `iter ${iteration} seed=${seed}: inv4 unhandled rejection: ${record.reason}`,
          );
        }
      }
      ...
      if (failures.length > 0) {
        console.error(
          `[chaos] ${failures.length} violation(s):\n${failures.join("\n")}`,
        );
      }
