...
function managerRegistry(getManager: () => TaskManager): ResidencyRegistry {
  ...
  return {
    ...
    entries: () =>
      getManager().residentTaskIds().map((id) =>
        adapt(getManager().getResidentHandle(id))
  ...
  for (let index = 0; index < 3; index += 1) {
    ...
    const getManager = (): ReturnType<typeof createTaskManager> => {
      if (managerRef === undefined) {
        throw new Error(`${id} manager unavailable`);
      }
      return managerRef;
    };
    ...
    const lifecycle = createTaskLifecycle({
      ...
      config: input.config,
      now: input.clock,
      signaller: input.processes,
      hostPid,
      orphanKillDelayMs: 0,
      respawn: (record, sessionPath) =>
        getManager().respawn(record, sessionPath),
    ...
    managerRef = createTaskManager({
      ...
      planner: singleModelPlanner(input.model),
      config: input.config,
      cwd: input.project,
      now: input.clock,
      destruction,
      admit: async (parentSessionId) => {
        ...
        return result.kind === "rejected"
          ? { kind: "rejected", message: result.error.message }
          : result;
      },
      rpcRespawnRunner: new ChaosRpcRespawnRunner(
        input.store,
        input.processes,
        input.observations,
      ),
