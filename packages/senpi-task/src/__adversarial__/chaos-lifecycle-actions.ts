...
export async function suspendSession(state: ChaosState): Promise<void> {
  ...
  if (
    state.harness.store.list().records.some((record) =>
      record.status === "pending"
    )
  ) return;
  await state.harness.engines[0]?.lifecycle.suspendOnSessionShutdown({
    parentSessionId: CHAOS_SESSION,
    reason: "quit",
  });
}
...
export async function resumeSession(
  ...
  await state.harness.engines[1]?.lifecycle.reconcileOnSessionStart(
    parentSessionId,
  );
...
export async function crashMidSuspend(state: ChaosState): Promise<void> {
  ...
  const id = pick(
    ...
    candidates(
      state,
      (record) =>
        record.execution_mode === "process" && record.status === "running" &&
        record.residency_state === "resident",
    ),
  );
  if (id === undefined) return;
  const engine = state.harness.engines.find((candidate) =>
    candidate.manager.getResidentHandle(id) !== undefined
  );
...
export async function siblingReconcileRace(state: ChaosState): Promise<void> {
  ...
  await Promise.all(
    state.harness.engines.slice(0, 2).map((engine) =>
      engine.lifecycle.reconcileOnSessionStart(CHAOS_SESSION)
    ),
  );
  state.harness.observeLiveHandles();
}
...
function syntheticId(state: ChaosState, offset: number): string {
  const suffix = (0xca00 + state.taskIds.length + offset).toString(16).padStart(
    4,
    "0",
  );
...
export async function massReviveAtCap(state: ChaosState): Promise<void> {
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    state.harness.store.save({
      ...
      spawn_spec: {
        version: 1,
        cwd: state.harness.store.stateDir,
        prompt: `mass ${index}`,
      },
  ...
  const resident = state.harness.store.list().records.find((record) =>
    ...
    record.residency_state === "resident" && record.status === "running"
  );
  if (resident === undefined) return;
  const owner = state.harness.engines.find((engine) =>
    engine.manager.getResidentHandle(resident.task_id) !== undefined
  );
  owner?.manager.forget(resident.task_id);
  const before = state.harness.store.load(resident.task_id)?.notification
    .run_epoch;
  ...
  if (before !== undefined && after?.notification.run_epoch !== before + 1) {
    state.seamBreaches.push(
      `reclaimed ${resident.task_id} epoch did not advance exactly once`,
    );
  }
  ...
  const killedOwner = state.harness.engines.find((engine) =>
    engine.manager.getResidentHandle(killed.task_id) !== undefined
  );
  killedOwner?.manager.forget(killed.task_id);
  state.harness.store.mutate(
    killed.task_id,
    (fresh) => ({ ...fresh, killed: true }),
  );
  await killedOwner?.lifecycle.reconcileOnSessionStart(CHAOS_SESSION);
  if (
    state.harness.store.load(killed.task_id)?.residency_state !== "disposed"
  ) {
    state.harness.observations.lifecycleBreaches.push(
      `killed orphan ${killed.task_id} was not disposed`,
    );
  }
}
