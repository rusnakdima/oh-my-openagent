...
async function drain(state: ChaosState): Promise<void> {
  ...
  for (let round = 0; round < cap; round += 1) {
    ...
    for (const engine of state.harness.engines) {
      for (const handle of engine.inProcessRunner.allHandles) {
        handle.settle({ status: "completed", finalResponse: "drain" });
      }
      for (const handle of engine.processRunner.allHandles) {
        handle.settle({ status: "completed", finalResponse: "drain" });
      }
    }
    ...
    state.harness.notifier.flushBuffered({
...
export type LifecycleProbeReport = {
  readonly actionCounts:
    ChaosState["harness"]["lifecycleObservations"]["actionCounts"];
  readonly violations: readonly Violation[];
};

async function startProbeTask(
  state: ChaosState,
  ...
  if (result.kind !== "started") {
    throw new Error(`probe start failed: ${result.kind}`);
  }
...
export async function runLifecycleMutationProbe(
  mutation: LifecycleMutation,
): Promise<LifecycleProbeReport> {
  ...
  try {
    ...
    if (mutation === "double_revive") {
      ...
      if (first === undefined || second === undefined) {
        throw new Error("mutation engines unavailable");
      }
      state.harness.store.mutate(
        started,
        (record) => ({ ...record, host_pid: second.hostPid }),
      );
      ...
      const handle = await second.inProcessRunner.start({
        taskId: started,
        cwd: state.harness.store.stateDir,
        stateDir: state.harness.store.stateDir,
        prompt: "double claim",
        depth: 1,
        parentSessionId: CHAOS_SESSION,
        rootSessionId: CHAOS_SESSION,
      });
      ...
      if (record !== null) {
        state.harness.store.replace({
          ...record,
          status: "lost",
          error_message: "mutant",
        });
      }
      ...
      if (record !== null) {
        state.harness.store.replace({
          ...
          notification: { ...record.notification, run_epoch: 5 },
        });
      }
      ...
      if (owner === undefined) {
        throw new Error("capacity-gate mutation owner unavailable");
      }
      seedMutationRecord(state, taskId, {
        residency_state: "resident",
        host_pid: owner.hostPid,
      });
      ...
      if (admission.kind !== "rejected") {
        throw new Error("capacity-gate mutation did not reach a full cap");
      }
