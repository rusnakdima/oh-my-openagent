...
export function createNotificationEpochTracker(): NotificationEpochTracker {
  return {
    ...
    bufferedByTask: new Map(),
    detailEpochs: new WeakMap(),
  };
}

function rememberBufferedEpoch(
  tracker: NotificationEpochTracker,
  taskId: string,
  epoch: number,
): void {
...
function resolveDetailEpoch(
  ...
  const epoch = bufferedEpoch ??
    store.load(detail.task_id)?.notification.run_epoch ?? 0;
...
export function createChaosNotifier(
  ...
  return {
    ...
    enqueue(message) {
      const tagged = message.details.map((detail) => ({
        detail,
        epoch: resolveDetailEpoch(epochs, store, detail),
      }));
      ...
      for (const entry of tagged) {
        ...
        observations.enqueueByEpoch.set(
          key,
          (observations.enqueueByEpoch.get(key) ?? 0) + 1,
        );
      }
    },
  };
  ...
  return {
    notifyTerminal(request) {
      ...
      try {
        ...
        if (result.kind === "buffered") {
          rememberBufferedEpoch(
            epochs,
            taskId,
            request.record.notification.run_epoch,
          );
        }
        return result;
      } finally {
    ...
    reconcileUnnotifiedNotifications(input) {
      ...
      for (const record of records) {
        epochs.activeByTask.set(record.task_id, record.notification.run_epoch);
      }
      try {
        notifier.reconcileUnnotifiedNotifications(input);
      } finally {
        for (const record of records) {
          epochs.activeByTask.delete(record.task_id);
        }
      }
    },
...
function checkExactlyOnce(state: ChaosState): Violation[] {
  ...
  for (const [key, count] of state.harness.observations.enqueueByEpoch) {
    if (count > 1) {
      violations.push({
        invariant: 1,
        detail: `parent enqueued ${count} times for (task:epoch) ${key}`,
      });
    }
  }
  for (const [key, count] of state.harness.observations.notifyCommits) {
    if (count > 1) {
      violations.push({
        invariant: 1,
        detail: `notification persisted ${count} times for ${key}`,
      });
    }
  }
  for (const breach of state.seamBreaches) {
    violations.push({ invariant: 1, detail: breach });
  }
  return violations;
}
...
async function checkNoSlotLeak(state: ChaosState): Promise<Violation[]> {
  ...
  if (pending.length > 0) {
    violations.push({
      invariant: 3,
      detail: `${pending.length} task(s) still pending after drain`,
    });
  }
  const stuck = records.filter((record) =>
    !isTerminalStatus(record.status) && record.residency_state !== "disposed" &&
    record.residency_state !== "evicted"
  );
  if (stuck.length > 0) {
    const detail = stuck.map((record) => {
      const owners = state.harness.engines.filter((engine) =>
        engine.manager.getResidentHandle(record.task_id) !== undefined
      ).map((engine) => engine.id);
      return `${record.task_id}:${record.status}/${record.residency_state}:host=${
        record.host_pid ?? "none"
      }:handles=${owners.join("+") || "none"}`;
    }).join(",");
    violations.push({
      invariant: 3,
      detail: `${stuck.length} non-terminal task(s) after drain: ${detail}`,
    });
  }
  ...
  if (queued > 0) {
    violations.push({
      invariant: 3,
      detail:
        `slot leak: ${queued}/${state.harness.limit} probe task(s) queued despite all prior tasks terminal`,
    });
  }
  for (const id of probeIds) {
    for (
      const handle of state.harness.probeEngine.inProcessRunner.allHandles
        .filter((entry) => entry.handle.task_id === id)
    ) {
      handle.settle({ status: "completed", finalResponse: "probe" });
    }
    for (
      const handle of state.harness.probeEngine.processRunner.allHandles.filter(
        (entry) => entry.handle.task_id === id,
      )
    ) {
      handle.settle({ status: "completed", finalResponse: "probe" });
    }
...
function checkWaitersAndPendingCancellation(state: ChaosState): Violation[] {
  ...
  if (settlements !== registrations) {
    violations.push({
      invariant: 5,
      detail:
        `waitFor settled ${settlements}/${registrations} registered waiter(s)`,
    });
  }
  ...
  for (const taskId of state.harness.pendingCancelledTaskIds) {
    if (started.has(taskId)) {
      violations.push({
        invariant: 5,
        detail: `task ${taskId} cancelled from pending reached runner start`,
      });
    }
  }
  return violations;
...
function checkLifecycleInvariants(state: ChaosState): Violation[] {
  ...
  for (const detail of state.harness.observations.lifecycleBreaches) {
    const invariant = detail.includes("recoverable")
      ? 7
      : detail.includes("run_epoch") || detail.includes("suspension")
      ? 8
      : 10;
  ...
  const referencedPids = new Set(
    state.harness.store.list().records.flatMap((record) =>
  ...
  for (const pid of state.harness.processes.alive) {
    if (pid >= 20_000 && !referencedPids.has(pid)) {
      violations.push({
        invariant: 9,
        detail: `orphan fake pid ${pid} remained alive after reconcile`,
      });
    }
  }
  const killedResidents = state.harness.store.list().records.filter((record) =>
    record.killed === true && record.residency_state === "resident"
  );
  for (const record of killedResidents) {
    violations.push({
      invariant: 10,
      detail: `killed task ${record.task_id} still pins a residency slot`,
    });
  }
  const maxResidents = state.harness.observations.maxResidentsByParent.get(
    state.harness.sessionId,
  ) ?? 0;
  if (maxResidents > state.harness.residencyMax) {
    violations.push({
      invariant: 11,
      detail:
        `resident count ${maxResidents} exceeded cap ${state.harness.residencyMax}`,
    });
  }
  for (
    const detail of state.harness.lifecycleObservations.reclamationBreaches
  ) {
  ...
  for (const detail of state.harness.lifecycleObservations.terminalRelaunches) {
    violations.push({
      invariant: 13,
      detail: `terminal record without session relaunched: ${detail}`,
    });
  }
  return violations;
}

export async function collectInvariantViolations(
  state: ChaosState,
): Promise<Violation[]> {
  return [
