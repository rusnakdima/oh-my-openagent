...
const [stateDir, parentSessionId, capRaw, barrierDir, tag] = process.argv.slice(
  2,
);
...
if (
  ...
  process.stderr.write(
    "Expected argv: [stateDir, parentSessionId, cap, barrierDir, tag]\n",
  );
  process.exit(1);
}
...
const claimed = result.outcomes.filter((outcome) => outcome.kind === "claimed")
  .map((outcome) => outcome.task_id);
process.stdout.write(
  `${JSON.stringify({ pid: process.pid, lease: result.lease, claimed })}\n`,
);
process.exit(0);
