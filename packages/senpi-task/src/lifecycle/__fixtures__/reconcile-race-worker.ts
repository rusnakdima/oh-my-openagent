...
if (
  projectDir === undefined || parentSessionId === undefined ||
  launchLog === undefined
) {
  throw new Error(
    "usage: reconcile-race-worker <projectDir> <parentSessionId> <launchLog>",
  );
}

const backing = createTaskRecordStore({
  project_dir: projectDir,
  task: { state_dir: projectDir },
});
let firstList = true;
