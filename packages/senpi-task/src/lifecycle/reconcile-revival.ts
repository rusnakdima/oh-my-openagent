...
export async function reconcileScopedRevival(
  ...
  const excludedFromAdmission = new Set(
    context.reconcileAdmission.excludeTaskIds,
  );
  for (const observed of sessionRecords) {
    if (
      !isSuspended(observed) || !TERMINAL_STATUSES.has(observed.status) ||
      observed.status === "lost"
    ) continue;
    if (
      observed.killed === true || sessionPathFor(observed.task_id) !== undefined
    ) continue;
    excludedFromAdmission.add(observed.task_id);
    const disposal = disposeSuspendedTerminalWithoutTranscript(
      context,
      observed,
    );
  ...
  if (context.config.reattach_on_reconcile === false) {
    outcomes.push(
      ...candidates.map(({ record }) =>
        deferred(record.task_id, "reattach_disabled")
      ),
    );
    return outcomes;
  }

  const priorResidencies = new Map(
    candidates.map((
      candidate,
    ) => [candidate.record.task_id, candidate.priorResidency]),
  );
  ...
  for (const outcome of admission.outcomes) {
    ...
    if (outcome.kind === "deferred") {
      outcomes.push(
        deferred(
          outcome.task_id,
          outcome.reason === "lease_lost" ? "lock_contended" : outcome.reason,
        ),
      );
      continue;
    }
    ...
    outcomes.push(
      await reviveClaimed(
        ...
        claimed,
        priorResidency,
        sessionPathFor(claimed.task_id),
      ),
    );
  }
...
function suspendedCandidates(
  context: LifecycleContext,
  parentSessionId: string,
): readonly RevivalCandidate[] {
  return context.store.list().records.flatMap(
    (record): readonly RevivalCandidate[] => {
      if (
        record.parent_session_id !== parentSessionId || !isSuspended(record)
      ) return [];
      if (!REVIVABLE_STATUSES.has(record.status) || record.killed === true) {
        return [];
      }
...
function isSuspended(
  record: TaskRecord,
): record is TaskRecord & { readonly residency_state: SuspendedResidency } {
