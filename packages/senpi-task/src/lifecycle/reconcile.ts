...
import {
  beginLocalReclamation,
  reconcileScopedRevival,
} from "./reconcile-revival";
...
async function reconcileLegacyRecord(
  context: LifecycleContext,
  observed: TaskRecord,
): Promise<ReconcileOutcome> {
  if (observed.residency_state !== "resident") {
    return reconcileLegacyRecordExclusive(context, observed);
  }
  ...
  if (release === undefined) {
    return {
      ...
      kind: "foreign_live_owner",
      reason: "orphan ownership claim in flight",
    };
  }
  try {
...
async function reconcileLegacyRecordExclusive(
  context: LifecycleContext,
  observed: TaskRecord,
): Promise<ReconcileOutcome> {
  let record = observed;
  ...
  if (TERMINAL_STATUSES.has(record.status)) {
    return reconcileLegacyTerminal(context, record);
  }
  ...
  if (record.execution_mode !== "process") {
    ...
    return {
      task_id: record.task_id,
      kind: "lost",
      reason: "previous-process in-process",
    };
  }
  ...
  if (context.config.reattach_on_reconcile === false) {
    if (!alive) {
      await markLost(
        context,
        record,
        `rpc pid=${pid} is dead; mapping exit facts only`,
      );
      return {
        task_id: record.task_id,
        kind: "lost",
    ...
    await markLost(
      context,
      record,
      `rpc orphan pid=${pid} session=${
        record.child_session_id ?? "unknown"
      } heartbeat=${heartbeat}; reattach disabled, terminating orphan`,
    );
    return {
      task_id: record.task_id,
      kind: "lost_and_terminated",
      reason: `live orphan, heartbeat=${heartbeat}`,
    };
  }
  ...
  if (!await terminateClaimedPid(context, record)) {
    await markLost(
      context,
      record,
      `rpc orphan pid=${pid} could not be terminated`,
    );
    return {
      task_id: record.task_id,
      kind: "lost_and_terminated",
  ...
  if (sessionPath === undefined) {
    await markLost(
      ...
      `rpc orphan pid=${pid} session=${
        record.child_session_id ?? "unknown"
      } heartbeat=${heartbeat}; terminating before reattach`,
    );
    return {
      task_id: record.task_id,
      kind: "lost_and_terminated",
  ...
  return reattachLegacyRecord(
    context,
    context.store.load(record.task_id) ?? record,
    sessionPath,
  );
}

async function reconcileLegacyTerminal(
  context: LifecycleContext,
  record: TaskRecord,
): Promise<ReconcileOutcome> {
  if (record.status === "lost" || record.status === "cancelled") {
    ...
    return {
      task_id: record.task_id,
      kind: record.status === "lost" ? "lost" : "resumed",
      reason: `already ${record.status}`,
    };
  }
  const pid = record.pid;
  if (
    record.execution_mode !== "process" ||
    record.residency_state !== "resident" || pid === undefined
  ) {
  ...
  if (alive) {
    ...
    if (
      ...
      context.config.reattach_on_reconcile === false
    ) {
      await destroyResidentTask(context, record.task_id, "reconcile_lost");
      return {
        task_id: record.task_id,
        kind: "lost_and_terminated",
        reason: `terminal resident orphan pid ${pid}`,
      };
    }
    return reattachLegacyRecord(
      context,
  ...
  if (
    sessionPath !== undefined && context.config.reattach_on_reconcile !== false
  ) {
    return reattachLegacyRecord(context, record, sessionPath);
  }
  return { task_id: record.task_id, kind: "resumed" };
...
async function reattachLegacyRecord(
  ...
  const ports = context.reattachPorts ??
    getLifecycleReattachPorts(context.store);
  if (ports === undefined) {
    await markLost(context, record, "reattach ports unavailable");
    return {
      task_id: record.task_id,
      kind: "lost",
      reason: "reattach ports unavailable",
    };
  }
...
async function terminateClaimedPid(
  context: LifecycleContext,
  record: TaskRecord,
): Promise<boolean> {
  const pid = record.pid;
...
function hasForeignLiveOwner(
  context: LifecycleContext,
  record: TaskRecord,
): boolean {
  return record.host_pid !== undefined && record.host_pid !== context.hostPid &&
...
function hasLiveResidentHandle(
  context: LifecycleContext,
  taskId: string,
): boolean {
  return context.registry.get(taskId) !== undefined ||
    context.registry.entries().some((handle) => handle.task_id === taskId);
}

function isSuspended(record: TaskRecord): boolean {
  return record.residency_state === "persisted_only" ||
...
function newestSessionPath(
  ...
  const sessionDir = resolveChildSessionDir(
    join(context.store.stateDir, "children", taskId),
    taskId,
  );
  try {
    ...
    for (const entry of readdirSync(sessionDir, { withFileTypes: true })) {
      ...
      if (
        newest === undefined || mtimeMs > newest.mtimeMs ||
...
function heartbeatState(
  ...
  return context.now() - Date.parse(record.updated_at) < HEARTBEAT_FRESH_MS
    ? "fresh"
    : "stale";
}
