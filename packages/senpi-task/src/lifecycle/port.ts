...
export type ReattachResult =
  ...
  | {
    readonly ok: false;
    readonly kind: "already_attached" | "failed";
    readonly reason: string;
  };

export type RespawnPort = (
  record: TaskRecord,
  resumeSessionPath?: string,
) => Promise<RespawnResult>;
export type ReattachPort = (
  record: TaskRecord,
...
const registeredReattachPorts = new WeakMap<
  TaskRecordStore,
...
export function getLifecycleReattachPorts(
  store: TaskRecordStore,
): LifecycleReattachPorts | undefined {
...
export function injectedLifecycleReattachPorts(
  deps: LifecycleDeps,
): LifecycleReattachPorts | undefined {
  if (deps.respawn === undefined || deps.reattach === undefined) {
    return undefined;
  }
