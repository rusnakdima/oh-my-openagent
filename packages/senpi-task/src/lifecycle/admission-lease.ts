...
export function admissionLeasePath(
  stateDir: string,
...
export function resolveAdmissionLeaseTiming(
...
export async function acquireSessionAdmissionLease(
  ...
  for (;;) {
    if (
      tryCreateLease(path, { pid: process.pid, token, renewed_at: Date.now() })
    ) {
    ...
    const stale = observed === "corrupt" ||
      Date.now() - observed.renewed_at > timing.staleMs;
    if (stale && tryTakeover(path, observed, token, timing.staleMs)) {
      return { kind: "acquired", lease: startHolder(path, token, timing) };
    }
    if (Date.now() - startedAt >= timing.acquireTimeoutMs) {
...
function startHolder(
  ...
  const timer = setInterval(() => {
    try {
      withTaskRecordLock(path, () => {
        const fresh = readLeaseBody(path);
        if (
          fresh === "missing" || fresh === "corrupt" || fresh.token !== token
        ) {
          clearInterval(timer);
          return;
        }
  ...
  return {
    ...
    isOwner: () => {
      const fresh = readLeaseBody(path);
      return fresh !== "missing" && fresh !== "corrupt" &&
        fresh.token === token;
    },
    release: () => {
      clearInterval(timer);
...
function readLeaseBody(path: string): LeaseBody | "missing" | "corrupt" {
  ...
  if (
    typeof candidate.pid !== "number" || typeof candidate.token !== "string" ||
    typeof candidate.renewed_at !== "number"
  ) {
    return "corrupt";
  }
  return {
