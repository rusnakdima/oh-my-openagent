...
const raceChildFixturePath = resolve(
  import.meta.dir,
  "__fixtures__",
  "batch-admission-race-child.ts",
);
...
function parseRaceResult(output: string): ChildRaceResult {
  const line = output
    ...
    .at(-1);
  if (line === undefined) {
    throw new Error(`Child did not emit a JSON result:\n${output}`);
  }
...
describe("admitSuspendedBatch two-OS-process race", () => {
  test(
    ...
    async () => {
      ...
      while (
        !(existsSync(join(barrierDir, "ready-a")) &&
          existsSync(join(barrierDir, "ready-b")))
      ) {
        if (Date.now() > deadline) {
          throw new Error("children never signalled ready");
        }
      ...
      const fresh = createTaskRecordStore({
        project_dir: store.stateDir,
        task: { state_dir: store.stateDir },
      });
      const records = fresh.list().records.filter((record) =>
        record.parent_session_id === "parent-1"
      );
      expect(records.filter((record) => record.residency_state === "resident"))
        .toHaveLength(5);
      expect(
