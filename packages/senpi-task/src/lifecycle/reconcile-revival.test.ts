...
const hostPid = 2222;
const now = () => 9_000_000;
const raceWorkerPath = resolve(
  import.meta.dir,
  "__fixtures__",
  "reconcile-race-worker.ts",
);
...
function persistSession(store: TaskRecordStore, taskId: string): string {
  const directory = resolveChildSessionDir(
    join(store.stateDir, "children", taskId),
    taskId,
  );
...
function injectedHarness(options: {
  ...
  const lifecycle = createTaskLifecycle({
    ...
    respawn: async (record, sessionPath) => {
      ...
      return options.onRespawn?.(record, sessionPath) ??
...
describe("reconcileOnSessionStart scoped revival", () => {
  test("#given a suspended in-process child of the resumed session #when session start reconciles #then it resumes through the in-process port", async () => {
    ...
    // when
    const result = await harness.lifecycle.reconcileOnSessionStart(
      parentSessionId,
    );
    ...
    expect(result.outcomes).toContainEqual({
      task_id: record.task_id,
      kind: "resumed",
      reason: "respawned and reattached",
    });
    expect(harness.launches).toEqual([{ taskId: record.task_id, sessionPath }]);
    expect(harness.store.load(record.task_id)?.residency_state).toBe(
      "resident",
    );
  ...
  test("#given a legacy crash orphan of another session #when this session resumes #then the global process reattach still runs", async () => {
    ...
    // then
    expect(
      result.outcomes.find((entry) => entry.task_id === other.task_id)?.kind,
    ).toBe("resumed");
    expect(harness.launches).toContainEqual({
  ...
  test("#given two managers racing one suspended child #when both reconcile #then one claims and the other defers", async () => {
    ...
    expect(
      results.flatMap((entry) => entry.outcomes).map((entry) => entry.kind)
        .sort(),
    ).toEqual(["deferred", "resumed"]);
  });
  ...
  test("#given one in-process and one rpc child of this session plus another-session child #when reconciled #then only this session revives end to end", async () => {
    ...
    // then
    expect(harness.launches.map((launch) => launch.taskId).sort()).toEqual([
      ownInProcess.task_id,
      ownRpc.task_id,
    ]);
    expect(result.outcomes.filter((outcome) => outcome.kind === "resumed"))
      .toHaveLength(2);
    expect(harness.store.load("st_10000019")?.residency_state).toBe(
      "persisted_only",
    );
  });
  ...
  test("#given terminal no-transcript disposal contends once #when admission follows #then the prompt is never relaunched and disposal defers", async () => {
    ...
    const store: TaskRecordStore = {
      ...
      mutate: (taskId, mutation) => {
        if (taskId === record.task_id && mutations++ === 0) {
          throw new Error("one-shot disposal lock contention");
        }
  ...
  test("#given retryable respawn failure and rollback lock failure #when reconciled #then the outcome loudly names the failed rollback", async () => {
    ...
    const store: TaskRecordStore = {
      ...backing,
      mutate: (taskId, mutation) => {
        if (taskId === record.task_id && mutations++ === 1) {
          throw new Error("rollback lock failed");
        }
        return backing.mutate(taskId, mutation);
      },
    };
  ...
  test("#given one record lock throws #when a batch reconciles #then only that record defers and the other revives", async () => {
    ...
    const store: TaskRecordStore = {
      ...backing,
      mutate: (taskId, mutation) => {
        if (taskId === blocked.task_id) {
          throw new Error("record lock contended");
        }
        return backing.mutate(taskId, mutation);
      },
    };
    ...
    expect(
      result.outcomes.find((entry) => entry.task_id === healthy.task_id)?.kind,
    ).toBe("resumed");
  });
  ...
  test("#given a pending child without a transcript and with durable steering #when resumed twice #then it launches once from its prompt and drains steering in order", async () => {
    ...
    store.replace({
      ...withV1(store, seeded, "persisted effective prompt"),
      pending_steering: queue,
    });
    const lifecycle = createTaskLifecycle({
      store,
      registry: {
        get: (taskId) =>
          manager.getResidentHandle(taskId) === undefined ? undefined : {
            task_id: taskId,
            kind: "in-process",
            pid: undefined,
            abort: () =>
              manager.getResidentHandle(taskId)?.abort() ?? Promise.resolve(),
            dispose: () =>
              manager.getResidentHandle(taskId)?.dispose() ?? Promise.resolve(),
            terminate: async () => undefined,
          },
        entries: () => [],
    ...
    expect(inProcess.startedSpecs[0]?.prompt).toBe(
      "persisted effective prompt",
    );
    expect(inProcess.handles.get(seeded.task_id)?.steerCalls).toEqual([
      "first",
    ]);
    expect(inProcess.handles.get(seeded.task_id)?.followUpCalls).toEqual([
      "second",
    ]);
  ...
  test("#given legacy records without transcript or v1 spec #when resumed #then non-terminal is lost and terminal is disposed with its result", async () => {
    ...
    expect(harness.store.load(terminal.task_id)?.final_response).toBe(
      "durable result",
    );
    expect(harness.store.load(terminal.task_id)?.residency_state).toBe(
      "disposed",
    );
  });
  ...
  test("#given a terminal v1 record without transcript #when resumed #then it is disposed without rerunning its prompt", async () => {
    ...
    expect(
      result.outcomes.find((entry) => entry.task_id === record.task_id)?.reason,
    ).toContain("terminal without transcript");
  });
  ...
  test("#given a killed orphaned resident at the cap #when reconciled #then it is disposed and its slot can be reused", async () => {
    ...
    expect(harness.store.load(waiting.task_id)?.residency_state).toBe(
      "resident",
    );
  });
  ...
  test("#given a same-process orphan and a full cap #when its session resumes #then reclamation bypasses capacity and revives it", async () => {
    ...
    // then
    expect(
      result.outcomes.find((entry) => entry.task_id === orphan.task_id)?.kind,
    ).toBe("resumed");
    expect(harness.launches).toHaveLength(1);
  });

  test(
    "#given two OS processes observe one dead-owner resident #when their ownership CAS races #then exactly one respawns and the loser defers",
    async () => {
      ...
      await Promise.all(
        readers.map((read) =>
          withTimeout(read(), 10_000, "worker scan barrier")
        ),
      );
      ...
      expect(
        results.flatMap((result) => result.outcomes).map((outcome) =>
  ...
  test("#given reattach and child resume are disabled #when suspended records exist #then they remain suspended with deferred outcomes", async () => {
    // given
    const disabledReattach = injectedHarness({
      config: { reattach_on_reconcile: false },
    });
    const first = withV1(
      disabledReattach.store,
      seedRecord(disabledReattach.store, {
        task_id: "st_10000011",
        parent_session_id: parentSessionId,
        status: "running",
        residency_state: "persisted_only",
    ...
    const disabledResume = injectedHarness({
      config: { resume_children: false },
    });
    const second = withV1(
      disabledResume.store,
      seedRecord(disabledResume.store, {
        task_id: "st_10000012",
        parent_session_id: parentSessionId,
        status: "running",
        residency_state: "persisted_only",
    ...
    // when
    const reattachResult = await disabledReattach.lifecycle
      .reconcileOnSessionStart(parentSessionId);
    await disabledResume.lifecycle.reconcileOnSessionStart(parentSessionId);

    // then
    expect(reattachResult.outcomes).toContainEqual({
      task_id: first.task_id,
      kind: "deferred",
      reason: "reattach_disabled",
    });
    expect(disabledReattach.store.load(first.task_id)?.residency_state).toBe(
      "persisted_only",
    );
    expect(disabledResume.store.load(second.task_id)?.residency_state).toBe(
      "persisted_only",
    );
  ...
  test("#given ten suspended children and capacity eight #when resumed #then eight revive and overflow defers without loss", async () => {
    ...
    expect(
      result.outcomes.filter((entry) =>
        entry.kind === "deferred" && entry.reason === "capacity"
      ),
    ).toHaveLength(2);
    expect(
      harness.store.list().records.filter((record) => record.status === "lost"),
    ).toHaveLength(0);
  });
  ...
  test("#given the admission lease is held past the bounded wait #when the session resumes #then the batch defers without throwing", async () => {
    ...
    const held = await acquireSessionAdmissionLease(
      harness.store.stateDir,
      parentSessionId,
      {
        renewMs: 10,
    ...
    if (held.kind !== "acquired") {
      throw new Error("expected held admission lease");
    }
...
function lineReader(stream: ReadableStream<Uint8Array>): () => Promise<string> {
  ...
  return async () => {
    for (;;) {
      ...
      if (next.done) {
        throw new Error("worker stdout ended before the expected line");
      }
