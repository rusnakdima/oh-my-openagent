  ...
  async start(spec: RpcRunnerSpec): Promise<RpcChildHandle> {
    ...
    const handle: RpcChildHandle = {
      ...
      waitForExit: () =>
        Promise.resolve({
          kind: "clean",
          facts: { pid, code: 0, signal: null, stderrTail: "" },
        }),
      lastSeen: () => undefined,
...
type HarnessOptions = {
  ...
  readonly processRunner?: FakeRunner;
};

function createHarness(options: HarnessOptions) {
  const store = tempStore();
  seedProcessRecord(store, options.taskId, options.status);
  const sessionPath = options.sessions === false
    ? undefined
    : persistSessions(store, options.taskId);
  ...
  const lifecycle = createTaskLifecycle({
    store,
    registry: options.registry ?? new FakeRegistry(),
    config: settings(options.config),
    now,
    signaller: fakeSignaller(alive, signals),
    orphanKillDelayMs: 0,
  });
...
describe("reconcileOnSessionStart reattach", () => {
  test(" w2reattach #given a dead process and persisted sessions #when reconciled #then the newest session is respawned running at epoch plus one", async () => {
    // given
    const { store, sessionPath, respawnRunner, manager, lifecycle } =
      createHarness({ taskId: "st_00000001" });
    ...
    if (sessionPath === undefined) {
      throw new Error("expected a persisted session path");
    }
  ...
  test(" w2reattach #given a dead process without a session #when reconciled #then it remains lost without respawn", async () => {
    // given
    const { store, respawnRunner, lifecycle } = createHarness({
      taskId: "st_00000002",
      sessions: false,
    });
  ...
  test(" w2reattach #given a live foreign process and persisted session #when reconciled #then it is terminated before respawn reattach", async () => {
    // given
    const { store, respawnRunner, signals, lifecycle } = createHarness({
      taskId: "st_00000003",
      alive: true,
    });
  ...
  test(" w2reattach #given an orphan whose reattach respawn then fails #when reconciled #then the true terminal reason replaces the first lost reason", async () => {
    ...
    const lifecycle = createTaskLifecycle({
      ...
      config: settings(),
      now,
      signaller: fakeSignaller(new Set([901]), []),
      orphanKillDelayMs: 0,
    });
    ...
    expect(result.outcomes[0]?.kind).toBe("lost");
    expect(store.load(taskId)?.error_message).toBe(
      "reattach failed: persisted spawn spec unavailable",
    );
  ...
  test(" w2reattach #given this process already owns the live handle #when reconciled #then the record is skipped without signalling or respawn", async () => {
    ...
    const { store, signals, lifecycle } = createHarness({
      taskId: "st_00000006",
      alive: true,
      registry,
    });
  ...
  test(" w2reattach #given a completed resident daemon with a dead pid #when reconciled #then its process returns while the record stays terminal", async () => {
    // given
    const { store, manager, lifecycle } = createHarness({
      taskId: "st_00000007",
      status: "completed",
    });
  ...
  test(" w2reattach #given a completed resident daemon with a live foreign pid #when reconciled #then it is terminated before reattach", async () => {
    // given
    const { store, manager, signals, lifecycle } = createHarness({
      taskId: "st_0000000a",
      status: "completed",
      alive: true,
    });
  ...
  test(" w2reattach #given overlapping reconcile sweeps #when ownership claims race #then exactly one child respawns and the loser defers", async () => {
    // given
    const { store, respawnRunner, manager, lifecycle } = createHarness({
      taskId: "st_0000000b",
    });

    // when
    const results = await Promise.all([
      lifecycle.reconcileOnSessionStart(),
      lifecycle.reconcileOnSessionStart(),
    ]);
    ...
    expect(respawnRunner.startedSpecs).toHaveLength(1);
    expect(
      respawnRunner.controls.map((
        control,
      ) => [control.terminated(), control.disposed()]),
    )
      .toEqual([[0, 0]]);
    expect(
  ...
  test(" w2reattach #given one trusted launch resolver rejection #when reconciling multiple process tasks #then later tasks still reattach", async () => {
    ...
    createTaskManager({
      ...
      trustedRespawnLaunch: async (record) => {
        if (record.task_id === rejectedRecord.task_id) {
          throw new Error("current team runtime unavailable");
        }
        return undefined;
      },
    });
    ...
    // then
    expect(
      result.outcomes.find((outcome) =>
        outcome.task_id === rejectedRecord.task_id
      )?.kind,
    ).toBe("lost");
    expect(
      result.outcomes.find((outcome) =>
        outcome.task_id === healthyRecord.task_id
      )?.kind,
    ).toBe("resumed");
    expect(respawnRunner.startedSpecs.map((spec) => spec.task_id)).toEqual([
      healthyRecord.task_id,
    ]);
  });
  ...
  test(" w2reattach #given a team member whose current runtime is missing #when reconciling #then it is lost without blocking later records", async () => {
    ...
    // then
    expect(
      result.outcomes.find((outcome) => outcome.task_id === teamRecord.task_id)
        ?.kind,
    ).toBe("lost");
    expect(
      result.outcomes.find((outcome) =>
    ...
    expect(respawnRunner.startedSpecs[0]?.extensions).toEqual([
      "/trusted/provider-extension.js",
    ]);
  ...
  test(" w2reattach #given one concurrency slot and two queued tasks #when a reattached task completes #then only the first queued task starts", async () => {
    // given
    const { store, respawnRunner, manager, inProcess, lifecycle } =
      createHarness({
        taskId: "st_00000009",
        concurrency: 1,
    ...
    if (firstQueued.kind !== "started" || secondQueued.kind !== "started") {
      throw new Error("expected queued tasks");
    }
    ...
    inProcess.handles.get(firstQueued.task_id)?.settle({
      status: "completed",
      finalResponse: "done",
    });
    await flush();
...
describe("reconcileOnSessionStart cross-process ownership", () => {
  ...
  function crossProcessHarness(
    options: { readonly host_pid?: number; readonly ownerAlive: boolean },
  ) {
    const store = tempStore();
    seedRecord(store, {
      task_id: "st_00000021",
    ...
    if (options.ownerAlive && options.host_pid !== undefined) {
      alive.add(options.host_pid);
    }
  ...
  test("#given a running process-mode record owned by a LIVE foreign process #when reconciled #then it is skipped with no signals and no loss", async () => {
    ...
    const { store, calls, alive } = crossProcessHarness({
      host_pid: foreignPid,
      ownerAlive: true,
    });
    const lifecycle = createTaskLifecycle({
