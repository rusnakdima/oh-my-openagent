...
export async function suspendOnSessionShutdown(
  ...
  if (context.config.resume_children === false) {
    return disposeAllOnShutdown(context);
  }
  ...
  for (const handle of [...context.registry.entries()]) {
    ...
    if (
      record.killed === true || record.status === "cancelled" ||
      record.status === "lost"
    ) {
      try {
...
async function suspendHandle(
  ...
  context.store.appendEvent(handle.task_id, {
    type: "suspended",
    payload: { reason },
  });
}
...
async function disposeAllOnShutdown(
  context: LifecycleContext,
): Promise<SuspendSummary> {
  ...
  log("senpi-task session shutdown dispose-all (resume_children=false)", {
...
async function bestEffort(
  ...
  try {
    ...
    log("senpi-task suspend pre-dispose step rejected", {
      taskId,
      step,
      error: String(error),
    });
