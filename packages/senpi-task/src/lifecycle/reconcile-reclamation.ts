...
export const REVIVABLE_STATUSES = new Set([
  "pending",
  "running",
  "completed",
  "error",
...
async function reclaimResidentExclusive(
  ...
  if (
    claimed === null || claimed.host_pid !== context.hostPid ||
    claimed.residency_state !== "resident"
  ) {
    return deferred(observed.task_id, "foreign_live_owner");
  }

  if (
    claimed.killed === true || claimed.status === "cancelled" ||
    claimed.status === "lost"
  ) {
    ...
    return {
      ...
      reason: claimed.killed === true
        ? "killed orphan disposed"
        : `${claimed.status} orphan disposed`,
    };
  }
  ...
  if (TERMINAL_STATUSES.has(claimed.status) && sessionPath === undefined) {
    ...
    return {
      ...
      kind: "resumed",
      reason:
        "terminal without transcript disposed; persisted result preserved",
    };
  }

  const rollbackResidency: SuspendedResidency =
    claimed.execution_mode === "process" ? "rpc_detached" : "persisted_only";
  if (context.config.reattach_on_reconcile === false) {
    const marked = await markCrashedResident(
      context,
      claimed,
      "reattach disabled for crashed resident",
    );
    if (marked) {
      await destroyResidentTask(context, claimed.task_id, "reconcile_lost");
    }
...
export async function reviveClaimed(
  ...
  if (
    !isClaimHeld(context, fresh, claimed.parent_session_id) ||
    fresh.killed === true || !REVIVABLE_STATUSES.has(fresh.status)
  ) {
    return rollbackOrDeferred(
      context,
      claimed.task_id,
      rollbackResidency,
      "foreign_live_owner",
  ...
  if (sessionPath === undefined && !isSpawnSpecV1Record(fresh)) {
    if (TERMINAL_STATUSES.has(fresh.status)) {
      context.store.transition(fresh.task_id, {
        type: "dispose",
        timestamp: nowIso(context),
      });
      return {
...
async function terminateOldRpc(
  context: LifecycleContext,
  record: TaskRecord,
): Promise<boolean> {
...
async function markLost(
  ...
  context.store.mutate(record.task_id, (fresh) => {
    if (
      fresh.host_pid !== context.hostPid || fresh.residency_state !== "resident"
    ) return fresh;
...
export function isOrphan(
  ...
  if (record.host_pid === context.hostPid) {
    return !hasLiveHandle(context, record.task_id);
  }
  return record.host_pid === undefined ||
...
export function beginLocalReclamation(
  context: LifecycleContext,
  taskId: string,
): (() => void) | undefined {
...
export function deferred(
  taskId: string,
  reason: ReconcileDeferredReason,
): ReconcileOutcome {
