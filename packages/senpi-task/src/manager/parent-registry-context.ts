...
export type ChildModelRegistry = NonNullable<
...
export function createParentRegistrySessionContext(
  ...
  const provide = (spec: ManagedStartSpec): InProcessSessionContext => {
    ...
    const model = spec.model === undefined
      ? undefined
      : findModelReference(registry, spec.model);
  ...
  return Object.assign(provide, {
    resolveResumeContext: (spec: ManagedStartSpec) =>
      resolveResumeContext(resolveRegistry, spec),
  });
}
 ...
 */
export function findModelReference<TModel>(
  registry: ModelFinder<TModel>,
  modelReference: string,
): TModel | undefined {
  const slash = modelReference.indexOf("/");
  if (slash <= 0 || slash === modelReference.length - 1) return undefined;
  return registry.find(
    modelReference.slice(0, slash),
    modelReference.slice(slash + 1),
  );
}
...
export function resolveResumeContext(
  ...
  if (resolvedModel === undefined) {
    return {
      ok: false,
      code: "model_unavailable",
      reason: "spec carries no resolved_model to match against",
    };
  }
  ...
  if (model === undefined) {
    return {
      ...
      code: "model_unavailable",
      reason:
        `provider "${resolvedModel.provider}" model "${resolvedModel.model_id}" not found in the live registry`,
    };
  }
