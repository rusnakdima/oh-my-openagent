...
export function applyRuntimeFallbackEvent(
  ...
  if (
    event.type !== "retry_fallback_applied" || store.load === undefined ||
  ...
  const resolvedModel = parseModelSelector(
    selector,
    record.resolved_model?.source ?? "category",
  );
  if (resolvedModel === undefined) return;
  const fallbackModels = remainingFallbacks(
...
function parseModelSelector(
  ...
  return {
    ...
    ...(hasThinking
      ? {
        reasoning: selector.slice(colon + 1),
        reasoning_effort: selector.slice(colon + 1),
      }
      : {}),
  };
}
  ...
  for (const candidate of [previous, selected]) {
    if (
      ...
      !next.some((attempt) =>
        ...
        attempt.model_id === candidate.model_id
      )
    ) {
      next.push(candidate);
    }
