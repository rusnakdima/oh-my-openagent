...
describe("TaskManager.continueTask", () => {
  ...
  test("#given a cancelled child #when continued #then it is not continuable and suggests task_output", async () => {
    ...
    const started = await manager.start(baseSpec());
    if (started.kind !== "started") throw new Error("expected started");
    store.transition(started.task_id, {
      type: "cancel",
    ...
    if (result.kind !== "not_continuable") {
...
describe("TaskManager.sendToTask one-shot refusal", () => {
  test("#given a momus child started through manager.start #when sent #then the outcome is one_shot_agent (guards the subagent_type -> agent_type rename)", async () => {
    ...
    const outcome = await manager.sendToTask({
      idOrName: started.task_id,
      message: "mid-run note",
    });

    // then
    if (outcome.kind !== "one_shot_agent") {
      throw new Error("expected one_shot_agent");
    }
    expect(outcome.task_id).toBe(started.task_id);
    expect(outcome.agent).toBe("momus");
    expect(outcome.message).toBe(
      AGENT_INTERACTION_POLICIES.momus.sendDenialReminder,
    );
    expect(inProcess.handles.get(started.task_id)?.followUpCalls).toEqual([]);
    expect(inProcess.handles.get(started.task_id)?.steerCalls).toEqual([]);
  });
...
describe("TaskManager.list and get", () => {
  test("#given tasks across two parents #when listing a parent scope #then only that parent's tasks appear", async () => {
    // given
    const { manager } = makeManager({});
    const mine = await manager.start(
      baseSpec({ parent_session_id: "parent-1", name: "mine" }),
    );
    await manager.start(
      baseSpec({ parent_session_id: "parent-2", name: "theirs" }),
    );
  ...
  test("#given a queued task #when listing #then its queue position is exposed", async () => {
    ...
    const queuedEntry = listed.find((entry) =>
