...
import type { ResidencyRegistry, ResidentHandle } from "../lifecycle";
...
function toResidentHandle(
  handle: ManagedChildHandle | undefined,
): ResidentHandle | undefined {
...
function makeLifecycleManager(
  ...
  const getManager = (): TaskManager => {
    if (managerRef === undefined) {
      throw new Error("manager accessed before test composition finished");
    }
    return managerRef;
  };
  const registry: ResidencyRegistry = {
    ...
    entries: () =>
      getManager()
        .residentTaskIds()
        .map((taskId) =>
          toResidentHandle(getManager().getResidentHandle(taskId))
        )
  ...
  const manager = createTaskManager({
    ...
    cwd: project,
    destruction: {
      destroyResidentTask: (taskId) =>
        lifecycle.destroyResidentTask(taskId, "cancel"),
    },
    admit: async (parentSessionId): Promise<SpawnAdmission> => {
      ...
      switch (admission.kind) {
        ...
        case "evicted":
          return {
            kind: "evicted",
            evicted_task_id: admission.evicted_task_id,
          };
        case "rejected":
...
describe("TaskManager.start", () => {
  ...
  test("#given a full model slot #when a further same-model task starts #then it queues FIFO and starts when a slot frees", async () => {
    ...
    inProcess.handles.get(first.task_id)?.settle({
      status: "completed",
      finalResponse: "ok",
    });
    await flush();
  ...
  test("#given a runner whose start throws #when a task starts #then the slot is released, the record is error, and a failure event is logged", async () => {
    ...
    if (result.kind !== "start_failed") {
      throw new Error("expected start_failed");
    }
    expect(store.load(result.task_id)?.status).toBe("error");
    const jsonl = readFileSync(
      join(project, ".omo", "senpi-task", "logs", `${result.task_id}.jsonl`),
      "utf8",
    );
  ...
  test("#given a resolved model plan #when started #then manager metadata surfaces persist resolved_model without prompt payloads", async () => {
    ...
    const result = await manager.start(
      baseSpec({ prompt: "private prompt payload" }),
    );
    ...
    expect(manager.list({ scope: "all" })[0]?.record.resolved_model).toEqual(
      resolvedModel,
    );

    const rawRecord = readFileSync(
      join(store.stateDir, "tasks", `${result.task_id}.json`),
      "utf8",
    );
...
describe("TaskManager.waitFor", () => {
  ...
  test("#given two concurrent waiters for one task w2waitfor #when one signal aborts #then the surviving waiter still resolves on settle", async () => {
    ...
    const abandoned = manager.waitFor(started.task_id, {
      signal: controller.signal,
    });
...
describe("TaskManager child subscriptions", () => {
  test("#given a pending task #when its concurrency slot is promoted #then the deferred child listener attaches to its managed handle", async () => {
    ...
    if (first.kind !== "started" || queued.kind !== "started") {
      throw new Error("expected starts");
    }
    ...
    runner.handles.get(first.task_id)?.settle({
      status: "completed",
      finalResponse: "done",
    });
    await flush();
...
describe("TaskManager pending cancellation", () => {
  test("#given a queued task at the residency cap w2pend #when cancelled #then its wait settles cancelled and a later spawn is admitted", async () => {
    ...
    const { manager, store } = makeLifecycleManager(
      ...
      settings({
        ...
        max_depth: 1,
        residency_max_children: 2,
      }),
    );
    const first = await manager.start(baseSpec({ name: "running" }));
    const queued = await manager.start(
      baseSpec({ name: "queued", run_in_background: true }),
    );
    if (first.kind !== "started" || queued.kind !== "started") {
      throw new Error("expected started");
    }
    ...
    // when
    const cancelled = await manager.cancelTask(
      queued.task_id,
    ...
    expect(
      manager.list({ scope: "all" }).find((entry) =>
        entry.record.task_id === queued.task_id
      )?.queue_position,
    ).toBeUndefined();
    ...
    if (later.kind !== "started") {
      throw new Error("expected later spawn admitted");
    }
  ...
  test("#given a running task with one queued behind it w2pend #when the running task is cancelled #then the queued task receives the released slot", async () => {
    ...
    const queued = await manager.start(baseSpec({ name: "queued" }));
    if (running.kind !== "started" || queued.kind !== "started") {
      throw new Error("expected started");
    }
