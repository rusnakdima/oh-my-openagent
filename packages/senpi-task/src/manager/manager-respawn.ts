...
export async function respawnManagedTask(input: {
  ...
  if (input.record.execution_mode === "in-process") {
    return respawnInProcess({ ...input, sessionPath: input.sessionPath });
  }
...
async function respawnFresh(input: {
  ...
  if (!rebuilt.ok) {
    return failure("unrecoverable", rebuilt.code, rebuilt.reason);
  }
  if (input.record.execution_mode === "in-process") {
  ...
  try {
    const trusted = input.trustedLaunch === undefined
      ? undefined
      : await input.trustedLaunch(input.record);
    handle = await input.rpcRunner.start({
      ...
      ...(input.record.resolved_model?.variant === undefined
        ? {}
        : { variant: input.record.resolved_model.variant }),
      ...(trusted?.extensions === undefined
        ? {}
        : { extensions: trusted.extensions }),
      ...(trusted?.memberEnv === undefined
        ? {}
        : { memberEnv: trusted.memberEnv }),
    });
    ...
    log("senpi-task fresh rpc respawn failed", {
...
async function respawnProcess(input: {
  ...
  try {
    ...
    if (handle.switchSession === undefined) {
      return cleanupFailure(
        handle,
        "respawned RPC handle cannot switch sessions",
      );
    }
    const switched = await handle.switchSession(input.sessionPath);
    if (switched.cancelled) {
      return cleanupFailure(handle, "switch_session was cancelled");
    }
    await continueInterruptedTurn(
      input.record,
      input.sessionPath,
      adaptRpcHandle(handle),
    );
    return { ok: true, handle: adaptRpcHandle(handle) };
  } catch (error) {
    ...
    log("senpi-task rpc respawn failed", {
      taskId: input.record.task_id,
      error: String(error),
    });
    return failure(
      "retryable",
      "respawn_failed",
      cleaned ? "rpc respawn failed" : RESPAWN_CLEANUP_FAILURE_REASON,
    );
  }
}

async function continueInterruptedTurn(
  record: TaskRecord,
  sessionPath: string,
  handle: ManagedChildHandle,
): Promise<void> {
  if (
    !isTerminalRecord(record) && await sessionTailNeedsContinuation(sessionPath)
  ) {
...
async function cleanupFailure(
  ...
  return failure(
    "retryable",
    "respawn_failed",
    await disposeRpc(handle) ? reason : RESPAWN_CLEANUP_FAILURE_REASON,
  );
}

async function discardManagedHandleBestEffort(
  handle: ManagedChildHandle,
): Promise<void> {
  try {
    await discardManagedHandle(handle);
  } catch (error) {
    log("senpi-task fresh in-process respawn cleanup failed", {
...
async function disposeRpc(handle: RpcChildHandle): Promise<boolean> {
  try {
    ...
    log("senpi-task failed respawn cleanup rejected", {
      taskId: handle.task_id,
      error: String(error),
    });
    return false;
...
function classifyResumeFailure(error: unknown): RespawnResult {
  if (RunnerError.is(error)) {
    ...
    if (
      kind === "model_unavailable" || kind === "tools_unavailable" ||
...
function isTeamRuntimeUnavailable(error: unknown): boolean {
  if (
    ...
    error.name !== "TeamMemberRespawnLaunchError" || !("code" in error)
  ) return false;
...
export async function reattachManagedTask(input: {
  ...
  if (
    fresh?.host_pid !== input.hostPid || fresh.residency_state !== "resident"
  ) {
    await discardManagedHandle(input.handle);
    return {
      ok: false,
      kind: "failed",
      reason: "task ownership claim is not held by this host",
    };
  }
  if (input.isAttached(fresh.task_id)) {
    ...
    return {
      ...
      reason: "task already has a live handle",
    };
  }
  ...
  try {
    ...
    if (unsubscribe !== undefined) {
      input.detachLive(fresh.task_id, input.handle, unsubscribe);
    }
    await discardManagedHandle(input.handle);
    log("senpi-task reattach failed", {
