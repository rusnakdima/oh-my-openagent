...
export function adaptInProcessHandle(
...
export function adaptRpcHandle(handle: RpcChildHandle): ManagedChildHandle {
  ...
  return {
    ...
    subscribe: (listener) => handle.subscribe(listener),
    waitForOutcome: () =>
      handle.waitForOutcome === undefined
        ? rpcOutcome(handle)
        : handle.waitForOutcome(),
    ...(switchSession === undefined
      ? {}
      : { switchSession: (sessionPath: string) => switchSession(sessionPath) }),
    ...(getEntries === undefined
      ? {}
      : { getEntries: (since?: string) => getEntries(since) }),
    lastAssistantText: () => handle.lastAssistantText(),
...
async function rpcOutcome(handle: RpcChildHandle): Promise<RunnerOutcome> {
  ...
  if (exit !== undefined && exit.kind !== "clean") {
    ...
    return {
      status: "error",
      failure: { kind: "child-prompt-failed", message },
      killed: facts?.killed === true,
    };
  }
    ...
    return {
      ...
      failure: {
        ...
        message: terminal.errorMessage ??
          `child turn ended with stopReason "${terminal.stopReason}"`,
      },
    };
  }
  ...
  const finalResponse = hasTerminalReader
    ? terminal?.text
...
export async function discardManagedHandle(
