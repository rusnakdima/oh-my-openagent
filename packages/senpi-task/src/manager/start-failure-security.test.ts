...
import {
  ...
  CTX,
  makeDeps,
} from "../tools/task/__fixtures__/task-tool-fakes";
...
describe("TaskManager start failure security", () => {
  test("#given an unknown runner start error containing secrets #when the task tool reports the failure #then every public and persisted surface receives only the stable classification", async () => {
    ...
    // then
    if (capturedStart === undefined || capturedStart.kind !== "start_failed") {
    ...
    const eventLog = readFileSync(
      join(store.stateDir, "logs", `${capturedStart.task_id}.jsonl`),
      "utf8",
    );
    ...
    const persistedRecord = readFileSync(
      join(store.stateDir, "tasks", `${capturedStart.task_id}.json`),
      "utf8",
    );
    expect(
      JSON.stringify({ capturedStart, persistedRecord, eventLog, result, row }),
    ).not.toContain(ADVERSARIAL_ERROR);
  });
  ...
  test("#given a non-Error runner start throw containing secrets #when a team member spawn fails #then the team error contains only the stable classification", async () => {
    ...
    expect(result.failure?.message).toBe(
      `member 'alpha' failed to start: ${GENERIC_START_FAILURE}`,
    );
  ...
  test.each([
    ...
    [
      "ordinary Error",
      Object.assign(new Error(ADVERSARIAL_ERROR), {
        name: "RunnerError",
        failure: { kind: "depth-exceeded", message: ADVERSARIAL_ERROR },
      }),
    ],
