...
import {
  applyRuntimeFallbackEvent,
  type RuntimeFallbackStore,
} from "./runtime-fallback-event";
...
export function logTranscriptEvent(
  store: TranscriptLogStore,
  taskId: string,
...
export function subscribeTranscriptLog(
...
function toPersistedEvent(
  event: ManagedChildEvent,
): PersistedTaskEvent | undefined {
  if (event.type === "message_end") {
    const failure = assistantFailure(event.message);
    if (failure !== undefined) {
      return { type: TRANSCRIPT_ERROR_EVENT, payload: failure };
    }
    const text = assistantText(event.message);
    return text === undefined
      ? undefined
      : { type: TRANSCRIPT_ASSISTANT_EVENT, payload: { text } };
  }
  if (
    event.type === "tool_execution_end" && typeof event.toolName === "string"
  ) {
    return {
      type: TRANSCRIPT_TOOL_EVENT,
      payload: { tool: event.toolName, is_error: event.isError === true },
    };
  }
...
function assistantFailure(
  message: unknown,
): { readonly message: string; readonly stop_reason: string } | undefined {
