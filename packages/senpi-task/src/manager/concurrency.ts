...
export class TaskConcurrency {
  ...
  getLimit(model: string): number {
    ...
    if (modelLimit !== undefined) {
      return modelLimit === 0 ? Number.POSITIVE_INFINITY : modelLimit;
    }
    const providerLimit = ownNumber(
      this.#config.provider_concurrency,
      providerOf(model),
    );
    if (providerLimit !== undefined) {
      return providerLimit === 0 ? Number.POSITIVE_INFINITY : providerLimit;
    }
    const defaultLimit = this.#config.default_concurrency;
    if (defaultLimit !== undefined) {
      return defaultLimit === 0 ? Number.POSITIVE_INFINITY : defaultLimit;
    }
    return DEFAULT_LIMIT;
  }

  getKey(model: string): string {
    if (ownNumber(this.#config.model_concurrency, model) !== undefined) {
      return model;
    }
    const provider = providerOf(model);
    if (ownNumber(this.#config.provider_concurrency, provider) !== undefined) {
      return provider;
    }
    return model;
  }
  ...
  queuePosition(model: string, taskId: string): number | undefined {
    ...
    const index = queue.findIndex((waiter) =>
      waiter.taskId === taskId && !waiter.settled
    );
