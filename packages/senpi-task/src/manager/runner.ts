...
import { type ChildSpec, RunnerError } from "../runners/in-process";
...
export type InProcessSessionContextProvider =
  & ((spec: ManagedStartSpec) => InProcessSessionContext)
  & {
    readonly resolveResumeContext?: (
    ...
    async resume(
      ...
      if (runner.resume === undefined) {
        throw new RunnerError({
          ...
          message: "in-process runner cannot resume sessions",
        });
      }
...
function toChildSpec(
  spec: ManagedStartSpec,
  context: InProcessSessionContext,
): ChildSpec {
  return {
    taskId: spec.taskId,
    cwd: spec.cwd,
    ...
    ...(context.authStorage !== undefined
      ? { authStorage: context.authStorage }
      : {}),
    ...(context.modelRegistry !== undefined
      ? { modelRegistry: context.modelRegistry }
      : {}),
    ...(context.modelRuntime !== undefined
    ...
    ...(spec.memberScopedToolNames !== undefined
      ? { memberScopedToolNames: spec.memberScopedToolNames }
      : {}),
