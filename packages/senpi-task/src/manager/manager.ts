...
const NOOP_DESTRUCTION: DestructionPort = {
  destroyResidentTask: () => Promise.resolve(),
};
...
class TaskManagerImpl implements TaskManager {
  ...
  readonly #childSubscribers = new Map<
    string,
    Map<ManagedChildListener, () => void>
  >();
  ...
  constructor(options: TaskManagerImplOptions) {
    ...
    try {
      ...
      if (listed.diagnostics.length > 0) {
        log(
          ...
          { count: listed.diagnostics.length },
        );
      }
      const maxId = listed.records.reduce<string | undefined>(
        (
          maximum,
          record,
        ) => (maximum === undefined || record.task_id > maximum
          ? record.task_id
          : maximum),
        undefined,
      );
      ...
      log("senpi-task manager failed to seed task id floor", {
        error: String(error),
      });
    }
    ...
    this.#concurrency = new TaskConcurrency({
      ...
      ...(options.config.provider_concurrency !== undefined &&
        { provider_concurrency: options.config.provider_concurrency }),
      ...(options.config.model_concurrency !== undefined &&
        { model_concurrency: options.config.model_concurrency }),
    });
    const port: SteeringPort = {
      ...
      runStatsSnapshot: (taskId) =>
        this.#runStats.get(taskId)?.snapshot(this.#now()),
      now: this.#now,
    };
    ...
    this.#outcome = createOutcomeTracker({
      ...
      runStatsSnapshot: (taskId) =>
        this.#runStats.get(taskId)?.snapshot(this.#now()),
      releaseSlot: (taskId, model, epoch) =>
    ...
    registerLifecycleReattachPorts(options.store, {
      respawn: (record, resumeSessionPath) =>
        this.respawn(record, resumeSessionPath),
  ...
  async start(spec: ManagerStartSpec): Promise<StartResult> {
    ...
    if (resolution.kind === "error") {
      return { kind: "plan_unresolved", error: resolution.error };
    }

    if (this.#options.admit !== undefined) {
      const admission = await this.#options.admit(spec.parent_session_id);
      if (admission.kind === "rejected") {
        return { kind: "residency_denied", reason: admission.message };
      }
    }
  ...
  #ownedResult(owner: DagTaskOwner): OwnedStartResult | undefined {
    ...
    return {
      ...
      ...(record.resolved_model !== undefined
  ...
  async #startResolved(
    ...
    const executionMode: ExecutionMode = resolveExecutionMode({
      ...(spec.execution_mode !== undefined &&
        { specMode: spec.execution_mode }),
      ...(plan.agentExecutionMode !== undefined &&
    ...
    try {
      ...
      const claimDraft: TaskRecord = {
        ...draft,
        name: requestedRegistration?.name ?? draft.task_id,
      ...
      claimed = claimTaskRecord(this.#options.store, claimDraft, {
        ...
        ...(requestedRegistration === undefined
          ? {
            nameAvailable: (name) =>
              this.#names.isAvailable(spec.parent_session_id, name),
          }
          : {}),
      });
    } catch (error) {
      if (
        !(error instanceof TaskRecordCollisionError) &&
        !(error instanceof TaskIdSpaceExhaustedError)
      ) throw error;
      if (requestedRegistration !== undefined) {
        this.#names.release(spec.parent_session_id, requestedRegistration.name);
      }
      return {
        kind: "start_failed",
        task_id: "",
        name: requestedName ?? "",
        ...(spec.category ?? plan.category !== undefined
          ? { category: spec.category ?? plan.category }
          : {}),
        ...(spec.subagent_type ?? plan.agentType !== undefined
          ? { subagent_type: spec.subagent_type ?? plan.agentType }
          : {}),
    ...
    const registration = requestedRegistration ??
      this.#names.register(spec.parent_session_id, undefined, claimed.task_id);
    let finalRecord: TaskRecord;
    let managedSpec: ManagedStartSpec;
    try {
      const renamedRecord: TaskRecord = registration.name === claimed.name
        ? claimed
        : { ...claimed, name: registration.name };
      ...
      finalRecord = {
        ...renamedRecord,
        spawn_spec: buildSpawnSpecV1(managedSpec),
      };
      this.#options.store.replace(finalRecord);
      if (spec.run_in_background === true) {
        this.#background.add(finalRecord.task_id);
      }
    } catch (error) {
      if (registration.name !== claimed.name) {
        this.#names.release(spec.parent_session_id, registration.name);
      }
      this.#background.delete(claimed.task_id);
      const timestamp = nowIso(this.#now);
      const started = this.#options.store.transition(claimed.task_id, {
        type: "start",
        timestamp,
      });
      ...
      if (!started.applied || !failed.applied) {
        throw new Error(
      ...
      return {
        ...
        ...(claimed.category !== undefined
          ? { category: claimed.category }
          : {}),
        ...(claimed.agent_type !== undefined
          ? { subagent_type: claimed.agent_type }
          : {}),
        execution_mode: executionMode,
        model: claimed.model,
        ...(claimed.resolved_model !== undefined
          ? { resolved_model: claimed.resolved_model }
          : {}),
        run_in_background: spec.run_in_background === true,
        error_message: "spawn bookkeeping failed",
      };
    ...
    const startParts = {
      ...(plan.resolved_model !== undefined
        ? { resolved_model: plan.resolved_model }
        : {}),
      ...(registration.warning !== undefined
        ? { name_warning: registration.warning }
        : {}),
    };
    ...
    if (this.#concurrency.hasFreeSlot(plan.model)) {
      ...
      if (!launched.ok) {
        return {
          ...
          name: registration.name,
          ...(finalRecord.category !== undefined
            ? { category: finalRecord.category }
            : {}),
          ...(finalRecord.agent_type !== undefined
            ? { subagent_type: finalRecord.agent_type }
            : {}),
          execution_mode: executionMode,
          model: finalRecord.model,
          ...(finalRecord.resolved_model !== undefined
            ? { resolved_model: finalRecord.resolved_model }
            : {}),
          run_in_background: spec.run_in_background === true,
    ...
    const position = this.#concurrency.enqueue(
      plan.model,
      finalRecord.task_id,
      () => {
  ...
  async continueTask(
    ...
    const outcome = await this.#steering.sendToTask({
      idOrName: taskIdOrName,
      message: prompt,
      deliverAs,
    });
  ...
  list(scope: ListScope): readonly ListedTask[] {
    ...
    const filtered = scope.scope === "all"
      ? records
      : records.filter((record) => inSession(record, scope.session_id));
    return filtered.map((record) => {
      const position = record.status === "pending"
        ? this.#concurrency.queuePosition(record.model, record.task_id)
        : undefined;
  ...
  getResidentHandle(taskId: string): ManagedChildHandle | undefined {
    return this.#live.get(taskId)?.handle;
  }
  ...
  subscribeChild(taskId: string, listener: ManagedChildListener): () => void {
    ...
    const subscribers = this.#childSubscribers.get(taskId) ??
      new Map<ManagedChildListener, () => void>();
  ...
  runStatsSnapshot(taskId: string): TaskRunStats | undefined {
    return this.#runStats.get(taskId)?.snapshot(this.#now());
  }
  ...
  promoteToBackground(taskId: string): boolean {
    ...
    this.#options.store.mutate(taskId, (record) => {
      ...
      if (
        record.notify_on_terminal && record.background_mode === backgroundMode
      ) return record;
      return {
        ...record,
  ...
  reattach(
    ...
    return reattachManagedTask({
      ...
      attachLive: (fresh, attachedHandle) => {
        const unsubscribe = this.#subscribeChildFacts(
          attachedHandle,
          fresh.task_id,
        );
        this.#live.set(fresh.task_id, {
      ...
      detachLive: (taskId, attachedHandle, unsubscribe) => {
        unsubscribe();
        if (this.#live.get(taskId)?.handle === attachedHandle) {
          this.#live.delete(taskId);
        }
      },
  ...
  waitFor(
    ...
    if (signal?.aborted) {
      return Promise.reject(signal.reason ?? new Error("waitFor aborted"));
    }
    const id = parseTaskId(taskId);
    const current = this.#tryLoad(id);
    if (
      current !== null && current !== undefined && isTerminalRecord(current)
  ...
  async #launch(
    ...
    const startResult = this.#options.store.transition(record.task_id, {
      type: "start",
    ...
    if (current?.status === "cancelled") {
      ...
      await (this.#options.destruction ?? NOOP_DESTRUCTION).destroyResidentTask(
        record.task_id,
        "cancel",
      );
      this.#releaseSlot(record.task_id, model, record.notification.run_epoch);
  ...
  #recordSpawnFacts(taskId: string, handle: ManagedChildHandle): void {
    ...
    const updated: TaskRecord = spawnSpec === undefined ||
      ...
      : {
        ...
        spawn_spec: {
          cwd: spawnSpec.cwd,
          ...(spawnSpec.extensions === undefined
            ? {}
            : { extensions: spawnSpec.extensions }),
          ...(spawnSpec.memberEnv === undefined
            ? {}
            : { member_env: spawnSpec.memberEnv }),
        },
      };
  ...
  #subscribeChildFacts(handle: ManagedChildHandle, taskId: string): () => void {
    const transcript = subscribeTranscriptLog(
      handle,
      this.#options.store,
      taskId,
    );
...
export function createTaskManager(
  options: TaskManagerImplOptions,
): ReattachingTaskManager {
