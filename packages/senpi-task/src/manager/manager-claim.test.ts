...
import { createTaskRecordStore, TaskRecordCollisionError } from "../store";
...
function firstAllocationFailsStore(inner: TaskRecordStore): TaskRecordStore {
  ...
  return {
    ...
    save(record) {
      if (armed) {
        armed = false;
        throw new TaskRecordCollisionError({
          taskId: "st_ffffffff",
          path: "injected",
        });
      }
...
const seedFloorChildFixturePath = resolve(
  import.meta.dir,
  "__fixtures__",
  "seed-floor-child.ts",
);
...
async function expectSeedFloorChildModeToSucceed(mode: string): Promise<void> {
  const child = Bun.spawn([process.execPath, seedFloorChildFixturePath, mode], {
    stdout: "pipe",
    stderr: "pipe",
  });
...
describe("TaskManager claim characterization", () => {
  ...
  test("#given an id-shaped requested name #when an unnamed task claims its fallback #then every saved sibling name is unique", async () => {
    ...
    const invariantStore: TaskRecordStore = {
      ...
      save(record) {
        for (const sibling of inner.list().records) {
          if (
            sibling.parent_session_id === record.parent_session_id &&
            sibling.name === record.name
          ) {
    ...
    const clock = () =>
      (Math.floor(nowSnapshot / 65_536) + 10_000_000) * 65_536;
    const firstTaskId = `st_${
      Math.floor(clock() / 65_536).toString(16).padStart(8, "0")
    }`;
    const { manager } = managerWithStore(
      invariantStore,
      new FakeRunner(),
      new FakeRunner(),
      clock,
    ...
    const first = await manager.start(
      baseSpec({ name: bumpTaskId(firstTaskId as `st_${string}`) }),
    );
    const second = await manager.start(baseSpec());

    // then
    if (first.kind !== "started" || second.kind !== "started") {
      throw new Error("expected both tasks to start");
    }
  ...
  test.each(["", "   "])(
    "#given a blank requested name %p #when started #then it falls back to the task id",
    async (name) => {
      // given
      const { manager } = makeManager();
  ...
  test("#given a foreign winner for the first candidate #when started #then it claims the next id", async () => {
    ...
    expect(result.task_id).toBe(
      bumpTaskId(collision.firstCandidate() as `st_${string}`),
    );
    expect(result.name).toBe(result.task_id);
    expect(inner.load(collision.firstCandidate() as string)?.name).toBe(
      "foreign-winner",
    );
  ...
  test("#given a requested name and a foreign winner #when started #then only the loser keeps the requested name", async () => {
    ...
    // then
    if (result.kind !== "started") throw new Error("expected started");
    const records = inner.list().records.filter((record) =>
      record.parent_session_id === "parent-1"
    );
    expect(inner.load(collision.firstCandidate() as string)?.name).toBe(
      "foreign-winner",
    );
    expect(result.name).toBe("todo11");
    expect(records.filter((record) => record.name === "todo11")).toHaveLength(
      1,
    );
    expect(new Set(records.map((record) => record.name)).size).toBe(
      records.length,
    );
  });
  ...
  test("#given a real manager under collision #when team members spawn #then no member start is rejected", async () => {
    ...
    if (result.failure !== undefined) {
      expect(result.failure.message).not.toContain("member_start_rejected");
    }
  ...
  test("#given bookkeeping fails after a claim #when process mode starts #then it records a terminal failure", async () => {
    ...
    const result = await manager.start(
      baseSpec({ execution_mode: "process", run_in_background: true }),
    );
    ...
    expect(inner.load(result.task_id)?.error_message).toBe(
      "spawn bookkeeping failed",
    );
