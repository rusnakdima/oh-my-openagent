...
export function parseTaskRecord(
  ...
  if (!isRecord(value)) {
    throw new Error(`JSON record at ${path} is not an object`);
  }
  ...
  const notifyOnTerminal = readOptionalBoolean(value, "notify_on_terminal") ??
    false;
  const requestedModel = parseOptionalResolvedModel(value, "requested_model");
  const fallbackModels = parseOptionalResolvedModelArray(
    value,
    "fallback_models",
  );
  const fallbackAttempts = parseOptionalResolvedModelArray(
    value,
    "fallback_attempts",
  );
  ...
  const runStats = value["run_stats"] === undefined
    ? undefined
    : parseRunStats(value["run_stats"]);
  ...
  return {
    ...
    ...(pendingSteering !== undefined && pendingSteering.length > 0
...
function readOptionalBackgroundMode(
  ...
  switch (mode) {
    ...
    default:
      throw new Error(
        `background_mode must be one of ${BACKGROUND_MODES.join(", ")}`,
      );
  }
}
...
function readResidencyState(
  record: Record<string, unknown>,
): TaskRecord["residency_state"] {
