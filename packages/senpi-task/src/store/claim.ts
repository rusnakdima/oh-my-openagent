...
export function claimTaskRecord(
  ...
  for (;;) {
    ...
    while (
      options?.nameFollowsId === true && nameAvailable !== undefined &&
      !nameAvailable(candidate.name ?? candidate.task_id)
    ) {
    ...
    try {
      ...
      if (
        !(error instanceof TaskRecordCollisionError) || attempts >= maxAttempts
      ) throw error;
      const nextId = bumpTaskId(error.taskId);
      candidate = {
        ...candidate,
        task_id: nextId,
        ...(options?.nameFollowsId === true ? { name: nextId } : {}),
      };
    }
  }
}
