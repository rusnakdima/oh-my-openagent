...
export function parseRunStats(value: unknown): TaskRunStats {
  ...
  return {
    ...
    ...(cacheHitRateLast === undefined
      ? {}
      : { cache_hit_rate_last: cacheHitRateLast }),
    ...(resolvedCacheHitRateRun === undefined
      ? {}
      : { cache_hit_rate_run: resolvedCacheHitRateRun }),
...
function readTokenStatus(
  ...
  switch (status) {
    ...
    default:
      throw new Error(
        `run_stats.token_status must be one of ${
          TOKEN_COVERAGE_STATUSES.join(", ")
        }`,
      );
  }
}
...
function readCostStatus(
  ...
  switch (status) {
    ...
    default:
      throw new Error(
        `run_stats.cost_status must be one of ${
          COST_REPORT_STATUSES.join(", ")
        }`,
      );
  }
}

function readDurationStatus(
  record: Record<string, unknown>,
): DurationSourceStatus | undefined {
  ...
  switch (status) {
    ...
    default:
      throw new Error(
        `run_stats.duration_status must be one of ${
          DURATION_SOURCE_STATUSES.join(", ")
        }`,
      );
  }
}
