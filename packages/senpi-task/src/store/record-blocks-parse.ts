...
export function parseOptionalOwner(
  record: Record<string, unknown>,
): DagTaskOwner | undefined {
  const value = record["owner"];
  if (value === undefined) return undefined;
  if (!isRecord(value)) throw new Error("owner is not an object");
  if (readString(value, "kind") !== "dag") {
    throw new Error("owner.kind is not dag");
  }
  return {
    kind: "dag",
...
export function parseOptionalSpawnSpec(
  record: Record<string, unknown>,
): TaskSpawnSpec | undefined {
  ...
  if (value["version"] === 1) {
    ...
    const memberScopedToolNames = readOptionalStringArray(
      value,
      "member_scoped_tool_names",
    );
    return {
      version: 1,
      cwd,
      ...
      ...(memberScopedToolNames === undefined
        ? {}
        : { member_scoped_tool_names: memberScopedToolNames }),
    };
  }
...
export function parseOptionalPendingSteering(
  ...
  if (!Array.isArray(value)) {
    throw new Error("pending_steering is not an array");
  }
...
export function parseNotification(
  ...
  const failedEpoch = readOptionalNumber(
    notification,
    "notification_failed_epoch",
  );
  const livenessNotifiedEpoch = readOptionalNumber(
    notification,
  ...
  return {
    ...
    ...(failedEpoch === undefined
      ? {}
      : { notification_failed_epoch: failedEpoch }),
    ...(livenessNotifiedEpoch === undefined
      ? {}
      : { liveness_notified_epoch: livenessNotifiedEpoch }),
  };
}
...
function readResolvedModelSource(
  record: Record<string, unknown>,
): ResolvedModelRecord["source"] {
  ...
  switch (source) {
    ...
    default:
      throw new Error(
        `resolved_model.source must be ${RESOLVED_MODEL_SOURCES.join(" or ")}`,
      );
  }
}
