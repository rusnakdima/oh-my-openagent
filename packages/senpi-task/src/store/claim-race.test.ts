...
const claimRaceChildFixturePath = resolve(
  import.meta.dir,
  "__fixtures__",
  "claim-race-child.ts",
);
...
function parseChildResult(
  output: string,
): { readonly ids: readonly string[]; readonly retries: number } {
...
describe("claimTaskRecord cross-process race", () => {
  test("#given two processes in one frozen id bucket #when they claim task records concurrently #then every record is unique and collisions retry", async () => {
    ...
    expect(readdirSync(join(project, ".omo", "senpi-task", "tasks")))
      .toHaveLength(100);
    expect(results.reduce((total, result) => total + result.retries, 0))
      .toBeGreaterThanOrEqual(1);
    for (const output of stderrOutput) {
