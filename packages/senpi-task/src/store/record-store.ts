  ...
  return {
    ...
    transition(taskId, transition) {
      ...
      return withTaskRecordLock(path, () => {
        ...
        appendTaskEvent(stateDir, parsedTaskId, {
...
function removeRecord(
  ...
  rmSync(join(stateDir, "children", String(taskId)), {
    recursive: true,
    force: true,
  });
  // (2) completion spill file
  rmSync(join(stateDir, "completion-results", `${taskId}.txt`), {
    force: true,
  });
  // (3) task event log
...
function listRecords(
  ...
  for (
    const file of readdirSync(tasksDir).filter((entry) =>
      entry.endsWith(".json")
    ).toSorted()
  ) {
...
function readCached(
  ...
  if (
    hit !== undefined && hit.mtimeMs === stat.mtimeMs && hit.size === stat.size
  ) {
    return hit.record;
  }
