...
function fakeStore(seed: readonly TaskRecord[]) {
  ...
  const store = {
    ...
    appendEvent: (taskId: string, _event: PersistedTaskEvent): string =>
      `${taskId}.jsonl`,
  };
...
function fakeScheduler() {
  ...
  const run = (index: number): void => {
    ...
    if (call === undefined) {
      throw new RangeError(`missing scheduled call ${index}`);
    }
    call.run();
  };
...
describe("createCompletionNotifier scheduled retries", () => {
  test("#given two immediate failures w2notif #when the first timer fires #then delivery persists exactly once", () => {
    ...
    const completion = createCompletionNotifier(
      notifierDeps({ store, notifier: parent.notifier, scheduler }),
    );
    ...
    expect(mutated.filter((item) => item.notification.notified_epoch === 0))
      .toHaveLength(1);
  });
  ...
  test("#given a failed epoch w2notif #when revive advances the epoch before retry #then stale delivery drops", () => {
    ...
    // when
    scheduler.run(0);
    const nextEpoch = completion.notifyTerminal({
      record: revived,
      parentState: { kind: "idle" },
      runInBackground: true,
    });
  ...
  test("#given persistent enqueue failures w2notif #when eight timers exhaust #then the cycle caps and the count is cleaned so a later reconcile restarts fresh", () => {
    ...
    expect(records.get(record.task_id)?.notification.notification_failed_epoch)
      .toBe(0);

    // when a later reconcile retries the same failed epoch
    completion.reconcileFailedNotifications({
      sessionId: "session-a",
      parentState: { kind: "idle" },
    });
  ...
  test("#given failed records from mixed sessions w2notif #when reconciled twice #then only eligible work delivers once", () => {
    ...
    const alreadyNotified = failedRecord({
      task_id: "st_notified",
      notification: {
        run_epoch: 0,
        notified_epoch: 0,
    ...
    const otherSession = failedRecord({
      task_id: "st_other",
      parent_session_id: "session-b",
    });
    const nonNotifying = failedRecord({
      task_id: "st_cancelled",
      status: "cancelled",
    ...
    // then
    expect(parent.calls).toHaveLength(1);
    expect(parent.calls[0]?.details.map((detail) => detail.task_id)).toEqual([
      "st_eligible",
    ]);
  });
  ...
  test("#given retries keep failing w2notif #when timers reschedule #then delays follow the capped ladder", () => {
    ...
    expect(scheduler.calls.every((call, index) => {
      ...
      return base !== undefined && call.delayMs >= base &&
        call.delayMs <= Math.min(30_000, base + 199);
    })).toBe(true);
  });
