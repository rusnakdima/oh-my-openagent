...
export function createCompletionNotifier(
  ...
  function runScheduledRetry(entry: BufferedEntry): void {
    ...
    if (fresh.notification.notified_epoch >= entry.epoch) {
      return finishRetryChain(entry);
    }

    const decision = routeCompletion(getParentState());
    if (fresh.parent_session_id !== getCurrentSessionId()) {
      return finishRetryChain(entry);
    }
    ...
    const delivered = deliverWithRetry(
      deps.notifier,
      buildDeliveryMessage([entry.details], decision),
    );
    if (delivered.ok) {
      finishRetryChain(entry);
  ...
  function deliverRecord(
    ...
    const delivered = deliverWithRetry(
      deps.notifier,
      buildDeliveryMessage([details], decision),
    );
    if (delivered.ok) {
      finishRetryChain(entry);
  ...
  function notifyTerminal(request: CompletionRequest): NotifyResult {
    if (!request.runInBackground) {
      return { kind: "skipped", reason: "sync-task" };
    }
    const record = deps.store.load(request.record.task_id) ?? request.record;
    if (!TERMINAL_STATUSES.has(record.status)) {
      return { kind: "skipped", reason: "not-terminal" };
    }
    if (!shouldNotifyStatus(record.status)) {
      return { kind: "skipped", reason: "non-notifying-terminal" };
    }
  ...
  function flushBuffered(input: FlushInput): FlushResult {
    ...
    if (!delivered.ok) {
      for (const entry of entries) {
        recordEntryFailure(deps.store, entry, delivered.error);
      }
  ...
  function reconcileUnnotifiedNotifications(
    input: ReconcileUnnotifiedNotificationsInput,
  ): void {
    ...
    for (const record of listed.records) {
      ...
      if (
        hasBuffered(buffered, record.parent_session_id, record.task_id, epoch)
      ) continue;
  ...
  function owesNotification(record: TaskRecord): boolean {
    return record.notify_on_terminal ||
      record.notification.notification_failed_epoch !== undefined;
  }
...
function hasBuffered(
  buffered: Map<string, BufferedEntry[]>,
  sessionId: string,
  taskId: string,
  epoch: number,
): boolean {
  return (buffered.get(sessionId) ?? []).some((entry) =>
    entry.task_id === taskId && entry.epoch === epoch
  );
}
...
function pushBuffered(
  ...
  if (
    existing.some((buffered) =>
      buffered.task_id === entry.task_id && buffered.epoch === entry.epoch
    )
  ) return;
  existing.push(entry);
...
function persistNotified(
  ...
  store.mutate(
    taskId,
    (fresh) =>
      fresh.notification.notified_epoch >= epoch ? fresh : {
        ...fresh,
        notification: { ...fresh.notification, notified_epoch: epoch },
      },
  );
}
...
function recordFailure(
  ...
  store.appendEvent(taskId, {
    ...
    payload: { epoch, error: String(error) },
  });
  store.mutate(
    taskId,
    (fresh) =>
      fresh.notification.notified_epoch >= epoch ||
        fresh.notification.notification_failed_epoch === epoch
        ? fresh
        : {
          ...fresh,
          notification: {
            ...fresh.notification,
            notification_failed_epoch: epoch,
          },
        },
  );
...
function recordEntryFailure(
  ...
  if (store.load(entry.task_id) !== null) {
    recordFailure(store, entry.task_id, entry.epoch, error);
  }
}

function dropEntry(store: CompletionNotifierStore, entry: BufferedEntry): void {
  store.appendEvent(entry.task_id, {
    type: "notification_dropped",
    payload: { epoch: entry.epoch },
  });
  log("senpi-task completion dropped for replaced session", {
