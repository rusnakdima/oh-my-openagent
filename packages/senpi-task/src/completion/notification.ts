...
export function buildCompletionDetails(
  ...
  const base: CompletionDetails = {
    ...
    ...(finalResponse.file === undefined
      ? {}
      : { final_response_file: finalResponse.file }),
...
export function completionMessageLines(
  details: readonly CompletionDetails[],
  width?: number,
): readonly string[] {
...
function finalResponseForNotification(
  record: TaskRecord,
  stateDir: string | undefined,
): { readonly text: string; readonly file?: string } {
  ...
  if (stateDir === undefined) {
    return { text: source.slice(0, FINAL_RESPONSE_TRANSPORT_LIMIT) };
  }
  ...
  return {
    text: source.slice(0, FINAL_RESPONSE_TRANSPORT_LIMIT),
    file: `local://${path}`,
  };
}
...
function completionDetailLines(
  ...
  const summaryLines = width === undefined
    ...
    : [
      excerptRendererPromptText(identity, width),
      excerptRendererPromptText(stats, width),
    ];
  const response = width === undefined
    ? detail.final_response
    : normalizeRendererText(detail.final_response);
  const continuation = width === undefined
    ? detail.continuation_hint
    : normalizeRendererText(detail.continuation_hint);
...
function fallbackToken(detail: CompletionDetails): string | undefined {
  ...
  return `fallback:${normalizeRendererText(requested)}->${
    normalizeRendererText(resolved)
  }`;
}

function excerptForWidth(
  value: string,
  width: number | undefined,
  prefix: string,
  suffix: string,
  ...
  return excerptRendererPromptText(
    value,
    availableExcerptWidth(width, prefix, suffix),
  );
}

function availableExcerptWidth(
  width: number | undefined,
  prefix: string,
  suffix: string,
): number | undefined {
  ...
  return Math.max(
    0,
    width - rendererVisibleWidth(prefix) - rendererVisibleWidth(suffix),
  );
}
...
function formatDuration(durationMs: number): string {
  ...
  const seconds = (durationMs / 1_000).toFixed(2).replace(/\.00$/u, "").replace(
    /(\.\d)0$/u,
    "$1",
  );
  return `${seconds}s`;
