...
export function taskIdentityLabel(input: TaskIdentityInput): string {
  const label = optionalRendererText(input.taskSummary) ??
    optionalRendererText(input.description) ?? optionalRendererText(input.name);
  return label === undefined
    ? excerptRendererText(input.taskId, IDENTITY_MAX_WIDTH)
    : excerptRendererText(label, IDENTITY_MAX_WIDTH);
}
...
export function formatTargetIdentity(
  input: Pick<StatusTargetInput, "category" | "agentType">,
): string | undefined {
...
export function formatTargetWithModel(
  input: StatusTargetInput,
): string | undefined {
  const identity = formatTargetIdentity(input);
  const model = formatStatusModel(input.resolvedModel) ??
    optionalRendererText(input.model);
  if (identity !== undefined) {
    return model === undefined ? identity : `${identity}(${model})`;
  }
...
export function formatStatusTarget(
  input: StatusTargetInput,
): string | undefined {
  const target = formatTargetWithModel(input);
  if (target === undefined) return undefined;
  const fallback = input.fallbackCount === undefined || input.fallbackCount <= 0
    ? undefined
    : `fallback:${input.fallbackCount}`;
...
export function composeStatusLine(input: StatusLineInput): string {
  const tokens = [
    ...
    input.stats === undefined
      ? undefined
      : `turn ${input.stats.turns}${toolCountSuffix(input.stats.tool_calls)}`,
    input.verb,
    input.stats === undefined ? undefined : formatLiveSpend(input.stats),
    input.stats?.tokens_per_second === undefined
      ? undefined
      : `${input.stats.tokens_per_second} tok/s`,
  ];
...
export function formatLiveSpend(
  stats: Pick<TaskRunStats, "cost_usd">,
): string | undefined {
...
function formatStatusModel(
  resolved: ResolvedModelRecord | undefined,
): string | undefined {
  ...
  const effort = optionalRendererText(resolved.reasoning) ??
    ...
    optionalRendererText(resolved.variant);
