...
export const DEFAULT_CATEGORIES: Readonly<Record<string, OmoCategoryConfig>> =
  Object.fromEntries(
    BUILTIN_CATEGORY_DEFAULTS.map((
      definition,
    ) => [definition.name, definition.config]),
  );

export const CATEGORY_DESCRIPTIONS: Readonly<Record<string, string>> = Object
  .fromEntries(
    BUILTIN_CATEGORY_DEFAULTS.map((
      definition,
...
export const CATEGORY_CALLER_GUIDANCE: Readonly<
  Record<string, string | undefined>
> = Object.fromEntries(
  BUILTIN_CATEGORY_DEFAULTS.map((
    definition,
  ) => [definition.name, definition.callerGuidance]),
);

export const CATEGORY_PROMPT_APPENDS: Readonly<Record<string, string>> = Object
  .fromEntries(
    BUILTIN_CATEGORY_DEFAULTS.map((
      definition,
    ) => [definition.name, definition.promptAppend]),
  );

function hasRequiresModel(
  definition: BuiltinCategoryDefinition,
): definition is BuiltinCategoryDefinition & {
  readonly requiresModel: string;
} {
  return definition.requiresModel !== undefined;
}

export const BUILTIN_CATEGORY_REQUIRES_MODEL: Readonly<Record<string, string>> =
  Object.fromEntries(
    BUILTIN_CATEGORY_DEFAULTS.filter(hasRequiresModel).map((
      definition,
    ) => [definition.name, definition.requiresModel]),
  );
...
export const CATEGORY_PROMPT_APPEND_RESOLVERS: Readonly<
  Record<string, (model: string | undefined) => string>
> = Object.fromEntries(
