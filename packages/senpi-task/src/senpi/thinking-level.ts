...
export type SenpiThinkingLevel = NonNullable<
  CreateAgentSessionOptions["thinkingLevel"]
>;

const SENPI_THINKING_LEVEL_NAMES = [
  "off",
  "minimal",
  "low",
  "medium",
...
function isSenpiThinkingLevelName(
  value: string,
): value is SenpiThinkingLevelName {
...
export function asSenpiThinkingLevel(
  reasoning: string | undefined,
): SenpiThinkingLevel | undefined {
  if (reasoning === undefined) return undefined;
  const normalized = reasoning === "none" ? "off" : reasoning;
  return normalized === "auto"
    ? undefined
    : (isSenpiThinkingLevelName(normalized) ? normalized : undefined);
}
