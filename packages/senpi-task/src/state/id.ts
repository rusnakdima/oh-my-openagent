...
export function createTaskId(nowMs = Date.now()): TaskId {
  const nextValue = nextMonotonicValue(
    uuidV7TimestampHighBits(nowMs),
    lastTaskIdValue,
  );
...
export function parseTaskId(value: string): TaskId {
  if (!isTaskId(value)) {
    throw new Error("Invalid task id; expected st_[0-9a-f]{8}");
  }
  return value;
}
...
function uuidV7TimestampHighBits(nowMs: number): number {
  ...
  return Math.floor(integerMs / UUIDV7_TIMESTAMP_HIGH_BITS_DIVISOR) %
    TASK_ID_SPACE_SIZE;
}

function nextMonotonicValue(
  candidate: number,
...
export class TaskIdSpaceExhaustedError extends Error {
  constructor() {
    super(
      "Task id space exhausted for this process; restart before creating more task ids",
    );
