...
const NORMAL_TRANSITION_EVENTS = [
  "start",
  "complete",
  "fail",
  "cancel",
...
> = {
  pending: {
    start: {
      ...
      expectedAudit: "transition_applied",
    },
    complete: {
      expectedApplied: false,
      expectedStatus: "pending",
      expectedAudit: "invalid_transition_ignored",
    },
    fail: {
      expectedApplied: false,
      expectedStatus: "pending",
...
describe("transitionTaskRecord exhaustive lifecycle table", () => {
  test("#given every status and normal transition #when applied #then allowed and rejected outcomes match the lifecycle table", () => {
    ...
    expect(results).toHaveLength(
      TASK_STATUSES.length * NORMAL_TRANSITION_EVENTS.length,
    );
    for (const { key, expected, result } of results) {
      expect({ key, applied: result.applied }).toEqual({
        key,
        applied: expected.expectedApplied,
      });
      expect({ key, status: result.record.status }).toEqual({
        key,
        status: expected.expectedStatus,
      });
      expect({ key, audit: result.audit.type }).toEqual({
        key,
        audit: expected.expectedAudit,
      });
    }
  });
});
...
function terminalExpectations(
  ...
  return {
    start: {
      ...
      expectedAudit: "late_transition_ignored",
    },
    complete: {
      expectedApplied: false,
      expectedStatus: status,
...
function assertNeverNormalTransitionEvent(value: never): never {
  throw new Error(
    `Unexpected normal transition event: ${JSON.stringify(value)}`,
  );
}
