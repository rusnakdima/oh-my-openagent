...
export type ResolvedRuntimeModelIdentity<TModel extends SenpiModelPort> =
  & ParsedRuntimeModelIdentity<TModel>
  & {
...
const GATEWAY_UPSTREAM_VENDOR_PREFIXES: ReadonlySet<string> = new Set([
  "openai",
  "anthropic",
  "google",
]);
...
export function compileSenpiOpenAiOnlyModelRecommendations<
  TModel extends SenpiModelPort,
>(
  ...
  const inventory: readonly OpenAiRuntimeModelIdentity[] =
    resolveRuntimeModelIdentities(registry, models);
...
export function resolveRuntimeModelIdentities<TModel extends SenpiModelPort>(
  ...
  return models.map((model) => {
    ...
    const identity: OpenAiRuntimeModelIdentity = {
      ...
      ...(upstreamIdentity.kind === "valid"
        ? { upstreamModelId: upstreamIdentity.modelId }
        : {}),
      ...(upstreamIdentity.kind === "invalid"
        ? { upstreamIdentityInvalid: true }
        : {}),
    };
    ...
    return {
      ...
      ...(upstreamIdentity.kind === "invalid"
        ? { upstreamIdentityInvalid: true }
        : {}),
      ...(canonicalOpenAiModelId === undefined
        ? {}
...
export function filterAutomaticRuntimeModelIdentities<
  TModel extends SenpiModelPort,
>(
  ...
  return models.filter((model) =>
    !isUnprovenOpenAiRoute(model) && !isUnprovenNestedGatewayRoute(model)
  );
}
...
export function runtimeModelIds<TModel extends SenpiModelPort>(
  ...
  for (const model of models) {
    ...
    if (
      options.includeUpstreamModelIds !== false &&
  ...
  for (const entry of chain) {
    ...
    for (const model of models) {
      ...
      if (
        ...
        knownGatewayModelId(model) === entry.model
      ) continue;
      if (
        entry.providers.includes(model.provider) &&
      ...
      if (!projected.some((candidate) => sameFallbackEntry(candidate, alias))) {
...
export function recommendationToFallbackEntry(
  ...
  if (
    separatorIndex <= 0 || separatorIndex === recommendation.model.length - 1
  ) return undefined;
  return {
    providers: [recommendation.model.slice(0, separatorIndex)],
    model: recommendation.model.slice(separatorIndex + 1),
    ...(recommendation.variant === undefined
      ? {}
      : { variant: recommendation.variant }),
  };
}

function isUnprovenOpenAiRoute(
  model: ResolvedRuntimeModelIdentity<SenpiModelPort>,
): boolean {
  const localOpenAiModelId = localOpenAiModelIdCandidate(model.modelId);
  if (localOpenAiModelId === undefined) return false;
  if (model.upstreamIdentityInvalid === true) return true;
...
function localOpenAiModelIdCandidate(modelId: string): string | undefined {
  ...
  return nestedModelId.length > 0 && !nestedModelId.includes("/") &&
      isOpenAiUpstreamModelId(nestedModelId)
    ? nestedModelId
    : undefined;
}
...
function isUnprovenNestedGatewayRoute(
  model: ResolvedRuntimeModelIdentity<SenpiModelPort>,
): boolean {
...
function sameFallbackEntry(
  left: DelegateFallbackEntry,
  right: DelegateFallbackEntry,
): boolean {
  return left.model === right.model &&
    left.variant === right.variant &&
    left.providers.length === right.providers.length &&
    left.providers.every((provider, index) =>
      provider === right.providers[index]
    );
}
