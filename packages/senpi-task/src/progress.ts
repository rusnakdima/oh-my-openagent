  ...
  const activity = (stats: ReturnType<typeof tracker.snapshot>): string =>
    composeStatusLine({
      identity: taskIdentityLabel({
        ...
        taskSummary: target.taskSummary,
      }),
  ...
  return {
    accept(event): boolean {
      ...
      if (
        ...
        currentTool = formatToolActivity(
          event.toolName,
    ...
    contentText(): string {
      return lastAssistantLine === undefined
        ? ""
...
export function readToolProgressDetails(
  value: unknown,
): ToolProgressDetails | undefined {
  if (!isRecord(value) || !isRecord(value.progress)) return undefined;
  if (
    typeof value.progress.activity !== "string" ||
    typeof value.progress.startedAt !== "number"
  ) return undefined;
  if (typeof value.childId !== "string" || typeof value.turns !== "number") {
    return undefined;
  }
  if (
    value.currentTool !== undefined && typeof value.currentTool !== "string"
  ) return undefined;
  if (
    value.lastAssistantLine !== undefined &&
    typeof value.lastAssistantLine !== "string"
  ) return undefined;
  ...
  if (
    optionalNumbers.some((entry) =>
      entry !== undefined && typeof entry !== "number"
    )
  ) return undefined;
...
function oneLineArgument(value: unknown): string {
  ...
  return typeof first === "string"
    ? truncate(first.replace(/\s+/g, " ").trim(), 80)
    : "";
}
