...
describe("taskIdentityLabel", () => {
  ...
  test("#given a blank task summary #when labelled #then it falls back to the description", () => {
    ...
    expect(
      taskIdentityLabel({
        taskId: "st_00000001",
        description: "Audit renderers",
        taskSummary: "  ",
      }),
    ).toBe("Audit renderers");
  });
  ...
  test("#given no description #when labelled #then the stable name is used", () => {
    // given / when / then
    expect(taskIdentityLabel({ taskId: "st_00000001", name: "reviewer" })).toBe(
      "reviewer",
    );
  });
...
describe("formatStatusTarget", () => {
  ...
  test("#given a category but only a raw model #when formatted #then the sanitized raw model qualifies the target", () => {
    ...
    expect(formatStatusTarget({ model: "anthropic/claude-sonnet-4-5" })).toBe(
      "model:anthropic/claude-sonnet-4-5",
    );
    expect(
      formatStatusTarget({ category: "quick", model: "raw\u001b[31m-model" }),
    ).toBe("category:quick(raw-model)");
  });
...
describe("composeStatusLine", () => {
  ...
  test("#given a single tool call #when composed #then the tool noun is singular", () => {
    ...
    expect(
      composeStatusLine({
        identity: "t",
        stats: { runtime_ms: 0, turns: 1, tool_calls: 1 },
        verb: "running",
      }),
  ...
  test("#given no stats #when composed #then only known tokens are emitted", () => {
    // given / when / then
    expect(composeStatusLine({ identity: "t", verb: "waiting (running)" }))
      .toBe("t · waiting (running)");
  });
});
