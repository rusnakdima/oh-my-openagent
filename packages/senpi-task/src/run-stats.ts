  ...
  return {
    accept(event) {
      if (event.type === "tool_execution_start") {
        ...
        const isEvalRun = event.toolName === "eval" &&
          (!isRecord(evalInput) ||
            (evalInput.action !== "peek" && evalInput.action !== "stop"));
        if (isEvalRun) {
      ...
      if (event.type === "tool_execution_end") {
        ...
        if (event.toolName === "eval") {
          if (event.toolCallId !== undefined) {
            countNestedEvalTools = evalRunCallIds.delete(event.toolCallId);
          } else if (anonymousEvalRuns > 0) {
            anonymousEvalRuns -= 1;
            countNestedEvalTools = true;
          }
        }
        const details = countNestedEvalTools && isRecord(event.result) &&
      ...
      const requestCacheableTokens = (usage.input ?? 0) +
        requestCacheReadTokens + requestCacheWriteTokens;
      const requestCacheHitRate = boundedCacheHitRate(
        requestCacheReadTokens,
        requestCacheableTokens,
      );
      if (requestCacheHitRate !== undefined) {
        latestCacheHitRate = requestCacheHitRate;
      }
    ...
    snapshot(nowMs) {
      ...
      const throughputWindowMs = collapsedWindows > 0
        ? undefined
        : generationMs > 0
        ? generationMs
        : runtimeMs;
      const tps = throughputWindowMs === undefined
        ? undefined
        : tokensPerSecond(outputTokens, throughputWindowMs);
      const runCacheHitRate = boundedCacheHitRate(
        cacheReadTokens,
        cacheableTokens,
      );
      ...
      return {
        ...
        ...(latestCacheHitRate === undefined
          ? {}
          : { cache_hit_rate_last: latestCacheHitRate }),
        ...(runCacheHitRate === undefined
          ? {}
          : { cache_hit_rate_run: runCacheHitRate }),
      };
    },
  };
}
...
function positiveFiniteTotal<K extends string>(
  key: K,
  total: number,
): Partial<Record<K, number>> {
  return total > 0 && Number.isFinite(total)
    ? ({ [key]: total } as Record<K, number>)
    : {};
}
...
function boundedCacheHitRate(
  cacheReadTokens: number,
  cacheableTokens: number,
): number | undefined {
  if (
    !Number.isFinite(cacheReadTokens) || !Number.isFinite(cacheableTokens) ||
    cacheableTokens <= 0
  ) {
    return undefined;
  }
...
function readUsage(message: Record<string, unknown>): UsageFacts {
  ...
  return {
    ...optionalNumber(
      "output",
      firstFiniteNonNegativeNumber(usage.output, usage.output_tokens),
    ),
    ...optionalNumber(
      "total",
      firstFiniteNonNegativeNumber(usage.totalTokens, usage.total_tokens),
    ),
    ...optionalNumber("cost", readCost(usage.cost)),
    ...optionalNumber(
      "input",
      firstFiniteNonNegativeNumber(usage.input, usage.input_tokens),
    ),
    ...optionalNumber(
      "cacheRead",
