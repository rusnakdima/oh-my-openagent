...
export function createSteeringEngine(port: SteeringPort): SteeringEngine {
  ...
  async function sendToTask(input: SendInput): Promise<SendOutcome> {
    ...
    if (mode === "not-continuable") {
      return {
        ...
        reason: notContinuableReason(record),
    ...
    if (handle === undefined) {
      return {
        ...
        task_id: record.task_id,
        reason:
          `Task ${record.task_id} has no resident session in this process.`,
        suggestion: TASK_OUTPUT_SUGGESTION,
      };
    }

    if (mode === "steer") {
      return steerRunning(record, handle, input.message, deliverAs);
    }
  ...
  async function reviveTerminal(
    ...
    port.store.appendEvent(record.task_id, {
      type: "revived",
      payload: { run_epoch: revived.notification.run_epoch },
    });
    port.reacquireForRevive(record.task_id);
    return {
      kind: "revived",
      task_id: record.task_id,
      run_epoch: revived.notification.run_epoch,
    };
  }
  ...
  function enqueuePending(
    ...
    if (updated === null) {
      return {
        kind: "not_found",
        reason: `No task found for "${record.task_id}".`,
        suggestion: NOT_FOUND_SUGGESTION,
      };
    }
    port.store.appendEvent(record.task_id, {
      ...
      payload: {
        ...
        deliverAs,
        run_epoch: updated.notification.run_epoch,
      },
    });
    return {
      kind: "queued",
  ...
  function clearPersistedQueue(
    ...
    port.store.mutate(taskId, (fresh) => {
      ...
      const remaining = drainedIds === undefined
        ? []
        : queue.filter((entry) => !drainedIds.has(entry.id));
  ...
  async function notifyStarted(taskId: string): Promise<void> {
    ...
    const queue = fresh?.pending_steering;
    if (fresh === undefined || queue === undefined || queue.length === 0) {
      return;
    }
    ...
    for (const entry of queue) {
      try {
        ...
        port.store.appendEvent(taskId, {
          type: "steered",
          payload: {
            delivered: entry.deliver_as,
            queued: true,
            run_epoch: fresh.notification.run_epoch,
          },
        });
      } catch (error) {
  ...
  async function interruptTask(idOrName: string): Promise<InterruptOutcome> {
    ...
    if (record === undefined) {
      return { kind: "not_found", reason: `No task found for "${idOrName}".` };
    }
    if (record.status !== "running") {
      return {
        ...
        task_id: record.task_id,
        status: record.status,
        reason: `Task ${record.task_id} is ${record.status}, not running.`,
      };
    }
    ...
    if (!result.applied) {
      return {
        ...
        reason: `Task ${record.task_id} could not be interrupted from running.`,
      };
    }
    const handle = port.liveHandle(record.task_id);
  ...
  async function cancelTask(
    ...
    const record = resolve(idOrName);
    const destructionCause = options?.abort === "skip"
    ...
    if (record.status === "pending") {
      ...
      if (!result.applied) {
        return {
          ...
          task_id: record.task_id,
          status: result.record.status,
          reason: `Task ${record.task_id} could not be cancelled from pending.`,
        };
      }
      ...
      await port.destruction.destroyResidentTask(
        record.task_id,
        destructionCause,
      );
      return {
    ...
    if (record.status !== "running") {
      const reasonText = record.status === "cancelled"
        ? `Task ${record.task_id} is already cancelled.`
        : `Task ${record.task_id} is ${record.status}, not running.`;
      return {
        kind: "noop",
        task_id: record.task_id,
        status: record.status,
    ...
    if (!result.applied) {
      return {
        ...
        task_id: record.task_id,
        status: result.record.status,
        reason: `Task ${record.task_id} could not be cancelled from running.`,
      };
    }
    const handle = port.liveHandle(record.task_id);
    ...
    if (
      options?.abort === "skip" && handle !== undefined &&
  ...
  function destroyAfterSettlement(
    handle: ManagedChildHandle,
    taskId: string,
  ): void {
    const destroy = (): Promise<void> =>
      port.destruction.destroyResidentTask(taskId, "cancel_without_abort");
    void handle.waitForOutcome().then(destroy, destroy).catch(
      (error: unknown) => {
        log("senpi-task deferred cancel destruction rejected", {
          taskId,
          error: String(error),
        });
      },
...
function oneShotPolicyDenial(record: TaskRecord): SendOutcome | undefined {
  ...
  return {
    ...
    task_id: record.task_id,
    agent: agentType,
    message: policy.sendDenialReminder,
  };
}
...
function scopeDenied(
  ...
  if (input.callerSessionId === undefined || input.allScope === true) {
    return undefined;
  }
  const caller = input.callerSessionId;
  if (
    caller === record.parent_session_id || caller === record.root_session_id
  ) return undefined;
  return {
    kind: "scope_denied",
    task_id: record.task_id,
    ...
    reason:
      `Task ${record.task_id} belongs to session ${record.parent_session_id}; pass all_scope to send across sessions.`,
  };
}
...
function notContinuableReason(record: TaskRecord): string {
  ...
  if (
    record.residency_state === "persisted_only" ||
    record.residency_state === "rpc_detached"
  ) {
    return `Task ${record.task_id} is suspended - resumes when its session is resumed.`;
  }
  if (record.residency_state === "disposed") {
    return `Task ${record.task_id} was disposed and can no longer be continued.`;
  }
  if (record.residency_state === "evicted") {
    return `Task ${record.task_id} was evicted from residency and can no longer be continued.`;
  }
...
function buildRevived(record: TaskRecord, timestamp: string): TaskRecord {
  ...
  return {
    ...
    notification: {
      ...record.notification,
      run_epoch: record.notification.run_epoch + 1,
    },
  };
}
