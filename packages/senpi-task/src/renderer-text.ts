...
export function excerptRendererText(
  value: string,
  width = DEFAULT_EXCERPT_WIDTH,
): string {
...
export function excerptRendererPromptText(
  value: string,
  width = DEFAULT_EXCERPT_WIDTH,
): string {
  const normalized = normalizeRendererText(value);
...
export function joinRendererTokens(
  tokens: readonly (string | undefined | false)[],
): string {
  return tokens.filter((token) => typeof token === "string" && token.length > 0)
    .join(" ");
}

export function optionalRendererText(
...
function stripTerminalControls(value: string): string {
  ...
  while (index < value.length) {
    ...
    if (
      code === 0x90 || code === 0x98 || code === 0x9d || code === 0x9e ||
      code === 0x9f
    ) {
...
function skipEscapeSequence(value: string, escapeIndex: number): number {
  ...
  if (
    next === 0x50 || next === 0x58 || next === 0x5d || next === 0x5e ||
    next === 0x5f
  ) {
  ...
  let index = nextIndex;
  while (
    index < value.length && value.charCodeAt(index) >= 0x20 &&
    value.charCodeAt(index) <= 0x2f
  ) index++;
  if (
    index < value.length && value.charCodeAt(index) >= 0x30 &&
