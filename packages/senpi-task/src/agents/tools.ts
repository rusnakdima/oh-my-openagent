...
export function normalizeToolRules(
  input: ToolsInput | undefined,
): readonly AgentToolRule[] | undefined {
  if (input === undefined) return undefined;
  const rules = Array.isArray(input)
    ? input.flatMap((entry) => normalizeToolRuleEntry(entry))
    : Object.entries(input).flatMap(([tool, value]) =>
      normalizeToolPermission(tool, value)
    );
  return rules;
}
...
): readonly AgentToolRule[] {
  if (typeof value === "boolean") return [{ pattern: tool, allow: value }];
  if (typeof value === "string") {
    return [{ pattern: tool, allow: value === "allow" }];
  }
...
function normalizeToolRuleEntry(
  entry: ToolRuleEntry,
): readonly AgentToolRule[] {
  if (typeof entry === "string") {
    if (entry.startsWith("!") || entry.startsWith("-")) {
      return [{ pattern: entry.slice(1), allow: false }];
    }
  ...
  if (entry.action !== undefined) {
    return [{ pattern, allow: entry.action === "allow" }];
  }
