...
export type ParsedAgentRegistryModel<TModel extends SenpiModelPort> =
  & ParsedAgentModel
  & {
...
export function findExactAgentModel<TModel extends SenpiModelPort>(
  ...
  return expected === undefined ? undefined : parseRegistryModel(
    registry.find(expected.provider, expected.modelId),
    expected,
  );
}

export function parseAvailableAgentModels(
  models: unknown,
...
export function parseAvailableAgentRegistryModels<
...
function hasSecretLikeModelField(model: object): boolean {
  return Object.getOwnPropertyNames(model).some((key) =>
    SECRET_LIKE_MODEL_FIELD_NAMES.has(
