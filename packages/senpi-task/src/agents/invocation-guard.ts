...
const CONDITIONS: Readonly<Record<string, AgentInvocationCondition>> =
  AGENT_INVOCATION_CONDITIONS;

export const PLAN_GATED_AGENT_NAMES: ReadonlySet<string> = new Set(
  Object.keys(AGENT_INVOCATION_CONDITIONS),
);
...
export function invocationConditionForAgent(
  agentName: string,
): AgentInvocationCondition | undefined {
  return CONDITIONS[agentName];
}

export function evaluateInvocationGuard(
  agentName: string,
  ...
  const violated = condition.forbidsSkills.find((skill) =>
    state.hasInvoked(skill)
  );
  ...
  const missing = condition.requiresSkills.filter((skill) =>
    !state.hasUserRequested(skill)
  );
  if (missing.length > 0) {
  ...
  if (condition.requiresPlanArtifact && !state.hasPlanArtifact()) {
    return {
      ...
      message:
        ...
        `plan to review yet. Complete the ${
          condition.requiresSkills.join(", ")
        } workflow so it produces a plan file, then retry.`,
    };
  }
