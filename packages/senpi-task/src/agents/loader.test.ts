...
describe("loadAgents", () => {
  test("#given home and project files #when loading #then later configured locations override earlier names", () => {
    ...
    writeText(
      join(fixture.home, ".pi", "agent", "agent", "finder.md"),
      agentMarkdown("home-pi", "home pi"),
    );
    writeText(
      join(fixture.home, ".senpi", "agent", "agent", "finder.md"),
      agentMarkdown("home-senpi", "home senpi"),
    );
    writeText(
      join(fixture.project, ".pi", "agent", "finder.md"),
      agentMarkdown("project-pi", "project pi"),
    );
    writeText(
      join(fixture.project, ".senpi", "agents", "agents", "finder.md"),
      agentMarkdown("project-senpi", "project senpi"),
    );
  ...
  test("#given markdown at a configured root #when loading #then only agent subdirectories are scanned", () => {
    ...
    const fixture = makeFixture();
    writeText(
      join(fixture.project, ".senpi", "ignored.md"),
      agentMarkdown("ignored", "ignored"),
    );
    writeText(
      join(fixture.project, ".senpi", "agent", "kept.md"),
      agentMarkdown("kept", "kept"),
    );
  ...
  test("#given file and programmatic definitions #when loading #then registerAgent overrides file values", () => {
    ...
    const fixture = makeFixture();
    writeText(
      join(fixture.project, ".pi", "agents", "writer.md"),
      agentMarkdown("file-model", "file prompt"),
    );
    registerAgent({
      name: "writer",
  ...
  test("#given malformed and valid frontmatter #when loading #then diagnostics are per file and valid agents still load", () => {
    ...
    writeText(badPath, "---\nmodel: [unterminated\n---\nBad");
    writeText(
      join(fixture.project, ".senpi", "agent", "valid.md"),
      agentMarkdown("valid-model", "Valid"),
    );
  ...
  test("#given project scan root is a symlink #when loading #then external agents are blocked with a read diagnostic", () => {
    ...
    expect(
      result.diagnostics.find((diagnostic) => diagnostic.path === linkedRoot)
        ?.message,
    ).toContain("symlink");
  });
...
`,
    ...
    expect(
      result.diagnostics.filter((diagnostic) => diagnostic.path === agentPath),
    ).toEqual([]);
    expect(resolveToolRule(rules, "read")).toBe(true);
...
`,
    ...
    expect(
      result.diagnostics.some((diagnostic) => diagnostic.kind === "validation"),
    ).toBe(false);
  ...
  test("#given activated Senpi agent overlays and model catalog aliases #when loading #then the Senpi profile view and concrete model chain win", () => {
    ...
    writeText(
      ...
      JSON.stringify({
        ...
        profiles: {
          focused: {
            "[senpi]": { agents: { finder: { model: "provider/focused" } } },
          },
        },
      }),
    );
