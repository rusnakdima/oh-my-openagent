...
import { normalizeAgentDefinition, RawAgentDefinitionSchema } from "./schema";
...
export function loadMarkdownAgent(path: string): ParseMarkdownAgentResult {
  ...
  if (parsed.parseError) {
    return {
      ...
      diagnostic: {
        kind: "frontmatter",
        path,
        message: `Malformed YAML frontmatter in ${path}`,
      },
    };
  }
  ...
  if (!validation.success) {
    const issuePaths = validation.error.issues.map((issue) =>
    ...
    return {
      ...
      diagnostic: {
        kind: "validation",
        path,
        message: `Invalid agent frontmatter in ${path}: ${
          issuePaths.join(", ")
        }`,
        issuePaths,
      },
  ...
  return {
    ok: true,
    agent: normalizeAgentDefinition(
      agentNameFromPath(path),
      validation.data,
      parsed.body,
    ),
  };
