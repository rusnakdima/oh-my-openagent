...
export function resolveAgentDefinitionLocations(
...
export function listMarkdownAgentFiles(
  location: string,
): ListMarkdownAgentFilesResult {
...
function listMarkdownFiles(
  ...
  try {
    ...
    if (error instanceof Error) {
      return { files: [], diagnostics: [readDiagnostic(dir, "scan", error)] };
    }
    throw error;
  }
...
function inspectDirectory(
  ...
  try {
    ...
    if (stat.isSymbolicLink()) {
      return { ok: false, diagnostics: [symlinkDiagnostic(dir)] };
    }
    if (!stat.isDirectory()) {
      return { ok: false, diagnostics: [notDirectoryDiagnostic(dir)] };
    }
    return { ok: true };
  } catch (error) {
    if (error instanceof Error) {
      if (ignoreMissing && errorCode(error) === "ENOENT") {
        return { ok: false, diagnostics: [] };
      }
      return {
        ok: false,
        diagnostics: [readDiagnostic(dir, "inspect", error)],
      };
    }
    throw error;
  }
...
function readDiagnostic(
  ...
  return {
    ...
    path,
    message:
      `Failed to ${operation} agent directory ${path}${codeText}: ${error.message}`,
  };
}
