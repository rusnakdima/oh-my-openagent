...
export function loadOmoAgentOverlays(
  options: Required<LoadAgentsOptions>,
): OmoAgentOverlayResult {
  const env = {
    ...process.env,
    ...options.env,
  ...
  const diagnostics = [...loaded.diagnostics, ...resolvedModels.diagnostics]
    .map(toAgentLoaderDiagnostic);
  return {
    agents: Object.values(mapOmoConfigAgents(resolvedModels.view)),
    diagnostics,
  };
}

function toAgentLoaderDiagnostic(
...
function issuePaths(
  ...
  return diagnostic.kind === "model_catalog_cycle"
    ? undefined
    : diagnostic.issuePaths;
}
