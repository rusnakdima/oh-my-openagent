...
import { normalizeToolRules, resolveToolRule, ToolsInputSchema } from "./tools";
...
describe("tools pi-task contract shapes", () => {
  ...
  test("#given a nested command-pattern map #when resolving #then last matching pattern wins", () => {
    // given
    const parsed = ToolsInputSchema.parse({
      bash: { "*": "deny", "rg *": "allow" },
    });
  ...
  test("#given an ask action #when resolving #then it is a non-grant and overrides a broader allow", () => {
    // given
    const standalone =
      normalizeToolRules(ToolsInputSchema.parse({ danger: "ask" })) ?? [];
    const scoped = normalizeToolRules(
      ToolsInputSchema.parse({ bash: { "*": "allow", "danger*": "ask" } }),
    ) ?? [];
