...
export function agentModelCandidates(
  ...
  const primaryCandidate = primary === undefined
    ? []
    : [withDefaults({ model: primary }, defaults)];
  const chain = (entries ?? []).map((entry) =>
    withDefaults(normalizeEntry(entry), defaults)
  );
...
function withDefaults(
  ...
  const reasoningEffort = candidate.reasoning ?? candidate.reasoningEffort ??
