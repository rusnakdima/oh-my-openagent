...
export function buildRuntimeModelChain(
  options: BuildModelChainOptions,
): RuntimeModelChain {
  ...
  const selectedIndex = candidates.findIndex((candidate) =>
    candidate.model === options.selectedModel
  );
  const fallbackModels = selectedIndex === -1 ? [] : candidates
    .slice(selectedIndex + 1)
    .filter((candidate) =>
      options.availableModels === undefined ||
      options.availableModels.has(candidate.model)
    )
    .map((candidate) => toResolvedModelRecord(candidate, options.source))
    .filter((candidate): candidate is ResolvedModelRecord =>
      candidate !== undefined
    );
...
export function chainRungCandidates(
  options: ChainRungCandidateOptions,
  ...
  for (const entry of options.chain.slice(selectedIndex + 1)) {
    for (const provider of entry.providers) {
      const concrete = `${provider}/${
        transformModelForProvider(provider, entry.model)
      }`;
...
function locateSelectedRung(options: ChainRungCandidateOptions): number {
  ...
  if (selectedRungEntry !== undefined) {
    ...
    const byShape = chain.findIndex((entry) =>
      ...
      entry.providers.every((provider, index) =>
        provider === selectedRungEntry.providers[index]
      )
    );
  ...
  return chain.findIndex((entry) =>
    entry.providers.some(
      (provider) =>
        `${provider}/${transformModelForProvider(provider, entry.model)}` ===
          selectedModel,
    )
  );
}
...
function deduplicateCandidates(
  candidates: readonly ModelChainCandidate[],
): readonly ModelChainCandidate[] {
...
function toResolvedModelRecord(
  ...
  if (separatorIndex <= 0 || separatorIndex === candidate.model.length - 1) {
    return undefined;
  }
