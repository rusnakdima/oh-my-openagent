...
const SUSPENDED_RESIDENCIES: ReadonlySet<TaskRecord["residency_state"]> =
  new Set(["persisted_only", "rpc_detached"]);
...
export function buildTaskSnapshot(
  ...
  return {
    ...
    ...(isSuspended(record)
      ? { suspended: { explanation: SUSPENDED_EXPLANATION } }
      : {}),
    ...
    ...(record.status === "lost"
      ? { lost: lostBreadcrumbs(record, stateDir) }
      : {}),
  };
}
