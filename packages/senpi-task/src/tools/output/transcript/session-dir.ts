...
export function readSessionDirTranscript(
  stateDir: string,
  taskId: string,
...
export function readSessionDirTranscriptResult(
  stateDir: string,
  taskId: string,
  ...
  if (files.length === 0) {
    return { entries: [], source: "session-jsonl", truncated: false };
  }
  const selected = files.length <= 2
    ? files
    : [files[0], files.at(-1)].filter((file) => file !== undefined);
