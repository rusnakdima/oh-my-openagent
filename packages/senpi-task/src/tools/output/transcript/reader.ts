import type { TranscriptReader, TranscriptReadResult } from "../types";
...
export const defaultTranscriptReader: TranscriptReader = (
  { taskId, stateDir },
): TranscriptReadResult => {
  const eventLog = readEventLogTranscriptResult(stateDir, taskId);
  if (eventLog.entries.length > 0 || eventLog.truncated === true) {
    return eventLog;
  }
