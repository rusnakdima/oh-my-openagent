...
export function readEventLogTranscript(
  stateDir: string,
  taskId: string,
...
export function readEventLogTranscriptResult(
  stateDir: string,
  taskId: string,
): TranscriptReadResult {
  const raw = readBoundedFileText(join(stateDir, "logs", `${taskId}.jsonl`));
  if (raw === undefined) {
    return { entries: [], source: "event-log", truncated: false };
  }
...
function transcriptEntryOf(entry: unknown): TranscriptEntry | undefined {
  ...
  if (
    entry.type === TRANSCRIPT_ASSISTANT_EVENT &&
    typeof payload.text === "string"
  ) {
    return { kind: "assistant", text: payload.text };
  }
  if (
    entry.type === TRANSCRIPT_TOOL_EVENT && typeof payload.tool === "string"
  ) {
    return {
      kind: "tool",
      tool: payload.tool,
  ...
  if (
    entry.type === TRANSCRIPT_ERROR_EVENT && typeof payload.message === "string"
  ) {
    return { kind: "error", message: payload.message };
  }
  if (
    entry.type === "team_message_waited" && typeof payload.from === "string" &&
    typeof payload.body === "string"
  ) {
    return {
      kind: "assistant",
      text: `[team message from ${payload.from}] ${payload.body}`,
    };
  }
  return undefined;
}
