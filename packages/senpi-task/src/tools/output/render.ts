...
export function renderTranscript(
  ...
  const tail = options.mode === "tail"
    ? takeTail(lines, options.tailLines)
    : { lines, cut: false };
...
function takeTail(
  ...
  tailLines: number,
): { readonly lines: readonly string[]; readonly cut: boolean } {
  if (tailLines <= 0 || lines.length <= tailLines) {
    return { lines, cut: lines.length > tailLines };
  }
...
function capText(
  ...
  return {
    text: `${text.slice(0, headLen)}${marker(elidedCount)}${
