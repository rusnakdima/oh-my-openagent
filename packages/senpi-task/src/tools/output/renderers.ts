...
export function renderTaskOutputCall(
  ...
  return {
    render: (width: number): string[] =>
      linesComponent([theme.fg("toolTitle", taskOutputCallLine(args, width))])
        .render(width),
    invalidate: (): void => {},
  };
}
...
function taskOutputCallLine(args: TaskOutputInput, width: number): string {
  ...
  const tail = mode === "tail"
    ? `tail_lines:${args.tail_lines ?? DEFAULT_TAIL_LINES}`
    : undefined;
  ...
  const available = Math.min(
    ...
    Math.max(
      0,
      width - rendererVisibleWidth(beforeTarget) -
        rendererVisibleWidth(afterTarget) - 1,
    ),
  );
  const target = excerptRendererText(
...
function taskOutputResultRow(details: TaskOutputDetails): ResultRow {
  switch (details.kind) {
    case "status": {
      ...
      const idSuffix = identity === details.snapshot.task_id
        ? undefined
        : `(${details.snapshot.task_id})`;
      const target = formatStatusTarget({
      ...
      const statusLabel = details.snapshot.suspended !== undefined
        ? "suspended"
        : normalizeRendererText(details.snapshot.status);
      return {
        color: statusThemeColor(details.snapshot.status),
        text: `${
          joinRendererTokens([`task_output ${identity}`, idSuffix, statusLabel])
        } · ${target}${runStatsSuffix(details.snapshot.run_stats)}`,
      };
    }
    case "transcript":
      return {
...
function notFoundRow(
  details: Extract<TaskOutputDetails, { readonly kind: "not_found" }>,
): string {
  const known = details.known_tasks.length > 0
    ? `known:${excerptRendererText(details.known_tasks.join(","))}`
    : undefined;
  return joinRendererTokens([
    `task_output not found: ${details.reason}`,
    known,
  ]);
}
...
export function taskOutputModelText(snapshot: TaskSnapshot): string {
  ...
  const reasoning = nonEmpty(snapshot.resolved_model?.reasoning) ??
    nonEmpty(snapshot.resolved_model?.reasoning_effort);
