...
import type { ListedTask, ListScope } from "../../manager";
...
export const TaskOutputParams = Type.Object({
  task_id: Type.Optional(
    Type.String({
      description:
        "Task id (st_...) of the child to read. Provide exactly one of task_id or name.",
    }),
  ),
  name: Type.Optional(
    Type.String({
  ...
  mode: Type.Optional(
    Type.Union([
      ...
      description:
        "status (default) = record snapshot + final result; tail = last lines of the transcript; full = whole transcript.",
    }),
  ),
...
const BLOCKING_REMOVED_GUIDANCE =
...
export function runTaskOutput(
  ...
  if (hasLegacyBlockingParam(params)) {
    return Promise.resolve(invalidArguments(BLOCKING_REMOVED_GUIDANCE));
  }

  const idOrName = params.task_id ?? params.name;
  if (idOrName === undefined) {
    return Promise.resolve(
      invalidArguments("Provide task_id or name to identify the child task."),
    );
  }

  const candidates = scopedCandidates(
    deps.manager.list.bind(deps.manager),
    callerSessionId,
  );
  const record = resolveTarget(candidates, idOrName);
  if (record === undefined) {
    return Promise.resolve(notFound(candidates, idOrName));
  }
...
function hasLegacyBlockingParam(params: object): boolean {
  return Reflect.get(params, "block") !== undefined ||
    Reflect.get(params, "timeout_ms") !== undefined;
}
...
function transcriptResult(
  ...
  const reader: TranscriptReader = deps.transcriptReader ??
  ...
  return toolResult(
    `${record.task_id} [${record.status}] transcript via ${source}:\n${rendered.text}`,
    details,
  );
}
...
function scopedCandidates(
  ...
  return list({ scope: "parent-session", session_id: callerSessionId }).map((
    entry,
  ) => entry.record);
}
...
function resolveTarget(
  ...
  return candidates.find((record) => record.task_id === idOrName) ??
    candidates.find((record) => record.name === idOrName);
}

function statusText(snapshot: TaskSnapshot): string {
  const parts = [
    `${snapshot.task_id} [${snapshot.status}] ${taskOutputModelText(snapshot)}`,
  ];
  if (snapshot.suspended !== undefined) {
    parts.push(snapshot.suspended.explanation);
  }
  if (snapshot.pid !== undefined) parts.push(`pid ${snapshot.pid}`);
  if (snapshot.lost !== undefined) parts.push(snapshot.lost.explanation);
  if (snapshot.error_message !== undefined) {
    parts.push(`error: ${snapshot.error_message}`);
  }
  if (snapshot.final_response !== undefined) {
    parts.push(snapshot.final_response);
  }
  return parts.join("\n");
}
...
function notFound(
  ...
  const listText = known.length > 0
    ? ` Known tasks in this session: ${known.join(", ")}.`
    : "";
  return toolResult(`No task '${idOrName}' in this session.${listText}`, {
    kind: "not_found",
    reason: `No task '${idOrName}' in this session.`,
    known_tasks: known,
  });
}
...
export function createTaskOutputTool(
  deps: TaskOutputDeps,
): ToolDefinition<typeof TaskOutputParams, TaskOutputDetails> {
  const resolveCaller = deps.resolveCallerSessionId ??
  ...
  return {
    ...
    execute: (_toolCallId, params, _signal, _onUpdate, ctx) =>
      runTaskOutput(deps, params, resolveCaller(ctx)),
    renderCall: (args, theme) => renderTaskOutputCall(args, theme),
    renderResult: (result, options, theme) =>
      renderTaskOutputResult(result, options, theme),
  };
}
