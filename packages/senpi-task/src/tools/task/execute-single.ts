...
function syncResult(
  ...
  const body = record.final_response ?? record.error_message ??
    `Task ${record.status}`;
  return result(
    appendMissingSkills(body + continuationFooter(record.task_id), skills),
    {
      ...recordDetails(record, mode),
...
export async function runSpawn(
  ...
  const effectiveParams = policy?.kind === "force"
    ? { ...params, prompt: policy.prompt, load_skills: [] }
    : params;
  ...
  if (started.kind === "plan_unresolved") {
    ...
    const agentSuffix = agents && agents.length > 0
  ...
  const progress = createChildProgress(
    started.task_id,
    {
      ...
      ...(started.resolved_model !== undefined &&
        { resolvedModel: started.resolved_model }),
  ...
  try {
    ...
    return result(
      `Task ${started.task_id} cancelled: ${reason}.${
        continuationFooter(started.task_id)
      }`,
      {
        ...startedDetails(started, params, spec.execution_mode, spec.skills),
        status: "cancelled",
