...
describe("momus one-shot policy over the real TaskManager", () => {
  test("#given an open plan session #when the full momus lifecycle is driven #then forcing, refusal, cancel, and re-spawn all hold", async () => {
    ...
    const runningSend = await manager.sendToTask({
      idOrName: firstId,
      message: "please reconsider your verdict",
      deliverAs: "steer",
    });
    ...
    if (runningSend.kind === "one_shot_agent") {
      expect(runningSend.message).toBe(
    ...
    store.transition(firstId, {
      type: "complete",
      timestamp: new Date().toISOString(),
      final_response: "[OKAY]",
    });
    const completedSend = await manager.sendToTask({
      idOrName: firstId,
