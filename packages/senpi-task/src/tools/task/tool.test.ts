...
import { createTaskTool, TASK_TOOL_NAME } from "./tool";
...
function deps(manager: TaskManager): TaskToolDeps {
  return {
    ...
    agents: { momus: { name: "momus", description: "Deep reasoning" } },
  };
}
...
function renderedLines(component: unknown, width: number): string[] {
  ...
  if (typeof render !== "function") {
    throw new Error("renderer component is missing render()");
  }
  const rendered: unknown = Reflect.apply(render, component, [width]);
  if (!Array.isArray(rendered)) {
    throw new Error("renderer component did not return lines");
  }
  const lines: string[] = [];
  for (const line of rendered) {
    if (typeof line !== "string") {
      throw new Error("renderer component returned a non-string line");
    }
    lines.push(line);
  }
  return lines;
...
describe("createTaskTool", () => {
  ...
  test("#given a partial child progress result #when rendered #then only the last-line row renders (status lives in senpi's progress line)", () => {
    ...
    if (renderResult === undefined) {
      throw new Error("task renderResult is missing");
    }
