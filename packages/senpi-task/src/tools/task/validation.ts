...
export type TaskTargetErrorCode =
  | "both_targets"
  | "no_target"
  | "category_with_model";
...
export type BatchShapeErrorCode =
  | "prompt_and_tasks"
  | "no_prompt_or_tasks"
  | "empty_tasks";
...
export type ResolveSpawnItemsResult =
  ...
  | {
    readonly kind: "error";
    readonly error: BatchShapeError | SpawnItemTargetError;
  };
...
const PROMPT_AND_TASKS_MESSAGE =
  "Provide EITHER prompt OR tasks, not both. Remove one and retry.";

const NO_PROMPT_OR_TASKS_MESSAGE =
  "Provide EITHER prompt OR tasks. One field is required.";
...
export function validateTaskTarget(params: TargetInput): TaskTargetSelection {
  ...
  if (hasCategory && hasSubagent) {
    return {
      kind: "error",
      error: { code: "both_targets", message: BOTH_TARGETS_MESSAGE },
    };
  }
  if (hasCategory && present(params.model)) {
    return {
      kind: "error",
      error: {
        code: "category_with_model",
        message: CATEGORY_WITH_MODEL_MESSAGE,
      },
    };
  }
  ...
  return {
    kind: "error",
    error: { code: "no_target", message: NO_TARGET_MESSAGE },
  };
}
...
export function validateBatchShape(params: SpawnParamsInput): BatchShapeResult {
  ...
  if (hasPrompt && hasTasks) {
    return {
      kind: "error",
      error: { code: "prompt_and_tasks", message: PROMPT_AND_TASKS_MESSAGE },
    };
  }
  if (!hasPrompt && !hasTasks) {
    return {
      kind: "error",
      error: {
        code: "no_prompt_or_tasks",
        message: NO_PROMPT_OR_TASKS_MESSAGE,
      },
    };
  }
  if (params.tasks !== undefined && params.tasks.length === 0) {
    return {
      kind: "error",
      error: { code: "empty_tasks", message: EMPTY_TASKS_MESSAGE },
    };
  }
  return hasTasks ? { kind: "batch" } : { kind: "single" };
}

export function resolveSpawnItems(
  params: SpawnParamsInput,
): ResolveSpawnItemsResult {
  const shape = validateBatchShape(params);
  if (shape.kind === "error") return shape;

  const inputs: readonly SpawnItemInput[] = params.tasks ??
    (params.prompt === undefined ? [] : [
      {
        prompt: params.prompt,
        ...(params.task_summary === undefined
          ? {}
  ...
  for (const [index, input] of inputs.entries()) {
    ...
    const category = itemDefinesCategory
      ? input.category
      : itemDefinesSubagent
      ? undefined
      : params.category;
    ...
    const target = validateTaskTarget({
      category,
