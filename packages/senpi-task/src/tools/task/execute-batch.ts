...
function failedStartDetail(
  item: ResolvedSpawnItem,
  start: FailedStartResult,
  ...
  switch (start.kind) {
    case "plan_unresolved": {
      return itemError(
        item,
        "",
        start.error.message + categoryListSuffix(start.error),
        skills,
      );
    }
    case "depth_denied":
...
function startedDetail(
  ...
  return {
    ...
    ...(start.resolved_model === undefined
      ? {}
      : { resolved_model: start.resolved_model }),
    status: start.status,
    ...(start.queue_position !== undefined &&
      ...
      starts.push(
        start.kind === "started"
          ...
          : {
            kind: "failed",
            item,
            detail: failedStartDetail(item, start, skills),
            ...(skills === undefined ? {} : { skills }),
          },
      );
    } catch (error) {
...
function categoryListSuffix(error: PlanResolutionError): string {
  ...
  if (error.code === "model_unavailable") {
    return ` Valid category names: ${
      available.join(", ")
    }. Retry one of these, or configure categories.<name>.models in omo.json — model overrides cannot be combined with category.`;
  }
...
function backgroundText(
  ...
  const lines = starts.map((start, index) => {
    if (start.kind === "failed") {
      return `${index + 1}. ${start.detail.name ?? "task"} (error): ${
        start.detail.error_message ?? "start failed"
      }`;
    }
    const queue = start.result.queue_position === undefined
      ? ""
      : ` queue:${start.result.queue_position}`;
    return `${
      index + 1
    }. ${start.result.name} ${start.result.task_id} (${start.result.status})${queue}${
      continuationFooter(start.result.task_id)
    }`;
  });
...
function backgroundResult(
  ...
  const live = starts.filter((
    start,
  ): start is Extract<BatchStart, { kind: "started" }> =>
    start.kind === "started"
  );
  ...
  return result(
    appendMissingSkills(
      backgroundText(starts, status),
      starts.map((start) => start.skills),
    ),
    {
      task_id: taskId,
      status,
...
function recordOutput(
  ...
  return {
    ...
    body: record.final_response ?? record.error_message ??
      `Task ${record.status}`,
    continuation: true,
  };
}

function promotedOutput(
  start: Extract<BatchStart, { kind: "started" }>,
  ...
  return {
    ...
    body: backgroundConversionText(start.result, {
      taskSummary: start.item.task_summary,
...
function aggregateStatus(
  items: readonly TaskToolItemDetail[],
  aborted: boolean,
): "running" | "error" | "cancelled" | "completed" {
  if (
    items.some((item) => item.status === "running" || item.status === "pending")
  ) return "running";
  if (items.some((item) => item.status === "error" || item.status === "lost")) {
    return "error";
  }
  if (
    aborted ||
...
function syncText(
  status: "running" | "error" | "cancelled" | "completed",
  outputs: readonly BatchItemOutput[],
): string {
  const lines = outputs.map((output, index) => {
    const label = output.detail.name ?? (output.detail.task_id || "task");
    const footer = output.continuation
      ? continuationFooter(output.detail.task_id)
      : "";
    return `${
      index + 1
    }. ${label} (${output.detail.status}): ${output.body}${footer}`;
  });
  return [`Batch ${status}.`, ...lines].join("\n");
}
...
async function syncResult(
  ...
  const settled = await Promise.allSettled(
    live.map((start) =>
      waitForForegroundTask({
        ...
        ...(input.scheduleDeadline !== undefined &&
          { scheduleDeadline: input.scheduleDeadline }),
      })
    ),
  );
  const batchAborted = settled.some(
    (entry) =>
      entry.status === "rejected" && input.signal?.aborted === true &&
      entry.reason === input.signal.reason,
  );
  const unsettledIndexes = batchAborted
    ? settled.flatMap((entry, index) =>
      entry.status === "rejected" ? [index] : []
    )
    : [];
  await Promise.allSettled(unsettledIndexes.map((index) => {
    ...
    return start === undefined
      ? Promise.resolve()
      : input.manager.cancelTask(start.result.task_id, "parent turn aborted");
  }));
  ...
  const outputs = starts.map((start): BatchItemOutput => {
    if (start.kind === "failed") {
      return {
        detail: start.detail,
        body: start.detail.error_message ?? "start failed",
        continuation: false,
      };
    }
    ...
    liveIndex += 1;
    if (entry === undefined) {
      return rejectedOutput(start, false, "missing wait result");
    }
...
export async function executeBatch(
  ...
  return input.runInBackground
    ? backgroundResult(starts)
    : syncResult(input, starts);
}
