...
const PROVIDER_PADDING_PROMPTS = new Set([
  "unused",
  "placeholder",
  "not used",
  "n/a",
...
export function normalizeTaskToolArguments(raw: unknown): TaskToolParamsStatic {
  ...
  const tasksAreSinglePadding = prompt !== undefined &&
    normalizedTasks !== undefined &&
    (normalizedTasks.length === 0 ||
      normalizedTasks.every(isProviderPaddingTask));
  ...
  const runInBackground = typeof raw.run_in_background === "boolean"
