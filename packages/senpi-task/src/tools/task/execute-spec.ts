...
type SingleSpawnParams = Omit<TaskToolParamsStatic, "prompt" | "tasks"> & {
...
function resolvedAgentMode(
  ...
  return toExecutionMode(deps.agents[target.subagentType]?.executionMode) ??
    deps.omoConfig.agents?.[target.subagentType]?.execution_mode;
}
...
export function singleSpawnParams(
