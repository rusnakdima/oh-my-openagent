...
export function listTaskCategories(
  config: OmoConfig,
): readonly TaskCategoryInfo[] {
  const userCategories: Readonly<Record<string, OmoCategoryConfig>> =
    config.categories ?? {};
  ...
  for (const name of names) {
    ...
    const description = userConfig?.description ??
      ownValue(CATEGORY_DESCRIPTIONS, name);
    const annotated = userConfig === undefined
      ? withGateAnnotation(name, description)
      : description;
    entries.push(
      annotated !== undefined ? { name, description: annotated } : { name },
    );
  }
  return entries;
}
