...
describe("backgroundStartText", () => {
  test("#given a task_summary #when the start text is built #then the summary labels the task over description and name", () => {
    ...
    expect(
      backgroundStartText(STARTED, {
        taskSummary: "Audit the boundary",
  ...
  test("#given only a description #when the start text is built #then the description labels the task", () => {
    // given / when / then
    expect(backgroundStartText(STARTED, { description: "auditing" })).toContain(
      "Started task auditing (st_00000001, running)",
    );
  });

  test("#given no labels #when the start text is built #then the name is used and the id form stays stable", () => {
    // given / when / then
    expect(backgroundStartText(STARTED, {})).toContain(
      "Started task auditor (st_00000001, running)",
    );
    expect(backgroundStartText({ ...STARTED, name: STARTED.task_id }, {}))
      .toContain("Started task st_00000001 (running)");
  });
});
