...
export function buildSkillPrepend(
...
function readSkill(
  ...
  for (const dir of dirs) {
    const skill = directSkill(name, dir) ??
      discoveredSkill(name, dir, discover, cache);
...
export function createFsSkillLoader(
  options: FsSkillLoaderOptions = {},
): SkillLoader {
  ...
  return (names, cwd): SkillResolution => {
    ...
    const discoveryCache = new Map<
      string,
      ReturnType<typeof discoverSkillsFromDir>
    >();
