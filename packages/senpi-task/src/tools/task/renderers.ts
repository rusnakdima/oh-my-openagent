...
export function taskResultLines(details: TaskToolDetails): readonly string[] {
  const mode = details.run_in_background === undefined
    ? undefined
    : formatTaskMode(details.run_in_background);
  return [
    taskResultLine(details, mode),
    ...(details.items ?? []).map(taskItemResultLine),
  ];
}

export function renderTaskResultLines(
  details: TaskToolDetails,
  theme: RendererTheme,
): readonly string[] {
  const mode = details.run_in_background === undefined
    ? undefined
    : theme.italic(formatTaskMode(details.run_in_background));
  return [
    taskResultLine(details, mode),
    ...(details.items ?? []).map(taskItemResultLine),
  ];
...
export function renderTaskResultComponent(
  details: TaskToolDetails,
  theme: RendererTheme,
): LinesComponent {
  return {
    render: (width: number): string[] => {
      ...
      const aggregate = truncateToWidth(
        theme.fg(statusThemeColor(details.status), line),
        width,
        ELLIPSIS,
      );
      const items = (details.items ?? []).map((item) =>
        truncateToWidth(
          theme.fg(statusThemeColor(item.status), taskItemResultLine(item)),
          width,
          ELLIPSIS,
        )
      );
...
export function linesComponent(
  lines: readonly string[] | WidthAwareLines,
): LinesComponent {
  return {
    render: (width: number): string[] => {
...
type TargetIdentity = Pick<
  TaskToolDetails,
  "category" | "subagent_type" | "model" | "resolved_model"
>;
...
function fallbackCountToken(
  details: Pick<TaskToolDetails, "fallback_attempts">,
): string | undefined {
  const count = details.fallback_attempts?.length ?? 0;
  return count > 0 ? `fallback:${count}` : undefined;
}

function taskResultLine(
  details: TaskToolDetails,
  ...
  return joinRendererTokens([
    ...
    details.queue_position === undefined
      ? undefined
      : `queue:${details.queue_position}`,
    reason === undefined
      ? undefined
      : `reason:${excerptRendererText(reason, TASK_REASON_EXCERPT_WIDTH)}`,
  ]);
}
...
function taskItemResultLine(item: TaskToolItemDetail): string {
  ...
  return joinRendererTokens([
    ...
    error === undefined
      ? undefined
      : `error:${excerptRendererText(error, TASK_REASON_EXCERPT_WIDTH)}`,
  ]);
}

function taskResultLineForWidth(
  details: TaskToolDetails,
  mode: string | undefined,
  width: number,
): string {
  ...
  const targetWidth = Math.max(
    0,
    width -
      requiredWithoutTarget.reduce(
        (total, token) => total + rendererVisibleWidth(token),
        0,
      ) -
      requiredSpaces,
  );
...
function compactTargetToken(
  details: TargetIdentity,
  maxWidth: number,
): string | undefined {
  const token = taskTargetToken(details);
  if (token === undefined || rendererVisibleWidth(token) <= maxWidth) {
    return token;
  }
