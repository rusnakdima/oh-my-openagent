...
function textOf(
  result: Awaited<ReturnType<ReturnType<typeof buildTaskExecute>>>,
): string {
...
describe("prompt-cache-safe foreground wait budget", () => {
  test("#given a context getter and env bridge #when resolving the budget #then the feature-detected getter wins", () => {
    ...
    // when
    const fromGetter = resolvePromptCacheSafeWaitSeconds(ctx, {
      PI_PROMPT_CACHE_SAFE_WAIT_SECONDS: "12",
    });
    const fromEnv = resolvePromptCacheSafeWaitSeconds(CTX, {
      PI_PROMPT_CACHE_SAFE_WAIT_SECONDS: "45",
    });
  ...
  test("#given a running foreground child #when its prompt-cache-safe budget expires #then it is promoted without cancellation and returns background guidance", async () => {
    ...
    // when
    expect(timer.deadlines.map((deadline) => deadline.delayMs)).toEqual([
      270_000,
    ]);
    ...
    expect(textOf(output)).toContain(
      "Completion will arrive as a notification",
    );
  ...
  test("#given a foreground batch with changing budgets #when waits start per item #then settled items stay inline and each remaining item converts on its own deadline", async () => {
    ...
    waits[1]?.resolve(
      makeRecord({ task_id: BATCH_IDS[1], status: "completed" }),
    );
    waits[2]?.resolve(
      makeRecord({ task_id: BATCH_IDS[2], status: "completed" }),
    );
    await Promise.resolve();
  });
});
