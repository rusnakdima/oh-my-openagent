...
export function formatTaskTarget(
  args: Pick<TaskCallArgs, "category" | "subagent_type">,
): string {
...
function callRowText(args: TaskCallArgs): string | undefined {
  return optionalRendererText(args.task_summary) ??
    optionalRendererText(args.prompt);
}
...
function taskCallLineForWidth(
  ...
  if (width <= 0) return "";
  const fixedWidth = rendererVisibleWidth("task") +
    rendererVisibleWidth(plainMode) + 4;
  const available = Math.min(
    TASK_PROMPT_EXCERPT_WIDTH,
  ...
  return truncateToWidth(
    joinRendererTokens(["task", prompt, mode]),
    width,
    ELLIPSIS,
  );
}
...
function promptToken(text: string | undefined): string | undefined {
  ...
  return normalized === undefined
    ? undefined
    : `"${excerptRendererPromptText(normalized, TASK_PROMPT_EXCERPT_WIDTH)}"`;
}
