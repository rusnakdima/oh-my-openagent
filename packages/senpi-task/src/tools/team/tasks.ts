...
export const TeamTaskListParams = Type.Object({
  ...
  owner: Type.Optional(
    Type.String({ description: "Filter by owning member." }),
  ),
});

export const TeamTaskGetParams = Type.Object({
  team_run_id: Type.String({
    description: "Team run id (returned by team_create).",
  }),
  task_id: Type.String({
    description: "Team tasklist task id (not a child st_... id).",
  }),
});
...
export const TeamTaskUpdateParams = Type.Object({
  ...
  owner: Type.Optional(
    Type.String({ description: "Owning member (defaults to the lead)." }),
  ),
});
...
export type TeamTaskCreateDetails = {
  readonly kind: "created";
  readonly task: Task;
};
export type TeamTaskListDetails = {
  readonly kind: "list";
  readonly tasks: readonly Task[];
};
export type TeamTaskGetDetails =
  | { readonly kind: "task"; readonly task: Task }
  | { readonly kind: "not_found"; readonly task_id: string };
...
export async function runTeamTaskCreate(
  service: TeamToolsService,
  params: TeamTaskCreateInput,
): Promise<AgentToolResult<TeamTaskCreateDetails>> {
  const task = await service.createTask(params.team_run_id, {
    subject: params.subject,
    description: params.description,
    status: "pending",
    ...(params.blocked_by !== undefined
      ? { blockedBy: params.blocked_by }
      : {}),
  });
  const blockers = task.blockedBy.length > 0
    ? `, blocked by: ${collapseEcho(task.blockedBy.join(", "))}`
    : "";
  return toolResult(
    `Created task ${task.id}: '${
      collapseEcho(task.subject)
    }' (status: ${task.status}${blockers}).`,
    { kind: "created", task },
  );
}

export async function runTeamTaskList(
  service: TeamToolsService,
  params: TeamTaskListInput,
): Promise<AgentToolResult<TeamTaskListDetails>> {
  const filter = {
...
export async function runTeamTaskGet(
  service: TeamToolsService,
  params: TeamTaskGetInput,
): Promise<AgentToolResult<TeamTaskGetDetails>> {
  try {
    ...
    const lines = [
      ...
      ...(task.blocks.length > 0
        ? [`blocks: ${collapseEcho(task.blocks.join(", "))}`]
        : []),
    ...
    if (isMissingStateError(error)) {
      return toolResult(`No task '${params.task_id}'.`, {
        kind: "not_found",
...
export async function runTeamTaskUpdate(
  service: TeamToolsService,
  params: TeamTaskUpdateInput,
): Promise<AgentToolResult<TeamTaskUpdateDetails>> {
  try {
    ...
    const owner = task.owner === undefined
      ...
      : ` (owner: ${collapseEcho(task.owner)})`;
    return toolResult(
      `Updated task ${task.id} to ${task.status}${owner}: '${
        collapseEcho(task.subject)
      }'.`,
      { kind: "updated", task },
    );
    ...
    if (error instanceof TeamTaskAlreadyClaimedError) {
    ...
    if (error instanceof TeamTaskBlockedByError) {
      return toolResult(reason, {
        kind: "blocked_by",
        task_id: params.task_id,
        reason,
    ...
    if (error instanceof TeamTaskInvalidTransitionError) {
      return toolResult(reason, {
    ...
    if (error instanceof TeamTaskCrossOwnerUpdateError) {
      return toolResult(reason, {
        kind: "cross_owner",
        task_id: params.task_id,
        reason,
...
export function createTeamTaskCreateTool(deps: TeamToolDeps): ToolDefinition {
  return {
    ...
    label: "Task Create",
    description:
      "Create an entry on the team tasklist (starts pending). Does NOT spawn an agent; use the 'task' tool to spawn child work.",
    parameters: TeamTaskCreateParams,
    execute: (_toolCallId: string, params: TeamTaskCreateInput) =>
      runTeamTaskCreate(deps.service, params),
  };
}
...
export function createTeamTaskListTool(deps: TeamToolDeps): ToolDefinition {
  return {
    ...
    label: "Task List",
    description:
      "Team tasklist: list entries, optionally filtered by status (pending, claimed, in_progress, completed, deleted) or owner. Child-agent state lives in task_output, not here.",
    parameters: TeamTaskListParams,
    execute: (_toolCallId: string, params: TeamTaskListInput) =>
      runTeamTaskList(deps.service, params),
  };
}
...
export function createTeamTaskGetTool(deps: TeamToolDeps): ToolDefinition {
  return {
    ...
    label: "Task Get",
    description:
      "Read one team tasklist entry by id; returns not_found if absent. The task_id is a tasklist id, not a child st_... id; use task_output for child agents.",
    parameters: TeamTaskGetParams,
    execute: (_toolCallId: string, params: TeamTaskGetInput) =>
      runTeamTaskGet(deps.service, params),
  };
}
...
export function createTeamTaskUpdateTool(deps: TeamToolDeps): ToolDefinition {
  return {
    ...
    label: "Task Update",
    description:
      "Update a team tasklist entry's status. Transitions run pending -> claimed -> in_progress -> completed, with deleted allowed from any state; status='claimed' claims it for owner (defaults to the lead). Illegal moves return already_claimed, blocked_by, invalid_transition, or cross_owner.",
    parameters: TeamTaskUpdateParams,
    execute: (_toolCallId: string, params: TeamTaskUpdateInput) =>
      runTeamTaskUpdate(deps.service, params),
  };
}
