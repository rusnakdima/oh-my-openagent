...
const InlineTeamSpecMemberSchema = Type.Object(
  {
    name: Type.Optional(
      Type.String({
        description:
          "Member name; unique within the team, lowercase-stem normalized.",
      }),
    ),
    kind: Type.Optional(
      Type.Union([
        Type.Literal("category"),
        Type.Literal("subagent_type"),
        Type.Literal("agent"),
      ], {
        description:
          "Member kind; 'agent' is an alias for subagent_type. Inferred from the category/subagent_type field when omitted.",
      }),
    ),
    ...
    subagent_type: Type.Optional(
      Type.String({
        description:
          "Agent definition to run this member as (kind subagent_type or agent).",
      }),
    ),
    ...
    task_summary: Type.Optional(
      Type.String({
        ...
        description:
          "One-line summary of this member's assigned work, shown in the task footer/widget UI. Longer values are force-truncated to 80 chars.",
      }),
    ),
  },
...
const InlineTeamSpecSchema = Type.Object(
  {
    name: Type.Optional(
      Type.String({
        description:
          "Team name; defaults to a derived inline name when omitted.",
      }),
    ),
    members: Type.Optional(
      Type.Union([
        Type.Array(InlineTeamSpecMemberSchema),
        InlineTeamSpecMemberSchema,
      ], {
        description:
          "Team members (an array, or a single member object which is wrapped into one). The current session is always the lead; do not declare a lead member.",
      }),
    ),
  },
  { additionalProperties: true },
...
export const TeamCreateParams = Type.Object({
  team_name: Type.Optional(
    Type.String({
      description:
        "Named team spec (project .omo/teams or omo.json) to create. Ignored when inline_spec is also provided.",
    }),
  ),
  inline_spec: Type.Optional(
    Type.Union([
      InlineTeamSpecSchema,
      Type.String({
        description:
          "The same spec as a JSON string; parsed automatically. Passing the object form is preferred.",
      }),
    ], {
      description:
        "Inline team spec, e.g. { name, members: [{ name, category|subagent_type, prompt? }] }. A JSON string of the same object is also accepted and parsed automatically. Takes precedence when team_name is also provided.",
    }),
  ),
});
...
export type TeamCreateDetails =
  | {
    ...
    readonly members: readonly TeamCreateMemberView[];
  }
  | { readonly kind: "invalid_arguments"; readonly reason: string }
  | {
    readonly kind: "spec_error";
...
const DELETE_DESCRIPTION =
  "Delete a team run and cancel its members (terminal, not resumable). Lead-only. Pass force=true to tear it down while members are still active.";

function coerceInlineSpec(
  input: unknown,
  ...
  try {
    ...
    return {
      ok: false,
      reason:
        `inline_spec is a string that is not valid JSON (${detail}). Pass the spec as a nested object like { name, members: [{ name, category|subagent_type, prompt? }] }, or as a valid JSON string of that object.`,
    };
  }
}
...
export async function runTeamCreate(
  ...
  if (!hasName && !hasInline) {
    return toolResult("Provide team_name or inline_spec.", {
      kind: "invalid_arguments",
      reason: "provide team_name or inline_spec",
    });
  }
  ...
  if (hasInline) {
    ...
    if (!coerced.ok) {
      return toolResult(coerced.reason, {
        kind: "invalid_arguments",
  ...
  try {
    ...
    const members: TeamCreateMemberView[] = result.members.map((member) => ({
      ...
      ...(member.promptExcerpt !== undefined
        ? { prompt_excerpt: member.promptExcerpt }
        : {}),
      ...(member.taskSummary !== undefined
        ? { task_summary: member.taskSummary }
        : {}),
    }));
    const lines = [
...
export async function runTeamDelete(
  ...
  try {
    const result = await service.deleteTeam({
    ...
    if (error instanceof SenpiTeamRuntimeError) {
      return toolResult(error.message, {
        kind: "invalid_state",
        team_run_id: params.team_run_id,
        code: error.code,
        reason: error.message,
      });
    }
...
function formatCreatedMemberLine(member: CreatedMemberInfo): string {
  const target = formatTargetWithModel({
    category: member.role.kind === "category"
      ? member.role.category
      : undefined,
    agentType: member.role.kind === "category"
      ? undefined
      : member.role.subagentType,
    resolvedModel: member.model,
  });
  return `- ${member.name} [${member.status}] ${
    target ?? formatMemberRole(member.role)
  } task:${member.taskId}`;
}
...
export function createTeamCreateTool(deps: TeamToolDeps): ToolDefinition {
  return {
    ...
    execute: (_toolCallId: string, params: TeamCreateInput) =>
      runTeamCreate(deps.service, params),
  };
}
...
export function createTeamDeleteTool(deps: TeamToolDeps): ToolDefinition {
  return {
    ...
    execute: (_toolCallId: string, params: TeamDeleteInput) =>
      runTeamDelete(deps.service, params),
  };
}
