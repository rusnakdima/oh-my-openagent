...
export type ShutdownErrorView =
  | {
    readonly kind: "unknown_member";
    readonly member: string;
    readonly reason: string;
  }
  | {
    readonly kind: "no_pending_request";
    readonly member: string;
    readonly reason: string;
  };

export type TeamShutdownRequestDetails = {
  readonly kind: "requested";
  readonly team_run_id: string;
  readonly member: string;
} | ShutdownErrorView;
export type TeamApproveShutdownDetails = {
  readonly kind: "approved";
  readonly team_run_id: string;
  readonly member: string;
} | ShutdownErrorView;
export type TeamRejectShutdownDetails = {
...
): Promise<AgentToolResult<TeamShutdownRequestDetails>> {
  try {
    await service.requestShutdown(params.team_run_id, params.member);
    return toolResult(
      `Requested shutdown for '${params.member}' (team ${params.team_run_id}).`,
      {
        kind: "requested",
...
export async function runTeamApproveShutdown(
  service: TeamToolsService,
  params: TeamApproveShutdownInput,
): Promise<AgentToolResult<TeamApproveShutdownDetails>> {
  try {
    await service.approveShutdown(params.team_run_id, params.member);
    return toolResult(
      `Approved shutdown for '${params.member}' (team ${params.team_run_id}).`,
      {
        kind: "approved",
        team_run_id: params.team_run_id,
        member: params.member,
...
export async function runTeamRejectShutdown(
  service: TeamToolsService,
  params: TeamRejectShutdownInput,
): Promise<AgentToolResult<TeamRejectShutdownDetails>> {
  try {
    await service.rejectShutdown(
      params.team_run_id,
      params.member,
      params.reason,
    );
    return toolResult(
      `Rejected shutdown for '${params.member}' (team ${params.team_run_id}): ${
        params.reason.replace(/\s+/g, " ").trim().slice(0, 160)
      }`,
      {
        kind: "rejected",
        team_run_id: params.team_run_id,
