...
import {
  ...
  fakeTask,
} from "./__fixtures__/team-tool-fakes";
import {
...
describe("task_create tool", () => {
  test("#given a new task #when create runs #then it reports the created task", async () => {
    const service = createFakeTeamService({
      createTask: async () => fakeTask(),
    });
    const result = await runTeamTaskCreate(service, {
      team_run_id: "run-1",
      subject: "s",
      description: "d",
    });
  ...
  test("#given a blocked task #when create runs #then the text names the blockers", async () => {
    const service = createFakeTeamService({
      createTask: async () => fakeTask({ blockedBy: ["task-0"] }),
    });
    const result = await runTeamTaskCreate(service, {
      team_run_id: "run-1",
      subject: "s",
  ...
  test("#given a subject with embedded newlines #when create runs #then the text collapses them onto one line", async () => {
    // given
    const service = createFakeTeamService({
      createTask: async () => fakeTask({ subject: "line one\nline two" }),
    });
  ...
  test("#given adversarial owner and blocked_by values #when list and get run #then the echoed fields are collapsed and bounded", async () => {
    ...
    const fetched = await runTeamTaskGet(service, {
      team_run_id: "run-1",
      task_id: "task-1",
    });
    ...
    const listText = listed.content[0]?.type === "text"
  ...
  test("#given the factory #when built #then it names the tool task_create", () => {
    expect(createTeamTaskCreateTool({ service: createFakeTeamService() }).name)
      .toBe("task_create");
  });
});
...
describe("task_list tool", () => {
  test("#given tasks #when list runs #then it reports them, forwarding the filter", async () => {
    const service = createFakeTeamService({
      listTasks: async () => [
        fakeTask(),
        fakeTask({ id: "task-2", status: "claimed", owner: "alpha" }),
      ],
    });
    const result = await runTeamTaskList(service, {
      team_run_id: "run-1",
      status: "pending",
    });
    ...
    expect(service.calls[0]).toMatchObject({
      method: "listTasks",
      args: ["run-1", { status: "pending" }],
    });
  });

  test("#given the factory #when built #then it names the tool task_list", () => {
    expect(createTeamTaskListTool({ service: createFakeTeamService() }).name)
      .toBe("task_list");
  });
});
...
describe("task_get tool", () => {
  test("#given an existing task #when get runs #then it reports the task", async () => {
    const service = createFakeTeamService({
      getTask: async () => fakeTask({ owner: "alpha" }),
    });
    const result = await runTeamTaskGet(service, {
      team_run_id: "run-1",
      task_id: "task-1",
    });
  ...
  test("#given the factory #when built #then it names the tool task_get", () => {
    expect(createTeamTaskGetTool({ service: createFakeTeamService() }).name)
      .toBe("task_get");
  });
});
...
describe("task_update tool", () => {
  test("#given a status update #when update runs #then it reports the updated task", async () => {
    const service = createFakeTeamService({
      updateTask: async () => fakeTask({ status: "in_progress" }),
    });
    const result = await runTeamTaskUpdate(service, {
      team_run_id: "run-1",
      task_id: "task-1",
      status: "in_progress",
    });
  ...
  test("#given the factory #when built #then it names the tool task_update", () => {
    expect(createTeamTaskUpdateTool({ service: createFakeTeamService() }).name)
      .toBe("task_update");
  });
});
