...
export type TeamSendMemberView = {
  readonly member: string;
  readonly outcome: MemberDeliveryOutcome;
};
...
export type TeamSendDetails =
  ...
  | {
    readonly kind: MailboxErrorKind;
    readonly to: string;
...
export async function runTeamSend(
  ...
  try {
    ...
    switch (result.kind) {
      case "to_lead":
        return toolResult(
          `Message enqueued to lead (id: ${result.messageId}).`,
          { kind: "to_lead", message_id: result.messageId },
        );
      case "to_members":
        return toolResult(
