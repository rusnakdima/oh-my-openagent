...
describe("control tool renderers", () => {
      ...
      renderTaskSendCall(
        {
          to: "st_00000001",
          message:
            "Please inspect the database migration and report only the risky steps. tail-marker",
        },
        ANSI_THEME,
      ),
      96,
      ...
      renderTaskSendCall(
        {
          to: "st_1",
          message:
            "한국어로 긴 후속 작업 지시를 작성하고 동일한 세션의 맥락을 검증하세요.",
        },
        ANSI_THEME,
      ),
      72,
  ...
  test("#given structured shutdown task_send messages #when rendering calls #then summaries name request approve reject and reason without object stringification", () => {
      ...
      renderTaskSendCall(
        {
          to: "atlas",
          team_run_id: "team-9",
          message: { type: "shutdown_request", reason: "done for today" },
        },
        TEST_THEME,
      ),
      120,
    ...
    expect(request).toContain(
      "task_send shutdown:request to:atlas team:team-9",
    );
  ...
  test("#given whitespace-only control text #when rendering calls #then empty message and reason labels are omitted", () => {
    const send = firstLine(
      renderTaskSendCall({ to: "atlas", message: " \n\t " }, TEST_THEME),
      80,
    );
    ...
    const cancel = firstLine(
      renderTaskCancelCall({ task_id: "st_1", reason: " \n\t " }, TEST_THEME),
      80,
    );
  ...
  test("#given task_cancel arguments and result variants #when rendering #then identifier reason and status rows are concise", () => {
    const call = firstLine(
      renderTaskCancelCall(
        { name: "alpha", reason: "no longer needed" },
        TEST_THEME,
      ),
      80,
    );
