...
type ShutdownFailureDetails = Extract<
  SendResultDetails,
  { readonly kind: "shutdown_failed" }
>;
type ShutdownFailureContext = Pick<
  ShutdownFailureDetails,
  "operation" | "team_run_id" | "member"
>;
...
export async function resolveSendTeamRunId(
  ...
  if (teamRouting.resolveDefaultTeamRunId === undefined) {
    return {
      kind: "error",
      reason: "team_run_id is required to message a team member",
    };
  }
  const resolved = await teamRouting.resolveDefaultTeamRunId();
  if (resolved.kind === "resolved") {
    return { kind: "resolved", teamRunId: resolved.teamRunId };
  }
...
export async function routeStructuredMessage(
  ...
  if (teamRouting.from !== TEAM_LEAD_SENTINEL) {
    return invalidArguments("shutdown is lead-only");
  }
  ...
  if (message.type === "shutdown_request") {
    try {
      ...
      if (
        !(error instanceof SenpiShutdownError) && !isMissingStateError(error)
      ) throw error;
      return shutdownFailure(error, {
        operation: "request",
        team_run_id: runId,
        member: to,
      });
    }
    return toolResult(`Shutdown requested for ${to} (team ${runId}).`, {
  ...
  try {
    ...
    if (!(error instanceof SenpiShutdownError) && !isMissingStateError(error)) {
      throw error;
    }
    return shutdownFailure(error, {
      operation: "reject",
...
function shutdownFailure(
  ...
  return toolResult(
    `Shutdown ${context.operation} failed for ${context.member}: ${reason}`,
    {
