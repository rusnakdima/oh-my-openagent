...
export function renderTaskSendCall(
  args: TaskSendInput,
  theme: ControlRenderTheme,
): RenderComponent {
  return widthComponent((width) =>
    theme.fg("toolTitle", taskSendCallLine(args, theme, width))
  );
}

export function renderMemberScopedTaskSendCall(
...
export function renderTaskCancelCall(
  args: TaskCancelInput,
  theme: ControlRenderTheme,
): RenderComponent {
  return widthComponent((width) =>
    theme.fg("warning", taskCancelCallLine(args, theme, width))
  );
}
...
function widthComponent(
  ...
  return {
    render: (
      width: number,
    ): string[] => [truncateToWidth(renderLine(width), width, ELLIPSIS)],
    invalidate: (): void => {},
  };
}

function taskSendCallLine(
  args: TaskSendInput,
  theme: ControlRenderTheme,
  width: number,
): string {
  if (typeof args.message === "object" && args.message !== null) {
    return shutdownCallLine(args, args.message, theme, width);
  }
  ...
  if (typeof args.message === "string") {
    return withExcerpt(base, "message", args.message, theme, width);
  }
  return base;
}
...
function shutdownCallLine(
  ...
  switch (message.type) {
    case "shutdown_request": {
      const base = joinRendererTokens([
        "task_send shutdown:request",
        target,
        team,
      ]);
      return message.reason === undefined
        ? base
        : withExcerpt(base, "reason", message.reason, theme, width);
    }
...
function taskCancelCallLine(
  args: TaskCancelInput,
  theme: ControlRenderTheme,
  width: number,
): string {
  const target = normalizeRendererText(
    args.task_id ?? args.name ?? "<missing>",
  );
  const base = joinRendererTokens(["task_cancel", `target:${target}`]);
  return args.reason === undefined
    ? base
    : withExcerpt(base, "reason", args.reason, theme, width);
}
...
function withExcerpt(
  ...
  const available = Math.min(
    maxExcerpt,
    Math.max(0, width - rendererVisibleWidth(prefix) - quoteOverhead),
  );
  if (
    rendererVisibleWidth(normalized) > available &&
    available < MIN_MEANINGFUL_TRUNCATED_EXCERPT_WIDTH
  ) return base;
  ...
  switch (details.kind) {
    case "steered":
      return {
        ...
        text:
          `task_send delivered ${details.task_id} as ${details.delivered} (${details.status})`,
      };
    case "revived":
      return {
        color: "success",
        text: `task_send revived ${details.task_id} epoch ${details.run_epoch}`,
      };
    case "queued":
      return {
        color: "muted",
        text:
          `task_send queued ${details.task_id} position ${details.queue_position}`,
      };
    case "not_continuable":
      return {
        color: "warning",
        text:
          `task_send not continuable ${details.task_id}: ${details.reason} ${details.suggestion}`,
      };
    case "one_shot_agent":
      return {
        color: "error",
        text: `task_send denied ${details.task_id} one-shot:${details.agent}`,
      };
    case "scope_denied":
      return {
        color: "error",
        text:
          `task_send denied ${details.task_id} owner:${details.owning_session_id}`,
      };
    case "not_found":
    ...
    case "shutdown_requested":
      return {
        color: "warning",
        text:
          `task_send shutdown requested ${details.team_run_id} member:${details.member}`,
      };
    case "shutdown_responded":
      return {
        color: details.approved ? "success" : "warning",
        text: `task_send shutdown ${
          details.approved ? "approved" : "rejected"
        } ${details.team_run_id} member:${details.member}`,
      };
    case "shutdown_failed":
      return {
        color: "error",
        text:
          `task_send shutdown ${details.operation} failed ${details.team_run_id} member:${details.member}: ${details.reason}`,
      };
    default:
...
function notFoundText(
  details: Extract<SendResultDetails, { readonly kind: "not_found" }>,
): string {
  if (details.known_tasks.length === 0) {
    return `task_send not found: ${details.reason}`;
  }
  return `task_send not found: ${details.reason} known:${
    details.known_tasks.join(",")
  }`;
}
...
function teamMessageRow(
  ...
  switch (details.kind) {
    case "to_lead":
      return {
        color: "success",
        text: `task_send team message ${details.message_id} enqueued to lead`,
      };
    case "to_members":
      return {
        color: "success",
        text:
          `task_send team message ${details.message_id} enqueued to ${details.recipients.length} member(s)`,
      };
    ...
    case "team_deleting":
      return {
        color: "error",
        text:
          `task_send team ${details.kind} to:${details.to}: ${details.reason}`,
      };
    default:
      return assertNever(details);
  }
  ...
  switch (details.kind) {
    case "cancelled":
      return {
        color: statusThemeColor(details.status),
        text:
          `task_cancel cancelled ${details.task_id} (${details.previous_status} -> ${details.status})`,
      };
    case "noop":
      return {
        color: statusThemeColor(details.status),
        text:
          `task_cancel no change ${details.task_id} (${details.status}): ${details.reason}`,
      };
    case "not_found":
      return {
        color: "error",
        text: `task_cancel not found: ${details.reason}`,
      };
    case "invalid_arguments":
