...
type SendResultRenderCase = readonly [
  name: string,
...
const SEND_RESULT_RENDER_CASES = [
  ...
  ["team_message", {
    kind: "team_message",
    team: { kind: "to_lead", message_id: "msg-1" },
...
describe("task_send result renderers", () => {
  ...
  test("#given injected controls in task_send and task_cancel results #when rendered #then dynamic controls are removed before trusted theme styling", () => {
    ...
    const cancelDetails: CancelResultDetails = {
      kind: "not_found",
      reason:
        "없음 \u001b]8;;https://example.com\u001b\\링크\u001b]8;;\u001b\\\u007f",
    };

    // when
    const send = firstLine(
      renderTaskSendResult(
        toolResult("ignored", sendDetails),
        RESULT_OPTIONS,
        ANSI_THEME,
      ),
      120,
    ...
    const cancel = firstLine(
      renderTaskCancelResult(
        toolResult("ignored", cancelDetails),
        RESULT_OPTIONS,
        ANSI_THEME,
      ),
      120,
    );
    const plainSend = firstLine(
      renderTaskSendResult(
        toolResult("ignored", sendDetails),
        RESULT_OPTIONS,
        TEST_THEME,
