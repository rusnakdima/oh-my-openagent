...
const commitSubscribers = new WeakMap<
  DagFileStore,
  Map<DagRunId, Set<CommitSubscriber>>
>();
...
export function subscribeDagJournal(
  ...
  const storeSubscribers = commitSubscribers.get(store) ??
    new Map<DagRunId, Set<CommitSubscriber>>();
  const runSubscribers = storeSubscribers.get(runId) ??
    new Set<CommitSubscriber>();
...
export function createDagJournal<TCheckpoint extends DagJournalCheckpoint>(
  ...
  const ringSize = options.subscriberRing ??
    DAG_SETTINGS_DEFAULTS.subscriber_ring;
  if (!Number.isInteger(ringSize) || ringSize <= 0) {
    throw new Error("subscriber ring must be a positive integer");
  }
  const now = options.now ?? Date.now;
  const subscribers = new Set<Subscriber>();
  let checkpoint = options.store.withRunLock(
    options.runId,
    () => recoverCheckpoint(options),
  );

  const appendPayload = (
    payload: DagRunEventPayload,
    directSubscriber?: Subscriber,
  ): DagRunEvent => {
    ...
    checkpoint = options.store.withRunLock(options.runId, () => {
      ...
      const tailSeq = eventLogTailSeq(
        options.store,
        options.runId,
        recovered.checkpointSeq,
      );
  ...
  return {
    ...
    subscribe(listener) {
      const subscriber: Subscriber = {
        ...
        commitOverflow: (overflow) =>
          appendPayload(dagStreamOverflowEvent(overflow), subscriber),
    ...
    whenIdle: async () => {
      await Promise.all(
        [...subscribers].map((subscriber) => subscriber.drainPromise),
      );
    },
  };
}
...
function recoverCheckpoint<TCheckpoint extends DagJournalCheckpoint>(
  ...
  let checkpoint = options.store.readCheckpoint<TCheckpoint>(options.runId) ??
    options.initialCheckpoint;
  let sinceSeq = checkpoint.checkpointSeq;
  let replayed = false;
  for (;;) {
    const page = options.store.readEvents(options.runId, sinceSeq, {
...
async function drain(subscriber: Subscriber): Promise<void> {
  try {
    ...
    if (
      subscriber.active &&
      (subscriber.overflow !== undefined || subscriber.queue.length > 0)
    ) {
