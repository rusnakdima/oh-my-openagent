...
class FakeTaskManager implements TaskManager {
  ...
  readonly #startedSignals = new Map<
    string,
  ...
  complete(nodeId: string, status: TaskStatus = "completed"): void {
    const task = [...this.#tasks.values()].find((entry) =>
      entry.record.owner?.nodeId === nodeId
    );
    if (task === undefined) throw new Error(`unknown fake task for ${nodeId}`);
    if (task.record.status === "pending" || task.record.status === "running") {
      this.#residents -= 1;
    }
    task.record = {
      ...task.record,
  ...
  async startOwned(
    ...
    owner: DagTaskOwner,
  ): Promise<OwnedStartResult> {
    const nodeId = owner.nodeId as string;
    this.attempts.push(nodeId);
    this.owners.push(owner);
    if (this.#options.rejectStartNodeIds?.includes(nodeId) === true) {
      throw new Error(`start rejected ${nodeId}`);
    }
    this.startedSpecs.push(spec);
    const existing = [...this.#tasks.values()].find((entry) =>
    ...
    if (existing !== undefined) {
      ...
      const terminal = existing.record.status !== "pending" &&
        existing.record.status !== "running";
      if (
        existing.record.owner?.fingerprint === owner.fingerprint || !terminal
      ) {
        return {
          kind: "started",
          reused: true,
    ...
    const startFailureKind = this.#options.startFailureKinds?.[nodeId] ??
      (this.#options.startFailureNodeIds?.includes(nodeId) === true
        ? "start_failed"
        : undefined);
    if (startFailureKind === "plan_unresolved") {
      return {
        kind: "plan_unresolved",
        error: { code: "unknown_target", message: `unresolved ${nodeId}` },
      };
    }
    if (startFailureKind === "depth_denied") {
      return {
        kind: "depth_denied",
        reason: `depth denied ${nodeId}`,
        child_depth: 2,
        max_depth: 1,
      };
    }
    ...
    if (this.#options.autoComplete !== false) {
      queueMicrotask(() => this.complete(nodeId));
    }
    return {
      kind: "started",
      reused: false,
  ...
  findOwnedTask(
    owner: Pick<DagTaskOwner, "kind" | "runId" | "nodeId">,
  ...
  seed(
    ...
    const record: TaskRecord = {
      ...
      owner: {
        ...
        fingerprint: `fingerprint-${nodeId}`,
      },
      status,
      residency_state: "resident",
      created_at: "2026-08-14T00:00:00.000Z",
  ...
  sendToTask(input: SendInput): Promise<SendOutcome> {
    ...
    if (task === undefined) {
      return Promise.resolve({
        kind: "not_found",
        reason: `No task found for "${input.idOrName}".`,
    ...
    if (
      task.record.status === "completed" || task.record.status === "error" ||
    ...
    return Promise.resolve({
      ...
      task_id: task.record.task_id,
      reason:
        `Task ${task.record.task_id} is ${task.record.status} and can no longer be continued.`,
      suggestion: "use task_output",
    });
  }
  interruptTask(): Promise<never> {
    throw new Error("not implemented");
  }
  async cancelTask(
    taskId: string,
  ): Promise<
    {
      ...
      readonly previous_status: TaskStatus;
    }
  > {
    const task = this.#tasks.get(taskId);
    if (task === undefined) throw new Error(`unknown fake task ${taskId}`);
    ...
    if (this.#options.rejectCancelNodeIds?.includes(nodeId) === true) {
      throw new Error(`cancel rejected ${nodeId}`);
    }
...
function schedulerFixture(
  ...
  const events = (): readonly DagRunEvent[] =>
    store.readEvents(runId, 0, { limit: 100 }).events;
  return {
    scheduler,
    events,
    store,
    resultPathCallsUnderLock: () => resultPathCallsUnderLock,
  };
}

function waveMembership(
  events: readonly DagRunEvent[],
  type: "dag.wave.started" | "dag.wave.completed",
): readonly string[][] {
  return events
    .filter((event): event is Extract<DagRunEvent, { type: typeof type }> =>
      event.type === type
    )
...
describe("DAG scheduler terminal result persistence", () => {
  test("#given only the senpi-task scheduler #when a node completes #then output and run stats are persisted without an adapter", async () => {
    ...
    const { scheduler, store, resultPathCallsUnderLock } = schedulerFixture(
      definition([node("artifact")]),
      manager,
    );
    ...
    expect(fs.readFileSync(store.paths.result(runId, "artifact"), "utf8")).toBe(
      "done artifact",
    );
    expect(
      JSON.parse(
        fs.readFileSync(
          store.paths.result(runId, "artifact").replace(
            /\.txt$/,
            ".stats.json",
          ),
          "utf8",
...
describe("DAG scheduler subscriber backpressure", () => {
  test("#given a non-default subscriber ring #when a scheduler listener falls behind #then overflow occurs at the configured bound", async () => {
    ...
    let overflow:
      | Extract<DagRunEvent, { type: "dag.stream.overflow" }>
      | undefined;
...
describe("DAG scheduler failure semantics", () => {
  test("#given every terminal task status #when folded #then each maps to its exact node outcome and error code", async () => {
    ...
    for (const outcome of cases) {
      ...
      const { scheduler } = schedulerFixture(
        definition([node(`task-${outcome.status}`)]),
        manager,
      );
      const running = scheduler.run();
  ...
  test("#given every start denial #when admission fails #then each maps to its exact node error code", async () => {
    ...
    const { scheduler } = schedulerFixture(
      definition(Object.keys(expected).map((id) => node(id))),
      manager,
    );
    ...
    expect(
      Object.fromEntries(
        result.nodes.map((entry) => [entry.id, entry.error?.code]),
      ),
    ).toEqual(expected);
  });
  ...
  test("#given a failed root with a descendant chain #when failure cascades #then every descendant skip is persisted separately", async () => {
    ...
    const skipEvents = events().filter((
      event,
    ): event is Extract<DagRunEvent, { type: "dag.node.transitioned" }> =>
      event.type === "dag.node.transitioned" && event.to === "skipped"
    );
    expect(skipEvents.map((event) => String(event.nodeId))).toEqual([
      "child",
      "grandchild",
    ]);
  ...
  test("#given graph-ordered failures finish out of order #when independent work settles #then the run uses the first wave and declaration failure", async () => {
    ...
    scheduler.subscribe((event) => {
      if (
        event.type === "dag.node.transitioned" &&
        event.nodeId === "completion-first" && event.to === "failed"
      ) {
    ...
    const completionFirst = result.nodes.find((entry) =>
      entry.id === "completion-first"
    );
    const graphFirst = result.nodes.find((entry) => entry.id === "graph-first");
    expect(Date.parse(completionFirst?.completedAt ?? "")).toBeLessThan(
      Date.parse(graphFirst?.completedAt ?? ""),
    );
    const failedEvent = events().find((event) =>
      event.type === "dag.run.failed"
    );
    expect(failedEvent).toEqual(
      expect.objectContaining({
        error: expect.objectContaining({ nodeId: "graph-first" }),
      }),
    );
  });
});
...
describe("DAG scheduler cancellation", () => {
  test("#given durable waiters armed before and during cancellation #when the scheduler cancels through a separate journal path #then every wait and re-attach settles cancelled", async () => {
    ...
    const { scheduler, store } = schedulerFixture(
      definition([node("CA"), node("CB", ["CA"])]),
      manager,
    );
    ...
    expect(results.every((result) => result.nodes.CA?.state === "cancelled"))
      .toBe(true);
    expect(results.every((result) => result.nodes.CB?.state === "cancelled"))
      .toBe(true);
    expect(
      results.every((result) =>
        result.nodes.CA?.state === "cancelled" &&
        result.nodes.CA.reason === "live cancel"
      ),
    ).toBe(true);
  ...
  test("#given a running wave and pending descendants #when cancelled #then tasks cancel, admission stops, and waiters resolve cancelled", async () => {
    ...
    scheduler.subscribe((event) => {
      if (
        event.type === "dag.node.task-attached" && event.nodeId === "running"
      ) attached.resolve();
      if (
        event.type === "dag.node.transitioned" && event.nodeId === "finished" &&
        event.to === "completed"
      ) {
    ...
    expect(runResult.nodes.map((entry) => `${entry.id}:${entry.state}`))
      .toEqual([
        "finished:completed",
        "running:cancelled",
        "queued:cancelled",
  ...
  test("#given one startOwned rejects after a sibling starts #when cancellation follows #then admission clears and the sibling is attached and cancelled", async () => {
    ...
    const manager = new FakeTaskManager({
      autoComplete: false,
      rejectStartNodeIds: ["reject"],
    });
    const { scheduler } = schedulerFixture(
      definition([node("sibling"), node("reject")]),
      manager,
    );
    const running = scheduler.run();
    ...
    expect(result.nodes.find((entry) => entry.id === "sibling")?.state).toBe(
      "cancelled",
    );
  });
  ...
  test("#given an AbortError returned by task cancellation #when the run is cancelled #then durable cancellation settles and the rejection still surfaces", async () => {
    ...
    // when
    const cancellationFailure = expect(
      scheduler.cancel(runId, "intentional cancel"),
    ...
    expect(after.nodes.a).toEqual(
      expect.objectContaining({ state: "cancelled" }),
    );
    expect(after.nodes.b).toEqual(
      expect.objectContaining({ state: "cancelled" }),
    );
  });
  ...
  test("#given a genuine task cancellation failure #when a wave is cancelled #then the run settles cancelled and the failure still surfaces", async () => {
    ...
    // when
    const cancellationFailure = expect(scheduler.cancel(runId, "reject one"))
      .rejects.toThrow("cancel rejected a");
    const [runResult, firstResult, secondResult] = await Promise.all([
      running,
      firstWaiter,
      secondWaiter,
    ]);
    ...
    expect(
      firstResult.snapshot.nodes.every((entry) => entry.state === "cancelled"),
    ).toBe(true);
  });
  ...
  test("#given a paused unclaimed run #when cancelled #then it ends cancelled without task cancellation", async () => {
    ...
    expect(
      scheduler.snapshot().nodes.every((entry) => entry.state === "cancelled"),
    ).toBe(true);
  });
});
...
describe("DAG scheduler strict wave barrier", () => {
  test("#given a linear three-wave DAG #when run #then nodes and wave events are admitted in order", async () => {
    ...
    const manager = new FakeTaskManager();
    const { scheduler, events } = schedulerFixture(
      definition([node("a"), node("b", ["a"]), node("c", ["b"])]),
      manager,
    );
    ...
    expect(waveMembership(events(), "dag.wave.started")).toEqual([
      ["a"],
      ["b"],
      ["c"],
    ]);
    expect(waveMembership(events(), "dag.wave.completed")).toEqual([["a"], [
      "b",
    ], ["c"]]);
    expect(result.nodes.map((entry) => `${entry.id}:${entry.state}`)).toEqual([
      "a:completed",
  ...
  test("#given a diamond DAG #when run #then the fan-out shares one wave and the join waits for it", async () => {
    ...
    // then
    expect(waveMembership(events(), "dag.wave.started")).toEqual([["a"], [
      "b",
      "c",
    ], ["d"]]);
  ...
  test("#given one waitFor rejects while a sibling is running #when the wave settles #then the rejection becomes one node failure and the sibling completes", async () => {
    ...
    const manager = new FakeTaskManager({
      autoComplete: false,
      rejectWaitNodeIds: ["reject"],
    });
    const { scheduler } = schedulerFixture(
      definition([node("reject"), node("sibling")]),
      manager,
    );
    const rejected = deferred<void>();
    scheduler.subscribe((event) => {
      if (
        event.type === "dag.node.transitioned" && event.nodeId === "reject" &&
        event.to === "failed"
      ) rejected.resolve();
    });
    const running = scheduler.run();
    await Promise.all([
      manager.whenStarted("reject"),
      manager.whenStarted("sibling"),
    ]);
    ...
    expect(result.nodes.find((entry) => entry.id === "reject")?.error).toEqual(
      expect.objectContaining({
        code: "task_error",
  ...
  test("#given one root start fails #when its wave is admitted #then siblings and the independent branch still run", async () => {
    ...
    expect(waveMembership(events(), "dag.wave.started")).toEqual([["a", "b"], [
      "c",
    ]]);
  });
  ...
  test("#given a same-wave node is residency denied #when an attached sibling frees a slot #then admission retries without failing it", async () => {
    ...
    expect(
      events().filter((event) =>
        event.type === "dag.node.transitioned" && event.nodeId === "b" &&
        event.to === "failed"
      ),
    ).toEqual([]);
    manager.complete("b");
    const result = await running;
    expect(result.nodes.find((entry) => entry.id === "b")?.state).toBe(
      "completed",
    );
  });
  ...
  test("#given a wave wider than residency capacity #when tasks settle #then all nodes are admitted in batches without a second concurrency limit", async () => {
    ...
    const { scheduler } = schedulerFixture(
      definition([node("a"), node("b"), node("c"), node("d"), node("e")]),
      manager,
    );
    ...
    expect(result.nodes.every((entry) => entry.state === "completed")).toBe(
      true,
    );
  });
});
...
describe("DAG scheduler node spawn policy", () => {
  test("#given a policy that denies an agent-routed node #when the wave admits #then the node fails with the denial message and startOwned is never called", async () => {
    ...
    expect(result.nodes[0]?.error?.message).toContain(
      "momus requires a plan gate",
    );
  ...
  test("#given a policy that forces the prompt #when the node starts #then the child receives the forced prompt", async () => {
    ...
    const manager = new FakeTaskManager();
    const canonical =
      "Review the work plan at .omo/plans/x.md for contradictions and blocking issues.";
    const { scheduler } = schedulerFixture(
      definition([{
        id: "review",
        prompt: "caller wording",
        subagent_type: "momus",
      }]),
      manager,
      undefined,
...
function settledFixture(
  ...
  const scheduler = createDagScheduler({
    ...
    ...(lease.isProcessAlive === undefined
      ? {}
      : { isProcessAlive: lease.isProcessAlive }),
  });
  const events = (): readonly DagRunEvent[] =>
    store.readEvents(runId, 0, { limit: 200 }).events;
...
describe("DAG scheduler node controls", () => {
  test("#given a completed node #when retry is requested #then it refuses with node_not_retryable", () => {
    ...
    const { scheduler } = settledFixture(
      definition([node("done"), node("broken")]),
      manager,
      {
    ...
    try {
      ...
      expect((error as DagNodeControlError).nodeIds.map(String)).toEqual([
        "done",
      ]);
    }
    expect(
      scheduler.snapshot().nodes.find((entry) => entry.id === "done")?.state,
    ).toBe("completed");
  });
  ...
  test("#given a cancelled node child #when send is requested #then it refuses with node_not_continuable and names retry", async () => {
    ...
    // when
    const sent = scheduler.sendToNode(
      runId,
      "cancelled" as DagNodeId,
      "try again",
    );
  ...
  test("#given a failed node with skipped dependents #when retried #then execAttempt increments, dependents unblock, and the run re-enters to completion", async () => {
    ...
    const retriedNode = retried.record.nodes.find((entry) =>
      entry.id === "broken"
    );
    ...
    expect(
      retried.record.nodes.find((entry) => entry.id === "dependent")?.state,
    ).toBe("pending");
    expect(record.status).toBe("completed");
    ...
    expect(record.nodes.find((entry) => entry.id === "root")?.taskId).toBe(
      "task-root",
    );
    const retryEvent = events().find((event) =>
      event.type === "dag.node.retried"
    );
    expect(retryEvent).toMatchObject({
      ...
      execAttempt: 1,
      promptChanged: false,
      priorTaskId: "task-broken",
    });
    const types = events().map((event) => event.type);
    expect(types.indexOf("dag.run.resumed")).toBeGreaterThan(
      types.indexOf("dag.node.retried"),
    );
  });
  ...
  test("#given a retried node #when it is admitted again #then the owner fingerprint folds the new execAttempt", async () => {
    ...
    expect(manager.owners[0]?.fingerprint).toBe(
  ...
  test("#given an explicit retry of only the failed ancestor #when re-entered #then its skipped dependents are restored to blocked", async () => {
    ...
    const { scheduler, events } = settledFixture(
      definition([
        node("broken"),
        node("dependent", ["broken"]),
        node("grandchild", ["dependent"]),
      ]),
      manager,
      {
        broken: { state: "failed", taskId: "task-broken", attempt: 1 },
    ...
    // then
    expect(retried.record.nodes.map((entry) => `${entry.id}:${entry.state}`))
      .toEqual([
        "broken:pending",
        "dependent:blocked",
        "grandchild:blocked",
    ...
    expect(
      retried.record.nodes.filter((entry) => entry.execAttempt !== undefined)
        .map((entry) => String(entry.id)),
    )
      .toEqual(["broken"]);
    expect(
      events().filter((event) =>
        event.type === "dag.node.transitioned" && event.to === "blocked"
      ).map((event) =>
  ...
  test("#given a skipped node whose failed ancestor stays failed #when retried alone #then it refuses and names the ancestor", () => {
    ...
    // when
    const refusal = () =>
      scheduler.retryNode(runId, ["dependent" as DagNodeId]);
  ...
  test("#given a running run #when retry is requested #then it refuses with run_still_active and journals nothing", () => {
    ...
    const manager = new FakeTaskManager();
    const { scheduler, events } = settledFixture(
      definition([node("broken"), node("busy")]),
      manager,
      {
        broken: { state: "failed", taskId: "task-broken", attempt: 1 },
  ...
  test("#given a foreign pid holds the lease and is alive #when retry is requested for THIS run #then it refuses with run_not_owned naming that pid", () => {
    ...
    const probed: number[] = [];
    const { scheduler, runId } = settledFixture(
      definition([node("broken")]),
      manager,
      {
        broken: { state: "failed", taskId: "task-broken", attempt: 1 },
    ...
    try {
      ...
      expect((error as DagNodeControlError).message).toContain(
        String(foreignPid),
      );
    }
  ...
  test("#given the lease pid is dead #when retry is requested #then the guard lets it through and the node re-runs", () => {
    ...
    const manager = new FakeTaskManager();
    const foreignPid = 424242;
    const { scheduler, runId, events } = settledFixture(
      definition([node("broken")]),
      manager,
      {
        broken: { state: "failed", taskId: "task-broken", attempt: 1 },
    ...
    // then
    expect(events().some((event) => event.type === "dag.node.retried")).toBe(
      true,
    );
  });
  ...
  test("#given a running node #when send delivers a steer #then the node stays running and the journal records delivery steer", async () => {
    ...
    // when
    const sent = await scheduler.sendToNode(
      runId,
      "chatty" as DagNodeId,
      "focus on the tests",
    );
    ...
    // then
    expect({ ...sent, nodeId: String(sent.nodeId) }).toEqual({
      delivery: "steer",
      nodeId: "chatty",
      taskId: "task-1",
    });
    expect(manager.sends.map((entry) => entry.message)).toEqual([
      "focus on the tests",
    ]);
    expect(scheduler.snapshot().nodes[0]?.state).toBe("running");
    expect(store.readCheckpoint<DagRunRecordV1>(runId)?.nodes[0]?.state).toBe(
      "running",
    );
    expect(events()).toContainEqual(expect.objectContaining({
  ...
  test("#given a failed node whose child is still resident #when send revives it #then the node runs again and its new outcome settles exactly once", async () => {
    // given
    const manager = new FakeTaskManager();
    const { scheduler, events, store, durable } = settledFixture(
      definition([node("revivable")]),
      manager,
      {
        revivable: {
          state: "failed",
    ...
    expect(events()).toContainEqual(
      expect.objectContaining({ type: "dag.node.steered", delivery: "revive" }),
    );
    expect(
      events().filter((event) =>
        event.type === "dag.node.transitioned" && event.to === "completed"
      ),
    ).toHaveLength(1);
    expect(fs.readFileSync(store.paths.result(runId, "revivable"), "utf8"))
      .toBe("done revivable");
  });
  ...
  test("#given the full send outcome vocabulary #when a node child is messaged #then each outcome maps to its exact result or refusal", async () => {
    ...
    const manager = new FakeTaskManager({
      sendOutcomes: {
        "one-shot": {
          ...
          agent: "momus",
          message: "momus takes no follow-ups",
        },
        denied: {
          kind: "scope_denied",
          task_id: "task-denied",
          owning_session_id: "other",
          reason: "belongs to another session",
        },
        detached: {
          kind: "not_continuable",
          task_id: "task-detached",
          reason: "suspended",
          suggestion: "task_output",
        },
      },
    });
    const { scheduler } = settledFixture(
    ...
    // when
    const queued = await scheduler.sendToNode(
      runId,
      "queued" as DagNodeId,
      "queued message",
    );
    ...
    for (
      ...
      try {
        ...
        outcomes[nodeId] = `${(error as DagNodeControlError).code}:${
          (error as DagNodeControlError).message
        }`;
      }
    }
  ...
  test("#given a single explicit node and a prompt override #when retried #then the amendment is journaled before the retry and the new prompt runs", async () => {
    ...
    expect(types.indexOf("dag.definition.amended")).toBeLessThan(
      types.indexOf("dag.node.retried"),
    );
    expect(events().find((event) => event.type === "dag.node.retried"))
      .toMatchObject({ promptChanged: true });
    expect(manager.startedSpecs.map((spec) => spec.prompt)).toEqual([
      "do broken differently",
      "do dependent",
    ]);
  });
  ...
  test("#given a prompt override the definition compiler rejects #when retried #then the refusal is total: invalid_arguments, nothing journaled, no task started", () => {
    ...
    const manager = new FakeTaskManager();
    const { scheduler, events, runId: id } = settledFixture(
      definition([node("broken")]),
      manager,
      {
        broken: { state: "failed", taskId: "task-broken", attempt: 1 },
    ...
    // when
    const refusal = () =>
      scheduler.retryNode(id, ["broken" as DagNodeId], { prompt: oversized });
  ...
  test("#given a prompt override with more than one target #when retried #then it refuses with invalid_arguments", () => {
    ...
    // when
    const refusal = () =>
      scheduler.retryNode(runId, ["a" as DagNodeId, "b" as DagNodeId], {
        prompt: "changed",
      });
...
describe("applyDagSchedulerEvent resume reducer", () => {
  ...
  test("#given a retried event #when applied through the scheduler reducer #then the shared record mutation runs", () => {
    ...
    // then
    expect(retried.nodes[0]).toMatchObject({
      state: "pending",
      execAttempt: 1,
      attempt: 1,
    });
...
describe("DAG scheduler control-event replay", () => {
  test("#given a retried and revived run #when the checkpoint is rebuilt from the WAL alone #then it matches the live record", async () => {
    ...
    const replayed = createDagJournal<DagRunRecordV1>({
      ...
      applyEvent: (checkpoint, event) =>
        applyDagSchedulerEvent(checkpoint, event),
    });
    ...
    expect(
      replayed.snapshot().nodes.map((entry) =>
        `${entry.id}:${entry.state}:${entry.execAttempt ?? 0}`
      ),
    ).toEqual(
      live.nodes.map((entry) =>
        `${entry.id}:${entry.state}:${entry.execAttempt ?? 0}`
      ),
    );
