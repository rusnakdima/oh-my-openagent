...
describe("createDagFileStore event WAL", () => {
  ...
  test("#given an existing event seq #when a distinct event reuses it #then the WAL rejects the duplicate", () => {
    ...
    const appendDuplicate = () =>
      store.appendEvent(event(1, { generation: 2 }));

    // then
    expect(appendDuplicate).toThrow(
      "DAG event seq must be strictly increasing",
    );
  ...
  test("#given fsync is disabled #when WAL, lock, and checkpoint writes run #then ordering remains without durability barriers", () => {
    ...
    expect(
      store.readEvents(runId, 0, { limit: 10 }).events.map(({ seq }) => seq),
    ).toEqual([1]);
    expect(
      store.readCheckpoint<{ checkpointSeq: number }>(runId)?.checkpointSeq,
    ).toBe(1);
  });
  ...
  test("#given a WAL ending in a torn fragment #when a new store opens #then valid events remain and the tail is diagnosed and discarded", () => {
    ...
    fs.appendFileSync(
      writer.paths.event(runId),
      '{"schemaVersion":1,"runId":"run-1","seq":3',
    );
  ...
  test("#given a future-schema event file #when a store opens #then it fails closed with a journal_corrupt diagnostic", () => {
    ...
    fs.writeFileSync(
      seeded.paths.event(runId),
      `${JSON.stringify({ ...event(1), schemaVersion: 2 })}\n`,
    );
...
describe("createDagFileStore checkpoints and layout", () => {
  test("#given POSIX checkpoint persistence #when an existing checkpoint is replaced #then readers see the old complete file until rename and both file and directory are fsynced", () => {
    ...
    fs.writeFileSync(
      store.paths.run(runId),
      JSON.stringify({ schemaVersion: 1, runId, generation: 1 }),
    );
    ...
    spyOn(fs, "renameSync").mockImplementation((from, to) => {
      ...
      observedBeforeRename.push(
        JSON.parse(fs.readFileSync(to, "utf8")) as unknown,
      );
      realRename(from, to);
    });
    ...
    // then
    expect(observedBeforeRename).toEqual([{
      schemaVersion: 1,
      runId,
      generation: 1,
    }]);
    expect(store.readCheckpoint<{ generation: number }>(runId)?.generation)
      .toBe(2);
  ...
  test("#given a future-schema checkpoint #when it is opened #then it fails closed with a journal_corrupt diagnostic", () => {
    ...
    fs.writeFileSync(
      store.paths.run(runId),
      JSON.stringify({ schemaVersion: 2, runId }),
    );
  ...
  test("#given a parent session and run key #when writing the key #then its filename is the exact nul-delimited sha256", () => {
    ...
    const expectedHash = createHash("sha256").update(
      `${parentSessionId}\0${runKey}`,
    ).digest("hex");

    // when
    const path = store.writeKey({
      schemaVersion: 1,
      parentSessionId,
      runKey,
      runId,
...
describe("createDagFileStore locks and retention", () => {
  test("#given a lock held by a dead pid #when the run lock is acquired #then the stale lock is reclaimed", () => {
    ...
    fs.writeFileSync(
      store.paths.runLock(runId),
      JSON.stringify({ hostPid: 2_147_483_647 }),
    );
    let entered = false;
  ...
  test("#given a Windows lock already held by a live process #when a second acquirer races #then it observes the holder instead of crashing", () => {
    ...
    fs.writeFileSync(
      store.paths.runLock(runId),
      JSON.stringify({ hostPid: holderPid, owner: "live" }),
    );
  ...
  test("#given a concurrent observer watches stale reclamation #when ownership changes #then the canonical lock path is never vacant", () => {
    ...
    fs.writeFileSync(
      store.paths.runLock(runId),
      JSON.stringify({ hostPid: 2_147_483_647, token: "stale" }),
    );
    ...
    const observe = () => {
      ...
      while (Atomics.load(signal, 0) !== 2) {
        if (Atomics.wait(signal, 0, 1, 1_000) === "timed-out") {
          throw new Error("lock observer timed out");
        }
      }
    ...
    spyOn(fs, "renameSync").mockImplementation((from, to) => {
      realRename(from, to);
      if (
        observingReclamation &&
        (from === store.paths.runLock(runId) ||
          to === store.paths.runLock(runId))
      ) observe();
    });
  ...
  test("#given two reclaimers validate one stale holder #when they contend at publication #then reclamation stays exclusive", () => {
    ...
    const isProcessAlive = (pid: number) =>
      pid === process.pid || pid === contenderPid;
    ...
    const contenderHolder = JSON.stringify({
      hostPid: contenderPid,
      owner: "contender",
    });
    ...
    spyOn(fs, "renameSync").mockImplementation((from, to) => {
      if (
        !probingPublication && !publicationProbed && typeof from === "string" &&
        ...
        try {
          ...
          if (
            ...
            !error.message.includes("Timed out acquiring DAG lock")
          ) throw error;
        }
    ...
    const finalHolder = fs.existsSync(canonical)
      ? JSON.parse(fs.readFileSync(canonical, "utf8")) as {
  ...
  test("#given a second reclaimer observes sentinel initialization #when the first is preempted after open #then no ownerless sentinel is visible", () => {
    ...
    const first = createDagFileStore({ project_dir: project }, {
      isProcessAlive,
    });
    const second = createDagFileStore({ project_dir: project }, {
      isProcessAlive,
    });
    ...
    fs.writeFileSync(
      canonical,
      JSON.stringify({ hostPid: 101, token: "stale-holder" }),
    );
    ...
    spyOn(fs, "linkSync").mockImplementation((existingPath, newPath) => {
      ...
      const value = JSON.parse(
        fs.readFileSync(reclaimSentinel, "utf8"),
      ) as unknown;
      publishedSentinelHadOwnerRecord = typeof value === "object" &&
        value !== null &&
    ...
    spyOn(fs, "openSync").mockImplementation((path, flags, mode) => {
      ...
      if (
        preempted || flags !== "wx" || typeof path !== "string" ||
        !path.startsWith(reclaimSentinel)
      ) return fd;
      preempted = true;
      ...
      if (!sentinelAbsentDuringInitialization) {
        ...
        try {
          ...
          ownerlessSentinelObserved = typeof value !== "object" ||
            value === null ||
            !("hostPid" in value) || typeof value.hostPid !== "number" ||
            !("token" in value) || typeof value.token !== "string";
        } catch (error) {
  ...
  test("#given a crashed reclaimer left its sentinel #when another process reclaims the stale holder #then the sentinel cannot wedge acquisition", () => {
    ...
    fs.writeFileSync(
      reclaimSentinel,
      JSON.stringify({ hostPid: 202, token: "crashed-reclaimer" }),
    );
    let entered = false;
  ...
  test("#given a contender probes every reclamation transition #when a stale lock is replaced #then only the reclaimer concludes it holds the lock", () => {
    ...
    spyOn(fs, "renameSync").mockImplementation((from, to) => {
      realRename(from, to);
      if (
        !probed && !reclaimerEntered &&
        (from === reclaimer.paths.runLock(runId) ||
          to === reclaimer.paths.runLock(runId))
      ) {
        probed = true;
        try {
          contender.withRunLock(runId, () => {
          ...
          contenderRejected = error instanceof Error &&
            error.message.includes("Timed out acquiring DAG lock");
        }
      }
    });
  ...
  test("#given a fresh holder acquires between stale inspection and removal #when the run lock retries #then the fresh lock survives and the reclaimer never enters", () => {
    ...
    spyOn(fs, "openSync").mockImplementation((path, flags, mode) => {
      const fd = realOpen(path, flags, mode);
      if (
        !replaced && typeof path === "string" && path.endsWith(".successor")
      ) {
        replaced = true;
    ...
    expect(acquire).toThrow(
      `Timed out acquiring DAG lock: ${store.paths.runLock(runId)}`,
    );
  ...
  test("#given a contender replaces the observed stale holder before atomic takeover #when validation runs #then the contender wins and the reclaimer never enters", () => {
    ...
    const fresh = JSON.stringify({
      hostPid: freshPid,
      owner: "fresh-before-quarantine",
    });
    const contender = JSON.stringify({
      hostPid: contenderPid,
      owner: "contender",
    });
    fs.writeFileSync(store.paths.runLock(runId), stale);
    ...
    spyOn(fs, "openSync").mockImplementation((path, flags, mode) => {
      const fd = realOpen(path, flags, mode);
      if (
        !successorPrepared && typeof path === "string" &&
        path.endsWith(".successor")
      ) {
  ...
  test("#given a dead holder is replaced before reclamation #when the run lock retries #then it never deletes the fresh holder", () => {
      ...
      { project_dir: project },
      {
        ...
        isProcessAlive: (pid) => {
          if (pid === stalePid && !replaced) {
            ...
            fs.writeFileSync(
              store.paths.runLock(runId),
              JSON.stringify({ hostPid: freshPid }),
            );
            return false;
          }
          return pid === freshPid;
    ...
    fs.writeFileSync(
      store.paths.runLock(runId),
      JSON.stringify({ hostPid: stalePid }),
    );
    let entered = false;
    ...
    expect(entered).toBe(false);
    expect(JSON.parse(fs.readFileSync(store.paths.runLock(runId), "utf8")))
      .toEqual({ hostPid: freshPid });
  });
  ...
  test("#given expired terminal artifacts and equally old live artifacts #when retention runs #then only the terminal run is pruned", () => {
    ...
    store.writeCheckpoint(
      runId,
      checkpoint({ status: "completed", completedAt: old, taskId }),
    );
    store.writeCheckpoint(
      otherRunId,
      checkpoint({ id: otherRunId, status: "running", completedAt: old }),
    );
    store.appendEvent(event(1));
    ...
    fs.writeFileSync(
      store.paths.runLock(runId),
      JSON.stringify({ hostPid: 1, runId }),
    );
    fs.writeFileSync(
      store.paths.keyLock("parent-session", "key-run-1"),
      JSON.stringify({ hostPid: 1, runId }),
    );
    fs.writeFileSync(
      store.paths.taskOwnerLock(taskId),
      JSON.stringify({ hostPid: 1, runId }),
    );
    ...
    expect(fs.existsSync(store.paths.runLock(runId))).toBe(false);
    expect(fs.existsSync(store.paths.keyLock("parent-session", "key-run-1")))
      .toBe(false);
  ...
  test("#given expired terminal and equally old live skill sidecars #when retention runs #then only the terminal sidecar is pruned", () => {
    ...
    store.writeCheckpoint(
      runId,
      checkpoint({ status: "completed", completedAt: old }),
    );
    store.writeCheckpoint(
      otherRunId,
      checkpoint({ id: otherRunId, status: "running", completedAt: old }),
