...
function skillContent(
  ...
  return {
    name,
    content: block.slice(prefix.length, block.length - suffix.length),
  };
}
...
function materializeNode(
  ...
  readonly node: DagNodeSkillMaterialization;
  readonly missing: readonly string[];
} {
 ...
 */
export function createDagSkillMaterializer(
  options: DagSkillMaterializerOptions,
): DagMaterializeSkills {
  const cwd = options.cwd;
  const loadSkills = options.loadSkills ?? createFsSkillLoader({
    ...(options.homeDir === undefined ? {} : { homeDir: options.homeDir }),
    ...(options.extraDirs === undefined
      ? {}
  ...
  return ({ runId, definition, at }) => {
    ...
    const manifest: DagSkillManifest = {
    ...
    return {
      nodes: nodes.map((node) => ({
        nodeId: node.nodeId,
        effectivePrompt: node.effectivePrompt,
      })),
      diagnostics,
    };
  };
...
export function readDagSkillManifest(
  store: DagFileStore,
  runId: DagRunId,
): DagSkillManifest | null {
  try {
    return JSON.parse(
      fs.readFileSync(manifestPath(store, runId), "utf8"),
    ) as DagSkillManifest;
  } catch (error) {
