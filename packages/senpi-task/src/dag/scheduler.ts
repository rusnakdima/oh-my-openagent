...
import {
  type DagExecutionModeSources,
  resolveDagNodeExecutionMode,
} from "./execution-mode";
...
import {
  createDagJournal,
  type DagJournal,
  type DagJournalListener,
} from "./journal";
import {
  applyDagRunMutation,
  type DagMaterializeSkills,
  type DagPersistedNode,
  type DagRunRecordV1,
} from "./manager";
import {
  type DagRetryOptions,
  type DagRunReentry,
  retryDagNodes,
} from "./node-retry";
import { type DagNodeSendResult, sendToDagNode } from "./node-send";
...
const schedulerObservers = new WeakMap<
  TaskManager,
  Set<DagSchedulerObserver>
>();

export function observeDagSchedulers(
  taskManager: TaskManager,
  observer: DagSchedulerObserver,
): () => void {
  const observers = schedulerObservers.get(taskManager) ??
    new Set<DagSchedulerObserver>();
...
export function createDagScheduler(options: DagSchedulerOptions): DagScheduler {
  ...
  const journal = createDagJournal<DagRunRecordV1>({
    ...
    ...(options.subscriberRing === undefined
      ? {}
      : { subscriberRing: options.subscriberRing }),
    now,
  });
  const context: SchedulerContext = {
    ...
    definitionNodes: new Map(
      options.initialRecord.definition.nodes.map((
        node,
      ) => [node.id as DagNodeId, node]),
    ),
    ...(options.executionMode === undefined
      ? {}
      : { executionMode: options.executionMode }),
    ...(options.ancestry === undefined ? {} : { ancestry: options.ancestry }),
    ...(options.nodeSpawnPolicy === undefined
      ? {}
      : { nodeSpawnPolicy: options.nodeSpawnPolicy }),
    now,
    pendingErrors,
  ...
  const scheduler: DagScheduler = {
    ...
    retryNode: (runId, nodeIds, retryOptions) =>
      retryDagNodes(options, runId, nodeIds, retryOptions),
    sendToNode: (runId, nodeId, message) =>
      sendToDagNode(
        options,
        runId,
        nodeId,
        message,
        (revivedNodeId, taskId) =>
          context.journal.snapshot().status === "running"
            ? watchRevivedInScheduler(context, revivedNodeId, taskId)
            : undefined,
      ),
  };
  for (const observer of schedulerObservers.get(options.taskManager) ?? []) {
    observer(scheduler);
  }
  return scheduler;
}
  ...
  switch (event.type) {
    ...
    case "dag.node.transitioned": {
      ...
      const replayedResult =
        terminalResult === undefined && terminalResults !== undefined &&
          event.to === "completed"
          ? replayDagNodeResult(
            terminalResults.store,
            record.runId,
            event.nodeId,
          )
          : undefined;
      const persisted =
        terminalResult === undefined || terminalResults === undefined
          ? replayedResult === undefined
            ? undefined
            : { kind: "persisted" as const, artifact: replayedResult.artifact }
          : persistDagNodeResult({
      ...
      return {
        ...
        nodes: record.nodes.map((node) => {
          ...
          return {
            ...
            ...(terminalResult?.record.run_stats === undefined &&
                replayedResult?.runStats === undefined
              ? {}
              : {
                runStats: terminalResult?.record.run_stats ??
                  replayedResult?.runStats,
              }),
          };
        }),
        diagnostics: persisted?.kind === "failed"
          ? [
            ...record.diagnostics,
            {
              ...
              ...(persisted.diagnostic.runId === undefined
                ? {}
                : { runId: persisted.diagnostic.runId }),
    ...
    case "dag.node.steered":
      if (event.delivery !== "revive") {
        return { ...record, updatedAt: event.at };
      }
      return {
        ...record,
        nodes: record.nodes.map((node) =>
          node.id === event.nodeId ? { ...node, taskId: event.taskId } : node
        ),
        updatedAt: event.at,
      };
...
async function cancelRun(
  ...
  if (snapshot.runId !== runId) {
    throw new Error(`scheduler does not own DAG run "${runId}"`);
  }
  if (
    snapshot.status === "completed" || snapshot.status === "failed" ||
    snapshot.status === "cancelled"
  ) return;
  if (context.cancellationOperation !== undefined) {
...
async function performCancellation(
  ...
  try {
    ...
    const cancellationResults = await Promise.allSettled(
      [...context.attachedTaskIds.values()].map((taskId) =>
...
async function cancelledSnapshot(
  context: SchedulerContext,
): Promise<DagRunRecordV1> {
...
function whenAdmissionIdle(context: SchedulerContext): Promise<void> {
  ...
  return new Promise<void>((resolve) =>
    context.admissionIdleWaiters.add(resolve)
  );
}
...
async function runWaves(context: SchedulerContext): Promise<DagRunRecordV1> {
  if (context.journal.snapshot().status === "pending") {
    context.journal.append(
      dagRunStartedEvent({ generation: context.journal.snapshot().generation }),
    );
  }
  ...
  for (;;) {
    for (const wave of context.journal.snapshot().waves) {
      ...
      const runnable = wave.nodeIds.filter((nodeId) =>
        isRunnable(context.journal.snapshot(), nodeId)
      );
      if (runnable.length === 0) continue;

      for (const nodeId of runnable) {
        transition(context, nodeId, "scheduled", { kind: "scheduled" });
      }
      context.journal.append(
        dagWaveStartedEvent({ waveIndex: wave.index, nodeIds: runnable }),
      );
      if (!await admitAndSettleWave(context, runnable)) {
        return cancelledSnapshot(context);
      }
      context.journal.append(
        dagWaveCompletedEvent({ waveIndex: wave.index, nodeIds: runnable }),
      );
    }

    applyDependentSkipCascade(context);
    const current = context.journal.snapshot();
    if (current.nodes.every((node) => TERMINAL_NODE_STATES.has(node.state))) {
      break;
    }
    if (context.attachedTasks.size === 0) {
      throw new Error(
        `DAG run "${current.runId}" cannot terminalize while nodes are active`,
      );
    }
  ...
  if (failed !== undefined) {
    const error = failed.error ??
      nodeError(failed.id, "start_failed", "DAG node failed", context.now);
    context.journal.append(
      dagRunFailedEvent({ error, counts: countNodes(snapshot.nodes) }),
    );
  } else {
    context.journal.append(
      dagRunCompletedEvent({ counts: countNodes(snapshot.nodes) }),
    );
  }
  return context.journal.snapshot();
}

async function admitAndSettleWave(
  context: SchedulerContext,
  nodeIds: readonly DagNodeId[],
): Promise<boolean> {
  ...
  while (awaitingAdmission.length > 0) {
    ...
    let results: PromiseSettledResult<
      { readonly nodeId: DagNodeId; readonly result: OwnedStartResult }
    >[];
    try {
      results = await Promise.allSettled(
        awaitingAdmission.map(async (nodeId) => ({
          nodeId,
          result: await context.taskManager.startOwned(
            startSpec(context, nodeId),
            owner(context, nodeId),
          ),
        })),
      );
    } finally {
    ...
    for (let index = 0; index < results.length; index += 1) {
      ...
      if (settled === undefined || nodeId === undefined) {
        throw new Error("DAG admission result lost its node");
      }
...
function attachStarted(
  ...
  context.journal.append(
    dagNodeTaskAttachedEvent({ nodeId, taskId: result.task_id, attempt }),
  );
  if (result.status === "pending") {
    ...
    transition(
      ...
      nodeId,
      "running",
      result.reused ? { kind: "resumed" } : { kind: "started" },
    );
  }
...
function attachTaskSettlement(
  ...
  const task: AttachedTask = {
    ...
    settled: context.taskManager.waitFor(taskId).then(
      ...
      (error: unknown): AttachedTaskSettlement => ({
        nodeId,
        kind: "error",
        error,
      }),
...
function foldTaskOutcome(
  ...
  if (task.status === "pending" || task.status === "running") {
    throw new Error(
      `TaskManager.waitFor returned nonterminal task ${task.task_id}`,
    );
  }
...
function applyDependentSkipCascade(context: SchedulerContext): void {
  ...
  while (changed) {
    ...
    for (const node of snapshot.nodes) {
      ...
      const dependencies = node.dependsOn.map((nodeId) =>
        nodeById(snapshot, nodeId)
      );
      if (
        dependencies.some((dependency) =>
          TERMINAL_NODE_STATES.has(dependency.state) &&
...
function isRunnable(record: DagRunRecordV1, nodeId: DagNodeId): boolean {
  ...
  return (node.state === "pending" || node.state === "blocked") &&
    node.dependsOn.every((dependencyId) =>
      nodeById(record, dependencyId).state === "completed"
    );
}
...
function transition(
  ...
  context.journal.append(
    dagNodeTransitionedEvent({ nodeId, from, to, reason }),
  );
}
...
function failNode(
  ...
  context.pendingErrors.set(
    nodeId,
    nodeError(nodeId, code, message, context.now),
  );
...
function startSpec(
  ...
  if (persisted === undefined) {
    throw new Error(`missing persisted definition for DAG node "${nodeId}"`);
  }
  const spec: ManagerStartSpec = {
    prompt: persisted.effectivePrompt,
    ...(persisted.task_summary === undefined
      ? {}
      : { task_summary: persisted.task_summary }),
    ...
    ...(node.route.kind === "category" ? { category: node.route.category } : {
      subagent_type: node.route.agent,
      ...(node.route.model === undefined ? {} : { model: node.route.model }),
    }),
    name: node.id,
    ...(persisted.description === undefined
      ? {}
  ...
  if (node.route.kind !== "category" && context.nodeSpawnPolicy !== undefined) {
    ...
    if (verdict.kind === "deny") {
      throw new Error(
        `DAG node "${nodeId}" denied by spawn policy: ${verdict.message}`,
      );
    }
...
function startFailure(
  result: Exclude<
    OwnedStartResult,
    { readonly kind: "started" | "residency_denied" }
  >,
): {
  ...
  switch (result.kind) {
    ...
    case "owner_conflict":
      return {
        code: "start_failed",
        message:
          `DAG task owner conflicts with existing task ${result.task_id}`,
      };
  }
}
  ...
  switch (status) {
    ...
    case "interrupted":
      return {
        ...
        message: message ?? "task was interrupted",
      };
    case "lost":
    ...
    case "cancelled":
      return {
        code: "task_cancelled",
        message: message ?? "task was cancelled",
      };
  }
}

function transitionedNode(
  node: DagNode,
  state: DagNodeState,
  at: string,
  error?: DagNodeError,
  ...
  return {
    ...base,
    state,
    ...(state === "running" && node.startedAt === undefined
      ? { startedAt: at }
      : {}),
