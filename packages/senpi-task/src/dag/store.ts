...
const TERMINAL_STATUSES = new Set<DagRunStatus>([
  "completed",
  "failed",
  "cancelled",
]);
...
export type DagFileStore = {
  ...
  readonly withKeyLock: <T>(
    parentSessionId: string,
    runKey: string,
    operation: () => T,
  ) => T;
  readonly withTaskOwnerLock: <T>(
    taskOwner: string,
    runId: DagRunId,
    operation: () => T,
  ) => T;
...
export function createDagFileStore(
  ...
  const maxRunsPerSession = config.task?.dag?.max_runs_per_session ??
    DAG_SETTINGS_DEFAULTS.max_runs_per_session;
  const retentionDays = config.task?.dag?.retention_days ??
    DAG_SETTINGS_DEFAULTS.retention_days;
  ...
  const store: DagFileStore = {
    ...
    appendEvent(event) {
      ...
      if (!Number.isSafeInteger(event.seq) || event.seq !== expectedSeq) {
        throw new Error(
          `DAG event seq must be strictly increasing: expected ${expectedSeq}, received ${event.seq}`,
        );
      }
    ...
    readEvents(runId, sinceSeqExclusive, readOptions) {
      ...
      if (!fs.existsSync(path)) {
        return {
          events: [],
          nextSinceSeq: sinceSeqExclusive,
          headSeq: 0,
          hasMore: false,
        };
      }
      inspectEventLog(path, runId, diagnosticLog, recoveredPaths, now);
      const wantedTypes = readOptions.types === undefined
        ? undefined
        : new Set(readOptions.types);
      ...
      forEachJsonLine(path, (value) => {
        ...
        if (
          readOptions.throughSeq !== undefined &&
          event.seq > readOptions.throughSeq
        ) return;
        if (readOptions.lane !== undefined && event.lane !== readOptions.lane) {
          return;
        }
    ...
    writeKey(record) {
      assertSupportedSchema(
        record,
        paths.key(record.parentSessionId, record.runKey),
        record.runId,
        now,
      );
    ...
    withKeyLock: (parentSessionId, runKey, operation) =>
      withLock(
        paths.keyLock(parentSessionId, runKey),
        undefined,
        operation,
        isProcessAlive,
        now,
    ...
    withTaskOwnerLock: (taskOwner, runId, operation) =>
      withLock(
        paths.taskOwnerLock(taskOwner),
        runId,
        operation,
        isProcessAlive,
        now,
    ...
    pruneExpired(pruneNow = now()) {
      ...
      for (const entry of fs.readdirSync(paths.runs, { withFileTypes: true })) {
        ...
        if (terminalAt === undefined || Date.parse(terminalAt) > cutoff) {
          continue;
        }
...
function createPaths(stateDir: string): DagStorePaths {
  ...
  return {
    ...
    results,
    locks,
    key: (parentSessionId, runKey) =>
      join(keys, `${dagKeyHash(parentSessionId, runKey)}.json`),
    ...
    keyLock: (parentSessionId, runKey) =>
      join(locks, `key-${dagKeyHash(parentSessionId, runKey)}.lock`),
    taskOwnerLock: (taskOwner) =>
      join(locks, `task-owner-${sha256(taskOwner)}.lock`),
  };
}
...
function writeCheckpointWithinSessionLimit(
  ...
  const capacityLock = join(
    paths.locks,
    `session-runs-${sha256(parentSessionId)}.lock`,
  );
  withLock(
    capacityLock,
    undefined,
    () => {
      ...
      for (const entry of fs.readdirSync(paths.runs, { withFileTypes: true })) {
        ...
        if (
          readOptionalString(existing, "parentSessionId") === parentSessionId
        ) runCount += 1;
      }
...
function fsyncParentDirectoryAfterRename(
  path: string,
  platform: NodeJS.Platform,
): void {
...
function inspectEventLog(
  ...
  try {
    ...
    if (
      error instanceof DagJournalCorruptError &&
      error.diagnostic.message.includes("schemaVersion")
    ) throw error;
...
function parseEvent(
  ...
  assertSupportedSchema(value, path, runId, now);
  if (
    !isRecord(value) || typeof value.seq !== "number" ||
...
function withLock<T>(
  ...
  for (;;) {
    ...
    if (
      ...
      const reclaimedHolder = reclaimObservedLock(
        path,
        observedHolder,
        isProcessAlive,
        runId,
      ...
      if (now() - startedAt >= LOCK_WAIT_TIMEOUT_MS) {
        throw new Error(`Timed out acquiring DAG lock: ${path}`);
      }
  ...
  if (acquiredHolder === undefined) {
    throw new Error(`Failed to acquire DAG lock: ${path}`);
  }
  try {
    return operation();
  } finally {
...
function readLockHolder(path: string): LockHolder | undefined {
  try {
    ...
    try {
      ...
      if (isRecord(value) && typeof value.hostPid === "number") {
        return { pid: value.hostPid, content };
      }
    } catch (error) {
...
function sameLockHolder(
  left: LockHolder | undefined,
  right: LockHolder | undefined,
): boolean {
...
function publishLockExclusively(
  path: string,
  content: string,
  fsyncWrites: boolean,
): boolean {
...
function reclaimObservedLock(
  ...
  const reclaimHolder = tryAcquireReclaimMutex(
    reclaimPath,
    isProcessAlive,
    now,
    fsyncWrites,
  ...
  try {
    ...
    if (
      ...
      !sameLockHolder(observedHolder, currentHolder) ||
...
function tryAcquireReclaimMutex(
  ...
  if (
    ...
    removeObservedLock(
      ...
      observedHolder,
      (movedHolder) =>
        movedHolder === undefined || movedHolder.pid === undefined ||
        !isProcessAlive(movedHolder.pid),
    );
  }
  return undefined;
}
...
function pruneRunArtifacts(
  ...
  for (const entry of fs.readdirSync(paths.keys, { withFileTypes: true })) {
    ...
    fs.rmSync(join(paths.locks, `key-${entry.name.slice(0, -5)}.lock`), {
      force: true,
    });
  }
  for (const node of checkpoint.nodes ?? []) {
    if (node.taskId !== undefined) {
      fs.rmSync(paths.taskOwnerLock(node.taskId), { force: true });
    }
  }
