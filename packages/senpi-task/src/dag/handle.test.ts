...
function applyEvent(
  checkpoint: DagRunRecordV1,
  event: DagRunEvent,
): DagRunRecordV1 {
  if (event.type === "dag.run.completed") {
    return { ...checkpoint, status: "completed", completedAt: event.at };
  }
  if (event.type === "dag.run.failed") {
    return { ...checkpoint, status: "failed", completedAt: event.at };
  }
  if (event.type === "dag.run.cancelled") {
    return { ...checkpoint, status: "cancelled", completedAt: event.at };
  }
  return checkpoint;
}
...
function fixture(
  nodes: readonly DagNode[],
  overrides: { readonly cancel?: (id: DagRunId, reason?: string) => void } = {},
): Fixture {
  ...
  return {
    ...
    settle(settledNodes, status, reason) {
      ...
      if (status === "completed") {
        journal.append(dagRunCompletedEvent({ counts: counts(settledNodes) }));
...
describe("createDagWaitSurface wait terminal projection", () => {
  test("#given a two node run #when both nodes complete #then wait resolves with each node output and run stats", async () => {
    // given
    const running = [
      node("plan", { state: "running", taskId: "task-plan" }),
      node("build", { state: "running", taskId: "task-build" }),
    ];
    ...
    harness.settle(
      [
        node("plan", {
          ...
          taskId: "task-plan",
          completedAt: at,
          runStats: { runtime_ms: 10, turns: 2, tool_calls: 1 },
        }),
        node("build", {
          state: "completed",
          taskId: "task-build",
  ...
  test("#given a failed node with a dependent #when the run ends failed #then wait resolves with the node error and the dependency skip", async () => {
    ...
    const harness = fixture([
      node("plan", { state: "running", taskId: "task-plan" }),
      node("ship", { state: "blocked", dependsOn: ["plan" as DagNodeId] }),
    ]);
  ...
  test("#given a run that is already terminal #when wait is called #then it resolves without any further journal event", async () => {
    ...
    harness.store.writeResult(runId, "plan", "plan output");
    harness.settle(
      [node("plan", { state: "completed", taskId: "task-plan" })],
      "completed",
    );
    const seqBefore =
      harness.store.readEvents(runId, 0, { limit: 100 }).headSeq;
    ...
    expect(harness.store.readEvents(runId, 0, { limit: 100 }).headSeq).toBe(
      seqBefore,
    );
  });
...
describe("createDagWaitSurface attach", () => {
  ...
  test("#given an attached handle #when cancel is called #then the injected cancellation runs for the owned run only", async () => {
    ...
    const harness = fixture([
      ...
      cancel: (id, reason) => {
        cancelled.push(
          reason === undefined ? { runId: id } : { runId: id, reason },
        );
      },
    });

    // when
    await harness.surface.attach(runId, parentSessionId).cancel(
      "user_requested",
    );

    // then
    expect(cancelled).toEqual([{ runId, reason: "user_requested" }]);
    expect(() => harness.surface.attach(runId, otherSessionId)).toThrow(
      DagWaitError,
    );
  });
});
...
describe("createDagWaitSurface detach safety", () => {
  test("#given a caller that abandons its wait promise #when the run later completes #then the run is untouched and a fresh attach still returns the result", async () => {
    ...
    expect(harness.store.readCheckpoint<DagRunRecordV1>(runId)?.status).toBe(
      "completed",
    );
    expect(later.nodes.plan).toEqual({
      state: "completed",
      taskId: "task-plan",
      output: "plan output",
    });
...
describe("createDagWaitSurface pre-dispatch rejections", () => {
  ...
  test("#given an unknown run id #when waited or attached #then run_not_found is raised before any subscription", async () => {
    ...
    expect(() => harness.surface.attach(missing, parentSessionId)).toThrow(
      DagWaitError,
    );
  });
  ...
  test("#given malformed wait params #when waited #then invalid_arguments is raised and no run is touched", async () => {
    ...
    await expect(blankSession).rejects.toMatchObject({
