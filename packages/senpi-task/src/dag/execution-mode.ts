...
export type DagExecutionModeSources = {
  ...
  readonly config: {
    readonly task?: { readonly default_execution_mode?: ExecutionMode };
  };
};
...
export function resolveDagNodeExecutionMode(
  sources: DagExecutionModeSources,
): ExecutionMode {
  const agentName = sources.route.kind === "agent"
    ? sources.route.agent
    : undefined;
  const definition = agentName === undefined
    ? undefined
    : sources.agents[agentName];
  const agentMode =
    agentName !== undefined && CURATED_READONLY_AGENT_NAMES.has(agentName)
      ? "in-process" as const
