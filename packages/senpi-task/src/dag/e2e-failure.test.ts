...
import { type ChildSession, InProcessRunner } from "../runners/in-process";
...
type ScriptedOutcome =
  ...
  | {
    ...
    readonly failure: {
      readonly kind: "child-prompt-failed";
...
class ControlledRunner implements ManagedRunner {
  ...
  settle(name: string, outcome: ScriptedOutcome): void {
    ...
    if (controlled === undefined) {
      throw new Error(`missing controlled child ${name}`);
    }
    this.#onSettle?.();
...
class ImmediateRunner implements ManagedRunner {
  ...
  start(spec: ManagedStartSpec): Promise<ManagedChildHandle> {
    ...
    return Promise.resolve({
      ...
      subscribe: () => () => undefined,
      waitForOutcome: () =>
        Promise.resolve({ status: "completed", finalResponse: output }),
...
function resultPersistingManager(
  manager: TaskManager,
  store: DagFileStore,
): TaskManager {
      ...
      return async (
        ...
        if (owner?.kind === "dag" && record.status === "completed") {
          ...
          if (persisted.kind === "failed") {
            throw new Error(persisted.diagnostic.message);
          }
        }
        return record;
      };
...
type E2eFixture = {
  ...
  readonly whenEvent: (
    runId: DagRunId,
    predicate: (event: DagRunEvent) => boolean,
  ) => Promise<void>;
...
function e2eFixture(options: E2eFixtureOptions = {}): E2eFixture {
  ...
  const taskManager = resultPersistingManager(
    createTaskManager({
      store: taskStore,
      runners: {
        "in-process": runner,
        process: options.processRunner ?? runner,
      },
  ...
  const manager = createDagManager({
    ...
    ...(options.materializeSkills === undefined
      ? {}
      : { materializeSkills: options.materializeSkills }),
  });
  ...
  const waitSurface = createDagWaitSurface({
    ...
    subscribe: (runId, listener) => {
      ...
      if (scheduler === undefined) {
        throw new Error(`missing scheduler for ${runId}`);
      }
  ...
  return {
    ...
    running(runId) {
      ...
      if (running === undefined) {
        throw new Error(`missing run promise for ${runId}`);
      }
      return running;
    },
    events: (runId) =>
      manager.history({ runId, parentSessionId, limit: 256 }).events,
    ...
    retry(runId, nodeIds) {
      ...
      const reentry = nodeIds === undefined
        ? controls.retryNode(runId)
        : controls.retryNode(runId, nodeIds);
    ...
    send: (runId, nodeId, message) =>
      nodeControls(runId).sendToNode(runId, nodeId, message),
  };
...
const identityMaterializer: ReturnType<typeof createDagSkillMaterializer> = (
  { definition: input },
) => ({
  nodes: input.nodes.map((node) => ({
    nodeId: node.id,
    effectivePrompt: node.prompt,
  })),
});
...
function extensionValues(args: readonly string[]): readonly string[] {
  ...
  for (let index = 0; index < args.length; index += 1) {
    if (args[index] === "--extension" && args[index + 1] !== undefined) {
...
describe("DAG failure, crash, and policy end to end", () => {
  test(
    "#given two failures whose completion order opposes graph order #when the real engine runs #then siblings and independent branches finish, dependents skip, and graph order selects the primary failure",
    async () => {
      // given
      ...
      runner.settle(
        "wall-clock-first-failure",
        failed("wall-clock-first-failure"),
      );
      ...
      runner.settle(
        ...
        completed("independent-dependent"),
      );
      ...
      expect(
        Object.fromEntries(
          Object.entries(result.nodes).map(([id, node]) => [id, node.state]),
        ),
      ).toEqual({
      ...
      const terminal = fixture.events(result.runId).find((event) =>
        event.type === "dag.run.failed"
      );
      expect(terminal).toMatchObject({
        type: "dag.run.failed",
        error: {
          nodeId: "graph-first-failure",
          message: "failure:graph-first-failure",
        },
      });
    },
    { timeout: 15_000 },
  ...
  test("#given a crash after an owned task spawns but before task attachment #when a fresh manager resumes #then startOwned recovers the existing task and spawn count remains exactly one", async () => {
    ...
    const input = definition("spawn-before-attach", [
      categoryNode("recover-me"),
    ]);
    ...
    const recoveredTaskManager = resultPersistingManager(
      createTaskManager({
    ...
    expect(outcomes[0]?.record?.nodes[0]).toMatchObject({
      state: "completed",
      taskId: owned.task_id,
    });
    expect(firstRunner.startedSpecs).toHaveLength(1);
    expect(recoveredRunner.startedSpecs).toHaveLength(0);
    expect(
      firstRunner.startedSpecs.length + recoveredRunner.startedSpecs.length,
    ).toBe(1);
  });
  ...
  test("#given an existing run key #when its prompt changes #then definition_conflict leaves the original checkpoint, events, and task count untouched", async () => {
    ...
    const checkpointBefore = fs.readFileSync(
      fixture.store.paths.run(first.snapshot.runId),
      "utf8",
    );
    const eventsBefore = fs.readFileSync(
      fixture.store.paths.event(first.snapshot.runId),
      "utf8",
    );
    const tasksBefore = fixture.taskStore.list().records.length;

    // when
    const conflict = fixture.start(
      definition("conflict", [
        categoryNode("only", [], { prompt: "changed prompt" }),
      ]),
    );
    ...
    expect(
      fs.readFileSync(fixture.store.paths.run(first.snapshot.runId), "utf8"),
    ).toBe(checkpointBefore);
    expect(
      fs.readFileSync(fixture.store.paths.event(first.snapshot.runId), "utf8"),
    ).toBe(eventsBefore);
  ...
  test("#given spawn-capable parent tools #when an in-process DAG child resolves its real session options #then task, task_*, team_*, and dag are absent", async () => {
    ...
    const fixture = e2eFixture({
      runner: createInProcessManagedRunner(inProcess),
    });

    // when
    const started = await fixture.start(
      definition("in-process-policy", [categoryNode("policy")]),
    );
    ...
    expect(
      names.some((name) =>
        name.startsWith("task_") || name.startsWith("team_")
      ),
    ).toBe(false);
  });
  ...
  test("#given inherited rpc extensions #when DAG and non-DAG children spawn #then only the DAG child omits the leading omo-senpi entry", async () => {
    ...
    const fixture = e2eFixture({
      processRunner: rpcRunner,
      childPlanner: processPlanner,
    });
    const dag = await fixture.start(
      definition("rpc-policy", [categoryNode("dag-rpc")]),
    );
    ...
    if (ordinary.kind !== "started") {
      throw new Error("ordinary rpc child did not start");
    }
    await fixture.taskManager.waitFor(ordinary.task_id);

    // then
    expect(spawnedArgs).toHaveLength(2);
    expect(extensionValues(spawnedArgs[0]?.args ?? [])).toEqual([
      "/extensions/provider.js",
    ]);
    expect(extensionValues(spawnedArgs[1]?.args ?? [])).toEqual(
      inheritedExtensions,
    );
  });
  ...
  test("#given a corrupt trailing JSONL fragment #when the store reopens #then the tail is discarded, diagnosed, and earlier events remain authoritative", async () => {
    ...
    const started = await fixture.start(
      definition("event-tail", [categoryNode("done")]),
    );
    await fixture.wait(started.snapshot.runId);
    const authoritative = fixture.events(started.snapshot.runId);
    fs.appendFileSync(
      fixture.store.paths.event(started.snapshot.runId),
      '{"schemaVersion":1,"seq":999',
    );

    // when
    const reopened = createDagFileStore({ project_dir: project });
    const recovered = reopened.readEvents(started.snapshot.runId, 0, {
      limit: 256,
    });
    ...
    expect(
      fs.readFileSync(reopened.paths.event(started.snapshot.runId), "utf8"),
    ).toEndWith("\n");
  });
...
`,
    ...
    const child = Bun.spawn(
      [process.execPath, script, project, String(stopAt)],
      {
        stdin: "pipe",
        stdout: "pipe",
        stderr: "pipe",
    ...
    const marker = JSON.parse(await readLine(child.stdout)) as {
    ...
    await child.exited;
    const reopened = createDagFileStore({ project_dir: project });
    const beforeReplay = reopened.readCheckpoint<
      { readonly checkpointSeq: number; readonly applied: readonly number[] }
    >(runId as DagRunId);
    const journal = createDagJournal({
      ...
      applyEvent: (checkpoint, event) => ({
        ...checkpoint,
        applied: [...checkpoint.applied, event.seq],
      }),
    });
    ...
    expect(beforeReplay?.applied).toEqual(
      Array.from({ length: stopAt - 1 }, (_, index) => index + 1),
    );
    expect(journal.snapshot().checkpointSeq).toBe(stopAt);
    expect(journal.snapshot().applied).toEqual(
      Array.from({ length: stopAt }, (_, index) => index + 1),
    );
    expect(reopened.readEvents(runId as DagRunId, 0, { limit: 32 }).events)
      .toHaveLength(stopAt);
    expect(
      fs.readdirSync(reopened.paths.runs).some((name) => name.endsWith(".tmp")),
    ).toBe(false);
  });

  test(
    "#given a wave wider than residency_max_children #when admission frees slots in batches #then every node completes without a residency failure",
    async () => {
      // given
      const cap = 2;
      let residents = 0;
      ...
      const input = definition(
        "batched-wave",
        Array.from({ length: 7 }, (_, index) => categoryNode(`wide-${index}`)),
      );
      ...
      for (
        ...
        completedCount < input.nodes.length;
        completedCount += 1
      ) {
        await runner.whenStarted(completedCount + 1);
        runner.settle(
          `wide-${completedCount}`,
          completed(`wide-${completedCount}`),
        );
      }
      const result = await fixture.wait(started.snapshot.runId);
      ...
      expect(result.snapshot.counts).toEqual(
        expect.objectContaining({ total: 7, completed: 7, failed: 0 }),
      );
      expect(peakResidents).toBe(cap);
      expect(runner.startedSpecs).toHaveLength(7);
      expect(
        fixture.events(result.runId).filter((event) =>
...
describe("DAG retry, amend, and revive end to end", () => {
  test("#given a failed node in a completed-upstream run #when retried #then the run completes and every upstream task id is reused untouched", async () => {
    ...
    const upstreamTaskId = failedResult.snapshot.nodes.find((entry) =>
      entry.id === "upstream"
    )?.taskId;
    ...
    expect(result.status).toBe("completed");
    expect(result.snapshot.nodes.map((entry) => `${entry.id}:${entry.state}`))
      .toEqual([
        "upstream:completed",
        "flaky:completed",
    ...
    expect(
      result.snapshot.nodes.find((entry) => entry.id === "upstream")?.taskId,
    ).toBe(upstreamTaskId);
    expect(runner.startedSpecs.filter((spec) => spec.prompt === "do upstream"))
      .toHaveLength(1);
    expect(runner.startedSpecs.filter((spec) => spec.prompt === "do flaky"))
      .toHaveLength(2);
    expect(
      result.snapshot.nodes.find((entry) => entry.id === "flaky")?.execAttempt,
    ).toBe(1);
    expect(
      fixture.events(runId).some((event) => event.type === "dag.node.retried"),
    ).toBe(true);
    expect(
      fixture.events(runId).some((event) => event.type === "dag.run.resumed"),
    ).toBe(true);
    expect(fs.readFileSync(fixture.store.paths.result(runId, "flaky"), "utf8"))
      .toBe("output:flaky");
  });
  ...
  test("#given a node that fails twice #when retried twice #then each attempt owns a distinct child and the last one settles the run", async () => {
    ...
    const fixture = e2eFixture({ runner });
    const started = await fixture.start(
      definition("retry-twice", [categoryNode("stubborn")]),
    );
    const runId = started.snapshot.runId;
    await runner.whenStarted(1);
    ...
    expect(
      fs.readFileSync(fixture.store.paths.result(runId, "stubborn"), "utf8"),
    ).toBe("output:stubborn");
  });
  ...
  test("#given a node failed by the residency cap #when retried after the storm clears #then it runs and the run completes", async () => {
    ...
    const runner = new ControlledRunner();
    const admit: AdmitResident = () =>
      capOpen ? Promise.resolve({ kind: "admitted" }) : Promise.resolve({
        kind: "rejected",
        message: "resident child cap reached",
      });
    const fixture = e2eFixture({ runner, admit });
    const started = await fixture.start(
      definition("residency-rescue", [categoryNode("stormy")]),
    );
    const runId = started.snapshot.runId;
    ...
    expect(stormed.nodes.stormy).toMatchObject({
      state: "failed",
      error: { code: "residency_denied" },
    });
    expect(record.status).toBe("completed");
  ...
  test("#given a failed node #when its prompt is amended #then only it and its dependents re-run on re-entry", async () => {
    ...
    const fixture = e2eFixture({
      runner,
      materializeSkills: identityMaterializer,
    });
    ...
    await fixture.wait(runId);
    const stableTaskId = fixture.manager.record(runId, parentSessionId).nodes
      .find((entry) => entry.id === "stable")?.taskId;
    ...
    expect(record.nodes.find((entry) => entry.id === "stable")?.taskId).toBe(
      stableTaskId,
    );
  ...
  test("#given a settled run #when an amendment adds a node #then the added node executes on re-entry", async () => {
    ...
    // when
    const grown = definition("amend-add-node", [
      categoryNode("first"),
      categoryNode("second", ["first"]),
    ]);
    ...
    expect(fs.readFileSync(fixture.store.paths.result(runId, "second"), "utf8"))
      .toBe("output:second");
  });
  ...
  test("#given a live wave with one failed node revived before its sibling settles #when the sibling completes #then the run awaits the revival and schedules their dependent", async () => {
    ...
    const runId = started.snapshot.runId;
    await runner.whenStarted(2);
    const bFailed = fixture.whenEvent(
      runId,
      (event) =>
    ...
    const sent = await fixture.send(
      runId,
      "B" as DagNodeId,
      "continue after the transient abort",
    );

    // when - A is the last settlement still known to the original wave await set.
    const aCompleted = fixture.whenEvent(
      runId,
      (event) =>
        event.type === "dag.node.transitioned" && event.nodeId === "A" &&
        event.to === "completed",
    );
    ...
    expect(fixture.manager.record(runId, parentSessionId).status).toBe(
      "running",
    );
    expect(
      fixture.events(runId).some((event) =>
        event.type === "dag.run.completed" || event.type === "dag.run.failed"
      ),
    ).toBe(false);

    const bCompleted = fixture.whenEvent(
      runId,
      (event) =>
        event.type === "dag.node.transitioned" && event.nodeId === "B" &&
        event.to === "completed",
  ...
  test("#given a failed node whose child is still resident #when send revives it #then the child continues in place and the node reaches completed once", async () => {
    ...
    const fixture = e2eFixture({ runner });
    const started = await fixture.start(
      definition("revive-resident", [categoryNode("stuck")]),
    );
    const runId = started.snapshot.runId;
    await runner.whenStarted(1);
    ...
    expect(
      fixture.events(runId).filter((event) =>
        event.type === "dag.node.transitioned" && event.nodeId === "stuck" &&
        event.to === "completed"
      ),
    ).toHaveLength(1);
    ...
    expect(fs.readFileSync(fixture.store.paths.result(runId, "stuck"), "utf8"))
      .toBe("output:stuck-revived");
  });
});
