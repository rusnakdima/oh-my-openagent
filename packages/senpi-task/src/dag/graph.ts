...
function compareSequences(
  a: readonly DagNodeId[],
  b: readonly DagNodeId[],
): number {
...
function failure(
  errors: readonly DagCompileError[],
  at: string,
): DagCompileResult {
...
function findCycles(
  ids: readonly DagNodeId[],
  dependencies: ReadonlyMap<DagNodeId, readonly DagNodeId[]>,
): DagNodeId[][] {
  ...
  for (const start of [...remaining].sort(byId)) {
    ...
    let current = start;
    for (;;) {
      const next = (dependencies.get(current) ?? []).filter((dep) =>
        remaining.has(dep)
      ).sort(byId)[0];
      if (next === undefined) {
        break;
      ...
      if (seenAt !== undefined) {
        ...
        const ordered = [
          members[0] as DagNodeId,
          ...members.slice(1).reverse(),
        ];
...
export function compileDag(
  ...
  if (definition.nodes.length > settings.max_nodes_per_run) {
    errors.push({
      ...
      message:
        `dag has ${definition.nodes.length} nodes, exceeding max_nodes_per_run ${settings.max_nodes_per_run}`,
      nodeIds: [],
    });
  }
  ...
  for (const input of definition.nodes) {
    ...
    if (bytes > settings.max_prompt_bytes) {
      errors.push({
        ...
        message:
          `node "${id}" prompt is ${bytes} bytes, exceeding max_prompt_bytes ${settings.max_prompt_bytes}`,
        nodeIds: [id],
      });
    }
    ...
    if (deps.length > settings.max_nodes_per_run) {
      errors.push({
        ...
        message:
          `node "${id}" depends on ${deps.length} nodes, exceeding max_nodes_per_run ${settings.max_nodes_per_run}`,
        nodeIds: [id],
      });
    }
    for (const dep of deps) {
      if (dep === input.id) {
        errors.push({
          ...
          message: `node "${id}" depends on itself`,
          nodeIds: [id],
        });
        continue;
      }
  ...
  const uniqueInputs = definition.nodes.filter((input) =>
    declarationIndex.get(input.id as DagNodeId) !== undefined
  );
  const ids = uniqueInputs.map((input) => input.id as DagNodeId);
  const dependencies = new Map<DagNodeId, readonly DagNodeId[]>();
  for (const input of uniqueInputs) {
    const id = input.id as DagNodeId;
    const deps = (input.dependsOn ?? []).filter((dep) =>
      dep !== input.id && seenIds.has(dep)
    ) as DagNodeId[];
  ...
  edges.sort((a, b) => {
    const byTo = (declarationIndex.get(a.to) ?? 0) -
      (declarationIndex.get(b.to) ?? 0);
    return byTo !== 0 ? byTo : (declarationIndex.get(a.from) ?? 0) -
      (declarationIndex.get(b.from) ?? 0);
  });
  ...
  const resolveDepth = (id: DagNodeId): number => {
    ...
    const value = deps.length === 0
      ? 0
      : 1 + Math.max(...deps.map(resolveDepth));
    depth.set(id, value);
    return value;
  };
  for (const id of ids) {
  ...
  const waveIndexes = [...new Set(ids.map((id) => depth.get(id) ?? 0))].sort((
    a,
    b,
  ) => a - b);
  const waves: DagWave[] = waveIndexes.map((index) => ({
    ...
    nodeIds: ids
      ...
      .sort((a, b) => {
        const declared = (declarationIndex.get(a) ?? 0) -
