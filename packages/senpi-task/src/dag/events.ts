...
export function dagRunResumedEvent(
  input: { generation: number },
): DagRunEventPayload {
  return { type: "dag.run.resumed", generation: input.generation };
}

export function dagRunCompletedEvent(
  input: { counts: DagNodeCounts },
): DagRunEventPayload {
