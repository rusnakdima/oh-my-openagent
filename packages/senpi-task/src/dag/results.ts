 ...
 */
export function persistDagNodeResult(
  input: DagNodeResultPersistInput,
  ...
  try {
    ...
    return {
      ...
      diagnostic: {
        ...
        runId: input.runId,
        path: outputPath,
        message: `failed to persist dag node result for "${input.nodeId}": ${
          errorMessage(error)
        }`,
 ...
 */
export function readDagNodeResult(
  input: DagNodeResultReadInput,
): DagNodeResultRead | null {
  const outputPath = input.store.paths.result(input.runId, input.nodeId);
...
function writeStatsSidecar(
  input: DagNodeResultPersistInput,
  path: string,
): DagResultArtifactRef | undefined {
