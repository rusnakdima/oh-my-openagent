...
const RETRYABLE_NODE_STATES: ReadonlySet<DagNodeState> = new Set([
  "failed",
  "cancelled",
  "skipped",
]);
...
export function reenterDagRun(options: DagSchedulerOptions): DagRunReentry {
  const current = readRecord(options.store, options.initialRecord.runId) ??
...
export function retryDagNodes(
  ...
  if (record.status === "running" || record.status === "pending") {
    throw new DagNodeControlError({
      ...
      message:
        `dag run "${runId}" is still active; wait for it to settle before retrying a node`,
      runId,
    });
  }
  if (
    retryOptions?.prompt !== undefined &&
  ...
  const targets = nodeIds === undefined
    ? defaultRetryTargets(record)
    : [...nodeIds];
  if (targets.length === 0) {
    throw new DagNodeControlError({
...
function defaultRetryTargets(record: DagRunRecordV1): DagNodeId[] {
  ...
  return [
    ...seeds,
    ...skippedDependentsOf(record, seeds).map((node) => node.id),
  ];
}
...
function skippedDependentsOf(
  ...
  while (changed) {
    ...
    for (const node of record.nodes) {
      ...
      if (!node.dependsOn.some((dependencyId) => selected.has(dependencyId))) {
        continue;
      }
...
function assertRetryable(
  ...
  for (const nodeId of targets) {
    ...
    if (node === undefined) {
      throw new DagNodeControlError({
        code: "node_not_found",
        message: `unknown dag node "${nodeId}"`,
        runId,
        nodeIds: [nodeId],
      });
    }
    if (node.state === "completed") {
      throw new DagNodeControlError({
        code: "node_not_retryable",
        message:
          `dag node "${nodeId}" already completed; amend the definition to re-run it`,
        runId,
        nodeIds: [nodeId],
      });
    }
    if (!RETRYABLE_NODE_STATES.has(node.state)) {
      throw new DagNodeControlError({
        code: "node_not_retryable",
        message:
          `dag node "${nodeId}" is ${node.state}; only failed, cancelled, or skipped nodes can be retried`,
        runId,
        nodeIds: [nodeId],
      });
    }
    ...
    const blocking = blockingAncestors(record, node).filter((ancestorId) =>
      !selected.has(ancestorId)
    );
    if (blocking.length > 0) {
      throw new DagNodeControlError({
        code: "node_not_retryable",
        message: `dag node "${nodeId}" was skipped because ${
          blocking.join(", ")
        } did not complete; retry those in the same call`,
        runId,
...
function blockingAncestors(
  ...
  while (queue.length > 0) {
    ...
    if (ancestor.state === "failed" || ancestor.state === "cancelled") {
      blocking.add(id);
    }
...
function amendRetryPrompt(
  ...
  const definition: DagDefinition = {
    ...
    nodes: record.definition.nodes.map((node) =>
      definitionNode(node, node.id === nodeId ? prompt : node.prompt)
    ),
  };
  ...
  if (
    applied === null ||
    applied.definitionFingerprint === record.definitionFingerprint
  ) {
    throw new DagNodeControlError({
      code: "invalid_arguments",
...
function definitionNode(node: DagNodeInput, prompt: string): DagNodeInput {
  const { effectivePrompt: _effectivePrompt, ...input } = node as
    & DagNodeInput
