...
export type DagRecovery = {
  readonly pauseRunsForShutdown: (
    parentSessionId: string,
  ) => readonly DagRunId[];
  readonly resumePausedRuns: (
    parentSessionId: string,
  ) => Promise<readonly DagRecoveryOutcome[]>;
};

type RecoveryContext =
  & Required<Pick<DagRecoveryOptions, "store" | "taskManager">>
  & {
    readonly hostPid: number;
    readonly isProcessAlive: (pid: number) => boolean;
    readonly now: () => number;
...
export function createDagRecovery(options: DagRecoveryOptions): DagRecovery {
  const context: RecoveryContext = {
    ...
    ...(options.stopAdmission === undefined
      ? {}
      : { stopAdmission: options.stopAdmission }),
    ...(options.reattach === undefined ? {} : { reattach: options.reattach }),
  };

  return {
    pauseRunsForShutdown: (parentSessionId) =>
      pauseRunsForShutdown(context, parentSessionId),
    resumePausedRuns: (parentSessionId) =>
      resumePausedRuns(context, parentSessionId),
  };
}

function pauseRunsForShutdown(
  ...
  for (const observed of listRunRecords(context.store)) {
    if (
      ...
      !LIVE_RUN_STATUSES.has(observed.status)
    ) continue;
    ...
    context.store.withRunLock(observed.runId, () => {
      const fresh = context.store.readCheckpoint<RecoverableRecord>(
        observed.runId,
      );
      ...
      context.store.writeCheckpoint(observed.runId, {
        ...released,
        previousLeaseHolderPid,
      });
    });
...
function claimPausedRun(
  ...
  return context.store.withRunLock(runId, () => {
    ...
    if (fresh === null || fresh.status !== "paused") {
      return { kind: "skipped", reason: "not_paused" };
    }
...
async function resumeClaimedRun(
  context: RecoveryContext,
  claimed: RecoverableRecord,
): Promise<DagRecoveryOutcome> {
  const pendingErrors = new Map<DagNodeId, DagNodeError>();
  const pendingTerminalResults = new Map<
    DagNodeId,
    RecoveryPendingTerminalResult
  >();
  ...
  try {
    ...
    const scheduler = createDagScheduler({
      ...
      ...(context.subscriberRing === undefined
        ? {}
        : { subscriberRing: context.subscriberRing }),
      ...(context.nodeSpawnPolicy === undefined
        ? {}
        : { nodeSpawnPolicy: context.nodeSpawnPolicy }),
      now: context.now,
    });
...
async function reconcileNodes(
  ...
  for (const observed of journal.snapshot().nodes) {
    if (observed.state === "completed") {
      const result = readDagNodeResult({
        store: context.store,
        runId: journal.snapshot().runId,
        nodeId: observed.id,
      });
      if (result !== null) {
    ...
    if (
      observed.state === "failed" || observed.state === "cancelled" ||
      observed.state === "skipped"
    ) continue;
    if (observed.state !== "scheduled" && observed.state !== "running") {
      continue;
    }

    const owned = context.taskManager.findOwnedTask(
      ownerKey(journal.snapshot(), observed.id),
    );
    let task = observed.taskId === undefined
      ? owned
      : context.taskManager.get(observed.taskId) ?? owned;
    if (task !== undefined && observed.taskId !== task.task_id) {
      attachTask(journal, observed.id, task.task_id);
    }

    if (
      task === undefined && observed.state === "scheduled" &&
      observed.taskId === undefined
    ) {
    ...
    if (task === undefined) {
      ...
      const transcript = taskId === undefined
        ? false
        : hasTranscript(context.store.stateDir, taskId);
      failNode(
        journal,
        observed.id,
        "resume_task_missing",
        transcript
          ...
          : `task ${
            taskId ?? "unknown"
          } and all recovery artifacts are missing`,
        context.now,
        pendingErrors,
      );
      continue;
...
function applyRecoveryEvent(
  ...
  terminalResults: {
    readonly store: DagFileStore;
    readonly pendingTerminalResults: Map<
      DagNodeId,
      RecoveryPendingTerminalResult
    >;
    readonly now: () => number;
  ...
  if (event.type === "dag.run.paused") {
    return { ...record, status: "paused", updatedAt: event.at };
  }
...
function attachTask(
  ...
  journal.append(
    dagNodeTaskAttachedEvent({ nodeId, taskId, attempt: node.attempt + 1 }),
  );
}
...
function releaseLease(context: RecoveryContext, runId: DagRunId): void {
  context.store.withRunLock(runId, () => {
    ...
    const {
      leaseHolderPid: _leaseHolderPid,
      previousLeaseHolderPid: _previousLeaseHolderPid,
      ...released
    } = fresh;
...
function listRunRecords(store: DagFileStore): readonly RecoverableRecord[] {
  return fs.readdirSync(store.paths.runs, { withFileTypes: true })
    ...
    .map((entry) =>
      store.readCheckpoint<RecoverableRecord>(
        entry.name.slice(0, -5) as DagRunId,
      )
    )
...
function hasTranscript(stateDir: string, taskId: string): boolean {
  ...
  try {
    return fs.readdirSync(childDir, { recursive: true }).some((entry) =>
