...
export async function sendToDagNode(
  ...
  if (taskId === undefined) {
    throw new DagNodeControlError({
      ...
      message:
        `dag node "${nodeId}" never started a child; retry the run to dispatch it`,
      runId,
      nodeIds: [nodeId],
    });
  }
  if (options.taskManager.get(taskId) === undefined) {
    throw new DagNodeControlError({
      code: "node_not_found",
      message:
        `dag node "${nodeId}" points at task ${taskId}, which no longer exists`,
      runId,
      nodeIds: [nodeId],
    });
  }
  ...
  switch (outcome.kind) {
    case "queued":
      ...
      return {
        ...
        taskId,
        delivery: "queued",
        queuePosition: outcome.queue_position,
      };
    case "steered":
      controlJournal(options, record).append(
        dagNodeSteeredEvent({ nodeId, taskId, delivery: "steer" }),
      );
      return { nodeId, taskId, delivery: "steer" };
    case "revived": {
      const journal = controlJournal(options, record);
      journal.append(
        dagNodeSteeredEvent({ nodeId, taskId, delivery: "revive" }),
      );
      journal.append(
      ...
      return {
        ...
        taskId,
        delivery: "revive",
        settled: watchRevived?.(nodeId, taskId) ??
          watchRevivedTask(options, nodeId, taskId),
      };
    }
    case "one_shot_agent":
      throw new DagNodeControlError({
        ...
        message:
          `dag node "${nodeId}" runs one-shot agent ${outcome.agent}: ${outcome.message} Retry the node instead.`,
        runId,
        nodeIds: [nodeId],
      });
    case "scope_denied":
      throw new DagNodeControlError({
        code: "node_not_continuable",
        message:
          `dag node "${nodeId}" cannot be messaged: ${outcome.reason} Retry the node instead.`,
        runId,
        nodeIds: [nodeId],
      });
    case "not_continuable":
      throw new DagNodeControlError({
        code: "node_not_continuable",
        message:
          `dag node "${nodeId}" cannot be continued: ${outcome.reason} Retry the node to run it again.`,
        runId,
        nodeIds: [nodeId],
      });
    case "not_found":
...
async function watchRevivedTask(
  ...
  if (task.status === "pending" || task.status === "running") {
    failRevivedNode(
      ...
      nodeId,
      "task_error",
      `revived task ${taskId} settled nonterminal`,
      now,
    );
    return;
  }
