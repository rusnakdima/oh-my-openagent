...
function node(
  ...
  overrides: Partial<DagNodeInput> = {},
): DagNodeInput {
  return {
    id,
    prompt: `do ${id}`,
...
describe("compileDag structure", () => {
  ...
  it("#given a downstream node #when compiled #then the prompt is never templated with upstream ids", () => {
    ...
    const templated = definition([node("plan"), {
      id: "build",
      prompt: "use {{plan}}",
      category: "quick",
      dependsOn: ["plan"],
...
describe("compileDag validation", () => {
  ...
  it("#given an unknown dependency #when compiled #then rejected naming the missing id", () => {
    const result = compileDag(definition([node("a"), node("b", ["ghost"])]), {
      at: AT,
    });
  ...
  it("#given a cyclic 3-node graph #when compiled #then rejected listing the exact cycle members deterministically", () => {
    ...
    expect(result.errors[0]?.nodeIds).toEqual([
      nodeId("a"),
      nodeId("b"),
      nodeId("c"),
      nodeId("a"),
  ...
  it("#given the same cyclic graph declared in a different order #when compiled #then the cycle listing is identical", () => {
    const first = compileDag(
      definition([node("c", ["b"]), node("b", ["a"]), node("a", ["c"])]),
      { at: AT },
    );
    const second = compileDag(
      definition([node("a", ["c"]), node("c", ["b"]), node("b", ["a"])]),
      { at: AT },
    );
...
describe("compileDag size bounds", () => {
  it("#given 65 nodes #when compiled with default settings #then rejected on max_nodes_per_run", () => {
    const nodes = Array.from(
      { length: 65 },
      (_, index) => node(`n${String(index).padStart(3, "0")}`),
    );
  ...
  it("#given a multibyte prompt #when measured #then bytes are counted, not code units", () => {
    ...
    const multibyte = definition([
      node("a", [], { prompt: "é".repeat(131073) }),
    ]);

    expect(compileDag(multibyte, { at: AT }).ok).toBe(false);
    expect(
      compileDag(definition([node("a", [], { prompt: "é".repeat(131072) })]), {
        at: AT,
      }).ok,
    ).toBe(true);
  });
  ...
  it("#given a dependsOn fan-out over max_nodes_per_run #when compiled #then rejected on fan-out", () => {
    ...
    const wide = definition([
      ...upstream,
      node("join", ["u0", "u1", "u2", "u3"]),
    ]);

    const result = compileDag(wide, {
      at: AT,
      settings: { max_nodes_per_run: 3 },
    });
    ...
    expect(
      result.errors.find((error) => error.code === "dependency_fanout_exceeded")
        ?.nodeIds,
    ).toEqual([nodeId("join")]);
  });
});
