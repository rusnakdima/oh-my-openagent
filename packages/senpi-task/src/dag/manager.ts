...
export function createDagManager(options: DagManagerOptions): DagManager {
  ...
  return {
    ...
    snapshot: (runId, parentSessionId) =>
      projectSnapshot(ownedRecord(runId, parentSessionId)),
    record: ownedRecord,
    list(parentSessionId, listOptions) {
      ...
      for (
        const entry of fs.readdirSync(store.paths.runs, { withFileTypes: true })
      ) {
        if (!entry.isFile() || !entry.name.endsWith(".json")) continue;
        const record = store.readCheckpoint<DagRunRecordV1>(
          entry.name.slice(0, -5) as DagRunId,
        );
        if (record === null || record.parentSessionId !== parentSessionId) {
          continue;
        }
        summaries.push({
          runId: record.runId,
    ...
    history(params) {
      ...
      return store.readEvents(params.runId, sinceSeq, {
        limit: resolveLimit(
          params.limit,
          settings.history_default_limit,
          settings.history_max_limit,
        ),
        ...(params.lane === undefined ? {} : { lane: params.lane }),
        ...(params.types === undefined ? {} : { types: params.types }),
        ...(params.throughSeq === undefined
          ? {}
...
type DagDefinitionAmendedMutationEvent =
  & Extract<DagRunEvent, { readonly type: "dag.definition.amended" }>
  & {
    readonly definition: DagPersistedDefinition;
    readonly nodes: readonly DagNode[];
    readonly edges: readonly DagEdge[];
...
function startRun(
  ...
  return context.store.withKeyLock(
    params.parentSessionId,
    definition.key,
    () => {
      const existingKey = context.store.readKey(
        params.parentSessionId,
        definition.key,
      );
      ...
      const existing = existingKey === null
        ? null
        : context.store.readCheckpoint<DagRunRecordV1>(existingKey.runId);
      if (existing !== null) {
        if (existing.definitionFingerprint !== definitionFingerprint) {
          throw new DagManagerError({
            code: "definition_conflict",
            message:
              `dag run key "${definition.key}" already exists with a different definition`,
      ...
      const materialized = context.materializeSkills?.({
        runId,
        definition,
        at,
      });
      const effectivePrompts = new Map(
        (materialized?.nodes ?? []).map((node) =>
          [node.nodeId, node.effectivePrompt] as const
        ),
      );
...
function amendRun(
  params: DagAmendParams,
  context: AmendContext,
): DagRunRecordV1 {
  if ((params.runId === undefined) === (params.key === undefined)) {
    throw new DagManagerError({
      code: "invalid_arguments",
      message: "amend requires exactly one of runId or key",
    });
  }
  const selectedRunId = params.runId ??
    context.store.readKey(params.parentSessionId, params.key as string)?.runId;
  if (selectedRunId === undefined) {
    throw new DagManagerError({
      code: "run_not_found",
      message: `unknown dag run key "${params.key}"`,
    });
  }
  ...
  if (oldRecord === null) {
    throw new DagManagerError({
      code: "run_not_found",
      message: `unknown dag run "${selectedRunId}"`,
      runId: selectedRunId,
    });
  }
  if (oldRecord.parentSessionId !== params.parentSessionId) {
    throw new DagManagerError({
      code: "run_not_owned",
      message: `dag run "${selectedRunId}" belongs to another session`,
      runId: selectedRunId,
    });
  }
  if (oldRecord.status === "paused" && liveLeaseHolder(oldRecord)) {
    throw new DagManagerError({
      ...
      message: `dag run "${selectedRunId}" is paused by another live lease`,
      runId: selectedRunId,
    });
  }

  const at = new Date(context.now()).toISOString();
  const compiled = compileDag(params.definition, {
    at,
  ...
  if (params.definition.key !== oldRecord.runKey) {
    throw new DagManagerError({
      code: "invalid_amendment",
      message: "an amendment cannot change the run key",
      runId: selectedRunId,
    });
  }
  ...
  if (unsafeIds.length > 0) {
    throw new DagManagerError({
      ...
      message: `cannot amend scheduled or running nodes: ${
        unsafeIds.join(", ")
      }`,
      runId: selectedRunId,
      nodeIds: unsafeIds,
  ...
  if (oldRecord.status === "running") {
    throw new DagManagerError({
      ...
      message: `dag run "${selectedRunId}" is still active`,
      runId: selectedRunId,
    });
  }

  const invalidatedIds = transitiveDependents(compiled.edges, [
    ...diff.changedIds,
    ...diff.addedIds,
  ])
  ...
  const oldNodes = new Map(
    oldRecord.nodes.map((node) => [node.id, node] as const),
  );
  ...
  if (amended.definitionFingerprint !== definitionFingerprint) {
    throw new DagManagerError({
      code: "run_still_active",
      message:
        `dag run "${selectedRunId}" changed before the amendment could be applied`,
      runId: selectedRunId,
    });
  }
  return amended;
...
export function applyDagRunMutation(
  record: DagRunRecordV1,
  event: DagRunEvent,
): DagRunRecordV1 {
  if (event.type === "dag.run.created") {
  ...
  const removedNodeIds = record.nodes.filter((node) =>
    !mutationNodeIds.has(node.id)
  ).map((node) => node.id);
  const unsafeIds = [...event.changedNodeIds, ...removedNodeIds].filter(
    (id) => {
...
function nodeFingerprintFromInput(
  node: DagNodeInput,
...
function fingerprintDefinition(definition: DagDefinition): string {
  ...
  return dagDefinitionFingerprint({
    name: definition.name,
...
function liveLeaseHolder(record: DagRunRecordV1): boolean {
  const leaseRecord = record as DagRunRecordV1 & {
...
function materializeAmendedDefinition(
  ...
  const oldDefinition = new Map(
    oldRecord.definition.nodes.map((node) => [node.id, node] as const),
  );
  if (
    context.materializeSkills === undefined || rematerializedIds.length === 0
  ) {
    return {
      definition: {
        key: definition.key,
        ...
        nodes: definition.nodes.map((node) => ({
          ...node,
          effectivePrompt: oldDefinition.get(node.id)?.effectivePrompt ??
            node.prompt,
        })),
      },
  ...
  const manifestPath = join(
    context.store.paths.root,
    "skills",
    `${oldRecord.runId}.json`,
  );
  const oldManifest = fs.existsSync(manifestPath)
    ...
    : undefined;
  const selected = definition.nodes.filter((node) =>
    rematerializedIds.includes(node.id as DagNodeId)
  );
  const materialized = context.materializeSkills({
    runId: oldRecord.runId,
    definition: { ...definition, nodes: selected },
    at,
  });
  ...
  if (oldManifest !== undefined && fs.existsSync(manifestPath)) {
    const freshManifest = JSON.parse(fs.readFileSync(manifestPath, "utf8")) as {
      nodes?: readonly { nodeId: string }[];
    };
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        cwd: oldManifest.cwd,
        nodes: [
          ...(oldManifest.nodes ?? []).filter((node) =>
            !replaced.has(node.nodeId)
          ),
          ...(freshManifest.nodes ?? []),
        ],
      }),
  ...
  return {
    definition: {
      ...
      nodes: definition.nodes.map((node) => ({
        ...node,
        effectivePrompt: effective.get(node.id) ??
          oldDefinition.get(node.id)?.effectivePrompt ?? node.prompt,
      })),
    },
    diagnostics: [
      ...oldRecord.diagnostics,
      ...(materialized.diagnostics ?? []),
    ],
  };
}
...
function resolveLimit(
  ...
  if (!Number.isInteger(requested) || requested <= 0) {
    throw new DagManagerError({
      code: "invalid_arguments",
      message: `limit must be a positive integer, received ${requested}`,
    });
  }
