...
const sdkPath = join(
  import.meta.dir,
  "../../../omo-senpi/plugin/runtime/dag/sdk.js",
);
...
type HeldChild = {
  ...
  readonly settle: (
    outcome: { readonly status: "completed"; readonly finalResponse: string },
  ) => void;
...
function e2eFixture(
  options: { readonly admit?: AdmitResident; readonly runner?: ManagedRunner } =
    {},
): E2eFixture {
  ...
  return {
    ...
    attached: (runId, nodeId) =>
      new Promise<void>((resolve) => {
        ...
        const unsubscribe = scheduler.subscribe((event) => {
          if (
            event.type !== "dag.node.task-attached" || event.nodeId !== nodeId
          ) return;
          unsubscribe();
          resolve();
        });
    ...
    send: (runId, nodeId, message) =>
      controls(runId).sendToNode(runId, nodeId, message),
  };
...
function expectedSuccessSequence(
...
function assertArtifacts(
  ...
  expect(fs.existsSync(fixture.store.paths.key(parentSessionId, key))).toBe(
    true,
  );
...
describe("DAG happy-path end to end", () => {
  test("#given a linear three-node definition #when the real engine runs #then every event, output, snapshot, and artifact is wave ordered", async () => {
    ...
    // then
    expect(eventSequence(fixture.events(result.runId))).toEqual(
      expectedSuccessSequence([
        ["plan"],
        ["build"],
        ["review"],
      ]),
    ...
    expect(result.snapshot.counts).toEqual(
      expect.objectContaining({ total: 3, completed: 3 }),
    );
    expect(
      Object.fromEntries(
        Object.entries(result.nodes).map((
          [id, node],
        ) => [id, node.state === "completed" ? node.output : ""]),
      ),
    ).toEqual({
      plan: "output:plan",
      build: "output:build",
    ...
    assertArtifacts(fixture, result.runId, input.key, [
      "plan",
      "build",
      "review",
    ]);
  ...
  test("#given a diamond fan-out and join #when the real engine runs #then the middle nodes share one strict wave", async () => {
    ...
    // then
    expect(result.snapshot.waves.map((wave) => wave.nodeIds.map(String)))
      .toEqual([
        ["root"],
        ["left", "right"],
        ["join"],
  ...
  test("#given eight mixed-route nodes across four waves #when the real engine runs #then routes, membership, events, and outputs stay intact", async () => {
    ...
    expect(
      ...
      {
        id: "design",
        route: { kind: "category", category: "visual-engineering" },
      },
    ...
    expect(
      fixture.runner.startedSpecs.map((
        spec,
      ) => [spec.prompt.replace(/^do /, ""), spec.model, spec.agentType]),
    ).toEqual([
    ...
    expect(
      Object.values(result.nodes).every((node) =>
        node.state === "completed" && node.output.startsWith("output:")
      ),
    ).toBe(true);
    assertArtifacts(
      fixture,
  ...
  test("#given a completed run key #when the identical definition starts again #then the same run is reused without a second run file or event", async () => {
    ...
    expect(eventSequence(fixture.events(second.snapshot.runId))).toEqual(
      beforeEvents,
    );
  ...
  test("#given the shipped SDK builder #when define, node, start, and wait call the real engine #then wait returns a terminal DagRunResult with node outputs", async () => {
    ...
    const sdk = await import(sdkPath) as {
      readonly define: (
        input: { readonly key: string; readonly name: string },
      ) => {
      ...
      readonly wait: (
        runId: string,
      ) => Promise<{ readonly details: { readonly result: DagRunResult } }>;
    };
    const flow = sdk
    ...
    expect(eventSequence(fixture.events(started.details.run_id))).toEqual(
      expectedSuccessSequence([
        ["spec"],
        ["implement"],
        ["verify"],
    ...
    assertArtifacts(fixture, started.details.run_id, "sdk-flow", [
      "spec",
      "implement",
      "verify",
    ]);
  ...
  test("#given all first-wave starts hit the resident child cap #when the shipped SDK waits #then it returns the terminal residency failures instead of hanging", async () => {
    ...
    const sdk = await import(`${sdkPath}?residency-denied`) as {
      ...
      ) => Promise<{ readonly run_id: DagRunId }>;
      readonly wait: (
        runId: string,
      ) => Promise<{ readonly details: { readonly result: DagRunResult } }>;
    };
...
describe("DAG node control end to end", () => {
  test("#given a running node #when a message is sent #then the live child receives it and the node keeps running to completion", async () => {
    ...
    const started = await fixture.start(
      definition("steer-running", [categoryNode("worker")]),
    );
    const runId = started.snapshot.runId;
    ...
    expect(fixture.manager.record(runId, parentSessionId).nodes[0]?.state).toBe(
      "running",
    );
    ...
    runner.child("worker").settle({
      status: "completed",
      finalResponse: "output:worker",
    });
    const result = await fixture.wait(runId);
    expect(result.status).toBe("completed");
    expect(result.nodes.worker).toMatchObject({
      state: "completed",
  ...
  test("#given a settled run #when retry is followed immediately by wait #then wait blocks until the NEW settle", async () => {
    ...
    // then
    runner.child("first").settle({
      status: "completed",
      finalResponse: "output:first",
    });
    await runner.whenStarted(3);
    runner.child("second").settle({
      status: "completed",
      finalResponse: "output:second",
    });
    await reentry.run;
    ...
    expect(result.status).toBe("completed");
    expect(result.nodes.first).toMatchObject({
      state: "completed",
      output: "output:first",
    });
    expect(result.nodes.second).toMatchObject({
      state: "completed",
