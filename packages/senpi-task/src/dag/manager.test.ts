...
function runFiles(
  store: ReturnType<typeof createDagFileStore>,
): readonly string[] {
  return fs.readdirSync(store.paths.runs).filter((entry) =>
    entry.endsWith(".json")
  );
}
...
function raceWorkerSource(projectDir: string, prompt: string): string {
  return [
    ...
    `const { createDagFileStore } = await import(${
      JSON.stringify(join(import.meta.dir, "store.ts"))
    })`,
    `const { readSync } = await import("node:fs")`,
    `const store = createDagFileStore({ project_dir: ${
    ...
    `try {`,
    `  const started = await dag.start({ definition, parentSessionId: ${
...
async function raceStarts(
...
): Promise<readonly RaceOutcome[]> {
  const children = prompts.map((prompt) =>
    Bun.spawn(
  ...
  const outcomes = await Promise.all(
    readers.map(async (read) => JSON.parse(await read()) as RaceOutcome),
  );
...
describe("createDagManager start", () => {
  test("#given a valid two node definition #when started #then a run, a key file, and one created event are persisted", async () => {
    ...
    expect(started.snapshot.nodes.map((node) => `${node.id}:${node.state}`))
      .toEqual([
        "plan:pending",
        "build:pending",
      ]);
    expect(started.snapshot.waves.map((wave) => wave.nodeIds.join(",")))
      .toEqual(["plan", "build"]);
    expect(started.snapshot.criticalPath).toEqual([
      "plan" as DagNodeId,
      "build" as DagNodeId,
    ]);
    ...
    const events =
      store.readEvents(started.snapshot.runId, 0, { limit: 10 }).events;
  ...
  test("#given a run created from a definition #when the identical definition is resubmitted #then the existing run is reused without new writes", async () => {
    ...
    expect(store.readEvents(first.snapshot.runId, 0, { limit: 10 }).events)
      .toHaveLength(1);
  });
  ...
  test("#given a run created from a definition #when the same key is resubmitted with an edited prompt #then definition_conflict is thrown and the run is untouched", async () => {
    ...
    const conflicting = dag.start({
      definition: definition({
        nodes: [{
          id: "plan",
          prompt: "draft the plan differently",
          category: "quick",
        }],
      }),
      parentSessionId,
    ...
    expect(store.readEvents(first.snapshot.runId, 0, { limit: 10 }).events)
      .toHaveLength(1);
    expect(
      store.readKey(parentSessionId, "release-plan")?.definitionFingerprint,
    )
  ...
  test("#given a key file whose run record was pruned #when a changed definition reuses the key #then a fresh run is created instead of a conflict", async () => {
    ...
    const second = await dag.start({
      definition: definition({
        nodes: [{ id: "plan", prompt: "a different plan", category: "quick" }],
      }),
      parentSessionId,
      rootSessionId,
    });
  ...
  test("#given a definition with a cyclic dependency #when started #then invalid_definition is thrown and nothing is persisted", async () => {
    ...
    await rejected.catch((error: unknown) => {
      ...
      expect((error as DagManagerError).errors.map((entry) => entry.code))
        .toEqual(["cycle"]);
    });
  ...
  test("#given a configured node ceiling #when a definition exceeds it #then invalid_definition names the configured bound", async () => {
    ...
    await rejected.catch((error: unknown) => {
      expect((error as DagManagerError).errors.map((entry) => entry.code))
        .toEqual(["node_count_exceeded"]);
      expect((error as DagManagerError).message).toContain(
        "max_nodes_per_run 1",
      );
    });
    expect(runFiles(store)).toEqual([]);
  ...
  test("#given the same run key in two different sessions #when both start #then each session owns its own run", async () => {
    ...
    // when
    const mine = await dag.start({
      definition: definition(),
      parentSessionId,
      rootSessionId,
    });
    const theirs = await dag.start({
      definition: definition(),
...
describe("createDagManager skill materialization seam", () => {
  test("#given a materializeSkills hook #when a run is created #then per node effectivePrompt slots are persisted without entering the fingerprint", async () => {
    ...
    // when
    const materialized = await withSkills.start({
      definition: definition(),
      parentSessionId,
      rootSessionId,
    });
    const bare = await plain.start({
      definition: definition(),
      parentSessionId: otherSessionId,
      rootSessionId,
    });
    ...
    // then
    expect(
      withSkills.record(materialized.snapshot.runId, parentSessionId).definition
        .nodes.map((node) => node.effectivePrompt),
    )
      .toEqual([
    ...
    expect(
      plain.record(bare.snapshot.runId, otherSessionId).definition.nodes.map((
        node,
      ) => node.effectivePrompt),
    )
      .toEqual(["draft the plan", "build it"]);
    expect(materialized.snapshot.definitionFingerprint).toBe(
      bare.snapshot.definitionFingerprint,
    );
  });
});
...
describe("createDagManager attach, snapshot, and history", () => {
  test("#given a run owned by one session #when another session attaches #then run_not_owned is thrown", async () => {
    ...
    // when
    const attachOther = () =>
      dag.attach(started.snapshot.runId, otherSessionId);
    ...
    expect(dag.attach(started.snapshot.runId, parentSessionId).runId).toBe(
  ...
  test("#given an unknown run id #when attached or snapshotted #then run_not_found is thrown", () => {
    ...
    expect(() => dag.snapshot(missing, parentSessionId)).toThrow(
      DagManagerError,
    );
    try {
...
describe("createDagManager list", () => {
  test("#given runs across two sessions #when listed #then only this session's runs appear sorted by updatedAt desc then runId asc", async () => {
    ...
    const first = await dag.start({
      definition: definition({ key: "alpha" }),
      parentSessionId,
      rootSessionId,
    });
    const second = await dag.start({
      definition: definition({ key: "beta" }),
      parentSessionId,
      rootSessionId,
    });
    clock += 60_000;
    const third = await dag.start({
      definition: definition({ key: "gamma" }),
      parentSessionId,
      rootSessionId,
    });
    await dag.start({
      definition: definition({ key: "delta" }),
      parentSessionId: otherSessionId,
      rootSessionId,
    });
  ...
  test("#given more runs than the requested limit #when listed #then the limit is clamped to at most 256 and defaults to 100", async () => {
    ...
    for (let index = 0; index < 3; index += 1) {
      await dag.start({
        definition: definition({ key: `key-${index}` }),
        parentSessionId,
        rootSessionId,
      });
    }
    ...
    expect(() => dag.list(parentSessionId, { limit: 0 })).toThrow(
      DagManagerError,
    );
  });
});
...
describe("createDagManager amend", () => {
  const diamond = (
    bPrompt = "build b",
    extraNodes: DagDefinition["nodes"] = [],
  ): DagDefinition => ({
    key: "diamond",
    name: "diamond",
    nodes: [
  ...
  test("#given a completed diamond #when B's prompt is amended #then only B and D are invalidated and cached siblings survive", async () => {
    ...
    const amended = await dag.amend({
      runId: started.snapshot.runId,
      definition: diamond("build b differently"),
      parentSessionId,
    });

    expect(
      amended.nodes.map((node) => ({
        id: node.id as string,
        state: node.state,
        taskId: node.taskId,
        attempt: node.attempt,
        execAttempt: node.execAttempt,
      })),
    ).toEqual([
      {
        id: "A",
    ...
    expect(
      (amended.nodes[0] as typeof amended.nodes[number] & {
        resultArtifact?: unknown;
      })?.resultArtifact,
    ).toBeDefined();
    expect(
      (amended.nodes[2] as typeof amended.nodes[number] & {
        resultArtifact?: unknown;
      })?.resultArtifact,
    ).toBeDefined();
    ...
    expect(
      store.readEvents(started.snapshot.runId, 0, { limit: 10 }).events.at(-1)
        ?.type,
    ).toBe("dag.definition.amended");
    ...
    expect(dag.snapshot(started.snapshot.runId, parentSessionId).amendHistory)
      .toHaveLength(1);
    expect(
      dag.snapshot(started.snapshot.runId, parentSessionId).nodes[1]?.attempt,
    ).toBe(1);

    const reused = await dag.start({
      definition: diamond("build b differently"),
      parentSessionId,
      rootSessionId,
    });
    expect(reused.reused).toBe(true);
    const original = dag.start({
      definition: diamond(),
      parentSessionId,
      rootSessionId,
    });
  ...
  test("#given changed or invalid graph members #when amended #then running nodes and dangling removals are typed refusals while leaf removal is legal", async () => {
    ...
    store.writeCheckpoint(started.snapshot.runId, {
      ...
      nodes: completed.nodes.map((node) =>
        node.id === "B" ? { ...node, state: "running" as const } : node
      ),
    });
    ...
    const withoutD = {
      ...diamond(),
      nodes: diamond().nodes.filter((node) => node.id !== "D"),
    };
    const amended = await dag.amend({
      key: "diamond",
      definition: withoutD,
    ...
    expect(amended.nodes.map((node) => node.id as string)).toEqual([
      "A",
      "B",
      "C",
    ]);
  ...
  test("#given a completed run #when a node is added #then compiled projections are replaced so the new node is admissible", async () => {
    ...
    expect(amended.nodes.find((node) => node.id === "E")).toMatchObject({
      state: "pending",
      attempt: 0,
    });
    expect(amended.waves.map((wave) => wave.nodeIds.map((id) => id as string)))
      .toEqual([["A"], ["B", "C"], ["D"], ["E"]]);
    expect(
      amended.edges.map((edge) => ({
        from: edge.from as string,
        to: edge.to as string,
      })),
    ).toContainEqual({ from: "D", to: "E" });
  });
  ...
  test("#given a materialized run #when changed and added nodes are amended #then skills rematerialize once for only those nodes", async () => {
      ...
      materializeSkills: ({ definition: input }) => {
        ...
        return {
          nodes: input.nodes.map((node) => ({
            nodeId: node.id,
            effectivePrompt: `skill:${node.prompt}`,
          })),
        };
      },
    });
    ...
    expect(
      amended.definition.nodes.find((node) => node.id === "A")?.effectivePrompt,
    ).toBe("skill:build a");
    expect(
      amended.definition.nodes.find((node) => node.id === "B")?.effectivePrompt,
    ).toBe("skill:changed");
    expect(
      amended.definition.nodes.find((node) => node.id === "E")?.effectivePrompt,
    ).toBe("skill:build e");
  });
  ...
  test("#given amended and retried WAL events #when replayed from the old checkpoint #then the shared reducer rebuilds the identical checkpoint", async () => {
    ...
    const amendmentEvent = store.readEvents(started.snapshot.runId, 1, {
      limit: 10,
    }).events[0] as DagRunEvent;
    const retriedPayload = dagNodeRetriedEvent({
      nodeId: "B" as DagNodeId,
      priorTaskId: "task-B",
      execAttempt: 2,
      promptChanged: false,
    });
    ...
    const replay = createDagJournal<DagRunRecordV1>({
      store,
      runId: started.snapshot.runId,
      initialCheckpoint: old,
      applyEvent: applyDagRunMutation,
    });
  ...
  test("#given a durable amendment whose preconditions no longer hold #when the WAL is replayed #then replay no-ops instead of wedging the run forever", async () => {
    ...
    const replay = (): DagRunRecordV1 =>
      createDagJournal<DagRunRecordV1>({
        store,
        runId: started.snapshot.runId,
    ...
    expect(replayed.definitionFingerprint).toBe(
      "stale-fingerprint-that-never-matches",
    );
    expect(replayed.nodes.find((node) => node.id === "B")?.state).toBe(
      "running",
    );
  });
});
...
describe("createDagManager concurrent starts", () => {
  test(
    "#given two OS processes starting the same run key #when both are released together #then exactly one run file exists and the loser reuses it",
    async () => {
      // given
      const projectDir = tempProject();

      // when
      const outcomes = await raceStarts(projectDir, [
        "draft the plan",
        "draft the plan",
      ]);
      ...
      expect(outcomes.filter((outcome) => outcome.reused === false))
        .toHaveLength(1);
      expect(outcomes.filter((outcome) => outcome.reused === true))
        .toHaveLength(1);
    },
    30_000,
  );
  ...
  test(
    "#given two OS processes racing the same key with different prompts #when both are released together #then one run is created and the conflicting submission mutates nothing",
    async () => {
      // given
      const projectDir = tempProject();
      ...
      expect(
        outcomes.filter((outcome) => outcome.code === "definition_conflict"),
      ).toHaveLength(1);
      ...
      expect(
        store.readEvents(winner, 0, { limit: 10 }).events.map((event) =>
          event.type
        ),
      ).toEqual(["dag.run.created"]);
    },
    30_000,
  );
});
