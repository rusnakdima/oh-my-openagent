...
function recoverableRecord(
  ...
  const record: DagRunRecordV1 = {
    ...
    createdAt,
    updatedAt: createdAt,
    nodes: compiled.nodes.map((entry) => ({
      ...entry,
...
function taskRecord(
  ...
  return {
    ...
    ...(status === "completed"
      ? { final_response: `done ${owner.nodeId}` }
      : {}),
    ...(status === "lost"
      ? { error_message: "previous-process in-process" }
      : {}),
...
class RecoveryTaskManager implements TaskManager {
  ...
  complete(taskId: string, status: TaskStatus = "completed"): void {
    ...
    task.record = {
      ...
      ...(status === "completed"
        ? { final_response: `done ${task.record.owner?.nodeId ?? taskId}` }
        : {}),
    };
  ...
  findOwnedTask(
    ...
    return [...this.#tasks.values()].find(({ record }) =>
      record.owner?.kind === owner.kind && record.owner.runId === owner.runId &&
  ...
  sendToTask(): Promise<never> {
    throw new Error("not implemented");
  }
  interruptTask(): Promise<never> {
    throw new Error("not implemented");
  }
  cancelTask(): Promise<never> {
    throw new Error("not implemented");
  }
  list(): readonly [] {
    return [];
...
describe("DAG crash recovery", () => {
  test("#given a paused run after wave one #when the session restarts #then completed work is reused, the running child folds, and the incomplete wave resumes", async () => {
    ...
    expect(outcomes[0]?.reusedOutputs?.get("done" as DagNodeId)).toBe(
      "durable done output",
    );
    ...
    expect(
      outcomes[0]?.record?.nodes.map((entry) => `${entry.id}:${entry.state}`),
    )
      .toEqual(["done:completed", "running:completed", "next:completed"]);
    expect(
      events(store).some((event) =>
        event.type === "dag.node.reused" && event.nodeId === "done"
      ),
    ).toBe(true);
    expect(events(store).some((event) => event.type === "dag.run.resumed"))
      .toBe(true);
  });
  ...
  test("#given no injected liveness probe #when a paused run's previous holder is this live process #then the default probe skips it as a live lease", async () => {
    ...
    store.writeCheckpoint(
      runId,
      recoverableRecord(definition([node("only")]), {
  ...
  test("#given scheduled nodes with and without durable owners #when resumed #then the owner is attached and only never-dispatched work starts", async () => {
    ...
    store.writeCheckpoint(
      runId,
      recoverableRecord(definition([node("owned"), node("fresh")]), {
    ...
    expect(
      outcome?.record?.nodes.map((entry) =>
        `${entry.id}:${entry.taskId}:${entry.state}`
      ),
    )
  ...
  test("#given an attached node whose task, owner, result, and transcript vanished #when resumed #then it fails closed without dispatch", async () => {
    ...
    store.writeCheckpoint(
      runId,
      recoverableRecord(definition([node("uncertain")]), {
  ...
  test("#given reconcile marked an in-process child lost #when its paused DAG resumes #then task_lost is folded and never re-dispatched", async () => {
    ...
    store.writeCheckpoint(
      runId,
      recoverableRecord(definition([node("lost")]), {
  ...
  test("#given a paused run owned by another parent session #when recovery scans #then it is not claimed", async () => {
    ...
    store.writeCheckpoint(
      runId,
      recoverableRecord(definition([node("foreign")]), {}, {
  ...
  test("#given two managers race a paused run #when the first claim holder is live #then exactly one resumes and the other observes the live lease", async () => {
    ...
    storeA.writeCheckpoint(
      runId,
      recoverableRecord(input, {
    ...
    const recoveryA = createDagRecovery({
      store: storeA,
      taskManager: managerA,
      hostPid: 101,
      isProcessAlive: alive,
    });
    const recoveryB = createDagRecovery({
      store: storeB,
      taskManager: managerB,
      hostPid: 202,
      isProcessAlive: alive,
    });
    ...
    // then
    expect(outcomesA.filter((outcome) => outcome.kind === "resumed"))
      .toHaveLength(1);
    expect(outcomesB).toEqual([{
      runId,
      kind: "skipped",
      reason: "live_lease",
    }]);
  ...
  test("#given a crash after a terminal transition reaches the WAL but before its reducer #when recovery reopens #then output artifact metadata and run stats are rebuilt", async () => {
    ...
    baseStore.writeCheckpoint(
      runId,
      recoverableRecord(definition([node("wal-artifact")]), {
        "wal-artifact": {
          state: "running",
          taskId: "task-wal-artifact",
          attempt: 1,
        },
      }, { previousLeaseHolderPid: 9001 }),
    );
    ...
    const crashingStore: DagFileStore = {
      ...
      appendEvent(event) {
        ...
        if (
          crashAfterTerminalWal && event.type === "dag.node.transitioned" &&
          event.to === "completed"
        ) {
    ...
    ).rejects.toThrow("injected crash after terminal WAL append");
    ...
    // then
    const checkpoint = reopenedStore.readCheckpoint<
      DagRunRecordV1 & {
    ...
    const statsPath = reopenedStore.paths.result(runId, "wal-artifact").replace(
      /\.txt$/,
      ".stats.json",
    );
    ...
    expect(reopenedStore.readResult(runId, "wal-artifact")).toBe(
      "wal boundary output",
    );
  ...
  test("#given a crash after a recovered task transition reaches the WAL #when the engine reopens #then artifact metadata is rebuilt from the durable copy", async () => {
    ...
    baseStore.writeCheckpoint(
      runId,
      recoverableRecord(definition([node("artifact")]), {
    ...
    const crashingStore: DagFileStore = {
      ...
      writeCheckpoint(resultRunId, checkpoint) {
        ...
        if (
          crashTerminalCheckpoint &&
          record.nodes.some((entry) => entry.state === "completed")
        ) {
    ...
    // when
    await expect(firstRecovery.resumePausedRuns(parentSessionId)).rejects
      .toThrow("injected terminal checkpoint crash");
    const reopenedStore = createDagFileStore({ project_dir: projectDir });
    const [outcome] = await createDagRecovery({
      store: reopenedStore,
      taskManager: manager,
    ...
    const statsRaw = fs.readFileSync(
      reopenedStore.paths.result(runId, "artifact").replace(
        /\.txt$/,
        ".stats.json",
      ),
      "utf8",
    ...
    expect(reopenedStore.readResult(runId, "artifact")).toBe(
      "recovered artifact output",
    );
    expect(checkpoint?.nodes[0]?.runStats).toEqual(completed.run_stats);
    expect(artifact).toEqual({
      relativePath: join("dag", "results", runId, "artifact.txt"),
      sha256: createHash("sha256").update("recovered artifact output").digest(
        "hex",
      ),
  ...
  test("#given a live run #when shutdown pause starts #then admission stops before pause persistence and its lease is released", () => {
    ...
    store.writeCheckpoint(
      runId,
      recoverableRecord(definition([node("active")]), {
        active: { state: "running", taskId: "task-active", attempt: 1 },
    ...
    const checkpoint = store.readCheckpoint<
      DagRunRecordV1 & {
        leaseHolderPid?: number;
        previousLeaseHolderPid?: number;
      }
    >(runId);
...
describe("DAG recovery attempt-scoped ownership", () => {
  test("#given a pre-change checkpoint with legacy owner fingerprints #when resumed #then it completes and the legacy fingerprint is reused verbatim", async () => {
    ...
    expect(legacyRecord.nodes.every((entry) => entry.execAttempt === undefined))
      .toBe(true);
    ...
    expect(outcome?.reusedOutputs?.get("legacy-done" as DagNodeId)).toBe(
      "legacy output",
    );
  ...
  test("#given a reattach whose observed taskId differs #when resumed #then the display attempt bumps without a new execution and the owner fingerprint still matches", async () => {
    ...
    store.writeCheckpoint(
      runId,
      recoverableRecord(definition([node("drifted")]), {
    ...
    expect(dagFingerprint(ownerFingerprintInput({
      definitionFingerprint: "definition-fingerprint",
      nodeId: "drifted" as DagNodeId,
      ...(node0?.execAttempt === undefined
        ? {}
        : { execAttempt: node0.execAttempt }),
  ...
  test("#given a retried pending node with execAttempt #when resumed #then it starts fresh under the attempt-scoped fingerprint", async () => {
    ...
    store.writeCheckpoint(
      runId,
      recoverableRecord(definition([node("retried")]), {
        retried: {
          state: "pending",
          taskId: "task-retried-1",
          attempt: 1,
          execAttempt: 2,
        },
      }, { previousLeaseHolderPid: 9001 }),
