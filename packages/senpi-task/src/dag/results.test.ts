...
describe("persistDagNodeResult durable node artifacts", () => {
  test("#given a terminal node record #when persisted #then the response file carries the sha256 and byte count of the copy", () => {
    ...
    if (outcome.kind !== "persisted") {
      throw new Error("expected persisted outcome");
    }
    expect(outcome.artifact.relativePath).toBe(
      join("dag", "results", runId, `${nodeId}.txt`),
    );
    expect(outcome.artifact.sha256).toBe(sha256(finalResponse));
    expect(outcome.artifact.bytes).toBe(
      Buffer.byteLength(finalResponse, "utf8"),
    );
    expect(
      fs.readFileSync(
        join(store.stateDir, outcome.artifact.relativePath),
        "utf8",
      ),
    ).toBe(finalResponse);
  });
  ...
  test("#given run stats on the terminal record #when persisted #then a stats sidecar holds them with its own digest", () => {
    ...
    expect(sidecar.relativePath).toBe(
      join("dag", "results", runId, `${nodeId}.stats.json`),
    );
    const raw = fs.readFileSync(
      join(store.stateDir, sidecar.relativePath),
      "utf8",
    );
  ...
  test("#given an unwritable result path #when persisting #then a journal_corrupt diagnostic is returned instead of throwing", () => {
    ...
    expect(persistDagNodeResult({ store, runId, nodeId, record }).kind).toBe(
      "persisted",
    );
  });
  ...
  test("#given a node with no persisted artifact #when reuse reads it #then it reports nothing rather than falling back to the record", () => {
    ...
    expect(
      readDagNodeResult({ store, runId, nodeId: "build" as DagNodeId })?.output,
    ).toBe("record-only response");
  });
  ...
  test("#given a terminal record without run stats #when persisted #then no sidecar is written and reuse reports undefined stats", () => {
    ...
    const store = createDagFileStore({ project_dir: projectDir });
    const record: TaskRecord = {
      ...terminalRecord(projectDir, "no stats output"),
      run_stats: undefined,
    };
    ...
    expect(
      fs.existsSync(
        join(store.stateDir, "dag", "results", runId, `${nodeId}.stats.json`),
      ),
    ).toBe(false);
    const reused = readDagNodeResult({ store, runId, nodeId });
  ...
  test("#given a persisted node #when persisted again with a different body #then the second body replaces the first and the stats sidecar is replaced", () => {
    ...
    // when
    const firstOutcome = persistDagNodeResult({
      store,
      runId,
      nodeId,
      record: firstRecord,
    });
    const secondOutcome = persistDagNodeResult({
      store,
      runId,
      nodeId,
      record: secondRecord,
    ...
    expect(fs.readdirSync(resultDir).sort()).toEqual([
      `${nodeId}.stats.json`,
      `${nodeId}.txt`,
    ]);
  });
  ...
  test("#given a fault mid-write during re-persist #then the prior artifact body remains intact and no orphan temp file is left", () => {
    ...
    const faultingWriteSync = (
      ...
      if (
        typeof data === "string" &&
        data.includes("second attempt body that will crash")
      ) {
      ...
      if (typeof data === "string") {
        return originalWriteSync(
          fd,
          data,
          ...(args as [(number | null)?, BufferEncoding?]),
        );
      }
      return originalWriteSync(
        fd,
        data,
        ...(args as [(number | null)?, (number | null)?, (number | null)?]),
      );
    };
    const writeSpy = spyOn(fs, "writeSync").mockImplementation(
      faultingWriteSync as unknown as typeof fs.writeSync,
    );

    // when
    let outcome: ReturnType<typeof persistDagNodeResult> | undefined;
    try {
      outcome = persistDagNodeResult({
        store,
        runId,
        nodeId,
        record: secondRecord,
