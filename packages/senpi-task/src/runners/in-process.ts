...
import {
  buildChildSessionOptions,
  requireChildSessionDir,
  resolveMemberScopedToolNames,
} from "./in-process/child-options";
...
export class InProcessRunner {
  ...
  constructor(options: InProcessRunnerOptions = {}) {
    ...
    this.#depthPolicy = options.depthPolicy ??
      { maxDepth: DEFAULT_MAX_CHILD_DEPTH };
  ...
  async start(spec: ChildSpec): Promise<ChildHandle> {
    if (spec.depth > this.#depthPolicy.maxDepth) {
      throw new RunnerError({
        kind: "depth-exceeded",
        message:
          `Child depth ${spec.depth} exceeds max depth ${this.#depthPolicy.maxDepth}.`,
      });
    }
    ...
    try {
      ...
      throw new RunnerError({
        ...
        message: sessionCreateMessage(error),
        cause: error,
      });
    }
  ...
  async resume(spec: ChildSpec, sessionPath: string): Promise<ChildHandle> {
    const memberScopedTools = resolveMemberScopedToolNames(
      spec.memberScopedToolNames ?? [],
      this.#sharedParentTools,
    );
    ...
    try {
      ...
      throw new RunnerError({
        ...
        message: sessionResumeMessage(error),
        cause: error,
      });
    }
...
function sessionCreateMessage(error: unknown): string {
  return `Failed to create in-process child session: ${
    error instanceof Error ? error.message : String(error)
  }`;
}

function sessionResumeMessage(error: unknown): string {
  return `Failed to resume in-process child session: ${
    error instanceof Error ? error.message : String(error)
  }`;
}
...
function isSessionHeaderEntry(
  entry: unknown,
): entry is { readonly type: "session"; readonly id: string } {
  return (
    typeof entry === "object" &&
    entry !== null &&
    "type" in entry &&
