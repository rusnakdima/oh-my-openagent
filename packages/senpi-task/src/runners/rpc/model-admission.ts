...
function readCatalog(
  ...
  if (result.code !== 0) {
    ...
    throw admissionFailure(
      model,
      `catalog probe exited ${result.code}${
        detail.length === 0 ? "" : `: ${detail}`
      }`,
    );
  }
  return {
    models: parseModelCatalog(result.stdout),
    stderrTail: result.stderr.trim().slice(-1_000),
  };
}
...
function absenceFailure(model: string, confirmed: ProbedCatalog): RunnerError {
  return admissionFailure(
    ...
    `model is not visible in the child profile after a confirming re-probe (probed catalog has ${confirmed.models.size} models${
      confirmed.stderrTail.length === 0
        ? ""
        : `; child stderr: ${confirmed.stderrTail}`
 ...
 */
export function createRpcModelAdmission(
  options: RpcModelAdmissionOptions = {},
): RpcModelAdmission {
  ...
  return async (spec) => {
    ...
    let catalog = cached !== undefined &&
        now() - cached.cachedAt < MODEL_CATALOG_CACHE_TTL_MS
      ? cached.catalog
      : undefined;
