...
const CONSOLE_HOST_PATH = fileURLToPath(
  new URL("./windows-console-host.ps1", import.meta.url),
);
...
async function runCase(mode: ProbeMode, root: string): Promise<ProbeCase> {
  ...
  const ready = waitForFile(
    readyPath,
    AbortSignal.timeout(PROBE_STEP_TIMEOUT_MS),
  );
  const parent = spawn(
    "powershell.exe",
    [
  ...
  const closed = once(parent, "close", {
    signal: AbortSignal.timeout(PROBE_STEP_TIMEOUT_MS),
  });
  ...
  try {
    ...
    if (outcome.kind === "closed") {
      const detail = existsSync(errorPath)
...
async function runProbe(): Promise<void> {
  ...
  const root = realpathSync.native(
    mkdtempSync(join(tmpdir(), "omo-rpc-window-probe-")),
  );
  ...
  try {
    ...
    const credentialsUntouched =
      JSON.stringify(credentialsBefore) === JSON.stringify(credentialsAfter);
    const pass = visible.consoleAttached &&
  ...
  if (probeResult === undefined) {
    throw new Error("Windows console probe did not produce a result");
  }
...
if (parentIndex >= 0) {
  ...
  if (
    (mode !== "visible-control" && mode !== "hidden-fixed") ||
    root === undefined
  ) {
    throw new Error(
      "usage: windows-console-probe.ts --parent <visible-control|hidden-fixed> <root>",
    );
  }
