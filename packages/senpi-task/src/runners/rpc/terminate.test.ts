...
const PROCESS_TREE_PATH = fileURLToPath(
  new URL("./__fixtures__/process-tree.mjs", import.meta.url),
);
...
describe("terminateRpcChild", () => {
  test.skipIf(isWin32)(
    "#given a cooperating child #when terminating #then SIGTERM ends it without escalation",
    async () => {
      // given
  ...
  test.skipIf(isWin32)(
    "#given a non-group-leader child #when terminating #then direct escalation remains available",
    async () => {
      // given
      ...
      const pid = child.pid;
      if (pid === undefined) {
        throw new Error("non-group-leader child did not receive a pid");
      }
