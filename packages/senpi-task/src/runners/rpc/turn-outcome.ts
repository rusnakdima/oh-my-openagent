...
export function agentEndOutcome(
  ...
  const messages: readonly unknown[] = Array.isArray(event.messages)
    ? event.messages
    : [];
  ...
  const stopReason = assistant === undefined
    ? undefined
    : readString(assistant.stopReason);
  const errorMessage = assistant === undefined
    ? undefined
    : readString(assistant.errorMessage);
    ...
    return {
      ...
      failure: {
        ...
        message: errorMessage ??
          `RPC child turn ended with stopReason "${stopReason ?? "aborted"}"`,
      },
    };
  }
  const messageText = assistant === undefined
    ? undefined
    : extractAssistantText(assistant);
  const final = messageText ??
    (observedText !== baseline ? observedText : undefined);
  ...
  return {
    ...
    failure: {
      kind: "child-turn-failed",
      message: errorMessage ?? "RPC child turn produced no assistant output",
    },
  };
}

export function exitTurnOutcome(
  exit: ChildExitOutcome,
  finalText: string | undefined,
): RunnerOutcome {
  if (
    exit.kind === "clean" && finalText !== undefined && finalText.length > 0
  ) {
  ...
  return {
    ...
    failure: {
      ...
      message: facts?.error_message ?? "RPC child terminated abnormally",
    },
