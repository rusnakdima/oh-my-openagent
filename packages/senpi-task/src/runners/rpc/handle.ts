 ...
 */
export function createRpcChildHandle(
  options: CreateRpcChildHandleOptions,
): TrackedRpcChildHandle {
  ...
  client.onEvent((event) => {
    ...
    if (event.type === "agent_end" && event.willRetry === false) {
      settleTurn(
        abortedByUser
          ? { status: "cancelled" }
          : agentEndOutcome(event, turnBaseline, finalText),
      );
    }
  });

  const heartbeat = setInterval(() => {
    if (client.exited || child.stdin?.writableEnded || child.stdin?.destroyed) {
      return;
    }
    client
      .send({ type: "get_state" })
      ...
      .catch((error: unknown) => {
        ...
        log("senpi-task heartbeat get_state failed", {
          taskId,
          error: String(error),
        });
      });
  ...
  const settleExit = (built: ChildExitOutcome): void => {
    ...
    if (turnOutcome === undefined) {
      settleTurn(exitTurnOutcome(built, finalText));
    }
  ...
  const runCommand = async (
    command: Parameters<RpcProtocolClient["send"]>[0],
    label: string,
  ): Promise<void> => {
  ...
  const deliverPrompt = async (
    text: string,
    streamingBehavior: RpcStreamingBehavior,
  ): Promise<void> => {
    try {
      await runCommand(
      ...
      if (streamingBehavior === "followUp" || !isBusyChildRejection(error)) {
        throw error;
      }
      await runCommand({
        type: "prompt",
  ...
  const runPrompt = async (
    text: string,
    streamingBehavior: RpcStreamingBehavior = "steer",
  ): Promise<void> => {
    beginTurn();
    try {
  ...
  return {
    ...
    waitForExit:
      () => (outcome
        ...
        : new Promise<ChildExitOutcome>((resolve) =>
    ...
    terminate: (terminateOptions?: TerminateOptions) =>
      terminateRpcChild(child, terminateOptions),
...
function extractTerminalAssistantMessage(
  message: unknown,
): RpcTerminalAssistantMessage | undefined {
