...
} from "../../team/member-extension/identity";
...
export function detectBunBinary(metaUrl: string): boolean {
  return metaUrl.includes("$bunfs") || metaUrl.includes("~BUN") ||
 ...
 */
export function resolveSenpiExecutable(
  ...
  if (override !== undefined && override.length > 0) {
    if (
      override.includes("/") || override.includes(sep) || isAbsolute(override)
    ) {
...
function normalizeSenpiLauncher(
  executable: string,
  runtime: RpcSpawnRuntime,
): SenpiLauncher | null {
  if (
    runtime.platform !== "win32" || executable.toLowerCase().endsWith(".exe")
  ) {
...
export function resolveSenpiLauncher(
  runtime: RpcSpawnRuntime,
): SenpiLauncher | null {
  const executable = (runtime.resolveSenpiExecutable ?? resolveSenpiExecutable)(
    runtime,
  );
  if (executable !== null) {
...
function isDagOwnedChild(spec: RpcRunnerSpec): boolean {
  if (
    basename(dirname(spec.state_dir)) !== "children" ||
    basename(spec.state_dir) !== spec.task_id
  ) return false;
  const stateDir = dirname(dirname(spec.state_dir));
  const record = JSON.parse(
    readFileSync(join(stateDir, "tasks", `${spec.task_id}.json`), "utf8"),
  ) as unknown;
  if (typeof record !== "object" || record === null || !("owner" in record)) {
    return false;
  }
  const owner = record.owner;
  return typeof owner === "object" && owner !== null && "kind" in owner &&
    owner.kind === "dag";
}
...
export function buildChildArgs(spec: RpcRunnerSpec): readonly string[] {
  ...
  const extensions = isDagOwnedChild(spec)
    ? (spec.extensions ?? []).slice(1)
    : spec.extensions ?? [];
...
function buildChildProfile(
  ...
  const extensions = spec.memberEnv === undefined
    ? spec.extensions?.filter((entry) =>
      basename(entry) !== MEMBER_EXTENSION_BUNDLE_NAME
    )
    : spec.extensions;
  return {
    env,
    spec: extensions === spec.extensions ? spec : { ...spec, extensions },
  };
}

export function buildRpcSpawn(
  spec: RpcSpawnSpec,
  ...
  return {
    command: resolved.execPath,
    args: [resolved.resolveRpcEntry(), ...childArgs],
    cwd: spec.cwd,
    env,
  };
}
