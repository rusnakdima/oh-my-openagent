...
export class RpcProtocolClient {
  ...
  constructor(options: RpcProtocolClientOptions) {
    ...
    this.onMalformedLine = options.onMalformedLine ??
      ((line, error) =>
        log("senpi-task rpc malformed line", { line, error: String(error) }));
  ...
  send(command: RpcCommand): Promise<RpcResponse> {
    ...
    if (
      this.isExited || stdin === null || stdin === undefined ||
      stdin.writableEnded || stdin.destroyed
    ) {
      return Promise.reject(
        new Error(`RPC process is not running. Stderr: ${this.stderrTail}`),
      );
    }
  ...
  async switchSession(sessionPath: string): Promise<RpcSwitchSessionResult> {
    ...
    if (response.success && response.command === "switch_session") {
  ...
  async getEntries(since?: string): Promise<RpcEntriesResult> {
    const command: RpcCommand = since === undefined
      ? { type: "get_entries" }
  ...
  private finalize(error?: Error): void {
    ...
    this.isExited = true;
    const failure = error ??
      new Error(`RPC process exited. Stderr: ${this.stderrTail}`);
...
function commandError(
  ...
  if (!response.success) {
    return new RpcCommandError(expectedCommand, response.error);
  }
