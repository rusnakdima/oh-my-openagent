...
describe("in-process access_terminated_error fallback", () => {
  test(
    "#given access_terminated_error makes the first category candidate unusable #when the child runs #then the next configured candidate produces the result",
    async () => {
      // given
      ...
      try {
        await harness.session.prompt(
          "complete through the configured fallback",
        );
  ...
  test("#given context overflow is deliberately non-fallback #when the child runs #then it stays terminal on the first candidate", async () => {
    ...
    try {
      ...
      expect(
        harness.events.some((event) => event.type === "retry_fallback_applied"),
      ).toBe(false);
