...
export function requireChildSessionDir(spec: ChildSpec): string {
  if (typeof spec.sessionDir !== "string" || spec.sessionDir.length === 0) {
    throw new RunnerError({
      ...
      message:
        `Child ${spec.taskId} has no sessionDir; refusing to fall back to a default session location.`,
    });
  }
  ...
  for (const name of names) {
    if (seen.has(name)) {
      throw new RunnerError({
        ...
        message:
          `Member-scoped tool name "${name}" is duplicated in the persisted spec.`,
      });
    }
    seen.add(name);
 ...
 */
export function buildChildSessionOptions(
  input: BuildChildSessionOptionsInput,
): CreateAgentSessionOptions {
  const { spec, sessionManager, uiOnlyToolNames } = input;
  const mergedCustomTools = mergeChildCustomTools(
    input.sharedParentTools,
    spec.memberScopedTools,
    {
      uiOnlyToolNames,
    },
  );
  const customTools = spec.agentType !== undefined &&
      CURATED_READONLY_AGENT_NAMES.has(spec.agentType)
    ? [
      ...mergedCustomTools.filter((tool) => tool.name !== "bash"),
      createCuratedReadonlyBashTool(spec.cwd),
    ]
    : mergedCustomTools;
  ...
  return {
    ...
    ...(spec.toolDenylist !== undefined &&
      { excludeTools: [...spec.toolDenylist] }),
  };
}
