...
function observeTurnEvent(
  ...
  observation.stopReason = typeof message.stopReason === "string"
    ? message.stopReason
    : undefined;
  observation.errorMessage = typeof message.errorMessage === "string"
...
function turnOutcome(
  ...
  if (
    observation.stopReason === "error" || observation.stopReason === "aborted"
  ) {
    return {
      status: "error",
      failure: {
        ...
        message: observation.errorMessage ??
          `child turn ended with stopReason "${observation.stopReason}"`,
      },
    };
  }
  if (observation.text !== undefined) {
    return { status: "completed", finalResponse: observation.text };
  }
  const final = session.getLastAssistantText();
  if (
    final !== undefined && final.length > 0 && final !== observation.baseline
  ) {
  ...
  return {
    ...
    failure: {
      kind: "child-turn-failed",
      message: observation.errorMessage ??
        "child turn produced no assistant output",
    },
  };
}
...
function settledSessionOutcome(session: ChildSession): RunnerOutcome {
  ...
  return {
    ...
    failure: {
      kind: "child-turn-failed",
      message: "restored session has no assistant output",
    },
  };
}
...
function createTrackedChildHandle(
  ...
  let running: Promise<RunnerOutcome> = Promise.resolve(
    settledSessionOutcome(session),
  );
...
export function createRestoredChildHandle(
  input: CreateRestoredChildHandleInput,
): ChildHandle {
