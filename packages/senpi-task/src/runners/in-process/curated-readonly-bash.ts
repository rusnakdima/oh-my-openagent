...
export const CuratedReadonlyBashParams = Type.Object({
  ...
  args: Type.Array(Type.String(), {
    description:
      "Argument vector. Shell syntax, writes, uploads, and mutation-capable flags are rejected.",
    minItems: 1,
    maxItems: 64,
  }),
...
const GH_SEARCH_KINDS: ReadonlySet<string> = new Set([
  "code",
  "commits",
  "issues",
  "prs",
...
const GH_SEARCH_BOOLEAN_FLAGS: ReadonlySet<string> = new Set([
  "--archived",
  "--include-forks",
]);
...
const GH_VIEW_VALUE_FLAGS: ReadonlySet<string> = new Set([
  "--json",
  "--jq",
  "--repo",
  "--template",
]);
const GH_API_BOOLEAN_FLAGS: ReadonlySet<string> = new Set([
  "--include",
  "--paginate",
  "--slurp",
]);
...
export function planCuratedReadonlyCommand(
  request: CuratedReadonlyRequest,
): CuratedReadonlyCommand {
  const args = [...request.args];
  if (request.program === "curl" && isReadonlyCurl(args)) {
    return {
      program: "curl",
      args: args[0] === "--version" ? args : ["--disable", ...args],
    };
  }
  if (request.program === "gh" && isReadonlyGitHub(args)) {
    return { program: "gh", args };
  }
  throw new CuratedReadonlyCommandError(
...
export function createCuratedReadonlyBashTool(
  ...
  return defineTool({
    ...
    description:
      "Run a structured read-only gh or curl request directly, without a shell or filesystem-writing flags.",
    promptSnippet:
      "Structured read-only remote research through gh or curl; arbitrary shell commands are unavailable.",
    ...
    execute: async (_toolCallId, input, signal) => {
      ...
      try {
        ...
        return {
          content: [{ type: "text", text: capResultText(text) }],
          details: undefined,
        };
      } catch (error) {
        if (!(error instanceof CuratedReadonlyCommandError)) throw error;
        throw new CuratedReadonlyCommandError(capResultText(error.message), {
          cause: error,
        });
      }
    },
...
function capResultText(text: string): string {
  const boundary = truncateHead(text, {
  ...
  for (let pass = 0; pass < 4; pass += 1) {
    ...
    content = truncation.firstLineExceedsLimit
      ? truncateUtf8Prefix(text, contentBytes)
      : truncation.content;
    const nextNotice = buildTruncationNotice(
      boundary.truncatedBy,
      content,
      boundary.totalLines,
      boundary.totalBytes,
...
function isReadonlyCurl(args: readonly string[]): boolean {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (!CURL_VALUE_FLAGS.has(arg) || args[index + 1] === undefined) {
      return false;
    }
    index += 1;
  }
...
function isReadonlyGitHub(args: readonly string[]): boolean {
  ...
  if (
    command === "search" && subcommand !== undefined &&
    GH_SEARCH_KINDS.has(subcommand)
  ) {
    return flagsAndPositionalsAreSafe(
      rest,
      GH_SEARCH_BOOLEAN_FLAGS,
      GH_SEARCH_VALUE_FLAGS,
      1,
    );
  }
  ...
  if ((command === "issue" || command === "pr") && subcommand === "view") {
    return flagsAndPositionalsAreSafe(
      rest,
      GH_VIEW_BOOLEAN_FLAGS,
      GH_VIEW_VALUE_FLAGS,
      1,
    );
  }
  if (
    command === "release" && (subcommand === "view" || subcommand === "list")
  ) {
    return flagsAndPositionalsAreSafe(rest, new Set(), GH_VIEW_VALUE_FLAGS, 1);
  }
  if (
    command !== "api" || subcommand === undefined || subcommand === "graphql" ||
    subcommand.startsWith("-")
  ) return false;
  return flagsAndPositionalsAreSafe(
    rest,
    GH_API_BOOLEAN_FLAGS,
    GH_API_VALUE_FLAGS,
    0,
  );
}
...
function executeCommand(
  ...
  return new Promise((resolve, reject) => {
    execFile(command.program, command.args, {
      ...
      if (error !== null) {
        reject(
          new CuratedReadonlyCommandError(
            `Read-only ${command.program} request failed: ${
