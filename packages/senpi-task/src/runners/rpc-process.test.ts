...
function makeRunner(
  extra: { heartbeatIntervalMs?: number; now?: () => number } = {},
): {
...
describe("RpcProcessRunner", () => {
  ...
  test("#given a completing child #when started #then the handle reports final text and a clean exit", async () => {
    ...
    const handle = await runner.start(
      makeSpec({ prompt: "finish:final answer" }),
    );
    ...
    expect(mapExitOutcomeToError(outcome, { alreadyTerminal: true }))
      .toBeNull();
  });
  ...
  test("#given a busy child #when steered #then the steer is acked while the turn is in flight", async () => {
    ...
    expect(queued && "steering" in queued ? queued.steering : []).toContain(
      "mid-course",
    );
  });
  ...
  test("#given a busy child #when followUp is sent #then it is routed as a prompt with followUp streaming behavior", async () => {
    ...
    await waitFor(() =>
      events.some((e) =>
        e.type === "queue_update" && "followUp" in e && e.followUp.length > 0
      )
    );
    const queued = events.find((e) =>
      e.type === "queue_update" && "followUp" in e && e.followUp.length > 0
    );
    expect(queued && "followUp" in queued ? queued.followUp : []).toContain(
      "later note",
    );
  });
  ...
  test("#given a child that exits nonzero before terminal #when it crashes #then the outcome carries the stderr tail", async () => {
    ...
    const start = runner.start(
      makeSpec({ prompt: "crash:4:boom stderr detail" }),
    );
    ...
    await Promise.race([
      ...
      new Promise<never>((_, reject) => {
        const timeout = setTimeout(
          () => reject(new Error("timed out waiting for heartbeat update")),
          2_000,
        );
        timeout.unref?.();
      }),
  ...
  test("#given a spawn #when the descriptor is built #then the child gets an isolated session dir, not the real HOME", async () => {
    ...
    expect(
      sessionDir.startsWith(join(spec.state_dir, "sessions", spec.task_id)),
    ).toBe(true);
  ...
  test("#given prior assistant text #when a revived turn produces no output #then the stale text cannot become a fresh success", async () => {
    ...
    const { runner } = makeRunner();
    const handle = await runner.start(makeSpec({ prompt: "first" }));
    if (handle.waitForOutcome === undefined) {
      throw new Error("waitForOutcome was not exposed");
    }
    expect(await handle.waitForOutcome()).toEqual({
      status: "completed",
      finalResponse: "first",
    });
    ...
    expect(outcome).toMatchObject({
      ...
      failure: {
        ...
        message: "RPC child turn produced no assistant output",
      },
    });
    expect(handle.lastAssistantText()).toBe("first");
  });
