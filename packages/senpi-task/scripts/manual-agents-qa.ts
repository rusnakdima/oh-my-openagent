...
if (evidenceDir === undefined) {
  throw new Error(
    "Usage: bun packages/senpi-task/scripts/manual-agents-qa.ts <evidence-dir>",
  );
}
...
const configuredEscapedAgentPath = join(
...
const brokenDirectorySymlink = join(
  projectDir,
  ".pi",
  "agents",
  "nested",
...
symlinkSync(
  join(projectDir, "missing-directory"),
...
if (finder.model !== "omo-override") {
  throw new Error("omo.json model override did not win");
}
if (finder.models?.[0] !== "file-primary") {
  throw new Error("finder models fallback list did not load");
}
if (resolveToolRule(finder.tools ?? [], "read") !== true) {
  throw new Error("finder read tool allow did not load");
}
const brokenDiagnostic = loaded.diagnostics.find((diagnostic) =>
  diagnostic.path === brokenPath
);
if (brokenDiagnostic?.kind !== "frontmatter") {
  throw new Error("broken frontmatter diagnostic missing");
}
if (loaded.agents.linked !== undefined) {
  throw new Error("symlinked external agent loaded");
}
const symlinkDiagnostic = loaded.diagnostics.find((diagnostic) =>
  diagnostic.path === symlinkedScanRoot
);
if (symlinkDiagnostic?.kind !== "read") {
  throw new Error("symlinked scan root diagnostic missing");
}
if (loaded.agents.escaped !== undefined) {
  throw new Error("symlinked configured-location external agent loaded");
}
const configuredSymlinkDiagnostic = loaded.diagnostics.find((diagnostic) =>
  diagnostic.path === symlinkedConfiguredRoot
);
if (configuredSymlinkDiagnostic?.kind !== "read") {
  throw new Error("symlinked configured-location diagnostic missing");
}
const brokenDirectoryDiagnostic = loaded.diagnostics.find((diagnostic) =>
  diagnostic.path === brokenDirectorySymlink
);
if (brokenDirectoryDiagnostic?.kind !== "read") {
  throw new Error("broken directory symlink diagnostic missing");
}
const userConfigDiagnostic = loaded.diagnostics.find((diagnostic) =>
  diagnostic.path === userConfigPath
);
if (userConfigDiagnostic?.kind !== "read") {
  throw new Error("unreadable user config diagnostic missing");
}
...
console.log(
  JSON.stringify(
    { ...summary, fixtureExistsAfterCleanup: existsSync(fixtureRoot) },
    null,
    2,
  ),
);
