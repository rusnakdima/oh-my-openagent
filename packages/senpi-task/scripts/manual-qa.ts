...
if (evidenceDir === undefined) {
  throw new Error(
    "Usage: bun packages/senpi-task/scripts/manual-qa.ts <evidence-dir>",
  );
}
...
if (!isDeepStrictEqual(reloaded, completed)) {
  throw new Error("Reloaded task facts changed");
}
...
if (!eventLog.includes('"apiKey":"[REDACTED]"')) {
  throw new Error("Redacted apiKey marker missing");
}
if (eventLog.includes("manual-redaction-sentinel")) {
  throw new Error("Sentinel leaked to event log");
}
...
if (listed.records.some((entry) => entry.task_id === "st_c0ffee00")) {
  throw new Error("Corrupt task was loaded");
}
const corruptDiagnostic = listed.diagnostics.find((entry) =>
  entry.path === corruptPath
);
if (corruptDiagnostic?.type !== "parse_error") {
  throw new Error("Corrupt task diagnostic missing");
}
if (listed.records.some((entry) => entry.task_id === "st_bad00004")) {
  throw new Error("Malformed optional task was loaded");
}
const malformedOptionalDiagnostic = listed.diagnostics.find((entry) =>
  entry.message === "pid is not a number"
);
...
if (rejected.applied) {
  throw new Error("Illegal completed to running transition applied");
}
if (store.load(completed.task_id)?.status !== "completed") {
  throw new Error("Completed status changed");
}
...
if (!evictedCompleted.applied) {
  throw new Error("Completed resident eviction was rejected");
}
if (evictedCompleted.record.status !== "completed") {
  throw new Error("Completed eviction changed lifecycle status");
}
if (
  messageability(
...
if (normalLost.record.status !== "running") {
  throw new Error("Rejected normal lost transition changed status");
}

const outsidePath = join(evidenceDir, "manual-outside.jsonl");
let traversalRejected = false;
try {
  store.appendEvent("../../../../manual-outside", {
    type: "manual_probe",
    payload: {},
  });
} catch (error) {
...
if (existsSync(outsidePath)) {
  throw new Error("Traversal attempt wrote outside state dir");
}
...
const summary = {
  ...
  completedResidentMessageability: messageability(
    completed.status,
    completed.residency_state,
  ),
