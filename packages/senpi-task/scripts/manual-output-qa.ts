...
} from "../src/manager/transcript-log";
import { createTaskOutputTool } from "../src/tools/output";
import type { ListedTask, ListScope, OutputManager } from "../src/index";
  ...
  return {
    ...
    list(scope: ListScope): readonly ListedTask[] {
      const filtered = scope.scope === "all"
        ? records
        : records.filter((r) => r.parent_session_id === scope.session_id);
...
async function main(): Promise<void> {
  ...
  store.appendEvent(done.task_id, {
    type: TRANSCRIPT_ASSISTANT_EVENT,
    payload: { text: "reading the codebase" },
  });
  store.appendEvent(done.task_id, {
    type: TRANSCRIPT_TOOL_EVENT,
    payload: { tool: "grep", is_error: false },
  });
  store.appendEvent(done.task_id, {
    type: TRANSCRIPT_ASSISTANT_EVENT,
    payload: { text: "final: the report is written" },
  });
  ...
  const output = createTaskOutputTool({
    manager: managerFrom([done, lost]),
    stateDir,
    waitConfig,
  });

  const happy = await output.execute(
    "call",
    { task_id: "st_0000abcd", mode: "tail" },
    new AbortController().signal,
    undefined,
    ctx("session-live"),
  );
  const lostView = await output.execute(
    "call",
    { name: "ghost", mode: "tail" },
    new AbortController().signal,
    undefined,
    ctx("session-live"),
  );
  const crossSession = await output.execute(
    "call",
    { task_id: "st_0000abcd" },
    new AbortController().signal,
    undefined,
    ctx("session-intruder"),
  );
  ...
  console.log(
    ...
    happy.details.kind,
    "| source:",
    happy.details.kind === "transcript" ? happy.details.source : "-",
  );
  console.log("\n=== LOST: task_output tail on lost 'ghost' (no throw) ===");
  console.log("details.kind:", lostView.details.kind);
  console.log(
    JSON.stringify(
      lostView.details.kind === "status"
        ? lostView.details.snapshot.lost
        : lostView.details,
      null,
      2,
    ),
