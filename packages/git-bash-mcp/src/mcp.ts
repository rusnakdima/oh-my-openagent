...
import {
  type GitBashRunResult,
  type RunGitBashCommand,
  runGitBashCommand,
} from "./runner";
...
async function callTool(
  ...
  if (name === "diagnose") {
    return toolResponse(
      id,
      diagnosePayload(resolve(options), platformFromOptions(options)),
    );
  }
      ...
      inputSchema: {
        ...
        properties: {
          ...
          timeout: {
            ...
            description:
              `Optional timeout in milliseconds. If omitted, uses the inherited exec_command timeout when configured; otherwise ${
                defaultTimeoutMs(options)
              }ms.`,
          },
          workdir: {
            type: "string",
...
function diagnosePayload(
  ...
  const payload = {
    ...
    status: platform === "win32"
      ? (enabled ? "ready" : "missing-git-bash")
...
function protocolVersionFromInput(
