...
} from "./installed-plugin-database";
...
export function discoverInstalledPlugins(
  ...
  const pluginManifestLoader = options?.loadPluginManifestOverride ??
    loadPluginManifest;
  ...
  for (const [pluginKey, installation] of extractPluginEntries(db)) {
    ...
    const installPath = resolveActualInstallPath(
    ...
    log(
      `Discovered plugin: ${loadedPlugin.name}@${installation.version} (${installation.scope})`,
      {
        installPath,
