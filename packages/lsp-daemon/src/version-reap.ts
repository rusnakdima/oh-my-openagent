...
export async function reapStaleDaemonVersions(
  ...
  const attest = deps.attest ??
    ((pid: number) => attestDaemonCliProcess(pid, platform));
  ...
  for (
    ...
    const siblingPaths = daemonPaths({ [OMO_LSP_DAEMON_DIR]: baseDir }, {
      cliPath: ownPaths.cliPath,
      version,
    });
    results.push(
...
async function reapOneVersion(
  context: ReapOneContext,
): Promise<VersionReapResult> {
  ...
  if (!owner) {
    ...
    if (lockPid !== null && context.isAlive(lockPid)) {
      ...
      return {
        version,
        status: "spared",
        reason: `owner metadata missing but lock held by live pid ${lockPid}`,
      };
    }
    ...
    return {
      version,
      status: "removed",
      reason: "removed stale version dir without readable owner metadata",
    };
  }
  ...
  if (!context.isAlive(owner.pid)) {
    ...
    return {
      version,
      status: "removed",
      reason: `removed stale version dir for dead owner pid ${owner.pid}`,
    };
  }
...
async function terminateAttestedOwner(
  context: ReapOneContext,
  pid: number,
): Promise<VersionReapResult> {
  const { version, versionDir } = context;
  if (!context.sendSignal(pid, "SIGTERM")) {
    ...
    return {
      ...
      status: "removed",
      reason:
        `owner pid ${pid} exited before SIGTERM; removed stale version dir`,
    };
  }
  if (await context.waitForExit(pid, context.termGraceMs)) {
    ...
    return {
      version,
      status: "terminated",
      reason: `terminated attested older daemon pid ${pid} with SIGTERM`,
    };
  }
  context.log(
  ...
  if (
    !context.sendSignal(pid, "SIGKILL") ||
    !(await context.waitForExit(pid, context.killGraceMs))
  ) {
    context.log(`reap: deferring v${version}: pid ${pid} survived SIGKILL`);
    return {
      version,
      status: "deferred",
      reason: `attested daemon pid ${pid} survived SIGKILL; dir kept`,
    };
  }
  await context.removeDir(versionDir);
  return {
    ...
    status: "terminated",
    reason:
      `terminated attested older daemon pid ${pid} after SIGKILL escalation`,
  };
}
