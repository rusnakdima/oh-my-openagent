...
export async function callToolViaDaemon(
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    try {
      ...
      const sendOptions = options.signal === undefined
        ? { timeoutMs }
      ...
      lastError = error;
      if (
        error instanceof DaemonAuthenticationRejectedError && !authRefreshUsed
      ) {
      ...
      if (
        ...
        (error.requestWritten || !isRetryableTool(name))
      ) break;
    }
  }
...
function ensureDaemonAvailable(
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    const onAbort = (): void =>
      finish(() => reject(new DaemonRequestCancelledError(false)));
    if (signal.aborted) {
      onAbort();
      return;
    }
    ...
    Promise.resolve()
      ...
      .then(
        ...
        (error: unknown) =>
          finish(() =>
            reject(
              signal.aborted ? new DaemonRequestCancelledError(false) : error,
            )
          ),
      );
  });
...
function sendToolCall(
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      finish(() =>
        reject(
          new DaemonRequestTimedOutError(requestWritten, options.timeoutMs),
        )
      );
    }, options.timeoutMs);
    timer.unref();
    ...
    const decoder = createLineDecoder((message) => {
      ...
      if (result) finish(() => resolve(result));
          ...
          reject(
            new DaemonRequestError("invalid daemon response", requestWritten),
          )
        );}
    });
    socket.once("connect", () => {
    ...
    socket.once(
      ...
      (error) =>
        finish(() =>
          reject(new DaemonRequestError(error.message, requestWritten))
        ),
    );
    socket.once(
      "close",
      () =>
        finish(() =>
          reject(
            new DaemonRequestError("daemon connection closed", requestWritten),
          )
        ),
    );
  });
