...
export type PrivateDirectoryStats = Pick<
  Stats,
  "dev" | "ino" | "uid" | "isDirectory" | "isSymbolicLink"
>;
...
): AuthenticatedMessage | JsonRpcErrorResponse {
...
export function isAuthErrorResponse(message: unknown): boolean {
  ...
  return error["code"] === AUTH_ERROR_CODE && isPlainRecord(data) &&
...
function validatePrivateDirectory(
  path: string,
  options: PrivateDirectoryOptions,
): PrivateDirectoryStats {
  const stats = options.lstat
    ? options.lstat(path)
    : lstatPrivateDirectory(path);
...
function sameDirectory(
  a: PrivateDirectoryStats,
  b: PrivateDirectoryStats,
): boolean {
