    ...
    socket.once("connect", () => {
      socket.write(
        encodeJsonLine({
          ...
          id: 1,
          method: "omo/ping",
          params: { _omo: authEnvelope(token) },
        }),
      );
    });
...
export function spawnDaemonProcess(paths: DaemonPaths): void {
  ...
  try {
    const child = spawn(resolveDaemonNodeExecutable(), [
      paths.cliPath,
      "daemon",
    ], {
      detached: true,
    ...
    child.once("error", (error) => {
      writeSync(
        logFd,
        `[lsp-daemon] failed to spawn daemon: ${error.message}\n`,
      );
      closeSync(logFd);
    });
    child.unref();
...
function abortError(signal: AbortSignal): Error {
  ...
  const error = new Error(
    typeof reason === "string" ? reason : "daemon startup cancelled",
  );
