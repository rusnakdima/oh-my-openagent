...
import {
  ...
  getLspManager,
} from "@oh-my-opencode/lsp-core/lsp/manager";
...
export async function startDaemonServer(
  ...
  const idleCheckIntervalMs = options.idleCheckIntervalMs ??
    DEFAULT_IDLE_CHECK_INTERVAL_MS;

  const lease = await acquireStartupLease(
    paths,
    (token) => pingDaemon(paths, token),
  );
  ...
  void reapStaleDaemonVersions(paths).catch((error: unknown) =>
...
async function assertSelfProbe(
  paths: DaemonPaths,
  token: string,
  owner: DaemonOwner,
): Promise<void> {
