...
export function resolveDaemonVersion(
  requireFn: (id: string) => unknown = requireFromHere,
): string {
...
export function daemonBaseDir(
  ...
  return platform.path.resolve(
    platform.path.join(platform.homedir(), ".omo", "lsp-daemon"),
  );
}
...
export function daemonPaths(
  ...
  const dir = platform.path.resolve(
    platform.path.join(baseDir, `v${runtime.version}`),
  );
  return {
...
function resolveSocketPath(
  ...
  if (platform.platform === "win32") {
    const currentUserDiscriminator = `${
      platform.getuid() ?? "win"
    }:${platform.username()}:${platform.path.resolve(platform.homedir())}`;
    const digest = shortDigest(
      `${canonicalVersionDir}\0${currentUserDiscriminator}`,
    );
  ...
  return platform.path.join(
    platform.tmpdir(),
    `omo-lsp-${version}-${shortDigest(canonicalVersionDir)}`,
    "daemon.sock",
  );
}
