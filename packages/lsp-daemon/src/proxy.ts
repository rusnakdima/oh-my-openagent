...
} from "@oh-my-opencode/lsp-core/mcp";
import {
  createStandaloneMcpRequestContext,
  runWithRequestContext,
} from "@oh-my-opencode/lsp-core/request-context";
import {
  jsonRpcId,
...
export async function runMcpStdioProxy(
  ...
  if (startupTimeoutMs > 0) {
    startupTimer = setTimeout(() => {
      ...
      try {
        stderr.write(
          `[lsp-daemon] no MCP request received within ${startupTimeoutMs}ms; exiting\n`,
        );
      } catch (error) {
        if (stderr !== process.stderr) {
          process.stderr.write(
            `[lsp-daemon] startup diagnostic failed: ${
  ...
  try {
    ...
    const cwd = options.cwd ??
      inferOpenCodeProjectCwd(env["LSP_TOOLS_MCP_PROJECT_CONFIG"]);
    ...
    const context = options.context ??
      createStandaloneMcpRequestContext(contextInput);
    ...
    await runJsonRpcStdioServer({
      ...
      handler: (request, requestOptions) => {
        ...
        return runWithRequestContext(
          context,
          () => handleProxyRequest(request, requestOptions),
        );
      },
      handlerOptions: callOptions,
      onHandlerError: (error: unknown) => {
        stderr.write(
          `[lsp-daemon] proxy error: ${
            error instanceof Error ? error.message : String(error)
          }\n`,
        );
      },
...
function inferOpenCodeProjectCwd(
  projectConfigEnv: string | undefined,
): string | undefined {
...
function canonicalizeContextEnv(
  env: Record<string, string | undefined>,
): Record<string, string | undefined> {
  return {
    ...env,
    LSP_TOOLS_MCP_PROJECT_CONFIG: canonicalizePathList(
      env["LSP_TOOLS_MCP_PROJECT_CONFIG"],
    ),
    LSP_TOOLS_MCP_USER_CONFIG: canonicalizePath(
      env["LSP_TOOLS_MCP_USER_CONFIG"],
    ),
    LSP_TOOLS_MCP_INSTALL_DECISIONS: canonicalizePath(
