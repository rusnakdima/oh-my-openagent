...
export function removeDaemonMetadataForOwner(
  paths: DaemonPaths,
  owner: DaemonOwner,
): void {
...
async function validateExistingOwner(
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    if (
      ping && owner.nonce === ping.nonce &&
      sameEndpoint(owner.endpoint, ping.endpoint)
    ) {
      throw new DaemonAlreadyRunningError("LSP daemon already running");
    }
    if (ping) continue;
    ...
    if (
      !reread || reread.nonce !== owner.nonce ||
      !sameEndpoint(endpoint, owner.endpoint)
    ) {
...
function parseOwner(value: unknown): DaemonOwner | null {
  ...
  if (
    typeof pid !== "number" || typeof nonce !== "string" ||
    typeof startedAt !== "string" || !endpoint
  ) return null;
