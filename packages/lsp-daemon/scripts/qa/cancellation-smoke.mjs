...
const { callToolViaDaemon } = await import(
  pathToFileURL(join(root, "packages/lsp-daemon/src/daemon-client.ts")).href
);
...
const { handleDaemonMessage } = await import(
  pathToFileURL(join(root, "packages/lsp-daemon/src/request-routing.ts")).href
);
...
const { JsonRpcConnection } = await import(
  pathToFileURL(join(root, "packages/lsp-core/src/lsp/json-rpc-connection.ts"))
    .href
);
const { disposeDefaultLspManager } = await import(
  pathToFileURL(join(root, "packages/lsp-core/src/lsp/manager.ts")).href
);
...
try {
  ...
  const resultPromise = callToolViaDaemon(
    ...
    { filePath: source, severity: "error" },
    {
      ...
      context: {
        ...
        projectConfigPaths: [
          join(canonicalWorkspace, ".codex", "lsp-client.json"),
        ],
        userConfigPath,
  ...
  if (first.kind !== "lsp-request") {
    throw new Error(
      `Daemon call completed before fake LSP diagnostic request: ${
        JSON.stringify({
          result: first.result,
          daemonMessages,
  ...
  const lspCancel = await waitForEvent(
    lspEvents,
    (event) =>
      event.type === "clientNotification" && event.method === "$/cancelRequest",
  );
  const lateResponse = await waitForEvent(
    lspEvents,
    (event) =>
      event.type === "serverLateResponse" && event.id === lspRequest.id,
  );
  await waitUntil(
    () => activeRequests.size === 0,
    "daemon active request cleanup",
  );

  const daemonRequest = daemonMessages.find((message) =>
    message.method === "tools/call"
  );
  const daemonCancel = daemonMessages.find((message) =>
    message.method === "$/cancelRequest"
  );
  assert(daemonRequest?.id !== undefined, "daemon request id missing");
  assert(
    daemonCancel?.params?.id === daemonRequest.id,
    "daemon cancel id did not match proxy request id",
  );
  assert(
    lspCancel.params?.id === lspRequest.id,
    "LSP cancel id did not match LSP request id",
  );
  assert(
    result.isError === true,
    "daemon call did not report cancellation as an error result",
  );
  assert(activeRequests.size === 0, "daemon active request map leaked");

  const direct = await directJsonRpcCancellationCheck();
  const daemonEndpointKind = paths.socket.startsWith("\\\\.\\pipe\\")
    ? "named-pipe"
    : "unix-socket";
  assert(
    daemonEndpointKind ===
