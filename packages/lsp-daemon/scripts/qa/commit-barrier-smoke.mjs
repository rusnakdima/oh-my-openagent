...
const {
  ...
  pathToFileURL(
    join(
      root,
      "packages/lsp-core/src/lsp/workspace-apply-edit-test-support.ts",
    ),
  ).href
);
...
try {
  ...
  const preRename = pre.client.rename(
    pre.source,
    1,
    6,
    "after",
  ...
  assert(
    preResult instanceof Error && /cancel/i.test(preResult.message),
    "pre-gate rename did not cancel",
  );
  ...
  const postResult = await post.client.rename(
    post.source,
    1,
    6,
    "after",
  ...
  assert(
    postResult.apply.success === true,
    "post-gate rename did not complete",
  );
  assert(
    postResult.apply.lateAbort === true,
    "post-gate abort was not reported as late",
  );
  assert(
    postResult.apply.totalEdits === 1,
    "post-gate rename did not report exactly one edit",
  );
  assert(
    writeCount === 1,
    "post-gate rename did not perform exactly one write",
  );
