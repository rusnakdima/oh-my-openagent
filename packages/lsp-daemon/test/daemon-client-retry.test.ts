    ...
    waitForCount(count: number): Promise<void> {
      ...
      return new Promise((resolve, reject) => {
        const timer = setTimeout(() => {
          reject(
            new Error(
              `waited for ${count} daemon messages, observed ${messages.length}`,
            ),
          );
        }, 1_000);
        const wake = (): void => {
...
function recordMessages(
  socket: Socket,
  recorder: ReturnType<typeof createMessageRecorder>,
): void {
...
describe("daemon-client retry discipline", () => {
  ...
  it("#given a daemon request timeout #when request bytes were written #then an authenticated cancel is sent for the proxy id before close", async () => {
    ...
    const request = recorder.messages.find((message) =>
      jsonRpcMethod(message) === "tools/call"
    );
    const cancel = recorder.messages.find((message) =>
      jsonRpcMethod(message) === "$/cancelRequest"
    );
    expect(result.isError).toBe(true);
...
function extractToken(message: unknown): string | null {
  ...
  return Reflect.get(omo, "protocolVersion") === expected.protocolVersion &&
      typeof token === "string"
    ? token
    : null;
}
