...
describe("public client package surface", () => {
  ...
  it("#given omitted context #when calling the public client #then rejection happens before daemon dispatch", async () => {
    ...
    const paths = daemonTestPaths(
      fileURLToPath(new URL("client-surface-daemon", import.meta.url)),
    );

    await expect(
      Reflect.apply(callToolViaDaemon, undefined, ["status", {}, {
        paths,
        ensure,
      }]),
    ).rejects.toThrow(
  ...
  it.each([
    ...
    async ({ name, context }) => {
      ...
      const paths = daemonTestPaths(
        fileURLToPath(
          new URL(`client-surface-malformed-${name}`, import.meta.url),
        ),
      );
