...
describe("immutable legacy daemon path vectors", () => {
  it("#given the captured Unix vectors #when checking the pre-migration digest contract #then they retain exact natural and hashed paths", () => {
    ...
    // when
    const hashedDigest = createHash("sha256").update(hashed.expected.versionDir)
      .digest("hex").slice(0, 16);

    // then
    expect(natural.expected.socket).toBe(
      posix.join(natural.expected.versionDir, "daemon.sock"),
    );
    expect(hashed.expected.versionDir.length).toBeGreaterThanOrEqual(100);
    expect(hashed.expected.socket).toBe(
      posix.join(
        hashed.inputs.tmpdir,
        `omo-lsp-${vectors.version}-${hashedDigest}.sock`,
      ),
    );
  });
  ...
  it("#given the captured Windows vector #when checking the pre-migration digest contract #then it retains the exact named pipe", () => {
    ...
    // when
    const digest = createHash("sha256").update(windows.expected.versionDir)
      .digest("hex").slice(0, 16);

    // then
    expect(windows.expected.versionDir).toBe(
      win32.join(windows.expected.baseDir, `v${vectors.version}`),
    );
    expect(windows.expected.socket).toBe(
      `\\\\.\\pipe\\omo-lsp-${vectors.version}-${digest}`,
    );
  });
