...
const DEFAULT_RUNTIME = {
  cliPath: "/packaged/cli.js",
  version: "1.2.3",
} as const;

function unixPlatform(
  home: string = "/Users/qa",
  temp: string = "/tmp",
): DaemonPlatform {
  return {
    platform: "linux",
...
describe("OMO daemon environment contract", () => {
  it("#given the public environment names #when enumerated #then exactly the three approved OMO variables exist", () => {
    expect(
      [OMO_LSP_DAEMON_DIR, OMO_LSP_DAEMON_CLI, OMO_LSP_DAEMON_VERSION].sort(),
    ).toEqual([
      "OMO_LSP_DAEMON_CLI",
      "OMO_LSP_DAEMON_DIR",
      "OMO_LSP_DAEMON_VERSION",
  ...
  it("#given an absolute directory override with parent segments #when resolving the base #then it is normalized", () => {
    expect(
      daemonBaseDir(
        { [OMO_LSP_DAEMON_DIR]: "/tmp/omo/../shared" },
        unixPlatform(),
      ),
    ).toBe("/tmp/shared");
  });
  ...
  it("#given a relative directory override #when resolving the base #then it is rejected", () => {
    expect(() =>
      daemonBaseDir({ [OMO_LSP_DAEMON_DIR]: "relative/state" }, unixPlatform())
    ).toThrow(
      InvalidDaemonDirectoryError,
    );
  });
...
describe("daemon version grammar", () => {
  it.each([
    ...
    "#given valid version %s #when validating #then it is preserved",
    (version) => {
  ...
  it.each([
    ...
    "#given invalid version %s #when validating #then it is rejected",
    (version) => {
      expect(() => validateDaemonVersion(version)).toThrow(
        InvalidDaemonVersionError,
      );
    },
  );
...
describe("daemon path derivation", () => {
  ...
  it("#given a long Unix version directory #when deriving the socket #then the historical short hash fallback remains", () => {
    ...
    const paths = daemonPaths(
      { [OMO_LSP_DAEMON_DIR]: base },
      DEFAULT_RUNTIME,
      unixPlatform(),
    );
    expect(paths.socket).toBe(
      "/tmp/omo-lsp-1.2.3-b174cf2e082d87e3/daemon.sock",
    );
  ...
  it("#given deterministic Windows identity helpers #when deriving the pipe #then the digest binds version directory and user", () => {
    ...
    const second = daemonPaths(
      env,
      DEFAULT_RUNTIME,
      windowsPlatform("other-user"),
    );
...
describe("paired daemon runtime overrides", () => {
  ...
  it.each([
    ...
    "#given $name #when resolving runtime #then invalid_runtime_override is thrown",
    ({ env }) => {
      try {
        ...
        if (error instanceof InvalidRuntimeOverrideError) {
          expect(error.code).toBe("invalid_runtime_override");
        }
      }
    },
  );
  ...
  it.each([
    ...
    {
      name: "missing",
      makePath: () => join(tmpdir(), "missing-omo-lsp-cli.js"),
    },
    { name: "directory", makePath: () => createCliFixture("directory") },
  ])(
    "#given a $name CLI override #when resolving runtime #then it is rejected",
    ({ makePath }) => {
      expect(() =>
        resolveDaemonRuntime({
          [OMO_LSP_DAEMON_CLI]: makePath(),
