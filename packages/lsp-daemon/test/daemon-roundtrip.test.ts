...
describe("daemon roundtrip", () => {
  ...
  it("#given the daemon is unreachable #when a tool is called #then it returns a structured error without running locally", async () => {
    const paths = tempPaths();
    const failingEnsure = (): Promise<void> =>
      Promise.reject(new Error("spawn disabled in test"));
  ...
  it("#given project config env in the request context #when a tool runs on the daemon #then config-loader honors it per-request", async () => {
    ...
    const baseline = await callToolViaDaemon("status", {}, {
      paths,
      ensure: noSpawn,
      context: defaultContext(),
    });
    ...
    writeFileSync(
      configPath,
      JSON.stringify({ lsp: { [builtinId]: { disabled: true } } }),
    );
    ...
    const unscoped = await callToolViaDaemon("status", {}, {
      paths,
      ensure: noSpawn,
      context: defaultContext(),
    });
    expect(unscoped.content[0]?.text ?? "").toContain(`- ${builtinId}: `);
    expect(unscoped.content[0]?.text ?? "").not.toContain(
      `- ${builtinId}: disabled`,
    );
  });
