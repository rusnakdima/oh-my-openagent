    ...
    socket.once("connect", () => {
      socket.write(
        encodeJsonLine({
          ...
          method: "tools/call",
          params: {
            _omo: { protocolVersion: 1, token },
            name: "status",
            arguments: { _context: existingContext(paths.dir) },
          },
        }),
      );
    });
...
describe("daemon IPC authentication and ownership", () => {
  it("given a running daemon when pinging and calling then auth is required and tokens are never forwarded", async () => {
    ...
    expect((missingAuth["error"] as { data?: { code?: string } }).data?.code)
      .toBe("daemon_authentication_failed");
  ...
  it("given an authenticated request with the wrong protocol when routed then it is rejected before dispatch", async () => {
    ...
    expect((response["error"] as { data?: { code?: string } }).data?.code).toBe(
  ...
  it("given two authenticated contexts when routed through one daemon then both existing cwd scopes are honored", async () => {
    ...
    const server = await startDaemonServer(paths, { onIdleShutdown: () => {} });
    openServers.push(server);
    const firstRoot = realpathSync(
      mkdtempSync(join(tmpdir(), "lsp-context-a-")),
    );
    const secondRoot = realpathSync(
      mkdtempSync(join(tmpdir(), "lsp-context-b-")),
    );
    tempDirectories.push(firstRoot, secondRoot);

    const first = await callToolViaDaemon("status", {}, {
      paths,
      ensure: async () => {},
      context: existingContext(firstRoot),
    });
    const second = await callToolViaDaemon("status", {}, {
      paths,
      ensure: async () => {},
      context: existingContext(secondRoot),
    });
  ...
  it("given a pre-created hashed fallback endpoint directory symlink when daemon starts then startup fails before binding", async () => {
    ...
    const hostileTarget = mkdtempSync(
      join(tmpdir(), "lsp-daemon-hostile-target-"),
    );
    tempDirectories.push(hostileTarget);
    mkdirSync(dirname(endpointDir), { recursive: true });
    symlinkSync(hostileTarget, endpointDir, "dir");

    await expect(startDaemonServer(paths, { onIdleShutdown: () => {} })).rejects
      .toMatchObject({
