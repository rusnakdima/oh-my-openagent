...
describe("LSP QA driver portability", () => {
  ...
  it("#given the native platform #when the cancellation smoke runs #then it binds the production endpoint kind", () => {
    const output = execFileSync("bun", [
      join(repositoryRoot, cancellationSmoke),
      repositoryRoot,
    ], {
      cwd: repositoryRoot,
      encoding: "utf8",
    ...
    const expectedEndpointKind = process.platform === "win32"
      ? "named-pipe"
      : "unix-socket";

    expect(JSON.parse(output)).toMatchObject({
      daemonEndpointKind: expectedEndpointKind,
    });
  });
    ...
    >(
      (resolve, reject) => {
        ...
        const timeout = setTimeout(() => {
          ...
          reject(
            new Error(
              "cancellation smoke did not settle after its output reader closed",
            ),
          );
        }, 10_000);
