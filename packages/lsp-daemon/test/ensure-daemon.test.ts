...
function makeHarness(
  config: { probeQueue: boolean[]; onSpawnPush?: boolean[] },
): Harness {
...
describe("ensureDaemonRunning", () => {
  ...
  it("#given spawn never becomes reachable #when ensure #then throws", async () => {
    ...
    await expect(
      ensureDaemonRunning(PATHS, deps, {
        readyTimeoutMs: 300,
        pollIntervalMs: 100,
      }),
  ...
  it("#given daemon probing is pending #when startup is aborted #then ensure settles without spawning", async () => {
    ...
    const ensure = ensureDaemonRunning(PATHS, deps, {
    ...
    if (!(outcome instanceof Error)) {
      throw new Error("ensure did not reject with an Error");
    }
  ...
  it("#given a probe is already aborted #when its socket cannot connect #then the socket error stays contained", async () => {
    ...
    const script = `
			...
			const value = await pingDaemon({ socket: ${
      JSON.stringify(PATHS.socket)
    } }, "test-token", 100, controller.signal);
