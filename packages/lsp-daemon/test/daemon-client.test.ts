...
describe("currentRequestContext", () => {
  it("#given unrelated env #when building request context #then constructs exact typed Codex defaults without forwarded env", () => {
    ...
    expect(context.projectConfigPaths).toEqual([
      join(process.cwd(), ".codex", "lsp-client.json"),
    ]);
    expect(context.userConfigPath).toBe(
      join("/home/me", ".codex", "lsp-client.json"),
    );
    expect(context.installDecisionsPath).toBe(
      join("/home/me", ".codex", "lsp-install-decisions.json"),
    );
