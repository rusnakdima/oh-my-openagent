...
describe("mcp stdio proxy lifecycle", () => {
  it("#given an in-flight daemon request #when parent input closes #then the proxy cancels that request and closes its socket", async () => {
    ...
    await bounded(
      requestAccepted.promise,
      "fake daemon did not accept the proxy request",
    );
    ...
    await bounded(
      ...
      `fake daemon accepted tools/call id=${
        String(daemonRequestId)
      }; proxy did not settle it after lifecycle cancellation`,
    );
  ...
  it("#given daemon startup is pending #when parent input ends #then the proxy cancels startup without retrying", async () => {
    ...
    input.emit("end");
    const responseWrittenBeforeEnsureRelease = await settlesWithin(
      responseWritten.promise,
      100,
    );
    input.end();
    ...
    await bounded(
      proxy,
      "proxy did not settle after releasing the startup probe",
    );

    const response = JSON.parse(outputChunks.join("").trim()) as Record<
      string,
      unknown
    >;
  ...
  it("#given an initialized proxy whose stdin stays open #when its parent process dies #then the parent watchdog exits the proxy without waiting for stdin close", async () => {
    ...
    try {
      ...
      await bounded(
        proxy,
        "proxy did not settle after the parent watchdog fired",
      );
  ...
  it("#given synchronous proxy setup failure #when the proxy rejects #then parent lifecycle listeners are removed", async () => {
    ...
    await expect(runMcpStdioProxy(options)).rejects.toThrow(
...
function tempPaths(): DaemonPaths {
  ...
  if (process.platform !== "win32") {
    ...
    const socketDirectoryIsExternal = isAbsolute(socketDirectoryFromRoot) ||
