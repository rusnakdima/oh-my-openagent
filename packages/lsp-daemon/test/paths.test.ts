...
const stampScript = fileURLToPath(
  new URL("../scripts/stamp-dist-version.mjs", import.meta.url),
);
...
describe("resolveDaemonVersion injection", () => {
  ...
  it.each([
    ...
    "#given invalid stamped package version %s #when resolving version #then it is rejected instead of downgraded to zero",
    (version) => {
...
describe("stamp-dist-version script", () => {
  ...
  it("#given a missing dist dir #when running the stamp script #then exits with non-zero", () => {
    ...
    expect(() =>
      execFileSync(process.execPath, [stampScript, missingDir], {
