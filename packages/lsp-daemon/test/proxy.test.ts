...
describe("mcp stdio proxy", () => {
  it("#given initialize and tools/call #when proxied #then initialize is local and the tool goes to the daemon", async () => {
    ...
    expect((responses[0]?.["result"] as { serverInfo?: unknown }).serverInfo)
      .toBeDefined();
    const toolResult = responses[1]?.["result"] as {
  ...
  it("#given OpenCode LSP env #when tools are proxied #then daemon requests use the env-derived typed context", async () => {
    ...
    const projectRoot = mkdtempSync(
      join(tmpdir(), "lsp-daemon-opencode-context-"),
    );
    ...
    writeFileSync(
      opencodeConfig,
      JSON.stringify({ lsp: { typescript: { disabled: true } } }),
    );
    writeFileSync(
      omoConfig,
      JSON.stringify({ lsp: { typescript: { priority: 99 } } }),
    );
    writeFileSync(
      omoClientConfig,
      JSON.stringify({ lsp: { deno: { disabled: true } } }),
    );
    writeFileSync(
      userConfig,
      JSON.stringify({
        lsp: {
          "user-context-proof": { command: ["user-ls"], extensions: [".user"] },
        },
      }),
    );
    ...
    try {
        ...
        input: inputStream([
          ...
          },
          {
            ...
            params: {
              ...
              arguments: { server_id: "deno", decision: "declined" },
            },
          },
        ]),
        output: collectingWritable(out),
      ...
      const status = responses.find((response) => response["id"] === 3)
        ?.["result"] as
          ...
          | undefined;
      const decision = responses.find((response) => response["id"] === 4)
        ?.["result"] as
      ...
      expect(statusText).toContain(
        "- user-context-proof: missing; source=user",
      );
  ...
  it("#given an unreachable daemon #when a tool is proxied #then it returns a structured error instead of running locally", async () => {
    ...
    const responses = parseResponses(out);
    const toolResult = responses[0]?.["result"] as {
      content: Array<{ text: string }>;
      isError?: boolean;
    };
  ...
  it("#given an initialized idle proxy #when the next request arrives after ten minutes #then it still responds", async () => {
    ...
    try {
      ...
      expect((responses[0]?.["result"] as { serverInfo?: unknown }).serverInfo)
        .toBeDefined();
      expect((responses[1]?.["result"] as { serverInfo?: unknown }).serverInfo)
        .toBeDefined();
    } finally {
      vi.useRealTimers();
    }
