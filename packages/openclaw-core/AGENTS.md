...
Package: `@oh-my-opencode/openclaw-core`. Harness-neutral Core for OpenClaw
bidirectional integration. Outbound dispatch fires HTTP webhooks or shell
commands on session events; inbound reply-listener daemon polls Discord/Telegram
and injects replies into tracked tmux panes. Consumed by the OpenCode adapter at
[`packages/omo-opencode/src/openclaw/`](../omo-opencode/src/openclaw/AGENTS.md)
via re-export shims.
...
- Session registry is a file-locked JSONL written via
  `session-registry-storage.ts`.
- Daemon state persists to disk via `reply-listener-state.ts` (PID, config
  signature, poll tracking).
- Rate limiter defaults to 10 injections per minute per pane.
- `tmux.ts` pane confidence check skips injection if `analyzePaneContent()`
  returns below 0.3.
