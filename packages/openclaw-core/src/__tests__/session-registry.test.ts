...
function createMapping(
  overrides: Partial<SessionMapping> = {},
): SessionMapping {
  return {
    sessionId: "session-1",
...
describe("session-registry", () => {
  test("appends mappings and loads only valid JSONL records", () => {
    ...
    writeFileSync(
      registryPath,
      `${readFileSync(registryPath, "utf-8")}not-json\n`,
    );
    expect(sessionRegistryModule.registerMessage(secondMapping)).toBe(true);

    // then
    expect(sessionRegistryModule.loadAllMappings()).toEqual([
      firstMapping,
      secondMapping,
    ]);
  ...
  test("keeps the registry path stable after first path resolution", () => {
    ...
    const otherDataHome = mkdtempSync(
      join(tmpdir(), "openclaw-session-registry-other-"),
    );
    ...
    try {
      ...
      expect(readFileSync(registryPath, "utf-8")).toContain(
  ...
  test("#given a stale registry lock from a dead process #when registering #then it recovers and releases the lock", () => {
    ...
    writeFileSync(
      lockPath,
      JSON.stringify({ pid: 999_999_999, acquiredAt: 0, token: "stale-token" }),
    );
  ...
  test("looks up mappings by platform and message id", () => {
    // given
    const discordMapping = createMapping({
      platform: "discord-bot",
      messageId: "message-1",
    });
    const telegramMapping = createMapping({
      platform: "telegram",
      messageId: "message-1",
    });
    ...
    // when/then
    expect(sessionRegistryModule.lookupByMessageId("telegram", "message-1"))
      .toEqual(telegramMapping);
    expect(sessionRegistryModule.lookupByMessageId("discord-bot", "missing"))
      .toBeNull();
  });

  test("removes mappings by session and pane while preserving other records", () => {
    // given
    const keepMapping = createMapping({
      sessionId: "session-keep",
      tmuxPaneId: "%keep",
      messageId: "keep",
    });
    const removeBySessionMapping = createMapping({
      sessionId: "session-remove",
      tmuxPaneId: "%session",
      messageId: "session",
    });
    const removeByPaneMapping = createMapping({
  ...
  test("prunes mappings older than the registry retention window", () => {
    // given
    const freshMapping = createMapping({
      sessionId: "fresh",
      messageId: "fresh",
      createdAt: new Date().toISOString(),
    });
    const staleMapping = createMapping({
      sessionId: "stale",
      messageId: "stale",
      createdAt: "2026-01-01T00:00:00.000Z",
    });
    const invalidDateMapping = createMapping({
      sessionId: "invalid",
      messageId: "invalid",
