...
function isSessionMapping(value: unknown): value is SessionMapping {
  ...
  if (!("tmuxSession" in value) || typeof value.tmuxSession !== "string") {
    return false;
  }
  if (!("tmuxPaneId" in value) || typeof value.tmuxPaneId !== "string") {
    return false;
  }
  if (!("projectPath" in value) || typeof value.projectPath !== "string") {
    return false;
  }
...
export function rewriteRegistryUnsafe(
  ...
  const content =
    mappings.map((mapping) => JSON.stringify(mapping)).join("\n") + "\n";
