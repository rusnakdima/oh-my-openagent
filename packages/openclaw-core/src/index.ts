...
import { captureTmuxPane, getCurrentTmuxSession } from "./tmux";
...
export async function wakeOpenClaw(
  ...
  try {
    ...
    const replyChannel = context.replyChannel ??
      process.env.OPENCLAW_REPLY_CHANNEL;
    const replyTarget = context.replyTarget ??
      process.env.OPENCLAW_REPLY_TARGET;
    const replyThread = context.replyThread ??
      process.env.OPENCLAW_REPLY_THREAD;
    ...
    const tmuxSession = enrichedContext.tmuxSession ??
      getCurrentTmuxSession() ?? undefined;

    let tmuxTail = enrichedContext.tmuxTail;
    if (
      !tmuxTail && (event === "stop" || event === "session-end") &&
      process.env.TMUX
    ) {
      try {
