...
export function resolveReplyListenerDaemonScript(
...
> {
  ...
  if (await isDaemonRunning()) {
    ...
    const stopped = runningPid === null
      ...
      : await waitForReplyListenerProcessExit(
        runningPid,
        REPLY_LISTENER_STOP_TIMEOUT_MS,
      );
    if (!stopped) {
      return {
        success: false,
  ...
  pendingState.configSignature = getReplyListenerRuntimeSignature(
    normalizedConfig,
  );
