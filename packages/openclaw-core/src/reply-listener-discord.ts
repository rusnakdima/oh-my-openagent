...
export async function pollDiscordReplies(
  ...
  if (!replyListener?.discordBotToken || !replyListener.discordChannelId) {
    return;
  }
  if (
  ...
  try {
    const after = state.discordLastMessageId
      ...
      : "?limit=10";
    const url =
      `https://discord.com/api/v10/channels/${replyListener.discordChannelId}/messages${after}`;
    ...
    if (remaining !== null && Number.parseInt(remaining, 10) < 2) {
      ...
      logReplyListenerMessage(
        `WARN: Discord rate limit low (remaining: ${remaining}), backing off until ${
    ...
    for (const message of [...messages as DiscordMessage[]].reverse()) {
      ...
      if (success) {
        ...
        try {
          ...
          logReplyListenerMessage(
            `WARN: Failed to acknowledge Discord message ${message.id}: ${
