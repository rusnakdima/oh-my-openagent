...
export async function pollSlackReplies(
  ...
  try {
    ...
    const url =
      `https://slack.com/api/conversations.history?channel=${replyListener.slackChannelId}&oldest=${oldest}&limit=10`;
    ...
    if (!data.ok) {
      state.errors += 1;
      const errorMsg = (data as unknown as { error?: string }).error ??
        "unknown";
    ...
    for (const message of [...data.messages].reverse()) {
      ...
      if (
        replyListener.authorizedSlackUserIds &&
        replyListener.authorizedSlackUserIds.length > 0
      ) {
        if (!replyListener.authorizedSlackUserIds.includes(message.user)) {
          continue;
        }
      }
      ...
      if (!rateLimiter.canProceed()) {
        logReplyListenerMessage(
          `WARN: Rate limit exceeded, dropping Slack message ${message.ts}`,
        );
        state.errors += 1;
        continue;
      }
      ...
      if (success) {
        ...
        try {
          ...
          logReplyListenerMessage(
            `WARN: Failed to acknowledge Slack message ${message.ts}: ${
