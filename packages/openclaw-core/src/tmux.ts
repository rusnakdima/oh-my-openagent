...
export async function getTmuxSessionNameWithDeps(
  deps: OpenClawTmuxDeps,
): Promise<string | null> {
  try {
    const result = await runOpenClawTmuxCommand(
      ["display-message", "-p", "#S"],
      deps,
    );
...
export async function sendToPaneWithDeps(
  ...
  try {
    const literalResult = await runOpenClawTmuxCommand([
      "send-keys",
      "-t",
      paneId,
      "-l",
    ...
    const enterResult = await runOpenClawTmuxCommand([
      "send-keys",
      "-t",
      paneId,
      "Enter",
...
export async function isTmuxAvailableWithDeps(
  deps: OpenClawTmuxDeps,
): Promise<boolean> {
  try {
...
export function analyzePaneContent(
  content: string | null,
): { confidence: number } {
