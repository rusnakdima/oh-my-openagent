...
const { describe, expect, test, beforeEach, afterEach, mock, spyOn } = require(
  "bun:test",
);
import * as connectedProvidersCache from "./connected-providers-cache";

let readConnectedProvidersCacheSpy: ReturnType<typeof spyOn> | undefined;
const { shouldRetryError, selectFallbackProvider, isRetryableModelError } =
...
describe("model-error-classifier", () => {
  ...
  test("GLM fair use policy violation 429 does NOT trigger fallback (STOP pattern wins)", () => {
    ...
    const error = {
      statusCode: 429,
      message:
        "Request blocked under Fair Use Policy. Your request rate has been restricted.",
    };
