...
function isRuntimeFallbackModelRecord(
  model: unknown,
): model is RuntimeFallbackModelRecord {
...
export function stringifyRuntimeFallbackModel(
  model: unknown,
): string | undefined {
  ...
  const trimmedVariant = typeof model.variant === "string"
    ? model.variant.trim()
    : undefined;
...
export function stringifyRuntimeFallbackModelWithVariant(
  model: unknown,
  variant: unknown,
): string | undefined {
  const baseModel = stringifyRuntimeFallbackModel(model);
  const fallbackVariant = typeof variant === "string"
    ? variant.trim()
    : undefined;
  if (!baseModel || !fallbackVariant) return baseModel;

  const parsed = parseModelString(baseModel);
  if (!parsed?.providerID || !parsed.modelID || parsed.variant) {
    return baseModel;
  }
...
function canonicalizeRuntimeFallbackProviderFamily(
  providerID: string,
  modelID: string,
): string {
...
function parseCanonicalRuntimeFallbackModel(
  model: string,
  ...
  return {
    providerID: canonicalizeRuntimeFallbackProviderFamily(
      parsed.providerID,
      parsed.modelID,
    ),
...
export function areRuntimeFallbackModelsEquivalent(
