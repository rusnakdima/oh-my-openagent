...
function stripSameProviderPrefix(
  providerID: string,
  modelID: string,
): string | undefined {
  const prefix = `${providerID.trim()}/`;
  return prefix !== "/" &&
      modelID.slice(0, prefix.length).toLowerCase() === prefix.toLowerCase()
...
function buildLookupCandidates(providerID: string, modelID: string): string[] {
  const bareModelID =
    parseVariantFromModelID(modelID, { allowMaxSuffix: true }).modelID;
  const candidates = [
    modelID,
...
function findProviderMetadata(
  ...
  for (const candidate of buildLookupCandidates(providerID, modelID)) {
    const match = providerCache.findProviderModelMetadata(
      providerID,
      candidate,
    );
...
function findSnapshotEntry(
  ...
  for (const modelID of modelIDs) {
    ...
    const unqualifiedModelID = strippedModelID ??
      (modelID.includes("/") ? undefined : modelID);
    if (!unqualifiedModelID) {
      generalCandidates.push(...buildLookupCandidates(providerID, modelID));
      continue;
    }
    const bareModelID =
      parseVariantFromModelID(unqualifiedModelID, { allowMaxSuffix: true })
        .modelID;
  ...
  const seen = new Set<string>();
  for (
    const candidate of [...providerSpecificCandidates, ...generalCandidates]
  ) {
...
export function getModelCapabilities(
  ...
  const canonicalization = resolveCapabilityModelAlias(
    input.modelID,
    input.providerID,
  );
  ...
  const heuristicFamily = detectHeuristicModelFamily(
    canonicalization.canonicalModelID,
  );
  ...
  const variantsSource: ModelCapabilitiesDiagnostics["variants"]["source"] =
    runtimeVariants
      ...
      ? "heuristic"
      : "none";
  const reasoningEffortsSource:
    ModelCapabilitiesDiagnostics["reasoningEfforts"]["source"] =
      providerOverride?.reasoningEfforts
        ? "override"
  ...
  const reasoningSource: ModelCapabilitiesDiagnostics["reasoning"]["source"] =
    runtimeReasoning === undefined
      ? snapshotEntry?.reasoning === undefined ? "none" : snapshotSource
      : "runtime";
  const supportsThinkingSource:
    ModelCapabilitiesDiagnostics["supportsThinking"]["source"] =
  ...
  const supportsTemperatureSource:
    ModelCapabilitiesDiagnostics["supportsTemperature"]["source"] =
  ...
  const supportsTopPSource:
    ModelCapabilitiesDiagnostics["supportsTopP"]["source"] =
      runtimeTopP !== undefined
        ? "runtime"
        : override?.supportsTopP !== undefined
        ? "override"
        : "none";
  const maxOutputTokensSource:
    ModelCapabilitiesDiagnostics["maxOutputTokens"]["source"] =
  ...
  const resolutionMode: ModelCapabilitiesDiagnostics["resolutionMode"] =
    canonicalization.source === "canonical"
      ? snapshotSource !== "none"
        ? "snapshot-backed"
        : familySource === "heuristic" || variantsSource === "heuristic" ||
  ...
  return {
    ...
    variants: runtimeVariants ?? providerOverride?.variants ??
      override?.variants ?? heuristicFamily?.variants,
    reasoningEfforts: providerOverride?.reasoningEfforts ??
      override?.reasoningEfforts ?? heuristicFamily?.reasoningEfforts,
    reasoning: runtimeReasoning ?? snapshotEntry?.reasoning,
    supportsThinking: override?.supportsThinking ?? runtimeThinking ??
      snapshotEntry?.reasoning ?? heuristicFamily?.supportsThinking,
    supportsTemperature: runtimeTemperature ?? override?.supportsTemperature ??
