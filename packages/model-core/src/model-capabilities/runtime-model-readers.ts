...
function normalizeVariantKeys(value: unknown): string[] | undefined {
  ...
  if (arrayVariants) {
    return arrayVariants.filter((v): v is string => typeof v === "string").map((
      variant,
    ) => variant.toLowerCase());
  }
...
function readModalityKeys(value: unknown): string[] | undefined {
  ...
  if (stringArray) {
    return stringArray.filter((entry): entry is string =>
      typeof entry === "string"
...
function readRuntimeModelCapabilities(
  ...
  return isRecord(runtimeModel?.capabilities)
...
export function readRuntimeModelThinkingSupport(
  ...
  const thinkingSupport = readRuntimeModelBoolean(runtimeModel, [
    "thinking",
    "supportsThinking",
  ]);
...
export function readRuntimeModelToolCallSupport(
  ...
  return readRuntimeModelBoolean(runtimeModel, [
    "toolCall",
    "tool_call",
    "toolcall",
  ]);
