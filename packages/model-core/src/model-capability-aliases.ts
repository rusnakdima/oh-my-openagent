...
const PATTERN_ALIAS_RULES: ReadonlyArray<PatternAliasRule> = [
  {
    ...
    description:
      "Normalizes OpenCode's OpenAI GPT-5.6 fast service-tier IDs to canonical snapshot IDs.",
    providerIDs: ["openai"],
    allowedSubproviderHosts: ["vercel"],
    match: (normalizedModelID) =>
      /^gpt-5\.6-(?:sol|terra|luna)-fast$/.test(normalizedModelID),
    canonicalize: (normalizedModelID) =>
      normalizedModelID.slice(0, -"-fast".length),
  },
  {
    ruleID: "claude-thinking-legacy-alias",
    description:
      "Normalizes the legacy claude-opus-4-7-thinking id to the canonical snapshot ID.",
    match: (normalizedModelID) =>
      /^claude-opus-4-7-thinking$/.test(normalizedModelID),
...
export function resolveModelIDAlias(
  ...
  const aliasLookupModelID = stripProviderPrefixForAliasLookup(
    requestedModelID,
  );
  const normalizedProviderID = providerID
    ? normalizeLookupModelID(providerID)
    : undefined;
  const providerPrefixEnd = requestedModelID.indexOf("/");
  const embeddedProviderID = providerPrefixEnd > 0
    ? requestedModelID.slice(0, providerPrefixEnd)
    : undefined;
  ...
  for (const rule of PATTERN_ALIAS_RULES) {
    if (rule.providerIDs) {
      const matchesProviderID = normalizedProviderID !== undefined &&
        rule.providerIDs.includes(normalizedProviderID);
      const matchesEmbeddedProviderID = normalizedProviderID !== undefined &&
