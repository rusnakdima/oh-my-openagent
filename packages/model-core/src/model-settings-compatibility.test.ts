...
describe("resolveCompatibleModelSettings", () => {
  ...
  describe("model family registry coverage", () => {
    const familyCases: Array<{
      ...
      },
      {
        name: "GLM",
        modelID: "glm-5.2",
        expectedVariants: ["low", "medium", "high", "max"],
        hasReasoningEffort: true,
      },
      {
        name: "Minimax",
    ...
    for (
      const { name, modelID, expectedVariants, hasReasoningEffort }
        of familyCases
    ) {
  ...
  test.each([
    ...
    "keeps temperature for %s/%s when capability metadata explicitly enables it",
    (providerID, modelID) => {
      const result = resolveCompatibleModelSettings({
        providerID,
        modelID,
  ...
  test.each([
    ...
    "drops temperature for reasoning-suffixed o-series model %s",
    (modelID) => {
      const result = resolveCompatibleModelSettings({
        providerID: "openai",
        modelID,
