...
function resolveField(
  ...
  if (
    aliased &&
    (metadataOverride?.includes(aliased) || familyCaps?.includes(aliased))
  ) {
...
export function resolveCompatibleModelSettings(
  ...
  const familyDisallowsTemperature =
    ...
    (isClaudeOpus47OrLaterModel(input.modelID) ||
      family?.supportsTemperature === false);
  if (
    ...
    changes.push({
      ...
      to: undefined,
      reason: metadataSupportsTemperature === false
        ? "unsupported-by-model-metadata"
        : "unsupported-by-model-family",
    });
    temperature = undefined;
  ...
  if (
    thinking !== undefined && input.capabilities?.supportsThinking === false
  ) {
    changes.push({
      field: "thinking",
