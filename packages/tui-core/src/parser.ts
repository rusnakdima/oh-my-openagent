...
import type { TuiElement, TuiLayout, TuiLine } from "./types";
...
function isHighlighted(text: string): boolean {
  return (
    text.includes(ANSI_BOLD) || text.includes(ANSI_INVERSE) ||
    text.includes(ANSI_UNDERLINE) || text.includes(ANSI_BRIGHT)
  );
}
...
function detectPanelBorder(line: string): boolean {
  ...
  return BOX_TOP.test(clean) || BOX_SINGLE.test(clean) ||
    BOX_DOUBLE.test(clean);
}
...
function detectElementType(
  line: string,
  context: { prevLine?: string; nextLine?: string; index: number },
): TuiElement["type"] {
  const clean = stripAnsi(line).trim();
 ...
 */
export function findElementByTextMatch(
  elements: TuiElement[],
  text: string,
): TuiElement | null {
  const normalized = text.toLowerCase().trim();
  return elements.find((e) => e.text.toLowerCase().includes(normalized)) ??
    null;
}
