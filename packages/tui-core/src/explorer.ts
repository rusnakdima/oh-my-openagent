...
export async function sendMouseClick(
  ...
  const { x, y, button = "left", type = "click", windowName, paneIndex = 0 } =
    options;
  const target = getTarget(sessionName, windowName, paneIndex);
...
export async function waitForScreenChange(
  ...
  const { windowName, paneIndex = 0, timeoutMs = 30000, pollIntervalMs = 500 } =
    options;
  ...
  const finalResult = await captureScreen(sessionName, {
    windowName,
    paneIndex,
  });
  return {
    changed: finalResult.output !== expectedOutput,
    currentOutput: finalResult.output,
  };
}
