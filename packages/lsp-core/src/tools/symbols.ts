...
function isDocumentSymbol(
  symbol: DocumentSymbol | SymbolInfo,
...
export async function executeLspSymbols(
  ...
  const limit = Math.min(
    optionalNumber(params, "limit") ?? DEFAULT_MAX_SYMBOLS,
    DEFAULT_MAX_SYMBOLS,
  );
  ...
  try {
    ...
    const symbols = await withLspClient(
      ...
      async (client, _workspaceRoot, resolvedFilePath) =>
        client.documentSymbols(resolvedFilePath, signal),
      "documentSymbols",
      clientOptions(signal),
    );
    ...
    const query = optionalString(params, "query");
    const missingDependency = missingDependencyResult(
      error,
      {
        filePath,
        scope,
...
function formatSymbolsResult(
  ...
  if (documentSymbols.length === limited.length) {
    lines.push(
      ...documentSymbols.map((symbol) => formatDocumentSymbol(symbol)),
    );
  } else {
    lines.push(
      ...limited.filter((symbol): symbol is SymbolInfo =>
        !isDocumentSymbol(symbol)
      ).map(formatSymbolInfo),
    );
  }
