import {
  isDirectoryPath,
  resolveReadablePathInsideContext,
  withLspClient,
} from "../lsp/client-wrapper.js";
...
function asDiagnosticArray(
  result: { items?: Diagnostic[] } | Diagnostic[] | null | undefined,
): Diagnostic[] {
  if (!result) return [];
  ...
  try {
    ...
    const result = await withLspClient(
      ...
      async (client, _workspaceRoot, resolvedFilePath) =>
        client.diagnostics(resolvedFilePath, signal),
      "diagnostics",
