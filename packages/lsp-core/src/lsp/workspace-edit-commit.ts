...
function snapshotsEqual(
  ...
  if (
    expected.kind === "directory" && actual.kind === "directory" &&
    expected.children !== undefined
  ) {
    return JSON.stringify(expected.children) ===
      JSON.stringify(actual.children);
  }
  return true;
}
...
function verifySnapshots(plan: WorkspaceEditPlan): WorkspaceEditCommit | null {
  for (const [path, expected] of plan.snapshots) {
    ...
    try {
      ...
      const changeIndex = plan.firstChangeByPath.get(path) ??
        firstOperationIndex(plan);
...
function commitOperation(
  ...
  if (operation.kind === "rename") {
    if (operation.replaceDestination) {
      const targetKind = existsSync(operation.newPath) &&
          lstatSync(operation.newPath).isDirectory()
        ? "directory"
        : "file";
