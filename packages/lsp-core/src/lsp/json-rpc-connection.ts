...
export class JsonRpcConnection {
  ...
  async sendRequest<T>(
    ...
    let settled = false;
    const writeCancel = (): Promise<void> =>
      this.writeMessage({
        jsonrpc: "2.0",
  ...
  private drainInputBuffer(): void {
    while (true) {
      ...
      if (contentLength === null) {
        ...
        this.emitError(
          new Error("JSON-RPC message is missing Content-Length header"),
        );
        return;
      }
      ...
      const body = this.inputBuffer.subarray(bodyStart, bodyEnd).toString(
        "utf8",
      );
  ...
  private dispatchBody(body: string): void {
    ...
    if (!isJsonRpcObject(parsed)) {
      void this.writeError(null, INVALID_REQUEST, "Invalid JSON-RPC message")
    ...
    if (typeof parsed["method"] !== "string") {
      ...
      void this.writeError(id, INVALID_REQUEST, "Invalid JSON-RPC method")
        .catch((error) => this.emitError(toError(error)));
      return;
    }
  ...
  private handleRequest(message: Record<string, unknown>): void {
    ...
    if (!handler) {
      void this.writeError(id, METHOD_NOT_FOUND, `Method not found: ${method}`)
        .catch((error) => this.emitError(toError(error)));
      return;
    }
