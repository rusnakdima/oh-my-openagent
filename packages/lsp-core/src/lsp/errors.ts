...
export type FailedServerLookupResult = Exclude<
  ServerLookupResult,
  { status: "found" }
>;
...
export class LspProcessExitedError extends Error {
  ...
  constructor(
    ...
    super(
      `LSP server ${serverId} at ${root} exited with code ${
        exitCode ?? "null"
      }${stderrSuffix}`,
    );
  }
...
export function isLspDeadConnectionError(
  ...
  return err instanceof LspConnectionClosedError ||
    err instanceof LspProcessExitedError;
}
