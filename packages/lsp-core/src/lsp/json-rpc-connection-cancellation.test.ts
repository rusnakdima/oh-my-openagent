    ...
    waitForCount(count: number): Promise<void> {
      ...
      return new Promise((resolve, reject) => {
        const timer = setTimeout(() => {
          reject(
            new Error(
              `waited for ${count} JSON-RPC messages, observed ${messages.length}`,
            ),
          );
        }, 1_000);
        const wake = (): void => {
...
describe("JsonRpcConnection cancellation", () => {
  it("#given an active request #when its signal aborts #then cancel uses the same id and late responses are ignored", async () => {
    ...
    const request = connection.sendRequest("textDocument/diagnostic", {
    ...
    expect(typeof requestId === "string" || typeof requestId === "number").toBe(
      true,
    );
    ...
    expect(cancelTarget(recorder.messages[1] ?? {})).toBe(
