...
export function normalizeWorkspaceEditContractEvidence(
  value: unknown,
  workspace: string,
): unknown {
  ...
  if (Array.isArray(value)) {
    return value.map((entry) =>
      normalizeWorkspaceEditContractEvidence(entry, workspace)
    );
  }
  ...
  for (const key of Object.keys(value).sort()) {
    ...
    normalized[key] = key === "failureReason" && typeof entry === "string"
...
export function workspaceEditContractFailureEvidence(
  value: unknown,
  workspace: string,
) {
  ...
  return {
    normalized,
    sha256: createHash("sha256").update(JSON.stringify(normalized)).digest(
      "hex",
    ),
  };
}
