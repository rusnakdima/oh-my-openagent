...
export class WorkspaceDocumentState {
  ...
  constructor(
    ...
    this.versionlessPublishQuiescenceMs =
      options.versionlessPublishQuiescenceMs ??
        DEFAULT_VERSIONLESS_PUBLISH_QUIESCENCE_MS;
  }
  ...
  getPullCache(snapshot: DiagnosticSnapshot): PullDiagnosticsCacheHit | null {
    ...
    if (
      !state?.pullCache || state.pullCache.documentVersion !== snapshot.version
    ) return null;
    return state.pullCache;
  }

  recordPullDiagnostics(
  ...
  ): DocumentVersionFailure | null {
    const versions = new Map(
      [...this.openDocuments].map(([path, state]) => [path, state.version]),
    );
    for (const operation of operations) {
      if (operation.kind === "text") {
        const current = versions.get(operation.path);
        if (
          operation.documentVersion !== null &&
          current !== operation.documentVersion
        ) {
          const observed = current === undefined
            ? "closed document"
            : `open document version ${current}`;
          return {
            changeIndex: operation.changeIndex,
            message:
              `document version ${operation.documentVersion} does not match ${observed} for ${operation.path}`,
          };
        }
      ...
      if (operation.kind === "rename") {
        const moved = [...versions].filter(([path]) =>
          isSameOrDescendant(path, operation.oldPath)
        );
        for (const [path] of moved) versions.delete(path);
        for (const [path] of moved) {
          versions.set(
            movedPath(path, operation.oldPath, operation.newPath),
            1,
          );
        }
        continue;
      ...
      if (
        operation.kind === "create" && operation.replaced &&
        versions.has(operation.path)
      ) {
  ...
  async synchronize(delta: WorkspaceMutationDelta): Promise<void> {
    ...
    for (
      let index = 0;
      index < watched.length;
      index += WATCHED_FILE_BATCH_SIZE
    ) {
  ...
  private async synchronizeMutation(
    mutation: WorkspaceMutation,
    watched: WatchedFileEvent[],
  ): Promise<void> {
    ...
    if (mutation.kind === "create") {
      ...
      if (state) {
        ...
        await this.openDocumentSingleFlight(
          mutation.path,
          readFileSync(mutation.path, "utf-8"),
        );
      } else {
        watched.push({
          uri: pathToFileURL(mutation.path).href,
          type: mutation.replaced ? 2 : 1,
        });
      }
      return;
    }
    ...
    if (removed.length === 0) {
      watched.push({ uri: pathToFileURL(mutation.path).href, type: 3 });
    }
  }

  private async openDocumentSingleFlight(
    path: string,
    text: string,
  ): Promise<void> {
