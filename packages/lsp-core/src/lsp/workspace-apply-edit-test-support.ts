...
export function createWorkspaceEditTestHarness() {
  ...
  return {
    ...
    async makeBareConnection(
      ...
      { readonly connection: LspClientConnection; readonly events: string }
    > {
      const fixture = createFixture(scenario, tempDirectories);
...
function createFixture(
  ...
  writeFileSync(
    scenarioPath,
    JSON.stringify(replacePlaceholderUris(scenario, uriByPlaceholder)),
    "utf-8",
  );
  ...
  return {
    workspace,
    server,
    paths: { workspace, source, destination, closed, created, events },
  };
}

function replacePlaceholderUris(
  value: unknown,
  uriByPlaceholder: ReadonlyMap<string, string>,
): unknown {
  if (Array.isArray(value)) {
    return value.map((item) => replacePlaceholderUris(item, uriByPlaceholder));
  }
  if (typeof value !== "object" || value === null) {
    return typeof value === "string"
      ? (uriByPlaceholder.get(value) ?? value)
      : value;
  }
  return Object.fromEntries(
    Object.entries(value).map((
      [key, item],
    ) => [key, replacePlaceholderUris(item, uriByPlaceholder)]),
  );
}
...
export function renameTextEdit(
  before: string,
  after: string,
  target: RenameTextEditTarget,
): WorkspaceEdit {
