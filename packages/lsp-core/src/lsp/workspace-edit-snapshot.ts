...
class WorkspaceSnapshotBuilder {
  ...
  private add(path: string, includeChildren: boolean): void {
    ...
    while (true) {
      ...
      if (
        existing === undefined ||
        (includeChildren && existing.kind === "directory" &&
          existing.children === undefined)
      ) {
        this.snapshots.set(
          candidate,
          snapshotPath(candidate, includeChildren && candidate === path),
        );
      }
    ...
    if (
      !includeChildren || !existsSync(path) || !lstatSync(path).isDirectory()
    ) return;
