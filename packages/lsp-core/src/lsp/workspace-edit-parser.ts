...
export function failureResult(failures: readonly ParseFailure[]): ApplyResult {
  ...
  return {
    ...
    filesModified: [],
    totalEdits: 0,
    errors: sorted.map((failure) =>
      `change ${failure.changeIndex}: ${failure.message}`
    ),
...
export function parseWorkspaceEdit(
  ...
  if (!isRecord(edit)) {
    return {
      operations: [],
      failures: [{ changeIndex: 0, message: "No edit provided" }],
    };
  }
  if (edit["changeAnnotations"] !== undefined) {
    return {
      ...
      failures: [{
        changeIndex: 0,
        message: "change annotations are unsupported",
      }],
    };
  }
  ...
  if (hasChanges && hasDocumentChanges) {
    return {
      ...
      failures: [{
        changeIndex: 0,
        message: "changes and documentChanges cannot be combined",
      }],
    };
  }
...
function parseChanges(
  ...
  if (!isRecord(value)) {
    return {
      ...target,
      failures: [{ changeIndex: 0, message: "changes must be an object" }],
    };
  }
...
function parseDocumentChanges(
  ...
  if (!Array.isArray(value)) {
    return {
      ...
      failures: [{
        changeIndex: 0,
        message: "documentChanges must be an array",
      }],
    };
  }
...
function parseDocumentChange(input: DocumentChangeInput): void {
  ...
  if (!isRecord(change)) {
    throw new WorkspaceEditValidationError(
      changeIndex,
      "document change must be an object",
    );
  }
  ...
  const resolvedPath = uriToCanonicalWorkspacePath(
    identifier["uri"],
    workspaceRoot,
  );
  if (!resolvedPath.success) {
