...
describe("LspClient workspace edit synchronization", () => {
  it("#given an open edited document #when applyEdit commits #then didChange precedes didSave with the next version", async () => {
    ...
    const notifications = readEvents(context.events).filter((event) =>
      event.type === "clientNotification"
    );
    const changeIndex = notifications.findIndex((event) =>
      event.method === "textDocument/didChange"
    );
    const saveIndex = notifications.findIndex((event) =>
      event.method === "textDocument/didSave"
    );
    expect(changeIndex).toBeGreaterThan(-1);
    expect(saveIndex).toBeGreaterThan(changeIndex);
    expect(notifications[changeIndex]?.params).toMatchObject({
  ...
  it("#given an open file resource rename #when applyEdit commits #then didClose and didOpen move document state", async () => {
    const edit: WorkspaceEdit = {
      documentChanges: [{
        kind: "rename",
        oldUri: "file:///placeholder",
        newUri: "file:///destination",
      }],
    };
    const context = await harness.makeClient({
      renameSteps: [{ applyEdit: edit, renameResult: "null" }],
    });
    await context.client.openFile(context.source);
    ...
    expect(readFileSync(context.destination, "utf-8")).toBe(
      "const before = 1;\n",
    );
    expect(context.client.getOpenDocumentVersion(context.source))
      .toBeUndefined();
    ...
    expect(methods.indexOf("textDocument/didClose")).toBeLessThan(
      methods.lastIndexOf("textDocument/didOpen"),
    );
  });

  it("#given an open file deletion #when applyEdit commits #then didClose removes document state", async () => {
    const edit: WorkspaceEdit = {
      documentChanges: [{ kind: "delete", uri: "file:///placeholder" }],
    };
    const context = await harness.makeClient({
      renameSteps: [{ applyEdit: edit, renameResult: "null" }],
    });
    ...
    expect(context.client.getOpenDocumentVersion(context.source))
      .toBeUndefined();
    expect(
      readEvents(context.events).some((event) =>
  ...
  it("#given a closed edited file #when applyEdit commits #then one changed watched-file event is emitted", async () => {
    const edit = renameTextEdit("closed", "after_", {
      version: null,
      uri: "file:///closed",
    });
    const context = await harness.makeClient({
    ...
    expect(watched?.params).toEqual({
      changes: [{
        uri: pathToFileURL(realpathSync(context.closed)).href,
        type: 2,
      }],
    });
  });
  ...
  it("#given a new closed file #when applyEdit creates it #then one created watched-file event is emitted", async () => {
    const edit: WorkspaceEdit = {
      documentChanges: [{ kind: "create", uri: "file:///created" }],
    };
    const context = await harness.makeClient({
      renameSteps: [{ applyEdit: edit, renameResult: "null" }],
    });
    ...
    expect(watched?.params).toEqual({
      changes: [{
        uri: pathToFileURL(realpathSync(context.created)).href,
        type: 1,
      }],
    });
  });
