...
describe("workspace edit adversarial inputs", () => {
  it("#given malformed and decorated URIs #when planned #then every variant is rejected before mutation", () => {
    ...
    for (
      const uri of ["untitled:source.ts", decorated, "file:///%ZZ"] as const
    ) {
      const result = planWorkspaceEdit(
        { changes: { [uri]: [insertion()] } },
        workspace,
      );
  ...
  it("#given invalid ranges and resource options #when planned #then each failure retains its operation index", () => {
    ...
    if (!invalidRange.success) {
      expect(invalidRange.result.errors[0]).toContain("change 0");
    }
    expect(invalidOption.success).toBe(false);
    if (!invalidOption.success) {
      expect(invalidOption.result.errors[0]).toContain("change 1");
    }
    expect(readFileSync(source, "utf-8")).toBe("const before = 1;\n");
  });
  ...
  it("#given an in-workspace symlink to an outside file #when applying an edit #then rejects without modifying target", () => {
    ...
    const result = applyWorkspaceEdit(
      {
        changes: {
          [pathToFileURL(join(linkedDirectory, "outside.ts")).href]: [
            insertion(),
          ],
        },
      },
  ...
  it("#given annotations or mixed edit representations #when planned #then unsupported claims are rejected", () => {
    ...
    for (const edit of unsupported) {
      expect(planWorkspaceEdit(edit, workspace).success).toBe(false);
    }
  });
});
