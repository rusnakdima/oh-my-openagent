...
import {
  type NormalizedTextEditResult,
  WorkspaceEditValidationError,
...
function rangesEqual(left: Range, right: Range): boolean {
  return positionsEqual(left.start, right.start) &&
    positionsEqual(left.end, right.end);
}
...
function validatePosition(
  ...
  if (
    ...
    throw new WorkspaceEditValidationError(
      changeIndex,
      `${label} position must use integer line and character`,
    );
  }
  if (position.line < 0 || position.character < 0) {
    throw new WorkspaceEditValidationError(
      changeIndex,
      `${label} position cannot be negative`,
    );
  }
  ...
  if (line === undefined) {
    throw new WorkspaceEditValidationError(
      changeIndex,
      `${label} line ${position.line} is outside the document`,
    );
  }
...
function validateRange(
  ...
  if (comparePosition(range.start, range.end) > 0) {
    throw new WorkspaceEditValidationError(
      changeIndex,
      `range ${formatRange(range)} ends before it starts`,
    );
  }
}
...
function sortAndDeduplicate(edits: readonly TextEdit[]): readonly TextEdit[] {
  const sorted = edits
    ...
    .sort((left, right) => {
      const positionOrder = comparePosition(
...
function validateNoOverlap(
  ...
  for (let index = 0; index < edits.length - 1; index += 1) {
    ...
    if (comparePosition(earlier.range.end, later.range.start) > 0) {
      throw new WorkspaceEditValidationError(
        changeIndex,
        `overlapping edits ${formatRange(earlier.range)} and ${
          formatRange(later.range)
        }`,
      );
    }
  }
...
function applyNormalizedTextEdits(
  content: string,
  edits: readonly TextEdit[],
): string {
  ...
  for (const edit of edits) {
    ...
    const replacement = startLine.slice(0, start.character) + edit.newText +
...
export function normalizeTextEdits(
  ...
  return {
    edits: normalized,
    text: applyNormalizedTextEdits(content, normalized),
  };
}
