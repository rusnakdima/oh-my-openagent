...
export function resolvePathInsideContext(filePath: string): string {
  ...
  if (!isPathInside(cwd, canonical)) {
    throw new LspInvalidPathError(
      `LSP file path must be inside request cwd: ${filePath}`,
    );
  }
  return canonical;
}
...
function rebaseThroughCanonicalAncestor(
  ...
  while (true) {
    if (existsSync(current)) {
      ...
      if (isPathInside(cwd, canonical)) {
        return suffix.length === 0 ? canonical : join(canonical, ...suffix);
      }
    }
...
function existingDirectoryInsideContext(
  directory: string,
  cwd: string,
): string | undefined {
...
function nearestExistingDirectoryInsideContext(
  directory: string,
  cwd: string,
): string | undefined {
...
export function formatServerLookupError(
  ...
  const firstProjectConfigPath = context.projectConfigPaths[0] ??
    "<project lsp config>";
...
function formatNotInstalled(
  result: Extract<ServerLookupResult, { status: "not_installed" }>,
): string {
...
export async function withLspClient<T>(
  ...
  const acquireAndCall = async (allowRetry: boolean): Promise<T> => {
    ...
    try {
      ...
      if (
        allowRetry && READ_ONLY_TOOLS.has(toolName) &&
        isLspDeadConnectionError(err)
      ) {
