...
const CONCURRENT_FAILURE_REASON_BY_PHASE: Readonly<
  Record<WorkspaceApplyEditConcurrentPhase, string>
> = {
  applying:
    "workspace/applyEdit is already in progress for this workspace mutation",
  settled:
    "workspace/applyEdit was already handled for this workspace mutation",
};
...
export function workspaceApplyEditConcurrentFailureReason(
  phase: WorkspaceApplyEditConcurrentPhase,
): string {
  return CONCURRENT_FAILURE_REASON_BY_PHASE[phase];
}

export function canonicalizeWorkspaceApplyEditFailureReason(
  reason: string,
): string {
