...
export async function aggregateDiagnosticsForDirectory(
  ...
  const serverResult = options.server === undefined
    ? findServerForExtension(extension)
    : { status: "found" as const, server: options.server };
  if (serverResult.status !== "found") {
    throw new LspServerLookupError(formatServerLookupError(serverResult));
  }

  const server = serverResult.server;
  const allFiles = (options.listFiles ?? collectFilesWithExtension)(
    absDir,
    extension,
    maxFiles + 1,
  );
  ...
  const maxConcurrency = Math.max(
    1,
    options.maxConcurrency ?? DIRECTORY_DIAGNOSTICS_MAX_CONCURRENCY,
  );
  ...
  try {
    ...
    const workers = Array.from({
      length: Math.min(maxConcurrency, filesToProcess.length),
    }, async () => {
      for (;;) {
  ...
  const lines: string[] = [
    `Directory: ${absDir}`,
    `Extension: ${extension}`,
    `Files scanned: ${filesToProcess.length}${
      wasCapped ? ` (capped at ${maxFiles})` : ""
    }`,
  ...
  if (displayDiagnostics.length > 0) {
    ...
    if (wasDiagCapped) {
      lines.push(
        "",
        `... (${
          allDiagnostics.length - DEFAULT_MAX_DIAGNOSTICS
