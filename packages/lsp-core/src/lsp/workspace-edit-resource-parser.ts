...
export function parseResourceChange(input: ResourceChangeInput): void {
  ...
  if (kind !== "rename") {
    throw new WorkspaceEditValidationError(
      input.changeIndex,
      `unsupported resource operation ${String(kind)}`,
    );
  }
  parseRename(input);
}

function parseSinglePathResource(
  ...
  if (kind === "create") {
    const options = parseOptions(change["options"], [
      "overwrite",
      "ignoreIfExists",
    ], changeIndex);
...
function parseRename(input: ResourceChangeInput): void {
  const { change, changeIndex, workspaceRoot, target } = input;
  if (
    typeof change["oldUri"] !== "string" || typeof change["newUri"] !== "string"
  ) {
    throw new WorkspaceEditValidationError(
      changeIndex,
  ...
  target.operations.push({
    ...
    overwrite: options["overwrite"] ?? false,
    ignoreIfExists: options["ignoreIfExists"] ?? false,
    followedSymbolicLink: oldPath.followedSymbolicLink ||
      newPath.followedSymbolicLink,
  });
}
