...
function moveVirtualSubtree(
  virtual: Map<string, WorkspaceSnapshotEntry>,
  oldPath: string,
  newPath: string,
): void {
  const moved = [...virtual.entries()].filter(([candidate]) =>
    isSameOrDescendant(candidate, oldPath)
  );
...
function rejectSymbolicLink(
  operation: Exclude<ParsedWorkspaceOperation, { readonly kind: "text" }>,
): void {
...
function simulateRename(
  ...
  if (source.kind === "missing") {
    throw new WorkspaceEditValidationError(
      operation.changeIndex,
      `rename source does not exist: ${operation.oldPath}`,
    );
  }
  ...
  if (destination.kind !== "missing" && !operation.overwrite) {
    ...
    throw new WorkspaceEditValidationError(
      operation.changeIndex,
      `rename target already exists: ${operation.newPath}`,
    );
  }
...
function simulateDelete(
  ...
  if (
    target.kind === "directory" && !operation.recursive &&
    virtualDirectoryHasChildren(virtual, operation.path)
  ) {
    throw new WorkspaceEditValidationError(
      operation.changeIndex,
      `directory is not empty: ${operation.path}`,
    );
  }
