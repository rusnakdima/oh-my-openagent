    ...
    for (const operation of operations) {
      switch (operation.kind) {
        case "rename":
          this.add(
            operation.oldPath,
            operation.reportedOldPath,
            operation.changeIndex,
          );
          this.add(
            operation.newPath,
            operation.reportedNewPath,
            operation.changeIndex,
          );
          break;
        case "text":
...
export function fingerprintWorkspaceEdit(
  edit: unknown,
  ...
  if (!root.success) {
    return {
      success: false,
      result: failureResult([{ changeIndex: 0, message: root.error }]),
    };
  }
