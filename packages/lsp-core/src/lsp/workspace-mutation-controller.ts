...
import {
  fingerprintWorkspaceEdit,
  planWorkspaceEdit,
} from "./workspace-edit-plan.js";
import type {
  ApplyResult,
  LspRenameResult,
...
export class WorkspaceMutationController {
  ...
  acquire(signal?: AbortSignal): AcquireMutationLeaseResult {
    if (this.activeLease) {
      return {
        success: false,
        result: failure("workspace mutation is already in progress"),
      };
    }
    if (signal?.aborted) {
      return {
        success: false,
        result: failure("cancelled before mutating request"),
      };
    }
  ...
  isBeforeCommit(lease: WorkspaceMutationLease): boolean {
    return this.activeLease?.id === lease.id &&
      this.activeLease.phase === "idle";
  }
  ...
  async handleApplyEdit(params: unknown): Promise<WorkspaceApplyEditResponse> {
    ...
    if (!lease) {
      return {
        applied: false,
        failureReason:
          "workspace/applyEdit requires an active workspace mutation",
      };
    }
    if (lease.phase !== "idle") {
      return {
        applied: false,
        failureReason: workspaceApplyEditConcurrentFailureReason(
          lease.phase === "applying" ? "applying" : "settled",
        ),
      };
    }
    ...
    const record = edit === undefined
      ? {
        fingerprint: null,
        result: failure("workspace/applyEdit params.edit is required", 0),
      }
  ...
  private reconcileServerApply(
    ...
    if (
      fingerprint.success && record.fingerprint !== null &&
  ...
  private async applyEdit(
    ...
    if (commit.delta.operations.length > 0) {
      try {
        ...
        result = failure(
          `document synchronization failed after filesystem commit: ${message}`,
          undefined,
          result,
        );
      }
