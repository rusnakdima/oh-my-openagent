...
function applyResponses(eventsPath: string): readonly unknown[] {
  return readEvents(eventsPath)
    .filter((event) =>
      event.type === "clientResponse" && event.method === "workspace/applyEdit"
    )
...
if (!outputPath) {
  throw new WorkspaceEditContractProbeError("output path is required");
}
...
try {
  ...
  const successResult = await success.client.rename(
    success.source,
    1,
    6,
    "after",
  ...
  const successEvidence = {
    serverAppliedRename: successResponses.length === 1 &&
      isRecord(successResponses[0]) && successResponses[0]["applied"] === true,
    recordedResultReused: successResult.edit !== null &&
      successResult.apply.success && successWrites === 1,
    writeCount: successWrites,
    secondWritePrevented: successWrites === 1 && successVersions.length === 1,
    synchronizedDocumentVersion:
      success.client.getOpenDocumentVersion(success.source) ?? null,
  ...
  requireProbe(
    successEvidence.serverAppliedRename,
    "successful server apply was not reported truthfully",
  );
  requireProbe(
    successEvidence.recordedResultReused,
    "rename did not reuse the recorded server result",
  );
  requireProbe(
    successEvidence.secondWritePrevented,
    "server edit was written or synchronized more than once",
  );
  requireProbe(
    successEvidence.synchronizedDocumentVersion === 2,
    "open document version did not advance to 2",
  );
  requireProbe(
    successEvidence.immediateDiagnostics,
    "fresh diagnostics were not visible immediately after applyEdit",
  );
  requireProbe(
    successEvidence.finalContent === "const after = 1;\n",
  ...
  requireProbe(
    isRejectedApply(unscopedFailure),
    "unscoped applyEdit was not rejected",
  );
  requireProbe(
    readFileSync(unscoped.source, "utf-8") === "const before = 1;\n",
  ...
  const concurrentResult = await concurrent.client.rename(
    concurrent.source,
    1,
    6,
    "after",
  );
  const concurrentFailure = applyResponses(concurrent.events).find(
    isRejectedApply,
  );
  requireProbe(
    concurrentResult.apply.success,
    "the winning concurrent applyEdit did not complete",
  );
  requireProbe(
    concurrentWrites === 1,
    "concurrent applyEdit wrote more than once",
  );
  requireProbe(
    isRejectedApply(concurrentFailure),
    "concurrent applyEdit loser was not rejected",
  );

  const mismatched = await harness.makeClient({
    renameSteps: [{
      applyEdit: renameTextEdit("before", "after", { version: 9 }),
      renameResult: "same",
    }],
  });
  await mismatched.client.openFile(mismatched.source);
  const mismatchedResult = await mismatched.client.rename(
    mismatched.source,
    1,
    6,
    "after",
  );
  const mismatchedFailure = applyResponses(mismatched.events).find(
    isRejectedApply,
  );
  requireProbe(
    !mismatchedResult.apply.success,
    "mismatched document version unexpectedly succeeded",
  );
  requireProbe(
    isRejectedApply(mismatchedFailure),
    "mismatched version was not rejected over applyEdit",
  );
  requireProbe(
    readFileSync(mismatched.source, "utf-8") === "const before = 1;\n",
  ...
  const pendingRename = preGate.client.rename(
    preGate.source,
    1,
    6,
    "after",
  ...
  await waitForEventCount(
    preGate.events,
    (event) =>
      event.type === "clientRequest" && event.method === "textDocument/rename",
    1,
  );
  controller.abort();
  ...
  const preGateMessage = preGateError instanceof Error
    ? preGateError.message
    : String(preGateError);
  ...
  requireProbe(
    preGateError !== null,
    "pre-gate cancellation unexpectedly succeeded",
  );
  requireProbe(
  ...
  requireProbe(
    readFileSync(preGate.source, "utf-8") === "const before = 1;\n",
    "pre-gate cancellation mutated bytes",
  );

  const failureCases = {
    unscoped: workspaceEditContractFailureEvidence(
      unscopedFailure,
      unscoped.workspace,
    ),
    concurrent: workspaceEditContractFailureEvidence(
      concurrentFailure,
      concurrent.workspace,
    ),
    mismatched: workspaceEditContractFailureEvidence(
      mismatchedFailure,
      mismatched.workspace,
    ),
    preGate: workspaceEditContractFailureEvidence(
      preGateFailure,
      preGate.workspace,
    ),
  };
