...
function sendPublishDeliveryBarrier() {
  const id = nextServerRequestId;
  nextServerRequestId += 1;
  pendingServerRequests.set(String(id), {
    kind: "publishDeliveryBarrier",
...
function publishSteps(trigger) {
  ...
  return scenario.publishDiagnostics.filter((step) =>
    step?.trigger === trigger
  );
}

function applyPublishSteps(trigger, fallbackUri, fallbackVersion) {
  for (const step of publishSteps(trigger)) {
    schedulePublish(step, fallbackUri, fallbackVersion);
  }
}
...
function handleClientResponse(message) {
  ...
  if (
    context.kind === "publishDeliveryBarrier" || context.kind === "unscoped"
  ) return;
...
function handleRequest(message) {
  ...
  if (message.method === "textDocument/diagnostic") {
    ...
    const step = Array.isArray(scenario.diagnosticResponses)
      ? scenario.diagnosticResponses[diagnosticIndex]
      : undefined;
    diagnosticIndex += 1;
