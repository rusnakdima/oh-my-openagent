...
if (!outputPath) {
  throw new DiagnosticsFreshnessContractProbeError("output path is required");
}
...
try {
  ...
  const exactCurrentEvidence = {
    ...
    ok: exactDidOpenCount === 1 &&
      JSON.stringify(exactFirst.items) ===
        JSON.stringify([diagnostic("exact-current")]) &&
      JSON.stringify(exactSecond.items) ===
        JSON.stringify([diagnostic("exact-current")]),
  };
  requireProbe(
    exactCurrentEvidence.ok,
    "exact-current diagnostics did not resolve immediately with a single didOpen",
  );
  ...
  await postGenerationVersionless.client.openFile(
    postGenerationVersionless.source,
  );
  ...
  const versionlessResult = await postGenerationVersionless.client.diagnostics(
    postGenerationVersionless.source,
  );
  ...
  const postGenerationVersionlessEvidence = {
    ...
    ok: JSON.stringify(versionlessResult.items) ===
        JSON.stringify([diagnostic("post-generation-versionless")]) &&
  ...
  const staleEvidence = {
    ...
    ok: staleResult.items.length === 0 &&
      staleResult.transientError?.kind === "freshness_timeout",
  };
  requireProbe(
    staleEvidence.ok,
    "stale diagnostics unexpectedly satisfied the current request",
  );
  ...
  const futureEvidence = {
    ...
    ok: futureResult.items.length === 0 &&
      futureResult.transientError?.kind === "freshness_timeout",
  };
  requireProbe(
    futureEvidence.ok,
    "future diagnostics unexpectedly satisfied the current request",
  );
  ...
  await waitForEventCount(
    ...
    (event) =>
      event.type === "clientRequest" &&
      event.method === "textDocument/diagnostic",
    1,
  );
  ...
  const pullOvertakenEvidence = {
    ...
    ok: JSON.stringify(pullOvertakenResult.items) ===
        JSON.stringify([diagnostic("pull-fresh")]) &&
      pullRequestCount === 2,
  };
  requireProbe(
  ...
  const silent = await harness.makeClient({}, {
    diagnosticsFreshnessTimeoutMs: 35,
  ...
  const silentEvidence = {
    ...
    ok: silentResult.items.length === 0 &&
      silentResult.transientError === undefined,
  };
  requireProbe(
    silentEvidence.ok,
    "silent no-pull server that never published did not resolve clean after the freshness window",
  );
  ...
  requireProbe(
    closedServerEvidence.ok,
    "closed-server diagnostics failure was not surfaced as a transport error",
  );
  ...
  const unsupportedPullResult = await unsupportedPull.client.diagnostics(
    unsupportedPull.source,
  );
  const unsupportedPullEvidence = {
    outcome: "unsupported-pull",
    items: unsupportedPullResult.items.map((entry) => entry.message),
    ok: JSON.stringify(unsupportedPullResult.items) ===
      JSON.stringify([diagnostic("push-fallback")]),
  };
  requireProbe(
    unsupportedPullEvidence.ok,
    "explicitly unsupported pull did not fall back to push diagnostics",
  );
      ...
      diagnosticResponses: [
        {
          report: {
            kind: "full",
            resultId: "same-version",
            items: [diagnostic("cached-full")],
          },
        },
        {
  ...
  const unchangedEvidence = {
    ...
    ok: JSON.stringify(unchangedFirst.items) ===
        JSON.stringify([diagnostic("cached-full")]) &&
      JSON.stringify(unchangedSecond.items) ===
        JSON.stringify([diagnostic("cached-full")]),
  };
  requireProbe(
    unchangedEvidence.ok,
    "unchanged pull did not reuse the cached same-version result",
  );
