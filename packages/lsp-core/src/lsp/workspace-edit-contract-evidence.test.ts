...
describe("workspace edit contract evidence", () => {
  ...
  it("#given workspace-scoped evidence and a non-concurrent failure #when normalized #then paths are sanitized and the reason is preserved", () => {
    ...
    expect(
      normalizeWorkspaceEditContractEvidence(
        {
          applied: false,
          failureReason:
            "document version 9 does not match open document version 1 for /tmp/workspace-edit-contract/source.ts",
      ...
      applied: false,
      failureReason:
        "document version 9 does not match open document version 1 for <workspace>/source.ts",
