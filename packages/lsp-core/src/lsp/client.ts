...
export class LspClient extends LspClientConnection {
    ...
    this.documents = new WorkspaceDocumentState(
      ...
      (uri) => this.diagnosticsStore.delete(uri),
      {
        versionlessPublishQuiescenceMs:
          options.versionlessPublishQuiescenceMs ??
            VERSIONLESS_PUBLISH_QUIESCENCE_MS,
      },
    );
    this.workspaceMutations = new WorkspaceMutationController(
      root,
      this.documents,
    );
    this.setWorkspaceApplyEditHandler((params) =>
      this.workspaceMutations.handleApplyEdit(params)
    );
  }
  ...
  async documentSymbols(
    filePath: string,
    signal?: AbortSignal,
  ): Promise<Array<DocumentSymbol | SymbolInfo>> {
    const absPath = this.resolveWorkspacePath(filePath);
    await this.openFile(absPath);
    const options = signal === undefined ? {} : { signal };
    return this.sendRequest<Array<DocumentSymbol | SymbolInfo>>(
  ...
  async workspaceSymbols(
    ...
    const options = signal === undefined ? {} : { signal };
    return this.sendRequest<SymbolInfo[]>(
      "workspace/symbol",
      { query },
      options,
    );
  ...
  private isUnsupportedDiagnosticPullError(error: unknown): boolean {
    ...
    return /unsupported|not supported|method not found|unknown request/i.test(
      error.message,
    );
  }
  ...
  private freshnessTimeout(absPath: string): LspDiagnosticsResult {
    return {
      ...
      transientError: {
        ...
        message:
          `Timed out waiting for fresh diagnostics for ${absPath} within ${this.diagnosticsFreshnessTimeoutMs}ms.`,
      },
    };
  }

  private parseDiagnosticPullReport(
    value: { items?: Diagnostic[]; kind?: string; resultId?: string },
  ): {
    readonly type: "full";
    readonly diagnostics: readonly Diagnostic[];
  ...
  async diagnostics(
    ...
    for (;;) {
      ...
      if (!pushFallbackOnly) {
        ...
        try {
          ...
          const result = await this.sendRequest<
            { items?: Diagnostic[]; kind?: string; resultId?: string }
          >(
            "textDocument/diagnostic",
            {
              ...
              ...(cached?.resultId === undefined
                ? {}
                : { previousResultId: cached.resultId }),
            },
            {
          ...
          if (this.isUnsupportedDiagnosticPullError(error)) {
            ...
            this.diagnosticPullErrors.push(
      ...
      if (remainingMs <= 0) {
        ...
        if (
          !this.isDiagnosticPullSupported() && snapshot.publishGeneration === 0
        ) {
          const cached = this.documents.getPullCache(snapshot);
      ...
      const waitMs = push.status === "wait"
        ? Math.min(push.waitMs, remainingMs)
        : remainingMs;
      await waitForDiagnosticsActivity(
        this.documents.waitForDiagnosticsActivity(snapshot, waitMs),
        signal,
      );
    }
  }
  ...
  async rename(
    ...
    const preCommitSignal = createPreCommitAbortSignal(
      signal,
    ...
    try {
      ...
      const edit = preCommitSignal === undefined
        ? await this.sendRequest<WorkspaceEdit | null>(
          "textDocument/rename",
          renameParams,
        )
        : await this.sendRequest<WorkspaceEdit | null>(
      ...
      return await this.workspaceMutations.reconcileRename(
        acquired.lease,
        edit,
      );
    } finally {
...
function waitForDiagnosticsActivity(
  wait: Promise<void>,
