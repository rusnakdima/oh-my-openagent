...
export function parseConfigurationItems(params: unknown): ConfigurationItem[] {
  ...
  for (const item of params["items"]) {
    ...
    items.push(
      section === undefined || typeof section !== "string" ? {} : { section },
    );
  }
  return items;
}

export function parseDiagnosticsParams(
  params: unknown,
): DiagnosticsParams | null {
  ...
  const diagnostics = Array.isArray(params["diagnostics"])
    ...
    : [];
  const version = typeof params["version"] === "number"
    ? params["version"]
    : undefined;
  return {
    uri: params["uri"],
...
function isDiagnostic(value: unknown): value is Diagnostic {
  return isRecord(value) && isRange(value["range"]) &&
