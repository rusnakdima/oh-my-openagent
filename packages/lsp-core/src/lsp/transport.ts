...
type WorkspaceApplyEditHandler = (
  params: unknown,
) => Promise<WorkspaceApplyEditResponse>;
...
export class LspClientTransport {
  ...
  protected setWorkspaceApplyEditHandler(
    handler: WorkspaceApplyEditHandler,
  ): void {
  ...
  async start(): Promise<void> {
    ...
    if (this.workspaceApplyEditHandler) {
      this.connection.onRequest(
        "workspace/applyEdit",
        this.workspaceApplyEditHandler,
      );
    }
  ...
  private isConnectionClosedError(error: unknown): error is Error {
    ...
    return (
      ...
      /connection closed|connection is disposed|stream was destroyed/i.test(
        error.message,
      )
    );
  }
  ...
  protected async sendRequest<T>(
    ...
    const timeoutHandle = setTimeout(() => {
      ...
      timeoutController.abort(
        new LspRequestTimeoutError(method, stderrTail || undefined),
      );
    }, timeoutMs);
    ...
    try {
      const result = args.length === 0
        ? await this.connection.sendRequest<T>(method, undefined, {
          signal: combinedSignal.signal,
        })
        : await this.connection.sendRequest<T>(method, args[0], {
          signal: combinedSignal.signal,
        });
      return result;
    } catch (error) {
          ...
          try {
            ...
            await Promise.race([
              ...
              new Promise<void>((resolve) =>
                setTimeout(resolve, STOP_SIGKILL_GRACE_MS)
              ),
            ]);
          } catch (error) {
            reportBestEffortCleanupError(
