...
export function canonicalWorkspaceRoot(
  ...
  try {
    ...
    return {
      ...
      followedSymbolicLink: existsSync(resolve(workspaceRoot)) &&
        lstatSync(resolve(workspaceRoot)).isSymbolicLink(),
    };
  } catch (error) {
    const detail = error instanceof Error ? error.message : String(error);
    return {
      success: false,
      error: `workspace root ${workspaceRoot}: ${detail}`,
    };
  }
}

export function uriToCanonicalWorkspacePath(
  uri: string,
  workspaceRoot: string,
): WorkspacePathResult {
  ...
  try {
    ...
    return {
      ...
      path: canonical,
      requestedPath,
      followedSymbolicLink: existsSync(requestedPath) &&
        lstatSync(requestedPath).isSymbolicLink(),
    };
  } catch (error) {
    const detail = error instanceof Error ? error.message : String(error);
...
export function snapshotPath(
  ...
  if (stats.isFile()) {
    return { kind: "file", content: readFileSync(path, "utf-8") };
  }
  if (stats.isDirectory()) {
    return includeChildren
      ? { kind: "directory", children: readdirSync(path).sort() }
      : { kind: "directory" };
  }
