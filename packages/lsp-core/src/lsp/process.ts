...
export function validateCwd(cwd: string): { valid: boolean; error?: string } {
  try {
    ...
    return {
      valid: false,
      error: `Cannot access working directory: ${cwd} (${
...
function wrap(proc: ChildProcess): SpawnedProcess {
  ...
  if (!proc.stdin || !proc.stdout || !proc.stderr) {
    throw new LspProcessSpawnError(
      "Spawned process is missing one of stdin/stdout/stderr pipes",
    );
  }
...
function resolveWindowsCommand(
  ...
  const baseDirectories = hasPathSeparator
    ? [""]
    : splitPath(pathValue, "win32");
