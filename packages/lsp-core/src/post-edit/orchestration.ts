...
export function resetPostEditNotConfiguredCache(
...
export async function collectPostEditDiagnostics(
  ...
  const results = await runBoundedDiagnostics(
    filePaths,
    input.runDiagnostics,
...
function firstSeenDiagnosticTargets(
  ...
  for (const filePath of filePaths) {
    ...
    if (
      extension !== undefined && cache.notConfiguredExtensions.has(extension)
    ) continue;
...
async function runBoundedDiagnostics(
  ...
  const workerCount = Math.min(
    Math.max(1, maxConcurrency ?? DEFAULT_MAX_CONCURRENCY),
    filePaths.length,
  );
  let nextIndex = 0;
...
function normalizeDiagnosticsOutcome(
