...
export function createStandaloneMcpRequestContext(
  ...
  const projectConfigPaths = translateProjectConfigEnv(
    env["LSP_TOOLS_MCP_PROJECT_CONFIG"],
    cwd,
  );
  const userConfigPath = translateHomeConfigEnv(
...
export function parseLspRequestContext(value: unknown): LspRequestContext {
  if (!isRecord(value)) {
    throw new LspRequestContextParseError(
      "invalid_context",
      "LSP request context must be an object.",
    );
  }
  ...
  return {
    ...
    projectConfigPaths: projectConfigPaths.map((path) =>
      canonicalizeExistingOrNearestAncestor(path)
    ),
    userConfigPath,
    installDecisionsPath,
    capabilities,
...
function translateProjectConfigEnv(
...
function translateHomeConfigEnv(
  value: string | undefined,
  home: string,
  fallback: string,
): string {
...
function canonicalCwd(cwd: string): string {
  ...
  if (!existsSync(resolved) || !statSync(resolved).isDirectory()) {
    throw new LspRequestContextParseError(
      "invalid_cwd",
      `LSP request cwd must be an existing directory: ${cwd}`,
    );
  }
...
function capabilitiesField(value: unknown): LspRequestContext["capabilities"] {
  if (!isRecord(value)) {
    throw new LspRequestContextParseError(
      ...
      "LSP request capabilities must be an object.",
    );
  }
...
function stringField(value: Record<string, unknown>, field: string): string {
  ...
  if (typeof fieldValue !== "string" || fieldValue.length === 0) {
    throw new LspRequestContextParseError(
      "invalid_field",
      `LSP request context.${field} must be a non-empty string.`,
    );
  }
  return fieldValue;
}
...
function stringArrayField(
  ...
  if (
    ...
    !fieldValue.every((item) => typeof item === "string" && item.length > 0)
  ) {
    throw new LspRequestContextParseError(
      "invalid_field",
...
function requireAbsolutePath(path: string, field: string): void {
  if (!isAbsolute(path)) {
    throw new LspRequestContextParseError(
      "relative_path",
      `LSP request context.${field} must be absolute: ${path}`,
    );
  }
}
