import { fuzzyMatchModel, normalizeModel, parseModelString, transformModelForProvider } from "@oh-my-opencode/model-core"
...
export function resolveModelForDelegateTask(
  ...
  if (userModel) {
    ...
    if (input.availableModels.size > 0) {
      ...
      const match = fuzzyMatchModel(result.model, new Set(input.availableModels), providerHint)
      if (match) {
        deps.log?.("[resolveModelForDelegateTask] model resolved via user selection (availability confirmed)", {
          model: match,
        })
        return result
      }
      // User explicitly chose this model — honor it even if not in cache
      deps.log?.("[resolveModelForDelegateTask] user model not in available models, using as-is", {
        model: result.model,
      })
    }

    deps.log?.("[resolveModelForDelegateTask] model resolved via user selection", {
      model: result.model,
    })
    return result
  }
  ...
  if (
    ...
    deps.log?.("[resolveModelForDelegateTask] cold cache — skipping resolution")
    return { skipped: true }
  }
  ...
  if (systemDefaultModel) {
    deps.log?.("[resolveModelForDelegateTask] model resolved via system default", {
  ...
  // No model available
  deps.log?.("[resolveModelForDelegateTask] no model resolved — no user selection, no system default")
  return undefined
}
