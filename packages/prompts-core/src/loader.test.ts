...
describe("loadPrompt", () => {
  ...
  test("#given frontmatter fixture #then returns parsed frontmatter", async () => {
    const prompt = await loadPrompt<
      { readonly title: string; readonly enabled: boolean }
    >({
      source: fixtureSource,
      name: "test-prompt",
      variant: "default",
  ...
  test("#given bundled prompt source #then returns synchronously with multiple injections", () => {
    ...
    expect(prompt.body).toBe(
      "Bundled prompt body with Alpha, Beta, and Gamma.\n",
    );
    expect(prompt.filePath).toBe(
      "packages/prompts-core/prompts/test/default.md",
    );
  });
...
async function captureError(operation: () => Promise<unknown>): Promise<Error> {
  ...
  try {
    ...
    if (!(error instanceof Error)) {
      throw new ExpectedErrorMissingError(
        "Expected operation to throw an Error instance",
      );
    }
    capturedError = error;
  }
