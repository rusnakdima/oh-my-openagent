...
describe("resolveVariant", () => {
  test("#given Claude Opus 4.7 model #then resolves default variant", () => {
    expect(resolveVariant({ modelID: "claude-opus-4-7", variants })).toBe(
      "default",
    );
  });
  ...
  test("#given Gemini model #then resolves gemini variant", () => {
    expect(resolveVariant({ modelID: "gemini-3-1-pro", variants })).toBe(
      "gemini",
    );
  });
  ...
  test("#given a kimi-k2-7 variant ordered before kimi #then K2.7 wins and K2.6 stays on kimi", () => {
    ...
    expect(resolveVariant({ modelID: "kimi-k2.7", variants: orderedVariants }))
      .toBe("kimi-k2-7");
    expect(resolveVariant({ modelID: "kimi-k2-6", variants: orderedVariants }))
      .toBe("kimi");
  });
  ...
  test("#given a kimi-k3 variant ordered before kimi #then K3 wins and K2.x stays on its own variants", () => {
    ...
    expect(resolveVariant({ modelID: "kimi-k3", variants: orderedVariants }))
      .toBe("kimi-k3");
    expect(
      resolveVariant({
        modelID: "kimi-for-coding/k3p1",
  ...
  test("#given Prometheus agent #then planner overrides model variant", () => {
    expect(
      resolveVariant({ agentName: "prometheus", modelID: "gpt-5-5", variants }),
    ).toBe(
      "planner",
    );
  });
  ...
  test("#given unknown model #then falls back to default variant", () => {
    expect(resolveVariant({ modelID: "claude-haiku-4-5", variants })).toBe(
      "default",
    );
  });

  test("#given empty variants table #then throws TypeError", () => {
    expect(() => resolveVariant({ modelID: "gpt-5-5", variants: {} })).toThrow(
      TypeError,
    );
  });
});
