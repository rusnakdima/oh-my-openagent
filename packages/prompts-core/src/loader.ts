...
export class PromptFileNotFoundError extends Error {
  ...
  constructor(
    ...
    super(
      `Prompt file not found for ${promptName}/${variant}: ${filePath}`,
      options,
    );
  }
}
...
function isLoadBundledPromptInput(
  input: LoadPromptInput,
): input is LoadBundledPromptInput {
...
async function loadFilesystemPrompt<TFrontmatter = Record<string, unknown>>(
  ...
  const filePath = resolvePromptFilePath(
    input.source.baseDir,
    input.name,
    input.variant,
  );
...
async function readPromptFile(
  ...
  try {
    ...
    if (error instanceof Error && getErrorCode(error) === "ENOENT") {
      throw new PromptFileNotFoundError(promptName, variant, filePath, {
        cause: error,
      });
    }
    throw error;
