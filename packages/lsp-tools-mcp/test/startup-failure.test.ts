...
} from "../src/lsp/startup-failure.js";
...
describe("formatKnownLspStartupFailure", () => {
  ...
  it("#given unrelated process exits #when formatting startup failure #then returns null", () => {
    ...
    const rustPanic = new LspProcessExitedError(
      "rust",
      "/repo",
      1,
      "thread panicked while loading crate graph",
    );
...
describe("handleMissingDependencyError", () => {
  it("#given existing dependency messages #when handling error #then preserves current messages", () => {
    ...
    // when / then
    expect(handleMissingDependencyError(notInstalled)).toBe(
      notInstalled.message,
    );
    expect(handleMissingDependencyError(notConfigured)).toBe(
