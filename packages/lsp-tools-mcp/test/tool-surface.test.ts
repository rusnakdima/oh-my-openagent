...
const expectedToolSurface = [
    ...
    inputSchema: {
      ...
      properties: {
        ...
        scope: {
          ...
          description:
            "Use document for file outline or workspace for project-wide search.",
        },
  ...
  },
  {
    ...
    title: "LSP Rename",
    description:
      "Rename a symbol across the workspace and apply the returned workspace edit.",
    inputSchema: {
      type: "object",
      properties: {
        filePath: { type: "string", description: "Source file path." },
    ...
    inputSchema: {
      ...
      properties: {
        server_id: {
          type: "string",
          description:
            "The LSP server id from the not-installed message (e.g. 'rust').",
        },
        decision: {
          type: "string",
          enum: ["declined", "allowed"],
          description:
            "'declined' silences future prompts; 'allowed' pre-authorizes installation.",
        },
      },
  ...
  it("#given a legacy lsp-prefixed tool alias #when called through JSON-RPC #then aliases remain callable but unlisted", async () => {
    ...
    const request = {
      ...
      params: {
        ...
        arguments: { filePath: "module.wat" },
      },
    };

    // when
    const callResponse = await runWithRequestContext(
      createStandaloneMcpRequestContext(),
      () => handleLspMcpRequest(request),
    );
    const listResponse = await handleLspMcpRequest({
      jsonrpc: "2.0",
      id: 23,
      method: "tools/list",
    });
    ...
    expect(callResponse?.result?.content?.[0]?.text).toContain(
      "No LSP server configured for extension: .wat",
    );
