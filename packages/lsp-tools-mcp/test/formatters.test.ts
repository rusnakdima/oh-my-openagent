...
describe("filterDiagnosticsBySeverity", () => {
  ...
  it("#given mixed severities #when filtering diagnostics #then returns only matching diagnostics", () => {
    ...
    // when / then
    expect(filterDiagnosticsBySeverity(diagnostics, "error")).toEqual([
      diagnostics[0],
    ]);
    expect(filterDiagnosticsBySeverity(diagnostics, "warning")).toEqual([
      diagnostics[1],
    ]);
    expect(filterDiagnosticsBySeverity(diagnostics, "information")).toEqual([
      diagnostics[2],
    ]);
    expect(filterDiagnosticsBySeverity(diagnostics, "hint")).toEqual([
      diagnostics[3],
    ]);
  });
});
