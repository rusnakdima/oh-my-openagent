...
import {
  FakeLspClient,
  type FakeLspClientOptions,
  makeServer,
} from "./helpers/fake-lsp-client.js";
...
describe("LspManager", () => {
  it("#given failed start #when later getClient #then failed client is stopped and a fresh client is built", async () => {
    ...
    const { manager, clients } = setupManager({
      clientFactoryOptions: failingFactory,
    });
    const server = makeServer("typescript");

    // when
    await expect(manager.getClient("/root/a", server)).rejects.toThrow(
      "fake start failed",
    );
