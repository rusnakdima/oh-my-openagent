...
describe("lazycodex bin wrapper", () => {
  test("runs the platform package launcher through Node", async () => {
    ...
    expect((await readFile(join(fixture.captureDir, "env"), "utf8")).trim())
      .toBe("lazycodex");
    expect(await canonicalizePackageRootCapture(fixture)).toBe(
      await realpath(fixture.root),
    );
    expect(
      await realpath(
        (await readFile(join(fixture.captureDir, "exec-path"), "utf8")).trim(),
      ),
    ).toBe(await realpath(nodePath));
    expect(
      (await readFile(join(fixture.captureDir, "args"), "utf8")).trim().split(
        "\n",
      ),
    ).toEqual([
      "install",
  ...
  test("runs the platform binary when published under an npm scope", async () => {
    // #given
    const fixture = await createLazyCodexFixture({
      packageName: "@code-yeongyu/lazycodex",
    });
    const nodePath = Bun.which("node") ?? "node";
  ...
  test("routes npm shim execution from the lazycodex package to the Codex installer", async () => {
    // #given
    const fixture = await createLazyCodexFixture({
      wrapperFileName: "oh-my-opencode.js",
    });
    const nodePath = Bun.which("node") ?? "node";
  ...
  test("routes lazycodex update to the Node installer before platform binary resolution", async () => {
    ...
    expect(
      (await readFile(join(fixture.captureDir, "node-installer-args"), "utf8"))
        .trim().split("\n"),
    ).toEqual([
      "update",
      "--dry-run",
...
function createWrapperTestEnv(
  ...
  const {
    OMO_INVOCATION_NAME: _invocationName,
    OMO_WRAPPER_PACKAGE_ROOT: _wrapperPackageRoot,
    ...baseEnv
  } = process.env;
  return {
    ...baseEnv,
...
async function createLazyCodexFixture(
  options: { packageName?: string; wrapperFileName?: string } = {},
) {
  ...
  await cp(
    fileURLToPath(new URL("./oh-my-opencode.js", import.meta.url)),
    wrapperBin,
  );
  ...
  await cp(
    fileURLToPath(new URL("./platform.js", import.meta.url)),
    join(binDir, "platform.js"),
  );
  await writeFile(
    join(root, "package.json"),
    JSON.stringify({
      name: options.packageName ?? "lazycodex",
      type: "module",
    }),
  );
...
async function canonicalizePackageRootCapture(
  fixture: { readonly captureDir: string },
): Promise<string> {
  return realpath(
    (await readFile(join(fixture.captureDir, "wrapper-root"), "utf8")).trim(),
  );
}
