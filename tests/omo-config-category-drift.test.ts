...
describe("omo config category drift guard", () => {
  test("#given omo category schemas #when comparing zod object shape keys #then omo-config-core matches omo-opencode", () => {
    ...
    // when
    const opencodeThinkingKeys = Object.keys(
      CategoryConfigSchema.shape.thinking.unwrap().shape,
    );
    const omoConfigThinkingKeys = Object.keys(
      OmoCategoryConfigObjectSchema.shape.thinking.unwrap().shape,
    );
