...
The plugin's hook wiring lives in `packages/omo-codex/plugin/hooks/hooks.json`.
Each hook runs `node "${PLUGIN_ROOT}/components/<c>/dist/cli.js" hook <event>`,
reading the event JSON on stdin and writing zero-or-one line of JSON on stdout.
...
Many components are **conditional** (only emit on a matching prompt / OS /
state). For a stable always-fires assertion, prefer:

- Live: `sessionStart` and `userPromptSubmit` `hook/completed` (several
  components wire them, so the events always fire).
  `app-server-drive.sh --plugin` defaults to
  `--expect sessionStart,userPromptSubmit`.
- Unit: `ultrawork` on an `ulw` prompt deterministically injects
  `<ultrawork-mode>`.
