...
1. Builds + copies the plugin to
   `$CODEX_HOME/plugins/cache/sisyphuslabs/omo/<version>/` (then
   `npm ci --omit=dev`).
...
4. Writes a marketplace snapshot under
   `$CODEX_HOME/.tmp/marketplaces/sisyphuslabs/`.
5. Edits `$CODEX_HOME/config.toml`: enables `[plugins."omo@sisyphuslabs"]`, the
   `[marketplaces.sisyphuslabs]` local source, `[features]`
...
- The only thing outside CODEX_HOME is the `omo` runtime wrapper, which targets
  the repo's `dist/cli/index.js` (the CLI ships from the repo).
  `dist/cli/index.js` must exist (run `bun run build` if missing) or that link
  is skipped.
- Cleanup: for an isolated home just `rm -rf "$CODEX_HOME"` (the harness does
  this on exit). For a normal home,
  `node packages/omo-codex/scripts/install-local.mjs uninstall`.
