...
> Note: Each `*_CATEGORY_PROMPT_APPEND` contains the full template string from
> the original. Abbreviated with `...` here for readability. The actual code
> would contain the complete unmodified prompt text.
...
export const PLAN_AGENT_SYSTEM_PREPEND_STATIC_AFTER_SKILLS =
...
> Note: Template strings abbreviated with `...`. Full unmodified content in the
> actual file.
...
export function isPlanAgent(agentName: string | undefined): boolean {
  ...
  return PLAN_AGENT_NAMES.some((name) =>
    lowerName === name || lowerName.includes(name)
  );
}
...
> Note: Each category's `fallbackChain` contains the exact same entries as the
> original `model-requirements.ts`. Abbreviated here.

## 8. MODIFIED: `src/shared/model-requirements.ts`

**Remove** `CATEGORY_MODEL_REQUIREMENTS` from the file body. **Add** re-export
at the end:
...
**Zero consumer files modified.** Backward compatibility maintained through
barrel re-exports.
