...
`src/tools/delegate-task/constants.ts` is **654 lines** with 6 distinct
responsibilities. Violates the 200 LOC modular-code-enforcement rule.
`CATEGORY_MODEL_REQUIREMENTS` is actually in `src/shared/model-requirements.ts`
(311 lines, also violating 200 LOC), not in `constants.ts`.
...
5. **Plan agent prompts** (2 template strings + 4 builder functions, ~250 LOC
   prompt text)
...
- ~280 LOC (mostly prompt text, exempt from 200 LOC per
...
- Imports: `AvailableCategory`, `AvailableSkill` from agents,
  `truncateDescription` from shared
...
Replace entire contents with re-exports from the 5 new files. This maintains
100% backward compatibility for all existing importers.
...
- Keep types (`FallbackEntry`, `ModelRequirement`) and
...
- Single atomic commit:
  `refactor: split delegate-task constants and category model requirements into focused modules`
...
**Zero changes to any consumer files.** All existing imports work via barrel
re-exports.
