...
- `bun test` (split: mock-heavy isolated + batch) - must include new
  `arxiv.test.ts` and updated `index.test.ts`
- `bun run typecheck` - validates `McpNameSchema` enum change propagates
  correctly
