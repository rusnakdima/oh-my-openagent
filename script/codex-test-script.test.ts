...
describe("Codex compatibility test script", () => {
  test("runs the vendored LSP package tests after building its package", () => {
    ...
    // #when
    const lspBuildIndex = packageManifest.indexOf(
      "bun run build:lsp-tools-mcp",
    );
    const lspTestIndex = packageManifest.indexOf(
      "npm --prefix packages/lsp-tools-mcp test",
    );
    const testsLspAfterBuild = lspBuildIndex >= 0 &&
      lspTestIndex > lspBuildIndex;

    // #then
    expect(
      testsLspAfterBuild,
      "test:codex must run the vendored LSP package test suite after building it",
    ).toBe(true);
  });
  ...
  test("runs the CodeGraph component tests before packaged Codex checks", () => {
    ...
    // #when
    const pluginBuildIndex = packageManifest.indexOf(
      "bun run --cwd packages/omo-codex/plugin build",
    );
    const codegraphTestIndex = packageManifest.indexOf(
      "npm --prefix packages/omo-codex/plugin/components/codegraph test",
    );
    const noticesIndex = packageManifest.indexOf(
      "node scripts/check-third-party-notices.mjs --ship",
    );
    const testsCodegraphBeforePackagedChecks = pluginBuildIndex >= 0 &&
  ...
  test("builds lsp-daemon before installer tests copy packaged runtimes", () => {
    // #given
    const testCodexScript =
      JSON.parse(readFileSync(packageManifestPath, "utf8")).scripts
        ?.["test:codex"] ?? "";

    // #when
    const lspDaemonBuildIndex = testCodexScript.indexOf(
      "bun run build:lsp-daemon",
    );
    const pluginBuildIndex = testCodexScript.indexOf(
      "bun run --cwd packages/omo-codex/plugin build",
    );
    const installerTestIndex = testCodexScript.indexOf(
  ...
  test("builds git-bash MCP before installer tests copy packaged runtimes", () => {
    ...
    // #when
    const gitBashBuildIndex = testCodexScript.indexOf(
      "bun run build:git-bash-mcp",
    );
    const installerTestIndex = testCodexScript.indexOf(
      "packages/omo-codex/src/install/install-codex-packaged.test.ts",
    );
    const buildsGitBashBeforeInstallerTests = gitBashBuildIndex >= 0 &&
      installerTestIndex > gitBashBuildIndex;
