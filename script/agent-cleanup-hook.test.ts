...
function subscribeToExactLines(
  path: string,
  ...
  const timeout = setTimeout(
    () =>
      finish(
        new Error(
          `timed out waiting for completion line ${
            expectedLines[nextLine]
          } from ${path}`,
        ),
      ),
    timeoutMs,
  ...
  stream.on("data", (chunk) => {
    ...
    for (
      ...
      if (line !== expected) {
        finish(
          new Error(`expected completion line ${expected}, received ${line}`),
        );
        return;
      }
  ...
  stream.on("end", () => {
    if (nextLine !== expectedLines.length) {
      finish(
        new Error(
          `completion FIFO ended after ${nextLine} of ${expectedLines.length} lines`,
        ),
      );
    }
  });
...
function removeFixture(
  fixture: { readonly repo: string; readonly logDir: string },
): void {
...
describe("agent cleanup and SessionEnd launcher", () => {
  test("#given cleanup scripts #when inspected #then they are executable bash with strict cleanup and deep mode", () => {
    expect(existsSync(cleanup), "script/agent/cleanup.sh must exist").toBe(
      true,
    );
    expect(existsSync(cleanupHook), "script/agent/cleanup-hook.sh must exist")
      .toBe(true);
    if (process.platform !== "win32") {
      expect(
        (statSync(cleanup).mode & 0o111) !== 0,
        "cleanup.sh must be executable",
      ).toBe(true);
      expect(
        (statSync(cleanupHook).mode & 0o111) !== 0,
        "cleanup-hook.sh must be executable",
      ).toBe(true);
    }
  ...
  posixBashTest(
    "#given the SessionEnd launcher #when sync mode runs #then it executes cleanup and logs successfully",
    () => {
...
`);
      try {
        const result = Bun.spawnSync({
          ...
          env: {
            ...
            TMPDIR: fixture.logDir,
          },
        });
        expect(result.exitCode).toBe(0);
        expect(read(join(fixture.repo, "worker.marker"))).toBe("worker-ran\n");
        expect(read(join(fixture.logDir, "oh-my-openagent-cleanup.log")))
  ...
  posixBashTest(
    "#given the SessionEnd launcher #when async mode runs #then it returns before cleanup finishes",
    async () => {
      const fixture = createFixture(`#!/usr/bin/env bash
...
`);
      ...
      expect(Bun.spawnSync(["mkfifo", releasePath, completionPath]).exitCode)
        .toBe(0);
      const releaseFd = openSync(
        releasePath,
        constants.O_RDWR | constants.O_NONBLOCK,
      );
      const [workerStarted, workerPublishing, workerFinished] =
        subscribeToExactLines(completionPath, [
          "started",
          "publishing",
          "finished",
        ]);
  ...
  posixBashTest(
    "#given a hostile cleanup log override #when the launcher runs #then it keeps logs in temp",
    () => {
      ...
      try {
        const result = Bun.spawnSync({
          ...
          env: {
            ...process.env,
            CLAUDE_PROJECT_DIR: fixture.repo,
            OMO_AGENT_CLEANUP_LOG: hostileLog,
            OMO_AGENT_CLEANUP_SYNC: "1",
            TMPDIR: fixture.logDir,
          },
        });
  ...
  test("#given cleanup.sh #when inspected for safety #then it guards repo root and never nukes source or host", () => {
    ...
    for (
      ...
      expect(body, `cleanup must never contain '${pattern}'`).not.toContain(
        pattern,
      );
    }
  });
  ...
  posixBashTest(
    "#given transient files in source and skipped trees #when cleanup runs #then it prunes skipped trees",
    () => {
      ...
      for (
        const [path, content] of [
          [join(repo, "src", "app.tsbuildinfo"), "source transient"],
          [join(repo, "src", ".DS_Store"), "source os transient"],
          [
            join(repo, "node_modules", "pkg", "cache.tsbuildinfo"),
            "dependency cache",
          ],
          [join(repo, ".git", "objects", "cache.tsbuildinfo"), "git cache"],
        ]
      ...
      try {
        ...
        expect(
          existsSync(join(repo, "node_modules", "pkg", "cache.tsbuildinfo")),
        ).toBe(true);
        expect(existsSync(join(repo, ".git", "objects", "cache.tsbuildinfo")))
          .toBe(true);
      } finally {
