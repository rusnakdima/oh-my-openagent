...
const FORBIDDEN_RULES = [
  {
    ...
    matches: (path) => path.includes("node_modules/"),
  },
  {
    name: "senpi payload",
    matches: (path) => path.startsWith("packages/omo-senpi/"),
  },
  {
    name: "retired workflow-selector component",
    matches: (path) => path.includes("components/workflow-selector/"),
  },
];
...
if (offenders.length > 0) {
  console.error(
    `npm payload containment violation (${offenders.length} offending path(s)):`,
  );
  ...
  if (offenders.length > MAX_REPORTED_OFFENDERS) {
    console.error(
      `  ... and ${offenders.length - MAX_REPORTED_OFFENDERS} more`,
    );
  }
  process.exit(1);
}
...
console.log(
  `npm payload containment OK (${paths.length} packed paths, 0 offenders)`,
);
