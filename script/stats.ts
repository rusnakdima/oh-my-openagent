...
async function defaultRunGhApi(): Promise<unknown> {
  const output =
    await $`gh api repos/${GITHUB_REPOSITORY}/releases --paginate --slurp`
      .text();
  return JSON.parse(output);
}

async function fetchNpmStat(
  packageName: string,
  fetchJson: StatsDeps["fetchJson"],
): Promise<DownloadStat> {
  ...
  const count = releases.reduce(
    (total, release) =>
      ...
      release.assets.reduce(
        (assetTotal, asset) => assetTotal + asset.download_count,
        0,
      ),
    0,
  );
...
export async function collectDownloadStats(
  deps: StatsDeps,
): Promise<readonly DownloadStat[]> {
  const npmStats = await Promise.all(
    NPM_PACKAGES.map((packageName) =>
      fetchNpmStat(packageName, deps.fetchJson)
    ),
  );
...
export async function runStats(
  ...
  const events = createPostHogDownloadEvents(
    stats,
    process.env.POSTHOG_KEY ?? "dry-run",
  );
