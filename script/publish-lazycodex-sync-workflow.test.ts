...
describe("LazyCodex marketplace sync workflow", () => {
  test("builds bundled MCP dists before the release marketplace sync builds the Codex plugin", () => {
    ...
    const rootCliBuildIndex = syncStep.indexOf(
    ...
    const codexPluginBuildIndex = syncStep.indexOf(
      "bun run --cwd packages/omo-codex/plugin build",
    );
    const syncScriptIndex = syncStep.indexOf(
      "bun run script/sync-lazycodex-marketplace.ts",
    );
    const buildsMcpDistsBeforeCodexPlugin = gitBashBuildIndex >= 0 &&
      lspBuildIndex >= 0 &&
    ...
    const buildsCodexPluginBeforeMarketplaceSync = codexPluginBuildIndex >= 0 &&
      syncScriptIndex > codexPluginBuildIndex;
    const buildsRootCliBeforeMarketplaceSync = rootCliBuildIndex >= 0 &&
      nodeCliBuildIndex > rootCliBuildIndex &&
      syncScriptIndex > nodeCliBuildIndex;
    ...
    expect(
      buildsCodexPluginBeforeMarketplaceSync,
      "release marketplace sync must build the Codex plugin before copying it",
    ).toBe(true);
    expect(
      buildsRootCliBeforeMarketplaceSync,
      "release marketplace sync must build root omo CLI dists before copying it",
    ).toBe(true);
  });
  ...
  test("uses the vendored LSP tools package directly during release marketplace sync", () => {
    ...
    expect(
      gitBashBuildIndex,
      "release marketplace sync must build the vendored Git Bash MCP package",
    ).toBeGreaterThanOrEqual(0);
    expect(
      lspBuildIndex,
      "release marketplace sync must build the vendored LSP package",
    ).toBeGreaterThanOrEqual(0);
    expect(
      lspDaemonBuildIndex,
      "release marketplace sync must build the vendored LSP daemon package",
    ).toBeGreaterThanOrEqual(0);
  });
  ...
  test("stages generated LazyCodex repository workflow changes", () => {
    ...
    // #when
    const stagesWorkflow = syncStep.includes(
    ...
    expect(
      stagesWorkflow,
      "release marketplace sync must stage generated LazyCodex repository workflow changes",
    ).toBe(true);
  });
});
