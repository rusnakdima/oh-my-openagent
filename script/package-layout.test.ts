...
class PackDryRunError extends Error {
  constructor(readonly exitCode: number, readonly stderr: string) {
    super(
      `bun pm pack --dry-run --ignore-scripts failed with exit code ${exitCode}: ${stderr}`,
    );
...
function collectPackagePathsRecursively(rootPath: string): string[] {
  ...
  while (directories.length > 0) {
    ...
    for (
      const entry of readdirSync(currentDirectory, { withFileTypes: true })
    ) {
...
describe("published package layout", () => {
  test("#given vendored LSP MCP package #when inspecting tracked package files #then it is not a git submodule", () => {
    ...
    // when
    const trackedEntries = execFileSync("git", [
      "ls-files",
      "--stage",
    ...
    const trackedGitmodulesPath = execFileSync("git", [
      "ls-files",
      "--",
      ".gitmodules",
    ], {
    ...
    const gitmodules = trackedGitmodulesPath && existsSync(gitmodulesPath)
      ? Bun.file(gitmodulesPath).text()
      : Promise.resolve("");
  ...
  test(
    "#given Codex LSP file dependency #when packing package #then lsp-tools-mcp package metadata ships",
    async () => {
      // given
  ...
  test(
    "#given generated Codex installer bundle #when packing package #then generated output ships and stale forks do not",
    async () => {
      // given
      const expectedGeneratedInstaller =
        "packages/omo-codex/scripts/install-dist/install-local.mjs";
      const obsoleteForkPrefix = "packages/omo-codex/scripts/install/";

      // when
      const packedPaths = await packDryRunPaths();
      const packedObsoleteForks = [...packedPaths].filter((packagePath) =>
        packagePath.startsWith(obsoleteForkPrefix)
      );
  ...
  test(
    "#given Codex materialized runtime payloads #when packing package #then every current payload target ships",
    async () => {
      // given
      ...
      // when
      const packedPaths = await packDryRunPaths();
      const missingRuntimePaths = expectedRuntimePaths.filter((expectedPath) =>
        !packedPaths.has(expectedPath)
      );
  ...
  test(
    "#given shipped QA skills reference web terminal helpers #when packing package #then helper runtime ships",
    async () => {
      // given
  ...
  test(
    "#given shipped QA skills reference guidance docs #when packing package #then referenced docs ship",
    async () => {
      // given
      ...
      // when
      const packedPaths = await packDryRunPaths();
      const missingDocPaths = expectedDocPaths.filter((expectedPath) =>
        !packedPaths.has(expectedPath)
      );
  ...
  test("#given Codex installer source tree #when checking obsolete forks #then hand-written install mjs files are absent", () => {
    ...
    const obsoleteForks = existsSync(obsoleteForkRoot)
      ? collectPackagePathsRecursively(obsoleteForkRoot).filter((packagePath) =>
  ...
  test(
    "#given dot-directory command and skill assets #when packing package #then slash-command discovery assets ship",
    async () => {
      // given
      const expectedAssetPaths = collectExpectedAssetPaths();
      expect(expectedAssetPaths).toContain(
        ".opencode/command/security-research.md",
      );
      expect(expectedAssetPaths).toContain(
        ".agents/command/security-research.md",
      );
      expect(expectedAssetPaths).toContain(
        ".agents/skills/security-research/SKILL.md",
      );
      expect(expectedAssetPaths).not.toContain(
        ".opencode/command/security-review.md",
      );
      expect(expectedAssetPaths).not.toContain(
        ".agents/command/security-review.md",
      );
      ...
      // then
      const missingPaths = expectedAssetPaths.filter((expectedPath) =>
        !packedPaths.has(expectedPath)
      );
