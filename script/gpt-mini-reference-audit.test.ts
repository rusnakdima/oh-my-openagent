...
describe("GPT Mini reference audit", () => {
  test("repository root decodes a platform-native file URL", () => {
    ...
    const scriptUrl =
      pathToFileURL(join(repositoryDir, "script", "audit.test.ts")).href;
  ...
  test(
    "tracked shipped files no longer reference the retired GPT Mini model",
    async () => {
      // given
      ...
      for (const file of files) {
        ...
        if (
          content.toLowerCase().includes(OLD_MODEL_ID) ||
          content.includes(OLD_DISPLAY_NAME)
        ) {
          matches.push(file);
        }
      }
...
async function trackedFiles(): Promise<string[]> {
  ...
  return stdout.split("\0").filter((file) =>
    file.length > 0 && !file.startsWith(".omo/evidence/")
  );
}
