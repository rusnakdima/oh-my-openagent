...
describe("shared core extraction guardrails", () => {
    ...
    for (const file of files) {
      ...
      for (const forbidden of forbiddenSourcePatterns) {
        const allowedByPackage =
          forbidden.allowPackagePaths?.some((packagePath) =>
  ...
  test("#given core package manifests #when scanned #then they do not depend on harness adapters", async () => {
    // given
    const packageJsonFiles = corePackages.map((packagePath) =>
    ...
    for (const file of packageJsonFiles) {
      ...
      if (
        manifest.includes("@opencode-ai/") ||
        manifest.includes("@oh-my-opencode/omo-codex")
      ) {
  ...
  test("#given the shared extraction plan #when documented #then every PR key has a QA matrix entry", async () => {
    ...
    const missingKeys = requiredPlanKeys.filter((key) =>
      !doc.includes(`PR ${key}`)
    );
    ...
    const missingQaTerms = requiredQaTerms.filter((term) =>
      !doc.includes(term)
    );
