...
export const resumeGuardFindings = {
  prepareExistingTagReuse:
    "prepare-release-state must reuse an existing release tag",
  prepareExistingCommitReuse:
    "prepare-release-state must reuse an existing release commit",
  dispatchTagShaMismatchRefusal:
    "dispatch-provenance-safe-publish must reject an existing tag at a different SHA",
  ohMyOpencodePublishedProbe:
    "publish-main must skip an already-published oh-my-opencode version",
  ohMyOpenagentPublishedProbe:
    "publish-main must skip an already-published oh-my-openagent version",
  lazycodexPublishedProbe:
    "publish-main must skip an already-published lazycodex-ai version",
  omoAiPublishedMetadata:
    "release-metadata must expose whether omo-ai is already published",
  marketplaceUnchangedNoOp:
    "release must leave an unchanged LazyCodex marketplace untouched",
  preparedShaDispatchRouting:
    "the prepared SHA must select and pin the provenance-bearing publish run",
} as const;
...
function section(
  ...
  const end = endMarker === undefined
    ? workflowText.length
    : workflowText.indexOf(endMarker, start);
...
export function assertResumeGuards(workflowText: string): string[] {
  ...
  const releaseMetadata = section(
    workflowText,
    "  release-metadata:",
    "  prepare-release-state:",
  );
  const prepareReleaseState = section(
    workflowText,
    "  prepare-release-state:",
  ...
  const packageProbes = [
    [
      resumeGuardFindings.ohMyOpencodePublishedProbe,
      opencodeProbe,
      "id: check",
      "https://registry.npmjs.org/oh-my-opencode/${VERSION}",
      "if: steps.check.outputs.skip != 'true'",
    ],
    [
      resumeGuardFindings.ohMyOpenagentPublishedProbe,
      openagentProbe,
      "id: check-openagent",
      "https://registry.npmjs.org/oh-my-openagent/${VERSION}",
      "if: steps.check-openagent.outputs.skip != 'true'",
    ],
    [
      resumeGuardFindings.lazycodexPublishedProbe,
      lazycodexProbe,
      "id: check-lazycodex",
      "https://registry.npmjs.org/lazycodex-ai/${VERSION}",
      "steps.check-lazycodex.outputs.skip != 'true'",
    ],
  ] as const;
  for (
    const [finding, probe, stepId, registryUrl, publishCondition]
      of packageProbes
  ) {
    if (
      !hasAll(probe, [
      ...
      ]) || !publishMain.includes(publishCondition)
    ) {
