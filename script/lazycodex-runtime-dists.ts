...
const PLUGIN_COMPONENTS_SOURCE_PATH = join(
  "packages",
  "omo-codex",
  "plugin",
  "components",
...
export async function copyLazycodexRuntimeDists(
  input: CopyLazycodexRuntimeDistsInput,
): Promise<void> {
...
// a broken plugin.
async function collectComponentBinDists(
  sourceRoot: string,
): Promise<RuntimeDist[]> {
  ...
  for (const componentName of await listComponentNames(componentsRoot)) {
    if (
      !(await declaresDistBin(
        join(componentsRoot, componentName, "package.json"),
      ))
    ) continue;
    dists.push({
      ...
      destinationPath: join(
        PLUGIN_COMPONENTS_DESTINATION_PATH,
        componentName,
        "dist",
      ),
    });
...
async function declaresDistBin(manifestPath: string): Promise<boolean> {
  ...
  return Object.values(parsed.bin).some((target) =>
    typeof target === "string" && isDistTarget(target)
  );
}
...
async function copyRuntimeDist(
  input: CopyLazycodexRuntimeDistsInput,
  dist: RuntimeDist,
): Promise<void> {
  ...
  if (!(await isDirectory(sourcePath))) {
    if (input.skipMissing) {
      console.warn(
        `[sync-lazycodex-marketplace] previous-payload reconstruction: skipping missing ${dist.label} at ${sourcePath}`,
      );
      return;
    }
