...
describe("download stats automation", () => {
  test("#given package and release counts #when collected #then stats use aggregate download sources", async () => {
    ...
    const stats = await collectDownloadStats({
      fetchJson: async (url) => ({
        downloads: url.includes("lazycodex-ai") ? 30 : 10,
      }),
      runGhApi: async () => [
    ...
    expect(events.filter((event) => event.properties.source === "npm"))
      .toHaveLength(3);
  });
  ...
  test("#given stats workflow #when inspected #then it is weekly manual and scopes POSTHOG_KEY to send only", async () => {
    ...
    expect(workflow.slice(fetchStepIndex, sendStepIndex)).toContain(
      "GH_TOKEN: ${{ github.token }}",
    );
    expect(workflow.slice(fetchStepIndex, sendStepIndex)).not.toContain(
      "POSTHOG_KEY",
    );
    expect(workflow.slice(sendStepIndex)).toContain(
      "GH_TOKEN: ${{ github.token }}",
    );
    expect(workflow.slice(sendStepIndex)).toContain(
