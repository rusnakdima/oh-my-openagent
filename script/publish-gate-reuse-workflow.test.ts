...
describe("publish gate reuse", () => {
  test("waits for a successful CI push workflow run on the exact prepared SHA", () => {
    ...
    const gateReuseJob = sliceWorkflowSection(
      workflow,
      "  gate-reuse:",
      "  preflight-trust:",
    );
  ...
  test("uses a workflow-capable token and retries release PR writes", () => {
    ...
    expect(prepareJob).toContain(
      'retry_gh "Read release-state PR state" gh pr view',
    );
    expect(prepareJob).toContain(
      'retry_gh "Read release-state PR checks" gh pr view',
    );
  });
  ...
  test("scopes the release PAT away from repo-controlled generation", () => {
    ...
    expect(privilegedStep).toContain(
      "https://github.com/\${GITHUB_REPOSITORY}.git\"' EXIT",
    );
