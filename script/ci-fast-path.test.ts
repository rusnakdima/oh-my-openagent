...
function parseWorkflow(path: URL): Record<string, unknown> {
  ...
  if (!isRecord(parsed)) {
    throw new Error(`${path.pathname} must parse as an object`);
  }
  return parsed;
}
...
function classify(
  ...
  const pathInput = changedPaths.length === 0
    ? ""
...
describe("CI fast-path classifier", () => {
  ...
  test.each([
    ...
    },
    {
      ...
      event: "push",
      message:
        "Merge pull request #6955 from code-yeongyu/release/v5.0.0-beta.8-source-state",
...
describe("CI fast-path workflow wiring", () => {
  ...
  test("keeps schema generation on master and skips duplicate draft generation", () => {
    ...
    if (!isRecord(schema) || !isRecord(draft)) {
      throw new Error("CI write jobs must exist");
    }

    expect(String(schema["if"])).toContain("refs/heads/master");
    expect(String(schema["if"])).not.toContain("run_heavy");
    expect(String(draft["if"])).toContain(
  ...
  test("keeps the Web CI path contract aligned with classifier inputs", () => {
    ...
    if (!isRecord(pullRequest)) {
      throw new Error("Web CI must define pull_request");
    }
