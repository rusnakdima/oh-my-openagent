...
describe("removeStaleSelfPackageTests", () => {
  ...
  test("#given root and Senpi CI jobs #when steps are ordered #then stale self tests are removed before each suite", async () => {
    ...
    const senpiInstallIndex = workflow.indexOf(
      "      - name: Install dependencies",
      senpiJobIndex,
    );
    const senpiCleanupIndex = workflow.indexOf(
      "      - name: Remove stale self-package test copies",
      senpiJobIndex,
    );
    const senpiTestIndex = workflow.indexOf(
      "      - name: Run Senpi compatibility tests",
      senpiJobIndex,
    );
    const cleanupCount =
      workflow.match(/      - name: Remove stale self-package test copies/g)
        ?.length ?? 0;
    ...
    expect(
      workflow.match(
        /run: bun run script\/remove-stale-self-package-tests\.ts/g,
      ),
    ).toHaveLength(2);
  });
});
