...
export function assertAllowlistCoverage(allowlists) {
  ...
  const missing = [...expectedNativeEvents].filter((name) => !actual.has(name))
    .sort();
  const extra = [...actual].filter((name) => !expectedNativeEvents.has(name))
    .sort();
  if (missing.length > 0 || extra.length > 0) {
    throw new Error(
      `OmO Native QA allowlist event coverage diverged: missing=${
        missing.join(",") || "none"
...
function runHelp(senpiBin) {
  const result = spawnSync(senpiBin, ["--help"], {
    cwd: repoRoot,
    encoding: "utf8",
    timeout: 30_000,
  });
  ...
  if (result.status !== 0) {
    throw new Error(
      `senpi --help failed with exit ${result.status}\n${output}`,
    );
  }
  if (!/(?:--print, -p|--print\b[\s\S]*\b-p\b)/.test(output)) {
    const error = new Error(
      "installed Senpi help does not expose a non-interactive print flag",
    );
...
async function startCaptureServer() {
  ...
  if (bunBin === null) {
    throw new Error("Bun executable is required for the capture server");
  }
  const child = spawn(bunBin, [serverPath, capturePath], {
  ...
  const started = await new Promise((resolveStart, rejectStart) => {
    const timeout = setTimeout(
      () =>
        rejectStart(new Error(`capture server startup timed out: ${stderr}`)),
      10_000,
    );
    ...
    child.stdout.on("data", (chunk) => {
      ...
      try {
        resolveStart(JSON.parse(stdout.slice(0, newline)));
      } catch (error) {
        rejectStart(error);
      }
    });
    child.once(
      ...
      (code) =>
        rejectStart(
          new Error(
            `capture server exited before startup with ${code}: ${stderr}`,
          ),
        ),
    );
  });
  if (
    !isRecord(started) || typeof started.port !== "number" ||
    typeof started.pid !== "number"
  ) throw new Error("capture server startup receipt was invalid");
  return {
    child,
    capturePath,
    root,
...
function stepFor(context) {
  ...
  if (promptBIndex === latestPromptIndex) {
    ...
    if (!readUsed) return { type: "tool_call", name: "read", arguments: { path: ${
    JSON.stringify(join(pluginRoot, "skills", "debugging", "SKILL.md"))
  } } }
...
function cleanEnvironment(sandbox, port, extraEnv) {
  ...
  for (const [key, value] of Object.entries(process.env)) {
    ...
    if (
      ...
      key === "SENPI_CODING_AGENT_SESSION_DIR" || key === "XDG_CONFIG_HOME"
    ) continue;
    if (
      key === "DO_NOT_TRACK" || key.startsWith("OMO_") || key === "POSTHOG_HOST"
    ) continue;
    env[key] = value;
  }
  return {
...
async function driveCli(
  { senpiBin, port, label, configEnabled = true, extraEnv = {} },
) {
  ...
  const completed = new Promise((resolveRun, rejectRun) => {
    timeoutHandle = setTimeout(
      () =>
        rejectRun(
          new Error(`${label} Senpi RPC drive timed out after 120000ms`),
        ),
      120_000,
    );
    ...
    child.on("close", (code, signal) => {
      ...
      if (code === 0 && agentEndCount === prompts.length) {
        resolveRun({ code, signal });
      } else {rejectRun(
          new Error(
            `${label} Senpi RPC exited code=${code} signal=${signal} agent_end=${agentEndCount}\n${
              stderr.slice(-4000)
            }`,
          ),
        );}
    ...
    child.stdin.write(
      `${JSON.stringify({ type: "prompt", message: prompts[0] })}\n`,
    );
  });
  try {
...
function assertEnabled(events) {
  ...
  const realPrompts = native.filter((event) =>
    event.event === "prompt_submitted" &&
    event.properties.is_real_user_prompt === true
  );
  check(
    "exactly-three-real-prompts",
    realPrompts.length === 3,
    `observed ${realPrompts.length}; events=${
      events.map((event) => event.event).join(",")
    }`,
  );
  const first = realPrompts.find((event) =>
    event.properties.real_prompt_ordinal_bucket === "1"
  );
  check(
    "first-prompt-ulw-classification",
    first !== undefined &&
      first.properties.is_effective_ultrawork_invocation === true &&
      first.properties.keyword_variant === "ulw" &&
      first.properties.keyword_occurrence_bucket === "1" &&
      first.properties.invocation_stage === "first_arm",
    JSON.stringify(first?.properties ?? null),
  );
  const controls = realPrompts.filter((event) => event !== first);
  check(
    "two-keyword-negative-controls",
    controls.length === 2 &&
      controls.every((event) => event.properties.keyword_any === false),
    controls.map((event) => JSON.stringify(event.properties)).join(" | "),
  );
  for (const name of nativeEvents) {
    check(
      `event-present-${name}`,
      native.some((event) => event.event === name),
      `captured ${name}`,
    );
  }
  check(
    "turn-completed-positive-tokens",
    native.some((event) =>
      event.event === "turn_completed" &&
      Number(event.properties.total_tokens) > 0
    ),
    "at least one turn_completed has total_tokens > 0",
  );
  check(
    "feature-goal-tool",
    native.some((event) =>
      event.event === "feature_used" && event.properties.feature === "goal_tool"
    ),
    "captured feature_used goal_tool",
  );
  check(
    "legacy-dual-emit-presence-only",
    events.some((event) => event.event === "omo_senpi_daily_active"),
    "legacy event present and excluded from all scans",
  );
...
function privacyScan(events, checks) {
  const addPass = (name, detail) =>
    checks.push({ name, result: "PASS", detail });
  for (const event of events) {
    const allowed = new Set([
      ...OMO_NATIVE_PROPERTY_ALLOWLISTS[event.event],
      ...sharedKeys,
      ...sdkAddedKeys,
      ...sdkWireKeys,
    ]);
    for (const key of Object.keys(event.properties)) {
      if (!allowed.has(key)) {
        throw new Error(
          `property-allowlist: ${event.event}.${key} is not documented or SDK-added`,
        );
      }
      if (
        key.startsWith("$") &&
        !OMO_NATIVE_PROPERTY_ALLOWLISTS[event.event].includes(key) &&
        !sharedKeys.has(key) && !sdkAddedKeys.has(key)
      ) {
        throw new Error(
          `dollar-property-allowlist: ${event.event}.${key} is not permitted`,
        );
      }
    }
    for (const [key, value] of walkValues(event.properties)) {
      ...
      if (key === "model_id" || key === "default_model") {
        if (value !== "custom" && !knownModels.has(value)) {
          throw new Error(`known-model-allowlist: ${key}=${value}`);
        }
        continue;
      }
      if (key === "provider" || key === "default_provider") {
        if (value !== "custom" && !knownProviders.has(value)) {
          throw new Error(`known-provider-allowlist: ${key}=${value}`);
        }
        continue;
      }
      if (key === "providers") {
        const providers = value === "" ? [] : value.split(",");
        if (providers.some((provider) => !knownProviders.has(provider))) {
          throw new Error(`known-provider-allowlist: ${key}=${value}`);
        }
        continue;
      }
      if (/(^|\s)(\/|~\/|[A-Za-z]:\\)/.test(value)) {
        throw new Error(`path-privacy: ${event.event}.${key}=${value}`);
      }
      for (const prompt of prompts) {
        for (const fragment of promptFragments(prompt)) {
          if (value.toLowerCase().includes(fragment)) {
            throw new Error(
              `prompt-fragment-privacy: ${event.event}.${key} contains ${
  ...
  addPass(
    "native-event-property-allowlists",
    `all ${events.length} OmO Native events use documented or SDK-added keys`,
  );
  addPass(
    "native-event-path-privacy",
    "no scanned value contains an absolute/home path pattern; model fields use known-model or custom masking",
  );
  addPass(
    "native-event-prompt-fragment-privacy",
    "no scanned value contains any 8+ character driven-prompt substring",
  );
  addPass(
    "legacy-event-scan-exclusion",
    "omo_senpi_daily_active was presence-checked only",
  );
}
...
function promptFragments(prompt) {
  ...
  for (let index = 0; index <= normalized.length - 8; index += 1) {
    fragments.add(normalized.slice(index, index + 8));
  }
  return fragments;
}
...
function* walkValues(value, key = "") {
  ...
  if (isRecord(value)) {
    for (const [childKey, child] of Object.entries(value)) {
      yield* walkValues(child, childKey);
    }
    return;
  }
  yield [key, value];
...
function sanitizeTranscript(text, sandboxes) {
  ...
  for (const sandbox of sandboxes) {
    sanitized = sanitized.replaceAll(
      sandbox.root,
      `<temp-${basename(sandbox.root).split("-").at(-2) ?? "sandbox"}>`,
    );
  }
...
async function closeCaptureServer(capture) {
  ...
  const portProbe = spawnSync("lsof", [
    "-nP",
    `-iTCP:${capture.port}`,
    "-sTCP:LISTEN",
  ], { encoding: "utf8" });
...
function removeSandboxes(sandboxes) {
  return sandboxes.map((sandbox) => {
    ...
    return {
      path: `<temp-${basename(sandbox.root).split("-").at(-2) ?? "sandbox"}>`,
...
function evidenceMarkdown(input) {
  const assertionLines = input.checks.map((check) =>
    `- PASS ${check.name}: ${redactEvidenceText(check.detail)}`
  ).join("\n");
...
- Capture server process receipt: pid ${input.cleanup.serverPid}; server listening false: ${!input
    .cleanup
    .serverListening}; kill-zero-equivalent check failed as required: ${input.cleanup.killZeroFails}.
- Port ${input.cleanup.port} free after close: ${input.cleanup.portFree}; \`lsof -nP -iTCP:${input.cleanup.port} -sTCP:LISTEN\` output was empty: ${
    input.cleanup.lsofOutput === ""
  }.
${
    input.tempCleanup.map((receipt) =>
      `- Removed ${receipt.path}: ${receipt.removed}.`
    ).join("\n")
  }
...
async function main() {
  ...
  const senpiBin = args.senpiBin ??
    findExecutable(process.env.SENPI_BIN?.trim() || "senpi");
  if (senpiBin === null) {
    throw new Error(
      "BlockedClaim: installed senpi CLI was not found on PATH or in node_modules/.bin",
    );
  }
  const help = runHelp(senpiBin);
  if (!existsSync(join(pluginRoot, "extensions", "omo.js"))) {
    throw new Error("built omo-senpi plugin is missing");
  }
  if (!existsSync(goalExtension)) {
    throw new Error("pi-goal extension source is missing");
  }
  ...
  try {
    ...
    if (optOutDntRequests !== 0) {
      throw new Error(
        `DO_NOT_TRACK opt-out emitted ${optOutDntRequests} requests`,
      );
    }

    const configStart = readCaptureRequests(capture).length;
    const config = await driveCli({
      senpiBin,
      port: capture.port,
      label: "config",
      configEnabled: false,
    ...
    const optOutConfigRequests = readCaptureRequests(capture).length -
      configStart;
    if (optOutConfigRequests !== 0) {
      throw new Error(
        `omo.json telemetry.enabled:false emitted ${optOutConfigRequests} requests`,
      );
    }

    const redactedEvents = redactEvents(events);
    writeFileSync(
      join(args.evidenceDir, "captured-payloads.json"),
      `${JSON.stringify(redactedEvents, null, 2)}\n`,
    );
    const transcript = [enabled, dnt, config].map((run) => {
      ...
      for (const line of run.transcript) {
        try {
          ...
          eventTypes.set(
            event.type ?? "unknown",
            (eventTypes.get(event.type ?? "unknown") ?? 0) + 1,
          );
        } catch {}
      }
      return `## ${
        basename(run.sandbox.root)
      }\nagent_end=${run.agentEndCount}\nevent_types=${
        JSON.stringify(Object.fromEntries(eventTypes))
      }\nstderr_tail=${JSON.stringify(run.stderr.slice(-2000))}`;
    }).join("\n\n");
    writeFileSync(
      join(args.evidenceDir, "transcript.txt"),
      `${sanitizeTranscript(transcript, sandboxes).trimEnd()}\n`,
    );

    cleanup = await closeCaptureServer(capture);
    tempCleanup = removeSandboxes([...sandboxes, { root: capture.root }]);
    if (
      !cleanup.killZeroFails || !cleanup.portFree ||
      tempCleanup.some((receipt) => !receipt.removed)
    ...
    writeFileSync(
      join(args.evidenceDir, "cleanup-receipt.json"),
      `${
        JSON.stringify(
          { captureServer: cleanup, tempDirectories: tempCleanup },
          null,
          2,
        )
      }\n`,
    ...
    process.stdout.write(`${
      JSON.stringify(
        {
          result: "PASS",
          evidenceDir: relative(repoRoot, args.evidenceDir),
          assertions: checks.length + 2,
          enabledRequests: enabledRequests.length,
    ...
    if (capture.child.exitCode === null) {
      await closeCaptureServer(capture).catch(() => {});
    }
    const remaining = [...sandboxes, { root: capture.root }].filter((sandbox) =>
      existsSync(sandbox.root)
    );
