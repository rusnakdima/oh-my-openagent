...
function mapOmoAiVersion(rootVersion: string): string {
  ...
  return prereleaseIndex === -1
    ? `${rootVersion}-1`
    : `${rootVersion.slice(0, prereleaseIndex)}-0.${
      rootVersion.slice(prereleaseIndex + 1)
    }`;
}
...
function extractDistTagBlock(text: string): string {
  ...
  if (start < 0 || end < 0) {
    throw new Error("missing DIST_TAG derivation block");
  }
...
describe("omo-ai publish workflow shape", () => {
  test("preserves an explicit prerelease version and derives its beta dist tag", () => {
    ...
    // when
    const explicitVersionPrecedesBump =
      versionRun.indexOf('VERSION="$RAW_VERSION"') <
        versionRun.indexOf('if [ -z "$VERSION" ]');

    // then
    expect(explicitVersionPrecedesBump).toBe(true);
    expect(versionRun).toContain(
  ...
  test("maps every root release to a unique ordered prerelease", () => {
    ...
    const metadataRun =
      namedStep("release-metadata", "Calculate omo-ai metadata").run ?? "";

    expect(outputs).toEqual(expected);
    expect(new Set(outputs).size).toBe(outputs.length);
    expect(outputs.every((version) => version.includes("-"))).toBe(true);
    for (let index = 1; index < outputs.length; index += 1) {
      expect(Bun.semver.order(outputs[index - 1]!, outputs[index]!))
        .toBeLessThan(0);
    }
    expect(metadataRun).toContain('OMO_AI_VERSION="${VERSION}-1"');
    expect(metadataRun).toContain('OMO_AI_VERSION="${VERSION/-/-0.}"');
    expect(metadataRun).toContain(
      "https://registry.npmjs.org/omo-ai/${OMO_AI_VERSION}",
    );
    expect(job("release-metadata").outputs?.omo_ai_version).toBe(
      "${{ steps.omo-ai.outputs.omo_ai_version }}",
    );
    expect(job("release-metadata").outputs?.already_published).toBe(
      "${{ steps.omo-ai.outputs.already_published }}",
    );
  });

  test("checks out before asserting root bin ownership", () => {
    const metadataSteps = steps("release-metadata");
    const checkoutIndex = metadataSteps.findIndex((step) =>
      step.uses === "actions/checkout@v5"
    );
    const assertionIndex = metadataSteps.findIndex((step) =>
      step.name === "Assert omo bin ownership"
    );
  ...
  test("stamps omo-native in both release paths and stages its manifest", () => {
    const prepare = namedStep(
      ...
      "Prepare release state (generation)",
    );
    ...
    expect(prepare.env?.OMO_AI_VERSION).toBe(
      "${{ needs.release-metadata.outputs.omo_ai_version }}",
    );
    expect(update.env?.OMO_AI_VERSION).toBe(
      "${{ needs.release-metadata.outputs.omo_ai_version }}",
    );
    expect(prepare.run).toContain(stampLine);
    expect(update.run).toContain(stampLine);
    expect(prepare.run).toContain(
      "git add package.json packages/omo-native/package.json ",
    );
  });

  test("builds and verifies the payload before stripping token auth", () => {
    const publishSteps = steps("publish-main");
    const containmentIndex = publishSteps.findIndex((step) =>
      step.name === "Verify npm payload containment"
    );
    const buildIndex = publishSteps.findIndex((step) =>
      step.name === "Build omo-ai payload"
    );
    const verifyIndex = publishSteps.findIndex((step) =>
      step.name === "Verify omo-ai payload"
    );
    const originalStripIndex = publishSteps.findIndex((step) =>
      step.name === "Strip token auth from .npmrc to force OIDC"
    );
  ...
  test("publishes omo-ai through beta-only OIDC after every wrapper publish", () => {
    ...
    const lastWrapperPublishIndex = publishSteps.findIndex((step) =>
      step.name === "Publish lazycodex-ai"
    );
    const dedicatedStripIndex = publishSteps.findIndex((step) =>
      step.name === "Strip token auth before omo-ai publish"
    );
    const publishIndex = publishSteps.findIndex((step) =>
      step.name === "Publish omo-ai (beta only)"
    );
    ...
    expect(dedicatedStrip.if).toBe(
      "needs.release-metadata.outputs.already_published != 'true'",
    );
    ...
    expect(
      publish.run,
      "omo-ai publish must hardcode --tag beta rather than DIST_TAG",
    ).not.toContain("$DIST_TAG");
  ...
  test("always runs readiness, dist-tag guard, and live verification", () => {
    ...
    for (
      ...
      expect(step.env?.OMO_AI_VERSION).toBe(
        "${{ needs.release-metadata.outputs.omo_ai_version }}",
      );
      expect(step.env?.ALREADY_PUBLISHED).toBe(
        "${{ needs.release-metadata.outputs.already_published }}",
      );
    }

    expect(
      namedStep("post-publish-verify", "Wait for omo-ai registry readiness")
        .run,
    ).toContain("npm view omo-ai@$OMO_AI_VERSION version");
    expect(namedStep("post-publish-verify", "Guard omo-ai dist-tags").run)
      .toContain("0.0.0-beta.0");
    const liveRun =
      namedStep("post-publish-verify", "Verify omo-ai live install").run ?? "";
  ...
  test("keeps trusted publishing unconditional and DIST_TAG derivation byte-pinned", () => {
    const preflightRun = namedStep(
      "preflight-trust",
      "Verify trusted publisher for release packages",
    ).run ?? "";

    expect(preflightRun).toContain(
