...
const nodes: BuildNode[] = [
  {
    id: "git-bash-mcp",
    command: "bun",
    args: ["run", "build:git-bash-mcp"],
    deps: [],
  },
  {
    id: "ast-grep-mcp",
    command: "bun",
    args: ["run", "build:ast-grep-mcp"],
    deps: [],
  },
  {
    id: "lsp-tools-mcp",
    command: "bun",
    args: ["run", "build:lsp-tools-mcp"],
    deps: [],
  },
  {
    id: "lsp-daemon",
    command: "bun",
    args: ["run", "build:lsp-daemon"],
    deps: [],
  },
  {
    id: "codex-plugin",
    command: "bun",
    args: ["run", "build:codex-plugin"],
    deps: ["git-bash-mcp", "lsp-tools-mcp", "lsp-daemon"],
  },
  {
    id: "senpi-plugin",
    command: "bun",
    args: ["run", "build:senpi-plugin:stage"],
    deps: ["ast-grep-mcp", "lsp-daemon", "codex-plugin"],
  },
  {
    id: "index",
    command: "bun",
  ...
  },
  {
    ...
    args: [
      "build",
      "packages/omo-opencode/src/mcp/openspec-mcp-cli.ts",
      "--outdir",
      "dist/mcp",
      "--target",
      "bun",
  ...
  {
    id: "openspec-mcp-server",
    command: "bun",
    args: [
      "build",
      "packages/omo-opencode/src/mcp/openspec-mcp.ts",
      "--outdir",
      "dist/mcp",
      "--target",
      "bun",
  ...
  {
    id: "shared-skills-assets",
    command: "bun",
    args: ["run", "build:shared-skills-assets"],
    deps: ["index"],
  },
  {
    id: "node-require-shim",
    command: "bun",
    args: ["run", "build:node-require-shim"],
    deps: ["index"],
  },
  {
...
async function materializeOnce() {
  await runNode({
    id: "materialize",
    command: "bun",
    args: ["run", "build:materialize-frontend"],
    deps: [],
  }, process.env);
}
  ...
  try {
    while (done.size < graph.length) {
      for (const id of pending) {
        ...
        running.set(
          id,
          runNode(node, env, liveChildren).then(() => {
            running.delete(id);
            done.add(id);
          }),
        );
      ...
      if (running.size === 0) {
        throw new Error(
          `build: dependency cycle or unresolved deps among ${
            [...pending].join(", ")
          }`,
        );
      }
...
function runNode(
  ...
  liveChildren?: Set<ReturnType<typeof spawn>>,
): Promise<void> {
