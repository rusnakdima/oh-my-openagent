...
async function validateRootCliRuntime(
  pluginRoot: string,
  issues: string[],
): Promise<void> {
  await collectBundleFileIssue(
    pluginRoot,
    pluginRoot,
    "dist/cli/index.js",
    "missing root CLI runtime path",
    issues,
    {
      allowEscape: false,
    },
...
async function validatePluginMcpManifests(
  pluginRoot: string,
  issues: string[],
): Promise<void> {
...
async function validatePluginMcpManifest(
  pluginRoot: string,
  manifestPath: string,
  issues: string[],
): Promise<void> {
  ...
  for (const [serverName, server] of Object.entries(parsed.mcpServers)) {
    ...
    for (const arg of server.args) {
      ...
      await collectBundleFileIssue(
        ...
        manifestRoot,
        arg,
        `missing MCP runtime path for ${serverName}`,
        issues,
        {
...
async function validatePluginHookCommands(
  pluginRoot: string,
  issues: string[],
): Promise<void> {
  ...
  for (const hookManifestPath of hookManifestPaths) {
    const parsed: unknown = JSON.parse(
      await readFile(hookManifestPath, "utf8"),
    );
    ...
    for (const command of commands) {
      for (const relativePath of extractPluginRootPaths(command)) {
        const hookCommandRoot = relativePath.startsWith("components/")
          ? pluginRoot
          : hookPluginRoot;
        await collectBundleFileIssue(
          pluginRoot,
...
async function validatePluginManagedBins(
  pluginRoot: string,
  issues: string[],
): Promise<void> {
  ...
  for (const entry of entries) {
    ...
    for (
      const binTarget of await readBinTargets(
        join(componentsRoot, entry.name, "package.json"),
      )
    ) {
      // Marketplace payload paths are POSIX; join() would emit backslashes on win32.
      const relativePath = join("components", entry.name, binTarget).split(sep)
        .join("/");
      await collectBundleFileIssue(
        pluginRoot,
        pluginRoot,
        relativePath,
        "missing managed bin target",
        issues,
        {
          allowEscape: false,
        },
...
async function readBinTargets(manifestPath: string): Promise<string[]> {
  ...
  return Object.values(parsed.bin).filter((target): target is string =>
...
async function collectBundleFileIssue(
  ...
  if (
    !options.allowEscape &&
    !(await isRealPathWithinRoot(bundleRoot, targetPath))
  ) {
    pushIssue(issues, `${message}: ${relativePath} escapes plugin root`);
    return;
  }
