...
const WINDOWS_INTEGRATION_TEST_TIMEOUT = process.platform === "win32"
  ? 20_000
  : 5_000;
...
async function writePluginFixture(
  sourceRoot: string,
  options: WritePluginFixtureOptions = {},
): Promise<void> {
  await writeJson(
    join(sourceRoot, "packages", "omo-codex", "marketplace.json"),
    {
      name: "sisyphuslabs",
  ...
  await writeJson(
    join(sourceRoot, "packages", "omo-codex", "plugin", "package.json"),
    {
  ...
  await writeJson(
    join(sourceRoot, "packages", "omo-codex", "plugin", "hooks", "hooks.json"),
    {
      hooks: {
        SessionStart: [
          {
            hooks: [
              {
                type: "command",
                command:
                  'node "${PLUGIN_ROOT}/components/bootstrap/dist/cli.js" hook session-start',
                commandWindows:
                  'powershell -NoProfile -ExecutionPolicy Bypass -File "${PLUGIN_ROOT}\\components\\bootstrap\\scripts\\bootstrap.ps1"',
                timeout: 30,
                statusMessage:
                  "LazyCodex(1.2.3): Checking Bootstrap Provisioning",
              },
            ],
          },
        ],
            ...
            hooks: [
              {
                type: "command",
                command:
                  'node "${PLUGIN_ROOT}/components/comment-checker/dist/cli.js" hook post-tool-use',
  ...
  await writeJson(
    join(sourceRoot, "packages", "omo-codex", "plugin", ".mcp.json"),
    {
      mcpServers: {
        git_bash: {
          command: "node",
  ...
  await writeFile(
    join(sourceRoot, "packages", "omo-codex", "plugin", "README.md"),
    "omo\n",
  );
  ...
  await writeManagedBinComponentFixture(sourceRoot, "start-work-continuation", [
    "omo-start-work-continuation",
  ]);
  await writeManagedBinComponentFixture(sourceRoot, "ulw-loop", [
    "omo-ulw-loop",
    "ulw",
    "ulw-loop",
  ]);
  await mkdir(join(sourceRoot, "packages", "git-bash-mcp", "dist"), {
    recursive: true,
  });
  await writeFile(
    join(sourceRoot, "packages", "git-bash-mcp", "dist", "cli.js"),
    "#!/usr/bin/env node\n",
  );
  await mkdir(join(sourceRoot, "packages", "lsp-tools-mcp", "dist"), {
    recursive: true,
  });
  await writeFile(
    join(sourceRoot, "packages", "lsp-tools-mcp", "dist", "cli.js"),
    "#!/usr/bin/env node\n",
  );
  await mkdir(join(sourceRoot, "packages", "lsp-daemon", "dist"), {
    recursive: true,
  });
  await writeFile(
    join(sourceRoot, "packages", "lsp-daemon", "dist", "cli.js"),
    "#!/usr/bin/env node\n",
  );
  await mkdir(join(sourceRoot, "dist", "cli"), { recursive: true });
  await writeFile(
    join(sourceRoot, "dist", "cli", "index.js"),
    "#!/usr/bin/env node\n",
  );
  await mkdir(join(sourceRoot, "dist", "cli-node"), { recursive: true });
  await writeFile(
    join(sourceRoot, "dist", "cli-node", "index.js"),
    "#!/usr/bin/env node\n",
  );
  await mkdir(
    join(
  ...
  await mkdir(
    join(sourceRoot, "packages", "omo-codex", "plugin", ".ulw", "evidence"),
    { recursive: true },
  );
  await writeFile(
    join(
  ...
  await mkdir(join(sourceRoot, "packages", "omo-codex", "plugin", ".claude"), {
    recursive: true,
  });
  await writeFile(
    join(
...
async function writeManagedBinComponentFixture(
  sourceRoot: string,
  ...
  await writeJson(join(componentRoot, "package.json"), {
    ...
    bin: Object.fromEntries(
      binNames.map((binName) => [binName, "./dist/cli.js"]),
    ),
  });
          ...
          hooks: [
            {
              type: "command",
              command:
                `node "\${PLUGIN_ROOT}/components/${componentName}/dist/cli.js" hook stop`,
            },
          ],
        },
      ],
...
describe("sync-lazycodex-marketplace", () => {
  test("#given marketplace sync #when copying plugin bundle #then emits self-contained mcp paths", async () => {
    ...
    const marketplace = JSON.parse(
      await readFile(
        join(lazycodexRoot, ".agents", "plugins", "marketplace.json"),
        "utf8",
      ),
    );
    ...
    const rootMarketplace = JSON.parse(
      await readFile(join(lazycodexRoot, "marketplace.json"), "utf8"),
    );
    expect(rootMarketplace).toEqual(marketplace);
    const manifest = JSON.parse(
      await readFile(
        join(lazycodexRoot, "plugins", "omo", ".codex-plugin", "plugin.json"),
        "utf8",
      ),
    );
    expect(manifest).toMatchObject({ name: "omo", version: "1.2.3" });
    const workflow = await readFile(
      join(lazycodexRoot, ".github", "workflows", "pr-source-guidance.yml"),
      "utf8",
    );
    expect(workflow).toContain("PR source guidance");
    const mcpManifest = JSON.parse(
      await readFile(
        join(lazycodexRoot, "plugins", "omo", ".mcp.json"),
        "utf8",
      ),
    );
    expect(Object.hasOwn(mcpManifest.mcpServers, astGrepMcpServerName)).toBe(
      false,
    );
    expect(mcpManifest.mcpServers.git_bash.args[0]).toBe(
      "./components/git-bash-mcp/dist/cli.js",
    );
    expect(mcpManifest.mcpServers.lsp.args[0]).toBe(
      "./components/lsp-daemon/dist/cli.js",
    );
    ...
    await expectPathMissing(
      join(lazycodexRoot, "plugins", "omo", "node_modules"),
    );
  ...
  test("rejects a source tree without a Codex plugin manifest", async () => {
    ...
    const lazycodexRoot = await mkdtemp(
      join(tmpdir(), "omo-sync-bad-lazycodex-"),
    );
    await writeJson(
      join(sourceRoot, "packages", "omo-codex", "marketplace.json"),
      {
  ...
  test(
    "#given older package payload without repository workflow #when syncing marketplace #then plugin bundle still syncs",
    async () => {
      // given
      const sourceRoot = await mkdtemp(join(tmpdir(), "omo-sync-old-source-"));
      const lazycodexRoot = await mkdtemp(
        join(tmpdir(), "omo-sync-old-lazycodex-"),
      );
      await writePluginFixture(sourceRoot, {
        includeLazycodexRepositoryWorkflow: false,
      });
  ...
  test("#given release version env #when syncing marketplace #then repository payload is stamped with release version", async () => {
    ...
    const sourceRoot = await mkdtemp(
      join(tmpdir(), "omo-sync-release-source-"),
    );
    const lazycodexRoot = await mkdtemp(
      join(tmpdir(), "omo-sync-release-lazycodex-"),
    );
    await writePluginFixture(sourceRoot);
    ...
    const packageJson = JSON.parse(
      await readFile(
        join(lazycodexRoot, "plugins", "omo", "package.json"),
        "utf8",
      ),
    );
    const hooks = JSON.parse(
      await readFile(
        join(lazycodexRoot, "plugins", "omo", "hooks", "hooks.json"),
        "utf8",
      ),
    );
    ...
    expect(hooks.hooks.PostToolUse[0].hooks[0].statusMessage).toBe(
      "LazyCodex(4.7.9): Checking Comments",
    );
    expect(componentHooks.hooks.PostToolUse[0].hooks[0].statusMessage).toBe(
      "LazyCodex(4.7.9): Checking Comments",
    );
  });

  test(
    "#given stale mcp runtime path #when syncing marketplace #then rejects the broken bundle",
    async () => {
      // given
      const sourceRoot = await mkdtemp(
        join(tmpdir(), "omo-sync-stale-mcp-source-"),
      );
      const lazycodexRoot = await mkdtemp(
        join(tmpdir(), "omo-sync-stale-mcp-lazycodex-"),
      );
      await writePluginFixture(sourceRoot);
      await writeJson(
        join(sourceRoot, "packages", "omo-codex", "plugin", ".mcp.json"),
        {
          mcpServers: {
            lsp: {
              command: "node",
              args: [
                "./components/lsp/packages/lsp-tools-mcp/dist/cli.js",
                "mcp",
              ],
              cwd: ".",
            },
  ...
  test(
    "#given missing hook command target #when syncing marketplace #then rejects the broken bundle",
    async () => {
      // given
      const sourceRoot = await mkdtemp(
        join(tmpdir(), "omo-sync-missing-hook-source-"),
      );
      const lazycodexRoot = await mkdtemp(
        join(tmpdir(), "omo-sync-missing-hook-lazycodex-"),
      );
      await writePluginFixture(sourceRoot);
      await writeJson(
        join(
                ...
                hooks: [
                  {
                    type: "command",
                    command:
                      'node "${PLUGIN_ROOT}/components/rules/dist/cli.js" hook session-start',
                  },
                ],
              },
            ],
  ...
  test("#given a previous payload without lsp-daemon dist #when syncing with allowMissingBundledDists #then reconstructs and skips the missing dist", async () => {
    ...
    const lazycodexRoot = await mkdtemp(
      join(tmpdir(), "omo-sync-prev-lazycodex-"),
    );
    await writePluginFixture(sourceRoot);
    await rm(join(sourceRoot, "packages", "lsp-daemon", "dist"), {
      recursive: true,
      force: true,
    });
    await rm(join(sourceRoot, "dist", "cli"), { recursive: true, force: true });
    await rm(join(sourceRoot, "dist", "cli-node"), {
      recursive: true,
      force: true,
    });
    await writeJson(
    ...
    // when
    await syncLazycodexMarketplace({
      sourceRoot,
      lazycodexRoot,
      allowMissingBundledDists: true,
    });
    ...
    await expectPathMissing(
      join(lazycodexRoot, "plugins", "omo", "dist", "cli", "index.js"),
    );
    await expectPathMissing(
      join(lazycodexRoot, "plugins", "omo", "dist", "cli-node", "index.js"),
    );
    expect(
      (await stat(
        join(
  ...
  test(
    "#given complete tree with bootstrap command and commandWindows targets #when syncing marketplace #then bundle validation passes",
    async () => {
      // given
      const sourceRoot = await mkdtemp(
        join(tmpdir(), "omo-sync-complete-source-"),
      );
      const lazycodexRoot = await mkdtemp(
        join(tmpdir(), "omo-sync-complete-lazycodex-"),
      );
      await writePluginFixture(sourceRoot);
      ...
      await expectPathMissing(
        join(lazycodexRoot, "plugins", "omo", "components", "lsp", ".mcp.json"),
      );
    },
    WINDOWS_INTEGRATION_TEST_TIMEOUT,
  );
  ...
  test("#given missing bootstrap commandWindows target #when syncing marketplace #then rejects naming the bootstrap.ps1 path", async () => {
    // given
    const sourceRoot = await mkdtemp(
      join(tmpdir(), "omo-sync-missing-ps1-source-"),
    );
    const lazycodexRoot = await mkdtemp(
      join(tmpdir(), "omo-sync-missing-ps1-lazycodex-"),
    );
    await writePluginFixture(sourceRoot);
    await rm(
      join(
  ...
  test("#given a zero-byte component dist #when syncing marketplace #then rejects it as zero bytes", async () => {
    // given
    const sourceRoot = await mkdtemp(
      join(tmpdir(), "omo-sync-zero-byte-source-"),
    );
    const lazycodexRoot = await mkdtemp(
      join(tmpdir(), "omo-sync-zero-byte-lazycodex-"),
    );
    await writePluginFixture(sourceRoot);
    await writeFile(
      join(
  ...
  test("#given a component ships a nested .mcp.json #when syncing marketplace #then it is dropped from the bundle and the sync succeeds", async () => {
    // given
    const sourceRoot = await mkdtemp(
      join(tmpdir(), "omo-sync-nested-mcp-source-"),
    );
    const lazycodexRoot = await mkdtemp(
      join(tmpdir(), "omo-sync-nested-mcp-lazycodex-"),
    );
    await writePluginFixture(sourceRoot);
    await writeJson(
      join(
  ...
  test("#given multiple missing referenced targets #when syncing marketplace #then reports the full list", async () => {
    // given
    const sourceRoot = await mkdtemp(
      join(tmpdir(), "omo-sync-multi-missing-source-"),
    );
    const lazycodexRoot = await mkdtemp(
      join(tmpdir(), "omo-sync-multi-missing-lazycodex-"),
    );
    await writePluginFixture(sourceRoot);
    await rm(
      join(
  ...
  test("#given a managed-bin component dist is not built #when syncing marketplace #then rejects naming the missing component dist", async () => {
    // given
    const sourceRoot = await mkdtemp(
      join(tmpdir(), "omo-sync-missing-component-dist-source-"),
    );
    const lazycodexRoot = await mkdtemp(
      join(tmpdir(), "omo-sync-missing-component-dist-lazycodex-"),
    );
    await writePluginFixture(sourceRoot);
    await rm(
      join(
    ...
    // when/then
    await expect(syncLazycodexMarketplace({ sourceRoot, lazycodexRoot }))
      .rejects.toThrow(/missing built start-work-continuation component dist/);
  });
  ...
  test("#given a missing lsp-daemon dist without the flag #then still hard-throws", async () => {
    ...
    const lazycodexRoot = await mkdtemp(
      join(tmpdir(), "omo-sync-strict-lazycodex-"),
    );
    await writePluginFixture(sourceRoot);
    await rm(join(sourceRoot, "packages", "lsp-daemon", "dist"), {
      recursive: true,
    ...
    // when/then
    await expect(syncLazycodexMarketplace({ sourceRoot, lazycodexRoot }))
      .rejects.toThrow(/missing built LSP daemon dist/);
  });
});
