...
const passthrough = args.filter((arg) =>
  arg !== "--no-uninstall" && !arg.startsWith("--version=")
);
...
if (!skipUninstall) {
  console.log(
    "[install-codex-dev] Removing the current Codex Light install...",
  );
  const uninstallStatus = runInstaller(["uninstall"]);
  if (uninstallStatus !== 0) {
    console.error(
      `[install-codex-dev] uninstall exited with code ${uninstallStatus}`,
    );
...
console.log(
  `[install-codex-dev] Installing the dev build from ${repositoryRoot} as version "${devVersion}"...`,
);
const installStatus = runInstaller(["install", "--no-tui", ...passthrough]);
if (installStatus !== 0) {
  console.error(
    `[install-codex-dev] install exited with code ${installStatus}`,
  );
  process.exit(installStatus);
}

console.log(
  `[install-codex-dev] Done. Installed the dev build stamped as "${devVersion}".`,
);
