...
async function updateAllPackageVersions(newVersion: string): Promise<void> {
  ...
  for (const platformPackage of PLATFORM_PACKAGES) {
    const pkgPath = new URL(
      `../packages/${platformPackage.packageDir}/package.json`,
      import.meta.url,
    ).pathname;
...
async function generateChangelog(
  ...
  try {
    const log = await $`git log v${compareTag}..HEAD --oneline --format="%h %s"`
      .text();
    const commits = log
      .split("\n")
      .filter((line) =>
...
async function getContributors(previous: string): Promise<string[]> {
  ...
  try {
    const compare =
      await $`gh api "/repos/code-yeongyu/oh-my-openagent/compare/v${previous}...HEAD" --jq '.commits[] | {login: .author.login, message: .commit.message}'`
        .text();
...
async function publishPackage(
  ...
  const provenanceArgs = process.env.CI && useProvenance
    ? ["--provenance"]
    : [];
    ...
    for (let i = 0; i < PLATFORM_PACKAGES.length; i += BATCH_SIZE) {
      ...
      console.log(
        `\n  Batch ${batchNum}/${totalBatches}: ${
          batch.map((platformPackage) => platformPackage.platform).join(", ")
        }`,
      );
...
async function gitTagAndRelease(
  ...
  for (const platformPackage of PLATFORM_PACKAGES) {
    await $`git add packages/${platformPackage.packageDir}/package.json`
      .nothrow();
  }
...
async function main() {
  ...
  const newVersion = versionOverride ||
