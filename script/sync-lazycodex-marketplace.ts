...
const MARKETPLACE_SOURCE_PATH = join(
  "packages",
  "omo-codex",
  "marketplace.json",
);
...
const MARKETPLACE_DESTINATION_PATH = join(
  ".agents",
  "plugins",
  "marketplace.json",
);
...
const LAZYCODEX_PR_SOURCE_GUIDANCE_DESTINATION_PATH = join(
  ".github",
  "workflows",
  "pr-source-guidance.yml",
);
...
export async function syncLazycodexMarketplace(
  input: SyncLazycodexMarketplaceInput,
): Promise<void> {
  ...
  if (marketplace.name !== "sisyphuslabs") {
    throw new Error(
      `Sisyphus Labs marketplace manifest must be named sisyphuslabs, got ${marketplace.name}`,
    );
  }
  ...
  if (pluginManifest.name !== "omo") {
    throw new Error(
      `Sisyphus Labs plugin manifest must be named omo, got ${pluginManifest.name}`,
    );
  }

  const marketplaceContents = await readFile(marketplacePath, "utf8");
  const destinationMarketplacePath = join(
    lazycodexRoot,
  ...
  await writeFile(
    join(lazycodexRoot, MARKETPLACE_ROOT_DESTINATION_PATH),
    marketplaceContents,
  );
  ...
  await stampReleaseVersion(
    destinationPluginRoot,
    input.releaseVersion ?? process.env.LAZYCODEX_RELEASE_VERSION,
  );
...
async function copyLazycodexRepositoryWorkflow(
  sourceRoot: string,
  ...
  const destinationPath = join(
    lazycodexRoot,
    LAZYCODEX_PR_SOURCE_GUIDANCE_DESTINATION_PATH,
  );
...
async function collectHookManifestPathsInto(
  root: string,
  paths: string[],
): Promise<void> {
  let entries;
  ...
  for (const entry of entries) {
    ...
    if (
      entry.isFile() && entry.name === "hooks.json" &&
      path.endsWith(`${sep}hooks${sep}hooks.json`)
    ) {
      paths.push(path);
    }
  }
...
if (import.meta.main) {
  ...
  if (lazycodexRoot === undefined) {
    throw new Error(
      "Usage: bun run script/sync-lazycodex-marketplace.ts <source-root> <lazycodex-root>",
    );
  }
  await syncLazycodexMarketplace({
    sourceRoot,
    lazycodexRoot,
    allowMissingBundledDists: args.includes("--previous-payload"),
  });
}
