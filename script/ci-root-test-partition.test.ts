...
const rootConfig = readFileSync(
  new URL("../bunfig.root.toml", import.meta.url),
  "utf8",
);
const win2ConfigPath = new URL("../bunfig.win2.toml", import.meta.url);
const win2ParallelConfigPath = new URL(
...
function quotedPatterns(config: string): readonly string[] {
  return [...config.matchAll(/"([^"]+\/\*\*)"/g)].map((match) =>
    match[1] ?? ""
  );
}
...
describe("root test CI partition", () => {
  ...
  test("#given global zauc mocks #when Windows root tests are partitioned #then omo-opencode stays in one process", () => {
    ...
    expect(job).toContain(
      "bun test packages/senpi-task/src/runners/rpc-process.windows.test.ts",
    );
    expect(job).toContain(
    ...
    expect(quotedPatterns(readFileSync(win2ConfigPath, "utf8"))).toContain(
    ...
    expect(parallelConfig).toContain(
