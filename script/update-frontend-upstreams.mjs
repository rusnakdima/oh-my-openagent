...
function attributionShas(content) {
  return [...content.matchAll(/Pinned upstream commit:\s*([0-9a-f]{40})/g)].map(
    (match) => match[1],
  );
}
...
function check() {
  ...
  const drifted = upstreams.filter((upstream, position) =>
    pinned[position] !== heads[position]
  );
  if (drifted.length > 0) {
    for (const upstream of drifted) {
      process.stderr.write(
        `[update-upstreams] ATTRIBUTION pin drift for ${upstream.name}\n`,
      );
    }
    return 1;
  }
  process.stdout.write(
    "[update-upstreams] all ATTRIBUTION pins match the submodule HEADs\n",
  );
  return 0;
}
...
function update() {
  for (const upstream of upstreams) {
    execFileSync("git", ["submodule", "update", "--remote", upstream.path], {
      cwd: repoRoot,
      stdio: "inherit",
    });
  }
  ...
  process.stdout.write(
    `[update-upstreams] refreshed ${upstreams.length} pins; review + commit the submodule + ATTRIBUTION changes\n`,
  );
  return 0;
}
