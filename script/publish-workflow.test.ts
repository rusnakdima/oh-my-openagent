...
function sliceWorkflowSectionToEnd(
  workflow: string,
  startMarker: string,
): string {
  const start = workflow.indexOf(startMarker);
...
function readPackageScript(scriptName: string): string {
  ...
  if (typeof script !== "string") {
    throw new Error(`package.json scripts.${scriptName} must be a string`);
  }
  return script;
}
...
describe("test workflows", () => {
  ...
  test("builds publish-main from the prepared release SHA", () => {
    ...
    // #when
    const checksOutPreparedRelease = publishMainJob.includes(
    ...
    // #then
    expect(
      checksOutPreparedRelease,
      "publish-main must build the release-state commit after version synchronization",
    ).toBe(true);
    expect(
      regeneratesInstallerBeforeMainBuild,
      "publish-main must regenerate the embedded Codex installer from that release commit",
    ).toBe(true);
  });
  ...
  test("dispatches a source-pinned publish run before provenance-bearing release operations", () => {
    // #given
    const workflow = readFileSync(publishWorkflowPath, "utf8");
    const prepareJob = sliceWorkflowSection(
      workflow,
      "  prepare-release-state:",
      "  dispatch-provenance-safe-publish:",
    );
    const dispatchJob = sliceWorkflowSection(
      workflow,
      "  dispatch-provenance-safe-publish:",
      "  publish-main:",
    );
    ...
    const publishPlatformJob = sliceWorkflowSection(
      workflow,
      "  publish-platform:",
      "  release:",
    );
    ...
    // #when
    const exposesPreparedSourceInput = workflow.includes(
      "prepared_release_sha:",
    );
    const validatesDispatchSource = prepareJob.includes(
    ...
    const releaseChecksOutPreparedSource = releaseJob.includes(
      "ref: ${{ needs.prepare-release-state.outputs.release_sha }}",
    );
    ...
    // #then
    expect(
      exposesPreparedSourceInput,
      "the follow-up publish run must receive the exact prepared source SHA",
    ).toBe(true);
    expect(
      validatesDispatchSource,
      "the provenance-bearing run must reject a dispatch SHA different from the prepared source",
    ).toBe(true);
    expect(
      dispatchesPinnedTagRun,
      "the preparation run must tag and dispatch the exact prepared source",
    ).toBe(true);
    expect(
      provenanceOperationsRequirePinnedRun,
      "npm, platform, marketplace, and GitHub release operations must only run from the pinned follow-up dispatch",
    ).toBe(true);
    expect(
      releaseChecksOutPreparedSource,
      "GitHub release and marketplace operations must check out the prepared source directly",
    ).toBe(true);
    expect(
      releaseDoesNotRestampOrRetag,
      "the provenance-bearing release run must not mutate its release source",
    ).toBe(true);
  });
  ...
  test("exercise root checks across linux macos and windows", () => {
    ...
    // #when
    const hasCrossOsMatrix = workflow.includes(
    ...
    expect(
      hasCrossOsMatrix,
      "CI root checks must cover Linux, macOS, and Windows",
    ).toBe(true);
    expect(hasMatrixRunner, "CI root checks must run on the selected matrix OS")
      .toBe(true);
  });
  ...
  test("runs codex compatibility checks on every supported os without serializing build", () => {
    ...
    const codexCompatibilityJob = sliceWorkflowSection(
      workflow,
      "  codex-compatibility:",
      "  lazycodex-published-smoke:",
    );
    const buildJob = sliceWorkflowSection(
      workflow,
      "  build:",
      "  auto-commit-schema:",
    );
    const autoCommitSchemaJob = sliceWorkflowSection(
      workflow,
      "  auto-commit-schema:",
      "  draft-release:",
    );
    const draftReleaseJob = sliceWorkflowSectionToEnd(
      workflow,
      "  draft-release:",
    );
    ...
    const hasSupportedOsMatrix = codexCompatibilityJob.includes(
      "os: [ubuntu-latest, macos-latest, windows-latest]",
    );
    const hasCodexCommand = workflow.includes("run: bun run test:codex");
    const buildWaitsOnlyForMode = buildJob.includes("needs: [ci-mode]") &&
      !buildJob.includes("needs: [test") &&
      !buildJob.includes("needs: [typecheck");
    const buildHasReadOnlyContentsPermission = buildJob.includes(
      "permissions:\n      contents: read",
    );
    const allRootChecks =
    ...
    // #then
    expect(hasCodexMatrixJob, "CI must expose a Codex compatibility matrix job")
      .toBe(true);
    expect(
      hasSupportedOsMatrix,
      "CI Codex compatibility must cover supported OSes",
    ).toBe(true);
    expect(
      hasCodexCommand,
      "Codex compatibility job must run the shared Codex test script",
    ).toBe(true);
    expect(
      buildWaitsOnlyForMode,
      "Build may wait for classification but must stay parallel with validation jobs",
    ).toBe(true);
    expect(
      buildHasReadOnlyContentsPermission,
      "Parallel build must explicitly stay read-only; write actions belong behind the all-check gate",
    ).toBe(true);
    expect(
      writeGateNeedsAllChecks,
      "Schema auto-commit must wait for all root checks and build",
    ).toBe(true);
    expect(
      draftReleaseNeedsAllChecks,
      "Draft release must wait for all root checks and build",
    ).toBe(true);
  });
  ...
  test("runs Codex compatibility tests with Node available for the self-built MCP runtimes", () => {
    ...
    const runsCodexTests = codexCompatibilityJob.includes(
      "run: bun run test:codex",
    );

    expect(
      hasNodeSetup,
      "Codex compatibility must setup Node for MCP package builds",
    ).toBe(true);
    expect(runsCodexTests, "Codex compatibility must run bun run test:codex")
      .toBe(true);
  });
  ...
  test("builds bundled MCP runtimes before Codex compatibility tests", () => {
    ...
    const requiredPrerequisites = [
      ...
      [
        "vendored lsp-tools package tests",
        "npm --prefix packages/lsp-tools-mcp test",
      ],
      [
        "nested Codex plugin npm install",
        "npm --prefix packages/omo-codex/plugin ci",
      ],
      [
        "nested Codex plugin build",
        "bun run --cwd packages/omo-codex/plugin build",
      ],
      [
        "CodeGraph component tests",
        "npm --prefix packages/omo-codex/plugin/components/codegraph test",
      ],
      [
        "third-party notices ship check",
    ...
    for (const [description, command] of requiredPrerequisites) {
      ...
      expect(index, `test:codex must run ${description}`).toBeGreaterThan(
        previousIndex,
      );
      previousIndex = index;
    }
  ...
  test("runs Git Bash installer regressions in Codex compatibility checks", () => {
    ...
    // #when
    const codexTestScriptRunsGitBashRegressions = packageManifest.includes(
      "packages/omo-codex/scripts/install-local-git-bash-preflight.test.mjs",
    ) &&
      packageManifest.includes(
        "packages/omo-codex/scripts/install-generated-bundle.test.mjs",
      );

    // #then
    expect(
      codexTestScriptRunsGitBashRegressions,
      "test:codex must cover Windows Git Bash preflight and install guidance",
    ).toBe(true);
  });
  ...
  test("tracks the nested Codex plugin lockfile used by npm ci", () => {
    ...
    // #when
    const lockfileIsUnignored = gitignore.includes(
      "!packages/omo-codex/plugin/package-lock.json",
    );
    const trackedLockfile = execFileSync("git", [
      "ls-files",
      "packages/omo-codex/plugin/package-lock.json",
    ], {
    ...
    expect(
      lockfileIsUnignored,
      "the aggregate Codex plugin lockfile must escape the root package-lock ignore",
    ).toBe(true);
    expect(
      trackedLockfile,
      "npm ci in CI requires the nested Codex plugin package-lock.json to be tracked",
    ).toBe("packages/omo-codex/plugin/package-lock.json");
  });
  ...
  test("pins every workflow Bun setup to the tested runtime", () => {
    for (const workflowPath of workflowPaths) {
      ...
      // #when
      const unpinnedBunLines = bunVersionLines.filter((line) =>
        line !== pinnedBunVersion
      );

      // #then
      expect(unpinnedBunLines, `${workflowPath.pathname} must pin Bun to 1.4.0`)
        .toEqual([]);
    }
  });
});
