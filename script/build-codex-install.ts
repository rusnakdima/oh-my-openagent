...
const installerSourceRoot = join(
  repositoryRoot,
  "packages",
  "omo-codex",
  "src",
...
async function installerSourceFiles(): Promise<readonly string[]> {
  const entries = await readdir(installerSourceRoot, {
    recursive: true,
    withFileTypes: true,
  });
  return entries
...
export function parseCodexInstallerArtifact(
  ...
  const match = /^\/\/ omo-codex-install:([a-f0-9]{64}):([a-f0-9]{64})$/.exec(
    markerLine,
  );
...
export async function buildCodexInstaller(
  ...
  const body = nodeBuiltinSource.startsWith("#!/usr/bin/env node")
    ...
    : nodeBuiltinSource;
  const marker =
    `${CODEX_INSTALL_BUILD_MARKER_PREFIX}${await digestCodexInstallerSources()}:${
      digestText(body)
    }`;
