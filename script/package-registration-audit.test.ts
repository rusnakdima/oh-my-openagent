...
const skillPackagePaths: readonly string[] = [
  "packages/shared-skills",
] as const;
const shimSourceRoots: readonly string[] = [
  "packages/omo-opencode/src",
  "packages/omo-codex/src",
] as const;
const reExportShimFirstLinePattern =
...
function readStringRecordField(
  ...
  for (const [entryKey, entryValue] of Object.entries(value)) {
    if (typeof entryValue !== "string") {
      throw new Error(`Expected ${key}.${entryKey} to be a string`);
    }
...
async function collectTrackedFiles(
  ...
  return (await Promise.all(roots.map((root) => collectFiles(root, predicate))))
    .flat().toSorted();
}
...
function unclassifiedRootPackageWorkspaces(
  workspaces: readonly string[],
): readonly string[] {
  return workspaces.filter((path) =>
    path.startsWith("packages/") && !isManagedWorkspacePackage(path)
  ).toSorted();
}
...
function extractTypecheckPackagePaths(command: string): readonly string[] {
  return [
    ...command.matchAll(/tsgo --noEmit -p (packages\/[^ ]+\/tsconfig\.json)/g),
  ]
    .map((match) => match[1])
...
function extractSharedCoreGuardPackagePaths(source: string): readonly string[] {
  ...
  if (body === undefined) {
    throw new Error(
      "Could not find corePackages in shared-core-extraction-guard.test.ts",
    );
  }
...
describe("package registration audit", () => {
  test("#given managed packages #when root registration is audited #then workspaces typecheck and dev deps stay aligned", async () => {
    ...
    const managedPackagePaths = (await discoverPackagePaths()).filter(
      isManagedWorkspacePackage,
    );
    const managedWorkspacePaths = managedPackagePaths.filter((path) =>
      !mcpPackagePaths.includes(path) || root.workspaces.includes(path)
    );
    const expectedTypecheckPaths = managedWorkspacePaths.filter((path) =>
      existsSync(join(path, "tsconfig.json"))
    );
    const expectedDevDependencyNames = (
      await Promise.all(
        managedWorkspacePaths
          ...
          .map((path) =>
            readManifest(join(path, "package.json")).then((manifest) =>
              manifest.name
            )
          ),
      )
    ...
    // when
    const unknownWorkspacePaths = unclassifiedRootPackageWorkspaces(
      root.workspaces,
    );
    const actualWorkspacePaths = root.workspaces.filter(
      isManagedWorkspacePackage,
    ).toSorted();
    const actualTypecheckPaths = extractTypecheckPackagePaths(
      root.scripts["typecheck:packages"] ?? "",
    ).filter(
      isRootManagedTypecheckPackage,
    );
    const actualDevDependencyNames = Object.entries(root.devDependencies)
      .filter((entry) =>
        entry[1] === "workspace:*" && entry[0].startsWith("@oh-my-opencode/")
      )
      .map((entry) => entry[0])
      .toSorted();
  ...
  test("#given an unknown packages workspace #when audited #then classification rejects it by path", () => {
    ...
    // then
    expect(unknownWorkspacePaths).toEqual([
      "packages/zz-qa-unregistered-probe",
    ]);
  });
  ...
  test("#given shared extraction guard #when audited #then every core package is covered", async () => {
    ...
    const guardSource = await readFile(
      "script/shared-core-extraction-guard.test.ts",
      "utf8",
    );
  ...
  test("#given package test scripts #when nested tests exist #then recursive globs are registered", async () => {
    // given
    const packagePaths = (await discoverPackagePaths()).filter(
      isManagedWorkspacePackage,
    );
    ...
    for (const packagePath of packagePaths) {
      ...
      if (
        !testScript.startsWith("bun test") ||
      ...
      const hasNestedTests = tests.some((path) =>
        relative(join(packagePath, "src"), path).includes("/")
      );
      if (
        hasNestedTests && testScript.includes("src/*.test.ts") &&
        !testScript.includes("src/**/*.test.ts")
      ) {
  ...
  test("#given package dependencies #when ROADMAP layers are checked #then reverse edges stay at zero", async () => {
    ...
    const manifests = await Promise.all(
      packagePaths.map((path) => readManifest(join(path, "package.json"))),
    );
    const packagePathByName = new Map(
      manifests.map((manifest, index) => [manifest.name, packagePaths[index]]),
    );
    ...
    for (const [index, manifest] of manifests.entries()) {
      ...
      const sourceLayer = sourcePath === undefined
        ? undefined
        : packageLayer(sourcePath);
      ...
      for (const dependencyName of Object.keys(dependencies)) {
        ...
        const targetLayer = targetPath === undefined
          ? undefined
          : packageLayer(targetPath);
  ...
  test(
    "#given exact re-export shim scan #when inventory docs are checked #then total targets and every path are present",
    async () => {
      // given
      const docPath = "docs/reference/re-export-shim-inventory.md";
      const [doc, shims] = await Promise.all([
        readFile(docPath, "utf8"),
        collectReExportShims(),
      ]);

      // when
      const totalMatch = /Total shim exports found: (\d+)\./.exec(doc);
      if (totalMatch?.[1] === undefined) {
        throw new Error(`${docPath} is missing the total shim count`);
      }

      const documentedTotal = Number.parseInt(totalMatch[1], 10);
      const targetPackages = [
        ...new Set(shims.map((shim) => shim.targetPackage)),
      ].toSorted();
      const missingTargetPackages = targetPackages.filter((targetPackage) =>
        !doc.includes(`\`${targetPackage}\``)
      );
      const missingPaths = shims.map((shim) => shim.path).filter((path) =>
        !doc.includes(`\`${path}\``)
      );
