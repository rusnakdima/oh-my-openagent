...
class PackageFilesAnchorError extends Error {
  constructor() {
    super(
      "package.json files list no longer contains the postinstall.mjs anchor",
    );
...
function withPackageAssetRoots(packageJsonText: string): string {
  const missingAssetRoots = packageAssetRoots.filter((rootPath) =>
    !packageJsonText.includes(`"${rootPath}"`)
  );
  ...
  const insertedAssetRoots = missingAssetRoots.map((rootPath) =>
    `    "${rootPath}",`
  ).join("\n");
  return packageJsonText.replace(
    filesAnchor,
    `${filesAnchor}${insertedAssetRoots}\n`,
  );
}
...
function preparePackageJsonForDotAssetPacking(): void {
  ...
  packageJsonWasTemporarilyModified =
    packageJsonTextWithAssetRoots !== packageJsonText;
...
function collectExistingFakeInternalSkillArtifactPaths(): string[] {
  return fakeInternalSkillArtifactRootPaths
    ...
    .flatMap((packagePath) =>
      collectPackagePathsRecursively(join(repositoryRoot, packagePath))
    )
    .sort();
}
...
describe("published package layout exclusions", () => {
  beforeAll(() => {
    ...
    try {
      ...
      writeFakeInternalArtifacts([
        ...fakeInternalSkillArtifactPaths,
        ...fakeInternalCommandArtifactPaths,
      ]);
    } catch (error) {
      removeFakeInternalArtifacts();
  ...
  test(
    "#given internal-only skill assets #when packing package #then forbidden skill assets do not ship",
    async () => {
      // given
      expect(collectExistingFakeInternalSkillArtifactPaths()).toEqual(
        fakeInternalSkillArtifactPaths.toSorted(),
      );
      for (const packagePath of fakeInternalSkillArtifactPaths) {
        expect(readFileSync(join(repositoryRoot, packagePath), "utf8"))
          .toStartWith("---\n");
      }
      ...
      // then
      const packedInternalSkillPaths = fakeInternalSkillArtifactPaths.filter((
        packagePath,
      ) => packedPaths.has(packagePath));
  ...
  test(
    "#given internal-only command assets #when packing package #then forbidden command assets do not ship",
    async () => {
      // given
      ...
      // then
      const packedInternalCommandPaths = fakeInternalCommandArtifactPaths
        .filter((packagePath) => packedPaths.has(packagePath));
