...
describe("sync-lazycodex-marketplace root CLI payload", () => {
  test("#given root CLI dists #when syncing marketplace #then the plugin payload carries the omo runtime targets", async () => {
    ...
    const sourceRoot = await mkdtemp(
      join(tmpdir(), "omo-sync-root-cli-source-"),
    );
    const lazycodexRoot = await mkdtemp(
      join(tmpdir(), "omo-sync-root-cli-lazycodex-"),
    );
    ...
    await expect(readOptionalFile(bunRuntime)).resolves.toContain(
      "omo bun runtime",
    );
    await expect(readOptionalFile(nodeRuntime)).resolves.toContain(
      "omo node runtime",
    );
  });
  ...
  test("#given missing root CLI dist #when syncing marketplace #then validation rejects the broken payload", async () => {
    // given
    const sourceRoot = await mkdtemp(
      join(tmpdir(), "omo-sync-missing-root-cli-source-"),
    );
    const lazycodexRoot = await mkdtemp(
      join(tmpdir(), "omo-sync-missing-root-cli-lazycodex-"),
    );
