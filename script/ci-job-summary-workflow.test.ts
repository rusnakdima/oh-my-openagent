...
const workflowExpectations = [
  ...
  {
    path: ".github/workflows/package-labels.yml",
    jobs: ["ensure-labels", "label-pull-request", "label-issue"],
  },
  {
  ...
  { path: ".github/workflows/stats.yml", jobs: ["stats"] },
  {
    ...
    jobs: ["format-lint-typecheck-build"],
  },
  ...
  for (const line of lines) {
    ...
    if (jobMatch !== null) {
      ...
      if (nextJob === undefined) {
        throw new Error(`Unable to read job name from line: ${line}`);
      }
      currentJob = nextJob;
    ...
    if (currentJob !== undefined && /^    steps:\s*$/.test(line)) {
      currentJobHasSteps = true;
    }
  }
...
function hasSummaryWriter(jobSection: string): boolean {
  return jobSection.includes("name: Write job summary") &&
    jobSection.includes("GITHUB_STEP_SUMMARY");
}
...
describe("GitHub workflow job summaries", () => {
  test("#given a new step-based workflow job #when workflow coverage is checked #then the job is discovered automatically", () => {
    ...
    expect(discoverStepBasedJobs(workflow)).toEqual([
      "existing",
      "newly-added",
    ]);
  });
  ...
  test("#given repository workflows #when inspected #then every step-based job writes a concise Markdown summary", () => {
    const expectedWorkflowPaths = workflowExpectations.map((expectation) =>
      expectation.path
    ).sort();
    ...
    for (const expectation of workflowExpectations) {
      ...
      expect(
        discoverStepBasedJobs(workflow),
        `${expectation.path} step-based job list must stay covered`,
      ).toEqual(
        expectation.jobs,
      );
      ...
      for (const job of expectation.jobs) {
        ...
        expect(
          hasSummaryWriter(jobSection),
          `${expectation.path} ${job} must write a job summary`,
        ).toBe(true);
      }
    }
  });
  ...
  test("#given a privileged publish summary #when it renders dispatch inputs #then raw inputs are passed through env", () => {
    ...
    const summaryStep = sliceWorkflowSectionToEnd(
      workflow,
      "      - name: Write job summary",
    );
  ...
  test(
    "#given summary inputs #when the shared writer runs #then it emits the Markdown contract GitHub renders",
    () => {
      ...
      const result = spawnSync(
        "bash",
        [".github/scripts/write-job-summary.sh"],
        {
      ...
      try {
        ...
        expect(summary).toContain(
          "[Open run](https://github.com/code-yeongyu/oh-my-openagent/actions/runs/42/attempts/2)",
        );
      } finally {
