...
describe("web terminal visual QA guidance", () => {
  test("#given QA skill guidance #when TUI visual evidence is required #then central guidance owns helper details", () => {
    ...
    const centralDoc = readFileSync(
      new URL("docs/reference/web-terminal-visual-qa.md", repo),
      "utf8",
    );
    const pointerFiles = [
    ...
    for (const pointer of pointers) {
      expect(
        pointer.text,
        `${pointer.path} must point at central terminal evidence guidance`,
      ).toContain(
  ...
  test("#given PR visual evidence docs #when attaching screenshots #then GitHub user attachment guidance is discoverable", () => {
    // given
    const repo = new URL("..", import.meta.url);
    const attachmentDoc = readFileSync(
      new URL("docs/reference/github-attachment-upload.md", repo),
      "utf8",
    );
    const pointerFiles = [
      "docs/AGENTS.md",
    ...
    expect(attachmentDoc).toContain(
      "https://github.com/user-attachments/assets/<uuid>",
    );
    for (const pointer of pointers) {
      expect(
        pointer.text,
        `${pointer.path} must point at attachment upload guidance`,
      ).toContain(
