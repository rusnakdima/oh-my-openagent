...
function parseMergeParents(value) {
  ...
  if (!Number.isInteger(count) || count < 0) {
    throw new Error(
      `expected non-negative integer merge parents, received ${value}`,
    );
  }
  return count;
}
...
function parseLabels(value) {
  ...
  if (
    !Array.isArray(parsed) || parsed.some((label) => typeof label !== "string")
  ) {
...
export function classifyCiMode({
  ...
  const generatedReleasePush = eventName === "push" &&
    diffAvailable &&
    isRealMerge &&
    generatedReleaseMerge.test(subject);
  const webOnly = diffAvailable && changedPaths.length > 0 &&
    changedPaths.every(isWebPath);
  ...
  const fullMatrix = eventName === "push" ||
    !diffAvailable ||
...
if (
  ...
  if (
    ...
    throw new Error(
      "--event, --message, --diff-available, and --merge-parents are required",
    );
  }
