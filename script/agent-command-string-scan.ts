...
export const HUMAN_COMMAND_RE =
  /\bomo (install|uninstall|cleanup|doctor|run|get-local-version|version|mcp)\b/g;
export const BARE_BIN_RE =
  /(command -v omo|\/bin\/omo|=omo|\$\(which omo\))(?![\w-])/g;

export const ALLOWLIST_RELATIVE_PATH =
  "script/agent-command-string-audit.allowlist.json";
export const AUDIT_TEST_RELATIVE_PATH =
  "script/agent-command-string-audit.test.ts";
export const SCAN_MODULE_RELATIVE_PATH = "script/agent-command-string-scan.ts";
export const SCAN_TEST_RELATIVE_PATH =
  "script/agent-command-string-scan.test.ts";
...
export function isExcluded(filePath: string): boolean {
  return filePath === ALLOWLIST_RELATIVE_PATH ||
    ...
    filePath.split("/").some((part) =>
      part === "node_modules" || part === "install-dist" || part === "dist"
    );
}
...
export function collectHits(workspaceRoot: string): string[] {
  return trackedSourceFiles(workspaceRoot)
    .flatMap((filePath) =>
      fingerprintSource(
        filePath,
        readFileSync(resolve(workspaceRoot, filePath), "utf8"),
      )
    )
    .sort();
}
