...
export function generateTelemetrySchemaBlock(
  schemas = OMO_NATIVE_EVENT_SCHEMAS,
) {
  if (
    schemas === null || typeof schemas !== "object" || Array.isArray(schemas)
  ) {
  ...
  const rows = entries.flatMap(([eventName, properties]) => {
    ...
    if (
      ...
      throw new Error(
        `Telemetry event ${eventName} must contain at least one property schema`,
      );
    }
    ...
    return Object.entries(properties).map(([propertyName, schema]) => {
      ...
      if (
        ...
        throw new Error(
          `Telemetry property ${eventName}.${propertyName} must contain schema metadata`,
        );
      }
      const type = schema.type;
      if (type !== "boolean" && type !== "number" && type !== "string") {
        throw new Error(
          `Telemetry property ${eventName}.${propertyName} has invalid type ${
            String(type)
          }`,
        );
      }
      ...
      if (
        ...
        throw new Error(
          `Telemetry property ${eventName}.${propertyName} values must be a non-empty array`,
        );
      }
      const valueCell = values === undefined ? "-" : values.map((value) => {
        assertMarkdownIdentifier(
          value,
          `allowed value for ${eventName}.${propertyName}`,
        );
        return `\`${value}\``;
      }).join(", ");
...
function assertMarkdownIdentifier(value, description) {
  if (
    ...
    throw new Error(
      `Telemetry ${description} must be a non-empty Markdown-safe string`,
    );
  }
}
