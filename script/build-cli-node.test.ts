...
describe("node-target CLI build (lazycodex#47)", () => {
  test("build:cli-node produces a bundle that runs under plain node", () => {
    ...
    expect(buildScript, "root package.json must define build:cli-node")
      .toBeDefined();

    const build = spawnSync("bun", ["run", "build:cli-node"], {
      cwd: repoRoot,
      encoding: "utf8",
    });
    ...
    // #when
    const help = spawnSync("node", ["dist/cli-node/index.js", "--help"], {
      cwd: repoRoot,
      encoding: "utf8",
    });
    const version = spawnSync("node", ["dist/cli-node/index.js", "--version"], {
      cwd: repoRoot,
      encoding: "utf8",
    });
    ...
    expect(
      help.status,
      `node dist/cli-node/index.js --help failed:\n${help.stderr}`,
    ).toBe(0);
  ...
  test(
    "the node CLI bundle runs from an isolated payload with no node_modules in reach",
    async () => {
      ...
      // #when
      const version = spawnSync("node", [
        join(isolated, "index.js"),
        "--version",
      ], { encoding: "utf8" });
      const help = spawnSync("node", [join(isolated, "index.js"), "--help"], {
        encoding: "utf8",
      });
      ...
      expect(
        version.status,
        `isolated node CLI --version failed:\n${version.stderr}`,
      ).toBe(0);
  ...
  test("doctor json runs under plain node without Bun globals", () => {
    ...
    if (doctorStatus === null) {
      throw new Error(`node doctor exited without a status:\n${doctor.stderr}`);
    }
  ...
  test("the main build chain and the lazycodex-ai payload carry the node CLI", () => {
    ...
    expect(
      buildOrchestrator,
      "the build orchestrator must produce dist/cli-node",
    ).toContain("build:cli-node");
    expect(workflow, "lazycodex-ai files list must ship dist/cli-node")
      .toContain('"dist/cli-node"');
  });
});
