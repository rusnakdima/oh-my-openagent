...
describe("LazyCodex release version stamping workflow", () => {
  test("stamps hook status messages with the release version before publishing lazycodex-ai", () => {
    ...
    // #when
    const exportsReleaseVersionForHookBuild = buildStep.includes(
      "LAZYCODEX_RELEASE_VERSION: ${{ needs.release-metadata.outputs.version }}",
    );

    // #then
    expect(
      exportsReleaseVersionForHookBuild,
      "lazycodex npm build must pass the release version into hook status message generation",
    ).toBe(true);
  });
  ...
  test("stamps hook status messages with the release version before syncing the LazyCodex repository", () => {
    ...
    // #when
    const exportsReleaseVersionForRepoSync = syncStep.includes(
      "LAZYCODEX_RELEASE_VERSION: ${{ needs.release-metadata.outputs.version }}",
    );

    // #then
    expect(
      exportsReleaseVersionForRepoSync,
      "lazycodex repository sync must pass the release version into copied plugin metadata",
    ).toBe(true);
  });
});
