...
export function isExcludedReleaseNoteSubject(subject: string): boolean {
  return EXCLUDED_PREFIX_PATTERN.test(subject) ||
    CONTAINED_SURFACE_PATTERN.test(subject);
}
...
export function selectPreviousReleaseTag(
  ...
  const candidates = tags.flatMap((tag) => {
    ...
    if (
      ...
      releaseChannel(version) !== targetChannel ||
      Bun.semver.order(version, target) >= 0
    ) return [];
    return [{ tag, version }];
  });
...
async function getLatestReleasedTag(
  currentVersion: string,
): Promise<string | null> {
  try {
    const output =
      await $`gh release list --exclude-drafts --limit 100 --json tagName --jq '.[].tagName'`
        .text();
...
async function generateChangelog(previousTag: string): Promise<string[]> {
  ...
  try {
    const log = await $`git log ${previousTag}..HEAD --oneline --format="%h %s"`
      .text();
    const commits = log
      .split("\n")
      .filter((line) =>
        line && !isExcludedReleaseNoteSubject(line.replace(/^\w+ /, ""))
      );
...
async function getContributors(previousTag: string): Promise<string[]> {
  ...
  try {
    const compare =
      await $`gh api "/repos/code-yeongyu/oh-my-openagent/compare/${previousTag}...HEAD" --jq '.commits[] | {login: .author.login, message: .commit.message}'`
        .text();
