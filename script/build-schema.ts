...
async function main() {
  ...
  await Bun.write(
    OMO_SCHEMA_OUTPUT_PATH,
    JSON.stringify(createOmoJsonSchema(), null, 2),
  );

  const legacySchema = createOhMyOpenCodeJsonSchema();
  await Bun.write(
    LEGACY_SCHEMA_OUTPUT_PATH,
  ...
  console.log(
    `✓ JSON Schemas generated: ${OMO_SCHEMA_OUTPUT_PATH}, ${LEGACY_SCHEMA_OUTPUT_PATH}`,
  );
}
