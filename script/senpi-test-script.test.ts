...
describe("Senpi compatibility test script", () => {
  test("#given published root package #when payload contract is inspected #then senpi payload is contained while local build stays available", () => {
    ...
    const buildOrchestrator = readFileSync(
    ...
    const hasStandaloneBuildScript =
      manifest.scripts?.["build:senpi-plugin"] === [
    ...
    const senpiNode =
      /id: "senpi-plugin"[\s\S]*?args: \["run", "build:senpi-plugin:stage"\][\s\S]*?deps: \["ast-grep-mcp", "lsp-daemon", "codex-plugin"\]/
        .test(
          buildOrchestrator,
        );
    ...
    expect(
      hasStandaloneBuildScript,
      "standalone Senpi build must build the shared daemon once before staging",
    ).toBe(true);
    expect(
      hasStageScript,
      "root scripts must expose a stage-only Senpi artifact build",
    ).toBe(true);
    expect(
      buildOrchestrator,
      "the build orchestrator must generate Senpi plugin artifacts before publishing",
    ).toContain(
      "build:senpi-plugin:stage",
    );
    expect(
      senpiNode,
      "build graph senpi-plugin must wait for every shared runtime and plugin dependency",
    ).toBe(true);
    expect(
      prepublishOnlyScript,
      "prepublishOnly must route through build, which includes the Senpi plugin build",
    ).toContain(
      "bun run build",
    );
  });
  ...
  test("#given a packed-layout root with only senpi plugin artifacts #when installer runs #then settings points at that shipped plugin tree", async () => {
    ...
    try {
      ...
      await writeFile(
        join(pluginRoot, "extensions", "omo-task.js"),
        "export const createTaskComponent = () => ({})\n",
      );
      await writeFile(
        join(pluginRoot, "extensions", "omo-member.js"),
        "export const runMember = () => undefined\n",
      );
      await writeFile(
        join(pluginRoot, "extensions", "reflection-persona.md"),
        "# reflection persona fixture\n",
      );
      await writeFile(
        join(pluginRoot, "extensions", "dream-persona.md"),
        "# dream persona fixture\n",
      );
      await writeFile(
        join(pluginRoot, "extensions", "facts-persona.md"),
        "# facts persona fixture\n",
      );
      ...
      await writeFile(
        join(pluginRoot, "extensions", "memory-run-supervisor.mjs"),
        "#!/usr/bin/env node\n",
      );
      ...
      await mkdir(join(pluginRoot, "runtime", "lsp-daemon", "dist"), {
        recursive: true,
      });
      await writeFile(
        join(pluginRoot, "runtime", "lsp-daemon", "dist", "cli.js"),
        "console.log('cli')\n",
      );
      await writeFile(
        join(pluginRoot, "runtime", "lsp-daemon", "dist", "index.js"),
        "export {}\n",
      );
      await writeFile(
        join(pluginRoot, "runtime", "lsp-daemon", "dist", "index.d.ts"),
        "export {}\n",
      );
      await writeFile(
        join(pluginRoot, "runtime", "lsp-daemon", "dist", "daemon-client.js"),
        "export {}\n",
      );
      await writeFile(
        join(pluginRoot, "runtime", "lsp-daemon", "dist", "daemon-client.d.ts"),
        "export {}\n",
      );
      await writeFile(
      ...
      await writeFile(
        join(
          ...
          "lsp-daemon",
          "dist",
          ".omo-runtime-manifest.json",
        ),
        "{}\n",
      );
      await mkdir(join(pluginRoot, "runtime", "agent-toolkit", "ulw-loop"), {
        recursive: true,
      });
      await writeFile(
        join(pluginRoot, "runtime", "agent-toolkit", "cli.js"),
        "console.log('agent-toolkit')\n",
      );
      await writeFile(
        join(pluginRoot, "runtime", "agent-toolkit", "ulw-loop", "cli.js"),
        "console.log('ulw-loop')\n",
      );
      const toolkitShim = join(
        pluginRoot,
        "runtime",
        "agent-toolkit",
        "omo-agent-toolkit",
      ...
      await writeFile(
        join(pluginRoot, "runtime", "agent-toolkit", "omo-agent-toolkit.cmd"),
        '@echo off\r\nnode "%~dp0cli.js" %*\r\n',
      );
      await mkdir(join(pluginRoot, "runtime", "ast-grep-mcp"), {
        recursive: true,
      });
      const astGrepRuntime = join(
        pluginRoot,
        "runtime",
        "ast-grep-mcp",
        "cli.js",
      ...
      const astGrepManifestPath = join(
        pluginRoot,
        "runtime",
        "ast-grep-mcp",
        "manifest.json",
      ...
      await writeFile(
        astGrepManifestPath,
        JSON.stringify({
          sha256: createHash("sha256").update(astGrepRuntimeContent).digest(
            "hex",
          ),
          mode: 0o755,
      ...
      expect(
        commands,
        "packed installs must use shipped artifacts without requiring source rebuild scripts",
      ).toEqual([]);
    } finally {
      await Promise.all([
  ...
  test("#given root scripts #when test:senpi is inspected #then it runs the hermetic adapter gate in order", () => {
    ...
    const commandIndexes = expectedCommands.map((command) =>
    ...
    for (let index = 0; index < commandIndexes.length; index += 1) {
      ...
      if (
        current === undefined || previous === undefined || current < 0 ||
        current <= previous
      ) isOrdered = false;
    }
    ...
    expect(
      isOrdered,
      "test:senpi must build the daemon, stage artifacts, typecheck, then run package tests",
    ).toBe(true);
    expect(
      script,
      "test:senpi must stay hermetic and not run a live senpi install",
    ).not.toContain("senpi install");
  });
  ...
  test("#given CI workflow #when inspected #then senpi compatibility is a merge-blocking matrix job", () => {
    ...
    // #when
    const senpiJob = sliceWorkflowSection(
      workflow,
      "  senpi-compatibility:",
      "  lazycodex-published-smoke:",
    );
    const needsReferences =
      workflow.match(/needs: \[[^\]]*senpi-compatibility[^\]]*\]/g) ?? [];

    // #then
    expect(senpiJob).toContain(
    ...
    expect(
      needsReferences.length,
      "senpi-compatibility must be included in both downstream needs lists",
