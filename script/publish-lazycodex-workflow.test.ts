...
function sliceWorkflowSection(
  ...
  if (start < 0 || end < 0 || end <= start) {
    throw new Error(
      `missing workflow section between ${startMarker} and ${endMarker}`,
    );
  }
...
describe("LazyCodex publish workflow", () => {
  test("publishes a LazyCodex GitHub release only when the marketplace payload changed", () => {
    ...
    const releaseMetadataJob = sliceWorkflowSection(
      workflow,
      "  release-metadata:",
      "  publish-main:",
    );
    ...
    // #when
    const stampsCodexPluginMetadata = workflow.includes(
      "jq --arg v \"$VERSION\" '.version = $v' packages/omo-codex/plugin/.codex-plugin/plugin.json",
    ) &&
      workflow.includes(
        "jq --arg v \"$VERSION\" '.version = $v' packages/omo-codex/plugin/package.json",
      );
    const lazycodexSyncHasNoManualOptOut = !workflow.includes(
    ...
    const syncsLazycodexMarketplace = workflow.includes(
      "bun run script/sync-lazycodex-marketplace.ts",
    );
    const syncBuildsMcpDists =
      ...
      workflow.indexOf("bun run build:git-bash-mcp") <
        workflow.indexOf("bun run script/sync-lazycodex-marketplace.ts") &&
      workflow.indexOf("bun run build:lsp-tools-mcp") <
        workflow.indexOf("bun run script/sync-lazycodex-marketplace.ts") &&
      workflow.indexOf("bun run build:lsp-daemon") <
        workflow.indexOf("bun run script/sync-lazycodex-marketplace.ts");
    const syncBuildsCodexPlugin =
      workflow.includes("bun run --cwd packages/omo-codex/plugin build") &&
      workflow.indexOf("bun run --cwd packages/omo-codex/plugin build") <
        workflow.indexOf("bun run script/sync-lazycodex-marketplace.ts");
    const npmPublishBuildsReleaseStampedInstallerAndLspDistBeforeMainPackage =
      buildMainPackageStep.includes(
        "bun run build:codex-install && bun run build:lsp-tools-mcp && bun run build",
      );
    const syncStampsMetadataBeforeBuild = syncMarketplaceStep.indexOf(
          "jq --arg v \"$VERSION\" '.version = $v' packages/omo-codex/plugin/.codex-plugin/plugin.json",
        ) >= 0 &&
      syncMarketplaceStep.indexOf(
          "jq --arg v \"$VERSION\" '.version = $v' packages/omo-codex/plugin/package.json",
        ) >= 0 &&
      syncMarketplaceStep.indexOf(
          "jq --arg v \"$VERSION\" '.version = $v' packages/omo-codex/plugin/.codex-plugin/plugin.json",
    ...
    const syncInstallsCodexPluginDeps =
      ...
      workflow.indexOf("npm --prefix packages/omo-codex/plugin ci") <
        workflow.indexOf("bun run --cwd packages/omo-codex/plugin build");
    const pushesLazycodexMarketplace = workflow.includes(
    ...
    const alwaysChecksLazycodexNpm =
      workflow.includes("name: Check if lazycodex-ai already published") &&
      workflow.includes("https://registry.npmjs.org/lazycodex-ai/${VERSION}");
    const publishesLazycodexNpm =
      publishLazycodexStep.includes("name: Publish lazycodex-ai") &&
      publishLazycodexStep.includes(
    ...
    const syncsLazycodexMarketplaceOnStableReleases =
      workflow.includes("name: Sync LazyCodex Codex marketplace") &&
      syncMarketplaceStep.includes(
        "if: needs.release-metadata.outputs.dist_tag == ''",
      ) &&
      lazycodexReleaseStateStep.includes(
        "if: needs.release-metadata.outputs.dist_tag == ''",
      );
    const tokenRequirementBeforePublish =
      workflow.indexOf("name: Require LazyCodex sync token") <
        workflow.indexOf("publish-main:");
    const requiresLazycodexSyncToken = workflow.includes(
    ...
    const capturesPreviousLazycodexBeforePublishing =
      ...
      releaseMetadataJob.includes(
        'npm view "lazycodex-ai@${LAZYCODEX_COMPARE_TAG}" version',
      ) &&
      lazycodexReleaseStateStep.includes(
    ...
    const comparesAgainstPreviousLazycodexVersion =
      lazycodexReleaseStateStep.includes(
        'npm pack "lazycodex-ai@${PREVIOUS_LAZYCODEX_VERSION}"',
      ) &&
      lazycodexReleaseStateStep.includes(
        'bun run script/sync-lazycodex-marketplace.ts "$PREVIOUS_PACKAGE_ROOT" "$PREVIOUS_MARKETPLACE_ROOT" --previous-payload',
      ) &&
      lazycodexReleaseStateStep.includes(
        'diff -qr "$PREVIOUS_MARKETPLACE_ROOT/.agents/plugins/marketplace.json" "$CURRENT_MARKETPLACE_ROOT/.agents/plugins/marketplace.json"',
      ) &&
      lazycodexReleaseStateStep.includes(
      ...
      lazycodexReleaseStateStep.includes(
        'diff -qr "$PREVIOUS_MARKETPLACE_ROOT/plugins/omo" "$CURRENT_MARKETPLACE_ROOT/plugins/omo"',
      ) &&
    ...
    const exposesChangedOutput =
      ...
      lazycodexReleaseStateStep.includes(
        "previous_lazycodex_version=${PREVIOUS_LAZYCODEX_VERSION}",
      );
    const createsLazycodexReleaseOnlyWhenChanged =
      ...
      lazycodexReleaseStep.includes(
        'gh release create "v${VERSION}" --repo code-yeongyu/lazycodex',
      ) &&
      lazycodexReleaseStep.includes(
        "--notes-file /tmp/lazycodex-release-notes.md",
      );

    // #then
    expect(
      stampsCodexPluginMetadata,
      "LazyCodex plugin metadata must be stamped with the release version",
    ).toBe(true);
    expect(
    ...
    expect(
      publishAliasDefaultsOn,
      "LazyCodex npm alias publish must stay enabled by default",
    ).toBe(true);
    expect(
      syncsLazycodexMarketplace,
      "release must sync the LazyCodex marketplace bundle",
    ).toBe(true);
    expect(
      syncBuildsMcpDists,
      "release must build bundled MCP dists before LazyCodex marketplace sync",
    ).toBe(true);
    expect(
      npmPublishBuildsReleaseStampedInstallerAndLspDistBeforeMainPackage,
      "release must regenerate the Codex installer and build LSP dist before npm packages include them",
    ).toBe(true);
    expect(
      syncInstallsCodexPluginDeps,
      "release must install nested Codex plugin deps before building the aggregate plugin",
    ).toBe(true);
    expect(
      syncBuildsCodexPlugin,
      "release must build the aggregate Codex plugin before LazyCodex marketplace sync",
    ).toBe(true);
    expect(
      syncStampsMetadataBeforeBuild,
      "release must stamp Codex plugin metadata before LazyCodex marketplace build",
    ).toBe(true);
    expect(
      pushesLazycodexMarketplace,
      "release must target the LazyCodex repository",
    ).toBe(true);
    expect(
      alwaysChecksLazycodexNpm,
      "release must always check lazycodex using the release version",
    ).toBe(true);
    expect(
      publishesLazycodexNpm,
      "lazycodex npm publish must be part of the normal release, tag stable releases as latest, and fail loudly",
    ).toBe(true);
    expect(
    ...
    expect(
      ...
      "release must require a cross-repo token for LazyCodex push",
    ).toBe(true);
    expect(
      capturesPreviousLazycodexBeforePublishing,
      "release metadata must capture the previous lazycodex-ai version before publishing the new one",
    ).toBe(true);
    expect(
      comparesAgainstPreviousLazycodexVersion,
      "LazyCodex release state must compare current payload with the previous lazycodex-ai package",
    ).toBe(true);
    expect(
      exposesChangedOutput,
      "LazyCodex release state must expose changed and previous-version outputs",
    ).toBe(true);
    expect(
      createsLazycodexReleaseOnlyWhenChanged,
      "LazyCodex GitHub release must target code-yeongyu/lazycodex only when payload changed",
    ).toBe(true);
  });
  ...
  test("skips the LazyCodex GitHub release when the previous payload is unchanged", () => {
    ...
    // #when
    const unchangedPathWritesFalse = lazycodexReleaseStateStep.includes(
      "LazyCodex payload unchanged from lazycodex-ai@${PREVIOUS_LAZYCODEX_VERSION}",
    ) &&
      lazycodexReleaseStateStep.includes(
        'echo "lazycodex_changed=false" >> "$GITHUB_OUTPUT"',
      );
    const releaseStepRequiresChangedOutput = lazycodexReleaseStep.includes(
      "steps.lazycodex-release-state.outputs.lazycodex_changed == 'true'",
    );
    const mainReleaseStillExists =
      workflow.includes("      - name: Create GitHub release") &&
      workflow.includes("GH_TOKEN: ${{ secrets.GITHUB_TOKEN }}");

    // #then
    expect(
      unchangedPathWritesFalse,
      "unchanged LazyCodex payload must write lazycodex_changed=false",
    ).toBe(true);
    expect(
      releaseStepRequiresChangedOutput,
      "LazyCodex release step must skip when lazycodex_changed is false",
    ).toBe(true);
    expect(
      mainReleaseStillExists,
      "main repository release creation must stay independent",
    ).toBe(true);
  });
  ...
  test("can skip LazyCodex npm alias publishing when npm holds the package name", () => {
    // #given
    const workflow = readFileSync(publishWorkflowPath, "utf8");
    const preflightJob = sliceWorkflowSection(
      workflow,
      "  preflight-trust:",
      "  release-metadata:",
    );
    ...
    // #when
    const preflightMakesLazycodexConditional = preflightJob.includes(
    ...
    // #then
    expect(
      preflightMakesLazycodexConditional,
      "trusted-publisher preflight must omit lazycodex when alias publishing is disabled",
    ).toBe(true);
    expect(
      checkStepIsConditional,
      "lazycodex npm status check must be skipped when alias publishing is disabled",
    ).toBe(true);
    expect(
      rebuildsOnlyWhenEnabledOrOtherPackagesNeedPublishing,
      "undefined lazycodex check outputs must not force rebuilds",
    ).toBe(true);
    expect(
      publishStepIsConditional,
      "lazycodex publish must be guarded by publish_lazycodex",
    ).toBe(true);
    expect(
      restoreStepIsConditional,
      "lazycodex restore must only run after a lazycodex publish attempt",
    ).toBe(true);
  });
  ...
  test("publishes lazycodex as a Node installer without Bun-backed runtime dependencies", () => {
    // #given
    const workflow = readFileSync(publishWorkflowPath, "utf8");
    const platformResolver = readFileSync(
      new URL("../bin/platform.js", import.meta.url),
      "utf8",
    );
    ...
    const lazycodexStepUsesNodeInstallerBin = workflow.includes(
      '.bin = { "lazycodex-ai": "packages/omo-codex/scripts/install-local.mjs", "lazycodex": "packages/omo-codex/scripts/install-local.mjs" }',
    );
    const lazycodexStepDoesNotRenameOptionalDeps = !workflow.includes(
      'sub("^oh-my-opencode-"; "lazycodex-")',
    );
    const lazycodexStepDropsLifecycleScripts = workflow.includes(
      ".scripts = {}",
    );
    const lazycodexStepDropsPlatformOptionalDeps = workflow.includes(
      ".optionalDependencies = {}",
    );
    const lazycodexStepDropsRuntimeDependencies = workflow.includes(
      ".dependencies = {}",
    );
    ...
    const missingLazycodexRuntimePayloadPaths = lazycodexRuntimePayloadPaths
      .filter(
        (runtimePath) => !publishLazycodexStep.includes(`"${runtimePath}"`),
      );
    const missingWebTerminalRuntimePaths = webTerminalVisualQaRuntimePaths
      .filter(
        (runtimePath) => !publishLazycodexStep.includes(`"${runtimePath}"`),
      );
    ...
    const publishMainJob = sliceWorkflowSection(
      workflow,
      "  publish-main:",
      "  publish-platform:",
    );
    const lazycodexShipsRootCliDistAfterBuild = publishMainJob.indexOf(
          "bun run build:codex-install && bun run build:lsp-tools-mcp && bun run build:lsp-daemon && bun run build",
        ) >= 0 &&
      publishMainJob.indexOf(
          "bun run build:codex-install && bun run build:lsp-tools-mcp && bun run build:lsp-daemon && bun run build",
        ) <
        publishMainJob.indexOf("name: Publish lazycodex-ai");
    const shimKeepsLazycodexMappedForSharedWrapper = platformResolver.includes(
      'lazycodex: "oh-my-openagent"',
    );

    // #then
    expect(
      lazycodexStepUsesReleaseVersion,
      "lazycodex publish step must use the release version so unpublished versions are not reused",
    ).toBe(true);
    expect(
      lazycodexStepUsesNodeInstallerBin,
      "lazycodex must execute the Node installer directly instead of the Bun-backed shared CLI",
    ).toBe(true);
    expect(
      lazycodexStepDoesNotRenameOptionalDeps,
      "lazycodex must never depend on lazycodex-named platform packages",
    ).toBe(true);
    expect(
      lazycodexStepDropsLifecycleScripts,
      "lazycodex publish step must not ship Bun-backed prepare/build lifecycle scripts",
    ).toBe(true);
    expect(
      lazycodexStepDropsPlatformOptionalDeps,
      "lazycodex publish step must not install Bun-backed platform launchers",
    ).toBe(true);
    expect(
      lazycodexStepDropsRuntimeDependencies,
      "lazycodex publish step must not install OpenCode CLI runtime dependencies",
    ).toBe(true);
    expect(
    ...
    expect(
      shimKeepsLazycodexMappedForSharedWrapper,
      "platform resolver keeps lazycodex mapped when the shared wrapper is used outside the lazycodex package",
    ).toBe(true);
  });
  ...
  test("smoke tests the published LazyCodex alias after npm publish", () => {
    ...
    const smokesReleaseVersion = smokeStep.includes(
      'smoke_lazycodex_package "lazycodex-ai@${OMO_VERSION}"',
    );
    const smokesStableLatestOnly =
      smokeStep.includes('if [ -z "$DIST_TAG" ]; then') &&
      smokeStep.includes('smoke_lazycodex_package "lazycodex-ai@latest"');
    const retriesRegistryPropagation =
      smokeStep.includes("for attempt in $(seq 1 12)") &&
      smokeStep.includes("registry propagation");
    const isolatesCodexState =
      smokeStep.includes('export HOME="$SMOKE_DIR/home"') &&
      smokeStep.includes('export CODEX_HOME="$SMOKE_DIR/codex"') &&
      smokeStep.includes('export CODEX_LOCAL_BIN_DIR="$SMOKE_DIR/bin"');
    const assertsDryRunRouting = smokeStep.includes(
      'npx -y "$package_spec" --dry-run install --no-tui --codex-autonomous',
    ) &&
    ...
    const installsRealPackageAndVerifiesOmoAgentToolkitBin = smokeStep.includes(
    ...
    const simulatesLegacyOmoUpgrade = smokeStep.includes(
      'simulate_legacy_omo_upgrade "$package_spec" "$SMOKE_DIR/upgrade-marked" marked',
    ) &&
      smokeStep.includes(
        'simulate_legacy_omo_upgrade "$package_spec" "$SMOKE_DIR/upgrade-unmarked" unmarked',
      ) &&
    ...
    expect(
      smokesReleaseVersion,
      "post-publish smoke must verify the exact release version",
    ).toBe(true);
    expect(
      smokesStableLatestOnly,
      "post-publish smoke must verify latest only for stable releases",
    ).toBe(true);
    expect(
      retriesRegistryPropagation,
      "post-publish smoke must tolerate npm registry propagation",
    ).toBe(true);
    expect(
      isolatesCodexState,
      "post-publish smoke must isolate HOME and Codex paths",
    ).toBe(true);
    expect(
      assertsDryRunRouting,
      "post-publish smoke must assert the expected dry-run routing output",
    ).toBe(true);
    expect(
  ...
  test("builds the Codex plugin components in publish-main before any package that ships the plugin tree", () => {
    ...
    // #when
    const installDepsIndex = publishMainJob.indexOf(
      "npm --prefix packages/omo-codex/plugin ci",
    );
    const buildComponentsIndex = publishMainJob.indexOf(
      "bun run --cwd packages/omo-codex/plugin build",
    );
    const opencodePublishIndex = publishMainJob.indexOf(
      "name: Publish oh-my-opencode",
    );
    const openagentPublishIndex = publishMainJob.indexOf(
      "name: Publish oh-my-openagent",
    );
    const lazycodexPublishIndex = publishMainJob.indexOf(
      "name: Publish lazycodex-ai",
    );
    const buildStepStart = publishMainJob.indexOf(
      "name: Build Codex plugin components",
    );
    const buildStepSection = buildStepStart >= 0
      ? publishMainJob.slice(
        buildStepStart,
        publishMainJob.indexOf("- name:", buildStepStart + 1),
      )
      : "";

    const buildsPluginComponents = buildComponentsIndex >= 0;
    const installsPluginDepsBeforeBuild = installDepsIndex >= 0 &&
      buildComponentsIndex >= 0 && installDepsIndex < buildComponentsIndex;
    ...
    const buildsBeforeOpencodePublish = buildComponentsIndex >= 0 &&
      opencodePublishIndex > buildComponentsIndex;
    const buildsBeforeOpenagentPublish = buildComponentsIndex >= 0 &&
      openagentPublishIndex > buildComponentsIndex;
    const buildsBeforeLazycodexPublish = buildComponentsIndex >= 0 &&
      lazycodexPublishIndex > buildComponentsIndex;
    const buildStepRunsForEveryPluginShippingPackage =
      buildStepSection.includes("steps.check.outputs.skip != 'true'") &&
      buildStepSection.includes(
        "steps.check-openagent.outputs.skip != 'true'",
      ) &&
      buildStepSection.includes(
        "inputs.publish_lazycodex == true && steps.check-lazycodex.outputs.skip != 'true'",
      );
    const metadataStampIndex = publishMainJob.indexOf(
      "jq --arg v \"$VERSION\" '.version = $v' packages/omo-codex/plugin/.codex-plugin/plugin.json",
    );
    const stampsMetadataBeforeBuild = metadataStampIndex >= 0 &&
      buildComponentsIndex >= 0 && metadataStampIndex < buildComponentsIndex;

    // #then
    expect(
      buildsPluginComponents,
      "publish-main must build the Codex plugin components so published tarballs ship compiled dist (B1)",
    ).toBe(true);
    expect(
      installsPluginDepsBeforeBuild,
      "publish-main must install nested Codex plugin deps before building the components",
    ).toBe(true);
    expect(
      stampsMetadataBeforeBuild,
      "publish-main must stamp the Codex plugin release version before building status messages",
    ).toBe(true);
    expect(
      buildsBeforeOpencodePublish,
      "Codex plugin components must be built before the oh-my-opencode npm publish step",
    ).toBe(true);
    expect(
      buildsBeforeOpenagentPublish,
      "Codex plugin components must be built before the oh-my-openagent npm publish step",
    ).toBe(true);
    expect(
      buildsBeforeLazycodexPublish,
      "Codex plugin components must be built before the lazycodex-ai npm publish step",
    ).toBe(true);
    expect(
      buildStepRunsForEveryPluginShippingPackage,
      "plugin component build must run whenever any plugin-shipping package publishes",
    ).toBe(true);
  });
});
