...
function requiredRecord(value: unknown, path: string): Record<string, unknown> {
  if (!isRecord(value)) {
    throw new Error(`Expected generated omo schema ${path} to be an object`);
  }
  return value;
}
...
export function createOmoJsonSchema(): Record<string, unknown> {
  ...
  const profileProperties = requiredRecord(
    profile.properties,
  ...
  return {
    ...
    description:
      "Configuration schema for the omo.json / omo.jsonc harness-neutral config surface",
    ...jsonSchema,
  };
}
