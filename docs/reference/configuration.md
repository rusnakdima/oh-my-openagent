...
- `image-proxy` analyzes image attachments through the configured `multimodal-looker` model when the active model is known to accept text only. Disable it with `disabled_hooks: ["image-proxy"]`; disabling `multimodal-looker` also disables the hook.
