...
`packages/shared-skills/upstreams/` and materializes their references (both
steps are non-fatal). The container has network on create, so the submodules
clone and the frontend brand / taste / ui-ux references materialize
automatically; if you build the image offline they are skipped and the frontend
skill simply lacks those brand refs until the next online
`script/agent/setup.sh` run.
...
- For QA that must NOT touch any real config,
  `source script/agent/qa-sandbox.sh` points `XDG_*` and `CODEX_HOME` at a
...
[`script/agent/setup.sh`](../script/agent/setup.sh), this README, and the
"Development Environment" sections of the root [`AGENTS.md`](../AGENTS.md) and
[`CONTRIBUTING.md`](../CONTRIBUTING.md) together. See the maintenance directive
in `AGENTS.md`.
