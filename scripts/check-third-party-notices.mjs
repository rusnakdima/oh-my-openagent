...
function readComponentPackageNames() {
  ...
  return readdirSync(componentsPath, { withFileTypes: true })
    ...
    .map((entry) =>
      join("packages/omo-codex/plugin/components", entry.name, "package.json")
    )
    .filter((packagePath) => existsSync(join(repoRoot, packagePath)))
    .map((packagePath) =>
      JSON.parse(readFileSync(join(repoRoot, packagePath), "utf8")).name
    );
}
  ...
  for (const requirement of CODEX_COMPONENT_NOTICE_REQUIREMENTS) {
    ...
    for (const term of requirement.requiredTerms) {
      if (!noticeText.includes(term)) {
        failures.push(
          `${requirement.path}/NOTICE is missing required term: ${term}`,
        );
      }
    }
    for (const term of requirement.forbiddenTerms ?? []) {
      if (noticeText.includes(term)) {
        failures.push(
          `${requirement.path}/NOTICE must be self-contained and not reference missing payload path: ${term}`,
        );
      }
    }
  }
...
function runScope(scopeName) {
  ...
  const missing = requiredComponents.filter((component) =>
    !headingExists(noticeText, component)
  );
  ...
  if (missing.length > 0 || componentFailures.length > 0) {
    if (missing.length > 0) {
      console.error(
        `${scope.noticePath} is missing ${missing.length} required notice entries:`,
      );
    }
  ...
  console.log(
    `${scope.noticePath}: ${requiredComponents.length} required notice entries present`,
  );
}
...
function runShipCheck() {
  ...
  for (const requirement of CODEX_COMPONENT_NOTICE_REQUIREMENTS) {
    ...
    for (const filename of requirement.requiredFiles ?? ["LICENSE", "NOTICE"]) {
      if (
        existsSync(join(repoRoot, requirement.path, filename)) &&
  ...
  for (const path of unique(requiredPackPaths)) {
    if (!packFiles.has(path)) {
      failures.push(`npm pack dry-run output is missing ${path}`);
    }
  }
  ...
  console.log(
    `ship verification passed: ${requiredPackPaths.length} notice/license files present in root npm pack payload`,
  );
}
...
function readRootDryRunPackFiles() {
  ...
  if (result.status !== 0) {
    ...
    throw new Error(
      `npm pack --dry-run --json --ignore-scripts failed with exit ${result.status}`,
    );
  }
...
export function resolveSpawnSyncInvocation(
  command,
  args,
...
export function parseNpmPackJson(output) {
  ...
  throw new Error(
    "npm pack --dry-run --json did not produce a parseable file list",
  );
}
